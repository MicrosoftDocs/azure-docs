...
|14.17|Apps that depend on external accounts or services must provide a clear and simple sign in/sign out and signup experience. Teams apps which depend on authentication to an external service to allow content sharing in channels must clearly state in their help documentation (or similar location) how a user can disconnect or unshare any shared content (if the same feature is supported on the external service).  The ability to unshare this content does not have to be present in the Teams app, but the process should be clearly documented, in documentation reachable from within the Teams app.|
