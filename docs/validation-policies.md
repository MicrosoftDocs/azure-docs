...
|14.18| To help to ensure the security of your app and users, domains outside of your organization's control (including wildcards) and tunneling services cannot be included in the valid domains in your manifest. **Exception:** If you're using OAuthcard,  `Token.botframework.com` must be in the valid domains list. Teams apps that require their own SharePoint URLs to function can include {teamsitedomain} in their valid domain list.
