...
- For content and task pane Office Add-ins, when the add-in is launched and the Office application requests the add-in home page, the Office application passes the license token as a query parameter in the HTTP request. The add-in code must extract and cache this information, so that the license checking code in the add-in can later access it.If the user is not signed in to their Microsoft account, the Office application requesting the add-in home page does not append the license token parameter. See [Add-in license tokens and anonymous access for Office Add-ins](#bk_anonymous) for more information.
...
![Office app license verification process](..\media\O15_app_license_Verification.png)
...
![SharePoint app license verification process](..\media\SP15_app_license_Verification.png)
