<properties linkid="develop-mobile-tutorials-get-started-with-data-android" urlDisplayName="Get Started with Data" pageTitle="Get started with data (Android) - Windows Azure Mobile Services" metaKeywords="Windows Azure Android data, Azure mobile services data, " metaDescription="Learn how to store and access data from your Windows Azure Mobile Services Android app." metaCanonical="" disqusComments="1" umbracoNaviHide="1" />
...
_The Android client library for Mobile Services is currently under development on [GitHub][GitHub repo]. We welcome feedback on and contributions to this library._
...
1. Download the GetStartedWithData sample app from [GitHub] and expand the files on your computer. 
...
2. In Package Explorer, expand **TodoActivity**, **src**, and **.com.example.getstartedwithdata**, then examine the ToDoActivity.java file.

   ![][12]

   Notice that there are XXX `// TODO` comments that specify the steps you must take to make this app work with your mobile service.

4. From the **Run** menu, click **Run As** and then click **1 Android Application** to start the project in the Android emulator.
...
3. In Package Explorer in Eclipse, right-click **ToDoActivity**, click **Properties**, click **Java Build Path**, click **Libraries**, then click **Add External Jars...**.

  ![][11]

4. Browse to the download location of the Mobile Services SDK, select the `.jar` file, click **OK**, then click **OK** again to close the **Properties for ToDoActivity** window.
...
5. Back in the activity, locate and comment-out the following line of code, which defines the existing `toDoItemList` list:

		public List<ToDoItem> toDoItemList = new ArrayList<ToDoItem>();

6. In the **onCreate** method, uncomment the following lines of code that define the **MobileServiceClient** variable:
		...
		try {
			...
			createAndShowDialog(
					new Exception(
							"There was an error creating the Mobile Service." +
					"Verify the URL"), "Error");
		}

  This creates a new instance of MobileServiceClient that is used to access your mobile service.

7. In the code above, replace `MobileServiceUrl` and `AppKey` with the URL and application key from your mobile service, in that order.
...
7. Replace the body of the existing **addItem** method with the following code:
...
9. Replace the body of the **checkItem** method with the following code:
