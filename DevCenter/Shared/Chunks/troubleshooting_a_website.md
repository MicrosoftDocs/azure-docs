...
- **Detailed Error Messages** - Logs detailed error information for HTTP status codes that indicate a failure (status code 400 or greater).
...
Diagnostics log files can be saved to the local file system of the web site, and can be retrieved using FTP, Windows Azure PowerShell, or the Windows Azure Command-line Tools. Diagnostics log files can also be written to a Windows Azure Storage account.

For more information on these diagnostics settings, see the [Configuring Diagnostics section of How to Monitor Web Sites](http://www.windowsazure.com/en-us/manage/services/web-sites/how-to-monitor-websites/#howtoconfigdiagnostics). For information on Application Diagnostics, see [Troubleshooting Windows Azure Web Sites in Visual Studio](http://www.windowsazure.com/en-us/develop/net/tutorials/troubleshoot-web-sites-in-visual-studio/).
...
On the **Configure** management page, under **Site Diagnostics**, the following logging and tracing options are available:

- **Web Server Logging**  - Turn on Web Server logging to save web site logs using the W3C extended log file format. There are options to save the logs to a Windows Azure Storage account or to the File System. In this tutorial, you will use the File System, where you can access the logs by using FTP.
...
1.  Open the web site's **Dashboard** management page and make note of the FTP site listed under **Diagnostics Logs** and the account listed under **Deployment / FTP User**. The FTP site is where the log files are located and the account listed under Deployment User is used to authenticate to the FTP site. The FTP credentials are available in the publish profile for your site, which you can obtain from the dashboard by choosing **Download the publish profile**.
