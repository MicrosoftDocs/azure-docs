...
information on using the Windows Azure Caching (Preview), refer to the [Next Steps][] section.
...
Windows Azure Caching (Preview) has the following features:
...
There are two ways to create a DataCache object. To use the first way, in your application, create a new **DataCacheFactory** object using the default constructor. This causes the cache client to use the settings in the configuration file. Call the **GetDefaultCache** method of the new **DataCacheFactory** instance which returns a **DataCache** object that can then be used to programmatically access the cache.
...
state provider, first ensure that your ASP.NET application is configured to access the desired cache, as described in either [Getting Started with the Windows Azure Caching Service][] or [Getting Started with Windows Azure Caching (Preview)][]. Once these steps are completed, a **sessionState** section can be added
...
use the output cache provider, first ensure that your ASP.NET application is configured to access the desired cache, as described in either [Getting Started with the Windows Azure Caching Service][] or [Getting Started with Windows Azure Caching (Preview)][]. Once these steps are completed, a **caching** section can be added to the web.config file. To configure your ASP.NET application to use the output cache provider, paste this snippet into your web.config file. In this snippet, the default cache is specified. To use a different cache, specify the desired the cache in the **cacheName** attribute.
