...
4. Fix version-pinned links: remove a `view=` query parameter (for example, `?view=azuresql`) unless the link intentionally targets a non-default version, in which case add `&preserve-view=true`. Removing the `view` parameter clears the `preserve-view-not-set` validation suggestion. Preserve other meaningful query parameters such as `tabs` and any URL fragment.
5. Review Learn-specific constructs when present in the file: `:::image`, `:::code`, monikers, includes, tab groups, and zone pivots. Correct obvious syntax issues, but do not introduce new platform-specific constructs unless needed.
6. Tighten wording only when it removes ambiguity, duplication, or validation-adjacent quality problems.
...
- Version-pinned links that carry a `view=` parameter without `preserve-view=true`, which triggers the `preserve-view-not-set` suggestion.
...
- Links aren't pinned to a version with a `view=` parameter unless `preserve-view=true` is intentionally set.
