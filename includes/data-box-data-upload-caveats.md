...
- In Linux, after you upload your data to Azure Files, you need to use a different tool if you want to copy the metadata for those files. For Windows-based data copies, the `robocopy` tool copies the metadata along with the files when you upload data to Azure Files. To make sure CreationTime on directories is copied, include the `/dcopy:DAT` option in your `robocopy` command. For more information, see [Preserving file ACLs, attributes, and timestamps with Azure Data Box](../articles/databox/data-box-file-acls-preservation.md).