...
- Merging folder contents:
  ...
  - An upload to a blob in the Archive tier will fail if the container has an existing blob with the same name. While a blob is in the Archive tier, it can't be read or modified. If you need to overwrite a blob, make sure the blob and its container aren't set to archive. For more information, see [Archive access tier](../articles/storage/blobs/access-tiers-overview.md#archive-access-tier).
