...
- Copying via SMB, NFS:
  - We recommend that you not use the same Data Box to copy to both Windows (via SMB) and Linux (via NFS). 
  - If you do use the same Data Box to copy via SMB and NFS:
    - Don't use both protocols concurrently. Instead, complete your SMB copy before you start the NFS copy. 
    - Don't use both protocols to copy to the same location in Azure. 
- Upload management: 
