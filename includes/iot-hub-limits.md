...
| Maximum number of concurrently connected device streams | 50/unit (for S1, S2, S3 only) |
| Maximum device stream data transfer (aggregate volume per billing cycle) | 10 GB (for S1, S2, S3 only) |
...
| Device stream initiation rate | 5 new streams/sec/unit (for S1, S2, S3 only) |