...
Deploy the RDMA-capable HPC VMs in the same availability set or VM scale set (when you use the Azure Resource Manager deployment model) or the same cloud service (when you use the classic deployment model). If you use a VM scale set, ensure that you limit the deployment to a single placment group; for example, in a Resource Manager template, set the *singlePlacementGroup* property to *true*. Additional requirements for RDMA-capable HPC VMs to access the Azure RDMA network follow.