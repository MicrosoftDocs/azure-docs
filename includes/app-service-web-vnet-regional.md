...
> [!NOTE]
> When you route all of your outbound traffic into your VNet, it's subject to the NSGs and UDRs that are applied to your integration subnet. Outbound addresses remain the outbound addresses displayed in your app properties, unless you provide routes to send the traffic elsewhere.
> 
...
* The feature is available from all App Service scale units in Premium V2 and Premium V3. It's also available in Standard but only from newer App Service scale units. If you are on an older scale unit, you can only use the feature from a Premium V2 App Service plan. If you want make sure you can use the feature in a Standard App Service plan, create your app in a Premium V3 App Service plan. Those plans are only supported on our newest scale units. You can scale down if you desire after that.  
...
1. Configure regional VNet Integration with your web app to connected to a specific subnet for integration.
1. Go to the destination service and configure service endpoints against the integration subnet.

To learn more, see [Set a service endpoint-based rule](../articles/app-service/app-service-ip-restrictions.md#set-a-service-endpoint-based-rule).
