...
| Maximum job stream limit|1MB|For a single object on a single stream the limit is 1MB. If the stream objects output by a script don’t each exceed 1MB, then the script can have more output.|
