...
If the connectivity is to be setup over BGP, you’ll need to select  the **Enable BGP** option and provide the BGP ASN and BGP peer IP addresses during the setup. Note that the BGP ASN for local network gateways is 65515 (the same settings as VPN gateway instances of virtual WAN). For Azure virtual network gateway VPN, the BGP ASN is a value other than 65515 (same settings as VPN sites of the virtual WAN).
