...
The following were issues discovered in many of the larger migrations.  This is not an exhaustive list and you should refer to the [unsupported scenarios](https://azure.microsoft.com/en-us/documentation/articles/virtual-machines-windows-migration-classic-resource-manager/#unsupported-features-and-configurations) for more detail.  You may or may not encounter these technical issues.
...
- **Web/Worker Role Deployments** -  Cloud Services containing web and worker roles cannot migrate to Azure Resource Manager. The web/worker roles must first be removed from the virtual network before migration can start.  A typical solution is to just move web/worker role instances to a separate Classic virtual network that is also linked to an ExpressRoute circuit, or to migrate the code to newer PaaS App Services (this discussion is beyond the scope of this document). In the former redeploy case, create a new Classic virtual network, move/redeploy the web/worker roles to that new virtual network, then delete the deployments from the virtual network being moved. No code changes required. The new [Virtual Network Peering](https://azure.microsoft.com/en-us/documentation/articles/virtual-network-peering-overview/) capability can be used to peer together the classic virtual network containing the web/worker roles and other virtual networks in the same Azure region such as the virtual network being migrated (**after virtual network migration is completed as peered virtual networks cannot be migrated**), hence providing the same capabilities with no performance loss and no latency/bandwidth penalties. Given the addition of [Virtual Network Peering](https://azure.microsoft.com/en-us/documentation/articles/virtual-network-peering-overview/), web/worker role deployments can now easily be mitigated and not block the migration to Azure Resource Manager.

- **Azure Resource Manager Quotas** - Azure regions have separate quotas/limits for both Classic and Azure Resource Manager. Even though in a migration scenario new hardware isn't being consumed *(we're swapping existing VMs from Classic to Azure Resource Manager)*, Azure Resource Manager quotas still need to be in place with enough capacity before migration can start. Listed below are the four major limits we've seen cause problems.  Open a quota support ticket to raise the limits.
  1. Azure Resource Manager network limit – this represents the number of NICs needed in Azure Resource Manager after migration is complete. The number should be raised to cover the total number of NICs used in the Classic vNet being migrated.
  2. Azure Resource Manager maxloadbalancers – the number of load balancers needed.
  3. Azure Resource Manager public IP addresses – the number of public IPs needed in Azure Resource Manager. Should be raised to the same number of VIPs used in Classic.
  4. Azure Resource Manager static public IP addresses - the number of static public IPs in Azure Resource Manager. This is equivalent to the reserved IPs in Classic.
  5. Azure Resource Manager cores – should be raised to the same number of cores used in Classic.
...
Now that you are in Azure Resource Manager, maximize the platform.  Read the [overview of Azure Resource Manager](../articles/resource-group-overview/) to find out about additional benefits.
...
2. Azure Resource Manager templates for easier and more controlled deployment.
3. Azure Resource Manager only available services.
