...
1. In the Azure portal, in the navigation menu, under **Tables**, click **Data Explorer**. 
2. In the Data Explorer blade, click **New Collection**, then fill in the page using the following information.
    ...
    Collection id|sample-table|The ID for your new collection. Collection names have the same character requirements as database ids.
    ...
    Partition key|/databases|A partition key that will distribute data evenly to each partition. Selecting the correct partition key is important in creating a performant collection, read more about it in [Designing for partitioning](../articles/documentdb/documentdb-partition-data.md#designing-for-partitioning).
