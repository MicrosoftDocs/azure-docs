...
<sup>1<sup> Backend IP configurations are aggregated throughout all load balancer rules including load balancing, inbound NAT, and outbound rules. Each rule a backend pool instance is configured to counts as one configuration.
