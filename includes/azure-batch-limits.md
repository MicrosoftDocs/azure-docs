...
<sup>1</sup> Cores quotas shown are only for accounts with the pool allocation mode set to **Batch service**. For accounts with the mode set to **user subscription**, cores quotas are based on the VM cores quota at a regional level or per VM family in your subscription. Learn more about [Batch accounts](../batch/batch-api-basics.md#account).
