...
Follow these steps if you want to allow users not in your directory to be able to request this access package:
...
1. Now determine if requests will require single or multi-stage approval. Follow either the single-stage approval or multi-stage approval procedure: 
...
1. Add the approver(s):
    
    1. If the policy is set to govern access **For users in your directory**, you can select **Manager as approver**. Or, add a specific user by clicking **Add approvers** after selecting **Choose specific approvers**.
    ![Access package - Requests - For users in directory](./media/active-directory-entitlement-management-request-policy/approval-single-stage-first-approver-manager.png)

    1. If this policy is set to govern access **For users not in your directory**, you can select **External sponsor** or **Internal sponsor**. Or, add a specific user by clicking **Add approvers** or groups under Choose specific approvers.
    ![Access package - Requests - For users out of directory](./media/active-directory-entitlement-management-request-policy/out-directory-first-approver-multistage.png)
...
1. If you selected **Manager** as the first approver, click **Add fallback** to select one or more users or groups in your directory to be a fallback approver. Fallback approvers receive the request if entitlement management can't find the manager for the user requesting access.
    ...
    The manager is found by entitlement management using the **Manager** attribute in the user's profile in Azure AD. For more information, see [Add or update a user's profile information using Azure Active Directory](../articles/active-directory/fundamentals/active-directory-users-profile-azure-portal.md).
...
### Multi-stage approvals (in PREVIEW)

1. If your access package requires multiple stages of approvals, set the **How many stages** toggle to **2**.
...
1. You will need to select at least two approvers, one for each stage. 

Under **First Approver**:
    1. If the policy is set to govern access **For users in your directory**, select **Manager as approver** or **Choose specific approvers**. 
    1. If the policy is set to govern access **For users not in your directory**, select **External sponsor**, **Internal sponsor**, or **Choose specific approvers**. 

1. If you selected **Manager as approver**, **External sponsor**, or **Internal sponsor** as the first approver, click **Add fallback** to select one or more users or groups in your directory to be a fallback approver, in case the listed approver is not found in the directory. 

1. If you selected **Choose specific approvers**, click **Add approvers** to select users or a group of users in your directory to be the first approver.

1. Specify the number of days the first approver(s) have to approve the request in the box under **Decision must be made in how many days?**. 

1. Set the **Require approver justification** toggle to **Yes** or **No**. 

1. Under **Second Approver**:
    1. If this policy is **for users are in your directory**, add a specific users or a group of users as the second approver(s) by clicking **Add approvers** under **Choose specific approvers**. 
    1. If this policy is **for users not in your directory**, select **Internal sponsor** or **External sponsor** or **specific approvers** as the second approver(s). If you selected the internal sponsor or external sponsor, add the fallback approver(s). 

1. Specify the number of days the second approver(s) have to approve the request in the box under **Decision must be made in how many days?**. 

1. Set the **Require approver justification** toggle to **Yes** or **No**.

### Alternate approvers

You can specify alternate approvers, similar to specifying the primary approvers who can approve requests. This will help ensure that the requests are approved or denied before they expire (timeout).
