...
>
> The keys that App Service uses to encrypt and sign authentication tokens are shared across all of an app's deployment slots. By default, this is an automatically generated key (surfaced through `MACHINEKEY_DecryptionKey` and `MACHINEKEY_ValidationKey`); the same shared behavior applies if you explicitly set `WEBSITE_AUTH_ENCRYPTION_KEY` or `WEBSITE_AUTH_SIGNING_KEY`. Because every slot of the same app can therefore decrypt the tokens these keys protect—and, in Azure Functions, the function access keys they also protect—don't treat a deployment slot as a security isolation boundary for these secrets.
