...
>[AZURE.IMPORTANT] For a listener using the cloud service's public VIP (external load balancer), any data returned through the listener is considered egress and charged at normal data transfer rates in Azure. This is not the case with a listener using ILB. However, when the client is located in the same Azure region as the listener and databases there are no charges for data transfer through an external load balancer. 
