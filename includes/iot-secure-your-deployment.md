...
* [Security token structure](../articles/iot-hub/iot-hub-devguide-sas.md#security-token-structure)

* [Using SAS tokens as a device](../articles/iot-hub/iot-hub-devguide-sas.md#use-sas-tokens-as-a-device)
...
[IoT Hub supports protocols such as MQTT, AMQP, and HTTP](../articles//iot-hub/iot-hub-devguide-sas.md). Each of these protocols uses security tokens from the IoT device to IoT Hub differently:
...
Azure IoT Hub allows definition of [access control policies](../articles/iot-hub/iot-hub-devguide-sas.md) for each security key. It uses the following set of permissions to grant access to each of IoT Hub's endpoints. Permissions limit the access to an IoT Hub based on functionality.
...
There are two ways to obtain **DeviceConnect** permissions with IoT Hub with [security tokens](../articles/iot-hub/iot-hub-devguide-sas.md#use-sas-tokens-as-a-device): using a device identity key, or a shared access key. Moreover, it is important to note that all functionality accessible from devices is exposed by design on endpoints with prefix `/devices/{deviceId}`.

[Service components can only generate security tokens](../articles/iot-hub/iot-hub-devguide-sas.md#use-security-tokens-from-service-components) using shared access policies granting the appropriate permissions.
