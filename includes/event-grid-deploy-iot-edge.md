...
>[!IMPORTANT]
> In this tutorial, Event Grid module will be deployed with client authentication turned-off, and allow HTTPS subscribers. For production workloads, we recommend that you enable only HTTPS requests and subscribers with client authentication enabled. For more information on how to configure Event Grid module securely, see [Security and authentication](../articles/event-grid/edge/security-authentication.md).
