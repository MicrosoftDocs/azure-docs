...
| /R:n                | Speed of a RoboCopy run can be improved by specifying a maximum retry count for a file that fails to copy on first attempt. n = maximum number of retries before the file permanently fails to copy in this RoboCopy run. This switch works best in scenarios where it's already clear that there will be subsequent RoboCopy runs. If the file fails to copy, the next RoboCopy job will try again and often files that failed because they were in use or because of timeout issues, might eventually be copied successfully with this approach. |
| /W:n                | This switch specifies the time RoboCopy waits before attempting to copy a file that didn't successfully copy at the last attempt. n = the number of seconds to wait between retries. `/W:n` is often used in conjunction with `/R:n`. |
