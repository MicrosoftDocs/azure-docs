...
1. **Frontend IP Configuration:** Create two frontend IP, one for ASCS and another for ERS. Select the same virtual network and subnet as that of your ASCS/ERS virtual machines.
...
3. **Inbound rules:** Create two load balancing rule, one for ASCS and another for ERS. Follow the same steps for both load balancing rules.
     ...
     - Protocol: TCP
           - Health Probe: Create health probe with below details (applies for both ASCS or ERS)
             - Protocol: TCP
             - Port: [eg: 620<Instance-no.> for ASCS, 621<Instance-no.> for ERS]
             - Interval: 5
             - Probe Threshold: 2
