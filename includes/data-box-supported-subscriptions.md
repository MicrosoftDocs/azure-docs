...
* The subscription you use for Data Box service is one of the following types:
  ...
  * Cloud Solution Provider (CSP). Learn more about [Azure CSP program](/azure/cloud-solution-provider/overview/azure-csp-overview).
    > [!NOTE]
    > This service is supported for the Azure CSP program in India only if you are using the latest billing APIs. If you are using the legacy version of the billing APIs, you will not be able to create orders for CSP.  
