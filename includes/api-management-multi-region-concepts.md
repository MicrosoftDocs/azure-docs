...
* When API Management receives public HTTP requests to the traffic manager endpoint(applies for external vnet and none modes of Apim), traffic is routed to a regional gateway based on lowest latency, which can reduce latency experienced by geographically distributed API consumers. 
