...
The `AcquireTokenSilentAsync` method gets a token without any user interaction.  `AcquireTokenSilentAsync` can be treated as a best-effort request, with a fallback to `AcquireTokenAsync` when the user needs to sign in again or perform extra authorization, like multi-factor auth. 
...
* Retry `AcquireTokenSilentAsync` later. This pattern is frequently used when users can use other application functionality without disruption--for example, when offline content is available in the application. The application can decide to retry `AcquireTokenSilentAsync` when the network is restored after having been temporarily unavailable. 
<!--end-collapse-->
...
The `onSignOutClicked()` method removes users from the MSAL cache. This means MSAL will no longer have any state for the signed in user, and they'll be logged out of the application. 

### More information on multi-user scenarios
