...
TODO:
[] Do we have an extending your solution section or article to link to in the overview?
[] Check how we get the URL of the solution dashboard (is this the correct terminology?)
...
The device you create will send data to an instance of the [remote monitoring][lnk-remote-monitoring] preconfigured solution. Visit [Get started with Azure IoT Suite][lnk-getstarted] to create an Azure account and provision IoT Suite. Choose **Remote monitoring** when you create your new solution.
...
For a device to connect to the preconfigured solution, it must be able to identify itself using valid credentials. You can obtain the device credentials from the solution dashboard and then include them in your client application. 
...
5. Make a note the device credentials (Device ID, IoT Hub Hostname, and Device Key). Your client application will need them to connect your device to the remote monitoring solution.

    ![][4]

6. Ensure your device displays correctly in the devices section. The status is **Pending** until the device establishes a connection to the remote monitioring solution.
