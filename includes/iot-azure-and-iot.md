...
A device can communicate directly with a cloud gateway endpoint, or if the device cannot use any of the communications protocols that the cloud gateway supports, it can connect through an intermediate gateway. For example, the [Azure IoT protocol gateway](../articles/iot-hub/iot-hub-protocol-gateway.md) can perform protocol translation if devices cannot use any of the protocols that IoT Hub supports.
