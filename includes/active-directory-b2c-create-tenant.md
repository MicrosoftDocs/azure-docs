...
Select **Create a new Azure AD B2C Tenant**. The settings specified in the following table use the fictitious company Contoso as an example. You will need to provide your own organization name and a unique tenant name when creating your tenant.  
...
| **Initial domain name** |  ContosoB2CTenant | Domain name for the B2C tenant. By default, the initial domain name includes .onmicrosoft.com. You can add a domain name your organization uses later. You cannot create a tenant with the same name as a previously deleted tenant. If you are creating a test tenant, choose a non-production name such as ContosoB2CTesting. |
