...
7. For **Resource group** you can use an existing resource group, or create a new one. To see the available list of resource groups in your subscription, select **Use existing**, and click the drop-down menu. To create a new resource group, select **Create new** and type the name. For complete information on resource groups, see [Azure Resource Manager overview](../azure-resource-manager/resource-group-overview.md).
