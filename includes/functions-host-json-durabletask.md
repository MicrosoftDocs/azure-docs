...
|controlQueueBufferThreshold| **on consumption plan**: 32 <br> **other plans**: 256 |The number of control queue messages that can be buffered in memory at a time, at which point the dispatcher will wait before dequeuing any additional messages.|
...
|maxConcurrentOrchestratorFunctions | **on consumption plan**: 5 <br> **other plans**: 10X the number of processors on the current machine elsewhere|The maximum number of orchestrator functions that can be processed concurrently on a single host instance.|
...
|useLegacyPartitionManagement|true|When set to `false`, uses a partition management algorithm that reduces the possibility of duplicate function execution when scaling out.  Available starting in v2.3.0. The default will be changed to `false` in a future release.|
