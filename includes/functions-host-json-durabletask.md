...
|maxGrpcMessageSizeInBytes|4194304| An integer value that sets the maximum size, in bytes, of messages that the gRPC client for DurableTaskClient can receive. This applies to Durable Functions .NET Isolated and Java (and may apply to other languages in the future).|
|grpcHttpClientTimeout|100 seconds|Sets the timeout duration for the HTTP client used by the gRPC client, which is used by Durable Functions .NET Isolated and Java (and may apply to other languages in the future). Applies to .NET 6 or greater.|
