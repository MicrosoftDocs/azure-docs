...
|controlQueueBufferThreshold| 32 on consumption plan, 256 elsewhere |The number of control queue messages that can be buffered in memory at a time, at which point the dispatcher will wait before dequeuing any additional messages.|
...
|maxConcurrentOrchestratorFunctions |5 on consumption plan, 10X the number of processors on the current machine elsewhere|The maximum number of orchestrator functions that can be processed concurrently on a single host instance.|
