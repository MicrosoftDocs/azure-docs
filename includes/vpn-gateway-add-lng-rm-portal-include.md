...
5. **Configure BGP settings:** Only use when configuring BGP. Otherwise, don't select this.
6. For **Subscription**, verify that the correct subscription is showing.
7. For **Resource Group**, select the resource group that you want to use. You can either create a new resource group, or select one that you have already created.
8. For **Location**, select the location in which this resource will be created. You may want to select the same location that your VNet resides in, but you are not required to do so.
9. Click **Create** to create the local network gateway.
