...
- If you used a certificate that was issued by an Enterprise CA solution and can't authenticate, verify the authentication order on the client certificate. Check the authentication list order by double-clicking the client certificate, and then selecting **Details > Enhanced Key Usage**. Make sure *Client Authentication* is the first item in the list. If it isn't, issue a client certificate based on the user template that has *Client Authentication* as the first item in the list.
