...
- Disks created from custom images that are encrypted using server-side encryption and customer-managed keys must be encrypted using the same customer-managed keys. Your disks and the images must be in the same subscription, the keys used to encrypt your disks can be in a different subscription.
