...
3. In the **New account** blade, specify the desired configuration for the Azure Cosmos DB account. 
    ...
    API|SQL (DocumentDB)|We'll be programming against the [DocumentDB API](../articles/cosmos-db/documentdb-introduction.md) later in this article.|
...
5. On the top toolbar, click **Notifications** to monitor the deployment process.
    ...
    ![Deployment started notification](./media/cosmos-db-create-dbaccount/notification.png)

6.  When the deployment is complete, open the new account from the All Resources tile. 

    ![DocumentDB account on the All Resources tile](./media/cosmos-db-create-dbaccount/all-resources.png)