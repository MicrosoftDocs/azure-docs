...
1. Update your client app to start registering as FCMv1 devices: once you are ready to start supporting FCMv1 devices, update your client app so that any new devices will start registering as FCM v1 instead of the legacy. When existing FCM legacy registrations expire, they are re-registered as FCM v1. This ensures that notifications are sent by Azure Notification Hubs to users appropriately, once the FCM legacy is deprecated.
1. Delete the FCM legacy registrations: once you have created the FCM v1 registrations, delete the corresponding records from the FCM legacy. This prevents duplicate notifications from being sent to your users if you use both APIs. Duplicate notifications can occur and be sent to users if FCM legacy registrations are not deleted.
