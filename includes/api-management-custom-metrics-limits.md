...
* The number of active time series generated by the policy is the product of the number of unique values of each configured dimension per namespace. For example, if three custom dimensions were configured in the policy, and each dimension had 10 possible values within the period, the policy would contribute 1,000 (10 x 10 x 10) active time series.

* API Management limits each dimension to 100 unique values and each metric namespace to 1,000 active time series. When either limit is reached, any new dimension values or time series are not tracked, and the corresponding metric data is silently discarded.
