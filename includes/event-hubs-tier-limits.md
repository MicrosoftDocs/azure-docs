...
The following table shows limits that may be different for basic, standard, and dedicated tiers. In the table CU is [capacity unit](../articles/event-hubs/event-hubs-dedicated-overview.md), PU is [processing unit](event-hubs-scalability.md#processing-units), TU is [throughput unit](../articles/event-hubs/event-hubs-faq.yml#what-are-event-hubs-throughput-units-). 
...
| Throughput per unit | Ingress - 1 MB/s or 1000 events per TU<br/>Egress – 2 Mb/s or 4096 events Per TU | Ingress - 1 MB/s or 1000 events per TU<br/>Egress – 2 Mb/s or 4096 events Per TU | No limits per PU * | No limits per CU * |
