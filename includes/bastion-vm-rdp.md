...
1. The RDP connection to this virtual machine via Bastion will open directly in the Azure portal (over HTML5) using port 443 and the Bastion service. When you connect, the desktop of the VM may look different than the example screenshot. Note that when using keyboard shortcut keys while connected to a VM, they may not result in the same behavior as shortcut keys on a local computer.
