...
RADIUS authentication is supported for VpnGw1, VpnGw2, and VpnGw3 SKUs. If you are using legacy SKUs, it is also supported on Standard and High Performance SKUs. It is not supported on the Basic Gateway SKU. 
 
### Are 3rd-party RADIUS servers supported?

Yes, 3rd-party RADIUS servers are also supported.
 
### What are the connectivity requirements to ensure that the Azure VPN gateway is able to reach an on-premises RADIUS server?
...
There is no change in the maximum number of SSTP connections supported on a gateway with RADIUS authentication. It remains 128.  
 
### What is the difference between using certificate authentication using a RADIUS server vs. using Azure native certificate authentication?
