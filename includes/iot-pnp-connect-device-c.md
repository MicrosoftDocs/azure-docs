...
> [!TIP]
> On Windows, you can open the solution generated by the `cmake` command in Visual Studio. Open the *azure_iot_sdks.sln* project file in the _cmake_ directory and set the **IoTHub_Samples/pnp_simple_thermostat** project as the startup project in the solution. You can now build the sample in Visual Studio and run it in debug mode.
