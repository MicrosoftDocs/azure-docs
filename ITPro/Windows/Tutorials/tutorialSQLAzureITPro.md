...
You must already have a Windows Azure subscription. For instructions on how to get started with the trial version, see [Windows Azure Free Trial]([http://www.windowsazure.com/en-us/pricing/free-trial/) or [watch this video](http://www.youtube.com/watch?v=_m9_eO3CtV0).
...
4.  The Database Settings page appears first when you click **Custom Create**. In this page, you provide basic information that creates an empty database on the server. Adding tables and data will come in a later step. 
...
1. Click **Servers** at the top of the page. Next, click on the server you just created so that you see a white arrow to the right. Click on the arrow to open the server page.

    ![Image5] []

2. On the server page, click **Configure** to open the firewall configuration settings. 

3. Specify the rule as follows:
...
In a new query window, copy and run the following T-SQL script to retreive some of the data you just added.
...
In SQL Database, database access is configured using T-SQL. In this lesson, using T-SQL, you will do three things: create a server login, create  a database user, and grant permissions via role membership.

A SQL login is used for server connections. All users who want to access a database on SQL Database server do so by providing a SQL login name and password. 

To create a login, you must first connect to the **master** database.

<h4 id=CreateLogin>Create a server login</h4>

1. Go back to the Windows Azure portal, select **SQL Databases**, click **Servers**, choose the server and then click the white arrow to open the server page. On that page, click **Manage Server** to open a new SQL Database management portal connection. Enter the administrator name and password.

    ![Image5] []

2. The SQL Database management portal opens in a new browser window. Click **Select a Database** at the top, and click **master**.

	![Image14] []

3. If you see an error on the page similar to the following, ignore it. Click **New Query** to open a query window that lets you execute T-SQL commands on the **master** database.

	![Image15] []

4. Copy and paste the following command into the query window.

    CREATE LOGIN mynewlogin WITH password='Password1';

5. Run the command to create a new SQL Server login named 'mynewlogin'.


<h4 id=CreateDBuser>Create a database user and assign permissions</h4>

After you create a server login, map it the databases to which it has access. You do this by creating a **database user** on each database.
...
1. Go back to the SQL Database management portal page that connects to the **School** database. If you closed the browser window, start a new connection to **School** database using the steps from the previous lesson, "Add data and a schema using T-SQL script". 
	...
	On the SQL Database management portal page, the **School** database name is visible in the top right corner.

	![Image12] []

2. Click **New Query** to open a new query window and copy in the following statement. 

	CREATE USER mynewuser FROM LOGIN mynewlogin;
...
1. In Excel, on the Data tab, click **From Other Sources**, and then click **SQL Server**.

2. In the Database Connection wizard, enter the fully-qualified domain name of your SQL Database server, followed by a SQL Server login that has permission to access the database. 

  The server name can be found on the Windows Azure management portal, on SQL Database, on the Server page. The server name consists of a series of letters and numbers. Copy this name, and then append '.database.windows.net' to specify server name in the Database Connection wizard.
...
4. In the worksheet, you should see a table similar to the following. 
...
[Image11]: media/DBPortalLogin.png
