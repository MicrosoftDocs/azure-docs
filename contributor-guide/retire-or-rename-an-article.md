...
First off, in our current repo configuration, you must not ever delete, move, or rename an article. Here's how to think about these concepts:

  - **Delete**: If you want to delete an article, you have to turn it into a redirect and leave the article in place.
  - **Move**: If you want to delete an article, turn the current article into a redirect and then create a new article in the new folder location, and copy the content to the new article.
  - **Rename**: Functionally, the same as moving. You turn the current article into a redirect and then create a new article with the same file name in the new folder location, and copy the content to the new article.
