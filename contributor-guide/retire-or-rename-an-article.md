...
First off, in our current repo configuration, you must not ever actually delete, move, or rename an article. Here's how to think about these concepts:
  ...
  - **Rename**: Functionally, renaming is the same as moving. You turn the current article into a redirect and then create a new article with the same file name in the new folder location, and copy the content to the new article.
