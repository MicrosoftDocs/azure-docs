...
1. Set up a custom domain for the service instances. For more information, see [Map an existing custom domain to Azure Spring Apps](how-to-custom-domain.md). After successful setup, both service instances will bind to the same custom domain, such as `bcdr-test.contoso.com`.
