...
* To deploy the static files with the default server configuration file generated automatically, select a web server to use: nginx or httpd, use the following command:
...
|BP_WEB_SERVER_ROOT|Use a custom directory for static files. Defaults to public, setting this allows you to modify the location of the static files served by the web server with either an absolute file path or a file path relative to /workspace|
|BP_WEB_SERVER_ENABLE_PUSH_STATE|Enable push state routing functionality. This is useful for single-page web applications|
...
You can configure web server by providing a Customized server configuration file. Your web server configuration must conform to the following restrictions:
|Configuration|Description|Nginx Configuration|Httpd Configuration|
|-------------|-----------|-----|-----|
|Listening port|Web server must listen on port 8080. The service checks the port on TCP for readiness and liveness. It's requried to use templated variable PORT in the configuration file and the appropriate port number is injected when the web server is lanched|listen {{PORT}};|Listen "${PORT}"|
|Log path|Config log path to the console| access_log /dev/stdout; error_log stderr;| ErrorLog "/proc/self/fd/2"|
|File path with write permission|Web server only has write permssion for directory /tmp, configure all the path requires write permission under directory /tmp | For instance: client_body_temp_path /tmp/client_body_temp;||
