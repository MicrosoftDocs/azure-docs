...
1. In the **Artifact** textbox, select **Maven:hellospring(Java 17)**.
...
1. In the **App name:** textbox under **App Basics**, enter *hellospring*, and then select **More settings**.
...
1. To start the deployment, select the **Run** button at the bottom of the **Deploy to Azure** dialog box. The plug-in runs the command `mvn package` on the `hellospring` app and deploys the *.jar* file generated by the `package` command.
