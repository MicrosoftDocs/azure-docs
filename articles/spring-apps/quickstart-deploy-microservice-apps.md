...
> [!NOTE]
> 1. The article uses a simplified version of PetClinic, using an in-memory database, just to quickly deploy to Azure Spring Apps, it's not a production-ready database.
> 2. The deployed app `admin-server` also exposes public access, which is a risk point, and the production environment needs to secure Spring Boot Admin application.
