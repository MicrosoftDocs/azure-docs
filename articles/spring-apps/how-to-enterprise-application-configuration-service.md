...
However, we still support polyglot apps, and in these cases, we inject an environment named `AZURE_SPRING_APPS_CONFIG_FILE_PATH` with the configuration file path. To access the property values, you will need to write some code to read the files under these paths.
