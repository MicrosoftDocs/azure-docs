...
- If you'd like to write files to the same path for every individual application instance but keep them in separate subdirectories from the file share perspective, you could use the *SubPath* option. The *SubPath* option is exclusive to *Read only* because *SubPath* is used for writing new files and *Read only* is used for reading existing files.
