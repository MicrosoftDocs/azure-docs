...
> [!NOTE]
> When VNET integration is used, ensure the ports 80 and 445 are open.
...
- If you would like to write files to the same path for every individual application instance but keep them in separate subdirectories from the file share perspective, you could use the *SubPath* option. Be attention that the *SubPath* is exclusive to *Read only* because *SubPath* is used for writing new files and "Read only* is used for reading existing files.
