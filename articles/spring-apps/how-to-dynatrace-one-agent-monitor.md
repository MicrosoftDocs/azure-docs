...
You can apply some environment variables provided by Dynatrace to configure logging for the Dynatrace OneAgent. For example, `DT_LOGLEVELCON` controls the level of logs. Default value for `DT_LOGLEVELCON` is `info`. You can disable logs of the agent by setting `DT_LOGLEVELCON` to `off`. If logging is disabled, Dynatrace support will request logging to be enabled first to diagnose any agent issues effectively and restarting the app will be necessary for the change to take effect. For other log levels, please consult Dynatrace support team.
