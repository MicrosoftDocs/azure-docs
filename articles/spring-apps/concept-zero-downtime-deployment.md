...
A blue-green deployment eliminates downtime by running two deployment versions, with only one of the deployments to serve production traffic at any time. A blue-green deployment provides zero downtime by allowing you to switch to the other deployment version if something disrupts the live deployment.

When you perform a blue-green switch, Azure Spring Apps performs the following underlying operations on that are clients enabled for deployment:

- Overrides and sets the eureka registry status to `UP` for instances under production deployment.
- Overrides and sets the eureka registry status to `OUT_OF_SERVICE` for instances under staging deployment.
- Sets ingress rules to route public traffic to instances under production deployment.

> [!NOTE]
> For a blue-green deployment, you can achieve zero down time even for a single replica deployment.

## Zero downtime with rolling update strategy

For a deployment with a replica number two or higher, you can achieve zero downtime with a rolling update strategy from Azure Spring Apps. When you deploy a new version to an existing deployment, or restart a deployment, Azure Spring Apps uses underlying Kubernetes rolling updates to perform the update.
...
> [!WARNING]
> For a single replica deployment, you could see downtime during the deployment update. To ensure application availability, deployment with at least two replicas for your production workload is highly recommended.
...
1. Overrides and sets the instance's eureka registry status to `OUT_OF_SERVICE`, if the eureka client is enabled.
