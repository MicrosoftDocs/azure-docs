...
The application server can use the [service health check API](/rest/api/webpubsub/dataplane/webpubsub/get-service-status/get-service-status) to detect if its **primary** and **secondary** services are healthy. For example, for a Web PubSub service called `demo`, the endpoint `https://demo.webpubsub.azure.com/api/health` returns 200 when the service is healthy. The app server can periodically call the endpoints or call the endpoints on demand to check if the endpoints are healthy. WebSocket clients usually **negotiate** with their application server first to get the URL connecting to the Web PubSub service. The application uses this **negotiate** step to fail over the clients to other healthy **secondary** services. The detailed steps are:
...
1. Health check. The application can either check if the service is healthy by using the [service health check API](/rest/api/webpubsub/dataplane/webpubsub/get-service-status/get-service-status) periodically in the background or on demand for every **negotiate** call.
