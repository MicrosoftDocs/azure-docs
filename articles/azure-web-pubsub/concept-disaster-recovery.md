...
1. Health check. Application can either check if the service is healthy using [service health check API](/rest/api/webpubsub/dataplane/webpubsub/get-service-status/get-service-status) periodically in the background or on demand for every **negotiate** call.
