...
The event handler handles the incoming client events. Event handlers are registered and configured in the service through the Azure portal or Azure CLI beforehand so that when a client event is triggered, the service can identify if the event is expected to be handled or not. The Web PubSub service now supports the event handler as the server-side, which exposes the publicly accessible endpoint for the service to invoke when the event is triggered. In other words, it acts as a **webhook**. 
...
- Azure Active Directory(Azure AD) authentication.  For more information see [Managed identities for Web PubSub](howto-use-managed-identity.md).
...
1. Select **Add** to create a hub and configure your server-side webhook URL. For an existing hub configuration, select the hub and select **Edit**.
...
1. Then in the event handler page enter:
    1. The enter the server webhook URL in the **URL Template** field.
