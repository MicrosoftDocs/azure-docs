...
- If you're using Microsoft Entra ID, you can also invoke the [Generate Client Token REST API](/rest/api/webpubsub/dataplane/webpubsub/generate-client-token/generate-client-token).
...
> [!IMPORTANT]
> If you authenticate `WebPubSubServiceClient` with Microsoft Entra ID, assign the credential's identity an Azure role that includes both of the following data actions:
>
> - `Microsoft.SignalRService/WebPubSub/clientConnection/generateToken/action` to generate the client access token.
> - `Microsoft.SignalRService/WebPubSub/clientConnection/write` to allow the client to connect with the generated token.
>
> The built-in **Web PubSub Service Owner** role includes both permissions but grants access to all data-plane APIs. No narrower built-in role includes both permissions. For least-privilege access, use a custom role that contains only the permissions your application needs. For setup instructions, see [Use Azure Identity with `WebPubSubServiceClient`](howto-use-azure-identity.md).
...
You can also use Microsoft Entra ID to generate the token by invoking [Generate Client Token REST API](/rest/api/webpubsub/dataplane/webpubsub/generate-client-token/generate-client-token).

> [!IMPORTANT]
> Before you invoke `:generateToken`, assign the Microsoft Entra identity an Azure role that includes both of the following data actions:
>
> - `Microsoft.SignalRService/WebPubSub/clientConnection/generateToken/action` to generate the client access token.
> - `Microsoft.SignalRService/WebPubSub/clientConnection/write` to allow the client to connect with the generated token.
>
> The built-in **Web PubSub Service Owner** role includes both permissions but grants access to all data-plane APIs. No narrower built-in role includes both permissions. For least-privilege access, use a custom role that contains only these permissions.
>
> When the token is generated by using Microsoft Entra ID, Azure Web PubSub checks the identity's `clientConnection/write` permission again during the client handshake. Client roles included in the generated token separately control operations such as joining groups and sending messages.
