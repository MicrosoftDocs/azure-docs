...
## Create a Azure Function project locally

You should follow the steps below to initiate a local Azure Function project.
...
1. Currently, the Socket.IO Function Binding hasn't been included in Function Bundle, you need to manually add the package.

    1. Edit the `host.json` and remove the following lines to remove function bundle reference.
...
This creates a function `negotiate` with Http Trigger and `SocketIONegotiation` output binding, which means you can use a Http call to trigger the function and return a negotiation result that generated by `SocketIONegotiation` binding.
...
1. Create a function to return an index html for visiting.
    ...
    1. Create a html file `index.html` with the following content.
    ...
    1. Create a function to return the Html package
...
An easiest way to achieve this is to use [Tunnel Tool](https://learn.microsoft.com/azure/azure-web-pubsub/howto-web-pubsub-tunnel-tool)
