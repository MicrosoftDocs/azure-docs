...
**Example.** Assume the client VM is in *East US*, the primary Web PubSub resource is in *West US*, and a replica is in *East US*. When the client connects to Web PubSub through the private endpoint, the *East US* replica is selected for its better performance in routing.
