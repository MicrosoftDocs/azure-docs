...
|`Active Tokens`| Usage | Sum | Total tokens minus cached tokens over a period of time. Applies to PTU deployments. PTU utilization increases as the active tokens per minute (TPM) increases. |`ModelDeploymentName`,`ModelName`,`ModelVersion` `Region`|
