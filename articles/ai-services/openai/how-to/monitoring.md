...
:::image type="content" source="../media/monitoring/dashboard.png" alt-text="Screenshot that shows how to monitor and analyze metrics data for an Azure OpenAI resource in the Azure portal." lightbox="../media/monitoring/dashboard.png" border="false":::
...
:::image type="content" source="../media/monitoring/metrics.png" alt-text="Screenshot that shows how to monitor and analyze metrics data for an Azure OpenAI resource in the Azure portal." lightbox="../media/monitoring/metrics.png" border="false":::
...
|Metric|Category|Unit|Aggregation|Description|Dimensions|
|---|---|---|---|---|---|
...
|`Active Tokens`| Usage | Sum | Total tokens - cached tokens over a period of time. | Total tokens - cached tokens over a period of time |`ModelDeploymentName`,`ModelName`,`ModelVersion`m `Region`|
| `Generated Completion Tokens` | Usage | Sum | Number of generated tokens (output) from an OpenAI model. Applies to PayGo, PTU, and PTU-manged SKUs | `ApiName`, `ModelDeploymentName`,`ModelName`, `Region`, `UsageChannel`|
| `Processed FineTuned Training Hours` | Usage | Number of Training Hours Processed on an OpenAI FineTuned Model |  `ApiName`, `ModelDeploymentName`,`ModelName`, `Region`, `UsageChannel`|
| `Processed Inference Tokens` | Usage | Number of inference tokens processed by an OpenAI model. Calculated as prompt tokens (input) + generated tokens. Applies to PayGo, PTU, and PTU-manged SKUs.|`ApiName`, `ModelDeploymentName`,`ModelName`, `Region`, `UsageChannel`|
| `Processed Input Tokens` | Usage | Total number of prompt tokens (input) processed on an OpenAI model. Applies to PayGo, PTU, and PTU-managed SKUs.|`ApiName`, `ModelDeploymentName`,`ModelName`, `Region`, `UsageChannel`||
| `Provision-managed Utilization` | Usage | Provision-managed utilization is the utilization percentage for a given provisioned-managed deployment. Calculated as (PTUs consumed/PTUs deployed)*100. When utilization is at or above 100%, calls are throttled and return a 429 error code. | `ModelDeploymentName`,`ModelName`,`ModelVersion`, `Region`, `StreamType`|
