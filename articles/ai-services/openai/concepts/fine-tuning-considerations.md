...
description: Learn more about what you should take ino consideration before fine-tuning with Azure OpenAI Service 
...
When deciding whether or not fine-tuning is the right solution to explore for a given use case there are some key terms that it is helpful to be familiar with:
...
- If you identify cost as your primary motivator, proceed with caution. Fine-tuning may reduce costs for certain use cases by shortening prompts or allowing you to use a smaller model but there’s a higher upfront cost to training and you will have to pay for hosting you own custom model. Refer to the [pricing page](https://azure.microsoft.com/pricing/details/cognitive-services/openai-service/) for more information on Azure OpenAI fine-tuning costs.
- If you want to add out of domain knowledge to the model, you should start with retrieval augmented generation (RAG) with features like like Azure OpenAI's [on your data](./use-your-data.md) or [embeddings](../tutorials/embeddings.md). Often, this is a cheaper, more adaptable, and potentially more effective option depending on the use case and data.
...
Having a baseline for performance without fine-tuning is essential for knowing whether or not fine-tuning has improved model performance. Fine-tuning with bad data makes the base model worse, but without a baseline, it is hard to detect regressions.
...
- Be able to share specific experiences and challenges with techniques other than fine-tuning that have been tried for your use case.
...
Another important point is even with high quality data if your data is not in the necessary format for fine-tuning you will need to commit engineering resources in order to properly format the data.
...
- Dataset has not been identified yet
- Dataset format does not match the model you wish to fine-tune

## How will you measure the quality of your fine-tuned model?

There isn’t a single right answer to this question, but you should have clearly defined goals for what success with fine-tuning looks like. Ideally, this should not just be qualitative but should include quantitative measures of success like utilyzing a holdout set of data for validation, as well as user acceptance testing or A/B testing the fine-tuned model against a base model.
