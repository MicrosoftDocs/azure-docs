...
1. Optionally (and recommended), add the [LLM tool](llm-tool.md) to your flow. The LLM tool should output a list of vectors that will be used as inputs to the Faiss Index Lookup tool. For more information, see [LLM tool](llm-tool.md).
1. Select **+ More tools** > **Faiss Index Lookup** to add the Faiss Index Lookup tool to your flow.

    :::image type="content" source="../../media/prompt-flow/faiss-index-lookup-tool.png" alt-text="Screenshot of the playground page of the Azure AI Studio with python sample code in view" lightbox="../../media/prompt-flow/faiss-index-lookup-tool.png":::

1. Enter values for the Faiss Index Lookup tool input parameters described [here](#inputs). The vector input can be generated by the [LLM tool](llm-tool.md) as mentioned earlier.
...
| vector | list[float] | The target vector to be queried, which can be generated by the [LLM tool](llm-tool.md). | Yes |
