...
[`k4a_imu_sample_t *`](~/api/0.3.0/k4a-imu-sample-t.md) `imu_sample`
...
[K4A_WAIT_RESULT_SUCCEEDED](~/api/0.3.0/k4a-wait-result-t.md)
 if a capture is returned. If a capture is not available before the timeout elapses, the function will return 
[K4A_WAIT_RESULT_TIMEOUT](~/api/0.3.0/k4a-wait-result-t.md)
. All other failures will return 
[K4A_WAIT_RESULT_FAILED](~/api/0.3.0/k4a-wait-result-t.md)
.
