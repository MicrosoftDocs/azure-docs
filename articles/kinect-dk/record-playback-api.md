...
provided [k4arecorder](k4a-recordplayback.md) command line utility, or if customization is needed, the record API can be used directly
(see [k4a_record_create](~/api/k4a-record-create.md)).

More info on the Matroska file format specifications can be found on the [Recording File Format](~/sdk-record-format.md) page.
...
In the following example, we open a recording using [k4a_playback_open](~/api/k4a-playback-open.md), print the recording length,
and then close the file with [k4a_playback_close](~/api/k4a-playback-close.md).
...
[k4a_playback_get_previous_capture](~/api/k4a-playback-get-previous-capture.md), but this may be very slow depending on the length of the recording.
Instead we can use the [k4a_playback_seek_timestamp](~/api/k4a-playback-seek-timestamp.md) function to go to a specific point in the file.
...
stored in recording tags, which can be accessed using the [k4a_playback_get_tag](~/api/k4a-playback-get-tag.md) function.
...
the [k4a_record_configuration_t](~/api/k4a-record-configuration-t.md) struct, and can be read with the
[k4a_playback_get_record_configuration](~/api/k4a-playback-get-record-configuration.md) function.

If a tag does not exist, it is assumed to have the default value.

| Tag Name                   | Default Value      | [k4a_record_configuration_t](~/api/k4a-record-configuration-t.md) Field | Notes                                          |
|----------------------------|--------------------|-----------------------------------|--------------------------------------------------------------------------------------|
...
| K4A_CALIBRATION_FILE       | "calibration.json" | N/A                               | See [k4a_device_get_raw_calibration](~/api/k4a-device-get-raw-calibration.md)        |
...
| K4A_START_OFFSET_NS        | "0"                | start_timestamp_offset_usec       | See [Timestamp Synchronization](~/sdk-record-playback.md#timestamp-synchronization). |
...
The start offset is also available in the [k4a_record_configuration_t](~/api/k4a-record-configuration-t.md) struct.
