...
- Compatibility issues with ASMedia USB host controllers (e.g. ASM1142 chipset)
...
- Compute shader produces incorrect results on AMD graphics card
  ...
  - Workaround to use driver 18.12.1.1 or older OR avoid using NFOV 2x2 binned mode with latest driver
...
provided power adapter. Please note that while the power cable has a USB type A connected, the device requires more power than the a PC USB port can supply, so don't connect to it to 
...
- Check if you have any other application using the device (e.g. Windows camera application). Only 1 application at a time can access the device.
...
- You can try to reset this by going to Device Manager, right clicking on "Azure Kinect Microphone Array",  and select "Uninstall device". Once that is complete detach and reattach the sensor.
...
2) Take recording using [Azure Kinect Recorder](azure-kinect-sensor-viewer.md), e.g. `k4arecorder.exe -l 5 -r 5 output.mkv`
...
ensure the USB3 driver is a Microsoft driver, this is indicated in Device Manager with a '(Microsoft)' after the name. To remove a 3rd party driver,
right click the device in Device Manager and click "Uninstall Device". From there select the checkbox to delete the driver, and then hit the button 'Uninstall'
