...
The Azure Kinect SDK on Windows platforms rely on a unified container ID, and it must span USB 2.0 and 3.0 devices so that the SDK can find the depth, color, and audio devices that are physically located on the same device. 
...
The topic of USB host controllers gets even more complicated when a PC has more than 1 host controller installed. When host controllers are mixed a user may experience issues where some ports work fine and other do not work at all. Depending on how the ports are wired to the case, you may see all front ports having issues with the Azure Kinect
...
To better understand which USB port is connected on a PC, repeat these steps for each USB port as you connect the Azure Kinect is connect to different USB ports on the PC.
...
With a manual exposure set, it can take up to 8 captures from the device before images and framerate stabilize. 
With auto exposure, it can take up to 20 captures before images and framerate stabilize.

### Depth Camera Auto Powers Down

The laser used by the depth camera to calculate image depth data, has a limited lifespan. To maximize the life of these laser’s, the depth camera will detect when depth data is not being consumed. The depth camera will auto power down when in a condition where the device is streaming for several minutes but the host PC is not reading data. 
This also impacts Multi Device Synchronization where subordinate devices start up in a state where the depth camera is streaming and depth frames are actively help up waiting for the master device to start synchronizing captures. To avoid this problem in Multi Device capture scenarios, ensure the master devices starts within a minute of the first subordinate being started. 
