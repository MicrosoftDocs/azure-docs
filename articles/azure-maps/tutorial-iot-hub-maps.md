...
When your Azure function is running, you can now send telemetry data to the IoT hub, which will route it to Event Grid. Use a C# application to simulate location data for an in-vehicle device of a rental car. To run the application, you need the .NET Core SDK 3.1 or later on your development computer. Follow these steps to send simulated telemetry data to the IoT hub:
