...
3. Navigate to the **Access keys** blade in your storage account and copy the "storage account name" and "access key". They are needed in a later step.

    ![access-keys](./media/tutorial-iot-hub-maps/access-keys.png)


Now, we have a storage account and a container to log event data. Next, we will create an IoT hub.

### Create an IoT Hub

An IoT Hub is a managed service in the cloud that acts as a central message hub for bi-directional communication between an IoT application and the devices managed by it. In order to route device telemetry messages to an Event Grid, create an IoT Hub within the "ContosoRental" resource group. Set up a message route integration where we will filter messages based on the car's engine status. We will also send device telemetry messages to the Event Grid whenever the car is moving.
