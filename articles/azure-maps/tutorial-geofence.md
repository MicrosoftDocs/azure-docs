...
Azure Maps supports three event types. You can have a look at the Azure Maps supported event types [here](../../event-grid/event-schema-azure-maps.md). We need two different event subscriptions, one for the enter event and one for the exit events.
