...
Azure Maps uses an extended version of the GeoJSON schema which provides a definition for circles as noted [here](extend-geojson.md#circle). A circle can be rendered on the map by creating a `Point` feature that has a `subType` property with a value of `Circle` and a `radius` property which has a number which represents the radius in meters. For example:
