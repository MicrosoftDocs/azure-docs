...
| `OnClick`              | `(double lat, double lon): boolean` | Fired when the map is pressed and released at the same point on the map. Return true indicating if event should be consumed and not passed further to other listeners registered afterwards, false otherwise. |
| `OnFeatureClick`       | `(List<Feature>): boolean`    | Fired when the map is pressed and released at the same point on a feature. Return true indicating if event should be consumed and not passed further to other listeners registered afterwards, false otherwise. |
...
| `OnLongClick`          | `(double lat, double lon): boolean` | Fired when the map is pressed, held for a moment, and then released at the same point on the map. Return true indicating if event should be consumed and not passed further to other listeners registered afterwards, false otherwise. |
| `OnLongFeatureClick `  | `(List<Feature>): boolean`    | Fired when the map is pressed, held for a moment, and then released at the same point on a feature. Return true indicating if event should be consumed and not passed further to other listeners registered afterwards, false otherwise. |
