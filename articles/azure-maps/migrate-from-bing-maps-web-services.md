...
| Directions (including truck)          | [Route directions](https://docs.microsoft.com/rest/api/maps/route/getroutedirections)                          |
| Distance Matrix                       | [Route Matrix](https://docs.microsoft.com/rest/api/maps/route/postroutematrixpreview)                          |
| Imagery – Static Map                  | [Render](https://docs.microsoft.com/rest/api/maps/render/getmapimage)                                   |
| Isochrones                            | [Route Range](https://docs.microsoft.com/rest/api/maps/route/getrouterange)                                    |
| Local Insights                        | [Search](https://docs.microsoft.com/rest/api/maps/search) + [Route Range](https://docs.microsoft.com/rest/api/maps/route/getrouterange)    |
| Local Search                          | [Search](https://docs.microsoft.com/rest/api/maps/search)     |
| Location Recognition (POIs)           | [Search](https://docs.microsoft.com/rest/api/maps/search)     |
| Locations (forward/reverse geocoding) | [Search](https://docs.microsoft.com/rest/api/maps/search)                                               |
| Snap to Road                          | [POST Route directions](https://docs.microsoft.com/rest/api/maps/route/postroutedirections)                         |
| Spatial Data Services (SDS)           | [Search](https://docs.microsoft.com/rest/api/maps/search) + [Route](https://docs.microsoft.com/rest/api/maps/route) + other Azure Services |
| Time Zone                             | [Time Zone](https://docs.microsoft.com/rest/api/maps/timezone)  |
| Traffic Incidents                     | [Traffic Incident Details](https://docs.microsoft.com/rest/api/maps/traffic/gettrafficincidentdetail)                     |
...
-   [Map Tiles](https://docs.microsoft.com/rest/api/maps/render/getmaptile) – Access road and imagery tiles from Azure Maps as raster and vector tiles.
-   [Batch routing](https://docs.microsoft.com/rest/api/maps/route/postroutedirectionsbatchpreview) – Allows up to 1,000 route requests to be made in a single batch over a period of time. Routes are calculated in parallel on the server for faster processing.
-   [Traffic](https://docs.microsoft.com/rest/api/maps/traffic) Flow – Access real-time traffic flow data as both raster and vector tiles.
-   [Geolocation API](https://docs.microsoft.com/rest/api/maps/geolocation/getiptolocationpreview) – Get the location of an IP address.
-   [Weather Services](https://docs.microsoft.com/rest/api/maps/weather) – Gain access to real-time and forecast weather data.

Be sure to also review the following best practices guides:

-   [Best practices for search](https://docs.microsoft.com/azure/azure-maps/how-to-use-best-practices-for-search)
-   [Best practices for routing](https://docs.microsoft.com/azure/azure-maps/how-to-use-best-practices-for-routing)
...
An example of how to use the search service is documented [here](https://docs.microsoft.com/azure/azure-maps/how-to-search-for-address). Be sure to review the [best practices for search](https://docs.microsoft.com/azure/azure-maps/how-to-use-best-practices-for-search) documentation.
...
| `PopulatedPlace`      | `Municipality` or `MunicipalitySubdivision`     | *City / Town or Sub / Super City*          |
| `Postcode1`           | `PostalCodeArea`                                | *Postal Code / Zip Code*                   |
| `AdminDivision1`      | `CountrySubdivision`                            | *State or Province*                        |
| `AdminDivision2`      | `CountrySecondarySubdivison`                    | *County, districts*                        |
...
Azure Maps can snap coordinates to roads by using the [route directions](https://docs.microsoft.com/rest/api/maps/route/postroutedirections) API. This service can be used to reconstruct a logical route between a set of coordinates and is comparable to the Bing Maps Snap to Road API.
...
| Bing Maps API parameter    | Comparable Azure Maps API parameter                                 |
|----------------------------|---------------------------------------------------------------------|
| `points`                   | `supportingPoints` – pass this in the body of the post request      |
| `interpolate`              | N/A                                                                 |
...
Azure Maps provides an API for rendering the static map images with data overlaid. The Azure Maps [Map image render](https://docs.microsoft.com/rest/api/maps/render/getmapimagerytile) API is comparable to the static map API in Bing Maps.
...
| `imagerySet`             | `layer` and `style` – See [Supported map styles](https://docs.microsoft.com/azure/azure-maps/supported-map-styles) documentation.   |
| `mapArea` (`ma`)         | `bbox`                                         |
| `mapLayer` (`ml`)        | N/A                                            |
...
See the [How-to guide on the map image render API](https://docs.microsoft.com/azure/azure-maps/how-to-render-custom-data) for more information.
...
In Bing Maps, pushpins can be added to a static map image by using the `pushpin` parameter in the URL. The `pushpin` parameter takes in a location in `latitude,longitude` format, an icon style and text label (up to 3 characters) as shown below:
...
For example, in Bing Maps, a blue line with 50% opacity and a thickness of 4 pixels can be added to the map between coordinates (longitude: -110, latitude: 45 and longitude: -100, latitude: 50) with the following URL parameter:
...
For example, in Azure Maps, a blue line with 50% opacity and a thickness of 4 pixels can be added to the map between coordinates (longitude: -110, latitude: 45 and longitude: -100, latitude: 50) with the following URL parameter:
...
-   [Route](https://docs.microsoft.com/rest/api/maps/route/getrouterange) Range**: Calculate a polygon covering an area that can be traveled to in any direction from an origin point within a specified amount of time, distance or amount of fuel/charge available.
...
-   **Local search:** Searches for points of interest that are nearby (radial search), by name, or by entity type (category). The Azure Maps [POI search](https://docs.microsoft.com/rest/api/maps/search/getsearchpoi) and [POI category search](https://docs.microsoft.com/rest/api/maps/search/getsearchpoicategory) APIs are most like this API.
-   **Location recognition**: Searches for points of interests that are within a certain distance of a location. The Azure Maps [nearby search](https://docs.microsoft.com/rest/api/maps/search/getsearchnearby) API is most like this API.
-   **Local insights:** Searches for points of interests that are within a specified maximum driving time or distance from a specific coordinate. This is achievable with Azure Maps by first calculating an isochrone and then passing it into the [search within geometry](https://docs.microsoft.com/rest/api/maps/search/postsearchinsidegeometry) API.
...
1.  Pass a query for the boundary you want to receive into one of the following search APIs.
	...
	-   [Fuzzy search](https://docs.microsoft.com/rest/api/maps/search/getsearchfuzzy)
	-   [Fuzzy batch search](https://docs.microsoft.com/rest/api/maps/search/postsearchfuzzybatchpreview)
2.  If the desired result(s) has a geometry ID(s), pass it into the [Search Polygon API](https://docs.microsoft.com/rest/api/maps/search/getsearchpolygon).
...
-   [Azure SQL Spatial Data Types overview](https://docs.microsoft.com/sql/relational-databases/spatial/spatial-data-types-overview)
-   [Azure SQL Spatial – Query nearest neighbor](https://docs.microsoft.com/sql/relational-databases/spatial/query-spatial-data-for-nearest-neighbor)
-   [Azure Cosmos DB geospatial capabilities overview](https://docs.microsoft.com/azure/cosmos-db/geospatial)
