...
The second code block uses the map's [setStyle](https://docs.microsoft.com/javascript/api/azure-maps-control/atlas.map?view=azure-iot-typescript-latest) method to set the map style to satellite, after a 5 second delay.
...
The second code block constructs a style selector using the atlas [StyleControl](/javascript/api/azure-maps-control/atlas.controls.stylecontrol) constructor. By default, all styles available with the S0 license tier appear in the control. The `mapStyles` option can be used to limit which map styles to display in the control.  If using the S1 tier license, you can use the `mapStyles` option to add the `'satellite'` and `'satellite_road_labels'` styles to the control. See the [Supported Styles](https://docs.microsoft.com/azure/azure-maps/supported-map-styles) documentation for a complete list of supported styles.   

A style picker enables style selection for the map. The third code block adds the style picker to the map using the map's [controls.add](https://docs.microsoft.com/javascript/api/azure-maps-control/atlas.controlmanager#add-control---control----controloptions-
) method. The style selector is within the map **event listener** to ensure that it loads after the map loads fully.
