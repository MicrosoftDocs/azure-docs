...
- Using a touch screen, touch the map with two fingers and pinch together too zoom out or spread the fingers apart to zoom in.
...
- Press and hold the Ctrl button and press the left mouse button down on the map and drag to draw out an area to zoom the map into.
...
Users can navigate the map using the keyboard. If a screen reader is running, the map will notify the user of changes to its state. For example, users are notified of map changes when the map is panned or zoomed. By default, the map provides simplified descriptions which exclude the zoom level and coordinates of the center of the map. The user can toggle the detail level of these descriptions by using the keyboard short cut `Ctrl` + `Shift` + `D`.
...
- Keep your screen reader (i.e. ARIA, alt, and title attributes) messages short, descriptive and meaningful. Avoid unnecessary jargon and acronyms.
- Try to optimize messages sent to the screen reader to provide short meaningful information which is easy for the user to digest. For example, if you want to update the screen reader at a high frequency, such as when the map is moving, consider doing the following:
...
- Some visualizations such as heatmaps, tile layers, and image layers are not generally accessible for users with vision impairments. Some considerations:
