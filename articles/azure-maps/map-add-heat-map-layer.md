...
* Rendering data for noise sensors as a heat map not only shows the insanity of the noise where the sensor is but can also provide insights into the dissipation over a distance. The noise level at any one site may not be that high, however if the noise coverage area from multiple sensors overlap, its possible that this overlapping area may experience higher noise levels which would be visible in the heat map.
...
In this sample each heat point will has a radius of 10 pixels at all zoom levels. When adding the heat map layer to the map, this samples inserts it below the label layer. This creates a better user experience as the labels are clearly visible above the heat map. The data in this sample is sourced from the [USGS Earthquake Hazards Program](https://earthquake.usgs.gov/) and consists of points that represents significant earthquakes that have occurred in the last 30 days.
...
* `radius`: Defines a pixel radius in which to render each data point. The radius can be set as a fixed number or as an expression. Setting the radius as an expression which sets the radius based on the zoom level can result in heat maps which appear to have a radius that represents a consistent spatial area on the map.
* `color`: Specifies how the heat map is colorized. A gradient color palette is often used for heat maps, but stepped color palettes are also useful if you want make your heat map look more like contour data. These color palettes define the colors from the minimum to the maximum density value. Color values for heat maps are specified as an expression on the “heatmap-density” value. The color at index 0 in a gradient expression or the default color of a step color defines the color of the area where there is no data, or background color. Many prefer to set this to transparent or a semi-transparent black. Here are examples of color expressions;
...
* `weight`: By default, all data points have a weight of 1, thus all data points are weighted equally. The weight option acts as a multiplier and can be set as a number or an expression. If a number is set as number, say 2, it would be the equivalent of placing each data point on the map twice, thus doubling the density. Setting the weight option to a number renders the heat map in a similar way to using the intensity option. However, if an expression is used, the weight of each data point can be based on different and based on some metric in the properties of the point. Take earthquake data for example, each data point represents an earthquake and an important metric each earthquake has is its magnitude. Earthquakes happen all the time, but most have a low magnitude and aren’t even felt. Using the magnitude value in an expression to assign the weight option will allow the more significant earthquakes to be increase be better represented within the heat map.
