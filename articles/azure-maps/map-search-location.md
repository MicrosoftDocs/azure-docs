...
The last block of code adjusts the camera bounds for the map by using the Map's [setCameraBounds](/javascript/api/azure-maps-control/atlas.map?view=azure-iot-typescript-latest#setcamerabounds-cameraboundsoptions-) property.
...
The last block of code parses the response and adjusts the adjusts the camera bounds for the map by using the Map's [setCameraBounds](/javascript/api/azure-maps-control/atlas.map?view=azure-iot-typescript-latest#setcamerabounds-cameraboundsoptions-) to render the result pins.
