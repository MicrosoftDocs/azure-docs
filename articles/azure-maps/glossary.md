...
<a name="advanced-routing"></a> **Advanced routing**: A collection of services that perform advance operations using road routing data, such as calculating reachable ranges (isochrones), distance matrices, and performing batch route requests.
...
<a name="altitude"></a> **Altitude**: The height or vertical elevation of a point above a reference surface. Altitude measurements are usually based on a given reference datum, such as mean sea level. See also elevation.
...
<a name="antimeridian"></a> **Antimeridian**: Also known as the 180<sup>th</sup> Meridian is the point where -180 degrees and 180 degrees of longitude meet. Which is completely opposite the prime meridian on the globe.
...
<a name="asynchronous-request"></a> **Asynchronous request**: A HTTP request which opens a connection and makes a request to the server which returns an identifier for the asynchronous request, then closes the connection. The server continues to process the request and the user can check the status using the identifier. When the request is finished processing, the user can then download the response. This type of request is usually used for long running processes.
...
<a name="base-map"></a> **Base map**: A map depicting background reference information such as roads, landmarks, and political boundaries.

<a name="batch-request"></a> **Batch request**: The process of combining multiple requests into a single request.

<a name="bearing"></a> **Bearing**: The horizontal direction of a point in relation to another point, expressed as an angle from a known direction, usually north, and usually measured from 0 degrees at the reference direction clockwise through 360 degrees. 

<a name="boundary"></a> **Boundary**: A line or polygon separating adjacent political entities, such as countries or districts; adjacent tracts of privately-owned land, such as parcels; or adjacent geographic zones, such as ecosystems. A boundary is a line that may or may not follow physical features, such as rivers, mountains, or walls.
...
<a name="centroid"></a> **Centroid**: The geometric center of a feature. Of a line, it is the midpoint; of a polygon, the center of area; of a three-dimensional figure, the center of volume.
...
<a name="convex-hull"></a> **Convex hull**: A convex hull is a shape that represents that minimum convex geometry that encloses all shapes in the specified data set. The generated shape is similar to wrapping an elastic band around the data set.

<a name="coordinate"></a> **Coordinate**: Consists of the longitude and latitude values used to represent a location on a map.

<a name="coordinate-system"></a> **Coordinate system**: A reference framework consisting of a set of points, lines, and/or polygons, and a set of rules, used to define the positions of points in space in two or three dimensions.

<a name="country-code"></a> **Country code**: A unique identifier for a country based on the ISO standard. ISO2 is a two-character code for a country (i.e. US), which ISO3 represents a three-character code (i.e. USA).
...
<a name="datum"></a> **Datum**: The reference specifications of a measurement system, usually a system of coordinate positions on a surface (a horizontal datum) or heights above or below a surface (a vertical datum).

<a name="dbf-file"></a> **DBF file**: A database file format that is usually used in combination with Shapefiles (SHP).
...
<a name="destination"></a> **Destination**: An end point or location in which someone is travelling to.

<a name="digital-elevation-model-dem"></a> **Digital Elevation Model (DEM)**: The representation of continuous elevation values over a topographic surface by a regular array of z-values, referenced to a common datum. DEMs are typically used to represent terrain relief.
...
<a name="extended-postal-code"></a> **Extended postal code**: A postal code that may include additional information. For example, in the USA, zip codes have 5 digits but an extended zip code, known as zip+4, will include 4 additional digits to identify a geographic segment within the five-digit delivery area, such as a city block, a group of apartments, an individual high-volume receiver of mail, a post office box, or any other unit that could use an extra identifier to aid in efficient mail sorting and delivery.
...
<a name="find-nearby"></a> **Find nearby**: A spatial query which searches a fixed straight-line distance (as the crow flies) from a point.

<a name="fleet-management"></a> **Fleet management**: The management of commercial vehicles such as cars, trucks, ships and planes. Fleet management can include a range of functions, such as vehicle financing, maintenance, telematics (tracking and diagnostics) as well as driver, speed, fuel and health and safety management. Fleet Management is a function which allows companies which rely on transportation in business to remove or minimize the risks associated with vehicle investment, improving efficiency, productivity and reducing their overall transportation and staff costs, and ensure compliance with government legislation.
...
<a name="fuzzy-search"></a> **Fuzzy search**: A search which takes in a free form string of text which may be an address or point of interest. 
...
<a name="geojson"></a> **GeoJSON**: Is a common JSON based file format used for storing geographical vector data such as points, lines and polygons. Note that Azure Maps uses an extended version of GeoJSON as [documented here](extend-geojson.md).
...
<a name="georeference"></a> **Georeference**: The process of aligning geographic data or imagery to a known coordinate system. This may consist of shifting, rotating, scaling, or skewing the data.
...
<a name="gps"></a> **GPS**: Also known as Global Positioning System, is a system of satellites used for determining a devices positions on the earth. The orbiting satellites transmit signals that allow a GPS receiver anywhere on earth to calculate its own location through trilateration.
...
<a name="haversine-formula"></a> **Haversine formula**: A common equation used for calculating the great-circle distance between two points on a sphere given their longitude and latitude values.

<a name="hd-maps"></a> **HD maps**: Also known as High Definition Maps are high fidelity road network information, specifically attributes such as lane markings, signage and direction lights required for autonomous driving.
...
<a name="isochrone"></a> **Isochrone**: Given a location, an isochrone defines the area in which someone can travel within a specified time for a mode of transportation in any direction. See also [Reachable Range](#reachable-range).
...
<a name="landsat"></a> **Landsat**: Multispectral, earth-orbiting satellites developed by NASA (National Aeronautics and Space Administration) that gather imagery for land-use inventory, geological and mineralogical exploration, crop and forestry assessment, and cartography.
...
<a name="linestring"></a> **LineString**: A geometry used to represent a line. This is also known as a polyline. 
...
<a name="nearby-search"></a> **Nearby search**: A spatial query which searches a fixed straight-line distance (as the crow flies) from a point.
...
<a name="polyline"></a> **Polyline**: A geometry used to represent a line. This is also known as a LineString. 
...
<a name="prime-meridian"></a> **Prime meridian**: A line of longitude that represents 0 degrees longitude. Generally, the longitude values increase when traveling in a westerly direction until 180 degrees is reached and decreases when traveling in easterly directions to -180 degrees. 

<a name="prj"></a> **PRJ**: A text file that often accompanies a Shapefile file and contains information about the projected coordinate system in which the data set is in.
...
<a name="quadkey"></a> **Quadkey**: A base-4 address index for a tile within a quadtree tiling system. See [Zoom levels and tile grid](zoom-levels-and-tile-grid.md) documentation for more information.

<a name="quadtree"></a> **Quadtree**: A data structure in which each node has exactly four children. The tiling system used in Azure Maps uses a quadtree structure such that as a user zooms in one level, each map tile breaks up into four sub-tiles.  See [Zoom levels and tile grid](zoom-levels-and-tile-grid.md) documentation for more information.
...
<a name="radial-search"></a> **Radial search**: A spatial query which searches a fixed straight-line distance (as the crow flies) from a point. 
...
<a name="reachable-range"></a> **Reachable range**: Given a location, a reachable range defines the area in which someone can travel within a specified time or distance for a mode of transportation to travel in any direction. See also [Isochrone](#isochrone) and [Isodistance](#isodistance).
...
<a name="sql-spatial"></a> **SQL spatial**: Refers to the spatial functionality built into SQL Azure and SQL Server 2008 and above. This spatial functionality is also available as a.NET library which can be used independently of SQL Server. See the [Spatial Data (SQL Server) documentation](https://docs.microsoft.com/sql/relational-databases/spatial/spatial-data-sql-server) for more information.

<a name="synchronous-request"></a> **Synchronous request**: An HTTP request that is opens a connection and waits for a response. If multiple long running synchronous request are made at the same time in a web browser, some requests may be delayed as browsers limit the number of concurrent HTTP requests that can be open.
...
<a name="tile-layer"></a> **Tile layer**: A layer displayed by assembling map tiles (rectangular sections) into a continuous layer. The tiles are either raster image tiles or vector tiles. A raster tile layer is typically rendered on ahead of time or on the server, while vector tile layers are rendered on the fly, usually within the client application.

<a name="time-zone"></a> **Time zone**: A region of the globe that observes a uniform standard time for legal, commercial, and social purposes. Time zones tend to follow the boundaries of countries and their subdivisions because it is convenient for areas in close commercial or other communication to keep the same time.
...
- One transaction is created for each API call to the search, routing, time zone, and non-map tile render service (static image service) and non-traffic tile services (i.e. incident details).
...
<a name="travelling-salesmen-problem-tsp"></a> **Travelling Salesmen Problem (TSP)**:  A Hamiltonian circuit problem in which a salesperson must find the most efficient way to visit a series of stops, then return to the starting location.  

<a name="trilateration"></a> **Trilateration**: Determining the position of a point on the earth's surface with respect to two other points by measuring the distances between all three points.

<a name="turn-by-turn-navigation"></a> **Turn-by-turn navigation**: An application which provides route instructions for each step of a route as the users approaches the next maneuver.
...
<a name="vehicle-routing-problem-vrp"></a> **Vehicle Routing Problem (VRP)**: A class of problems in which a set of routes for a fleet of vehicles to service a set of orders with the goal of minimizing some objective (i.e. operating cost), while satisfying certain constraints. These constraints may include time windows, multiple route capacities, travel duration constraints, route zone and route seed point constraints, specialties constraints, and paired order constraints.

<a name="voronoi-diagram"></a> **Voronoi diagram**: A partition of space into areas, or cells, that surround a set of geometric objects (usually points). These cells, or polygons, must satisfy the criteria for Delaunay triangles. All locations within an area are closer to the object it surrounds than to any other object in the set. Voronoi diagrams are often used to delineate areas of influence around geographic features. 
...
<a name="waypoint-optimization"></a> **Waypoint optimization**: The process of reordering a set of waypoints to minimize the travel time or distance required to pass through all provided waypoints. Often referred to as the [Travelling Salesmen Problem](#travelling-salesmen-problem-tsp) or [Vehicle Routing Problem](#vehicle-routing-problem-vrp) depending on the complexity of the optimization.
