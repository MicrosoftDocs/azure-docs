...
In cases where a multi-tenant architecture is required, Azure Lighthouse can help centralize and streamline management operations. Subscriptions from multiple tenants can be onboarded for [Azure Lighthouse](azure-delegated-resource-management.md), allowing designated users in one managing tenant to perform [cross-tenant management functions](cross-tenant-management-experience.md) in a centralized, scalable manner.
...
In most enterprise scenarios, you’ll want to delegate a full subscription to Azure Lighthouse, although you can choose to delegate only specific resource groups within a subscription.
...
Since enterprise users will normally be able to gain direct access to the enterprise’s tenants, and there's no need to market or promote a management offering, it's usually faster and more straightforward to deploy Azure Resource Manager templates. While the [onboarding guidance](../how-to/onboard-customer.md) refers to service providers and customers, enterprises can use the same processes to onboard their tenants.
