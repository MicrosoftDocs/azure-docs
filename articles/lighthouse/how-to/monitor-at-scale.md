...
> [!IMPORTANT]
> Even if all of the workspaces are created in the customer tenant, the Microsoft.Insights resource provider must also be registered on a subscription in the managing tenant. If your managing tenant does not have an existing Azure Subscription then you can register the resource provider manually using the following PowerShell commands:
> ```powershell
