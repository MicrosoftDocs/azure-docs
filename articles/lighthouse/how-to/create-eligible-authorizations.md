...
Specify whether or not to require Azure multifactor authentication in order for an eligible role to be activated.
...
- `multiFactorAuthProvider` can either be set to **Azure**, which will require authentication using Azure multifactor authentication, or to **None** if no multifactor authentication will be required.
