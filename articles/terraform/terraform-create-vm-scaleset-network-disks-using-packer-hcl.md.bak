---
title: Use Terraform to create an Azure VM scale set from a Packer custom image
...
# Use Terraform to create an Azure VM scale set from a Packer custom image

In this article, you use use [Terraform](https://www.terraform.io/) to create and deploy an [Azure virtual machine scaleset](/azure/virtual-machine-scale-sets/virtual-machine-scale-sets-overview) created with a custom image produced using [Packer](https://www.packer.io/intro/index.html) with managed disks using the [Hashicorp Configuration Language](https://www.terraform.io/docs/configuration/syntax.html) (HCL).  
...
> * Create a custom VM image using Packer
> * Create and deploy a vm scaleset using the custom image
...
![VMSS terraform fully-qualified domain name for Public IP address](./media/terraform-create-vm-scaleset-network-disks-using-packer-hcl/tf-create-vmss-step4-fqdn.png)

The resource group contains the following resources: 
![VMSS terraform network resources](./media/terraform-create-vm-scaleset-network-disks-using-packer-hcl/tf-create-vmss-step4-rg.png)


## Create a Azure image using Packer
...
Follow the tutorial till you create deprovisioned Ubuntu image with NGINX installed.
...
![Terraform vm scaleset resource group](./media/terraform-create-vm-scaleset-network-disks-using-packer-hcl/tf-create-vmss-step6-apply.png)
...
This optional step enables SSH access to the instances of the VM scale set by using a jumpbox.
...
![Terraform vm scaleset resource group](./media/terraform-create-vm-scaleset-network-disks-using-packer-hcl/tf-create-create-vmss-step8.png)
