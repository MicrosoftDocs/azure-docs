...
If you want to run the execute the plan from the latest execution plan, run the following command:

  ```cmd
  terraform apply
  ```

If you want to run the execute the plan from a saved execution plan, run the following command (replacing the &lt;path> placeholder with the path that contains the saved execution plan):
