...
A private zone [virtual network link](private-dns-virtual-network-links.md) enables DNS resolution of your private zone in the linked VNet. This method is simple, however might not always be ideal. For example, you might wish to resolve some names differently in different VNets, or need to resolve on-premises domains in a spoke VNet. Azure DNS Private Resolver adds more options to DNS resolution on your network, and presents some design options.

The private resolver provides two components that you can use to resolve names in your private DNS zone, and other namespaces: forwarding rulesets and inbound endpoints. Which of these methods to use depends on your network design. 
- **Forwarding rulesets** can be linked to a VNet to provide DNS forwarding capabilities. A ruleset containing a rule to forward queries to your private resolver's inbound endpoint enables resolution of the private zone in any VNet that is linked to the ruleset. Spoke VNets that are linked to a ruleset with a forwarding rule for the private zone do not need to be linked to the private zone itself. DNS resolution of the private zone is carried out by the inbound endpoint in the hub VNet.
