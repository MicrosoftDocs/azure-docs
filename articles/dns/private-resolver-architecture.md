...
A private zone [virtual network link](private-dns-virtual-network-links.md) enables DNS resolution of your private zone in the linked VNet. This method is simple, might not be ideal for every network design. For example, you might have multiple private zones, wish to resolve names differently in different VNets, or need to resolve on-premises domains in some locations and not others. Azure DNS Private Resolver adds more flexibility and scalability to DNS resolution on your network, and provides some additional design options.
...
- **Forwarding rulesets** are linked to a VNet to provide DNS forwarding capabilities. A ruleset can contain a rule to forward queries to the private resolver's inbound endpoint, enabling resolution of the private zone in any VNet that is linked to the ruleset. Spoke VNets that are linked to a ruleset with a forwarding rule for the private zone do not need to be linked to the private zone itself. DNS resolution of the private zone is carried out by the inbound endpoint in the hub VNet.
