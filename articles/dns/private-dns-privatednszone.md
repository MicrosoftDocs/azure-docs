...
To understand how many private DNS zones you can create in a subscription and how many record sets are supported in a private DNS zone refer to [Azure DNS limits](https://docs.microsoft.com/azure/azure-subscription-service-limits#azure-dns-limits)

## Restrictions

* Single labeled private DNS zones are not supported. Your private DNS zone must have two or more labels. For example contoso.com has two labels separated by a dot. A private DNS zone can have 34 labels at max.
* You can't create zone delegations (NS records) in a private DNS zone. If you intend to use a child domain, you can directly create the domain as a private DNS zone and link it to virtual network without having to setup nameserver delegation from parent zone.
