...
# How to manage DNS records with CLI
...
Azure DNS supports the following record types: A, AAAA, CNAME, MX, NS, SOA, SRV, TXT.  Record sets of type SOA are created automatically with each zone, they cannot be created separately.
	...
	azure network record-set create -g myresourcegroup -z contoso.com -n www -y A --ttl 300
...
## Get a record set
To retrieve an existing record set, use `azure network dns-record-set show`, specifying the record set relative name, the record type, and the zone:

	azure network dns-record-set show -g myresourcegroup -z contoso.com -n www -y A

As with New-AzureDnsRecordSet, the record name must be a relative name, i.e. excluding the zone name.  The zone can be specified by zone name and resource group name:
...
List all record sets.  This will return all record sets, regardless of name or record type:
	...
	azure network dns-record-set list -g myresourcegroup -z contoso.com
...
List record sets of a given record type.  This will return all record sets matching the given record type (in this case, A records):
	...
	azure network dns-record-set list -g myresourcegroup -z contoso.com -y A 

In both cases you will specify the resource group name (-g switch) and Zone name (-z switch).
...
### Create A record set with single record
	...
	azure network dns-record-set create -g myresourcegroup -z contoso.com -n test-a -y A --ttl 300

	azure network dns-record-set add-record -g myresourcegroup -z contoso.com -n test-a -a 192.168.1.1 

### Create AAAA record set with single record

	azure network dns-record-set create -g myresourcegroup -z contoso.com -n test-aaaa -y AAAA

	azure network dns-record-set add-record -g myresourcegroup -z contoso.com -n test-aaaa -b "2607:f8b0:4009:1803::1005"

### Create CNAME record set with single record

	azure network dns-record-set create -g myresourcegroup -z contoso.com -n test-cname -y CNAME --ttl 300
	
	azure network dns-record-set add-record -g myresourcegroup -z contoso.com -n test-cname -c "www.contoso.com" 
...
In this example, we use the record set name "@" to create the MX record at the zone apex (e.g. "contoso.com").  This is common for MX records.
	...
	azure network dns-record-set create -g myresourcegroup -z contoso.com -n "@" -y MX --ttl 300

	azure network dns-record-set add-record -g myresourcegroup -z contoso.com -n "@" -e "mail.contoso.com" -f 5


### Create NS record set with single record

	azure network dns-record-set create -g myresourcegroup -z contoso.com -n test-ns -y NS --ttl 300
	
	azure network dns-record-set add-record -g myresourcegroup -z contoso.com -n "test-ns" -d "ns1.contoso.com" 
...
If creating an SRV record in root of zone, just specify _service and _protocol in the record name—there is no need to also include ‘.@’ in the record name
	...
	azure network dns-record-set create -g myresourcegroup -z contoso.com -n "_sip._tls" -y SRV --ttl 300 

	azure network dns-record-set add-record -g myresourcegroup -z contoso.com -n "_sip._tls" -p 0 - w 5 -o 8080 -u "sip.contoso.com" 

### Create TXT record set with single record

	azure network dns-record-set create -g myresourcegroup -z contoso.com -n "test-TXT" -y TXT --ttl 300

	azure network dns-record-set add-record -g myresourcegroup -z contoso.com -n "test-txt" -x "this is a TXT record" 
