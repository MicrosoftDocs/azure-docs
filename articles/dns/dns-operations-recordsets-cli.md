...
You can't remove or modify the pre-populated Azure DNS name servers in the automatically created NS record set at the zone apex (`--Name "@"`, including quote marks). For this record set, you can add more name servers to support cohosting a domain with more than one DNS provider, and you can modify the record set TTL and metadata. For more information, see [To modify NS records at the zone apex](#to-modify-ns-records-at-the-zone-apex).
