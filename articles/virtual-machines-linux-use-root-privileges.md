...
2. **SSH key only** - the virtual machine was provisioning with a certificate (`.CER` file), but no password.  In this case `sudo` **will not** prompt for the user's password before executing the command.
