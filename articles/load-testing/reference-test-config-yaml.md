...
| `failureCriteria` | object |  | Criteria that indicate failure of the test. Each criterion is in the form of:<BR>`[Request_name]:[Aggregate_function] ([client_metric]) > [value]`<BR><BR>- `[Request_name]` is optional. If you don't provide a request, the criteria applies on the aggregate values. For more information on the supported values, see [Load test pass/fail criteria](./how-to-define-test-criteria.md#load-test-passfail-criteria). |
