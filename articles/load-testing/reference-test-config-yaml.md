...
| `failureCriteria` | object |  | Criteria that indicate failure of the test. Each criterion is in the form of:<BR>`[Request_name]:[Aggregate_function] ([client_metric]) > [value]`<BR><BR>- `[Request_name]` is optional. If not provided, the criteria is applied on aggregate values. For more information on the supported values see [Load test pass/fail criteria](./how-to-define-test-criteria#load-test-passfail-criteria). |
