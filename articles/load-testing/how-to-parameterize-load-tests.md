...
In this article, you'll learn how to parameterize your load tests with Azure Load Testing.  
...
- Environment Variables: Non-sensitive information that may change based on the test run. These are exposed as environment variables for access by the load test engine at runtime. Configuration variables could be for different environments such as dev, test, or prod, for example.  
...
Add a user defined variable in your Apache JMeter script. Add the value of the variable using the custom function `{{__GetSecret(SecretName)}` as shown below.
...
### Granting your Azure Load Testing resource access to Key Vault  
...
1. Create a system assigned managed identity for your Azure Load Testing resource from Azure portal by turning on in the identity tab of the resource as shown below.
    ...
    :::image type="content" source="media/how-to-parameterize-load-tests/system-assigned-managed-identity.png" alt-text="Turn on system assigned managed identity from the identity tab of the resource in Azure portal":::  
...
1. For secret value, enter the Secret Uri only. The Secret Uri should be the full data-plane URI of a secret in Key Vault. You can also include a version. For example, `https://myvault.vault.azure.net/secrets/mysecret/` or `https://myvault.vault.azure.net/secrets/mysecret/ec96f02080254f109c51a1f14cdb1931`.  
...
1. If you're using the load test YAML configuration file to create and run tests, use the below syntax to provide secret values. The name of the secret should be the same as it is in your Apache JMeter script.
...
1. Use the below syntax to pass secret values to the Azure Pipelines task or GitHub action. The name of the secret should be the same as it is in your Apache JMeter script. The value is the name of the secret variable in your secret store.  
...
Add a user defined variable in your Apache JMeter script. Add the value of the variable using the custom function `${__BeanShell( System.getenv("variable") )}` as shown below.
...
Use the below syntax to pass secret values to the Azure Pipelines task or GitHub Action. The name of the variable should be the same as it is in your Apache JMeter script.
