...
| Plugins | Azure Load Testing lets you use plugins from https://jmeter-plugins.org, or upload a Java archive (JAR) file with your own plugin code.<br/> With a [Web Driver sampler](https://jmeter-plugins.org/wiki/WebDriverSampler/), you can run tests with a load of upto 4 virtual users, due to the resource intensive nature of WebDriver tests. Tests with higher load can result in errors. <br/> Any plugins that use backend listeners aren't supported. | [Customize a load test with plugins](./how-to-use-jmeter-plugins.md) |
