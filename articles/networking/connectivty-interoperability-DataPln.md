...
In this article let's go through the data plane analysis of the test setup. If you want to review the Test Setup, see the [Test Setup][Setup]. If you want to review the Test Setup configuration detail, see [Test Setup Configuration][Configuration]. To review control plane analysis of the test setup, see [Control Plane Analysis][Control-Analysis].

Data plane analysis examines the path taken by packets traversing from one local network (LAN/VNet) to another within a topology. The data path between two local network may not be necessarily symmetrical. Therefore, in this article let's analyze forwarding path from a local network to another separately from the reverse path.

##Data Path from Hub VNet

###Path to Spoke VNet

VNet peering emulates Network Bridge functionality between the two VNets that are peered. Tracert output from a Hub VNet to a VM in the spoke VNet is listed below:

	C:\Users\rambala>tracert 10.11.30.4
...
###Path to Branch VNet
	...
	C:\Users\rambala>tracert 10.11.30.68
...
In the above tracert, the first hop is the VPN GW of the Hub VNet. The second hop is the VPN GW of the Branch VNet, whose IP address is not advertised within the Hub VNet, and the third hop is the VM on the Branch VNet.
...
###Path to On-Premises Location-1
	...
	C:\Users\rambala>tracert 10.2.30.10
...
In the above tracert, the first hop is the ExpressRoute GW tunnel endpoint to MSEE. The second and the third hop respectively are CE router and on-premises Location 1 LAN IPs, these IP addresses are not advertised within the Hub VNet, and the fourth hop is the VM on the on-premises Location-1.


###Path to On-Premises Location-2

	C:\Users\rambala>tracert 10.1.31.10
...
In the above tracert, the first hop is the ExpressRoute GW tunnel endpoint to MSEE. . The second and the third hop respectively are CE router and on-premises Location 2 LAN IPs, these IP addresses are not advertised within the Hub VNet, and the fourth hop is the VM on the on-premises Location-2.

###Path to Remote VNet

	C:\Users\rambala>tracert 10.17.30.4
...
In the above tracert, the first hop is the ExpressRoute GW tunnel endpoint to MSEE. The second hop is the Remote VNet’s GW IP. The second hop IP range is not advertised within the Hub VNet. The third hop is the VM on the Remote VNet.
...
###Path to Hub VNet
	...
	C:\Users\rambala>tracert 10.10.30.4
...
In the above tracert, the first hop is the VPN GW of the Hub VNet. The second hop is the VPN GW of the Branch VNet, whose IP address is not advertised within the Hub/Spoke VNet, and the third hop is the VM on the Branch VNet.
...
In the above tracert, the first hop is the Hub VNet’s ExpressRoute GW tunnel endpoint to MSEE. . The second and the third hop respectively are CE router and on-premises Location 1 LAN IPs, these IP addresses are not advertised within the Hub/Spoke VNet, and the fourth hop is the VM on the on-premises Location-1.
...
In the above tracert, the first hop is the Hub VNet’s ExpressRoute GW tunnel endpoint to MSEE. . The second and the third hop respectively are CE router and on-premises Location 2 LAN IPs, these IP addresses are not advertised within the Hub/Spoke VNets, and the fourth hop is the VM on the on-premises Location-2.
...
In the above tracert, the first hop is the Hub VNet’s ExpressRoute GW tunnel endpoint to MSEE. The second hop is the Remote VNet’s GW IP. The second hop IP range is not advertised within the Hub/Spoke VNet. The third hop is the VM on the VM on the Remote VNet.
...
In the above tracert, the first hop is the VPN GW of the Branch VNet. The second hop is the VPN GW of the Hub VNet, whose IP address is not advertised within the Remote VNet, and the third hop is the VM on the Hub VNet.
...
In the above tracert, the first hop is the VPN GW of the Branch VNet. The second hop is the VPN GW of the Hub VNet, whose IP address is not advertised within the Remote VNet, and the third hop is the VM on the Spoke VNet.
...
As we discussed prior in the control plane analysis, the branch VNet has no visibility either to on-premises location-2 or to remote VNet per the network configuration. The following ping results confirm the fact. 
	...
	C:\Users\rambala>ping 10.1.31.10
	...
	C:\Users\rambala>ping 10.17.30.4
...
In the above tracert, the first two hops are part of the On-Premises network. The third hop is the primary MSEE interface facing the CE router. The fourth hop is ExpressRoute G/W of the hub VNet, whose IP range is not advertised to the On-Premises network. The fifth hop is the destination VM.

The Azure built-in Network Watcher provides only Azure centric view. Therefore, for on-premises centric view we have used Azure Network Performance Monitor (NPM). NPM provides agents that can be installed servers in network outside Azure and do data-path analysis.
...
Let us bring up the primary ExpressRoute-1 connectivity  for the remainder of the datapath analysis in the document.
