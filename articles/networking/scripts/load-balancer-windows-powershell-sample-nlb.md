...
| [New-AzureRmLoadBalancer](/powershell/module/azurerm.network/new-azurermloadbalancer)  | Creates an Azure Network Load Balancer (LB). |
| [New-AzureRmLoadBalancerProbeConfig](/powershell/module/azurerm.network/new-azurermloadbalancerprobeconfig) | Creates an LB probe. An LB probe is used to monitor each VM in the LB set. If any VM becomes inaccessible, traffic is not routed to the VM. |
| [New-AzureRmLoadBalancerRuleConfig](/powershell/module/azurerm.network/new-azurermloadbalancerruleconfig) | Creates an LB rule. In this sample, a rule is created for port 80. As HTTP traffic arrives at the LB, it is routed to port 80 one of the VMs in the LB set. |
| [New-AzureRmLoadBalancerInboundNatRuleConfig](/powershell/module/azurerm.network/new-azurermloadbalancerinboundnatruleconfig) | Creates an LB Network Address Translation (NAT) rule.  NAT rules map a port of the LB to a port on a VM. In this sample, a NAT rule is created for SSH traffic to each VM in the LB set.  |
