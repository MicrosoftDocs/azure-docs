...
## VNET
When you design a network from bottom up, you take into account some basic information such as number of IP endpoints (hosts, servers, network devices), number of subnets, routing between subnets, isolation domains such VLANs or security zones etc. This information helps in sizing the network gear (switches, routers, firewalls) as well creating the architecture to support applications and services.

Similarly, when you plan to deploy your applications and services in Azure, you will start by creating a logical boundary in Azure which is called a virtual network (VNET). This virtual network is akin to a physical network boundary. As it is a virtual network, you do not need physical gear but you still have to plan for the logical entities such as IP addresses, IP subnets, routing and policies.

When you create a VNET in Azure, it is pre-configured with an IP range (10.0.0.0/16). This range is not fixed, you can define your own IP range. You can define both IPv4 and IPv6 address ranges. IP ranges defined for the VNET will not be advertised to Internet. You will create multiple subnets from your IP range. These subnets will be used to assign IP addresses to virtual network interfaces (vNICs). First four IP addresses from each subnet are reserved and cannot be used for IP allocation. There is no concept of VLANs in a public cloud. However, you can create isolation within a VNET based on your defined subnets.

You can create one large subnet encompassing all the VNET address space or choose to create multiple subnets. However, if you are using a VNET gateway, Azure requires you to create a subnet with the name "gateway subnet". Azure will use this subnet to assign IP addresses to virtual network gateways. 
...
1. Public IP addresses - Used to communicate inbound and outbound (without network address translation (NAT)) with the Internet and other Azure resources not connected to a VNet. Assigning a public IP address to a NIC is optional. Public IP addresses belong to Microsoft IP address space.
1. Private IP addresses - Used for communication within a VNet, your on-premises network, and the Internet (with NAT). IP address space that you define in the VNET is considered private even if you configure your public IP address space. Microsoft does not advertise this space to Internet. You must assign at least one private IP address to a VM. 

Just as with physical hosts or devices, there are two ways to allocate an IP address to a resource - dynamic or static. In Azure, default allocation method is dynamic, where an IP address is allocated when you create a virtual machine (VM) or start a stopped VM. The IP address is released when you stop or delete the VM. To ensure the IP address for the VM remains the same, you can set the allocation method explicitly to static. In this case, an IP address is assigned immediately. It is released only when you delete the VM or change its allocation method to dynamic. 

Private IP addresses are allocated from the subnets you have defined within a VNET. For a VM you choose a subnet for the IP allocation. If a VM contains multiple NICs, you can choose a different subnet for each NIC.

## Routing
When you create a VNET, Azure creates a routing table for your network. This routing table contains following types of routes.
1. System routes
1. Subnet default routes
1. Routes from other VNETs
1. BGP routes
1. Service endpoint routes
1. User Defined Routes (UDR)

When you create a VNET for the first time without defining any subnets, Azure creates routing entries in the routing table. These are called system routes. System routes are defined at this location. You cannot modify these routes. However, you can override systems routes by configuring UDRs.

When you create one or multiple subnets inside a VNET, Azure creates default entries in the routing table to enable communication between these subnets within a VNET. These routes can be modified by using static routes which are User Defined Routes (UDR) in Azure.
...
You can filter network traffic to and from resources in a virtual network using network security groups and network virtual appliances (NVA) such as Azure Firewall or firewall from other vendors. You can control how Azure routes traffic from subnets. You can also limit who in your organization can work with resources in virtual networks.
...
[vnet-routing]: https://docs.microsoft.com/en-us/azure/virtual-network/virtual-networks-udr-overview
