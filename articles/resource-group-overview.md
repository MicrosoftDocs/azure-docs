...
> [AZURE.NOTE] This topic describes resources, groups, and templates using the portal to demonstrate the concepts. However, you can also create, manage, and delete Azure resources using the [Azure CLI for Mac, Linux, and Windows](xplat-cli-azure-resource-manager.md) as well as [PowerShell](powershell-azure-resource-manager.md).
...
In the Azure portal, all new resources are created in a resource group. Even if you create just a single resource such as a web site, you must decide whether to add that resource to an existing group or create a new group for that resource.
...
From the portal, you can easily view costs, monitor events, and manage alerts. The following image shows the consolidated billing for a group.
...
In the portal, you can start working with tags by clicking on the tag icon.
...
In the portal, you can define access control by clicking on the access button.
