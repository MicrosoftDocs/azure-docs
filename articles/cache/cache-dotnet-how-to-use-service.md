...
Each tier differs in terms of features and pricing. The features are covered later in this guide, and for more information on pricing, see [Cache Pricing Details][Cache Pricing Details].

This guide provides an overview of getting started with Managed Cache Service. For more detailed information on these features that are beyond the scope of this getting started guide, see [Overview of Azure Managed Cache Service][Overview of Azure Managed Cache Service].
...
> Once a Managed Cache Service instance is created using the PowerShell cmdlets it can be viewed and configured in the [Azure Classic Portal][Azure Classic Portal].
> 
> 

To create a Managed Cache Service instance, open an Azure PowerShell command window.

> For instructions on installing and using Azure PowerShell, see [How to install and configure Azure PowerShell][How to install and configure Azure PowerShell].
> 
> 

Invoke the [Add-AzureAccount][Add-AzureAccount] cmdlet, and enter the email address and password associated with your account. A subscription is chosen by default and is displayed after you invoke the [Add-AzureAccount][Add-AzureAccount] cmdlet. To change the subscription, invoke the [Select-AzureSubscription][Select-AzureSubscription] cmdlet.

> If you have configured Azure PowerShell with a certificate for your account then you can skip this step. For more information about connecting Azure PowerShell with your Azure account, see [How to install and configure Azure PowerShell][How to install and configure Azure PowerShell].
> 
> 

A subscription is chosen by default and is displayed. To change the subscription, invoke the [Select-AzureSubscription][Select-AzureSubscription] cmdlet.

Invoke the [New-AzureManagedCache][New-AzureManagedCache] cmdlet and specify the name, region, cache offering, and size for the cache.
...
Choose the **Sku** and **Memory** that meets the needs of your application. Note that some cache features, such as notifications and high availability, are only available with certain cache offerings. For more information on choosing the cache offering and size that's best for your application, see [Cache offerings][Cache offerings].
...
> For a complete list of parameters and values that can be used when creating a cache, see the [New-AzureManagedCache][New-AzureManagedCache] cmdlet documentation.
> 
> 

Once the PowerShell cmdlet is invoked, it can take a few minutes for the cache to be created. After the cache has been created, your new cache has a `Running` status and is ready for use with default settings, and can be viewed and configured in the [Azure Classic Portal][Azure Classic Portal]. To customize the configuration of your cache, see the following [Configure the cache][Configure the cache] section.

You can monitor the creation progress in the Azure PowerShell window. Once the cache is ready for use, the [New-AzureManagedCache][New-AzureManagedCache] cmdlet will display the cache information, as shown in the following example.
...
For more information about eviction and expiration policies, see [Expiration and Eviction][Expiration and Eviction].

## Notifications
Cache notifications that allow your applications to receive asynchronous notifications when a variety of cache operations occur on the cache cluster. Cache notifications also provide automatic invalidation of locally cached objects. For more information, see [Notifications][Notifications].

> Notifications are only available in the Standard and Premium cache offerings, and are not available in the Basic cache offering. For more information, see [Cache offerings][Cache offerings].
> 
> 
...
By definition, the use of high availability multiplies the amount of required memory for each cached item by two. Consider this memory impact during capacity planning tasks. For more information, see [High Availability][High Availability].

> High availability is only available in the Premium cache offering, and is not available in the Basic or Standard cache offerings. For more information, see [Cache offerings][Cache offerings].
> 
> 
...
> The client diagnostic level configures the level of caching diagnostic information collected for cache clients. For more information, see [Troubleshooting and Diagnostics][Troubleshooting and Diagnostics]
> 
> 
...
> profile for the project is .NET Framework 4 or higher, and be sure to select one of the profiles that does not specify **Client Profile**. For instructions on configuring cache clients, see [Configure the cache clients][Configure the cache clients].
> 
> 
...
state provider, first configure your cache, and then configure your ASP.NET application for Cache using the Cache NuGet package as described in [Getting Started with Managed Cache Service][Getting Started with Managed Cache Service]. When the Cache NuGet package is installed, it adds a commented out section in web.config that contains the required configuration for your ASP.NET application to use the Session State Provider for Azure Cache.
...
> If your web.config does not contain this commented out section after installing the Cache NuGet package, ensure that the latest NuGet Package Manager is installed from [NuGet Package Manager Installation][NuGet Package Manager Installation], and then uninstall and reinstall the package.
> 
> 
...
provider, see [Session State Provider for Azure Cache][Session State Provider for Azure Cache].
...
For more information about using the Output Cache Provider for Azure Cache, see [Output Cache Provider for Azure Cache][Output Cache Provider for Azure Cache].
...
* See the MSDN Reference: [Managed Cache Service][Managed Cache Service]
* Learn how to migrate to Managed Cache Service: [Migrate to Managed Cache Service][Migrate to Managed Cache Service]
* Check out the samples: [Managed Cache Service Samples][Managed Cache Service Samples]
