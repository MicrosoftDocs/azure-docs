...
- The ingestion private endpoints can be created via the Microsoft Purview governance portal experience described in the steps [here]([url](https://docs.microsoft.com/en-us/azure/purview/catalog-private-link-end-to-end#option-2---enable-account-portal-and-ingestion-private-endpoint-on-existing-microsoft-purview-accounts)). They can't be created from the Private Link Center.
...
### Resolution 
Validate if name resolution is successful between the VM running Self-Hosted Integration Runtime and Microsoft Purview managed resources such as blob queue and Event Hubs through port 443 and private IP addresses (step 8 above).
