...
* **Data Share Contributor** roles to a Purview collection. If you created the Microsoft Purview account, you are automatically assigned this role to the root collection. Refer to [Microsoft Purview permissions](catalog-permissions.md) to learn more about the Microsoft Purview collection and roles.
...
* A source storage account created after the registration step is completed. Source storage account can be in a different Azure region from your Microsoft Purview account.
...
![Screenshot delete share.](./media/how-to-share-data/delete-sent-share.png "Delete sent share.") 
...
If you cannot select a collection when creating a share or register a data source, you do not have proper permission to the collection. You need to have **Data Share Contributor** role to a Microsoft Purview collection in order to register data source and add asset to a share. 
...
* The storage account is not supported. Microsoft Purview Data sharing only [supports storage accounts with specific configurations](#azure-storage-account-prerequisites).
