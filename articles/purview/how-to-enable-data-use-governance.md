...
*Data use governance* (DUG) is an option in the data source registration in Azure Purview. This option allows you to enable Azure Purview to manage data access for your resources.

Currently, DUG allows Azure purview to grant access to data sources through these two paths:

* [Self-service access policies](concept-self-service-data-access-policy.md) - access policies generated by Azure Purview when a [self-service access request](how-to-request-access.md) is approved.
...
For a user in Azure Purview to be able to create a data policy on any resource, DUG must be enabled on that resource. This article will explain how to enable data access on your resources in Azure Purview.
