...
*Data use governance* (DUG) is an option within the data source registration in Microsoft Purview. This option lets Microsoft Purview manage data access for your resources. The high level concept is that the data owner allows its data resource to be available for access policies by enabling *DUG*. 

Currently, a data owner can enable DUG on a data resource for these types of access policies:
