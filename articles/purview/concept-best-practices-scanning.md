...
- The hierarchy that aligns with the organization's strategy like geographical, business function, or source of data, defining the data sources to be registered and scanned needs to be created by using Collections.
...
- Use the **Azure Multiple** option if you need to register multiple sources, such as Azure subscriptions or resource groups, in the cloud. See the following documentation for more information:
...
- Before you create any credentials, consider your data source types and networking requirements to decide which authentication method and integration runtime is needed for your scenario.

### Design recommendations

To avoid unexpected cost and rework, plan and follow this order when you set up the scan, after you register your source in the relevant [collection](./how-to-create-and-manage-collections.md):
...
1. Identify your classification requirements from the system in-built classification rules or create specific custom classification rules as necessary. Base them on the specific industry, business, or regional requirements, which are not available out of the box:
...
1. Create scan rule sets before you configure the scan.
   ...
   When you create the scan rule set, ensure the following:
    - Verify if the system default scan rule set is sufficient for the data source being scanned or else define your custom scan rule set.
    ...
    - Where necessary, create a custom rule set to exclude unwanted classification labels. For example, the system rule set contains generic government code patterns for the planet, not just the United States. Be aware that your data might match the pattern of some other type, such as "Belgium Driver's License Number."
    ...
    > [!NOTE]
    > When you scan a storage account, Azure Purview uses a set of defined patterns to determine if a group of assets forms a resource set. Resource set pattern rules allow you to customize or override how Azure Purview detects which assets are grouped as resource sets and how they're displayed within the catalog.
    > See [Create resource set pattern rules](./how-to-resource-set-pattern-rules.md) for more information.
...
1. Set up a scan for the registered data sources.
    ...
    - **Authentication**: Azure Purview offers various authentication methods for scanning data sources, depending on the type of source. It could be Azure cloud or on-premises or third-party sources. Follow the least-privilege principle for the authentication method in this order of preference:
        ...
        > [!Note]
        > If you have firewall enabled for the storage account, you must use the managed identity authentication method when you set up a scan.
    ...
    - **Integration runtime**
        ...
        - If self-hosted integration runtime (SHIR) is deleted, any ongoing scans relying on it will fail.
    ...
    - **Scope scan**
        ...
        - In general, use "ignore patterns," where they're supported, based on wild card (for example, for data lakes) to exclude temp, config files, RDBMS system tables, or backup or STG tables.
    ...
    - **Scan rule set**
        ...
        - There's an option to create custom filetypes, and you can fill in the details accordingly. Currently, Azure Purview supports only one character in Custom Delimiter. If you use custom delimiters, such as ~, in your actual data, you need to create a new scan rule set.
    ...
    - **Scan type and schedule**
        ...
        - The frequency of scans should align with the change management schedule of the data source or business requirements. For example:
            - If the source structure could potentially change (new assets or fields within an asset are added, modified, or deleted) weekly, scan frequency should be in sync with the same.
            - If the classification or sensitivity labels are expected to be up to date on a weekly basis, perhaps for regulatory reasons, scan frequency should be weekly.
            For example, if partitions files are being added every week in a source data lake, you might schedule monthly scans instead of weekly scans because there's no change in metadata. This suggestion assumes there are no new classification scenarios.
            ...
            - The maximum duration that the scan can run is seven days (possibly due to memory issues), this excludes the ingestion process. If progress hasn't been updated after seven days, the scan is marked as failed. The ingestion (into catalog) process currently doesn't have any such limitation.
    ...
    - **Canceling scans**
        ...
        - Canceling an individual child scan is not supported.
