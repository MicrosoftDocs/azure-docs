...
This guide lists step-by-step configuration on how to enable and capture Azure Purview Audit Logs via Azure Diagnostics Event Hub. As an Azure Purview administrator, or Azure Purview data source admin, I want to view and monitor timestamped activity history on actions taken by every user on the [Azure Purview Portal](https://ms.web.purview.azure.com). To enable audit logging on Purview there is an easy and clean technique to capture streaming audit events via Azure Diagnostics Event Hub. Let's see the step by step guide on how to configure this Diagnostics Audit logging on Azure Purview.
