...
3. The integration runtimes are specific to a Purview account. Hence, if scans must run in master and secondary Purview accounts parallelly, multiple self-hosted integration runtimes must be maintained. This limitation will also be addressed when automated BCDR is supported. 
