...
in a subscription or management group of subscriptions. This control can be exercised in many different ways, such as
...
- A scope with an assignment is deployed to via Resource Manager, REST, Azure CLI, or Azure PowerShell. In this scenario, the effect event (append, audit, deny, deploy) and compliant status information will be available in the portal and SDKs within 15 minutes.
- Standard compliance evaluation cycle. Once every 24 hours, assignments will automatically be re-evaluated. A large policy or initiative evaluated against a large scope of resources can take time, so there is no pre-defined expectation of when the evaluation cycle will complete. Once it completes, updated compliance results will be available in the portal and SDKs.
...
Right-click on the row of the event you would like to gather more details on and select **Show activity logs**. The activity log page will open and be pre-filtered to the search showing details for the assignment and the events. The activity log provides additional context and information about those events.
