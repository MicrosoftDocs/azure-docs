...
| Encryption | Data Lake Store also provides encryption for data that is stored in the account. You specify the encryption settings while creating a Data Lake Store account. Encryption is enabled by default for all Data Lake Store accounts and cannot be turned off. For more information on how to provide encryption-related configuration, see [Get started with Azure Data Lake Store using the Azure Portal](data-lake-store-get-started-portal.md).|
