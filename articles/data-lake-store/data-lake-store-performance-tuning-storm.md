...
Note that the throughput in this case is lower, but with a slow rate of events, maximum throughput is not the biggest objective anyway. These mitigations allow you to reduce the total time that it takes for a tuple to flow through to the store. This may matter if you want a real-time pipeline even with a low event rate. Also note that if your incoming tuple rate is low, you should adjust the topology.message.timeout_secs parameter, so the tuples don’t time out while they are getting buffered or processed.
...
* **Total process execution latency.** This is the average time one tuple takes to be emitted by the spout, processed by the bolt, and get acknowledged.
...
If you hit the limits of bandwidth provided by Data Lake Store, you could see task failures. Check task logs for throttling errors. You can decrease the parallelism by increasing container size.    
