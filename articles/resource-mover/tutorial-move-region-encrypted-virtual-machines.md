...
If you're moving VMs that have Azure disk encryption enabled, you need to run a script as mentioned [below](#copy-the-keys-to-the-destination-key-vault) for which you should have necessary permissions to the key vaults in the source and destination region
...
5. In **Select principal**, search for the [Managed System Identity that Resource Mover](./common-questions#how-is-managed-identity-used-in-resource-mover) uses for accessing the Azure resource in your behalf. The MSI name is ```movecollection-<sourceregion>-<target-region>-<metadata-region>```
