...
- **Additional features**: The API offers parameters that aren't shown in the Azure portal. For example, you can add tags to your automation resource and define your export based on a wider set of alert and recommendation properties than the ones offered on the **Continuous Export** page in the Azure portal.
...
1. Select the policy in this table that you want to apply:
...
1. Select each tab and set the parameters to meet your requirements:
    ...
    1. On the **Basics** tab, set the scope for the policy. To use centralized management, assign the policy to the management group that contains the subscriptions that use continuous export configuration.
    ...
    1. On the **Parameters** tab, set the resource group and data type details.
        ...
        >
        > The Azure Policy **Parameters** tab (1) provides access to similar configuration options as the Defender for Cloud **Continuous export** page (2).
        >
...
You *can't* configure data to be exported to a Log Analytics workspace in another tenant if you use Azure Policy to assign the configuration. This process works only when you use the REST API to assign the configuration, and the configuration is unsupported in the Azure portal (due to requiring multitenant context). Azure Lighthouse *doesn't* resolve this issue with Azure Policy, although you can use Azure Lighthouse as the authentication method.

When you collect data into a tenant, you can analyze the data from one, central location.
...
1. If you use Microsoft Entra B2B Guest access, ensure that the user accepts the invitation to access the tenant as a guest.
...
You need to add the relevant role assignment to the destination event hub.
...
1. On the **Create rule** pane, set up your new rule the same way you'd configure a [log alert rule in Azure Monitor](../azure-monitor/alerts/alerts-unified-log.md):
    ...
    - Optionally, configure the [action group](../azure-monitor/alerts/action-groups.md) that you'd like to trigger. Action groups can trigger sending an email, creating an ITSM ticket, running a WebHook, and more.
