...
1. For the plans that you want to connect, turn the toggle to **On**. By default, all necessary prerequisites and components are provisioned. (Optional) Learn how to [configure each plan](#optional-configure-selected-plans).
...
After the connector is created, a scan starts on your GCP environment. New recommendations will appear in Defender for Cloud after up to 6 hours. If you enabled auto-provisioning, Azure Arc and any enabled extensions are installed automatically for each new resource detected.
...
Connect your GCP VM instances to Azure Arc to have full visibility to Microsoft Defender for Servers security content.
...
- Azure Arc for servers installed on your VM instances.
  - **Auto-provisioning** (recommended): Autoprovisioning is enabled by default in the onboarding process and requires owner permissions on the subscription. Arc autoprovisioning process is using the OS config agent on GCP end. Learn more about the [OS config agent availability on GCP machines](https://cloud.google.com/compute/docs/images/os-details#vm-manager).
...
- Other extensions should be enabled on the Arc-connected machines.
  ...
  - Log Analytics (LA) agent on Arc machines or Azure Monitor agent (AMA). Ensure the selected workspace has security solution installed.
      ...
      The LA agent and AMA are currently configured at the subscription level, such that all the multicloud accounts and projects (from both AWS and GCP) under the same subscription inherits the subscription settings regarding the LA agent and AMA.
...
1. On the **Auto-provisioning configuration** screen, turn the toggles on or off depending on your need.
...
1. On the **Defender for Containers configuration** screen, toggle the switches **On**.
