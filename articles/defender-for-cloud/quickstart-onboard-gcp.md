...
- Additional extensions should be enabled on the Arc-connected machines.
    ...
    - Log Analytics (LA) agent on Arc machines, or Azure Monitor agent (AMA). Ensure the selected workspace has security solution installed.
        ...
        The LA agent and AMA are currently configured in the subscription level, such that all the multicloud accounts and projects (from both AWS and GCP) under the same subscription will inherit the subscription settings with regard to the LA agent/ AMA.
