...
| **Qualys vulnerability assess** | As an alternative to Microsoft Defender Vulnerability Management, Defender for Cloud integrates with the Qualys scanner to [identify vulnerabilities](deploy-vulnerability-assessment-vm.md). You don't need a Qualys license or account. | | :::image type="icon" source="./media/icons/yes-icon.png"::: |
...
| **Just-in-time VM access** | [Just-in-time VM access](just-in-time-access-overview.md) locks down machine ports to reduce the attack surface. Defender for Cloud must be enabled on a subscription to use this feature. | | :::image type="icon" source="./media/icons/yes-icon.png"::: |
