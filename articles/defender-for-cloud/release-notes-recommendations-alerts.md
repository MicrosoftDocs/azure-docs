...
This page is updated frequently with the latest recommendation and alert information. 
...
- If you want to get the latest on other Defender for Cloud features, review the [feature release notes](release-notes.md).
...
--- | --- | --- | --- 
March 18 | New recommendation |  [EDR solution should be installed on Virtual Machines](recommendations-reference.md#edr-solution-should-be-installed-on-virtual-machines) |
...
End March | Deprecation  | [Endpoint protection should be installed on your machines](recommendations-reference.md#endpoint-protection-should-be-installed-on-machines)<br/> MMA/AMA (preview)
...
January 25 | Deprecated alert | `Anomalous pod deployment (Preview) (K8S_AnomalousPodDeployment)`
January 25 | Deprecated alert | `Excessive role permissions assigned in Kubernetes cluster (Preview) (K8S_ServiceAcountPermissionAnomaly)`
January 25 | Deprecated alert | `Anomalous access to Kubernetes secret (Preview) (K8S_AnomalousSecretAccess)`
January 25 | Updated to Informational (Windows machines) | `Adaptive application control policy violation was audited (VM_AdaptiveApplicationControlWindowsViolationAudited)`
January 25 | Updated to Informational Windows machines) | `Adaptive application control policy violation was audited (VM_AdaptiveApplicationControlLinuxViolationAudited)`
January 25 | Updated to Informational (Containers) | `Attempt to create a new Linux namespace from a container detected (K8S.NODE_NamespaceCreation)`
January 25 | Updated to Informational (Containers) | `Attempt to stop apt-daily-upgrade.timer service detected (K8S.NODE_TimerServiceDisabled)`
January 25 | Updated to Informational (Containers) | `Command within a container running with high privileges (K8S.NODE_PrivilegedExecutionInContainer)`
January 25 | Updated to Informational (Containers) | `Container running in privileged mode (K8S.NODE_PrivilegedContainerArtifacts)`
January 25 | Updated to Informational (Containers) | `Container with a sensitive volume mount detected (K8S_SensitiveMount)`
January 25 | Updated to Informational (Containers) | `Creation of admission webhook configuration detected (K8S_AdmissionController)`
January 25 | Updated to Informational (Containers) | `Detected suspicious file download (K8S.NODE_SuspectDownloadArtifacts)`
January 25 | Updated to Informational (Containers) | `Docker build operation detected on a Kubernetes node (K8S.NODE_ImageBuildOnNode)`
January 25 | Updated to Informational (Containers) | `New container in the kube-system namespace detected (K8S_KubeSystemContainer)`
January 25 | Updated to Informational (Containers) | `New high privileges role detected (K8S_HighPrivilegesRole)`
January 25 | Updated to Informational (Containers) | `Privileged container detected (K8S_PrivilegedContainer)`
January 25 | Updated to Informational (Containers) | `Process seen accessing the SSH authorized keys file in an unusual way (K8S.NODE_SshKeyAccess)`
January 25 | Updated to Informational (Containers) | `Role binding to the cluster-admin role detected (K8S_ClusterAdminBinding)`
January 25 | Updated to Informational (Containers) | `SSH server is running inside a container (K8S.NODE_ContainerSSH)`
January 25 | Updated to Informational (DNS) | `Communication with suspicious algorithmically generated domain (AzureDNS_DomainGenerationAlgorithm)`
January 25 | Updated to Informational (DNS) |`Communication with suspicious algorithmically generated domain (DNS_DomainGenerationAlgorithm)`
January 25 | Updated to Informational (DNS) |`Communication with suspicious random domain name (Preview) (DNS_RandomizedDomain)`
January 25 | Updated to Informational (DNS) |`Communication with suspicious random domain name (AzureDNS_RandomizedDomain)`
January 25 | Updated to Informational (DNS) |`Communication with possible phishing domain (AzureDNS_PhishingDomain)`
January 25 | Updated to Informational (DNS) |`Communication with possible phishing domain (Preview) (DNS_PhishingDomain)`
January 25 | Updated to Informational (Azure App Service) |`NMap scanning detected (AppServices_Nmap)`
January 25 | Updated to Informational (Azure App Service) |`Suspicious User Agent detected (AppServices_UserAgentInjection)`
January 25 | Updated to Informational (Azure networking) |`Possible incoming SMTP brute force attempts detected (Generic_Incoming_BF_OneToOne)`
January 25 | Updated to Informational (Azure networking) |`Traffic detected from IP addresses recommended for blocking (Network_TrafficFromUnrecommendedIP)`
January 25 | Updated to Informational (Azure Resource Manager) |`Privileged custom role created for your subscription in a suspicious way (Preview)(ARM_PrivilegedRoleDefinitionCreation)`
January 4 | New (Microsoft Cloud Security Benchmark) | [Cognitive Services accounts should have local authentication methods disabled](recommendations-reference.md#cognitive-services-accounts-should-have-local-authentication-methods-disabled)
January 4 | New (Microsoft Cloud Security Benchmark) | [Cognitive Services should use private link](recommendations-reference.md#cognitive-services-should-use-private-link)
January 4 | New (Microsoft Cloud Security Benchmark) | [Virtual machines and virtual machine scale sets should have encryption at host enabled](recommendations-reference.md#virtual-machines-and-virtual-machine-scale-sets-should-have-encryption-at-host-enabled)
January 4 | New (Microsoft Cloud Security Benchmark) | [Azure Cosmos DB should disable public network access](recommendations-reference.md#azure-cosmos-db-should-disable-public-network-accesss)
January 4 | New (Microsoft Cloud Security Benchmark) | [Cosmos DB accounts should use private link](recommendations-reference.md#cosmos-db-accounts-should-use-private-link)
January 4 | New (Microsoft Cloud Security Benchmark) | [VPN gateways should use only Azure Active Directory (Azure AD) authentication for point-to-site users](recommendations-reference.md#vpn-gateways-should-use-only-azure-active-directory-(azure-ad)-authentication-for-point-to-site-users)
...
We are announcing new  that discover and assesses the configuration of supported endpoint detection and response solutions. If issues are found, these recommendations offer remediation steps.

The following new agentless endpoint protection recommendations are now available if you have Defender for Servers Plan 2 or the Defender CSPM plan enabled on your subscription with the agentless machine scanning feature enabled. The recommendations support Azure and multicloud machines. On-pr
emises machines are not supported. 
