...
- **Simultaneous onboarding limited support**: For servers simultaneously onboarded using multiple methods (for example - Direct onboarding combined with Log Analytics workspace-based onboarding), Defender for Cloud makes best effort to correlate them into single device representation. However, certain limitations apply to devices using older versions of Defender for Endpoint. In certain cases this might result in overcharges on certain devices. We always advise using the latest agent versions, and specifically to avoid this potential limitation, use these minimum Defender for Endpoint agent versions:
- |Operating System|Minimum agent version|
| -------- | -------- |
|Windows 2019| 10.8555|
|Windows 2012, 2016 (modern, unified agent)|10.8560|
