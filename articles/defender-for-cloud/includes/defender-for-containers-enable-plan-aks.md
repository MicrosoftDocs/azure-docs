...
Once the Defender extension has been deployed, a default workspace will be automatically assigned. You can [assign a custom workspace](../defender-for-containers-enable.md?pivots=defender-for-container-aks&tabs=aks-deploy-portal%2ck8s-deploy-asc%2ck8s-verify-asc%2ck8s-remove-arc%2caks-removeprofile-api#assign-a-custom-workspace) in place of the default workspace through Azure Policy.

> [!NOTE]
> The Defender extension is deployed to each node to provide the runtime protections and collect signals from those nodes using [eBPF technology](https://ebpf.io/).
...
- **Visibility** about which of your clusters has the Defender extension deployed
...
1. Select the clusters to see the details of the healthy and unhealthy resources - clusters with and without the extension.
...
### Use the REST API to deploy the Defender extension
...
1. Enable the Defender extension on your containers:

    - Run the following command to create a new cluster with the Defender extension enabled:

        ```azurecli
        az aks create --enable-defender --resource-group <your-resource-group> --name <your-cluster-name>
        ```

    - Run the following command to enable the Defender extension on an existing cluster:
...
1. To verify that the extension was successfully added, run the following command on your machine with the `kubeconfig` file pointed to your cluster:

    ```console
    kubectl get pods -n kube-system
    ```

    When the extension is added, you should see a pod called `microsoft-defender-XXXXX` in `Running` state. It might take a few minutes for pods to be added.
