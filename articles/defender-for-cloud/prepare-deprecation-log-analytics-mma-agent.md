...
Deploying Azure Monitor Agent (AMA) with Defender for Cloud portal is available for SQL servers on machines, with the [new deployment policy](#migrate-to-the-sql-server-targeted-ama-autoprovisioning-process).

If you enabled the AMA-related public preview policy initiative, support will continue until August 2024.  

To disable AMA provisioning, edit the configuration of the “Log Analytics/Azure Monitor Agent” extension through “Settings and Monitoring” of Defender for Servers plan in the Defender for Cloud portal. Alternately, you can manually remove the policy initiative.
