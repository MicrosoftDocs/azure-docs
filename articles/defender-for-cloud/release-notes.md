...
FIM is now available in a new version based on Azure Monitor Agent (AMA). Last month, we announced the [preview of Defender for Cloud's integration with AMA](#azure-monitor-agent-integration-now-in-preview) and [how to deploy it through Defender for Cloud](auto-deploy-azure-monitoring-agent.md).
