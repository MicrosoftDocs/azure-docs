...
>[!NOTE]
>If your Resource Manager template requires multiple tracking IDs, add the resource of type [Microsoft.Resources/deployments](/azure/templates/microsoft.resources/deployments) yourself but use the tracking ID visible in Partner Center on your plan's Technical Configuration page (see [Add a GUID to a Resource Manager template](#add-a-guid-to-a-resource-manager-template) below).
