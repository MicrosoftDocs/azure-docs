...
| Usage Quantity | Usage Quantity | The total usage units consumed by the asset which is deployed by the customer.<br>This is based on Usage type item. For example, if the Usage Type is Normalized usage, then Usage Quantity is for Normalized Usage. | UsageQuantity |
...
| MeteredUsage | Metered Usage | The total usage units consumed by the meters that are configured with the offer which is deployed by the customer. | MeteredUsage |
| RawUsage | Raw Usage | The total raw usage units consumed by the asset which is deployed by the customer.<br>Raw usage hours are defined as the amount of time VMs have been running in terms of usage units. | RawUsage |
