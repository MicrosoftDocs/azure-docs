...
This article describes how to configure Table storage to manage sales. Table storage helps you store and modify customer information.
...
1. In the Azure portal, follow these steps to create a storage account:  
    1. Select **+Create a resource** in the pane on the left side. The **New** pane will appear on the right side.
    1. In the **New** pane, select **Storage** \. A **Featured** list will appear on the right side.
    1. To begin account creation, select **Storage Account**.  Then, follow the instructions in [Create a storage account](https://docs.microsoft.com/azure/storage/common/storage-quickstart-create-account?tabs=azure-portal).
    ...
    For more information about storage accounts, see [Quickstart tutorials](https://docs.microsoft.com/azure/storage/). For information about storage pricing, see [storage pricing](https://azure.microsoft.com/pricing/details/storage/).
...
You can use [Microsoft Flow](https://docs.microsoft.com/flow/) to automate notifications to be sent every time a lead is added to Table storage. If you don’t have an account, [sign up for a free Microsoft Flow account](https://flow.microsoft.com/).


### Lead notification example

This example shows how to create a basic flow that automatically sends an email notification when a new lead is added to Table storage. This example sets up a flow to send lead information every hour if Table storage is updated.
...
3. In the **Get entities** window, fill in the following fields:
   ...
   - **Filter query**: Click this field, and the **Get past time** icon is displayed in a pop-up window. Select **Past time** to use this value as timestamp to filter the query. Or, you can paste the following function into the field: CreatedTime `Timestamp gt datetime'@{body('Get_past_time')}'` 
...
12. In the **Send an email** window, enter information in the following fields:
...
The flow keeps running until you use the **Turn flow off** option.

If you’re not getting any lead email notifications, it means that new leads haven’t been added to the Table storage. You'll get an email like the following example if there are any flow failures:
