...
-   [Technical prerequisites for creating a virtual machine image for the Azure Marketplace](../cloud-partner-portal/virtual-machine/cpp-create-technical-assets.md)
...
-   [How to troubleshoot common issues encountered during VHD creation](../cloud-partner-portal/virtual-machine/cpp-common-vhd-creation-issues.md)
...
-   You provide content that meets the criteria for your offer to be listed on Azure Marketplace and in the Azure portal.
...
publishers on the VM sizes. If the **hourly billing** model is used, 
the total price will be the summation of the software cost charged
...
You can set pricing per core for your SKU. For this option, you just need to
...
You can set the pricing for each set of cores individually if you would
...
via the portal to make changes via the excel interface. This option also allows
...
details pre-populated. You will be able to edit these details within the spreadsheet
...
Select up to six recommended virtual machine sizes. These recommendations are displayed to customers in Azure Marketplace and in the Pricing tier blade in the Azure portal when they decide to purchase and deploy your image. *These sizes are only recommendations. The customer is able to select any VM size that accommodates the disks specified in your image.*  The following screen capture shows the recommended VM sizes that a customer will see in the Azure portal.
...
eight disk versions per SKU. Only the highest disk version number for a
particular SKU will show up in Azure Marketplace. Others will be visible
via APIs.

Under **Disk version**, select **+ New version**. This option shows the following fields that you need to fill out.
...
Enter the [shared access signature URI](../cloud-partner-portal/virtual-machine/cpp-get-sas-uri.md) created for the operating system VHD.
...
Enter the [shared access signature URI](../cloud-partner-portal/virtual-machine/cpp-get-sas-uri.md) for your data disk.

#### LUN Number

Assign this LUN a number. This number will be reserved for this data disk in this SKU.
