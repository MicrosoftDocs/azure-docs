---
title: Lead Management For Cloud Marketplace | Azure
...
In this document, we will go through all you need to know about these generated leads, how to connect them to your CRM system, and how to integrate them into sales pipeline.
...
To start getting leads, Lead Management connector on the Cloud Partner Portal are designed so that it can be easily plug in with your CRM information to a list of CRM system available. Now you can easily leverage the leads generated by the marketplace without a significant engineering effort to integrate with an external system.
...
The lead only gets written at the end of the day when you select Azure BLOB storage as your lead destination. Switch to Azure table to receive the lead real-time 
...
Yes, follow the instructions to setup Azure Table + Function on the documentation [here](https://cloudpartner.azure.com/#documentation/lead-management-instructions-azure-table). 



**I have configured Salesforce as my lead destination, but I can’t find the leads.** 

Check if the web to lead form is a mandatory field based on a picklist. If yes, please switch over the field to a non-mandatory text field.  
...
Lead gen data is very low (<1GB for almost all publishers). The cost will depend on number of leads received, if 1,000 leads is received in a month, it costs around 50 cents. 
...
**Could not save the lead to Dynamics CRM using AAD. Exception:: Tenant not found. This may happen if there are no active subscriptions for the tenant.**  
