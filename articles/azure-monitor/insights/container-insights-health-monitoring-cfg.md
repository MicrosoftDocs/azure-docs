...
|System workload pods ready |Unit monitor |This monitor reports status based on percentage of pods in ready state in a given workload. This is a unit monitor, and its state will be 'Fail' if less than 100% of the pods are in 'Ready' state |ConsecutiveSamplesForStateTransition<br> FailIfLessThanPercentage |2<br> 10 ||
|Kube API status |Unit monitor |This monitor reports status of Kube Api service. Monitor is in Critical state in case Kube Api endpoint is unavailable. For this particular monitor, the state is determined by making a query to the 'nodes' endpoint for the kube-api server. Anything other than a OK response code flips the monitor to a 'Fail' state | No configuration properties |||

### Aggregate monitors

|**Monitor name** | Monitor type | **Description** | **Algorithm** |
|-----------------|--------------|-----------------|---------------|
|Node |Aggregate monitor |This monitor is an aggregate of the all the node monitors. It is a worst of monitor, and its state is the worst state of its child monitors<br> Node CPU<br> Node Memory<br> Node Status | Worst of|
|Node pool |Aggregate monitor |This monitor reports combined health status of all nodes in the node pool agentpool. This is a three state monitor, whose state is the worst state of 80% of the nodes in the node pool, sorted in descending order of severity of node states (that is, Fail, Warning, Pass).|Percentage |
|Nodes (parent of Node pool) |The nodes is an aggregate monitor of all the node pools.It is a worst of monitor, and its state is the worst state of its child monitors i.e. the node pools present in the cluster. |Wost of |
...
|Container |This monitor reports overall health status of a container in a given workload. This monitor is an aggregate monitor and its status is calculated as the 'worst of' its child monitor states i.e. Container CPU Utilization and Container Memory Utilization. |Worst of |
