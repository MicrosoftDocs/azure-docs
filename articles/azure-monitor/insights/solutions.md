...
> [!CAUTION]
> Many monitoring solutions are no longer in active development.  We suggest you check each solution to see if it has a replacement and not deploy new instances of solutions that do even if available. Many have been replaced by a [newer curated visualization or insight](/azure/azure-monitor/monitor-reference#insights-and-curated-visualizations).  
