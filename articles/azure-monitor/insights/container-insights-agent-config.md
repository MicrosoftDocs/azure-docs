...
| `[log_collection_settings.enrich_container_logs] enabled =` |Boolean | true or false | This setting controls container log enrichment (to populate Name and Image fields for every log record the ContainerLog table) for all container logs in the cluster and defaults to `enabled = false` when not specified in ConfigMap. |
