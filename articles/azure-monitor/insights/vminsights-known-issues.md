...
- After you disable monitoring for a VM by using the supported methods and you try deploying it again, you should deploy it in the same workspace. If you use a new workspace and try to view the health state for that VM, it might show anomalous behavior.
...
- The Alerts summary included with Azure Monitor for VM displays only alerts that result from health issues detected with monitored Azure VMs.
