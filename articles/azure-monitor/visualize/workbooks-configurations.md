...
There are several ways that you can create interactive reports and experiences in workbooks.
 ...
 - **Grid Cell Clicks**: You to add interactivity with a special type of grid column renderer called a **[link renderer](#link-renderers-actions)**. A link renderer converts a grid cell into a hyperlink based on the contents of the cell. Workbooks support many kinds of link renderers including renderers that open resource overview blades, property bag viewers, App Insights search, usage, transaction tracing, etc. Learn how to [#set up a grid cell click](#set-up-grid-cell-clicks).
