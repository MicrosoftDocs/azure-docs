...
| [Azure Application Insights for JavaScript web apps](app/javascript.md) | Our Java on-premises page has been retired and redirected to [Azure Monitor OpenTelemetry-based auto-instrumentation for Java applications](app/java-in-process-agent).|
...
| [Tools for migrating to Azure Monitor Agent from legacy agents](articles/azure-monitor/agents/azure-monitor-agent-migration-tools) | New article that explains how to install and use tools for migrating from legacy agents to the new Azure Monitor agent (AMA).|
