...
The [Application Insights JavaScript SDK](app/javascript.md) does not include any personal identifiable information (PII) in its autocompletion by default. However, some PII used in your application may be picked up by the SDK (for example, full names in `window.title` or account IDs in XHR URL query parameters). For custom PII masking, add a [telemetry processor](app/api-filtering-sampling.md).
