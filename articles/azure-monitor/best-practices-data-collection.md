...
## Monitor containers
Virtual machines generate similar data as other Azure resources, but they require a containerized version of the Log Analytics agent to collect required data. Container insights helps you prepare your containerized environment for monitoring and works in conjunction with third party tools for providing comprehensive monitoring of AKS and the workflows it supports. See [Monitoring Azure Kubernetes Service (AKS) with Azure Monitor](../aks/monitor-aks?toc=/azure/azure-monitor/toc.json) for a dedicated scenario on monitoring AKS with Azure Monitor.
