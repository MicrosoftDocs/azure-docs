...
| profiles | rules | Rules |Autoscale automatically scales between the maximum and minimum capacities, by using the rules in the profile. Define up to 10 individual rules in a profile. Typically rules are defined in pairs: one to determine when to scale out, and the other to determine when to scale in. e. Typically rules are defined in pairs, one to determine when to scale out, and the other to determine when to scale in. ||
