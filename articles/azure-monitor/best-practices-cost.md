...
| Custom Logs API | Custom tables<br>Azure tables | Use [Custom Logs API](logs/logs-ingestion-api-overview.md) to send data to custom tables in the workspace using REST API. | Configure ingestion-time transformation in the DCR for the custom log. |
