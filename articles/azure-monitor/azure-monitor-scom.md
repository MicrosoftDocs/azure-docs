...
These layers can be simplified into the following categories which are described in the rest of this article. While every workload in your environment may not fit neatly into one of these categories, each should be close enough to a particular category for the general recommendations to apply.
