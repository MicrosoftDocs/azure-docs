...
## Configuration goals
The goal of a complete deployment of Azure Monitor is to collect all available data from all of your cloud resources and applications to enable as many features in Azure Monitor as possible. 
...
See [What is monitored by Azure Monitor?](monitor-reference.md) for a list of available insights and solutions in Azure Monitor. See the documentation for each for any unique configuration or pricing information. See [Install a monitoring solution](insights/solutions.md#install-a-monitoring-solution) for details on adding a solution to your subscription.
...
[Azure Monitor for VMs](insights/vminsights-overview.md) uses the Log Analytics agent and Dependency agent to collect data from the guest operating system of virtual machines, so you can deploy these agents as part of the implementation of this insight. This enables the agent for other services that use it such as Azure Security Center.
...
Azure Monitor for VMs requires a Log Analytics workspace which will typically be the same workspace as the one created to collect data from other Azure resources. Before you onboard any virtual machines, you must add the solution required for Azure Monitor for VMs.
...
Azure Monitor for VMs will send performance data to a Log Analytics workspace but not to Azure Monitor Metrics. Sending this data into Metrics allow it to be analyzed with Metrics Explorer and used with metric alerts. This requires the diagnostic extension on Windows and the Telegraf agent on Linux.
...
## Visualize data
Insights and solutions will include their own workbooks and views for analyzing their data. You can create your own [visualizations using different methods](visualizations.md).


### Create workbooks
[Workbooks](platform/workbooks-overview.md) in Azure Monitor allow you to create rich visual reports in the Azure portal. You can combine different sets of metric and log data collected in Azure Monitor to create unified interactive experiences. Insights include pre-built workbooks, and you can access a gallery of workbooks in the **Workbooks** tab of the Azure Monitor menu. 
