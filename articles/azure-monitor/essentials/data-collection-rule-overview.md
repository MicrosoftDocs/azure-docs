...
To view your DCRs in the Azure portal, select **Data Collection Rules** from the **Monitor** menu.
...
| Method | Resources |
|:---|:---|
| API        | Directly edit the DCR in any JSON editor and then [install by using the REST API](/rest/api/monitor/datacollectionrules). |
| CLI        | Create DCR and associations with the [Azure CLI](https://github.com/Azure/azure-cli-extensions/blob/master/src/monitor-control-service/README.md). |
...
## Supported regions
Data collection rules are available in all public regions where Log Analytics workspace are supported, and the Azure Government and China clouds. Air-gapped clouds aren't yet supported.
