...
To apply a transformation to data that doesn't use a data collection rule, create the transformation in the [workspace transformation DCR](../data-collection.md#workspace-transformation-dcr). This DCR can contain a transformation for one more [supported tables](../logs/tables-feature-support.md). Any data sent to these tables not using another DCR will have the transformation applied. 
