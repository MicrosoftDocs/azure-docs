...
1. **Are your email server and mailbox accepting external emails?**
    ...
    It is common that internal mailing lists or distribution lists block emails from external email addresses. You will need to whitelist the above email addresses.  
...
1. **Have you accidentally unsubscribed from the action group?**
    ...
    The alert emails provide a link to unsubscribe from the action group. To check if you have accidentaly unsubscribed from this action group, either:
...
1. **Has this email address been receiving a lot of other emails from Azure?**

   Email is [rate limited](alerts-rate-limiting.md) to no more than 100 emails every hour to each email address. If you pass this threshold, additional email notifications are be dropped.  Check if you have received a message indicating that your email address has been temporarily rate limited: 
...
1. **Did a webhook not trigger?**
    ...
    1. **Are you calling Slack or Microsoft Teams?**  Each of these endpoints expect a specific JSON format.Please follow [these instructions](action-groups-logic-app.md) to configure a logic app action instead. 
...
[Activity log alerts](activity-log-alerts.md) are alerts that are based on events written to the Azure Activity Log, such as events about creating, updating or deleting Azure resources, service health and resource health events, or findings from Azure Advisor and Azure Policy. If you have received an alert that is based on the activity log, but some fields that you need are missing (or are incorrect), check first the events in the activity log itself. If the Azure resource did not write the fields you are looking for in its activity log event, those fields will not be included in the corresponding alert. 
...
1. **Did the action rule apply on your alert?**

    Check if the action rule have affected your alert, by clicking on the fired alert in the portal, and look at the history tab. 
...
If you have got an error while trying to create, update or delete an [action rule](action-rules.md), follow these steps: 
