...
-	**Guest OS metrics sent to Azure Monitor Metrics**. These are performance counters collected by the Windows Diagnostic Extension (WAD) and sent to the [Azure Monitor Sink](diagnostics-extension-overview.md#data-storage), or via the [InfluxData Telegraf Agent](https://www.influxdata.com/time-series-platform/telegraf/) on Linux machines. Retention for these metrics is 93 days.
