...
-	**Guest OS metrics sent to Azure Monitor Metrics**. These are performance counters collected by the [Windows Diagnostic Extension (WAD)](diagnostics-extension-overview.md) and sent to the [Azure Monitor data sink](diagnostics-extension-overview.md#data-detsinations), or via the [InfluxData Telegraf Agent](https://www.influxdata.com/time-series-platform/telegraf/) on Linux machines. Retention for these metrics is 93 days.
