...
Perform the following series of steps to configure your Operations Manager management group to connect to one of your Log Analytics workspaces.
   ...
   >[!NOTE]
   >If you notice that Log Analytics stop coming in from a particular Agent or Management Server, you can try resetting the Winsock Catalog `netsh winsock reset` and rebooting the server. Resetting the Winsock Catalog will allow network connections to be re-established that were once broken.
