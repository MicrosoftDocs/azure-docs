...
Splitting by Azure resource ID column is recommended when you want to monitor the same condition on multiple Azure resources. For example, monitoring all virtual machines for CPU utilization over 80%. You may also decide to not split when you want a condition on multiple resources in the scope. For instance, monitoring that at least five machines in the resource group scope have CPU utilization over 80%.
