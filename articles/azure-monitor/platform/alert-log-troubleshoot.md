...
Each log alert rule created in Azure Monitor as part of its configuration must specify an analytics query to be executed periodically by the alert service. While the analytics query may have correct syntax at the time of rule creation or update. Some time the query develops over a period of time, some syntax issues. Which prevent is from being successfully executed by Log Alert service. Some common reasons why analytics query provided in a log alert rule can develop errors are:
