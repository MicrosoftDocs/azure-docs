...
> [!NOTE]
> Using the webhook action requires that the target webhook endpoint either does not require details of the alert to function successfully or is capable of parsing the alert context information that is provided as part of the POST operation.  If the webhook endpoint cannot handle the alert context information on its own, then using a solution such as a [Logic App action](./action-groups-logic-app) would allow for a custom manipulation of the alert context information to match the webhook expected data format.
