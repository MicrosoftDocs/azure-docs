...
| Data source<br>configuration | Each agent | Centrally on workspace | None |
...
| Data source<br>configuration | Each agent | Each agent | Centrally on workspace | None |



## Azure diagnostics extension
The [Azure Diagnostics extension](diagnostics-extension-overview.md) collects monitoring data from the guest operating system and workloads of Azure virtual machines and other resources with a guest operating system. It primarily collects data into Azure Storage but allows you to define data sinks to also send data to other destinations such as Azure Monitor Metrics and Azure Event Hubs.
...
- Send data to [Azure Monitor Metrics](data-platform-metrics.md) to analyze it with [metrics explorer](metrics-getting-started.md) take advantage of features such as near real-time [metric alerts](../../azure-monitor/platform/alerts-metric-overview.md) and [autoscale](autoscale-overview.md) (Windows only).
- Send data to third-party tools using [Azure Event Hubs](diagnostics-extension-stream-event-hubs.md) to investigate VM boot issues.
- Collect [Boot Diagnostics](../../virtual-machines/troubleshooting/boot-diagnostics.md) .
...
* Send data to [Azure Monitor Metrics](data-platform-metrics.md) to analyze it with [metrics explorer](metrics-getting-started.md) take advantage of features such as near real-time [metric alerts](../../azure-monitor/platform/alerts-metric-overview.md) and [autoscale](autoscale-overview.md) (Linux only). 

## Log Analytics agent
The [Log Analytics agent](log-analytics-agent.md) collects monitoring data from the guest operating system and workloads of virtual machines in Azure, other cloud providers, and on-premises. It collects data into a Log Analytics workspace. The Log Analytics agent is the same agent used by System Center Operations Manager, and you multihome agent computers to communicate with your management group and Azure Monitor simultaneously. This agent is also required by certain solutions in Azure Monitor.
