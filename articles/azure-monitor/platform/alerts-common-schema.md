...
This means that you can potentially have fewer integrations, making the process of managing and maintaining them a _much_ simplet task. Additionally, future alert payload enrichments (e.g. customization, diagnostic enrichment, etc.) will only surface up in the common schema.
...
| Email | A consistent and detailed email template, allowing you to easily diagnose issues at a glance. Embedded deeplinks to the alert instance on the portal and the affected resource ensure that you can quickly jump into the remediation process. |
...
![Common alert schema opt-in](media/alerts-smartgroups-overview/smart-group-detail.png)
