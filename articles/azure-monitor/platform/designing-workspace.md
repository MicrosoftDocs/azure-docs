...
* Centralized: All logs are stored in a central workspace. In this scenario, it is easy to search across resources and cross-correlate logs but, since the workspace can grow significantly as a result of the extensive amount of data collected from multiple resources in your subscription, it became difficult to maintain access control on them.
...
If you are using System Center Operations Manager, each Operations Manager management group can be [connected with only one workspace](../platform/om-agents.md). You can install the Log Analytics agent for Windows or Linux on computers managed by Operations Manager and have the agent report to both Operations Manager and a different Log Analytics workspace.
