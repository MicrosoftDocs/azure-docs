...
|EventGridDeliveries|Event Grid deliveries (preview)|Count|Total|The number of requests for IoT Hub to emit events to Event Grid. Use the Result dimension for the number of successful and failed requests. To see the where the requests come from, use the EventType dimension.|Result, EventType|
|EventGridLatency|Event Grid latency (preview)|Milliseconds|Average|The average latency (milliseconds) between IoT Hub event ingress/generation and event egress to Event Grid. This number is an average between all event types. Use the EventType dimension to see latency of a specific type of event.|EventType|
