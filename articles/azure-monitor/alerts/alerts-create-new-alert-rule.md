...
1. <a name="custom-props">(Optional)</a> If you've configured action groups for this alert rule, you can add custom properties in key:value pairs to the alert payload to add more information to the payload in the **Custom properties** section. Add the property **Name** and **Value** for the custom property you want included in the payload.
...
1. Define the **Alert rule details**.
    ...
    1. Enter values for the **Alert rule name** and the **Alert rule description**.
    1. Select the **Region**.
    1. <a name="managed-id">In the **Identity** section</a>, select which identity is used by the log alert rule to send the log query. This identity is used for authentication when the alert rule executes the log query.
