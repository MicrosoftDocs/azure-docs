...
- If you can see some metrics for the resource but can't find a specific metric, [check if that metric is available](../reference/supported-metrics/metrics-index.md). If so, see the metric description to check if it's only available in specific versions or editions of the resource.
...
Review the metric description in [Supported metrics with Azure Monitor](../reference/supported-metrics/metrics-index.md) to check if it's only available in specific versions or editions of the resource or this specific type.
