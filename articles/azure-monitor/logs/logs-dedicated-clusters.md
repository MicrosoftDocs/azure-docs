...
> - Linking a workspace to a cluster involves syncing multiple components in backend and cache hydration, which can take up to two hours.

Use the following steps to link a workspace to a cluster. Process can be automated to link multiple workspaces as batch:
