...
When a Log Analytics workspace is linked to a dedicated cluster, the workspace billing plan in workspace is changed per cluster plan, new data ingested to the workspace is routed to the cluster, and existing data remains in Log Analytics cluster. Linking a workspace has no affect on data ingestion and query experiences. Log Analytics stitches behind the scenes data from the new cluster to data from the pervious location so the queries will have a complete and accurate results. It might impact query performance in the short term but over time, it would not be felt. 
