...
| Call the Log Ingestion API. | <ul>`Microsoft.Insights/DataCollectionEndpoints/Write` permissions as provided by the [Monitoring Contributor built-in role](../../role-based-access-control/built-in-roles.md#monitoring-contributor), for example.</ul><ul>`Microsoft.Insights/DataCollectionRules/Read` permissions as provided by the [Monitoring Contributor built-in role](../../role-based-access-control/built-in-roles.md#monitoring-contributor)</ul> <ul>`microsoft.operationalinsights/workspaces/tables/write` permissions permissions as provided by the [Log Analytics Contributor built-in role](./manage-access.md#log-analytics-contributor), for example.</ul> |
