...
| `binSize` |`20`, `30`, `60`, `120`, `180`, `360`, `720`, or `1,440` (minutes) | Defines the interval query to perform and the query time range. The bin summarization is at the whole hour, for example, `02:00 to 04:00, 04:00 to 06:00` when the bin is 120. When the bin is smaller than an hour, execution is at the bin fraction, 20, and 30 minutes. |
| `query` | KQL query | Defines the query to execute in the rule. A time range isn't needed because the `binSize` property determines the value, such as `02:00 to 03:00` for a 60-minutes bin. If you add a time filter in the query, the time rage used in the query is the intersection between the filter and the bin size. |
