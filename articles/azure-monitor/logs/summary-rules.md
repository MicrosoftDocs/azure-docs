...
| `query` | Defines the query to execute in the rule. A time range isn't needed because the value is determined by the bin size, such as 02:00 to 03:00 for 60-minutes bin. If you add a time filter in the query, the time rage used in the query is the intersection between the filter and the bin size. |
...
- [Tutorial: Send data to Azure Monitor Logs with Logs ingestion API (Azure portal)](tutorial-logs-ingestion-portal)
