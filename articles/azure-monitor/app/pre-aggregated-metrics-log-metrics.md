...
The newer SDKs ([Application Insights 2.7](https://www.nuget.org/packages/Microsoft.ApplicationInsights/2.7.2) SDK or later for .NET) pre-aggregate metrics during collection. This applies to all [standard metrics sent by default](../platform/metrics-supported.md#microsoftinsightscomponents) so the accuracy isn't affected by sampling and filtering and to all custom metrics sent using [GetMetric](./api-custom-events-metrics.md#getmetric).
