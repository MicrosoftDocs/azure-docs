...
By design, E2E Transaction view shows all telemetry in a single transaction (sharing an Operation Id), because all items sharing an Operation Id are expected to be a logical operation. [Operation Id](https://docs.microsoft.com/azure/azure-monitor/app/data-model-context#operation-id) is a unique identifier of the root operation, and is used to correlate logical operations. By design, Application Insights SDK will create a new Operation Id per page view. Check an example [here](https://docs.microsoft.com/azure/azure-monitor/app/correlation#example). With a SPA application, only one page view event will be generated and a single Operation Id will be used for all telemetry generated. You can turn on [enableAutoRouteTracking](https://docs.microsoft.com/azure/azure-monitor/app/javascript#single-page-applications) so a page view is generated every time the URL route is updated (logical page view happens). You can manually call trackPageView as well. If you want to manually refresh the Operation Id, you can use `appInsights.properties.context.telemetryTrace.traceID = Microsoft.ApplicationInsights.Telemetry.Util.generateW3CId()`.
