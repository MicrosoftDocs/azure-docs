...
1. **Webpage code:** Add the following script to your webpage before the closing ``</head>``. Replace the instrumentation key with the appropriate value for your Application Insights resource.
    ...
    The current SDK Loader Script is version 6 and is listed here. The version is encoded in the SDK Loader Script as sv:"#". The [current version is also available on GitHub](https://go.microsoft.com/fwlink/?linkid=2156318).
