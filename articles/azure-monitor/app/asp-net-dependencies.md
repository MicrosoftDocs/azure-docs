...
OpenCensus Python SDK for Azure Monitor allows you to send "in-process" dependency telemetry (information and logic that occurs within your application). In-process depedencies will have the `type` field as `INPROC` in analytics.
...
Additional configuration can be provided, please read [customizations](https://github.com/census-instrumentation/opencensus-python#customization) for a complete reference.
