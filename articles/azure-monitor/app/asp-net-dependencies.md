...
| IIS Server (Azure VM, on-prem, and so on.) | Use the Status Monitor PowerShell Module to [install the Instrumentation Engine](../../azure-monitor/app/status-monitor-v2-api-reference#application-insights-agent-api-enable-instrumentationengine) and restart IIS. |
