...
* Components are different from "observed" external dependencies such as SQL, Event Hubs, which your team/organization might not have access to (code or telemetry).
...
You can see the full application topology across multiple levels of related application components. Components can be different Application Insights resources or different roles in a single resource. The app map finds components by following HTTP dependency calls made between servers with the Application Insights SDK installed.

This experience starts with progressive discovery of the components. When you first load the Application Map, a set of queries is triggered to discover the components related to this component. A button at the upper-left corner updates with the number of components in your application as they're discovered.
...
> [!NOTE]
> Adding initializer by using `ApplicationInsights.config` or `TelemetryConfiguration.Active` isn't valid for ASP.NET Core applications.

**ASP.NET Core apps: Load initializer to TelemetryConfiguration**
...
### Understand the cloud role name within the context of the Application Map

To help you understand **cloud role name**, look at an Application Map that has multiple cloud role names present.
...
Alternatively, **cloud role instance** can be helpful for scenarios where **cloud role name** tells you the problem is somewhere in your web front-end. But you might be running multiple load-balanced servers across your web front-end. Being able to drill in a layer deeper via Kusto queries and knowing if the issue is affecting all web front-end servers or instances or just one can be important.
...
After the Intelligent View has been enabled, select one of the highlighted edges to see the "actionable insights." The insights appear in the panel on the right and explain why the edge was highlighted.
...
1. If you're using Azure Functions with C#, upgrade to [Functions V2](../../azure-functions/functions-versions.md).
...
If there are more than 10,000 nodes in your telemetry, Application Map won't be able to fetch all the nodes and links, so your map will be incomplete. If this scenario occurs, a warning message appears when you view the map.
...
Common troubleshooting issues for Intelligent View.
...
:::image type="content" source="media/app-map/intelligent-view-try-preview.png" alt-text="Screenshot that shows the Application Map user interface Try preview button." lightbox="media/app-map/intelligent-view-try-preview.png":::

#### Intelligent View takes a long time to load

Avoid selecting the **Update map components**.
...
![Screenshot that shows the Application map screen.](./media/app-map/14-updated.png)

## Next steps

* To learn more about how correlation works in Application Insights, see the [Telemetry correlation](correlation.md).
