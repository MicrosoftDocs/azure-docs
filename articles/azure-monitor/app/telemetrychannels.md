...
`ServerTelemetryChannel` is a more advanced channel which has retry policies and capability to store data in local disk. This channel retries telemetry sending in the event of transient errors. This channel also uses a local disk storage to keep items in disk to survive application restarts. Due to these retry mechanisms and local disk storage, this channel is considered more reliable, and is recommended for all production scenarios. This is the default channel for [Asp.Net](https://docs.microsoft.com/azure/azure-monitor/app/asp-net) and [Asp.Net Core](https://docs.microsoft.com/azure/azure-monitor/app/asp-net-core) applications which are configured as per the linked official docs. This channel is shipped as the nuget package `Microsoft.ApplicationInsights.WindowsServer.TelemetryChannel`, and is brought automatically when using either of the nuget packages `Microsoft.ApplicationInsights.Web` or `Microsoft.ApplicationInsights.AspNetCore`.

## Configuring TelemetryChannel

Telemetry Channel can be setup by setting `TelemetryChannel` on the active `TelemetryConfiguration`. For Asp.Net applications, this involves setting `TelemetryChannel` on `TelemetryConfiguration.Active`, or modifying `ApplicationInsights.config`. For ASP.NET Core applications, this involves adding the channel to the Dependency Injection Container.
...
The following code sets up ServerTelemetryChannel with `StorageFolder` set to a custom location. This should be done at the beginning of the application, typically in Application_Start() method in `Global.aspx.cs`
...
1. MaxTransmissionBufferCapacity - Maximum amount of memory, in bytes, used by the channel to buffer transmissions in memory. Once this capacity is reached, new items will be stored directly into local disk. The default is 5MB. Setting this to higher values leads to lesser disk usage, but it is important to note that items in memory is lost if application crashes.
...
1. The default disk locations for storing telemetry in Windows are %LocalAppData% or %Temp%. These are typically local to the machine. If the application migrates physically from one location to another, any telemetry stored in these location is lost.
