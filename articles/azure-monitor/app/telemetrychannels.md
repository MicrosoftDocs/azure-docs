...
Every telemetry item tracked with `TrackXYZ()` api of `TelemetryClient` goes through a series of steps. First all configured `TelemetryInitializer`s are called, followed by `TelemetryProcessors`. TelemetryChannel.Send() is called after all Telemetry Processors in the pipeline are called. This means telemetry, which are dropped by any  TelemetryProcessors won't reach channel. Once items reach `TelemetryChannel`, it's responsible for serializing and sending them to Azure Application Insights service, where it is stored for querying and analysis.
