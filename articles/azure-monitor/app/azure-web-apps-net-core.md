...
> [!NOTE]
> When you create a web app with the `ASP.NET Core` runtimes in Azure App Services it deploys a single static HTML page as a starter website. It is **not** recommended to troubleshoot an issue with default template. Please deploy an application before troubleshooting an issue.
