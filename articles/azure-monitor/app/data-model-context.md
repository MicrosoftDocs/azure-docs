...
Anonymous user IDs may consider a unique user as multiple users when they are logged in on unique devices and browsers.
...
Authenticated User IDs persist across browsers and devices, assuming that the user remains authenticated throughout their interaction with the application.
During authentication a unique ID is assigned to the user, all telemetry items are then attributed to that unique ID. This enables querying for all telemetry collected for a specific user, subject to sampling configurations and telemetry filtering. 
