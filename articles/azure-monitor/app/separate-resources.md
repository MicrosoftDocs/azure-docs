...
(If your system is an Azure Cloud Service, there's [another method of setting separate ikeys](./cloudservices.md).)
...
### Web pages
The iKey is also used in your app's web pages, in the [script that you got from the quickstart pane](./javascript.md). Instead of coding it literally into the script, generate it from the server state. For example, in an ASP.NET app:
...
You can set the Application Version property so that you can filter [search](./diagnostic-search.md) and [metric explorer](../platform/metrics-charts.md) results.
...
* Wrap that line in a [telemetry initializer](./api-custom-events-metrics.md#defaults) to ensure that all TelemetryClient instances are set consistently.
...
When it has the build info, the Application Insights web module automatically adds **Application version** as a property to every item of telemetry. That allows you to filter by version when you perform [diagnostic searches](./diagnostic-search.md), or when you [explore metrics](../platform/metrics-charts.md).
...
### Release annotations
If you use Azure DevOps, you can [get an annotation marker](./annotations.md) added to your charts whenever you release a new version. 
