...
By default, Application Insights Java 3.0 expects the configuration file to be named `applicationinsights.json`, and to be located in the same directory as `applicationinsights-agent-3.0.2-BETA.jar`.
...
If you specify a relative path, it will be resolved relative to the directory where `applicationinsights-agent-3.0.2-BETA.jar` is located.
...
> [!NOTE]
> Starting from version 3.0.2-BETA, if you add a custom dimension named `service.version`, the value will be stored
...
Starting from version 3.0.2-BETA, specific auto-collected telemetry can be suppressed using these configuration options:
...
`applicationinsights-agent-3.0.2-BETA.jar` is located.
