...
If you see a message that tells you this method is obsolete, see [microsoft/ApplicationInsights-dotnet#1152](https://github.com/microsoft/ApplicationInsights-dotnet/issues/1152) for further details.
...
The telemetry is available in the `customEvents` table on the [Application Insights Logs tab](../logs/log-query-overview.md) or [Usage Experience](usage-overview.md). Events might come from `trackEvent(..)` or the [Analytics autocollection plug-in](javascript-click-analytics-plugin.md).
...
In a device or webpage app, page view telemetry is sent by default when each screen or page is loaded. But you can change the default to track page views at additional or different times. For example, in an app that displays tabs or blades, you might want to track a page whenever the user opens a new blade.
...
`properties` | Map of string to string. Additional data is used to [filter exceptions](#properties) in the portal. Defaults to empty.
