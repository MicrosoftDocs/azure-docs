...
This data is gathered on demand when trigger conditions are met. The available triggers are thresholds over CPU usage and Memory consumption, and Request (SLA triggers).
...
The ApplicationInsights Java Agent monitors CPU and memory consumption and request duration (e.g., a business transaction) and if it breaches a configured threshold a profile is triggered. 
...
For instance, consider a scenario where you want to trigger a profile when more than 30% of requests to a specific endpoint (/users/.*) take longer than 50 ms within a 10000 ms window.
...
1. Configure the resource thresholds that will cause a profile to be collected:
    ...
    3. Configure the required CPU and Memory thresholds, or add request triggers (if enabled) and select Apply.
...
- `true` Profiling will be triggered if a request trigger criteria are met.
