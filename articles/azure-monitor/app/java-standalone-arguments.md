...
Add the JVM arg `-javaagent:path/to/applicationinsights-agent-3.2.2.jar` somewhere before `-jar`, for example:
...
If you are using the *exec* form, add the parameter `"-javaagent:path/to/applicationinsights-agent-3.2.2.jar"` to the parameter list somewhere before the `"-jar"` parameter, for example:

```
ENTRYPOINT ["java", "-javaagent:path/to/applicationinsights-agent-3.2.2.jar", "-jar", "<myapp.jar>"]
```

If you are using the *shell* form, add the JVM arg `-javaagent:path/to/applicationinsights-agent-3.2.2.jar` somewhere before `-jar`, for example:
...
If the file `<tomcat>/bin/setenv.sh` already exists, then modify that file and add `-javaagent:path/to/applicationinsights-agent-3.2.2.jar` to `CATALINA_OPTS`.
...
If the file `<tomcat>/bin/setenv.bat` already exists, just modify that file and add `-javaagent:path/to/applicationinsights-agent-3.2.2.jar` to `CATALINA_OPTS`.

### Running Tomcat as a Windows service

Locate the file `<tomcat>/bin/tomcat8w.exe`.  Run that executable and add `-javaagent:path/to/applicationinsights-agent-3.2.2.jar` to the `Java Options` under the `Java` tab.
...
Add `-javaagent:path/to/applicationinsights-agent-3.2.2.jar` to the existing `JAVA_OPTS` environment variable in the file `JBOSS_HOME/bin/standalone.conf` (Linux) or `JBOSS_HOME/bin/standalone.conf.bat` (Windows):
...
Add `-javaagent:path/to/applicationinsights-agent-3.2.2.jar` to the existing `jvm-options` in `JBOSS_HOME/domain/configuration/host.xml`:
...
Add `-javaagent:path/to/applicationinsights-agent-3.2.2.jar` to the existing `jvm-options` in `glassfish/domains/domain1/config/domain.xml`:
