...
Application Insights SDKs poll QuickPulse endpoints with REST API calls once every five seconds to check if the Live Metrics pane is being viewed. These requests cannot be seen in Application Insights logs and won't impact dependency calls or other metrics. However, they can be seen by other network monitoring tools.

If the Live Metrics pane is open, heavier traffic is expected because the SDKs send new metrics to QuickPulse *every second*. To restore normal flow of traffic, navigate away from the Live Metrics pane.
