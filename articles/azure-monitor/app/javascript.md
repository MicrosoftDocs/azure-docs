...
2. Copy the _instrumentation key_ (aka "iKey") for the resource where you want your JavaScript telemetry to be sent (the resource created in step 1.) You will add it to the `instrumentationKey` setting of the Application Insights JavaScript SDK.
...
This version of the snippet detects and reports failures to load the SDK from the CDN source as an exception to the Azure Monitor portal (under the failures / exceptions / browser), this provides visibility into this type of failure so that you will be aware that your site / application is not reporting telemetry (or other exceptions) as expected. This can be a very important signal as failure to load and initialize the SDK can cause:
- Under reporting of how users are using (or trying to us) your site;
- Missing telemetry on who your end-users are using your site;
- Missing JavaScript errors which could potentially be blocking your end-users from successfully using your site.

For details on this exception see [SDK Load Failure](javascript-troubleshoot.md#sdk-load-failure) in the troubleshooting page.
...
Reporting of SDK load failures is specifically NOT supported on IE 8 (or less). This keeps the minified size of the snippet as small as possible by assuming that most environments are not exclusively IE 8 or less. If this is required you will need to either include a fetch polyfill or create you own snippet version which uses ```XDomainRequest``` instead of ```XMLHttpRequest```, it is recommended that you use the [provided snippet source code](https://github.com/microsoft/ApplicationInsights-JS/blob/master/AISKU/snippet/snippet.js) as a starting point.

Note: If you are using a previous version of the snippet it is highly recommended that you update to the latest version so that you will be notified about this previously unreported issue.

#### Snippet configuration options

All of the configuration options have now been move towards the end of the script to help avoid accidently introducing a JavaScript error which would not just cause the SDK to never be loaded, but also it would disable the reporting of the failure.

Each configuration option is shown above on a new line, if you don't wish to override the default value of a config item listed as [optional] you can simply remove that line to minimize the resulting size of your returned page.
...
| name | string *[optional]* | This is the global name for the initialized SDK, defaults to appInsights. So ```window.appInsights``` will be a reference to the initialized instance. Note: if you provide a name value or a previous instance appears to be assigned (via the global name appInsightsSDK) then this name value will also be defined in the global namespace as ```window.appInsightsSDK=<name value>```, this is required by the SDK initialization code to ensure it's initializing and updating the correct snippet skeleton and proxy methods.
| ld | number in ms *[optional]* | Defines the load delay to wait before attempting to load the SDK. Default value is 0ms and any negative value will immediately add a script tag to the &lt;head&gt; regions of the page which will block the page load event until to script is loaded (or fails).
| useXhr | boolean *[optional]* | This setting is specifically used only for reporting SDK load failures. Reporting will first attempt to use fetch() if available and then fallback to XHR, setting this value to true just bypasses the fetch check. Use of this value would only be required if you know your app/site is being used in an environment where fetch would fail to send the failure events.
...
| cfg | object **[required]** | This is the Application Insights configuration that is passed to initialize your sdk.
...
> - ![](https://img.badgesize.io/https://js.monitor.azure.com/scripts/b/ai.2.min.js.svg?compression=gzip)
...
* [javascript-troubleshoot](javascript-troubleshoot.md)