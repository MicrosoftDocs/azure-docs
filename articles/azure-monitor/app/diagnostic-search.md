...
You can search by using the Azure portal or Visual Studio.

### In the Azure portal

You can open transaction search from the Application Insights **Overview** tab of your application. You can also select **Search** under **Investigate** on the left pane.
...
You can filter events on the values of their properties. The available properties depend on the event types you selected. Select the filter icon ![Filter icon](./media/diagnostic-search/filter-icon.png) to start.
...
To find all the items with the same property value, either enter it in the search bar or select the checkbox when you look through properties on the **Filter** tab.
...
> [!NOTE]
> To write more complex queries, open [Logs (Analytics)](../logs/log-analytics-tutorial.md) from the top of the Search pane.
>
