...
> [!NOTE] 
> At this time, Microsoft.HybridCompute ([Azure Arc-enabled servers](../../azure-arc/servers/overview.md)) resources cannot be viewed in [Metrics Explorer](../essentials/metrics-getting-started.md) (the Azure Portal UX), but can be acquired via the Metrics REST API (Metric Namespaces - List, Metric Definitions - List, and Metrics - List).
