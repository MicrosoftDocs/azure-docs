...
> [!IMPORTANT]
> This article covers collecting text logs with the [Log Analytics agent](./log-analytics-agent.md). The Log Analytics agent **will be deprecated by August 2024**. Please plan to [migrate to Azure Monitor agent](./azure-monitor-agent-migration.md) before August 2024. See [Collect text logs with Azure Monitor agent (preview)](../agents/data-collection-text-log.md) for details on collecting text logs with [Azure Monitor agent](azure-monitor-agent-overview.md).
