...
The [Azure Monitor agent](azure-monitor-agent-overview.md) is meant to replace the Log Analytics agent, Azure Diagnostic extension and Telegraf agent for both Windows and Linux machines. It can send data to both Azure Monitor Logs and Azure Monitor Metrics and uses [Data Collection Rules (DCR)](../platform/data-collection-rule-overview.md) which provide a more scalable method of configuring data collection and destinations for each agent.
...
- Manage data collection configuration centrally, using [data collection rules](../platform/data-collection-rule-overview.md) and use Azure Resource Manager (ARM) templates or policies for management overall.
