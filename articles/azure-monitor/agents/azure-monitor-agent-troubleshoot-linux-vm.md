...
3. Validate the layout of the Syslog collection workflow to ensure all necessary pieces are in place and accessible:
	1. For `rsyslog` users, ensure the `/etc/rsyslog.d/10-azuremonitoragent.conf` file is present, isn't empty, and is accessible by the `rsyslog` daemon (syslog user). Additionally, ensure that the messages you are having trouble collecting are not from an input bound to a non-default ruleset. For instance, if you have added an input to `/etc/rsyslog.conf` like `input(type="imtcp" port="514" `**`ruleset="myruleset"`**`)`, messages ingested by rsyslog through this input will not be forwarded to AMA.
