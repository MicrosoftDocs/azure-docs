...
On some popular distros (for example Ubuntu 18.04 LTS), rsyslog ships with a default configuration file (`/etc/rsyslog.d/50-default.conf`) which will log events from nearly all facilities to disk at `/var/log/syslog`. Note that for other distros such as RedHat/CentOS syslog events will be stored under `/var/log/` but in a  different file such as `/var/log/messages`.
