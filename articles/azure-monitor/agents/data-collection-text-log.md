...
This tutorial walks through configuring the collection of file-based text logs with the [Azure Monitor agent](azure-monitor-agent-overview.md). and sending the collected data to a custom table in a Log Analytics workspace. This feature uses a [data collection rule](../essentials/data-collection-rule-overview.md), you can define the structure of the log file and its target table. This same process can be used for Wide Web Consortium (W3C) formatted logs that are internally generated by IIS on Windows servers since they're stored in text files.

 This tutorial includes two *MyTable_CL* and *MyIISTable_CL*. 
...
- The log file must be stored on a local drive of a virtual machine, virtual machine scale set, or Arc enabled server.
...
The steps to configure log collection are as follows. The detail steps for each are provided in the sections below:
...
3. Paste the resource manager template below into the editor and then change the following values:
    ...
    - `recordStartTimestampFormat`: Defined the time stamp format that identifies each entry in the log file.
    - `logDirectories`: Specifies the location of IIS log files.
