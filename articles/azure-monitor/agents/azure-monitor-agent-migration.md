...
- ***IIS Logs***: When IIS log collection is enabled, AMA might not populate the `sSiteName` column of `W3CIISLog` table. This field gets collected by default when IIS log collection is enabled for legacy agent. If you need to collect the `sSiteName` field using AMA, enable the "Service Name (s-sitename)" field in W3C logging of IIS. For steps to enable this field, see [Select W3C Fields to Log](https://learn.microsoft.com/en-us/iis/manage/provisioning-and-managing-iis/configure-logging-in-iis#select-w3c-fields-to-log)
