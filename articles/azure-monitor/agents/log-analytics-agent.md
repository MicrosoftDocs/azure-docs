...
- Use [VM insights](../vm/vminsights-enable-overview.md) to install the agent for a [single machine using the Azure portal](vminsights-enable-portal.md) or for [multiple machines at scale](vminsights-enable-policy.md). This will install the Log Analytics agent and [Dependency agent](../agents/agents-overview#.mddependency-agent). 
