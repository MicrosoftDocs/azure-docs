...
You can use either [ConfigMap](./container-insights-data-collection-configure.md#configure-data-collection-using-configmap) or [Data collection rule (DCR)](./container-insights-data-collection-configure.md#configure-data-collection-using-dcr) to enable or disable stdout and stderr logs and filter specific namespaces from each. Enable the `log_collection_settings.enrich_container_logs` setting to add the container Name and Image to each log record. This applies to both stderr and stdout logs if they're enabled.

Settings for Container logs and namespace filtering are included in the cost presets configured in the Azure portal, and you can set these values individually using the other DCR configuration methods.
