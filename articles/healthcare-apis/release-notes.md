...
| Enhancements | Related information |
| :------------------------ | :------------------------------- |
| `$import` is generally available    |`$import` API is now generally available in Azure Health Data Services API version 2022-06-01. Executing the import by invoking `$import` operation on FHIR service in [Azure Health Data Services | Microsoft Docs](azure-docs-pr\articles\healthcare-apis\fhir\import-data.md). |
| `$convert-data` updated by adding STU3-R4 support. |`$convert-data` added support for FHIR STU3-R4 conversion. See [Data conversion for Azure API for FHIR](azure-docs-pr\articles\healthcare-apis\azure-api-for-fhir\convert-data.md).   |  
