...
IoMT connector is an optional feature of Azure API for FHIR that provides capability to ingest data from IoMT (Internet of Medical Things) devices. Internet of Medical Things is a category of IoT devices that can collect or generate health data and exchange it with other healthcare IT systems over internet. IoMT connector feature enables you to quickly setup a service to ingest high frequency IoMT data into Azure API for FHIR in a scalable, secure, and compliant manner.

IoMT connector can accept any Json based messages sent out by an IoMT device. This data is first transformed into appropriate FHIR based Observation resources and then persisted into Azure API for FHIR. The data transformation logic is defined through a pair of mapping templates that you configure based on your message schema and FHIR requirements. Device data can be pushed directly to the IoMT Connector or seamlessly used in concert with other Azure IoT solutions ([Azure IoT Hub](https://docs.microsoft.com/en-us/azure/iot-hub/) and [Azure IoT Central](https://docs.microsoft.com/en-us/azure/iot-central/)). The IoMT connector does not provide device security or management which are covered by the Azure IoT solutions mentioned.

### Applications of IoMT Connector (preview)

Use of IoMT devices rapidly expanding in healthcare and IoMT connector is designed to bridge the gap of bringing device data into Azure API for FHIR to enable holistic data insights and innovative clinical workflows. Some common scenarios where IoMT connector could be used are:
- **Remote Patient Monitoring/Telehealth:** Remote patient monitoring provides the ability to gather patient health data outside of traditional healthcare settings. Healthcare institutions interested in remotely monitoring patients can use IoMT connector to bring health data generated by remote devices into Azure API for FHIR. This data can then be used to track patients health status, monitor patients adherence to the treatment plan, and generate actionable insights to provide better quality of service.
- **Research and Life Sciences:** Clinical trials are rapidly adopting IoMT devices like biosensors, wearables, mobile apps etc. to capture trial data. These trials can harness IoMT connector to transmit device data to Azure API for FHIR in a secure, efficient, and effective manner. Once in Azure API for FHIR, trial data could be used to run real time analysis of trial data.
- **Advanced Analytics:** IoMT devices typically provide large volume and variety of data at a high velocity, which makes them a great fit for serving training and testing data for your machine learning models. IoMT connector’s inherent capability to work with flexible schema and at a cloud scale with low latency makes it an excellent choice for capturing device data for your advanced analytics needs.
- **Smart Hospitals/Clinics:** Smart hospitals and clinics are being built using digitized infrastructure of interconnected assets to achieve better patient care and operational efficiency through optimization of clinical workflows and management systems. IoMT connector could be used to capture data from underlying interconnected assets and make it available to provide actionable insights, which is one of the most critical components of such smart systems.
