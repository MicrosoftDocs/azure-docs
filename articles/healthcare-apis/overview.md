...
IoMT connector is an optional feature of Azure API for FHIR that provides the capability to ingest data from Internet of Medical Things (IoMT) devices. Internet of Medical Things is a category of IoT devices that can collect or generate health data and exchange it with other healthcare IT systems over the internet. The IoMT connector feature enables you to quickly set up a service to ingest IoMT data, generated at any frequency, into Azure API for FHIR in a scalable, secure, and compliant manner.

IoMT connector can accept any Json based messages sent out by an IoMT device. This data is first transformed into appropriate FHIR-based Observation resources and then persisted into Azure API for FHIR. The data transformation logic is defined through a pair of mapping templates that you configure based on your message schema and FHIR requirements. Device data can be pushed directly to the IoMT Connector or seamlessly used in concert with other Azure IoT solutions ([Azure IoT Hub](https://docs.microsoft.com/azure/iot-hub/) and [Azure IoT Central](https://docs.microsoft.com/azure/iot-central/)). The IoMT connector doesn't provide device security or management, which are covered by the Azure IoT solutions mentioned.

### Applications of IoMT Connector (preview)

Use of IoMT devices is rapidly expanding in healthcare. And IoMT connector is designed to bridge the gap of bringing device data into Azure API for FHIR to enable holistic data insights and innovative clinical workflows. Some common scenarios for the IoMT connector are:
- **Remote Patient Monitoring/Telehealth:** Remote patient monitoring provides the ability to gather patient health data outside of traditional healthcare settings. Healthcare institutions needing remote  patient monitoring can use IoMT connector to bring health data generated by remote devices into Azure API for FHIR. This data is then used to closely track patients health status and monitor patients adherence to the treatment plan.
...
- **Advanced Analytics:** IoMT devices typically provide large volume and variety of data at a high velocity, which makes them a great fit for serving training and testing data for your machine learning models. IoMT connector provides capability to work with flexible schema and cloud scaling with low latency, making it an excellent choice for capturing device data for your advanced analytics needs.
- **Smart Hospitals/Clinics:** Today smart hospitals and clinics are setting up an infrastructure of interconnected digital assets. IoMT connector can be used to capture data from these components and enable optimization of clinical workflows and management systems through actionable insights, providing better patient care and operational efficiency.
...
To try out the IoMT connector feature, checkout a quickstart to deploy IoMT connector using Azure portal.

>[!div class="nextstepaction"]
>[Deploy IoMT connector](iomt-fhir-paas-portal-quickstart.md)
