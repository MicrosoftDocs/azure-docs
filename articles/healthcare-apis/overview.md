...
IoMT connector can accept any JSON based messages sent out by an IoMT device. This data is first transformed into appropriate FHIR-based Observation resources and then persisted into Azure API for FHIR. The data transformation logic is defined through a pair of mapping templates that you configure based on your message schema and FHIR requirements. Device data can be pushed directly to IoMT Connector or seamlessly used in concert with other Azure IoT solutions ([Azure IoT Hub](https://docs.microsoft.com/azure/iot-hub/) and [Azure IoT Central](https://docs.microsoft.com/azure/iot-central/)). IoMT connector doesn't provide device security or management, which are covered by the Azure IoT solutions mentioned.

### Applications of IoMT Connector (preview)

Use of IoMT devices is rapidly expanding in healthcare. And IoMT connector is designed to bridge the gap of bringing device data into Azure API for FHIR to enable holistic data insights and innovative clinical workflows. Some common scenarios for IoMT connector are:
...
>[!div class="nextstepaction"]
>[Deploy IoMT connector](iomt-fhir-portal-quickstart.md)
