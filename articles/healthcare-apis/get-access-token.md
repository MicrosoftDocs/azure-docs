...
To use the FHIR service or the DICOM service, you need an access token that verifies your identity and permissions to the server. You can use [PowerShell](https:./../powershell/scripting/overview?view=powershell-7.3) or the [Azure Command-Line Interface (CLI)](./../cli/azure/what-is-azure-cli), which are tools for creating and managing Azure resources, to obtain an access token.
