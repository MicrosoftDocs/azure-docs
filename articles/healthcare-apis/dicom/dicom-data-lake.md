...
The DICOM service will write a small file to the data lake every 30 seconds, following the [Data Contract](#data-contracts), to ensure it maintains access. Making any changes to files stored under the `healthCheck` sub-directory could result in incorrect status of the health check.
