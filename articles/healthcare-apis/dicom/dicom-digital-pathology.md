...
Historically biopsies of tissues are stored in glass slides and investigated under microscope.This creates challenges when clinicians and pathologists needs to share information for consultations and diagnosis as well as for research.  
...
WSI generated by most acquisition scanners are not generally already in DICOM format. Thus, there is an additional step to convert WSIs to DICOM files before uploading. This conversion is not currently supported by the DICOM service. Instead, customers are encouraged to use partner solutions or adopt open source solutions to their particular needs.

Here are sample Open source tools to build your own converter:
...
Viewers retrieve tiles which are stored as frames in a DICOM instance. Each DICOM instance can contain multiple frames. We recommend using parallel single part GET frame for better performance, as this avoids CORS preflight.
...
Follow the CORS guidelines below if the Viewer directly interacts with the DICOM service

- [Configure cross-origin resource sharing in DICOM service in Azure Health Data Services | Microsoft Docs](configure-cross-origin-resource-sharing.md)

## Recommended ISV's

Please reach out to dicom-support@microsoft.com if you want to work with our partner ISV's that provide E2E solution and support.
