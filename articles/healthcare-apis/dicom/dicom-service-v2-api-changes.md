...
Failed validation of attributes not required by the API results in the file being stored with a warning.  Warnings will results in an HTTP return code of `202 Accepted` and the response payload will contain the `WarningReason` tag (`0008, 1196`).  
...
#### Null padded attributes can be searched for with our without padding
