...
> [!NOTE]
> When using `PATCH` against STU3, and if you are requesting a History bundle, the patched resource's `Bundle.entry.request.method` is mapped to `PUT`. This is because STU3 doesn't contain a definition for the `PATCH` verb in [Value Set http://hl7.org/fhir/ValueSet/http-verb](http://hl7.org/fhir/STU3/valueset-http-verb.html).
