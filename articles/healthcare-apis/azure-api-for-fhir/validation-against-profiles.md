...
In the [Store profiles in Azure API for FHIR](store-profiles-in-fhir.md) article, you were walked through the basics of FHIR profiles and storing them. Azure API for FHIR allows validating resources against profiles to see if the resources conform to the profiles. This article will guide you through how to use `$validate` for validating resources against profiles. For more information about FHIR profiles outside of this article, visit 
...
In the previous example, you'd be validating the existing Patient resource `a6e11662-def8-4dde-9ebc-4429e68d130e` against the base Patient resource. If it's valid, you'll get an `OperationOutcome` such as the following code example:
...
In this article, you've learned how to validate resources against profiles using `$validate`. To learn about the other Azure API for FHIR supported features, see
