...
description: In this tutorial, you'll learn how to connect IoT Connector with Azure IoT Hub and enable device data routing from IoT Hub into Azure API for FHIR through IoT Connector.
...
- Azure API for FHIR resource with at least one IoT Connector - [Deploy IoMT connector (preview) using Azure portal](iomt-fhir-portal-quickstart.md)
...
Azure IoT Hub requires a connection string to securely connect with your IoT Connector. Create a new connection string for your IoT Connector as described in [Generate a connection string](iomt-fhir-portal-quickstart.md#generate-a-connection-string). Preserve this connection string to be used in the next step.
...
|EndpointResourceGroup|endpoint-resource-group|Resource group name for your IoMT Connector's Azure API for FHIR resource. You can get this value from the Overview page of Azure API for FHIR.|
|EndpointSubscriptionId|endpoint-subscription-id|Subscription Id for your IoMT Connector's Azure API for FHIR resource. You can get this value from the Overview page of Azure API for FHIR.|
...
Use your device (real or simulated) to send the sample heart rate message shown below to Azure IoT Hub. This message will get routed to IoT Connector, where the message will be transformed into a FHIR-based Observation resource and stored into the Azure API for FHIR.
...
```
> [!IMPORTANT]
> Make sure to send the device message that conforms to the mapping templates configured with your IoT Connector.
