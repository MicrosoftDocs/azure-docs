...
| _\_anonymizationConfig_   |DemoConfig.json|Required for de-identified export |Name of the configuration file. See the configuration file format [here](https://github.com/microsoft/FHIR-Tools-for-Anonymization#configuration-file-format). By default, we will search and use this file inside a container named **anonymization** within the same Azure storage account that is configured as the export location.If _\_anonymizationConfigCollectionReference_ is provided, we will search and use this file from the specified image.|
