...
After setting up FHIR service to connect with an ADLS Gen2 storage account, you can call the `$export` endpoint and the FHIR service will export data into a container within the storage account. The example request below has the FHIR service export all resources into a specified container (`{{containerName}}`) in the configured ADLS Gen2 storage account. Note that the container must be created beforehand in the storage account in order to specify the `{{containerName}}` in the request.

```
GET {{fhirurl}}/$export?_container={{containerName}}
```

If you don't specify a container name in the request, then a new container with an auto-generated name will be created for the data export in your ADLS Gen2 account.
...
When data is exported, a separate file is created for each resource type. The FHIR service creates a new file once the size of a single exported file exceeds 64 MB. The result is that you may get multiple files for a resource type, which will be enumerated (e.g., Patient-1.ndjson, Patient-2.ndjson). 
...
### Headers
There are two required header parameters that must be set for `$export` jobs. The values are set according to the current [$export specification](https://hl7.org/Fhir/uv/bulkdata/export/index.html#headers) from HL7.
...
In this article, you've learned how to export FHIR resources using the `$export` operation. For information about how to set up and use de-identified export or other methods of exporting to Azure Synapse Analytics, see
