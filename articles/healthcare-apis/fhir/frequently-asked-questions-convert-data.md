...
In this article, you learned about the frequently asked questions (FAQs) about the FHIR Converter, the `$convert-data` operation, and the `$convert-data` endpoint for converting health data to FHIR by using the FHIR service in Azure Health Data Services. 
