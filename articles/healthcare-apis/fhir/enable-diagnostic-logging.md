---
title: Enable diagnostic logging in FHIR service
description: This article explains how to enable diagnostic logging in FHIR service®
...
# Enable Diagnostic Logging in FHIR service

In this article, you will learn how to enable diagnostic logging in FHIR service and be able to review some sample queries for these logs. Access to diagnostic logs is essential for any healthcare service where compliance with regulatory requirements (such as HIPAA) is a must. The feature in FHIR service that enables diagnostic logs is the [**Diagnostic settings**](../../azure-monitor/essentials/diagnostic-settings.md) in the Azure portal. 
...
You can view the metrics under Monitoring | Metrics from the portal. The metrics include Number of Requests, Average Latency, Number of Errors, Data Size, RUs Used, Number of requests that exceeded capacity, and Availability (in %). The screenshot below shows RUs used for a sample environment with very few activities in the last 7 days. You can download the data in Json format.
   ...
   :::image type="content" source="media/diagnostic-logging/fhir-metrics-rus-screen.png" alt-text="FHIR service Metrics from the portal" lightbox="media/diagnostic-logging/fhir-metrics-rus-screen.png":::

## Enable audit logs
1. To enable diagnostic logging in FHIR service, select your FHIR service service in the Azure portal 
...
> [!Note] 
> It might take up to 15 minutes for the first Logs to show in Log Analytics. Also, if FHIR service is moved from one resource group or subscription to another, update the setting once the move is complete. 
...
## Audit log details
At this time, the FHIR service service returns the following fields in the audit log: 
...
## Conclusion 
Having access to diagnostic logs is essential for monitoring a service and providing compliance reports. FHIR service allows you to do these actions through diagnostic logs. 
