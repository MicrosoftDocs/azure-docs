...
The `$convert-data` custom endpoint in the FHIR service is meant for converting health data from different legacy formats to FHIR. The `$convert-data` operation uses Liquid templates from the [FHIR Converter](https://github.com/microsoft/FHIR-Converter) project as the default templates for FHIR data mapping. You can customize these conversion templates as needed. Currently the `$convert-data` operation supports three types of data conversion: **HL7v2 to FHIR**, **C-CDA to FHIR**, and **JSON to FHIR** (JSON to FHIR conversion is intended for custom conversion mapping).
