...
Operations beyond read and search, such as `$import`, `$validate`, `$reindex`, and `$convert-data`, aren't part of the SMART user role. Each one requires the Azure role-based access control (RBAC) role that authorizes it, such as FHIR Data Importer, FHIR Data Converter, or FHIR Data Contributor. Assign that role to a separate identity that doesn't hold the FHIR SMART User role. The FHIR SMART User role forces scope evaluation on every request, so a principal that holds both roles is still denied. For more information, see [Configure Azure RBAC for FHIR](../configure-azure-rbac.md).
