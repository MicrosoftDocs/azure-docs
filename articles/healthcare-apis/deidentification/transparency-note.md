...
- Compliance: The de-identification service's performance is dependent on the user’s data. The service does not guarantee compliance with HIPAA’s Safe Harbor method nor unlinked pseudonymization aligned with GDPR principle. Our service leverages state-of-the-art machine learning models to identify and handle sensitive information, to help support customers’ own compliance efforts with HIPAA's Safe Harbor standards and unlinked pseudonymization aligned with GDPR principle.  We encourage users to obtain appropriate legal review of your solution, particularly for sensitive or high-risk applications.
...
- Languages: Currently, the de-identification service is enabled for the languages and locales list in [this page](languages-supported.md). Code-switching or using languages with the wrong language-locale pairing results in worse performance. 
