...
|Reached the maximum number of IoT Connector instances that can be provisioned in this subscription|Azure IoT Connector for FHIR subscription quota reached (Default is (2) Azure IoT Connector for FHIR per subscription)|Delete one of the existing instances of Azure IoT Connector for FHIR, use a different subscription that has not reached the (2) Azure IoT Connector for FHIR per subscription quota or request a subscription quota increase|
