...
description: Learn about Events message structure and required values.
...
In this article, learn about the Events message structure, required and nonrequired elements, and samples of Events message payloads.
...
|`data.resourceFhirId`|string|Yes|The resource ID of the FHIR account. This ID is randomly generated by the FHIR service of the Azure Health Data Services when a customer creates the Resource. Customer can also use customized ID in FHIR resource creation; however the ID should **not** include or infer any PHI/PII information. It should be a system metadata, not specific to any personal data content.|
