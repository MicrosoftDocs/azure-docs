...
description: In this article, you'll learn the transformation steps the IoT connector performs before storing IoMT data in the FHIR service.
...
Below are the different stages that data goes through once it's received by Azure API for IoT Connector.
...
The normalization process not only simplifies data processing at later stages but also provides for the projection one input message into multiple normalized messages. For instance, a device could send multiple vital signs for body temperature, pulse rate, blood pressure, and respiration rate in a single message. This input message would create four separate FHIR resources. Each resource would represent different vital sign, with the input message projected into four different normalized messages.
...
For more information about the IoT connector mapping templates, see these user guides:
