...
As a prerequisite you will need an Azure subscription and have been granted proper perssions to create Azure resource groups and deploy Azure resources. You can follow the steps one by one or skip some if you have an existing environment. Also, you can combine all the steps and complete them in PowerShell, Azure CLI and REST API scripts.
...
The IoT connector works with Azure event hub and the FHIR service in Healthcare APIs. You can create a new [FHIR service](../fhir/get-started-with-fhir.md) or use an existing one in the same or different workspace. Similarly, you can create a new [event hub](../../event-hubs/event-hubs-create.md) or use an existing one.
