---
title: IoMT connector (preview) data flow
...
## Ingest ##
Ingest is the first stage where device data is received into IoMT connector. The ingestion endpoint for device data is hosted on an [Azure Event Hub](https://docs.microsoft.com/azure/event-hubs/). Azure Event Hub platform supports high scale and throughput with ability to receive and process millions of messages per second. It also enables IoMT connector to consume messages asynchronously, thus removing the need for devices to wait while device data gets processed.
