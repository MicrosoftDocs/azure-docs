...
- **VNet-Onprem**: The on-premises virtual network represents an on-premises network. In an actual deployment, it can be connected to with either a VPN or an Azure ExpressRoute connection. For simplicity, this procedure uses a VPN gateway connection, and an Azure-located virtual network represents an on-premises network.

![Screenshot that shows a firewall in a hybrid network.](media/tutorial-hybrid-ps/hybrid-network-firewall.png)

If you want to use Azure PowerShell instead to complete this procedure, see [Deploy and configure Azure Firewall in a hybrid network by using Azure PowerShell](tutorial-hybrid-ps.md).

> [!NOTE]
> This article uses classic Azure Firewall rules to manage the firewall. The preferred method is to use an [Azure Firewall Manager policy](../firewall-manager/policy-overview.md). To complete this procedure using an Azure Firewall Manager policy, see [Tutorial: Deploy and configure Azure Firewall and policy in a hybrid network using the Azure portal](tutorial-hybrid-portal-policy.md).
...
To learn how to create these routes, see the [Create the routes](#create-the-routes) section in this article.
...
1. From the Azure portal home page, select **Create a resource**.
1. Search for **Firewall** and select it.
1. Select **Create**.
1. On the **Create a Firewall** page, use the following table to configure the firewall:
   ...
   |Public IP address     |Add new: **fw-pip** |

1. Select **Review + create**.
1. Review the summary, and then select **Create** to create the firewall.

   This takes a few minutes to deploy.
1. After deployment completes, go to the **RG-fw-hybrid-test** resource group, and select the **AzFW01** firewall.
...
1. Under **Rules IP Addresses**, for **Name**, type **AllowWeb**.
...
1. From the Azure portal home page, select **Create a resource**.
1. In the search text box, type **virtual network gateway**.
1. Select **Virtual network gateway**, and select **Create**.
1. For **Name**, type **GW-hub**.
...
1. From the Azure portal home page, select **Create a resource**.
1. In the search text box, type **virtual network gateway** and press **Enter**.
1. Select **Virtual network gateway**, and select **Create**.
1. For **Name**, type **GW-Onprem**.
1. For **Region**, select the same region that you used previously.
1. For **Gateway type**, select **VPN**.
1. For **VPN type**, select **Route-based**.
1. For **SKU**, select **Basic**.
...
1. Select **Add**.
1. For the connection name, type **Hub-to-Onprem**.
...
1. Select **Connections** in the left column.
1. Select **Add**.
1. For the connection name, type **Onprem-to-Hub**.
1. Select **VNet-to-VNet** for **Connection type**.
...
1. From the Azure portal home page, select **Create a resource**.
1. In the search text box, type **route table** and press **Enter**.
1. Select **Route table**.
1. Select **Create**.
1. Select the **RG-fw-hybrid-test** for the resource group.
...
1. Select **Create**.
1. Select the **RG-fw-hybrid-test** for the resource group.
1. For **Region**, select the same location that you used peviously.
...
1. From the **VM-Onprem** virtual machine, open a remote desktop to **VM-spoke-01** at the private IP address.
