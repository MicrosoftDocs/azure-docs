...
- **Multiple IPs in DNS resolution**: If an FQDN resolves to multiple IP addresses in a private DNS zone or custom DNS servers, the DNAT rule translates to the first IP address in the resolved list. The DNS proxy itself returns all resolved records; selecting a single address is specific to DNAT translation. This behavior is by design.
...
- **Multiple IPs in DNS resolution**: DNAT translation always uses the first IP address from the resolved list (Private DNS zone or custom DNS server), even though the DNS proxy returns all resolved records. This is an expected behavior.
