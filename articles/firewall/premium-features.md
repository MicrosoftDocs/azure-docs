...
|Severity      |Each signature has an associated **Priority** level that indicates the probability that the signature is an actual attack.<br>- **Priority 3 (Low)**: An abnormal event is one that doesn't normally occur on a network or Informational events are logged. Probability of attack is low.<br>- **Priority 2 (Medium)**: The signature indicates an attack of a suspicious nature. The administrator should investigate further.<br>- **Priority 1 (High)**: The attack signatures indicate that an attack of a severe nature is being launched. There's little probability that the packets have a legitimate purpose.|
