...
|KeyVault Private Endpoint|KeyVault supports Private Endpoint access to limit its network exposure. Trusted Azure Services are able to bypass this limitation if exception is configured as detailed on [https://docs.microsoft.com/en-us/azure/key-vault/general/overview-vnet-service-endpoints#trusted-services](KeyVault documentation). Firewall is currently not listed as trusted service and will not be able to access the Key Vault.|Fix scheduled for GA.|
