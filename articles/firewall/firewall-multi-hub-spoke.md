...
- Configuring a route in the gateway subnet for the whole hub VNet (`10.1.0.0/24` in the example above) will override the system route for the hub VNet. Consequently, intra-subnet control traffic between the VPN / ExpressRoute gateways will also be sent to the firewall, potentially disrupting the correct operation of the gateways.
- Configuring a route in the spoke subnets for the whole hub VNet (`10.1.0.0/24` in this example) will override the system route created by the peering with the hub VNet. All traffic sent to the hub would be sent to the Azure Firewall, including traffic that is sourced from the Azure Firewall itself, such as Internet-to-spoke traffic that has gone through Source Network Address Translation (SNAT). This will introduce asymmetric traffic in the design and the effect will be that a part of your flows will be dropped.
