...
- >Traffic between directly peered VNets is routed directly even if UDE points to Azure Firewall as a default gateway. To send subnet to subnet traffic to the firewall in this scenario, UDR must contain the target subnet network prefix explicitly on both subnets.
