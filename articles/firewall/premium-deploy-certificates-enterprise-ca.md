...
Azure Firewall Premium Preview includes a TLS termination feature, which requires a certificate authentication chain. For production deployments, you should use an Enterprise PKI to generate the certificates that you use with Azure Firewall Premium. Use this article to create and manage Enterprise PKI certificates for Azure Firewall Premium Preview.

For more information, see [Azure Firewall Premium Preview certificates](premium-certificates.md).
...
1. .pfx file format is selected by default. Un-check **Include all certificates in the certification path if possible**. If you export the entire certificate chain, the import process to Azure Firewall will fail.
