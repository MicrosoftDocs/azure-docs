...
Azure Firewall uses the primary public IP address first before it utilizes other associated public IP addresses. If your traffic workload requires a large volume of SNAT ports for connecting outbound, the Azure Firewall will use the primary public IP address until it exhausts the SNAT ports with that primary public IP address and then start utilizing other public IPs.

One of the challenges with using a large number of public IP addresses is when there are downstream IP address filtering requirements. While Azure Firewall does use a primary public IP address before utilizing other associated public IPs, having multiple public IPs associated to your Firewall means you need to allow all public IP addresses associated with it. Even if you use [Public IP address prefixes](../virtual-network/ip-services/public-ip-address-prefix.md) and you need to associate 250 public IP addresses to meet your outbound SNAT port requirements, you still need to create and allow 16 public IP address prefixes.
