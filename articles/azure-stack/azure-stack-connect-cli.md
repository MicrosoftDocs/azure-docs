...
2. Get your Azure Stack environment’s active directory endpoint and active directory resource Id endpoint by navigating to the following link in a browser: 
   ...
   When you navigate to the previous link, a file named **endpoints** is downloaded. Open this file and make a note of the value assigned to the **loginEndpoint** and **audiences** parameters, you will use these values in the next step. 
   
   The "audiences" parameter has the format- `https://management.<aadtenant>.onmicrosoft.com/<active-directory-resource-id>`. 

   The "loginEndpoint" has the format- `https://login.windows.net/` for AAD based deployments and `https://adfs.local.azurestack.external/adfs`for AD FS based deployments.
