...
1. The Azure Stack marketplace doesn't contain a Linux image by default. So, before you can create a Linux virtual machine, make sure that the Azure Stack operator download the “Ubuntu Server 16.04 LTS” image by using the steps described in the Download marketplace items topic.   
...
4. A public SSH key with the name id_rsa.pub needs to be created in the .ssh directory of your Windows user profile. For detailed information on creating SSH keys, see Creating SSH keys on Linux/macOs  or Creating SSH keys on Windows.  
