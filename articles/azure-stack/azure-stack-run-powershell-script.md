...
7. Optionally, set the following values:
    ...
    - **Time server**: This required field sets the time server and must be an IP address. Top find a time server IP address, visit [pool.ntp.org](http:\\pool.ntp.org) or ping time.windows.com. 
8. Click **Next**. 
