...
After the development kit host computer successfully boots into the CloudBuilder.vhdx image, open an elevated PowerShell console and run this command to start the InstallAzureStackPOC.ps1 PowerShell installer script: 
...
- To deploy the development kit with AD FS, run the following PowerShell commands (you just need to add the -UseADFS parameter): 
  ...
  In AD FS deployments, the default stamp Directory Service is used as the identity provider. The default account to sign in with is azurestackadmin@azurestack.local, and the password to use is the one you provided as part of the setup.
...
1. From an elevated Powershell console, run the command:
