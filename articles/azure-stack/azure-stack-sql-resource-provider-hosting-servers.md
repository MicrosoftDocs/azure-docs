...
* You will need to configure an account with appropriate privileges for use by the RP.
* Network traffic from the RP to SQL uses port 1433; this cannot be changed
* The RP and tenants such as Web Apps use the tenant network, so connectivity to the SQL instance on this network is required. This typically means the IP for your SQL instances must be on a public network.
...
A number of SQL IaaS virtual machine images are available through the Marketplace Management feature. Make sure you always download the latest version of the SQL IaaS Extension before you deploy a VM using a Marketplace item. The SQL images are the same as the SQL VMs that are available in Azure. The IaaS extension and corresponding portal enhancements give you additional features such as automatic patching and backup capabilities for SQL VMs created from these images.
...
You can use use standalone (non-HA) SQL servers using any edition of SQL Server 2014 or SQL Server 2016. Make sure you have the credentials for an account with system admin privileges.
...
> [!NOTE]
> The SQL adapter RP _only_ supports SQL 2016 SP1 Enterprise or later instances for AlwaysOn, as it requires new SQL features such as automatic seeding. In addition to the common list of requirements above:

* You will need to provide a file server in addition to the SQL AlwaysOn computers. There is an [Azure Stack Quickstart template](https://github.com/Azure/AzureStack-QuickStart-Templates/tree/master/sql-2016-ha) that can create this environment for you. It also can serve as a guide to building your own instance.

* You will need to setup the SQL servers. Specifically, you will have to enable [Automatic Seeding](https://docs.microsoft.com/sql/database-engine/availability-groups/windows/automatically-initialize-always-on-availability-group) on each availability group for each instance of SQL Server.
...
3. Fill the form with the connection details of your SQL Server instance. You will need to provide the account information for the account you configured with system admin privileges.
...
5. Provide the information for the file server share. The RP will need a share that allows read/write access for the SQL instance administrative account. This is used during database creation; user data is not stored here.

6. Add the SQL AlwaysOn instance to a SKU. You cannot mix standalone servers with AlwaysOn instances in the same SKU. That will be determined when adding the first hosting server, and attempting to mix types afterwards will result in an error.
...
### Patching and updating
 The SQL Adapter is not serviced as part of Azure Stack as it is an add-on component. Microsoft will be providing updates to the SQL Adapter as necessary. Since this is instantiated on a _tenant_ virtual machine under the Default Provider Subscription, it will be necessary to provide Windows patches, anti-virus signatures, etc. The Windows update packages that are provided as part of the patch-and-update cycle can be used to apply updates to the Windows VM. When an updated adapter is released, a script will be provided to apply the update. This script will create a new RP VM and migrate any state that you already have.

 ### Backup/Restore/Disaster Recovery
 The SQL Adapter is not backed up as part of Azure Stack BC-DR process, as it is an add-on component. Scripts will be provided to facilitate backing up of necessary state information (stored in an Azure Stack storage account) and restoring the RP in the event a complete stack recovery becomes necessary. Database servers must be recovered first (if necessary), before the RP is restored.

 ### Updating SQL credentials
 You are responsible for creating and mainting system admin accounts on your SQL servers. The RP needs an account with these privileges to manage databases on behalf of tenants - it does not need access to the data in those databases. In the event that you need to update the sa passwords on your SQL servers, you can use the update capability of the RP's administrator interface to change the stored password used by the RP as well. These passwords are stored in a Key Vault on your Azure Stack instance.

Click **Browse** &gt; **ADMINISTRATIVE RESOURCES** &gt; **SQL Hosting Servers** &gt; **SQL Logins** and select a login name to modify the settings. Note that the change must be made on the SQL instance first (and any replicas, if necessary). In the Settings panel, click on Password.
