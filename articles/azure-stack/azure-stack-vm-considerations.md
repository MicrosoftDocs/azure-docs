...
| Virtual Machine quotas | [Quota limits](.../azure-subscription-service-limits#virtual-machines-limits.md) are set by Microsoft | The Azure Stack administrator must assign quotas before they offer virtual machines to their users. |
...
| Virtual machine network | <ul><li>Public IP addresses assigned to tenant virtual machine are accessible over the Internet.</li><br><br><li>Azure Virtual Machines have a fixed DNS name</li> | <ul><li>Public IP addresses assigned to a tenant virtual machine are accessible within the POC environment only. A user must have access to the POC environment via [RDP](azure-stack-connect-azure-stack.md#connect-with-remote-desktop) or [VPN](azure-stack-connect-azure-stack.md#connect-with-vpn) to connect to a virtual machine that is created in Azure Stack.</li><br><br><li>Virtual machines created within a specific Azure Stack instance have a DNS name based on the value that is configured by the administrator.</li> |
