...
| --- | --- |
| *armendpoint* |Azure Resource Manager endpoint of your Azure Stack environment. The Azure Stack convention is that the name of ARM endpoint is in the format https://api.{region}.{domain-name}'. For example, if the region is local and the domain name is azurestack.external, then the ARM endpoint will be https://api.local.azurestack.external |
