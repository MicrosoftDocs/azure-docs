...
1. When you are instructed to do so by the CSS representative, access the privileged endpoint (PEP). Run the following commands to establish a privileged endpoint (PEP) session:
...
2. Once you are in the PEP, run: 
   ...
   Refer to the Test-AzureStack [parameter considerations](azure-stack-diagnostic-test.md#parameter-considerations) and [use cases](azure-stack-diagnostic-test.md#use-cases) sections for more information.
...
4. The CSS representative will require the logs you collected to continue troubleshooting your issue.
...
- You need to supply the **ServiceAdminCredential** parameter to run cloud scenario tests as described in the [use cases](azure-stack-diagnostic-test.md#use-cases) section.
- A tenant VM is deployed as part of one the cloud scenario tests. You can use **DoNotDeployTenantVm** to disable this.
- **BackupSharePath** and **BackupShareCredential** are used when testing infrastructure backup settings as shown in the [use cases](azure-stack-diagnostic-test.md#use-cases) section.
