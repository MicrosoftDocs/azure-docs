<properties
  pageTitle="Create a MongoDB cluster on Ubuntu using a Resource Manager Template"
  description="Create a MongoDB cluster on Ubuntu using a Resource Manager Template via PowerShell or the Azure CLI"
...
In addition to the various flavors that were already available in the Azure Marketplace, now you can also easily deploy a new MongoDB cluster on Ubuntu VMs using a Resource Manager template deployed through [Azure PowerShell](powershell-install-configure.md) or the [Azure CLI](xplat-cli.md).
