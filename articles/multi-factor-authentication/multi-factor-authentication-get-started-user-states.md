...
2. Find the user who you want to enable for Azure MFA. You might need to change the view at the top. 
...
Conditional access is a paid feature of Azure Active Directory, with many configuration options. These steps walk through one way to create a policy. For more information, read about [Conditional Access in Azure Active Directory](../active-directory/active-directory-conditional-access-azure-portal.md).
...
The other options in the conditional access policy give you the ability to specify exactly when two-step verification should be required. For example, you can make a policy such as this one: when contractors try to access our procurement app from untrusted networks on devices that are not domain-joined, require two-step verification. 
