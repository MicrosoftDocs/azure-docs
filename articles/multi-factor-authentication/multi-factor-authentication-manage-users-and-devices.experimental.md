...
## Require users to provide contact methods again
This setting forces the user to complete the registration process again. If the user has app passwords for non-browser apps, they continue to work.  You can delete the users app passwords by also selecting **Delete all existing app passwords generated by the selected users**.
...
Users can opt out of two-step verification for a configurable number of days on their regular devices. In the scenario that a trusted account is compromised or a trusted device is lost, you need to be able to remove the trusted status and require two-step verification again.

The **Restore multi-factor authentication on all remembered devices** setting means that the user will be prompted to perform two-step verification the next time they sign in, even if they chose to mark their device as trusted. 
