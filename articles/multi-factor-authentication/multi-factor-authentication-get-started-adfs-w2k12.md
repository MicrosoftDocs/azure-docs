...
* For information about installing the Web Service SDK with the user portal, see [deploying the user portal for Azure Multi-Factor Authentication Server.](multi-factor-authentication-get-started-portal.md)
...
1. In MFA Server UX, AD FS section, check the “Enable logging” checkbox.
2. On each AD FS server, use regedit.exe to create string value registry key Computer\HKEY_LOCAL_MACHINE\SOFTWARE\WOW6432Node\Positive Networks\PhoneFactor\InstallPath with value “C:\Program Files\Multi-Factor Authentication Server\” (or different path of admin’s choosing).  Note, the trailing backslash is important.
3. Create C:\Program Files\Multi-Factor Authentication Server\Logs directory (or Logs directory under the different path created in step 1).
