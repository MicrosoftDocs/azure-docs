...
- The three artefacts, the service definition (.csdef), the service config (.cscfg), and a service package (.cspkg) of a cloud service are carried forward and there is no change in the their [formats](cloud-services-model-and-package.md). 
...
- Underlying update process with respect to update domains, how upgrade proceeds, rollabck and allowed service changes during an update don't change
...
Cloud Services (extended support) provides two paths for you to migrate from [Azure Service Manager](https://docs.microsoft.com/powershell/azure/servicemanagement/overview?view=azuresmps-4.0.0&preserve-view=true ) to [Azure Resource Manager](https://docs.microsoft.com/azure/azure-resource-manager/management/overview). 
