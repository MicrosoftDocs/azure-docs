...
## What does not change w.r.t Cloud Services (classic)  
...
- Cloud Services (extended support) do not have a concept of Hosted Service. Each deployment is its own separate cloud service. Likewise, the concept of Staging and Production slots do not exist for Cloud Services (extended support).
...
- Network Configuration File (netcfg) does not exist in ARM. Virtual networks & subnets in ARM are created through existing ARM APIs (Note: netcfg is not supported in ARM)   and referenced in the CSCFG, within the NetworkConfiguration section 
