...
|XrpPaaSMigrationCscfgCsdefValidationMismatch: Migration could not be completed on deployment "deployment name" in hosted service "hosted service name" because the deployment's metadata is stale. Please abort the migration and upgrade the deployment before retrying migration. Validation Message: The service name 'Service name 1'in the service defintion file does not match the name 'Service name 2' in the service configuration file | Update your configuration files to ensure that service name matches in both .csdef and .cscfg files and update your deployment before migration |
