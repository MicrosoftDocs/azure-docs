...
| Side-by-side (SxS) Stack Listener | Verifies that the side-by-side stack is up and running, listening, and ready to receive connections. | If this check fails, it's fatal, and users won't be able to connect to the session host. Try restarting your virtual machine (VM). If this doesn’t work, open a support ticket. |
...
- You have a local host configured to block the required websites.
...
If your local host is blocking the required URLs, make sure none of the required URLs are in the **Hosts** file on your device. You can find the Hosts file by going to **HKEY_LOCAL_MACHINE** > **SYSTEM** > **CurrentControlSet** > **Tcpip** > **Parameters** > **DataBasePath**.
