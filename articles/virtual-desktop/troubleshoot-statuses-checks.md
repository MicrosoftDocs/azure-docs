...
|Needs Assistance|The session host didn't pass one or more of the following non-fatal health checks: the Geneva Monitoring Agent health check, the Azure Instance Metadata Service (IMDS) health check, or the URL health check. You can find which health checks have failed in the session hosts detailed view in the Azure portal. |Follow the directions in [Error: VMs are stuck in "Needs Assistance" state](#error-vms-are-stuck-in-needs-assistance-state) to resolve the issue.|  
|Shutdown| The session host has been shut down. This status doesn't always update immediately and will display “Unavailable” until the session host is turned back on. |Turn on the session host. | 
|Unavailable| The session host is either turned off or hasn't passed fatal health checks, which prevents user sessions from connecting to this session host. |If the session host is off, turn it back on. If the session host didn't pass the domain join check or SxS side stack listener health checks, refer to the table in [Health check](#health-check) for ways to resolve the issue. If the status is still "Unavailable" after following those directions, open a support case.|
...
|---------------------|:------:|:------:|  
| Domain joined | Verifies that the session host is joined to a domain controller. | If this check fails, Users won't be able to join the session host. To solve this issue, join your session host to a domain. | 
| Geneva Monitoring Agent | Verifies that the session host has a healthy monitoring agent by checking if the monitoring agent is installed and running in the expected registry location. | If this check fails, it's semi-fatal. There may be successful connections, but they'll contain no logging information. To resolve this, make sure a monitoring agent is installed. If it's already installed, open a support ticket. | 
| IMDS reachable | Verifies that the service can't access the IMDS endpoint. | If this check fails, it's semi-fatal. There may be successful connections, but they won't contain logging information. To resolve this issue, you'll need to reconfigure your networking, firewall, or proxy settings. |
| SxS Stack Listener | Verifies that the side-by-side stack is up and running, listening, and ready to receive connections. | If this check fails, it's fatal, and users won't be able to connect to the session host. Try restarting your virtual machine (VM). If this doesn’t work, open a support ticket. |
...
If the session host doesn't pass the UrlsAccessibleCheck health check, you'll need to identify which [required URL](safe-url-list.md) is blocked. Once you know which URL is blocked, identify which setting is blocking that URL and remove it.

There are two reasons why the service is blocking a required URL:

- You have an active firewall that's blocking most outbound traffic and access to the required URLs. To resolve this issue, add a rule that allows outbound connections to TCP port 80/443 associated with the blocked URLs.

    -	Add a rule that allows outbound connections to the TCP port 80/443 associated with the blocked URLs.
- You have a local host configured to block websites. If your local host blocks the required URLs, make sure none of the required URLs are in the "Hosts" file on your device. You can find the "Host" file by going to **HKEY_LOCAL_MACHINE** > **SYSTEM** > **CurrentControlSet** > **Tcpip** > **Parameters** > **DataBasePath**.
    -	If your local host is blocking the required URLs, make sure none of the required URLs are in the "Hosts" file in Windows. You can find the "Hosts" file by going to HKEY_LOCAL_MACHINE > SYSTEM > CurrentControlSet > Tcpip > Parameters, then looking under DataBasePath.
...
- Reconfigure your networking, firewall, or proxy settings to unblock the IP address at 169.254.169.254.
...
If your issue is caused by a web proxy, add an exception for 169.254.169.254 in the web proxy's configuration. To add this exception, open a command-line interface and run the following command:
