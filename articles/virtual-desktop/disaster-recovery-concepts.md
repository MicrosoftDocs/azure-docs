...
:::image type="content" source="media/shared-host-pool-recovery.png" alt-text="A diagram of a deployment using the recommended shared host pool disaster recovery strategy described in the previous paragraph. On the left is the primary region in West US with the host pool with two personal hosts, a domain and account controller, a profile storage with Azure Files Premium, a Storage account, and a VNET labeled Infra1. The right sde is the secondary region in East US, which is a mirror image of the primary region. The host pools and profiles in both regions are connected by two rectangles labeled AVD Host Pool 1 and Cloud Cache FSLogix GPO. The two VNETs are connected by a line labeled VNET Peering. Above both rectangles is a blue triangle logo labeled Azure Active Directory, which is connected by a line to the account controller.":::
