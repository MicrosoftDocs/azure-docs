...
:::image type="content" source="media/personal-host-pool-recovery.png" alt-text="A diagram of a deployment using the recommended personal host pool disaster recovery strategy described in the previous paragraph. On the left is the primary region in West US with the host pool with two personal hosts, two local disks with user profile data, a VNET, and an Azure Backup Vault. On the right is the secondary region, with Azure Site Recovery Vault, two replicated VMs, and two local disks with user profile data icons that are grayed out and transparent, indicating that they're absent from this region. It also has its own VNET. Between the two regions is the blue triangle Azure logo labeled Azure AD Joined. However, the session hosts being joined to Azure AD isn't a prerequisite for this recovery option.":::
