...
We've made some improvements to Microsoft Teams for Windows Virtual Desktop. Most importantly, we now audio and visual redirection for calls. Redirection improves latency by creating a direct path between users when they use audio and video. Less distance means fewer hops, which makes calls look and sound smoother.
