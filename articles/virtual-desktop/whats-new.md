...
- Version 1.0.3855.1400: This update was released December 2021 and has the following changes:
    - Has a fix to resolve an unhandled exception.
    - Adds support for HCI by retrieving VM metadata from the Azure Arc service.
    - Allows the stack to be updated if the stack is built-in and the version is below the threshold.
    - Has a change to the UrlsAccessibleCheck to only get the URL up until the path delimiter to prevent 404 errors.
