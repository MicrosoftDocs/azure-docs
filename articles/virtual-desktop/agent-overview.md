...
- Your VM keeps three versions of the side-by-side stack at a time. This allows for quick recovery if something goes wrong with the update. The earliest version of the stack is removed from the VM whenever the stack updates. If you delete these components prematurely and the stack has a failure, it will fail to roll back and will put your VM in an unavailable state.
- Your VM keeps a minimum of 3 versions of the agent to allow for a quick recovery if something goes wrong during an update. The agent automatically removes all agent components 3 versions old or older. If you delete these components prematurely and the agent has a failure, it will fail to roll back and will put your VM in an unavailable state.
