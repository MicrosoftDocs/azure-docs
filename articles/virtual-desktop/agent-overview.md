...
The Windows Virtual Desktop service updates the agent whenever an update becomes available using a flighting system. Windows Virtual Desktop flighting allows the service to deploy agent updates to a subset of VMs based on the different broker regions. This system enables us to assess the impact and stability of the agent through moderated progressions or flights. Flights are rolled out based on your Azure Active Directory tenant ID, which means that all VMs under a single tenant ID will be in the same flight and will receive the agent update at the same time. Once a flight has been deployed, Windows Virtual Desktop will observe the flight metrics to ascertain the efficacy of the update.
Agent updates can include new functionality or fixes for previous issues. Once the initial version of the Windows Virtual Desktop agent is installed, the agent regularly queries the Windows Virtual Desktop service to determine if there's a newer version of the agent and/or stack and monitoring agent components available. If a new version of any of the components has already been deployed, the updated component is automatically installed.
