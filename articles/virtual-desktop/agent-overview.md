...
New versions of the agent are rolled out in moderated progressions called flights over the course of a weeklong deployment period to all Azure subscriptions. With the flighting of the agent update, you may see VMs in your host pool receive the agent update at different times. All VM agents in all subscriptions will be updated by the end of the deployment period. The Windows Virtual Desktop flighting system enhances the reliability of the service by ensuring the stability and quality of the agent update.
