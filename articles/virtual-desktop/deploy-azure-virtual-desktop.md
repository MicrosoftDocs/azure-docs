...
1. On the **Basics** tab, complete the following information:
   ...
   | Host pool type | Select whether you want your host pool to be Personal or Pooled.<br /><br />If you select **Personal**, a check box option appears for **Assign multiple desktops to a single user**. Check this box if you would like to enable the [multi-personal desktop assignment](configure-host-pool-personal-desktop-assignment-type.md#assign-multiple-personal-desktops-to-a-single-user-preview) feature.<br /><br />Another option that appears if you select **Personal** is **Assignment type**. Select either **Automatic** or **Direct**. If you enabled multi-personal desktop assignment, then the **Direct** assignment type will automatically be selected.<br /><br />If you select **Pooled**, two new options appear for **Load balancing algorithm** and **Max session limit**.<br /><br />- For **Load balancing algorithm**, choose either **breadth-first** or **depth-first**, based on your usage pattern.<br /><br />- For **Max session limit**, enter the maximum number of users you want load-balanced to a single session host. For more information, see [Host pool load balancing algorithms](host-pool-load-balancing.md) |
