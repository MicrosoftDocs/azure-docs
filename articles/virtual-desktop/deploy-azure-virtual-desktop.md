...
1. On the **Basics** tab, complete the following information:
   ...
   | **Host pool type** | Select whether you want your host pool to be **Personal** or **Pooled**.<br /><br />If you select **Personal**, two new options appear for **Assign multiple desktops to a single user** and **Assignment type**.<br /><br />- If you select the **Assign multiple desktops to a single user** option, **Assignment type** will automatically be set to **Direct**. For more information, see [Assign multiple personal desktops to a single user (preview)](configure-host-pool-personal-desktop-assignment-type.md#assign-multiple-personal-desktops-to-a-single-user-preview).<br /><br />- If you don't select the **Assign multiple desktops to a single user** option, select **Automatic** or **Direct** for **Assignment type**.<br /><br />If you select **Pooled**, two new options appear for **Load balancing algorithm** and **Max session limit**.<br /><br />- For **Load balancing algorithm**, choose either **breadth-first** or **depth-first**, based on your usage pattern.<br /><br />- For **Max session limit**, enter the maximum number of users that you want load-balanced to a single session host. For more information, see [Host pool load-balancing algorithms](host-pool-load-balancing.md). |
