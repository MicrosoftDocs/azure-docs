...
description: How to estimate costs and pricing for using Azure Insights.
author: Heidilohr
ms.topic: conceptual
ms.date: 03/29/2021
ms.author: helohr
...
The predefined datasets for Azure Insights include:
...
Performance counters show how the system resources are performing. Performance counter data ingestion depends on your environment size and usage. In most cases, performance counters should make up 80 to 99% of your data ingestion for Azure Insights.
...
The following table lists the 20 performance counters Azure Insights collects and their default rates:
...
These are the default Windows Events for Azure Insights:
...
In this example, the total ingested data for Azure Insights is between 92 to 145 megabytes per VM per day. In other words, every 31 days, each VM ingests roughly 3 to 5 gigabytes of data.
...
>[!NOTE]
>Removing data points will impact their corresponding visuals in Azure Insights.
...
Our default configuration is the only set of data we recommend for Azure Insights. You always have the option to add additional data points and view them in the Host Diagnostics: Host browser or build custom charts for them, however added data will increase your Log Analytics cost. These can be removed for cost savings.
...
The performance counters the session hosts use will probably be your largest source of ingested data for Azure Insights. The following custom query template for a Log Analytics workspace can track frequency and megabytes ingested per performance counter over the last day:
...
This query will show all performance counters you have enabled on the environment, not just the default ones for Azure Insights. This information can help you understand which areas to target to reduce costs, like reducing a counter’s frequency or removing it altogether.
...
Learn more about Azure Insights at these articles:

- [Use Azure Insights to monitor your deployment](azure-monitor.md).
