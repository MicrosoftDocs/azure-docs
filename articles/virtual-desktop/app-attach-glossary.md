...
This is a network share that holds expanded MSIX packages. MSIX shares support SMB 3 or later. From this MSIX share applications get staged without having to move application files to the system drive.
...
Staging in MSIX app attach involves two things:
...
In regular registration, each application assigned to a user is fully registered. This happens during the time the user signs in to the session, which might impact the time it takes for them to start using Windows Virtual Desktop.

### Delayed registration

In delayed registration, each application assigned to the user is only partially registered. Partial registration means that the Start menu tile and double-click file associations are registered. This happens while the user signs in to their session, so it has minimal impact on the time it takes to start using Windows Virtual Desktop. Registration completes only when the user runs the application in the MSIX package.

Delayed registration is currently the default configuration for MSIX app attach.

## Deregistration

Deregistreation removes a registered but non-running MSIX package for a user. This happens while the user signs out of their session. During deregistration, MSIX app attach pushes application data specific to the user to the local user profile.

## Destage

Destaging notifies the OS that a MSIX package or application that currently isn't runningg and isn't staged for any user can be unmounted. This removes all reference to it in the OS.
...
The following table is a performance comparison between VHD and CimFS. These numbers were the result of a test run with 500 300 MB files in each format run on a DSv4 machine.
