...
An MSIX share is a network share that holds expanded MSIX packages. MSIX shares must support SMB 3 or later. Be accessible by the Virtual Machines (VM) in the host pool system account. MSIX packages get staged from the MSIX share without having to move application files to the system drive. 
