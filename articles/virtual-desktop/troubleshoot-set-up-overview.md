...
| Managing Windows Virtual Desktop configuration tied to host pools and application groups (app groups)      | See [Windows Virtual Desktop PowerShell](troubleshoot-powershell.md), or **open a support request for Windows Virtual Desktop (Compute)**.|
| Deploying and manage FSLogix Profile Containers | See [Troubleshooting guide for FSLogix products](https://docs.microsoft.com/fslogix/fslogix-trouble-shooting-ht) and if that doesn't resolve the issue, **open a support request for FSLogix**. |
