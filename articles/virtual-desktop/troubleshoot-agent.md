...
9. If this does not resolve your issue, make sure that you do not have any group policies with ciphers that block the agent to broker connection. Windows Virtual Desktop uses the same TLS 1.2 ciphers as Azure Front Door. For more information, see [Connection Security](network-connectivity#connection-security).
