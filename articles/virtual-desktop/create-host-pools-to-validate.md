...
Host pools are a collection of one or more identical virtual machines within Windows Virtual Desktop Preview tenant environments. Before deploying host pools to your production environment, we highly recommend you create a validation host pool so that you can monitor service updates before rolling out the host pools to your production environment. The validation host pool should be as similar to the configuration of your production environment as possible, with users connecting frequently and with automated testing to ensure that your apps work with the latest updates. Without the validation host pool, you may not discover changes that introduce errors, which could result in downtime for  users in your production environment.
...
## Create your validation host pool
