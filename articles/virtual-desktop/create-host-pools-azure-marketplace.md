...
3. Specify the type of credentials you want to use to authenticate as the Windows Virtual Desktop tenant RDS Owner. If you completed the [Create service principals and role assignments with PowerShell tutorial](./create-service-principal-role-powershell.mcd), select **Service principal**. You will notice that the **Azure AD tenant ID** associated with the service principal is required.
