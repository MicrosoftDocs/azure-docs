...
>[!NOTE]
>This list refers to the list of regions where the *metadata* for the host pool will be stored.  Session hosts in a host pool can be located in any region, as well as on-premises when using [Azure Virtual Desktop on Azure Stack HCI](azure-stack-hci-overview.md).
