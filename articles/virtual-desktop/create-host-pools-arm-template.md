...
2. If deploying in an Enterprise subscription, perform the following steps to deploy to Azure:
    1. Scroll down and select **Deploy to Azure**.
    2. Skip ahead to step 4.
3. Or, if deploying in a CSP subscription, perform the following steps to deploy to Azure:
...
4.	Fill out the following parameters based on the source of the image you want to use for the virtual machines in the host pool:
    1.	Azure Gallery
        ...
        4.	For **Rdsh is 1809 Or Later**, select the appropriate value based on the image. Note that Windows Server 2019 is an 1809 release.
    ...
    3.	Azure Image resource you have created from a VHD file or captured from a virtual machine:
        ...
        5.	For **Rdsh is 1809 Or Later**, select the appropriate value based on the image. Note that Windows Server 2019 is an 1809 release.
        6.	You can then leave the following parameters empty or unchanged:
            1.	**Vm Image Vhd Uri**
...
5.	Enter or change additional information regarding the virtual machines:
    ...
    3.	For **Rdsh VM Disk Type**, select the disk type you would like to use for the virtual machines. If you selected **CustomVHD** as the **Rdsh Image Source** and **false** for **Rdsh Use Managed Disks**, ensure that this parameter matches the storage account type where the image is located.
6.	Enter the appropriate domain and network properties:
    1.	For **Domain To Join**, enter the name of the domain (such as “contoso.com”) you would like the virtual machines to join.
    2.	For **Existing Domain UPN**, enter the UPN of a user (such as “vmadmin@contoso.com”) that has appropriate permissions to join the virtual machines to the domain and organizational unit.
    3.	For **Existing Domain Password**, enter the domain password for the defined **Existing Domain UPN**.
...
7.	Enter the appropriate information to connect and authenticate to Windows Virtual Desktop:
    ...
    4.	For **Host Pool Name**, enter the name of the Windows Virtual Desktop host pool you would like to register these virtual machines to once they are created. You can enter the name of an existing host pool or the name of a new host pool that will be created as part of this template.
    5.	For **Tenant Admin Upn or Application Id**, enter the appropriate UPN or Application ID that will be used to authenticate to Windows Virtual Desktop. If you are creating a new host pool, this principal must be assigned either the *RDS Owner* or *RDS Contributor* role at the Windows Virtual Desktop tenant scope, at minimum. If you are adding these virtual machines to an existing host pool, this principal must be assigned either the *RDS Owner* or *RDS Contributor* role at the Windows Virtual Desktop host pool scope, at minimum.
    > [!Warning]
    You cannot enter a UPN that requires MFA to successfully authenticate. If you do, this template will create the virtual machines but fail to connect them to a Windows Virtual Desktop host pool.
...
After the GitHub ARM template completes, the last step before testing the full session desktops to your virtual machines is assigning user access.
...
```
6.	Set the context to the tenant group specified in the ARM template.
```powershell
Set-RdsContext -TenantGroupName <Tenant Group name>
```
7.	Add users to the desktop application group.
```powershell
...
Users added to the desktop application group can now log in to Windows Virtual Desktop using the supported Remote Desktop clients and see a resource for a session desktop.
