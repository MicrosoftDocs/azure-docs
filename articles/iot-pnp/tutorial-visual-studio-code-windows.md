---
title: Use VS Code IoT Plug and Play Preview tools (Windows) | Microsoft Docs
description: As a device developer, learn about how to use VS Code to create and test a new device capability model for an IoT Plug and Play Preview device on a Windows machine.
author: dominicbetts
ms.author: dobett
ms.date: 03/17/2020
ms.topic: tutorial
...
# As a device builder, I want to use VS Code on Windows to create and test a new device capability model so I can prepare devices that are simple to connect an IoT solution.
---
...
1. Open a command prompt. Execute the following commands to install Vcpkg:
...
1. Run the following commands to build the generated code stub (replacing the placeholder with the directory of your Vcpkg repo):
    ...
    If you're using Visual Studio 2017, use `-G "Visual Studio 15 2017"` in the previous command. If you're using Visual Studio 2015, use `-G "Visual Studio 14 2015"` in the previous command.

    > [!NOTE]
    > If cmake can't find your C++ compiler, you get build errors when you run the previous command. If that happens, try running the command at the [Visual Studio command prompt](https://docs.microsoft.com/dotnet/framework/tools/developer-command-prompt-for-vs).
...
1. At a command prompt, navigate to the **devicemodel** folder where you created the interface and capability model. Then navigate to the **sensorbox_app\\cmake\\Release** folder.
