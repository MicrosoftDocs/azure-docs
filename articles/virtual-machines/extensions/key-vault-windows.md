...
We rely on the default behavior of [PFXImportCertStore API](../../../windows/win32/api/wincrypt/nf-wincrypt-pfximportcertstore.md). By default, if a certificate has Provider Name attribute that matches with CAPI1, certificate will be imported using CAPI1 certificate, else it will be imported using CNG APIs.
