...
The following table describes the options that can be configured for Automated Backup. The actual configuration steps vary depending on whether you use the Azure preview portal or Azure Windows PowerShell commands.
...
## Configure Automated Backup in the Azure preview portal

You can use the Azure preview portal to configure Automated Backup when you create a new SQL Server 2014 Virtual Machine. 
...
The following Azure preview portal screenshot shows these options under **OPTIONAL CONFIGURATION** | **SQL AUTOMATED BACKUP**.

![SQL Automatic Backup configuration in Azure preview portal](./media/virtual-machines-sql-server-automated-backup/IC778483.jpg)
...
![Automated Backup Configuration in Azure preview portal](./media/virtual-machines-sql-server-automated-backup/IC792133.jpg)

>[AZURE.NOTE] When you enable Automated Backup for the first time, Azure configures the SQL Server IaaS Agent in the background. During this time, the Azure preview portal will not show that Automated Backup is configured. Wait several minutes for the agent to be installed, configured. After that the Azure preview portal will reflect the new settings.
