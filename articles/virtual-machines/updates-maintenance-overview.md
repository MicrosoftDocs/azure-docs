...
Update management center (preview) is a new-age unified service in Azure to manage and govern operating system updates (Windows and Linux), both on-premises and other cloud platforms, across hybrid environments from a single dashboard. The new functionality provides granular access controls, flexibility to create schedules, makes updates in real-time, checks updates automatically and much more. The enhanced functionality ensures that the administrators have visibility into the health of all systems in the environment. For more information, see the [Key benefits](../update-center/overview.md#key-benefits)
