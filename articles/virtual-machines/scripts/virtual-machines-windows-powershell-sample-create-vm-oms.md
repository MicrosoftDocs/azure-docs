...
If needed, install the Azure PowerShell using the instruction found [here]( https://docs.microsoft.com/powershell/azureps-cmdlets-docs/), and then run `Login-AzureRmAccount` to create a connection with Azure. Also, you need to update the OMS workspace ID and workspace key.
