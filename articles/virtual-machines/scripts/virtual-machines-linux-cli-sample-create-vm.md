...
This script creates an Azure Virtual Machine with an Ubuntu operating system. Once the script has been successfully run, the virtual Machine can be accessed over SSH. This sample works in Bash. For options on running Azure CLI scripts on Windows, see [Running the Azure CLI in Windows](../virtual-machines-windows-cli-options.md).
