...
If needed, install the Azure CLI using the instruction found [here](/cli/azure/install-azure-cli), and then run `az login` to create a connection with Azure. Also, you will need an existing virtual machine. Update the name and Resource Group of the existing virtual machine in the script sample.
