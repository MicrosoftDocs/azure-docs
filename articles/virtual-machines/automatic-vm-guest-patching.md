...
- During the installation process, this mode will [assess the VM](https://learn.microsoft.com/en-us/rest/api/compute/virtual-machines/assess-patches) for available patches and save the details in [Azure Resource Graph](https://learn.microsoft.com/en-us/azure/update-center/query-logs). (preview).
...
>Azure recommends that [Assessment Mode](https://learn.microsoft.com/en-us/rest/api/compute/virtual-machines/assess-patches) be enabled on a VM even if Azure Orchestration is not enabled for patching. This will allow the platform to assess the VM every 24 hours for any pending updates, and save the details in [Azure Resource Graph](https://learn.microsoft.com/en-us/azure/update-center/query-logs). (preview)
