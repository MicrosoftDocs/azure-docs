...
>Azure recommends that [Assessment Mode](https://learn.microsoft.com/en-us/azure/update-center/assessment-options#periodic-assessment) be enabled on a VM even if Azure Orchestration is not enabled for patching. This will allow the platform to assess the VM every 24 hours for any pending updates, and save the details in [Azure Resource Graph](https://learn.microsoft.com/en-us/azure/update-center/query-logs). (preview)
