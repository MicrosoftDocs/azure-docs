...
One of the benefits of using availability sets over availability zones are improved VM to VM latencies, since VMs in an availability set are allocated in closer proximity. Availability sets come with fault isolation for many possible failures, to minimize single points of failure, and to offer high availability. Availability sets are still susceptible to certain shared infrastructure failures, like if the datacenter's network fails, which can impact multiple fault domains.

If you need more reliability than availability sets, use [availability zones](availability.md#availability-zones). With availability zones, each VM is deployed in multiple datacenters, protecting you from loss of either power, networking, or cooling in an individual datacenter. If your highest priority is the best reliability for your workload, replicate your VMs across multiple availability zones.
