...
4. Use snapshot to create a new diskless VM (such as, Dv4, Dsv4, Ev4, Esv4 series)[Azure VM sizes with no local temporary disk](azure-vms-no-temp-disk.md) by following the steps outlined in [Create a virtual machine from a snapshot with CLI](./scripts/virtual-machines-linux-cli-sample-create-vm-from-snapshot.md). 
