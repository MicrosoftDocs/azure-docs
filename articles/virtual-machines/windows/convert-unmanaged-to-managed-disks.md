...
Test the migration process by migrating a test virtual machine before performing the migration in production because the migration process is not reversible. Please also note that the Control plane is locked during the conversion, which means you can't Start/Stop/Delete the VM.
