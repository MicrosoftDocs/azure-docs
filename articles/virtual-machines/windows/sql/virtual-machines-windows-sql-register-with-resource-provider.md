...
### Azure CLI
  ...
  ```
`ProvisioningState` will show `Succeeded` if registration was successful. 
...
The console app will shut itself down after verifying that there's at least one SQL Server instance running on the VM. Registering with the SQL VM resource provider in lightweight mode will not restart SQL Server and will not create an agent on the VM.
