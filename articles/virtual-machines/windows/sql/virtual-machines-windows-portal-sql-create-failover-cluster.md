...
1. Open the firewall ports.
   ...
   On each virtual machines, open the following ports:
...
1. Configure networking on the virtual machines. 
   ...
   >[!WARNING]
   >In some cases an Azure virtual machine will become non-responsive immediately after you change the network interface settings. If this happens, reboot the virtual machine. 
...
1. [Validate the cluster](http://technet.microsoft.com/library/jj134244.aspx#BKMK_RUN_TESTS).
   ...
   SQL Server setup requires that the cluster has a current validation. The validation that you ran when you created the cluster did not include the storage resources. You need to run the validation after you create the storage volume.


1. Install a SQL Server Failover Cluster.

   On the node that owns all of the cluster resources, [create and configure a single-node SQL Server failover cluster instance](http://msdn.microsoft.com/library/ms179530.aspx).

   After the you create a single-node failover cluster instance, connect to the other node and run **Setup with Add Node**.
...
## Recommendations to installing FCI on Azure VMs
...
Why does this require a load balancer? [Configure the listener](https://technet.microsoft.com/en-us/windows-server-docs/networking/sdn/manage/guest-clustering)

The load balancer for an FCI requires a floating IP address with direct server return. 

## DTC Support

DTC is not supported on failover cluster instances becase the RPC port is not supported by the load balancer.
