...
> [!TIP]
> To understand the VM and SQL pricing for these images, see [Pricing guidance for SQL Server Azure VMs](virtual-machines-windows-sql-server-pricing-guidance.md). To understand how images are updated and removed over time, see the [SQL VM lifecycle](#lifecycle) section of this article.
...
## <a id="lifecycle"></a> SQL VM image lifecycle
Azure only maintains one virtual machine image for each supported operating system, version, and edition combination. This means that over time images are refreshed, and older images are removed. For more information, see the [Images](virtual-machines-windows-sql-server-iaas-faq.md#images) section of the [SQL Server VMs FAQ](virtual-machines-windows-sql-server-iaas-faq.md).
