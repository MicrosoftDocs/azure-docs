...
# example: az sql vm group create -n Cluster -l 'West US' -g SQLVM-RG --image-offer SQL2017-WS2016 --image-sku Enterprise --domain-fqdn domain.com --operator-acc myvmadmin@domain.com --bootstrap-acc myvmadmin@domain.com --service-acc sqlservice@domain.com --sa-key '4Z4/i1Dn8/bpbseyWX' --storage-account 'https://cloudwitness.blob.core.windows.net/'
...
Quickstart Templates to partially automate the deployment of an Always On availability group configuration for SQL Server Virtual Machines in Azure. There are two Azure Quickstart Templates that are used in this process. 
