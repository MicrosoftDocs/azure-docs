...
The selected availability group used in the Azure quickstart template for the AG listener already contains a listener. Either it's physically within the availability group, or it's metadata within the SQL VM resource provider.  Remove the listener by using [PowerShell](#remove-availability-group-listener) before redeploying the **101-sql-vm-aglistener-setup** quickstart template. 
