...
## Limitations
 ...
 - The ability to convert the licensing model is currently only available when starting with a pay-as-you-go SQL Server VM image. If you start with a bring-your-own-license image from the portal, you will not be able to convert that image to pay-as-you-go.
  - Currently, changing the licensing model is only supported for virtual machines deployed using the Resource Manager model. VMs deployed using the classic model are not supported. 
   ...
   - Currently this procedure is supported only on virtual machines with a single NIC (network interface). On virtual machines with more than one NIC, you should first remove one of the NICs (using the Azure portal) before attempting the procedure. If not, you will run into an error similar to the following
The virtual machine '<vmname>' has more than one NICs associated.
 Though you may be able to add the NIC back to the VM after changing the licensing mode, operations done through SQL configuration blade like automatic patching, backup etc. will no longer be considered supported.
