...
Enabling the preview functionality requires a one-time opt-in for the feature *InGuestPatchVMPreview* per subscription. The feature preview for on-demand patch assessment can be enabled following the same preview enablement process followed for automatic VM guest patching as described earlier in the [preview enablement section](automatic-vm-guest-patching.md#requirements-for-enabling-automatic-vm-guest-patching) of document.
