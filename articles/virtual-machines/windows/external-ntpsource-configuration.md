...
Time synchronization in Active Directory should be managed by only allowing the PDC to access an external time source or NTP Server. All other Domain Controllers would then sync time against the PDC. All other members will get their time from the Domain Controller that satisfied that member's authentication request.
...
>[!NOTE]
>This guide focuses on usign the **Group Policy Management** console to perform the configuration. You can achieve the same results by using the Command Prompt, PowerShell, or by modifying the Registry; however those methods are not in scope in this article. 
...
10. In the **Security Filtering** of the newly created GPO highlight the *Authenticated Users* group -> Click the button *Remove* -> *OK* -> *OK*
11. In the **Security Filtering** of the newly created GPO click the button *Add* and browse for the computer object that holds the PDC role in your domain, then click *OK*
...
>[!TIP]
>If you want to speed-up the process of changing the NTP source on your PDC, from an elevated command prompt run *gpupdate /force*, followd by *w32tm /resync /nowait*, then rerun *w32tm /query /source*; the output should be the NTP Server you used in the above GPO.
...
4. Right-click on the newly created GPO and Edit.
5. Navigate to *Computer Configuration* -> *Preferences* -> Right-clink on *Registry* -> *New* -> *Registry Item*
