...
Force Delete allows you to forcefully delete your virtual machine, reducing delete latency and immediately freeing up attached resources. Force Delete should only be used when you are not intended to re-use the virtual hard disks. You can use Force Delete through Portal, CLI, PowerShell, and Rest API.
...
Use the `--force-deletion` parameter for [az vm delete](/cli/azure/vm?view=azure-cli-latest#az-vm-delete). 
...
Use the `--force-deletion` parameter for [az vmss delete](/cli/azure/vmss?view=azure-cli-latest#az-vmss-delete). 
