---
title: Troubleshoot RDP issues for Windows VMs in Azure | Microsoft Docs
description: Troubleshoot RDP issues in Azure for Windows VMs.
services: virtual-machines-windows
documentationcenter: ''
author: genlin
manager: jeconnoc
editor: ''
tags: azure-resource-manager

ms.assetid: 45c69812-d3e4-48de-a98d-39a0f5675777
ms.service: virtual-machines-windows
ms.workload: infrastructure-services
ms.tgt_pltfrm: vm-windows
ms.topic: troubleshooting
ms.date: 12/13/2018
ms.author: genli
---

# Troubleshoot RDP issues

[Reset RDP](reset-rdp.md)
[RDP troubleshooting](troubleshoot-rdp-connection.md)
[Detailed RDP troubleshooting](detailed-troubleshoot-rdp.md)
[Troubleshoot RDP error because the DHCP is disabled](troubleshoot-rdp-dhcp-disabled.md)
[Troubleshoot RDP error because of the NSG setting](troubleshoot-rdp-nsg-problem.md)
[Troubleshoot specific errors](troubleshoot-specific-rdp-errors.md)
[Troubleshoot no license server errors](troubleshoot-rdp-no-license-server.md)
[Troubleshoot remote desktop services issues](troubleshoot-remote-desktop-services-issues.md)
[Troubleshoot An internal error](Troubleshoot-rdp-internal-error.md)
[Troubleshoot connection disconnects frequently](troubleshoot-rdp-intermittent-connectivity.md)
[Troubleshoot a general error](troubleshoot-rdp-general-error.md)
[Troubleshoot authentication errors](troubleshoot-authentication-error-rdp-vm.md)
[Troubleshoot Azure VM RDP connection issues by Event ID](event-id-troubleshoot-vm-rdp-connecton.md)
[Troubleshoot RDP error in VM because of static IP](troubleshoot-rdp-static-ip.md)
[Troubleshoot RDP error in VM because the NIC is disabled](troubleshoot-rdp-nic-disabled.md)
[Troubleshoot RDP error caused by Safe Mode](troubleshoot-rdp-safe-mode.md)
[Disable the guest OS Firewall in Azure VM](disable-guest-os-firewall-windows.md)
[Enable or disable a firewall rule on a guest OS](enable-disable-firewall-rule-guest-os.md)
[Guest OS firewall is blocking inbound traffic](guest-os-firewall-blocking-inbound-traffic.md)
[Guest OS firewall is misconfigured](guest-os-firewall-misconfigured.md) 
[Troubleshoot RDP error caused by netvsc.sys](troubleshoot-rdp-driver-netvsc.md)

