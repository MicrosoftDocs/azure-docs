...
# VM is unresponsive when applying Group Policy Local Users & Groups

This article provides steps to resolve issues where the load screen doesn't respond when applying a policy during startup in an Azure Virtual Machine (VM).
...
1. Locate the key **HKEY_LOCAL_MACHINE** and select **File** > **Load Hive** from the menu.
    ...
    - Load Hive enables you to load registry keys from an offline system, in this case the broken disk attached to the repair VM.
...
1. In the attached disk, go to the `\windows\system32\config\SOFTWARE` file and open it.
    ...
    1. When prompted for a name, enter BROKENSOFTWARE.
...
To enable memory dump collection and the Azure Serial Console, run this script:

1. Open an elevated command prompt session. (Run as administrator.)
1. Run these commands to enable Serial Console:
...
1. Verify if the free space on the OS disk is at least equal to the VM’s memory size (RAM).
    ...
    If there isn’t enough space on the OS disk, change the memory dump location and refer it to an attached data disk with enough free space. To change the location, replace “%SystemRoot%” with the drive letter (e.g., “F:”) of the data disk in the following commands.
