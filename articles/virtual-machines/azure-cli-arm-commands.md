...
>[AZURE.TIP]Starting with CLI version 0.10, you can provide a short alias such as `UbuntuLTS` or `Win2012R2Datacenter` for some popular Marketplace images. Run `azure help vm quick-create` for options. Additionally, starting with version 0.10, `azure vm quick-create` uses premium storage by default if it's available in the selected region.
