...
If you intend to use the specialized VHD as-is to create a new VM, ensure the following steps are completed. 
  * If you are going to use Managed Disks, review [Plan for the migration to Managed Disks](windows/on-prem-to-azure.md#plan-for-the-migration-to-managed-disks).
...
For more information about endpoints and NSG rules, see [Opening ports to a VM in Azure using PowerShell](windows/nsg-quickstart-powershell.md?toc=%2fazure%2fvirtual-machines%2fwindows%2ftoc.json).
...
The specialised OS could be a VHD that you [uploaded to Azure](windows/upload-image.md) or a [copy the VHD from an existing Azure VM](virtual-machines-windows-vhd-copy.md). 
