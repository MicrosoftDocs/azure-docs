...
Most of the VMs contains a temporary disk, which is not a managed disk. The temporary disk provides short-term storage for applications and processes and is intended to only store data such as page or swap files. Data on the temporary disk may be lost during a [maintenance event](~/articles/virtual-machines/windows/manage-availability.md) or when you [redeploy a VM](~/articles/virtual-machines/troubleshooting/redeploy-to-new-node-windows.md). During a successful standard reboot of the VM, the data on the temporary disk will persist. For more information about VMs without Temporary disk, see [Azure VM sizes with no local temporary disk](~/articles/virtual-machines/azure-vms-no-temp-disk.md)
