...
The solutions allow two ways of deploying WLS domains to AKS. It is possible for domains to be deployed directly to Kubernetes Persistent Volumes. This capability is suitable if you wish to migrate to AKS but still want to administer WLS using the Admin Console or the WebLogic Scripting Tool (WLST). The capability also allows you to move to AKS without adopting Docker development. The more Kubernetes native way of deploying WLS domains to AKS is to build custom Docker images based on official WLS images from the Oracle Container Registry, publish the custom images to ACR and deploy the domain to AKS using the Operator. This capability in the solution also allows for post-deployment changes to the domain through Kubernetes ConfigMaps.
