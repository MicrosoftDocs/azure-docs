...
4.  Verify or change the backup file location and size:
    ...
    ```
5. Use RMAN to back up the database:
...
```
### 9. In the following script, RMAN is used to recover the database.
```bash
# sudo su - oracle
$ rman target /
RMAN> startup mount;
...
7.  To restore the VM, in the lower-left corner of the UI, click **Restore**.
...
2.  In the upper-left corner, click **Add**. For **Name**, select the public IP name, for **Resource group**, select **Use existing**, and then click **Create**.
