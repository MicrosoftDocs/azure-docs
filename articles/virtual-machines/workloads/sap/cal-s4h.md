...
The following sequence of screenshots show you how to deploy S/4HANA on Azure using SAP CAL. The process works the same way for other solutions, like BW/4HANA.
...
type your User ID (lower red circle) into the field on the right marked in red and press the button 'add'. This step should associate your account with your user that you use to log into SAP CAL. 
...
- A 'basic' one with one real screen to define the system(s) to be deployed.
- An 'advanced' sequence, where you have certain choices on VM sizes. 

We will demonstrate the 'basic' path to deployment here.

As the first screen (below) appears, you need to:

- Select a CAL account (use an account that is associated to deploy into Azure using ARM)
- Enter an instance name
- Choose an Azure region. Please keep in mind that CAL will suggest a region. If you need another Azure region and you do not have a CAL subscription, you should order a SAP CAL subscription with SAP) 
...
After pressing the button 'Create' (lower right corner), the following screen will appear that you need to confirm with pressing 'OK'

![Screenshot of SAP Cloud Appliance Library Solutions window](./media/cal-s4h/s4h-pic10b.png)

After confirming another screen in regards to your private key will show up. We usually store it in CAL and also download it password protected. 

![Screenshot of SAP Cloud Appliance Library Solutions window](./media/cal-s4h/s4h-pic10c.png)

A last information screen by SAP CAL will appear that you need to confirm

![Screenshot of SAP Cloud Appliance Library Solutions window](./media/cal-s4h/s4h-pic10d.png)

Now the the deployment is taking place, depending on the size and complexity of the solution (an estimate is provided by SAP CAL), after a time, the solution is shown as 'Active' and ready for use.
...
Before you proceed you need to click on the link of the 'Getting Started Guide' before you can take one of the options to connect to the deployed systems. The documentation will name the users for each of the connectivity methods. The passwords for those users are set to the master password you defined at the beginning of the deployment process. In the documentation, other more functional users are listed with their passwords that you can use to log into the deployed system. E.g. using SAP GUI that is preinstalled on the Windows Remote Desktop machine, The S/4 system itself could look like:
