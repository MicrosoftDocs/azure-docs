...
An Azure proximity placement group is a logical construct, that at definition phase is bound to an Azure region and an Azure resource group. During deployment of VMs, a proximity placement group is referenced by:

- The first deployed Azure VM to settle on the datacenter. The first virtual machine can be seen as an anchor VM that gets deployed in a datacenter based on Azure allocation algorithms eventually combined with user definitions for a specific Azure Availability Zone.
...
As you have SAP systems deployed already, you might want to optimize the network latency of some of your critical systems and locate the application layer and the DBMS layer in the same datacenter. In the stage of public preview of proximity placement group functionality, a deletion of the VMs and a new creation of the VMs is necessary to perform such a move into proximity placement groups. At this stage of the functionality, it is not good enough to shut down the VMs to be able to assign those to proximity placement groups.
