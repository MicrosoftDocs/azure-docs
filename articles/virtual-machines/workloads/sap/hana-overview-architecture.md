...
- **Tenant:** Customer deployed in HANA Large Instances stamp gets isolated into a "tenant". A tenant is isolated in the networking, storage and compute layer from other tenants, so, that storage and compute units assigned to the different tenants can not see each other or communicate with each other on the HANA Large Instance stamp level.
