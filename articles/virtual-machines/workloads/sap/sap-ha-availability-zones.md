...
- Deploying with an availability set is lining up the VMs within the set in a single zone or datacenter (whatever applies for the specific region). As a result the deployment through the availability set is not protected by outside power, cooling or networking issues. On the plus side, the VMs are aligned with update and fault domains within that zone or datacenter. Specifically for the SAP ASCS or DBMS layer where we protect two VMs per availability set, the alignment with fault and update domains prevents that both VMs are ending up on the same host hardware 
...
- The pair of VMs running ASCS/SCS and the pair of VMS running the DBMS layer are distributed across two zones. The number of VMs running the SAP application layer are deployed to an even number across the same two zones. If a DBMS or ASCS/SCS VM is failing over, some of the open and active transactions might be rolled back. but users are remaining logged in. This architecture is the preferred architecture to deploy across zones.
- The pair of VMs running ASCS/SCS and the pair of VMS running the DBMS layer are distributed across two zones. The number of VMs running the SAP application layer are deployed into one of the Availability Zones. Ideally you run the application layer in the same zone as the active ASCS/SCS and DBMS instance. You usually use this deployment architecture if the network latency across the different zones is too high to run the application layer distributed across the zones. If an ASCS/SCS or DBMS VM failing over, you might encounter higher network latency and with that a reduction of throughput. Or you fail the application layer over to the other zone. An activity that users experience as complete system shutdown. This is the less preferred method of deployment. However, in some Azure regions the only viable architecture when you want to use Availability Zones, instead of staying with availability sets.
