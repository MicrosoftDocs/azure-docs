...
- Azure availability zones are not an ideal DR solution. Natural disasters can impact whole regions and cause heavy damage to power infrastructure. The distance between different zones might be not large enough to qualify as a proper DR solution.
- The network latency across availability zones varies in the different Azure regions. You can deploy and run the SAP application layer across different zones, where the network latency from one zone to the active DBMS VM is acceptable from a business process impact. But in some Azure regions the latency between the active DBMS VM and the SAP application instance, deployed in different zones, can be too intrusive and unacceptable for the SAP business processes. As a result, the deployment architecture needs to be different with an active/active architecture for the application or active/passive architecture where cross zone latency is too high.
