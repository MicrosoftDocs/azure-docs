...
What is the difference between Azure Availability Sets and Availability Zones? For Azure Regions where Availability Zones are going to be offered, the regions will have multiple data centers which can be in a distance far enough to be considered as valid disaster recovery location. Hence the Azure data centers would be grouped into so called 'Availability Zones'. You as a customer are going to have the possibility to deploy into a specific Availability Zone. Think Azure Availability Sets as a grouping which works either within one Azure Region that does not support Availability Zones or a construct within an Azure Availability Zone where you can make sure that two or multiple VMs are getting deployed into different fault and update domains, so, that not all VMs are affected by eventual issues or updates to host nodes. So far for SAP HANA, we will focus on deployment scenarios in Availability Sets only.   
