...
The RPO time depends on the frequency of the transaction log backups and the copy of those to the secondary host. The RTO time is dependent on the way you set up the restore. The more recent the backups are you restore, the smaller the RTO time. this method can be used to hang back for a few hours. You can restore back to a certain point in time in case you need to recover specific data within a specific table that got deleted by a user or administrator error. The disadvantage is that you need to have a full VM running and that you need to script quite a lot around a solution like this.
