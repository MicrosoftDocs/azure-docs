...
The /hana/log volume isn't replicated because the SAP HANA transaction log isn't needed when restoring from those volumes. 

The basis of the disaster recovery functionality offered is the storage replication offered by the HANA Large Instance infrastructure. The functionality used on the storage side isn't a constant stream of changes that replicate in an asynchronous manner as changes happen to the storage volume. Instead, it's a mechanism that relies on snapshots of these volumes being created on a regular basis. The delta between an already replicated snapshot and a new snapshot that isn't yet replicated is then transferred to the disaster recovery site into target disk volumes.  These snapshots are stored on the volumes and, if there's a disaster recovery failover, need to be restored on those volumes.  
...
If there's a failover, you also can choose to restore to an older storage snapshot instead of the latest storage snapshot.
