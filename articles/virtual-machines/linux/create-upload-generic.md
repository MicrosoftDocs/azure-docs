...
> [!NOTE]
> Make sure **'udf'** and **'vfat'** modules are enable. Blacklisting the udf module will cause a provisioning failure and blacklisting vfat module will cause both provisioning and boot failures. Cloud-init >= 21.2 can provision VMs without requiring UDF if: 1) the VM was created using SSH public keys and not password, 2) no custom data was provided
> 
