...
* Supporting first party Azure services, such as Azure Backup, Monitoring, Disk Encrytion, Security, Site Replication etc.
* SSH / Password resets
* VM configuration - Running custom scripts, installing Chef, Puppet agents etc..
* Third party products, such as AV products,VM vulnerability tools, VM and App monitoring tooling etc.
...
In a typical scenario of preparing custom images with the Azure Linux Agent, the step "waagent -deprovision+user" is recommended. Some of the actions performed by that step is listed below: 
...
After disabling the Linux Agent, the user can create a custom image by running the following steps using az cli:
...
**Create an image verison in a Shared Image Gallery**
...
To deploy the VM with extensions disabled, uou can use the Azure CLI with [--enable-agent](https://docs.microsoft.com/cli/azure/vm#az-vm-create).
