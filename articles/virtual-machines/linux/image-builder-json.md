...
> [!IMPORTANT]
> You will receive an authorization error when trying to specify a pre-existing resource group and VNet to the Azure Image Builder service with a Windows source image. You will need to assign the contributor role to the resource group for the service principal corresponding to Azure Image Builder's first party app by using the CLI command or portal instructions shown in the following documentation [Troubleshoot VM Azure Image Builder: Authorization error creating disk](https://learn.microsoft.com/en-us/azure/virtual-machines/linux/image-builder-troubleshoot#authorization-error-creating-disk)
