...
The SAP HANA Azure backup guide will only describe Azure-specific topics. For general SAP HANA backup related items one should check the SAP HANA documentation. See _SAP HANA backup documentation_ later in this document.
...
The following three figures give an overview of the SAP HANA backup options using native Azure capabilities at this point in time, and also show three potential future backup scenarios. The following sections of this guide describe these options in more detail, and also discuss size and performance considerations regarding SAP HANA backups which are of multi-terabytes in size.

![This figure shows two possibilities for saving the current state of the whole VM](./media/sap-hana-backup-guide/image001.png)
...
This figure shows a potential future SAP HANA backup scenario. If SAP HANA were to allow taking backups from a replication secondary, it would add additional options for backup strategies. Right now this isn&#39;t possible according to a post in the SAP HANA Wiki, which states:
...
In both cases, one has to think about copying the backups to another location afterwards. For example, for additional safety via geo-redundancy (see [Azure Storage replication](../../storage/storage-redundancy.md?toc=%2fazure%2fvirtual-machines%2flinux%2ftoc.json)), or simply because of space limitations within the Azure Linux VM or using cool blob storage for cost reasons (see [Azure Blob Storage: Hot and cool storage tiers](../../storage/storage-blob-storage-tiers.md?toc=%2fazure%2fvirtual-machines%2flinux%2ftoc.json)).
...
- In the case of storage snapshots, the same procedure should always be used no matter if there is a single disk, or a software raid across multiple disks, to guarantee file system and SAP HANA data consistency

See detailed discussions about topics related to SAP HANA backup on Azure later in this document.
...
As a minimum for &quot;backup&quot; one could therefore just rely on the Azure SLAs and keep the SAP HANA data and log files on Azure VHD data disks attached to the SAP HANA server VM. This would cover VM failures, but not potential damage of the SAP HANA data and log files on OS level, or logical errors like deleting data or files by accident. (Not to mention compliance or legal reasons to keep backups.)  There will always be a need for SAP HANA backups.

## How to verify correctness of SAP HANA backup

When using storage snapshots, it is recommended to run a test restore on a different system to be 100% sure that a backup is correct, and that internal processes for backup and restore work as expected. While this is a big hurdle on-premises, it is much easier to accomplish this in the cloud by providing necessary resources temporarily for this purpose.

Keep in mind that doing a simple restore and checking if HANA is up and running is not sufficient. Ideally, one should, at least, run a table consistency check to be sure that the restored database is fine. SAP HANA offers several kinds of consistency checks described in [SAP Note 1977584](https://launchpad.support.sap.com/#/notes/1977584).
...
In order to go completely without storage snapshots this part could be replaced by HANA
delta backups, like incremental or differential backups (see [Delta Backups](https://help.sap.com/saphelp_hanaplatform/helpdata/en/c3/bb7e33bb571014a03eeabba4e37541/content.htm)).
...
The HANA Studio backup console allows one to restrict the max file size of HANA backup files. In the sample environment this gives the possibility of finally getting multiple smaller backup files instead of one big 230 GB backup file. This has a significant impact on performance, as is demonstrated later in this document.
...
A click on the backup job in the Azure portal shows the job details. Here, one can see the two phases. It might take a few minutes until it shows the snapshot phase as completed. The majority of the time will be spent in the data transfer phase.

### HANA VM backup automation via Azure Backup service

One could confirm the SAP HANA snapshot once the Azure Backup snapshot phase is completed, as described earlier. It is helpful to consider automation, because an admin will not check this by looking at the backup job list in the Azure portal.
...
**Get-AzureRmRecoveryServicesVault -Name hana-backup-vault** will retrieve the corresponding object. This object will then be used to set the backup context as seen on the next figure.

![One can check for the backup job which is currently in progress](./media/sap-hana-backup-guide/image017.png)

After setting the correct context, one can check for the backup job which is currently in progress, and then look for its job details. The subtask list will show if the snapshot phase of the Azure backup job is already completed:
```
...
During the creation of this backup guide, a new Azure Backup feature was presented in preview mode. Because there was no time to test it out, this document will be updated later, once this
feature is GA. It will allow a file level restore based on the VM snapshot that was taken for the VM backup. This would avoid the need to deploy a new VM, and therefore the unique VM ID would stay the same and no new SAP HANA license key would be required.

Azure backup will eventually allow for backup of individual Azure virtual disks, plus files and directories from inside the VM. A big advantage of Azure Backup is the management of all the backups. Otherwise it is the task of the customer to take care of the bookkeeping of the list of backups, and to pick the right ones in case a restore is necessary.
