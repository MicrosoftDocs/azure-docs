...
## Cloud-init support for VM provisioning without a UDF driver  
Starting with cloud-init 21.2, cloud-init supports VM provisioning on Azure without requiring a UDF driver. If a UDF driver isn't available in the image, cloud-init uses the metadata that's available in the Azure Instance Metadata Service to provision the VM. Note that this option works only for SSH key and [user data](../user-data.md). To pass in password or custom data to a VM during provisioning, you must enable a UDF driver.
