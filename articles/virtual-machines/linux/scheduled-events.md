...
| DurationInSeconds | The estimated duration of the interruption caused by the event.  The actual duration may be shorter than the estimate, but will not exceed it.  <br><br> Example: <br><ul><li> `9`: The interruption caused by the event will last for 9 seconds. <li>`-1`: The default value used if the impact duration is either unknown or not applicable. |
