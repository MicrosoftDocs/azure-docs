---
title: Azure Disk Storage Disk overview for Linux VMs 
...
Azure Disks are a block-level offering of Azure Storage, primarily, Azure Disks offering are managed disks, which can be thought of like a virtual hard disk (VHD) that is managed by Azure. You can think of it like a physical disk in an on-premises server but, virtualized. Azure managed disks are stored as page blobs, which are a random IO storage object in Azure. We call a managed disk ‘managed’ because it is an abstraction over page blobs, blob containers, and Azure storage accounts. With managed disks, all you have to do is provision the disk, and Azure takes care of the rest.
