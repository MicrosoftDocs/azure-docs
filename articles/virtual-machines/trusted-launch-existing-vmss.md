...
- Register Preview Feature `ImageSkuGenUpdateWithVMSS` on subscription.

### [Portal](#tab/portal)

1. Navigate to Subscription page -> **Preview features** and search for `ImageSkuGenUpdateWithVMSS`.
2. Select the `ImageSku GenUpdate With VMSS` feature and click **Register**.

    :::image type="content" source="./media/trusted-launch/00-enable-tl-vmss-feature-registration.png" alt-text="Screenshot of enabling required subscription feature.":::

### [Template](#tab/template)

Use `Microsoft.Features/providers/features` in ARM template to register feature `Microsoft.Compute/ImageSkuGenUpdateWithVMSS`
  ...
  "resources": [
    {
      ...
      "name": "Microsoft.Compute/ImageSkuGenUpdateWithVMSS",
      "properties": {
        "state": "Registered"
      }
    }
...
2. Register feature `Microsoft.Compute/ImageSkuGenUpdateWithVMSS` using command [az feature registration create](/cli/azure/reference-index)

    ```azurecli-interactive
    az feature registration create --namespace Microsoft.Compute --name ImageSkuGenUpdateWithVMSS
    ```
...
2. Register feature `Microsoft.Compute/ImageSkuGenUpdateWithVMSS` using command [Register-AzProviderFeature](/powershell/module/az.compute/register-azproviderfeature)

    ```azurepowershell-interactive
    Register-AzProviderFeature -FeatureName ImageSkuGenUpdateWithVMSS -ProviderNamespace Microsoft.Compute
    ```
