<properties
	...
	description="Learn to set up endpoints in the Azure preview portal to allow communication with a virtual machine in Azure."
...
All virtual machines that you create in Azure can automatically communicate with other virtual machines in the same cloud services or virtual network by using a private network channel. However, computers on the Internet or other virtual networks require endpoints to direct the inbound network traffic to a virtual machine.

When you create a virtual machine in the Azure management portal, endpoints for Remote Desktop, Windows PowerShell Remoting, and Secure Shell (SSH) are created automatically. You can configure additional endpoints while creating the virtual machine or after you created it, as needed.
...
5.	On the **Add an endpoint to a virtual machine** page, select the type of endpoint.

	- If you're creating a new endpoint that is not part of a load-balanced set or the first member in a new load-balanced set, click **Add a stand-alone endpoint**, and then click the left arrow.
	- Otherwise, click **Add an endpoint to an existing load-balanced set**, select the name of the load-balanced set, and then click the left arrow. On the **Specify the details of the endpoint** page, type a name for the endpoint in **Name**, and then click the check mark to create the endpoint.
...
7.	For a customized endpoint, in **Protocol**, select either **TCP** or **UDP**.
...
To define the set of computers that can send traffic, the ACL on an endpoint can restrict traffic based on source IP address. Follow these steps to add, modify, or remove an ACL on an endpoint.
...
5.	In the taskbar, click **Manage ACL**. The **Specify ACL details** dialog box appears.

    ![Specify ACL details](./media/virtual-machines-set-up-endpoints/EndpointACLdetails.png)

6.	Use rows in the list to add, delete, or edit rules for an ACL and change their order. The **Remote Subnet** value is an IP address range for incoming traffic from the Internet that the Azure load balancer either permits or denies based on the traffic's source IP address. You must specify the IP address range in CIDR format, also known as address prefix format. An example is 131.107.0.0/16.
