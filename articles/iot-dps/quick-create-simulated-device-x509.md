...
2. The sample code is set up to use X.509 certificates that are stored within a password-protected PKCS12 formatted file (`certificate.pfx`). Additionally, you'll need a public key certificate file (`certificate.cer`) to create an individual enrollment later in this quickstart. To generate the self-signed certificate and its associated `.cer` and `.pfx` files, run the following command:
    ...
    The certificate generated by this command has a subject common name (CN) of _iothubx509device1_. For an individual enrollment, the [Registration ID](./concepts-service.md#registration-id) is set to the common name. The registration ID is a case-insensitive string (up to 128 characters long) of alphanumeric characters plus the special characters: `'-'`, `'.'`, `'_'`, `':'`. The Last character must be alphanumeric or dash (`'-'`). The common name must adhere to this convention.
...
2. Create a _leaf_ X.509 certificate by running the script using your own _certificate-name_. For an individual enrollment, the leaf certificate's common name becomes the [Registration ID](./concepts-service.md#registration-id). The registration ID is a case-insensitive string (up to 128 characters long) of alphanumeric characters plus the special characters: `'-'`, `'.'`, `'_'`, `':'`. The Last character must be alphanumeric or dash (`'-'`). The _certificate-name_ parameter must adhere to this convention.
...
The certificate file has its subject common name (CN) set to `Python-device-01`. For an individual enrollment, the [Registration ID](./concepts-service.md#registration-id) is set to the common name. The registration ID for an enrollment is a case-insensitive string (up to 128 characters long) of alphanumeric characters plus the special characters: `'-'`, `'.'`, `'_'`, `':'`. The Last character must be alphanumeric or dash (`'-'`). The common name must adhere to this convention.
...
3. Enter **N** for _Do you want to input common name_. This creates a certificate with a subject common name (CN) of _microsoftriotcore_.
    ...
    For an individual enrollment, the [Registration ID](./concepts-service.md#registration-id) is set to the common name. The registration ID is a case-insensitive string (up to 128 characters long) of alphanumeric characters plus the special characters: `'-'`, `'.'`, `'_'`, `':'`. The Last character must be alphanumeric or dash (`'-'`). The common name must adhere to this convention.
