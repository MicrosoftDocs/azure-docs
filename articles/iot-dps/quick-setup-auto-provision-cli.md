...
The Azure CLI is used to create and manage Azure resources from the command line or in scripts. This QuickStart details using the Azure CLI to create an IoT hub and a new IoT Hub Device Provisioning Service, and link the two services together. 

If you don't have an Azure subscription, create a [free account](https://azure.microsoft.com/free/?WT.mc_id=A261C142F) before you begin.

> [!IMPORTANT]
> Both the IoT hub and the provisioning service you create in this QuickStart will be publicly discoverable as DNS endpoints, so make sure to avoid any sensitive information if you decide to change the names used in this QuickStart.
>
...
Create an IoT IoT hub with the [az iot hub create](/cli/azure/iot#az_iot_hub_create) command. 
...
Create a provisioning service with the [az iot hub create](/cli/azure/iot/dps#az_iot_dps_create) command. 
...
> [!TIP]
> The previous example creates the provisioning service in the West US location. You can view a list of available locations by running the command `az provider show --namespace Microsoft.Devices --query "resourceTypes[?resourceType=='ProvisioningServices'].locations | [0]" --out table` or by going to the [Azure Status](https://azure.microsoft.com/en-us/status/) page and searching for "Device Provisioning Service".
>


## Get the connection string for your IoT hub

To link your IoT Hub to the device provisioning service, you need the hub's connection string. Use the [az iot hub show-connection-string](/cli/azure/iot/hub#az_iot_hub_show_connection_string) command to get the connection string and set a variable that you will use when you link the two resources. 

The following example sets the *hubConnectionString* variable to the value of the connection string for the primary key of the hub's *iothubowner* policy . If desired, you can specify a different policy with the `--policy-name` parameter. The command uses the Azure CLI [query](/cli/azure/query-azure-cli) and [output](/cli/azure/format-output-azure-cli#tsv-output-format) options to extract the connection string from the command output. 
...
The following example links an IoT hub named *my-sample-hub* in the *westus* location to a device provisioning service named "my-sample-dps". It uses the connection string for *my-sample-hub* stored in the *hubConnectionString* variable.  
...
Show the details of your provisioning service with the [az iot dps show --name](/cli/azure/iot/dps#az_iot_dps_show) command.
