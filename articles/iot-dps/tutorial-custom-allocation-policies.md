...
The IoT extension for the Azure CLI provides the [`compute-device-key`](../..cli/azure/iot/dps?view=azure-cli-latest.md#az_iot_dps_compute_device_key) command for generating derived device keys. This command can be used on Windows-based workstations and Linux systems from a PowerShell or Bash shell.
