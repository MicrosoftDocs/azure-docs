...
Symmetric key attestation with the Device Provisioning Service is performed using the same [Security tokens](../iot-hub/iot-hub-devguide-sas.md#security-token-structure) supported by IoT hubs to identify devices. These security tokens are [Shared Access Signature (SAS) tokens](../service-bus-messaging/service-bus-sas.md). 
...
For code examples that create a SAS token, see [Security Tokens](../iot-hub/iot-hub-devguide-sas.md#security-token-structure).
