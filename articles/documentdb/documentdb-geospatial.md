...
How is spatial data indexed in DocumentDB? In a nutshell, the 3D ellipsoid surface of the Earth is projected onto a 2D plane and divided progressively into cells using a **quadtree**. Then these cells are mapped to 1D (single cell ID) based on the location of the cell within a **Hilbert space filling curve**, which preserves locality of points in their original 3D representations. Additionally when location data is indexed, it goes through a process known as tessellation, i.e. all the cells that intersect a location are identified, then hashed and stored within the DocumentDB index.  At query time, query arguments like points and polygons are also tessellated to extract the relevant cell IDs, then used to match against the data retrieved from the index.
