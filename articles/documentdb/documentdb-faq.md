...
DocumentDB supports optimistic concurrency control (OCC) through HTTP entity tags or etags. Every DocumentDB resource has an etag, and the etag is set on the server every time a document is updated. The etag header and the current value are included in all response messages. To check for consistency, the client must include the most recent value in the If-Match header in the next request to modify the resource. If the etag provided by the client in the write request matches the server etag value, the change is committed. If the value has been changed externally, the server rejects the write with an "HTTP 412 Precondition failure" response code. Clients must read the latest version of the resource and retry the request. 
