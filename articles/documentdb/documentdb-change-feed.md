...
![Azure Cosmos DB based lambda pipeline for ingestion and query](./media/documentdb-change-feed/lambda.png)
...
![Distributed processing of Azure Cosmos DB change feed](./media/documentdb-change-feed/changefeedvisual.png)
...
Azure Cosmos DB provides elastic containers of storage and throughput called **collections**. Data within collections is logically grouped using [partition keys](documentdb-partition-data.md) for scalability and performance. Azure Cosmos DB provides various APIs for accessing this data, including lookup by ID (Read/Get), query, and read-feeds (scans). The change feed can be obtained by populating two new request headers to the DocumentDB `ReadDocumentFeed` API, and can be processed in parallel across ranges of partition keys.
...
![Using the Azure Cosmos DB change feed processor host](./media/documentdb-change-feed/changefeedprocessor.png)
...
* Learn more about [Azure Cosmos DB's resource model and hierarchy](documentdb-resources.md)
