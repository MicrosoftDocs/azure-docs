---
title: Install Azure IoT OPC UA Broker Preview
description: How to install Azure IoT OPC UA Broker Preview using helm
author: timlt
ms.author: timlt
...
> [!NOTE]
> In the OPC UA Broker release, the broker does not re-balance endpoint profiles or assets after the initial assignment. OPC UA Broker also does not delete Connector deployments that are no longer used.
...
> [!NOTE]
> It's not possible to define a publishing interval on asset level in the OPC UA Broker. OPC UA Broker assumes publishing for data points and events each second. You can adjust `opcUaConnector.maxDataPointsPerSecond` if needed.
