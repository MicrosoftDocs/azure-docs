...
| Secret | string | The [secret reference](../deploy-azure-iot-operations/howto-manage-secrets.md) for the password to use when `Authentication` is set to `Username/Password`. | No | - | `mysecret` |
...
| Secret | string | The [secret reference](../deploy-azure-iot-operations/howto-manage-secrets.md) to use when `Authentication` is set to `Header`. | No | - | `mysecret` |
