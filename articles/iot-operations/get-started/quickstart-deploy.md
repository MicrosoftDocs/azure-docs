...
* An Azure Arc-enabled Kubernetes cluster. If you don't have one, follow the steps in [Prepare your Azure Arc-enabled Kubernetes cluster](../deploy-iot-ops/howto-prepare-cluster.md?tabs=codespaces). Using the Github codespace is the simplest way to get a new Kubernetes cluster, but you can use an existing environment like K3s or AKS Edge Essentials.
