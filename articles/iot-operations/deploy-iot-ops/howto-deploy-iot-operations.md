...
1. On the **Configuration** tab, provide the following information:
   ...
   | **MQTT broker configuration** | *Optional*: Edit the default settings for the MQTT broker. In Azure portal it's possible to configure [cardinality](./deployment-planning.md#understand-broker-cardinality) and [memory profile](./deployment-planning.md#choose-your-memory-profile) settings. To configure other settings including [disk-backed message buffer](./deployment-planning-disk-buffer.md), [persistence](./deployment-planning-persistence.md), [diagnostics](./deployment-planning-diagnostics.md), and [advanced MQTT client options](./deployment-planning-mqtt-options.md), see [Azure CLI support for advanced MQTT broker configuration](https://aka.ms/aziotops-broker-config). |
...
- [Enable secure settings](./howto-enable-secure-settings.md) to set up secrets management and managed identities.
