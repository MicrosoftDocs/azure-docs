...
- **Azure CLI**. The Azure CLI flow assumes the certificate is already stored as a secret in Azure Key Vault. You use `az iot ops secretsync secret set` to create a synced secret on the cluster that references the Key Vault secret, and then `az iot ops connector template update` to add a reference to the synced secret in the connector template's trust list. To learn more, see [Use CLI commands to create certificates](../secure-iot-ops/howto-manage-certificates.md#use-cli-commands-to-create-certificates). To learn how to add a certificate to Azure Key Vault, see [Add certificates as secrets to Azure Key Vault](../secure-iot-ops/howto-manage-certificates.md#add-certificates-as-secrets-to-azure-key-vault).
