...
Before you begin, [install or deploy IoT Operations](../get-started/quickstart-deploy.md). Then, use the following options to test connectivity to IoT MQ with MQTT clients.
...
Then use `kubectl apply -f client.yaml` to deploy it. It should only take a few seconds to start.

Once the pod is running, you can use `kubectl exec` to run commands inside the pod. For example, to publish a message to the broker, first get a shell inside the pod:

```bash
kubectl exec --stdin --tty mqtt-client -n azure-iot-operations -- sh
```

Then, inside the pod, run the following command to publish a message to the broker:
...
Here, the mosquitto client uses the service account token mounted at `/var/run/secrets/tokens/mq-sat` to authenticate with the broker. The token is valid for 24 hours. The client also uses the default root CA cert mounted at `/opt/certs/ca.crt` to verify the broker's TLS certificate chain.
...
To use mqttui, the command is similar:
...
> [!WARNING]
> Turning off authentication should only be used for testing purposes with a throwaway cluster that's not accessible from the internet.

### Port connectivity

Some Kubernetes distributions can [expose](https://k3d.io/v5.1.0/usage/exposing_services/) IoT MQ to a port on the host system (localhost). This approach is recommended because it makes it easier for clients on the same host to access IoT MQ. For example, to create a K3d cluster with mapping the IoT MQ's default MQTT port 8883 to localhost:8883:
...
If your cluster - like Azure Kubernetes Service - automatically assign an external IP address to the load balancer, you can use the external IP address to connect to IoT MQ over the internet. Just make sure to use the external IP address instead of `localhost` in the above command, and remove the `--insecure` flag.
...
With [minikube](https://minikube.sigs.k8s.io/docs/), [kind](https://kind.sigs.k8s.io/), and other cluster emulation systems, an external IP might not be automatically assigned (might show as "Pending" state). To access the broker, forward port 8883, the port the broker listens on, to the host.
...
Note the new port 1883.

Then, use port forwarding to access the broker from outside the cluster:

```bash
kubectl port-forward service/aio-mq-dmqtt-frontend 1883:1883 -n azure-iot-operations
```

Then, use mosquitto client to connect to the broker:
