...
1. Ensure the cluster where you deployed Azure IoT Operations in earlier quickstarts is running. If you're using a local cluster that you shut down between quickstarts, start it now. (Cloud clusters such as Azure Kubernetes Service run continuously.) The OPC PLC simulator you deployed with your Azure IoT Operations instance should be running and sending data. You can confirm this step by [verifying that your event hub is receiving messages](quickstart-configure.md#verify-data-is-flowing-to-event-hubs) in the Azure portal.
