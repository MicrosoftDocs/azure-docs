...
## Configure PKI security validation settings

The connector for OPC UA enforces PKI security restrictions by default. If you need to communicate with OPC UA servers that use legacy certificates, you can relax these settings. For example, some older OPC UA servers might use certificates with RSA keys smaller than 2048 bits or certificates signed with the SHA-1 algorithm.

Use the following command to lower the minimum certificate key size and allow SHA-1 signed certificates:

```azurecli
az k8s-extension update --name <azure-iot-operations-extension-name> \
...
> [!WARNING]
> Only relax these settings in test or QA environments where target OPC UA servers use legacy certificates. For production environments, keep the defaults (`minimumCertificateKeySize=2048`, `rejectSha1SignedCertificates=true`).
