...
1. Follow the steps in [Configure TLS with manual certificate management to secure MQTT communication](../administer/mq/howto-configure-tls-manual.md). When creating the certificate, use `azedge-dmqtt-frontend.level-4.com` as one of the Subject Alternative Names (SANs) (in --san option of step CLI).
...
1. Use the guidance in [Connect MQTT bridge cloud connector to other MQTT brokers](../connect-to-cloud/howto-configure-mqtt-bridge.md) to create the MQTT bridge. 
