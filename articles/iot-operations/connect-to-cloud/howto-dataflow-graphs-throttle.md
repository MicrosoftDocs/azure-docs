---
title: Throttle data in data flow graphs
...
```
---
With this rule, the transform forwards the first `sensors/temperature` message it processes, then drops any further messages on that topic that it processes less than 100 milliseconds later (`1000 / 10`). Topics other than `sensors/temperature` aren't affected.
