...
> [!TIP]
> Data flow graphs support enrichment with expanded capabilities, including enrichment in filter and branch transform, and trigger rules for window transforms, which require version 1.1 or later. For new projects that use MQTT, Kafka, or OpenTelemetry endpoints, see [Enrich with external data in data flow graphs](howto-dataflow-graphs-enrich.md).
