...
| mqttSettings.maxInflightMessages         | The max number of messages to keep in flight. For subscribe, it is the receive maximum. For publish, it is the maximum number of messages to send before waiting for an acknowledgement. Default is *100*. |
...
To configure an MQTT broker endpoint with default settings, you can omit the host field, along with other optional fields. This allows you to connect to the default MQTT broker without any additional configuration in a durable way, no matter how the broker changes.
