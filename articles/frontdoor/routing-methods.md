...
If Azure Front Door cannot establish a TCP connection to the selected origin, it makes a best-effort attempt to retry the request against another eligible origin according to the origin group’s routing configuration. This can occur even when health probes report the selected origin as healthy.
