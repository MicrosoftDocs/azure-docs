...
1. On the **Add Route** page, enter, or select the following information.
    ...
    | Origin path | This path is used to rewrite the URL that Azure Front Door will use when constructing the request forwarded to the origin. By default, this path isn't provided. As such, Azure Front Door will copy the incoming URL path to the URL used in the request to the origin. You can also specify a wildcard path, which will copy any matching part of the incoming path to the request path to the origin. The ending `/` is required for origin path to work correctly. </br></br>Pattern to match: `/foo/*`</br>Origin path: `/fwd/`</br></br>Incoming URL path: `/foo/a/b/c/`</br>URL from Azure Front Door to origin: `fwd/a/b/c`. |
