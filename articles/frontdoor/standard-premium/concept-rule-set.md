...
* Support server variables to dynamically change the request/response headers or URL rewrite paths/query strings, for example, when a new page load or when a form is posted. Server variable is currently supported on **[rule set actions](concept-rule-set-actions)** only.
...
:::image type="content" source="media/front-door-rule-set/front-door-rule-set-architecture-1.png" alt-text="Create a web app in the Azure portal":::
...
For more quota limit, refer to [Azure subscription and service limits, quotas and constraints](https://docs.microsoft.com/en-us/azure/azure-resource-manager/management/azure-subscription-service-limits).

* *Rules set*: A set of rules that gets associated to one or multiple [Routes](https://docs.microsoft.com/en-us/azure/frontdoor/front-door-route). Each configuration is limited to 25 rules. You can create up to 10 configurations.
...
* *Match Condition*: There are many match conditions that can be utilized to parse your incoming requests. A rule can contain up to 10 match conditions. Match conditions are evaluated with an **AND** operator. *Regular expression is supported in conditions*. A full list of match conditions can  be found in [Rule Set Condition](front-door-rule-set-match-conditions.md).
...
- Learn how to configure your first [Rule Set](front-door-configure-rule-set). 
- Learn about rule set match conditions and actions [Rule Set Conditions and Actions](front-door-rule-set-conditions-actions.md). 
