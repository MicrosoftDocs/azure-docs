...
description: This articles helps you understand how Azure Front Door Service does URL Rewrite for your routes, if configured.
services: front-door
documentationcenter: ''
author: sharadag
...
Azure Front Door Service (AFD) supports URL rewrite by allowing you to configure an optional **Custom Forwarding Path** to use when constructing the request to forward to the backend. By default, if no custom forwarding path is provided, then AFD will simply copy the incoming URL path to the URL used in the forwarded request. The Host header used in the forwarded request is as configured for the selected backend. Read [Backend Host Header](front-door-backend-pool.md#hostheader) to learn what it does and how you can configure it. 

The powerful part of the custom forwarding path is that it will copy any part of the incoming path that matches to a wildcard path to the forwarded path (this is the **green** segments in the example below):
</br>
