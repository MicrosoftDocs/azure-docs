...
def analyze_document(args):
    ...
    for document in result.documents:
        ...
        elif document.doc_type == "idDocument.nationalIdentityCard":
            process_id_document_national_identity_card(document)
        elif document.doc_type == "idDocument.residencePermit":
            process_id_document_residence_permit(document)
        elif document.doc_type == "idDocument.usSocialSecurityCard":
            process_id_document_us_social_security_card(document)
        elif document.doc_type == "idDocument":
            process_id_document(document)
...
def process_id_document_passport(document):
    ...
    middle_name_field = document.fields.get("MiddleName")
    if middle_name_field:
        print(f"    MiddleName: Value={middle_name_field.value}   Confidence={middle_name_field.confidence}")
    
    last_name_field = document.fields.get("LastName")
    if last_name_field:
        print(f"    LastName: Value={last_name_field.value}   Confidence={last_name_field.confidence}")
    
    aliases_field = document.fields.get("Aliases")
    if aliases_field:
        for index, item in enumerate(aliases_field.value):
            print(f"    Aliases[{index}]: Value={item.value}   Confidence={item.confidence}")
    ...
    document_type_field = document.fields.get("DocumentType")
    if document_type_field:
        print(f"    DocumentType: Value={document_type_field.value}   Confidence={document_type_field.confidence}")
    
    nationality_field = document.fields.get("Nationality")
    if nationality_field:
        print(f"    Nationality: Value={nationality_field.value}   Confidence={nationality_field.confidence}")
    
    place_of_birth_field = document.fields.get("PlaceOfBirth")
    if place_of_birth_field:
        print(f"    PlaceOfBirth: Value={place_of_birth_field.value}   Confidence={place_of_birth_field.confidence}")
    
    place_of_issue_field = document.fields.get("PlaceOfIssue")
    if place_of_issue_field:
        print(f"    PlaceOfIssue: Value={place_of_issue_field.value}   Confidence={place_of_issue_field.confidence}")
    
    issuing_authority_field = document.fields.get("IssuingAuthority")
    if issuing_authority_field:
        print(f"    IssuingAuthority: Value={issuing_authority_field.value}   Confidence={issuing_authority_field.confidence}")
    
    personal_number_field = document.fields.get("PersonalNumber")
    if personal_number_field:
        print(f"    PersonalNumber: Value={personal_number_field.value}   Confidence={personal_number_field.confidence}")
...
def process_id_document_national_identity_card(document):
    print(f"Document: {document.doc_type}")
...
def process_id_document_residence_permit(document):
    print(f"Document: {document.doc_type}")
    ...
    category_field = document.fields.get("Category")
    if category_field:
        print(f"    Category: Value={category_field.value}   Confidence={category_field.confidence}")
    
def process_id_document_us_social_security_card(document):
    print(f"Document: {document.doc_type}")
...
def process_id_document(document):
    print(f"Document: {document.doc_type}")
