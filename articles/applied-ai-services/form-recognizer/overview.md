...
|<ul><li>**Mostly text-containing document**</li></yl>| Is your document _printed_ in a [supported language](language-support.md#read-layout-and-custom-form-template-model) and only interested in just text or also the tables, selection marks, and the structure?|<ul><li>If **Yes** to text-only extraction, use the [**Read**](concept-read.md) model.<li>If **No** because you need structure information too, use the [**Layout**](concept-layout.md) model.</li></ul>
|<ul><li>**General structured document**</li></yl>| Does your mostly structured document contain several fields and values that may not be covered by the other prebuilt models?|<ul><li>If **Yes**, use the [**General document (preview)**](concept-general-document.md) model. Additionally, you can [**Train and build a custom model**](how-to-guides/build-custom-model-v3.md).</li></ul>
