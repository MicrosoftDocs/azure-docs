---
title: Form Recognizer analyzes document API response
...
| tables| Tables identified and extracted from the document. Tables only relate to tables identified by the pre trained layout model. Content labeled as tables is extracted as structured fields in the documents object  | Layout, General Document, **Some** Prebuilt models and Custom models |
...
Bounding regions describe the visual position of each element in the file.  An element may not be visually contiguous (ex. entities) or may cross pages (ex. tables). Thus, the position of most elements is described via an array of bounding regions, where each region specifies the page number (1-indexed) and bounding polygon.  The bounding polygon is described as a sequence of points, clockwise from the left relative to the natural orientation of the element.  For quadrilaterals, the coordinates would be the top-left, top-right, bottom-right, and bottom-left corners.  The x, y coordinates represent each point in the page unit specified by the unit property.  In general, unit of measure for images is pixels while PDFs use inches.
...
A line is an ordered sequence of consecutive content elements separated by a visual space, or ones that are immediately adjacent for languages without space delimiters between words.  Content elements in the same horizontal plane (ex. row) but separated by more than a single visual space split into multiple lines.  While splits are sometimes semantically contiguous content into separate lines, it enables the representation of textual content split into multiple columns or cells.  Lines in vertical writing will be detected in the vertical direction.

:::image type="content" source="media/lines.png" alt-text="Lines":::

#### Paragraph

A paragraph is an ordered sequence of lines that form a logical unit.  Typically, the lines share common alignment and spacing between lines.  Paragraphs are often delimited by indentation, extra spacing, or bullets/numbering.  Content can only be assigned to a single paragraph.
Select paragraphs may also be associated with a functional role in the document.  Currently supported roles include
* Page header
* Page footer
* Page number
* Title
...
Document field is a similar but distinct concept from general form fields.  The field label (key) in a general form field must appear in the document.  Thus, it can't capture information like the merchant name in a receipt. Document fields are labeled and don't extract a key, document fields only map an extracted value to a labeled key. See [document fields](#document-fields) for more details.

### Style elements

#### Style

A style element describes the font style to apply to text content.  The content is specified via spans into the global content property.  Currently, the only detected font style is whether the text is handwritten.  As other styles are added, text can be described by multiple non-conflicting style objects.  For compactness, all text sharing the particular font style (with the same confidence) are described via a single style object.
...
A document is a semantically complete unit.  A file may contain multiple documents, such as multiple tax forms within a PDF file, or multiple receipts within a single page.  The ordering of documents within the file doesn't affect the information it conveys.
...
* Currency: Currency amount with optional currency unit. For example, a value such as ```InvoiceTotal: $123.45```
    ```json
    {
        "amount": 123.45,
        "currencySymbol": "$"
