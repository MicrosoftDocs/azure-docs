...
[Hypervisor](http://en.wikipedia.org/wiki/Hypervisor) layers such as Windows Hyper-V and [KVM](http://www.linux-kvm.org/page/Main_Page) on Linux (as well as many others) virtualize the underlying operating system to enable complete operating systems to run inside the hypervisor as if they were an application. 
