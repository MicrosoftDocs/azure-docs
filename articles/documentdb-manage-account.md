...
When you create a DocumentDB account, two master access keys are
generated which can be used for authentication when the DocumentDB
...
4.      Press Ctrl+C to copy the key.
...
your keys. Here is one process which you can follow:
...
2.      In the Configuration lens, click **Consistency**.
...
To remove a DocumentDB account that you are no longer using, use
**Delete** command on the DocumentDB Account blade.

**Warning**

*In the preview release, there is no way to restore the content from a
...
2.      On the account blade, click the **Delete** command.
