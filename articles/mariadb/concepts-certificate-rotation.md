...
1. Server Name - `mariadbsslverify.mariadb.database.azure.com`
2. User Name - `mariadbuser@mariadbsslverify`
3. Location - `Japan East`

*   Failure

    ```azurecli-interactive
    C:>mysql.exe -hmariadbsslverify.mariadb.database.azure.com -u mariadbuser@mariadbsslverify --ssl-mode=VERIFY_CA --ssl-ca "<location to BaltimoreCyberTrustRoot.crt.pem>"
    ```
...
*   Success
    ...
    ```azurecli-interactive
    C:>mysql.exe -hmariadbsslverify.mariadb.database.azure.com -u mariadbuser@mariadbsslverify --ssl-mode=VERIFY_CA --ssl-ca "<location to DigiCertGlobalRootG2.crt.pem>"
    ```
...
If you are trying to connect to the Azure Database for MariaDB using Azure Kubernetes Services (AKS), it is similar to access from a dedicated customers host environment. Refer to the steps [here](../aks/ingress-own-tls.md).
