---
title: Deploy Service Bus from the service catalog into a workload
titleSuffix: Azure Enclave
description: Deploy Service Bus from the service catalog into a workload.
...
1. Select the `Service Bus` service template from the [service catalog list](./list-service-catalog-templates.md) dropdown, confirm the version you need (default: `latest`), and select `Next`.

![Screenshot showing the Service Bus template selected from the service catalog list.](./media/service-catalog-list-service-bus.png)
...
1. Sign-in to the Admin VM you have deployed for your enclave or [create a new Admin VM](./deploy-admin-vm-service-catalog.md)
