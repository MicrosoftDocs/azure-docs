...
You can use the `az staticwebapp appsettings set` command to do bulk uploads of your settings to Azure. The command accepts application settings as key value pairs in a parameter called `--setting-names`.

The Azure CLI command only requires the static site name, you can optionally also provide the resource group name or subscription in situations where you have multiple matching static web apps. From the Overview window of your Static Web Apps resource, you have access to the following information:
...
4. From a terminal or command line, execute the following command. Make sure to replace the placeholders of `<YOUR_STATIC_SITE_NAME>`, `<YOUR_RESOURCE_GROUP_NAME>`, and `<YOUR_SUBSCRIPTION_ID>` with your values from the _Overview_ window.
   ...
   ```bash
   az staticwebapp appsettings set -n <YOUR_STATIC_SITE_NAME> --setting-names DATABASE_CONNECTION_STRING="<YOUR_DATABASE_CONNECTION_STRING>" key2=val2 --resource-group <YOUR_RESOURCE_GROUP_NAME> --subscription <YOUR_SUBSCRIPTION_ID>
   ```
...
- From a terminal or command line, execute the following command. Make sure to replace the placeholders `<YOUR_SUBSCRIPTION_ID>`, `<YOUR_RESOURCE_GROUP_NAME>`, `<YOUR_STATIC_SITE_NAME>` with your values. Resource Group and Subscription are optional and only required when there are multiple Static Web Apps with the same name.

   ```bash
   az staticwebapp appsettings list -n <YOUR_STATIC_SITE_NAME> --resource-group <YOUR_RESOURCE_GROUP_NAME> --subscription <YOUR_SUBSCRIPTION_ID>
   ```
