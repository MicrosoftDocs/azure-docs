...
Single Page Applications, whether they are using front-end JavaScript frameworks or WebAssembly platforms like Blazor, often rely on client-side routing for web app navigation. These client-side routing rules update the browser's window location without making requests back to the server. If you refresh the page, or navigate directly to locations generated by client-side routing rules, a server-side fallback route is required to serve the appropriate HTML page.

Configure your app to use response overrides to implement a fallback route as shown in the following example:
