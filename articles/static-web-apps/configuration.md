...
To define a fallback route, add a `navigationFallback` section to your config file. The following example returns _/index.html_ for all static file requests that do not match a deployed file.
...
You can control which requests return the fallback route by defining a file filter. In the following example, requests for certain files in the _/images_ folder and all files in the _/css_ folder are excluded from returning the fallback file.
