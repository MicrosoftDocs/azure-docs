...
> [!NOTE]
> Resources are still partially evaluated when conditionally deployed. If the `condition` value is `false` the resource still needs to be valid (ex: in a valid location, has a unique name, etc.)
