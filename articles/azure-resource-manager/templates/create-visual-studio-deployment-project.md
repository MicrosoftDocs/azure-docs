...
1. Visual Studio creates a resource group deployment project for the web app. To see the files for your project, look at the node in the deployment project.
   ...
   | WebSite.parameters.json |A parameters file that has values needed by the template. You pass in parameter values to customize each deployment. Notice that **Build Action** is set to **Content**. If you add more parameter files, the build action to defaults to **None**. Change that setting to **Content**. |
