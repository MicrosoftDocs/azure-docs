...
To help users deploy a [template spec](template-specs.md), you can create a form that is displayed in the Azure portal. The form lets users enter values for parameters that are passed to the template spec.
...
First, you'll create the Azure Resource Manager template (ARM template) to use in the template spec. To show the different portal elements in you can create in the form, you'll use an ARM template with several parameters. The following template creates a key vault, configures permissions to key vault for a user, and adds a secret.
...
The Azure portal provides a sandbox for creating forms. This sandbox includes the ability to generate a default form from an existing ARM template. You'll use this default form to get started with creating form for your template spec.
...
1. The form has fields for selecting a subscription, resource group, and region. It also fields for all of the parameters from the template. When your template specifies allowed values for a parameter, the autogenerated form uses a drop-down control with the allowed values as options.
...
First, let's set the correct schema. Replace the placeholder text about setting the relative path to:
...
Your default form had all of the fields for your template, but they were all on one form called **Basics**. To simplify the form, divide the fields into steps. You'll keep the **Basics** step, but add steps for configuring the key vault, setting user permissions, and specifying the secret.

The following example shows steps added for key vault, permissions, and secret. Add those steps to your file.
...
Select **Preview**. You'll see the steps, but most of them don't have any elements.

:::image type="content" source="./media/template-specs-create-portal-forms/view-steps.png" alt-text="Screenshot of form steps":::

Now, move elements to the appropriate steps. For example, you'll move `secretName` and `secretValue` from the **basics** step to the **secret** step.
