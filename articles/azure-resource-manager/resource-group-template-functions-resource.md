...
If you use a **list** function with a resource that is conditionally deployed, the function throws an error if the resource doesn't exist. Use the **if** function to make sure the function is only evaluated for conditions when the resource is deployed. See the [if function](resource-group-template-functions-logical.md#if) for a sample template that uses if and list with a conditionally deployed resource.
...
If you use the **reference** function with a resource that is conditionally deployed, the function throws an error if the resource doesn't exist. Use the **if** function to make sure the function is only evaluated for conditions when the resource is deployed. See the [if function](resource-group-template-functions-logical.md#if) for a sample template that uses if and reference with a conditionally deployed resource.
