...
When constructing a fully qualified reference to a resource, the order to combine segments from the type and name  is not simply a concatenation of the two.  Instead, after the namespace, use a sequence of *type/name* pairs from least specific to most specific: `{resource-provider-namespace}/{parent-resource-type}/{parent-resource-name}[/{child-resource-type}/{child-resource-name}]*`.  For example, use `Microsoft.Compute/virtualMachines/myVM/extensions/myExt`, not  `Microsoft.Compute/virtualMachines/extensions/myVM/myExt`.
