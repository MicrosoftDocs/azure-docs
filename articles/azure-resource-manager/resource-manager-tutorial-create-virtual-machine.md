---
title: Create an Azure Resource Manager template with dependent resources | Microsoft Docs
description: Learn how to create an Azure Resource Manager template with multiple resources, and who to deploy it
...
# Create a Resource Manager template with dependent resources

Learn how to create an Azure Resource Manager template with multiple by generating one using the Azure portal, and how to edit and deploy the template from the portal.

Resource Manager templates are JSON files that define the resources you need to deploy for your solution. To create a template, you don't have to always start from scratch. In this tutorial, you learn how to generate a template using the Azure portal. You can then customize the template and deploy it.
...
2. Select **Create a resource** > **Storage** > **Storage account - blob, file, table, queue**.
    ...
    ![Create an Azure storage account using the Azure portal](./media/resource-manager-tutorial-create-first-template/azure-resource-manager-template-tutorial-create-storage-account-portal.png)
3. Enter the following information. Make sure to select **Automation options** instead of **Create** in the next step, so you can see the template before it is deployed.
    ...
    ![Create an Azure storage account configuration using the Azure portal](./media/resource-manager-tutorial-create-first-template/azure-resource-manager-template-tutorial-create-storage-account.png)

4. Select **Automation options** at the bottom of the screen. The portal shows the template on the **Template** tab:

    ![Generate a template from the portal](./media/resource-manager-tutorial-create-first-template/azure-resource-manager-template-tutorial-create-storage-account-template.png)
...
5. Select the **Parameters** tab:
    ...
    ![Generate a template from the portal](./media/resource-manager-tutorial-create-first-template/azure-resource-manager-template-tutorial-create-storage-account-template-parameters.png)
...
1. From the Azure portal, select **All services** from the left menu, enter **template** in the filter box, and then select **Template (PREVIEW)**.
    ...
    ![Azure Resource Manager templates](./media/resource-manager-tutorial-create-first-template/azure-resource-manager-template-library.png)
...
3. Select **Edit**, and then select **Template added**.
    ...
    ![Azure Resource Manager templates](./media/resource-manager-tutorial-create-first-template/azure-resource-manager-template-tutorial-edit-storage-account-template.png)
