...
To accommodate different regions, add an input parameter location to the template with a defautValue. The defaultValue will be used if no value is specified during deployment. 
...
* **mystorageaccount1.blob.core.windows.net** when created on the public Azure cloud.
...
The Azure cloud provides a rich selection of VM images. These images are either created and prepared for deployment by Microsoft or by partners in the Azure ecosystem and from the foundation for VMs on the platform. However, a cloud-consistent template should refer to available parameters only — in particular, the publisher, offer, and SKU of the VM images available to the Azure public cloud, an Azure sovereign cloud, or an Azure Stack solution.
...
In contrast, to specify a managed disk configuration in a template, remove the vhd element from the disk configuration like this:
