...
Currently, Azure CLI doesn't support deploying remote Bicep files. You can use [Bicep CLI](./install.md#vs-code-and-bicep-extension) to [build](https://docs.microsoft.com/cli/azure/bicep?view=azure-cli-latest#az_bicep_build) the Bicep file to a JSON template, and then load the JSON file to the remote location.
