...
* Parameter names must be valid Bicep identifiers: they can contain letters, digits, and underscores, but not periods or other special characters. Unlike ARM template JSON, Bicep doesn't support periods in parameter names. If you're migrating from ARM templates, be aware that parameter names containing periods are renamed with underscores during decompilation. For example, a parameter named `Security.Authentication.AAD.Tenant` becomes `Security_Authentication_AAD_Tenant`.
