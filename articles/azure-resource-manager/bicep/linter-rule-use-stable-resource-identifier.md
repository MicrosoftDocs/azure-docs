...
Resource name shouldn't use a non-deterministic value, for example, the [`newGuid()`](./bicep-functions-string.md#newguid) or [`utcNow()`](./bicep-functions-date.md#utcnow) functions are used in resource name, or resource name contains a parameter/variable that calls these functions.
