...
- **Analyze dependencies**: Your resources will probably have dependendies on other resources. You need to figure out dependencies to ensure that moved resources will continue to function as expected after the move.
...
- **Commit the move**: After verifying resources in the target region, and making sure everything's as expected, some resources might require a final commit action. For example, a target region that's now the primary region might need disaster recovery set up to a new secondary region. 
