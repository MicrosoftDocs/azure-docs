...
A CreateUiDefinition always contains three properties: `handler`, `version`, and `parameters`. For managed applications, the `handler` should always be `Microsoft.Compute.MultiVm`, and the latest supported `version` is `0.1.2-preview`.
