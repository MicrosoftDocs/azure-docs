...
* Each resource type may have its own unique considerations when working with tags. For instance, tags on virtual machine extensions can only be updated when the VM is running. If you encounter an error message while attempting to update a tag, please carefully follow the instructions provided in the error message.
