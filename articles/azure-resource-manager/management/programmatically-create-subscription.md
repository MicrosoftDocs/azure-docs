...
In the response, you get back a `Location` url for monitoring. When the subscription creation is finished, a GET on `Location` url will return a `subscriptionLink` object, which has the subscription ID. For more details, refer [Subscription API documentation](https://docs.microsoft.com/en-us/rest/api/subscription/)
