...
> | resourcegroups | subscription | 1-90 | Letters or digits as determined by the [Char.IsLetterOrDigit](/dotnet/api/system.char.isletterordigit) function. Valid characters are members of the following categories in [UnicodeCategory](/dotnet/api/system.globalization.unicodecategory): `UppercaseLetter`, `LowercaseLetter`, `TitlecaseLetter`, `ModifierLetter`, `OtherLetter`, or `DecimalDigitNumber`.<br><br>Can't end with period. |
