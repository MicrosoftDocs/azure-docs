<properties 
	pageTitle="Azure Multi-Factor Authentication - What's Next" 
	description="This is the Azure Multi-factor authentication page that describes what to do next with MFA." 
	services="multi-factor-authentication" 
	documentationCenter="" 
	authors="billmath" 
	manager="terrylan" 
	editor="bryanla"/>

<tags 
	ms.service="multi-factor-authentication" 
	ms.workload="identity" 
	ms.tgt_pltfrm="na" 
	ms.devlang="na" 
	ms.topic="article" 
	ms.date="06/02/2015" 
	ms.author="billmath"/>

# What's Next with Azure Multi-Factor Authentication

[Whats Is It](multi-factor-authentication.md)<br> 
[How it Works](multi-factor-authentication-how-it-works.md)<br>
[Getting Started](multi-factor-authentication-get-started.md)<br>
[What's Next](multi-factor-authentication-whats-next.md)<br>
[Learn More](multi-factor-authentication-learn-more.md)

The following article will help you manage Azure Multi-Factor Authentication.  It covers a variety of topics that will allow you to manage Azure Multi-Factor Authentication.

Feature| Description
:------------- | :------------- | 
[Reports](multi-factor-authentication-manage-reports.md)| Information on the various reports that are available.
[Fraud alert](multi-factor-authentication-manage-fraud.md)|How to setup, configure and report fraud
[One-time bypass](multi-factor-authentication-manage-one-time-bypass.md) |How to setup and configure a one-time bypass
[Custom Voice Messages](multi-factor-authentication-manage-custom.md) |How to setup and configure custom greetings and messages
[Caching](multi-factor-authentication-manage-caching.md)|How to setup and configure authentication caching.
[Trusted IPs](multi-factor-authentication-trusted-ips.md)|Configure and setup IP addresses that are exempt for multi-factor authentication	
[App Passwords](multi-factor-authentication-manage-app-passwords.md)|Information about app passwords.

**Additional Resources**

* [Azure Multi-Factor Authentication on MSDN](https://msdn.microsoft.com/library/azure/dn249471.aspx)