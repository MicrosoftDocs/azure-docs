...
1. You can confirm that your service is running in its own space by running `azds resource list` again. First, you'll notice an instance of `mywebapi` is now running in the `scott` space (the version running in the `default` space is still running but it's not listed). Second, the access point URL for `webfrontend` now has the prefix `scott.s.`. This URL is unique to the `scott` space, and means that requests sent to the "scott URL" will first attempt to route to services in the `scott` space, and if there is no specific `scott` instance for the service, requests fall back to services in the `default` space.
