...
1. Create an ASP.NET Core web app in a folder named `webfrontend`: `dotnet new mvc --name webfrontend`

   Or, **download sample code from GitHub** by navigating to [https://github.com/Azure/dev-spaces](https://github.com/Azure/dev-spaces) and select **Clone or Download** to download the GitHub repository to your local environment. The code for this guide is in `samples/dotnetcore/getting-started/webfrontend`.
