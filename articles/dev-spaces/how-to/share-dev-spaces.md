...
1. Configure the Azure Dev Space's **resource group** to [grant Contributor access](https://docs.microsoft.com/en-us/azure/active-directory/role-based-access-control-configure) for each team member. You can check an environment's resource group by running this command: `azds resource list`
1. Ask team members to **select the environment** in order to develop in it.
     ...
     * **Visual Studio IDE**: Open a project in Visual Studio, select **Azure Dev Spaces for AKS** from the launch settings drop-down. In the dialog that opens, select an existing development environment.
