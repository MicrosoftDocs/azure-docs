...
The "azds-route-as" key signals whether headers must be propagated to handle routing to a dev space. If it's present, we add code to propagate specific headers from an existing ASP.NET HttpRequest object into an outgoing HttpRequestMessage object.. You'll see later how this facilitates a more productive development experience in team scenarios.
