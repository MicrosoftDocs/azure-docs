...
> [!NOTE]
> You must perform the steps in this article on a computer running Windows 10. The PowerShell cmdlets that are required to generate certificates using PowerShell are part of the Windows 10 operating system and do not work on other versions of Windows. You only need these cmdlets to generate the certificates. Once the certificates are generated, you can install them on any supported client operating system. If you do not have access to a Windows 10 computer, you can use makecert to generate certificates. However, the certificates generated by using makecert, while compatible with Point-to-Site, are not SHA-2. For makecert instructions, see [Create certificates using makecert](vpn-gateway-certificates-point-to-site-makecert.md). The certificates that you create, using either PowerShell or makecert, can be installed on any [supported client operating system](vpn-gateway-howto-point-to-site-resource-manager-portal.md#faq), not just the operating system that you used to create them.
> 
>
