...
The tasks below will help you create a root certificate using makecert and then generate client certificates from the root certificate. The steps below are written for makecert on Windows 10. Makecert has been validated to create certificates that are compatible with P2S connections. 

Additionally, you can use certificates generated by your Enterprise CA solution if you issue the client certificates with the common name value format 'name@yourdomain.com', rather than the 'NetBIOS domain name\username' format.

At this time, we are aware that makecert has been deprecated and we are working on a new solution. You can still use the following method. We'll update this page when we have validated a replacement method to create compatible self-signed certificates.
