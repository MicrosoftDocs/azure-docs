...
* Two public IP addresses are required for the gateway instances. Both must be **Standard SKU** public IP addresses that are assigned as **Static**.
* If the gateway has P2S enabled, a third public IP address is required for the P2S endpoint. For Basic-to-Standard public IP migration requirements, see [About migrating a Basic SKU public IP address to Standard SKU](basic-public-ip-migrate-about.md#active-active-vpngw1-5-gateway-skus).
