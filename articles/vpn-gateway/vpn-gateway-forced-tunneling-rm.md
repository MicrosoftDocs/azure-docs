...
* Forced tunneling must be associated with a VNet that has a route-based VPN gateway. You need to set a "default site" among the cross-premises local sites connected to the virtual network. Also, the onpremises VPN device must be configured with using 0.0.0.0/0 as traffic selectors.
