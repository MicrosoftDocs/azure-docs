---
title: 'Download VPN device configuration scripts for S2S VPN connections: Azure Resource Manager: PowerShell | Microsoft Docs'
description: This article walks you through downloading VPN device configuration scripts for S2S VPN connections with Azure VPN Gateways using Azure Resource Manager and PowerShell.
...
This article walks you through downloading VPN device configuration scripts for S2S VPN connections with Azure VPN Gateways using Azure Resource Manager and PowerShell. The following diagram shows the high-level workflow.
...
A cross-premises VPN connection consists of an Azure VPN gateway, an on-premises VPN device, and an IPsec S2S VPN tunnel connecting the two. The typical work flow involves the following steps:
...
4. Configure the on-premises VPN device represented by the local network gateway resource to establish the actual S2S VPN tunnel with the Azure VPN gateway

You can configure steps 1 through 3 using the Azure Portal, PowerShell, or CLI. But the last step involves configuring the on-premises VPN devices outside of Azure. This feature allows you to download a configuration script for your VPN device with the parameter values of your Azure VPN gateway, virtual network and on-premises network address spaces, and VPN connection properties. You can use the script as a starting point, or apply the script directly to your on-premises VPN devices via the configuration console.

> [!IMPORTANT]
> 1. The syntax for each device configuration script is different, and heavily dependent on the models and firmware versions. Please pay special attention to your device model and version information against the templates supplied by this feature.
