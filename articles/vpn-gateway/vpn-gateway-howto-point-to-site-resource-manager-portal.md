...
You need to obtain the .cer file for the root certificate. You can either use a root certificate from an enterprise solution, or you can [create a self-signed root certificate using makecert](vpn-gateway-certificates-point-to-site.md). If you want to use a self-signed root certificate, don't create the certificate using PowerShell. While it is possible to use PowerShell to create self-signed certificates, the certificate that is generated by using PowerShell does not contain the fields necessary for P2S connections.
