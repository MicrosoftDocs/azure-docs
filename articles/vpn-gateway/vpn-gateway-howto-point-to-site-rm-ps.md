...
* If you are not using an enterprise certificate solution, you need to generate a self-signed root certificate. The recommended method for creating a self-signed certificate for P2S use is makecert. While it is possible to use PowerShell to create self-signed certificates, the certificate that is generated by using PowerShell does not contain the fields necessary for P2S connections. For Windows 10 steps, you can refer to [Working with self-signed root certificates for Point-to-Site configurations](vpn-gateway-certificates-point-to-site.md).
