...
- **Simple deployment experience:** You can enable Azure AD Domain Services for your Azure AD tenant using a few clicks. Regardless of whether your Azure AD tenant is a cloud-tenant or synchronized with your on-premises directory, your domain can be up and running quickly.
- **A single domain instance per Azure AD directory:** You can create a single Active Directory domain for each Azure AD directory.
...
- **Basic Group Policy:** You can leverage a single built-in GPO each for the users and computers containers in order to enforce compliance with required security policies for user accounts as well as domain joined computers.
- **Availability in multiple Azure regions:** See the [supported Azure regions](active-directory-domain-services-regions.md) page for a list of Azure regions in which Azure AD Domain Services are available. 
