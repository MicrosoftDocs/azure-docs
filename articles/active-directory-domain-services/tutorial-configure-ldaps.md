...
To communicate with your Microsoft Entra Domain Services (Microsoft Entra DS) managed domain, the Lightweight Directory Access Protocol (LDAP) is used. By default, the LDAP traffic isn't encrypted, which is a security concern for many environments.

With Microsoft Entra DS, you can configure the managed domain to use secure Lightweight Directory Access Protocol (LDAPS). When you use secure LDAP, the traffic is encrypted. Secure LDAP is also known as LDAP over Secure Sockets Layer (SSL) / Transport Layer Security (TLS).

This tutorial shows you how to configure LDAPS for a Microsoft Entra DS managed domain.

In this tutorial, you learn how to:

> [!div class="checklist"]
> * Create a digital certificate for use with Microsoft Entra DS
> * Enable secure LDAP for Microsoft Entra DS
...
* **Subject name** - The subject name on the certificate must be your managed domain. For example, if your domain is named *aaddscontoso.com*, the certificate's subject name must be **.aaddscontoso.com*.
  * The DNS name or subject alternate name of the certificate must be a wildcard certificate to ensure the secure LDAP works properly with the Microsoft Entra Domain Services. Domain Controllers use random names and can be removed or added to ensure the service remains available.
...
### Export a certificate for Microsoft Entra DS
...
1. On the **File to Export** page, specify the file name and location where you'd like to export the certificate, such as *C:\Users\accountname\Microsoft Entra ID-ds.pfx*. Keep a note of the password and location of the *.PFX* file as this information would be required in next steps.
...
Client computers must trust the issuer of the secure LDAP certificate to be able to connect successfully to the managed domain using LDAPS. The client computers need a certificate to successfully encrypt data that is decrypted by Microsoft Entra DS. If you use a public CA, the computer should automatically trust these certificate issuers and have a corresponding certificate.
...
1. On the **File to Export** page, specify the file name and location where you'd like to export the certificate, such as *C:\Users\accountname\Microsoft Entra ID-ds-client.cer*.
...
1. Open File Explorer and browse to the location where you saved the *.CER* certificate file, such as *C:\Users\accountname\Microsoft Entra ID-ds-client.cer*.
...
1. On the left-hand side of the Microsoft Entra DS window, choose **Secure LDAP**.
...
1. To apply the replacement certificate to Microsoft Entra DS, in the left menu for Microsoft Entra DS in the Microsoft Entra admin center, select **Secure LDAP**, and then select **Change Certificate**.
...
For the certificate subject name match, the DC will use the Microsoft Entra DS domain name (not the Microsoft Entra domain name) to search its certificate store for the certificate. Spelling mistakes, for example, prevent the DC from selecting the right certificate.
