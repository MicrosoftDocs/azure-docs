...
Once appropriately configured, the password hashes are stored in the Azure AD DS managed domain itself. They're not stored in Azure AD. If you delete the Azure AD DS managed domain, any password hashes stored at that point are also deleted. If you later create an Azure AD DS managed domain, you must configure the password hash synchronization to store the password hashes again. Previously domain-joined VMs or users won't be able to immediately authenticate - Azure AD needs to generate and store the password hashes in the new Azure AD DS managed domain.
