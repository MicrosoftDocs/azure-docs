...
In contrast, your Azure AD tenant is a much simpler and flat namespace. You will need to handle UPN conflicts across user accounts in different forests in order to enable users to reliably access applications secured by Azure AD. Your Azure AD Domain Services managed domain bears close resemblance to your Azure AD tenant. Therefore, you will see a flat OU structure in your managed domain. All users and groups will be stored within the 'AADDC Users' container, regardless of the on-premises domain or forest from which they were synced in. You may have configured a hierarchical OU structure on-premises. However, your managed domain will still have a simple flat OU structure.
...
- **Excluded attributes:** If you choose to exclude certain attributes from synchronizing to your Azure AD tenant from your on-premises domain using Azure AD Connect, these attributes will not be available in your managed domain.
...
- **Computer objects:** Computer objects for computers joined to your on-premises domain are not synchronized to your managed domain. These computers do not have a trust relationship with your managed domain and belong to your on-premises domain only. In your managed domain, you will find computer objects only for computers you have explicitly domain-joined to the managed domain.

- **SidHistory attributes for users and groups:** While the primary user and primary group SIDs are synchronized to your managed domain, existing SidHistory attributes for users and groups are not synchronized from your on-premises domain to your managed domain.
...
> [AZURE.NOTE] **Sign-in to the managed domain using the UPN format:** The SAMAccountName attribute may be auto-generated for some user accounts in your managed domain. This could occur if there are multiple users with the same mailNickname attribute or users who have overly long UPN prefixes. Therefore, the SAMAccountName format (for example, 'CONTOSO100\joeuser') is not always a reliable way to sign-in to the domain. Users' auto-generated SAMAccountName may differ from their UPN prefix. Use the UPN format (for example, 'joeuser@contoso100.com') to sign-in to the managed domain reliably.
