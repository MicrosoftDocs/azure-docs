...
## Partition Sizing ###
For best performance, SQL Data Warehouse partition boundaries usually need to encompass a broader range of data than your SQL Server partitions.

Since SQL Data Warehouse distributes the rows into distributions, each partition is  also automatically distributed. Consequently, data that would have normally existed in one table in one partition in a SQL Server database, now exists in one partition stored across many tables in SQL Data Warehouse. Each of the distributed partitions also has its own columnstore index.
