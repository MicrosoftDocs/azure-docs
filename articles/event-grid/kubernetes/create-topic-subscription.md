...
1. Run the following **Curl** command to post the event. Specify the endpoint URL and key from step 1 and 2 before running the command. 
    ...
    If the topic endpoint URL from step 1 is a private IP address, such as in the case when Event Grid broker's service type is ClusterIP, you can execute **Curl** from within another pod in the cluster in order to have access to that IP address. For example, you can perform the following steps:
