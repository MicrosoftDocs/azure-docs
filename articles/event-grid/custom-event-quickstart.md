...
Create a resource group with the [az group create](/cli/azure/group#az-group-create) command. The following example creates a resource group named *gridResourceGroup* in the *westus2* location. Change the name of the resource group and the location if you want.
...
1. Copy the following command, specify a name for the topic, and select **Enter** to run the command.
    ...
    ```
1. Use the [`az eventgrid topic create`](/cli/azure/eventgrid/topic#az-eventgrid-topic-create) command to create a custom topic.
...
1. Copy the following command, specify a name for the web app (Event Grid Viewer sample), and select **Enter** to run the command. Replace `<your-site-name>` with a unique name for your web app. The web app name must be unique because it's part of the DNS entry.
    ...
    ```
1. Run the [`az deployment group create`](/cli/azure/deployment/group#az-deployment-group-create) to deploy the web app by using an Azure Resource Manager template.
...
1. Run the following command to create a subscription to the custom topic using the endpoint.
    ...
    :::image type="content" source="./media/custom-event-quickstart/viewer-subscription-validation-event.png" alt-text="Screenshot showing the Event Grid Viewer sample with a subscription validation event.":::
