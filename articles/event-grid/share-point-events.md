---
title: Microsoft Sharepoint events
description: This article describes Microsoft Sharepoint event types and provides event samples.
ms.topic: conceptual
ms.date: 12/6/2023
---
...
This article provides the properties and schema for Microsoft Sharepoint events, which are published by Microsoft Graph API. For an introduction to event schemas, see [CloudEvents schema](cloud-event-schema.md). 

## Available event types
These events are triggered when a [listItem](https://learn.microsoft.com/en-us/graph/api/resources/list?view=graph-rest-1.0) is updated by operating over those resources using Microsoft Graph API. 

 | Event name | Description |
 | ---------- | ----------- |
 | **Microsoft.Graph.ListUpdated** | Triggered when a list in Microsoft Sharepoint is updated. |

## Example event
When an event is triggered, the Event Grid service sends data about that event to subscribing destinations. This section contains an example of what that data would look like for each Microsoft Sharepoint event.

### Microsoft.Graph.ListUpdated event
...
| `tenantId` | string | The organization ID where the list is kept. |
