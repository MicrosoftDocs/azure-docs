...
When resolved from the VNet hosting the private endpoint, the topic or domain endpoint URL resolves to the private endpoint's IP address.
The DNS resource records for the topic 'topicA', when resolved from outside the VNet hosting the private endpoint, will be:
