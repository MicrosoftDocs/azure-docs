...
Managed System Identity (MSI) <br/>&<br/> Role-based access control (Azure RBAC) |  <ul><li>Event Hubs</li><li>Service Bus</li><li>Storage Queues</li><li>Storage  Blobs (Deadletter)</li></ul> | Enable managed system identity for the topic and add it to the appropriate role on the destination. See the following articles:<br/><ul><li>[Enable MSI for a system topic](enable-identity-system-topics.md)</li><li>[Enable MSI for a custom topic or a domain](enable-identity-custom-topics-domains.md)</li><li>[Grand identity the access to Event Grid destination](add-identity-roles.md)</li><li>[Create an event subscription that uses the identity](managed-service-identity.md)</li></ul> |
|Bearer token authentication with Azure AD protected webhook | Webhook | See [Create an app registration in Azure AD & use Azure RBAC](secure-webhook-delivery.md). |
