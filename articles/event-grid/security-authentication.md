...
Once you register a URI to receive webhooks, every time there is a new event ready to be delivered, Event Grid will send an HTTP request with anto that URI with the event in the body.

When you register your own WebHook endpoint with Event Grid, it will send you a GET request with a simple validation code in order to prove endpoint ownership. Your app needs to respond by echoing back the validation code. Event Grid will not deliver events for to WebHook endpoints that have not passed the validation.
 
### Validation details:

* At the time of event subscription creation/update, Event Grid will post a “SubscriptionValidationEvent” event to the target endpoint.
* The event will contain a header value “Event-Type: Validation”.
* The event body will have the same schema as other Event Grid events.
* The event data will include a “validation_code” property with a randomly generated string e.g. “validation_code: acb13…”.

Echo back the validation code in order to prove endpoint ownership e.g “validation_response: acb13…”.

Finally, it is important to note that Azure Event Grid will only support HTTPS webhook endpoints.
## Event subscription
