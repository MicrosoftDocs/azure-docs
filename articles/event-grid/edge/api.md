...
## 2. Edge Hub as Destination

Use this destination to send events to Edge Hub and be subjected to edge hub's routing/filtering/forwarding subsystem.
...
You'll need to first setup a user topic in the cloud to which events should be sent to, before creating an event subscription on the edge.
...
- ```EventTime``` is mandatory, its not validated but should be a proper DateTime.
