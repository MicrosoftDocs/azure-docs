...
| url | string | The path to the blob. <br>If the client uses a Blob REST API, this path has the following format `https://<storage-account-name>.blob.core.windows.net/<container-name>/<file-name>` (For example: "https://my-storage-account.blob.core.windows.net/my-container/new-file.txt")<br>If the client uses a Data Lake Storage REST API, this path has the following format `https://<storage-account-name>.dfs.core.windows.net/<file-system-name>/<file-name>` (For example: "https://my-storage-account.dfs.core.windows.net/my-file-system/new-file.txt").
|
