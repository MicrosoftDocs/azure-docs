...
The possible values of `Outcome` are: `Aborted`, `TimedOut`, `GenericError`, and `Busy`. Event Grid logs any information it receives from the event handler in this message. For example, for `GenericError`, it logs the HTTP status code, error code, and the error message. 
