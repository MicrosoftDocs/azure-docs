...
* **Managed workspace gateway** - In service tiers that support [workspaces](workspaces-overview.md), workspaces route API traffic through a gateway. In the Premium v2 tier, a workspace can use the service's default managed gateway (no additional gateway resource needed), or you can associate one or more separate, managed [workspace gateways](workspaces-overview.md#workspace-gateway) with the workspace for greater isolation. In the Premium tier, a dedicated workspace gateway is required. A dedicated workspace gateway is a standalone Azure resource with the same core functionality as the default managed gateway.
