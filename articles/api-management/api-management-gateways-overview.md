...
* **Managed workspace gateway** - In service tiers that support [workspaces](workspaces-overview.md), the workspace API traffic flows through an associated gateway. A workspace can be associated one or more separate, managed [workspace gateways](workspaces-overview.md#workspace-gateway) for runtime isolation. A workspace gateway is a standalone Azure resource with the same core functionality as the default managed gateway. In the v2 tiers, you can also associate a workspace with the service's default managed gateway (no separate gateway resource needed).
