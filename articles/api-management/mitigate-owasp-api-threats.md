...
- Ensure your backends are protected against path traversal (directory traversal) attacks. API Management may forward requests containing `..%2f` or similar disguised traversal sequences (for example `%2e%2e` or `..;`) in the URL path to a backend. If the backend decodes them to `../`, it could be susceptible to a path traversal attack. You can also apply a policy in API Management to detect and block requests containing such sequences in the path.
