...
> [!IMPORTANT]
> Built-in cache is volatile and is shared by all units in the same region in the same API Management service. Regardless of cache type being used (internal or external), if the cache related operations fail to connect to the cache due to the volatility of the cache or any other reason, the API call that uses the cache related operation does not raise an error and the cache operation completes successfully. In the case of a read operation, a null value is returned to the calling policy expression. Your policy code should be designed to ensure that that there is a "fallback" mechanism to retrieve data not found in the cache.
