...
Azure Container Apps distributes requests linearly across each available and healthy replica. In order to achieve some form of rate limiting, you may divide the desired limit by the number of replicas you wish to run and set the resulting value in configuration. Understand that this has its own downsides as you may not be able to account for adjusted counters if and when your container apps scale.
