...
* A [network security group](../virtual-network/network-security-groups-overview.md) attached to the subnet above. A network security group (NSG) is required to explicitly allow inbound connectivity as the Standard SKU Load Balancer we are using is secure by default and rejects all inbound traffic.
