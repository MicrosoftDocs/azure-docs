...
> [!IMPORTANT]
> To enable the test console to acquire a valid OAuth 2.0 token for API testing, add an OAuth 2.0 authorization server to your instance; you can use any OAuth 2.0 provider, including Azure AD, Azure AD B2C, or a third-party identity provider. Then, configure the API with settings for that authorization server. In the portal, configure OAuth 2.0 authorization on the API's **Settings** page > **Security** > **User authorization**.
