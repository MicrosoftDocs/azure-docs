...
1. On the **Gateway** tab, configure settings for the workspace gateway.
    ...
    * If you choose to create a new gateway:
        ...
        * If you select either **Inbound public access, outbound private access** (virtual network integration) or **Inbound private access, outbound private access (virtual network injection)g, select a **Virtual network** and **Subnet** to isolate the workspace gateway, or create a new one. For network requirements, see [Network resource requirements for workspace gateways](virtual-network-workspaces-resources.md).
