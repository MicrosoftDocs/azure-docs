...
The process is almost identical to [setting up a generic self-hosted portal](dev-portal-self-hosting-the-portal.md) with one exception in the configuration step: the Storage Account in the `config.design.json` file needs to be the same as the Storage Account of the managed version of the portal. Refer to [this article](../active-directory/managed-identities-azure-resources/tutorial-linux-vm-access-storage-sas.md#get-a-sas-credential-from-azure-resource-manager-to-make-storage-calls) **<-- is this a goog replacement?** || **original link -->** (Retrieve-managed-portal-storage-account-and-SAS-URL) for instructions on how to retrieve this SAS URL.
