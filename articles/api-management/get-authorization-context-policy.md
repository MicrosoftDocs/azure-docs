...
* Configure `identity-type=jwt`  when the [access policy](authorizations-overview.md#step-3---access-policy) for the authorization is assigned to a service principal or to a user-assigned managed identity. Only `/.default` app-only scopes are supported for the JWT.
