...
+ IP addresses from the subnet IP range (DIP) will be assigned to each VM in the service and will used to access resources within the vnet. A public IP address (VIP) will be used to access resources outside the vnet. If IP restriction lists are used to secure resources within the VNET, the entire range for the subnet where the API Management service is deployed must specified to grant or restrict access from the service.
