...
> [!NOTE]
> None of the service endpoints are registered on the public DNS. The service endpoints remain inaccessible until you [configure DNS](#apim-dns-configuration) for the VNET.
...
> [!NOTE]
> When you deploy an API Management service in an internal virtual network, it's hosted behind an internal load balancer in the [Basic SKU](../load-balancer/skus.md).
...
> [!NOTE]
> When you deploy an API Management service in an internal virtual network, it's hosted behind an internal load balancer in the [Standard SKU](../load-balancer/skus.md), using the public IP address resource.
...
To access these API Management service endpoints, you can create a virtual machine in a subnet connected to the VNET in which API Management is deployed. Assuming the internal virtual IP address for your service is 10.1.0.5, you can map the hosts file, (on Windows,`%SystemDrive%\drivers\etc\hosts`), as follows:
...
* [Network configuration details while setting up Azure API Management in a VNET][Common network configuration problems]
