...
Subscriptions are an important concept in Azure API Management. They're the most common way for API consumers to obtain access to APIs published through an API Management instance. This article provides an overview of the concept.
...
To obtain a subscription key for accessing APIs, a subscription is required. A subscription is essentially a named container for a pair of subscription keys. Subscriptions can be obtained by developers who need to consume the published APIs, with or without approval by API publishers. API publishers can also create subscriptions directly, on behalf of API consumers.
...
Traditionally, subscriptions in API Management were always associated with a single [API product](api-management-terminology.md) scope. Developers found the list of products on the Azure portal. Then they'd submit subscription requests for the products they wanted to use. After a subscription request is approved, either automatically or by API publishers, the developer can use the keys in it to access all APIs in the product.
