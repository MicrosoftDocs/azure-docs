...
|backend-id|Identifier of the backend to route to. (Backend entities are managed via [Azure portal](backends.md), [API](/rest/api/apimanagement/2019-12-01/backend), and [PowerShell](https://www.powershellgallery.com/packages?q=apimanagement).)|One of `base-url` or `backend-id` must be present.|N/A|
