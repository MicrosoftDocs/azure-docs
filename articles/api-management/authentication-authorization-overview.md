...
|---------|---------|---------|
|[mTLS](api-management-howto-mutual-certificates-for-clients.md)     |   [Validate certificate](validate-client-certificate-policy.md) presented by the connecting client and check certificate properties against a certificate managed in API Management     |  Certificate can be stored in a key vault.       |
...
> [!TIP]
> For defense in depth, we strongly recommend deploying a web application firewall upstream of the API Management instance. For example, use [Azure Application Gateway](/azure/architecture/reference-architectures/apis/protect-apis) or [Azure Front Door](front-door-api-management.md).
...
|[Certificate authentication](authentication-certificate-policy.md)     |    Authenticate to backend API using a client certificate.      |  Certificate can be stored in key vault.      |
