...
| ----------------- | ------------------------------------------------------ | -------- | ------- |
|base-url|New backend service base URL. Usually, the format is `https://backend.com/api`. Avoid adding a slash at the end to prevent misconfigurations. Typically, the `base-url` and HTTP(S) endpoint value in the backend should match to enable seamless integration between frontend and backend. Note that API Management instances may require the backend service name to be included in the `base-url` explicitly for proper routing. For example, if you have an Azure Function named `MyFunctionName` within an Azure Function App named `MyFunctionAppName`, and the backend URL is configured as `https://MyFunctionAppName.azurewebsites.net/api`, API Management will route requests to `https://MyFunctionAppName.azurewebsites.net/api/MyFunctionName` if the backend configuration explicitly includes the function name. Policy expressions are allowed.|One of `base-url` or `backend-id` must be present.|N/A|
...
|sf-replica-type|Only applicable when the backend is a Service Fabric service. Specifies whether the request should be routed to the primary or secondary replica of a partition. Valid values are `primary` and `secondary`. Policy expressions are allowed.|No|N/A|
