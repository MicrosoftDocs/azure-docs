...
8. Depending on your scenario, you might need to change the [service type](../aks/concepts-network.md#services). The default value is `LoadBalancer` which is the external load balancer. You can use the [internal load balancer](../aks/internal-lb.md) if you want to restrict the access to the self-hosted gateway to internal users. The sample below uses `NodePort`. 
