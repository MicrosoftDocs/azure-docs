...
> [!NOTE]
> In the Premium service tier, you can optionally configure availability zones and a virtual network in a selected location. For more information, see [the Deploy API Management service to an additional region section of Deploy API Management service to an additional location](api-management-howto-deploy-multi-region.md).
