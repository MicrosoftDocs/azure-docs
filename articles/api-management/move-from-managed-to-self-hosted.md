...
The process is almost identical to [setting up a generic self-hosted portal](Self-hosting-the-portal) with one exception in the configuration step: the Storage Account in the `config.design.json` file needs to be the same as the Storage Account of the managed version of the portal. Refer to [this article](Retrieve-managed-portal-storage-account-and-SAS-URL) for instructions on how to retrieve this SAS URL.
