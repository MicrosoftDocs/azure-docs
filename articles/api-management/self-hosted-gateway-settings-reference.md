...
| config.service.endpoint.disableCertificateValidation | Defines if the self-hosted gateway should validate the server-side certificate of the Configuration API. It is recommended to not enable this as it can introduce security risk. | No | `false` |  v2.0+ |
