...
* **A network security group** attached to the subnet above. A network security group (NSG) is required to explicitly allow inbound connectivity as the [Standard SKU load Balancer](../load-balancer/skus.md) we are using is secure by default and rejects all inbound traffic. For more strict configuration refer to **Required ports** below.
