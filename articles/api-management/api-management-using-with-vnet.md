...
+ **Application Insights**: If the service enabled [Azure Application Insights](api-management-howto-app-insights.md) monitoring on API Management service, then we need to allow outbound connectivity to the [Telemetry endpoint](/azure/azure-monitor/app/ip-addresses.md#outgoing-ports) from the Virtual Network. 
