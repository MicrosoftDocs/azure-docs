...
![Virtual WAN Gateway Name](./media/virtual-wan-pcap-screenshots/vpngatewayname.png)
...
![Virtual WAN Connection Names](./media/virtual-wan-pcap-screenshots/sampleconnection.png)

The name of the links connected to a specific VPN site can be found by clicking on the VPN site

![Virtual WAN Link Names](./media/virtual-wan-pcap-screenshots/linknamesample.png)

   ```azurepowershell-interactive
Start-AzVpnConnectionPacketCapture -ResourceGroupName $rg -Name "<name of the VPN connection>" -ParentResourceName “<name of the Gateway>” -LinkConnection “<comma separated list of links>” -Sasurl $sasurl 
   ```
...
Below is an Example Packet Capture Using a Filter String. You may change the parameters to suit your needs based on the preceding table.  
   ...
   ```azurepowershell-interactive
$filter="{`"TracingFlags`":11,`"MaxPacketBufferSize`":120,`"MaxFileSize`":500,`"Filters`":[{`"SourceSubnets`":[`"10.19.0.4/32`",`"10.20.0.4/32`"],`"DestinationSubnets`":[`"10.20.0.4/32`",`"10.19.0.4/32`"],`"IpSubnetValueAsAny`":true,`"TcpFlags`":9,`"PortValueAsAny`":true,`"CaptureSingleDirectionTrafficOnly`":true}]}"
