...
1. On the **Download virtual WAN user VPN** page, select the authentication method that matches your User VPN configuration, then click **Generate and download profile**. If your configuration uses certificate authentication, select **EAPTLS**. A profile package (zip file) containing VPN client configuration settings is generated and downloads to your computer. The contents of the package depend on the hubs and the authentication and tunnel type choices for the configuration that you selected.
