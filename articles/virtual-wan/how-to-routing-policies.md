...
* Routing intent can only be configured where there are no custom route tables enabled on the VWAN Virtual Hub, and no static routes in the defaultRouteTable with next hop Virtual Network Connection. For more information, see [prerequisites](#prereq). However it is supported to have User Defined Routes (UDR) in the Spokes connected to the Hub, as long as "Propagate gateway routes" is set to "Yes" for that custom Route Table. For more information, see [troubleshooting data path](#troubleshooting-data-path).
