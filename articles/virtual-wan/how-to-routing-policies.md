...
* Routing intent can only be configured where there are no custom route tables enabled on the Virtual WAN Hub, and no static routes in the defaultRouteTable with next hop Virtual Network Connection. For more information, see [prerequisites](#prereq). However, you can associate [User Defined Routes](../virtual-network/virtual-networks-udr-overview.md#user-defined-routes) (UDRs) to spoke Virtual Networks connected to the Virtual WAN hub. To ensure the spoke Virtual Network learns routes from Virtual WAN, ensure "Propagate gateway routes" is set to "Yes" for the user-defined route table. For more information, see [troubleshooting data path](#troubleshooting-data-path).
