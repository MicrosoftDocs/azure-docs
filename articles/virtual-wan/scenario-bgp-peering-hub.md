...
* BGP peering between an NVA in a spoke VNet and a secured virtual hub (hub with an integrated security solution) if Routing Intent is configured on the hub. BGP peering feature is not supported for secured virtual hubs where routing intent is **not** configured. 
