...
All connections to Azure SQL Database require encryption (SSL/TLS) at all times while data is "in transit" to and from the database. In your application's connection string, you must specify **Encrypt=True**, and **TrustServerCertificate=False** (this is done for you if you copy your connection string out of the Azure Management Portal), otherwise the connection will not verify the identity of the server and will be susceptible to "man-in-the-middle" attacks. For more information, see [Azure SQL Database Connection Encryption and Certificate Validation](https://msdn.microsoft.com/library/azure/ff394108#encryption).
...
Managing Azure SQL Database Servers and Database from the Azure Management Portal or using the Azure Resource Manager API is controlled by your portal user account's role assignments. For more information on this topic, see [Role-based access control in Azure Preview portal](role-based-access-control-configure.md).


## Encryption: Storing secret data
Coming soon, Azure SQL Database will help you protect your data by encrypting your data when it is "at rest," or stored in database files and backups, using Transparent Data Encryption. This feature is based on SQL Server's [Transparent Data Encryption](https://msdn.microsoft.com/library/bb934049).

If you cannot wait, need a Hardware Security Module, or require central management of your encryption key hierarchy, consider using [Azure Key Vault with SQL Server in an Azure VM](http://blogs.technet.com/b/kv/archive/2015/01/12/using-the-key-vault-for-sql-server-encryption.aspx).
