...
The walkthrough below will go through the steps involved in enabling logging to various source, consuming from those sources, and the schema of the data involved.
...
This section describes the storage layout and schema of the CDN core analytics and provides sample code to download the analytics into a CSV file.

### Using Microsoft Azure Storage Explorer
Before you can access the core analytics data from the storage account, you first need a tool to view contents in a storage account. While there are several tools available in the market, the one that we recommend is the Microsoft Azure Storage Explorer. You can download the tool from [here](http://storageexplorer.com/). After downloading and installing the software, please configure to use the storage account which was configured as a destination to the CDN Diagnostics Logs.
...
> [!NOTE]
> Blob path format
> Please note that this is just one file and there are no real directory hierarchy for a blob. But the Storage explorer tool interprets '/' as a directory separator and shows the hierarchy for convenience. 
...
[Sample CSV](./media/result.csv)

## Diagnostic logs types

Only Core Analytics is available currently. More coming in the future.

**Core Analytics:** capture the metrics of your CDN delivery, including usage, errors, hits, cache efficiency, etc, for a richer view into CDN core analytics. Core analytics has hourly granularity for both Azure CDN from Verizon and Akamai. Data is delayed for one hour with Verizon and 24 hour with Akamai. 

### Core Analytics metrics
Below is a list of metrics available for each provider.
*Note that there are slight differences in available metrics, and metrics unavailable to your profile type will show up as null. Also note that there must be some traffic on the CDN endpoint before logs will start flowing.
...
| RequestCountHttpStatusOthers |  Count of all other HTTP codes | Yes  |Yes   |
...
| RequestCountCacheMiss | Count of all requests that resulted in a Cache Miss. This means the asset was not found on the POP closest to the client, but was retrieved from the Origin.              |Yes   | No  |
...
| EgressCacheHit |  Outbound data transfer for responses that were delivered directly from the POP server |Yes   |  No |
...
Where the ‘time’ represents the start time of the hour boundary for which the statistics is reported. Please note that when a metric is not supported by a CDN provider, instead of a double or integer value, there will be a null value. This value indicates the absence of a metric, and this is different from a 0 value. Also note that there will be one set of these metrics per domain configured on the endpoint. 
