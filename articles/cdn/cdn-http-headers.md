...
X-EC-_&lt;name&gt;_ | Request headers that start with *X-EC* (for example, `X-EC-Tag`, `X-EC-Debug`) are reserved for use by the CDN.| waf-production
...
#### Usage
The response sent from the POP servers to a user only includes the X-EC-Debug header when the following conditions are true:
...
- Status code: Indicates how the requested content was handled by the CDN, which is represented through a cache status code.
...
The `X-EC-Debug` response header reports whether a request could have been cached in the following format:

`X-EC-Debug: x-ec-check-cacheable: Cacheable`
...
If a query string is recorded in the cache-key, it's converted to its hash equivalent and then inserted between the name of the requested asset and its file extension (for example, asset_&lt;hash value&gt;_.html).
...
- ExpiresSeconds: Indicates the number of seconds remaining before the date/time specified in the Expires response header. If the Expires response header was not included in the response, then this term will report "none."
