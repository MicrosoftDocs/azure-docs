...
1. A user (Alice) requests a file (also called an asset) by using a URL with a special domain name, such as `<endpointname>.azureedge.net`. DNS routes the request to the best performing Point-of-Presence (POP) location, which is usually the POP that is geographically closest to the user.
...
3. The origin returns the file to the edge server, including optional HTTP headers that describe the file's time-to-live (TTL).
...
5. Additional users can then request the same file using that same URL, and can also be directed to that same POP.
