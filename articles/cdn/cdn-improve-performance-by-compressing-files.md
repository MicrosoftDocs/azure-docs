...
- **Accept-Encoding Request Header** - This header indicates the compression methods a user agent supports. It must be included in the request header.
...
2. Edge server check whether there is Accept-Encoding header.
	1. Included: this header identifies the requested compression method.
	1.	Missing: This type of request will be served in an uncompressed format
3.	The closest Edge POP will check the cache status, compression method and if it still has a valid TTL
	1.	Cache Miss:  if the requested version is not cached, the request will be forwarded to the origin 
	2.	Cache HIT with same compression method: An edge server will immediately deliver the compressed content to the client
	3.	Cache HIT with different compression method: An edge server will transcode the asset to the requested compression method. 
	4.	Uncompressed cache HIT: if the initial request caused the asset to be cached in an uncompressed format, then a check will be performed to see whether the request is eligible for edge server compression (based on the criteria in the definition/requirement section above.)
		1.	Eligible:  An edge server will compress the file and serve it to the client
		2.	Ineligible: an edge server will immediately deliver the uncompressed content to the client 
...
2. Only one file version (compressed or uncompressed) will be cached on an Edge server. A request for a different version will result in a transcoding from the Edge side.  
