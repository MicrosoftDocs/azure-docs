...
## Overview
The rules engine allows you to customize how HTTP requests are handled. For example, blocking the delivery of certain content types, defining a caching policy, or modifying an HTTP header. This tutorial demonstrates how to create a rule that changes the caching behavior of CDN assets. For more information about the rules engine syntax, see [Azure CDN rules engine reference](cdn-rules-engine-reference.md).

## Access
To access the rules engine, you must first select **Manage** from the top of the **CDN profile** page. From the Azure CDN management page, you can then access two different types of rules engines:

- HTTP rules engine: To create a rule that affects all your profile endpoints, except those endpoints optimized for [dynamic site acceleration (DSA)](cdn-dynamic-site-acceleration.md), select the **HTTP Large** tab, then select **Rules Engine**.

- ADN rules engine: To create a rule that affects only endpoints that are optimized for [dynamic site acceleration (DSA)](cdn-dynamic-site-acceleration.md), select the **ADN** tab, then select **Rules Engine**. ADN (application delivery network) is a term used by Verizon to specify dynamic content delivery. Any rules you create with the ADN rules engine are ignored by any endpoints in your profile that are not optimized for DSA. 
