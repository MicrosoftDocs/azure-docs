...
- When used in a response header: Azure CDN honors the following Cache-Control directives when it is using general web delivery, large file download, and general/video-on-demand media streaming optimizations:  
   ...
   - no-cache: Cache the content, but must revalidate the content every time before delivering it from the cache. Equivalent to `Cache-Control: max-age=0`.
...
- The ETag specification is more current than Last-Modified and is useful when the last modified date is difficult to determine.
