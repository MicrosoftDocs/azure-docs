...
This article provides an overview of general caching concepts and how Azure CDN uses caching to improve performance. If you’d like to learn about how to customize caching behavior on your CDN endpoint instead, see [Control Azure CDN caching behavior with query strings](cdn-query-string.md) and [Control Azure CDN caching behavior with Delivery Rules](cdn-delivery-rules.md).
...
Similarly, caching is used by a content delivery network (CDN) on edge servers close to the user to avoid requests traveling back to the origin and reducing end-user latency. Unlike a web browser cache, which is used only for a single user, the CDN has a shared cache. In a CDN shared cache, a file that is requested by one user can be accessed later by other users, which greatly decreases the number of requests going back to the origin.
...
Caching can occur at multiple levels between the origin web server and the end user:

- Web server: Uses a shared cache (for multiple users).
- Content delivery network: Uses a shared cache (for multiple users).
- ISP: Uses a shared cache (for multiple users).
- Web browser: Uses a private cache (for one user).

Each cache typically manages its own resource freshness and performs revalidation when a file is stale. This behavior is defined in the HTTP caching specification, RFC 7234.

### Resource freshness

Because a cached resource can potentially be out-of-date, or stale (as compared to the corresponding resource on the origin server), it is important for any caching mechanism to control when content is refreshed. A cached resource is considered to be fresh (that is, able to be sent to a client without checking the data on the origin server) when its age is less than the age or period defined by a cache setting.  For example, when a browser reloads a web page, it verifies that each cached resource on your hard drive is up-to-date and loads it. If the resource is not up-to-date, a fresh copy is loaded from the server.

### Revalidation

If a resource is considered to be stale, the origin server is asked to revalidate it, that is, determine whether the data in the cache still matches what’s on the origin server. If the file has been modified on the origin server, the cache updates its version of the resource. Otherwise, if the resource is fresh, the data is delivered directly from the cache without revalidation.

### CDN caching

Caching is integral to the way a CDN operates in order to speed up delivery and reduce origin load for static assets such as images, fonts, and videos. In CDN caching, static resources are selectively stored on strategically placed servers that are more local to a user and offers the following advantages:
...
- Overrides the Expires header if both it and the Cache-Control header are defined.
- When used in a request header: Ignored by Azure CDN, by default.
- When used in a response header: Azure CDN honors the following directives when using general web delivery, large file download, and general/video-on-demand media streaming optimizations:  

   - max-age: A cache can store this content for the number of seconds specified.
   - private: Content is for a single user only; don’t store in the shared caches such as CDN.
   - no-cache: Cache, but must revalidate before delivering this content from the cache each time. 
   - no-store: Never cache this content.

Expires:  
   - Legacy header introduced in HTTP 1.0 and supported for backwards compatibility.
...
When the cache is stale, validators are used to compare the cached version of a file with the version on the origin server. **Azure CDN from Verizon** supports both validators by default, while **Azure CDN from Akamai** supports only Last-Modified by default.
...
- The ETag specification is more recent than Last-Modified and is useful when the last modified date is difficult to determine.
- ETag supports both strong validation and weak validation; however, Azure CDN supports only strong validation. For strong validation, the two resource representations must be byte-for-byte identical. 
- A cache revalidates a file using ETag by sending an If-None-Match header with one or more ETags in the request. For example, If-None-Match: "17f0ddd99ed5bbe4edffdd6496d7131f". If the server’s version matches an ETag on the list, it sends a "304 not modified" response. If the version is different, the server responds using a 200 code with the updated resource.
...
- A cache revalidates a file using Last-Modified by sending an If-Modified-Since header with a date and time in the request. The origin server compares that date with the Last-Modified header of the latest resource, and returns a ‘304 Not Modified’ response if it has not been modified since the specified time. If it has been modified, the server responds using a 200 with the updated resource.
