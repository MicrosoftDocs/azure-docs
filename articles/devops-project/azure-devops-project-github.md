...
1. In the left pane, select **+Create a resource** in the left navigation bar. Search for **DevOps Projects**, and then select **Create**.
...
1. You can select either **GitHub** or an external **Git** code repository. For this tutorial, select **GitHub**. You might be required to authenticate with GitHub the first time to allow Azure to access your GitHub repository.

1. Select a **Repository** and a **Branch**, and then select **Next**.

1. If you're using Docker containers, you can change **Is app Dockerized** to **YES**. For this tutorial, leave **NO** selected, and then select **Next**. For more information about using Docker containers, hover over the **i** icon.

   ![Application framework selection on dropdown menu](_img/azure-devops-project-github/appframework.png)

1. From the dropdowns, select an **application runtime** and an **application framework**, and then select **Next**. The application framework dictates the type of Azure service deployment target that’s available.
...
1. Create a new free **Azure DevOps Organization** or select an existing organization from the dropdown.

1. Select your **Azure Subscription**, and either enter a name for your **Web app** or use the default. Select a **Location**, and then select **Done**. After a few minutes, the DevOps Projects deployment overview will be displayed in the Azure portal.
...
1. Once your **Azure Pipelines** page opens, you'll see a history of the most recent builds and the status for each build.

   ![Azure Pipelines builds page](_img/azure-devops-project-github/pipelinesbuildpage.png)

1. In the upper-right corner of the **Builds** page, you can **Edit** the current build, **Queue** a new build, or select the vertical ellipsis button (**&#8942;**) to open a menu with more options. Select **Edit**.
...
1. To see an audit trail of your recent changes for the build, select the **History** tab. You can see changes made to the build pipeline, and you can compare versions.
