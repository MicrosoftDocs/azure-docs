...
You can define a search space for the object in the app by finger clicking in the air with either your right or left hand. The search space will switch among a sphere of 2 meters radius, a 4 m^3 bounding box and a view frustum. Each time when search area changes, the app will remove instances currently being tracked, then try to find them from scratch in the new search area.

This app can track multiple objects at one time. To do that, upload multiple OU models to **LocalState** folder, then set a search area that covers both. Note that it may take longer time to detect and track multiple objects.
