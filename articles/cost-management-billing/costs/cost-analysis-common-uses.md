...
Forecast costs are shown in Cost Analysis when using area and stacked column charts. The forecast is based on your historical resource use. Changes to your resource use affect forecast costs.
...
The default view doesn't show forecasted costs grouped by a service, so you have to add a group by selection.
...
Looking at daily and monthly costs can help you to better understand if there's a time of the week or year where your costs are higher. If you have more customer traffic during a holiday period, does that lead to a corresponding increase in your Azure costs? Is Friday a more costly day than Monday?
...
The Pricing Model dimension is also used to view on demand, reservation and savings plan charges.
...
**Amortized cost** - Shows a reservation purchase spread as an amortized cost over the duration of the reservation term. Using the same example above, Cost Analysis shows a varying cost for each month throughout the year, because of the varying number of days in a month. If you group costs by VM in this example, you'd see cost attributed to each VM that received the reservation benefit.
...
To identify the potential cost that is currently being wasted each month for your reservation purchase, follow the steps below.
...
Your usage details report file, known as the Cost and Usage Details file, in CSV format, provides a breakdown of all the charges that accrued towards an invoice. You can use the report to compare it to, and better understand, your invoice. Each billed charge on your invoice corresponds to broken-down charges in the usage report.
...
1.  Select the usage file to download.  
    ...
    Alternatively, you can use the Exports feature, to get the file sent to your storage account. See [Create and manage Cost Management exports](..\costs\tutorial-improved-exports.md)
