...
Exports are recurring data dumps to storage that can be configured to run on a custom schedule. Exports is the recommended solution for ingesting cost details data and is the most scalable for large enterprises. Exports are [configured in the Azure portal](../costs/tutorial-export-acm-data.md) or using the [Exports API](/rest/api/cost-management/exports). Please find some considerations below for analyzing whether this solution is best for your particular data ingestion workload.

- Exports is most scalable solution for your workloads.
...
The [Cost Details API-UNPUBLISHED-UNPUBLISHED](../index.yml) is the go to solution for on demand download of the cost details dataset. Please find some considerations below for analyzing whether this solution is best for your particular data ingestion workload.

- Useful for small cost datasets. Does not scale as well as Exports and may not be a good solution if you need to ingest many Gbs worth of cost data month over month. A Gb of cost details data is roughly 1 million rows of data.
- Useful for scenarios when Exports to Azure storage aren't feasible due to security or manageability concerns.

If the Cost Details API be your chosen solution, please find best practices for calling the API below.
...
- If you are bound by rate limits at a lower scope, consider calling a higher scope to download data.
...
Power BI is another solution that's used to work with cost details data. The following PowerBI solutions are available:
