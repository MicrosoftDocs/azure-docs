---
title: Save costs with Microsoft Foundry Provisioned Throughput Reservations
description: Save costs with Microsoft Foundry Provisioned Throughput Reservations by committing to a reservation for your provisioned throughput units.
author: pri-mittal
...
You can buy a Microsoft Foundry Provisioned Throughput reservation in the [Azure portal](https://portal.azure.com/#blade/Microsoft_Azure_Reservations/ReservationsBrowseBlade). Pay for the reservation [up front or with monthly payments](prepare-buy-reservation.md). To buy a reservation:
...
- For the Cloud Solution Provider (CSP) program, only the admin agents or sales agents can purchase Microsoft Foundry Provisioned Throughput Reservations.
...
## Buy a Microsoft Foundry Provisioned Throughput reservation

When you buy a reservation, the current UTC date and time are used to record the transaction.

To buy a Microsoft Foundry Provisioned Throughput reservation, follow these steps:

1. Sign in to the [Azure portal](https://portal.azure.com/).
2. Select **All services** > **Reservations** and then select **Microsoft Foundry Provisioned Throughput**  
...
When you exchange a Microsoft Foundry Provisioned Throughput reservation, you can change the following attributes:
...
If you want to exchange your Microsoft Foundry Provisioned Throughput reservation, you can do so by following these steps:

1. Sign in to the Azure portal and go to the Reservations page.
2. Select the Microsoft Foundry Provisioned Throughput reservation that you want to refund and select **Exchange**.
3. Select the new reservation to purchase.
4. Review the terms and conditions and agree to them.

If you want to request a refund for your Microsoft Foundry Provisioned Throughput reservation, you can do so by following these steps:

1. Sign in to the Azure portal and go to the Reservations page.
2. Select the Microsoft Foundry Provisioned Throughput reservation that you want to refund and select **Return**.
...
The application of the Microsoft Foundry Provisioned Throughput reservation is based on an hourly comparison between the reserved and deployed PTUs of the models available in Microsoft Foundry Models. The sum of deployed PTUs up-to the amount reserved are covered (paid for) via the reservation, while any deployed PTUs more than the reserved PTUs get charged at the hourly, pay-as-you-go rate. There are a few other points to keep in mind:
...
>[!NOTE]
>The application of the Microsoft Foundry Provisioned Throughput reservation works on models available in Microsoft Foundry Models (e.g. Azure OpenAI Service, DeepSeek, etc.) For the list of the Microsoft Foundry Models, please refer to [Microsoft Foundry Models pricing page](https://azure.microsoft.com/pricing/details/phi-3/#pricing). 

### Discount examples

The following examples show how the Microsoft Foundry Provisioned Throughput reservation discount applies, depending on the deployments.
...
You can't change the size of a purchased reservation. If you want to increase your Microsoft Foundry Provisioned Throughput reservation capacity to cover more hourly PTUs, you can buy more Microsoft Foundry Provisioned Throughput reservations or exchange the existing reservation with the quantity you need.
