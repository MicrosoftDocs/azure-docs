...
description: Learn how you can save costs for Azure App Service Premium plan and Isolated plan reserved instances.
author: pri-mittal
...
This article explains how you can save with Azure App Service reserved instances for Premium plan (v3 and later) and Isolated plan (v2 and later) instances.

## Save with reserved instances

When you commit to an Azure App Service Premium plan or Isolated plan reserved instance, you can save money. The reservation discount is applied automatically to the number of running instances that match the reservation scope and attributes. You don't need to assign a reservation to an instance to get the discounts.
...
- Purchase recommendations and recommended quantities are shown when you purchase a reserved instance in the Azure portal.
...
## Buy a Premium plan reserved instance

You can buy a reserved Premium plan instance in the [Azure portal](https://portal.azure.com/#blade/Microsoft_Azure_Reservations/CreateBlade/referrer/documentation/filters/%7B%22reservedResourceType%22%3A%22VirtualMachines%22%7D). Pay for the reservation [up front or with monthly payments](prepare-buy-reservation.md). These requirements apply to buying a Premium plan reserved instance:
...
4. Enter required fields. Running Premium plan reserved instances that match the attributes you select qualify for the reservation discount. The actual number of your Premium plan reserved instances that get the discount depend on the scope and quantity selected.
...
| Premium plan reserved instance size | The size of the Premium plan reserved instances. |
| Term | One year or three years. |
| Quantity | The number of instances being purchased within the reservation. The quantity is the number of running Premium plan reserved instances that can get the billing discount. For example, if you are running 10 Premium plan reserved instances in the East US, then you would specify quantity as 10 to maximize the benefit for all running Premium plan reserved instances. |

## Buy an Isolated plan reserved instance

You can buy a reserved Isolated plan instance in the [Azure portal](https://portal.azure.com/#blade/Microsoft_Azure_Reservations/CreateBlade/referrer/documentation/filters/%7B%22reservedResourceType%22%3A%22VirtualMachines%22%7D). Pay for the reservation [up front or with monthly payments](prepare-buy-reservation.md). These requirements apply to buying an Isolated plan reserved instance:
...
2. Select **All services** > **Reservations**.
3. Select **Add** to purchase a new reservation and then select **Instance**.
4. Enter required fields. Running Isolated plan reserved instances that match the attributes you select qualify for the reservation discount. The actual number of your Isolated plan reserved instances that get the discount depend on the scope and quantity selected.
...
| Scope | The reservation's scope can cover one subscription or multiple subscriptions (shared scope). If you select:<UL><LI>**Single resource group scope** — Applies the reservation discount to the matching resources in the selected resource group only.</li><li>**Single subscription scope** — Applies the reservation discount to the matching resources in the selected subscription.</li><li>**Shared scope** — Applies the reservation discount to matching resources in eligible subscriptions that are in the billing context. For EA customers, the billing context is the enrollment. For individual subscriptions with pay-as-you-go rates, the billing scope is all eligible subscriptions created by the account administrator.</li><li>**Management group** - Applies the reservation discount to the matching resource in the list of subscriptions that are a part of both the management group and billing scope.</li></ul> |
| Region | The Azure region that's covered by the reservation. |
| Isolated plan reserved instance size | The size of the Isolated plan reserved instances. |
| Term | One year or three years. |
| Quantity | The number of instances being purchased within the reservation. The quantity is the number of running Isolated plan reserved instances that can get the billing discount. For example, if you are running 10 Isolated plan reserved instances in the East US, then you would specify quantity as 10 to maximize the benefit for all running Isolated plan reserved instances. |
