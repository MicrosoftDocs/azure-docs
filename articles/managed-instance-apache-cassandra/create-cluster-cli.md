...
> [!NOTE]
> It is **not necessary** to configure or install certificates (rootCA, node or client, truststores etc) to connect to Azure Managed Instance for Apache Cassandra. Only enabling SSL encryption (with host/certificate verification disabled) from the client is required. 
