...
> [!NOTE]
> The time it takes to respond to a request to restore from backup will depend on the severity of support case you raise. For example, if you raise a Sev-A support case, the SLA for response to the ticket is 15 minutes. However, we do not provide an SLA for time to complete the restore, as this is very dependent on the volume of data being restored.
