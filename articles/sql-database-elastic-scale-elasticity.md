...
**Horizontal scaling** is a design pattern in which databases ("shards," in Elastic Scale terms) are added or removed from a **shard set** to grow or shrink capacity or performance. 
...
To handle the dramatic increase in transactions, the application scales horizontally. The application can now distribute the transaction load across many shards. When the the additional resources are no longer needed, the database tier shrinks back for normal usage. Here horizontal scaling enables an application to scale-out to match customer demand and scale-down when the resources are no longer needed.   
...
A canonical scenario for vertical scaling is an application that uses a shard set to store operational telemetry. In this scenario, it is better to co-locate all telemetry data for a single day on a single shard. That topography improves the ability to perform complex joins on data. In such an application, data for the current day is ingested into a shard and a new shard is provisioned for subsequent days. The operational data can then be aged and queried as appropriate. 

As the database ingests telemetry data at high loads, it is better to employ a higher performance service tier. In other words, a premium database is better than a basic database. Once the database reaches its capacity, it switches from ingestion to analysis and reporting. In this case, the standard tier's performance is equal to the task. Thus one can vertically scale down the service tier (or performance level) to fit the application pattern. 

Vertical scaling can also be used to increase the performance of a single database in order to achieve increased performance. An example is a a tax filing application. At filing time, it is better to keep a single customer’s data all on the same database. then increase the performance of that shard. Depending on the application, vertically scaling up and down resources is advantageous to optimize for both cost and performance requirements. 
...
As data is ingested into a particular shard, it is useful to project forward a day and determine if the shard has sufficient capacity to handle the coming workload. While not a true implementation of linear regression, the following query returns the maximum delta in database capacity between two consecutive days.  Such telemetry can then be applied to a rule that would then result in an action (or non-action) being taken. 
	...
	WITH MaxDatabaseDailySize AS( 
		...
		FROM [sys].[resource_usage] 
		WHERE  
			database_name = 'AdventureWorksElasticScale_20140617' 
...
The rule is the decision engine that determines whether or not an action is taken. Some rules are very straightforward and some are much more complicated. As shown in the code snippet below, a capacity-focused rule can be configured so that when a shard reaches 80% ($SafetyMargin) of its maximum capacity, a new shard is provisioned.
