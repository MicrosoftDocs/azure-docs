...
1.	Sign in the [Azure Management portal][azure.management.portal].
2.	Click **CREATE** in the bottom of the Service Bus page.  
...
2.	Select the newly create storage account and then click **MANAGE ACCESS KEYS** at the bottom of the page.
3.	Copy the account name and one of the access keys.
...
6.	Run the solution.  The usage is as follows: 
...
1.	In the Service Bus workspace, click the Service Bus namespace name.
...
1.	From the Management portal, click **NEW**, **SQL DATABASE**, **QUICK CREATE**.  Specify a database name on an existing or a new SQL Database server.
...
3.	Use the following T-SQL statements to create two tables to your database:
	...
	>[WACOM.NOTE] Clustered indexes are required on all tables in order to insert data.
...
5.	Type or select the following values on the third page: 
	...
	If your Event Hub is in a different subscription, select Use Event Hub from Another Subscription and manually enter the Service Bus Namespace, Event Hub Name, Event Hub Policy Name, Event Hub Policy Key, and Event Hub Partition Count.  Note that this sample uses the default number of partitions, which is 16.
...
8.	Click the check button to add this source and to verify that Stream Analytics can successfully connect to it.
...
3.	Click **DASHBOARD** on the top of the page, and then click **START** on the bottom of the page to start the Stream Analytics job.  The job will change its state to Starting and may take a couple of minutes to complete the starting process and move into the Running state.  You can view the status of the job on the Dashboard. 
