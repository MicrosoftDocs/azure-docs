...
| Total Keys  | The maximum number of keys in the cache during the past reporting time period. This maps to the Redis INFO `keyspace` command. Due to a limitation of the underlying metrics system, for caches with clustering enabled, Total Keys returns the maximum number of keys of the shard that had the maximum number of keys during the reporting interval.  |
