...
* Requests to the Azure Digital Twins API require a User or Service Principal that is a part of the same [Azure Active Directory](https://docs.microsoft.com/en-us/azure/active-directory/fundamentals/active-directory-whatis) (AAD) Tenant that the Azure Digital Twins Instance resides in.  To prevent bad actors from scanning urls to discover where Azure Digital Twins instances live, requests with access tokens from outside the originanting tenant will be returned a "404 Sub-Domain not found" error message. This error will be returned *even if* the User or Service Principal was given an Azure Digital Twins Data Owner or Azure Digital Twins Data Reader role through [AAD B2B](https://docs.microsoft.com/en-us/azure/active-directory/external-identities/what-is-b2b) collaboration.
