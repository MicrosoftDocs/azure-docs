...
For **right-to-left**, use this syntax:
...
For **non-directional**, use the following syntax. This will not specify a direction for the relationship, so relationships of any direction will be included in the result.
...
* T targets C with any type of relationship 
...
* C targets T with any type of relationship 
* C's `$dtId` is 'ABC'
* T's 'temp' value is greater than 50
...
>[!IMPORTANT]
> Assigning a query variable to the relationship is only supported when the query specifies a single hop. You must choose between specifying a relationship name and [specifying a greater number of hops](#specify-number-of-hops).
...
>[!NOTE]
> The examples below focus on relationship name. They all show non-directional relationships and don't assign query variables to the relationships. For instructions on how to do more with these other conditions, see [Specify relationship direction](#specify-relationship-direction) and [Assign query variable to relationship](#assign-query-variable-to-relationship-and-specify-relationship-properties).  For information about how to use several of these together in the same query, see [Combining MATCH operations](#combining-match-operations).
...
The following example has **no specified relationship name**. That means a relationship with any name can be included in the query result. This query finds twins t and c where...
...
>[!IMPORTANT]
>If you specify a number of hops that is greater than one, you can't specify a [relationship name](#specify-relationship-name). You must choose one or the other.
...
>[!NOTE]
>The examples below focus on number of hops. They all show non-directional relationships without specifying names or assigning query variables to the relationships. For instructions on how to do more with these other conditions, see [Specify relationship direction](#specify-relationship-direction), [Specify relationship name](#specify-relationship-name), and [Assign query variable to relationship](#assign-query-variable-to-relationship-and-specify-relationship-properties). For information about how to use several of these together in the same query, see [Combining MATCH operations](#combining-match-operations).
...
To specify a **range of hops**, use this syntax. The starting limit is not included in the range, while the ending limit is.
...
The following example specifies an **exact number of hops**. Only relationships that are exactly 3 hops between twins r and c will be returned.
...
The following example specifies a **range of hops**. Relationships that are between 1 and 3 hops (meaning the number of hops is either 2 or 3) between twins r and c will be returned.
...
You can also show a range by providing only one boundary. In the following example, relationships that are at most 2 hops (meaning the number of hops is either 1 or 2) between twins r and c will be returned.
...
>[!NOTE]
>The examples below focus on a query variable for the relationship. They all show non-directional relationships without specifying names or number of hops. For instructions on how to do more with these other conditions, see [Specify relationship direction](#specify-relationship-direction), [Specify relationship name](#specify-relationship-name), and [Specify number of hops](#specify-number-of-hops). For information about how to use several of these together in the same query, see [Combining MATCH operations](#combining-match-operations).
...
The following example assigns a query variable 'r' to the relationship. Later, in the `WHERE` clause, it specifies that the relationship r should have a length property of 10.
...
>[!NOTE]
>As per the options for [specifying relationship direction](#specify-relationship-direction), you must pick between a left angle bracket for a left-to-right relationship or a right angle bracket for a right-to-left relationship. You can't include both on the same arrow.

You can also **chain** multiple relationship conditions together, like this:
...
Here is an example that combines relationship direction, relationship name, and number of hops. The following query finds twins t and c, where the relationship between t and c meets these conditions:
...
* the relationship is left-to-right, with t as the source and c as the target
* the relationship has a name of either 'contains' or 'isAssociatedWith'
* the relationship, given a name of r, has a length property equal to 10
...
MATCH (t1)-[r:contains|isAssociatedWith*3..5]->(c)<-[has|includes*1..]-(t2)  
