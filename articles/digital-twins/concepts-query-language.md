...
* **Escape single quotes**: If your query text includes a single quote character in the data, the quote will need to be escaped with the `\` character. Here is an example that deals with a property value of *D'Souza*: `SELECT T FROM digitaltwins T WHERE name = 'D\'Souza'`.
