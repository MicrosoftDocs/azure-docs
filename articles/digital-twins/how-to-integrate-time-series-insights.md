...
>[!TIP]
>The simplest way to analyze historical twin data over time is to use the [Data History](concepts-data-history.md) feature to connect an Azure Digital Twins instance to an ADX cluster, so that digital twin property updates are automatically historized to Azure Data Explorer. You can then query this data in ADX using the [Azure Digital Twins query plugin for Azure Data Explorer](concepts-data-explorer-plugin.md). If you don't need to use Time Series Insights specifically, you might consider this alternative for a simpler integration experience.
