...
Azure Digital Twins [3D Scenes Studio (preview)](https://explorer.digitaltwins.azure.net/3dscenes) is an immersive 3D environment, where business and frontline workers can consume and investigate operational data from their Azure Digital Twins solutions with visual context.

## Prerequisites

To use 3D Scenes Studio, you need the following resources.
...
* An Azure Digital Twins instance. For instructions, see [Set up an instance and authentication](how-to-set-up-instance-cli.md).
    ...
    * Take note of the *host name* of your instance for later use.
* An Azure storage account. For instructions, see [Create a storage account](../storage/common/storage-account-create.md?tabs=azure-portal).
    * Take note of the *URL* of your storage account for later use.
* A private container in the storage account. For instructions, see [Create a container](../storage/blobs/storage-quickstart-blobs-portal.md#create-a-container).
    * Take note of the *name* of your storage container for later use.
...
* Configure CORS for your storage account (see details in the following subsection).

### Configure CORS

You need to configure [CORS](/rest/api/storageservices/cross-origin-resource-sharing--cors--support-for-the-azure-storage-services) for your storage account, so that 3D Scenes Studio is able to access your storage container. 
...
These other CORS headers are required if you're planning on using private links functionality:
* Content-Type
* Content-Length
* x-ms-copy-source
* x-ms-requires-sync

Following is the [Azure CLI](/cli/azure/what-is-azure-cli) command that sets the methods, origins, and headers previously listed for CORS in your storage account. The command contains one placeholder for the name of your storage account.
...
In this section, you set the environment in *3D Scenes Studio* and customize your scene for the sample graph that's in your Azure Digital Twins instance.

1. Navigate to the [3D Scenes Studio](https://explorer.digitaltwins.azure.net/3dscenes). The studio opens, connected to the Azure Digital Twins instance that you accessed last in the Azure Digital Twins Explorer.
...
You can use 3D Scenes Studio with a 3D file that's already present in your storage container, or you can upload the file directly to 3D Scenes Studio, which adds it to the container automatically. Here are the steps to use a 3D file to create a new scene.
...
To edit or delete a scene after it's created, use the **Actions** icons next to the scene in the 3D Scenes Studio home page.
...
Editing a scene reopens all of the scene properties you set while creating it, allowing you to change them and update the scene.
...
You can switch to **View** mode to enable filtering on specific elements and visualization of element behaviors that you created.
...
You can view **All properties** of an element from here, and their values over time if [data history](concepts-data-history.md) is enabled on your instance. To view property history, select the **Open data history explorer** icon.
...
This selection opens the **Data history explorer** for the property. For more information about using the data history explorer, see [Validate and explore historized properties](how-to-use-azure-digital-twins-explorer.md#validate-and-explore-historized-properties). 
...
:::image type="content" source="media/how-to-use-3d-scenes-studio/new-element-start-mesh.png" alt-text="Screenshot of 3D Scenes Studio in the builder for a scene. The mesh for a tank is highlighted and there's an option to Create new element." lightbox="media/how-to-use-3d-scenes-studio/new-element-start-mesh.png":::

Creating a new element opens the **New element** panel, where you can fill in element information.
...
Select a twin to link to this element. Linking the twin to this element automatically applies the digital twin ID (`$dtId`) as the element **Name**. You can rename the element if you want, to make it understandable for both builders and consumers of the 3D scene.
...
If you started element creation by selecting a mesh in the visualization, that mesh is already filled in here. You can select meshes in the visualization now to add them to the element.
...
For more information about creating new behaviors, see [Add behaviors](#add-behaviors).

### Other twins

On the **other twins** tab, you can add secondary digital twin data sources for an element. You can add other twins to an element if the data on the primary twin isn't enough to define all the behaviors you want for the element, so you need access to the data of other twins.
...
Once there are other twins added to the element, you're able to view and modify them on this tab.

## Add behaviors

A *behavior* is a scenario for your scene that uses particular data on the related element's digital twin to drive viewer visualizations.
...
Creating a new behavior opens the **New behavior** panel, where you can fill in behavior information.
...
>[!TIP]
>Choose a name that is clear to end users viewing the scene, because this behavior name is displayed as part of the scene visualization. 
...
If you started the behavior creation process from a specific element, that element is already selected here. Otherwise, you can choose elements here for the first time.
...
On the **Twins** tab, you can modify the set of twins whose data is available to this behavior, including the targeted elements' primary twins and any other twins.

You can add secondary digital twin data sources for an element. After configuring other twins, you'll be able to use properties from those twins in your behavior expressions for this element. You should only add other twins when there are other twins with data beyond your primary twin that you want to use in your [visual rules](#visual-rules) and [widgets](#widgets) for this behavior.
...
Adding a new twin data source opens a **New twin** panel, where you can name the new twin and select a twin from your Azure Digital Twins instance to map.
...
Then, set the **Values** to identify value boundaries for the condition (the exact selection changes depending on the property type). For numerical properties, the min of each value range is inclusive, and the max is exclusive.
...
Widgets are managed on the **Widgets** tab. *Widgets* are data-driven visuals that provide more context and data, to help you understand the scenario that the behavior represents. Configuring widgets helps you make sure the right data is discoverable when a certain condition is active.
...
* **Gauge**: For visually representing numerical data points

    Enter a **Display name** and **Unit of measure**, then choose whether the gauge reflects a **Single property** or a **Custom (advanced)** property expression. For a **Single property**, you get a dropdown list of numeric properties on the primary twin. For **Custom (advanced)**, you get a text box where you can write a custom JavaScript expression using one or more properties. The expression should have a numeric outcome. For more information about writing custom expressions, see [Use custom (advanced) expressions](#use-custom-advanced-expressions).

    Once you define your property expression, set value ranges to appear in certain colors on the gauge. The min of each value range is inclusive, and the max is exclusive.
...
* **Link**: For including externally referenced content via a linked URL
    ...
    Link URLs can also include calculation expressions with this syntax: `${<calculation-expression>}`. The previous screenshot contains an expression for accessing a property of the primary twin. Expressions are computed and displayed dynamically in the [viewer](#view-scenes-individually).

* **Value**: For directly displaying twin property values

    Enter a **Display name** and select a **Property expression** that you want to display. This property expression can be a **Single property** of the primary twin, or a **Custom (advanced)** property expression. Custom expressions should be JavaScript expressions using one or more properties of the twin, and you select which outcome type the expression produces. If your custom property expression outputs a string, you can also use JavaScript's template literal syntax to include a dynamic expression in the string output. Format the dynamic expression with this syntax: `${<calculation-expression>}`. Then, wrap the whole string output with backticks (`` ` ``). For more information about writing custom expressions, see [Use custom (advanced) expressions](#use-custom-advanced-expressions).
    ...
    Here's the value expression: `` PrimaryTwin.InFlow > 99 ? `${PrimaryTwin.$dtId} has an InFlow problem`  : 'Everything looks good' ``. The computed result of the expression (the `$dtId`) is shown in the widget in the viewer.
...
* **Data history**: For displaying property values over time. This widget works with Azure Digital Twins' [data history feature](concepts-data-history.md), so it's only available if you [enable data history](how-to-create-data-history-connection.md) for the instance being used in this 3D scene. Once a data history connection exists for the instance, you're able to display that data with this widget.
    
    The widget fills the **Connection string** field automatically, by targeting data in the Azure Data Explorer instance already connected to this Azure Digital Twins instance.
...
While defining [visual rules](#visual-rules) and [widgets](#widgets) in your behaviors, you might want to use custom expressions to define a property condition.
...
Selecting **New layer** prompts you to enter a name for the new layer you want to create.

Alternatively, you can create layers while [creating or modifying a behavior](#name-and-scene-layers). The behavior pane is also where you can add the behavior to a layer you already created.
...
* *Storage Blob Data Reader* access (or greater) to the storage container
    * *Storage Blob Data Reader* allows them to view your scenes.
    * *Storage Blob Data Owner* or *Storage Blob Data Contributor* allows them to edit your scenes.
...
After you give them access, the recipient can view and interact with your scenes in the studio.
...
When the recipient pastes this URL into their browser, the specified scene opens in the viewer, with any chosen layers or elements selected.
