...
This document describes patterns that can be used to implement tags on digital twin entities in ADT. Some customers want to replicate tags from existing systems, like Haystack Tags, in ADT. 
...
Get all red entities: select * from digitaltwins where is_defined(tags.red) 

Get all entities that are round but not red: select * from digitaltwins where not is_defined(tags.red) and is_defined(tags.round) 
...
Get all red entities (red is a marker tag): select * from digitaltwins where is_defined(tags.red) 

Get all entities that are small but not red: select * from digitaltwins where not is_defined(tags.red) and tags.size = 'small' 