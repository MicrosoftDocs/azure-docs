...
If you have an instance set up earlier, you can search for your instance by its name in the search bar of the portal.
...
**Prerequisites**: 
* You'll need _Event hub namespace_. Refer to [this] (https://docs.microsoft.com/azure/service-bus-messaging/service-bus-create-namespace-portal) link to create event hub namespace
* You'll need _Event Hub_. Refer to [this](https://docs.microsoft.com/azure/event-hubs/event-hubs-create) article to create a event hub. 
...
Search for your Azure Digital Twins instance by typing the resource name in the top search bar in the Azure portal. From the results, select your resource to create an endpoint of type Service Bus.
...
Create an event route by selecting _Event routes_ from the navigation bar on the left. Then, select _+ Create an event route_ link which opens up to a window 'Create an event route' on the right. 
...
This will auto populate the filter events which you can see as 'type = 'Microsoft.DigitalTwins.Twin.Create'. Hit _save_ button to create event route.
