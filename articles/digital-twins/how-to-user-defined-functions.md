...
Matchers are topology objects which determine what user-defined functions will be executed for a given telemetry message.
...
The following will trigger a notification if the temperature level rises above the pre-defined constant.
...
Telemetry generated by the sensor described in the topology should trigger the execution of the user-defined function that was just uploaded. Once the telemetry is picked up by the data processor, an execution plan is created for the invocation of the user-defined function.

1. Retrieve the matchers for the sensor the reading was generated off of.
1. Dependening on what matchers evaluated successfully, retrieve the associated user-defined functions.
...
Given a space identifier, retrieves the space from the topology.
...
Given a sensor identifier, retrieves the sensor from the topology.
...
Given a device identifier, retrieves the device from the topology.
...
Given a sensor identifier and it's data type, retrieve the current value for that sensor.
...
Given a sensor identifier and it's data type, retrieve the historical values for that sensor.
...
Given a space identifier, retrieve it's parent space.
...
Given a sensor identifier, retrieve it's parent space.
...
Given a device identifier, retrieve it's parent space.
...
Given a sensor identifier, retrieve it's parent device.
...
Given a space identifier, retrieve the property and it's value from the space.
...
Given a sensor identifier, retrieve the property and it's value from the sensor.
...
Given a device identifier, retrieve the property and it's value from the device.
...
| Param  | Type                | Description  |
| ------ | ------------------- | ------------ |
| topologyObjectId  | `Guid` | topology object identifier (ex. space / sensor /device id)|
...
Returns the extended property and it's value for the current space.
...
Returns the extended property and it's value for the current device.
...
Returns the extended property and it's value for the current sensor.
