...
* **Use retry logic.** The [Azure Digital Twins SDKs](how-to-use-apis-sdks.md) implement retry logic for failed requests, so if you are working with a provided SDK, this is already provided. Otherwise, consider implementing retry logic in your own application. The interval for retrying may vary based on the API and scenario.
