...
The Current Month Projected Cost report shows the costs of your services from the beginning of the month, the previous month, and current month projected cost. On the reports menu at the top of the portal, click **Cost** > **Projection and Budget** > **Current Month Projected Cost**. The following image shows an example.
...
Follow the same basic steps above to look at monthly cost projections for other services you're interested in.
...
AWS Reserved instances are an open commitment useful when you have sustained usage for VMs because reserved instances are less expensive than on-demand instances. The commitment is to use resources, typically VMs, for a defined period—usually a year. When you make the commitment to buy, you prepay for the resources with a reservation. However, you might not always fully use what you've committed to in the reservation.

For example, you might assess your environment and determine that you had 20 standard D2 instances running constantly over the last year. You could purchase a reservation for them and potentially save a lot of money. In a different example, you might have committed to using ten MA4 instances for the year, but you might have only used five to-date. In this example, you have an inefficiency. Using Cloudyn optimization reports, there are two ways to optimize costs for reserved instances:
...
In this example, the Cloudyn\_A account has 32 reserve instance buying recommendations. If you follow all the buying recommendations, you could potential save $137,770 annually. Keep in mind that the purchase recommendations provided by Cloudyn assume that usage for your running workloads will remain consistent.
...
The example above shows that running the workload on-demand would cost $90,456 annually. However, if you purchase the reservation in advance, the same workload would cost $56,592 and save you $33,864 annually.
...
Unused reservations are common in many cloud resource consumer's computing environments. Ensuring that unused reservations are fully used can save you money when you modify the reservations to meet your current needs. For example, if have a subscription containing standard D3 instances running on Linux and you're not going to fully utilize the reservation, you might change the instance type. Or, you might move the unused resources to a different reservation or to a different account.
...
The following images shows the report with unused reserved instances.
...
In the example above, there are 77 unused reservations total in various availability zones. The first reservation has 51 unused instances. Looking lower in the list, there are potential reservation instance modifications that you can make using the **m3.2xlarge** instance type in the **us-east-1c** availability zone.
...
Reserve instances that you can modify are listed. In example image below, there are 51 unused reservations that you can modify but there is a need for 54 between the two reservations. If you modify your unused reservations to use them all, four instances will continue to run on demand. For this example, split your unused reservations where the first reservation will use 30 and the second reservation will use 21.
