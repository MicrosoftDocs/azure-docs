...
Indicates whether or not the Azure CDN integration for this StreamingEndpoint is enabled (disabled by default.)

To set the Azure CDN integration to true, the StreamingEndpoint must have at least one streaming (scale) unit and be in the stopped state. If later you want to set scale units to 0, you must first disable the CDN integration. 
