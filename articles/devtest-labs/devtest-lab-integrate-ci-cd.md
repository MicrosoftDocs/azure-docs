...
1. In the **Default branch** list, select the branch to checkout.
1. Select **Add**.
...
1. In the right pane, fill out the form as follows:
   ...
   - Expand **Output Variables**, and under **Reference name**, enter the variable for the created lab VM ID. If you use the default *labVmId*, you can refer to the variable in subsequent tasks as **$(labVmId)**.
...
1. On the release pipeline **Pipeline** tab, select the plus sign **+** next to **Agent job**.
...
1. In the right pane, fill out the form as follows:
   ...
   - **Azure PowerShell Version**: Select **Latest installed version**.
