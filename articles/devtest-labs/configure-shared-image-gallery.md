...
- When creating a virtual machine using a shared image gallery image, DevTest Labs always uses the latest published version of this image. However if an image has multiple versions, users can chose to create a machine from an earlier version by going to the Advanced settings tab during virtual machine creation.  
