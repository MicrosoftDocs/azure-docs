...
1. On the **Choose a base** page, search for and select **Windows Server 2019 Datacenter** as base for the VM. Some of the following options might be different if you use a different image.
...
1. Optionally, select the **Advanced Settings** tab to change any of the following settings:
   ...
   - **Make this machine claimable**: The default of **No** keeps the VM creator as the owner of the VM. For this tutorial, select **Yes** to allow another lab user to claim the VM after creation. For more information, see [Create and manage claimable VMs](devtest-lab-add-claimable-vm.md).
...
1. On the **Select members** screen, select the user or users to assign the role to, and then select **Select**.
...
If a lab user has a Microsoft account but isn't a member of your organization's Active Directory, they might see an error message when they try to access the shared link. Ask the user to first select their name in the upper-right corner of the Azure portal. They can then select the directory where the lab exists from the **Directory** section of the menu.
...
Deleting the lab removes all of the lab resources from the resource group. If you created the resource group for the lab, you can now delete the resource group. You can't delete a resource group that has a lab in it. Deleting the resource group that contained the lab deletes all resources in the resource group. To delete the resource group:
