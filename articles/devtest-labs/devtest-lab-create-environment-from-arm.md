...
Once you either enable the public environment repository, or add an environment template repository to a lab, lab users can use the repository templates to create environments:

1. On the lab's **Overview** page, select **Add** from the top toolbar.

1. On the **Choose a base** page, select the ARM environment template you want to use. The available environment templates appear first in the list of bases.
...
1. On the **Add** screen, enter an **Environment name**. The ARM template defines the rest of the input fields. You enter values for those input fields that the template *azuredeploy.parameters.json* file defines as blank or default.
...
1. In the [Azure portal](https://portal.azure.com), on the lab's **Overview** page, select **Configuration and policies** from the left navigation.
