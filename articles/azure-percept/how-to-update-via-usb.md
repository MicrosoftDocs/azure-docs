...
1. Set the dev kit into USB mode using SSH or DIP switches, so that it can be detected by the host computer and receive the update package
...
1. The command prompt window will display a message that say **Waiting for Known USB Device to Appear...** The UUU tool is now waiting for the dev kit to be detected by the host computer. **Proceed to the next steps and put the dev kit into USB mode.**
...
### Using SSH
Setting the dev kit into "USB mode" using SSH is the safest and preferred method as it does not require you to modify the dev kit hardware. However, it does require that you are able to connect to the dev kit's wi-fi access point. If you are unable to connect to the dev kit's wi-fi access point then you will need to use the DIP switch method.
...
Use the DIP switch method when you cannot SSH into to the device.

1. Unplug the dev board if it is plugged into the power cable.
...
1. Gently slide the dev board in the direction of the LEDs. Note that the heat sink will remained attached to the top of the dev board. Only slide the dev board 2 - 3 centimeters to avoid disconnecting any cables.
...
1. The dev kit is now "USB mode" and you can proceed with the next steps. **Once the update is completed, change the DIP switches back to the default position of up-down-down-up (1001).** Then slide the dev board back into position and reapply the four screws on the back.

## Connect the dev kit to the host computer via a USB-C cable
This procedure uses the dev kit's single USB-C port for updating.  If your computer has a USB-C port, you can use the USB-C to USB-C cable that came with the dev kit.  If your computer only has a USB-A port, you will need to use a USB-C to USB-A cable (sold separately).

1. Connect the dev kit to the host computer using the appropriate appropriate USB-C cable.
