...
|-------|---------|---------|
| On-boarding experience | Can’t complete on-boarding experience unless device’s Wi-Fi is configured (Azure login fails). | 1. SSH to the device access point (10.1.1.1) 2. Identify and copy the device ethernet IP address 3. Connect to on-boarding experience using the copied ethernet IP-based URL |
...
|Azure Percept Studio | "View stream" links within Azure Percept Studio do not open a new window showing the device's web stream. | Open the [Azure portal](https://ms.portal.azure.com/#home) and select **IoT Hub**. Click on the IoT Hub to which your device is connected. Select **IoT Edge** under **Automatic Device Management** on your IoT Hub page. Select your device from the list. Select **Set modules** at the top of your device page. Click the trashcan icon next to **HostIpModule** to delete the module. To confirm the action, click **Review + create** and then **Create**. Open [Azure Percept Studio](https://ms.portal.azure.com/#blade/AzureEdgeDevices/Main/overview) and click **Devices** on the left menu panel. Select your device from the list. On the **Vision** tab, click **View your device stream**. Your device will download a new version of HostIpModule and open a browser tab with your device's web stream. | 
