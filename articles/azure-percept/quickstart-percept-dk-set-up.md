...
- The Azure account must have the **owner** or **contributor** role on the subscription. Follow the steps below to check your Azure account role. For more information on Azure role definitions, click [here](https://docs.microsoft.com/azure/role-based-access-control/rbac-and-directory-admin-roles#azure-roles).
...
### Azure account role check

To verify if your Azure account is an “owner” or “contributor” on the subscription, follow these steps:
...
1. Check the role:
    - If your role is listed as **Reader** or if you get a message that says you do not have permissions to see roles, you will need to follow the necessary process in your organization to elevate your account role.
...
1. Once connected to the devkit’s Wi-Fi access point, the host device will automatically launch the Azure Percept DK setup experience in a new browser window showing **your.new.device/** in the address bar. If it does not automatically open, you can launch it manually by opening a browser window and navigating to [http://10.1.1.1](http://10.1.1.1).

    :::image type="content" source="./media/quickstart-percept-dk-setup/main-01-welcome.png" alt-text="Welcome page.":::

    > [!CAUTION]
    > The Azure Percept DK setup experience web service will shut down after 30 minutes of non-use and at the completion of the setup experience. When this happens, it is recommended to restart the dev kit to avoid connectivity issues with the devkit Wi-Fi access point.
...
1. Select your wi-fi network from the list of available networks and click **connect**. (This will require your local wi-fi password.)
    <!---
    :::image type="content" source="./media/quickstart-percept-dk-setup/main-03-select-wi-fi.png" alt-text="Select wi-fi network."::: 
    --->
1. Once your devkit has successfully connected to your network of choice, the next page will show the IPv4 address assigned to your dev kit. **Write down the IPv4 address displayed on the page.** You will need the IP address later in this quick start guide and when connecting to your devkit over SSH for troubleshooting and device updates.
...
1. Set up an SSH login for remote access to your devkit. SSH (Secure Shell) is a network protocol that enables you to connect to the dev kit's operating system remotely via a host computer. This comes in handy when troubleshooting issues with the dev kit. On this page you will create an SSH login account and password. **Write down your login information for later use**.
...
1. On the next page, click **Connect with a new device** to begin the process of connecting your dev kit to Azure IoT Hub. 
...
1. Click the **Copy** link to copy your device code. Then click **Login to Azure**. 
...
1. The next window requires you to **sign in with the Azure account you verified at the beginning of the Quick Start**. Enter those login credentials and click **Next**. 
...
1. When the deployment is completed, click the **Register** button
...
1. Your devkit has been successfully linked to your Azure IoT Hub and you should see a screen that says **Device setup complete!**. You are now ready to deploy your first vision AI model to the dev kit. At this point, the setup experience service will shut down and your dev kit will disconnect from the host computer. As a result, you may see these notifications: 
    ...
    **Before proceeding to Azure Percept Studio to deploy your vision AI models, connect your host computer to the Wi-Fi network you set up in Step 2**.

1. After connecting to your Wi-Fi network, click on the **Continue to the Azure portal** button to begin creating your custom AI models in Azure Percept Studio.
...
## View your Device in the Azure Percept Studio and deploy common prebuilt sample models

1. The [Azure Percept Studio Overview page](https://go.microsoft.com/fwlink/?linkid=2135819) is your launch point for accessing many different workflows for both beginning and advanced edge AI model and solution development. Click on **Devices** from the left menu. 
...
1. Click on **View stream** in the "View your device stream" tile. If this is the first time viewing the video stream of your device, you will see a notification that a new model is being deployed in the upper right-hand corner. This may take a few minutes.
    ...
    Once the model is finished deploying, you may get a notification with a **view stream** link. After clicking on that link, a new browser window will open showing the video output from your Azure Vision camera. It will also automatically perform object detection of many common objects.
...
1. Azure Precept Studio has a number of common pre-built sample models which can be deployed with a single click. Click **Deploy a model** in the "Deploy a sample model" tile.
...
1. Select a sample from the library and click on **Deploy to device**.
...
1. Once the model is successfully deployed, you will see a notification in the upper right corner of the screen. To view the output from the model, click on the **View stream** in that notification or from the main page. Any models previously running on the dev kit will now be replaced with the new model.
