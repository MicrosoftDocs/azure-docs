...
###<a name="Heading1"></a>Introduction to table design  
...
###<a name="Heading2"></a>Azure Table service overview  
This section highlights some of the key features of the Table service that are especially relevant to designing for performance and scalability. If you are new to Azure Storage and the Table service, you should read the articles "[Introduction to Microsoft Azure Storage](http://azure.microsoft.com/en-us/documentation/articles/storage-introduction/)" and "[How to use Table Storage from .NET](http://azure.microsoft.com/en-us/documentation/articles/storage-dotnet-how-to-use-tables/)" before reading the remainder of this article. Although the focus of this guide is on the Table service, it will include some discussion of the Azure Queue and Blob services, and how you might use them along with the Table service in a solution.  
...
####<a name="EGTs"></a>Entity Group Transactions (EGTs)  
...
For more information, see [Understanding the Table Service Data Model](http://msdn.microsoft.com/en-us/library/dd179338.aspx) on MSDN.  
...
For a comparison between Azure SQL Database (a relational database service) and the Table service, see [Windows Azure Table Storage and Windows Azure SQL Database - Compared and Contrasted](http://msdn.microsoft.com/en-us/library/jj553018.aspx) on MSDN.  

###<a name="Heading3"></a>Guidelines for table design  
...
###<a name="Heading4"></a>Design for querying  
Table service solutions may be read intensive, write intensive, or a mix of the two. This section focuses on the things to bear in mind when you are designing your Table service to support read operations efficiently. Typically, a design that supports read operations efficiently is also efficient for write operations. However, there are additional considerations to bear in mind when designing to support write operations, which the next section, "[Design for data modification](#DataMod)," discusses.
...
-	[How your choice of PartitionKey and RowKey impacts query performance](#Subheading4.1)
-	[Choosing an appropriate PartitionKey](#Subheading4.2)
-	[Optimizing queries with a key value store for the Table service](#Subheading4.3)
-	[Sorting data in a key value store in the Table service](#Subheading4.4)

####<a name="Subheading4.1"</a>How your choice of PartitionKey and RowKey impacts query performance  
...
-	[Retrieving a single entity using the Storage Client Library](#Subheading8.1.1)
-	[Retrieving multiple entities using LINQ](#Subheading8.1.2)
-	[Server-side projection](#Subheading8.1.4)  

For examples of client-side code that can handle multiple entity types stored in the same table, see:  

-	[Working with heterogeneous entity types](#Subheading8.1.3)  

####<a name="Subheading4.2"></a>Choosing an appropriate PartitionKey  
...
There are additional considerations in your choice of **PartitionKey** that relate to how you will insert, update, and delete entities: see the section "[Design for data modification](#DataMod)" below.  

####<a name="Subheading4.3"></a>Optimizing queries for the Table service  
...
Many designs must meet requirements to enable lookup of entities based on multiple criteria. For example, locating employee entities based on email, employee id, or last name. The following patterns in the section "[Table Design Patterns](#Heading7)" address these types of requirement and describe ways of working around the fact that the Table service does not provide secondary indexes:  

-	[Intra-partition secondary index pattern](#Subheading7.1) - Store multiple copies of each entity using different **RowKey** values (in the same partition) to enable fast and efficient lookups and alternate sort orders by using different **RowKey** values.  
-	[Inter-partition secondary index pattern](#Subheading7.2) - Store multiple copies of each entity using different RowKey values in separate partitions or in separate tables to enable fast and efficient lookups and alternate sort orders by using different **RowKey** values.  
-	[Index Entities Pattern](#Subheading7.4) - Maintain index entities to enable efficient searches that return lists of entities.  

####<a name="Subheading4.4"></a>Sorting data in in the Table service  
...
Many applications have requirements to use data sorted in different orders: for example, sorting employees by name, or by joining date. The following patterns in the section "[Table Design Patterns](#Heading7)" address how to alternate sort orders for your entities:  

-	[Intra-partition secondary index pattern](#Subheading7.1) - Store multiple copies of each entity using different RowKey values (in the same partition) to enable fast and efficient lookups and alternate sort orders by using different RowKey values.  
-	[Inter-partition secondary index pattern](#Subheading7.2) - Store multiple copies of each entity using different RowKey values in separate partitions in separate tables to enable fast and efficient lookups and alternate sort orders by using different RowKey values. 
-	[Log tail pattern](#Subheading7.7) - Retrieve the *n* entities most recently added to a partition by using a **RowKey** value that sorts in reverse date and time order.  

###<a name="DataMod"></a>Design for data modification
This section focuses on the design considerations for optimizing inserts, updates, and deletes. In some cases, you will need to evaluate the trade-off between designs that optimize for querying against designs that optimize for data modification just as you do in designs for relational databases (although the techniques for managing the design trade-offs are different in a relational database). The section, "[Table Design Patterns](#Heading7)," describes some detailed design patterns for the Table service and highlights some these trade-offs. In practice, you will find that many designs optimized for querying entities also work well for modifying entities.  
 
The topics covered in this section include:  
 
-	[Optimizing the performance of insert, update, and delete operations](#Subheading5.1)  
-	[Ensuring consistency in your stored entities](#Subheading5.2)  
-	[Ensuring your design for efficient modifications facilitates efficient queries](#Subheading5.3)  


####<a name="Subheading5.1"></a>Optimizing the performance of insert, update, and delete operations  
...
The following patterns in the section "[Table Design Patterns](#Heading7)" address optimizing the performance or your insert, update, and delete operations:  

-	[High volume delete pattern](#Subheading7.8) - Enable the deletion of a high volume of entities by storing all the entities for simultaneous deletion in their own separate table; you delete the entities by deleting the table.  
-	[Data series pattern](#Subheading7.9) - Store complete data series in a single entity to minimize the number of requests you make.  
-	[Wide entities pattern](#Subheading7.10) - Use multiple physical entities to store logical entities with more than 252 properties.  
-	[Large entities pattern](#Subheading7.11) - Use blob storage to store large property values.  

####<a name="Subheading5.2"></a>Ensuring consistency in your stored entities  
...
The following patterns in the section "[Table Design Patterns](#Heading7)" address managing consistency:  

-	[Intra-partition secondary index pattern](#Subheading7.1) - Store multiple copies of each entity using different **RowKey** values (in the same partition) to enable fast and efficient lookups and alternate sort orders by using different **RowKey** values.  
-	[Inter-partition secondary index pattern](#Subheading7.2) - Store multiple copies of each entity using different RowKey values in separate partitions or in separate tables to enable fast and efficient lookups and alternate sort orders by using different **RowKey** values.  
-	[Eventually consistent transactions pattern](#Subheading7.3) - Enable eventually consistent behavior across partition boundaries or storage system boundaries by using Azure queues. Enable eventually consistent behavior across partition boundaries or storage system boundaries by using Azure queues.  
-	[Index Entities Pattern](#Subheading7.4) - Maintain index entities to enable efficient searches that return lists of entities.  
-	[Denormalization pattern](#Subheading7.5) - Combine related data together in a single entity to enable you to retrieve all the data you need with a single point query.  
-	[Data series pattern](#Subheading7.9) - Store complete data series in a single entity to minimize the number of requests you make.  

For information about EGTs, see the section "[Entity Group Transactions (EGTs)](#EGTs)."  

####<a name="Subheading5.3"></a>Ensuring your design for efficient modifications facilitates efficient queries  

In many cases, a design for efficient querying results in efficient modifications, but you should always evaluate whether this is the case for your specific scenario. Some of the patterns in the section [Table Design Patterns](#Heading7) explicitly evaluate trade-offs between querying and modifying entities, and you should always take into account the number of each type of operation.  

The following patterns in the section "[Table Design Patterns](#Heading7)" address trade-offs between designing for efficient queries and designing for efficient data modification:  

-	[Compound key pattern](#Subheading7.6) - Use compound **RowKey** values to enable a client to lookup related data with a single point query.  
-	[Log tail pattern](#Subheading7.7) - Retrieve the *n* entities most recently added to a partition by using a **RowKey** value that sorts in reverse date and time order.  

###<a name="Heading6"></a>Modelling relationships  
...
####<a name="Subheading6.1"></a>One-to-many relationships  
...
For more information, see the "[Denormalization pattern](#Subheading7.5)" later in this guide.  
...
-	Handling large entities (for more information, see "[Working with large entities](#Subheading7.10)").  
-	Implementing access controls (for more information, see "[Controlling access with Shared Access Signatures](#Subheading8.4)").  

####<a name="Subheading6.3"></a>Join in the client  
...
####<a name="Subheading6.4"></a>Inheritance relationships  
...
For more information about working with multiple entity types in the same table in client code, see the section "[Working with heterogeneous entity types](#Subheading8.3)" later in this guide. This provides examples of how to recognize the entity type in client code.  

###<a name="Heading7"></a>Table Design Patterns
...
The pattern map above highlights some relationships between patterns (blue) and anti-patterns (orange) that are documented in this guide. There are of course many other patterns that are worth considering. For example, one of the key scenarios for Table Service is to store [Materialized Views](https://msdn.microsoft.com/en-us/library/dn589782.aspx) from the [Command Query Responsibility Segregation](https://msdn.microsoft.com/en-us/library/jj554200.aspx) (CQRS) pattern.  

####<a name="Subheading7.1"></a>Intra-partition secondary index pattern
...
$filter=(PartitionKey eq 'Sales') and (RowKey ge 'email_a') and (RowKey lt 'email_b')  
 ...
 Note that the filter syntax used in the examples above is from the Table service REST API, for more information see [Query Entities](http://msdn.microsoft.com/en-us/library/dd179421.aspx) on MSDN.  
...
-	You can keep your duplicate entities consistent with each other by using EGTs to update the two copies of the entity atomically. This implies that you should store all copies of an entity in the same partition. For more information, see the section "[Using Entity Group Transactions](#EGTs)."  
...
-	[Inter-partition secondary index pattern](#Subheading7.2)
-	[Compound key pattern](#Subheading7.6)
-	[Entity Group Transactions (EGTs)](#EGTs)
-	[Working with heterogeneous entity types](#Subheading8.3)

####<a name="Subheading7.2"></a>Inter-partition secondary index pattern
...
-	You can keep your duplicate entities eventually consistent with each other by using the [Eventually consistent transactions pattern](#Subheading7.3) to maintain the primary and secondary index entities.  
-	Table storage is relatively cheap to use so the cost overhead of storing duplicate data should not be a major concern. However, you should always evaluate the cost of your design based on your anticipated storage requirements and only add duplicate entities to support the queries your client application will execute.  
-	The value used for the **RowKey** must be unique for each entity. Consider using compound key values.  
-	Padding numeric values in the **RowKey** (for example, the employee id 000223), enables correct sorting and filtering based on upper and lower bounds.  
-	You do not necessarily need to duplicate all the properties of your entity. For example, if the queries that lookup the entities using the email address in the **RowKey** never need the employee’s age, these entities could have the following structure:
...
-	[Eventually consistent transactions pattern](#Subheading7.3)  
-	[Intra-partition secondary index pattern](#Subheading7.1)  
-	[Compound key pattern](#Subheading7.6)  
-	[Entity Group Transactions (EGTs)](#EGTs)  
-	[Working with heterogeneous entity types](#Subheading8.3)  

####<a name="Subheading7.3"></a>Eventually consistent transactions pattern  
...
The following patterns and guidance may also be relevant when implementing this pattern:  
-	[Entity Group Transactions (EGTs)](#EGTs)  
-	[Merge or replace](#Subheading8.3)  
...
####<a name="Subheading7.4"></a>Index Entities Pattern
...
-	You can implement a queue-based solution that delivers eventual consistency (see the [Eventually consistent transactions pattern](#Subheading7.3) for more details).  
...
####<a name="Subheading7.5"></a>Denormalization pattern  
...
####<a name="Subheading7.6"></a>Compound key pattern  
...
####<a name="Subheading7.7"></a>Log tail pattern  
...
-	[Prepend / append anti-pattern](#Subheading7.12)  
-	[Retrieving entities](#Subheading8.1)  

####<a name="Subheading7.8"></a>High volume delete pattern  
...
-	[Entity Group Transactions (EGTs)](#EGTs)
-	[Modifying entities](#Subheading8.3)  

####<a name="Subheading7.9"></a>Data series pattern  
...
-	[Large entity pattern](#Subheading7.10)  
-	[Merge or replace](#Subheading8.3)  
-	[Eventually consistent transactions pattern](#Subheading7.3) (if you are storing the data series in a blob)  

####<a name="Subheading7.10"></a>Wide entities pattern  
...
####<a name="Subheading7.11"></a>Large entities pattern  
...
-	To maintain eventual consistency between the entity in the Table service and the data in the Blob service, use the [Eventually consistent transactions pattern](#Subheading7.3) to maintain your entities.    
...
-	[Eventually consistent transactions pattern](#Subheading7.3)  
-	[Wide entities pattern](#Subheading7.10)

####<a name="Subheading7.12"></a>Prepend/append anti-pattern  
...
#####However, the problem with this schema is that to retrieve all the log messages for a specific time span you must search every partition in the table.Solution  
...
###<a name="Heading8"></a>Implementation considerations  
...
####<a name="Subheading8.1"></a>Retrieving entities  

As discussed in the section "[Design for querying](#Heading4)," the most efficient query is a point query. However, in some scenarios you may need to retrieve multiple entities. This section describes some common approaches to retrieving entities using the Storage Client Library.  

#####<a name="Subheading8.1.1"></a>	Executing a point query using the Storage Client Library  
...
#####<a name="Subheading8.1.2"></a>	Retrieving multiple entities using LINQ  
...
#####<a name="Subheading8.1.3"></a>	Retrieving large numbers of entities from a query  
...
#####<a name="Subheading8.1.4"></a>	Server-side projection  
...
####<a name="Subheading8.2"></a>Modifying entities  
...
####<a name="Subheading8.3"></a>Working with heterogeneous entity types  
...
The techniques discussed in this section are especially relevant to the discussion "[Inheritance relationships](#Subheading6.4)" earlier in this guide in the section "[Modelling relationships](#Heading6)."  
...
####<a name="Subheading8.4"></a>Controlling access with Shared Access Signatures  
...
For more information about using SAS tokens with the Table service, see [Shared Access Signatures, Part 1: Understanding the SAS Model](http://azure.microsoft.com/en-us/documentation/articles/storage-dotnet-shared-access-signature-part-1/).  
...
####<a name="Subheading8.5"></a>Asynchronous and parallel operations  
