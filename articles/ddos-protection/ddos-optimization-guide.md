...
| **Choose Network Protection** when you need comprehensive coverage of the public IP addresses in your virtual networks, or when you protect resources across multiple subscriptions in the same tenant. | Network Protection is a fixed monthly plan that covers up to 100 public IP addresses across all subscriptions in your tenant, so its effective per-IP cost falls as the number of protected resources grows. You also get simplified management with automatic protection for new resources. Compare the two models against your own resource counts before you choose: see [Compare pricing between Azure DDoS Protection tiers](ddos-pricing-guide.md) and [Azure DDoS Protection pricing](https://azure.microsoft.com/pricing/details/ddos-protection/). To configure Network Protection, see [DDoS Network Protection configuration](manage-ddos-protection.md). |
