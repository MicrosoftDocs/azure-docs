...
Use Durable Functions, a feature of [Azure Functions](../../azure-functions/functions-overview.md), to build stateful serverless functions in C#. Durable Functions automatically manages state persistence, checkpoints, and restarts, so you can focus on your orchestration logic.

Durable Functions supports two .NET process models for class library functions. To learn more, see [Differences between in-process and isolated worker process .NET Azure Functions](../../azure-functions/dotnet-isolated-in-process-differences.md).
...
1. At the prompts, select or enter the following values:
    ...
    | **Select a .NET runtime** | Select **.NET 8.0 isolated**. | Creates a Functions project that supports .NET 8 running in an isolated worker process and the Azure Functions Runtime 4.0. For more information, see [How to target Azure Functions runtime version](../../azure-functions/functions-versions.md). |
    ...
    | **Choose a durable storage type** | Select **Azure Storage**. | The default storage provider for Durable Functions. For more information, see [Durable Functions storage providers](../common/durable-task-storage-providers.md). |
...
* The latest [Azure Functions Core Tools](../../azure-functions/functions-run-local.md) is installed.
...
For more troubleshooting steps, see the [Azure Functions Core Tools reference](../../azure-functions/functions-core-tools-reference.md).
...
1. In Visual Studio Code, set a breakpoint in the `SayHello` activity function code, and then select F5 to start the function app project. The terminal panel displays output from Core Tools.
   ...
   >
   > If the message *No job functions found* appears, [update your Azure Functions Core Tools installation to the latest version](./../../azure-functions/functions-core-tools-reference.md).
...
1. Copy the URL value for `statusQueryGetUri`, paste it in your browser's address bar, and execute the request. Alternatively, you can also continue to use the HTTP test tool to issue the GET request.
   ...
   > [!TIP]
   > Learn how you can observe the [replay behavior](../common/durable-task-orchestrations.md#reliability) of a Durable Functions app through breakpoints.
...
1. On **Additional information**, use the settings that are described in the next table.
    ...
    | ------------ | ------- | ----------------------------------------- |
    | **Functions worker** | Select **.NET 8 Isolated (Long Term Support)**. | Creates an Azure Functions project that supports .NET 8 running in an isolated worker process and the Azure Functions Runtime 4.0. For more information, see [How to target the Azure Functions runtime version](../../azure-functions/functions-versions.md). |
...
1. To use the Azurite emulator, make sure that the **Use Azurite for runtime storage account (AzureWebJobStorage)** checkbox is selected. To create a Functions project by using a Durable Functions orchestration template, select **Create**. The project has the basic configuration files that you need to run your functions.
   ...
   > [!NOTE]
   > You can choose other storage options for your Durable Functions app. For more information, see [Durable Functions storage providers](../common/durable-task-storage-providers.md).
...
| `SayHello` | A simple function app that returns *hello*. This function contains the business logic that is orchestrated. |
| `HttpStart` | An [HTTP-triggered function](../../azure-functions/functions-bindings-http-webhook.md) that starts an instance of the orchestration and returns a *check status* response. |
