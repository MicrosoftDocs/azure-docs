...
Non-.NET languages use [extension bundles](../../azure-functions/extension-bundles.md) to manage the Durable Functions extension automatically. You only need to install the Durable Functions SDK for your language:
...
Durable Functions uses a [storage provider](../common/durable-task-storage-providers.md) to persist orchestration state, entity state, and internal messages. The recommended provider is [Durable Task Scheduler](../scheduler/durable-task-scheduler.md), an Azure-managed backend that requires no additional packages—only configuration. To get started, see [Configure Durable Functions with Durable Task Scheduler](../scheduler/quickstart-durable-task-scheduler.md).
...
> [!WARNING]
> The Netherite storage provider is retiring. If you currently use Netherite, plan to migrate to the [Durable Task Scheduler](../scheduler/durable-task-scheduler.md) or another supported provider.

Non-.NET languages manage the extension through [extension bundles](../../azure-functions/extension-bundles.md), so no additional storage provider package is needed in your application project.

> [!TIP]
> See the [storage providers guide](../common/durable-task-storage-providers.md) for complete instructions on comparing and configuring each backend.
