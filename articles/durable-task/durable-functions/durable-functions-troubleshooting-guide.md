...
If a .NET Durable Functions app uses steadily more memory while it runs with an attached managed debugger, the growth is often caused by the debugger rather than by your app or the Durable Functions runtime.

Every time an orchestrator function replays, it recreates the tasks that the orchestration awaits. Some of those tasks never complete, because the replay ends as soon as the orchestrator reaches work that isn't in the history yet. When a managed debugger is attached, the .NET runtime tracks incomplete async tasks so that debugger features such as async call stacks work. This tracking keeps the replayed tasks in memory, so memory use can grow steadily over a long debug session and eventually cause an out-of-memory error. Orchestrations that fan out to many tasks, run for hours, or call `ContinueAsNew` frequently replay the most, so they show the largest growth.
...
|taskHub|The name of your [task hub](../common/durable-task-hubs.md).|
...
Apps running on the Azure Functions Consumption plan are subject to [connection limits](../../azure-functions/manage-connections.md?toc=/azure/durable-task/durable-functions/toc.json). Common symptoms include:
...
The **Diagnose and solve problems** tab in the Azure portal can help monitor and diagnose issues related to your application and suggest potential solutions. For more information, see [Azure Function app diagnostics](../../azure-functions/functions-diagnostics.md).
