...
The following query shows historical tracking data for a single instance of the [Hello Sequence](../common/durable-task-sequence.md) function orchestration. It filters out replay execution so that only the *logical* execution path is shown. You can order events by sorting by `timestamp` and `sequenceNumber` as shown in the following query:
...
| `slotName` | The [deployment slot](../../azure-functions/functions-deployment-slots.md) in which the current function app is running. This field is useful when you use deployment slots to version your orchestrations. |
...
[Configure your function app with an Application Insights resource](../../azure-functions/configure-monitoring.md#enable-application-insights-integration).
...
1. Start a new debug session with a [fresh task hub](../common/durable-task-hubs.md#byo-storage-provider-task-hub-management) or clear the task hub contents between sessions. Leftover messages from previous runs can cause unexpected re-execution.
...
3. Step through your code as normal. Keep in mind the following behaviors:
    ...
    - **Replay:**  
       Orchestrator functions regularly [replay](../common/durable-task-orchestrations.md#reliability) when new inputs are received. A single *logical* execution of an orchestrator function can result in hitting the same breakpoint multiple times, especially if it's set early in the function code.
...
The Durable Functions Roslyn Analyzer is a live code analyzer that guides C# developers to follow Durable Functions specific [code constraints](../common/durable-task-code-constraints.md). For instructions on how to enable it in Visual Studio and Visual Studio Code, see [Durable Functions Roslyn Analyzer](durable-functions-roslyn-analyzer.md).
...
> [Learn more about monitoring in Azure Functions](../../azure-functions/functions-monitoring.md)
