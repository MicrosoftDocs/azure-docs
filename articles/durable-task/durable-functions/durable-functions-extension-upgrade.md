...
Update the Durable Functions NuGet package reference in your project. The correct package depends on your hosting model and [storage provider](../common/durable-task-storage-providers.md):
...
Non-.NET apps (JavaScript, Python, Java, PowerShell) use [extension bundles](../../azure-functions/extension-bundles.md) to access triggers and bindings, including the Durable Functions extension. Verify that the `extensionBundle` version range in your `host.json` includes the [latest bundle release](https://github.com/Azure/azure-functions-extension-bundles/releases):
...
- [Troubleshooting guide](../sdks/durable-task-sdk-troubleshooting.md)
