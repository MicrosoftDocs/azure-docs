...
> [!NOTE]
> If you use the [Durable Task Scheduler](../scheduler/durable-task-scheduler.md) instead of the default Azure Storage provider, Scenario 2 is the recommended disaster recovery approach.

## Background

In Durable Functions, all state is persisted in a storage backend. By default, this backend is Azure Storage, but you can also use the [Durable Task Scheduler](../scheduler/durable-task-scheduler.md) or a [SQL Server database](../common/durable-task-storage-providers.md). A [task hub](../common/durable-task-hubs.md) is a logical container for the storage resources that are used for [orchestrations](../common/programming-model-overview.md#orchestrators) and [entities](../common/programming-model-overview.md#entities). Orchestrator, activity, and entity functions can interact with each other only when they belong to the same task hub. This article refers to task hubs when describing scenarios for keeping these storage resources highly available.
