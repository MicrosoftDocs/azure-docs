...
description: Quickstart to set up and explore a sample Azure Digital Twins project.
author: baanders
...
The first step in creating an Azure Digital Twins solution is concepting and defining twin [**models**](concepts-models.md) for your environment. 
...
```cmd
CreateDigitalTwin urn:example:Floor:1 floor0
CreateDigitalTwin urn:example:Floor:1 floor1
CreateDigitalTwin urn:example:Room:2 room0 DisplayName string Room0 Temperature double 70 HumidityLevel double 30
CreateDigitalTwin urn:example:Room:2 room1 DisplayName string Room1 Temperature double 80 HumidityLevel double 60
```
...
* **What are all the rooms in my environment?** (query by model)
    ...
    ```cmd
    Query SELECT * FROM DIGITALTWINS T WHERE IS_OF_MODEL(T, 'urn:example:Room:2')
    ```
