...
| **Data privacy**<br>*Scalable* | In the previous release, only one instance of Azure Digital Twins was supported per subscription. As a result, the way to achieve data privacy was to create multiple tenants within your Azure Digital Twins instance. | The new release supports 10 instances per region, per subscription. See [Reference: Public preview service limits](reference-service-limits.md) for details of this limit and others. | To achieve data separation, the recommended best practice is to create a separate Azure Digital Twins instance for each region or sub-region. |
