...
* By default, an event route accepts and routes every single message generated by the system. The `filter` parameter allows you to selectively route messages based on event types (such as telemetry events, digital twin property change events, life-cycle events, etc.), or by other parameters such as twin type. In this way, you can create selective routes. 
