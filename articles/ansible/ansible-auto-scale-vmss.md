...
[Azure virtual machine scale sets](/virtual-machine-scale-sets/overview) is an Azure feature that lets you configure a group of identical, load balanced VMs. There is no additional cost to scale sets and they are built from virtual machines. You pay only for the underlying compute resources such as the VM instances, load balancer, or Managed Disk storage. With scale sets, the management and automation layers are provided to run and scale your applications. You could instead manually create and manage individual VMs. However, the benefit of the scale sets feature is that it's built into Azure and automatically scales your virtual machines to meet application needs. 
