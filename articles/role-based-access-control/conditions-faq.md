...
The `Exists` operator is not supported in the visual ABAC condition builder for all attributes. For a list of attributes for which it is supported, see [the *Exists* function operator](conditions-format.md#exists). To use the `Exists` operator with other attributes, you will need to add it to an expression using the **Code** editor.
