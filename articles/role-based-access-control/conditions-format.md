...
| DateTime comparison | `DateTimeEquals`<br/>`DateTimeNotEquals`<br/>`DateTimeGreaterThan`<br/>`DateTimeGreaterThanEquals`<br/>`DateTimeLessThan`<br/>`DateTimeLessThanEquals` | DateTime comparison is a full-precision check that is used mostly for blob version ID and blob snapshot ID. |
