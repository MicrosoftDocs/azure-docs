...
If you create role assignments using the command line, you'll need to specify the scope. For command-tools, scope is a potentially long string that identifies the exact scope of the role assignment.
...
Management groups are a level above subscriptions and have the broadest (least specific) scope. Role assignments at this level apply to a subscriptions within the management group. Each additional level of hierarchy makes the scope more specific. Management groups have the following pattern:
...
It's straightforward to determine the scope for a management group, subscription, or resource group. You just need to know the name and possibly the subscription ID. However, determining the scope for a resource is a little more complicated. Here are some ways that you can determine the scope for a resource.

- Assign a role at the resource scope using the Azure portal and then use Azure PowerShell or Azure CLI to list the role assignment. In the output, you can see the scope.
