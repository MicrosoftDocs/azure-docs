...
Permission strings are generally case-insensitive. However, depending on the resource, parts of a permission might need to be lowercase. For more information, see [Naming rules and restrictions for Azure resources](../azure-resource-manager/management/resource-name-rules.md). When you create your custom roles, it's a best practice to match the case that you see for permissions in [Azure resource provider operations](resource-provider-operations.md).
