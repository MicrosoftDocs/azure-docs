...
* [Node Drain Timeout][drain-timeout] on the node pool allows configuring the wait on eviction of pods and graceful termination per node during upgrade, typically for long running workloads. When the node drain timeout is specified to an amount of time (in minutes), AKS honors waiting on pod disruption budgets (if not specified, the default is 30 minutes).
* [Node Soak Time][soak-time] helps to stagger node upgrades in a controlled fashion, and can minimize downtime to your applications during upgrades. You can introduce waiting time, preferrably a short time close to 0 minutes, between nodes upgrading to ensure a check of application readiness (if not specified, the default is 0 minutes). Node soak time can work together with the max surge and node drain time out properties already existing in the nodepool, to bring about the right outcomes in terms of upgrade speed and application readiness.
