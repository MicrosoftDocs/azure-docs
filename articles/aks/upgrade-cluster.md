...
>
> When upgrading by a version alias, only a higher minor version is supported. For example, upgrading from `1.20.x` to `1.20` will not trigger an upgrade to the latest `1.20` patch, but upgrading to `1.21` will trigger an upgrade to the latest GA `1.21` patch.
