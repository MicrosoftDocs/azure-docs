...
| `node-image`| automatically upgrade the node-image to the latest version available.| Microsoft provides patches and new images for image nodes weekly, but doesn't automatically patch them by default. Turning on the node-image channel will automatically patch and update your images whenever a new version is available. |
