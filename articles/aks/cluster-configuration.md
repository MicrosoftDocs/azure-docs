...
> 
> Using `containerd` with Windows Server 2019 node pools is generally available, although the default for node pools created on Kubernetes v1.22 and earlier is still Docker. For more details, see [Add a Windows Server node pool with `containerd`](/learn/quick-windows-container-deploy-cli#add-a-windows-server-node-pool-with-containerd).
> 
