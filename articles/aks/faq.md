...
Other than the Log analytics agent (which is supported because it is a Microsoft managed extension) No, AKS is a managed service, and manipulation of the IaaS resources is not supported. To install custom components, etc. please leverage the Kubernetes APIs and mechanisms. For example, leverage DaemonSets to install required components.
