...
|storageAccount | Specify an Azure storage account name.| storageAccountName | - No | If the skuName is not specified, the driver will search for a suitable storage account that matches the `skuName` in the same resource group. However, if a storage account name is provided, the storage account must already exist. |
