...
This article describes how to deploy the cluster autoscaler on the agent nodes. Support for the cluster autoscaler in the control plane will be added in the next few weeks. However, since the cluster autoscaler is deployed in the kube-system namespace, the autoscaler will not scale down the node running that pod.
