...
Yes you can, however Azure Monitor is in public preview for gathering logs (stdout, stderr) from Windows containers. You can also attach to the live stream of stdout logs from a Windows container. The linux solution and metrics collection from windows nodes is GA (General Availability).
