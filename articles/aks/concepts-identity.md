...
- [Kubernetes role-based access control (Kubernetes RBAC)](#kubernetes-role-based-access-control-kubernetes-rbac)
...
The Role Grant referred to in the second column is the Azure RBAC role grant shown on the Access Control Tab of the Portal, while the Cluster Admin AAD Group is shown on the Configuration tab in the Portal (or with parameter name `--aad-admin-group-object-ids` in the Azure CLI).

| Description        | Role grant required| Cluster admin AAD group(s) | When to use |
| -------------------|------------|----------------------------|-------------|
