...
> [!NOTE]
> After disabling local accounts users on an already existing AKS cluster where users might have used local account/s, admin must [rotate the cluster certificates](https://docs.microsoft.com/en-us/azure/aks/certificate-rotation#rotate-your-cluster-certificates), in order to revoke the certificates those users might have access to.  If this is a new cluster than no action is required.
