...
Next, use the following manifest to create a ClusterRoleBinding for the Azure AD account. Update the user name with one from your Azure AD tenant. This example gives the account full access to all namespaces of the cluster.
...
A role bindgn can also be created for a all members of an Azure AD group. The following manifest gives all members of the `kubernetes-admin` group admin access to the cluster.
