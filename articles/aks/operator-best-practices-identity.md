...
**Best practice guidance** - Don't use fixed credentials within pods or container images, as they are at risk of exposure or abuse. Instead, use pod-managed identities to automatically request access using a central Azure AD identity solution. Pod-managed identities are intended for use with Linux pods and container images only.
...
![Pod-managed identities allow a pod to automatically request access to other services](media/operator-best-practices-identity/pod-identities.png)
...
To use Pod-managed identities, see [Use Azure Active Directory pod-managed identities in Azure Kubernetes Service (Preview)]( https://docs.microsoft.com/en-us/azure/aks/use-azure-ad-pod-identity).
