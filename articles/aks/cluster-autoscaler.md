...
AKS manages the cluster autoscaler on your behalf and runs it in the managed control plane. Control plane node logs must be configured to be viewed as a result.

To configure logs to be pushed from the cluster autoscaler into Log Analytics follow these steps.

1. Setup a rule for diagnostic logs to push cluster-autoscaler logs to Log Analytics. [Instructions are detailed here](https://docs.microsoft.com/azure/aks/view-control-plane-logs#enable-diagnostics-logs), ensure you check the box for `cluster-autoscaler` when selecting options for "Logs".
