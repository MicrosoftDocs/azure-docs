...
The virtual node add-on for Azure Kubernetes Services (AKS), is based on the open source project [Virtual Kubelet][virtual-kubelet-repo].

This article gives you an overview of the region availability and networking requirements for using virtual node, as well as the known limitations.
...
For available CPU and Memory SKUs in each region, please check the [Azure Container Instances Availability with virtual network](../container-instances-region-availability.md#availability---virtual-network-deployment)
...
* Virtual node with Private clusters.
* Using api server authorized ip ranges for AKS.

## Next steps

Configure virtual node for your clusters:
