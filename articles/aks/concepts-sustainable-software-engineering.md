...
_to scale workloads based on relevant business metrics (HTTP requests, queue length, Cloud Event, etc.)_.
...
_to reduce the in-memory or on-disk data required by the workload to function_.
...
_to reduce CPU compute and network traffic, generated by the service mesh implementation_
...
_to reduce energy and carbon footprint, impacted by the routing appliances and the distance traveled to transmit data_
