...
### Why is label \*kubernetes.io/\* blockign my upgrade to 1.16.*
As of [1.16](https://v1-16.docs.kubernetes.io/docs/setup/release/notes/) kuberenetes [only allows a certain subnet of labels with \*kubernetes.io prefix](https://github.com/kubernetes/enhancements/blob/master/keps/sig-auth/0000-20170814-bounding-self-labeling-kubelets.md#proposal). AKS does not yet allow mutating of labels and removing labels without customers explicit consent can cause down time for thier workload. At this time if you want to bring a nodepool with such a label up to 1.16 you must upgrade only hte control plane and then create a new nodepool without the label and delete your old node pool. Sorry for the inconvience we're looking at ways to allow you to mutate in place on an agentpool. 
