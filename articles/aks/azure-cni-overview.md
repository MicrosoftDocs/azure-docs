...
In most cases, Azure CNI Overlay is the recommended networking setup for clusters. In Azure CNI Overlay, pods receive their IP from a virtual IP space, similar to kubenet, except the scaling limitations around kubenet are eliminated. It is also more performant than kubenet while offering the same alleviation to VNet address space constraints. To setup a clsuter using Azure CNI Overlay please see the [Setup Azure CNI Overlay in AKS][azure-cni-overlay] page.
