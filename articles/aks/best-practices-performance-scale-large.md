...
* When upgrading clusters with more than 500 nodes, it is recommended to use a [max surge configuration](./upgrade-aks-cluster.md#set-max-surge-value) of 10-20% of the node pool's capacity.  The setting should be based on the tolerance for workload disruptions.  For more information, see [Customize node surge upgrade][max surge].
