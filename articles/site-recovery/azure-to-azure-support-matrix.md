...
RHEL, Rocky, Alma, Oracle Linux (All distros are based on RHEL kernels) | - A new kernel for RHEL 8.x or 9.y would be supported if the minor OS version (RHEL 8.x or RHEL 9.5) is supported and the kernel series is already supported for that minor OS version. For RHEL 8.x, this only applies if x ≥ 6. <br/><br/>For Oracle Linux UEK kernels, new kernels within a supported uek kernel series within a supported OS version. | -New Major version released.<br/>e.g., Assume RHEL 10.x, Rocky Linux 10.x, Alma Linux 10.x, or Oracle Linux 10.x is released which ASR does not support. <br/><br/>- Minor OS version released within a supported Major OS version.<br/>e.g., Assume RHEL 9.5 is released which ASR does not support.<br/><br/>- New kernels for RHEL 8.x where x < 6 (e.g., RHEL 8.4) are not supported within 30 days.<br/><br/>-New kernel releases for a UEK kernel series not yet supported by ASR. 
