...
Managed Premium SSD v2| Supported. | Since block blob storage accounts aren't supported in China North and China East regions, Site Recovery for Premium SSD v2 disks can't be supported. __This SKU is only supported for a new replication. Disks should be pv2 from enable time. Upgrade from Pv1 to Pv2 for an existing replication job is not supported and requires to re-enable replication.__
