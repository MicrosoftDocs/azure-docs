...
3. Customize target settings as follows:
    ...
    - **Target availability sets**: By default, Site Recovery creates a new availability set in the
      target region with the "asr" suffix for the VMs part of an availability set in source region. You can only add availability sets if VMs are part of a set in the source region.
