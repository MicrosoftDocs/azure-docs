...
If you want to replicate Hyper-V VMs located on Hyper-V hosts in VMM clouds and you only have a single VMM server, you can [replicate to Azure](site-recovery-vmm-to-azure.md) , or between clouds on the single VMM server.

We recommend that you replicate to Azure because failover and recovery aren't seamless when replicating between clouds, and a number of manual steps will be required. If you do want to replicate using the VMM server only, you can do the following:
...
1. Set up VMM on a Hyper-V VM. We suggest you install the SQL Server instance used by VMM on the same VM to save time. If you want to use a remote instance of SQL Server and an outage occurs, you'll need to recover that instance before you can recover VMM.
...
3. Create a Recovery Services vault, generate and download a vault registration key, and register the VMM server in the vault. During registration you'll install the Azure Site Recovery Provider on the VMM server.
...
3. Follow the instructions in [this article](site-recovery-vmm-to-vmm.md) to create a vault, register the server, and set up protection. You'll need to register each VMM server in the cluster in the Recovery Services vault. To do this you install the Provider on an active node, and register the VMM server. Then you install the Provider on other nodes.
