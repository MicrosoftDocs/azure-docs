...
This article describes how to set up disaster recovery to Azure for large numbers (more than 1000) of on-premises VMware VMs or physical servers in your production environment, using the [Azure Site Recovery](site-recovery-overview.md) service.


## Define your BCDR strategy

As part of your business continuity and disaster recovery (BCDR) strategy, you define recovery point objectives (RPOs) and recovery time objectives (RTOs) for your business apps and workloads. RTO measures the duration of time and service level within which a business app or process must be restored and available, in order to avoid continuity issues. Site Recovery provides continuous replication for VMware VMs and physical servers, and an [SLA](https://azure.microsoft.com/support/legal/sla/site-recovery/v1_2/) for RTO.

## Best practices

When you're setting up disaster recovery at scale with Site Recovery, follow these best practices:

- **Target requirements**: Figure out capacity needs before you start, so that you can plan for capacity and resources in your target Azure environment.
- **Site Recovery components**: Figure out what Site Recovery components (configuration server, process servers) you need to meet your expected capacity.
- **Scale-out process server**: Don't use the process server that's running by default on the configuration server. You should use scaled-out process servers.
- **Updates**: The Site Recovery team releases new versions of Site Recovery components on a regular basis, and you should make sure you're running the latest versions. To help with that, track [what's new](site-recovery-whats-new.md) for updates, and [enable and install updates](service-updates-how-to.md) as they release.
- **Monitoring**: As you get disaster recovery up and running, you should proactively monitor the status and health of replicated machines, and infrastructure resources.
...
Gather information about your on-premises environment, to help assess and estimate your capacity needs.
...
The Deployment Planner helps you to gather information about your on-premises environment.
...
- If you don't yet have a configuration server set up:
    - [Get an overview](vmware-physical-azure-config-process-server-overview.md) of the configuration server and other Site Recovery components.
...
1. [Get an overview](site-recovery-deployment-planner.md) of the Deployment Planner. You can download the latest version from the portal, or [download it directly](https://aka.ms/asr-deployment-planner).
...
3. Run the Deployment Planner on the configuration server machine. [Learn more](site-recovery-vmware-deployment-planner-run.md).
4. In the Planner, [generate a report](site-recovery-vmware-deployment-planner-run.md#generate-report) to summarize all deployment requirements.
...
>[!NOTE]
> By default, the tool is configured to profile and generate report for up to 1000 VMs. You can change this limit by increasing the MaxVMsSupported key value in the ASRDeploymentPlanner.exe.config file.
...
## Plan for target (Azure) capacity and requirements 

Using your gather capacity requirements, you can plan for target resources and capacity. If you ran the Deployment Planner for VMware VMs, you can use a number of the [report recommendations](site-recovery-vmware-deployment-planner-analyze-report.md#recommendations) to help you.
...
- **Required network bandwidth**: This shows you the bandwidth you need for delta replication of compatible VMs. 
    - You specified the desired RPO in minutes when you ran the Planner. The recommendations show you the bandwidth needed to meet that RPO 100% and 90% of the time. 
- **Required Azure cores**: This shows you the number of cores you need set up in the target Azure region, based on the number of compatible VMs. If you don't have enough cores, at failover Site Recovery won't be able to create the required Azure VMs.
...
You can use these recommendations to plan for Azure cores, network bandwidth, and VM batching.

## Plan Azure subscriptions and quotas

We want to make sure that we have available quota in the target subscription to handle failover.

**Task** | **Details** | **Action**
--- | --- | ---
**Check cores** | If cores in the available quota don't equal or exceed the total target count at the time of failover, failovers will fail. | For VMware VMs the target subscription, check you have enough cores in the target subscription to meet the Deployment Planner core recommendation.<br/><br/> For physical servers, check that Azure cores meet your manual machine count for physical machines.<br/><br/> To check quotas, in the Azure portal > **Subscription**, click **Usage + quotas**.<br/><br/> [Learn more](https://docs.microsoft.com/azure/azure-supportability/resource-manager-core-quotas-request) about increasing quotas.
**Check failover limits** | The number of failovers mustn't exceed Site Recovery failover limits. |  If failovers exceed the limits, you can add subscriptions for failover to multiple subscriptions, or increase quota with a support ticket. 
...
After failover to Azure you need your workloads to work as normal, and to enable user access to workloads running on the Azure VMs.
...
- **Set up configuration servers**: Make sure you have sufficient configuration servers, in accordance with sizing recommendations.
- **Set up process servers**: Make sure you've set up one or more scale-out process servers.
    - You shouldn't use the process server that's running by default on the configuration server.
    - Figure out many scale-out process servers you need in accordance with size limits.

>[!NOTE]
> For VMware VMs, the Deployment Planner makes some recommendations about the configuration and process servers you need. We recommend that you use the tables included in the following section, instead of following the Deployment Planner recommendation. 
...
### Add a scale-out process server
...
3. Configure replicated machines to use the new server:
...
2. If disk churn for a machine is high, or exceeds limits as noted in the Deployment Planner, you can move non-critical files that don't need replicating (such as log dumps or temp files) off the machine. For VMware VMs, you can move these files to a separate disk, and then exclude that disk from replication. [Learn more](vmware-azure-exclude-disk.md) about excluding disks.
...
After you kick off replication for the first batch of VMs, you should start monitorings your deployment as follows:  
...
- [Make preparations](#plan-the-target-infrastructure) so that your workloads run as expected on Azure VMs after failover, and so that users can access the Azure VMs.
- Note the [failover limits](#failover-limits) earlier in this document, and make sure your failovers will fall within these limits.
...
To run a large-scale failovers, we recommend the following:
