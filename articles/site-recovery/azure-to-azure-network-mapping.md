...
--- | ---
Target network is the production VNet | - Target IP address is static, but not the same IP address as reserved for failover.<br/><br/>  - The assigned address is the next available address from the end of the subnet range.<br/><br/> For example: If the source IP address is 10.0.0.19 and failover is on a production network range 10.0.0.0/24, then the next IP address assigned to the target VM is 10.0.0.254.
Target network isn't the production VNet | - Target IP address will be static with the same IP address reserved for failover.<br/><br/>  - If the same IP address is already assigned, then the IP address is the next one available at the eeach of the subnet range.<br/><br/> For example: If the source static IP address is 10.0.0.19 and failover is on a non-production network with the range 10.0.0.0/24, then the target static IP address will be 10.0.0.0.19 if available, and otherwise it will be 10.0.0.254.
