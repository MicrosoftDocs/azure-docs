...
- If you're already using Azure Site Recovery to replicate your servers, you do not need to deploy a Migrate appliance. Remove the BCDR protection, and replicate with a new appliance.
- However, there are benefits to conducting an assessment, dependency analysis, and business case review using the Azure Migrate discovery appliance, even for workloads that are already being replicated.
- There may be architecture changes needed to support the workload long term; in that case, address those but continue to use Azure Site Recovery to replicate so that you do not remove protections.
...
- There may be architecture changes required to support the workload in the long term. In this case, address those while continuing to use Azure Site Recovery to replicate so that you do not lose protections.
...
- **For new migration**: If you are beginning a new migration and do not have either Azure Site Recovery or Migrate in place, we recommend that you use Azure Migrate.
- **For disaster recovery of on-premises machines to Azure**: For disaster recovery of on-premises machines to Azure, we recommend that you use use Azure Site Recovery. You can also use this service to migrate machines to Azure once it has been determined that they should be moved off-premise. 
- **For disaster recovery of Azure VMs between Azure region**: For disaster recovery of Azure VMs between Azure regions, we recommend that you use Azure Site Recovery. Although, you may use Azure Migrate to initially move them into Azure.
- **If you're already using Azure Site Recovery**: If you are currently using Azure Site Recovery to actively protect your machines, continue to use it for replication. However, consider using Azure Migrate for conducting business cases and dependency analysis.
