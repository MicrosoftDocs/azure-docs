...
As part of your Site Recovery deployment, you need to figure out your replication and bandwidth requirements. The Hyper-V capacity planning tool for Site Recovery helps you determine these requirements for Hyper-V VM replication.
...
1. If you want to use a cluster node as the replica, configure the Hyper-V Replica Broker:
   ...
   d. Verify that the Hyper-V Replica Broker role comes online successfully and can fail over between all nodes of the cluster. Right-click the role, point to **Move**, and then select **Select Node**. Select a node, and then select **OK**.
...
8. If you configured a replica server or cluster, in **Network information**, specify the approximate WAN bandwidth you think will be used between the primary and replica sites. If you configured certificate authentication, select the certificates.
