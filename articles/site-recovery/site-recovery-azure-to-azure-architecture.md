...
**Target virtual network** | The virtual network in which replicated VMs are located after failover. A network mapping will be created between source and target virtual networks, and vice versa.
...
>[!IMPORTANT]
>
> Site Recovery never needs inbound connectivity to the VM. The VM only needs outbound connectivity to Site Recovery service URLs/IP addresses, Office 365 authentication URLs/IP addresses, and cache storage account IP addresses.  [Learn more](site-recovery-azure-to-azure-networking-guidance.md).
