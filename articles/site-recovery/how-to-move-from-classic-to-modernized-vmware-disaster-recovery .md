...
2. Select **Upgrade to modernized VMware replication**. The **Pre-requisites** details are displayed. Ensure you read through the prerequisites and then select **Next** to proceed to configure the migration settings.
    ...
    :::image type="Pre-requisites" source="media/migrate-tool/pre-requisites-inline.png" alt-text="Screenshot showing prerequisites." lightbox="media/migrate-tool/pre-requisites-expanded.png":::
...
While the migration is in progress, you can only perform **Failover** and **Disable replication** operations. Once the migration is complete, data will start replicating using the modernized architecture and the new vault. All the operations will be available for you to perform from the new vault.   
