...
Before you begin, make sure your system meets the following criteria:
...
- [Verify requirements and prerequisites](vmware-physical-azure-support-matrix.md). Important things to note include:
...
- To add virtual machines for replication without waiting for the scheduled discovery, highlight the configuration server (but don't select it), and select **Refresh**.
...
- Azure Site Recovery now replicates directly to managed disks for all new replications. The process server writes replication logs to a cache storage account in the target region. These logs are used to create recovery points in replica managed disks that follow the naming convention `asrseeddisk`.
...
- VMs that you previously configured to replicate to target storage accounts aren't affected.
...
1. Select the process server. If you don't create any additional process servers, the built-in process server of the configuration server is available in the dropdown menu. The health status of each process server is indicated according to recommended limits and other parameters. Choose a healthy process server. You can't choose a [critical](vmware-physical-azure-monitor-process-server.md#process-server-alerts) appliance. You can either [troubleshoot and resolve](vmware-physical-azure-troubleshoot-process-server.md) the errors **or** set up a [scale-out process server](vmware-azure-set-up-process-server-scale.md).
   ...
   > [!NOTE]
   > Starting with [version 9.24](site-recovery-whats-new.md), additional alerts enhance the health alerts of process server. Upgrade the Site Recovery components to version 9.24 or later for all alerts to be generated.
...
1. Select the Azure network and subnet that the Azure VMs connect to after failover. The network must be in the same region as the Site Recovery service vault.
...
1. From **Replication settings** > **Configure replication settings**, verify that the correct replication policy is selected. You can modify replication policy settings at **Settings** > **Replication policies** > _policy name_ > **Edit Settings**. Changes you make to a policy also apply to replicating and new virtual machines.
...
- Size of the source machine disks
- Bandwidth available to transfer the data to Azure (You can use deployment planner to identify the optimal bandwidth required)
- Process server resources like memory, free disk space, and CPU available to cache and process the data received from protected items (make sure the process server is [healthy](vmware-physical-azure-monitor-process-server.md#monitor-proactively))

To track the progress of initial replication, go to the recovery services vault in the Azure portal. Under replicated items, monitor the "Status" column value of the replicated item. The status shows the percentage completion of initial replication. When you hover over the Status, you see the "Total data transferred". When you select the status, a contextual page opens and displays the following parameters:

- Last refreshed at - shows the latest time that the replication information for the whole machine is refreshed by the service.
- Completed percentage - shows the percentage of initial replication completed for the VM.
...
- Synchronization progress (to track details at a disk level)
    - State of replication
      - If replication isn't started yet, the status shows "In queue". During initial replication, the service replicates only three disks at a time. This mechanism avoids throttling at the process server.
      - After replication starts, the status shows "In progress".
...
If the health is in critical or warning state, make sure that Replication Health of the machine and [Process Sever](vmware-physical-azure-monitor-process-server.md) are healthy. 

As soon as the enable replication job finishes, the replication progress is 0% and total data transferred is NA. When you select it, the data against each identified disk is "NA". This status indicates that the replication isn't started yet and Azure Site Recovery didn't receive the latest statistics. The progress refreshes every 30 minutes.
...
1. In **Properties**, view replication and failover information for the VM.
1. In **Compute and Network** > **Compute properties**, change multiple VM properties.
   ...
   - **Target VM size or VM type**: The default VM size is based on parameters that include disk count, NIC count, CPU core count, memory, and available VM role sizes in the target Azure region. Azure Site Recovery picks the first available VM size that satisfies all the criteria. You can select a different VM size based on your needs at any time before failover. The VM disk size is also based on source disk size, and you can only change it after failover. Learn more about disk sizes and IOPS rates at [Scalability and performance targets for VM disks](/azure/virtual-machines/disks-scalability-targets).
   - **Resource group**: Select a [resource group](../azure-resource-manager/management/overview.md#resource-groups), from which a virtual machine becomes a part of a post failover. You can change this setting at any time before failover. After failover, if you migrate the virtual machine to a different resource group, the protection settings for that virtual machine break.
   - **Availability set**: Select an [availability set](/azure/virtual-machines/windows/tutorial-availability-sets) if your virtual machine needs to be a part of a post failover. When you select an availability set, keep the following information in mind:
...
1. Add information about the target network, subnet, and IP address that's assigned to the Azure VM.
1. In **Disks**, see the operating system and data disks on the VM that you replicate.
...
The size that you specify for the target virtual machine determines the number of network adapters:

- If the number of network adapters on the source virtual machine is less than or equal to the number of adapters that the target VM's size allows, the target has the same number of adapters as the source.
- If the number of adapters for the source virtual machine exceeds the number that the target VM's size allows, the target size maximum is used. For example, if a source virtual machine has two network adapters and the target VM's size supports four, the target virtual machine has two adapters. If the source VM has two adapters but the target size supports one, the target virtual machine has one adapter.
- If the virtual machine has multiple network adapters, they all connect to the same network. Also, the first adapter in the list becomes the default network adapter in the Azure virtual machine.

### Azure Hybrid Benefit

Microsoft Software Assurance customers can use Azure Hybrid Benefit to save on licensing costs for Windows Server computers that they migrate to Azure. The benefit also applies to Azure disaster recovery. If you're eligible, assign the benefit to the virtual machine that Site Recovery creates during a failover.
...
1. Confirm that you have an eligible Windows Server license with Software Assurance that you can use to apply the benefit to the VM that Site Recovery creates at failover.
