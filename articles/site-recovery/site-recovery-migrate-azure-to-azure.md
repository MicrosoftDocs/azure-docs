...
- Read more about these components in [What do I need?](site-recovery-vmware-to-azure/#what-do-i-need.md)
- You should also read the guidelines on [capacity planning](site-recovery-vmware-to-azure/#capacity-planning.md) and make sure you have all the [deployment prerequisites](site-recovery-vmware-to-azure/#before-you-start.md) in place before you start.

## Deployment steps

1. [Create a vault](site-recovery-vmware-to-azure/#step-1-create-a-vault.md)
2. [Deploy a configuration server](site-recovery-vmware-to-azure/#step-2-deploy-a-configuration-server.md) as an Azure VM.
3. [Deploy the master target server](site-recovery-vmware-to-azure/#step-2-deploy-a-configuration-server.md) as an Azure VM.
4. [Deploy an on-premises process server](site-recovery-vmware-to-azure/#step-4-deploy-the-on-premises-process-server.md). Note that:
...
5. [Install the latest updates](site-recovery-vmware-to-azure/#step-5-install-latest-updates.md). Make sure all the component servers you've installed are up-to-date.
6. [Create a protection group](site-recovery-vmware-to-azure/#step-7-create-a-protection-group.md). In order to starting protecting migrated virtual machines using Site Recovery you need to set up a protection group. You specify replication settings for a group and they'll be applied to all machines that you add to that group. 
7. [Set up virtual machines](site-recovery-vmware-to-azure/#step-8-set-up-machines-you-want-to-protect.md). You'll need to get the Mobility service installed on each VM (either automatically or manually).
8. [Step 8: Enable protection for virtual machines](site-recovery-vmware-to-azure/#step-9-enable-protection.md). You enable protection for VMs by adding them to a protection group. Note that:
...
9. [Step 9: Run an unplanned failover](site-recovery-failover/#run-an-unplanned-failover.md). After initial replication is complete you can run an unplanned failover from one Azure region to another. Optionally, you can create a recovery plan and run an unplanned failover, to migrate multiple virtual machines between regions. [Learn more](site-recovery-create-recovery-plans.md) about recovery plans.
