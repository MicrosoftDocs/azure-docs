...
Welcome to the Azure Site Recovery! Start reading this article for a quick overview of the Site Recovery service, and how it contributes to your business continuity and disaster recovery (BCDR) strategy.
...
- **Eliminate secondary datacenter**—You can replicate to a secondary on-premises site, or to Azure. Using Azure as a destination for disaster recovery eliminates the cost and complexity maintaining a secondary site. Replicated data is stored in Azure Storage, with all the resilience that provides.
...
- **Flexible recovery plans**— You can create and customize recovery plans with external scripts, manual actions. Azure Automation runbooks that enable you to recover an entire application stack with a single click.
