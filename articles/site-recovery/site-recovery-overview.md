...
**Azure VM replication** | You can set up disaster recovery of Azure VMs from a primary region to a secondary region or from Azure Public MEC (preview) to the Azure region or from one Azure Public MEC (preview) to another Azure Public MEC (preview) connected to the same Azure region.
...
**Data resilience** | Site Recovery orchestrates replication without intercepting application data. When you replicate to Azure, data is stored in Azure storage, with the resilience that provides. When failover occurs, Azure VMs are created based on the replicated data. This also applies to Public MEC (preview) to Azure region Azure Site Recovery scenario. In case of Azure Public MEC (preview) to Public MEC (preview) Azure Site Recovery scenario, data is stored in the Public MEC (preview).
...
**Supported** | **Details**
--- | ---
**Replication scenarios** | Replicate Azure VMs from <br/>1. One Azure region to another.<br/>2. Azure Public MEC (preview) to the Azure region it's connected to.<br/>3. One Azure Public MEC (preview) to another Public MEC (preview) connected to same Azure region.<br/><br/>  Replicate on-premises VMware VMs, Hyper-V VMs, physical servers (Windows and Linux), Azure Stack VMs to Azure.<br/><br/> Replicate AWS Windows instances to Azure.<br/><br/> Replicate on-premises VMware VMs, Hyper-V VMs managed by System Center VMM, and physical servers to a secondary site.
...
- Get started with [Disaster recovery for VMs on Azure Public MEC (preview)](disaster-recovery-for-edge-zone-vm-tutorial.md).
