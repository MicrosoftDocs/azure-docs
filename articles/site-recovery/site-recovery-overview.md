...
**Azure VM replication** | You can set up disaster recovery of Azure VMs from a primary region to a secondary region or from Edge Zone to region or from one Edge zone to another Edge Zone connected to the same region.
...
**Data resilience** | Site Recovery orchestrates replication without intercepting application data. When you replicate to Azure, data is stored in Azure storage, with the resilience that provides. When failover occurs, Azure VMs are created based on the replicated data. This also applies to Edge Zone to Region ASR scenario. In case of Edge Zone to Edge Zone ASR scenario, data is stored in the Edge Zone.
...
**Supported** | **Details**
--- | ---
**Replication scenarios** | Replicate Azure VMs from <br/>1. One Azure region to another.<br/>2. Edge Zone to the region its connected to<br/>3. One Edge Zone to another Edge Zone connected to same Region.<br/><br/>  Replicate on-premises VMware VMs, Hyper-V VMs, physical servers (Windows and Linux), Azure Stack VMs to Azure.<br/><br/> Replicate AWS Windows instances to Azure.<br/><br/> Replicate on-premises VMware VMs, Hyper-V VMs managed by System Center VMM, and physical servers to a secondary site.
