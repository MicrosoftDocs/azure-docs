...
To generate a cost report, see [Run the Deployment Planner and generate a cost report for VMware disaster recovery](hyper-v-deployment-planner-run.md).
...
:::image type="content" source="media/hyper-v-deployment-planner-analyze-report/what-if-analysis-h2a.png" alt-text="Screenshot of the what-if analysis.":::
...
**VM Compatibility**: Explains why the VM isn't compatible with Site Recovery. Evaluate the report against the current [Hyper-V support matrix](hyper-v-azure-support-matrix.md), because current managed-disk limits can be higher than legacy Deployment Planner classifications. Reasons can include:

* A data disk exceeds 32,767 GiB (approximately 32 TiB), or doesn't meet the provider and managed-disk requirements in the support matrix.
...
* OS disk is greater than 4,095 GiB for a generation 2 (EFI boot type) VM, or the selected target VM size doesn't support generation 2.
