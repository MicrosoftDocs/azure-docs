...
You can remove the replication appliance by using either the **delete** or **reset** method. If all the components of the appliance are in a healthy state and the appliance is still accessible, you can only *reset* the appliance. Resetting moves the appliance to its factory state, enabling it to be associated with any Recovery Service vault again. 

If all the appliance components are in a critical state and there's no connectivity with the appliance, you can *delete* it from the Azure portal. Before deleting the Recovery Services vault, you must also *remove infrastructure* to ensure that all the resources created in the background for replication and appliance registration are also removed. However, before you delete the Azure Site Recovery replication appliance, complete some preparatory steps to avoid errors.


## Prerequisites

Before you delete the Azure Site Recovery replication appliance, ensure that you *disable replication of all servers* by using the Azure Site Recovery replication appliance. To do this, go to Azure portal, select the Recovery Services vault, and open the *Replicated items* blade. Select the servers you want to stop replicating, select **Stop replication**, and confirm the action.
...
You can delete the Azure Site Recovery replication appliance from the Azure portal only if all components are in a critical state and the appliance is no longer accessible.

> [!IMPORTANT]
> The appliance must be unhealthy (in a critical state) for at least 30 minutes before you can delete it. If the appliance is healthy, you can only reset it. Make sure you disable replication for all servers before deleting the appliance.

> [!NOTE]
> Trigger the delete appliance from the portal after shutting down the VM. Don't unregister any component from appliance to delete the appliance.

To delete an appliance, follow these steps:

1. Sign in to the Azure portal.
1. Go to the *Recovery Services vault* > *Site Recovery infrastructure* (under **Manage**), select Azure Site Recovery *replication appliances* under **For VMware & Physical machines**.
1. For the Azure Site Recovery replication appliance you want to delete, select **Delete** from its menu.

    :::image type="content" source="./media/delete-appliance/delete.png" alt-text="Screenshot of Site Recovery appliance page.":::

1. Confirm that no replicated items are associated with the replication appliance. If replicated items are associated, a pop-up appears and blocks the appliance deletion.

    :::image type="content" source="./media/delete-appliance/notification.png" alt-text="Screenshot of pop-up notification.":::

1. If no replicated items are associated with the appliance, a pop-up appears to inform you about the Microsoft Entra ID Apps that you must delete. Note these app IDs and proceed with deletion.


### Post-delete appliance

After you delete the Azure Site Recovery replication appliance, you can:
...
- Delete the Recovery Services vault if you no longer need it.
...
You can reset the Azure Site Recovery replication appliance only if all components are in a healthy state. To reset the appliance, follow these steps:
...
1. Once completed successfully, ensure the following:
   ...
   - Clear the cache for Microsoft Edge or other browsers you're using. Restart the browser after the cache cleanup. Learn more [here](https://www.microsoft.com/en-us/edge/learning-center/how-to-manage-and-clear-your-cache-and-cookies).
   - Restart the machine.
