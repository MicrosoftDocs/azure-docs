...
Yes, you can easily [change the type of managed disk](https://docs.microsoft.com/azure/virtual-machines/windows/convert-disk-storage) for ongoing replications. Before changing the type, ensure that no SAS URL is generated on the managed disk. Go to the Managed Disk resource on the Azure portal and check if you have a SAS URL banner on the Overview blade. If it is present, click it to cancel the ongoing export. Once done, change the type of the disk within the next few minutes. However, if you change the managed disk type, wait for fresh recovery points to be generated by Azure Site Recovery. Use the new recovery points for any test failover or failover going forward.
