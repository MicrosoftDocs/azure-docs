...
## Overview
This article provides information and instructions for doing a test failover or a DR drill of virtual machines and physical servers that are protected with Site Recovery.
...
Track progress on the **Jobs** tab. Test machines are created and start in the secondary location<br/><br/>Azure - connect to the machine in the Azure portal<br/><br/>Secondary site - access the machine on the same host and cloud<br/><br/>Complete testing and automatically clean up test failover settings. 

The table below provides information about the scenarios when Test failover is supported by Site Recovery: 

| **Failover direction** | **Test failover** | 
| --- | --- |
| Primary VMM site to Secondary VMM site |Supported |
| Secondary VMM site to Primary VMM site |Supported |
| Cloud to Cloud (single VMM server) |Supported |
| VMM site to Azure |Supported |
| Azure to VMM site |Unsupported |
| Hyper-V site to Azure |Supported |
| Azure to Hyper-V site |Unsupported |
| VMware site to Azure |Supported (enhanced scenario)<br/><br/> Unsupported (legacy scenario) |
| Azure to VMware site |Unsupported|
| Physical server to Azure |Supported (enhanced scenario)<br/><br/> Unsupported (legacy scenario) |
...
1. You can access replica machines in Azure from your on-premises site initiate an RDP connection to the virtual machine. port 3389 will need to be open on the endpoint for the virtual machine.
...
1. Click **The test failover is complete** to automatically clean up the test environment. After this is complete the test failover will show the **Complete** status.
...
1. Make sure that the primary virtual machine is shutdown when you are doing the test failover. If you don't do so there will be two virtual machines with the same identity running in the same network at the same time that can lead to undesired consequences. 
1. Any changes that you make into the test failover virtual machines would be lost when you cleanup the test failover environment. These changes will not be replicated back to the primary virtual machine.
1.  
