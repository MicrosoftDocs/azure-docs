...
> [!IMPORTANT]
> To move Azure virtual machines to an availability zone in a different region, use [Azure Resource Mover](../resource-mover/move-region-availability-zone.md). Resource Mover is in public preview and provides:
...
> - An easy way to identify dependencies across resources you want to move. This feature helps you move related resources together, so that everything works as expected in the target region after the move.
...
- Check account permissions. If you just created your free Azure account, you're the admin of your subscription. If you're not the subscription admin, work with the admin to assign the permissions you need. To enable replication for a virtual machine and eventually copy data to the target by using Azure Site Recovery, you must have:
    ...
    1. Permission to manage Azure Site Recovery tasks. The *Site Recovery Contributor* role has all permissions required to manage Site Recovery actions in a Recovery Services vault.
...
1. Check that all the latest root certificates are present on the Azure virtual machines you want to move. If the latest root certificates aren't present, security constraints prevent enabling data copy to the target region.

1. For Windows virtual machines, install all the latest Windows updates on the virtual machine, so that all the trusted root certificates are on the machine. In a disconnected environment, follow the standard Windows update and certificate update processes for your organization.

1. For Linux virtual machines, follow the guidance provided by your Linux distributor to get the latest trusted root certificates and certificate revocation list on the virtual machine.
1. Make sure you don't use an authentication proxy to control network connectivity for virtual machines that you want to move.

1. Verify [outbound connectivity requirements for virtual machines](azure-to-azure-tutorial-enable-replication.md#set-up-vm-connectivity).

1. Identify the source networking layout and the resources you currently use for verification, including load balancers, NSGs, and public IP.
...
1. Make sure your subscription has enough resources to support virtual machines with sizes that match your source virtual machines. If you use Site Recovery to copy data to the target, it picks the same size or the closest possible size for the target virtual machine.

1. Create a target resource for every component identified in the source networking layout. This action ensures that after you cut over to the target region, your virtual machines have all the functionality and features that you had in the source.

    > [!NOTE]
    > When you enable replication for the source virtual machine, Azure Site Recovery automatically discovers and creates a virtual network and storage account. You can also pre-create these resources and assign them to the virtual machine as part of the enable replication step. For any other resources, you need to manually create them in the target region.

     The following documents explain how to create the most commonly used network resources that are relevant to you, based on the source virtual machine configuration.
...
1. [Create a non-production network](../virtual-network/quick-create-portal.md) in the target region if you want to test the configuration before you cut over to the target region. This approach causes minimal interference with the production environment.

## Enable replication
The following steps guide you through using Azure Site Recovery to enable replication of data to the target region before you move the data into Availability Zones.
...
1. Select **Enable Replication**. This action starts a job to enable replication for the virtual machine.
...
1. You can check replication health, the recovery points that are created, and the source and target regions on the map.


## Test the configuration

1. In the virtual machine menu, select  **Disaster recovery**.
1. Select the **Test Failover** icon.
1. In **Test Failover**, select a recovery point to use for the failover:
   ...
   - **Latest processed**: Fails the virtual machine over to the latest recovery point that the
     Site Recovery service processes. The time stamp is shown. By using this option, the service spends no time processing
...
1. Select the test target Azure virtual network to which you want to move the Azure virtual machines to test the configuration. 

    > [!IMPORTANT]
    > Use a separate Azure virtual machine network for the test failure, and not the production network in the target region into which you want to move your virtual machines.
...
1. After the job finishes, check that the virtual machine appears in the target Azure region as expected.
1. In **Replicated items**, right-click the virtual machine > **Commit**. This action finishes the move process to the target region. Wait until the commit job finishes.
...
> [!IMPORTANT]
> To avoid charges for Site Recovery replication after the move, perform the preceding step. The source replication settings are cleaned up automatically. The Site Recovery extension that you install as part of the replication isn't removed and needs to be removed manually.
