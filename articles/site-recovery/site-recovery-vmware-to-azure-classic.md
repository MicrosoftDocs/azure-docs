...
6. Make sure these URLs are accessible from the management server:
	...
	If you have IP address-based firewall rules on the server, check that the rules allow communication to Azure. You'll need to allow the [Azure Datacenter IP Ranges](https://msdn.microsoft.com/library/azure/dn175718.aspx) and the HTTPS (433) protocol. You'll also need to white list IP address ranges for the Azure region of your subscription, and for West US. In addition, white list this URL for the MySQL download: http://cdn.mysql.com/archives/mysql-5.5/mysql-5.5.37-win32.msi. 
