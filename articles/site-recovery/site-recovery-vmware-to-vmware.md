...
1. The configuration server and the RX server fail to register to the Site Recovery vault when they're behind the proxy.
2. The number of hours RPO not met is not getting updated in the health report.
3. The configuration server is not syncing with the RX server when the ESX hardware details or network details contain any UTF-8 characters.
4. Windows Server 2008 R2 domain controllers fail to boot after recovery.
5. Offline sync is not working as expected.
6. After virtual machine (VM) failover, replication pair deletion get stuck in CX UI for long time  and user cannot do failback resume operation.
...
17. The following security fixes are added in RX.
...
>-	The configuration server and RX server updates can’t be rolled back after they're applied on the system.
...
- Support added for Linux operating systems on the source server:
	- Red Hat Enterprise Linux (RHEL) 6 Update 6
	- RHEL 5 Update 11
	- CentOS 6 Update 6
	- CentOS 5 Update 11
...
Post any questions you have on the [Azure Recovery Services forum](https://social.msdn.microsoft.com/forums/azure/home?forum=hypervrecovmgr).
