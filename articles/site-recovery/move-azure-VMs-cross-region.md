...
You might want to move Azure IaaS virtual machines from one region to another to improve reliability, availability, management, or governance. This tutorial shows how to move VMs to another region by using Azure Site Recovery.
...
> [!IMPORTANT]
> This document covers moving Azure VMs from one region to another *as is*. If your goal is to improve the availability of your infrastructure by moving VMs to availability zones, refer to the tutorial here.
...
- Verify account permissions. If you just created your free Azure account, *you* are the administrator of your subscription. If you aren't the administrator, work with the administrator to get the permissions that you need:
  -  To enable replication for a VM and copy data to the target by using Azure Site Recovery, you must have permissions to create a VM in Azure resources. The *Virtual Machine Contributor* built-in role has these permissions, which enable you to:
        - Create a VM in the selected resource group.
        - Create a VM in the selected virtual network.
        - Write to the selected storage account.

  - You also need permission to manage Azure Site Recovery operations. The *Site Recovery Contributor* role has all permissions required to manage Site Recovery operations in a Recovery Services vault.

## Prepare the source VMs

1. Check that the Azure VMs that you plan to move have the latest root certificates. If not, data copy to the target region can't be enabled due to security constraints.
...
1. Verify that your Azure subscription allows you to create VMs in the target region that's used for disaster recovery. Contact Support to enable the required quota if necessary.
...
4. Manually [create a non-production network](https://docs.microsoft.com/azure/virtual-network/quick-create-portal) in the target region if you want to test the configuration before you perform the actual move. This is recommended. It will create minimal interference with the production environment.
...
1. Navigate to the vault. In **Settings** > **Replicated items**, select the Virtual machine that you want to move to the target region. Then select the **+Test Failover** icon.
...
4. To start testing the move, select **OK**. To track progress, select the VM to open its **Properties.** Or,
   select the **Test Failover** job in *vault name* > **Settings** > **Jobs** > **Site Recovery jobs**.
...
Navigate to the vault in **Settings** > **Replicated items**, select the virtual machine, and then select **Failover**.
...
3. Once the job is complete, check that the VM appears in the target Azure region as expected.
...
- Navigate to the VM. Select **Disable Replication**.  This stops the process of copying the data for the VM.
  ...
  > [!IMPORTANT]
  > Perform the this step to avoid getting charged for Site Recovery replication after the move.
