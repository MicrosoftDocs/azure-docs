...
**Source VM storage** | Azure VMs must use managed disks.<br/><br/>[Learn about](azure-to-azure-support-matrix.md#replicated-machines---storage) supported Azure storage.
...
**Target availability zones** | Validate that the exact target VM size, required disk, and networking capabilities are available in the selected zone. Equal logical zone numbers don't identify equal physical zones across subscriptions or regions. For zone-to-zone recovery, select a logical zone that maps to a different physical zone for the target subscription.
