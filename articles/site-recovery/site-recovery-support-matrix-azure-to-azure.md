...
>[!NOTE]
>
> 1. Replicating virtual machines from one subscription to another for disaster recovery scenarios is not supported.
> 2. Migrating virtual machines across subscriptions is not supported.
> 3. Migrating virtual machines within the same region is not supported.
> 4. Migrating virtual machines from Classic deployment model to Resource manager deployment model is not supported.
