...
The failover shuts down the VM in the secondary region i.e DR region, and creates and boots VM in the primary region. Please **note** that the DR vms will remain in the shutdown deallocated state as shown below
![Right-click to reprotect](./media/azure-to-azure-tutorial-failover-failback/failback.png)

This behavior is by design because Azure Site Recovery save the information of the virtual machine which may be useful in failover for primary to secondary VM. You don't get charged for the deallocated virtual machines and hence it should kepts as it is.
In case 