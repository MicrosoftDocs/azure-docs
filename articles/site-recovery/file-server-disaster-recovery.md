...
[Azure Site Recovery](site-recovery-overview.md) helps you keep your business apps running during planned and unplanned outages. It supports your business continuity and disaster recovery (BCDR) strategy. Site Recovery manages and orchestrates disaster recovery for on-premises machines and Azure virtual machines (VMs). Disaster recovery includes replication, failover, and recovery of various workloads.
...
:::image type="content" source="media/site-recovery-file-server/dfsr-architecture.JPG" alt-text="DFSR architecture.":::
...
* **Replicate a file server by using Site Recovery**: You can replicate file servers to Azure by using Site Recovery. When one or more on-premises file servers are inaccessible, you can bring up the recovery VMs in Azure. The VMs can then serve requests from clients on-premises, provided there's site-to-site VPN connectivity and Active Directory is configured in Azure. You can use this method in the case of a DFSR-configured environment or a simple file server environment with no DFSR. 

* **Extend DFSR to an Azure IaaS VM**: In a clustered file server environment with DFSR implemented, you can extend the on-premises DFSR to Azure. You enable an Azure VM to perform the file server role. 

    * After you handle the dependencies of site-to-site VPN connectivity and Active Directory and DFSR is in place, when one or more on-premises file servers are inaccessible, clients can connect to the Azure VM, which serves the requests.

    * You can use this approach if your VMs have configurations that aren't supported by Site Recovery. An example is a shared cluster disk, which is sometimes commonly used in file server environments. DFSR also works well in low-bandwidth environments with medium churn rate. You need to consider the additional cost of having an Azure VM running all the time. 
...
:::image type="content" source="media/site-recovery-file-server/decisiontree.png" alt-text="Decision tree.":::
...
Because Site Recovery replication is application agnostic, these recommendations hold true for the following scenarios.
...
> [!IMPORTANT]
> Before you continue with any of the following three approaches, make sure that you address these dependencies.



**Site-to-site connectivity**: To allow communication between servers, you must establish a direct connection between the on-premises site and the Azure network. Use a secure site-to-site VPN connection to an Azure virtual network that you use as the disaster recovery site. For more information, see [Establish a site-to-site VPN connection between an on-premises site and an Azure virtual network](../vpn-gateway/tutorial-site-to-site-portal.md).

**Active Directory**: DFSR depends on Active Directory. This dependency means that the Active Directory forest with local domain controllers is extended to the disaster recovery site in Azure. Even if you aren't using DFSR, if the intended users need to be granted access or verified for access, you must take these steps. For more information, see [Extend on-premises Active Directory to Azure](./site-recovery-active-directory.md).

## Disaster recovery recommendation for Azure IaaS virtual machines

If you're configuring and managing disaster recovery of file servers hosted on Azure IaaS VMs, choose from two options based on whether you want to move to [Azure Files](../storage/files/storage-files-introduction.md):
...
You can use Azure Files to completely replace or supplement traditional on-premises file servers or NAS devices. You can also use File Sync to replicate Azure file shares to Windows servers, either on-premises or in the cloud, for performance and distributed caching of the data where it's used. The following steps describe the disaster recovery recommendation for Azure VMs that perform the same functionality as traditional file servers:
...
1. [Create a storage account in Azure](../storage/common/storage-account-create.md?toc=/azure/storage/files/toc.json). If you choose read-access geo-redundant storage for your storage accounts, you get read access to your data from the secondary region in case of a disaster. For more information, see [Disaster recovery and storage account failover](../storage/common/storage-disaster-recovery-guidance.md?toc=%2fazure%2fstorage%2ffiless%2ftoc.json).
1. [Create a file share](../storage/files/storage-how-to-create-file-share.md).
1. [Start File Sync](../storage/file-sync/file-sync-deployment-guide.md) on your Azure file server.
1. Create a sync group. Endpoints within a sync group are kept in sync with each other. A sync group must contain at least one cloud endpoint, which represents an Azure file share. A sync group also must contain one server endpoint, which represents a path on a Windows server.
1. Your files are now kept in sync across your Azure file share and your on-premises server.
1. In the event of a disaster in your on-premises environment, perform a failover by using a [recovery plan](site-recovery-create-recovery-plans.md). Add the script to [mount the Azure file share](../storage/files/storage-how-to-use-files-windows.md) and access the share in your virtual machine.
...
1. Establish a site-to-site VPN connection between the on-premises site and the Azure network. 
1. Extend on-premises Active Directory.
1. [Prepare on-premises VMware servers](./vmware-azure-tutorial-prepare-on-premises.md).
1. [Set up disaster recovery](./vmware-azure-tutorial.md) to Azure for on-premises VMs.

## Extend DFSR to an Azure IaaS virtual machine

1. Establish a site-to-site VPN connection between the on-premises site and the Azure network. 
1. Extend on-premises Active Directory.
1. [Create and provision a file server VM](/azure/virtual-machines/windows/quick-create-portal?toc=%2Fazure%2Fvirtual-machines%2Fwindows%2Ftoc.json) on the Azure virtual network.
...
1. Install and [configure DFSR](https://techcommunity.microsoft.com/t5/storage-at-microsoft/dfs-replication-initial-sync-in-windows-server-2012-r2-attack-of/ba-p/424877) on Windows Server.
1. [Implement a DFS namespace](/windows-server/storage/dfs-namespaces/deploying-dfs-namespaces).
1. With the DFS namespace implemented, you can fail over shared folders from production to disaster recovery sites by updating the DFS namespace folder targets. After these DFS namespace changes replicate via Active Directory, users connect to the appropriate folder targets transparently.
...
1. [Create a storage account in Azure](../storage/common/storage-account-create.md?toc=/azure/storage/files/toc.json). If you choose read-access geo-redundant storage (recommended) for your storage accounts, you have read access to your data from the secondary region in case of a disaster. For more information, see [Disaster recovery and storage account failover](../storage/common/storage-disaster-recovery-guidance.md?toc=%2fazure%2fstorage%2ffiless%2ftoc.json).
1. [Create a file share](../storage/files/storage-how-to-create-file-share.md).
1. [Deploy File Sync](../storage/file-sync/file-sync-deployment-guide.md) in your on-premises file server.
1. Create a sync group. Endpoints within a sync group are kept in sync with each other. A sync group must contain at least one cloud endpoint, which represents an Azure file share. The sync group also must contain one server endpoint, which represents a path on the on-premises Windows server.
1. Your files are now kept in sync across your Azure file share and your on-premises server.
1. In the event of a disaster in your on-premises environment, perform a failover by using a [recovery plan](site-recovery-create-recovery-plans.md). Add the script to mount the Azure file share and access the share in your virtual machine.
...
1. Select the recovery plan created for the file server environment.
1. Select **Test Failover**.
1. Select the recovery point and the Azure virtual network to start the test failover process.
1. After the secondary environment is up, perform your validations.
1. After the validations finish, select **Cleanup test failover** on the recovery plan. The process cleans the test failover environment.
...
1. Select the recovery plan created for the file server environment.
1. Select **Failover**.
1. Select the recovery point to start the failover process.
