...
6. For **Resource group**, select **Use existing**, and then select **migrationRG** from the drop-down menu.
7. For **Location**, select **West Europe**.
8. Under **Subnet**, leave the defaults for both **Name** and **IP range**.
...
When you are done setting up the configuration server, go back to the portal and select the server you created for **Configuration Server**. Select **OK** to go to step 3 Target Prepare.

### 3 Target Prepare

In this section, you enter information about the resources you created in [Prepare Azure resources](#prepare-azure-resources) earlier in this tutorial.
...
3. Site Recovery checks that you have one or more compatible Azure storage accounts and networks. These should be the resources you created in [Prepare Azure resources](#prepare-azure-resources) earlier in this tutorial
...
In **Have you completed deployment planning**, select **I will do it later** from the drop-down list, and then select **OK**.
...
2. Under **For on-premises machines and Azure VMs**, select **Step 1: Replicate application**. Complete the wizard pages with the following information. Select **OK** on each page when you are finished:
	...
	- 4 Properties Configure Properties
		...
		Select the account that you created on the configuration server in the drop-down list, and then select **OK**.
...
To track progress of the **Enable Protection** job, select **Monitoring and reports** > **Jobs** > **Site Recovery Jobs**. After the **Finalize Protection** job runs, the machine is ready for failover.		
...
- A prerequisites check runs to make sure that all of the conditions required for failover are in
   place.
- Failover processes the data so that an Azure VM can be created. If you select the latest recovery
...
1. On the page for your vault, go to **Protected items** > **Replicated Items**. Select the VM, then select **Test Failover**.

2. Select a recovery point to use for the failover:
    - **Latest processed**: Fails over the VM to the latest recovery point that was processed by
...
3. In **Test Failover**, select the target Azure network to which Azure VMs will be connected after
   failover occurs. This should be the network you created in [Prepare Azure resources](#prepare-azure-resources).
4. Select **OK** to begin the failover. To track progress, select the VM to open its
   properties. Or you can select the **Test Failover** job on the page for your vault. To do this, select **Monitoring and reports** > **Jobs** >  **Site Recovery jobs**.
5. After the failover finishes, the replica Azure VM appears in the Azure portal. To view the VM, select **Virtual
