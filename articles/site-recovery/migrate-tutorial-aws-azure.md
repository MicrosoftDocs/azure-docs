...
This tutorial teaches you how to migrate Amazon Web Services (AWS) virtual machines (VMs) to Azure VMs by using Azure Site Recovery. When you migrate EC2 instances to Azure, the VMs are treated like physical, on-premises computers. In this tutorial, you learn how to:
...
## Prerequisites
- Ensure that the VMs you want to migrate are running a supported OS version. Supported versions include: 
    - Windows Server 2016
...
Images of replicated machines are held in Azure Storage. Azure VMs are created from storage
...
2. Enter a name for your storage account. In these tutorials, we use the name
   **awsmigrated2017**. The name must be unique in Azure; it must be between 3 and 24
   characters; and it can contain only numbers and lowercase letters.
...
8. In **Location**, select **West Europe**.
9. Select **Create** to create the storage account.

### Create a vault

1. In the [Azure portal](https://portal.azure.com), in the left menu, select **All services**. Search for and then select **Recovery Services vaults**.
2. In the Recovery Services vaults page, select **+ Add**.
3. For **Name**, enter **myVault**.
4. For **Subscription**, select the subscription you want to use.
4. For **Resource Group**, select **Use existing**, and then select **migrationRG**.
5. In **Location**, select **West Europe**.
5. Select **Pin to dashboard** to quickly access the new vault from the dashboard in the future.
7. When you are done, select **Create**.

The new vault appears in **Dashboard** > **All resources**, and on the main **Recovery Services vaults** page.

### Set up an Azure network

When Azure VMs are created after the migration (failover), they're joined to this network.
...
5. For **Subscription**, select the subscription you want to use.
6. For **Resource group**, select **Use existing**, and then select **migrationRG** from the drop-down.
7. For **Location**, select **West Europe**.
8. Under **Subnet**, leave the defaults for both the **Name** and **IP range**.
9. Leave **Service Endpoints** disabled.
10. When you are done, select **Create**.


## Prepare the infrastructure

On your vault page in the Azure portal, in the **Getting Started** section, select **Site Recovery**, and then select **Prepare Infrastructure**.
...
When you are done, select **OK** to move to the next section.
...
3. Download the [Microsoft Azure Site Recovery Unified Setup](http://aka.ms/unifiedinstaller_wus) program. You can download it to your local machine and then copy it to the VM you're using as the configuration server.
...
5. On the VM, right-click the installer you downloaded for the **Microsoft Azure Site Recovery Unified Setup**, and then select **Run as administrator**.
	...
	5. The **Prerequisites Check** page runs checks for several items. When it is finished, select **Next**.
...
When you are done setting up the configuration server, go back to the portal and select the server you created for **Configuration Server**. Select *OK** to move on to step 3 Target Prepare.

### 3 Target Prepare

In this section, you enter information about the resources you created in the [Prepare Azure resources](#prepare-azure-resources) section earlier in this tutorial.
...
3. Site Recovery checks that you have one or more compatible Azure storage accounts and networks. These should be the resources you created in the [Prepare Azure resources](#prepare-azure-resources) section earlier in this tutorial
...
In **Have you completed deployment planning**, select **I will do it later** from the drop-down, and then select **OK**.

When you are finished with all five sections of **Prepare infrastructure**, select **OK**.
...
2. Under **For on-premises machines and Azure VMs**, select **Step 1:Replicate application**. Complete the wizard pages with the following information. Select **OK** on each page when you are finished:
	- 1 Source Configure:
	...
	| Storage account: | Select the storage account that you created in the [Prepare Azure resources](#prepare-azure-resources) section.|
	...
	- 3 Physical Machines Select
		...
		Select **Physical machine**, and then enter the **Name**, the **IP Address**, and the **OS Type** of the EC2 instance that you want to migrate. Select **OK**.

	- 4 Properties Configure Properties

		Select the account that you created on the configuration server from the drop-down, and then select **OK**.

	- 5 Replication Settings Configure replication settings

		Make sure that the replication policy selected in the drop-down is **myReplicationPolicy**, and then select **OK**.
...
When you run a test failover, the following events happens:

1. A prerequisites check runs to make sure that all of the conditions required for failover are in
   place.
...
3. An Azure VM is created by using the data processed in the preceding step.

In the portal, run the test failover:

1. On the page for your vault, go to **Protected items** > **Replicated Items**. Select the VM,  > **+ Test Failover**.
...
2. Select a recovery point to use for the failover:
    - **Latest processed**: Fails the VM over to the latest recovery point that was processed by
      ...
      it provides a low recovery time objective (RTO).
...
4. Select **OK** to begin the failover. To track progress, select the VM to open its
   properties. Or you can select the **Test Failover** job on the page for your vault: Select **Monitoring and reports** > **Jobs** >    **Site Recovery jobs**.
5. After the failover finishes, the replica Azure VM appears in the Azure portal: Select **Virtual
   Machines**. Ensure that the VM is the appropriate size, that it's connected to the right network,
