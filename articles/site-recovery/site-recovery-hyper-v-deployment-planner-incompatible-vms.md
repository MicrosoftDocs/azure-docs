...
* VHD is shared by two or more VMs. Azure Site Recovery does not support  VMs with shared VHD.
* VM with Virtual Fiber Channel is not supported. Azure Site Recovery does not support  VMs with Virtual Fiber Channel.
* Hyper-V cluster does not contain replication broker. Azure Site Recovery does not support a VMs of Hyper-V clusters if the Hyper-V replica broker is not configured on each cluster node.
