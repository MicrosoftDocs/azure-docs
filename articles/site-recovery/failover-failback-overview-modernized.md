...
---
# About on-premises disaster recovery failover and failback - Modernized

This article provides an overview of failover and failback during disaster recovery of on-premises machines to Azure by using [Azure Site Recovery](site-recovery-overview.md) - Modernized.

## Recovery stages

Failover and failback in Site Recovery have four stages:
...
- **Stage 3: Fail over from Azure**: When your on-premises site is running as normal again, you run another failover. This time, you fail back Azure VMs to your on-premises site. You can fail back to the original location from which you failed over, or to an alternate location. This activity is referred to as *planned failover*.
- **Stage 4: Reprotect on-premises machines**: After failing back, again enable replication of the on-premises machines to Azure.

## Failover

Perform a failover as part of your business continuity and disaster recovery (BCDR) strategy.

- As the first step in your BCDR strategy, replicate your on-premises machines to Azure on an ongoing basis. Users access workloads and apps running on the on-premises source machines.
- If the need arises, for example if there's an outage on-premises, fail the replicating machines over to Azure. Azure VMs are created using the replicated data.
...
--- | --- | --- | ---
**Test failover** | Use to run a drill that validates your BCDR strategy, without any data loss or downtime. | Creates a copy of the VM in Azure, with no impact on ongoing replication, or on your production environment. | 1. Run a test failover on a single VM, or on multiple VMs in a recovery plan.<br/><br/> 2. Select a recovery point to use for the test failover.<br/><br/> 3. Select an Azure network in which the Azure VM will be located when it's created after failover. The network is only used for the test failover.<br/><br/> 4. Verify that the drill worked as expected.<br/><br/> 5. Select **Cleanup test failover** and confirm that testing is complete. Site Recovery then deletes the resources created by the test failover.
**Planned failover-Hyper-V**  | Use for planned downtime.<br/><br/> Source VMs are shut down. The latest data is synchronized before initiating the failover. | Zero data loss for the planned workflow. | 1. Plan a downtime maintenance window and notify users.<br/><br/> 2. Take user-facing apps offline.<br/><br/> 3. Initiate a planned failover with the latest recovery point. The failover doesn't run if the machine isn't shut down, or if errors are encountered.<br/><br/> 4. After the failover, check that the replica Azure VM is active in Azure.<br/><br/> 5. Commit the failover to finish up. The commit action deletes all recovery points.
**Failover-Hyper-V** | Usually run if there's an unplanned outage, or the primary site isn't available.<br/><br/> Optionally shut down the VM and synchronize final changes before initiating the failover. | Minimal data loss for apps. | 1. Initiate your BCDR plan. <br/><br/> 2. Initiate a failover. Specify whether Site Recovery should shut down the VM and synchronize or replicate the latest changes before triggering the failover.<br/><br/> 3. Select a [recovery point option](#recovery-point-options).<br/><br/> If you don't enable the option to shut down the VM, or if Site Recovery can't shut it down, the latest recovery point is used.<br/>The failover runs even if the machine can't be shut down.<br/><br/> 4. After failover, check that the replica Azure VM is active in Azure. If necessary, select a different available recovery point before commit.<br/><br/> 5. Commit the failover to finish up. The commit action deletes all available recovery points.
**Failover-VMware** | Usually run if there's an unplanned outage, or the primary site isn't available.<br/><br/> Optionally specify that Site Recovery should try to trigger a shutdown of the VM, and to synchronize and replicate final changes before initiating the failover. | Minimal data loss for apps. | 1. Initiate your BCDR plan. <br/><br/> 2. Initiate a failover from Site Recovery. Specify whether Site Recovery should try to trigger VM shutdown and synchronize before running the failover.<br/> The failover runs even if the machines can't be shut down.<br/><br/> 3. After failover, check that the replica Azure VM is active in Azure. If necessary, select a different available recovery point before commit.<br/><br/> 4. Commit the failover to finish up. The commit action deletes all recovery points.<br/> For Windows VMs, Site Recovery disables the VMware tools during failover.
**Planned failover-VMware** | You can perform a planned failover from Azure to on-premises. | Since it's a planned failover activity, the recovery point is generated after the planned failover job is triggered. | When the planned failover is triggered, pending changes are copied to on-premises, a latest recovery point of the VM is generated and Azure VM is shut down.<br/><br/> Follow the [planned failover process](vmware-azure-tutorial-failover-failback-modernized.md#planned-failover-from-azure-to-on-premises). Post this, on-premises machine is turned on. After a successful planned failover, the machine will be active in your on-premises environment.

Configure recovery point retention in the replication policy. For modernized VMware and physical machine protection, configure it for up to 15 days.
...
**Option** | **Details**
--- | ---
**Latest (lowest RPO)** | This option provides the lowest recovery point objective (RPO). It first processes all the data that the Site Recovery service receives, to create a recovery point for each VM, before failing over to it. It initially attempts to process and apply all data that the Site Recovery service receives in the target location and create a recovery point by using the processed data. However, if at the time you trigger failover, there's no data uploaded to the Site Recovery service waiting to be processed, Azure Site Recovery doesn't perform any processing and hence, doesn't create a new recovery point. In this scenario, it instead fails over by using the previously processed recovery point only.
...
> [!NOTE]
> You can't migrate recovery points to another Recovery Services vault.

## Reprotection and planned failover

After failover to Azure, the replicated Azure VMs are in an unprotected state.

- To start the process of failing back to your on-premises site, you need to start replicating the Azure VMs to on-premises. The reprotection process depends on the type of machines you failed over.
...
- Only disks replicated from on-premises to Azure are replicated back from Azure during reprotect operation. Newly added disks to failed over Azure VM aren't replicated to on-premises machine. 
- An appliance can have up to 60 disks attached to it. If the VMs you're failing back have more than a collective total of 60 disks, or if you're failing back large volumes of traffic, create a separate appliance for failback.
...
- As this is a planned failover activity, you can select the type of recovery point you want to fail back to. Use a crash-consistent point.
	- There's also an app-consistent recovery point option. In this case, a single VM recovers to its latest available app-consistent recovery point. For a recovery plan with a replication group, each replication group recovers to its common available recovery point.
...
> [!NOTE]
> The failover VM boot might take longer on Windows Server 2012 or older versions when using crash consistent recovery points.

## VMware and physical server reprotection and failback

To reprotect and fail back VMware machines and physical servers from Azure to on-premises, make sure you have a healthy appliance.

**Appliance selection**

- Select any of the Azure Site Recovery replication appliances registered under a vault to reprotect to on-premises. You don't need a separate Process server in Azure for reprotect operation or a scale-out Master Target server for Linux VMs.
- Replication appliance doesn't need another network connection or ports (as compared with forward protection) during failback. You can use the same appliance for forward and backward protections if it's in a healthy state. It shouldn't affect the performance of the replications.
- When you select the appliance, ensure that the appliance can access the target datastore where the source machine is located. The appliance should always access the datastore of the source machine. Even if the machine and appliance are in different ESX servers, reprotection succeeds as long as the data store is shared between them. 
  > [!NOTE]
  > - Storage vMotion of replicated items isn't supported. Storage vMotion of replication appliance isn't supported after reprotect operation.
...
- If this job is a new reprotect operation, Azure Site Recovery automatically creates a new log storage account by default in the target region. Retention disk isn't required.
- In Alternate Location Recovery and Original Location Recovery, the original configurations of source machines are retrieved.
  > [!NOTE]
  > - You can't retain static IP address in case of Alternate location reprotect (ALR) or Original location reprotect (OLR).
  > - fstab and LVMconf files are changed.


**Failure**

- You can retry any failed reprotect job. During retry, you can choose any healthy replication appliance.
...
- **Original location recovery**: This operation fails back from Azure to the same source on-premises machine if it exists. In this scenario, only changes are replicated back to on-premises.
...
- **Alternate location recovery**: If the on-premises machine doesn't exist, you can fail back from Azure to an alternate location. When you reprotect the Azure VM to on-premises, the on-premises machine is created. Full data replication occurs from Azure to on-premises. [Review](concepts-types-of-failback.md) the requirements and limitations for location failback.
  - **Data store selection during ALR**: You can choose any data store managed by vCenter on which the appliance is situated and is accessible (read and write permissions) by the appliance. You can choose the original or new data store. You can choose cache storage account used for reprotection.

- After failover is complete, the mobility agent in the Azure VM automatically registers with Site Recovery Services. If registration fails, a critical health issue is raised on the failed over VM. After you resolve the issue, registration is automatically triggered. You can manually complete the registration after resolving the errors.


## Cancel failover

If your on-premises environment isn't ready or if you face any challenges, you can cancel the failover.

When you initiate the planned failover and it completes successfully, your on-premises environment becomes available for usage. But after the operation completes, if you want to failover to a different recovery point, you can cancel the failover.


- You can cancel only planned failover.

- You can cancel a planned failover from the **Replicated items** page in your Recovery Services vault.

- After you cancel the failover, your machines in Azure turn back on, and replication starts again from Azure to on-premises.
