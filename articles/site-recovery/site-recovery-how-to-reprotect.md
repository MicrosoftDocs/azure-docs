...
* If the virtual machine is not present on-premises on the vCenter server, the Site Recovery service needs to create a new virtual machine during reprotection. This virtual machine will be created on the ESX host on which you create the master target. Choose the ESX host carefully, so that the failback virtual machine is created on the host that you want.
...
* The master target should have at least one VMFS datastore attached. If there are none, the **Datastore** input on the reprotect page will be empty, and you will not be able to proceed.
