...
1. Create a test network that's isolated from your production network (the network specified in the
   **Compute and Network** settings for a VM). By default, an Azure virtual network is isolated when
   it's created.
...
> [!NOTE]
> When running a test failover, Site Recovery tries to create test VMs in a subnet with the same
> name and address as the VM's production network.
