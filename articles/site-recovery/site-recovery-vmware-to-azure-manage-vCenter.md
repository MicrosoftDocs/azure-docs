...
* **Migrate**: If you only want to migrate VMware virtual machines to Azure, without ever failing them back, you can use a VMware account with a read-only role. Such a role can run failover, but can't shut down protected source machines. This isn't necessary for migration.
...
2. In the Configuration server's drill-down page click on the +vCenter button.
