...
9. In **Physical machines**, click **+Physical machine** and enter the **Name** and **IP address**. Choose the operating system of the machine you want to replicate. It will take a few minutes until machines are discovered and shown in the list.
...
4. Modify settings for the target network, subnet, and IP address that will be assigned to the Azure VM:
    ...
    b.  If you don't provide an address, the failed over machine will use DHCP.

    c. If you set an address that isn't available at failover, failover won't work.
...
4. Click **OK** to begin the failover. You can track progress by clicking on the VM to open its properties or by clicking on the **Test Failover** job in vault name > **Settings** > **Jobs** > **Site Recovery jobs**.
...
7. After you're done, click **Cleanup test failover** on the recovery plan. In **Notes**, record and save any observations associated with the test failover. This will delete the virtual machines that were created during test failover.

For more details, refer to the [Test failover to Azure](site-recovery-test-failover-to-azure.md) document.
