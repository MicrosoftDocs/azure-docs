...
Form Recognizer can extract text, selection marks and table structure (the row and column numbers associated with the text) using high-definition optical character recognition (OCR) and an enhanced deep learning  model from docuements. See the [Layout](./concept-layout.md) conceptual guide for more info
