...
| <a name="enableEntityData" />enableEntityData  | A `true` or `false` value. Setting this value to `true` returns detailed information on the main entity in the image you upload, including links to the web, attribution information. <br /><br /> This parameter is set to `false` by default. | Boolean | No       |
