...
| -- |--|--|--|
| **Prebuilt** |[Custom](#prebuilt)| |  **Definition**<br>Built-in types that represent common concepts. <br><br>**Example**<br>:  number, ordinal, temperature, dimension, money, age, percentage, email, URL, phone number, and key phrase. <br><br>Prebuilt entity names are reserved. <br><br>All prebuilt entities that are added to the application are returned in the [endpoint](luis-glossary.md#endpoint) query. For more information, see [Prebuilt entities](./Pre-builtEntities.md). <!-- TBD: is this where the Sentiment Analysis Key phrase extraction entity concept should be? --><br/><br/>[Example response for entity](luis-concept-data-extraction.md#prebuilt-entity-data)|
...
| **List** | [Exact match](#exact-match)|| **Definition**<br>List entities represent a fixed, closed set of related words in your system. <br><br>Each list entity may have one or more forms. Best used for a known set of variations on ways to represent the same concept.<br/><br/>**Example**<br>Drink list:<br>
tea (synonyms: earl grey, blank, herbal, chai)<br>
coffee (synonyms: latte, mocha, espresso)<br>
soft drink: (synonyms: orange, cola, ginger)<br>
