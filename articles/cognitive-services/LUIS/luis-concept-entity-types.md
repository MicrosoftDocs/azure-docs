...
|<!-- added week of 3/21/08 --> **RegEx** | [RegEx](#regex)|X|Custom regular expression. If the regular expression is too complex, such as using many brackets, you are not able to add the expression to the model. Regular expression matching is applied before the utterance is changed into lower-case. If you need to ignore case in the regular expression for matching, add `/i` to end of expression. <br><br>**Example**<br>`kb[0-9]{6,}/i` or `(?i)kb[0-9]{6,}` matches KB123456, Kb123456, kb123456, and kB123456. `/i` and `(?i)` are both valid for ignore case.
