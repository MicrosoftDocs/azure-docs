...
## **Calendar**
Calendar is anything about personal meetings and appointments, NOT public event (e.g., world cup schedules, seattle event calendar) or generic calendar (e.g., what day is it today, what does fall begin, when is Labor Day).
### **Intents**
...
Duration | datetime | Duration of an meeting, appointment or remaining time. | Add to work calender meeting with Gary to discuss scholarship details tomorrow at 11 am for **20 minutes**. <br> Add to the calendar an event at subway on Friday I'll be eating with Thomas for **an hour** at 9 p.m. | an hour <br> 2 days <br> 20 minutes 
...
MoveEarlierTimeSpan | datetime | The time user want to move a meeting or appointment earlier. | Move my lunch date ahead by **30 minutes**. | 30 minutes <br> two hours 
MoveLaterTimeSpan |  datetime | The time user want to move a meeting or appointment later. | push my meeting with orchid box back **4 hours**. | 4 hours <br> 15 minutes 
...
SlotAttribute | simple | The attribute user want to query or edit. | change event **location** <br> change it the **time** to seven o'clock | location <br> time 
...
Subject | simple, pattern.Any | Subject i.e. title of a meeting or appointment. | What time is the **party preparation** meeting? | Dentist's <br> Lunch with Julia 
...
## **Communication**
Requests to make calls, send texts/ IMs, find/add contacts and various other communication-related requests (generally outgoing). Contact name only queries are not belong to Communication domain.
### **Intents**
Intent Name | Description | Examples
---------|----------|---------
...
SearchTexts | simple, pattern.any | The texts used for filtering emails or messages | Search all emails that contains "**Surface Pro**" | Surface Pro
SpeedDial | simple | The speed dial. | Call **three four five**. <br> Set speed dial **one**. | 345 <br> 5

## **Email**
Email is a sub-domain of the *Communication* domain. It mainly contains requests to send and receive messages through emails.
### **Intents**
Intent Name | Description | Examples
---------|----------|---------
...
## **Notes**
Note domain provides creating notes and write down items for users.
### **Intents**
Intent Name | Description | Examples
---------|----------|---------
AddToNote | Add information to an opened note. | Add to my planning note that contact  my boss about the project.
Clear | Clear all items from a pre-existing note. | Remove all items from my vacation note. <br>Clear all from my reading note.
Confirm | Confirm an action relating to a note. <br> ***Notice**: This intent mainly include "Confirm" action for Note scenario. If you need more general expressions on "Confirm", please leverage "Confirm" intent in **Utilities** domain.* | It's okay by me for this note. <br>I am confirming keeping all items on lists.
...
## **Places**
Places include businesses, institutions, restaurants, public spaces and addresses. The domain supports place finding and inquring the information a public place such as location, operating hours and distance. 
### **Intents**
Intent Name | Description | Examples
---------|----------|---------
...
GetReviews | Ask for reviews of a place. | Show me reviews for Cheesecase Factory. <br> Read Cineplex reviews. <br> Are there any recent reviews for Humperdinks?
...
Cuisine | simple | A type of food, cuisine or cuisine nationality. | Chinese <br> Italian <br> Sushi <br> Noodle <br> | Help me find a **Chinese** restaurant. <br> What is the opening hours of the **Sushi** restaurant? <br> Where is the nearest **steak** house?
...
Confirm | The user wants to confirm an ongoing action. The utterance contains explicit signal like "Yes" to confirm an operation. <br> <br> ***Notice**: This intent mainly include "Confirm" action for ToDo scenario. If you need more general expressions on "Confirm", please leverage "Confirm" intent in **Utilities** domain.* | Please delete the task. <br> I'm sure I want to add this task. <br> Yes, I want to do that.
Cancel | The user wants to cancel ongoing action.  <br> <br> ***Notice**: This intent mainly include "Confirm" action for Restaurant Reservation scenario. If you need more general expressions on "Confirm", please leverage "Confirm" intent in **Utilities** domain.* | No, forget it. <br> just cancel the task. <br> No, don't add it.
...
## **ToDo_IPA**
Based on ToDo domain, ToDo_IPA is a customized version to expand the entities in ToDo. The IPA version provides *FoodOfGrocery*, *ShopVerb* and *ShopContent* to support recognizion of a todo list type. The model defines three kinds of to-do lists:
- Grocery List
  - Either the utterance contains *FoodOfGrocery* and *ShopVerb* simultaneously,
  - Or *ListType* Contains grocery related keywords. 
- Shopping List
    - Either the utterance contains *ShopVerb* and is without *FoodOfGrocery*,
    - Or *ListType* Contains shopping related keywords and no *FoodOfGrocery* is detected.
- To-Do List
    - Other cases belong to a common to-do list.
### **Intents**
Intent Name | Description | Examples
---------|----------|---------
AddToDo | The user wants to add task items for any of the list types. |  Remind me to buy milk. <br> Add an item called "buy milk" to my to-do list
...
## **Weather**
Weather domain focues on checking weather condition and advisories with location and time or checking time by weather conditions.
### **Intents**
Intent Name | Description | Examples
---------|----------|---------
...
Date/Time   | datetime | Datetime or duration for quering the weather. | November<br>hourly<br>morning<br>This weekend<br>10 day<br>
