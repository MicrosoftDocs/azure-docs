...
LUIS doesn't ignore punctuation marks, by default, because some client applications may have significance. Make sure your example utterances use both punctuation and no punctuation in order for both styles to return the same relative scores. If punctuation has no specific meaning in your client application, consider [ignoring punctionation](#ignoring-words-and-punctuation) by using patterns. 
