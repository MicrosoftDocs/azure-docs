...
A LUIS app is a place for a developer to define a custom language model. The output of a LUIS application is a web service with an HTTP endpoint that you reference from your client application to add natural language understanding to it. A LUIS app takes a user utterance and extracts intents and entities that are relevant to your client application’s domain. Your client app can then take appropriate action based upon the user intentions that LUIS recognizes.
