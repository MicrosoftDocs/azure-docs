...
The speech-to-text integration is known as **Speech-to-intent**. This Speech-to-intent key is a separate key created from the Azure portal. Once the key is created, use this key during app creation, and at publish time. The process creates a new endpoint. The key usage is priced to include both Speech and LUIS. If the endpoint receives a text utterance, which doesn't require Speech-to-text conversion, only the LUIS endpoint quota is used.
