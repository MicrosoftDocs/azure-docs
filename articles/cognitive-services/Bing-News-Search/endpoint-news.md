...
Returns news items based on the user's search query. If the search query is empty, the call returns the top news articles. The query `?q=""` option can also be used with the `/news` URL. For availability, see [Supported countries and markets](language-support.md#supported-markets-for-news-search-endpoint).
