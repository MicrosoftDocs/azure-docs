...
1.  To start using Custom Decision Service for your application, register it on the portal. In the top ribbon, click **My Portal**, as highlighted in the image:

    ![Custom Decision Service home page](./media/custom-decision-service-get-started-browser/home.png)

    If you are not already signed in, the portal prompts you to sign in with your [Microsoft account](https://account.microsoft.com/account). After you have signed in, the portal displays your Microsoft account in the top-right corner of the page.
...
3. In the dialog box, choose an identifier for your application. Custom Decision Service requires a unique identifier for each application. If someone else has already taken the identifier, the system asks you to pick a different one.

4. Specify an action set API. This setting is an RSS or Atom feed that communicates the available content for your application to Custom Decision Service. Enter a name for the feed, along with the URL from which it is served. To do this step later, click the **Feeds** button and then click the **New Feed** button. An example for creating an RSS feed is described later.

5. To register your application in the [application-specific learning mode](custom-decision-service-overview.md#application-specific-learning-mode), click the **Advanced** check box in the lower-left corner of the dialog box. Enter a [connection string](../../storage/storage-configure-connection-string.md) for the Azure storage account where your application data is logged. For more information on how to create a storage account, see [How to create, manage, or delete a storage account](../../storage/storage-create-storage-account.md).
...
Here `data` is the argument to the `callback()` function, as described previously. An implementation example is provided in the [Tutorial](custom-decision-service-tutorial.md#our-apis).
...
Here each top-level `<item>` element describes an article. The `<link>` element is mandatory and is used as an action identifier by Custom Decision Service. Specify the `<date>` element (in a standard RSS format) if you have more than 15 articles. The 15 most recent articles are used. The `<title>` element is optional and is used in creating text-related features for the article.
