...
For best results, provide 4 `period`'s worth of data point, plus an additional one. For example, hourly data with a weekly pattern as described above should provide 673 data points in the request body (`7 * 24 * 4 + 1`). If this amount of data points exceeds the maximum number allowed (8640 data points) and your data has a stable seasonal pattern, consider sending a sample of your time series data at a larger time interval. Sampling your data can also decrease the time it takes to detect anomalies, without severely impacting your results.
