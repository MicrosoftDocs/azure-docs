...
The `ApiKey` setting specifies the Azure resource key used to track billing information for the container. You must specify a value for the ApiKey and the value must be a valid key for the _[Personalizer]_ resource specified for the [`Billing`](#billing-setting) configuration setting.
...
The `Billing` setting specifies the endpoint URI of the _[Personalizer]_ resource on Azure used to meter billing information for the container. You must specify a value for this configuration setting, and the value must be a valid endpoint URI for a _[Personalizer]_ resource on Azure.
...
The [Personalizer] containers don't use input or output mounts to store training or service data. 
