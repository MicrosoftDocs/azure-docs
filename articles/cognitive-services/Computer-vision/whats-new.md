...
* The *personcrossingpolygon* operation has been extended to allow the calculation of the dwell time a person spends in a zone. You can set the ‘type’ parameter in the Zone configuration for the operation to `zonedwelltime` and a new event of type personZoneDwellTimeEvent will include the `durationMs` field populated with the number of milliseconds that represent the time the person spent in the zone.

* **Breaking change**: The personZoneEvent event has been renamed to personZoneEnterExitEvent. This event is emitted by the personcrossingpolygon operation when a person enters or exits the zone and provides directional info with the numbered side of the zone that was crossed.
...
* Added support for auto recalibration (by default disabled) via the `enable_recalibration` parameter, please refer to the documentation for details

* Camera calibration parameters to the `DETECTOR_NODE_CONFIG`. Refer to the documentation for details.
