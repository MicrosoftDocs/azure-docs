...
|**[Get the region of interest](concept-generating-thumbnails.md#region-of-interest)**|Analyze the contents of an image to return the coordinates of the *region of interest*. This is the same function that is used to generate a thumbnail, but instead of cropping the image, Computer Vision returns the bounding box coordinates of the region, so the calling application can modify the original image as desired.|
