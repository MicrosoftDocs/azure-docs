...
To enable a visualization of AI Insights events in a video frame, you need to use the `.debug` version of a [spatial analysis operation](spatial-analysis-operations.md) on a Desktop machine. The visualization is not possible on Azure Stack Edge (ASE) devices. There are four debug operations available.
