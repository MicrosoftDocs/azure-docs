...
# Get started with Bing speech recognition in C&#35; for .NET Windows
...
Download the Speech API client library and the example through the [SDK](https://github.com/microsoft/cognitive-speech-stt-windows). Extract the downloaded zip file to a folder of your choice. Many users choose the Visual Studio 2015 folder.
...
Press Ctrl+Shift+B, or select **Build** on the ribbon menu. Select **Build Solution**.
...
2. In the **Project Oxford Speech To Text Sample** window, paste your subscription key in the  text box shown in the following screenshot. To persist your subscription key on your PC or laptop, select **Save Key**. To delete the subscription key from the system, select **Delete Key** to remove it from your PC or laptop.
...
Example audio files are available for use with this example application. The files are in the repository you downloaded with this example under **SpeechToText**, in the **Windows** folder, under **samples**, in the **SpeechRecognitionServiceExample** folder. These example audio files  run automatically if no other files are chosen when you select **Use wav file for Shortphrase mode** or **Use wav file for LongDictation mode**. Currently, only wav audio format is supported.
...
This event gets called every time the speech recognition service has an idea of what the speaker might be saying. It's called even before the speaker finishes speaking (if you use the microphone client) or finishes transferring data (if you use the data client).
#### Intent event
This event gets called on WithIntent clients (only in ShortPhrase mode) after the final reco result is parsed into structured JSON intent.
#### Result event
This event is called when you finish speaking (in ShortPhrase mode). You are provided with n-best choices for the result. In LongDictation mode, the handler associated with this event is called multiple times, based on where the service identifies sentence pauses.
...
* [Get started with the Bing Speech API and/or intent in Objective-C on iOS](Get-Started-ObjectiveC-iOS.md)
