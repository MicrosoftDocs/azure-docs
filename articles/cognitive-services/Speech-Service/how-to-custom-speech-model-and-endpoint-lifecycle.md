...
When a custom model or baseline model expires, typically speech recognition requests will fall back to the most recent baseline model for the same language. In this case, your implementation won't break, but recognition might not accurately transcribe your domain data. 
...
```json
{
  ...
  "description": "en-US baseline model"
}
```
