...
**Answer**: Baseline models have been trained with Microsoft owned data and are already deployed in the cloud. Custom models allow the user to adapt a model to better fit his domain be it a particular environment with specific ambient noise or language. Factory floors, cars, noisy streets would require adapted acoustic model whereas specific topics such as biology, physics, radiology, product names and custom acronyms would require a language model.

**Question**: Where do i start if i want to use baseline?

**Answer**: First you need to get a [subscription key](get-started.md). If you want to make REST calls to the predeployed baseline models please consult the [details here](rest-apis,md). If you want to use websockets then please download the [SDK](speech-sdk.md)

**Question**: Do I always need to build a custom speech model?

**Answer**: No, if your application is using generic day to day language without custom vocabulary or that maybe rare then you do not need to customize a model. In addition if you application is to be used in an environment where the there is little or no background noise then you do not need to customize either. Our portal allows you to deploy baseline and customized models and run accuracy tests against them. You can use these feature to measure accuracy of a baseline vs a custom model.
