...
Tenant Language Model is an opt-in service for Office365 enterprise customers that automatically generates a custom speech recognition model from your organization's Office365 data. The model that's created is optimized for technical terms, jargon and people's names, all in a secure and compliant way.
...
Before you can deploy your Tenant Language Model, first need to enroll using the Microsoft 365 Admin Center. This task can only be completed by your Microsoft 365 Admin.
...
In order to use a Tenant Language Model with the Speech SDK, you'll need a Speech resource and its associated subscription key.
...
2. In the upper right corner, locate and click the gear icon (settings), then select **Tenant Model settings**.
   ![Settings menu](tenant-language-model/tenant-laguage-settings.png)
...
When your Tenant Language Model is ready, follow these steps to deploy your model:
...
![App registration flow](tenant-language-model/tenant-laguage-model-app-registration.png)
