...
The REST APIs of the Azure Cognitive Services unified Speech service are similar to the APIs provided by the [Speech API](https://docs.microsoft.com/azure/cognitive-services/Speech) (formerly known as Bing Speech). The endpoints differ from the endpoints used by the previous Speech service.

## Speech to Text

In the Speech to Text API, only the endpoints used differ from the previous Speech service Speech Recognition API. The new endpoints are shown in the following table. Use the one that matches your subscription region.
...
The Speech to Text REST API supports only short utterances. Requests might contain up to 10 seconds of audio and last a maximum of 14 seconds overall. The REST API returns only final results, not partial or interim results.
...
The new Text to Speech API supports 24-KHz audio output. The `X-Microsoft-OutputFormat` header now might contain the following values.
...
The following REST endpoints are for the unified Speech service Text to Speech API. Use the endpoint that matches your subscription region.
...
Sending a request to the Speech service's REST API requires an access token. To obtain a token, provide your subscription key to a regional Speech service `issueToken` endpoint. Endpoints are shown in the following table. Use the endpoint that matches your subscription region.
...
> [!NOTE]
> Instead of using a timer, you can store a timestamp of when the current token was obtained. Then you can request a new one only if the current token is close to expiring. This approach avoids requesting new tokens unnecessarily. It might be more suitable for programs that make infrequent Speech requests.
