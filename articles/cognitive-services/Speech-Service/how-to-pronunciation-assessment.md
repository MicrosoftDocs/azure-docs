...
> [!NOTE]
> The pronunciation assessment feature currently supports `en-US` language, which is available on all speech-to-text regions. See supported [region list](regions.md#speech-to-text-text-to-speech-and-translation). The support for `en-GB` and `zh-CN` languages is under preview, which is available on `westus`, `eastasia` and `centralindia` regions.
