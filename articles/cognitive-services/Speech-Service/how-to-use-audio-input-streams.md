...
In general, the following steps are required when using audio input streams:

  - Identify and the format of the audio stream. The format must be supported by the SDK and the speech service. Currently only the following configuration is supported:
    ...
    The corresponding code in the SDK conceptually looks like this:
  ...
  - Create an audio configuration based on your audio format and input stream. Pass in both your regular speech config and the audio input config when you create your recognizer. For example:
