...
> [!NOTE]
> Viseme ID supports neural voices in [all viseme-supported locales](language-support.md). Scalable Vector Graphics (SVG) only supports neural voices in `en-US` locale, and blend shapes supports neural voices in `en-US` and `zh-CN` locales.
