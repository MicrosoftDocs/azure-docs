---
title: 'Quickstart: Custom speech assistant, C# (UWP) - Speech Service'
...
# Quickstart: Create a speech assistant with the Speech SDK, UWP
...
In this article, you'll develop a C# Universal Windows Platform (UWP) application using the [Speech SDK](speech-sdk.md). The program will connect to a previously authored and configured bot to enable a speech assistant experience from the client application. The application is built with the [Speech SDK NuGet Package](https://aka.ms/csspeech/nuget) and Microsoft Visual Studio 2019 (any edition).
...
* A previously created bot configured with the [Direct Line Speech channel](https://docs.microsoft.com/azure/bot-service/bot-service-channel-connect-directlinespeech).
  ...
  > [!NOTE]
  > Please refer to [the list of supported regions for speech assistants](regions.md#speech-assistants) and ensure your resources are deployed in one of those regions.
...
1. Choose **Debug** > **Start Debugging** (or press **F5**) to start the application. The **helloworld** window appears.
   ...
   ![Sample UWP speech assistant application in C# - quickstart](media/sdk/qs-speech-assistant-uwp-helloworld-window.png)
...
<!--
    ![Successful bot response](media/speech-assistants/quickstart-cs-uwp-bot-successful-turn.png)
-->
## Next steps
...
- [About speech assistants](speech-assistants.md)
