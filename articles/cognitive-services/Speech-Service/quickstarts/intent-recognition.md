...
2. Create a ````IntentRecognizer```` object using the ````SpeechConfig```` object from above.
3. Using the ````IntentRecognizer```` object, start the recognition process for a single utterance.
4. Inspect the ````IntentRecognitionResult```` returned.
...
2. [Setup your development environment](~/articles/cognitive-services/Speech-Service/quickstarts/setup-platform.md&tabs=dotnet%2CWindows%2Cjre). Use this quickstart to install and configure Visual Studio 2019.
