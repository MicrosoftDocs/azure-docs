...
You need to get the SharePoint file's URI before adding it to QnA Maker. There is a workaround to add latest SharepPoint content via API using Azure blob storage, below are the steps: 
1. Download the SharePoint files locally. The user calling the API needs to have access to SharePoint. 
2. Upload them on the Azure blob stoarge. This will create a secure shared access by [using SAS token.](https://docs.microsoft.com/azure/storage/common/storage-sas-overview#how-a-shared-access-signature-works) 
