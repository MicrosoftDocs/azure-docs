...
* organize your knowledge using one of the following:
    * restrict your QnA resource (and the resulting Azure Cognitive Search test index) to a single knowledge base. 
    * or, have two QnA Maker resources, using one for testing (with its own test and  production indexes) and one for product (also having its own test and production indexes)
