...
From the subscription combo box select the subscription you want to use. In case it is an S2 subscription, you can select scale units and content logging (please check [meter information](../cognitive-services-custom-speech-meters.md) for details on scale units and logging).

In terms of scale unit here is a mapping of what you get in as available concurrent requests support:
...
You can also select if content logging is switched on or off, means that the traffic of the endpoint will be stored for Microsoft internal use or not.
