...
Since beginning of July 2017 the Custom Speech Service offers a [new pricing model](https://azure.microsoft.com/en-us/pricing/details/cognitive-services/custom-speech-service/ ). The new model is **easier to understand**, **simpler to calculate costs** and **more flexible** in terms of scaling. For scaling we are introducing the concept of a scale unit. Each scale unit can handle 5 concurrent requests. The scaling for concurrent requests was fixed in the old model to 5 concurrent request for S0 and 12 concurrent request for S1. We are opening these limits to enable you to be more flexible with regards to your use case requirements.

We recommend migrating your existing deployments to the new tier (S2) no matter if you run on old S0 or S1 tier. The new S2 tier covers both S0 and S1 tier. You can see the available options in the figure below:

![try](../../../media/cognitive-services/custom-speech-service/custom-speech-pricing-tier.png)


We can handle the migration in a semi-automated way. You must trigger it but we handle the migration of your deployment then automatically.
The mapping from old tiers to scale units is as follows:

| Tier | Concurrent request (old model) | Migration | Concurrent requests |
|----- | ----- | ---- | ---- |
| S0 | 	5	|	=> **S2** with 1 scale units |	5 |
| S1 |	12	|	=> **S2** with 3 scale units |	15 |

To migrate to the new tier, please follow the steps:
...
Now, open another tab and log in into the [Azure portal](http://ms.portal.azure.com/) and find your custom speech subscription in the list of all resources. Select the correct subscription. This opens a tile in which you can select _Pricing tier_.

![try](../../../media/cognitive-services/custom-speech-service/custom-speech-update-tier.png)

On the pricing tier tile select _S2 Standard_ which is the new, simplified and more flexible pricing tier and click on **Select**.
...
In  _Deployment Options_  you find information about scale units and logging now. The scale units should reflect you previous paying tier as explained above. The logging should be turned on.
...
> [!NOTE]
> In case there are problems during the migration, please contact us.
>

## Next steps
Now you are ready to follow up with the next steps
* Adapt your [custom acoustic model](cognitive-services-custom-speech-create-acoustic-model.md)
* Adapt your [custom language model](cognitive-services-custom-speech-create-language-model.md)
