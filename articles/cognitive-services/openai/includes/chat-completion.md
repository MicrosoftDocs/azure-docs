...
Unlike previous GPT-3 and GPT-3.5 models, the `gpt-35-turbo` model as well as the `gtp-4` and `gpt-4-32k` models will continue to be updated. When creating a [deployment](../how-to/create-resource.md#deploy-a-model) of these models, you'll also need to specify a model version.
