...
| language | `{LANGUAGE-CODE}` |  A string specifing the langugage code for the document. In case this key ws not specified, the service will assume the default langugae of the project that was lected during project creation. Learn more about supported language codes [here](../../language-support.md) |`en-us`|
| text | `{DOC-TEXT}` | Document task to run the tasks on. | `Lorem ipsum dolor sit amet` |
|"tasks"|`[]`| List of tasks we want to perform.|`[]`|
|--|customMultiLabelClassificationTasks|Task identifer for task we want to perform. | |
...
| text | `{DOC-TEXT}` | Document task to run the tasks on. | `Lorem ipsum dolor sit amet` |
|"tasks"|`[]`| List of tasks we want to perform.|`[]`|
|--|customSingleLabelClassificationTasks|Task identifer for task we want to perform. | |
|parameters|[]|List of parameters to pass to the task.|[]|
| project-name |`{PROJECT-NAME}` | The name for your project. This value is case-sensitive.  | `myProject` |
| deployment-name |`{DEPLOYMENT-NAME}` | The name of your deployment. This value is case-sensitive.  | `prod` |
...
You can use this URL to query the task completinon status and get the results when task is completed.
