---
title: Character Counts - Translator API
titleSuffix: Azure Cognitive Services
description: This article explains how the Azure Cognitive Services Translator API counts characters so you can understand how it ingests content.
...
# How the Translator API counts characters

The Translator API counts every unicode code point of input text as a character. Each translation of a text to a language counts as a separate translation, even if the request was made in a single API call translating to multiple languages. The length of the response does not matter.

What counts is:

* Text passed to the Translator API in the body of the request
...
For scripts based on ideograms such as Chinese and Japanese Kanji, the Translator API still counts the number of Unicode code points, one character per ideogram. Exception: Unicode surrogates count as two characters.
