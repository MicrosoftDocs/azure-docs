...
- Dictionaries are not a substitute for training a model using training data. Best practice is to let the system learn from your training data and rely on the dictionary as sparingly as possible. However, when sentences or compound nouns must be rendered as-is, use a dictionary.
...
- When using a dictionary, capitalization and punctuation is important. Dictionary entries will only match words and phrases that have the same capitalization and punctuation as the entry found in the dictionary. Your translations will reflect the capitalization and punctuation provided in the target side of the dictionary file. As an example, you have trained an English to Spanish model using a dictionary that specified “hello” in the source file to be translated to “buenos dias” in the target file. When you request translation of a sentence that includes “hello”, the system will search your dictionary first and would find a match ("hello") and would return “buenos dias” in the final translation.
