...
### Authenticating with Azure Active Directory (Azure AD)
 ...
 Translator v3.0 supports Azure AD authentication, Microsoft’s cloud-based identity and access management solution.  Authorization headers enable the Translator service to validate the requesting client as authorized to use the resource and to complete the request. 
...
You can authenticate your client by including the following headers with your REST API request:
...
|Header|Value|
|:-----|:----|
|Authorization| The value is an access **bearer token** generated by Azure AD. The bearer token provides proof of authentication and validates the client's authorization to use the resource. An authentication token is valid for 10 minutes and should be reused when making multiple calls to the Translator.  *See* [Authenticating with an access token](#authenticating-with-an-access-token), above. |
|Ocp-Apim-Subscription-Region| The value is the region of the multi-service or regional **translator resource**. This value is optional when using a global translator resource.|
|Ocp-Apim-ResourceId| <ul><li>The value is the the Resource ID file path for your Translator resource instance.</li><li> You'll find the Resource ID in the Azure portal at **Translator Resource  → Properties**. </li><li>Resource ID file path components: </br>/subscriptions/<**subscriptionId**>/resourceGroups/<**resourceGroupName**>/providers/Microsoft.CognitiveServices/accounts/<**resourceName**></li></ul>|
...
```curl
 // Using headers, pass a bearer token generated by Azure AD, resource ID, and the global region using headers.
...
Translator v3.0 also supports authorizing access to managed identities. If the managed identity is enabled for a translator resource, you can pass the bearer token generated by managed identity in the request header. *See* [Managed identity](../document-translation/managed-identity.md)

##### **Example request using managed identities**

###### **Using the global endpoint**

```curl
// Using headers, pass a bearer token generated either by Azure AD or Managed Identities, resource ID, and the global region.
