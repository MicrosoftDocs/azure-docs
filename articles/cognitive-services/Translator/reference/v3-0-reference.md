...
Translator v3.0 supports Azure AD authentication, Microsoft’s cloud-based identity and access management solution. Azure AD provides an access token as proof of authentication. The **bearer token generated by Azure AD** and the **Resource ID** are sent to Translator service in the REST API request authorization header allowing it to validate the client is authorized to use the resource and complete the request.
...
|Headers|Value|
|:-----|:----|
|Authorization| The value is a **bearer token** generated by Azure AD.|
|Ocp-Apim-Subscription-Region| The value is the **translator resource region**. This value is optional if the resource is `global` .|
...
:::image type="content" source="../media/managed-identities/resource-id-property.png" alt-text="Screenshot:Translator properties in the Azure portal. ":::
...
##### **Example request using cURL to authorize access to managed identities**
