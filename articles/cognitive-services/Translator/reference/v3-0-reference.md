...
|Header|Value|
|:-----|:----|
|Authorization| The value is an access **bearer token** generated by Azure AD.<ul><li> The bearer token provides proof of authentication and validates the client's authorization to use the resource.</li><li> An authentication token is valid for 10 minutes and should be reused when making multiple calls to the Translator.</br></li>*See* [Authenticating with an access token](#authenticating-with-an-access-token), above. </ul>|
|Ocp-Apim-Subscription-Region| The value is the region of the multi-service or regional **translator resource**.<ul><li> This value is optional when using a global translator resource.</li></ul>|
...
#### **Example HTTP request using managed identities**

Translator v3.0 also supports authorizing access to managed identities. If the managed identity is enabled for a translator resource, you can pass the bearer token generated by managed identity in the request header.
