...
The first comment tells your Python interpreter to use UTF-8 encoding. Then required modules are imported to read your subscription key from an environment variable, construct the http request, create a unique identifier, and handle the JSON response returned by the Translator API.

## Set the subscription key, endpoint, and path

This sample will try to read your Translator API subscription key and endpoint from the environment variables: `TRANSLATOR_TEXT_KEY` and `TRANSLATOR_TEXT_ENDPOINT`. If you're not familiar with environment variables, you can set `subscription_key` and `endpoint` as a strings and comment out the conditional statements.
...
The Translator API global endpoint is set as the `endpoint`. `path` sets the `detect` route and identifies that we want to hit version 3 of the API.
