...
| Ocp-Apim-Subscription-Region | This header is only required when using a multi-service subscription key with the [Translator API](./Translator/reference/v3-0-reference.md). Use this header to specify the subscription region. |
...
This is a sample call to the Translator API:
```cURL
...
When using multi-service subscription key with the Translator API, you must specify the subscription region with the `Ocp-Apim-Subscription-Region` header.
...
After you get an authentication token, you'll need to pass it in each request as the `Authorization` header. This is a sample call to the Translator API:
