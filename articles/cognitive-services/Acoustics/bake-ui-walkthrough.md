...
8. Select this checkbox to configure your compute pool to use [low-priority nodes](https://docs.microsoft.com/en-us/azure/batch/batch-low-pri-vms). Low-priority compute nodes have much lower cost but they may not always be available or may be preempted at any time.
...
Local bake runs acoustics simulation on your own local PC instead of offloading it to the cloud compute cluster. This is a good option for experimenting with acoustics without requiring an Azure subscription. Please be aware  that acoustics simulation is computationally demanding and processing times can be very long depending on the size of the scene, simulation configuration and raw computing power of the processing machine.


### Minimum hardware requirements
We recommend 64-bit Intel processor with at least 8 cores and 32 GB of RAM or higher.
On a PC that meets these minimum specs -
...
### Setup Docker
Please install and configure Docker on the PC that will process the simulation -
1. Install [Docker](https://www.docker.com/products/docker-desktop) toolset.
2. Launch Docker settings, navigate to the "Advanced" settings and set the resources as shown below. ![Docker resources](media/DockerSettings.png)
...
### Run local bake
1. Click on "Prepare Local Bake" button on the Bake tab and select a folder where input files and execution scripts will be saved. You can run the bake on any machine as long as it meets the minimum hardware requirements and has Docker installed, simply copy the entire folder to that machine.
2. Start the simulation by executing "runlocalbake.bat" script. This script will fetch the ProjectAcoustics Docker image with the toolset necessary for simulation processing and start the simulation. 
3. On completion of simulation copy the resulting .ace file back to your Unity project. Please ensure you're using the same path as specified in the Probes tab and the destination filename conforms to Unity's requirements by appending ".bytes" to the file extension.
