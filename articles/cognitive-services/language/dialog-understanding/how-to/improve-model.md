...
* If you observe 2 intents that are being confused with each other consistently, they may be too similar. Consider merging those intents into one and using an entity to extract the the distinction.
    * You might also consider breaking the entity down into smaller ones that capture aspects of the entity you are trying to extract.
* If an entity is not being predicted correctly, consider adding more labelled examples.
* If an entity is incorrectly predicted while it wasn't tagged in your utterances, consider fixing your labels if they aren't complete, or adding more examples where similar terms are not labelled as entities. This will provide the model with different data points that may improve prediction.


## Examine data distribution

By clicking on **Examine data distribution**, you can observe the amount of data associated to each intent or entity. A large disparity may cause your model to be biased to a more heavily populated intent or entity. If this happens, consider adding more labels to other intents and entities. You want to ensure that there isn't under-representation or over-representation in your model.
