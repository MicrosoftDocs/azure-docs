...
| CONDITION_QUALIFIER | Condition levels such as `_mild_`, `_extended_`, or `_diffuse_`. | 
| DIAGNOSIS | Medical conditions. For example `_hypertension_`. | 
| DIRECTION | Directions such as `_left_` or `_anterior_`. | 
...
| FAMILY_RELATION | A familial relationship,  such as `_sister_`.  | 
| FREQUENCY | Frequencies.   | 
| GENDER | Genders. | 
| GENE | A gene entity such as `_TP53_`.   | 
| MEDICATION_CLASS | Medication classes. For example `_antibiotics_`.  | 
