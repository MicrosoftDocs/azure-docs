---
title: Text Analytics for health Kubernetes config and deploy steps
titleSuffix: Azure Cognitive Services
description: Text Analytics for health Kubernetes config and deploy steps
...
4. Within the text editor, create a new file named *textAnalyticsHealth.yaml*, and paste the following YAML into it. Be sure to replace `billing/value` and `apikey/value` with your own information.
    ...
    spec:
      template:
        metadata:
          labels:
            app: text-analytics-health-app
        spec:
          containers:
          - name: health
            image: containerpreview.azurecr.io/microsoft/cognitive-services-healthcare:latest
            ports:
            - containerPort: 5000
            resources:
              requests:
