...
**Inbound flows logged from internet IPs to VMs without public IPs**: VMs that do not have a public IP address assigned via a public IP address associated with the NIC as an instance-level public IP or are part of a basic load balancer backend pool use [default SNAT](../load-balancer/load-balancer-outbound-connections.md#defaultsnat) and have an IP address assigned by Azure to facilitate outbound connectivity.  As a result, you may see flow log entries for flows from internet IP addresses if the flow is destined to a port in the range of ports assigned for SNAT.  While the Azure will not allow these flows to the VM, the attempt is logged and appears in Network Watcher's NSG flow log by design.  Additionally, we also recommended that unwanted inbound internet traffic be explicitly blocked with NSG.
