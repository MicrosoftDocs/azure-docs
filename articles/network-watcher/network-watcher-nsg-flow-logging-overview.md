...
- Note: Rules are of two types terminating & non-terminating with different logging behaviours.
- - NSG deny rules are terminating and the NSG denying the traffic would be responsible to log it in flow logs and processing in this case would stop after any NSG denies traffic. 
- - NSG Allow rules are non-terminating, which means even if one NSG allows it, the processing would further continue to go through next NSG and the last NSG allowing the traffic would be responsible for logging the traffic in flow logs.
