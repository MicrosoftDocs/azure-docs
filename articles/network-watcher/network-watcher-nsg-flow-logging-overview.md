...
**Incorrect byte and packet counts for Inbound flows**: [Network Security Groups (NSGs)](https://docs.microsoft.com/azure/virtual-network/security-overview) are implemented as a [Stateful firewall](https://en.wikipedia.org/wiki/Stateful_firewall?oldformat=true). However, due to platform limitations, rules that control the inbound flow of traffic are implemented in a stateless fashion. Because of this bytes and packets counts are not recorded these flows (that is, the traffic flows going through inbound rules). Additionally, the inbound flow are non-terminating. Consequently the number of bytes and packets reported in NSG Flow Logs (and Traffic Analytics) could be different from actual flows. This limitation is scheduled to be fixed by June 2020.
