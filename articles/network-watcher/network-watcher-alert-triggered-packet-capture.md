...
2. On the **Function App**, enter the following values, and then select **OK** to create the app:
    ...
    |**Subscription**|[Your subscription]The subscription to create the function app in.||
    |**Resource Group**|PacketCaptureRG|The resource group to contain the function app.|
    |**Hosting Plan**|Consumption Plan| The type of plan your function app uses. Options are Consumption or App Service plan. |
    |**Location**|Central US| The region to create the function app in.|
    |**Storage Account**|{autogenerated}| The storage account that's needed by Azure Functions for general-purpose storage.|
...
4. Create a subfolder for each module that's needed.
...
6. Go to your Azure modules, and, in the local **AzureRM.Network** folder, select all the files in the folder. Then select **Ok**. 
...
The value of the AzureCredPassword environment variable is the value you get from running the following PowerShell sample. This example is the same one that's shown in the preceding **Encrypted Credentials** section. The value that's needed is the output of the `$Encryptedpassword` variable.  This is the service principal password that your encrypted by using the PowerShell script.
...
4. Poll packet capture periodically until its complete.
