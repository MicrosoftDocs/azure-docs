...
3. Enter, or select the following information, accept the remaining defaults, and then select **Create**.
    ...
    > [!NOTE]
    > While Microsoft.Insight and Microsoft.Network providers are currently supported for Azure Storage, NSG Flow logs is still not fully onboarded. To enable NSG Flow logging, **All Networks** must still be selected until this feature is fully onboarded. 
