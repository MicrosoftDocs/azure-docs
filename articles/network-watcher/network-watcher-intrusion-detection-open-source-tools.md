---
title: Perform network intrusion detection with Network Watcher and open source tools | Microsoft Docs
description: This article describes how to use Network Watcher and open source tools to perform network intrusion detection
...
The logs generated by Suricata are fed into an ELK stack, where it would be possible to visualize alerts and gain insights to potential network vulnerabilities.

## Prerequisites

* **[Understand Network Watcher](network-watcher-monitoring-overview.md)**
* **[Understand Variable Packet Capture](network-watcher-packet-capture-overview.md)**
* **[Suricata](https://suricata-ids.org/)** - Open Source Network Threat Detection Engine
* **[Elastic Stack](https://www.elastic.co/subscriptions)** - Open Source suite comprising Elastics Search, Logstash, and Kibana

## Set up

This article explains how to set up your environment to perform network intrusion detection using Suricata and the Elastic Stack. Both open source tools can be set up on an Azure VM, allowing you to perform this analysis within your own Azure network environment

* The setup has a VM serving web content.
* Suricata and the Elastic Stack are installed in another VM.
* Packets are captured on the first VM using Network Watcher. The captured packets are then stored in the blob storage.
* Suricata IDS engine is used to perform deep packet inspection, analyzing the packet for anomalous data and triggering alerts to a log file.
* Suricata logs are fed into the Elastic Stack so that a Kibana dashboard can be used for visualizing alerts.
...
All other methods of installation can be found at [http://suricata.readthedocs.io/en/latest/install.html](http://suricata.readthedocs.io/en/latest/install.html).
...
### Step 2 - Downloading the Emerging Threats Ruleset

At this stage, we do not have any rules for Suricata to run. If there are specific threats, you can create your own rules to your network you want to detect. You can also use developed rule sets from several providers, such as Emerging Threats, or VRT rules from Snort. We use the freely accessible Emerging Threats ruleset here:

1. Download the rule set and copy them into the directory:
...
### Step 3 - Using Suricata with packet captures
...
1. Make sure to give permission to the eve.json file so that Logstash can ingest the file.
...
For this article, we have provided a sample dashboard for you to view trends and details in your alerts. The benefit of using Kibana is that you can create your own visualizations and dashboards tailored towards your top network needs. 
