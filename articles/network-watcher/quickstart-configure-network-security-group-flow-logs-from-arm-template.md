---
title: 'Quickstart: Configure NSG flow logs by using an ARM template'
...
The following code shows an NSG flow logs object and its parameters. For a complete overview of the properties, see [Microsoft.Network networkWatchers/flowLogs](/azure/templates/microsoft.network/networkwatchers/flowlogs).
...
To create a `Microsoft.Network/networkWatchers/flowLogs` resource, add this JSON to the resources section of your template.
...
> [!NOTE]
> - The resource name uses the format _ParentResource_ChildResource_. In our example, the parent resource is the regional Network Watcher instance:
...
This tutorial assumes that you have an existing resource group and an NSG you can enable flow logging on.

You can save any of the example templates that are shown in this article locally as azuredeploy.json. Update the property values so that they point to valid resources in your subscription.
...
> [!NOTE]
> These commands deploy a resource to the NetworkWatcherRG resource group, and not to the resource group that contains the NSG.

## Validate the deployment

You can check to see whether your deployment succeeded in two ways:

- Your PowerShell console shows `ProvisioningState` as `Succeeded`.
- You can visit the [NSG flow logs portal page](https://ms.portal.azure.com/#blade/Microsoft_Azure_Network/NetworkWatcherMenuBlade/flowLogs) to confirm your changes. If there were issues with the deployment, see [Troubleshoot common Azure deployment errors with Azure Resource Manager](../azure-resource-manager/templates/common-deployment-errors.md).

## Clean up resources

You can delete Azure resources by using the `Complete` deployment mode. To delete a flow logs resource, specify a deployment in `Complete` mode without including the resource you want to delete. Read more about the [Complete deployment mode](../azure-resource-manager/templates/deployment-modes.md#complete-mode).

Alternatively, you can disable an NSG flow log in the Azure portal:
...
In this quickstart, you learned how to enable NSG flow logs. Next, learn how to visualize your NSG flow data by using one of these options:
