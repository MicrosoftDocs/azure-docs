...
> [!IMPORTANT]
> Packet capture requires an extension to be running on the virtual machine. For instructions on to install the VM extension `AzureNetworkWatcherExtension`, visit [Virtual Machine extensions and features](../virtual-machines/virtual-machines-windows-extensions-features.md) or visit [Manage Packet Capture with PowerShell](network-watcher-packet-capture-manage-powershell.md#install-vm-extension)
