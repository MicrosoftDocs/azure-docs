...
Only the production Operational Database (ODB) or Production Analytical Database (ADB/Clarity) require the use of Azure Large Instances. 
...
This Express route Circuit is available to Azure resources. If direct connectivity to the ALI instances is required from on-premises, an Express Route Global Reach Circuit is required.
...
Snapshots on the ALI NetApp should be transferred out of the ALI environment onto your Azure tenant using NetApp Cloud Volumes ONTAP. Microsoft works with you to create a SnapMirror relationship from ALI to your CVO instance.
After which the lifecycle and snapshot management are your responsibility.

CVO can be applied to create clones of the production database in which you can run integrity checks, send to Azure Blob or a 3rd party for long term backup retention, and clone the database to meet the support environment requirements.  
