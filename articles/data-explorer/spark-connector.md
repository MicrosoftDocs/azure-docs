...
1. When reading large amounts of data, transient blob storage must be provided. Provide storage container SAS key, or storage account name, account key, and container name. In the future, the transient storage will be managed by the connector and this step is only required for the current preview release.
