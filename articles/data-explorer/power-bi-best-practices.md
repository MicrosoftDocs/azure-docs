...
\\Todo: create an intro for Power BI, not Power BI connector. talk about different ways to connect...

\\Todo: Move this section to connector:
Using Azure Data Explorer native connector for creating Power BI dashboards is straightforward. 

## Import or Direct Query

The Power BI Kusto Connector supports [Import and Direct Query connectivity modes](https://docs.microsoft.com/en-us/power-bi/desktop-directquery-about).
You can build dashboards using either Import mode or DirectQuery mode depending on the scenario, scale, and performance requirements. 

\\Todo: change to table?
You should use **Import** mode when:

 * Your data set is relatively small - Import mode retrieves all the data and retain it with the Power BI report
 ...
 * You need near-real-time data - every time data is displayed, it is being queried directly from your Azure Data Explorer cluster

## Best practices for working with PowerBI
...
**Using complex queries in Power BI** - If you have complex queries - queries that are more easily expressed in CSL than in Power Query - it is better to implement those queries as [Kusto Functions](), and invoke those functions in Power BI. One such scenario where this is not only recommended, but required, it when you have let statements in your queries, and you are using Direct Query (which might result in joins). Because Power BI will take 2 queries and join them, this might result in syntax errors (since let statements cannot be used with the join operator). In this scenario, you should save each leg of the join as a Kusto Function and let Power BI join between these 2 functions.

## Tips and tricks with Power BI

### How to simulate `Timestamp < ago(1d)`

Power BI doesn't have a 'relative' date-time operator, such as `ago()`.
To simulate it, use a combination of PowerBI functions,
`DateTime.FixedLocalNow()` and `#duration`:
...
in
    #"Filtered Rows"
```
The query is equivalent to the following Kusto \\CSL\\ query:
...
Kusto queries return, by default, up to 500,000 rows or 64MB, as described in [Query Limits](../concepts/querylimits.md). You can override these defaults using **Advanced options** in the connection form:
...
1. If your query contains double-quotes, make sure to encode them correctly. For example, if we have the query: 
...
You can use Query Parameters in any query step that supports it. For example, to filter the results based on the value of a Parameter:
