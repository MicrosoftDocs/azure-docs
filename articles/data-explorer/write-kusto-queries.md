...
[**count**](https://review.docs.microsoft.com/kusto/query/countoperator): Returns the count of rows in the table.
...
[**take**](https://review.docs.microsoft.com/kusto/query/takeoperator): Returns up to the specified number of rows of data.
...
[**project**](https://review.docs.microsoft.com/kusto/query/projectoperator):
...
[**where**](https://review.docs.microsoft.com/kusto/query/whereoperator): Filters a table to the subset of rows that satisfy a predicate.
...
[**sort**](https://review.docs.microsoft.com/kusto/query/sortoperator): Sort the rows of the input table into order by one or more columns.
...
[**top**](https://review.docs.microsoft.com/kusto/query/topoperator): Returns the first *N* records sorted by the specified columns.
...
[**extend**](https://review.docs.microsoft.com/kusto/query/extendoperator): Computes derived columns.
...
[**summarize**](https://review.docs.microsoft.com/kusto/query/summarizeoperator): Aggregates groups of rows.
...
[**render**](https://review.docs.microsoft.com/kusto/query/renderoperator): Renders results as a graphical output.
...
[**bin()**](https://review.docs.microsoft.com/kusto/query/binfunction): Rounds values down to an integer multiple of a given bin size.
...
[**case()**](https://review.docs.microsoft.com/kusto/query/casefunction): Evaluates a list of predicates, and returns the first result expression whose predicate is satisfied, or the final **else** expression. You can use this operator to categorize or group data:
...
[**extract()**](https://review.docs.microsoft.com/kusto/query/extractfunction): Gets a match for a regular expression from a text string.
...
[**parse_json()**](https://review.docs.microsoft.com/kusto/query/parsejsonfunction): Interprets a string as a JSON value, and returns the value as dynamic. It is superior to using the **extractjson()** function when you need to extract more than one element of a compound JSON object.
...
[**ago()**](https://review.docs.microsoft.com/kusto/query/agofunction): Subtracts the given timespan from the current UTC clock time.
...
[**startofweek()**](https://review.docs.microsoft.com/kusto/query/startofweekfunction): Returns the start of the week containing the date, shifted by an offset, if provided
...
table of values. See also: [**startofday()**](https://review.docs.microsoft.com/kusto/query/startofdayfunction), [**startofweek()**](https://review.docs.microsoft.com/kusto/query/startofweekfunction), [**startofyear()**](https://review.docs.microsoft.com/kusto/query/startofyearfunction)), [**startofmonth()**](https://review.docs.microsoft.com/kusto/query/startofmonthfunction), [**endofday()**](https://review.docs.microsoft.com/kusto/query/endofdayfunction), [**endofweek()**](https://review.docs.microsoft.com/kusto/query/endofweekfunction), [**endofmonth()**](https://review.docs.microsoft.com/kusto/query/endofmonthfunction), and [**endofyear()**](https://review.docs.microsoft.com/kusto/query/endofyearfunction).

### between()

[**between()**](https://review.docs.microsoft.com/kusto/query/betweenoperator):
...
[**parse**](https://review.docs.microsoft.com/kusto/query/parseoperator): Evaluates a string expression and parses its value into one or more calculated columns. There are three ways to parse: simple (the default), regex, and relaxed.
...
For more information, review the full list of [series functions](https://review.docs.microsoft.com/kusto/query/scalarfunctions#series-processing-functions).
...
[**top-nested**](https://review.docs.microsoft.com/kusto/query/topnestedoperator): Produces hierarchical top results, where each level is a drill-down based on previous level values.
...
[**pivot() plugin**](https://review.docs.microsoft.com/kusto/query/pivotplugin): Rotates a table by turning the unique values from one column in the input table into multiple columns in the output table. The operator performs aggregations where they are required on any remaining column values in the final output.
...
[**dcount_hll()**](https://review.docs.microsoft.com/kusto/query/dcount-hllfunction):
Calculates the **dcount** from HyperLogLog results (generated by [**hll**](https://review.docs.microsoft.com/kusto/query/hll-aggfunction) or [**hll_merge**](https://review.docs.microsoft.com/kusto/query/hll-merge-aggfunction).
...
[**arg_max()**](https://review.docs.microsoft.com/kusto/query/arg-max-aggfunction):
...
[**makeset()**](https://review.docs.microsoft.com/kusto/query/makeset-aggfunction): Returns a dynamic (JSON) array of the set of distinct values that an expression takes in the group.
...
[**mvexpand**](https://review.docs.microsoft.com/kusto/query/mvexpandoperator):
...
[**percentiles()**](https://review.docs.microsoft.com/kusto/query/percentiles-aggfunction): Returns an estimate for the specified [**nearest-rank percentile**](https://review.docs.microsoft.com/kusto/query/percentiles-aggfunction) of the population defined by an expression. The accuracy depends on the density of population in the region of the percentile. Can be used only in the context of aggregation inside [**summarize**](https://review.docs.microsoft.com/kusto/query/summarizeoperator).
...
[**let**](https://review.docs.microsoft.com/kusto/query/letstatement): Improves modularity and reuse. The **let** statement allows you to break a potentially complex expression into multiple parts, each bound to a name, and compose those parts together. A **let** statement can also be used to create user-defined functions and views (expressions over tables whose results look like a new table). Expressions bound by a **let** statement can be of scalar type, of tabular type, or user-defined function (lambdas).
...
[**join**](https://review.docs.microsoft.com/kusto/query/joinoperator): Merge the rows of two tables to form a new table by matching values of the specified column(s) from each table. Kusto supports a full range of join types: **fullouter**, **inner**, **innerunique**, **leftanti**, **leftantisemi**, **leftouter**, **leftsemi**, **rightanti**, **rightantisemi**, **rightouter**, **rightsemi**.
...
[**serialize**](https://review.docs.microsoft.com/kusto/query/serializeoperator): Serializes the row set so you can use functions that require serialized data, like **row_number()**.
...
[**activity_engagement plugin**](https://review.docs.microsoft.com/kusto/query/activity-engagement-plugin): Calculates activity engagement ratio based on ID column over a sliding timeline window. **activity_engagement plugin** can be used for calculating DAU, WAU, and MAU (daily, weekly, and monthly active users).
...
[**activity_metrics plugin**](https://review.docs.microsoft.com/kusto/query/activity-metrics-plugin):
...
[**session_count plugin**](https://review.docs.microsoft.com/kusto/query/session-count-plugin): Calculates the count of sessions based on ID column over a timeline.
...
[**funnel_sequence plugin**](https://review.docs.microsoft.com/kusto/query/funnel-sequence-plugin):
...
[**funnel_sequence_completion plugin**](https://review.docs.microsoft.com/kusto/query/funnel-sequence-completion-plugin): Calculates the funnel of completed sequence steps within different time periods.
...
This section covers [**functions**](https://review.docs.microsoft.com/en-us/kusto/query/functions): reusable queries that are stored on the server. Functions can be invoked by queries and other functions (recursive functions are not supported).
