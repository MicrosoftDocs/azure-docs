...
Now connect to the event grid from Azure Data Explorer, so that data flowing into the blob container is streamed to the test table. To learn about how to add the data connection programmatically, see [Add an Event Grid data connection](data-connection-csharp.md#add-an-event-grid-data-connection) 
