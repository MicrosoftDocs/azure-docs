...
Now you connect to the event hub from Azure Data Explorer. When this connection is in place, data that flows into the event hub streams to the test table you created earlier in this article. Check [Add an Event Hub data connection](data-connection-csharp#add-an-event-hub-data-connection) for adding the data connection programmatically.
