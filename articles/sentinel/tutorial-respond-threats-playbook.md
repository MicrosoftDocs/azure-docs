...
1. When the playbook is invoked by an automation rule passing it an incident, the playbook opens a ticket in [ServiceNow](/connectors/service-now/) or any other IT ticketing system.
...
Now you can define what happens when you invoke the playbook. You can add actions, logical conditions, loops, or switch case conditions, all by selecting **New step**. This opens a new frame in the designer, where you can choose a system or an application to interact with. Enter the name of the system or application in the search bar at the top of the frame, and then choose from the available results. 
