...
The **Templates ready to be used** table on the **Configure** tab shows the [analytics rule templates](../detect-threats-built-in.md) from the Microsoft Sentinel solution for SAP applications that have not yet been implemented as active rules. You might need to create these rules to achieve compliance. For example:
...
- Note the count and graph lines generated by each rule in the **Incidents** and **Alerts** colums. Identical counts suggest that [alert grouping is disabled](../detect-threats-custom.md#alert-grouping).

- Note the **Incidents** and **Source** column values to understand whether the rule is [set to create incidents](../detect-threats-custom.md#configure-the-incident-creation-settings) .

- Note whether the **Recommended configuration** for a rule is *As alert only*. If so, consider [turning off the incident creation setting](../detect-threats-custom.md#configure-the-incident-creation-settings) in the rule.
...
Zoom in to the hive graph to make the text big enough to read clearly, and zoom out to see all the groupings together. Drag the whole graph to see different parts of it. For example:
