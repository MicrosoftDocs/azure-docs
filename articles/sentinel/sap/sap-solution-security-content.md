...
| **(Preview) SAP - AS JAVA - Login from unexpected network** | Identifies logons from an unexpected network. | Logon to the backend system from an IP address which is not assigned to one of the networks in the `SAP - Networks` watchlist. <br><br>**Data sources**: SAPcon - SAPJAVAFilesLog | Lateral Movement- Valid Accounts |
