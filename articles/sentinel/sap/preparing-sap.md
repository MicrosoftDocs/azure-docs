...
- **To include log retrieval only**, we recommend creating this role by loading the role authorizations from the [**MSFTSEN_SENTINEL_CONNECTOR**](https://aka.ms/SAP_Sentinel_Connector_Role) file, which incudes all the basic permissions for the data connector to operate.
