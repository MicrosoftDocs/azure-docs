...
Microsoft and other organizations author Azure Sentinel data connectors. This article lists the Azure Sentinel data connectors that Partner organizations author, in alphabetical order. For information about Microsoft-authored and service-to-service data connectors, see [Service to service integration](connect-data-sources.md#service-to-service-integration).
...
The Agari Phishing Defense and Brand Protection connector easily connects Brand Protection and Phishing Defense solution logs to Azure Sentinel. For more details, visit the [Agari Developers Site](https://developers.agari.com/agari-platform).

The Agari data connector ingests logs into Azure Sentinel using [Azure Functions and the REST API](connect-data-sources.md#data-connection-methods).

For more information about connecting to Azure Sentinel, see [Connect Agari Phishing Defense and Brand Protection to Azure Sentinel](connect-agari-phishing-defense.md).
...
The AI Vectra Detect data connector lets you easily bring your AI Vectra Detect data into Azure Sentinel. With the AI Vectra workbook, you can start investigating network attacks directly from Sentinel. You can view critical hosts, accounts, campaigns, and detections, and monitor Vectra's system health and audit logs.
...
Alcide kAudit connector lets you automatically export your Kubernetes cluster audit logs into Azure Sentinel in real time. The kAudit connector provides enhanced visibility and observability into your Kubernetes audit logs. Alcide kAudit gives you robust security and monitoring capabilities for forensics purposes.

The Alcide data connector ingests logs into Azure Sentinel using [Azure Functions and REST API](connect-data-sources.md#data-connection-methods).
...
The AWS data connector lets you import AWS CloudTrail management events into Azure Sentinel. To enable this connector, see [Connect Azure Sentinel to AWS CloudTrail](connect-aws.md).

The AWS data connector ingests data into Azure Sentinel over the **REST API**.

**Supported by:** Microsoft

## Apache HTTP Server (Preview)

The Apache HTTP Server data connector lets you ingest Apache HTTP Server events into Azure Sentinel. For more information, see the [Apache Logs documentation](https://httpd.apache.org/docs/2.4/logs.html).

The Apache HTTP Server data connector ingests data into Azure Sentinel using the **Log Analytics agent**. The connector also uses a log parser based on a Kusto function.

**Supported by:** Microsoft

## Apache Tomcat (Preview)

The Apache Tomcat data connector lets you ingest [Apache Tomcat](http://tomcat.apache.org/) events into Azure Sentinel. For more information, see the [Apache Tomcat documentation](http://tomcat.apache.org/tomcat-10.0-doc/logging.html).

The Apache Tomcat data connector ingests data into Azure Sentinel using the **Log Analytics agent**. The connector also uses a log parser based on a Kusto function.

**Supported by:** Microsoft

## Aruba ClearPass (Preview)

The Aruba ClearPass connector easily connects Aruba ClearPass Audit, Session, System, and Insight logs to Azure Sentinel. For more information on configuring the Aruba ClearPass solution to forward Syslog, see [Adding a Syslog Export Filter](https://www.arubanetworks.com/techdocs/ClearPass/6.7/PolicyManager/Content/CPPM_UserGuide/Admin/syslogExportFilters_add_syslog_filter_general.htm) .
...
The [Atlassian Confluence](https://www.atlassian.com/software/confluence) Audit data connector lets you ingest Confluence Audit Records. The connector can get events to examine potential security risks, analyze your team's collaboration, diagnose configuration problems, and more. For more information, see the [View the audit log](https://support.atlassian.com/confluence-cloud/docs/view-the-audit-log/).

The Atlassian Confluence data connector ingests data into Azure Sentinel using **Azure Functions and the REST API**.

**Supported by:** Microsoft

## Atlassian Jira Audit (Preview)

The Atlassian Jira Audit data connector lets you ingest Jira Audit Records events into Azure Sentinel. The connector can get events to examine potential security risks, analyze your team's collaboration, diagnose configuration problems, and more. For more information, see the [Audit records](https://developer.atlassian.com/cloud/jira/platform/rest/v3/api-group-audit-records/).

The Atlassian Jira Audit data connector ingests logs into Azure Sentinel using **Azure Functions and the REST API**.
...
The Barracuda WAF connector lets you easily connect your Barracuda WAF logs to Azure Sentinel. For more information, see [Configure the Barracuda Web Application Firewall](https://aka.ms/asi-barracuda-connector).

The Barracuda WAF data connector ingests logs into Azure Sentinel using native integration between Barracuda and the **Log Analytics agent**.
...
The BETTER MTD Connector lets enterprises connect their Better MTD instances to Azure Sentinel. This connector provides insight into an organization's mobile devices and current mobile security posture, improving overall SecOps capabilities. For more information, see the BETTER Mobile [Azure Sentinel setup documentation](https://mtd-docs.bmobi.net/integrations/how-to-setup-azure-sentinel-integration#mtd-integration-configuration).

The BETTER Mobile data connector ingests data into Azure Sentinel over the **REST API**.
...
The Beyond Security beSECURE connector easily connects your Beyond Security beSECURE scan events, scan results, and audit trail to Azure Sentinel. For more information, see the beSECURE [Overview](https://beyondsecurity.com/solutions/besecure.html) page.

The Beyond Security data connector ingests data into Azure Sentinel over the **REST API**.
...
The Check Point firewalls connector easily connects your Check Point logs to Azure Sentinel. To configure your Check Point product to export logs, see [Log Exporter - Check Point Log Export](https://supportcenter.checkpoint.com/supportcenter/portal?eventSubmit_doGoviewsolutiondetails=&solutionid=sk122323).
...
The Cisco ASA firewall connector easily connects your Cisco ASA logs to Azure Sentinel. To set up the connection, follow the instructions in the [Cisco ASA Series CLI Configuration Guide](https://aka.ms/asi-syslog-cisco-forwarding).

The Cisco ASA data connector ingests logs into Azure Sentinel using [CEF](connect-common-event-format.md) over Syslog.
...
The Cisco Firepower eStreamer connector easily connects your eStreamer logs to Azure Sentinel. Cisco Firepower eStreamer is a client-server API designed for the Cisco Firepower NGFW solution. For more information, see the [eStreamer eNcore for Sentinel Operations Guide](https://www.cisco.com/c/en/us/td/docs/security/firepower/670/api/eStreamer_enCore/eStreamereNcoreSentinelOperationsGuide_409.html).

The Cisco Firepower eStreamer data connector ingests logs into Azure Sentinel using [CEF](connect-common-event-format.md) over Syslog.
...
The Citrix WAF data connector easily connects your Citrix WAF logs to Azure Sentinel. Citrix WAF mitigates threats against your public-facing assets, including websites, apps, and APIs.
...
The Citrix WAF data connector ingests logs into Azure Sentinel using [CEF](connect-common-event-format.md) over Syslog.
...
The Cognni data connector ingests data into Azure Sentinel over the **REST API**.
...
The Cyberpion data connector ingests logs into Azure Sentinel over the **REST API**.

**Supported by:** Cyberpion

## ESET Enterprise Inspector (Preview)

The ESET Enterprise Inspector data connector lets you ingest detections from ESET Enterprise Inspector using the REST API provided in ESET Enterprise Inspector version 1.4 and later. For more information, see [REST API](https://help.eset.com/eei/1.5/en-US/api.html) in the ESET Enterprise Inspector documentation.

The ESET Enterprise Inspector data connector ingests data into Azure Sentinel using **Azure Functions and the REST API**.

**Supported by:** [ESET](https://support.eset.com/en)

## ESET Security Management Center (SMC) (Preview)

The ESET SMC data connector lets Azure Sentinel ingest ESET SMC threat events, audit logs, firewall events, and website filters. For more information, see [Syslog server](https://help.eset.com/esmc_admin/70/en-US/admin_server_settings_syslog.html) in the ESET SMC documentation.

The ESET Security Management Center data connector ingests logs into Azure Sentinel using the **Log Analytics agent**.

**Supported by:** [ESET](https://support.eset.com/en)

## Exabeam Advanced Analytics (Preview)

The Exabeam Advanced Analytics data connector lets you ingest Exabeam Advanced Analytics events like System Health, Notable Sessions/Anomalies, Advanced Analytics, and Job Status into Azure Sentinel. To send logs from Exabeam Advanced Analytics via Syslog, follow the instructions at [Configure Advanced Analytics system activity notifications](https://docs.exabeam.com/en/advanced-analytics/i54/advanced-analytics-administration-guide/113254-configure-advanced-analytics.html#UUID-7ce5ff9d-56aa-93f0-65de-c5255b682a08).

The Exabeam data connector ingests data into Azure Sentinel over [Syslog](connect-syslog.md). The connector also uses a log parser based on a Kusto function.

**Supported by:** Microsoft

## ExtraHop Reveal(x)

The ExtraHop Reveal(x) data connector lets you easily connect your Reveal(x) system to Azure Sentinel. Azure Sentinel integration requires the ExtraHop Detection SIEM Connector. To install the SIEM Connector on your Reveal(x) system, follow the instructions at [ExtraHop Detection SIEM Connector](https://aka.ms/asi-syslog-extrahop-forwarding).

The ExtraHop data connector ingests data into Azure Sentinel using [CEF](connect-common-event-format.md) over Syslog.
...
The F5 BIG-IP data connector ingests logs into Azure Sentinel over the **REST API**.
...
The F5 firewall data connector ingests data into Azure Sentinel using [CEF](connect-common-event-format.md) over Syslog.
...
The Forcepoint CASB data connector lets you automatically export CASB logs and events into Azure Sentinel in real time. For more information, see [Forcepoint CASB and Azure Sentinel](https://forcepoint.github.io/docs/casb_and_azure_sentinel/).

The Forcepoint CASB data connector ingests data into Azure Sentinel using [CEF](connect-common-event-format.md) over Syslog.
...
The Forcepoint DLP data connector lets you automatically export DLP incident data from Forcepoint DLP into Azure Sentinel in real time. For more information, see [Forcepoint Data Loss Prevention and Azure Sentinel](https://forcepoint.github.io/docs/dlp_and_azure_sentinel/).

The Forcepoint DLP data connector ingests data into Azure Sentinel over the **REST API**.
...
The Forcepoint NGFW data connector lets you automatically export user-defined Forcepoint NGFW logs into Azure Sentinel in real time. For more information, see [Forcepoint Next-Gen Firewall and Azure Sentinel](https://forcepoint.github.io/docs/ngfw_and_azure_sentinel/).

The Forcepoint NGFW data connector ingests logs into Azure Sentinel using [CEF](connect-common-event-format.md) over Syslog.
...
The Fortinet data connector ingests logs into Azure Sentinel using [CEF](connect-common-event-format.md) over Syslog.
...
The Google Workspace data connector lets you ingest Google Workspace Activity events into Azure Sentinel through the REST API. The ability to ingest events helps you:
...
The Google data connector ingests data into Azure Sentinel using **Azure Functions and the REST API**.
...
The Imperva connector lets you quickly connect your Imperva WAF Gateway alerts to Azure Sentinel. For more information, see [Steps for Enabling Imperva WAF Gateway Alert Logging to Azure Sentinel](https://community.imperva.com/blogs/craig-burlingame1/2020/11/13/steps-for-enabling-imperva-waf-gateway-alert).

The Imperva data connector ingests data into Azure Sentinel using [CEF](connect-common-event-format.md) over Syslog.
...
The Juniper data connector ingests logs into Azure Sentinel over [Syslog](connect-syslog.md). The connector also uses a log parser based on a Kusto function.
...
The Morphisec data connector ingests data into Azure Sentinel [CEF](connect-common-event-format.md) over Syslog. The connector also uses a log parser based on a Kusto function.

**Supported by:** Morphisec

## Netskope (Preview)

The Netskope Cloud Security Platform connector lets you ingest Netskope logs and events into Azure Sentinel. For more information, see [The Netskope Cloud Security Platform](https://www.netskope.com/platform).

The Netskope data connector ingests data into Azure Sentinel using **Azure Functions and the REST API**.

**Supported by:** Microsoft

## NGINX HTTP Server (Preview)

The NGINX HTTP Server data connector lets you ingest NGINX HTTP Server events into Azure Sentinel. For more information, see [Module ngx_http_log_module](https://nginx.org/en/docs/http/ngx_http_log_module.html).

The NGINX data connector ingests data into Azure Sentinel using the **Log Analytics agent**. The connector also uses a log parser based on a Kusto function.
...
The NXLog DNS data connector ingests data into Azure Sentinel over the **REST API**.
...
The Onapsis data connector lets you export the alarms triggered in the Onapsis Platform into Azure Sentinel in real time.

The Onapsis data connector ingests data into Azure Sentinel [CEF](connect-common-event-format.md) over Syslog.
...
The OracleWebLogicServer data connector lets you ingest OracleWebLogicServer events into Azure Sentinel. For more information, see the [Oracle WebLogic Server documentation](https://docs.oracle.com/en/middleware/standalone/weblogic-server/14.1.1.0/index.html).

The Oracle WebLogic Server data connector ingests data into Azure Sentinel using the **Log Analytics agent**. The connector also uses a log parser based on a Kusto function.

**Supported by:** Microsoft

## Orca Security Alert (Preview)

The Orca Security Alerts connector lets you easily export Alerts logs to Azure Sentinel. For more information, see [Azure Sentinel integration](https://orcasecurity.zendesk.com/hc/en-us/articles/360043941992-Azure-Sentinel-configuration).

The Orca data connector ingests logs into Azure Sentinel over the **REST API**.
...
The OSSEC data connector lets you ingest OSSEC events into Azure Sentinel. For more information, see the [OSSEC documentation](https://www.ossec.net/docs/). To configure OSSEC sending alerts via Syslog, follow the instructions at [Sending alerts via syslog](https://www.ossec.net/docs/docs/manual/output/syslog-output.html).

OSSEC data connector ingests data into Azure Sentinel [CEF](connect-common-event-format.md) over Syslog.
...
The Perimeter 81 data connector ingests logs into Azure Sentinel over the **REST API**.
...
The Proofpoint TAP connector lets you ingest Proofpoint TAP logs and events into Azure Sentinel. The connector provides visibility into Message and Click events in Azure Sentinel.

The Proofpoint TAP data connector ingests data into Azure Sentinel using **Azure Functions and the REST API**.
...
The Qualys VM KB data connector lets you ingest the latest vulnerability data from the Qualys KB into Azure Sentinel. You can use this data to correlate and enrich vulnerability detections by the Qualys VM data connector.

The Qualys VM KB data connector ingests data into Azure Sentinel over [Syslog](connect-syslog.md). The connector also uses a log parser based on a Kusto function.

**Supported by:** Microsoft

## Qualys VM (Preview)

The Qualys VM data connector lets you ingest vulnerability host detection data into Azure Sentinel through the Qualys API. The connector provides visibility into host detection data from vulnerability scans.

The Qualys data connector ingests data into Azure Sentinel using **Azure Functions and the REST API**.
...
The Salesforce Service Cloud data connector lets you ingest information about Salesforce operational events into Azure Sentinel through the REST API. The connector lets you review organizational events on an accelerated basis. You can get event logs in hourly increments for recent activity. For more information and to get credentials, see the Salesforce [REST API Developer Guide](https://developer.salesforce.com/docs/atlas.en-us.api_rest.meta/api_rest/quickstart.htm).

The Salesforce data connector ingests data into Azure Sentinel over [Syslog](connect-syslog.md). The connector also uses a log parser based on a Kusto function.
...
The SentinelOne data connector lets you ingest SentinelOne events into Azure Sentinel through the REST API. Events include server objects like Threats, Agents, Applications, Activities, Policies, Groups, and more. The connector can get events to examine potential security risks, analyze your team's collaboration, diagnose configuration problems, and more.

SentinelOne's data connector ingests data into Azure Sentinel using **Azure Functions and the REST API**.

**Supported by:** Microsoft

## SonicWall Firewall (Preview)

The SonicWall Firewall data connector lets you easily connect firewall logs to Azure Sentinel. For more information, see [Log > Syslog](http://help.sonicwall.com/help/sw/eng/7020/26/2/3/content/Log_Syslog.120.2.htm).

The SonicWall data connector ingests logs into Azure Sentinel [CEF](connect-common-event-format.md) over Syslog.
...
The Sophos Cloud Optix data connector ingests logs into Azure Sentinel over the **REST API**.
...
Sophos' XG data connector ingests logs into Azure Sentinel over [Syslog](connect-syslog.md). The connector also uses a log parser based on a Kusto function.
...
The Squadra Technologies secRMM data connector allows you to push USB removable storage security event data into Azure Sentinel. For more information, see the [secRMM Azure Sentinel Administrator Guide](https://www.squadratechnologies.com/StaticContent/ProductDownload/secRMM/9.9.0.0/secRMMAzureSentinelAdministratorGuide.pdf).

The Squadra data connector ingests data into Azure Sentinel over the **REST API**.
...
The Squid Proxy data connector ingests logs into Azure Sentinel using the **Log Analytics agent**. The connector also uses a log parser based on a Kusto function.
...
The Symantec ICDx data connector ingests logs into Azure Sentinel over the **REST API**.
...
The Trend Micro TippingPoint data connector ingests data into Azure Sentinel [CEF](connect-common-event-format.md) over Syslog. The connector also uses a log parser based on a Kusto function.
...
The Trend Micro XDR data connector lets you ingest workbench alerts from the Trend Micro XDR API into Azure Sentinel. To create an account and an API authentication token, follow the instructions at [Obtaining API Keys for Third-Party Access](https://docs.trendmicro.com/en-us/enterprise/trend-micro-xdr-help/ObtainingAPIKeys).

The Trend Micro XDR data connector ingests data into Azure Sentinel using **Azure Functions and the REST API**.

**Supported by:** [Trend Micro](https://success.trendmicro.com/technical-support)

## VMware Carbon Black Endpoint Standard (Preview)

The VMware Carbon Black Endpoint Standard data connector lets you ingest Carbon Black Endpoint Standard data into Azure Sentinel. To create an API key, follow the instructions at [Creating an API Key](https://developer.carbonblack.com/reference/carbon-black-cloud/authentication/#creating-an-api-key).

The VMware Carbon Black data connector ingests data into Azure Sentinel using **Azure Functions and the REST API**.
...
The WatchGuard Firebox data connector lets you ingest firewall logs into Azure Sentinel. For more information, see [Microsoft Azure Sentinel Integration Guide](https://www.watchguard.com/help/docs/help-center/en-US/Content/Integration-Guides/General/Microsoft%20Azure%20Sentinel.html).

The WatchGuard data connector ingests logs into Azure Sentinel over [Syslog](connect-syslog.md). The connector also uses a log parser based on a Kusto function.
...
The Zoom Reports data connector lets you ingest Zoom Reports events into Azure Sentinel. The connector can get events to examine potential security risks, analyze your team's collaboration, diagnose configuration problems, and more. To get credentials, follow the instructions at [JWT With Zoom](https://marketplace.zoom.us/docs/guides/auth/jwt).

The Zoom data connector ingests data into Azure Sentinel over the **REST API**.

**Supported by:** Microsoft

## Zscaler 

The Zscaler data connector connects Zscaler Internet Access (ZIA) logs to Azure Sentinel. Using Zscaler on Azure Sentinel gives you insights into your organization's internet usage and enhances your security operations capabilities.​ For more information, see the Zscaler [Zscaler and Microsoft Azure Sentinel Deployment Guide](https://aka.ms/ZscalerCEFInstructions).

The Zscaler ZIA data connector ingests logs into Azure Sentinel [CEF](connect-common-event-format.md) over Syslog.
...
The ZPA data connector lets you ingest Zscaler Private Access events into Azure Sentinel. For more information, see the [Zscaler Private Access documentation](https://help.zscaler.com/zpa).

Zscaler's ZPA data connector ingests data into Azure Sentinel over the [Log Analytics Agent](connect-data-sources.md#data-connection-methods). The connector also uses a log parser based on a Kusto function.
