...
The ASIM DHCP schema represents DHCP server activity, including serving requests for DHCP IP address leases from client systems and updating a DNS server with the leases granted. Therefore the most important fields in a DHCP event are the [SrcIpAddr](#srcipaddr) and [SrcHostname](#srchostname), which the DHCP server binds by granting the lease. Those are aliased by IpAddr and Hostname respectively. The [SrcMacAddr](#srcmacaddr) is also important as it represent the client machine in case that an IP address was not leased.  
...
Lastly, a lease has a duration which is stored in the [DhcpLeaseDuration](#dhcpleaseduration) field.  

## Schema details

ASIM is aligned with the the [Open Source Security Events Metadata (OSSEM)](https://github.com/OTRF/OSSEM) project. Note however that OSSEM does not have a DHCP schema comparable to the ASIM DHCP schema.
...
| **EventType** | Mandatory | Enumerated | Indicate the operation reported by the record. <br><Br> Possible values are Assigned, Renewed, Released and DNS Update. <br><br>Example: `Assigned`| 
...
| <a name=eventresultdetails></a>**EventResultDetails** | Alias | | Reason or details for the result reported in the **_EventResult_** field. <br><Br> Possible values are Exhausted, Quarantined, Denied. <br><br>Example: `Exhausted` |
...
| <a name=dvcaction></a>**DvcAction** | Optional | Enumerated | The action taken by the DHCP server. Possible values are `Allowed`, `Deny` and `Quarantined`. <br><br>Example: `Deny` |
...
The fields below are specific to DHCP events. That said, many of them do have similarities in other schemas and therefore follow the same naming convention.
...
| <a name=srcipaddr></a>**RequestedIpAddr** | Optional | IP Address | The IP address requested by the DHCP client, when available.<br><br>Example: `192.168.12.3` |
...
| **Username** | Alias | | Alias for SrcUsernae |
...
| <a name="srcmacaddr"></a>**SrcMacAddr** | Mandatory | Mac Address | The MAC address of the client requesting a DHCP lease. Note that the Windows DHCP server logs MAC address in a non-standard way, omitting the colons, which should be inserted by the parser.<br><br>Example: `06:10:9f:eb:8f:14` |
