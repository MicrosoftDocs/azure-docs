---
title: Microsoft Sentinel User and Entity Behavior Analytics (UEBA) reference
...
| **InvestigationPriority** | int      | The anomaly score, between 0-10 (0=benign, 10=highly anomalous). This score quantifies the degree of deviation from the expected behavior. Higher scores indicate greater deviation from the baseline and are more likely indicate true anomalies. Lower scores might still be anomalous, but are less likely to be significant or actionable. |
...
| **Is dormant account**<br>*(IsDormantAccount)* | The account hasn't been used for the past 180 days. | True, False |
...
| Enrichment name | Description | Sample value |
| --- | --- | --- |
| **Browser**<br>*(Browser)* | The browser used in the action. | Microsoft Edge, Chrome |
...
| **Action uncommonly performed by user**<br>*(ActionUncommonlyPerformedByUser)* | 10 | The action isn't commonly performed by the user. | True, False |
| **Action uncommonly performed among peers**<br>*(ActionUncommonlyPerformedAmongPeers)* | 180 | The action isn't commonly performed among user's peers. | True, False |
...
| **Action uncommonly performed in tenant**<br>*(ActionUncommonlyPerformedInTenant)* | 180 | The action isn't commonly performed in the organization. | True, False |
...
| **App uncommonly used by user**<br>*(AppUncommonlyUsedByUser)* | 10 | The app isn't commonly used by the user. | True, False |
| **App uncommonly used among peers**<br>*(AppUncommonlyUsedAmongPeers)* | 180 | The app isn't commonly used among user's peers. | True, False |
...
| **App uncommonly used in tenant**<br>*(AppUncommonlyUsedInTenant)* | 180 | The app isn't commonly used in the organization. | True, False |
...
| **Browser uncommonly used by user**<br>*(BrowserUncommonlyUsedByUser)* | 10 | The browser isn't commonly used by the user. | True, False |
| **Browser uncommonly used among peers**<br>*(BrowserUncommonlyUsedAmongPeers)* | 30 | The browser isn't commonly used among user's peers. | True, False |
...
| **Browser uncommonly used in tenant**<br>*(BrowserUncommonlyUsedInTenant)* | 30 | The browser isn't commonly used in the organization. | True, False |
...
| **Country uncommonly connected from among peers**<br>*(CountryUncommonlyConnectedFromAmongPeers)* | 90 | The geo location, as resolved from the IP address, isn't commonly connected from among user's peers. | True, False |
...
| **Country uncommonly connected from in tenant**<br>*(CountryUncommonlyConnectedFromInTenant)* | 90 | The geo location, as resolved from the IP address, isn't commonly connected from in the organization. | True, False |
...
| **Device uncommonly used by user**<br>*(DeviceUncommonlyUsedByUser)* | 10 | The device isn't commonly used by the user. | True, False |
| **Device uncommonly used among peers**<br>*(DeviceUncommonlyUsedAmongPeers)* | 180 | The device isn't commonly used among user's peers. | True, False |
...
| **Device uncommonly used in tenant**<br>*(DeviceUncommonlyUsedInTenant)* | 180 | The device isn't commonly used in the organization. | True, False |
...
| **Device family uncommonly used in tenant**<br>*(DeviceFamilyUncommonlyUsedInTenant)* | 30 | The device family isn't commonly used in the organization. | True, False |
...
| **ISP uncommonly used by user**<br>*(ISPUncommonlyUsedByUser)* | 10 | The ISP isn't commonly used by the user. | True, False |
| **ISP uncommonly used among peers**<br>*(ISPUncommonlyUsedAmongPeers)* | 30 | The ISP isn't commonly used among user's peers. | True, False |
...
| **ISP uncommonly used in tenant**<br>*(ISPUncommonlyUsedInTenant)* | 30 | The ISP isn't commonly used in the organization. | True, False |
...
| **Resource uncommonly accessed by user**<br>*(ResourceUncommonlyAccessedByUser)* | 10 | The resource isn't commonly accessed by the user. | True, False |
...
| **Resource uncommonly accessed in tenant**<br>*(ResourceUncommonlyAccessedInTenant)* | 180 | The resource isn't commonly accessed in the organization. | True, False |
...
- Every 14 days, Microsoft Sentinel resynchronizes with your entire Microsoft Entra ID (and your on-premises Active Directory, if applicable) to ensure that stale records are fully updated.

- Besides these regular full synchronizations, whenever changes are made to your user profiles, groups, and built-in roles in Microsoft Entra ID, the affected user records are reingested and updated in the *IdentityInfo* table within 15-30 minutes. This ingestion is billed at regular rates. For example:
    ...
    - Group A has 100 users in it. 5 users are added to the group or removed from the group. In this case, those five user records are reingested, and their *GroupMembership* fields updated.

    - Group A has 100 users in it. Ten users are added to Group A. Also, groups A1 and A2, each with 10 users, are added to Group A. In this case, 30 user records are reingested and their *GroupMembership* fields updated. This happens because group membership is transitive, so changes to groups affect all their subgroups.

    - Group B (with 50 users) is renamed to Group BeGood. In this case, 50 user records are reingested and their *GroupMembership* fields updated. If there are subgroups in that group, the same happens for all their members' records.
...
- The *GroupMembership* field supports listing up to 500 groups per user, including subgroups. If a user is a member of more than 500 groups, only the first 500 are synchronized with the *IdentityInfo* table. The groups aren't evaluated in any particular order, though, so at each new synchronization (every 14 days), it's possible that a different set of groups will be updated to the user record.

- When a user is deleted, that user's record isn't immediately deleted from the *IdentityInfo* table. The reason for this is that one of this table's purposes is to audit changes to user records. Therefore, we want this table to have a record of a user being deleted, which can only happen if the user record in the *IdentityInfo* table still exists, even though the actual user (say, in Entra ID) is deleted.
...
- When a group is deleted, or if a group with more than 100 members has its name changed, that group's member user records aren't updated. If a different change causes one of those users' records to be updated, the updated group information will be included at that point.

#### Other versions of the IdentityInfo table

There are multiple versions of the *IdentityInfo* table:
