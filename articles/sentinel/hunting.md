...
- Having discovered or created a hunting query that provides high value insights into possible attacks, you can create custom detection rules based on that query and surface those insights as alerts to your security incident responders. View the query's results in Log Analytics (see above), then click the **New alert rule** button at the top of the pane and select **Create Azure Sentinel alert**. The **Analytics rule wizard** will open. Complete the required steps as explained in [Create custom analytics rules to detect threats](tutorial-detect-threats-custom.md).
