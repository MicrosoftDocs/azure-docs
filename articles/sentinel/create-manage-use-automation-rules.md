...
1. Select an operator from the next drop-down box to the right.
    ...
    | - **Owner** | - Changed. If an incident's owner is updated via API, you must include the [*userPrincipalName* or *ObjectID*](/rest/api/securityinsights/automation-rules/get?view=rest-securityinsights-2024-09-01&tabs=HTTP&preserve-view=true)* for the change to be detected by automation rules. |
