...
Enabling this solution will help your security team to achieve the following goals:
...
The playbooks included in the solution at launch are:
 ...
 1. MDTI Intel Reputation
    ...
    This playbook uses the Microsoft Defender Threat Intelligence Reputation Data to automatically enrich incidents generated by Microsoft Sentinel. Reputation information helps analysts designate indicators as benign, suspicious, or malicious. Each reputation result is contained within a comment and will include detailed scoring information regarding why a given indicator is considered suspicious or malicious with links back to the MDTI platform for more information. 
 
1.	MDTI Data WebComponents 

    Leverage this playbook to enrich your incidents with MDTI WebComponents data hosted by the indicators found within the incident. These components allow a user to understand the makeup of a webpage or the technology and services driving a specific piece of infrastructure. Pivoting on unique components can find actors' infrastructure or other target sites compromised. Users can also determine if a website might be vulnerable to a specific attack or compromise based on the technologies that it is running. 
 
1.	MDTI Automated Triage 

    This playbook also uses Microsoft Defender Threat Intelligence Reputation data to automatically enrich incidents generated by Microsoft Sentinel. After indicators are scored the incident's severity is updated. For "suspicious", the incident  severity will be marked as "medium". If any indicator is labeled as "malicious", the incident will be tagged as "high" severity. Regardless of the reputation state, comments will be added to the incident outlining the reputation details with links to further information if applicable. 
  
Find out more information about this solution in the Content hub, or [here](https://azuremarketplace.microsoft.com/en-us/marketplace/apps/azuresentinel.azure-sentinel-solution-microsoft-defender-threat-intelligence) in the public Azure Marketplace.
