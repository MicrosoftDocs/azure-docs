...
This article explains how to override the default properties of alerts with content from the underlying events.
...
- Tailor an alert's name and description to its content. You can select alert properties or fields in your alert's query whose contents can be represented in the name or description of each instance of the alert. If the selected field has no value in a given instance, the alert details for that instance will revert to the defaults specified in the first page of the wizard.
