---
title: Tutorial - Use automation to triage incidents
description: In this tutorial, you use a combination of automation rules and playbooks to automate your incident triage, setting your analysts' priorities for addressing and investigating incidents further.
author: yelevin
ms.author: yelevin
ms.topic: tutorial
ms.date: 11/29/2022
...
# Tutorial: Use Microsoft Sentinel automation to triage your incidents

[Automation rules](automate-incident-handling-with-automation-rules.md) help you triage incidents in Microsoft Sentinel. You can use them to automatically assign incidents to the right personnel and create to-do lists of tasks for them, close noisy incidents or known [false positives](false-positives.md), change their severity, and add tags. They are also the mechanism by which you can run playbooks in response to incidents.

[Playbooks](automate-responses-with-playbooks.md) are collections of procedures, workflows built in [Azure Logic Apps](../logic-apps/logic-apps-overview.md), that can be run from Microsoft Sentinel in response to an alert or incident. A playbook can help automate and orchestrate your response, and can be set to run automatically when specific alerts or incidents are generated, by being attached to an analytics rule or an automation rule, respectively. It can also be run manually on-demand.

This tutorial shows you how to use a combination of automation rules and playbooks to automate your incident triage, reducing the noise in your incidents queue and helping your analysts prioritize the handling and investigation of incidents. When you complete this tutorial, you'll be able to:
...
To stop potentially compromised users from moving around your network and stealing information, you can create an automated, multifaceted response to incidents generated by rules that detect compromised users. You start by creating a playbook that takes the following actions:

### Create a playbook to stop a compromised user

This playbook will be called by an automation rule that passes information on an incident to the playbook.
...
1. The drop-down menu that appears under **Create** gives you three choices for creating playbooks. For this tutorial, we're going to select **Playbook with incident trigger**. The **Create playbook** wizard will appear.
...
1. In the **Basics** tab:
    ...
    1. Enter a name for your playbook under **Playbook name**. (Let's call it "MyPlaybook".)

    1. We'll leave the **Enable diagnostics logs in Log Analytics** and **Associate with integration service environment** check boxes alone in this exercise.

    1. Select **Next : Connections >**.

1. In the **Connections** tab:

    Leave this section as is, configuring the Logic Apps service to connect to Microsoft Sentinel using managed identity.

    :::image type="content" source="media/tutorial-respond-threats-playbook/create-playbook-LAC-connections.png" alt-text="Screenshot showing Connections tab of create playbook wizard.":::
...
1. In the **Review and create** tab:
    ...
    :::image type="content" source="media/tutorial-respond-threats-playbook/create-playbook-LAC-review-create.png" alt-text="Screenshot of review-and-create tab of playbook creation wizard.":::
...
1. Select **New step** in the logic app designer. In the **Choose an operation** window that opens, select the **All** tab.
1. In the **Search connectors and actions** field (above the **All** tab), enter **ServiceNow**.
1. In the **Actions** tab below, choose the **Create Record** action.
1. In the **Record Type** field, enter **Incident**.
