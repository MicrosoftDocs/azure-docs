...
# Get started with ML notebooks in Azure SentinelThis article walks through the Azure Sentinel **A Getting Started Guide For Azure Sentinel ML Notebooks** notebook. This notebook covers the basics of getting started with Azure Machine Learning (ML) notebooks in Azure Sentinel. The notebook focuses on setup and basic steps to query data. This article assumes you're running the notebook in Azure ML, but most of the guidance is applicable to other notebook environments, including local notebooks.> [!NOTE]> You can also see this content by running the notebook in the Azure Sentinel portal. To run or save the notebook, select **Notebooks** from the Azure Sentinel left navigation, and from the **Templates** tab, select **A Getting Started Guide For Azure Sentinel ML Notebooks**. Select **Save notebook** to save the notebook to your workspace. Select **Launch notebook** to run the notebook.> ## Jupyter notebooks
...
Most of the notebooks in the [Azure Sentinel GitHub repo](https://github.com/Azure/Azure-Sentinel-Notebooks) use Python. Python is a well-established language with many materials and libraries to use for data analysis and security investigation. This capability makes it ideal for Azure Sentinel notebooks.
...
> [!TIP]> If your notebook hangs or you want to start over, you can restart the kernel. Select the recycle symbol at upper left in the toolbar above the notebook. Restarting the kernel wipes all variables and other state, so you need to rerun any initialization and authentication cells after restarting.
> 
...
In a notebook, *Markdown* cells have text, including HTML, and static images. *Code* cells contain code. You can identify code cells by selecting them. After you select a code cell, you can run the code in the cell by selecting the **Play** icon to the left of the cell, or by pressing Shift+Enter.

Code and Markdown cells have different styles in different notebook environments, but it's usually easy to distinguish them.Always run notebook code cells in sequence. Skipping cells results in errors.Run the following code:```python# This is your first code cell. This cell contains basic Python code.# You can run a code cell by selecting it and clicking# the Run button to the left of the cell, or by pressing Shift+Enter.# Code output displays below the code.

print("Congratulations, you just ran this code cell")y = 2 + 2print("2 + 2 =", y)```

The code produces this output:```outputCongratulations, you just ran this code cell

2 + 2 = 4```Variables set within a notebook code cell persist between cells, so you can chain cells together.This example uses the value of `y` from the previous example:
...
Before you can use a package in a notebook, you need to:
...
> [!NOTE]
> A configuration warning like `Missing msticpyconfig.yaml` is expected, because you haven't configured anything yet.
...
The output provides initialization status.
## Configure the notebook

After basic initialization, make sure you have configurations in place.

Some notebook components need configuration to connect to the Azure Sentinel workspace and to external services. For example, you need an API key to get VirusTotal threat intelligence data. The easiest way to manage this configuration data is to store it in a configuration file, *msticpyconfig.yaml*. The only alternative is to type in configuration details and keys each time you use a notebook.
...
- The Azure-Sentinel-Notebooks GitHub repo also contains an template *msticpyconfig.yaml* file with commented-out sections, which might help you find your way around the settings.
### Azure Sentinel configuration settings
...
To use IPStack as an alernative to GeoLite2, see [MSTICPy GeoIP Providers documentation](https://msticpy.readthedocs.io/en/latest/data_acquisition/GeoIPLookups.html).
...
You might see some warnings about missing sections, but not about the Azure Sentinel, TIProviders, or GeoIP Providers settings.

Select the **Close** button to hide the validation output.

If you need to make any changes as a result of the validation, remember to save your changes by selecting **Save Settings**.

### Load saved settings

You've saved your settings to the *msticpyconfig.yaml* file on disk. YAML files are easy to read with text editors. However, MSTICPy doesn't automatically reload these settings. To force MSTICPy to reload from the new configuration file, run the next code cell:```pythonimport msticpymsticpy.settings.refresh_config()```
...
Device authorization adds an additional factor to the authentication by generating a one-time device code that you supply as part of the authentication process.
...
1. Run the following code cell to generate and display a device code:
   ...
   Some Jupyter notebook interfaces have a clickable button that copies the code to the clipboard and then opens a browser window to the devicelogin URI. The default Azure ML notebook interface doesn't do this. You need to manually select and copy the code to the clipboard..
   
1. Go to [http://microsoft.com/devicelogin](http://microsoft.com/devicelogin) and paste in the code.      > [!NOTE]   > The devicelogin URL might be different for government or national Azure clouds.   1. Authenticate with your Azure credentials.
