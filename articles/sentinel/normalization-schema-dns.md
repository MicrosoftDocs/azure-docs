...
| ------ | ------------- |
| Microsoft DNS Server Collected using the DNS connector and the Azure Monitor agent | The connector doesn't provide the mandatory DnsQuery field for original event ID 264 (Response to a dynamic update). The data is available at the source, but not forwarded by the connector. |
