...
| **NetworkDirection** | Optional | Enumerated | The direction of the connection or session, into or out of the organization. Supported values include `Inbound`, `Outbound`, and `Listen`. `Listen` indicates that a device has started accepting network connections but isn't actually, necessarily, connected.|
