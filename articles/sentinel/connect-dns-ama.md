---
title: Gather Windows DNS Logs with the AMA Connector 
description: 
author: limwainstein
ms.topic: how-to
ms.date: 01/05/2022
...
# Collect and Filter Data from Windows DNS Servers with the AMA Connector

[!INCLUDE [reference-to-feature-availability](includes/reference-to-feature-availability.md)]

This article describes how to ingest logs from your Windows DNS servers, so you can dive deep into your logs, and protect your DNS servers from threats and attacks.

To get this visibility, you use the Windows DNS Events via AMA connector. The Azure Monitor Agent (AMA) and its DNS extension are installed on your Windows Server to ingest DNS analytic logs to your Microsoft Sentinel workspace. Learn about the [benefits of using the AMA connector](#ama-connector-benefits) for DNS log ingestion.
...
The *Domain Name System* (DNS) is a widely used protocol, which maps between host names and computer readable IP addresses. DNS servers are highly vulnerable to threats and attacks.

Because DNS was designed with many security gaps, it is highly targeted by malicious activity, making its logging an essential part of security monitoring. Some well-known threats that target DNS servers include: DDoS attacks, DNS DDoS Amplification, DNS hijacking, DNS tunneling, DNS poisoning, DNS spoofing, NXDOMAIN attack, phantom domain attacks, and more. 

### AMA connector benefits

While some mechanisms were introduced to improve the overall security of this protocol, DNS servers are still a highly targeted service. The analysis of DNS logs can help find information about possible threats and attacks. 

Organizations can monitor DNS logs to better understand network activity, and to identify suspicious behavior or attacks targeting resources within the network.

Here are some benefits of using AMA for DNS log ingestion:
...
- AMA provides centralized configuration using DCR, and also supports multiple DCRs. 
- AMA supports transformation from the incoming stream into other data tables.
- AMA allows filtering and advanced filtering, which happens on the server and before the data is ingested, which saves time and resources.

With the AMA connector's DNS extension, you can:  
...
1. The AMA connector uses the installed DNS extension to collect and parse the logs. 
    ...
    > [!NOTE]
       > This connector currently supports analytic event activities only.
 
1. The connector streams the events to the Microsoft Sentinel workspace to be further analyzed. 
1. You can now use advanced filtering to filter out specific events or information. This allows you to ingest only the valuable data to monitor, and reduces costs and bandwidth usage. 

### Normalization using ASIM

This connector is the first fully normalized connector, using [Advanced Security Information Model (ASIM) parsers](normalization.md). The connector streams all events into the DNS normalized table **ASimDnsActivityLogs** for events originated from the analytical logs. This gives the table the power of one unified “language”, shared across all DNS connectors to come. To use the source-agnostic parsers that unify all parsers and ensure your analysis runs across all configured sources, there are ready made KQL functions.  

The ASIM parser normalizes the native DNS table named `ASimDnsActivityLogs` to the ASIM DNS activity normalized schema. While the native table is ASIM compliant, the parser is needed to add capabilities, such as aliases, available only at query time. The views to use are `vimDnsNative` and `ASimDnsNative`. 

The [ASIM DNS schema](dns-normalization-schema.md) represents the DNS protocol activity, as logged in the Windows DNS server in the analytical logs. The schema is governed by official parameter lists and RFCs that define fields and values. You can find filtering parser parameters and more information on the web site linked above. 
...
- [API](#set-up-the-connector-with-the-ui). With this setup, you can create, manage, and delete multiple DCRs. 
...
- A Windows DNS Server analytical logs enabled. 
...
1. In the **Configuration** area, select **Create data collection rule**. You can create a single DCR per workspace. If you need to create multiple DCRs, [use the API](#set-up-the-connector-via-the-api).
...
1. Set up the data collection filters to exclude undesired events.
1. Select parameters to reduce the number of collected events. The fields are normalized according to the DNS normalized schema. See the list of available [filters and fields for filtering](#use-advanced-filters).
...
DNS server event logs can contain a huge number of events. You can use advanced filtering to filter out unneeded events before ingestion, saving valuable triage time and costs. The filters remove the unneeded data from the stream of events uploaded to your workspace.

Fields and filters use this logic:
...
|EventResultDetails   |• NOERROR<hr>• FORMERR<hr>• SERVFAIL<hr>• NXDOMAIN<hr>• NOTIMP<hr>• REFUSED<hr>• YXDOMAIN<hr>• YXRRSET<hr>• NXRRSET<hr>• NOTAUTH<hr>• NOTZONE<hr>• DSOTYPENI<hr>• BADVERS<hr>• BADSIG<hr>• BADKEY<hr>• BADTIME<hr>• BADALG<hr>• BADTRUNC<hr>• BADCOOKIE  |The operation's DNS result string as defined by the Internet Assigned Numbers Authority (IANA).  |
...
- Create a filter with the **DnsQueryTypeName** field set to **AAAA**.
...
This table describes and translates Windows DNS server fields into the normalized field names as they appear in the DNS schema. Learn more about the [DNS normalization schema](dns-normalization-schema#schema-details.md). 
...
## Next steps
In this document, you learned how to set up the Windows DNS events via AMA connector to ingest and filter your Windows DNS logs. To learn more about Microsoft Sentinel, see the following articles:
