...
Bring high fidelity indicators of compromise (IOC) generated by Microsoft Defender Threat Intelligence (MDTI) into your Microsoft Sentinel workspace. Both the MDTI data connector and the Premium MDTI (PMDTI) version ingest these IOCs with a simple one-click setup. Use the threat intelligence to monitor, alert and hunt in the same way you utilize other feeds.

Here's a table showing the additional TI to expect with PMDTI. Acquire a license to access the Premium MDTI data connector feed capabilities.
