...
This article explains how to use [playbooks](tutorial-respond-threats-playbook.md) to create, and optionally perform, [incident tasks](incident-tasks.md) to manage complex analyst workflow processes in Microsoft Sentinel.
...
This section provides a sample procedure for adding a playbook action that adds a task to the incident, resetting a compromised user's password. It also adds another playbook action to send a signal to Microsoft Entra ID Protection (AADIP) to actually reset the password, and a final playbook action to mark the task in the incident complete.
...
In this example, we're going to add a playbook action that researches an IP address that appears in an incident. If the results of this research are that the IP address is malicious, the playbook creates a task for the analyst to disable the user using that IP address. If the IP address isn't a known malicious address, the playbook creates a different task, for the analyst to contact the user to verify the activity.
...
- [Create incident tasks in Microsoft Sentinel using automation rules](../create-tasks-automation-rule.md). For SOC managers, senior analysts, and automation engineers:
...
- [Work with incident tasks in Microsoft Sentinel](../work-with-tasks.md). For SOC analysts:
