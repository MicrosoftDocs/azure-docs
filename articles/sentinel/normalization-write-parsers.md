...
> [!NOTE]
> When using the ASIM source-agnostic parsers in the **Logs** page, the time range selector is set to `custom`. You can still set the time range yourself. Alternatively, specify the time range using parser parameters.
>
> Alternately, use the parameter-less parsers (which start with `\_ASim\_` for built-in parsers and `ASim` for workspace deployed parsers). Those parsers do not set the time-range picker to `custom` by default.
>
