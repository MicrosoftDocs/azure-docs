...
| EventEndtime<br>EventEndTime | Date/Time | See “data types” | The time in which the event ended | Event |
...
| EventProductVer<br>EventProductVersion | string | 9.0 |  The version of the product generating the event. | Event |
...
| ResultReasonType<br>EventResult | String | Wrong Password | Reason for the result reported in ResultType | Event |
| ResultType<br>EventResultDetails | Multivalue: Success, Partial, Failure, [Empty] (String) | Success | The result reported for the activity. Empty value when not applicable. | Event |
| SchemaVer<br>EventSchemaVersion | Real | 0.1 | Azure Sentinel Schema Version. Currently 0.1. | Event |
| EventSeverity | String | Low | If the activity reported has a security impact, denotes the severity of the impact. | Event |
| EventUid<br>EventOriginalUid | String | af6ae8fe-ff43-4a4c-b537-8635976a2b51 | The record id from the reporting device. | Event |
...
| EventRecordUid<br>EventUid | Guid (String) | 516a64e3-8360-4f1e-a67c-d96b3d52df54 | Unique identifier generated by Sentinel to mark a row. | Event |
...
| DstGeolocationCity<br>DstGeoCity | String | Burlington | The city associated with the destination IP address | Geo<br>Destination |
| DstGeolocationCountry<br>DstGeoCountry | Country (String) | USA | The country associated with the source IP address | Geo<br>Destination |
...
| DstDomain<br>DstDvcDomain | string | CONTOSO | The domain of the destination host | Destination<br>User |
...
| DstGeolocationLatitude<br>DstGeoLatitude | Latitude (Double) | 44.475833 | The latitude of the geographical coordinate associated with the destination IP address | Geo<br>Destination |
...
| DstGeolocationRegion<br>DstGeoRegion | Region (String) | Vermont | The region within a country associated with the destination IP address | Geo<br>Destination |
...
| DstGeolocationLongitude<br>DstGeoLongitude | Longitude (Double) | -73.211944 | The longitude of the geographical coordinate associated with the destination IP address | Geo<br>Destination |
...
| IcmpCode<br>NetworkIcmpCode | Integer | 34 | For an ICMP message, ICMP message type numeric value (RFC 2780 or RFC 4443). | Network |
| IcmpType<br>NetworkIcmpType | String | Destination Unreachable | For an ICMP message, ICMP message type text representation (RFC 2780 or RFC 4443). | Network |
...
| SrcGeolocationCity<br>SrcGeoCity | String | Burlington | The city associated with the source IP address | Geo<br>Source |
| SrcGeolocationCountry<br>SrcGeoCountry | Country (String) | USA | The country associated with the source IP address | Geo<br>Source |
| SrcHostName<br>SrcDvcHostName | Device name (String) |  villain | The device name of the source device | Source |
| SrcHostFqdn<br>SrcDvcFqdn | string | Villain.malicious.com | The fully qualified domain name of the host where the log was created | Source |
| SrcDomain<br>SrcDvcDomain | string | EVILORG | Domain of the device from which session was initiated | Source |
| SrcHostOs<br>SrcDvcOs | String | iOS | The OS of the source device | Source |
| SrcHostModel<br>SrcDvcModelName | String | Samsung Galaxy Note | The model name of the source device | Source |
| SrcDvcModelNumber | String | 10.0 | The model number of the source device | Source |
| SrcHostType<br>SrcDvcType | String | Mobile | The type of the source device | Source |
...
| SrcGeolocationLatitude<br>SrcGeoLatitude | Latitude (Double) | 44.475833 | The latitude of the geographical coordinate associated with the source IP address | Source<br>Geo |
| SrcGeolocationLongitude<br>SrcGeoLongitude | Longitude (Double) | -73.211944 | The longitude of the geographical coordinate associated with the source IP address | Source<br>Geo |
...
| SrcGeolocationRegion<br>SrcGeoRegion | Region (String) | Vermont | The region within a country associated with the source IP address | Source<br>Geo |
...
| FileHashSha256AC<br>FileHashSha512 | String | 5E127D…F69F73F01F361 | The SHA512 hash value of the file transmitted over the network connections for protocols. | File |
| FileExtension |  String | exe | The type of the file transmitted over the network connections for protocols such as FTP and HTTP. File types should be prioritized as follows:
• | Extension
• | Named type
• | Class | File |
...
| HttpRequestVer<br>HttpVersion | String | 2.0 | The HTTP Request Version for HTTP/HTTPS network connections. | Http |
| HttpResponseVer | String | 1.1 | The HTTP Response Version for HTTP/HTTPS network connections. | - |
...
| UserAgentOriginal<br>HttpUserAgentOriginal | String | Mozilla/5.0 (Windows NT 10.0; WOW64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/83.0.4103.97 Safari/537.36 | The HTTP user agent header for HTTP/HTTPS network sessions. | Http |
| HttpXff<br>HttpRequestXff | String | 120.12.41.1 | The HTTP X-Forwarded-For header for HTTP/HTTPS network sessions. | Http |
