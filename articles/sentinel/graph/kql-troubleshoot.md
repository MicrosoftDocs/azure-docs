...
| Queries timing out at Gateway. | Long-running queries without time filters. | Enforce time filters or apply additional filters. |
| No time range set. Add a time parameter to control query cost and avoid timeouts. | Queries that have unrestricted lookback, can cause timeouts. | Enforce time filters or apply additional filters. |
