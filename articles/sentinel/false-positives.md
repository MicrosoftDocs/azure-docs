...
|Automation rules|Rule query modifications|
|-|-|
...
|Keep an audit trail.<br /><br />The exceptions prevent incident creation, but the alerts are still recorded for audit purposes.|Are the most flexible and complete solution for false positives, but can be costly.|
|Are often generated by analysts.|Typically require implementation by Security Operations Center (SOC) engineers.|
|Allow applying exceptions for a limited time.<br /><br />For example, maintenance work might trigger false positives that outside the maintenance timeframe would be true incidents.||
