...
|Automation rules|Analytics rules modifications|
|-|-|
|Can apply to several analytics rules, or to detections not based on analytics rules.|Allow advanced boolean expressions and subnet-based exceptions.|
|Keep an audit trail. Exceptions prevent incident creation, but alerts are still recorded for audit purposes.|Let you use watchlists to centralize exception management.|
|Are often generated by analysts.|Typically require implementation by Security Operations Center (SOC) engineers.|
|Allow applying exceptions for a limited time. For example, maintenance work might trigger false positives that outside the maintenance timeframe would be true incidents.|Are the most flexible and complete false positive solution, but are more costly.|
