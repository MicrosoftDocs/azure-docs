...
|-|-|
|Let you apply the same exceptions to several alert rules.|Allow advanced boolean expressions and subnet-based exceptions.|

|Work for detections that aren't based on scheduled alert rules.|
|Allow applying exceptions for a limited time. For example, maintenance work might trigger false positives that outside the maintenance timeframe would be true incidents.|Let you use watchlists to centralize exception management.|
|Keep an audit trail. The exceptions prevent incident creation, but the alerts are still recorded for audit purposes.|Are the most flexible and complete solution for false positives, but can be costly.|
