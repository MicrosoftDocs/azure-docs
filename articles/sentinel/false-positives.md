...
[Azure Sentinel detection rules](tutorial-detect-threats-built-in.md) notify you when something suspicious occurs in your network. Detection rules include built-in templates to use when monitoring your environment.

No detection rule is perfect, and you're bound to get some false positives that need handling. This article describes how to handle false positives using automation or scheduled analytics rules.
...
- Intentional security scanning activity coming from known IP addresses is often detected as malicious.
...
- **Use automation rules** to create exceptions without modifying alert rules.
- **Modify scheduled analytics rules** for more detailed and permanent exceptions.

The following table describes characteristics of each method:

  
|Automation rules|Analytics rule query modifications|
|-|-|
|Let you apply the same exceptions to several analytics rules.|Let you use watchlists to centralize exception management.|
|Work for detections that aren't based on scheduled alert rules.|Allow advanced boolean expressions and subnet-based exceptions.|
|Keep an audit trail.<br>The exceptions prevent incident creation, but the alerts are still recorded for audit purposes.|Are the most flexible and complete solution for false positives, but can be costly.|
|Are often generated by analysts.|Typically require implementation by Security Operations Center (SOC) engineers.|
|Allow applying exceptions for a limited time.<br>For example, maintenance work might trigger false positives that outside the maintenance timeframe would be true incidents.|
...
### Exclude subnets
 ...
 The preceding use case, excluding IP ranges used by an organization, requires subnet exclusion. The following example shows how to exclude subnets.
