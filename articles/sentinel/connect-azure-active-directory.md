...
To query the Microsoft Entra logs, enter the relevant table name at the top of the query window. If an expected table is not available, verify the log categories are selected for your Microsoft Sentinel workspace in Entra diagnostic settings. For more informatoin, see [Configure Microsoft Entra diagnostic settings for activity logs](/entra/identity/monitoring-health/howto-configure-diagnostic-settings).
