...
1. Under **Query scheduling**:
    ...
    1. Set the **Lookup data from the last** field to to control the time window for how much data the query runs on - for example, it can run every hour across 60 minutes of data.
...
1. In the **Response automation tab** select any playbooks you want to run automatically when an alert is generated by the custom rule. For more information on creating and automating playbooks, see [Respond to threats](tutorial-respond-threats.md).
