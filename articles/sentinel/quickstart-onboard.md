...
1. Select the workspace you want to use or create a new one. You can run Azure Sentinel on more than one workspace, but the data is isolated to a single workspace.
   ...
   >[!IMPORTANT]
   > - Once you have enabled Azure Sentinel on a workspace, you cannot move the workspace to another resource group or subscription. Doing so will break the permissions on existing analytics rules, and other negative repercussions may result as well.
