...
1. Select the workspace you want to use or create a new one. You can run Azure Sentinel on more than one workspace, but the data is isolated to a single workspace.
   ...
   > - Azure Sentinel (such as incidents, bookmarks, alert rules, and machine learning processing data) is saved in either West Europe (for workspaces located in Euro - Azure Sentinel can run on workspaces in [any region supported by Log Analytics GA](https://azure.microsoft.com/global-infrastructure/services/?products=monitor). All raw data is saved in your workspace, locally. Data generated bype) or East US (for all US-based workspaces as well as any other region except Europe).
