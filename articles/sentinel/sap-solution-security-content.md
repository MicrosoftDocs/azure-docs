...
|**SAP - High - Sensitive Users Password Change and Login**     | Identifies password changes for privileged users.      |  Change the password for a privileged user and sign into the system. <br>Maintain privileged users in the SAP - Privileged Users watchlist.<br><br>**Data sources**: SAPcon -  Audit Log | Impact, Command and Control, Privilege Escalation |
...
|**SAP - High - Execution of Sensitive Function Module**     | Identifies the execution of a sensitive ABAP function module.     | Run a sensitive function module directly using SE37. <br>**Note: Meaningful for production systems only.** <br>Maintain sensitive functions in the SAP - Sensitive Function Modules watchlist, and make sure to activate table logging changes in the backend for the EUFUNC table. (SE13) <br><br>**Data sources**: SAPcon -  Table Data Log | Discovery, Command and Control 
|     |         |         |         |
...
|**SAP - Medium - Sensitive Tables Direct Access By RFC Logon**     |Identifies a generic table access by RFC sign in. | Open the table contents using SE11/SE16/SE16N.<br> **Note: Meaningful for production systems only.** <br> Maintain tables in the SAP - Sensitive Tables watchlist. <br><br>**Data sources**: SAPcon -  Audit Log   |  Collection, Exfiltration, Credential Access     |
|**SAP - Medium - Sensitive Roles Changes**     |Identifies changes in sensitive roles.       |  Change a role using PFCG. <br> Maintain sensitive roles in the SAP - Sensitive Roles watchlist. <br><br>**Data sources**: SAPcon - Change Documents Log, SAPcon – Audit Log   |  Impact, Privilege Escalation, Persistence     |
|**SAP - Medium - Sensitive Profile Changes**     |Identifies changes in sensitive profiles.       |  Change a role using PFCG. <br> Maintain sensitive profiles in the SAP - Sensitive Profiles watchlist. <br><br>**Data sources**: SAPcon - Change Documents Log, SAPcon – Audit Log   | Privilege, Escalation, Persistence, Command and Control |
|**SAP - Medium - Execution of Obsolete/Insecure Program**     |Identifies the execution of an obsolete or insecure ABAP program. | Run a program directly using SE38/SA38/SE80, or by using a background job. <br> **Note: Meaningful for production systems only.** <br> Maintain obsolete programs in the SAP - Obsolete Programs watchlist. <br><br>**Data sources**: SAPcon -  Audit Log   | Discovery, Command and Control |
|**SAP - Medium - Execution of Obsolete or Insecure Function Module**     |Identifies the execution of an obsolete or insecure ABAP function module. | Run an obsolete or insecure function module directly using SE37. <br> **Note: Meaningful for production systems only.** <br>Maintain obsolete functions in the SAP - Obsolete Function Modules watchlist. Make sure to activate table logging changes for the EUFUNC table in the backend. (SE13) <br><br>**Data sources**: SAPcon -  Table Data Log   | Discovery, Command and Control |
|     |         |         |         |
...
|**SAP - Low - Dynamic ABAP Program**     | Identifies the execution of dynamic ABAP programming. For example, when ABAP code was dynamically created, changed, or deleted. | Create an ABAP Report that uses ABAP program generation commands, such as INSERT REPORT, and then run the report. <br> Maintain excluded transaction codes in the SAP - Transactions for ABAP Generations" watchlist.  <br><br>**Data sources**: SAPcon - Audit Log      |    Discovery, Command and Control, Impact     |
...
|<a name="servers"></a>**SAP - FTP Servers**     |  FTP Servers for identification of unauthorized connections.    <br><br>- **Client**:such as 100.  <br>- **FTP_Server_Name**: FTP server name, such as http://ourorgftpserver.com/ <br>-**FTP_Server_Port**:FTP server port, such as 22. <br>- **Description**A meaningful FTP Server description |
| | |
