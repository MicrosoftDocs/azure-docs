---
title: Using SQL Database DACPAC and BACPAC packages - Azure SQL Edge (Preview)
description: Learn about using dacpacs and bacpacs in Azure SQL Edge (Preview)
...
|Field | Description |
|------|-------------|
| SqlPackage | Azure Blob storage URI for the *.zip* file that contains the SQL Database DAC or BACPAC package. The zip file can contain both multiple dac packages or bacpac files.
| ASAJobInfo | Azure Blob storage URI for the ASA Edge job.

## Use a SQL Database DAC package with SQL Edge

To use a SQL Database DAC package `(*.dacpac)` or a BACPAC file `(*.bacpac)` with SQL Edge, follow these steps:
...
5. After the module update, the package file is downloaded, unzipped, and deployed against the SQL Edge instance.

On each restart of the Azure SQL Edge container, the `*.dacpac` file package is download and evaluated for changes. If a new version of the dacpac file is encountered, the changes are deployed to the database in SQL Edge. Bacpac files 
