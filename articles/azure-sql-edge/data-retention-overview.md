...
Azure SQL Edge (Preview) CTP2.3 adds a new capability that allows database administrators to define data retention policy on a SQL Edge database and its underlying tables. Once the data retention policy is defined, a background system task will run to purge any obselete (old) data from the user tables. 
