...
**A.**  Beginning November 9, 2023, you'll no longer be able to create new Single Server instances for PostgreSQL version 11 through the Azure portal. However, you can still [make them via CLI until November 2024](https://azure.microsoft.com/updates/singlepg11-retirement/). We'll continue to support Single Server through our [versioning support policy.](/azure/postgresql/single-server/concepts-version-policy) You should start migrating to Azure Database for PostgreSQL - Flexible Server immediately.
...
**Q. Will I incur downtime when I migrate my Azure Database for PostgreSQL - Single Server to Flexible Server?**
...
**A.**  We recommend you migrate to Flexible Server if you must run on the latest PostgreSQL engine versions. We'll continue to deploy minor versions released by the community for Postgres version 11 till it's retired by the community in Nov'2023.
...
**Q. How does the Flexible Server's 99.99% availability SLA differ from Single Server?**
...
Cross-region data migrations should be avoided because the migration takes a long time to complete. A simpler way to do this will be to start a read-replica in the target GeoRegion, failover your application to it, and follow the steps outlined earlier.
...
| Suitability for Write-heavy workloads | Suitable but expected to reduce the workload during migration | This isn't a recommended solution unless you can disable writes during the migration. Any writes at the source aren't migrated to the target server after the migration begins. |
| Manual Cutover | Required | Not required |
| Downtime required | Small and fixed irrespective of the data size | More |
...
- You can reach out to the [Azure Database for PostgreSQL product team](mailto:AskAzureDBforMySQL@service.microsoft.com?subject=Azure%20Database%20for%20PostgreSQL%20-%20Single%20Server%20retirement).
