...
Azure Database for PostgreSQL provides business continuity features that include geo-redundant backups with the ability to initiate geo-restore, and deploying cross region read replicas. Both of these has different characteristics for the recovery time and potential data loss. With [Geo-restore](concepts-backup.md) feature, the recovery is performed using the backup data replicated from another region. The overall time it takes to restore and recover depends on the size of the database and the amount of logs to process. It typically varies from few minutes to few hours. With [read replicas],(concepts-read-replicas.md) transaction logs from the primary are asynchronously streamed to the replica.that are deployed on another region. The lag between the primary and the replica depends on the latency between the sites and also the type of operations being performed at the primary. In the event of a disaster recovery, promoting the replica provides a shorter RTO and reduced data loss. 
