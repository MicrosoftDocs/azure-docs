...
Azure Database for PostgreSQL is a relational database service in the Microsoft cloud. It is built for developers using the community version of open source [PostgreSQL](https://www.postgresql.org/). This service is in public preview. Azure Database for PostgreSQL delivers:
...
Azure's industry leading 99.99% availability (not available in preview) service level agreement (SLA), powered by a global network of Microsoft-managed datacenters, helps keep your app running 24/7. With every Azure Database for PostgreSQL server, you take advantage of built-in security, fault tolerance, and data protection that you would otherwise have to buy or design, build, and manage. With Azure Database for PostgreSQL, each service tier offers a comprehensive set of business continuity features. You can use [point-in-time restore](howto-restore-server-portal.md) to return a database to an earlier state, as far back as 35 days. In addition, if the datacenter hosting your databases experiences an outage, you can restore databases from geo-redundant copies of recent backups.
...
The Azure Database for PostgreSQL service uses storage encryption for data at-rest. Data including backups are encrypted on disk excapt  temporary files created while executing queries. The service uses AES 256-bit cipher that is included in Azure storage encryption, and the keys are system managed. Storage encryption is always on and cannot be disabled.
