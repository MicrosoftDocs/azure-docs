...
| ------------ | ---------------- | --------------------------------------|
| Restore point | Date and Time | Using the Date calendar and time picker, select a point-in-time to restore to. The time specified is in UTC, so you likely need to convert the local time zone into UTC. |
| Restore to a new server | New Server Name | Provide a new server name to restore the existing server into. The name is used for DNS, and therefore must be globally unique in Azure. |
