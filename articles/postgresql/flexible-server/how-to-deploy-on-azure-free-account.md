...
<!-- This no longer appears in the portal. When you select Create it immediately take you to the flexible server Create page
...
1. Enter the basic settings for a new **Azure Database for PostgreSQL flexible server** instance.
    ...
    |Availability zone	|No preference	|If your application (hosted on Azure VMs, virtual machine scale sets or AKS instance) is provisioned in a specific availability zone, create your Azure Database for PostgreSQL flexible server instance in the same availability zone. Collocating the application and database improves performance by reducing network latency across zones. If you choose **No preference**, a default AZ is selected for you.|
