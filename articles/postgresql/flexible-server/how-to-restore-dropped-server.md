...
> :::image type="content" source="![Activity log filtered for delete PostgreSQL server operation](./media/how-to-restore-server-portal/activity-log-azure.png)"   alt-text="Diagram of activity log.":::
...
5. Provide the **resourceGroupName**, **serverName** (Target server name), **subscriptionId** properties, based on the resourceId attribute JSON value captured in the preceding step 3. The api-version property is prepopulated and can be left as-is, as shown in the following image.
  ...
  > :::image type="content" source="![Create server using REST API](./media/how-to-restore-server-portal/create-server-from-rest-api-azure.png)" alt-text="Create server from Rest API.":::
