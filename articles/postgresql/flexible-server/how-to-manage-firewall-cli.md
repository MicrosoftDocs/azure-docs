...
- Create a flexible server with public access and with no IP address
    ```azurecli-interactive
    az postgres flexible-server create --public-access
    ```
    >[!Note]
...
```azurecli-interactive
az postgres flexible-server firewall-rule create --server-name mydemoserver --start-ip-address 13.83.152.0 --end-ip-address 13.83.152.15
```

To allow access for a single IP address, just provide single IP address, as in this example.
```azurecli-interactive
az postgres flexible-server firewall-rule create --server-name mydemoserver --start-ip-address 1.1.1.1
```
...
```azurecli-interactive
az postgres flexible-server firewall-rule create --server-name mydemoserver --start-ip-address 0.0.0.0
```
...
### List firewall rules 
Use the `az postgres flexible-server firewall-rule list` command to list the existing server firewall rules on the server. Notice that the server name attribute is specified in the **--server-name** switch and not in the **--name** switch. 
```azurecli-interactive
az postgres flexible-server firewall-rule list --server-name mydemoserver
```
...
```azurecli-interactive
az postgres flexible-server firewall-rule list --server-name mydemoserver --output table
```
...
```azurecli-interactive
az postgres flexible-server firewall-rule update --server-name mydemoserver --name FirewallRule1 --start-ip-address 13.83.152.0 --end-ip-address 13.83.152.1
```
...
```azurecli-interactive
az postgres flexible-server firewall-rule show --server-name mydemoserver --name FirewallRule1
```
...
```azurecli-interactive
az postgres flexible-server firewall-rule delete --server-name mydemoserver --name FirewallRule1
```
