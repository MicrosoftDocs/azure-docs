...
Executing the query following explain analyze output was retrieved from the body of message column 
...
AzureDiagnostics 
| where Category contains  'PostgreSQLLogs' 
| where Message contains "<add snippet of SQL text identified or add table name involved in the queries related to stored procedure>" 
| project TimeGenerated, Message
``` 
...
> [!NOTE]
> please note for demonstration purpose explain analyze output of only few queries used in procedure are shared. The idea is one can gather explain analyze output of all queries from the logs and then identify the slowest of those and try to tune them.
