...
- You fully control data access. You can remove the key and make the database inaccessible.

- You have full control over the key's life cycle, including rotation of the key to align with corporate policies.

- You can centrally manage and organize keys in Azure Key Vault.

- Enabling encryption doesn't have any additional performance impact with or without CMKs, because PostgreSQL relies on the Azure storage layer for data encryption in both scenarios. The only difference is when CMK is used **Azure Storage Encryption Key**, which performs actual data encryption, is encrypted using CMK.
...
Microsoft Entra [user- assigned managed identity](../../active-directory/managed-identities-azure-resources/overview.md) will be used to connect and retrieve CMK. Follow this [tutorial](../../active-directory/managed-identities-azure-resources/qs-configure-portal-windows-vm.md) to create identity.

For a PostgreSQL server to use CMKs stored in Key Vault for encryption of the DEK, a Key Vault administrator gives the following *access rights* to the managed identity created above:
...
> [!IMPORTANT]  
> Not providing above access rights to the Key Vault to managed identity for access to Key Vault might result in failure to fetch encryption key and subsequent failed setup of the CMK feature.

When the server is configured to use the CMK stored in the key Vault, the server sends the DEK to the key Vault for encryptions. Key Vault returns the encrypted DEK stored in the user database. Similarly, when needed, the server sends the protected DEK to the key Vault for decryption. Auditors can use Azure Monitor to review Key Vault audit event logs, if logging is enabled.
...
The following are requirements for configuring the CMK in Azure Database for PostgreSQL flexible server:

- The CMK to be used for encrypting the DEK can be only asymmetric, RSA or RSA-HSM. Key sizes of 2048, 3072, and 4096 are supported.
...
When you're using data encryption by using a CMK, here are recommendations for configuring Key Vault:
...
- Lock down the Azure Key Vault to only **disable public access** and allow only *trusted Microsoft* services to secure the resources.
...
- Revoking the Key Vault's **list**, **get**, **wrapKey**, and **unwrapKey** permissions from the identity used to retrieve key in Key Vault.
...
- On the restored/replica server, you can change the CMK and\or Microsoft Entra identity used to access Azure Key Vault in the data encryption settings. Ensure that the newly created server is given list, wrap and unwrap permissions to the key stored in Key Vault.
...
- If you delete the Key Vault, the Azure Database for PostgreSQL flexible server instance will be unable to access the key and will move to *Inaccessible*  state. [Recover the Key Vault](../../key-vault/general/key-vault-recovery.md) and revalidate the data encryption to make the server *Available*.
- If you delete the key from the Key Vault, the Azure Database for PostgreSQL flexible server instance will be unable to access the key and will move to *Inaccessible* state. [Recover the Key](../../key-vault/general/key-vault-recovery.md) and revalidate the data encryption to make the server *Available*.
- If you delete [managed identity](../../active-directory/managed-identities-azure-resources/how-manage-user-assigned-managed-identities.md) from Microsoft Entra ID that is used to retrieve a key from Key Vault, the Azure Database for PostgreSQL flexible server instance will be unable to access the key and will move to *Inaccessible* state.[Recover the identity](../../active-directory/fundamentals/recover-from-deletions.md) and revalidate data encryption to make server *Available*.
- If you revoke  the Key Vault's list, get, wrapKey, and unwrapKey access policies from the [managed identity](../../active-directory/managed-identities-azure-resources/how-manage-user-assigned-managed-identities.md) that is used to retrieve a key from Key Vault, the Azure Database for PostgreSQL flexible server instance will be unable to access the key and will move to *Inaccessible* state. [Add required access policies](../../key-vault/general/assign-access-policy.md) to the identity in Key Vault.
- If you set up overly restrictive Azure Key Vault firewall rules that cause Azure Database for PostgreSQL flexible server inability to communicate with Azure Key Vault to retrieve keys. If you enable [Key Vault firewall](../../key-vault/general/overview-vnet-service-endpoints.md#trusted-services), make sure you check an option to *'Allow Trusted Microsoft Services to bypass this firewall.'*
...
>
> Generally, server will become **inaccessible** within an 60 minutes after a key is either disabled, deleted,  expired, or cannot be reached. Similarly after key becomes available it might take up to 60 minutes until server becomes accessible again.

## Using data encryption with CMK and geo-redundant business continuity features
...
The following are current limitations for configuring the CMK in Azure Database for PostgreSQL flexible server:
