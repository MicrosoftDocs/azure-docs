...
Oracle supports Transparent Data Encryption (TDE) to encrypt table and tablespace data. In Azure for PostgreSQL, the data is automatically encrypted at various layers. See the above "At-rest" section and also various Security topics, including [customer managed keys](concepts-data-encryption-postgresql) and [Infrastructure double encryption](concepts-infrastructure-double-encryption). You can also consider using [pgcrypto](https://www.postgresql.org/docs/11/pgcrypto.html) extension which is supported in [Azure for PostgreSQL](concepts-extensions).
