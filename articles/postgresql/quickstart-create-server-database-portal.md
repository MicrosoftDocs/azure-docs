...
Once connected, the psql utility will display a `postgres=>` prompt for you to type commands. In the initial connection output, a warning may be displayed since the psql in the Cloud Shell may be a different version than the Azure Datbase for PostgreSQL server version.
