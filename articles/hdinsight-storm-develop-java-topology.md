...
Learn how to the basic process for creating a Java-based topology for Apache Storm on HDInsight using Maven. You will walk through the process of creating a basic word count application using Maven and Java. While instructions are provided for using Eclipse, you may also use the text editor of your choice.
...
* A text editor such as <a href="http://www.gnu.org/software/emacs/" target="_blank">Emacs<a>, <a href="http://www.sublimetext.com/" target="_blank">Sublime Text</a>, <a href="https://atom.io/" target="_blank">Atom.io</a>, <a href="http://brackets.io/" target="_blank">Brackets.io</a>, or notepad. Or an integrated development environment (IDE) such as <a href="https://eclipse.org/" target="_blank">Eclipse</a> (version Luna or later).

	> [AZURE.NOTE] Your editor or IDE may have specific functionality for working with Eclipse that is not addressed in this document. For information on the capabilities of your editing environment, see the documentation for the product you are using.

##Configure environment variables

The following environment variables may be set by the installation of Java and the JDK; however, you should check that they exist and contain the correct values for your system

* **JAVA_HOME** - should point to the directory where the Java runtime environment (JRE) is installed. For example, on a Unix or Linux distribution, it should have a value similar to `/usr/lib/jvm/java-7-oracle`. On Windows, it would have a value similar to `c:\Program Files (x86)\Java\jre1.7`
...
From the command line, use the following to create a new Maven project named **WordCount**.

	mvn archetype:generate -DarchetypeArtifactId=maven-archetype-quickstart -DgroupId=com.microsoft.example -DartifactId=WordCount -DinteractiveMode=false

This will create a new directory named **WordCount** at the current location, containing a basic Maven project.
...
Since we will be creating our application from scratch, delete the generated test and application files:
...
Since this is a Storm topology, you must add a dependency for Storm components. Open the **pom.xml** and add the following in the **&lt;dependencies>** section.
...
> [AZURE.NOTE] Note the `<scope>provided</scope>` line in the section we added. This tells Maven to exclude storm-core from any Jar files we create, as it will be provided by the system. This allows packages you create to be a little smaller, and ensures that they will use the storm-core bits included on the Storm on HDInsight cluster.

##Build configuration

Maven plugins allow you to customize the build stages of the project, such as how the project is compiled, or how to package it into a jar file. Open the **pom.xml** and add the following directly above the `</project>` line.
...
This section will be used to add plugins and other build configuration options.

###Add plugins

For Storm topologies, the <a href="http://mojo.codehaus.org/exec-maven-plugin/" target="_blank">Exec plugin</a> is useful, as it allows you to easily run the topology locally on your development environment. Add the following to the `<plugins>` section of the **pom.xml** to include the exec plugin.
...
Another useful plugin is the <a href="http://maven.apache.org/plugins/maven-compiler-plugin/" target="_blank">Compiler plugin</a>, which is used to change compilation options. The primary reason we need this is to change the Java version that Maven uses for source and target for your application - we want 1.7.

Add the following in the `<plugins>` section of the **pom.xml** to include the compiler plugin and set the source and target version to 1.7.
...
* **Spouts** - read data from external sources and emit streams of data into the topology

* **Bolts** - perform processing on streams emitted by spouts or other bolts and emit one or more streams

* **Topology** - defines how the spouts and bolts are arranged, and provides the entry point for the topology.

###Create the spout

To reduce requirements for setting up external data sources, the following spout simply emits random sentences. It is a modified version of a spout provided with the Storm-Starter examples (<a href="https://github.com/apache/storm/blob/master/examples/storm-starter/" target="_blank">https://github.com/apache/storm/blob/master/examples/storm-starter/</a>).
...
> [AZURE.NOTE] While this topology only uses one spout, others may have several that feed data from different sources into the topology.

###Create the bolts

Bolts handle the processing of data. For this topology, we have two:

* **SplitSentence** - splits the sentences emitted by the RandomSentenceSpout into individual words

* **WordCount** - counts how many times each word has occurred

> [AZURE.NOTE] Bolts can do literally anything - computation, persistence, talking to external components, etc.
...
Take a moment to read through the code comments to understand how the topology is defined, then submitted to the cluster.

##Test the topology locally

After saving the files, use the following command to test the topology locally.

	mvn compile exec:java -Dstorm.topology=com.microsoft.example.WordCountTopology

As it runs, the topology will display startup information, then begin displaying lines similar to the following as sentences are emitted from the spout, then processed by the bolts.
...
Looking at the data emitted by the count bolt, 'apple' has been emitted 53 times. The count will continue going up as long as the topology runs, as the same sentences are randomly emitted over and over and the count is never reset.

##Trident

Trident is a high-level abstraction provided by storm, that supports stateful processing. The primary advantage of Trident is that it can guarantee that every message entering the topology is processed only once - this is difficult to achieve in a raw Java topology, which guarantee's that messages will be processed at least once. There are other differences as well, such as built-in components that can be used instead of creating bolts. In fact, bolts are completely replaced by less generic components such as filters, projections, and functions.

Trident applications can be created using Maven projects using the same basic steps above, only the code is different.

For more information on Trident, see the <a href="http://storm.apache.org/documentation/Trident-API-Overview.html" target="_blank">Trident API Overview</a>.
...
You have learned how to create a Storm topology using Java, now learn how to:
