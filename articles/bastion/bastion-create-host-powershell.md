...
This article shows you how to create an Azure Bastion host using PowerShell. After deploying Bastion, you can connect to the VM via its private IP address through your browser using the Azure portal. Your VM does not need a public IP address, an additional client, or special software. Azure Bastion deployment is per virtual network, not per subscription/account or virtual machine. The seamless RDP/SSH experience is available to all of the VMs in the same virtual network.

These instructions are for PowerShell deployment. You can also create an Azure Bastion host by using the [Azure portal](tutorial-create-host-portal.md) or [Azure CLI](create-host-cli.md).
...
This section helps you create a new Azure Bastion resource for your virtual network using Azure PowerShell.
