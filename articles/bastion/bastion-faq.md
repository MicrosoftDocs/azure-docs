...
Azure Bastion needs to be able to communicate with certain internal endpoints to successfully connect to target resources, and therefore you can use Azure Bastion with Azure Private DNS Zones as long as the DNS zone name you select does not overlap with the naming conventions of these internal endpoints. Before you deploy your Azure Bastion resource, please make sure that the host virtual network is not linked to a private DNS zone with the following in the name:
* core.windows.net
* azure.com
Note that if you are using a Private endpoint integrated Azure Private DNS Zone, the [recommended DNS zone name](https://docs.microsoft.com/azure/private-link/private-endpoint-dns#azure-services-dns-zone-configuration) for several Azure services overlap with the names listed above. The user of Azure Bastion is *not* supported with these setups.
