...
* Use [Azure CLI](/cli/azure/create-an-azure-service-principal-azure-cli?toc=%2fazure%2fazure-resource-manager%2ftoc.json) or [Azure portal](/azure/azure-resource-manager/resource-group-create-service-principal-portal) to create an Azure service principal
* Within the Jenkins dashboard, click **Credentials -> System ->**. Click **Global credentials(unrestricted)**.
* Click **Add Credentials** to add a Microsoft Azure service principal by filling out the Subscription ID, Client ID, Client Secret, and OAuth 2.0 Token Endpoint. Provide an ID, **mySp**, for use in subsequent steps.
...
> * Create a Web App. You can either use the [Azure portal](/azure/app-service-web/web-sites-configure) or use the following Az CLI command:
...
Make sure you set up the Java runtime configuration that your app needs. The following Azure CLI command configures the web app to run on a recent Java 8 JDK and [Apache Tomcat](http://tomcat.apache.org/) 8.0.
...
* Add a post-build action by selecting **Publish an Azure Web App**.
* Supply, "mySp", the Azure service principal stored in previous step.
* In **App Configuration** section, choose the resource group and web app in your subscription. The plugin automatically detects whether the Web App is Windows or Linux-based. For a Windows-based Web App, the option "Publish Files" is presented. 
* Fill in the files you want to deploy (for example, a war package if you're using Java.) Source Directory and Target Directory are optional. The parameters allow you to specify source and target folders when uploading files. Java web app on Azure is run in a Tomcat server. So you upload you war package to webapps folder. For this example, set **Source Directory** to "target" and supply "webapps" for **Target Directory**.
* If you want to deploy to a slot other than production, you can also set **Slot** Name 
* Save the project and build it. Your web app is deployed to Azure when build is complete.
...
* In GitHub web UI, open **Jenkinsfile_ftp_plugin** file. Click the pencil icon to edit this file to update the resource group and name of your web app on line 11 and 12 respectively.
...
```
* Change line 14 to update credential ID in your Jenkins instance
...
* For **SCM**, select **Git**. * Enter the GitHub URL for your forked repo: https:\<your forked repo\>.git
* Update **Script Path** to "Jenkinsfile_ftp_plugin"
...
* Add a post-build action by selecting **Publish an Azure Web App**.
* Supply, **mySp**, the Azure service principal stored in previous step as Azure Credentials
* In **App Configuration** section, choose the resource group and a Linux web app in your subscription.  
* Choose Publish via Docker.
* Fill in **Dockerfile** path. You can keep the default "**/Dockerfile"
...
* For **Registry credentials**, add the credential for the Azure Container Registry. You can get the userid and password by running the following commands in Azure CLI. The first command enables the administrator account.  
...
* The docker image name and tag in **Advanced** tab are optional. By default, image name is obtained from the image name you configured in Azure portal (in Docker Container setting.) The tag is generated from $BUILD_NUMBER. Make sure you specify the image name in either Azure portal or supply a value for **Docker Image** in **Advanced** tab. For this example, supply "\<yourRegistry\>.azurecr.io/calculator" for **Docker image** and leave **Docker Image Tag** blank.
* Note deployment fails if you use built-in Docker image setting. Make sure you change docker config to use custom image in Docker Container setting in Azure portal. For built-in image, use file upload approach to deploy.
* Similar to file upload approach, you can choose a different slot other than production.
* Save and build the project. You see your container image is pushed to your registry and web app is deployed.


### Deploy to Web App on Linux through Docker using Jenkins pipeline
 
* In GitHub web UI, open **Jenkinsfile_container_plugin** file. Click the pencil icon to edit this file to update the resource group and name of your web app on line 11 and 12 respectively.
...
* Change line 13 to your container registry server

```java
def registryServer = '<registryURL>'
```

* Change line 16 to update credential ID in your Jenkins instance
...
* Update **Script Path** to "Jenkinsfile_container_plugin"
* Click **Save** and run the job
