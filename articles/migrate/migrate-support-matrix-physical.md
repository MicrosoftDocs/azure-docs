...
**Server access** | When mapping credentials to the servers you add on the appliance, if you want to perform software inventory on those servers, you must have provided a guest user account for Windows servers and a standard user account (non-`sudo` access) for all Linux servers.
