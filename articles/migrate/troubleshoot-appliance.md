...
## "Invalid OVF manifest entry" error occurs during appliance set up
...
## Connectivity check fails during 'Set up prerequisites'
...
1. If there's a proxy server configured on-premises, make sure that you provide the proxy details correctly by selecting **Setup proxy** in the same step. Make sure that you provide the authorization credentials if the proxy needs them.
...
## Connectivity check fails for aka.ms URL during 'Set up prerequisites'

**Error**

You get an error in the connectivity check on the appliance for aka.ms URL.

**Remediation**

1. Ensure that you have connectivity to the internet and have allowlisted the URL-aka.ms/* to download the latest versions of the services.
1. Check if there's a proxy/firewall blocking access to this URL. Ensure that you've provided the proxy details correctly in the prerequisites step of the configuration manager.
...
## Auto-update check fails during 'Set up prerequisites'
...
## Time sync check fails during 'Set up prerequisites'
...
## VDDK check fails during 'Set up prerequisites' on VMware appliance

**Error**

The VDDK check failed because the appliance couldn't find the required VDDK Kit installed on the appliance. This can result in failures with ongoing replication.

**Remediation**

1. Ensure that you've downloaded the VDDK Kit 6.7 and have copied its files to- **C:\Program Files\VMware\VMware Virtual Disk Development Kit** on the appliance server.
1. Ensure that no other software or application is using another version of the VDDK Kit on the appliance.
...
1. You can also assign the required roles and [permissions](./tutorial-discover-vmware.md#prepare-an-azure-user-account) to the other Azure user account being used for appliance registration
...
This issue usually happens when the Azure user account that was used to register the appliance is different from the account used to generate the Azure Migrate project key on the portal (that is, when the key vault was created).

**Remediation**

1. Ensure that the currently signed-in user account on the appliance has the required permissions on the key vault (mentioned in the error message). The user account needs permissions as mentioned [here](./tutorial-discover-vmware.md#prepare-an-azure-user-account).
1. Go to the key vault and ensure that your user account has an access policy with all the _Key, Secret, and Certificate_ permissions assigned under Key Vault Access Policy. [Learn more](../key-vault/general/assign-access-policy-portal.md)
1. If you've enabled the appliance for **private endpoint connectivity**, ensure that the appliance is either hosted in the same virtual network where the Key Vault has been created or it's connected to the Azure virtual network (where Key Vault has been created) over a private link. Make sure that the Key Vault private link is resolvable from the appliance. Go to **Azure Migrate**: **Discovery** and **assessment**> **Properties** to find the details of private endpoints for resources like the Key Vault created during the Azure Migrate key creation. [Learn more](./troubleshoot-network-connectivity.md).
...
- If the issue still occurs in the latest version, the appliance might be unable to resolve the specified vCenter Server name, or the specified port might be wrong. By default, if the port isn't specified, the collector will try to connect to port number 443.
...
## Server credentials (domain) fails validation on VMware appliance

**Error**

You get "Validation failed" for domain credentials added on the VMware appliance to perform software inventory, agentless dependency analysis.
...
1. You can select **Edit** to update the domain name or credentials. Select **Revalidate credentials** to validate the credentials again after some time.

## "Access is denied" error occurs when connecting to Hyper-V hosts or clusters during validation

**Error**

You're unable to validate the added Hyper-V host/cluster because of the error "Access is denied."
...
This error usually occurs when you've provided a proxy configuration on the appliance. The appliance connects to the clusters using the short name for the cluster nodes, even if you've provided the FQDN of the node. Add the short name for the cluster nodes to the bypass proxy list on the appliance, the issue gets resolved and validation of the Hyper-V cluster succeeds.

## "Can't connect to host or cluster" error occurs during validation on Hyper-V appliance
...
This usually happens when you have added the IP address of a host which can't be resolved by DNS. You might also see this error for hosts in a cluster. This indicates that the appliance can connect to the cluster, but the cluster returns host names that are not FQDNs.
...
- If it's a Linux server, ensure password-based authentication is enabled by following these steps:
    1. Sign in to the linux server and open the ssh configuration file using the command 'vi /etc/ssh/sshd_config'
    1. Set "PasswordAuthentication" option to yes. Save the file.
    1. Restart ssh service by running "service sshd restart"
...
## "Failed to fetch BIOS GUID" error occurs for server during validation
...
On executing the preceding code, you need to provide the hostname of the target server, which can be IP address/FQDN/hostname. After that, you'll be prompted to provide the credentials to connect to the server.

## "No suitable authentication method found" error occurs for server during validation

**Error**

You get this error when you're trying to validate a Linux server through the physical appliance "No suitable authentication method found."
...
## "Access is denied" error occurs when connecting to physical servers during validation

**Error**

You get this error when you're trying to validate a Windows server through the physical appliance- "WS-Management service cannot process the request. The WMI service returned an access denied error."

**Remediation**

- If you're getting this error, make sure that the user account provided(domain/local) on the appliance configuration manager has been added to these groups: Remote Management Users, Performance Monitor Users, and Performance Log Users.
...
- You can also check if the WS-Management protocol is enabled on the server by running following command in the command prompt of the target server.
    ...
    ```` winrm qc ````
- If you're still facing the issue, make sure that the user account has access permissions to CIMV2 Namespace and sub-namespaces in WMI Control Panel. You can set the access by following these steps:
