...
NTS | NFS volumes mounted as volumes on the VMs won't be replicated.
...
--- | ---
1. **Prepare Windows/Linux VMs**.<br/>You need to configure a number of settings on Windows and Linux VMs. | with agent
2. **Server Migration**: Add the server migration tool in the Azure Migrate project.
3. **Set up vCenter accounts**: Configure specific account permissions for migration.
4. **Replicate VMs**. Start VM replication. Azure Migrate can use Azure Migrate assessments for replication settings.<br/> After the initial replication, delta replication continues for VMs.
5. **Deploy the Azure Migrate appliance on-premises**. If you've already run Azure Migrate Server Assessment, then the appliance is already deployed.
6. **Run a test migration**. A test migration simulates a migration by creating an Azure VM using replicated data. Migration is usually to a different (non-production) Azure VNet, and doesn't impact on-premises servers operations or replication.
7. **Run a full migration**. After a full migration, you can manage the migrated VM as an Azure VM in the portal. You complete migration by stopping VM replication. This cleans up associated replication state information for the machine. 
