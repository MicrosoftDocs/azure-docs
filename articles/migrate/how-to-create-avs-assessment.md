...
3. Review the Suggested tool:
    ...
    - **Unknown**: For machines imported via a CSV file, teh default migration tool is unknown. Though, for VMware machines, its is suggested to use the VMWare Hybrid Cloud Extension (HCX) solution. 
...
1. Review the monthly total costs. Costs are aggregated for all VMs in the assessed group. 
    ...
    - As the pricing for Azure VMware Solution (AVS) is per node, the total cost does not have compute and storage cost distribution.
...
- For performance-based sizing, AVS assessments in Server Assessment needs the utilization data for CPU and VM memory. The following performance data is collected but not used in sizing recommendations for AVS assessments:
...
[Learn more](concepts-avs-assessment-calculation.md) about performance data 


## Next steps

- Learn how to use [dependency mapping](how-to-create-group-machine-dependencies.md) to create high confidence groups.
- [Learn more](concepts-avs-assessment-calculation.md) about how AVS assessments are calculated.
