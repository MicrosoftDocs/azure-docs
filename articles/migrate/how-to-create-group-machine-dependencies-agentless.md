...
- Dependency analysis gets auto-enabled by default on 1000 servers discovered by each Azure Migrate appliance that have passed prerequisite validation checks. You won't have to manually enable the dependency analysis on servers as was the case before.
...
- Review the requirements based on your environment and the appliance you are setting up to perform agentless dependency analysis:
...
6. Select on **Start discovery**, to initiate discovery.
 ...
 > [!Note]
 > Agentless dependency analysis will be auto-enabled for discovered servers where the prerequisite validation checks have passed. You will not have to manually enable the dependency analysis on servers as was the case before.
...
After initiating discovery from the appliance, you can come to Migrate project on the Azure portal and review the dependency data. It is recommended to wait for **atleast 24 hours** to allow for enough dependendency data to be gathered for your servers and shown in a visualization. 

In the project, you can review dependencies for each servers either through the **All inventory** or **Infrastruture inventory** view. 

On reviewing the **Dependencies** column for any server, you will see one of the following status:
...
3. **Validation failed:** when the validation checks on the server have failed. You can select the status to review error message which would mostly be related to missing prerequsites like insufficent credential permissions or invalid credentials etc.

After the validation succeeds, dependency analysis will be auto-enabled and you will see one of the following status:
...
5. **Not initiated:** when dependency analysis could not be enabled as Azure Migrate has reached the scale limit of 1000 servers per appliance for auto-enablement. If you want to perform dependency analysis on the specific servers, you can manually disable it on the other auto-enabled servers and enable for the ones you need by using the [PowerShell module]().
...
2. Serach for the server whose dependencies, you want to review. If dependency analysis was successfully performed on that server, you can click on **View dependencies** to go to the dependency visualization. 
3. The dependency visualization will show all incoming and outgoing dependencies for that server in a network diagram.
...
6. You can choose to change the process type from any of the folllowing:
    **Process** | **Type**
    --- | --- 
...
7. In the view, you will find the servers and connections represented as follows:
...
9. Similary you can also hover on the connection to see essential information like strength and frequencty of connections in the selected time range. 
10. You can select the Server name to see more details like Operating system, Power Satus, Software inventory discovered from the server and associated Tags.
...
13. From the expanded list of processes, you can click on a Process name to see its incoming and outgoing dependencies with processes on other servers in the view. The process to process dependency also indicates the destination port no on the connection.
...
4. Select the start time and end time. Note that you can download the data only for the last 30 days.
...
If you are an existing user who has already set up an Azure Migrate project, performed discovery and manually enabled dependency analysis on some servers, you need to perform following steps to get the new enhanced visualization:

1. Go to the inventory view and search for a server, for which you want to review dependencies.
2. Select on **View dependencies** and you will see the old visualization showing dependencies of that server.
...
3. On the dependency view, you will see an information box prompting you to complete some prequisites for switching to the new visualization.
4. As a prerequisite, you need to ensure that the discovery agent on the appliance(s) registered with the project have been upgraded to version <> or above.
...
1. Dependency analysis has been auto-enabled on all discovered in your project but you want to disable it on a few servers where you do not want to gather dependency data.
2. Dependency analysis has been auto-enabled on 1000 servers concurrently in your project but you have more servers where you want to enable it, then you can disable dependency analysis on or or more servers from the set of 1000 and enable others as needed.

> [!NOTE]
> Currently, it is not possible to disable dependendency analysis on servers from portal so you will need to install the PowerShell module to disable for servers that you do not want.
...
1. Get the list of discovered servers in your project using the following commands. In the example below, the project name is ContosoDemoProject, and the resource group it belongs to is ContosoDemoRG. The list of servers will be saved in ContosoDemo_VMs.csv
    ...
    **Filter name** | **Values**
    ...
    Some of the other fields are IPAddress, osType, osName, osArchitecture, osVersion
...
You may need to enable dependency analysis on one or more servers to restart dependency data collection from servers that you disabled using PowerShell module previoulsy.
