...
**Standard or Trusted Launch Virtual Machines**: Azure Migrate determines each VM’s compatibility with **Trusted Launch Virtual Machine (TVM)** requirements and recommends a VM security type, Trusted Launch or Standard. It verifies the supported operating systems, generation type (Gen 2), boot and disk configuration, and other prerequisites defined by Azure for Trusted Launch. If a VM meets the requirements, Azure Migrate recommends **Trusted Launch** by default to provide enhanced security features, such as **secure boot, vTPM**, and integrity monitoring at no extra cost. If the VM doesn't meet the criteria, it is assigned a **Standard security type**, ensuring compatibility while maintaining migration readiness. VMs that don't meet Trusted Launch requirements are assigned Standard security type to ensure migration compatibility.

**Confidential Virtual Machine**: For VM's requiring stronger security and compliance, you can also choose **Azure Confidential Virtual Machine (CVM)**, which provide hardware-backed memory encryption and trusted execution environments to help protect data in use. VMs that don't meet Confidential Virtual machine requirements are assigned Standard or Trusted Launch security type to ensure migration compatibility.

For more information on these security types, see [Trusted Launch Virtual Machines](/azure/virtual-machines/trusted-launch) and [Confidential Virtual Machines](/azure/confidential-computing/confidential-vm-overview).
