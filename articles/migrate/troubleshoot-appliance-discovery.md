...
9013: "A new temporary profile is created for every login to the VMware VM" | A new temporary profile is created for every login into the VM | Ensure the user name provided in the guest VM credentials is in UPN format. After changing the user name provided in the guest VM credentials to UPN format, we need to restart "Microsoft Azure VMWare Discovery Service" from the task manager in the appliance to find the new discovery.
