...
UPDATE_NIC1_ID | Specify the ID of the NIC to be updated. If left blank, then the script assumes the value to be the first NIC of the discovered VM. If you do not want to update the NIC of the VM, then leave all the fields containing NIC name blank. 
...
UPDATE_NIC2_ID | Specify the ID of the NIC to be updated. If left blank, then the script assumes the value to be the second NIC of the discovered VM. If you do not want to update the NIC of the VM, then leave all the fields containing NIC name blank.
...
6 | AzMigrate_StartMigration.ps1 | Start the migration for all VMs listed in the csv which are configured for migration. The script creates a CSV output with the job details for each VM.
