...
Create a key vault for VMware agentless migration | To migrate VMware VMs with agentless Azure Migrate Server Migration, Azure Migrate creates a Key Vault to manage access keys to the replication storage account in your subscription. To create the vault, you to set permissions on the resource group in which the Azure Migrate project resides. | [Set up](tutorial-prepare-vmware.md#assign-role-assignment-permissions) permissions.
