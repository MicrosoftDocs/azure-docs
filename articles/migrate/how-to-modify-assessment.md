...
2. Modify the properties in accordance with the following table:
    ...
    **VM series** | The VM series to be considered for right-sizing. For example, if you have a production environment that you do not plan to migrate to A-series VMs in Azure, you can exclude the A-series from the list of VM series and the right-sizing will be done only in the selected series. | By default, all the VM-series applicable to the Azure region are selected.  
