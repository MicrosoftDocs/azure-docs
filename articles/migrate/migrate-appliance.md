...
---|---|---|---
**Start discovery** | The appliance communicates with the vCenter server on TCP port 443 by default. If the vCenter server listens on a different port, you can configure it in the appliance configuration manager. | The appliance communicates with Hyper-V hosts via WinRM port 5986 (HTTPS) for enhanced security. If this port is blocked, it will default to port 5985 (HTTP). | TThe appliance communicates with Windows servers over WinRM port 5986 (HTTPS) as it is more secure and ft the port is blocked, it will fall to 5985 (HTTP), with Linux servers over port 22 (TCP). |
**Gather configuration and performance metadata** | The appliance collects the metadata of servers running on vCenter Server(s) using vSphere APIs by connecting on port 443 (default port) or any other port each vCenter Server listens on. | The appliance gathers metadata from servers operating on Hyper-V hosts through a Common Information Model (CIM) session over port 5986 (HTTPS) for enhanced security. If this port is blocked, it will default to port 5985 (HTTP). | The appliance gathers metadata from Windows servers through a Common Information Model (CIM) session over port 5986 (HTTPS) for enhanced security. If this port is blocked, it will default to port 5985 (HTTP). |
