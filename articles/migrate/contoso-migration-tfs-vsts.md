...
This article shows how Contoso is refactoring its on-premises Team Foundation Server (TFS) deployment by migrating it to Visual Studio Team Services (VSTS) in Azure. Contoso's development team have used TFS for team collaboration and source control for the past five years. Now, the team want to move to a cloud-based solution for dev and test work, and for source control. VSTS will play a role as the team move to a DevOps model, and develop new cloud-native apps.
...
[Article 6: Rehost to Azure VMs and SQL Server Availability Groups](contoso-migration-rehost-vm-sql-ag.md) | Contoso migrates the SmartHotel360 app. Contoso uses Site Recovery to migrate the app VMs. It uses the Database Migration Service to migrate the app database to a SQL Server cluster that's protected by an AlwaysOn availability group. | Available
...
Article 11: Refactor TFS on VSTS | Contoso migrates its on-premises Team Foundation Server deployment to Visual Studio Team Services in Azure. | This article.
...
- The cutover to Git will be a "tip migration" that imports only the latest version of the source code. It will happen during a downtime when all work will be halted as the codebase shifts. The team understands that only the current master branch history will be available after the move.
- The team is concerned about the change and want to test it before doing a full move. The team want to retain access to TFS even after the move to VSTS.
- Contoso has multiple collections, and want to start with one that has only a few projects to better understand the process.
- The team understand that TFS collections are a one-to-one relationship with VSTS accounts, with multiple URLs. However, this matches the current model of separation for code bases and projects.
...
- Contoso will leverage Azure Active Directory, which Contoso set up during deployment of the [Azure infrastructure](contoso-migration-infrastructure.md) at the beginning of migration planning. 
...
5. The import.json file provides import settings. It includes information such as the desired account name, and storage account information. Most of the fields are populated automatically. Some fields required user input. They open the file, and adds the VSTS account name to be created: **contosodevmigration**. With this name, their VSTS URL will be **contosodevmigration.visualstudio.com**.
...
4. **Fill out the import**: They can then fill out missing fields in the import file, including the DACPAC setting. To start with they'll specify that they want to do a **DryRun** import, to check that everything's working properly before the full migration.
...
Contoso admins create a backup (DACPAC) for import into VSTS.
...
8. After the migration finishes a Contoso Dev Leads logs onto VSTS to check that the migration worked properly. After login, the lead can see that projects have been migrated.
...
With migration complete, Contoso wants to move from TFVC to Git for source code management. Contoso admins need to import the source code currently in the VSTS account, as Git repos in the same account.
