...
> [!div class="checklist"]
> * **Step 1: Set up a site-to-site VPN connection**: DMS connects to your on-premises database over a VPN site-to-site connection. Note that we'll create a virtual network for the VPN connection. We'll reuse this network for Site Recovery, so it must be located in the same region in which you will create a Recovery Services vault.
> * **Step 2: Set up SA URI for DMS**: A shared access signature (SAS) Uniform Resource Identifier (URI) provides delegated access to resources in your storage account, so that you can grant limited permissions to storage objects. Set up a SAS URI so that the DMS can access the storage account container to which the service uploads the SQL Server back-up files.
> * **Step 3: Set up a SQL Azure Managed Instance**: You need a pre-created managed instance to which the on-premises SQL Server database will migrate.
> * **Step 4: Prepare DMS**: You register the Database Migration provider, create an instance, and then create a Database Migration Service project.
> * **Step 5: Prepare Azure for Site Recovery**: You create an Azure storage account to hold replicated data, and an Azure virtual network in which Azure VMs will be located when they're created after failover to Azure.
...
To prepare for the VPN site-to-site connection, you set up a virtual network and subnets, create a VPN gateway for the virtual network, set up a local network gateway so that Azure knows how to connect to your on-premises, and make sure your on-premises VPN network can connect. Then you create and verify the site-to-site connection.

### Set up a virtual network

For the Database Migration Service, you need an Azure virtual network to provide site-to-site connectivity to your on-premises SQL Server.
We'll reuse this network when we set up Site Recovery, so make sure that you create it in the same region in which you'll create the Site Recovery vault.
...
10. The virtual network takes a few seconds to create. After it's created, you verify it in the Azure portal dashboard.
11. MAke sure that these communication ports are allowed in your virtual network: 443, 53, 9354, 445, 12000.


### Set up subnets

We have four subnets in our Azure network:

![Subnet list](media/migrate-scenarios-lift-and-shift/vnet-subnet-list.png)

You set up the first subnet (Apps) when you created the network. Now, create the rest of the subnets. The gateway subnet is used by the VPN gateway connection that Azure uses to send traffic over a VPN connection to your on-premises site.

1. In the virtual network, under **Settings**, click **Subnet** > **+Subnet**.
2. Set up a subnet **Data**, with this address range, and then click **OK**.

    ![Data subnet](media/migrate-scenarios-lift-and-shift/vnet-subnet-data.png)

3. Set up a subnet **Identity**, with this address range, and then click **OK**.
    ![Identity subnet](media/migrate-scenarios-lift-and-shift/vnet-subnet-data.png)
...
6. **SKU**: Select the gateway SKU from the dropdown. The SKUs listed in the dropdown depend on the VPN type you select. Don't enable active-active mode. [Learn more](https://docs.microsoft.com/azure/vpn-gateway/vpn-gateway-about-vpn-gateway-settings#gwsku) about SKUs.
7. **Virtual network**: Click **Choose a virtual network**, and add the network you created. The gateway will be added to this network.
8. In **First IP configuration**, specify a public IP address.  A VPN gateway must have a public IP address. This address is dynamically assigned to the resource when the VPN gateway is created. Only dynamic addresses are currently supported but note that the IP address doesn't change after it's assigned.
...
3. In **Create local network gateway** > **Name**, specify a name that Azure will use for your local network gateway object.
...
5. In **Address space**, type in address ranges of your local network that will be routed thorugh the VPN gateway to the on-premises device. Make sure they don't overlap with ranges on the Azure virtual network.
...
Now create the site-to-site VPN connection between your virtual network gateway and your on-premises VPN device.
...
5. The **Virtual network gateway** value is fixed because you are connecting from this gateway.
6. In **Local network gateway**, specify the local network gateway you created.
7. In **Shared Key**, the value here must match the value that you are using for your local on-premises VPN device. The example uses 'abc123', but you can (and should) use something more complex. The important thing is that the value you specify here must be the same value that you specify when configuring your VPN device.
...
5. After the connection is created,  you can view it on the **Connections** page of the virtual network gateway. The status will go from Unknown > Connecting > Succeeded.
...
2. In the left pane, expand the storage account containing the blob container for which you wish to get a SAS. Expand the storage account's **Blob Containers**.
...
5. A second Shared Access Signature dialog displays the blob container along with the URL and QueryStrings you can use to access the storage resource. Select Copy to copy the values. Keep them in a safe place. You need them when you set up DMS.

## Step 3: Prepare a Azure SQL Managed Instance

### Set up a virtual network

For this tutorial we'll use the (ContosoVNET) subnet that we created earlier. Note the following for virtual network requirements for SQL Managed Instance:
...
Locate and then click Route table, and then click Create on the Route table page.
In **Create route table** > **Name**, specify a name for the table.
Select your subscription, resource group, and location
Leave BGP disabled, and click **Create**
AFter the route table has been created, open it, and click **Routes** > **+Add**.
In **Add route** > **Name**, specify a route name.
In **Address prefix**, specify the route.
In **Next hop type**, select ***
In **Next hop address**, select ... Then click **OK**.
...
9. Click **Apply** to save settings, and **Create** to deploy the managed instance. To view deployment status, click **Notifications**, and **Deployment in progress**.


## Step 4: Prepare DMS

You register the Database Migration provider, create an instance, and then create a migration project.
...
3. Specify a name for the service, the subscription, Resource groupth, e virtual network, and the pricing tier.
4. Select **Create** to create the service.

### Create a Database Migration project

1. After the service is created, locate it within the Azure portal and open it.
...
You need an Azure network in which the Azure VMs will be located when they're created after failover, and an Azure storage account in which replicated data is stored. For the purposes of this tutorial, we'll use the Azure network that we created earlier, for DMS. Note that the network you use must be in the same region as the Site Recovery vault.
...
- The Mobility service must be installed on the VM you want to replicate. Site Recovery installs this service automatically when you enable replication for the VM. For automatic installation, you need to prepare an account that Site Recovery will use to access the VM. You'll specify this account when you set up disaster recovery in the Azure console.
...
1. In **Install third-party software**, accept the license agreement. Select **Download and Install** to install MySQL Server.
2. Select **Install VMware PowerCLI**. Make sure all browser windows are closed before you do this. Then select **Continue**.
3. In **Validate appliance configuration**, prerequisites are verified before you continue.
4. In **Configure vCenter Server/vSphere ESXi server**, enter the FQDN or IP address of the vCenter server, or vSphere host, where the VMs you want to replicate are located. Enter the port on which the server is listening. Enter a friendly name to be used for the VMware server in the vault.
5. Enter credentials to be used by the configuration server to connect to the VMware server. Site Recovery uses these credentials to automatically discover VMware VMs that are available for replication. Select **Add**, and then select **Continue**.
    ![vCenter](./media/migrate-scenarios-lift-and-shift/cswiz2.png)
6. In **Configure virtual machine credentials**, enter the user name and password to be used to automatically install Mobility Service on machines, when replication is enabled. For Windows machines, the account needs local administrator privileges on the machines you want to replicate. 
...
Site Recovery checks that you have one or more compatible Azure storage accounts and networks.
   ...
   ![Target tab](./media/migrate-scenarios-lift-and-shift/storage-network.png)
...
To monitor VMs you add, check the last discovered time for VMs in **Configuration Servers** > **Last Contact At**. To add VMs without waiting for the scheduled discovery, highlight the configuration server (don't select it) and select **Refresh**. It can take 15 minutes or longer for changes to take effect and appear in the portal.

7. In **Properties** > **Configure properties**, select the account to be used by the process server to automatically install Mobility Service on the machine. 

    ![Enable replication](./media/migrate-scenarios-lift-and-shift/enable-replication4.png)

8. In **Replication settings** > **Configure replication settings**, verify that the correct replication policy is selected.
9. Select **Enable Replication**.

    ![Enable replication](./media/migrate-scenarios-lift-and-shift/enable-replication5.png)

You can track progress of the **Enable Protection** job in **Settings** > **Jobs** > **Site Recovery Jobs**. After the **Finalize Protection** job runs, the machine is ready for failover, and appears in **Overview** > **Essentials**.
...
1. In the project, click **+New Activity**.
2. In **Choose activity type**, select **Run Migration** > **OK**.
3. Specify the credentials for the source and target servers, and click **Save**.
4. In **Map to target databases**, select the source database you want to migrate. Then click **Save**.
5. In **Configure migration settings** > **Server backup location**, specify the network share you created on the on-premises machine. DMS takes source backups to this share.  Remember that the service account running the source SQL Server instance must have write privileges on this share.
6. Specify the user name and password used by the DMS to upload the backup files to the Azure storage container.
7. Specify the SAS URL that provides the DMS with access to your storage account container. Then click **Save**.
8. In **Migration summary**, specify a name for the migration activity > **Run the migration**.
9. In the project > **Overview**, monitor the migration status. After migration completes, verify that the target databases exist on the managed instance.
...
3. Select the **Latest processed**, to fail over the VM to the latest available point in time. The time stamp is shown. With this option, no time is spent processing data, so it provides a low RTO (recovery time objective).
4. In **Test Failover**, select the target Azure network to which Azure VMs will be connected after failover occurs.
5. Click **OK** to begin the failover. You can track progress bin the vault  > **Settings** > **Jobs** > **Test Failover**.
6. After the failover finishes, the replica Azure VM appears in the Azure portal > **Virtual Machines**. Check that the VM is the appropriate size, that it's connected to the right network, and that it's running. You should now be able to connect to the replicated VM in Azure.
1. To delete Azure VMs created during the test failover, click **Cleanup test failover** on the
...
> * Migrated our on-premises VMs to Azure VMs, with the Site Recovery service.
