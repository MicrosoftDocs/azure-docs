...
- Review the requirements based on your environment and the appliance you are setting up to perform inventory:
...
1. In **Step 3: Provide server credentials to perform software inventory and agentless dependency analysis**, click **Add credentials** to provide multiple server credentials to perform guest-based discovery like software inventory, agentless dependency analysis and discovery of databases and web applications.
...
Dependencies | Netowork dependencies of the server <br/> *Dependency analysis will be auto-enabled on upto 1000 servers per appliance if the validation checks succeed. [Learn more]()*
...
You can choose to view more attributes that are gathered by Azure Migrate as part of the discovery by selecting **Columns** from the command bar on top and choosing the additional attributes that you are interested to review. 
...
The data gathered varies as per the individual workload type. Details of each type of the workload can also be found in inventory views for Infrastructure, Databases and Web apps and can be reviewed below:
...
**Web apps** inventory view helps you review all the discovered ASP.NET and Java web apps running on the discovered servers. The default view will show all web apps, but you can select on **ASP.NET** or **Java** to filter respective web apps by type.  

 > [!Note]
 > If you are not able to see all the webapps discovered ensure that software inventory is completed for all the servers. [Learn more](/azure/migrate/add-server-credentials?view=migrate&branch=release-migrate-new-structure).  
...
You can select the name of a web app to see all the attributes and additional metadata discovered for that workload in a detailed view. Here you can also add tags to an individual server. Here are the details you can review for each web app: 
