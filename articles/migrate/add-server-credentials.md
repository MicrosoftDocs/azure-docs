...
PostgreSQL Authentication credentials | You can PostgreSQL authentication credentials, select the appropriate option from the drop-down in the **Add credentials** dialog. <br/><br/> Enter a friendly name, username, and password. Use these credentials to discover PostgreSQL instances and databases in your environment, provided your PostgreSQL servers are configured for password-based authentication (md5, scram-sha-256, or password). <br/><br/> Esure the PostgreSQL user has the required permissions. To complete the software inventory and discover PostgreSQL installations, provide at least one validated domain credential or one Windows/Linux (non-domain) credential. <br/><br/> After discovery, the appliance uses the PostgreSQL credentials to identify instances and databases.
...
Discovery of PostgreSQL instances and databases |Use a PostgreSQL user account with either superuser privileges provided in the [minimum user privilege]() for each PostgreSQL instance, `CONNECT, pg_read_all_settings, and USAGE` on relevant schemas.| User account with superuser privileges provided in the [minimum user privileges] for each PostgreSQL instance `CONNECT, pg_read_all_settings, USAGE` on schemas
