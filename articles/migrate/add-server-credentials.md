...
**PostgreSQL Authentication credentials** | You can add PostgreSQL authentication credentials, select the appropriate option from the drop-down in the **Add credentials** dialog. <br/><br/> You need to specify a friendly name, username, and password. Use these credentials to discover PostgreSQL instances and databases in your environment, provided your PostgreSQL servers are configured for password-based authentication (md5, scram-sha-256, or password). <br/><br/> You need to provide at least one validated domain or non-domain (Windows/Linux) credential so the appliance can discover PostgreSQL installations. It then uses PostgreSQL credentials to find instances and databases.
