...
- There's no direct download link for Hyper-V Integration Services, so you need to install as follows:
    - For Hyper-V VMs that don't have it installed, you need to extract installation files on a machine running Windows Server 2012 R2/Windows Server 2012 with the Hyper-V role, and then copy the installer to the Windows Server 2003 machine. The file isn't available on machines running Windows Server 2016 with the Hyper-V role.
    - For VMware VMs, you need to create a startup task that installs it when the Azure VM starts after migration.
