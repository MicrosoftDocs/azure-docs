...
Note that the `LoadTextureFromSASAsync` variant is used above because built-in textures are loaded. In case of loading from [linked blob storages](../../how-tos/create-an-account.md#link-storage-accounts), use the `LoadTextureAsync` variant (similar to how it work [for models](../../concepts/models.md#loading-models)).
