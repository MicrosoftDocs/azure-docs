...
1. Press Play in the Unity Editor to load and connect to a remote session. Ensure your **TestModel** is still configured to load into the remote session.
...
4. After connecting the local runtime to a remote session, navigate **AppMenu -> Scene Tools -> Remote Sky** to explore the different sky options and see how they affect the **TestModel**.
...
3. Drag the **RemoteLight** script onto the **DirectionalLight** GameObject.
...
1. If you don't have it added already, add the **AppMenu** prefab to the scene.
