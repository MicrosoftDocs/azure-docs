...
This tutorial assumes that you are familiar with [the previous tutorial](tutorial-2-working-with-remote-entities.md). However, you only need a Unity project with which you can connect to sessions and load a model, as shown in [the first tutorial](tutorial-1-unity-project-setup.md).
...
In the previous tutorial, we used [state override components](../sdk/features-override-hierarchical-state.md) to change the tint color of selected objects. Now we want to achieve a similar effect, but do so by modifying the [material](../sdk/concepts-materials.md) of an object.

First we need a script to pick objects, as we did in [the second tutorial](tutorial-2-working-with-remote-entities.md). If you don't have a **RemoteRaycaster** script yet, create it now. Replace its content with the following code:
...
[Textures](../sdk/concepts-textures.md) are typically part of a source model. During [model conversion](../quickstarts/quickstart-convert-model.md), all textures are converted to the necessary runtime format and packaged into the final model file. To replace a texture at runtime, you need to save it in [DDS file format](https://en.wikipedia.org/wiki/DirectDraw_Surface) and upload it to Azure blob storage. Consult [this quickstart guide](../quickstarts/quickstart-convert-model.md) for how to create an Azure blob container. Once you have a blob container, you can open it in Azure Storage Explorer and upload your file via drag and drop. Finally, right-click on the uploaded file and select **Get Shared Access Signature...**.
...
That concludes our introduction series on how to use Azure Remote Rendering with Unity. As a next step you should familiarize yourself with [the fundamental concepts](../concepts/sdk-concepts.md) of ARR to build a deeper understanding. There are also various features like [lights](../sdk/features-lights.md), [outline rendering](../sdk/features-outlines.md), [hierarchical state overrides](../sdk/features-override-hierarchical-state.md), and [materials](../concepts/materials-overview.md) that you should explore in more detail.

> [!div class="nextstepaction"]
> [Unity SDK concepts](../concepts/sdk-unity-concepts.md)
