...
When loading a model (e.g. via a Unity sample) fails albeit the blob configuration is correct, it is likely that the blob storage is not properly linked. This is explained in [linking of a storage account](../how-tos/create-an-account.md#link-storage-accounts) chapter. Note that after correct linking is can take up to 30min until the changes take effect.
