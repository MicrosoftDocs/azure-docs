...
When running ARR on a device that provides head-tracking data like the HoloLens 2, the head pose is sent to both the user application and the server. The space in which the head transform is defined in we call *stage space*.
...
To inform the server that an additional transform is applied to the stage space, its origin in the form of position and rotation in world space need to be sent over. This setting can be accessed via stage space setting.
