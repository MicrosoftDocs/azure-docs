...
| videoFramesDiscarded | Very similar to **videoFramesSkipped**, but the reason for being discarded is that a frame came in so late that is cannot even be correlated with any pending pose anymore. If this happens, there is some severe network contention.|
