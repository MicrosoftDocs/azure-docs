...
The delay between the remote and local rendering means that changing any of these settings has a delay. Therefore, any new value can't be used immediately the same frame.
...
Sometimes it is helpful to disable the depth buffer write of the remote image for debugging purposes. Disabling depth will also stop the server from sending depth data, reducing bandwidth.
