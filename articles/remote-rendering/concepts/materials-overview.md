...
> [!NOTE] If both `albedoMap` and `albedoColor` are defined, then the final color is a product of both values (per-pixel).
...
> [!NOTE] Since Albedo is a shared property between different types of materials, we use the term Albedo and not Base color.
...
> [!NOTE] A related concept is known as Bump mapping, but a bump map defines a height displacement for each pixel, rather than a normal.
...
> [!NOTE] Azure Remote Rendering assets pipeline supports only FBX 2011 and higher.
