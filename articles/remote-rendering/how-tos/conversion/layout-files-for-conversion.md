...
The request to convert an asset is given two parameters which determine how the conversion service finds these files: The `input.BlobPrefix` (which is optional) and the `input.relativeInputAssetPath`.
...
> [!Note]
> The service will download all files under the input.BlobPrefix. Ensure file names and paths do not exceed [Windows path length limits](https://docs.microsoft.com/en-us/windows/win32/fileio/maximum-file-path-limitation) to avoid issues on the service. 
