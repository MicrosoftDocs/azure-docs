...
1. In the next screen, click the "Create" button. A form opens to create a new Remote Rendering account:
    ...
    1. Select a region from the dropdown where this resource should be created in. See remarks on [Account regions](create-an-account.md#account-regions) below.
...
### Account regions
The region that is specified during account creation time of an account determines which region the account resource is assigned to. This cannot be changed after creation. However, the account can be used to connect to a session in any [supported region](./../reference/regions.md), ideally always the closest one to minimize latency.
