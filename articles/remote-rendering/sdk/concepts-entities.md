...
An entity is uniquely owned by its parent, meaning that when the parent is destroyed with `Entity.Destroy()`, so are its children and all connected components. Removing a model from the scene is accomplished by calling `Destroy` on the model root returned by `AzureSession.Actions.LoadModelAsync()`.
