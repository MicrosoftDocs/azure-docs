...
Users with access to the chamber can export data from the chamber via the data pipeline through a series of steps.

1. Identify file to export. The process is triggered by placing the file to export into a designated area within the chamber. Copying file to data pipeline data out folder can be performed by a Chamber Admin or a Chamber User. The copied file will be detected by the data pipeline and a files object is created. The file creation activity is traceable in the logs, and will enable the next step of the data pipeline.

1. Request file to export. A Chamber Admin can review files in the data pipeline and for data out files, request to export a file from the chamber. This operation triggers the creation of a files request object. The request to export activity is traceable in the logs, and will enable the next step of the data pipeline.

1. Approve/Reject file request. The file request object can only be approved or rejected for export by the Workbench Owner. In addition, the user approving the export can't be the same user who requested to export the data, and must be a Workbench Owner vs Chamber Admin or Chamber User.

1. Download file to export. If a file is approved for export, a download URI can be obtained from the file requests object, and copied out of the chamber using AzCopy. Note the URI has an expiration timestamp, you must complete the download within the expiration time frame, or request a new download URI.
