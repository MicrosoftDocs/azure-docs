...
Azure SignalR Service has some built-in metrics and you and set up [alerts](../azure-monitor/alerts/alerts-overview.md) and [autoscale](./signalr-howto-scale-autoscale.md) base on metrics.
...
There are two kind of dimensions available in some metrics:
...
* ConnectionCloseCategory: Describe the categories of why connection getting closed. Including dimension values:
    ...
    - NoAvailableServerConnection: Client connection can not be established (won't even pass handshake) as no available server connection.
...
The minimum grain of message count showed in metrics is 1, which means 2KB outbound data traffic. If user sending very small amount of messages such as several bytes in a sampling time period, the message count will be 0.

The way to check out small amount of message is using metrics *Outbound Traffic*, which is count by bytes.

### Understand System Errors and User Errors

The Errors are the percentage of failure operations. Operations are consist of connecting, sending message and so on. The difference between System Error and User Error is the former is the failure caused by our internal service error and the latter is caused by users. In normal case, the System Errors should be very low and near to zero.
