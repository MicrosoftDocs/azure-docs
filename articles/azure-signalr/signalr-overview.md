...
Currently, there are two versions of SignalR you can use with your web applications: *SignalR (for ASP.NET)*, and *ASP.NET Core SignalR (SignalR Core)*, which is the newest version. The Azure SignalR Service, also called *SignalR Service*, is an Azure-managed service built on SignalR Core. 
