...
You can view resource logs to get a richer view of connectivity to your Azure SignalR Service instance. The resource logs provide the following detailed information for every connection:

- Basic information such as user ID, connection ID, and transport type
- Event information such as connect, disconnect, and abort event

- For detailed instructions on how to enable, query, and troubleshoot with Azure SignalR Service resource logs, see [Monitor and troubleshoot Azure SignalR Service logs](signalr-howto-diagnostic-logs).
- For the available resource log categories, their associated Log Analytics tables, and the logs schemas for Azure SignalR Service, see [Azure SignalR Service monitoring data reference](monitor-signalr-reference.md).
...
For the available resource log categories, their associated Log Analytics tables, and the logs schemas for Azure SignalR Service, see [Azure SignalR Service monitoring data reference](monitor-signalr-reference.md#resource-logs).

### Resource logs collecting behaviors

There are two typical scenarios for using resource logs, especially for messaging logs.

- Message quality: Log whether the message got sent/received successfully, or record every message that is delivered via SignalR service.
- Performance: Log message latency, or track the message in a few connections instead of all the connections.
...
The following table lists some suggested alert rules for Azure SignalR Service. These alerts are just examples. You can set alerts for any metric, log entry, or activity log entry that's listed in the [Azure SignalR Service monitoring data reference](monitor-signalr-reference.md).
...
- For detailed instructions on how to enable, query, and troubleshoot with Azure SignalR Service logs, see [Monitor and troubleshoot Azure SignalR Service logs](signalr-howto-diagnostic-logs).