...
---|---
*aggregateFunction* |The name of the aggregate function (case insensitive). The following aggregate functions are supported :<p><p> * COUNT<p><p> * MAX<p><p> * MIN<p><p> * SUM<p><p> * AVG<p><p> * STDDEV
...
*Interval* |The time interval in the format: *nnnNAME* Where: nnn is the positive integer number*NAME* is the interval nameSupported interval names include (case sensitive): <p><p> * MILLISECOND[S]<p><p> * SECOND[S]<p><p> * MINUTE[S]<p><p> * HOUR[S]<p><p> * DAY[S]<p><p> * MONTH[S]<p><p> * YEAR[S]
...
SourceSystem|All|Where the data comes from (in terms of &#39;attach&#39; mode to the service -i.e. Operations Manager, Operational Insights (=the data is generated in the cloud), Azure Storage (data coming from WAD) and so on
ObjectName|PerfHourly|Windows perfomance object name
...
ObjectType|ConfigurationObject|Type (as in Operations Manager management pack&#39;s &#39;type&#39;/class) for this object discovered by Operational Insights configuration assessment
...
AlertParameters|ConfigurationAlert|XML with the parameters of the Operational Insights alert
Context|ConfigurationAlert|XML with the &#39;context&#39; of the Operational Insights alert
...
Name|Recommendation, ConfigurationObjectProperty|Name/title of the recommendation, or name of the property from Operational Insights Configuration Assessment
Value|ConfigurationObjectProperty|Value of a property from Operational Insights Configuration	Assessment
KBLink|Recommendation|URL to the KB article that describes this best practice or update
RecommendationStatus|Recommendation|Recommendations are among the few types whose records get &#39;updated&#39;, not just added to the search index. This status changes whether the recommendation is active/open or if Operational Insights detects that it has been resolved
...
SampleCount|PerfHourly|How many &#39;raw&#39; perf counter samples were used to produce this hourly	aggregate record
...
csUriQuery|W3CIISLog|URI query. URI queries are necessary only for dynamic pages, such a ASP pages, so this field usually contains a hyphen for static pages.
