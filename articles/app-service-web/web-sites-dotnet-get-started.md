...
> [AZURE.NOTE] Help us shape the scope and approach of this tutorial -- are there other topics you'd like to see covered in a getting-started tutorial, or is it best to keep this as short as possible and link to other topics for more info? For example, this tutorial could show how to: deploy a sample application that uses a database, debug remotely, use site telemetry and monitoring features of Azure, get application logs in Visual Studio, open files on the remote site in Visual Studio, and more. The getting started tutorials for ASP.NET that the preceding paragraph links to are much longer and have multiple chapters -- would you like to see this getting started with Azure tutorial move in that direction? Please leave feedback in a [Disqus comment](#comments) at the end of the tutorial.
