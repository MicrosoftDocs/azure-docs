<properties 
	pageTitle="How To Create an ILB ASE Using ARM Templates" 
	description="Learn how to create an internal load balancer ASE using ARM templates." 
...
# How To Create an ILB ASE Using ARM Templates

## Overview ##
App Service Environments can be created with a virtual network internal address instead of a public VIP.  This internal address is provided by an Azure component called the internal load balancer (ILB).  An ILB ASE can be created using the Azure portal.  It can also be created using automation by way of ARM templates.  This article walks through the steps and syntax needed to create an ILB ASE with ARM templates.
...
An example ARM template, and its associated parameters file, are available on GitHub [here][quickstartilbasecreate].
...
Once the *azuredeploy.parameters.json* file has been filled in for an ILB ASE, the ILB ASE can then be created using the following Powershell code snippet.  Change the file PATHs to match where the ARM template files are located on your machine.  Also remember to supply your own values for the ARM deployment name, and resource group name.
...
After the ARM template is submitted it will take a few hours for the ILB ASE to be created.  Once the creation completes, the ILB ASE will show up in the portal UX in the list of App Service Environments for the subscription that triggered the ARM deployment.
...
There are a variety of ways to obtain a valid SSL certificate including internal CAs, purchasing a certificate from an external issuer, and using a self-sign certificate.  Regardless of the source of the SSL certificate, the following certificate attributes need to be configured properly:
...
With a valid SSL certificate in hand, two additional preparatory steps are needed.  The SSL certificate needs to be converted/saved as a .pfx file.  The .pfx file needs to include all intermediate and root certificates, and also needs to be secured with a password.

Then the resultant .pfx file needs to be converted into a base64 string because the SSL certificate will be uploaded using an ARM template.  Since ARM templates are text files, the .pfx file needs to be converted into a base64 string so it can be included as a parameter of an ARM template.
...
Once the SSL certificate has been successfully generated and converted to a base64 encoded string, the example ARM template on GitHub for [configuring the default SSL certificate][configuringDefaultSSLCertificate] can be used.
...
- *certificateName*:  A friendly string identifier of your own choosing used to identity the certificate.  The name is used as part of the unique ARM identifier for the *Microsoft.Web/certificates* entity representing the SSL certificate.
...
However, just like apps running on the public multi-tenant service, developers can configure custom host names for individual apps, and then configure SNI SSL certificate bindings for individual apps.  
