---
title: Application FAQs for the Web Apps feature of Azure App Service | Microsoft Docs
...
---
# Availability, performance, and application FAQs for web apps in Azure
...
Numerous factors might contribute to slow app performance. For detailed troubleshooting steps, see [Troubleshoot slow web app performance issues](https://docs.microsoft.com/azure/app-service-web/app-service-web-troubleshoot-performance-degradation).
...
4. To view event logs, next to **eventlog.xml**, select the pencil icon.
...
## When I browse to my app, I see "Error 403 - This web app is stopped." How can I resolve this?
...
For information about quotas and limits, see [App Service limits](https://docs.microsoft.com/azure/azure-subscription-service-limits/#app-service-limits). 
...
## I see the message "Worker Process requested recycle due to 'Percent Memory' limit." How can I address this issue?

The maximum available amount of memory for a 32-bit process (even on a 64-bit OS) is 2 GB. By default, the worker process is set to 32-bit in Azure App Service (for compatibility with legacy web applications).
...
For more information, see [Configure web apps in Azure App Service](https://docs.microsoft.com/azure/app-service-web/web-sites-configure).
...
## ASP.NET Core applications might intermittently stop responding when hosted in App Service. How do I fix this issue?

If you notice your web app intermittently stops responding or you see a message that says "The specified CGI Application encountered an error and the server terminated the process" in your ASP.NET Core 1.0 app hosted in Azure App Service, you might be running into a [known issue](https://github.com/aspnet/KestrelHttpServer/issues/1182).

This issue is fixed in 1.0.2 version of Kestrel. This version is included in the ASP.NET Core 1.0.3 update. To resolve this issue, make sure you update your app dependencies to use Kestrel 1.0.2. Alternatively, you can use one of two workarounds that are described in the blog post [ASP.NET Core 1.0 slow perf issues on Azure App Service web apps](https://blogs.msdn.microsoft.com/waws/2016/12/11/asp-net-core-slow-perf-issues-on-azure-websites).


## I can't find my log files in the file structure of my web app. Where can I find them?

Using the Local Cache feature of App Service affects the folder structure of the LogFiles and Data folders for your App Service instance. When Local Cache is used, subfolders are created in the storage LogFiles and Data folders. The subfolders use the naming pattern "unique identifier" + time stamp. Each subfolder corresponds to a VM instance in which the web app is running or has run.
...
If you are not using Local Cache and are experiencing this issue, please create a support ticket.

## I see the message "An attempt was made to access a socket in a way forbidden by its access permissions." How can I resolve this?

This error typically happens if the *outbound TCP connections* on the VM instance are exhausted. In App Service, limits are enforced on the maximum number of outbound connections that can be made for each VM instance. For more information, see [Cross-VM numerical limits](https://github.com/projectkudu/kudu/wiki/Azure-Web-App-sandbox#cross-vm-numerical-limits).
...
You can also see a blog post about [outgoing connections to Azure websites](http://www.freekpaans.nl/2015/08/starving-outgoing-connections-on-windows-azure-web-sites/).

## I can't remote debug my web app in Visual Studio. How can I address this?
...
For a detailed step-by-step walkthrough that shows you how to debug your web app by using Visual Studio, see [Remote debug your App Service web app](https://blogs.msdn.microsoft.com/benjaminperkins/2016/09/22/remote-debug-your-azure-app-service-web-app/).
