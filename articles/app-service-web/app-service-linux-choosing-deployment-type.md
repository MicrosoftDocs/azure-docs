...
Which method is right for your app? The primary factors to consider are:
 ...
 - **Build requirements**: With [continuous deployment](app-service-continuous-deployment.md), you can get your app up and running on Azure directly from source code. No external build or publication process is required. However, there is a limit to the customizability and availability of build tools within the [Kudu](https://github.com/projectkudu/kudu/wiki) deployment engine. Your app may outgrow Kudu's capabilities as it grows in its dependencies or requirements for custom build logic.
 ...
 - **File permissions**: Permissions of directories and files on the app's storage volume cannot be changed. If your runtime requires permissions of files to be set in a specific way, you may need to deploy your app as a custom image to set the permissions appropriately.
