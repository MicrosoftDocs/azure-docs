...
## How to: set the startup time for your Docker image ##

By default, if your container does not start before 230 seconds, the platform will restart your container. If you your custom Docker image used for your web app starts in more than 230 seconds, you can use the `WEBSITES_CONTAINER_START_TIME_LIMIT` app setting, the value for this setting is in seconds, this will allow the platform keep your container running before restarting it. The default value is 230 seconds, and the max allowed value is 600 seconds.

## How to: unmount the platform provided storage ##

By default, the platform will mount a persistent storage share to the `\home\` directory. If your container image do not need a persistent share, you can disable mounting that storage by setting the `WEBSITES_ENABLE_APP_SERVICE_STORAGE` app setting to `false`. You will still have access to that storage from the scm site, and all Docker logs (if enabled) will be written to the log files generated by the platform.
