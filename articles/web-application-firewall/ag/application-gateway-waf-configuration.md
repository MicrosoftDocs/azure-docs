...
When processing exclusions, WAF engines running CRS 3.2 and above will perform a case sensitive match for all fields other than request header keys. Depending on your application, the names, and values, of your headers, cookies and query args can be case sensitive or insensitive. For WAF engines running CRS 3.1 and below, headers, cookies and query strings are case insensitive. For all WAF engines, regular expressions aren't allowed as selectors.
