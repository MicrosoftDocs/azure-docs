...
A web application delivered by Application Gateway can have only one WAF policy associated with it at a time. However, you can have an Application Gateway configuration without any WAF policies associated with it. If a WAF policy is present, it's replicated to all of our edge locations to ensure consistent security policies across the world.
