...
1. Use the following command to retrieve the URL for Spring Cloud Gateway:
   ...
   You can open the output URL in a browser to explore the updated application. The Log In function will now work, allowing you to add items to the cart and place orders. After signing in, the customer information button will display the signed-in username, as shown in the following screenshot.

   :::image type="content" source="media/quickstart-configure-single-sign-on-enterprise/login-success.png" alt-text="Screenshot of ACME Fitness Store app showing a signed-in user.":::
