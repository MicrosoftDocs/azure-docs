...
* In a new tab, open the [Azure portal](https://ms.portal.azure.com/). 
...
* On the Azure Spring Cloud page, click **+ Add**.
...
* Click the **Diagnostic Setting** tab to open the following dialog.

* You can set **Enable logs** to *yes* or *no* according to your requirements.
...
* In the **Default repository** section, set **URI** to "https://github.com/Azure-Samples/piggymetrics-config".
...
* Open an [Azure Cloud Shell](https://shell.azure.com)  and clone the sample app repository to your local machine.  Here, we first create a temporary directory called `source-code` before cloning the app.
...
* Create the `gateway` application and deploy the JAR file.
...
* Following the same pattern, create the `account-service` and `auth-service` applications and deploy their JAR files.
...
* Select the `gateway` application to show the **Overview** page.

* Select **Assign Endpoint** to assign a public endpoint to gateway. This can take a few minutes.
