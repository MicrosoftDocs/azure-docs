---
title: Azure Spring Cloud Troubleshooting Guide | Microsoft Docs
...
If the situation happens to be that all instances are experiencing high CPU/memory, you need to either scale out the application or scale up the CPU/Memory. For more details, please visit [Scale Applications](spring-cloud-howto-manual-scale.md)
...
- The configuration items have expected values. For more details, refer to [Config Server](spring-cloud-tutorial-config-server.md).
...
* It can contain only lowercase letters, numbers and hyphens. The first character must be a letter. The last character must be a letter or number. The value must be between 2 and 32 characters long.

If you try to provision the _Azure Spring Cloud_ service instance via the ARM template, please visit https://docs.microsoft.com/azure/azure-resource-manager/resource-group-authoring-templates to check the template syntax.
...
First of all, due to known limitation, you cannot upload JAR/source package via the portal or ARM template.
...
However, please note that one _Azure Spring Cloud_ service instance can only trigger one build job for one source package at one time. For more details, please refer to [Deploy an application](spring-cloud-quickstart-deploy-app-portal.md) and [Staging environment guide](spring-cloud-tutorial-staging-environment.md).
