...
1. In the **Before launch** section of the dialog, double click on *Run Maven Goal**.
...
You will have to edit the previous configuration and deploy both the `auth-service` and the `account-service` apps to Azure Spring Cloud.
...
1. This will open the configuration dialog with the options configured for the `gateway` app.
...
1. Build and deploy the `auth-service` from the **Azure/Deploy to Azure Spring Cloud** dialog as for the `gateway` app.
1. Repeat reconfiguration and deployment for the `account-service` app.
