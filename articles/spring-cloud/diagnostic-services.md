...
| **IngressLogs** | [Ingress logs](./ingress-log.md) of all customer's applications, only access logs. |
...
### Show ingress log entries containing specific host

To review log entries that generated by specific host, run the following query:
...
To review log entries for a specific requestId "request_id", run the following query:
