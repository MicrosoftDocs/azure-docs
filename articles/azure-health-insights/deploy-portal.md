...
It is necessary to associate an Azure AI Language resource with the Health Insights resource, to enable the use of Text Analytics for Health by the Health Insights AI models. Any additional costs for the Health Insights resource using the Language resources are calculated and paid on behalf of the Language resource.   
When a Language resource is associated with a Health Insights resource, the following happens in the background, in order to allow the Health Insights resource access to the Language resource:
...
Any of the following will disrupt the required access to the associated Language resource and cause API request failures: 
