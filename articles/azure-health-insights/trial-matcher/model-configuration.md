...
In the configuration below, the model will query trials that are in recruitment status ```recruiting``` or ```not yet recruiting```.
...
- Specifying multiple filter categories results in a trial set that is the combiniations of the sets.
