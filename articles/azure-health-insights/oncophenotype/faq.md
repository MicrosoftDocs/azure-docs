...
N+ means there's involvement of regional lymph nodes without explicitly mentioning the extent of spread. Our models were trained to classify whether or not there's regional lymph node involvement but not the extent of spread and hence we don't have ```N1, N2, N3``` inference values. We plan to support these values in the long term.
...
Yes, for clinical/pathologic TNM categories. For tumor site and histology, the values are only as exhaustive as the training data set labels. We plan to publish this set of labels soon.
