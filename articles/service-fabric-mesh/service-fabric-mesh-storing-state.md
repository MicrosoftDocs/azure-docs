...
Reliable Collections enable you to write highly available, scalable, and low-latency cloud applications. You can add a library to your service which implements data structures like queues and key-value pairs. Reliable collections provides the easiest and fastest way to interact with data, while providing easy partition routing in combination with Intelligent Routing in Service Fabric Mesh. Service state is kept locally in the service instance while also being replicated to provide high availability. This means that:
...
* When a service crashes or fails over, it starts up again on a node that already has your data on disk.  Fail over is extremely fast.
...
[image1]: ./media/service-fabric-mesh-storing-state/volumes.png