...
A Gateway resource is used to handle incoming traffic to the network that houses your application needs to be routed. You can configure it to specify rules through which requests are directed to specific services or endpoints based on the structure of the request. See [Introduction to networking in Service Fabric Mesh](service-fabric-mesh-networks-and-gateways.md) for more information on networks and gateways in Mesh. 
...
Since the Gateway resource serves as a bridge between your application's network and the underlying infrastructure's network (the `open` network). You should only need to configure one (in the Mesh preview, you are limited to one gateway per app). The declaration for the resource consists of two main parts: resource metadata and the properties. 

### Gateway resource metadata

Each gateway needs to be declared with the following:
...
Here's how this could look in your ARM (JSON) deployment template: 
...
Each gateway needs a specified `sourceNetwork` and `destinationNetwork`. The source network is defined as the network from which your app will receive inbound requests. Its name property should always be set to "Open". The destination network is the network in which your app lives. The name value for this should be set to the name of your app's local network (this should include the full reference to the resource). See below for a sample config of what this looks like for a deployment in a network called "myNetwork."
...
Routing rules are specific per port in an array of rules within the properties section of your gateway config. 
...
* `destination` - endpoint specification which includes the `applicationName`, `serviceName`, and `endpointName`, for where the requests need to be routed to
...
* `name` - reference to the rule that can be any string of your choice 
* `port` - port to listen on for incoming requests 
* `hosts` - an array of policies that apply to requests coming to the various "hosts" on the port specified above. Hosts in this case is the set of applications and services that may be running in the network and can serve incoming requests, i.e. a web app. Host policies are interpreted in order, so you should create the following in descending levels of specificity
    ...
    * `routes` - an array of policies for this specific host
        * `match` - specification of the incoming request structure for this rule to apply, based on a `path`
            * `path` - contains a `value` (incoming request structure), `rewrite` (how you want the request to be forwarded), and a `type` (can currently only be "Prefix")
        * `destination` - if the request matches, it will be routed to the this destination, which is specified using an `applicationName`, `serviceName`, and `endpointName`
...
This gateway is configured for a Linux application, "meshAppLinux", that consits of at least two services, "helloWorldService" and "counterService", which listens on port 80. Depending on the URL structure of the incoming request, it will route the request to one of these services. 
