...
|**Policy control**     |Fusion Core applies end to end policy control decisions and enforcement on a per PDU session and per flow basis. This provides the flexibility you need to apply different QoS levels to particular flows or UEs, as well as allowing you to selectively expose services or data networks to groups of UEs and controlling the types of traffic flowing over the network.         |
