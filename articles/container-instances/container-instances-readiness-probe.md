...
This article explains how to deploy a container group that includes a readiness probe, demonstrating the automatic restart of a simulated unhealthy container.

## YAML deployment

Create a `readiness-probe.yaml` file with the following snippet. This file defines a container group that consists of an NGNIX container that eventually becomes unhealthy.
...
This deployment defines a `readinessProbe` which supports an `exec` liveness command that acts as the readiness check. If this command exits with a non-zero value, the container will be killed and restarted, signaling the `healthy` file could not be found. If this command exits successfully with exit code 0, no action will be taken.
...
<!-- IMAGES -->
[portal-unhealthy]: ./media/container-instances-readiness-probe/unhealthy-killing.png
[portal-restart]: ./media/container-instances-readiness-probe/portal-restart.png
