<properties
	pageTitle="Working with the Azure App Service Mobile Apps .NET client library | Microsoft Azure"
	description="Learn how to use the .NET client library for Azure Mobile Apps."
...
This guide shows you how to perform common scenarios in Windows apps using a .NET client for Azure App Service Mobile Apps. The scenarios covered include querying for data, inserting, updating, and deleting data, authenticating users, and handling errors. If you are new to Azure App Service, you should consider first completing the [Azure Mobile Apps quickstart](app-service-mobile-dotnet-backend-windows-store-dotnet-get-started.md) tutorial.

>[AZURE>NOTE] Although the .NET client can be used in Universal Windows Platform (UWP) apps, Windows 10 is not currently a supported platform for the preview Mobile Apps service. 

##<a name="setup"></a>Setup and Prerequisites

This topic assumes you have created a Mobile App backend project that has a table, which in our examples is named `TodoItem` and has the following columns: `Id`, `Text`, and `Complete`. 
