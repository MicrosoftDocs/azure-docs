...
1. Select **Next: Networking** > **Next: Review + create** >**Create On Cloud Shell**.
1. The Cloud Shell is launched and executes the commands to create a connection. You might need to confirm some configuration changes during the command processing. Once the command runs successfully, it shows connection information, and you can select the refresh button in the **Service Connector** pane to show the latest result.
...
> [!WARNING]
> Microsoft recommends that you use the most secure authentication flow available. The authentication flow described in this procedure requires a high degree of trust in the application, and carries risks that aren't present in other flows. You should only use this flow when other more secure flows, such as managed identities, aren't viable. Select the authentication method *[Workload ID (Recommended)](tutorial-python-aks-sql-database-connection-string.md?pivots=workload-id#create-a-new-connection)*.
...
1. Select **Next: Authentication**, On the **Authentication** tab, enter your database username and password.
1. Select **Next: Networking** > **Next: Review + create** >**Create**.
...
1. To set the connection secrets as environment variables in your container, you have two options:
   ...
   - Directly create a deployment using the provided YAML snippet. This sample code snippet includes highlighted sections that show you the secret object injected as the environment variables. Select **Apply** to proceed with this method.
