...
1. Get the connection string generated by Service Connector and add passwordless authentication plugin. This connection string will be referenced in the application.

    ```azurecli-interactive
    AZURE_POSTGRESQL_CONNECTIONSTRING=$(az webapp config appsettings list --resource-group $RESOURCE_GROUP --name $APPSERVICE_NAME | jq -c '.[] | select ( .name == "AZURE_MYSQL_CONNECTIONSTRING" ) | .value' | sed 's/"//g')
