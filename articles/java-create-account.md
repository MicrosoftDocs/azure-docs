<properties pageTitle="Java create account" description="Create an account on Azure." documentationCenter="java" services="" manager="wpickett" editor="mollybos" authors="rmcmurray"/>

<tags ms.service="multiple" ms.workload="na" ms.tgt_pltfrm="na" ms.devlang="Java" ms.topic="article" ms.date="09/25/2014" ms.author="robmcm"/>

[AZURE.INCLUDE [create-an-azure-account](../includes/create-an-azure-account.md)]
