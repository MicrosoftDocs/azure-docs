...
- In Azure CycleCloud Slurm clusters, the `start-services.sh` script did not always exit with a non-zero exit code — potentially leaving services in a failed state rather than retrying as expected.
...
- The Support dialog details text box did not always expand to fill the available space.
