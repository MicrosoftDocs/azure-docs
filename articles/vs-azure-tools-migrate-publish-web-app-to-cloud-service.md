...
To take advantage of the hosting services and scalability of Azure, you might want to migrate and deploy your web application to an Azure cloud service. Only minimal changes are required. Note that this topic covers deploying to cloud services only; for App Service, see [Deploy a web app in Azure App Service](app-service/app-service-deploy-local-git.md).
...
1. Visual Studio also sets the **Copy Local** property to true for any assemblies that are required for MVC 2, MVC 3, MVC 4, and Silverlight Business Applications. This adds these assemblies to the service package that is used for deployment.
...
1. (Optional) To cancel the deployment process, right-click the line item in the activity log and choose **Cancel and remove**. This stops the deployment process and deletes the deployment environment from Azure. Note: to remove this deployment environment after it has been deployed, you must use the [Azure portal](https://portal.azure.com).
...
All application types can be started in a browser connected to the Visual Studio debugger by selecting **Debug > Start Debugging** (F5). Note that with an ASP.NET Empty Web Application project, you must first add an `.aspx` page in your application and set it as the start page for your web project.
...
Applications that can migrated and published to cloud services must use one of the templates in the following table. ASP.NET Core is not supported.
