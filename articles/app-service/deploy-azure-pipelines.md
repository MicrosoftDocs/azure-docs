...
Use [Azure Pipelines](/azure/devops/pipelines/) to automatically deploy your web app to [Azure App Service](./overview.md) on every successful build. Azure Pipelines lets you build, test, and deploy with continuous integration and continuous delivery (CI/CD) using [Azure DevOps](/azure/devops/).
...
- The ability to run pipelines on Microsoft-hosted agents. You can either purchase a [parallel job](/azure/devops/pipelines/licensing/concurrent-jobs) or you can request a free tier.
...
1. Create a [release pipeline](/azure/devops/pipelines/release/) by selecting **Releases** from the service menu and select **New pipeline**.
1. 
1. Select the **Azure App Service deployment** template for your stage, which automatically adds the necessary tasks.

    > [!NOTE]
    > If you're deploying a Node.js app to App Service on Windows, select the **Deployed Node.js App to Azure App Service** template. The only difference between these templates is that Node.js template configures the task to generate a `web.config` file containing a parameter that starts the `iisnode` service.
...
The Azure App Service deploy task `AzureRmWebAppDeployment` can deploy to App Service using Web Deploy.
...
> [!NOTE]
> With the [`AzureRmWebAppDeployment@3`](/azure/devops/pipelines/tasks/reference/azure-rm-web-app-deployment-v3) and [`AzureRmWebAppDeployment@4`](/azure/devops/pipelines/tasks/reference/azure-rm-web-app-deployment-v4) tasks, you should use the **Azure Resource Manager** connection type, or `AzureRM`, when deploying with **Web Deploy**. It uses publishing profiles for deployment when basic authentication is enabled for your app. When [basic authentication is disabled](configure-basic-auth-disable.md), it uses the more secure Microsoft Entra ID authentication.
