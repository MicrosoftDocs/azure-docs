...
description: Learn about the different authentication options available for web apps or web APIs hosted on App Service. This article provides recommendations on which auth solutions can be used for specific scenarios such as quickly and simply limiting access to your web app, custom authorization, and incremental consent. Learn about the benefits and drawbacks of using built-in authentication versus code implementation of authentication.
author: cephalin
ms.author: cephalin
...
If you have a web app or an API running in Azure App Service, you can restrict access to it based on the identity of the users or applications that request it. App Service offers several authentication solutions to help you achieve this goal. In this article, you'll learn about the different authentication options, their benefits and drawbacks, and which authentication solution to use for specific scenarios.  
...
The following table lists authentication scenarios and the authentication solutions you would use.
...
| Your app needs to handle the access token expiring without making the user sign in again (use a refresh token)? | ✅ | ✅ | ✅ |
...
- [Add sign-in with Microsoft to a web app](/entra/identity-platform/quickstart-web-app-sign-in)
- [Only allow authenticated user to access a web API](/entra/identity-platform/scenario-protected-web-api-app-configuration)
- [Sign in users to a single-page application (SPA)](/entra/identity-platform/scenario-spa-app-configuration)

To get started with [Microsoft.Identity.Web](/entra/msal/dotnet/microsoft-identity-web/), read:

- [Sign in users to a web app](/entra/identity-platform/quickstart-web-app-sign-in)
- [Protect a web API](/entra/identity-platform/scenario-protected-web-api-app-configuration)
- [Sign in users to a Blazor Server app](/entra/identity-platform/quickstart-single-page-app-sign-in)
