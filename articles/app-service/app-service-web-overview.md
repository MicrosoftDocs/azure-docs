...
*Azure App Service Web Apps* (or just Web Apps) is a service for hosting web applications, REST APIs, and mobile back ends. You can develop in your favorite language, be it .NET, .NET Core, Java, Ruby, Node.js, PHP, or Python. You can run and scale it with ease on Windows or Linux VM (see [App Service on Linux](containers/app-service-linux-intro.md)), depending on your preference. 

Web Apps not only adds the power of Microsoft Azure to your application or service, such as security, load balancing, autoscaling, and automated management. It also increases your DevOps capabilities, such as continuous deployment from VSTS, GitHub, Docker Hub, and other sources, package management, staging environments, domain and SSL certificates, and more. 

Web Apps is charged based the Azure compute resources you use, and the compute resources you use is determined by the _App Service plan_ that you run your Web Apps on. For more information, see [App Service plans in Azure Web Apps](azure-web-sites-web-hosting-plans-in-depth-overview.md).
...
* **Multiple languages and frameworks** - Web Apps has first-class support for .NET, .NET Core, Java, Ruby, Node.js, PHP, or Python. You can also run [PowerShell and other scripts or executables](web-sites-create-web-jobs.md) as background services.
...
* **API and mobile features** - Web Apps provides turn-key CORS support for RESTful API scenarios, and simplifies mobile app scenarios by enabling authentication, offline data sync, and push notifications.
