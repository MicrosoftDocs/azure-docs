...
On June 30th 2022 Debian 9 (also known as "Stretch") will reach EOL status. As of June 2022, a platform update is rolling out to provide an upgrade path to Debian 11 (also known as "Bullseye"). The runtimes listed below are currently using Debian 9; if you are using one of the listed runtimes, follow the instructions below to upgrade your site to Bullseye.
...
2. Under "Environment Variables", search for `PLATFORM_VERSION`. The value of this environment variable is the current platform version of your web app. 
3. If the value of `PLATFORM_VERSION` starts with "99" or greater, then your site is on the latest platform update and you can continue to step 4. If the value does **not** show "99" or greater, then your site has not yet received the latest platform update--please check again at a later date.
4. To upgrade to Debian 11 (Bullseye), create an app setting on your site named `APPSETTING_DEFAULT_OS` with a value of `bullseye`. Note that this will restart your web app.

    ```bash
    az webapp config appsettings set -g MyResourceGroup -n MyUniqueApp --settings APPSETTING_DEFAULT_OS=bullse
    ```
