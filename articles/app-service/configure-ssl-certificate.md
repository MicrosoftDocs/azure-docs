...
### Authorizing Web App resource provider read access to KV
By default, the Web App resource provider doesn’t have access to the Key Vault. In order to use a Key Vault for a certificate deployment, you need to [authorize the resource provider read access to the KeyVault](https://docs.microsoft.com/azure/key-vault/general/group-permissions-for-apps#grant-access-to-your-key-vault). 

```abfa0a7c-a6b6-4736-8310-5855508787cd``` is the resource provider service principal name and it remains same for all Azure subscriptions. For Azure Government cloud environment, you will need to use ```6a02c803-dafd-4136-b4c3-5a6f318b4714``` as the resource provider service principal name. 

### Importing a certificate from Key Vault to your web app
