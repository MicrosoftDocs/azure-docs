...
> [!NOTE]
> If your app requires credentials such as cookies or authentication tokens to be sent, the browser may require the `ACCESS-CONTROL-ALLOW-CREDENTIALS` header on the response. To enable this in App Service, set `supportCredentials` to `true` in your CORS config using [Resource Explorer](https://resources.azure.com).
