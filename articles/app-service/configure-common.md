...
- **Platform settings**: Lets you configure settings for the hosting platform, including:
    ...
    - **Always On**: Keeps the app loaded even when there's no traffic. When it's not turned on (default), the app is unloaded after 20 minutes without any incoming requests. The unloaded state can cause high latency for new requests because of the time it takes the app to warm up. When it's turned on, the front-end load balancer sends a GET request to the application root every five minutes. The continuous ping prevents the app from being unloaded.
