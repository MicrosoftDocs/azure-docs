...
| Secure application code | Follow the below steps to ensure static content of the web app is secured.
|  Static Content | Ensure that the web application doesn’t serve out source files or binary files as static content. Follow application runtime/framework’s best practices to ensure that only the files meant to be served as static content is served out as static files.
| Hidden Folders | Ensure hidden folders like .git, bin, obj, objd, etc., doesn’t get accidently included as part of deployment code payload. Take adequate steps to ensure deployment scripts only deploy required code and nothing more. 
| In place deployments | Understand nuances of In place deployments in Local Git. In place deployments can result in the creation and storage of the .git folder in the content root of the web application which may result in these files being served out as static content. Follow application runtime/framework’s best practices to ensure that the static content is shared only from the designated static content folders of the app, and not others.
| Local Git | Local Git can activate in-place deployments automatically in some scenarios, even if in-place deployment wasn’t explicitly selected. This can happen if the web app wasn’t empty (had content previously deployed) when the Local Git repository was initialized. Follow application runtime/framework’s best practices to ensure that the static content is shared only from the designated static content folders of the app, and not others.
