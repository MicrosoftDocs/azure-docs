...
| Hidden Folders | Ensure hidden folders like .git, bin, obj, objd, etc., doesn’t get accidently included as part of deployment artifact. Take adequate steps to ensure deployment scripts only deploy required files and nothing more.
| In place deployments | Understand nuances of [in place deployments](https://github.com/projectkudu/kudu/wiki/Deploying-inplace-and-without-repository/#inplace-deployment) in Local Git. In place deployments results in the creation and storage of the .git folder in the content root of the web application. Follow application runtime/framework’s best practices to secure the content.
| Local Git | Local Git can activate [in-place deployments](https://github.com/projectkudu/kudu/wiki/Deploying-inplace-and-without-repository/#inplace-deployment) automatically in some scenarios, even if in-place deployment wasn’t explicitly selected. This can happen if the web app wasn’t empty (had content previously deployed) when the Local Git repository was initialized. Follow application runtime/framework’s best practices to securet the content.
