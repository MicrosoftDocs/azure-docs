---
title: Configure deployment credentials for Azure App Service | Microsoft Docs
...
of the app's [resource page](../azure-resource-manager/resource-group-portal.md#manage-resources)). When a user is granted app access via Role Based Access Control (RBAC) or co-admin permissions, that user can use his/her own user-level credentials until the access is revoked. These deployment credentials should not be shared with other Azure users.

* **App-level credentials**: one set of credentials for each app. It can be used to deploy to that app only. The credentials for each app is generated automatically at app creation. These credentials can't be configured manually, but can be reset anytime. For a user to be granted access to app-level credentials via Role Based Access Control (RBAC), that user must be contributor or higher on the app. Readers are not allowed to publish, and can't access those credentials.
