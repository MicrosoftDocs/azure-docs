...
In some cases, your app needs to get data that only the signed-in user can access. Grant delegated permissions to your app to connect to ressources using the identity of the signed-in user.
...
[use Key Vault to store your secrets](app-service-key-vault-references.md) can be used to securely store secrets and keys, monitor access and use of secrets, and simplify admistration of application secrets.  If your app's downstream service doesn't support Microsoft Entra authentication or requires a connection string or key, use Key Vault to store your secrets and connect your app to Key Vault with a managed identity and retrieve the secrets. 
