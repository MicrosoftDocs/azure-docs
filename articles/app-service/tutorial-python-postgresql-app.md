...
Sample Python applications using the Flask and Django framework are provided to help you follow along with this tutorial. To deploy them without running them locally, skip this section and go directly to [Create App Service and PostgreSQL](#1-create-app-service-and-postgresql-1). 
...
In this step, you create the Azure resources and deploy a sample app to App Service on Linux using [Azure Developer CLI](/developer/azure-developer-cli/overview) in the [Cloud Shell](https://learn.microsoft.com/azure/cloud-shell/overview). The steps used in this tutorial create a set of secure-by-default resources that include App Service and Azure Database for PostgreSQL.

### Initialize the project

1. In the Cloud Shell, clone to sample app.

```bash
git clone https://github.com/cephalin/msdocs-flask-postgresql
```
...
1. Run the `azd init --template` command to initialize the sample app.

```bash
azd init --template cephalin/msdocs-django-postgresql-sample-infra
```
    You get prompted for an environment name, which sets the prefix for the resource group that will be created to hold the Azure resources.
...
1. Open the Environment variables page. See AZURE_POSTGRESQL_CONNECTIONSTRING. this is the connection string created for you.

1. (Currently SECRET_KEY is configured already. ideally we need to take it out of the template and lead users to create it)

1. In the cloudshell directory, run vscode: 

```bash
code . 
```

1. In azureproject/production.py, uncomment lines 12-20 (this is where the connection string is shown to be used)

1. In the cloudshell, re-deploy the app:

```bash
azd deploy
```

1. Go to https://<app-name>.scm.azurewebsites.net/webssh/host

1. In the terminal, run the database update command:

```bash
flask db upgrade
```

1. Go to https://<app-name>.azurewebsites.net
