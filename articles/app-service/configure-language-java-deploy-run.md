...
Set the app setting `JAVA_COPY_ALL` to `true` to copy your app contents to the local worker from the shared file system. This setting helps address file-locking issues. Note: JAVA_COPY_ALL is not compatible with the legacy convention of deploying to "home\site\wwwroot\webapps".
