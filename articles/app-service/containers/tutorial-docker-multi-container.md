---
title: Create a multi-container (preview) app on Web App for Containers - Azure | Microsoft Docs
...
To showcase the Multi-container feature, first we will create an app using a default runtime stack. Once finished, we will deploy a multi-container configuration to it.
...
## Modifying the Docker Compose sample

Depends_on needs to be removed from our configuration as it's not supported. In addition, if we want to persist the data, we need to map the container directory to our network storage. We can utilize *WEBAPP_STORAGE_HOME* variable to access this network directory.

The volumes option maps the file system to directories within the container.  
