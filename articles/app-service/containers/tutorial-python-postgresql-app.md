...
- [Start the Postgres server](https://www.postgresql.org/docs/12/server-start.html).

> [!TIP]
> If you have [Docker Desktop](https://www.docker.com/products/docker-desktop), you can run a Docker container with all the prerequisites preinstalled, by just running the following command:
>
> ```
> docker run -it -p 8000:8000 cephalin/py-postgres-az /bin/bash
> ```
...
When prompted, use the password you specified for the `postgres` user during installation.
...
If your connection is successful, your PostgreSQL database is running. If not, make sure that your local PostgresQL database is started.
...
Go to *http:\//localhost:8000* again to see the poll question and answer the question. The local Django sample application writes and stores user data to a local Postgres database, so you don't need to worry about messing up your production database.
