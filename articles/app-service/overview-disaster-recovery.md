...
Most organizations have a business continuity plan to maintain availability of their applications during downtime and the preservation of their data in a disaster. This article covers common strategies for web apps deployed to App Service.
...
- During a disaster, one of the regions becomes offline, and Azure Front Door routes traffic exclusively to the region that remains online.
...
- To save cost, the secondary App Service plan is configured to have fewer instances and/or be in a lower pricing tier. There are three possible approaches:
    - **Preferred**  The secondary App Service plan has the same pricing tier as the primary, but fewer instances. This approach ensures parity in both feature and VM sizing for the two App Service plans.
...
- The network volume triggers preconfigured autoscale rules to scale out the secondary web app.
...
- It's highly recommended that you have a comprehensive playbook outlining all the steps required to restore your web app backup to another Azure Region.
