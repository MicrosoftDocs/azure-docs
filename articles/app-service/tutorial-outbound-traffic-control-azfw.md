...
# Tutorial: App Service outbound traffic control with Azure Firewall 

Azure App Service network traffic control has two aspects, namely inbound traffic and outbound traffic. Inbound is the traffic of users accessing your App Service, and outbound is the traffic of your App Service accessing other network resources. For the detail network concepts and security enhancements, refer to [Networking features - Azure App Service](networking-features.md) and [Zero to Hero with App Service, Part 6: Securing your web app - Azure App Service](https://azure.github.io/AppService/2020/08/14/zero_to_hero_pt6.html).

In most companies, it's required to control the traffic of App Services access to external resources, prevent data exfiltration or the risk of malicious program implantation.
...
When regional virtual network integration is enabled, the **Route All** setting is enabled by default. In the earlier version of App Service, it can be set with an application setting `WEBSITE_VNET_ROUTE_ALL` = `1`. You can also use the **az cli** to enable the property. When all traffic routing is enabled, all outbound traffic is sent into your virtual network. If all traffic routing is not enabled, only private traffic (RFC1918) and service endpoints configured on the integration subnet will be sent into the virtual network and outbound traffic to the internet will go through the same channels as normal.
...
* Get the private IP address of Azure Firewall
    ...
    After Azure Firewall created, get the private IP address in the overview. The private IP address will be used as next hop address in the routing rule.
...
## Integrate Azure Firewall with route table (UDR) to traffic
...
To forward App Service outbound traffic to Azure Firewall, add a custom routing table (UDR) and associate to the App Service subnet.
...
To control App Service outbound traffic on Azure Firewall, add an application rule to the Azure Firewall policy.
