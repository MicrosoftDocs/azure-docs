...
In a zone redundant ASE, your apps spread across three zones in the same region. The zone redundant ASE is avilable in a subset of ASE capable regions primarily limited by the regions that support availability zones. When you have zone redundant ASE, the smallest size for your App Service plan is three instances. That ensures that there is an instance in each availability zone. You can scale up from there one instance or more. A zone redundant ASE has triple the infrastructure and is comprised of zone redundant components so that if even two of the three zones go down for whatever reason, your workloads remain available albeit at reduced capacity. Due to the increased system need, the minimum charge for a zone redundant ASE is nine instances. This means that if you have less than nine total App Service plan instances in your ASEv3, the difference will be charged as Windows I1v2. If you have nine or more intances, there is no added charge to have a zone redundant ASE. To learn more about zone redundancy, read [Regions and Availability zones][AZoverview].

In a host group deployment, your apps are deployed onto a dedicated host group. This dedicated host group is not zone redundant. The primary purpose this solves is having dedicated hardware utilized by your ASE. There is no minimum instance charge for use of an ASE on a dedicated host group but you do have to pay for the host group when provisioning the ASE. On top of that you pay a discounted App Service plan rate as you create your plans and scale out. There are a finite number of cores available with a dedicated host deployment that are used by both the App Service plans and the infrastructure roles. This prevents reaching the 200 total instance count normally available in an ASE. The exact number of total instances possible are related to the load on the system which drives the number of roles automatically added. 
