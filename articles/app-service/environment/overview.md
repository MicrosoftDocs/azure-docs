...
# Introduction to the App Service Environment V3 (Preview)
...
There are a number of networking features that enable apps in the multi-tenant App Service to reach network isolated resources or be network isolated themselves. These features are enabled at the application level.  With an ASE, there is no additional configuration on the apps themselves. The apps are deployed into a network isolated environment. On top of the ASE hosting network isolated apps, it is also a single-tenant system. There are no other customers using the ASE. If you really need a complete isolation story, you can also get your ASE deployed onto dedicated hardware. Between network isolated application hosting, single tenancy, and the ability 
