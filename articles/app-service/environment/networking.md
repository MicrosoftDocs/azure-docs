...
App Service Environment is a single-tenant deployment of Azure App Service that hosts Windows and Linux containers, web apps, API apps, logic apps, and function apps. When you install an environment, you pick the Azure virtual network that you want it to be deployed in. All of the inbound and outbound application traffic is inside the virtual network you specify. You deploy the environment into a single subnet in your virtual network, and nothing else can be deployed into that subnet.
...
The size of the subnet can affect the scaling limits of the App Service plan instances within the environment. It's a good idea to use a `/24` address space (256 addresses) for your subnet, to ensure enough addresses to support production scale.
...
- If you run out of addresses within your subnet, you can be restricted from scaling out your App Service plans in the environment. Another possibility is that you can experience increased latency during intensive traffic load, if Microsoft isn't able to scale the supporting infrastructure.
...
|--------------|-------------|
| App Service Environment virtual network | The virtual network the environment is deployed into. |
| App Service Environment subnet | The subnet that the environment is deployed into. |
| Domain suffix | The domain suffix that is used by the apps made in this environment. |
| Virtual IP (VIP) | The VIP type used by the environment. The two possible values are internal and external. |
| Inbound address | The inbound address is the address at which your apps on this environment are reached. If you have an internal VIP, it's an address in your App Service Environment subnet. If the address is external, it's a public-facing address. |
| Default outbound addresses | The apps in this environment use this address, by default, when making outbound calls to the internet. |

You can find details on the addresses used by the environment in the **IP Addresses** portion of the portal, as shown in the following screenshot:

![Screenshot that shows details about IP addresses.](./media/networking/networking-ip-addresses.png)

As you scale your App Service plans in your environment, you'll use more addresses out of your subnet. The number of addresses you use varies, based on the number of App Service plan instances you have, and how much traffic your environment is receiving. Apps in the App Service Environment don't have dedicated addresses in the subnet. The specific addresses used by an app in the subnet will change over time.

## Ports and network restrictions

For your app to receive traffic, ensure that inbound network security group (NSG) rules allow the App Service Environment subnet to receive traffic from the required ports. In addition to any ports you'd like to receive traffic on, you should ensure that Azure Load Balancer is able to connect to the environment subnet on port 80. This is used for health checks of the internal virtual machine. You can still control port 80 traffic from the virtual network to your environment subnet.
...
You can put your Azure Web Application Firewall devices, such as Azure Application Gateway, in front of inbound traffic to your environment. Doing so exposes specific apps on that environment. If you want to customize the outbound address of your applications on an App Service Environment, you can add a NAT gateway to your environment subnet.
...
### DNS configuration to your environment

If your App Service Environment is made with an external VIP, your apps are automatically put into public DNS. If your environment is made with an internal VIP, you might need to configure DNS for it. When you created your environment, if you selected having Azure DNS private zones configured automatically, then DNS is configured in your virtual network. If you chose to configure DNS manually, you need to either use your own DNS server or configure Azure DNS private zones. To find the inbound address of your environment, go to the App Service Environment portal, and select **IP Addresses**. 
...
### DNS configuration from your environment
