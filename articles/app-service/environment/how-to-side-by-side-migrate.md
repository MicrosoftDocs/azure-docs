...
An App Service Environment v1 and v2 can be automatically migrated to an [App Service Environment v3](overview.md) using the side by side migration feature. To learn more about the migration process using this feature and to see if your App Service Environment supports migration at this time, see [Migration to App Service Environment v3 using the side by side migration-feature](side-by-side-migrate.md).
...
Select a subnet in your App Service Environment v3 that meets the [subnet requirements for App Service Environment v3](./overview.md#subnet-requirements). Note the name of the subnet you select. You need this name for the next step.
