...
End-to-end (E2E) TLS encryption ensures that front-end to worker communication within App Service is encrypted via TLS. Without this feature, although incoming HTTPS requests are encrypted to the front ends, the traffic from front ends to workers running the application workloads travel unencrypted inside the Azure infrastructure.
...
- **App Service managed certificates** (Free)  
  ...
  - Don't support wildcard or custom root CAs.
