---
title: Integrate apps with Azure Virtual Network
...
This article describes the Azure App Service virtual network (VNet) Integration feature and how to set it up with apps in [Azure App Service](https://go.microsoft.com/fwlink/?LinkId=529714). With [Azure Virtual Network][VNETOverview], you can place many of your Azure resources in a non-internet-routable network.
...
1. The drop-down list contains all of the Azure Resource Manager virtual networks in your subscription in the same region. Underneath that is a list of the Resource Manager virtual networks in all other regions. Select the virtual network you want to integrate with.

   ![Select the virtual network][2]

   * If the virtual network is in the same region, either create a new subnet or select an empty preexisting subnet.
   * To select a virtual network in another region, you must have a Virtual Network gateway provisioned with point to site enabled.
   * To integrate with a classic virtual network, instead of selecting the **Virtual Network** drop-down list, select **Click here to connect to a Classic VNet**. Select the classic virtual network you want. The target virtual network must already have a Virtual Network gateway provisioned with point-to-site enabled.
...
When regional VNet Integration is enabled, your app makes outbound calls to the internet through the same channels as normal. The outbound addresses that are listed in the app properties portal are the addresses still used by your app. What changes for your app are the calls to service endpoint secured services, or RFC 1918 addresses go into your virtual network. If WEBSITE_VNET_ROUTE_ALL is set to 1, all outbound traffic can be sent into your virtual network.

The feature supports only one virtual interface per worker. One virtual interface per worker means one regional VNet Integration per App Service plan. All of the apps in the same App Service plan can use the same VNet Integration. If you need an app to connect to an additional virtual network, you need to create another App Service plan. The virtual interface used isn't a resource that customers have direct access to.
...
### Set up a gateway in your virtual network ###

To create a gateway:

1. [Create a gateway subnet][creategatewaysubnet] in your virtual network.  
...
If you create the gateway for use with App Service VNet Integration, you don't need to upload a certificate. Creating the gateway can take 30 minutes. You won't be able to integrate your app with your virtual network until the gateway is provisioned.
...
No additional configuration is required for the regional VNet Integration feature to reach through your virtual network to on-premises resources. You simply need to connect your virtual network to on-premises resources by using ExpressRoute or a site-to-site VPN.

> [!NOTE]
> The gateway-required VNet Integration feature doesn't integrate an app with a virtual network that has an ExpressRoute gateway. Even if the ExpressRoute gateway is configured in [coexistence mode][VPNERCoex], the VNet Integration doesn't work. If you need to access resources through an ExpressRoute connection, use the regional VNet Integration feature or an [App Service Environment][ASE], which runs in your virtual network.
> 
> 
...
If certificates or network information is changed, select **Sync Network**. When you select **Sync Network**, you cause a brief outage in connectivity between your app and your virtual network. While your app isn't restarted, the loss of connectivity could cause your site to not function properly.
...
* **Data transfer costs**: There's a charge for data egress, even if the virtual network is in the same datacenter. Those charges are described in [Data Transfer pricing details][DataPricing].
