...
| `WEBSITE_TIME_ZONE` | By default, the time zone for the app is always UTC. You can change it to any of the valid values that are listed in [TimeZone](/windows-hardware/customize/desktop/unattend/microsoft-windows-shell-setup-timezone). If the specified value isn't recognized, UTC is used. | `Atlantic Standard Time` |
