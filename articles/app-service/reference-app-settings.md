...
| `WEBSITE_AUTH_NONCE_DURATION`| A _timespan_ value in the form `_hours_:_minutes_:_seconds_`. This setting controls the lifetime of the [cryptographic nonce](https://en.wikipedia.org/wiki/Cryptographic_nonce) generated for all browser-driven logins. The default value is `00:05:00`, or 5 minutes. If a login fails to complete in the specified time, the login flow will be retried automatically. This setting corresponds to the `login.nonce.nonceExpirationInterval` configuration value in the V2 (non-classic) authentication configuration schema, but can also be set directly in V1 (classic) apps. |
...
| `WEBSITE_AUTH_DISABLE_WWWAUTHENTICATE` | `true` or `false`. This setting corresponds to the `identityProviders.azureActiveDirectory.login.disableWwwAuthenticate` configuration value in the V2 (non-classic) authentication configuration schema, but can also be set directly in V1 (classic) apps. When set to `true`, removes the [`WWW-Authenticate`](https://developer.mozilla.org/docs/Web/HTTP/Headers/WWW-Authenticate) HTTP response header from module-generated HTTP 401 responses. |
| `WEBSITE_AUTH_STATE_DIRECTORY`  | A local file system directory path where tokens are stored when the file-based token store is enabled. This setting corresponds to the `login.tokenStore.fileSystem.directory` configuration value in the V2 (non-classic) authentication configuration schema, but can also be set directly in V1 (classic) apps. The default value is `%HOME%\Data\.auth`. |
...
| `WEBSITE_AUTH_TOKEN_REFRESH_HOURS` | Any positive decimal number. This setting corresponds to the `login.tokenStore.tokenRefreshExtensionHours` configuration value in the V2 (non-classic) authentication configuration schema, but can also be set directly in V1 (classic) apps. The default value is `72` (hours). This setting controls the amount of time after a session token expires that the `/.auth/refresh` API can be used to refresh it. It is intended primarily for use with Azure Mobile Apps, which rely on session tokens. Refresh attempts after this period will fail and end-users will be required to sign-in again. |
...
| `WEBSITE_AUTH_VALIDATE_NONCE`| `true` or `false`. The default value is `true`. This setting corresponds to the `login.nonce.validateNonce` configuration value in the V2 (non-classic) authentication configuration schema, but can also be set directly in V1 (classic) apps. This value should never be set to `false` except when temporarily debugging [cryptographic nonce](https://en.wikipedia.org/wiki/Cryptographic_nonce) validation failures that occur during interactive logins. |
