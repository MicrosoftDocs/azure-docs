...
| `MACHINEKEY_ValidationKey` | By default, the automatically generated key is used as the validation key. To override, set to a desired key. This is recommended if you want to share keys across multiple apps or datecenters. This is used in encryption, and message authentication code (MAC) operations in Windows Forms authentication, view-state validation, and session-state application isolation. ||
| `MACHINEKEY_DecryptionKey` | By default, the automatically generated key is used as the decryption key. To override, set to a desired key. This is recommended if you want to share keys across multiple apps or datecenters. This is used in decryption, and message authentication code (MAC) operations in Windows Forms authentication, view-state validation, and session-state application isolation. ||
