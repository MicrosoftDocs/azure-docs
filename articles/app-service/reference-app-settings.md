...
| `WEBSITE_BYOS_BLOB_DIRECT_IO` | Set to `false` by default. If enabled, all transactions will query the remote storage directly and caching will be bypassed. Only relevant when using custom-mounted Azure Blob Storage. Linux containers only (not applicable to Windows). This setting is applied at the application level and therefore affects all blob shares mounted by the application. |
