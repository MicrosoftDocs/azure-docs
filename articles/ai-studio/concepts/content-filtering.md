...
To create a content filter, navigate to your deployment, select the **Content filters** tab on the left nav, and select the **Create content filter** button.
...
Give your filter a name and description. Next, you configure the input filter, which is applied to all input content for your model. Set the action and severity level threshold for each filter type. You configure both the default filters and other filters on this page.
