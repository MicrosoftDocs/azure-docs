...
Give your filter a name and description. Next, you configure the input filter, which will be applied to all input content for your model. Set the action and severity level threshold for each filter type. You configure both the default filters and additional filters on this page.

On the next page, you configure the output filter, which will be applied to all output content generated by your model. Configure the individual filters as before. This page also provides the Streaming mode option, which lets you filter content in near-real-time as it's generated by the model, reducing latency.

Next, you choose a connection (which Azure OpenAI resource to connect to), and then you select which deployments in your resource should use the new filter. If a selected deployment already has a filter attached, you must confirm that you want to replace it.

The Review page shows all the details of your created filter. If everything looks correct, select **Create filter**.

### Use a blocklist as a filter

You can apply a blocklist as either an input or output filter, or both. Enable the **Blocklist** option on the **Input filter** and/or **Output filter** page. Select your blocklist(s) from the dropdown, or use the built-in profanity blocklist. You can combine multiple blocklists into the same filter.
...
Select **Deployments** and choose one of your deployments, then select **Edit**, a window of updating deployment will open up. Then you can update the deployment by selecting one of your created content filters.
