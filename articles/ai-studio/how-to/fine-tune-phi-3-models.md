...
description: Learn how to fine-tune Phi-3 models in Azure AI Studio.
manager: scottpolly
...
In this article, you learn how to fine-tune the Phi-3 family of small language models (SLMs) in Azure AI Studio as a service, with pay-as you go billing.
...
- An [AI Studio hub](../how-to/create-azure-ai-resource.md).
    ...
    > [!IMPORTANT]
    > For Phi-3 family models, the pay-as-you-go model fine-tune offering is only available with hubs created in **East US 2** region.
...
1. Choose the model that you want to fine-tune from the Azure AI Studio [model catalog](https://ai.azure.com/explore/models).
1. On the **Details** page of the model, select **fine-tune**.
...
1. Enter a name for your fine-tuned model, the optional tags, and the description.
1. To fine-tune your model, select the training data. Visit [data preparation](#data-preparation) for more information.
   Verify that all of your training examples follow the expected format for inference. To effectively fine-tune models, ensure that you have an available balanced, diverse dataset. This involves data balance maintenance, which includes various scenarios, and periodic refinement of training data, to align with real-world expectations. This preparation ultimately leads to more accurate and balanced model responses.

   - The batch size to use for training: when set to -1, batch_size is calculated as 0.2% of examples in training set, and the max batch_size value is 256.
   - The fine-tuning learning rate is the original learning rate used for pretraining, multiplied by the fine-tuning learning rate value. We recommend that you experiment with values between 0.5 and 2. Empirically, we found that larger learning rates often perform better with larger batch sizes. The fine-tuning learning rate must be between 0.0 and 5.0.
   - The number of training epochs. An epoch refers to one full cycle through the data set.
...
You can delete a fine-tuned model from the fine-tuning model list in [Azure AI Studio](https://ai.azure.com), or from the model details page. Select the fine-tuned model to delete it from the Fine-tuning page, and then select the Delete button to delete the fine-tuned model.

> [!NOTE]
> You can't delete a custom model if it has an existing deployment. You must delete your model deployment before you can delete your custom model.
...
Models deployed as a service with pay-as-you-go are protected by Azure AI Content Safety. When deployed to real-time endpoints, you can opt out of this capability. With Azure AI content safety enabled, both the prompt and completion pass through an ensemble of classification models aimed at detecting and preventing the output of harmful content. The content filtering system detects and takes action on specific categories of potentially harmful content in both input prompts and output completions. For more information, visit [Azure AI Content Safety](../concepts/content-filtering.md).
