...
The Jamba model is available in Azure Machine Learning Studio with pay-as-you-go token based billing with Models as a Service. 

You can find the Jamba model in the model catalog by filtering on the Jamba collection.
...
- An Azure Machine Learning workspace. If you don't have these, use the steps in the [Quickstart: Create workspace resources](quickstart-create-resources.md) article to create them.
    ...
    > [!IMPORTANT]
    > For Jamba models, the pay-as-you-go model deployment offering is only available with workspaces created in East US 2 or Sweden Central region.
...
The previously mentioned Jamba 30b Chat model can be deployed as a service with pay-as-you-go, and is offered by Core42 through the Microsoft Azure Marketplace. Core42 can change or update the terms of use and pricing of this model.
...
1. Search for Jamba and select the jamba-30b-chat model from the [model catalog](https://ml.azure.com/model/catalog).
...
### Consume the Jamba 30b Chat model as a service
...
2. Select your deployment for Jamba 30b Chat.

3. You can test the deployment in the **Test** tab.

1. To use the APIs, copy the **Target** URL and the **Key** value.

For more information on using the APIs, see the [reference](#chat-api-reference-for-jamba-deployed-as-a-service) section.

## Chat API reference for Jamba deployed as a service
