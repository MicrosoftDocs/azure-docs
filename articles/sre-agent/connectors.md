...
description: Use connectors to extend your agent's capabilities to external data sources, collaboration tools, and custom APIs.
...
> [!NOTE]
> *Connectors* give your agent access to data and actions, such as querying logs, sending notifications, and reading code. *Incident platforms* are a separate concept: they control where alerts come from and how your agent responds to them automatically. This article covers connectors. For more information about incident platforms, see [Incident platforms](incident-platforms.md).

## What your agent can do without connectors

Even with no connectors set up, your agent has built-in capabilities through its managed identity and Azure role-based access control (Azure RBAC) permissions:
...
| Azure Monitor metrics | List and query metrics, and analyze trends and anomalies. |
| Resource Graph | Discover and query any Azure resource across subscriptions. |
| Azure Resource Manager / Azure CLI | Read and modify any Azure resource type. |
| Azure Kubernetes Service (AKS) diagnostics | Run kubectl commands, and diagnose Kubernetes issues. |

Resource Graph and Resource Manager operations work with any Azure resource type, including Azure App Service, Azure Container Apps, virtual machines, networking, and storage. If your logs and metrics live in Azure Monitor and Application Insights, your agent can start investigating issues immediately, with no connector setup required. Connectors become valuable when you need the agent to reach systems *outside* Azure.

## What connectors provide

Connectors fall into four categories based on what they give your agent: data sources, source code and knowledge, collaboration tools, and custom connectors.

### Data sources

These sources can be query logs, metrics, and telemetry from your data stores.
...
| Application Insights | Connect specific Application Insights resources so your agent has persistent context about your application telemetry. |
...
| GitHub OAuth | GitHub access via the OAuth authentication flow. |
...
With these connectors, your agent can search code for error patterns, and read wiki documentation. Your agent can also reference API docs during troubleshooting, and connect incidents to related pull requests.
...
With Model Context Protocol (MCP) servers, you can connect your agent to any system, including observability platforms, source code repositories, ticketing systems, and custom APIs. Your agent automatically discovers tools from connected servers, monitors connection health with 60-second heartbeats, and recovers from transient failures automatically.

Two transport types cover every deployment model: Streamable-HTTP for remote cloud services, and stdio for local processes running alongside your agent. Preconfigured partner connectors for resources such as GitHub, Datadog, Splunk, and New Relic provide one-click setup.

For a complete guide to MCP architecture, transport types, partner connectors, health monitoring, and tool management, see [MCP connectors and tools](mcp-connectors.md).

To set up your first MCP connector, see [Set up MCP connector](mcp-connector.md).

## Browse and manage connectors

To see your connectors organized into collapsible category groups, go to **Builder** > **Connectors**. On this page, all groups are expanded by default.
...
To manage a connector, you need write permission on the agent. The following table shows which role has this permission:
...
During setup, some connectors require OAuth consent from a user who has the right permissions in the external system (for example, a GitHub org member for GitHub connectors). This consent is about permissions in the *external* service, not SRE Agent roles.

For connectors that use the agent's managed identity (like Azure Data Explorer), an admin of the external system must list the identity as allowed.

When you configure connectors, all agent users benefit from them automatically. They just ask the agent questions and the agent uses the available connectors behind the scenes.
...
Assign tools individually in the portal tool picker, or use wildcard patterns (`connection-id/*`) in YAML to add all tools from a server at once. For details on tool assignment and wildcard syntax, see [MCP connectors and tools](mcp-connectors.md#how-tools-reach-your-agents).

In the portal, go to **Builder** > **Custom agent builder**. Create or edit a custom agent, and under **Advanced settings**, select **Choose tools**. The tool picker displays tools grouped by MCP connection. Select the ones your custom agent needs.
