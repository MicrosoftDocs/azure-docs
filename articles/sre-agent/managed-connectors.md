...
description: Connect your agent to Google Drive, SharePoint, Notion, Confluence, and more with per-tool governance controls.
...
ms.custom: managed-connectors, connectors-v2, connector-gateway, outlook, teams, oauth, governance, parameter-locking, approval, tool-selection
...
Your agent works with Azure services, Kusto, GitHub, and Azure DevOps. But your team also stores response procedures in Google Drive and SharePoint, tracks work in Trello or Planner, and keeps operational notes in Notion or Confluence. You want your agent to work with these services too.

When the agent operates against external services, you need governance. You need to control which operations the agent can call, what values it uses for sensitive fields like email recipients or destination folders, and whether certain actions require your approval first. This access control is the same kind you'd set up for any service account or automation that touches production systems.
...
For example, a Trello connector has operations like Get Card, Create Card, Search Cards, and Add Comment. You might enable Get Card and Search Cards for read access, but leave Create Card disabled so the agent can't create cards.
...
For example, on the Google Drive "Copy file" tool, you could lock **Source file** to a response-template document and **Overwrite** to No, while leaving **Destination file path** for the agent to fill. The agent can copy the approved template into a folder you chose, but it can't change the source document or overwrite existing files.
...
For example, on a Google Drive connector you might set "Find files in folder" to Allow since it's read-only, but set "Delete file" to Ask so you confirm before anything is removed.
