...
David runs Azure Security standard tier across all his subscriptions to get the full suite of recommendations and security features available. He also onboards all his existing on-premises servers that haven't yet been migrated to the cloud so that he can take advantage of Security Center's hybrid support across his [Windows](quick-onboard-windows-computer.md) and [Linux](quick-onboard-linux-computer.md) servers.

Jeff, a cloud workload owner, is responsible for applying security controls in accordance with Contoso’s security policies. 
...
As part of his daily monitoring activities, Jeff signs in to Azure portal and opens Security Center. 

1. Jeff selects the subscriptions that represent his workloads.

2. He checks his **secure score** to get an overall picture of how secure the subscriptions are and he sees that his score is 548.

3. He has to decide which recommendations to handle first. So he clicks on secure score and starts to handle recommendations based on how much it will improve his [secure score impact](security-center-secure-score.md).

4. Because he has lots of connected VMs and servers, he decides to focus on **Compute and apps**.

5. When he clicks on **Compute and apps**, he sees a list of recommendations and handles them according to the secure score impact.

6. Jeff has a lot of Internet facing VMs, and because their ports are exposed, he's worried that an attacker could gain control over the servers. So he chooses to use (**just-in-time VM access**)[security-center-just-in-time.md].

Jeff continues to move through the high priority and medium priority recommendations, making decisions on implementation. For each recommendation, Jeff looks at the detailed information provided by Security Center to understand which resources are impacted, what the secure score impact is, what each recommendation means and remediation steps for how to mitigate each issue.
