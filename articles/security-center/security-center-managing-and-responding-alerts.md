...
Azure Security Center automatically collects, analyzes and integrates log data from your Azure resources, the network, and partner solutions like antimalware and firewalls to detect real threats and reduce false positives. A list of prioritized alerts are surfaced as Security Alerts.  
...
In the bottom part of this blade you have the details for each alert. You can sort it for each column by click on the desire column. The definition for each column is shown below:
...
To take an action select the alert that you want to respond and a new blad will open on the right with more details as shown below: 
...
In this case the alerts that was triggered refers to a suspicious Remote Desktop Protocol (RDP) activity. The first column shows which resources was attacked, followed by the time that this attack was detected, the state of the alert and the severity. After reviewing this information you should click the resource that was attacked and a new blade will open up with more suggestions of what you should do next as shown the example below:
...
> [AZURE.NOTE] The remediation suggested by Azure Security Center will vary according to the security alert and in some cases, you may have to use another Azure capabilities in order to implement the recommended remediation. For example, the remediation for this attack is to blacklist the IP that is generating this attack by using [Network ACL](virtual-networks-acl.md) or a Network Security Group rule.

### Additional Tasks
You can filter alerts based on the date, state and severity. Under the Security Alerts blade, click Filter and enable the options that you want as shown below:
...
While the majority of the security alerts will have recommendations that should be applied, in some circumstances you may need to ignore an alert because for your environment is a false positive or is an expected behaviour for a particular resource. Wherever the case may be, you can hide recommendations for a particular resource by using the **dismiss** option.  
...
There is a collection of alerts and each one has a very specific description and remediation. The alerts you see in Azure Security Center are based on the attack scenario. The attack scenarios that follow are triggered by the Microsoft engine: 
...
- **Brute Force Detection over Endpoint Data**: In this detection Azure Security Center queries the machine logs directly in order to differentiate between an attempt and a success. 
- **VMs Communicating with Malicious IPs**: In this detection, Azure Security Center discovers machines that are compromised with bots and communicating with their Command and Control (C&C) Servers (and vice versa). 
