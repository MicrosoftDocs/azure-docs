...
|Monitoring agent should be enabled on your virtual machines|This action installs a monitoring agent on the selected virtual machines. Select a workspace for the agent to report to.If your update policy is set to automatic, it will deploy on new existing instances. If your update policy is set to manual and you would like to install the agent on existing instances, select the check box option. [Learn more](./../virtual-machine-scale-sets/virtual-machine-scale-sets-faq#how-do-i-add-an-extension-to-all-vms-in-my-virtual-machine-scale-set)|
