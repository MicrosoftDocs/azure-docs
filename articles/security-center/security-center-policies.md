...
Security policies can be configured for each subscription or resource group. To modify a security policy, you must be an [Owner or Contributor](../articles/active-directory/role-based-access-control-configure.md) of that subscription. Follow the steps below to configure security polices in Azure Security Center:
