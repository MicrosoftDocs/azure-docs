...
5. If your storage account is not configured yet, you may see a warning like in the following screenshot when you open **Security Policy**:
...
9. In the **Security Policy** blade, click **On** to enable the security recommendations that you want to use on this subscription. Click **Prevention policy** to see options like in the following screenshot:
...
After you select the resource group, the **Security policy** blade opens. By default, the **Inheritance** option is enabled, which means that all security policies for this resource group are inherited from the subscription level. You can change this configuration in case you want a custom security policy for a resource group. If that's the case, you need to select **Unique** and make the changes under the **Prevention policy** option.
