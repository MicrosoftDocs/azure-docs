...
- **SQL Server on Azure Arc** - If your SQL Server is hosted on an [Azure Arc enabled server](../azure-arc/servers/overview.md), you can deploy the Log Analytics agent using the Security Center recommendation “Log Analytics agent should be installed on your Windows-based Azure Arc machines”. Alternatively, you can follow the manual procedure in the [Azure Arc documentation](../azure-arc/servers/manage-vm-extensions-portal.md).
...
- **SQL Server on-prem** - If your SQL Server is hosted on an on-premises Windows machine without Azure Arc, you have two options for connecting it to Azure:
    ...
    - **Deploy Azure Arc enabled servers** - You can connect any Windows machine to Security Center. However, Azure Arc provides deeper integration across *all* of your Azure environment. If you set up Azure Arc enabled servers, you'll see the **SQL Server – Azure Arc** page in the portal and your security alerts will appear on a dedicated **Security** tab on that page. So the first and recommended option is to [set up Azure Arc on the host](../azure-arc/servers/onboard-portal#install-and-validate-the-agent-on-windows) and follow the instructions for **SQL Server on Azure Arc**, above.
