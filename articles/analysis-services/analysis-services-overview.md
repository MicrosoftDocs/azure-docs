...
Azure Analysis Services provides enterprise-grade data modeling in the cloud. With it's highly optimized data analytics engine, Analysis Services provides a semantic model layer between extremely large, complex, and often disparate data sources. By pulling your data together, you can define the relationships between data, the measures, and the security. With a trusted semantic model that’s easy to understand and use, and with Azure's legendary reliability, you too can enable self-service data discovery for your organization.

![Data sources](./media/analysis-services-overview/aas-overview-data-sources.png)

With Azure Analysis Services, you can reduce time-to-insights on large and complex datasets. Fast response times mean your BI solution can meet the needs of your users and keep pace with your business. Connect to cached, in-memory data or real-time operational data using DirectQuery and closely watch the pulse of your business.
...
Change tiers on-the fly in Azure portal or with the Set-​Azure​Rm​Analysis​Services​Server PowerShell cmdlet. To learn more about the different plans and tiers, and use the pricing calculator to dermine the right plan for you, see [Azure Analysis Services Pricing](https://azure.microsoft.com/pricing/details/analysis-services/).
...
By carefully monitoring service use patterns, and creating a scheduled, automated service management plan, you can reduce your costs while maintaining a high-level of service when you need it most, and not pay  when you don't.
...
## Data sources
Azure Analysis Services support connecting to data sources on-premises in your organization and in the cloud. Combine data from both on-premises and cloud data sources for a hybrid BI solution. The types of data sources supported different between tabular 1200 and tabular 1400 models. This is because tabular 1400 models support using the modern Get Data feature in SSDT based on the M formula query language.
...
Coming soon, you'll bea able to leverage your Power BI Desktop data models by importing them right into Azure Analysis Services.
...
Develop and deploy models with the free [SQL Server Data Tools (SSDT) for Visual Studio](https://msdn.microsoft.com/library/mt204009.aspx). SSDT includes Analysis Services project templates that will get you up and going quickly. Deploy to your servers in Azure right from SSDT. And,it's updated monthly, so you'll always be able to use the latest features. Coming soon, you''ll be able to create data models right in Azure portal!

#### Sql Server Management Studio
Manage your servers and model databases by using [SQL Server Management Studio (SSMS)](https://msdn.microsoft.com/library/mt238290.aspx). New features and functionality happen fast - SSMS is updated monthly. Run TMSL scripts right from the XMLA query window. Automate tasks by using SQL Server Agent and PowerShell and TMSL scripts.
...
#### Documentation
Azure Analysis Services is simple to set up and to manage. You can find all the info you need to create and manage you server services here. When creating a data model to deploy to your server, it's much the same as it is for creating a data model you deploy to an on-premises server. There's an extensive library of conceptual, procedural, tutorials, and reference articles at [Analysis Services](https://docs.microsoft.com/sql/analysis-services/analysis-services).
