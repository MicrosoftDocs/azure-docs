...
> [!NOTE]
> For Power BI Premium in Microsoft Cloud Germany only, data import (refresh) and paginated reports connections are not supported when a firewall is enabled. Live connect is supported when the Allow access from the Power BI Service setting is enabled, and all Power BI Pro connect scenarios are supported.
...
3. To allow Live connect access from Power BI service, in **Allow access from Power BI**, select **On**.  
