...
- The token must have the audience set to exactly `https://*.asazure.windows.net`. Note that `*` is neither a placeholder nor wildcard, and the audience must have the `*` character as the subdomain. Specifying an invalid audience will result in authentication failure.
