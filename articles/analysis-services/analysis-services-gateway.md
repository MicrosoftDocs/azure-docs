...
- Install the gateway on a computer that remains on and does not go to sleep. If the computer is not on, your Azure Analysis Services server will not be able to connect to your on-premises data sources to refresh data.

- Do not install the gateway on a computer wirelessly connected to your network. Performance can be diminished.

- In order to change the server name for a gateway that has already been configured, you need to re-install and configure a new gateway.
...
1.	A query is created by the cloud service along with the encrypted credentials for the on-premises data source. It's then  sent to a queue for the gateway to process.
...
The on-premises data gateway is configured to use *NT SERVICE\PBIEgwService* for the Windows service logon credential. By default, it has the right of Log on as a service. This is in the context of the machine that you are installing the gateway on. This is not the account used to connect to on-premises data sources or your Azure account.  
