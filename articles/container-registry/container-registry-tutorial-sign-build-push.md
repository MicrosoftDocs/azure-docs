...
5. Add the downloaded public certificate to named trust store for signature verification.
...
6. List the certificate to confirm
...
3. Sign the container image with the [COSE](https://datatracker.ietf.org/doc/html/rfc8152) signature format using the signing key in previous step.
 
    ```bash
    notation sign --signature-format cose --plugin azure-kv --id $KEY_ID $IMAGE
    ```
