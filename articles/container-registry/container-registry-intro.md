...
* **Access control** - Azure Container Registry provides native support for TLS to secure client connections. Log in to the registry using the Azure CLI or the standard `docker login` command.

  > [!IMPORTANT]
  > On January 1, 2020, the Azure Container Registry service will require all secure connections from servers and applications to use TLS 1.2. Support for TLS 1.0 and 1.1 will be retired.

  You [control access](container-registry-authentication.md) to a container registry using an Azure identity, an Azure Active Directory-backed [service principal](../active-directory/develop/app-objects-and-service-principals.md), or a provided admin account. Use role-based access control (RBAC) to assign fine-grained permissions to a registry.
