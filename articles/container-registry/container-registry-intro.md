...
* **Managed Registry** - A tier that offers additional capabilities for registries in three SKUs - Basic, Standard, and Premium. The images in these SKUs are stored in Storage Accounts managed by the ACR service, which improves reliability and enables new features such as webhook integration, repository authentication with Azure Active Directory, and support for delete functionality. When creating registries users have the option to choose between managed registries or to create a registry backed by their own Storage Accounts.
