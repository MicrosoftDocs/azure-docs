...
This error means that docker client for CLI could not be found, which precludes finding docker version, evaluating docker daemon status and ensuring docker pull command can be ran.
...
This error means that the CLI was unable to determine the Helm version installed. This can happen if the Azure CLI version (or if the helm version) being used is obsoleted.
...
This means that the challenge endpoint for the given registry responded with a 403 Forbidden HTTP status. This means that user don't have access to the registry, most likely due to a VNET connection.
...
*Potential solutions*: try again after some time. If error persists, contact the Azure Container Registry team.
...
*Potential solutions*: try other way of logging in, e.g., admin credentials. In case the user wants to log in with AAD support, contact the Azure Container Registry team.
...
This means that the CLI was unable to find the login server of the given registry, and no default suffix was found for the current cloud. This can happen if the registry does not exist, if the user does not have the right permissions on the registry, if the registry's cloud and the current Azure CLI cloud does not match, or if the Azure CLI version is obsolete.
