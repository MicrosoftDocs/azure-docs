...
In this article you'll build and run the multi-tier application in an Azure cloud service. The front end will be an ASP.NET MVC web role and the back end will be a worker-role. You can create the same multi-tier application with the front end as a web project that would be deployed to an Azure website instead of a cloud service. For instructions about what to do differently in an Azure website front end, see the [Next steps](#nextsteps) section.

A screen shot of the completed application appears next.
...
2. 	Click the link for the version of Visual Studio you are using. The steps in this article use Visual Studio 2013.
...
each application exposed through the Service Bus. A SAS key is
...
> [AZURE.NOTE] You can also manage namespaces and Service Bus messaging entities using the Visual Studio Server Explorer, but you can only create new namespaces from within the portal.
...
4.  On the **Add a new namespace** page, enter a namespace name. The system immediately checks to see if the name is available.
    ![][7]
...
9.  In the **Access connection information** page, find the connection string that contains the SAS key and key name.
...
5.  On the **New ASP.NET Project** dialog box, in the **Select a template** list, click **MVC**,
    then click **OK**.
...
5.  Now, you will create the view for the **Submit()** method you
    created earlier. Right-click within the **Submit()** method, and choose
    **Add View**
...
3.  You will now add code that encapsulates the connection information and initializes the connection to a Service Bus queue. In QueueConnector.cs, add the following code, and enter values for **Namespace** (your service namespace) and **yourKey**, which is the SAS key you obtained from the [Azure portal][Azure portal] earlier.
    ...
    Note that later in this article you will learn how to store the name of your Namespace and your SAS key value in a configuration file.
...
object (such as **QueueClient**, **TopicClient**, and **EventHubClient**
...
8.  In the **Settings** tab of the **Properties** dialog box, click inside the **Value** box for **Microsoft.ServiceBus.ConnectionString**, and then paste the endpoint value you copied in step 6.
...
To implement the application you create in this article as a standard web project instead of as a cloud service web role, follow the steps in this article with the following differences:
