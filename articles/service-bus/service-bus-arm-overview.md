...
This article shows how to create and deploy Service Bus and Event Hubs resources using Azure Resource Manager (ARM) templates and PowerShell using the Service Bus and Event Hubs resource providers.

ARM templates help you define the resources to deploy for a solution, and to specify parameters and variables that enable you to input values for different environments. The template consists of JSON and expressions which you can use to construct values for your deployment. For detailed information about writing ARM templates, and a discussion of the template format, see [Authoring Azure Resource Manager templates](../resource-group-authoring-templates.md). 

>[AZURE.NOTE] The examples in this article show how to use ARM to create a Service Bus namespace and messaging entity (queue). For other template examples, see the [Azure Quickstart Templates gallery][] and search for "Service Bus."

## Service Bus and Event Hubs ARM templates

These Service Bus and Event Hubs ARM templates are available for download and deployment. Click the following links for details about each one, with links to the templates on GitHub: 
...
5. If desired, set the [deployment mode](../resource-group-template-deploy.md#incremental-and-complete-deployments).
6. Deploy the template.

For complete information about deploying ARM templates, see [Deploy resources with Azure Resource Manager templates][].
...
Download or copy the [201-servicebus-create-queue](https://github.com/Azure/azure-quickstart-templates/blob/master/201-servicebus-create-queue/azuredeploy.json) template from GitHub:
...
This command returns a list of available Azure subscriptions. Choose and set your subscription for the current session by running the following command. Replace `<YourSubscriptionId>` with the GUID for the Azure subscription you want to use:
...
```
New-AzureRmResourceGroup -Name MyARMDemoRG -Location "West US"
```
...
Tags              :
ResourceId        : /subscriptions/<GUID>/resourceGroups/ARMDemoRG
```
...
```
Test-AzureRmResourceGroupDeployment -ResourceGroupName MyARMDemoRG -TemplateFile <path to template file>\azuredeploy.json
```

### Create the deployment

To create a new deployment, run the `New-AzureRmResourceGroupDeployment` command, and provide the necessary parameters when prompted. The parameters include a name for your deployment, the name of your resource group, and the path or URL to the template file. If the **Mode** parameter is not specified, the default value of **Incremental** is used. For more information, see [Incremental and complete deployments](../resource-group-template-deploy.md#incremental-and-complete-deployments).

The following command prompts you for these three parameters in the PowerShell window:

```
New-AzureRmResourceGroupDeployment -Name ARMDemoDeployment -ResourceGroupName MyARMDemoRG -TemplateFile <path to template file>\azuredeploy.json
```

To use a parameters file instead, use the following command.

```
New-AzureRmResourceGroupDeployment -Name ARMDemoDeployment -ResourceGroupName MyARMDemoRG -TemplateFile <path to template file>\azuredeploy.json -TemplateParameterFile <path to parameters file>\azuredeploy.parameters.json
```

You can also use inline parameters when you run the deployment cmdlet. The command is as follows:

```
New-AzureRmResourceGroupDeployment -Name ARMDemoDeployment -ResourceGroupName MyARMDemoRG -TemplateFile <path to template file>\azuredeploy.json -parameterName "parameterValue"
```
...
```
New-AzureRmResourceGroupDeployment -Name ARMDemoDeployment -Mode Complete -ResourceGroupName MyARMDemoRG -TemplateFile <path to template file>\azuredeploy.json 
```
...
```
DeploymentName    : ARMDemoDeployment
ResourceGroupName : MyARMDemoRG
ProvisioningState : Succeeded
...
You've now seen the basic workflow and commands for deploying an ARM template. For more detailed information, visit the following links:
...
[Azure Quickstart Templates gallery]: https://azure.microsoft.com/documentation/templates/