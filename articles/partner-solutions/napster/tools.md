---
title: Napster Companion API developer resources and tools (preview)
description: Learn about resources and developer tools available with Napster Companion API - Azure Native Integrations.

ms.topic: overview
ms.date: 05/20/2026

# CustomerIntent: As a developer building with Napster Companion API, I want to know about the resources and tools available so I can create, deploy, and monitor persistent multimodal AI agents (Omniagents).

---

# What are some resources and tools you can use with Napster Companion API? (preview)

In this article, you learn about resources and tools for use with Napster Companion API - Azure Native Integrations.

- Articles that you can use to understand Napster Companion API
- Tools for developers to work with Napster Companion API

## Get started with the Companion API Dashboard

After you create the Napster Companion API resource, select **Go to Napster Companion API** on the resource Overview blade to launch the Companion API Dashboard. The Dashboard is where you:

- Create and manage Omniagents
- Manage knowledge sources and FAQs
- Configure tools and integrations
- Generate API keys
- Test agents in the Playground before going live

## Napster developer resources

For full developer documentation, visit the [Napster developer portal](https://developers.napster.com/docs):

- [Quickstart](https://developers.napster.com/docs/introduction/quickstart)
- [Authentication](https://developers.napster.com/docs/introduction/authentication)
- [Build your Omniagent](https://developers.napster.com/docs/building-your-omniagent/overview)
- [Deploy your Omniagent](https://developers.napster.com/docs/deploying-your-omniagent/overview)
- [Monitor your Omniagent](https://developers.napster.com/docs/monitoring-your-omniagent/sessions)
- [API reference](https://developers.napster.com/docs/api-reference)
- [FAQs](https://developers.napster.com/docs/faq)

## Developer tools

Napster Companion API offers a suite of developer tools that facilitate integration with and management of your resource.

Key links to get started:

- [Companion API Dashboard](https://portal.azure.com/#browse/napster.companionapi%2Forganizations)
- [API reference](https://developers.napster.com/docs/api-reference)

## Next steps

- For more help with using Napster Companion API, see the [Napster developer documentation](https://developers.napster.com/docs).
- Get started with Napster Companion API on:

    > [!div class="nextstepaction"]
    > [Azure portal](https://portal.azure.com/#browse/napster.companionapi%2Forganizations)

    > [!div class="nextstepaction"]
    > [Azure Marketplace](https://marketplace.microsoft.com/en-us/product/touchcastinc1655995956899.napster_companion_api)
