...
New Relic organization -- This is the New Relic organization on New Relic Software as a Service (SaaS) which is used for user management and billing.

New Relic account - This is the New Relic account on New Relic Software as a Service (SaaS) which is used to store and process telemetry data.

Marketplace SaaS resource -- When you setup a new account and organization on New Relic using the Azure Native New Relic Service, the SaaS resource is created automatically, based on the plan you select from the Azure New Relic Marketplace offer. This resource is used for billing purposes.

## Prerequisites

Before you link the subscription to New Relic, complete the pre-deployment configuration.
...
1. If you have visited the Azure **Marketplace** in a recent session, select the icon from the available options. Otherwise, search for Marketplace.
...
1. In the Marketplace, search for New Relic.
 ...
 [Screenshot showing New Relic in the Marketplace.](media/new-relic-create/new-relic-marketplace.png)
...
If you opt to create a new organization, you can choose a plan from the list of available plans in the 'Change Plan' blade

  \[TODO\]: Update screenshot

:::image type="content" source="media/new-relic-create/new-relic-change-plan.png" alt-text="Screenshot of change plan.":::

If you opt to associate with an existing organization, the corresponding billing information that got set up at the time of creation of organization is shared with you for review
...
Subscription activity logs - These logs provide insight into the operations on your resources at the [control plane](/azure/azure-resource-manager/management/control-plane-and-data-plane). Updates on service-health events are also included. Use the activity log to determine what, who, and when for any write operations (PUT, POST, DELETE). There is a single activity log for each Azure subscription.
...
To send subscription level logs to New Relic, select Send subscription activity logs. If this option is left unchecked, none of the subscription level logs are sent to New Relic.

To send Azure resource logs to New Relic, select Send Azure resource logs for all supported resource types. The types of Azure resource logs are listed in [Azure Monitor Resource Log categories](/azure/azure-monitor/essentials/resource-logs-categories).
...
The logs sent to New Relic are charged by Azure. For more information, see the [pricing of platform logs](/azure.microsoft.com/pricing/details/monitor/) sent to Azure Marketplace partners.
...
Once you have completed configuring metrics and logs, select **Next**.

## Setup resource tags

In the 'Tags' tab, you can choose to set up tags for the New Relic resource. You can also skip this step and go directly to **Review and Create**.
...
You can review the resource set up information presented in the **Review and Create**, ensure that the validation passed, and click **Create** to begin the resource deployment
