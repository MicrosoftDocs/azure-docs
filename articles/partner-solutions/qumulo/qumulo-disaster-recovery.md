...
## Architecture
As shown in the following diagrams, the Azure Native Qumulo for DR solution can be deployed in one or more Azure availability zones depending on the primary site configuration and the level of recoverability required. 

In all versions of this solution, the customer’s Azure resources are deployed into the customer’s own Azure tenant, and the ANQ service instance is deployed in Qumulo’s Azure tenant in the same region(s). Customer access to the ANQ service instance and its data are enabled through a delegated subnet in the customer’s Azure tenant, using VNet injection to connect to the ANQ service instance. NOTE: Qumulo has no access to any customer data on any ANQ instance.

Data services are replicated from the primary-site Qumulo instance to the ANQ service instance using either Qumulo continuous replication (in which all changes on the primary file system are immediately replicated to the ANQ instance, overwriting older versions of the data), or using snapshots with replication (which maintains multiple versions of changed files to enable more granular data recovery in the event of data loss or corruption).

In the event of a primary-site outage, critical client systems and workflows will be able to use the ANQ service instance as the new primary storage platform, and can use the service’s native support for all unstructured-data protocols – SMB, NFS, NFSv4.1, and S3 – just as they were able to do on the primary-site storage.


### Solution Architecture

#### ANQ DR - On-prem or Other Cloud
In this setup, ANQ for DR is deployed into a single Azure region, with data replicating from the primary Qumulo storage instance to the ANQ service through the customer’s own Azure VPN Gateway or ExpressRoute connection.
...
#### ANQ DR - On-prem or Other Cloud (Multi-Region)
...
## Solution Workflow 
1.	Users and workflows access the primary storage solution via standard unstructured data protocols: SMB, NFS, NFSv4.1, S3.
2.	Users and / or workflows add, modify, or delete files on the primary storage instance as part of the normal course of business.
3.	The primary Qumulo storage instance identifies the specific 4K blocks in the file system that have been changed and replicates only the changed blocks to the ANQ instance designated as the secondary storage.
...
- Cloud DR for Remote Offices/Branch Offices (ROBO): Organizations can deploy Qumulo clusters at their ROBO locations and replicate data to a centralized DR repository on an ANQ service. In the event of a disaster at any remote site, organizations can continue to support the impacted site from the ANQ instance until a new Qumulo cluster can be deployed at the affected site, ensuring business continuity and data availability.
