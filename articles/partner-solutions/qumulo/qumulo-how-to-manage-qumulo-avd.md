...
With an increasingly distributed global workforce, and with many remote workers requiring secure access to critical resources and data from virtually anywhere, many of today’s enterprises have concluded that a remote desktop solution offers better security at a lower total cost of ownership. At the same time, these enterprises are recognizing that Azure Native Qumulo offers greater scalability and performance than other Azure-based file-data solutions, resulting in much shorter login times and better overall user productivity.
This solution combines multi-region deployments with failover capabilities for both regions. The benefits of this deployment model include:
...
|Cost efficiency |	For many enterprises, AVD offers a more cost-effective approach to securing critical business assets and data than a VPN-based solution with local clients for all users. ANQ’s overall TCO is comparable to on-prem file services, even at scale. Customers pay only for the capacity and throughput they use, while they use it.|
...
|Management simplicity	|ANQ’s support for exabyte-scale file services simplifies the process of managing user profile storage. The solution as a whole streamlines the process of enabling access for remote users and simplifies enforcement of enterprise security and software-usage policies.|
...
- Capacity and growth: Since an ANQ service instance can easily scale as needed in response to an increased user count or to a higher space allocation per user, enterprises can improve the overall TCO of the solution by not over-provisioning file capacity before it’s needed.
