...
One of the more common questions we have seen when creating Azure Service Fabric clusters is how to integrate the cluster with other Azure networking features. This article shows how to create clusters by using the following features:
...
I am starting with an existing Virtual Network named 'ExistingRG-vnet' in the resource group *ExistingRG*, with a subnet named 'default'. These resources are the default ones created when using the Azure portal to create a standard virtual machine. You could also create the virtual network and subnet without creating the virtual machine. The main goal of adding a cluster to an existing virtual network is to provide network connectivity to other VMs, though, so creating the VM gives a concrete example of how this is typically used. If your Service Fabric cluster only uses an internal load balancer without a public IP address, the VM with its public IP can also be used as a jump box.
...
3. Remove the *Microsoft.Network/virtualNetworks* from the Resources so that Azure does not create a new virtual network:
...
4. Comment out the virtual network from the *dependsOn* attribute of the *Microsoft.Compute/virtualMachineScaleSets* so that we don't depend on creating a new virtual network:
...
5. Deploy the template:
    ...
    After the deployment your Virtual Network should include the new scale set VMs and the virtual machine scale set node type should show the existing virtual network and subnet. You can also RDP to the existing VM that was already in the virtual network and ping the new scale set VMs:
...
4. Remove *Microsoft.Network/publicIPAddresses* from the *Resources* so Azure does not create a new IP address:
...
7. In the *Microsoft.ServiceFabric/clusters* resource, change the *managementEndpoint* to the DNS FQDN of the static IP. **If you are using a secure cluster, make sure you change 'http://' to 'https://'.** (Note: This instruction is only for Service Fabric clusters. If you are using a virtual machine scale set, skip this step):
...
After the deployment, you can see that your load balancer is bound to the public static IP address from the other resource group. The Service Fabric client connection endpoint and [Service Fabric explorer](service-fabric-visualizing-your-cluster.md) endpoint point to the DNS FQDN of the static IP address.
...
1. Remove the *dnsName* parameter because it is not needed:
...
2. Optionally, if you are using a static allocation method, you can add a static IP address parameter. If you are using a dynamic allocation method, you do not need to do this step:
...
3. Remove *Microsoft.Network/publicIPAddresses* from the Resources, so Azure does not create a new IP address:
...
4. Remove the IP address *dependsOn* attribute of the *Microsoft.Network/loadBalancers*, so you don't depend on creating a new IP address. Add the virtual network *dependsOn* attribute since the load balancer now depends on the subnet from the virtual network:
...
After the deployment, your load balancer uses the private static 10.0.0.250 IP address. If you have another machine in that same virtual network, you also can browse to the internal [Service Fabric explorer](service-fabric-visualizing-your-cluster.md) endpoint and see that it connects to one of the nodes behind the load balancer.

<a id="internalexternallb"></a>
## Internal and external load balancer

In this scenario, we take the existing single-node type external load balancer and add an internal load balancer for the same node type. A back-end port attached to a back-end address pool can be assigned only to a single load balancer. You have to decide which load balancer should have your application ports, and which load balancer should have your management endpoints (port 19000/19080). If you decide to put the management endpoints on the internal load balancer, keep in mind the SFRP restrictions discussed earlier. This sample keeps the management endpoints on the external load balancer. It also adds a port 80 application port, and places it on the internal load balancer.

If you want a two-node type cluster, with one node type on the external load balancer and the other on the internal load balancer, then simply take the portal-created two node type template (which comes with two load balancers) and switch the second load balancer to an internal load balancer per the 'Internal Only Load Balancer' section above.
