...
To support clusters that span across Availability Zones, Service Fabric deploys node types that are pinned to specific zones. Azure Availability Zones are only available in select regions. For more information, see [Azure Availability Zones Overview](../availability-zones/az-overview.md).
...
Standard Load Balancer and Standard Public IP introduce new abilities and different behaviors to outbound connectivity when compared to using Basic SKUs. If you want outbound connectivity when you're working with Standard SKUs, you must explicitly define it with either Standard Public IP addresses or Standard public Load Balancer. For more information, see [Outbound connections](../load-balancer/load-balancer-outbound-connections.md) and [Azure Standard Load Balancer](../load-balancer/load-balancer-overview.md).
...
1. To begin, add the new resources to your existing Resource Manager template. These resources include:
...
>[!IMPORTANT]
>The Service Fabric cluster resource API version should be 2020-12-01-preview or higher.
>
...
>[!TIP]
> We recommend setting `sfZonalUpgradeMode` to **Hierarchical** or omitting it. Deployment will follow the zonal distribution of VMs and impact a smaller amount of replicas or instances, making them safer.
...
* Migration from a Node Type that uses the Standard SKU Load Balancer and IP resources with an NSG:
    Follow the same procedure described previously. However, there's no need to add new Load Balancer, IP and NSG resources, and the same resources can be reused in the new Node Type.
