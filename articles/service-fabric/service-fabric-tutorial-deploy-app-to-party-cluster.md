...
> [!NOTE]
> A free account may not meet the requirements to create a virtual machine. This will prevent the completion of the tutorial. In addition, a non-work or non-school account may encounter permission issues while creating a keyvault to store the cluster certificate. If you experience an error related to keyvault creation use the Portal to create the cluster instead. 
