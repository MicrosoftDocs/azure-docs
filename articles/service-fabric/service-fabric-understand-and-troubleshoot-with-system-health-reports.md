...
> [AZURE.NOTE] The Naming service resolves service names to a location in the cluster and enables users to manage service names and properties. It is a Service Fabric partitioned persisted service. One of the partition represents the Authority Owner, which contains metadata about all System Fabric names and services. The Service Fabric names are mapped to different partitions, called Name Owner partitions, so the service is extensible. Read more about [Naming service](https://azure.microsoft.com/documentation/articles/service-fabric-architecture/). 
