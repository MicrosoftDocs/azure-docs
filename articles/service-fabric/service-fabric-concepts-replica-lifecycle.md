...
### Closing (CL)
In the closing state, Service Fabric is in the process of shutting down the instance on this node. This shutdown might be due to many reasons. For example, an application upgrade, load balancing, or the service being deleted. After shutdown finishes, it transitions to the dropped state.
...
If the application host or the node for an InBuild replica crash, it transitions to the down state.
   ...
   - **IdleSecondary InBuild replicas**: These are either new replicas that are created by the Cluster Resource Manager, or they can also be existing replicas that went down and need to be added back into the set. These replicas are seeded or built by the primary before they can join the replica set as ActiveSecondary and participate in quorum acknowledgement of operations.
...
If the application host or the node for a ready replica crash, it transitions to the down state.

### Closing (CL)
A replica enters the closing state in the following scenarios:

- **Shutting down the code for the replica**: Service Fabric might need to shut down the running code for a replica. This shutdown might be for many reasons, for example, it can happen due to an application, fabric, or infrastructure upgrade or due to a fault reported by the replica. When the replica close finishes, the replica transitions to the down state. The persisted state associated with this replica that's stored on disk is not cleaned up.

- **Removing the replica from the cluster**: Service Fabric might need to remove the persisted state and shut down the running code for a replica. This shutdown might be for many reasons, for instance, due to load balancing.
...
A down replica is opened by Service Fabric as required, for instance, when the upgrade finishes on the node.
...
- **ActiveSecondary (S)**: These are replicas that receive state updates from the primary, apply them, and then send back acknowledgements. There are multiple active secondaries in the replica set and the number of these active secondaries determines the number of faults the service can handle.
...
The following diagram illustrates the replica role transitions, and we give some example scenarios in which they can occur:
...
- I -> S: Promotion of the idle secondary to active secondary so that its acknowledgements contribute towards quorum.
