...
1. **Cluster Aware Patching and Health Integration**: Patch Orchestration Application, 
    ...
    If at any point of time it detects health of the cluster going down due 
...
lead to availability loss as multiple / quorum of nodes can restart at the same time to complete the update.
...
However till then, we recommend configuring Windows Azure Diagnostics (WAD) to have the logs uploaded from all nodes to a central location.
...
case your Service Fabric cluster comprises of multiple nodetypes then the below
...
Below are the configurations, which can be set by the user to tweak
...
| WUQuery               | string<br>(Default : "IsInstalled=0")                | Query to get windows updates, refer to criteria field at https://msdn.microsoft.com/library/windows/desktop/aa386526(v=vs.85).aspx for more details.
...
One can access the update results from Patch Orchestration Application by logging into the cluster and then finding out the replica address for the primary of coordinator service and hitting the url from the browser.
http://&lt;REPLICA-IP&gt;:&lt;ApplicationPort&gt;/PatchOrchestrationApplication/v1/GetWindowsUpdateResults
The REST endpoint for CoordinatorService will have dynamic port,
...
In case reverse proxy is enabled on the cluster, one can access the URL from outside of the cluster as well by hitting the cluster endpoint.
...
Till that time logs can be collected using one of the following ways
...
A. While the patches are being installed, Patch Orchestration Application
...
Eg: In below case Cluster went to error state temporarily due to 2 down nodes and
MaxPercentageUnhealthNodes policy got violated. Do note that this would happen temporarily till patching operation is ongoing.
...
application is causing this. In most cases this would
...
Please try to bring your cluster to healthy state for unblocking the Patch Orchestration Application workflow.
...
- Patch Orchestration Application collects telemetry for keeping track of
...
application/cluster, Patch Orchestration Application will not allow any
subsequent Windows Update operation until the cluster becomes healthy
again.

Administrator will have to intervene and understand why a Windows Update is causing
