...
*ServiceFabricPartitionID* is the string representation of the Service Fabric partition ID that the performance counter instance is associated with. The partition ID is a GUID(C#)/UUID(Java), and its string representation is generated through the [`Guid.ToString`](https://msdn.microsoft.com/library/97af8hh4.aspx) method with format specifier "D" or `UUID.toString()`.
...
| ActorMethodStop (.NET) |8 |Verbose |0x2 |An actor method has finished executing. That is, the runtime's asynchronous call to the actor method has returned, and the task returned by the actor method has finished. |
| ActorMethodFinish (Java) |8 |Verbose |0x2 |An actor method has finished executing. That is, the runtime's asynchronous call to the actor method has returned, and the task returned by the actor method has finished. |
...
| ActorSaveStateStop (.NET) |11 |Verbose |0x4 |Actors runtime has finished saving the actor state. |
| ActorSaveStateFinish (Java) |11 |Verbose |0x4 |Actors runtime has finished saving the actor state. |
