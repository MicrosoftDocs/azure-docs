...
Be aware that most of the time we’ve seen customers use these throttles it has been because
-	They were already in a resource constrained environment (such as limited network bandwidth into individual nodes) which meant that such operations wouldn’t succeed or would be slow anyway AND
-	They were comfortable knowing that they were potentially extending the amount of time it would take the cluster to reach a stable state, including knowing that they could end up running at a lower overall reliability while they were throttled.
