...
There is only one SetupEntrypoint, so setup/config scripts need to be bundled in a single batch file if the application's setup/config requires multiple scripts. Like the SetupEntryPoint element, SetupEntrypoint can execute any type of file--executable files, batch files, and PowerShell cmdlets.
'<--LocComment: Should the second "SetupEntrypoint" be changed to a different name? -->'
