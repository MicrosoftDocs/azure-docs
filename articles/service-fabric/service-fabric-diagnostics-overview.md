...
Monitoring and diagnostics are critical to developing, testing, and successfully deploying production evironments. Service Fabric solutions work best when you implement monitoring and diagnostics from the ground up, helping to ensure that services work seamlessly both in local development environments and in the real-world production cluster setup.

The main goals of monitoring and diagnostics are to:
* Detect and diagnose hardware and infrastructure issues
* Detect software and app inssues, reduce service downtime
...
More broadly, monitoring and diagnostics is extremely important when it comes to guaranteeing that everything is working as expected, and enabling you to respond to any kind of situation with minimum disruption of service. 
...
2. **Event aggregation**: generated events need to be collected and aggregated before they can be displayed; this can be through storage tables as set up by Windows Azure Diagnostics or simply be creating an EventFlow pipeline
...
Beyond this, depending on your needs, you can further extend diagnostics and monitoring to include automated alerts and mitigation, which often come built into the analytics tools you may choose to use. 
...
It is highly recommended that on cluster creation, "Diagnostics" are enabled. This can be done in the portal as seen below or via using an Azure Resource Manager template the includes diagnostics. 
