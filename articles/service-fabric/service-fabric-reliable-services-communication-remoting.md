<properties
   ...
   description="Service Fabric remoting allows clients and services to communicate with services using a remote procedure call."
...
For services that are not tied to a particular communication protocol or stack, such as WebAPI, WCF, or others, the framework provides a remoting mechanism to quickly and easily set up a remote procedure call for services.
...
Calling methods on a service using the remoting stack is done using a local proxy to the service through the **Microsoft.ServiceFabric.Services.Remoting.Client.ServiceProxy** class. The **ServiceProxy** method creates a local proxy by using the same interface that the service implements. With that proxy, you can simply call methods on the interface remotely.
