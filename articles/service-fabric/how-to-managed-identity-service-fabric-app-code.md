...
- 'IDENTITY_ENDPOINT': the localhost endpoint path corresponding to that service's managed identity
- 'IDENTITY_HEADER': an authentication code, a unique string represents the service on the current node
- 'IDENTITY_THUMBPRINT' : Thumbprint of service fabric managed identity server
> [!NOTE]
> The names 'MSI_ENDPOINT' and 'MSI_SECRET' refer to the previous designation of managed identities ("Managed Service Identity"), and which is now deprecated. 'IDENTITY_ENDPOINT' and 'IDENTITY_HEADER' are the new corresponding variables respectively. The names are consistent with the equivalent environment variable names used by other Azure services which support managed identities.
