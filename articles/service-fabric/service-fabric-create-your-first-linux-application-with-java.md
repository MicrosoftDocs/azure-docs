...
> [!Note]
> The test client uses the ActorProxy class to communicate with actors, which must run within the same cluster as the actor service.  You can run the test client on the same computer as the local development cluster.  To communicate with actors in a remote cluster, however, you must deploy a gateway on the cluster that handles external communication with the actors.
