...
> [!NOTE]
> After changing to automatic mode the cluster will be enrolled for the next upgrade period starting with a new release. New releases are announced on the [Service Fabric team blog](https://techcommunity.microsoft.com/t5/azure-service-fabric/bg-p/Service-Fabric). Per upgrade period the highest possible upgrade path is chosen, see [supported versions](https://docs.microsoft.com/en-us/azure/service-fabric/service-fabric-versions). The manual upgrade mode triggers an immediate upgrade. 
