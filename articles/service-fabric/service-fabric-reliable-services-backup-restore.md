...
* In the event of the permanent loss of an entire Service Fabric cluster or all nodes that are running a given partition (i.e., all nodes within the cluster). This may happen when, for example, if you aren’t geo replicated and your entire cluster is in one data center, and the entire data center goes down.
* Administrative errors whereby state accidentally gets deleted/corrupted. This may happen when an administrator with sufficient privilege erroneously delete the service.
* Bugs in the service causing data corruption. This may happen when, for example, there was a service upgrade that writes faulty data to a collection, with both the service and the data having to be reverted to an earlier state.
...
1. The service partition lost data. For example, the disk for two out of three replicas for a partition (including the primary replica) gets corrupted/wiped. The data for the new primary has to be restored.
...
One way to do this is to deploy a new Service Fabric cluster and restore the backups of affected partitions just like the above "Deleted Service" scenario.  Once the last backup before the data corruption occurred is found, one can move/delete all backups of this partition after this backup.  Now, when **OnDataLossAsync** is called on the partition in the production cluster, the last backup found in the external store will be the one picked by the above process.
...
Reliable State Manager provides ability to restore from a backup by using the IReliableStateManager.RestoreAsync API.  The RestoreAsync method can only be called inside the OnDataLossAsync method.  The bool returned by OnDataLossAsync indicates whether the service restored its state from an external source.  If the OnDataLossAsync returns true, Service Fabric will rebuild all other replicas from this Primary.  Replica that receive OnDataLossAsync will first transition to Primary role, but it will not be granted Read Status or Write Status.  This implies that for StatefulService implementers, RunAsync will not be called until OnDataLossAsync completes successfully.  Until a service completes this API successfully (by returning true or false) and finishes the relevant reconfiguration, the API will keep being called.
