...
Firstly Service Fabric has a choice of three partition schemes.

- Ranged partitioning (otherwise known as UniformInt64Partition)
- Named partitioning. Applications using this model usually have data that is bucket-able, within a bounded set. Some common examples of data fields used as named partition keys would be regions, zip-codes, customer groups, or other business boundaries.
...
A common approach is to create a hash based on a unique key within the dataset. Some common examples of keys would be a vehicle identification number (VIN), employee ID, or a unique string. Using this unique key you would then generate a hash code, modulus the key range, to use as your key. You can specify upper and lower bounds of the allowed key range.


### Selecting a hash algorithm
An important part of hashing is selecting your hash algorithm. A consideration is whether the goal is to group similar keys near each other (locality sensitive hashing), or if activity should be distributed broadly across all partitions (distribution hashing), which is the more common.

Characteristics of good distribution hashing algorithm are that it is easy to compute, that is has little collisions and that it distributes the keys evenly. A good example of an efficient hash algorithm is the [FNV-1](https://en.wikipedia.org/wiki/Fowler%E2%80%93Noll%E2%80%93Vo_hash_function) hash algorithm.
...
>[AZURE.NOTE] This is a simplified scenario as in reality the distribution would be uneven. Last names starting with letter S or M are more common than the ones starting with X or Y.
...
8. A recommended pattern for the url a replica listens on is the following format: `{scheme}://{nodeIp}:{port}/{partitionid}/{replicaid}/{guid}` so you want to configure your communication listener to listen on the correct endpoints and with this pattern.
