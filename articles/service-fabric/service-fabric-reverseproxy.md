...
> [!WARNING]
> When you configure the reverse proxy's port in Load Balancer, all microservices in the cluster that expose an HTTP endpoint are addressable from outside the cluster. This means that microsservices meant to be internal may be discoverable by a determined malicious user. This potenially presents serious vulnerabilities that can be exploited by a malicious user; for example:
>
