...
- Impose automation, prevent manual or out-of-band changes, and validate specific changes against the actual environment before enacting them.
...
## Managing failures
The goal of Service Fabric is almost always automatic management of failures. But to handle some types of failures, services must have additional code. Other types of failures should _not_ be automatically addressed because of safety and business continuity reasons. 
...
Coordinated failures can happen in a cluster because of either planned or unplanned infrastructure failures and changes, or planned software changes. Service Fabric models infrastructure zones that experience coordinated failures as *fault domains*. Areas that will experience coordinated software changes are modeled as *upgrade domains*. For more information about fault domains, upgrade domains, and cluster topology, see [Describe a Service Fabric cluster by using Cluster Resource Manager](service-fabric-cluster-resource-manager-cluster-description.md).
...
#### Stateless services
`InstanceCount` for the Service indicates the desired number of instances that need to be running. When any (or all) of the instances fail, Service Fabric will respond by automatically creating replacement instances on other nodes. Service Fabric will continue to create replacements until the service is back to its desired instance count.

In another example, assume that the stateless service has an `InstanceCount` value of -1. This value means that one instance should be running on each of the nodes in the cluster. If some of those instances fail, Service Fabric will detect that service is not in its desired state and will try to create the instances on the nodes where they're missing.
...
In a stateful service, incoming data is replicated between replicas (primary and active secondary). If majority of the replicas receive the data, data is considered *quorum* committed. (For five replicas, three will be _quorum_.) This means that at any point, there will be at least a quorum of replicas with the latest data. If replicas fail (say two out of five replicas failed), we can use the quorum value to calculate if we can recover. (Because the remaining three out of five replicas are still up, it's guaranteed that at least one replica will have complete data.)
...
1. Determining if there has been actual data loss, and restoring from backups.
   ...
   At this point, Service Fabric also knows the other replicas are not coming back. That was the decision made when we stopped waiting for the quorum loss to resolve itself. The best course of action for the service is usually to freeze and wait for specific administrative intervention. 
   
   So what does a typical implementation of the `OnDataLossAsync` method do?
   1. First, the implementation logs that `OnDataLossAsync` has been triggered. And it fires off any necessary administrative alerts.
   1. Usually at this point, the implementation pauses and waits for further decisions and manual actions to be taken. This is because even if backups are available, they might need to be prepared. 
   ...
   1. The implementation compares the remaining replica's state to that contained in any backups that are available. If you're using Service Fabric reliable collections, there are [tools and processes](service-fabric-reliable-services-backup-restore.md) available for doing so. The goal is to see if the state within the replica is sufficient, or also what the backup might be missing.
   1. After the comparison is done, and after the restore completed (if necessary), the service code should return **true** if any state changes were made. If the replica determined that it was the best available copy of the state and made no changes, the code returns **false**. 
...
It's critically important that service authors practice potential data-loss and failure scenarios before services are ever deployed in production. To protect against the possibility of data loss, it's important to periodically [back up the state](service-fabric-reliable-services-backup-restore.md) of any of your stateful services to a geo-redundant store. 
...
If you find out that the remaining replicas are insufficient to continue from in a data-loss scenario, and you can't reconstruct the service state from telemetry or exhaust, the frequency of your backups determines your best possible recovery point objective (RPO). Service Fabric provides many tools for testing various failure scenarios, including permanent quorum and data loss that requires restoration from a backup. These scenarios are included as a part of the testability tools in Service Fabric, managed by the Fault Analysis Service. For more info on those tools and patterns, see [Introduction to the Fault Analysis Service](service-fabric-testability-overview.md). 
...
Replicas might be down intermittently because of a transient failure. Wait for some time as Service Fabric will tries bring them up. If replicas have been down for more than an expected duration, follow these troubleshooting actions:
...
- The replica process might have become unresponsive. Inspect your application logs to verify this. Collect process dump and then stop the unresponsive process. Service Fabric will create replacement process and will try to bring the replica back.
- Nodes hosting the replicas might be down. Restart the underlying virtual machine to bring the nodes up.

Sometimes, it might not be possible to recover replicas. For example, the drives might have failed or the machines physically are not responding. In these cases, Service Fabric needs to be told not to wait for replica recovery.
...
- Run a single Service Fabric cluster that spans multiple datacenters or regions. The minimum supported configuration for this is three datacenters or regions. The recommended number of regions or datacenters is five. 
...
Seed nodes help to ensure that the cluster stays up by establishing leases with other nodes and serving as tiebreakers during certain kinds of network failures. If random failures remove a majority of the seed nodes in the cluster and they're not brought back, then your cluster federation ring collapses because you've lost seed node quorum and the cluster fails. 
