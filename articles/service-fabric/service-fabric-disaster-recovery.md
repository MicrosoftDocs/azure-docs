...
Service Fabric has the concept of Seed Nodes. These are nodes that maintain the availability of the underlying cluster. These nodes help to ensure the cluster remains up by establishing leases with other nodes and serving as tiebreakers during certain kinds of network failures. If random failures remove a majority of the seed nodes in the cluster and they are not brought back, the cluster automatically shuts down. In Azure, Service Fabric Resource Provider manages Service Fabric cluster configurations, and by default distributes Seed Nodes across Primary Node Type fault and and upgrade domains; if you scale back you provisioned virtual machine capacity that is hosting a Seed Node, the cluster will attempt to create a another seed node on available capacity, and will fail if you have less available capacity than your cluster Reliability level requires for your Primary Node Type.
