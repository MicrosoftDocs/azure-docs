...
Configure Linux Azure Diagnostics (LAD) to collect the files and place them into the customers storage account. This is explained as scenario 3("Upload your own log files") in the article [Using LAD to monitor and diagnose Linux VMs](https://azure.microsoft.com/documentation/articles/virtual-machines-linux-classic-diagnostic-extension/). This gets you access to the traces, which can be uploaded to a visualizer of your choice.
...
After this configuration is done, the LAD agent monitors the log files specified.  Whenever a new line is appended to teh file, it creates a syslog entry that is sent to the storage specified by the customer.
