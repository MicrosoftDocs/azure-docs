...
> [!WARNING]
> It is important that the Fault Domain information provided to Service Fabric is accurate. For example, let's say that your Service Fabric cluster's nodes are running inside ten virtual machines, running on five physical hosts. In this case, even though there are ten virtual machines, there's really only five different (top level) fault domains - this defines the maximum reliability of the cluster. Though each virtual machines may contribute to the total number and layers of unique fault domains, sharing the same host causes them to have the same root fault domain, since they would experience coordinated failure if their shared host failed.  
>
> Since Service Fabric also expects the Fault Domain not to change during the lifetime of the node, other mechanisms of ensuring high availability of the VMs (such as [HA-VMs](https://technet.microsoft.com/en-us/library/cc967323.aspx) where the VM can be transparently migrated from one host to another without failure and reconfiguration of the running code inside the VM) are *not supported* as environments for running Service Fabric clusters. Service Fabric should be the only utilized high-availability technology employed - you don't need things like live VM migration, SANs, or other mechanisms. In fact, if used in conjunction with Service Fabric, these mechanisms frequently _reduce_ real application availability and reliability as they introduce additional complexity, centralized sources of failure, and reliability and availability strategies that conflict with steps taken by Service Fabric. 
>
>
...
During runtime, the Service Fabric Cluster Resource Manager considers the Fault Domains in the cluster and plans layouts.  It attempts to spread out the stateful replicas or stateless instances for a given service so that they are in separate Fault Domains. This process helps ensure that if there is a failure of any one Fault Domain (at any level in the hierarchy), that the availability of that service is not compromised.

Service Fabric’s Cluster Resource Manager doesn’t care how many layers there are in the Fault Domain hierarchy. However, it tries to ensure that the loss of any one portion of the hierarchy doesn’t impact services running in it. Because of this, it is best if there are the same number of nodes at each level of depth in the Fault Domain hierarchy. 

If the “tree” of Fault Domains is unbalanced in your cluster, it makes it harder for the Cluster Resource Manager to figure out the best allocation of services. Imbalanced Fault Domains layouts mean that the loss of a particular domain can impact the availability of services more than others. As a result, the Cluster Resource Manager is torn between two goals: It was to use the machines in that “heavy” domain by placing services on them, and it wants to place services in other domains so that the loss of a domain doesn’t cause problems. 

What does this look like? In the diagram below, we show two different cluster layouts. In the first example the nodes are distributed evenly across the Fault Domains. In the second example one Fault Domain has many more nodes that the other Fault Domains. 
...
## Upgrade domains
Upgrade Domains are another feature that helps the Service Fabric Cluster Resource Manager understand the layout of the cluster so that it can plan ahead for failures and management operations. Upgrade Domains define sets of nodes that are upgraded at the same time.
