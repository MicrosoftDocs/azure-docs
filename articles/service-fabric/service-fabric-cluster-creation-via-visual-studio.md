...
Every Service Fabric cluster has a name. When a Fabric cluster is created in Azure, cluster name determines (together with the Azure region domain) the Domain Name System (DNS) name for the cluster. For example, if you name your cluster `myBigCluster`, and the `clusterLocation` parameter is set to East US, the DNS name of the cluster will be `myBigCluster.eastus.cloudapp.azure.com`.

By default the cluster name is generated automatically and made unique by attaching a random suffix to a "cluster" prefix. This makes it very easy to use the template as part of a **continuous integration** (CI) system. If you want to use a specific name for your cluster, one that is meaningful to you, set the value of the `clusterName` variable in the ARM template file (`ServiceFabricCluster.json`) to your chosen name. It is the first variable defined in that file.
