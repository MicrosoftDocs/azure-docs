...
A Service Fabric application can contain one or more services--each with a specific role in delivering the application's functionality. With the New Project wizard, you can create an application project, along with your first service project. You can add more services later.
...
4. On the next page, you will be asked to choose the first service type to include in your application. For the purposes of this tutorial, we will choose **Stateful**. Name it and click **OK**.
	...
	- **Publish profiles**: Profiles that are used to manage tooling preferences for different environments.
	...
	- **Application definition**: The application manifest and associated application parameter files that define the application and allow you to configure it specifically for a given environment.
