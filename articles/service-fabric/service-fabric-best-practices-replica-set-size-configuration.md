...
## Replica set size
Replica set size for stateful services is configured using 2 parameters.
...
The basic idea behind these 2 parameters is to allow for such a configuration that it allows for at least 2 concurrent failures to happen without partition going in quorum loss. That situation can happen when there is one planned failover (e.g., upgrade bringing node/replica down) and one unplanned failover (e.g., replica crashes).
...
This kind of configuration will often go into quorum loss (whenever planned and unplanned failover happens at the same time). To recover from quorum loss, it is not enough for only one replica to come back up – it is required for the exact replica which was part of replica set to come back up.

![image1]
1.	Partition has 3 replicas: A, B, C
...
4.	If replica A comes back, partition will remain in quorum loss state as A is not part of the current replica set (B, C). Quorum loss will be fixed only when replica B comes back.
