...
8.	Select **Create**.
9.	Switch to the **Variables** tab and add the following variables:
   1. Name: BuildConfiguration, Value: Release
   2. Name: BuildPlatform, Value: x64
9. Switch back to the **Build** tab and add the following build steps to the definition:
   1. NuGet Installer
   2. Visual Studio Build (name: Build solution)
      1. Platform: $(BuildPlatform)
      2. Configuration: $(BuildConfiguration)
   3. Visual Studio Build (name: Package application)
      1. Solution: \*\*\\*.sfproj
      2. MSBuild Arguments: /t:Package /p:PackageLocation=$(build.artifactstagingdirectory)\applicationpackage
      3. Platform: $(BuildPlatform)
      4. Configuration: $(BuildConfiguration)
   4. Update Service Fabric App Versions
      1. Application Package: $(build.artifactstagingdirectory)\applicationpackage
   5. Copy Files
      1. Source Folder: $(build.sourcesdirectory)
      2. Contents: \*\*\PublishProfiles\\\*.xml &lt;new line&gt; \*\*\ApplicationParameters\\\*.xml
      3. Target Folder: $(build.artifactstagingdirectory)\projectartifacts
   6. Publish Build Artifacts
      1. Path to Publish: $(build.artifactstagingdirectory)
      2. Artifact Name: drop
      3. Artifact Type: Server
10. Save the build definition and provide a name.

### Verify the default field values of the tasks
