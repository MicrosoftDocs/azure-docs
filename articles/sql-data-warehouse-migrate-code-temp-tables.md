...
However, in SQL Data Warehouse you can use that temporary table outside of the procedure that created it. This behavior is different than SQL Server. In fact the temporary table can be used anywhere inside the session.

This example uses temporary tables and their availability within the session to produce more modular and manageable code.
...
In some cases, you can use this technique to replace inline and multi-statement functions.

> [AZURE.NOTE] You can extend this solution. For example, to update a single table simply filter the #stats_ddl table.