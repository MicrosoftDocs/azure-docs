...
![2Green](./media/best-practices-availability-paired-regions/2Green.png) **Azure Storage** - Geo-Redundant storage (GRS) is configured by default when an Azure Storage account is created. With GRS, your data is replicated three times within the primary region, and three times in the paired region. GRS only permits replication to the paired region and cannot be configured differently. For more information,  see [Azure Storage Redundancy Options](./storage-redundancy/).
