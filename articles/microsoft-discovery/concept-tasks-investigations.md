---
title: Tasks and shared sessions in Microsoft Discovery
...
# Tasks and shared sessions

Tasks are how you define the work you want the [Discovery Engine](concept-discovery-engine.md) to carry out. Each task represents a discrete piece of work with a clear objective and success criteria. When [cognition](concept-cognition-overview.md) is running, it reads your tasks, understands their relationships, and orchestrates execution across agents and tools.
...
| Field | Purpose |
|-------|---------|
| **Name** | A concise name that summarizes what the task is intended to accomplish. Keep it short but specific. |
| **Shared session** | The shared session the task runs in. |
...
| **Dependent tasks (depends on)** | Other tasks that must complete before this task can start. Cognition respects these dependencies when sequencing work. |
...
| **Updates** | Notes, observations, and feedback from you, from agents, and from the validation process. Comments accumulate over the task's lifetime and provide a running record of decisions and context. |
...
- A **shared session** that the task belongs to.
...
Dependencies, parent relationships, and priority are optional but become important as your shared session grows. They give cognition the information it needs to sequence work correctly and focus effort where it matters most.

## Task status lifecycle

Every task moves through a series of statuses as it progresses from creation to completion. Understanding these statuses helps you interpret what's happening in your shared session and when to intervene.
...
|--------|--------------|-------------------|
| **New** | The task is created but cognition isn't working on it yet. It might be waiting for dependencies to complete, or cognition might not have reached it in its planning cycle yet. | If it stays New for a long time, check whether its dependencies are stuck or whether the Discovery Engine is running. |
...
| **Validating** | The agent finished its work and produced a result, but validation isn't yet complete. Cognition is evaluating the result against your validation requirements. The system sets this status automatically. | Normally you don't need to act on this status. Cognition moves tasks out of this state quickly. If a task stays here, the Discovery Engine might be stopped or might have encountered an issue. |
...
| **Removed** | The task was intentionally removed from the shared session. Use this status for tasks that are no longer relevant. | No action needed. Cognition ignores Removed tasks. |
...
> [!IMPORTANT]
> If you change a task's status while cognition is running, cognition might override the change based on the information collected in working memory. While the Discovery Engine is running, let cognition manage the task lifecycle.
...
A dependency means "don't start this task until the dependency reaches a completed state." Cognition checks dependencies before assigning an agent.
...
- **Context**: Why does this matter? What should the agent know about the broader shared session?
...
For guidance on balancing validation strictness, see [Trust relationship and basic shared session patterns](concept-trust-basic-investigation-patterns.md).
