...
A search service initiates the request to create a shared private link, but Azure Resource Manager performs the actual work. You can [check the deployment's status](search-indexer-howto-access-private.md#2--check-the-status-of-the-private-endpoint-creation) in the portal or by query, and address any errors that might occur.
