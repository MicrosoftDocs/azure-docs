...
This tutorial uses the hotels-sample-index, which you can create quickly on your own search service by stepping through the [Import data quickstart](https://learn.microsoft.com/azure/search/search-get-started-portal). The index contains fictitious hotel data, available as a built-in data source in every search service.
...
* [1-basic-search-page (GitHub)](https://github.com/Azure-Samples/azure-search-sample-archive/tree/main/create-first-app/v11)

## Prerequisites

* [Create](https://learn.microsoft.com/azure/search/search-create-service-portal) or [find an existing search service](https://portal.azure.com/#blade/HubsExtension/BrowseResourceBlade/resourceType/Microsoft.Search%2FsearchServices).

* [Sample index (hotels-sample-index)](https://learn.microsoft.com/azure/search/search-get-started-portal), hosted on your search service.
...
1. Locate the sample on GitHub: [Create first app](https://github.com/Azure-Samples/azure-search-sample-archive/tree/main/create-first-app/v11).

1. At the [root folder](https://github.com/Azure-Samples/azure-search-sample-archive/tree/main/create-first-app/v11), select **Code**, followed by **Clone** or **Download ZIP** to make your private local copy of the project.
...
In this step, set the endpoint and access key for connecting to the search service that provides the [hotels sample index](https://learn.microsoft.com/azure/search/search-get-started-portal).

1. Open **appsettings.json** and replace the default lines with the search service URL (in the format `https://<service-name>.search.windows.net`) and an [admin or query API key](https://learn.microsoft.com/azure/search/search-security-api-keys) of your search service. Since you don't need to create or update an index, you can use the query key for this tutorial.
...
1. Try entering "five star". Notice that this query returns no results. A more sophisticated search would treat "five star" as a synonym for "luxury" and return those results. Support for [synonyms](https://learn.microsoft.com/azure/search/search-synonyms) is available in Azure Cognitive Search, but isn't be covered in this tutorial series.
