...
The indexer solution is part of a collection of Azure Search samples delivered in the same download. The sample used in this tutorial is called *DotNetHowToIndexers*.
...
2. Click **Clone or Download** and then choose **Download ZIP**. By default, the file goes to the Downloads folder.

3. In File Explorer, right-click the file and choose **Extract all**.

4. On the extracted folder, turn off read-only permissions. Right-click the folder name > **Properties** > **General**, and then clear the **Read-only** attribute for the current folder, subfolders, and files.
...
## Set up connections
Connection information to services and datasets is specified in the *appsettings.json* file in the solution. 
...
Search service endpoint and key information is in the portal. A key provides access to service operations. Admin keys enable write-access, necessary for creating and deleting objects, such as indexes and indexers, in your service.
...
3. From **Essentials**, copy the URL and paste it into the "SearchServiceName" item in appsettings.json. The URL should look like this: `https://<your-service-name>.search.windows.net`

4. On the left, in **Settings** > **Keys**, copy one of the admin keys and paste it into the "SearchServiceAdminApiKey" item in the apsettings.json file. Keys are alphanumeric strings generated for your service during provisioning.
...
In this step, create an external data source that an indexer can crawl. If you aren't familiar with any of the data sources used in this tutorial, choose Azure SQL Database. This tutorial offers more details for indexing from that platform.
...
The following instructions assume no existing server or database, creating both resources in step 2. If you have an existing resource, you can add the hotels table, starting at step 4.

1. Sign in to the [Azure portal](https://portal.azure.com/). 

2. Click **New** > **SQL Database** to create a database, server, and resource group. You can use defaults and the lowest level tier. One advantage to creating a server is that you can specify an administrator user name and password, which you need to have for query execution in a later step.
...
6. Click **Open Query** and navigate to the data folder containing *hotels.sql*. Choose that file to open. By default, the file location is \Downloads\search-dotnet-getting-started-master\search-dotnet-getting-started-master\DotNetHowToIndexers\DotNetHowToIndexers\data\. The script should look similar to the following screenshot.
...
8. To return a rowset from this table, you can execute the following query:

   ```sql
   SELECT HotelId, Category, HotelName, Tags from Hotels
   ```

   Note that `SELECT * FROM Hotels` doesn't work in the Query Editor. The sample data includes geographic coordinates in the Location field, which is not handled in the editor at this time. For a list of other columns to query, you can execute this statement: `SELECT * FROM sys.columns WHERE object_id = OBJECT_ID('dbo.Hotels')`

9. Now that you have an external dataset, copy the ADO.NET connection string for the database. On the SQL Database page of your database, go to **Settings** > **Connection Strings**, and copy the ADO.NET connection string.
