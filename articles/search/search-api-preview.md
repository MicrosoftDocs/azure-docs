...
| [**Azure RBAC support**](search-security-rbac.md) | Security | Use new built-in roles to control access to indexes and indexing, eliminating or reducing the dependency on API keys. | [Sign up](SIGN-UP-URL-HERE) is required so that support can be enabled for your subscription on the backend. Use Azure portal or [Create or Update Service](/rest/api/searchmanagement/2021-04-01-preview/services/create-or-update) (Management REST API version 2021-04-01-Preview) to enable data plane authorization on a search service. |
