...
Throughout this tutorial series, results have been returned and displayed in a [default order](https://learn.microsoft.com/azure/search/index-add-scoring-profiles#what-is-default-scoring). In this tutorial, you will add primary and secondary sort criteria. As an alternative to ordering based on numerical values, the final example shows how to rank results based on a custom scoring profile. We will also go a bit deeper into the display of _complex types_.
...
* [5-order-results (GitHub)](https://github.com/Azure-Samples/azure-search-sample-archive/tree/main/create-first-app/v11)
...
[Scoring profiles](https://learn.microsoft.com/azure/search/index-add-scoring-profiles) are defined in the index schema. Several scoring profiles have been set up on the hotels data. Let's look at how a scoring profile is defined, then try writing code to search on them.
...
For more information, see the following [Add scoring profiles to an Azure Cognitive Search index](https://learn.microsoft.com/azure/search/index-add-scoring-profiles).
