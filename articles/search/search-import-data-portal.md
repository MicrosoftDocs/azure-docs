...
description: Use the Azure Search Import Data Wizard in the Azure Portal to crawl Azure data from NoSQL Cosmos DB, Blob storage, table storage, SQL Database, and SQL Server on Azure VMs.
services: search
documentationcenter: ''
author: HeidiSteen
manager: jhubbard
...
You can try out this workflow using sample data in Cosmos DB. Visit [Get started with Azure Search in the Azure Portal](search-get-started-portal.md) for instructions.
...
| **Cosmos DB** |Requirements include the account, database, and collection. All documents in the collection will be included in the index. You can define a query to flatten or filter the rowset, or to detect changed documents for subsequent data refresh operations. |
...
* [Indexing Cosmos DB](search-howto-index-documentdb.md)
