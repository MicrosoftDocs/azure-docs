...
1. The body of the request specifies the following built-in skills:
   ...
   | [Text Merge](cognitive-search-skill-textmerger.md)  | Creates "merged content" that reunites previously separated content, useful for files that include both text and images (PDF, DOCX, and so forth). Images and text are separated during the document cracking phase. The merge skill recombines them by inserting any recognized text, image captions, or tags created by image analysis into the same location where the image was extracted from in the document. </p>Merged content is inclusive of all content, in all documents in the source data set, regardless of whether the source file is text-only, image-only, or text with embedded images. In a mixed file data set, the output of Text Merge should be the input of all subsequent downstream skills. |
