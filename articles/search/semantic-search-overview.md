...
Semantic search is a collection of query-related features that add semantic relevance and language understanding to search results. *Semantic relevance* is used during ranking to look for context and relatedness among terms, elevating matches that make more sense to the query posed by the user. Language understanding finds *captions* and *answers* within your content that summarize the matching document or answer a question, which can then be rendered on a search results page for a more intuitive search experience.
