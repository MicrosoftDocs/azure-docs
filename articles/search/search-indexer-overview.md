...
You can use an indexer as the sole means for data ingestion, or use a combination of techniques that include the use of an indexer for loading just some of the fields in your index. The following table summarizes the main patterns of indexer usage.

| Scenario |Strategy |
|----------|---------|
| Single source | This is the simplest pattern: one Azure data source is the sole content provider for all fields in a search index. From the source, you'll identify one field containing unique values to use as the document key in the search index. The unique value will be used to identify each search document in the index. All other source fields are mapped implicitly or explicitly to other fields in an index. </br></br>The value of a document key comes from source data. A search service does not generate document keys for you. On subsequent runs, incoming documents with new key values are added, while incoming documents with existing key values are either merged or overwritten in the search index, depending on whether index fields are null or populated. |
| Multiple sources, different documents | A single index can accept content from multiple sources. For example, you could populate a search index using content from two SQL tables, or from a Blob container and a SQL table. The result will be an index containing identically structured search documents, populated with content from multiple sources. </br></br>The challenge for this scenario lies in designing an index schema that works for all incoming data, and a document key that is uniform in the search index. For example, if the values that uniquely identify a document are metadata_storage_path in a blob container and a primary key in a SQL table, you can imagine that one or both sources must be amended to provide key values in a common format, regardless of content origin. For this scenario, you should expect to perform some level of pre-processing to homogenize the data so that it can be pulled into a single index. |
| Multiple sources, same documents | A single search document in an index can accept values from different sources. Recall that indexer-based indexing relies on field mappings. As such, fields in an index can receive data from different data sources. Fields 1 through five might be from one data source, and six through 10 from another. For an example of this scenario, see [Tutorial: Index from multiple data sources](tutorial-multiple-data-sources.md).</br></br> The challenge of this pattern is making sure that each indexing run is targeting the same document keys. As with the previous pattern, pre-processing step to ensure that both data sets have common document keys is probably necessary.|
| Content transformation | Indexers drive the [AI enrichment](cognitive-search-concept-intro.md) behaviors of Azure Cognitive Search. AI enrichment is defined in a [skillset](cognitive-search-working-with-skillsets.md) that adds image analysis and natural language processing. A skillset is attached to an indexer. Running the indexer invokes the attached skillset. |
