...
| ---- | ---------------------------- |
| [Owner](../role-based-access-control/built-in-roles.md#owner) | Full access to the search resource, including the ability to assign Azure roles. Subscription administrators are members by default. This role is generally available.|
| [Contributor](../role-based-access-control/built-in-roles.md#contributor) | Same level of access as Owner, minus the ability to assign roles or change authorization options. This role is generally available. |
| [Reader](../role-based-access-control/built-in-roles.md#reader) | Limited access to partial service information. In the portal, the Reader role can access information in the service Overview page, in the Essentials section and under the Monitoring tab. All other tabs and pages are off limits. </br></br>This role has access to service information: resource group, service status, location, subscription name and ID, tags, URL, pricing tier, replicas, partitions, and search units. </br></br>This role also has access to service metrics: search latency, percentage of throttled requests, average queries per second. </br></br>There is no access to API keys, role assignments, content (indexes or synonym maps), or content metrics (storage consumed, number of objects). This role is generally available. |
