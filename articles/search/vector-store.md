...
Here's what a vector field looks like in a fields collection:
...
Vector fields, such as `"content_vector"` in the example, is of type `Collection(Edm.Single)`. Vector fields must be searchable and retrievable. They can't be filterable, facetable, or sortable, and they can't have analyzers, normalizers, or synonym map assignments. 
...
Vector fields are indexed using algorithms passed through a *vector search profile*, which isn't shown in the example. For more information, see [vector search configuration](vector-search-how-to-create-index.md).
