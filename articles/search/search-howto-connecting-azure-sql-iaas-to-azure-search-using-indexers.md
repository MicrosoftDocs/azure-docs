...
## Step 1: Enable encrypted connections
...
* The subject name of the SSL certificate must be the fully qualified domain name (or **FQDN**) of the SQL Azure VM. It's the same FQDN you'll specify in the database connection string when creating a data source in your search service. An FQDN is formatted as `<your-VM-name>.<geo-region>.cloudapp.azure.com`. You can find the FQDN of your SQL Azure VM as the DNS name label in the [Azure portal](https://portal.azure.com/).
...
## Step 2: Configure the firewall to allow connections from Azure Search
