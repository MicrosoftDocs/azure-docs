...
**Task Duration:** About 30 minutes, assuming you already installed a certificate the VM.

## Enable encrypted connections

First, enable encrypted connections from Azure Search to the SQL Azure VM. Azure Search requires an encrypted channel for all indexer requests over a public internet connection. This section lists the steps to make this work.

1. In certificate properties, verify the subject name of the certificate is the fully qualified domain name (FQDN) of the Azure VM. You can get the FQDN from the VM service blade's Essentials section, in the **Public IP address/DNS name label** field, in the [Azure portal](https://portal.azure.com/).
...
The following recommendations assume you already understand your NSG configuration and how to ACL an Azure Search endpoint based on its IP address.
...
If your search service has only one search unit (that is, one replica and one partition), the IP address changes during routine service restarts, invaliding an existing ACL with your search service's IP address.
...
A second approach is to allow the connection to fail, and then reconfigure the ACLs in the NSG. Although there is no guarantee, on average IP addresses change every few weeks. For customers who do controlled indexing on an infrequent basis, this approach might be viable.
