...
REST API version `2019-05-06-Preview` provides incremental enrichment through additional properties onindexers, skillsets, and data sources. In addition to the reference, see  [Configure caching for incremental enrichment](search-howto-incremental-index.md) for more details.

| APIs | Modified for incremental enrichment |
|------|-------------------------------------|
| [Create Indexer](https://docs.microsoft.com/rest/api/searchservice/create-indexer) | New properties. `StorageAccountConnectionString` specifies the storage account used to cache the intermediate results.
...
| [Update Indexer](https://docs.microsoft.com/rest/api/searchservice/update-indexer) | Same properties as those noted for Create Indexer. |

| [Update Skillset](https://docs.microsoft.com/rest/api/searchservice/update-skillset) | New parameter on the request: `disableCacheReprocessingChangeDetection`, which should be set to `true` when you want no updates to existing documents based on the current action.
|[Reset Skills](https://docs.microsoft.com/rest/api/searchservice/2019-05-06-preview/reset-skills) | A new operation used to invalidate a skillset so that you can rebuild the cache. |
