...
| [Reset Skills](preview-api-resetskills.md) | Specifies which skills to re-run on the next indexer run, even if you haven't modified any skills. The cache is updated accordingly. If a skillset includes a knowledge store, Reset Skills will update the projections. As such, if your objective is to rebuild or restore contents of a knowledge store, using Reset Skills and Run Indexer is a mechanism for doing so. |
