...
| [**Support for built-in vector quantization, narrow vector primitive data types, and new `stored` property (preview)**](vector-search-how-to-configure-compression-storage.md) | Feature | This preview adds *scalar quantization* that reduces vector index size in memory and on disk, [narrow data types](/rest/api/searchservice/supported-data-types), and more flexible vector field storage options.|
