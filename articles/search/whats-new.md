...
| [Double encryption availability](search-security-manage-encryption-keys.md#double-encryption) | For search indexes and objects that are protected through customer-managed keys, double encryption (encryption of static and temporary disks) is now implemented in all supported regions. | In all regions, subject to [service creation dates](search-security-manage-encryption-keys.md#double-encryption). |
