...
|------------------------------|---------------|---------------|
| [Semantic search](semantic-search-overview.md) | A collection of query-related features that improve the relevance of search results with very little effort. </br></br>[Semantic ranking](semantic-ranking.md) significantly improves the relevance of search results with very little effort. With small changes to a search request, you can improve relevance scores for queries on existing indexes.</br></br>[Semantic answers](semantic-answers.md) consist of sentences or phrases extracted from a search document that directly answers the query. </br></br>[Semantic query](semantic-how-to-query-request.md) is a query type that enables semantic ranking and answers. The response includes *captions*, with *highlights* over the most relevant terms and phrases.| Public preview ([by request](https://aka.ms/SemanticSearchPreviewSignup)). </br></br>Use [Search Documents (REST)](/rest/api/searchservice/preview-api/search-documents) api-version=2020-06-30-Preview and [Search explorer](search-explorer.md) in Azure portal. </br></br>Region and tier restrictions apply. |
