...
|------------------------------|---------------|---------------|
| [Semantic search](semantic-search-overview.md) | A collection of query-side features that modify search results to produce more intuitive outcomes. With small changes to query syntax, you can try out these features on existing search indexes.</br></br>[Semantic query](semantic-how-to-query-request.md) is a new query type that adds natural language understanding to query outcomes. Formulation of a semantic query invokes semantic ranking and responses.</br></br>[Semantic ranking and responses (answers and captions)](semantic-how-to-query-response.md) refer to a new search ranking algorithm that uses deep neural networks to re-rank results. Initial query results are still generated by the search engine, but those results are then pushed through state-of-the-art relevance models to deliver an intuitive match. </br></br>**Semantic answers** uses language representation models from Bing to extract and formulate 3 answers using relevant passages from the topmost matching documents, and then places those answers at the head of the results. </br></br>**Semantic captions** are similar to semantic answers, but instead extracts and returns the most relevant section of each of the top documents. You can quickly skim results to find the content you care about. </br></br> | Public preview ([by request](https://aka.ms/TBD)). </br></br>Use [Search Documents (REST)](/rest/api/searchservice/preview-api/search-documents) api-version=2020-06-30-Preview and [Search explorer](search-explorer.md) in Azure portal. </br></br>Region and tier restrictions apply. |
| [Spell check query terms](speller-how-to-add.md) | Before a query string reaches the search engine, you can have it checked for spelling errors. The `speller` option works with any query type (simple, full, or semantic). |  Public preview, REST only, api-version=2020-06-30-Preview|
