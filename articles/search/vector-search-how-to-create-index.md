...
1. Add vector fields to the fields collection. You can store one generated embedding per document field. For each field:
   ...
   + "retrievable" set to "true" allows you to display the raw vectors (for example, as a verification step), but doing so increases the size of the index.
   + "filterable", "facetable", "sortable" must be set to `false` because they aren't supported for vector fields.
