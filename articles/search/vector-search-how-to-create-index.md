...
+ scalar
+ binary (available in 2024-07-01 only)
...
+ `vectorSearch.compressions` with properties for scalar and binary quantization, oversampling, and reranking with original vectors.
...
1. Define a vector field with the following attributes. You can store one generated embedding per field. For each vector field:
   ...
   + `stored` can be true or false. It determines whether an extra copy of vectors is stored for retrieval. For more information, see [Reduce vecctor size](vector-search-how-to-configure-compression-storage.md).
...
You can use pull APIs (indexers, data sources, and skillsets) for integrated vectorization during indexing and query time.

+ Data sources must be a supported type.

+ Skillsets provide data chunking (Text Split skill) and skills that connect to embedding models. A few are generally available, others are still in preview. Skills and vectorizers are used to generate embeddings. For vectorization during indexing, choose from the following skills:
...
+ Indexes provide the vector field definitions and vector search configurations.

+ Indexers drive the indexing pipeline

If you're familiar with indexers and skillsets:

+ Field mappings, output field mappings, and deletion detection setttings apply to vector and nonvector fields equally.
