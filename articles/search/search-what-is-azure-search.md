...
+ [**Indexing**](search-what-is-an-index.md) is an intake process that loads content into your search service and makes it searchable. Internally, inbound text is processed into tokens and stored in inverted indexes for full text search. Inbound vectors are stored in [vector indices in an embedding space](vector-search-how-to-index.md) within a search index. You can upload JSON documents, or use an indexer to serialize your data into JSON.
