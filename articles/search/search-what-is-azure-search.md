...
Architecturally, a search service sits in between the external data stores that contain your un-indexed data, and a client app that sends query requests to a search index and handles the response. The two primary workloads of a search service are *indexing* and *querying*.
