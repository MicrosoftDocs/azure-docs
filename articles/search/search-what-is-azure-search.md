...
| Compared%20to | Key differences |
|--|--|
|Bing | Bing offers search solutions for web content, crawling a web site and web content types (HTML, XML, and so forth) for search terms. <br/><br/>Azure Search is for private data you own, stored in an index you create. Often, data originates from heterogeneous content types, ranging from databases to doc files to geospatial coordinates, or even metadata associated with video or images. |
|Database search | SQL Server and other database platforms often have full text search functionality for content internal to the DBMS. <br/><br/>Azure Search indexes can store content from heterogeneous data sources and offers specialized text processing features such as language and custom analyzers. The [full text search engine](search-lucene-query-architecture.md) in Azure Search is built on Lucene, an industry standard in information retrieval. <br/><br/>Resource utilization is another inflection point. Natural language search is often computationally intensive. Offloading search operations to a dedicated solution preserves machine resources for transaction processing. By externalizing search, you can easily adjust scale to match query volume.|
|Dedicated search solution | On premises or cloud service solutions are dedicated search solutions with full spectrum funtionality. Search technologies typically offer control over indexing or query pipelines, access to richer query and filtering syntax, control over rank and relevance, and features for self-directed and intelligent search. <br/><br/>Dedicated search is offered as a cloud service or an on-premises server. A cloud service is the right choice if you want a [turn-key solution with minimal overhead and maintenance, and adjustable scale](#cloud-service-advantage). <br/><br/>Within the cloud paradigm, several providers offer comparable baseline features, with full-text search, geo-search, and the ability to handle a certain level of ambiguity in search inputs. Typically, it's a [specialized feature](#feature-drilldown), or the ease and overall simplicity of APIs, tools, and management that determines the best fit. |
...
> [!Note]
> Nn-Azure data sources are fully supported, but rely on a more code-intensive push methodology rather than indexers. Using APIs, you can pipe any JSON document collection to an Azure Search index.
