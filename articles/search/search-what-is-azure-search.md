...
<!-- | Microsoft Search (alternative) | [Microsoft Search](https://docs.microsoft.com/microsoftsearch/overview-microsoft-search) searches over content stored in or accessible through SharePoint. It's offered as a ready-to-use search experience, enabled and configured by administrators, but it does not support the ability to customize the index as much as Cognitive Search, or the ability to run custom skills as part of ingestion of data. It can, however, accept external content through connectors, available from Microsoft and other sources. <br/><br/>Microsoft Search is for Microsoft 365 authenticated users who need to query over content in SharePoint. If this describes your user base, then start with Microsoft Search and Microsoft 365. Otherwise, we recommend Azure Cognitive Search.<br/><br/>Azure Cognitive Search searches an index you define, populated with data and documents you own, often from diverse sources. Azure Cognitive Search has crawler capabilities for some data sources through [indexers](search-indexer-overview.md), but you can push any JSON document that conforms to your index schema into a single, consolidated searchable resource. Because Cognitive Search is built to be a plug-in component in larger solutions, you can integrate search into almost any app, on any platform.| -->
