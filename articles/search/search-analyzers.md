...
Analysis converts a text input into primitive or root forms that are efficient for information retrieval. Conversion occurs during indexing, when the index is built, and then again during search when the index is read. You get the best results when applying the same processing algorithms for both activities.
...
3. Adding an analyzer to a field definition incurs a write operation on the index. If you add an **analyzer** to an existing index, note the following steps:
 ...
 | Scenario | Steps |
 |----------|-------|
 | Add a new field | If the field doesn't exist yet in the schema, there is no field revision to make. Texta analysis occurs whenever you add or update documents that provide content for the new field. Use [Update Index](https://docs.microsoft.com/rest/api/searchservice/update-index) and [mergeOrUpload](https://docs.microsoft.com/rest/api/searchservice/addupdate-or-delete-documents) for this task.|
...
During indexing, an analyzer transforms text into *tokens*, which are written as *tokenized terms* into the index. During search, an analyzer performs the same transformations, changing *query terms* into *tokenized terms*, creating the conditions for matching on content found in the index.
