...
Right-click on the query example URLs to open JSFiddle in a separate browser window. No additional software or configuration steps are required. You do not need an Azure Search service to try these queries.

> [AZURE.NOTE] The following examples leverage a search index consisting of jobs available based on a dataset provided by the [City of New York OpenData](https://nycopendata.socrata.com/) initiative.  This data should not be considered current or complete.
...
**Example 1**
Right-click the following URL to give it a try. This query returns documents from our Jobs index (loaded on a sandbox service):
...
**Example 2**
Give it a try by searching for business titles with the term senior in them, but not junior:
...
**Example 3**
Search for jobs with the term associate (where it is misspelled):
...
**Example 4**
Search for jobs with the term "senior analyst" where it is separated by no more than one word:
...
**Example 5**
Try it again removing the words between the term "senior analyst"
...
**Example 6**
Search for jobs with the term "computer analyst" where we see there are no results with both words computer and analyst, yet analyst jobs are at the top of the results.
...
**Example 7**
Try it again, this time boosting results with the term computer over the term analyst if both words do not exist.
...
**Example 8**
Search for jobs with either the term Senior or Junior.
...
**Example 9**
Search for jobs that contain the prefix 'prog' which would include business titles with the terms programming and programmer in it.
