...
The SRG focuses on physical segmentation of compute when &#39;processing&#39; data for Impact Level 5.  This focuses on ensuring that a virtual machine that compromises the physical host cannot impact a DoD workload.  To remove the risk of runtime attacks and ensure operational workloads are not compromised from other workloads on the same host, all Impact Level 5 workloads will be isolated on dedicated physical nodes.
...
\* Coming soon: CosmosDB will soon add support for customer managed keys, enabling Impact Level 5 workloads in all Azure Government regions.
...
To help support the consistent implementation of this guidance, we have provided a set of scripts to enforce policies or configurations that ensure these requirements are properly applied.  These can be downloaded [here](https://tbd.com), from our Samples pages.