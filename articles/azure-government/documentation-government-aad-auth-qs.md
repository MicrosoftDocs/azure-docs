...
5. Enter the name for your application, and select 'Web Application and/or Web API' as the Application Type. For the sign-on URL, enter the base URL for your application, which will be your Azure App URL + "/signin-oidc". 
...
3. Add an `Authentication` section. We will be filling out the properties with your AAD tenant information.
    ...
    ```
4. Fill out the `ClientId` property with the Client ID for your app from the Azure Government portal. We can find the Client ID by navigating to AAD -> App Registrations -> Your Application -> Application ID. 
5. Fill out the `TenantId` property with the Tenant ID for your app from the Azure Government portal. We can find the Tenant ID by navigating to AAD -> Properties -> Directory Id. 
...
In the same file, add this one line of code to the `Configure` method:
    ...
    ```
8. Navigate to your **Home** controller or whichever controller file is your home page, **where you want your users to login**. Add the `[Authorize]` tag before the class definition.

## Next Steps

* Navigate to our [Azure Government PaaS Sample](https://github.com/yujhongmicrosoft/gov-paas-sample 
