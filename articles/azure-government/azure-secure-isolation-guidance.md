...
-	*Data Encryption Key (DEK)* is a symmetric AES-256 key that is used for bulk encryption, and it's unique per storage account in Azure Storage. It's generated by the Azure Storage service as part of the storage account creation. The Key Encryption Key (KEK) always encrypts the DEK.
-	*Key Encryption Key (KEK)* is an asymmetric RSA (2048 or greater) key that is used to encrypt the Data Encryption Key (DEK) using Azure Key Vault or Managed HSM and exists only in Azure Key Vault or Managed HSM. It's never exposed directly to the Azure Storage service or other services. You must use Azure Key Vault or Managed HSM to store your customer-managed keys for Storage service encryption.
