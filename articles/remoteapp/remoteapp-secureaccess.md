...
We are going to walk through an example scenario â€“ the Azure RemoteApp administrator wants to block access to the environment when users are outside of the corporate network.
...
2. Click **Applications**, and then click **Microsoft Azure RemoteApp** to set up conditional access. Note that you can set up conditional access for each â€œsoftware as a serviceâ€? application in your directory separately.
...
4. You can now configure various rules and choose who to apply them to:
   ...
   2. Click the option below to define the IP address ranges that constitute your â€œtrusted networkâ€?. Everything outside of those will be rejected.
...
* Non-domain-joined: the applications will have â€œline of sightâ€? of the other resources in the VNET. For example, this can be used to connect applications to a SQL database that uses SQL authentication (applications authenticate the user directly against the database)
...
In â€œStage 2â€? we have exposed the backend resource only through the VNet/VPN configuration which we control. Azure RemoteApp has been placed in the same VNet. The end result is that the resource can only be accessed through the ARA environment.
