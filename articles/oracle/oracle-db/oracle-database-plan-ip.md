...
Consider the following points when you set up your network:
...
The following table shows the number of IPs available for virtual machine clusters and SCANs for various CIDR sizes, after you subtract the IP addresses that the Networking services require.

> [!TIP]
> Allocating a space for the subnet that's larger than the minimum required (for example, at least /25 instead of /27) can reduce the relative impact of those reserved addresses on the subnet's available space.
