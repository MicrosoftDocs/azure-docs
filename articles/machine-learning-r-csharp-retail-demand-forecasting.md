...
This web service implements Seasonal Trend decomposition (STL) and Exponential Smoothing model (ETS) to produce predictions based on the historical data provided by the user. Will the demand for a specific product increase this year? Can I predict my product sales for the Christmas season, so that I can effectively plan my inventory? Forecasting models are apt to address such questions. Given the past data, these models examine hidden trends and seasonality to predict future trends.  
...
####Module 2:
	...
	# fit model train_ts<- ts(values, frequency=frequency) fit1 <- stl(train_ts,  s.window="periodic") train_model <- forecast(fit1, h = horizon, method = 'ets')  data.forecast <- as.data.frame(t( train_model$mean)) 
