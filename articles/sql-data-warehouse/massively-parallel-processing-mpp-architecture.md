...
SQL Data Warehouse leverages a scale out architecture to distribute computational processing of data across multiple nodes. The unit of scale is an abstraction of compute that is known as a data warehouse unit. SQL Data Warehouse separates the compute from storage which enables you as the user to scale the compute independently of the data in your system.
...
The compute nodes provide the computational power of the system. Distributions are mapped to the compute nodes for processing. As you pay for more compute resources, SQL Data Warehouse re-maps the distributions to the compute available compute nodes. The number of compute nodes ranges from 1 to 60, and is determined by the service level objective.

Each compute node has a node ID that is visible in system views. You can see the compute node ID by looking for the node_id column in system views whose names begin with sys.pdw_nodes. For a list of the system views, see the [MPP system view](sql-data-warehouse-reference-tsql-statements.md).
