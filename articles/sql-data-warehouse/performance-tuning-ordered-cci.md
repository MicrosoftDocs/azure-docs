...
Here is an example of an ordered CCI table distribution that has zero segment overlapping following above recommendations.  The ordered CCI table is created via CTAS from a 20GB table using MAXDOP 1, xlargerc, and DWU1000c.  The CCI is ordered on a BIGINT column with no duplicates.  
