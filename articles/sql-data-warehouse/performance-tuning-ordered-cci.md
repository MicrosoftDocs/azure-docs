...
Here is an example of an ordered CCI table distribution that has zero segment overlapping following above recommendations.  The ordered CCI table is created via CTAS using MAXDOP 1 and xlargerc in a DWU1000c database.  The CCI is ordered on a BIGINT column with no duplicates.  The size of source data is 20GB.  
