---
title: CREATE TABLE AS SELECT (CTAS) in Azure Synapse Analytics (formerly SQL DW)| Microsoft Docs
description: Explanation and examples of the CREATE TABLE AS SELECT (CTAS) statement for developing solutions.
...
# CREATE TABLE AS SELECT (CTAS) in Azure Synapse Analytics
...
You can also use CTAS to work around a number of the unsupported features listed below. This method can often prove helpful, because not only will your code be compliant, but it will often run faster. This performance is a result of its fully parallelized design. Scenarios include:
