...
SELECT..INTO, however, does not allow you to change either the distribution method or the index type as part of the operation. `[dbo].[FactInternetSales_new]` is created using the default distribution type as ROUND_ROBIN and the default table structure as CLUSTERED COLUMNSTORE INDEX.

Using CTAS, you are able to specify both the distribution of the table data as well as the table structure type.
To convert the previous example to CTAS :
...
The original query might have looked something like the following:
...
Sometimes the best approach for deleting data is to use `CTAS`. Rather than deleting the data, select the data you want to keep. This is especially true for `DELETE` statements that use ANSI join syntax since SQL Data Warehouse does not support ANSI joins in the `FROM` clause of a `DELETE` statement.
...
Merge statements can be replaced, at least in part, by using CTAS. You can consolidate the INSERT and the UPDATE into a single statement. Any deleted records should be restricted from the `SELECT` statement to omit from the results.
...
To create your partitioned dataset, you might want to do the following:
...
The query would run perfectly fine. The problem comes when you try to perform the partition switch. The table definitions do not match. To make the table definitions match, the CTAS needs to be modified to add an `ISNULL` function to preserve the column's nullability attribute.
...
Refer to the [CTAS](/sql/t-sql/statements/create-table-as-select-azure-sql-data-warehouse) documentation. It is one of the most important statements in Azure SQL Data Warehouse. Make sure you thoroughly understand it.
