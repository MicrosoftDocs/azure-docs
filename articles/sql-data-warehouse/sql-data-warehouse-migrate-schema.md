...
- **uniqueidentifier**, use varbinary(16) or varchar(36) depending on the input format (binary or character) of your values. If the input format is character based a further optimisation is possible. By converting to using binary format the column storage can be reduced by over 50%. In very large tables these kinds of optimizations can be beneficial.
