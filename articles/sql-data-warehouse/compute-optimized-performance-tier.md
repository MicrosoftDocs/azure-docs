---
title: Concurrency and workload management optimized for compute - Azure SQL Data Warehouse | Microsoft Docs
...
---
# Compute optimized performance tier in Azure SQL Data Warehouse
Azure SQL Data Warehouse offers data warehouses at two different performance tiers: Optimized for Elasticity, and Optimized for Compute. This article provides details for managing your workload with a data warehouse that is optimized for compute. If your data warehouse is optimized for elasticity, see [Elasticity optimized performance tier](sql-data-warehouse-develop-concurrency.md).

## What is the Optimized for Compute Performance Tier?
A compute-optimized data warehouse runs compute-intensive queries and workloads. This performance tier offers high-scale performance by leveraging the latest Azure hardware; automatically creating a tiered storage architecture for your data. As the number of Compute nodes increase, queries run in parallel across more nodes, which improves query performance. 

Data is sharded in Azure Storage into pre-defined distributions. These distributions are then mapped to the compute layer. As the compute scale increases, the number of distributions per unit of compute decreases. Each query therefore has access to more compute and therefore can execute faster as it processes less data.  

The number of Compute nodes is determined by the service level objective (SLO), which is measured by an abstracted unit of measure known as a data warehouse units. The optimized for compute performance tier uses compute data warehouse units (cDWUs) whereas the optimized for elasticity performance tier uses data warehouse units (DWUs). For more information, see [dwus and cdwus](what-is-a-data-warehouse-unit-dwu-cdwu.md).

This table shows the number of compute nodes and distributions per node for each cDWU size.    

| Service Level | Compute nodes | Distributions per node |
|:-------------:|:-------------:|:----------------------:|
| DW1000c       | 2             | 30                     |
| DW1500c       | 3             | 20                     |
| DW2000c       | 4             | 15                     |
...
Workload management is the ability to optimize the performance of your system so that queries and load operations run efficiently. To manage your workload, SQL Data Warehouse helps you control the number of queries that run concurrently and the compute-resources assigned to each query. 

SQL Data Warehouse workload management helps you tune multi-user environments. It is not intended for multi-tenant workloads.

### Workload capacity limits
SQL Data Warehouse uses a feature called concurrency slots to measure the compute resources assigned to queries. A concurrency slot represents a specific amount of compute resources that the system allocates to a query. 
...
Each query will consume zero, one or more concurrency slots. For example, a query that is assigned 10 concurrency slots has access to 5 five times more compute resources than a query running with 2 concurrency slots. Similarly, if each query requires 10 concurrency slots and there are 40 concurrency slots, only 4 queries can run concurrently.

The following table shows the maximum concurrent queries and concurrency slots at the various cDWU sizes.
...
When one of these thresholds is met, new queries are queued and executed on a first-in, first-out basis.  As queries finish and the number of queries and slots falls below the limits, SQL Data Warehouse processes queued queries. 
...
## Resource classes
SQL Data Warehouse uses the resource class feature to assign compute resources to individual queries. Resource classes are implemented by assigning users to database roles. There are two types of resource classes: dynamic and static.

- Static Resource Classes (**staticrc10, staticrc20, staticrc30, staticrc40, staticrc50, staticrc60, staticrc70, staticrc80**) allocate the same amount of memory regardless of the current service level. This static allocation means on larger cDWUs you can run more queries in each resource class.
- Dynamic Resource Classes (**smallrc, mediumrc, largerc, xlargerc**) allocate a variable amount of memory depending on the current service level. When you scale up to a larger service level, your queries automatically get more memory. 

When database users are members of the **smallrc** or **staticrc10** database roles, their queries run with small amounts of memory.  With the smaller resource classes, users can take advantage of higher concurrency.

When database users are members of the **xlargerc** or **staticrc80** database roles, their queries run with large amounts of memory. With the larger resource classes, queries can run with more resources, but fewer queries can run concurrently.

### Assign a user to a resource class
By default, each user is a member of the small resource class, **smallrc**. To increase the resource class, use the stored procedure [sp_addrolemember](https://docs.microsoft.com/sql/relational-databases/system-stored-procedures/sp-addrolemember-transact-sql). To decrease the resource class, use [sp_droprolemember](https://docs.microsoft.com/sql/relational-databases/system-stored-procedures/sp-droprolemember-transact-sql).  For example, this command increases the resource class for loaduser to **largerc**:

```sql
EXEC sp_addrolemember 'largerc', 'loaduser'
```

### Queries that always run in small resource class

There are a few types of queries that do not benefit from a larger memory allocation. The system ignores their resource class allocation and always runs these queries in the small resource class instead. If these queries always run in the small resource class, they can run when concurrency slots are under pressure. They don't consume more slots than needed. 

### Resource class remarks
...
## Memory allocation
Increasing a resource class for a user, gives their queries access to more memory, which may mean queries execute faster.  However, higher resource classes also reduce the number of concurrent queries that can run. The trade-off is between allocating large amounts of memory to a single query or allowing other queries, which also need memory allocations, to run concurrently. If one user is given high allocations of memory for a query, other users can't have access to that same memory to run a query.

### Static Resource Class
The table below shows the system-wide memory allocations in GB for queries using the static resource classes.

| Service Level | staticrc10 | staticrc20 | staticrc30 | staticrc40 | staticrc50 | staticrc60 | staticrc70 | staticrc80 |
|:------------- |:----------:|:----------:|:----------:|:----------:|:----------:|:----------:|:----------:|:----------:|
| DW1000c       | 15         | 30         | 60         | 120        | 240        | 480        | 600        | 600        |
| DW1500c       | 15         | 30         | 60         | 120        | 240        | 480        | 900        | 960        |
| DW2000c       | 15         | 30         | 60         | 120        | 240        | 480        | 960        | 1,200      |
| DW2500c       | 15         | 30         | 60         | 120        | 240        | 480        | 960        | 1,500      |
| DW3000c       | 15         | 30         | 60         | 120        | 240        | 480        | 960        | 1,800      |
| DW5000c       | 15         | 30         | 60         | 120        | 240        | 480        | 960        | 1,920      |
| DW6000c       | 15         | 30         | 60         | 120        | 240        | 480        | 960        | 1,920      |
| DW7500c       | 15         | 30         | 60         | 120        | 240        | 480        | 960        | 1,920      |
| DW10000c      | 15         | 30         | 60         | 120        | 240        | 480        | 960        | 1,920      |
| DW15000c      | 15         | 30         | 60         | 120        | 240        | 480        | 960        | 1,920      |
| DW30000c      | 15         | 30         | 60         | 120        | 240        | 480        | 960        | 1,920      |

The table below shows the memory allocated to each distribution by service level while using the static resource class. 

| Service Level | staticrc10 | staticrc20 | staticrc30 | staticrc40 | staticrc50 | staticrc60 | staticrc70 | staticrc80 |
|:------------- |:----------:|:----------:|:----------:|:----------:|:----------:|:----------:|:----------:|:----------:|
| DW1000c       | 250        | 500        | 1,000      | 2,000      | 4,000      | 8,000      | 10,000     | 10,000     |
| DW1500c       | 250        | 500        | 1,000      | 2,000      | 4,000      | 8,000      | 15,000     | 15,000     |
| DW2000c       | 250        | 500        | 1,000      | 2,000      | 4,000      | 8,000      | 16,000     | 20,000     |
| DW2500c       | 250        | 500        | 1,000      | 2,000      | 4,000      | 8,000      | 16,000     | 25,000     |
| DW3000c       | 250        | 500        | 1,000      | 2,000      | 4,000      | 8,000      | 16,000     | 30,000     |
| DW5000c       | 250        | 500        | 1,000      | 2,000      | 4,000      | 8,000      | 16,000     | 32,000     |
| DW6000c       | 250        | 500        | 1,000      | 2,000      | 4,000      | 8,000      | 16,000     | 32,000     |
| DW7500c       | 250        | 500        | 1,000      | 2,000      | 4,000      | 8,000      | 16,000     | 32,000     |
| DW10000c      | 250        | 500        | 1,000      | 2,000      | 4,000      | 8,000      | 16,000     | 32,000     |
| DW15000c      | 250        | 500        | 1,000      | 2,000      | 4,000      | 8,000      | 16,000     | 32,000     |
| DW30000c      | 250        | 500        | 1,000      | 2,000      | 4,000      | 8,000      | 16,000     | 32,000     |

### Dynamic Resource Classes
The table below shows the system-wide memory allocations in GB for queries using the dynamic resource classes.

| Service Level | smallrc | mediumrc | largerc | xlargerc |
|:------------- |:-------:|:--------:|:-------:|:--------:|
| DW1000c       | 15      | 120      | 240     | 480      |
| DW1500c       | 15      | 120      | 240     | 480      |
| DW2000c       | 15      | 240      | 480     | 960      |
| DW2500c       | 15      | 240      | 480     | 960      |
| DW3000c       | 15      | 240      | 480     | 960      |
| DW5000c       | 15      | 480      | 960     | 1,920    |
| DW6000c       | 15      | 480      | 960     | 1,920    |
| DW7500c       | 15      | 960      | 1,920   | 3,840    |
| DW10000c      | 15      | 960      | 1,920   | 3,840    |
| DW15000c      | 15      | 960      | 1,920   | 3,840    |
| DW30000c      | 15      | 960      | 1,920   | 3,840    |

The table below shows the memory allocated to each distribution by service level while using the dynamic resource class. 

| Service Level | smallrc | mediumrc | largerc | xlargerc |
|:------------- |:-------:|:--------:|:-------:|:--------:|
| DW1000c       | 250     | 2,000    | 4,000   | 8,000    |
| DW1500c       | 250     | 2,000    | 4,000   | 8,000    |
| DW2000c       | 250     | 4,000    | 8,000   | 16,000   |
| DW2500c       | 250     | 4,000    | 8,000   | 16,000   |
| DW3000c       | 250     | 4,000    | 8,000   | 16,000   |
| DW5000c       | 250     | 8,000    | 16,000  | 32,000   |
| DW6000c       | 250     | 8,000    | 16,000  | 32,000   |
| DW7500c       | 250     | 16,000   | 32,000  | 64,000   |
| DW10000c      | 250     | 16,000   | 32,000  | 64,000   |
| DW15000c      | 250     | 16,000   | 32,000  | 64,000   |
| DW30000c      | 250     | 16,000   | 32,000  | 64,000   |
...
From these tables, you can see that SQL Data Warehouse running as DW2000c allocates a maximum of 32 concurrent queries and a total of 80 concurrency slots. If all users are running in smallrc, 32 concurrent queries would be allowed because each query would consume 1 concurrency slot. If all users on a DW2000c were running in mediumrc, each query would be allocated 4 GB per distribution for a total memory allocation of 240 GB per query, and concurrency would be limited to 5 users (80 concurrency slots / 16 slots per mediumrc user).
...
1.	If the expectations are loads with regular amount of data, a static resource class is a good choice. Later, when scaling up to get more computational power, the data warehouse is able to run more concurrent queries, as the load user does not consume more memory.
2.	If the expectations are bigger loads in some occasions, a dynamic resource class is a good choice. Later, when scaling up to get more computational power, the load user will gets more memory, which allows the load to perform faster.
3.	Use at least 200 MB of memory for loads. The memory needed to process loads efficiently depends on the nature of the table loaded and the amount of data processed. For more information, see [Maximizing rowgroup quality](sql-data-warehouse-memory-optimizations-for-columnstore-compression.md).
...
## Example for finding the best resource class
 
Here's the purpose of this stored procedure:  
1. To see the concurrency and memory grant per resource class at a given SLO. User needs to provide NULL for both schema and tablename as shown in the example below.  
...
- This stored proc is not designed to calculate the memory requirement for a partitioned cci table.    
- This stored proc doesn't take memory requirement into account for the SELECT part of CTAS/INSERT-SELECT and assumes it is a SELECT.
- This stored proc uses a temp table, which is availble in the session where this stored proc was created.    
- This stored proc depends on the current offerings (for example, hardware configuration, DMS config), and if any of that changes then this stored proc does not work correctly.  
...
- This stored proc depends on existing resource class offerings and if that changes then this stored proc will not work correctly.  
...
## Queries governed by concurrency limits
