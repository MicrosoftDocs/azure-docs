...
|Executed Checks|
|---|
|<ul><li>Is production slot deployment healthy across all role instances?</li><li>Is the role healthy across all it's VM instances?</li><li>What is the health status of each VM within a role of a cloud service?</li><li>Was the VM status change due to platform or customer initiated operation?</li><li>Has the booting of the guest OS completed?</li><li>Is there ongoing planned maintenance?</li><li>Is the host hardware degraded and predicted to fail soon?</li></ul>|
