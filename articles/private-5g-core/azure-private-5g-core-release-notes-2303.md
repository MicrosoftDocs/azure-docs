...
The following table provides a summary of known issues carried over from the previous releases.
  ...
  | 6 | AAD and Web Proxy | Where Azure Active Directory is used to authenticate access to Grafana/SAS, then this traffic does not transmit via the Web Proxy when enabled on the ASE (Azure Stack Edge) appliance that the packet core is running on |
