...
There are several reasons why your *Azure Private 5G Core (AP5GC)* may have *Azure Stack Edge (ASE)* disconnects. These can range from unplanned short term *temporary disconnects* to planned *disconnected mode* periods for up to two days.
...
Azure Stack Edge can also tolerate smaller periods of unplanned temporary disconnect. The following sections detail the behavior expected during these times and behavior after ASE reconnect.
...
It is common to see configuration and provisioning failures while ASE is disconnected. AP5GC can handle such events without major issues by automatically retrying configuration and provisioning actions once the ASE reconnects. If the action has not succeed within 10 minutes, an error is displayed and action will need to be repeated manually once ASE reconnects.
...
- **Monitoring** tabs will show metrics for sites after 10 minutes, but will not populate stats for the duration of the disconnect.
- **Kubernetes Arc Insights** will show new stats after 10 minutes, but will not populate stats for the duration of the disconnect.
- **Resource Health** views will be viewable immediately.
- [Workbooks](/azure/update-center/workbooks) for the ASE will be viewable immediately and will populate for the duration of the disconnect.
