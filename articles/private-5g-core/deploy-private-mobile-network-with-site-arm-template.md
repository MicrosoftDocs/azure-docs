...
1. Select or enter the following values, using the information you retrieved in [Prerequisites](#prerequisites).
    ...
    |**Control Plane Access Ip Address**    | Enter the IP address for the control plane interface on the access network.</br> You must use the same IP address that is configured for the **N2 POD IPs** field in ASE.       |
