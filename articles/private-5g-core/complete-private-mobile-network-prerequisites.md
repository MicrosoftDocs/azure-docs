...
Depending on your networking requirements, you may choose to allocate a single subnet for the Azure Stack Edge interfaces marked with an asterisk (*). This arrangement, called a flat network, should only be chosen if it makes sense for your network configuration (for example, if a limited set of subnets is available).
