...
| The network slice that SIMs using this SIM policy will use by default. | **Default slice** | ?<!-- TODO --> |
...
| Value | Azure portal field name | Included in example ARM template |
|--|--|--|
| The network slice to which the network scope settings will apply. </br></br>If any of the UEs connected to the data network support 4G technology, this slice must have a slice/service type (SST) value of 1 and empty slice differentiator (SD). | **Slice** | ?<!-- TODO --> |
