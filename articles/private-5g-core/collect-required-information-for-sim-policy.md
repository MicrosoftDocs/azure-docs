...
| The network slice that SIMs using this SIM policy will use by default. If any of the SIMs that will use this SIM policy support 4G, this slice must have a slice/service type (SST) value of 1 and empty slice differentiator (SD). | **Default slice** | ?<!-- TODO --> |
...
| Value | Azure portal field name | Included in example ARM template |
|--|--|--|
| The network slice to which the network scope settings will apply. | **Slice** | ?<!-- TODO --> |
