...
* Read the information in [Policy control](policy-control.md) and familiarize yourself with Azure Private 5G Core Preview policy control configuration.
...
1. On the **Add a network scope** blade, fill out the fields as follows.
    ...
    |**Session aggregate maximum bit rate - Uplink**     | `1 Gbps`        |
    |**Session aggregate maximum bit rate - Downlink**     | `1 Gbps`        |
    |**5G QoS Indicator (5QI)**     | `9`        |
...
In this step, we will provision two SIMs and assign a SIM policy to each one. This will allow the SIMs to use the private mobile network and receive the correct QoS policy.
...
<!--- need to confirm ordering of deletion --->
