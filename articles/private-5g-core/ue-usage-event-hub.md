...
In order to configure UE usage monitoring:

- You must already have an Event Hubs instance with a shared access policy.
    > [!NOTE]
...
- You must have a user assigned managed identity that has the Resource Policy Contributor or Owner role for the Event Hub and is assigned to the Packet Core Control Plane for the site.
