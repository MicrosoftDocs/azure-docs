...
2. Under **Virtual network**, select a virtual network. In VLAN-trunking mode, this will be either **N2**, **N3** and **N6**. Otherwise, choose from **N2**, **N3**, **N6-DNX** (where *X* is the DN number 1-10). In the side panel:
      1. Enable the virtual network for Kubernetes.
      2. If **NOT** in VLAN-trunking mode, also add a pool of IP addresses. Add a range of one IP address for the appropriate address (N2, N3, or N6-DNX as collected earlier). For example, *10.10.10.20-10.10.10.20*.
      3. Repeat for each virtual network.
      4. Select **Modify** at the bottom of the panel to save the configuration.
3. Select **Apply** at the bottom of the page and wait for the settings to be applied. Applying the settings will take approximately 5 minutes.
