...
You can use Azure Key Vault to securely access sensitive information from within your applications. Here are some of the benefits:

- You can manage credentials like passwords, [access keys](../keys/about-keys.md), and SAS tokens by storing them in Key Vault as [secrets](../secrets/about-secrets.md). 
- You allow customers to own and manage their own keys, secrets, and [certificates](../certificates/about-certificates.md) so you can concentrate on providing the core software features. In this way, your applications will not own the responsibility or potential liability for your customers' tenant keys, secrets, and certificates.
- Your application can use keys for signing and encryption yet keep the key management external from your application.

This article describes how you can use keys, secrets, and certificates from your applications and help ensure that they're protected without writing the code yourself. For more general information on Azure Key Vault, see [About Azure Key Vault](overview.md).
...
[Azure role-based access control (RBAC)](../../role-based-access-control/overview.md) controls access to the management layer. In Key Vault, the management layer, also known as the [management plane](security-features.md), lets you create and manage key vaults and their attributes. These attributes include access policies. They don't include keys, secrets, and certificates, which are managed on *data plane*. 
...
The preceding authentication scenarios are supported by the *Azure Identity client library* and integrated with Key Vault SDKs. You can use the Azure Identity library across environments and platforms without changing your code. The library automatically retrieves authentication token from users who are signed in to Azure user through the Azure CLI, Visual Studio, Visual Studio Code, and other means. 
...
The data plane controls access to keys, secrets, and certificates. You can use local vault access policies or Azure RBAC for access control through the data plane.
...
For more information about data plane security for Key Vault, see [Azure Key Vault security features](security-features.md).
...
- The Key Vault VM extension, which provides automatic refresh of certificates stored in an Azure key vault. For more information, see: 
...
- A cloud application running in a VM on Azure needs a certificate. How do you get this certificate into this VM today? See [Key Vault virtual machine extension for Windows](../../virtual-machines/extensions/key-vault-windows.md) or [Key Vault virtual machine extension for Linux](../../virtual-machines/extensions/key-vault-linux.md).
...
Other scenarios and services that use or integrate with Key Vault include:
...
- [Use secrets stored in Key Vault in Azure Databricks to connect to Azure Storage](./integrate-databricks-blob-storage.md).
...
- To learn about a feature that allows recovery of deleted objects, whether the deletion was accidental or intentional, see [Azure Key Vault soft-delete overview](soft-delete-overview.md).
- To learn the basic concepts of throttling and get an approach for your app, see [Azure Key Vault throttling guidance](overview-throttling.md).
- To learn about the relationships between regions and security areas, see [Azure Key Vault security worlds and geographic boundaries](overview-security-worlds.md).
