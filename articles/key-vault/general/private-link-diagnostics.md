...
- Your DNS queries still return a public IP address for the key vault, instead of a private IP address that you would expect from using the private link feature.
...
This troubleshooting guide is applicable to connections to key vault that originate from application code. Examples are applications and scripts that execute in virtual machines, Azure Service Fabric clusters, Azure App Service, Azure Kubernetes Service (AKS), and similar others.
...
>[!NOTE]
> If your key vault has multiple private endpoints, then it will have multiple private IP addresses. This is only useful if you have multiple Virtual Networks accessing the same key vault, each through its own Private Endpoint (the Private Endpoint belongs to a single Virtual Network). Make sure you diagnose the problem for the correct Virtual Network, and select the correct private endpoint connection in the procedure above. Furthermore, **do not** create multiple Private Endpoints for the same Key Vault in the same Virtual Network. This is not needed and is a source of confusion.
...
The above result is expected regardless of the machine be connected to the Virtual Network or be an arbitrary machine with an Internet connection. This happens because the key vault has no private link in approved state, and therefore there is no need for the key vault to support private link connections.
...
Normally this resource is created automatically when you create a Private Endpoint using a typical method. But there are cases where this resource is not created automatically and you will have to do it manually. This resource might have been accidentally deleted as well.
...
### Fact: the user controls DNS resolution

If you are a network scholar or curious person, you probably realized how DNS resolution works. As explained in the [previous section](#key-vault-with-private-link-resolving-from-arbitrary-internet-machine), a key vault with private links will have the alias `{vaultname}.privatelink.vaultcore.azure.net` in its *public* registration. The DNS server used by the Virtual Network will check every alias for a *private* name registration, and if one is found, it will stop following the aliases of public registration.

For example, consider that the Virtual Network is linked to a Private DNS Zone with name `privatelink.vaultcore.azure.net`, and the public DNS registration for the key vault has the alias `fabrikam.privatelink.vaultcore.azure.net`. Note that the suffix matches the Private DNS Zone name exactly. This means the resolution will look first for an `A` record with name `fabrikam` in the Private DNS Zone. If the `A` record is found, its IP address will be returned in the DNS query. And that IP address just happens to be the private IP address of the key vault.

As you can see, the entire name resolution is under user control.
