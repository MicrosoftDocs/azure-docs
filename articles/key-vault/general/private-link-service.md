...
* If you are connecting from an on-prem resource to a Key Vault, ensure you have all required conditional forwarders in the on-prem environment enabled.
    1. Review [Azure Private Endpoint DNS configuration](.../.../private-link/private-endpoint-dns#azure-services-dns-zone-configuration) for the zones needed, and make sure you have conditional forwarders for both `vault.azure.net` and `vaultcore.azure.net` on your on-prem DNS.
    2. Ensure that you have conditional forwarders for those zones that route to an [Azure Private DNS Resolver](.../.../dns/dns-private-resolver-overview) or some other DNS platform with access to Azure resolution.
