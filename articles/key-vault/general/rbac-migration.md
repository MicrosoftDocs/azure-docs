...
- Users or Service Principals now require higher permissions, such as Owner or User Access Administrator. Previously, automation and users could create a Key Vault and grant other service principals or managed identities access via access policies. However, this is no longer possible without the necessary permissions, which may not be suitable for most automated deployments (e.g., Azure Pipelines, Bicep, Terraform).
