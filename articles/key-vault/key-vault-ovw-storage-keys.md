...
An [Azure storage account](/azure/storage/storage-create-storage-account) uses a credential that consists of an account name and key. The key is autogenerated and serves as a password rather than a cryptographic key. Key Vault manages storage account keys by storing them as [Key Vault secrets](/azure/key-vault/about-keys-secrets-and-certificates#key-vault-secrets). 
...
## Managed storage account overview

The Key Vault managed storage account feature completes several management functions for you. The feature manages keys for both storage accounts and Classic storage accounts. Keys are listed (synced) with an Azure storage account and are periodically regenerated or _rotated_. 
