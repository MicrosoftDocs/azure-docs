...
A managed HSM is a single-tenant, FIPS (Federal Information Processing Standards) 140-2 validated, highly available hardware security module (HSM), with a customer-controlled security domain (SD). Every managed HSM must have a SD to operate.

The SD is the set of core credentials of the managed HSM, and serves the following purposes:
...
- Allows you to fully recover a managed HSM instance if there is a disaster. The following disaster scenarios are covered:
  - A catastrophic failure resulting in all member HSM instances of a managed HSM instance destroyed.
...
## Why is the security domain important?

The security domain contains extremely sensitive cryptographic information pertinent to your unique managed HSM and ensures that only you have sole control over your cryptographic key material on the managed HSM. Without the security domain, disaster recovery is not possible.  Protection of the security domain is of utmost importance for business continuity and to ensure you are not cryptographically locked out. 

Microsoft has no way of recovering the security domain, nor can it access your keys without the security domain. Extreme caution must therefore be exercised in implementing and protecting the security domain

## What does a Security Domain Contain?

The security domain's encrypted blob file contains core credentials such as the HSM backup, user credentials, the signing key, and the data encryption key unique to your managed HSM.

## How do I download the Security Domain ?
...
The private keys and the Security Domain must be protected just as you would protect crown jewels in a locked box.

## What are the best practices in protecting the security domain?
...
## Who should have access to the Quorum?
...
For example, a security domain quorum could consists of four key pairs, with each private key given to a different person. A minimum of two person would have to come together to reconstruct a security domain. The parts could be given to key personnel, such as:
...
## When should I review the Quorum?

Timing depends on your organization, but we recommend that you conduct a security domain review at least once every quarter, as well as when:
...
## Where can I store the Security Domain?
...
## What can I do if the Security Domain I downloaded is lost?

The security domain can be downloaded at any time from the active managed HSM instance. Follow the security domain storage guidance as recommended in the above section.

A lost security domain is considered compromised, and you will need to take corrective action. 

## What can happen if my security domain is compromised?

If your security domain is compromised, a malicious actor could use it to create their own managed HSM instance. The malicious actor could use the access to the key backups to start decrypting the data protected with the keys on the managed HSM instance.

## What can I do to protect my organization when the security domain is compromised?

If the security domain is compromised, all data encrypted with the current managed HSM instance must be decrypted with current key material. A new managed HSM instance must be provisioned, with a new security domain implemented and pointed to the new URL. 
...
It is highly recommended that you review NIST Special Publication 800-57 for key management best practices, before developing and implementing the policies, systems, and standards necessary to meet and enhance your organization's security goals.
