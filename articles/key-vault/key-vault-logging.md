---
title: Azure Key Vault Logging - Azure Key Vault | Microsoft Docs
...
* Azure PowerShell, **minimum version of 1.0.0**. To install Azure PowerShell and associate it with your Azure subscription, see [How to install and configure Azure PowerShell](/powershell/azure/overview). If you have already installed Azure PowerShell and do not know the version, from the Azure PowerShell console, type `$PSVersionTable.PSVersion`.  
...
For additional ease of management, we'll also use the same resource group as the one that contains our key vault. From the [getting started tutorial](key-vault-overview.md), this resource group is named **ContosoResourceGroup** and we'll continue to use the East Asia location. Substitute these values for your own, as applicable:
...
To enable logging for Key Vault, we'll use the Set-AzDiagnosticSetting cmdlet, together with the variables we created for our new storage account and our key vault. We'll also set the **-Enabled** flag to **$true** and set the category to AuditEvent (the only category for Key Vault logging):
...
Because the same storage account can be used to collect logs for multiple resources, the full resource ID in the blob name is useful to access or download just the blobs that you need. But before we do that, we'll first cover how to download all the blobs.
...
Pipe this list through 'Get-AzStorageBlobContent' to download the blobs into our destination folder:
...
You're now ready to start looking at what's in the logs. But before moving onto that, two more parameters for Get-AzDiagnosticSetting that you might need to know:
...
Individual blobs are stored as text, formatted as a JSON blob. Running
...
| properties |This field will contain different information based on the operation (operationName). In most cases, contains client information (the user agent string passed by the client), the exact REST API request URI, and HTTP status code. In addition, when an object is returned as a result of a request (for example, KeyCreate or VaultGet) it will also contain the Key URI (as "id"), Vault URI, or Secret URI. |
