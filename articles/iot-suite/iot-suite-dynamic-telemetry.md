...
Adding a **Telemetry** definition as shown in the prededing code snippet does not change the behavior of the dashboard. However, the metadata can also include a **DisplayName** attribute to customize the display in the dashboard. Update the **Telemetry** metadata definition as shown in the following snippet:
...
This change only affects the chart display, the data **ExternalTemperature** values are still stored and made available for any backend processing.
