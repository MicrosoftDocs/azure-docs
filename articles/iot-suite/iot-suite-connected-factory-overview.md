...
6. To view details on specific nodes in the station, click the name. This action launches the context panel with Time Series Insights visualizations. Click these graphs to do further analysis in the Time Series Insights explorer environment.
...
OEE identifies the percentage of manufacturing time that is productive. This measure is a standard industry measure and is calculated by multiplying the availability rate, performance rate, and quality rate: OEE = availability x performance x quality.
...
The filter is then applied for you. The filter state is also shown in the dashboard via a funnel that displays in the factories and alerts tables.
...
5. Right-clicking an item, depending on your permissions, enables you to read or publish that particular node. Click the read option, and a context panel displays showing the value of the specific node.
...
4. A context panel appears which tells you that the publish has succeeded. The published node appears in the station level view for the location, production line you choose to browse.
