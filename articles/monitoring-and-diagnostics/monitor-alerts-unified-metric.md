...
5.  Once signal is selected, logic for alerting can be stated. For reference, historic data of the selected signal is shown with option to tweak the time window from last six hours to last week. With the visualization in place, **Alert Logic** can be selected from shown options of Condition, Aggregation and finally Threshold. As preview of the logic provided, the condition is shown in the visualization along with signal history, so as to indicate when the alert would have been triggered in the past. Finally specify in the logic, for what time Alerts should look for the specified condition by choosing from the **Period** option along with how often Alert should run by selecting **Frequency**
   ![Configure signal logic](./media/monitor-alerts-unified/AlertsPreviewCriteria.png)

> [!TIP]
> Keeping **Period** value greater than **Frequency** minimizes risk of missing any issue. Also history period chosen for visualization has no impact on alert and only affected by evaluation period chosen
...
6. Using the top panel, changes to the alert can be reflective including: *Save* to commit any changes done to alert, *Discard* to go back without committing any changes made to alert, *Disable* to deactivate the alert - after which it will no longer run or trigger any action and finally,  *Delete* to permanently remove the entire alert rule from Azure.
