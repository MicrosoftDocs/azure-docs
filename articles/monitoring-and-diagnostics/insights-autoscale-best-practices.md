...
The following sections in this document will help you understand the best practices for autoscale in Azure. After reviewing this information, you'll be better able to effectively use Autoscale in your Azure infrastructure.
...
If you have a setting that has minimum=2, maximum=2 and the current instance count is 2, no scale action can occur. Keep an adequate margin between the maximum and minimum instance counts, which are inclusive. Autoscale will always scale between these limits.

### Manual scaling is reset by autoscale min and max
If you manually update the instance count to a value above or below the maximum, the autoscale engine will automatically scale back to the minimum (if below) or the maximum (if above). For example, if you set the range between 3 and 6, the autoscale engine will scale 1 running instance back to 3 on its next run. Likewise, it will scale down 8 instances back to 6 on its next run.  Manual scaling is very temporary unless you reset the autoscale rules as well.
...
5. Now the messages in the queue start getting smaller. With 3 instances, the first scale in action happens when the total messages in all queues adds up to 30, making it 30/3 which = 10 messages per instance, which is the scale in threshold.
