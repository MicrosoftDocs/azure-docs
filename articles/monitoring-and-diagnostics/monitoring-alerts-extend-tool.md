...
- **Error: Policy is present at subscription/resource group level**: 
    ...
    When [Azure Policy](../policy/overview.md) is applied, it restricts any new resource in a subscription or resource group that contains the Log Analytics (Operations Management Suite) workspace. The system is unable to extend alerts into Azure and create necessary action groups.
