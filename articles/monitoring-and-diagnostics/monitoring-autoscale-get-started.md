...
3. Click **Autoscale** to view all the resources for which Autoscale is applicable, along with its current Autoscale status.
...
For each resource, you will find the current instance count as well as its Autoscale status. The Autoscale status can be:
...
1. Open the **Autoscale** blade in Azure Monitor and select a resource that you want to scale. (The steps below use an App Service plan associated with a web app. You can [create your first ASP.NET web app in Azure in 5 minutes.][4])
...
4. Select the start/end dates as well as the start/end time for when the scale condition should be applied.
...
You can make changes in JSON directly, if required. These changes will be reflected after saving.
