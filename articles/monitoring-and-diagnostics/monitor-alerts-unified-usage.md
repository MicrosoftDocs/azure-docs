...
---
# Using Azure Monitor - Alerts (Preview)
> [!div class="op_single_selector"]
> * [Portal](insights-alerts-unified.md)
>

## Overview
This article shows you how to set up alerts using the new Alerts (Preview) interface inside Azure portal. Definition of Alerts is in three parts:
- Target: Specific Azure resource or service, which is to be monitored
- Signal: Specific data stream such as metrics from Azure monitoring service or an Azure Log Analytics](../log-analytics-tutorial-viewdata.md) query using combination of Azure resources, events, and custom log 
- Criteria: Specific condition or logic that when seen in Signal, should trigger action

Azure Alerts (Preview) uses the term **Log Alerts** to describe Alerts where signal is custom query based on [Azure Log Analytics](../log-analytics/log-analytics-tutorial-viewdata.md). And all other alerts, which use data stream from Azure platform or monitoring service - are referred to as **Metric Alerts**. In *Metric Alerts*, some platforms provide [multidimensional metrics](monitoring-metric-charts.md) for specific Azure resource and hence alerts for such resource be made more specific using additional data points; such alerts are referred to as **Multi-Dimensional Metric Alerts**. 
Azure Alerts (Preview) also provides a unified view to all your alert rules and ability to manage them a single place; including viewing any unresolved alerts. Learn more about functionality from [Azure Alerts(Preview) - Overview](monitor-overview-unified.md).

> [!NOTE]
> While Azure Alerts (Preview) offers a new and enhanced experience for creating alerts in Azure. The existing [Azure Alerts](monitoring-overview-alerts.md) and [Near Real-Time Alerts (Preview)](monitoring-near-real-time-metric-alerts.md) remain usable
>
...
As Azure Alerts (Preview) allows for creation of various types of alerts from a singular interface; based on the type of alert desired choose the next step as:
    - For **Metric Alerts**: follow steps 5 and 6; then go directly to step 11
    - For **Multidimensional Metric Alerts**: follow step 5 and skip step 6 - do step 7; then go directly to step 11
...
2. The **Alerts (Preview) Dashboard** is displayed - wherein all Azure Alerts are unified and displayed in a singular board
   ![Alert Dashboard](./media/monitor-alerts-unified/AlertsPreviewDashboard.png)
1. From Top Left to Right, the Dashboard shows at a glance, the following - which can be clicked to see a detailed listing:
    - *Total Alerts*: The number of Alerts active for the selected subscriptions until date
