...
## Reading this page  
 Tags below are roughly in order shown in the example above.  If you don't see a full description where you expect it, search the page for the element or attribute.  
  
## Common Attribute Types  
 **scheduledTransferPeriod**  attribute appears in several elements. It is the interval between scheduled transfers to storage rounded up to the nearest minute. Note that the value is an [XML “Duration Data Type”](http://www.w3schools.com/schema/schema_dtypes_date.asp).  
...
|--------------------|-----------------|  
|**PublicConfig**|Required. See description below.|  
|**PrivateConfig**|Optional. See description below.|  
|**IsEnabled**|Boolean. See description below.|  
...
|Child Elements|Description|  
|--------------------|-----------------|  
|**WadCfg**|Required. See description below.|  
...
|**CrashDumps**|See description below.|  
...
|**Directories**|See description below|  
|**EtwProviders**|See description below|  
|**Metrics**|See description below|  
|**PerformanceCounters**|See description below|  
|**WindowsEventLog**|See description below|  
...
|----------------|-----------------|  
|**containerName**|Optional.                      The name of the blob container in your [!INCLUDE[azure_2](../Token/azure_2_md.md)] Storage account that will be used to store crash dumps.|  
|**crashDumpType**|Optional.  Configures [!INCLUDE[azure_2](../Token/azure_2_md.md)] Diagnostics to collect mini or full crash dumps.|  
|**directoryQuotaPercentage**|Optional.  Configures the percentage of **overallQuotaInMB** that will be reserved for crash dumps on the VM.|  
  
|Child Elements|Description|  
|--------------------|-----------------|  
|**CrashDumpConfiguration**|Required. Defines configuration values for each process.<br /><br /> The following attribute is also required:<br /><br /> **processName** - The name of the process you want [!INCLUDE[azure_2](../Token/azure_2_md.md)] Diagnostics to collect a crash dump for.|  
...
|Child Elements|Description|  
|--------------------|-----------------|  
|**DirectoryConfiguration**|Required. The following attribute is required:<br /><br /> **containerName** - The name of the blob container in your [!INCLUDE[azure_2](../Token/azure_2_md.md)] Storage account that will be used to store the log files.|  
...
## Metrics Element  
 ...
 The **resourceId** attribute is required.  This is the resource ID of the Virtual Machine you are deploying [!INCLUDE[azure_2](../Token/azure_2_md.md)] Diagnostics to. Get the **resourceID** from the [Azure classic portal](https://portal.azure.com). Select **Browse** -> **Resource Groups** -> **<Name\>**. Click on the **Properties** tile and copy the value from the **ID** field.  
...
|**scheduledTransferLogLevelFilterr**|**string**|Optional. Specifies the minimum severity level for log entries that are transferred. The default value is **Undefined**, which will transfer all logs. Other possible values (in order of most to least information) are **Verbose**, **Information**, **Warning**, **Error**, and **Critical**.|  
...
|Element|Type|Description|  
|-------------|----------|-----------------|  
|**Channel**|string|See description below.|  
...
|----------------|----------|-----------------|  
|**logLevel**|**string**|Specifies the minimum severity level for log entries that are transferred. The default value is **Undefined**, which will transfer all logs. Other possible values (in order of most to least information) are **Verbose**, **Information**, **Warning**, **Error**, and **Critical**.|  
...
## PrivateConfig Element  
 ...
 Stores the private details of the storage account (name, key and endpoint). This information will be sent to the virtual machine, but cannot be retrieved from the virtual machine.  
