...
- Install the InfluxDB Telegraf agent on your Azure Linux VM and send metrics using the Azure Monitor Output Plugin (click here to learn how)
- Send custom metrics directly to Azure Monitor REST API https://<azureregion>.monitoring.azure.com/<AzureResourceID>/metrics (click here to learn how)
...
### Namespace
Namespaces are a way to categorize or group similar metrics together. This allows you to achieve isolation between groups of metrics that may be collecting different insights or performance indicators. For example, you could have a namespace called “ContosoMemoryMetrics” that is used track memory use metrics that profile of your app, and another namespace called “ContosoAppTransaction” that tracks all metrics about user transactions in your application.
...
### Dimension Values
When reporting a metric datapoint for each dimension key on the metric being reported, there will be a corresponding dimension value. For example if you wanted to report the memory utilized by the ContosoApp on your VM:
...
If your application is unable to pre-aggregate locally and needs to emit each discrete sample or event immediately upon collection you can simply emit the raw measure values.
