...
To take this capability for a spin, [sign-up for the preview](http://aka.ms/DynamicThresholdMetricAlerts). As always, we would love to hear your feedback, please keep it coming at [azurealertsfeedback@microsoft.com](mailto:azurealertsfeedback@microsoft.com)
...
1. From the Alerts blade under Monitor, select the **New Alert Rule** button to create a new alert in Azure.
...
6. Additional Alert logic settings:
   ...
   - Alert Sensitivity:
    ...
    ![Alert logic settings](./media/monitoring-alerts-dynamic-thresholds/006.png)
...
7. Evaluated based on:
   ...
   To reduce alert noise generated by transient spikes, we recommend using the “Number violations to trigger the alert” settings. This enables you to get an alert only if the threshold was violated X consecutive times, or Y times out of last Z periods. For example:
   ...
   ![Evaluated based on](./media/monitoring-alerts-dynamic-thresholds/008.png)
    ...
    To trigger an alert when the was a violation from a Dynamic threshold in 15 minutes out of the last 30 minutes with period of 5 minutes, use the following settings:
