...
To illustrate the autoscale setting schema, the following autoscale setting will be used. It is important to note that this autoscale setting has:
...
| metricTrigger |  metricResourceUri | The resource ID of the resource that emits the metric. In most cases, this will be the same as the resource being scaled. In some cases, it can be different, for example you can scale a virtual machine scale set based on the number of messages in a storage queue. |
...
3)	**Recurrence profile:** This type of profile enables you to ensure that this profile is always used on a particular day of the week. Recurrence profiles only have a start time, as a result they run until the next recurrence profile or fixed date profile is set to start. An autoscale setting with only one recurrence profile, will execute that profile even if there is a regular profile defined in the same setting. The two examples below illustrate the usage of this profile:
...
For example, in the preceding setting, “weekdayProfile” is set to start on Monday 12 a.m., that means this profile will start executing on Monday at 12.am. It will execute until Saturday  at 12a.m., when “weekendProfile”  is scheduled to start executing.
...
By looking at the preceding setting , “businessHoursProfile” will start executing on Monday 9 a.m. and will keep executing till 5 p.m. because that’s when “nonBusinessHoursProfile” starts executing. The “nonBusinessHoursProfile” will keep executing till 9 a.m. Tuesday and then “businessHoursProfile” will take over. This will happen till Friday 5 p.m., at that point “nonBusinessHoursProfile” will start executing all the way till Monday 9 a.m. since the “businessHoursProfile” does not start executing till Monday 9 a.m.
...
-   Autoscale will calculate the new capacity determined by the scaleAction of each of those rules. Then it will choose the scale action that results in the maximum of those capacities to ensure service availability.
