...
By default, a test run that uses reporting feature automatically generates a unique RUN_ID unless the user sets it, in which case, the set value is used. If the value of the variable remains same across runs, the results are reported together in the same run on the Playwright portal. 

> [!Tip]
> If you are using the cloud-hosted browsers provided by Microsoft Playwright Testing service to run your tests, you might already be setting this variable. To avoid overwrites, make sure you are setting it only once. 
