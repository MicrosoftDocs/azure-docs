<properties
   ...
   description="This article walks you through the steps for "
...
In order to understand the impact of above two, let’s go through some customer scenario. This customer has only one circuit to Internet and it consumes all Microsoft services via Internet. The traffic from customer to Microsoft and back traverses through the same Internet link and passes through the firewall. The firewall records the flow as it sees the first packet and return packets are allowed as the flow exists in the state table.

![Routing1](./media/expressroute-asymmetric-routing/AsymmetricRouting1.png)


Customer buys ExpressRoute and consumes some services offered by Microsoft over ExpressRoute. All other services from Microsoft will be consumed over the Internet. Microsoft will advertise more specific routes to customer over ExpressRoute for specific services. There is communication expected between customer’s public facing servers such as Web, SMTP, ADFS etc. and Microsoft. The customer’s routing infrastructure will choose ExpressRoute as preferred path for those prefixes. However, if customer is not advertising Public IP addresses of its servers such as Web, SMTP, ADFS etc. to Microsoft then Microsoft will initiate connection to these Public IP addresses via Internet. However, the return traffic from the servers to Microsoft will be via ExpressRoute. If there is a Stateful service deployed at the customer 
edge connecting to ExpressRoute such as Firewall, IDS, NAT etc.  Then this service has not seen the original request and hence it will not be present in its state table, so it will drop the return traffic. 

Customer could see similar issues with the clients on private IP addresses if the customer chooses to use the same NAT pool for ExpressRoute as well as for the Internet. Request for services such as Windows Update will go via Internet as IP addresses for these services are not advertised via ExpressRoute. However, the return traffic will come back via ExpressRoute because from Microsoft perspective if an IP address with same subnet mask is received from Internet as well as ExpressRoute, then ExpressRoute is preferred over Internet. This would mean that firewall or other Stateful device at the customer end facing ExpressRoute may drop the packets if it had no prior information about the flow. 
...
1. Routing 
    ...
    - Customer should ensure that its Public IP addresses are advertised to appropriate links. For example, if customer wants to use Internet for authentication traffic and ExpressRoute for its mail traffic. Then customer must not advertise its ADFS addresses over ExpressRoute. Similarly, on-premises ADFS server must not be exposed to IP addresses received over ExpressRoute as routes received over ExpressRoute are more specific, they will make ExpressRoute preferred path for authentication traffic to Microsoft thus causing asymmetric routing.

    - If customer wants to use ExpressRoute for authentication, then customer must make sure that it is advertising ADFS IP addresses over ExpressRoute without NAT. This way traffic originating from Microsoft to on prem ADFS server will go over ExpressRoute while return traffic from customer to Microsoft will also use ExpressRoute as it is preferred over Internet. 

2. Source based NAT

	Another way of solving Asymmetric Routing issues is via Source NAT (SNAT). For example, a customer has not advertised the public IP address of on-premises SMTP server over ExpressRoute intending to use Internet for this communication. When a request is originated from Microsoft to customer on-premises SMTP server it will traverse the Internet. Customer source NATs the incoming request to its internal IP address. When SMTP server will respond, the traffic will go to the edge firewall instead of ExpressRoute. This way the return traffic will go back via Internet. 
