...
For a like-for-like replacement that preserves the resiliency level of the existing circuit, select the **Standard Resiliency** option and follow the steps outlined in the [Create a circuit with ExpressRoute](expressroute-howto-circuit-portal-resource-manager.md) guide to create your new ExpressRoute circuit (Circuit B) in the desired peering location. Standard resiliency is a single-site configuration and isn't recommended for business or mission-critical workloads. If you're migrating a workload of that kind, evaluate High or Maximum resiliency instead. For more information, see [Design and architect Azure ExpressRoute for resiliency](design-architecture-for-resiliency.md). Then, follow the steps in [Configure peering for ExpressRoute circuit](expressroute-howto-routing-portal-resource-manager.md) to configure the required peering types: private and Microsoft.
