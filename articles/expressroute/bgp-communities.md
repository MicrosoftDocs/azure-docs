...
Managing a hybrid network can get increasingly complex as you deploy more ExpressRoute circuits and establish connections to workloads in many Azure regions. To help manage the complexity of your network and route traffic from Azure to on-premises efficiently, you can configure BGP communities on your Azure virtual networks.
...
Another reason to configure BGP communities on VNets connected to ExpressRoute is to understand where traffic is originating from within an Azure region. As customers deploy more VNets and adopt more complex network topologies within an Azure region, troubleshooting connectivity/performance issues may be more difficult. With custom BGP community values configured on each virtual network within a region, you can quickly find the specific VNet that traffic is originating from in Azure. Being able to identify the source virtual network will narrow down your investigation. 
