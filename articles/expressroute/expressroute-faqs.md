...
You will be restricted on each connection (primary and secondary) of your ExpressRoute circuit to your procured bandwidth, but you may burst up to higher speeds using the built-in redundancy of your circuit if necessary. When you pay for a circuit of a given bandwidth, it is configured to have primary and secondary connections of that bandwidth to two Microsoft Enterprise Edge routers (MSEEs) for higher reliability. The secondary connection is mainly intended for failover use, but you may use its bandwidth to burst up to two times the bandwidth limit you procured if necessary. Because this additional bandwidth is meant for redundancy, it is not guaranteed and should not be used for bursting purposes for a sustained period of time. If traffic flows through an ExpressRoute Gateway, the bandwidth for the SKU is fixed and not burstable.
