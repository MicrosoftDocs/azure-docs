...
* **Only route-based VPN gateway is supported.** You must use a route-based [VPN Gateway](../vpn-gateway/vpn-gateway-about-vpngateways.md). It is supported to use a route-based VPN gateway with a VPN connection configured for 'policy-based traffic selectors' as described in the [Connect to multiple policy-based vpn devices](../vpn-gateway/vpn-gateway-connect-multiple-policybased-rm-ps.md) article.
