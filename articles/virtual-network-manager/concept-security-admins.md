...
Security Admin rules are global network security rules that enforce security policies, based on a rule's condition set, against all virtual networks within a network group. These rules can be used to allow or deny traffic across your virtual networks. and they only apply to resources within the scope of the network manager instance. Thus, security admin rules don't apply to virtual networks not managed by a network manager instance. 
...
Admin rules are currently applied at the virtual network level, whereas network security groups can be associated at the subnet and NIC level. This table shows these differences and similarities:
...
If you create an *Always Allow* or *Deny* rule, it's evaluated first and stops the network security group evaluation of the traffic. With an Always Allow rule, the traffic goes directly to the resource and terminates further (and possibly conflicting) evaluation by network security groups rules. This action can be useful for enforcing traffic and preventing denial by network security group rules. With a Deny rule, the traffic is stopped without being delivered to the destination. Security admin rules don't depend on network security groups, so they can be used to create default security rules on their own.
