...
Once you have access to the public preview, you can deploy a security admin rule using network groups in the Azure portal. To do this, you create a security admin configuration, add a security admin rule, and deploy the security admin configuration. For more information, see [Create a security admin rule using network groups](./how-to-create-security-admin-rule-network-groups.md).
...
- Only supports manual aggregation of CIDRs in a network group. The CIDR range in a rule will only change upon the customer commit.
