...
This article covers the usage of [Azure Resource Graph](../governance/resource-graph/overview.md) with Azure Virtual Network Manager. Azure Resource Graph (ARG) extends Azure Resource Management by providing efficient and performant resource exploration with the ability to query at scale across a given set of subscriptions so that you can effectively govern your environment. With ARG integration, you can query ARG to get insights into their Azure Virtual Network Manager (AVNM) configurations. This can be done through customized queries offering resource level or at the regional level status data.

[!INCLUDE [virtual-network-manager-preview](../../includes/virtual-network-manager-preview.md)]

## Using ARG with virtual network manager

The following are some of the scenarios where Azure Resource Graph can be leveraged for insight into Azure Virtual Network Manager:

- Retrieve regional goal state data to understand the configurations that have been deployed in each region and their status.
- Discover all the resources that have a particular configuration applied.
- Retrieve effective configurations that have been applied to a virtual network and its provisioning state.
- Identify the number of virtual networks that have succeeded, failed, or are in progress during a deployment process.
...
4. Schema Overview
Security Admin
Please follow the document to check the schema for security admin -> Security Admin ARG schema doc

Connectivity
Please follow the document to check the schema for connectivity -> Connectivity ARG schema doc

## Get Started

To get started with querying your virtual network manager data in ARG, follow these steps:

1. Search for *Resource Graph Explorer* in the Azure portal. Select the same to get redirected to the ARG query editor.
...
You can download the output of these queries as CSV from the Resource Graph Explorer. You can also use these queries in custom automation using any automation clients supported by ARG, such as PowerShell, CLI, or SDK. You can also create custom workbooks in the Azure portal using ARG as a data source.
...
The following are some sample ARG queries on your virtual network manager data that you can use in custom dashboards and automations. listed with each query is the input involved and the output returned.
...
For a given security admin configuration, get count of virtual networks successfully applied this config and count of virtual networks failed to apply this config.

Input: {SecurityAdminConfigurationSnapshotId}
Example id - "/subscriptions/2c505610-a5dd-473e-aa33-b31aac398e29/resourceGroups/CnAISecurityAzureNetworkManager/providers/Microsoft.Network/networkManagers/NRMS-ZeroTrust-CorpProdNonprod/securityAdminConfigurations/config_2023-05-15-15-07-27/snapshots/0"
...
For a given security admin config, get how many vnets are in progress, succeeded or failed.
...
```
> [!NOTE]
>  Remove the last line in above query to get the failure message per vnet in case of failure.
