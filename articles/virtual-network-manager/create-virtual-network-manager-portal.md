---
title: 'Quickstart: Create a mesh network with Azure Virtual Network Manager using the Azure portal'
...
# Quickstart: Create a mesh network with Azure Virtual Network Manager using the Azure portal
...
1. Repeat steps 2-5 to create two more virtual networks with the following information:
    ...
    | Resource group | Select the same resource group you deployed **VNetA** into. |
...
1. Select **+ Create a resource** and search for **Network Manager**. Then select **Create** to begin setting up the Azure Virtual Network Manager instance.
...
1. On the *Basics* tab, enter or select the following information:
    ...
    | Setting | Value |
    | ------- | ----- |
    | Subscription | Select a subscription you want to manage AVNM from. |
    | Resource group | Select or create a resource group for AVNM. This example will use the **myAVNMResourceGroup**.
...
1. Go to Azure Virtual Network Manager you just created previously.
...
1. On the *Add a network group* page, enter a **Name** for the group. This example will use the name **myNetworkGroup**. You also have the option add a description for the group. Then select **Next: Static group members >** .
...
1. On the *Add virtual networks* page, select all three virtual networks you created in a previous step (VNetA, VNetB, and VNetC). Then select **Add** to commit the selection.
...
1. You'll then see the new network group added to the *Network Groups* page.
...
1. Select **Connectivity** from the menu to begin creating a connectivity configuration.
...
1. Once you've selected the *Mesh* topology, the **Global Mesh** and **Network Groups** option will appear. *Global Mesh* is not required for this set up since all the virtual networks are in the same region. Select **+ Add network groups** and then select the network group you created in the last section. Click **Select** to add the network group to the configuration.
...
1. You'll then see the new connectivity configuration added to the *Configuration* page.
...
To have your configurations applied to your environment, you'll need to commit a deployment of the configuration to the regions where the virtual networks were created.
...
1. Select the following settings:
    ...
    | Target regions | Select the region to deploy this configuration to. Select **West US**, since all the virtual networks create are in that region. |
...
1. To remove a deployment from a region, deploy a **None** configuration with a Target region*. Select **Deploy** and then select **OK** to confirm removal of the configuration.
...
1. To delete a configuration, select **Configurations** under *Settings. Select the checkbox next to the configuration and then select **Delete** at the top.
...
1. To delete a Network group, select **Network Groups** under *Settings. Select the checkbox next to the Network group and then select **Delete** at the top.
...
1. Once the Network groups have been removed, you can now delete the resource by selecting the Azure Virtual Network Manager from the list and selecting **Delete**.
