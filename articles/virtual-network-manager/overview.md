...
During the creation process, you define the scope for what your Azure Virtual Network Manager will manage. The scope can be defined at the subscription and [management group](../governance/management-groups/overview.md) level. After defining the scope, you enable features such as *Connectivity* and the *SecurityAdmin* role for your Virtual Network Manager.

After you create the Virtual Network Manager instance, you then create a *network group* by using conditional statements to select virtual networks by name, tags, or IDs (dynamic membership). You can also select specific virtual networks (static membership). You then create connectivity and/or security configuration(s) applied to those network groups based on your topology and security needs. 
