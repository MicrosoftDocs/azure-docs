...
In this article, you learn how to configure Intune Endpoint Privilege Management (EPM) for dev boxes so that dev box users don't need local administrative privileges.

Intune Endpoint Privilege Management allows your organization's users to run as a standard user (without administrator rights) and complete tasks that require elevated privileges. Tasks that commonly require administrative privileges are:

 - Installing applications (like Microsoft 365 applications).
 - Updating device drivers.
 - Running certain Windows diagnostics.

Endpoint Privilege Management is built into Intune, which means that all configuration is completed within the Microsoft Intune admin center. To get started with EPM, use this high-level process:

- **License Endpoint Privilege Management:** Before you can use Endpoint Privilege Management policies, you must license EPM in your tenant as an Intune add-on. For licensing information, see [Use Intune Suite add-on capabilities](/mem/intune-service/fundamentals/intune-add-ons).
- **Deploy an elevation settings policy:** An elevation settings policy activates EPM on the client device. With this policy, you can configure settings that are specific to the client but aren't necessarily related to the elevation of individual applications or tasks.
...
Endpoint Privilege Management requires either a stand-alone license that adds only EPM, or a license for EPM as part of the Intune Suite.
...
1. Configure an Intune admin role for EPM administration:
...
1. Assign Microsoft 365E5 and EPM licenses to target users in Microsoft Entra ID:

    1. In the Microsoft Intune admin center, go to **Users**, and select the user for whom you want to assign the Microsoft 365 E5 and EPM licenses.
...
1. Create a dev box for testing the policy:
   ...
   1. Determine the dev box hostname. You use this hostname to add the dev box and Intune group in the next step.
...
1. Create an EPM elevation settings policy and assign it to the group:
    ...
    1. On the **Create a profile** pane, select the following settings:
...
1. Verify that the Microsoft EPM Agent is installed on the dev box:
    ...
    1. Go to *c:\Program Files*, and verify that a folder named **Microsoft EPM Agent** exists.
