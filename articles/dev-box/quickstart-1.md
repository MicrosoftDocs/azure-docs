---
title: 'Quickstart: Create and run a load test with Microsoft Dev Box'
description: another placeholder for Microsoft Dev Box description
services: dev-box
ms.service: dev-box
ms.topic: quickstart
author: RoseHJM
...
# Quickstart: Create and run a load test with Microsoft Dev Box Preview

This quickstart describes how to create an Microsoft Dev Box Preview resource by using the Azure portal. With this resource, you'll create a load test with an Apache JMeter script and run the test against an external website. 
...
## <a name="create_resource"></a> Create an Microsoft Dev Box resource

First, you'll create the top-level resource for Microsoft Dev Box. It provides a centralized place to view and manage test plans, test results, and related artifacts.
...
1. Open *SampleTest.jmx* in a text editor and paste the following code snippet in the file:
    ...
    <jmeterTestPlan version="1.2" properties="5.0" jmeter="5.4.1">
      <hashTree>
        <TestPlan guiclass="TestPlanGui" testclass="TestPlan" testname="Microsoft Dev Box Quickstart" enabled="true">
...
With Microsoft Dev Box, you can use an Apache JMeter script to create a load test. This script defines the application test plan. It contains information about the web endpoint, the number of virtual users, and other test configuration settings.

To create a load test by using an existing Apache JMeter script:

1. Go to your Microsoft Dev Box resource, select **Tests** from the left pane, and then select **+ Create new test**.

    :::image type="content" source="./media/quickstart-create-and-run-loadtest/create-new-test.png" alt-text="Screenshot that shows the Microsoft Dev Box page and the button for creating a new test." :::
...
You now have an Microsoft Dev Box resource, which you used to load test an external website.
...
> [Identify performance bottlenecks](./tutorial-1.md)
