...
All the commands to run the Azure Hyperledger Fabric script can be executed through Azure Bash command-line interface (CLI). You can sign in to Azure Cloud Shell throughâ&euro;¯the ![Hyperledger Fabric on Azure Kubernetes Service Template](./media/hyperledger-fabric-consortium-azure-kubernetes-service/arrow.png) option at the upper-right corner of the Azure portal. On the command prompt, type `bash` and select the Enter key to switch to the Bash CLI, or select **Bash** from the Cloud Shell toolbar.
...
After you finish the initial setup, use the client application to achieve the following operations: â&euro;¯
...
# If you are using chaincode_example02 then set CC_NAME=â&euro;œchaincode_example02â&euro;
CC_NAME=<chaincodeName>
# If you are using chaincode_example02 then set CC_VERSION=â&euro;œ1â&euro; for validation
...
# If you are using chaincode_example02 to validate then CC_PATH=â&euro;œ/home/<username>/azhlfTool/samples/chaincode/src/chaincode_example02/goâ&euro;
...
Pass the invoke function name and space-separated list of arguments inâ&euro;¯`<invokeFunction>`â&euro;¯andâ&euro;¯`<invokeFuncArgs>`â&euro;¯respectively. Continuing with the chaincode_example02.go chaincode example, to perform an invoke operation, setâ&euro;¯`<invokeFunction>`â&euro;¯toâ&euro;¯`invoke`â&euro;¯andâ&euro;¯`<invokeFuncArgs>`â&euro;¯to `"a" "b" "10"`.
...
Pass the query function name and space-separated list of arguments inâ&euro;¯`<queryFunction>`â&euro;¯andâ&euro;¯`<queryFuncArgs>`â&euro;¯respectively. Again takingâ&euro;¯chaincode_example02.goâ&euro;¯chaincode as a reference, to query the value of "a" in the world state, setâ&euro;¯`<queryFunction>`â&euro;¯toâ&euro;¯`query` andâ&euro;¯`<queryArgs>` to `"a"`.
