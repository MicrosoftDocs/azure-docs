...
To build a Hyperledger Fabric network on Azure, you need to deploy an ordering service and organization with peer nodes. Using the Hyperledger Fabric on Azure Kubernetes Service solution template, you can create order nodes or peer nodes. You need to deploy the template for each node that you want to create.
...
- **Key vault**: Instance of the Azure Key Vault service that's deployed to save the Fabric CA credentials and the root certificates provided by customer. The vault is used in case of template deployment retry, to handle the mechanics of the template.
...
5. Enter the following details:
    ...
    - **Peer node world state database**: Choose between LevelDB and CoucbDB. This field is displayed when you choose **Peer nodes** in the **Fabric network component** drop-down list.
    - **Fabric CA username**: Enter the username that's used for the Fabric CA authentication.
...
6. Select the **AKS cluster Settings** tab to define the Azure Kubernetes Service cluster configuration that is the underlying infrastructure on which the Fabric network components will be set up.
...
7. Enter the following details:
    ...
    - **Kubernetes version**: Choose the version of Kubernetes that will be deployed on the cluster. Based on the region selected on the **Basics** tab, the available supported versions might change.
    ...
    - **Node count**: Enter the the number of Kubernetes nodes to be deployed in the cluster. We recommend keeping this node count equal to or more than the number of Hyperledger Fabric nodes specified on the **Fabric settings** tab.
    ...
    - **Log Analytics workspace**: The Log Analytics workspace will be populated with the default workspace that is created if monitoring is enabled.
...
9. After validation passes, select **Create**.
    ...
    The deployment usually takes 10-12 minutes. The time might vary depending on the size and number of AKS nodes specified.
...
The following image shows the step-by-step process to build a consortium between an orderer organization and peer organization. The following sections show detailed commands to complete these steps.
...
#### Set environment variables for the orderer organization client
...
#### Set the environment variables for the peer organization client
...
#### Set the environment variables for setting up an Azure storage account
...
Use the following commands to generate an Azure file share connection string.
...
From the orderer organization's client, use the following command to create a new channel. This command will create a channel with only the orderer organization in it.  
