...
description: Safely excecute compelx upgrades of workloads with Azure Operator Service Manager.
author: msftadam
ms.author: adamdor
ms.date: 08/16/2024
...
This article introduces Azure Operator Service Manager (AOSM) safe upgrade practices (SUP). This feature-set enables an end-user to safely excecute compelx upgrades of CNF workloads hosted on Azure Operator Nexus, in compliance with partner ISSU requirements, where applicable. Lookg for future artiles in these services to expand on SUP features and capabilities.
...
To update an existing Azure Operator Service Manager site network service (SNS), the Operator will execute a reput update request against the deployed SNS resource.  Where a SNS contains CNFs with multiple NfApps, this request will fan out execution of helm upgrades across any NfApps defined in the network function definition version NFDV, by default in the order which they appear, or optionally in the order defined by UpdateDependsOn parameter.
...
With onboarding complete, the Re-Put operation is submitted by an AOSM operator role using standard Azure interfaces. Depending on the number, size and complexity of the NfApps, the reput operation could take some time to complete (multiple hours).

![iFigre 5 - SNS Re-put](media/sup5.png)

#### Examine Re-Put Results

If the reput is reporting a successful result, the upgrade has been completed and the user should validate the state and availability of the service. If the reput is reporting a failure, follow the steps in the upgrade failure recovery section to continue.
...
* NFDV Rollback-On-Failure - Based on flag, rollback all completed NfApps on failure.
...
For the applicationEnablement property, the publisher has two options: either provide a default value or parameterize it. In the below example below, applicationEnablement is parameterized for the NfApp testroletwoso that its value must be provided by the operator at run-time.
