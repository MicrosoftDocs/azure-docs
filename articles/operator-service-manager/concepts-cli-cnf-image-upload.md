...
The Azure CLI AOSM extension NFDV input file, generated by `az aosm nfd generate-config --definition-type cnf`, contains one parameter that specifies the source registries (and optionally namespaces) the AOSM CLI will query during container image onboarding.
...
The Azure AOSM CLI extension will search for the images in `myregistry.azurecr.io`, where it will discover `contoso-a`, `contoso-b`, and `contoso-c`. It will then search in `myregistry.azurecr.io/test`, where it will discover `contoso-test`.
