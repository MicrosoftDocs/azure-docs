...
* These changes help to recognize decommissioned artifacts. For existing resources created with older API’s, the NSDVs and NFDVs should be updated to newer API version. First change the artifact store references to `artifact-manifest` references. Then update the `artifact-manifests` withe the required expanded references. 
