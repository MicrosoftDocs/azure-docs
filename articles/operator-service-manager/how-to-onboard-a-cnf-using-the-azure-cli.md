...
1. Generate BICEP files that define a Network Function Definition (NFD) based on your Helm charts and values.yaml.
1. Publish the NFD and upload the CNF images and charts to an Artifact Store (AOSM-managed Azure Container Registry (ACR)).
1. Add your published NFD to the BICEP files that define a Network Service Design (NSD).
1. Publish the NSD and associated network function ARM template.
