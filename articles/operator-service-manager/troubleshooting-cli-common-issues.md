...
### nfdv in Configuration Group Value (CGV) doesn't match the published Network Function Definition Version (NFDV)

Configuration group schemas generated by the Azure CLI AOSM Extension have a mandatory parameter called `nfdv`. `nfdv` is the name of the NFDV, which is a string in `1.0.0` format. It isn't the name of the NF or Network Function Definition Group (NFDG). The following example shows the correct usage.
