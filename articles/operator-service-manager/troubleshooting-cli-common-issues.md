...
Configuration group schemas generated by the Azure CLI AOSM Extension exposes a `deployParameters` field that, by default, is an array of JSON objects. There are several reasons you may want to create a CGV with an empty `deployParameters` field:
...
If you are creating a CGV with an empty `deployParameters` field, the field value must be an array containing an empty JSON object.
