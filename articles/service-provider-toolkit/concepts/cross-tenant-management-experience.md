...
Typically, in order to manage Azure resources for a customer, service providers need to sign in to the Azure portal using an account associated with the customer's tenant. This means that an administrator in the customer's tenant must create user accounts for the service provider.
