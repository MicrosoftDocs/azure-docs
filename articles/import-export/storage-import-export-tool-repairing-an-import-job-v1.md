...
|**/PathMapFile:**<DrivePathMapFile\>|**Optional.** Path to a text file used to resolve ambiguities if you were importing multiple files with the same name in the same job. The first time the tool is run, it can populate this file with all of the ambiguous names. Later runs of the tool use this file to resolve the ambiguities.|  
