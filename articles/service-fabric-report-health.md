...
The Service Fabric reporters are intended to monitor identified conditions of interest. They report on those conditions based on their local view. The [Health Store](service-fabric-health-introduction.md#Health-Store) aggregates health data sent by all reporters to determine whether entities are globally healthy. The model is intended to be rich, flexible and easy to use.
...
## Design health reporting: define health reports
...
Once the conditions are identified, watchdog writers need to figure out the best way to monitor them for best balance between overhead and usefulness. For example, consider a service that does some complex calculations using some temporary files on a share. A watchdog could monitor the share to make sure enough space is available. It could listen for notifications for file/directory changes. It can report an warning if an up-front threshold is reached and error if the share is full. On warning, a repair system could start clean up of older files on the share. On error, a repair system could move the service replica to another node. The condition states should be described in terms of health: what is the state of the condition that can be considered healthy or unhealthy (warning or error).

Once the monitoring details are set, watchdog writer need to figure out how to implement the watchdog. If the conditions can be determined from within the service, the watchdog can be part of the monitored service itself. For example, the service code can check the share usage and report using a local fabric client every time it tries to write a file. The advantages of this approach is that reporting is simple. Care must be taken so no watchdog bugs can impact the service functionality. This approach will not always work. An watchdog within the service may not be able to detect the conditions: either it doesn't have the logic, or the overhead of monitoring the conditions is high, or the conditions are not specific to a service, but affect interactions between services. Another option is to have watchogs in the cluster as separate process, watchdog that monitor the conditions and report, without affecting the services in any way. These watchdogs could for example be implemented as stateless services in the same application, deployed on all nodes or on the same nodes as the service. Sometimes, this approach doesn't work either. If the monitored conditions are the availability or the functionality of the service as users see it, it's best to have the watchdogs in the same place as the user clients, testing the operations in the same way users call them. For example, we can have a watchog leaving outside the cluster and issuing requests to the service, checking the latency and the correctness of the result (for a calculator service, does 2+2 return 4?).
...
- The Master primary replica reports on Master service. If replica goes down, the service still has the report about long execution time. We can have one property for all pending tasks in the queue: if at least one task takes longer, report on property "PendingTasks" warning or error, as appropriate. If there are no pending tasks or all are just started, report Ok. The tasks are persisted, so if replica goes down, the new promoted primary can continue to report properly.
- Another watchdog process (in the cloud or external) checks the tasks to see if they are completed. If they do not respect the thresholds, report on Master service. Report on each task and include the task identifier (eg: PendingTask+taskid). Report only on unhealthy states. Set TTL to a few minutes and mark the reports to be removed when expired to ensure cleanup.
...
All these reports are captured in application health when health is evaluated.

## Design health reporting: periodic vs. on transition
...
Based on the health data, service writers and cluster/application administrators can think of ways to consume the information, like alerts alerts they can set up to catch severe issues before provoking outages or automatic repair systems.
