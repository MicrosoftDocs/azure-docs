...
These X12 tracking schemas are supported:
...
| CorrelationMessageId | String | Correlation message ID, which is a combination of {AgreementName}*{GroupControlNumber}*{TransactionSetControlNumber} (Optional) |
...
| interchangeControlNumber | String | Interchange control number of the functional acknowledgement; value populates only for send side where functional acknowledgement is received for the messages sent to partner (Optional) |
| functionalGroupControlNumber | String | Functional group control number of the functional acknowledgement; value populates only for send side where functional acknowledgement is received for the messages sent to partner (Optional) |
| isaSegment | String | ISA segment of the message; value populates only for send side where functional acknowledgement is received for the messages sent to partner (Optional) |
| gsSegment | String | GS segment of the message; value populates only for send side where functional acknowledgement is received for the messages sent to partner (Optional) |
...
| segmentsCount | Enum | Acknowledgement status code: Accepted, Rejected, or AcceptedWithErrors (Mandatory) |
| processingStatus | Enum | Processing status of the acknowledgement: Received, Generated, or Sent (Mandatory) |
| CorrelationMessageId | String | Correlation message ID, which is a combination of {AgreementName}*{GroupControlNumber}*{TransactionSetControlNumber} (Optional) |
| isMessageFailed | Boolean | Whether the X12 message failed (Mandatory) |
...
| isMessageFailed | Boolean | Whether the X12 message failed (Mandatory) |
| statusCode | Enum | Interchange acknowledgement status code: Accepted, Rejected, or AcceptedWithErrors (Mandatory) |
| processingStatus | Enum | Acknowledgement status: Received, Generated, or Sent (Mandatory) |
...
| agreementName | String | Name of the X12 agreement to which the messages are resolved (Optional) |
| direction | Enum | Direction of the message flow, Receive or Send (Mandatory) |
| interchangeControlNumber | String | Interchange control number, which populates for the send side when a technical acknowledgement received from partners (Optional) |
...
| isMessageFailed | Boolean | Whether the X12 message failed  (Mandatory) |
| statusCode | Enum | Acknowledgement status code: Accepted, Rejected, or AcceptedWithErrors (Mandatory) |
| processingStatus | Enum | Processing status of the acknowledgement: Received, Generated, or Sent (Mandatory) |
| ak903 | String | Number of the transaction sets received (Optional) |
