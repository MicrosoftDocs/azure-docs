...
1. **Actions**: The actions lets you perform predefined actions on the Box account configured with the logic app. Following are the actions that can be performed on Box account using Box connector:
    ...
    d. Delete File: The action deletes specified file from a folder. If the file/folder is not found an exception is thrown. Following is the list of parameters required for the action:
 	...
 	<table>
	  ...
	  <tr>
	    <td>File Path</td>
	    <td>Complete File Path including Folders.</td>
	    <td>Yes</td>
	  </tr>
	</table>
...
1. Open the Azure Marketplace from the homepage of the Azure Portal.

2. Under "Everything" search for “Box Connector”.
...
3. If Box connector is selected at the start of the logic app it acts like trigger else actions could be taken on Box account using the connector. Note that Box Connector doesnt have any triggers as of this writing.
