...
To execute the CSV Parser DAG workflow, the user must first create and register the schema using the workflow service. Once the schema is created, the user then uses the File service to upload the CSV file to the Microsoft Energy Data Services Preview instances and also creates the file generic manifest record. Once the file generic manifest record is created, the file service provides a file id to the user. User then uses the file id to trigger the CSV Parser workflow using the Workflow service. The Workflow service provides a run id, which the user could use to track the status of the CSV Parser workflow run.
