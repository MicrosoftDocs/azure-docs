...
Azure Data Manager for Energy Preview supports zone-redundant instance by default and there's no additional configuration required.
...
Azure Data Manager for Energy Preview is a regional service and, therefore, is susceptible to region-down service failures. Azure Data Manager for Energy Preview follows an active-passive failover configuration to recover from regional disaster. An active-passive configuration keeps warm Azure Data Manager for Energy instance running in the secondary region, but doesn't send traffic there unless the primary region fails. 

:::image type="content" source="media/reliability-energy-data-services/cross-region-disaster-recovery.png" alt-text="Azure data manager for energy cross region disaster recovery workflow." lightbox="media/reliability-energy-data-services/cross-region-disaster-recovery.png":::
...
Azure Data Manager for Energy Preview service continuously monitors service health in the primary region. If a hard service down failure is detected in the primary region, we attempt recovery before initiating failover to the secondary region on your behalf. We will notify you about the failover progress. Once the failover completes, you could connect to the Azure Data Manager for Energy Preview instance in the secondary region and continue operations. However, there could be slight degradation in performance due to any capacity constraints in the secondary region. 
