...
This article provides an overview of security features in Azure Data Manager for Energy Preview. It covers the major areas of [encryption at rest](../security/fundamentals/encryption-atrest.md), encryption in transit, TLS, https, microsoft-managed keys, and customer managed key.
...
1.	When you enable customer-managed keys for an existing Azure Data Manager for Energy Preview instance you must specify a managed identity that will be used to authorize access to the key vault that contains the key. The managed identity must have permissions to access the key in the key vault.
