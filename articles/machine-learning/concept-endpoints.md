...
Learn how to [deploy and use batch endpoints with the Azure CLI](batch-inference/how-to-use-batch-endpoint.md) and the [studio web portal](batch-inference/how-to-use-batch-endpoints-studio.md)
...
You can [override compute resource settings](batch-inference/how-to-use-batch-endpoint.md#configure-the-output-location-and-overwrite-settings) (like instance count) and advanced settings (like mini batch size, error threshold, and so on) for each individual batch inference job to speed up execution and reduce cost.
...
For more information on supported input options, see [Batch scoring with batch endpoint](batch-inference/how-to-use-batch-endpoint.md#invoke-the-batch-endpoint-with-different-input-options).
