...
| `compute` | string | Name of the compute target to execute the job on. This value should be a reference to an existing compute in the workspace using the `azureml:<compute-name>` syntax. If omitted, Azure ML will use the compute defined in the pipeline job's `settings.compute` property. | | |
...
| `inputs.<input_name>` | number, integer, boolean, string, or object | One of a literal value (of type number, integer, boolean, or string), [JobInputUri](#jobinputuri), or [JobInputDataset](#jobinputdataset). You can also reference outputs from other job in same pipeline via `jobs.<COMPONENT_NAME>.outputs.<OUTPUT_NAME>` | | |
