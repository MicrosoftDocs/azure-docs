...
Managed online endpoints (preview) provide you the ability to deploy your model without you having to create and manage the underlying infrastructure. In this article, you'll start by deploying a model on your local machine to debug any errors, and then you'll deploy and test it in Azure. You'll also learn how to view the logs and monitor the Service Level Agreement (SLA). You start with a model and end up with a scalable HTTPS/REST endpoint that can be used for online/real-time scoring.
...
> [!Tip]
> The format of the scoring script for managed online endpoints is the same format used in previous version of the CLI and in the Python SDK
