...
7. See the [Results](#results) section for a description of the reports.
    ...
    To get a copy of the model for reuse later, switch to the **Outputs** tab in the right panel of the module that contains the algorithm (for example, the **Two Class Bayes Point Machine**). Then click on the icon of **Register dataset**, which will save a copy of the trained model in the module tree.
...
To view these results, in the pipeline, right-click the Cross Validate Model module. Select **Visualize Scored results**.
...
To view these results, in the pipeline, right-click the Cross Validate Model module. Select **Visualize Evaluation results by fold**.
