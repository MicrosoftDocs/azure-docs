...
If you want to save the data for reuse, rather than importing a new set of data each time the pipeline is run, click on the icon **Register dataset** under the **Outputs** tab in the right panel of the module. Choose a name for the dataset. The saved dataset preserves the data at the time of saving, and data is not updated when the pipeline is rerun, even if the dataset in the pipeline changes. This can be handy for taking snapshots of data.
