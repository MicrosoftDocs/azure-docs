...
1. Connect the **Scored dataset** output of the [Score Model](./score-model.md) to the **left-hand** input of **Evaluate Model**. 
2. [optional] Connect the **Scored dataset** output of the [Score Model](./score-model.md) for the second model to the **right-hand** input of **Evaluate Model**. This feature is useful because you can easily compare results from two different but **the same algorithm type** models on the same data. Or, you might compare scores from two different runs over the same data with different parameters.
Note: algorithm type refers to 'Two-class Classification', 'Multi-class Classification', 'Regression', 'Clustering' under 'Machine Learning Algorithms'. 
3. Click **Evaluate Model** module, and run the pipeline to generate the evaluation scores.
