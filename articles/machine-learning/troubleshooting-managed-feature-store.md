...
## Issues found when creating a feature store

When you create a feature store, you may encounter the following issues.
...
## Issues found when configuring offline storage

When configuring offline storage, you may encounter the following issues.

### Wrong Offline Store Configuration
...
The root cause is a wrong offline store target is put in the feature store yaml file. If a wrong URI is put, you will hit error in materialization job.

Please check `offline_store.target` and make sure it is a valid Azure lake gen2 storage URI.
```yaml
...
## Update Feature Store using CLI Multiple Times Hit Error
...
```
3. Update the feature store the second time
```yaml
az ml feature-store update --subscription {sub-id} --resource-group {rg} --name {your-featurestore-name} --file \path\featurestore-materialization.yaml
```
Error:
...
You develop feature set in an AzureML Spark notebook using following code snippet. If there are invalid schema in the feature set specification yaml, you will see various errors.
...
## Cannot Resolve Feature Retrieval Specification
...
## Consume Features in Training Job Issue 
...
## Missing Feature Retrieval Spec with Batch Inference using Model as Input
...
## Do not See Data When Run get_offline_features in a Notebook
### Symptom
...
Please make sure `temporal_join_lookback > source.source_delay`. In addition, please refer to the [todo: feature-retrieval-concept-todo] for detail.
