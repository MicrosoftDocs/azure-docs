...
* When a compute instance is deployed in a workspace with a private endpoint, it can be only be [accessed from within virtual network](./how-to-create-manage-compute-instance.md#advanced-settings). If you are using custom DNS or hosts file please add an entry for < instance-name >.< region >.instances.azureml.ms with private IP address of workspace private endpoint. For more information see the [custom DNS](./how-to-custom-dns.md?tabs=azure-cli) article.
