...
> [!NOTE]
> The `validation_data` parameter requires the training_data and label_column_name parameters to be set as well. You can only set one validation parameter, that is you can only specifiy either `validation_data` or `n_cross_validations`, not both parameters
