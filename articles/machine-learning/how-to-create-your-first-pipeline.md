...
ML pipelines use remote compute targets for computation and temporary data associated with that pipeline. They can read and write data to and from supported [Azure Storage](https://docs.microsoft.com/azure/storage/) locations.
...
* Configure a `Dataset` object to point to persistent data that lives in, or is accessible in, a datastore. Configure a `OutputFileDatasetConfig` object for temporary data passed between pipeline steps. 
...
> [!IMPORTANT]
> Writing output data back to a datastore using `OutputFileDatasetConfig` is only supported for Azure Blob, Azure File share, ADLS Gen 1 and ADLS Gen 1 datastores.
...
* Configures access to `Dataset` and `OutputFileDatasetConfig` objects. For as `as_mount()` access mode, FUSE is used to provide virtual access. If mount is not supported or if the user specified access as `as_upload()`, the data is instead copied to the compute target.
