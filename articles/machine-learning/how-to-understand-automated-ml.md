...
[!IMPORTANT]
> This chart is available in the training run for models generated from training and validation data as well as in the test run based on test data. We allow up to 20 data points before and up to 80 data points after the forecast origin.> For DNN models, this chart in the training run shows data from the last epoch i.e. after the model has been trained completely.> For DNN models, this chart in the test run can have missing data before the horizon line if validation data was provided during the training run.![Forecast horizon chart](./media/how-to-understand-automated-ml/forecast-horizon.png)
