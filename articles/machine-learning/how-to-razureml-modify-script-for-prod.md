...
You can store arbitrary script outputs like data files, images, serialized R objects, etc. that are generated by the R script in Azure Machine Learning. Create a `./outputs` directory to store any generated artifacts (images, models, data, etc.) Any files saved to `./outputs` will be automatically included in the run and uploaded to the experiment at the end of the run. Since you added a default value for the `--output` parameter in the [input parameters](#add-parsing-of-input-parameters) section, you can include the following code snippet in your R script to create the `output` directory.
