...
```
The code for the `azureml_utils.R` script is:
...
In the [parameters section](#add-parsing), you defined an input parameter called `--data_file`. The data_file parameter is a whole path, so that you can  use `read_csv(args$data_file)` to read the data asset.
...
Use these code snippet as a guide to structure your R script, following all the changes outlined in this article.
