...
The following **menu** links to topics that describe how to ingest data into other target environments where the data can be stored and processed during the Team Data Science Process (TDSP).
...
To create the linked service for the on-premise SQL Server, click the **Data Store** in the ADF landing page on Azure Classic Portal, select *SQL* and enter the credentials for the *username* and *password* for the on-premise SQL Server. You need to enter the servername as a **fully qualified servername backslash instance name (servername\instancename)**. Name the linked service *adfonpremsql*.

###<a name="adf-linked-service-blob-store"></a>Linked service for Blob
To create the linked service for the Azure Blob Storage account, click the **Data Store** in the ADF landing page on Azure Classic Portal, select *Azure Storage Account*, and enter the Azure Blob Storage account key and container name. Name the link service *adfds*.

###<a name="adf-linked-service-azure-sql"></a>Linked service for Azure SQL database
To create the linked service for the Azure SQL Database, click the **Data Store** in the ADF landing page on Azure Classic Portal, select *Azure SQL* and enter the credentials for the *username* and *password* for the Azure SQL Database. The *username* must be specified as *user@servername*.   
...
> [AZURE.NOTE]  The following procedures use Azure PowerShell to define and create the ADF pipeline. But this task can also be accomplished using the 
