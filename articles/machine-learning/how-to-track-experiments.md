...
|Images|Function:<br>`run.log_image(name, path=None, plot=None)`<br><br>Example:<br>`run.log_image("ROC", plot=plt)`<br>`run.log_image("Image", path="./pngfile.png")` | Log an image to the run record. Use log_image to log an image file (.png) or a matplotlib plot to the run.  These images will be visible and comparable in the run record.|
