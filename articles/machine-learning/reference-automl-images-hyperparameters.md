...
| `tile_grid_size` | The grid size to use for tiling each image. <br>*- `tile_grid_size` must not be None to enable [small object detection](how-to-use-automl-small-object-detect.md) logic.*<br>*- `tile_grid_size` is not supported for instance segmentation tasks.*<br> A tuple of two integers passed as a string. Example: --tile_grid_size "(3, 2)" | No Default |
