...
description: Learn how develop your own tool package in Prompt Flow. 
...
In this article, we will guide you through the process of developing your own tool package, offering detailed steps and advice on how to utilize your creation.
...
Create a new conda environment using Python 3.9 or 3.10. Run below command to install Prompt Flow dependencies:
...
The points outlined below explain the purpose of each folder/file in the package. If your aim is to develop multiple tools within your package, make sure to closely examine point 2 and 5:
...
1. Create a customized environment with docker context.
   ...
   Currently we support creating environment with "Create a new docker context" environment source. "Use existing docker image with optional conda file" has known [limitation](../how-to-manage-environments-v2?view=azureml-api-2&tabs=cli#create-an-environment-from-a-conda-specification) and is not supported now.
...
4. Go to the extension and open one flow folder. Select 'flow.dag.yaml' and preview the flow. Next, select `+` button and you will see your tools. You may need to reload the windows to clean previous cache if you don't see your tool in the list.
