...
* The input dataset for training, `titanic_ds`, as a script argument. `as_named_input()` is required so that the input dataset can be referenced by the assigned name `titanic` in your training script. 
...
src = ScriptRunConfig(source_directory=script_folder,
                      ...
                      arguments=['--dataset', titanic_ds.as_named_input('titanic')],
...
The following example creates a ScriptRunConfig that passes in the FileDataset via `arguments`. After you submit the run, data files referred by the `mnist` dataset will be mounted to the compute target.
...
src = ScriptRunConfig(source_directory=script_folder,
                      ...
                      arguments=['--data-folder', mnist_ds.as_named_input('mnist').as_mount(), '--regularization', 0.5],
                      compute_target=compute_target,
                      environment=myenv)
