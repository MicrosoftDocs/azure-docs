...
> [!Note]
> If you are using a custom Docker base image, you will need to install fuse via `apt-get install -y fuse` as a dependency for dataset mount to work. Learn how to build a custom build image [here](how-to-deploy-custom-docker-image.md#build-a-custom-base-image).
