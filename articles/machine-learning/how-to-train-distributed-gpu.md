...
You can view the full list of InfiniBand-enabled machine SKUs here. On that page, for instance, you'll see the VM SKU STANDARD_NC24S_V3 is not InfiniBand-enabled, but the SKU STANDARD_NC24RS_V3 is. The only difference in the names is that added 'R'. Aside from the InfiniBand capabilities, the specs between these two SKUs are largely the same – they have both 24 cores, 448 GB RAM, 4 NVIDIA Tesla V100 GPUS, etc. (Caveat: the older-generation machine SKU STANDARD_NC24R is a notable exception to the rule that SKU names containing 'R' are InfiniBand-enabled. The SKU is RDMA-enabled, but it does not have SR-IOV hardware required for InfiniBand.)
