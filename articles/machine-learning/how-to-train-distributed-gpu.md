...
As the number of VMs training a model increases, the time to train that model should decrease. The decrease in time, ideally, should be linearly proportional to the number of training VMs. For instance, if training a model on one VM takes 100 seconds, then training that same model using two VMs should ideally take 50 seconds. Training that same model using four VMs should ideally take 25 seconds, and so on.
