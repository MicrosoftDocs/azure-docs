...
|[Torch](http://torch.ch/) | No |Yes (Ubuntu)|Torch is installed in `/dsvm/tools/torch`. PyTorch is installed in Python 2.7 (_root_), as well as Python 3.5 (_py35_) environment. <br/><br/>Torch samples are in `/dsvm/samples/torch` and PyTorch samples are in `/dsvm/samples/pytorch`. |
