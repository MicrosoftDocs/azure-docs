...
Azure Machine Learning is a fully managed cloud service that enables you to build, deploy, and share predictive analytics solutions. You can build your experiments and models from Azure Machine Learning designer (preview). You can access it from a web browser on the Data Science Virtual Machine by going to the [Azure Machine Learning studio](https://ml.azure.com/).

After you sign in to the designer, you can use an experimentation canvas to build a logical flow for the machine learning algorithms. You also have access to a Jupyter notebook that is hosted on Azure Machine Learning and can work seamlessly with the experiments in the designer. 
