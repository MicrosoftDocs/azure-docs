...
When you open the "Sample 5: Train, Test, Evaluate for Binary Classification: Adult Dataset" sample it uses the sample Adult Census Income Binary Classification dataset. And the experiment in the canvas will looks similar to the image below.
...
Next you will set up the predictive experiment from which you will deploy your web service.
...
9.	In the **Data table name field**, type dbo.ScoredLabels. Note: If the table does not exist, it will be created when the experiment is run or the web service is called.
...
When you are done, your experiment should look similar to the image below.
...
3.	On the Deploy Experiment page, enter a name for you web service and select a pricing plan, then click **Delpoy**.
