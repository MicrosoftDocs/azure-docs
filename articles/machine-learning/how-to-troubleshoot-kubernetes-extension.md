...
This happens when an uninstallation operation is not finished and another installtion operation is triggered. You can run ```az k8s-extension show``` command to check the provisioning status of the extension and make sure the extension has been uninstalled before taking other actions.
...
This error means a confliction between existing cluster resources and AzureML extension. A full error message could be like this: 
```
...
* If the resource is also used by other components in your cluster and can't be modified. Please refer to [deploy AzureML extension](./how-to-deploy-kubernetes-extension.md#review-azureml-extension-configuration-settings) to see if there is a configuration setting to disable the conflict resource. 
...
|--|--|--|
|E40001 | LOAD_BALANCER_NOT_SUPPORT | Load balancer is not supported in your cluster. You need to configure load balancer in your cluster or consider to  set `inferenceRouterServiceType` to `nodePort` or `clusterIP`. |
|E40002 | INSUFFICIENT_NODE | You have enabled `inferenceRouterHA` that requires at least 3 nodes in your cluster. Please disable the HA if you have less than 3 nodes. |
...
|E40007 | INVALID_SSL_SETTING | The SSL key and certificate are not valid. The CNAME should be compatible with the certificate. Please check your SSL settings.|
|E45002 | PROMETHEUS_CONFLICT | The Prometheus Operator installed is conflict with your existing Prometheus Operator. Please refer to [Prometheus operator](#prometheus-operator) |
|E45003 | BAD_NETWORK_CONNECTIVITY | Please meet [network-requirements](./how-to-access-azureml-behind-firewall.md#kubernetes-compute).|
|E45004 | AZUREML_FE_ROLE_CONFLICT |AzureML extension is not supported in the [legacy AKS](./how-to-attach-kubernetes-anywhere.md#kubernetescompute-and-legacy-akscompute). To install AzureML extension, you need to [delete the legacy azureml-fe components](v1/how-to-create-attach-kubernetes.md#delete-azureml-fe-related-resources).|
|E45005 | AZUREML_FE_DEPLOYMENT_CONFLICT | AzureML extension is not supported in the [legacy AKS](./how-to-attach-kubernetes-anywhere.md#kubernetescompute-and-legacy-akscompute). To install AzureML extension, you need to [delete the legacy azureml-fe components](v1/how-to-create-attach-kubernetes.md#delete-azureml-fe-related-resources).|

## Open Souce components integration

AzureML extension leverages some open source components, including Prometheus Operator, Volcano Scheduler, and DCGM exporter. If the Kubernetes cluster already have some of them installed, you can read following sections to integrate your existing components with AzureML extension.

### Prometheus operator
[Promtheus operator](https://github.com/prometheus-operator/prometheus-operator) is an open source framework to help build metric monitoring system in kubernetes. AzureML extension also utilizes Prometheus operator to help monitor resource utilization of jobs.

If Prometheus operator has already been installed in cluster by other service, you can specify ```installPromOp=false``` to disable Prometheus operator in AzureML extension to avoid conflictions between two Prometheus operators.
In this case, all Prometheus instances will be managed by the existing promtheus operator. To make sure Prometheus works properly, the following things need to be paid attention to when you disable promtheus operator in Azureml extension.
...
### Volcano Scheduler
If your cluster already has the volcano suite installed, you can set `installVolcano=false`, so that the extension will not install the volcano scheduler. Volcano scheduler and volcano controller are required for training job submission and scheduling.
...
volcano-scheduler.conf: |
    ...
    - plugins:
        ...
        - name: binpack
```
Please use the same config settings as above, and disable `job/validate` webhook in the volcano admission, so that AzureML training workloads can perform properly.
