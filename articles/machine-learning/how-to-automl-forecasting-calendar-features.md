...
In an AutoML forecasting job, the given time index is decomposed into more granular level consisting of time information. Following is the example to illustrate these features.
For the given timestamp "2011-01-01 00:25:30" of parsable format ('YY-mm-dd %H-%m-%d'), AutoML job creates following features:
...
Please note that time driven features are only available for regular time series data that arrives predictably at predefined intervals. If the provided time index in time series data is irregular like different frequency intervals (for example, some observations recorded on daily basis, some on weekly basis) in the dataset, these features won’t get generated by AutoML job.
<br>
<br>
