...
| `compute` | string | Name of the compute target to execute the job on. This compucan be either a reference to an existing compute in the workspace (using the `azureml:<compute_name>` syntax) or `local` to designate local execution. <br> <br>  *Note:* jobs in pipeline don't support `local` as `compute`. * | | `local` |
