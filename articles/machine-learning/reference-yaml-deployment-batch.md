...
| `compute` | string | Name of the compute target to execute the batch scoring jobs on. This should be a reference to an existing compute in the workspace using the `azureml:<compute-name>` syntax. | | |
