...
Do consider migrating the code for deploying a workspace to v2. Typically Azure resources are managed via Azure Resource Manager (and Bicep) or similar resource provisioning tools. You can use the CLI (v2) and YAML files as well.
...
For prototyping, we recommend what is most comfortable for you. You may consider using the CLI for control plane (like submitting jobs iteratively), while your model training code is Python, or adopt a full-stack approach with Python only with the Azure ML SDK.

### Production model training

We firmly recommend v2 for production model training. Jobs consolidate the terminology and provide a set of consistency that allows for easier transition between types (for example, `command` to `sweep`) and a GitOps-friendly process for serializing jobs into YAML files.
