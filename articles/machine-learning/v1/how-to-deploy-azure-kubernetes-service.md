...
For accessing AzureML services behind a firewall, see [How to access azureml behind firewall](https://github.com/MicrosoftDocs/azure-docs/blob/main/articles/machine-learning/how-to-access-azureml-behind-firewall.md).
...
In the process of AKS create or attach, AzureML router (azureml-fe) is deployed into the AKS cluster. In order to deploy AzureML router, AKS node should be able to:
* Resolve DNS for AKS API server
* Resolve DNS for MCR in order to download docker images for AzureML router
...
The component that handles autoscaling for AzureML model deployments is azureml-fe, which is a smart request router. Since all inference requests go through it, it has the necessary data to automatically scale the deployed model(s).
