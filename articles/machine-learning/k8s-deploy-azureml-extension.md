...
description: Learn about what is AzureML extension, available configuration settings and different deployment scenarios, and verify and managed AzureML extension
...
You can use AzureML CLI command ```k8s-extension create``` to deploy AzureML extension. CLI ```k8s-extension create``` allows you to specify a set of configuration settings in ```key=value``` format using ```--config``` or ```--config-protected``` parameter. Following is the list of available coniguration settings to be specified during AzureML extension deployment.
...
As you can see from above configuration settings table, the combinations of different conigruation settings allow you to deploy AzureML extension for different ML workload scenarios:
  ...
  * For all kinds of ML workload, speciy both ```enableTraining=True``` and ```enableInference=True```

If you plan to deploy AzureML extension for real-time inference workload and want to specify ```enableInference=True```, please pay special attention to following additional configuration settings related to real-time inference workload:
...
## Review AzureML extension compoments

Upon AzureML extension deployment completes, you can use ```kubectl get deployments -n azureml``` to see list of resources created in the cluster. It usually consists a subset of following resources per coniguration settings specified. 
