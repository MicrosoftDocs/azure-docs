...
| `outputs.<output_name>` | object | You can leave the object empty, in which case by default the output will be of type `uri_folder` and Azure ML will system-generate an output location for the output. File(s) to the output directory will be written via read-write mount. If you want to specify a different type or mode for the output, provide an object containing the [job output specification](#job-outputs). | |
| `component` | string or object | In advanced scenarios where you already have a component defined that contains the template for the job you want to run, you can specify that component to this field rather than provide all the inline fields individually. If you specify the `component` field, the only other fields you will typically need to specify are `compute` for the compute target to execute the job on, the `inputs` and `outputs` of the job (the input and output keys should correspond to those of the component's inputs and outputs, and the values should correpond to the runtime job input / output values), the `limits` field if customizing the job timeout, and the `experiment_name` field for the experiment to organize the job under. <br><br> To reference an existing registered component, use the `azureml:<component-name>:<component-version>` syntax or `azureml:<component-name>@latest` (to reference the latest version of a component). <br><br> You can also define a component inline or in a separate YAML file, by following the [command component schema](reference-yaml-component-command.md#yaml-syntax). Exclude the `name` and `version` properties as they are not applicable for inline component specifications.| | |
...
| Key | Type | Description | Allowed values | Default value |
| --- | ---- | ----------- | -------------- | ------------- |
| `type` | string | The type of job output. Specify `uri_folder` for an output corresponding to a folder, or `uri_file` for an output corresponding to a single file. | `uri_folder`, `uri_file` | `uri_folder` |
