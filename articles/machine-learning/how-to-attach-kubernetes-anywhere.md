...
AzureML extension consists a set of system componenents deployed to your Kubernetes cluster so you can enable cluster to run AzureML workload - model training jobs or mdoel endpoints. You can use simple Azure CLI command ```k8s-extension create``` to deploy AzureML extension.
...
Azure Relay resource are created under the same Resource Group as the Arc cluster.
