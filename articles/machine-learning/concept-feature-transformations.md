---
title: Feature transfrormations
...
- `source_lookback`: this property is used when the feature is doing aggregation on time-series (e.g. window aggregation). The value of this property indicates the required time range of source data in the past for feature value at time T. Refer the [Best Practice](#Set-proper-source_lookback) section for more details.
...
Sliding window aggregation is often used when feature value presents statistics (e.g. sum, average) accumulates over time. One common way to do that is to use SparkSQL `Window` functions, which defines a sliding window that is around each row in the data. 
 
Window function can look into both future and past of each row. In the context of machine learning features, we recommend to define `Window` to only look back in time from each row. Refer the [Best Practice](#Prevent-data-leakage-in-feature-transformation) section for more details.
...
Another common way to aggregate data on time-series is to use tumbling window. The data is grouped into fix-size, non-overlapping and continuous time windows, and then aggregated. For example, users can define daily or hourly features.

We recommend to use `pyspark.sql.functions.window` functions to define tumbling window with consistent results. And the "timestamp" of the output feature should align to the end of each tumbling window.
...
And we define a new featureset, `user_sliding_24hr_spend`:
...
Here is a sample code snippet that provides intuition on how feature store API's computes the features:
...
For each calculated feature value, if its timestamp value is `ts_0`, it should be calculated using data in `source` that have timestamp on or before `ts_0` only. This avoids feature calculation using data from "the future of the feature event time", aka, data leakage.
...
`source_lookback` should be defined to a time delta value which presents the range of source data needed for a feature value of a given timestamp.
...
| sliding window | size of the largest window range in the transformer.<br> e.g.<br> source_lookback = 3days when the featureset defines 3 day rolling features <br> source_lookback = 7 days when the featureset defines both 3 day and 7 day rolling features |
...
When `source_lookback` is not set correctly, it can cause:
- Incorrectness in the calculated/materialized feature values
