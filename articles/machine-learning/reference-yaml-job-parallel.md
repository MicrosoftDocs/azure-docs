...
| `partition_keys` | list | The keys used to partition dataset into mini-batches.<br><br>If specified, the data with the same key will be partitioned into the same mini-batch. If both `partitions_key` and `mini_batch_size` are specified, the partition keys will take effect. |||
