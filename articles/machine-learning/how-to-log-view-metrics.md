...
This folder contains information about the user generated logs. This folder is open by default, and the **std_log.txt** log is selected. The **std_log.txt**  contains `stdout` log and `stderr` logs from your control script and training script, one per process. Standard output from your script. **std_log.txt** is where your code's logs (for example, print statements) show up. In the majority of cases, you will monitor the logs here.
