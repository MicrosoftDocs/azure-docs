...
* When creating a compute instance or compute cluster to the workspace, the compute instance and compute cluster must be in the same Azure region as the workspace.
* When creating or attaching an Azure Kubernetes Service cluster to the workspace, the cluster must be in the same region as the workspace.
