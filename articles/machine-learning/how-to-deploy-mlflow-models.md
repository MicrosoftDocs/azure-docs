...
In this article, learn about what is needed for deployment of [MLflow](https://www.mlflow.org) models to Azure Machine Learning for both real-time and batch inference. Learn also about the different tools you can use to manage the deployment.
...
MLflow models can include a signature that indicates the expected inputs and their types. For such models, whether they're deployed to online endpoints or batch endpoints, Azure Machine Learning enforces that the number of data inputs and their types comply with the signature. If the data can't be parsed as expected, the model invocation will fail.
...
Azure Machine Learning supports deploying models to both online and batch endpoints. Online endpoints are similar to the [MLflow built-in server](https://www.mlflow.org/docs/latest/models.html#built-in-deployment-tools) in that they provide a scalable, synchronous, and lightweight way to run models for inference. 

On the other hand, batch endpoints provide a way to run asynchronous inference over long-running inferencing processes that can scale to large amounts of data. The MLflow server currently lacks this capability that batch endpoints have, although a similar capability can be achieved by [using Spark jobs](how-to-deploy-mlflow-model-spark-jobs.md). To learn more about batch endpoints and MLflow models, see [Use MLflow models in batch deployments](how-to-mlflow-batch.md).
...
Regardless of the input type used, Azure Machine Learning requires you to provide inputs in a JSON payload, within the dictionary key `input_data`. Because this key isn't required when using the command `mlflow models serve` to serve models, payloads can't be used interchangeably.
...
> [!IMPORTANT]
> When you choose to specify a scoring script for an MLflow model deployment, you also need to provide an environment for it.
...
- Use the MLflow SDK if these conditions apply:

    1. You're familiar with MLflow, or you're using a platform that supports MLflow natively (like Azure Databricks).
    1. You wish to continue using the same set of methods from MLflow.

- Use the [Azure Machine Learning CLI v2](concept-v2.md) if any of these conditions apply:
