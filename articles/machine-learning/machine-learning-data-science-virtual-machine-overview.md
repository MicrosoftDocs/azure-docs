...
The DSVM provides a baseline configuration for data science teams looking to replace their local desktops with a managed cloud desktop. This ensures that all the data scientists on a team have a consistent setup with which to verify experiments, promote collaboration, and lower the total cost through reduced sysadmin burden and time saved to evaluate, install and maintain various software packages needed to do advanced analytics.  

### Data Science Training and Education 

Enterprise trainers and educators teaching data science classes usually provide a virtual machine image to ensure that their students have a consistent setup and the samples work predictably. The DSVM creates an on-demand environment with a consistent setup which eases the support and incompatibility challenges. Cases where these environments are built frequently, especially with shorter training classes will benefit substantially. 
