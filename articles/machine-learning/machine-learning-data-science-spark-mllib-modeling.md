...
Spark on Azure HDInsight is able to manipulate data in-memory while processing it on an HDInsight (Hadoop) cluster in a distributed way. This Spark cluster thus combines speed with capacity. It also includes support web-based Zeppelin and Jupyter notebooks to run Spark SQL interactive queries on the Spark cluster for transforming, filtering and visualizing data stored in Azure Blobs (WASB).
