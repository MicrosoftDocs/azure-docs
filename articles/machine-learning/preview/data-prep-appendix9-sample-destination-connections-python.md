...
Before writing to Excel, some other changes might be needed. Some of the datatypes that are used in data preparation are not supported in some destination formats. For example, if "Error" objects exist, these won't serialize correctly to Excel. Thus, before attempting to write to Excel, you need a "Replace Error Values" transform, which removes errors from any columns.

Assuming all of the previous work has been completed, the following line writes the data table to a single sheet in an Excel document. Add a Write DataFlow (Script) transform. Then enter the following code in an expression section:
