...
description: Learn how to secure a web service deployed through the Azure Machine Learning service by enabling HTTPS. HTTPS secures the data submitted by clients using transport layer security (TLS), a replacement for secure socket layers (SSL). It's also used by clients to verify the identity of the web service.
...
# Use SSL to secure web services with the Azure Machine Learning service
...
TLS and SSL both rely on *digital certificates*, which help with encryption and identity verification. For more information on how digital certificates work, see the Wikipedia entry on [public key infrastructure (PKI)](https://en.wikipedia.org/wiki/Public_key_infrastructure).

> [!Warning]
> If you don't enable and use HTTPS for your web service, data that's sent to and from the service might be visible to others on the internet.
>
...
> [!IMPORTANT]
> If you're deploying to Azure Kubernetes Service (AKS), you can provide your own certificate or use a certificate that's provided by Microsoft. If you use a Microsoft-provided certificate, you don't need to get a domain name or SSL certificate. For more information, see the [Enable SSL and deploy](#enable) section.
...
+ **Deploy on Azure Kubernetes Service AKS and FPGA**
  ...
  * When you use *a certificate that you purchased*, use the *ssl_cert_pem_file*, *ssl_key_pem_file*, and *ssl_cname* parameters. The following example demonstrates how to use *.pem* files to create configurations that use an SSL certificate that you provide:
...
+ **Deploy on Azure Container Instances (ACI)**
  ...
  When you deploy to ACI, provide values for SSL-related parameters, as the following code snippet shows:
...
+ **For AKS:**
  ...
  > [!WARNING]
  > If you used the *leaf_domain_label* to create the service with a certificate from Microsoft, don't manually update the DNS value for the cluster. The value should be set automatically.

  Update the DNS on the **Configuration** tab of the Public IP Address of the AKS cluster as shown in the following image. The Public IP Address is one of the resource types that's created under the resource group that contains the AKS agent nodes and other networking resources.
