...
description: Learn how to secure a  that's deployed through Azure Machine Learning by enabling HTTPS. HTTPS secures data from by clients by using transport layer security (TLS), a replacement for secure socket layers (SSL). Clients also use HTTPS to verify the identity of the .
