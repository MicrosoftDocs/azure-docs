...
+ Data scientists need the ability to explain their models to executives and compliance officers, so they can understand the value and accuracy of their findings 
+ Business decision makers need peace-of-mind that predictions are being made in compliance and the ability to provide transparency for end users to gain and maintain their trust

Enabling the capability of explaining a machine learning model is important:
+ During the training phase of the development cycle: As model designers and evaluators can use interpretability output of a model to verify hypotheses and build trust with stakeholders.  They also use the insights into the model for debugging, validating model behavior matches their objectives, and to check for bias.
+ During the inferencing phase: As it empowers the providers of AI solutions to the end users to understand “when deployed” how the model is working and how its decisions are treating and impacting people in real life. 
...
This package uses the explainers developed in [Interpret-Community](https://github.com/interpretml/interpret-community/), an open source python package for training interpretable models and helping to explain blackbox systems. It currently supports the following explainers:
