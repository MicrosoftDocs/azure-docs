...
This article provides detailed information on **advanced security settings**, and is not necessary for basic or experimental use-cases. Sections in this article provide configuration information for different scenarios, but do not need to be completed in order or in entirety.
...
The Key Vault instance associated with the workspace is used by Azure Machine Learning service to store credentials of various kinds:
* The associated storage account connection string
* Passwords to Azure Container Repository instances
* Connection Strings to data stores.

To use Azure Machine Learning experimentation capabilities with Key Vault behind a virtual network, use following the steps:
...
To use Azure Machine Learning compute in a virtual network, consider the following information about network requirements:

- The virtual network must be in the same subscription and region as the Azure Machine Learning service workspace.

- The subnet specified for the compute cluster must have enough unassigned IP addresses to accommodate the number of VMs targeted for the cluster. If the subnet doesn't have enough unassigned IP addresses, the cluster will be partially allocated.

- If you plan to secure the virtual network by restricting traffic, leave some ports open for the compute service. For more information, see [Required ports](#mlcports).
...
- If you are going to put multiple compute clusters in one virtual network, you may need to request a quota increase for one or more of your resources.

    Azure Machine Learning compute automatically allocates additional networking resources in the resource group that contains the virtual network. For each compute cluster, the service allocates the following resources:
...
Exercise caution if you modify or add inbound/outbound rules in Batch-configured NSGs. If an NSG blocks communication to the compute nodes, then compute service sets the state of the compute nodes to unusable.

You don't need to specify NSGs at the subnet level because Azure Batch service configures its own NSGs. However, if the specified subnet has associated NSGs and/or a firewall, configure the inbound and outbound security rules as mentioned earlier.
...
* A user-defined route must be established for each IP address used by the Azure Batch service in the region where your resources exist. These UDRs enable the Batch service to communicate with compute nodes for task scheduling. To get a list of the IP addresses of the Batch service, contact Azure Support.
...
To create an Azure Machine Learning compute cluster using the Azure portal, use the following steps:
...
1. To allow the Azure Machine Learning service to communicate with the SSH port on the VM or cluster, you must configure a source entry for the network security group. The SSH port is usually port 22. Use the following steps to allow traffic from this source:
...
To add Azure Kubernetes Service in a virtual network to your workspace, follow the following steps in the Azure portal:

1. Make sure network security group (NSG) that controls the virtual network has an inbound rule enabled for Azure Machine Learning service using __AzureMachineLearning__ as the **SOURCE**.
...
> [!IMPORTANT]
> Check the prerequisites and plan IP addressing for your cluster before proceeding with the above steps. For more information, see [Configure advanced networking in Azure Kubernetes Service](https://docs.microsoft.com/azure/aks/configure-advanced-networking).
>
>
