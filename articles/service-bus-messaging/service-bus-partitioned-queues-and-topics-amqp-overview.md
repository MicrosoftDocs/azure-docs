...
description: Learn about using the Advanced Message Queuing Protocol (AMQP) 1.0 with Azure Service Bus partitioned queues and topics.
...
**Partitioned queues and topics**, also known as *partitioned entities*, offer higher availability, reliability, and throughput than conventional non-partitioned queues and topics. For more information about partitioned entities, see [Partitioned queues and topics](service-bus-partitioning.md).
...
You can create a partitioned queue using the [Azure portal][Azure portal] (check **Enable partitioning** in the **Create queue** blade) or programmatically using the Service Bus APIs. To create a partitioned queue, set the [EnablePartitioning](https://docs.microsoft.com/dotnet/api/microsoft.servicebus.messaging.queuedescription#Microsoft_ServiceBus_Messaging_QueueDescription_EnablePartitioning) property to **true** in the [QueueDescription](https://docs.microsoft.com/dotnet/api/microsoft.servicebus.messaging.queuedescription) instance. The following code shows how to create a partitioned queue using the Service Bus SDK. 
...
You can send messages to, and receive messages from a partitioned queue using AMQP as protocol, by setting the [TransportType](https://docs.microsoft.com/dotnet/api/microsoft.servicebus.servicebusconnectionstringbuilder#Microsoft_ServiceBus_ServiceBusConnectionStringBuilder_TransportType) property to [TransportType.Amqp](https://docs.microsoft.com/dotnet/api/microsoft.servicebus.messaging.transporttype#fields_) as shown in the following code.  
...
You can create a partitioned topic using the [Azure portal][Azure portal] (check **Enable partitioning** in the **Create topic** blade) or programmatically using the Service Bus APIs. To create a partitioned topic, set the [EnablePartitioning](https://docs.microsoft.com/dotnet/api/microsoft.servicebus.messaging.topicdescription#Microsoft_ServiceBus_Messaging_TopicDescription_EnablePartitioning) property to **true** in the [TopicDescription](https://docs.microsoft.com/dotnet/api/microsoft.servicebus.messaging.topicdescription) instance. The following code shows how to create a partitioned topic using the Service Bus SDK.
...
### Send and receive messages using AMQP
You can send messages to, and receive messages from a partitioned topic subscription using AMQP as a protocol, by setting the [TransportType](https://docs.microsoft.com/dotnet/api/microsoft.servicebus.servicebusconnectionstringbuilder#Microsoft_ServiceBus_ServiceBusConnectionStringBuilder_TransportType) property to [TransportType.Amqp](https://docs.microsoft.com/dotnet/api/microsoft.servicebus.messaging.transporttype#fields_), as shown in the following code.  
