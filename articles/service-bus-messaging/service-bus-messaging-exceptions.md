---
title: "[Resolved] Messaging Errors"
description: Find error codes and resolution guidance for Service Bus messaging exceptions from deprecated .NET packages. Learn causes and fixes for each exception type.
ms.topic: error-reference
ms.date: 06/12/2026
ms.custom: sfi-ropc-nochange
ai-usage: ai-assisted

#customer intent: As a developer, I want to look up Service Bus messaging exceptions so that I can understand the cause and resolve the issue.
---

# [Resolved] Error codes: Service Bus messaging exceptions (deprecated)

This article lists the .NET exceptions generated by the deprecated .NET Framework APIs for Azure Service Bus. For each exception, find the cause and suggested resolution.

[!INCLUDE [service-bus-track-0-and-1-sdk-support-retirement](../../includes/service-bus-track-0-and-1-sdk-support-retirement.md)]

## Service Bus exception categories
...
1. Setup/configuration error ([Microsoft.ServiceBus.Messaging.MessagingEntityNotFoundException](/dotnet/api/microsoft.azure.servicebus.messagingentitynotfoundexception), [System.UnauthorizedAccessException](/dotnet/api/system.unauthorizedaccessexception)). General action: review your configuration and change if necessary.
1. Transient exceptions ([Microsoft.ServiceBus.Messaging.MessagingException](/dotnet/api/microsoft.servicebus.messaging.messagingexception), [Microsoft.ServiceBus.Messaging.ServerBusyException](/dotnet/api/microsoft.azure.servicebus.serverbusyexception), [Microsoft.ServiceBus.Messaging.MessagingCommunicationException](/dotnet/api/microsoft.servicebus.messaging.messagingcommunicationexception)). General action: retry the operation or notify users. The `RetryPolicy` class in the client SDK can be configured to handle retries automatically. For more information, see [Retry guidance](/azure/architecture/best-practices/retry-service-specific#service-bus).
1. Other exceptions ([System.Transactions.TransactionException](/dotnet/api/system.transactions.transactionexception), [System.TimeoutException](/dotnet/api/system.timeoutexception), [Microsoft.ServiceBus.Messaging.MessageLockLostException](/dotnet/api/microsoft.azure.servicebus.messagelocklostexception), [Microsoft.ServiceBus.Messaging.SessionLockLostException](/dotnet/api/microsoft.azure.servicebus.sessionlocklostexception)). General action: specific to the exception type; refer to the table in the following section:
...
## Service Bus exception types and resolution
...
A [MessageLockLostException](/dotnet/api/microsoft.azure.servicebus.messagelocklostexception) indicates that the lock held by the client on a message expired on the service side.
...
A [SessionLockLostException](/dotnet/api/microsoft.azure.servicebus.sessionlocklostexception) indicates that the lock held by the client on a session expired on the service side.
...
A **SocketException** indicates a network connectivity issue between the client and the Service Bus service.
...
If name resolution **works as expected**, check if connections to Azure Service Bus are allowed in the [connectivity troubleshooting guide](service-bus-troubleshooting-guide.md#connectivity-certificate-or-timeout-issues).

## MessagingException

A [MessagingException](/dotnet/api/microsoft.servicebus.messaging.messagingexception) is a generic messaging exception that can be thrown for various reasons.
...
A **StorageQuotaExceededException** indicates that the total size of entities in a premium namespace exceeded its storage limit.
