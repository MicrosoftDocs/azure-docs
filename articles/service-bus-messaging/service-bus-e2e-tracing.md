...
One part of this problem is tracking logical pieces of work. It includes request or message processing result and latency, external dependency calls; another part is correlation of these diagnostics events (telemetry) beyond process boundaries.
...
|  Correlation-Context | Operation context, which is propagated across all services involved in operation processing. For more insformation, see [Correlation-Context in HTTP protocol](https://github.com/dotnet/corefx/blob/master/src/System.Diagnostics.DiagnosticSource/src/HttpCorrelationProtocol.md#correlation-context) |

# Service Bus .NET Client auto-tracing

Starting with version 3.0.0 [Microsoft Azure ServiceBus Client for .NET](/dotnet/api/microsoft.azure.servicebus.queueclient) provides tracing instrumentation points that can be hoocked by tracing systems, or piece of client code.
...
In case you make calls to external components during message processing, they are also correlated (as long as they are automatically tracked by Application Insights SDK). Refer to [Track custom operations with Application Insights .NET SDK](../application-insights/application-insights-custom-operations-tracking.md) for manual tracking and correlation.
...
In this example, listener logs operation duration, result, unique identifier and start time for each Service Bus operation.

### Events

For every operation, two events are sent: 'Start' and 'Stop'. 
Most probably, you are only interested in 'Stop' events. They provide the result of operation and (thanks to the `Activity`) start time (that is 'Start' event timestamp) and duration (that is duration between 'Start' and 'Stop').
...
|----------------|-------------|---------|
| Microsoft.Azure.ServiceBus.Send | `ISenderClient.SendAsync` (that is `MessageSender`, `QueueClient`, `TopicClient`) | `IList<Message> Messages` - List of messages being sent |
...
In some cases, it's desirable to log only part of the events. It could 'Stop' events as we've done in above example or sampling percentage of the events to reduce performance overhead or storage consumption. 
`DiagnosticSource` provide way to achieve it with `IsEnabled` predicate. Refer to the [Context-Based Filtering in DiagnosticSource](https://github.com/dotnet/corefx/blob/master/src/System.Diagnostics.DiagnosticSource/src/DiagnosticSourceUsersGuide.md#context-based-filtering) for more details.
...
1. `IsEnabled(<OperationName>, string entity, null)` for example, `IsEnabled("Microsoft.Azure.ServiceBus.Send", "MyQueue1")`. Note there is no 'Start' or 'Stop' at the end. Use it to filter out particular operations or queues. If callback returns `false`, events for the operation will not be sent
...
2. `IsEnabled(<OperationName>.Start)` for example, `IsEnabled("Microsoft.Azure.ServiceBus.Send.Start")`. Check whether 'Start' event should be fired. It only affects 'Start' event and do not affect further instrumentation.
