...
When you create a premium tier namespace through the portal, the support for availability zones (if available in the selected region) is automatically enabled for the namespace. When creating a premium tier namespace in through other mechanisms, such as [ARM / Bicep templates](service-bus-resource-manager-overview.md), [CLI](/cli/azure/servicebus?view=azure-cli-latest), or [PowerShell](/powershell/module/az.servicebus), the property `zoneRedundant` needs to be explicitly set to `true` to enable availability zones (if available in the selected region). There's no additional cost for using this feature and you can't disable or enable this feature after namespace creation.
