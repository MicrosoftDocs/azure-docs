...
To launch the solution, we need to run both the Sender and the Receiver. If you're using Visual Studio Code, launch the "Debug All" configuration. If you're using Visual Studio, configure the solution to launch both the Sender and Receiver projects:
...
With our less reliable `PingHandler`, all of our messages fail. We can see the retry policy kicking in for those messages. The first time a message fails, it's immediately retried up to three times:
