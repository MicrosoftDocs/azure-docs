...
NServiceBus is a messaging framework built on top of Service Bus that helps developers focus on business logic by abstracting infrastructure concerns. In this guide, we'll build a solution which exchanges messages between two services. We'll also show how to automatically retry failing messages and review options for hosting these services in Azure.
...
The sample assumes you have [created a Service Bus namespace](https://docs.microsoft.com/en-us/azure/service-bus-messaging/service-bus-create-namespace-portal).

Note that NServiceBus requires at least the Standard tier. The Basic tier will not work.
...
![Sequence Diagram](./media/nservicebus/sequence-diagram.png)
...
The Shared class library is where we define the contracts used to send our messages. It includes a reference to the `NServiceBus` NuGet package which contains interfaces we can use to identify our messages. The interfaces are not required, but they give us some extra validation from NServiceBus and allow the code to be self-documenting.
...
The `Ping` class defines a message that the Sender sends to the Receiver. It is a simple C# class that implements `NServiceBus.ICommand`, an interface from the NServiceBus package. This is a signal to the reader and to NServiceBus that this is a command, although there are other ways to identify messages [without using interfaces](https://docs.particular.net/nservicebus/messaging/conventions).
...
`Pong` is also a simple C# object though this one implements `NServiceBus.IMessage`. The `IMessage` interface represents a generic message that is neither a command nor an event, and is commonly used for replies. In our sample, it is a reply that the Receiver sends back to the Sender to indicate that a message was received.

These are the two message types we will use. The next step is to configure the Sender to use Service Bus and to send a `Ping` message.

## Set up the sender

The Sender is an endpoint that sends our `Ping` message. Here, we configure the Sender to use Service Bus as the transport mechanism, then construct a `Ping` instance and send it.

In the `Main` method of `Program.cs`, we configure the Sender endpoint:
...
Hosting and logging are configured using standard [Microsoft Generic Host options](https://docs.microsoft.com/en-us/dotnet/core/extensions/generic-host). For now, the endpoint is configured to run as a console application but it can be modified to run in Azure Functions with minimal changes, which we'll discuss later in this article.

### Configure the NServiceBus endpoint

Next, we tell the host to use NServiceBus with the `.UseNServiceBus(…)` extension method. The method takes a callback function that returns an endpoint that will be started when the host runs.

In the endpoint configuration, we specify `AzureServiceBus` for our transport, providing a connection string from `appsettings.json`. Next, we set up the routing so that messages of type `Ping` are sent to an endpoint named "Receiver". This allows NServiceBus to automate the process of dispatching the message to the destination without requiring the receiver's address.
...
The `IMessageSession` used in `ExecuteAsync` is injected into `SenderWorker` and allows us to send messages using NServiceBus outside of a message handler. The routing we configured in `Sender` specifies the destination of the `Ping` messages. This keeps the topology of the system (which messages are routed to which addresses) as a separate concern from the business code.

The Sender application also contains a `PongHandler`. We'll get back to this after we've discussed the Receiver, which we'll do next.
...
Like the sender, we set up the receiver as a console application using the Microsoft Generic Host. It uses the same logging and endpoint configuration (with Service Bus as the message transport) but with a different name, to distinguish it from the sender:

```csharp
var endpointConfiguration = new EndpointConfiguration("Receiver");
```

Since this endpoint only replies to its originator and doesn't initiate new conversations, no routing configuration is required. It also doesn't need a background worker like the Sender does, since it only replies when it receives a message.
...
Our `PingHandler` is straightforward: when a `Ping` message is received, we log the message details and reply back to the sender with a new `Pong` message.

> [!NOTE]
> In the Sender's configuration, we specified that `Ping` messages should be routed to the Receiver. NServiceBus adds metadata to the messages indicating, among other things, the origin of the message. This is why we don't need to specify any routing data for the `Pong` reply message; it is automatically routed back to its origin: the Sender.
>

With the Sender and Receiver both properly configured, we can now run the solution.

## Run the solution

To launch the solution, we need to run both the Sender and the Receiver. If you are using Visual Studio Code, launch the "Debug All" configuration. If you are using Visual Studio, configure the solution to launch both the Sender and Receiver projects:
...
![Endpoint sending Ping messages to the Receiver](./media/nservicebus/sender.png)

# [Receiver](#tab/Receiver)

![Endpoint receiving Ping messages from the Sender](./media/nservicebus/receiver.png)
...
Now that we have everything working, let's break it.

## Resilience in action

Errors are a fact of life in software systems. It is inevitable that code will fail and it can do so for a variety of reasons, such as network failures, database locks, changes in a third-party API, and plain old coding errors.
...
Before we discuss how this policy works, let's see it in action. Before we test the recoverability policy we need to simulate an error. Open the `PingHandler` code in the Receiver project and uncomment this line:
...
With our less reliable `PingHandler`, all of our messages fail. We can see the retry policy kicking in for those messages. The first time a message fails, it is immediately retried up to 3 times:

![Immediate retry policy that retries messages up to 3 times](./media/nservicebus/immediate-retries.png)

Of course, it will continue to fail so when the 3 immediate retries are used up, the delayed retry policy kicks in and the message is delayed for 5 seconds:

![Delayed retry policy that delays the messages in increments of 5 seconds before attempting another round of immediate retries](./media/nservicebus/delayed-retries.png)

 After those 5 seconds have passed, the message is retried again another 3 times (i.e. another iteration of the immediate retry policy). These will also fail and NServiceBus will delay the message again, this time for 10 seconds, before trying again.

If, after running through the full retry policy, `PingHandler` still doesn't succeed, the message is placed in a _centralized_ error queue, named `error`, as defined by the call to `SendFailedMessagesTo`. 

![Failed message](./media/nservicebus/failed-message.png)
...
Once a message is in the error queue, you can examine the message details in the tool of your choice, then decide what to do with it. For example, using [ServicePulse](https://particular.net/servicepulse), we can view the message details and the reason for the failure:

![ServicePulse](./media/nservicebus/servicepulse.png)

After examining the details, we can send the message back to its original queue for processing. We can also edit the message before doing so. If there are multiple messages in the error queue which failed for the same reason, they can all be sent back to their original destinations as a batch.

Next, it's time to figure out where to deploy our solution in Azure.

## Where to host the services in Azure

In this sample, the Sender and Receiver endpoints are configured to run as console applications. They can also be hosted in various Azure services including Azure Functions, Azure App Services, Azure Container Instances, Azure Kubernetes Services, and Azure VMs. For example, this is how the Sender endpoint can be configured to run as an Azure Function:
