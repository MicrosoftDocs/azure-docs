---
title: SQLRuleAction syntax reference | Microsoft Docs
...
A *SqlRuleAction* represents set of actions written in SQL-language based syntax that is performed against a [BrokeredMessage](assetId:///T:Microsoft.ServiceBus.Messaging.BrokeredMessage?qualifyHint=False&autoUpgrade=True).   
...
- SET is used to create a new property or update value of existing property.
- REMOVE is used to remove a property.
- SET performs implicit conversion if possible when the expression's type and the existing property's type is different.
- Action fails if non-existing system properties were referenced.
- Action doesn't fail if non-existing user properties were referenced.
- Non-existing user property is evaluated as "Unknown" internally follows the same semantic as SqlFilter when evaluating operators.
