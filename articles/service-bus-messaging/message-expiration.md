...
> [!IMPORTANT]
> Setting the lock level to [`CanNotDelete`](../azure-resource-manager/management/lock-resources.md) on the namespace or at a higher level doesn't prevent entities with `AutoDeleteOnIdle` from being deleted. If you don't want the entity to be deleted, set the `AutoDeleteOnIdle` property to `DataTime.MaxValue`. 
