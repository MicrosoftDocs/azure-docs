...
- Verify that `DefaultAzureCredential` selects the intended credential. When multiple credentials are available in the host, it uses the first one in its resolution order. Enable Azure Identity logging or specify the credential explicitly to confirm which identity is used.
- Allow time for a new role assignment to propagate. It can take several minutes to take effect.
- Check that the host clock is accurate. Microsoft Entra tokens are validated against their issue and expiry times, so keeping the host clock in sync helps tokens be accepted.

> [!NOTE]
> The Azure Service Bus data roles (Data Sender, Data Receiver, Data Owner) control data-plane access. They're separate from management-plane roles such as Owner and Contributor, which manage the resource rather than its data.

#### Azure Kubernetes Service (AKS) workload identity
When you run on AKS with workload identity, the general checks above still apply. In addition, verify the workload identity configuration:
...
- A federated identity credential links the managed identity or Microsoft Entra application to the cluster's OIDC issuer.
...
> [!IMPORTANT]
> Resolve certificate chain failures by updating the trust store with the correct CA certificates. Keep TLS certificate validation enabled: bypassing it (for example, by setting `NODE_TLS_REJECT_UNAUTHORIZED=0` in Node.js or installing a certificate validation callback that accepts any certificate) removes protection against man-in-the-middle attacks.
