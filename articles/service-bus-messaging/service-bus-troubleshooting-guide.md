...
If either check shows an active service-side event, the service is the likely source, and mitigation is typically already underway. The Service Bus SDK's built-in [retry policy](/azure/architecture/best-practices/retry-service-specific#service-bus) reconnects your application automatically once the service recovers. If both checks show the service is healthy, continue with the client-side troubleshooting in the rest of this article.
