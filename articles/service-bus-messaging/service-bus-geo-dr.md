---
title: Azure Service Bus Geo-Disaster Recovery Guide
description: Discover how to configure Azure Service Bus Geo-Disaster Recovery for metadata replication and ensure high availability during outages or disasters.
#customer intent: As a cloud architect, I want to understand how to set up Azure Service Bus Geo-Disaster Recovery so that I can ensure high availability for my applications.
...
You can use the Service Bus Geo-Disaster Recovery feature to [insulate Azure Service Bus applications against outages and disasters](service-bus-outages-disasters.md). It helps preserve the integrity of the composite application configuration. 

> [!NOTE]
> This feature is available for the Premium tier of Azure Service Bus.

The Geo-Disaster Recovery feature continuously replicates the entire configuration of a namespace (entities, configuration, properties) from a primary namespace to a secondary paired namespace. You can initiate a one-time failover move from the primary to the secondary at any time. The failover move repoints the chosen alias name for the namespace to the secondary namespace and then breaks the pairing. The failover is nearly instantaneous once initiated. 
...
- This feature enables instant continuity of operations with the same configuration, but **it doesn't replicate the messages held in queues or topic subscriptions, or dead-letter queues**. To preserve queue semantics, such a replication requires not only the replication of message data but also every state change in the broker. This replication is offered in the [Geo-Replication feature](service-bus-geo-replication.md).
...
- Pairing a [partitioned namespace](enable-partitions-premium.md) with a nonpartitioned namespace isn't supported.
- If you enable `AutoDeleteOnIdle` for an entity, the entity might not be present in the secondary namespace when the failover occurs. When the secondary becomes primary, the last access status, which isn't part of the metadata, isn't available to the new primary and the entity might be deleted as part of `AutoDeleteOnIdle` cleanup.

> [!TIP]
> To replicate the contents of queues and topic subscriptions and operate corresponding namespaces in active/active configurations to cope with outages and disasters, don't lean on this Geo-Disaster Recovery feature set. Instead, use the [Geo-Replication feature](service-bus-geo-replication.md) or follow the [replication guidance](service-bus-federation-overview.md).  

## Basic concepts and terms

The Geo-Disaster Recovery feature implements metadata disaster recovery and relies on primary and secondary disaster recovery namespaces. The Geo-Disaster Recovery feature is available for the [Premium tier](service-bus-premium-messaging.md) only. You don't need to make any connection string changes, as the connection is made through an alias.

The following terms are used in this article:

-  *Alias*: The name for a disaster recovery configuration that you set up. The alias provides a single stable Fully Qualified Domain Name (FQDN) connection string. Applications use this alias connection string to connect to a namespace. Using an alias, the connection string stays the same when the failover starts.
...
The following section provides an overview of how to set up pairing between the namespaces.

:::image type="content" source="./media/service-bus-geo-dr/geo-dr-namespace-pairing.png" alt-text="Screenshot of screen on setting up pairing with Geo-Disaster Recovery." lightbox="./media/service-bus-geo-dr/geo-dr-namespace-pairing.png":::

First, create, or use an existing primary namespace and create a new secondary namespace. Then, pair the two namespaces. This pairing gives you an alias that you can use to connect. Because you use an alias, you don't have to change connection strings. You can only add new namespaces to your failover pairing. 
...
1. In the Azure portal, go to your primary namespace.
1. Select **Geo-Recovery** in the left menu, and select **Initiate pairing** in the toolbar. 

    :::image type="content" source="./media/service-bus-geo-dr/primary-namspace-initiate-pairing-button.png" alt-text="Screenshot showing the Geo-Recovery page with Initiate pairing link selected." lightbox="./media/service-bus-geo-dr/primary-namspace-initiate-pairing-button.png":::
1. On **Initiate pairing**, follow these steps:
    ...
    1. Select **Create**. 
        ...
        :::image type="content" source="./media/service-bus-geo-dr/initiate-pairing-page.png" alt-text="Screenshot showing the Initiate Pairing page in the Azure portal." lightbox="./media/service-bus-geo-dr/initiate-pairing-page.png":::
1. You should see the **Service Bus Geo-DR Alias** page as shown in the following image. You can also navigate to the **Geo-DR Alias** page from the primary namespace page by selecting the **Geo-Recovery** in the left menu. 

    :::image type="content" source="./media/service-bus-geo-dr/service-bus-geo-dr-alias-page.png" alt-text="Screenshot showing the Service Bus Geo-DR Alias page with primary and secondary namespaces." lightbox="./media/service-bus-geo-dr/service-bus-geo-dr-alias-page.png":::
1. On the **Geo-DR Alias** page, select **Shared access policies** in the left menu to access the primary connection string for the alias. Use this connection string instead of using the connection string to the primary or secondary namespace directly. Initially, the alias points to the primary namespace.
1. Switch to the **Overview** page. You can perform the following actions: 
    1. Break the pairing between primary and secondary namespaces. Select **Break pairing** in the toolbar. 
    1. Manually fail over to the secondary namespace. 
        1. Select **Failover** in the toolbar. 
        ...
        1. Select **Failover**. 
            ...
            :::image type="content" source="./media/service-bus-geo-dr/failover-page.png" alt-text="Screenshot showing the Failover page." lightbox="./media/service-bus-geo-dr/failover-page.png":::
...
1. Finally, add some monitoring to detect if a failover is necessary. In most cases, the service is one part of a large ecosystem, so automatic failovers are rarely possible, as often failovers must be performed in sync with the remaining subsystem or infrastructure.

### Service Bus standard to premium
If you [migrated your Azure Service Bus Standard namespace to Azure Service Bus Premium](service-bus-migrate-standard-premium.md), you must use the preexisting alias (your Service Bus Standard namespace connection string) to create the disaster recovery configuration through **PS/CLI** or **REST API**.

During migration, your Azure Service Bus standard namespace connection string and DNS name become an alias to your Azure Service Bus premium namespace.

Your client applications must use this alias (the Azure Service Bus standard namespace connection string) to connect to the premium namespace where you set up the disaster recovery pairing.

If you use the Azure portal to set up the disaster recovery configuration, the portal handles this detail for you.

## Failover flow

A customer manually triggers a failover (either explicitly through a command or through client owned business logic that triggers the command). Azure never triggers a failover. This approach gives the customer full ownership and visibility for outage resolution on Azure's backbone.

:::image type="content" source="./media/service-bus-geo-dr/failover-alias-update-flow.png" alt-text="Diagram of failover flow showing primary and secondary Azure Service Bus namespaces with alias redirected to secondary." lightbox="./media/service-bus-geo-dr/failover-alias-update-flow.png":::

After the failover triggers -
...
> [!NOTE]
> Only *fail-forward* semantics are supported. When you initiate a failover by using geo-disaster recovery, you can re-pair with a new namespace after failover completes. You can't fail back to the previous primary replica. These semantics might be different to other data stores you're used to, like Microsoft SQL Server clusters.

You can automate failover either by using monitoring systems or by using custom-built monitoring solutions. However, such automation takes extra planning and work, which is out of the scope of this article.

:::image type="content" source="./media/service-bus-geo-dr/automate-failover-workflow.png" alt-text="Diagram of automated failover workflow showing monitoring, failover condition check, and namespace switching steps." lightbox="./media/service-bus-geo-dr/automate-failover-workflow.png":::

## Management

If you make a mistake, such as pairing the wrong regions during the initial setup, you can break the pairing of the two namespaces at any time. If you want to use the paired namespaces as regular namespaces, delete the alias.
...
Keep the following considerations in mind with this release:

- In your failover planning, consider the time factor. For example, if you lose connectivity for longer than 15 to 20 minutes, you might decide to initiate the failover.
- Because no data is replicated, currently active sessions aren't replicated. Additionally, duplicate detection and scheduled messages might not work. New sessions, new scheduled messages, and new duplicates work. 
...
Suppose you have two virtual networks, VNET-1 and VNET-2, and these primary and secondary namespaces: `ServiceBus-Namespace1-Primary` and `ServiceBus-Namespace2-Secondary`. Complete the following steps: 
...
:::image type="content" source="./media/service-bus-geo-dr/private-endpoints-virtual-networks.png" alt-text="Diagram that shows private endpoints and virtual networks configuration for Geo-Disaster Recovery." lightbox="./media/service-bus-geo-dr/private-endpoints-virtual-networks.png":::

The advantage of this approach is that failover can happen at the application layer independent of Service Bus namespace. Consider the following scenarios: 
