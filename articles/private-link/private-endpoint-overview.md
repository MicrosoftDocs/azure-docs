...
|---------|---------|---------|
|Network Security Groups rules does not apply to Private Endpoint    |Network security group (NSG) is not supported on private endpoints. While subnets containing the privat endpoint can have NSG associated with it, the rules will not be effective on traffic processed by the private endpoint. You must have [network policies enforcement disabled](disable-private-endpoint-network-policy.md) to deploy private endpoints in a subnet. NSG is still enforced on other workloads hosted on the same subnet.   | Control the traffic by using Network Security Group rules for outbound traffic on source clients.        |
...
|private endpoint can allow be mapped to private link service (customer owned) in the smae region    |   Connecting to a private link service (your own) from a different region is not supported       |  During Preview, you must deploy your Private Link service in the same region.        |
...
|  Portal does not support creation of private endpoints using Alias  |   Portal only allows creating private endpoints using resource URI      | Use resource URI for requesting private endpoint connections        |
