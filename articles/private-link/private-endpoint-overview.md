...
> [!NOTE]
> Only a private endpoint in approved state is able to send traffic to a given private link resource. 
...
> [!IMPORTANT]
> It's not recommended to override a zone that is actively in use to resolve public endpoints, connections to resources won't be able to resolve properly without a DNS forwarding to the public DNS. To avoid issues, create a different domain name or follow the suggested name for each service below. 
...
For more details, please check the tutorial on how to configure the DNS settings for my private endpoints.
