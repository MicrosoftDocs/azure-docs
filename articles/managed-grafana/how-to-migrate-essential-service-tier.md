...
description: Transition from the deprecated Azure Managed Grafana Essential service tier. Step-by-step guide to move to Standard service tier or to Azure Monitor dashboards with Grafana.
author: maud-lv
ms.topic: how-to
ms.date: 06/24/2026
ms.author: malev
...
1. Export your dashboard from Azure Managed Grafana by using the Grafana UI or the Azure CLI.
    ...
    Run the `az grafana backup` command in preview to export dashboards, data sources, and other components.
    
    ```azurecli
    az grafana backup --name <grafana-workspace-name>
    ```
  
    For additional options and filters, see [az grafana backup](/cli/azure/grafana/#az-grafana-backup). For example, the following command backs up data sources, dashboards, and folders to a temp folder from the specified Grafana resource while excluding the "General" and "Azure Monitor" folders:
