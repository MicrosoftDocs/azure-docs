...
When you sign up for a Twilio account, you'll get a free phone number for your application. You'll also receive an account SID and an auth token. Both will be needed to make Twilio API calls. To prevent unauthorized access to your account, keep your authentication token secure. Your account SID and auth token are viewable in the [Twilio Console][twilio_console], in the fields labeled **ACCOUNT SID** and **AUTH TOKEN**, respectively.
...
First, [set-up a new Azure Linux VM][azure_vm_setup] to act as a host for your new Ruby web application. Once the Virtual Machine is running, you will need to expose your application on a public port as described below.
...
At this point you should be able the run the command `sudo ruby web.rb -p 80`. This will spin-up a small web server on port *80*. You should be able to browse to this app in your browser by visiting the DNS Name set for the Public IP in previous steps (your-domain-label.centralus.cloudapp.azure.com). Once you can reach your web app in the browser, you're ready to start building a Twilio app.
...
Add this function to `web.rb`:

    # Set your account ID and authentication token.
    sid = "your_twilio_console_sid";
    ...
    # This is the full Url for your webhook, which should be set in the Twilio Console later on.
    url = "http://your-domain-label.centralus.cloudapp.azure.com/voice_url";
...
If you open-up `http://your-domain-label.centralus.cloudapp.azure.com/make_call` in a browser, that will trigger the call to the Twilio API to make the phone call. The first two parameters in `client.account.calls.create` are fairly self-explanatory: the number the call is `from` and the number the call is `to`.

The third parameter (`url`) is the URL that Twilio requests to get instructions on what to do once the call is connected. In this case we set-up a URL (`http://your-domain-label.centralus.cloudapp.azure.com`) that returns a simple TwiML document and uses the `<Say>` verb to do some text-to-speech and say "Hello Monkey" to the person recieving the call.
...
First, log-in to your [Twilio Console][twilio_console]. Click on "Phone Numbers" in the left-hand nav and then click on the Twilio number you were provided. You'll see two URLs that you can configure. A Voice Request URL and an SMS Request URL. These are the URLs that Twilio calls whenever a phone call is made or an SMS is sent to your number. The URLs are also known as **web hooks**.

We would like to process incoming SMS messages, so let's update the URL to `http://your-domain-label.centralus.cloudapp.azure.com/sms_url`. Go ahead and click Save Changes at the bottom of the page. Now, back in `web.rb` let's program our application to handle this:
