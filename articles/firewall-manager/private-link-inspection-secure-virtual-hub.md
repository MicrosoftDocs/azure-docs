...
2. Configure [custom DNS servers](../virtual-network/manage-virtual-network.yml#change-dns-servers) for the virtual networks connected to the secured virtual hub:
   - **FQDN-based network rules** - configure the [custom DNS settings](../virtual-network/manage-virtual-network.yml#change-dns-servers) of the connected virtual networks to point to the Azure Firewall private IP address. Configure the [custom DNS server](../firewall/dns-settings.md#configure-custom-dns-servers) in the firewall policy to point to the DNS forwarder virtual machine IP address. Enable DNS proxy in the firewall policy associated with the Azure Firewall. To enable FQDN filtering in network rules, you must enable DNS proxy.
