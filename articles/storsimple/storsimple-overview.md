...
Microsoft Azure StorSimple is an efficient, cost-effective, and easily manageable SAN solution that eliminates many of the issues and expenses associated with enterprise storage and data protection. It uses a proprietary device (the Microsoft Azure StorSimple device), integrates with cloud services, and provides a set of management tools for a seamless view of all enterprise storage, including cloud storage.
...
2. The administrator uses the serial console and the StorSimple Manager service (running in the Azure Management Portal) to configure the device, create volumes and data protection policies. On-premises machines (such as file servers) use the Internet Small Computer System Interface (iSCSI) to access the StorSimple device.
