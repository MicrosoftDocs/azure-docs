...
|--------------------------|------------------------|
|Update 5.1 or later and the device is offline. <br> You see an alert that URL is not approved.|<ol><li>Modify the firewall rules to include the authentication URL. See [authentication URLs](#url-changes-for-aad-authentication).</li><li>[Get the AAD registration key from the service](#aad-based-registration-keys).</li><li>[Connect to the Windows PowerShell interface of the StorSimple 8000 series device](storsimple-8000-deployment-walkthrough-u2.md#use-putty-to-connect-to-the-device-serial-console).</li><li>Use `Redo-DeviceRegistration` cmdlet to register the device through the Windows PowerShell. Supply the key you got in the previous step.</li></ol> |
|Update 5.0 or earlier and the device is offline.<br>TLS 1.2 enforcement has been implemented in the region. | Transport Layer Security (TLS) 1.2 is being enforced by the StorSimple Data Manager service. The enforcement is being rolled out by region.<br>If TLS 1.2 is enforced and the device goes offline, install Update 5.1 (or higher):<ol><li>[Connect to Windows PowerShell on the StorSimple 8000 series device](storsimple-8000-deployment-walkthrough-u2.md#use-putty-to-connect-to-the-device-serial-console), or connect directly to the appliance via serial cable.</li><li>Use [Start-HcsUpdate](/powershell/module/hcs/start-hcsupdate?view=winserver2012r2-ps) to update the device. For steps, see [Install regular updates via Windows PowerShell](storsimple-update-device.md).md#to-install-regular-updates-via-windows-powershell-for-storsimple). This update is non-disruptive.</li><li>If Start-HcsUpdate doesn’t work because of firewall issues, [install Update 5.1 (or higher) via hotfix method](storsimple-8000-install-update-51.md#install-update-51-as-a-hotfix).</li></ol> |
| Update 4 or earlier and the device is offline. |<ol><li>Modify the firewall rules to include the authentication URL.</li><li>[Download Update 5 through catalog server](storsimple-8000-install-update-5.md#download-updates-for-your-device).</li><li>[Apply Update 5 through the hotfix method](storsimple-8000-install-update-5.md#install-update-5-as-a-hotfix).</li><li>[Get the AAD registration key from the service](#aad-based-registration-keys).<br>5. [Connect to the Windows PowerShell interface of the StorSimple 8000 series device](storsimple-8000-deployment-walkthrough-u2.md#use-putty-to-connect-to-the-device-serial-console).</li><li>Use `Redo-DeviceRegistration` cmdlet to register the device through the Windows PowerShell. Supply the key you got in the previous step.</li></ol> |
