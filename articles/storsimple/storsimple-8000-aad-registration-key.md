...
|--------------------------|------------------------|--------------------|--------------------------------------------------------------|
| Update 5 or later and the device is offline. <br> You see an alert that URL is not whitelisted.|1. Modify the firewall rules to include the authentication URL.<br>2. See [authentication URLs](#url-changes-for-aad-authentication).<br>3. [Get the AAD registration key from the service](#aad-based-registration-keys).<br>4. [Connect to the Windows PowerShell interface of the StorSimple 8000 series device](storsimple-8000-deployment-walkthrough-u2.md#use-putty-to-connect-to-the-device-serial-console).<br>5. Use `Redo-DeviceRegistration` cmdlet to register the device through the Windows PowerShell. Supply the key you got in the previous step.|
