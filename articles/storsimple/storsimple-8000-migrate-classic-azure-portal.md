...
You may need to move your StorSimple service to a new enrollment or to a new subscription. You mat also want to migrate your Azure classic storage account to Azure Resource Manager. All these migration scenarios are either account changes or datacenter changes. Use the following table to understand which of these scenarios are supported incuding the detailed steps to move.
...
| Classic storage account to an Azure Resource Manager storage account?   | Yes       | No       |Migration from classic to Azure Resource Manager| For detailed instructions, go to [Migrate a classic storage account](migration-classic-resource-manager-ps.md#step-62-migrate-a-storage-account). <br>After the migration, you cannot rotate the keys for the migrated storage account via StorSimple Device Manager service. <br>You need to create new StorSimple Cloud Appliances in the Azure portal and then failover from the older cloud appliances. For more information, go to [Deploy and manage a StorSimple Cloud Appliance](storsimple-8000-cloud-appliance-u2.md).|
| StorSimple service from one subscription to another subscription?      | No        | Manual process            | Yes      | None|<li>Migrate data off the StorSimple device.</li><li>Perform a factory reset of the device, this will delete any local data on the device.</li><li>Register the device with the new subscription to a StorSimple Device Manager service.</li><li>Migrate the data back to the device.|
| StorSimple device from one StotSimple Device Manager service to another service in a different region?      | No        | Manual process            | Yes      | None |<li>Migrate data off the StorSimple device.</li><li>Perform a factory reset of the device, this will delete any local data on the device.</li><li>Register the device with the new subscription to a StorSimple Device Manager service.</li><li>Migrate the data back to the device.|

### Datacenter changes

| Can you move …| Supported|Downtime| Azure Support process| Approach|
|-----|-----|-----|-----|-----|-----|
| A StorSimple service from one Azure datacenter to another? | No |  |  |<li>Migrate data off the StorSimple device.</li><li>Perform a factory reset of the device, this will delete any local data on the device.</li><li>Register the device with the new subscription to a new StorSimple Device Manager service.</li><li>Migrate the data back to the device.|
| A storage account from one Azure datacenter to another? | No |  |  | <li>Migrate data off the StorSimple device.</li><li>Perform a factory reset of the device, this will delete any local data on the device.</li><li>Register the device with the new subscription to a new StorSimple Device Manager service.</li><li>Migrate the data back to the device.|