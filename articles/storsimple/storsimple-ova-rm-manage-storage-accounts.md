...
* Access control records
  ...
  ![Device Manager Service dashboard](./media/storsimple-ova-rm-manage-storage-accounts/ova-storageaccts-dashboard.png)  
...
* **SSL enabled** – Whether the SSL is enabled and device-to-cloud communication is over the secure channel.
  ...
  ![Configuration section](./media/storsimple-ova-rm-manage-storage-accounts/ova-storageaccountcredentials-blade.png)
...
* **Auto-generated storage account credentials** – As the name suggests, this type of storage account credential is automatically generated when the service is first created. To learn more about how this storage account credential is created, see [Create a new service](storsimple-ova-rm-manage-service.md#create-a-service).
...
5. The newly created storage account credential is displayed on the StorSimple Configure Device Manager service blade under **Storage account credentials**.
    ...
    ![Add a storage account credential outside of the Device Manager service subscription](./media/storsimple-ova-rm-manage-storage-accounts/ova-add-outside-storageacct.png)
...
4. In the storage account credential **Properties** blade, do the following:
   ...
   3. Click **Save** at the top of the **Properties** blade to save the settings. The settings are updated on the **Storage account credentials** blade.
      ...
      ![Edit a storage account credential](./media/storsimple-ova-rm-manage-storage-accounts/ova-edit-storageacct.png)
...
4. In the storage account credential **Properties** blade, do the following:
   ...
   2. When prompted for confirmation, click **Yes** to continue with the deletion. The tabular listing is updated to reflect the changes.
      ...
      ![Delete a storage account credential](./media/storsimple-ova-rm-manage-storage-accounts/ova-del-storageacct.png)
...
4. In the StorSimple Device Manager service, you need to update the key that was previously changed in the Microsoft Azure Storage service. In the **Synchronize storage account key** blade, if the primary access key was changed (regenerated), click Primary, and then click **Sync Key**. If the secondary key was changed, click **Secondary**, and then click **Sync Key**.
    ...
    ![Sync access key](./media/storsimple-ova-rm-manage-storage-accounts/ova-sync-acess-key.png)
