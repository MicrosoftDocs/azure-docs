...
|:---|:-------------------|:-------------------|
|1|One controller is in a failed state, the other controller is healthy and active.|[Single controller replacement](#replace-a-single-controller) (which describes the logical sequence behind a single controller replacement, as well as the replacement steps.)|
|2|Both the controllers have failed and require replacement. The chassis, disks, and.disk enclosure are healthy.|[Dual controller replacement](#replace-both-controllers) (which describes the logical sequence behind a dual controller replacement, as well as the replacement steps.) |
...
|4|One controller is missing and the other controller fails.|[Dual controller replacement](#replace-both-controllers)(which describes the logical sequence behind a dual controller replacement, as well as the replacement steps.)|
