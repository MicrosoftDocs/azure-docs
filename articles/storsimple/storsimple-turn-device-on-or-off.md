...
The StorSimple 8100 model is a single enclosure device. Perform the following steps when you turn it on for the first time.

#### To turn on an 8100 model for the first time
...
3. Attach the power cords to the rack power distribution units (PDUs) as shown in the following image. Make sure that the two PCMs use separate power sources.
    ...
    **Figure 1** Power cabling on an 8100 device
...
1. Identify the primary and the EBOD enclosures. The two enclosures can be identified by looking at their respective back planes. See the following image for guidance. 
    ...
    ![Back plane of primary and EBOD enclosures](./media/storsimple-turn-device-on-or-off/HCSBackplaneofprimaryandEBODenclosure.png)

    **Figure 2** Back view of primary and EBOD enclosures
...
2. Locate the serial numbers on the primary and the EBOD enclosures. The serial number sticker is affixed to the back ear of each enclosure. The serial numbers must be identical on both enclosures. [Contact Microsoft Support](storsimple-contact-microsoft-support.md) immediately if the serial numbers do not match. See Figure 3 to help you locate the serial numbers.

    ![Rear view of enclosure showing serial number](./media/storsimple-turn-device-on-or-off/HCSRearviewofenclosureindicatinglocationofserialnumbersticker.png)

    **Figure 3** Location of serial number sticker
...
3. Use the provided SAS cables to connect the EBOD enclosure to the primary enclosure as follows:
    ...
    1. Identify the four SAS ports on the primary enclosure and the EBOD enclosure. The SAS ports are labeled as EBOD on the primary enclosure and CTRL on the EBOD enclosure, as shown in the Figure 4.
...
2. On the primary enclosure, connect the power cords to both PCMs. The power cords are identified in red in Figure 4.

3. Make sure that the two PCMs on the primary enclosure use separate power sources.

4. Attach the power cords to the power on the rack distribution units as shown in Figure 4.
...
10. Make sure that the connection between the EBOD controller and the device controller is active by verifying that the four LEDs next to the SAS port on the EBOD controller are green.
    ...
    ![Cable your 4U device for power](./media/storsimple-turn-device-on-or-off/HCSCableYour4UDeviceforPower.png)
