...
Learn more about [Redis Cache Server - RedisCacheMemoryFragmentation (Availability may be impacted from high memory fragmentation. Increase fragmentation memory reservation to avoid potential impact.)](http://aka.ms/redis/recommendations/memory-policies).
...
Group two or more virtual machines in an availability set to ensure at least one virtual machine is available during an outage.
Learn more about [Virtual machine - SIVM (Use availability sets for improved fault tolerance)](https://aka.ms/aa_sivmrec_learnmore).
...
### KustoMockRecommendationType

Mock recommendation type for Kusto-based ingestion.
Learn more about [Virtual machine - KustoMockRecommendationType (KustoMockRecommendationType)](aka.ms/advisordocs).
...
Configure virtual machine backup to protect your mission critical data against accidental deletion or corruption.
Learn more about [Virtual machine - VmNoBackup (Enable virtual machine backup to protect your data from corruption and accidental deletion)](https://aka.ms/aa_vmbackup_learnmore).
...
Learn more about [Cosmos DB account - CosmosDbPartitionedCollections (Configure your Azure Cosmos DB containers with a partition key)](https://aka.ms/cosmosdb/choose-partitionkey).
...
Your Azure Cosmos DB account is using an old version of the Cosmos DB Spark connector. We recommend upgrading to the latest version from Maven for latest fixes, performance improvements, and new feature capabilities.
Learn more about [Cosmos DB account - CosmosDbUpgradeSparkConnector (Upgrade your Azure Cosmos DB Spark Connector to the latest version from Maven)](https://aka.ms/cosmosdb/spark-connector).
...
### Basic support for Azure HDInsight 3.6 clusters

Starting July 1, 2021, Microsoft will offer Basic support for certain HDInsight 3.6 cluster types. This will be available till April 3, 2022. No action is required by you to opt-in. We don't recommend building any new solutions and freeze changes on existing 3.6 environments.
Learn more about [HDInsight cluster - Basic support for 3.6 (Basic support for Azure HDInsight 3.6 clusters)](https://aka.ms/hdinsight-components).
...
Learn more about [Virtual network gateway - ActiveActiveGatewaySingleTunnel (Configure redundant tunnel for your active-active VPN gateway)](https://aka.ms/aa_activeactive_singletun_learnmore).
...
Customers using Microsoft Peering for Office 365 services should have at least two ExpressRoute Circuits in different locations to avoid a single point of failure. Create an additional ExpressRoute circuit to have a redundant connection in case of any failure.
Learn more about [Subscription - ExpressRouteCircuitO365 (Create additional ExpressRoute Circuits in another location for redundancy)](https://aka.ms/o365ERAdvisor).
