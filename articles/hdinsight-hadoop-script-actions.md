...
When cluster nodes are re-imaged, the C:\ resource drive and D:\ system drive can be re-formatted resulting in the loss of data and applications that had been installed on those drives. This could also happen if an Azure virtual machine (VM) node which is part of the cluster goes down and is replaced by a new node. You can install components on the D:/ drive or in the C:\apps location on the cluster. All other locations on the C:\ drive are reserved. Specify the location where applications or libraries are to be installed in the cluster customization script. 
...
You can use Script Action to customize HDInsight clusters by using the Azure Management Portal, Azure PowerShell, or the HDInsight .NET SDK. For instructions, see [How to use Script Action](./hdinsight-hadoop-customize-cluster/#howto). 
...
**Set the execution policy for Azure PowerShell:** Open Azure PowerShell and run (as administrator) following command to set the execution policy to the *LocalMachine* and to be *Unrestricted*.
...
**Run the script action**: Open a new Azure PowerShell in admin mode and run the Spark or R installation script from the local location where they were saved.
