...
In theory marathon-lb could be the only LB used but using the Azure LB as the public facing LB provides additional features such as security and (*** TODO ***) that would have to be configured and managed separately if marathon-lb were the only LB. 
...
The Marathon LB solution will dynamically reconfigure itself based on the containers you have deployed. It's also resilient to the loss of a container or an agent; if this occurs, Mesos will simply restart the container elsewhere and reconfigure the LB. 
...
Now that we have the marathan-lb package, we can deploy a simple web server using the following configuration:
...
The key parts of this are: 
  ...
  * Set the servicePort to a port >=10000 (this identifies the service being run in this container; marathon-lb uses this to identify services it should balance across).
  * Set the HAPROXY_GROUP label to "external" 
  * Set hostPort to 0 (meaning marathon will allocate an available port) 
...
You can have a scenario where, for instance, you use different domains to expose the services. For example: 
...
Alternatively, you could expose different ports and remap them to the correct service behind marathon lb, this means that, for example, ports on the container could change at any time and won’t require any change in the Azure LB configuration:
