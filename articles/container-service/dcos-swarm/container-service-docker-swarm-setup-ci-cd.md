...
The objective is to deliver this application continuously in a Docker Swarm cluster, using Azure DevOps. The following figure details this continuous delivery pipeline:
...
1. GitHub triggers a build in Azure DevOps 
1. Azure DevOps gets the latest version of the sources and builds all the images that compose the application 
1. Azure DevOps pushes each image to a Docker registry created using the Azure Container Registry service 
1. Azure DevOps triggers a new release 
...
You also need an Ubuntu (14.04 or 16.04) machine with Docker installed. This machine is used by Azure DevOps during the build and release processes. 
...
* [Use Docker to run the Azure DevOps agent](https://hub.docker.com/r/microsoft/vsts-agent)
...
You are asked to connect to your Azure DevOps organization using your credentials. 

### Connect Azure DevOps and GitHub

Set up a connection between your Azure DevOps project and your GitHub account.

1. In your Azure DevOps project, click the **Settings** icon in the toolbar, and select **Services**.

    ![Azure DevOps - External Connection](./media/container-service-docker-swarm-setup-ci-cd/vsts-services-menu.png)

1. On the left, click **New Service Endpoint** > **GitHub**.

    ![Azure DevOps - GitHub](./media/container-service-docker-swarm-setup-ci-cd/vsts-github.png)

1. To authorize Azure DevOps to work with your GitHub account, click **Authorize** and follow the procedure in the window that opens.

    ![Azure DevOps - Authorize GitHub](./media/container-service-docker-swarm-setup-ci-cd/vsts-github-authorize.png)

### Connect Azure DevOps to your Azure container registry and Azure Container Service cluster
...
1. In the **Services** settings of your Azure DevOps project, add a service endpoint of type **Docker Registry**. 

1. In the popup that opens, enter the URL and the credentials of your Azure container registry.

    ![Azure DevOps - Docker Registry](./media/container-service-docker-swarm-setup-ci-cd/vsts-registry.png)
...
1. For the Docker Swarm cluster, add an endpoint of type **SSH**. Then enter the SSH connection information of your Swarm cluster.
    ...
    ![Azure DevOps - SSH](./media/container-service-docker-swarm-setup-ci-cd/vsts-ssh.png)
...
1. On the **Build Definitions** page, first open the **Repository** tab and configure the build to use the fork of the MyShop project that you created in the prerequisites. Make sure that you select *acs-docs* as the **Default branch**.
    ...
    ![Azure DevOps - Build Repository Configuration](./media/container-service-docker-swarm-setup-ci-cd/vsts-github-repo-conf.png)
...
1. On the **Triggers** tab, configure the build to be triggered after each commit. Select **Continuous integration** and **Batch changes**.
    ...
    ![Azure DevOps - Build Trigger Configuration](./media/container-service-docker-swarm-setup-ci-cd/vsts-github-trigger-conf.png)
...
1. To add a step in the build workflow, click **+ Add build step** and select **Docker**.
    ...
    ![Azure DevOps - Add Build Steps](./media/container-service-docker-swarm-setup-ci-cd/vsts-build-add-task.png)

1. For each image, configure one step that uses the `docker build` command.

    ![Azure DevOps - Docker Build](./media/container-service-docker-swarm-setup-ci-cd/vsts-docker-build.png)
...
1. For each image, configure a second step that uses the `docker push` command.
    ...
    ![Azure DevOps - Docker Push](./media/container-service-docker-swarm-setup-ci-cd/vsts-docker-push.png)
...
1. After you configure the build and push steps for each of the five images, add two more steps in the build workflow.
    ...
    ![Azure DevOps - Update Compose file](./media/container-service-docker-swarm-setup-ci-cd/vsts-build-replace-build-number.png)
...
![Azure DevOps - Release to ACS](./media/container-service-docker-swarm-setup-ci-cd/vsts-release-acs.png) 
...
1. To configure the release trigger, click **Triggers** and select **Continuous Deployment**. Set the trigger on the same artifact source. This setting ensures that a new release starts as soon as the build completes successfully.
    ...
    ![Azure DevOps - Release Triggers](./media/container-service-docker-swarm-setup-ci-cd/vsts-release-trigger.png) 
...
1. Configure a task to securely copy the compose file to a *deploy* folder on the Docker Swarm master node, using the SSH connection you configured previously. See the following screen for details.
    ...
    ![Azure DevOps - Release SCP](./media/container-service-docker-swarm-setup-ci-cd/vsts-release-scp.png)
...
1. Configure a second task to execute a bash command to run `docker` and `docker-compose` commands on the master node. See the following screen for details.
    ...
    ![Azure DevOps - Release Bash](./media/container-service-docker-swarm-setup-ci-cd/vsts-release-bash.png)
