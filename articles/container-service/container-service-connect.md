...
### Install kubectl
To connect to a Kubernetes cluster, you need to have the `kubectl` command-line tool installed. One way to install this
tool is to use the Azure CLI 2.0 (Preview) `az acs kubernetes install cli` command. To run this command, make sure that you [installed](/cli/azure/install-az-cli2) the latest Azure CLI 2.0 (Preview) and logged in to an azure account (`az login`).
