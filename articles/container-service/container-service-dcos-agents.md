...
The DC/OS agent pools In Azure Container Service are deployed as follows:

* The **private pool** consists of the agent nodes that you specify when you deploy the DC/OS cluster. Agent nodes that you add later by scaling the cluster are also deployed to the private pool.

* The **public pool** contains a predetermined number of agent nodes. The number of agents in this public pool is determined by the number of master nodes created in the cluster: one public agent for one master, and two public agents for three or five masters.
