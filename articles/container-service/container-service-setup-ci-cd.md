...
On first run, this command may take a minute or so to complete, so please be patient. Once completed, important information is returned regarding the build and release pipeline it created:
...
* `containerRegistry`: The Azure Container Registry is a managed service that runs a Docker container registry. A new Azure Container Registry is created with a default name if it does not exist, or you can alternatively specify an Azure Container Registry name via the `--registry-name` parameter.
