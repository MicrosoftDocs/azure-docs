---
title: Troubleshoot LocationNotFoundForRoleSize when deploying a cloud service to Azure | Microsoft Docs
description: This article shows how to resolve a LocationNotFoundForRoleSize exception when deploying a cloud service to Azure.
...
In this article, you will troubleshoot allocation failures where a Virtual Machine (VM) size isn't available when deploying an Azure Cloud Service.
...
|---------|---------|
|LocationNotFoundForRoleSize     |The operation '\*\*\*\*\*' failed: 'The requested VM tier is currently not available in Region (\*\*\*\*\*) for this subscription. Please try another tier or deploy to a different location.'.|

## Cause

There's a capacity issue with the region or cluster that the you're deploying to. The *LocationNotFoundForRoleSize* error occurs when the resource SKU you have selected (such as VM size) isn't available for the location you have selected. This could be due to the capacity of the region or cluster that the cloud service is pinned to, or because the legion doesn't support a VM size that is specified.
