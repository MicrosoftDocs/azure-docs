...
## My connection to RDP is failing with "The Specified User Name does not exist" 

Follow these steps to resolve this issue: 

1. Right-click on the downloaded .rdp file 
2. Select the **Always ask for credentials** check box 
3. Click **Connect**,  and then provide credentials and proceed further.

If this doesn't resolve the issue, try to reset the RDP password by reconfiguring RDP. For more information, see [Enable Remote Desktop Connection for a Role in Azure Cloud Services](../cloud-services/cloud-services-role-enable-remote-desktop.md)

##  How to prevent receiving thousands of hits from some unknown IP addresses symptomatically indicating some sort of malicious attempt or DOS attack to the cloud service? 

The Azure implements a multilayer network security to protect its platform services against distributed denial-of-service (DDoS) attacks. The Azure’s DDoS defense system is part of Azure’s continuous monitoring process which is continually improved through penetration-testing. This DDoS defense system is designed to not only withstand attacks from the outside, but also from other Azure tenants. For more information, see [Azure Network Security Whitepaper](http://download.microsoft.com/download/C/A/3/CA3FC5C0-ECE0-4F87-BF4B-D74064A00846/AzureNetworkSecurity_v3_Feb2015.pdf).

That being said, you may create a startup task to selectively block some specific IP addresses. For more information, see [Block a specific IP address](cloud-services-startup-tasks-common.md#block-a-specific-ip-address)

## When I try to RDP to my Cloud Service Instance I get "The user account has expired"

You may get this error message "This user account has expired" when you by-pass the expiration date configured on your RDP settings. You can change the expiration date from the portal by following these steps: 

1. Log in to the [Azure Management Console](https://manage.windowsazure.com), and then navigate to you cloud service and select the **Configure** tab.
2. Select **Remote**. 
3. Change the **Expires On** date, and then save the configuation. 

You should be able to RDP to your machine now. 

## LoadBalancer is not balancing traffic equally

The distribution algorithm used is a 5 tuple (source IP, source port, destination IP, destination port, protocol type) hash to map traffic to available servers. It provides stickiness only within a transport session. Packets in the same TCP or UDP session will be directed to the same datacenter IP (DIP) instance behind the load balanced endpoint. When the client closes and re-opens the connection or starts a new session from the same source IP, the source port changes and causes the traffic to go to a different DIP endpoint.  For more information about how internal load balancer works, see [Azure Load Balancer new distribution mode](https://azure.microsoft.com/blog/azure-load-balancer-new-distribution-mode/).

## System.Net.WebException:The remote certificate is invalid according to the validation procedure. 

Windows has an Automatic Root Certificates Update feature which automatically scans certs in the certificate store and removes any that don’t pass a validation check against a known public certificate authority. This means that any self-signed cert or custom cert signed by an internal CA which is installed will eventually get deleted from the Azure VM and your application will start failing. 
 
It is recommended for you to install the full certificate chain (leaf cert, intermediate certs, and root cert) instead of just the leaf certificate.  When installing just the leaf certificate then they are relying on Windows to build the certificate chain by walking the CTL. If there are intermittent network or DNS issues in Azure or Windows Update at the time that Windows tries to validate the certificate then the cert will be considered invalid. If they install the full certificate chain then these types of problems can be avoided. For more information,see [How to install a chained SSL certificate](https://blogs.msdn.microsoft.com/azuredevsupport/2010/02/24/how-to-install-a-chained-ssl-certificate/).
