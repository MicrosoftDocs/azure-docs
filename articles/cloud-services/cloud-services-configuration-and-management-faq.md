...
You can enable Antimalware extension using PS script in the StartUp Task. Follow the steps in the following the articles to implement it: 
 
...
For more information on Antimalware deployment scenarios and how to enable it from the portal, see [Antimalware Deployment Scenarios](../security/azure-security-antimalware#antimalware-deployment-scenarios.md).
...
Note that the above steps could be automated via a startup task so that whenever a new PaaS instance gets created, it can do the changes above in the system registry. For more on startup tasks, see [How to configure and run startup tasks for a cloud service](cloud-services-startup-tasks.md).

 
Once this has been done, you can verify whether the HTTP/2 has been really enabled or not by using one of the following methods:
