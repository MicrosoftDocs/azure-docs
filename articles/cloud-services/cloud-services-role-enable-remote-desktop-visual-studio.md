...
You can deploy a cloud service project from a build server (for example, with Azure DevOps) on which Visual Studio 2017 version 15.5 or later is installed in the build agent. With this arrangement, deployment happens from the same computer on which the encryption certificate is available.

To use the RDP extension from Azure DevOps, include the following details in your build definition:
