...
2. Create a file named **install.cmd** and add it to each role by using **Add** > **Existing Item** in **Solution Explorer** as described earlier in this topic. 
   ...
   ![Role contents with all files][2]
    ...
    > [!IMPORTANT]
    > Use a basic text editor like Windows Notepad to create the install.cmd file. If you use Visual Studio to create a text file and change the extension to .cmd, the file might still contain a UTF-8 byte order mark and running the first line of the script will result in an error. Make sure that the first line of the file is a REM statement that may skip the UTF-8 byte order mark processing. 
    > 
    >
...
3. Add the following script to the install.cmd file:
    ...
    The install script checks whether the specified version of the .NET Framework is already installed on the machine by querying the registry. If the .NET version is not installed, then the .NET web installer is opened. To help troubleshoot any issues, the script logs all activity to the file startuptasklog-(currentdatetime).txt that is stored in **InstallLogs** local storage.
