...
Device twins are JSON documents persisted in an IoT hub, that are used to store device meta-data and facilitate the synchronization of device configurations and conditions between an IoT solution's device app and back-end.
Common use cases include:

* Solution back-ends storing device meta-data,
* Device apps reporting current state information such as available capabilities and device conditions (e.g. connected through wifi),
* Device apps and back-ends synchronizing the state of long-running workflows (e.g. firmware/configuration updates), and
* Solution back-ends querying devices based on meta-data, configuration or conditions.

>AZURE.NOTE Twins are designed for synchronization and querying of device configurations and conditions. Use [device-to-cloud messages][lnk-d2c] for sequences of timestamped events (e.g. telemetry streams such as time series of sensor data, alarms) and [cloud-to-device methods][lnk-methods] for interactive control of devices, e.g. turn on a fan from a user-controlled app.

A device twin contains *tags*, device meta-data accessible only by the back-end, *desired properties*, a JSON object modifiable by the back-end and observable by the device app, and *reported properties*, a JSON object modifiable by the device app and readable by the back-end. Note that neither tags nor properties can contain arrays, but objects can be nested. Additionally, the app back-end can query device twins based on all the above data. 
...
- Create a cloud back-end app that adds *tags* to a device twin, and a simulated device that reports the its connectivity channel as a *reported property* on the device twin.
...
* **AddTagsAndQuery.js**, a node app meant to be run from the cloud, which adds device tags and queries twins.
* **TwinSimulatedDevice.js**, which connects to your IoT hub with the device identity created earlier, and reports its connectivity condition.
...
In this section, you'll create a Node.js console app that adds location meta-data to the twin associated with `myDeviceId`. It then queries the twin collection stored in the hub selecting the ones located in the US, and then the ones that reported to be connected using cellular network.
...
4. Add the following code to the **AddTagsAndQuery.js** file, and substitute the `{service connection string}` placeholder with the connection string you copied when you created your hub:
    ...
    The `Registry` object exposes all the methods required to interact with device twins from the service. The above code, after the initialization of the `Registry` object, retrieves the twin for `myDeviceId` and updates its tags with the desired location information.
...
7. Add the following code at the end of  **AddTagsAndQuery.js** to implement the `queryTwins` function:
    ...
    The above code executes two queries: the first selecting only the twins of devices located in the `Redmond43` plant, and the second refining the query to select only the ones which are also connected through cellular network.

    Note that the above code, when creating the `query` object, specifies a maximum number of returned documents. The `query` object contains a `hasMoreResults` boolean property which can be used to invoke the `nextAsTwin` methods multiple times to retrieve all results. A method called `next` is available for results that are not twins, e.g. results of aggregation queries.

8. Run the application with:

        node AddTagsAndQuery.js

    You should see one device in the results for the query asking for all devices located in `Redmond43` and none for the ones that restricts the query to the ones that use cellular network.

    ![][1]

In the next section we will create a device app which will report the connectivity information and will change the above result.

# Create the device app
In this section, you'll create a Node.js console app that connects to your hub as `myDeviceId`, and then updates its twin's reported properties to contain the information that it is connected using cellular network.
...
4. Add the following code to the **ReportConnectivity.js** file, and substitute the `{device connection string}` placeholder with the connection string you copied when you created the `myDeviceId` device identity:
    ...
    The `Client` object exposes all the methods required to interact with device twins from the device. The above code, after the initialization of the `Client` object, retrieves the twin for `myDeviceId` and updates its reported property with the connectivity information.
