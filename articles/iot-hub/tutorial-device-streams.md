...
description: how to use IoT Hub device streams.
author: rezasherafat
manager: briz
services: iot-hub
ms.service: iot-hub
...
The device and service programs can interact using a variety of standards-based or proprietary protocols. As such, device streams treat application traffic as an opaque bytes stream passing through. This makes device streams application and protocol agnostic and hence allows this feature to act as a building block for end-to-end connectivity between device and service applications.
...
The device stream initiation handshake can be thought of as a control-plane operation to be contrasted with the actual exchange of data between the device and the service as a data-plane operation. As part of the handshake, device and service each receive a temporary authentication token and the URI of a *device streaming endpoint* to which connect to for sending and receiving data. The authentication token is valid for 60 seconds and device and service must use it prior to expiration to create and authenticate a WebSocket client with the streaming endpoint.
...
2. The service authenticates with IoT Hub using valid *service connect* credentials and initiates a stream by providing the target device ID and the stream name - the latter must match the name used by the device in step 1.

3. IoT hub notifies the device's program by invoking the callback registered in step 1. The URI of the streaming endpoint and the authentication token will also be passed to the device using this callback. At this point, the device has an option to accept or reject the stream initiation request. The following steps assume that the device has accepted the stream request.
