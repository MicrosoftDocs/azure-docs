...
| iothub-connection-auth-method |An authentication method set by IoT Hub on device-to-cloud messages. This property contains information about the authentication method used to authenticate the device sending the message. <!-- ROBINSH For more information, see [Device to cloud anti-spoofing][lnk-antispoofing].-->|No|AuthMethod|| iothub-creation-time-utc | Date and time the message was created on a device. A device must set this value explicitly. |No|TBD (waiting on response from engg)|
