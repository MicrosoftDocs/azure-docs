...
* **Route device-to-cloud messages to Azure services based on declarative rules**. IoT Hub allows you to define routes based on message rules to control where device-to-cloud messages are sent. This requires no additional code to be written, and takes the place of a lot of post-ingestion message dispatcher.
