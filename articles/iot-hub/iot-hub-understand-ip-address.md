...
|------|-----------|----------|
| Ensure your devices communicate with IoT Hub endpoints only | [Device-to-cloud](./iot-hub-devguide-messaging.md), and [cloud-to-device](./iot-hub-devguide-messages-c2d.md) messaging, [direct methods](./iot-hub-devguide-direct-methods.md), [device and module twins](./iot-hub-devguide-device-twins.md) and [device streams](./iot-hub-device-streams-overview.md) | Use _AzureIoTHub_ service tags to discover IoT Hub IP address prefixes and add ALLOW rules on your devices' firewall for those IP prefixes accordingly; drop traffic to other destination IP addresses you do not want the device to communicate with. |
...
* IP filtering rules are only enforced on your IoT Hub endpoint and not your hub's built-in Event Hub. If you also intend to enforce IP filtering on the event hub where your messages are stored, you may do so only by provisioning your own event hub and setting up message routing to send your messages to that resource where you can set up IP filtering rules directly. For this purpose, you must use `True` as the route condition and have no other routes to the built-in event hub endpoint. This would mean that your messages will no longer be stored in your IoT Hub's built-in Event Hub. Subsequently, you may configure your desired IP filtering rules on your provisioned event hub resource directly. Finally, as discussed in the table above, to enable message routing functionality you also need to allow connectivity from IoT Hub's IP address prefixes to your provisioned event hub resource.
