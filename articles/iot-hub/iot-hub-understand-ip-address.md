...
* [IP filtering rules](iot-hub-ip-filtering.md) are only applied on your IoT Hub IP endpoints and not on your IoT hub's built-in Event Hub endpoint. If you also require IP filtering on the Event Hub where your messages are stored, you may do so bringing your own Event Hub resource where you can configure your desired IP filtering rules directly. To do so, you need to provision your own Event Hub resource and set up message routing to send your messages to that resource using `True` as the route condition. This setup will result all your messages to no longer be stored in your IoT Hub's built-in Event Hub and instead be stored in your provisioned Event Hub resource where you can configure your desired IP filtering rules directly. Subsequently, as discussed in the table above, to enable message routing functionality you also need to allow connectivity from IoT Hub's IP address prefixes to your provisioned Event Hub resource.
