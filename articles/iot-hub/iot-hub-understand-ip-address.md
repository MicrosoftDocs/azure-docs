...
* IP filtering rules are only enforced on your IoT Hub IP endpoints and not on your hub's built-in Event Hub endpoint. If you also require IP filtering on the Event Hub where your messages are stored, you may do so only by provisioning your own Event Hub resource and setting up message routing to send your messages to that resource. For this purpose, you must use `True` as the route condition and have no other routes to the built-in Event Hub endpoint. This setup will result all your messages to no longer be stored in your IoT Hub's built-in Event Hub and instead be stored in your provisioned Event Hub where you can configure your desired IP filtering rules directly. Subsequently, as discussed in the table above, to enable message routing functionality you also need to allow connectivity from IoT Hub's IP address prefixes to your provisioned Event Hub resource.
