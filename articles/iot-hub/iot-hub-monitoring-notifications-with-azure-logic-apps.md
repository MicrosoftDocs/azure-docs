...
Azure Logic Apps can help you automate and orchestrate workflows across both on-premises and cloud services, one or more enterprises, and across various protocols. A logic app workflow begins with a trigger, which is then followed by one or more actions that can be sequenced using built-in controls, such as conditions and iterators. This flexibility makes Logic Apps an ideal IoT solution for many scenarios. For example, the arrival of telemetry data from a device at an IoT Hub endpoint can initiate logic app workflows to warehouse the data in an Azure Storage blob, send email alerts in the case of data anomalies, schedule a technician visit if a device reports a failure, and so on.

OLDER STUFF: Azure Logic Apps provides a way to automate processes as a series of steps. A Logic Apps workflow can connect across various services and protocols. A logic app instance begins with a trigger such as 'When an account is added', which is followed by a one or more actions; for example, 'sending a push notification'. This trigger-response mechanism makes Logic Apps an ideal IoT solution for IoT monitoring, such as staying alert for anomalies.
...
Create a Service Bus queue. Later in this topic, you create a routing rule in your IoT hub to direct messages that contain a temperature alert to this queue, where they will be picked up by a logic app and trigger it to send a notification email.
...
1. Create a service bus connection.
   ...
   1. Select your service bus namespace from the list.
...
1. (Optional.) If you had to disable SSL to establish a connection with your email provider and want to enable it, follow these steps:
