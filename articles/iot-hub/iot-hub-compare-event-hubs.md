...
| Route messages to multiple endpoints | Up to ten additional endpoints are supported. Rules determine how messages are routed to additional endpoints. See [Send and receive messages with IoT Hub][lnk-devguide-messaging] for more information. | Requires additional code to be written and hosted for message dispatching. |
