...
| Route messages to multiple endpoints | Up to 10 additional endpoints are supported. Rules determine how messages are routed to additional endpoints. For more information, see [Send and receive messages with IoT Hub][lnk-devguide-messaging]. | Requires additional code to be written and hosted for message dispatching. |
