...
| Operations monitoring | Allows IoT solutions to subscribe to a reach set of device identity management and connectivity events (e.g. individual device authentication errors, throttling, and bad format exceptions). These events allows to quickly identify connectivity problems at the individual device level. | Exposes only aggregate metrics. |
