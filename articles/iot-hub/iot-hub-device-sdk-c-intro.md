...
There are a broad range of platforms on which the SDK has been tested (see the [platforms and compatibility list](https://azure.microsoft.com/en-us/documentation/articles/iot-hub-tested-configurations/) for details). Although this article includes walkthroughs of sample code running on the Windows platform, keep in mind that the code described in this article is exactly the same across the range of supported platforms.
