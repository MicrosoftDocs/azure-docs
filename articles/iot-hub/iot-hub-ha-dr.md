...
Failback can be achieved by triggering the failover action another time. Users can only perform 2 successful failover operations and 2 successful failback operations per day.
...
Time to recover = RTO [10 min - 2 hours for manual failover | 2 - 26 hours for Microsoft initiated failover] + DNS propagation delay + Time taken by the client application to refresh the cached IoT Hub IP address. 
