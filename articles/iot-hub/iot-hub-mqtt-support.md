...
Connecting to IoT Hub using module identify over MQTT is similar to the device (described [above](#using-the-MQTT-protocol-directly-as-a-device)) but you need to substitute the following information:
* Set the client id to `<deviceID>/<moduleID>`.
* Set the username to `<hubname>.azure-devices.net/<deviceID>/<moduleID>/?api-version=2018-06-30`.
* Use the SAS token associated with the module identity as your password.
