...
[Device client SDKs][lnk-mqtt-org] that support the MQTT protocol are available for Java, Node.js, C and C#. The device client SDKs use the standard IoT Hub connection string to establish a connection to an IoT hub. In order to use the MQTT protocol the client protocol parameter has to be set to **MQTT**. By default, the device clients SDKs connect to an IoT Hub with the **CleanSession** flag set to **0** and use **QoS 1** for message exchange with the IoT hub.
...
- `{iothubhostname}/{device_id}` in the **Username** field
