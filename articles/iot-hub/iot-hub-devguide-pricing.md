...
- A link to the documentation for billable REST APIs.
- If REST API documentation isn't available, or if the operation is only available over MQTT and/or AMQP, the operation endpoint. The endpoint value omits the leading reference to the target IoT hub; `{fully-qualified-iothubname}.azure-devices.net`.
- One or more terms in *italics* following each operation (or endpoint) that represents billable operations that may be returned by customer support. These are provided to help customers working with customer support to relate these terms to operations that produce the associated charges. If you're not working with customer support, you can ignore them.
...
| Digital twin updates | Digital twin updates from the device and from the solution back end are charged as messages in 4-KB chunks. For example, reading a 12-KB twin is charged as 3 messages. <br/><br/> [Update Digital Twin](/rest/api/iothub/service/digital-twin/get-digital-twin), *Update Digital Twin* |
...
| Device streams (preview) | Device streams is in preview and operations are not charged. <br/><br/> Endpoint: `/twins/{deviceId}/streams/{streamName}`, *Device Streams* <br/> Endpoint: `/twins/{deviceId}/modules/{moduleId}/streams/{streamName}`, *Device Streams Module* |
