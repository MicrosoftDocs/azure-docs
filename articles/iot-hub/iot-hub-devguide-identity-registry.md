...
| deviceScope | read-only | The scope of the device. Auto generated and immutable. Always present in edge devices. In non-edge devices, present only in child devices and set to the same value as the parentScopes property. For more information, see [IoT Edge as a gateway: Parent and child relationships](/azure/iot-edge/iot-edge-as-gateway#parent-and-child-relationships).|
| parentScopes | optional | The scope of a child device's direct parent (the deviceScope property of the parent). In edge devices, the value is empty if the device has no parent device. In non-edge devices, the property is not present if the device has no parent device. For more information, see [IoT Edge as a gateway: Parent and child relationships](/azure/iot-edge/iot-edge-as-gateway#parent-and-child-relationships). |
