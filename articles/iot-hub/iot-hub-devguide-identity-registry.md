...
| deviceScope | optional | The scope of the device. In edge devices, auto generated and immutable. In non-edge devices, present only in child (leaf) devices and must be set to the value of the **deviceScope** property of the parent device. For more information, see [IoT Edge as a gateway: Parent and child relationships](/azure/iot-edge/iot-edge-as-gateway#parent-and-child-relationships).|
| parentScopes | optional | The scope of a child device's direct parent (the value of the **deviceScope** property of the parent device). In edge devices, the value is empty if the device has no parent device. In non-edge devices, the property is not present if the device has no parent. For more information, see [IoT Edge as a gateway: Parent and child relationships](/azure/iot-edge/iot-edge-as-gateway#parent-and-child-relationships). |
