...
If your IoT solution needs to know if a device is connected, you should implement the *heartbeat pattern* using the **Device Connected** and **Device Disconnected** events. See [Tutorial on how to use IoT Hub events](../event-grid/publish-iot-hub-events-to-logic-apps.md). The IoT Hub identity registry contains a field called **connectionState**. We recommend using the **connectionState** field during development and debugging.
