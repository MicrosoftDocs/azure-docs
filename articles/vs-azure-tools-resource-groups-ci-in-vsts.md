<properties
	pageTitle="Continuous integration in VSTS using Azure Resource Group projects | Microsoft Azure"
...
To deploy an Azure template, you need to perform tasks to go through the various stages: Build, Test, Copy to Azure (also called “Staging”), and Deploy Template.  There are two different ways to do this in Visual Studio Team Services (VSTS). Both methods provide the same results, so choose the one that best fits your workflow.
...
-	Add multiple VSTS build steps, each one performing a stage task.

This article demonstrates how to use the first option (use a build definition to run the PowerShell script). One advantage of this option is that the script used by developers in Visual Studio is the same script that is used by VSTS. This procedure assumes you already have a Visual Studio deployment project checked into VSTS.
...
## Set up continuous deployment in VSTS

### Overview
To call the PowerShell script in VSTS, you need to update your build definition. In brief, the procedure is:

1.	Edit the build definition.
1.	Set up Azure authorization in VSTS.
...
1.	Set the value of the *-ArtifactsStagingDirectory* parameter to work with a project built in VSTS.

### Detailed walkthrough

1.	Edit your VSTS build definition and add an Azure PowerShell build step. Choose the build definition under the **Build definitions** category and then choose the **Edit** link.
...
1.	Choose the **Azure PowerShell** build step and then fill in its values.
	...
	1.	If you already have an Azure service endpoint added to VSTS, choose the subscription in the **Azure Subscription** drop down list box and then skip to the next section. 

		If you don’t have an Azure service endpoint in VSTS, you’ll need to add one. This subsection takes you through the process. If your Azure account uses a Microsoft account (such as Hotmail), you’ll need to take the following steps to get a Service Principal authentication.
	...
	1.	Add a name of your choice to the **Subscription** name box. This value will appear later in the **Azure Subscription** drop down list in VSTS. 
