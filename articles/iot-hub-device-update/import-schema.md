...
If an AAD application is used to sign the user in, the scope needs to have /user_impersonation. 

You will need to add permissions to your AAD app (in the API permissions tab in AAD Application view) to use Azure Device Update API. Request API permission to Azure Device Update (located in “APIs my organization uses”) and grant the delegated user_impersonation permission.

ADU accepts tokens acquiring tokens using any of the AAD supported flows for users, applications, or managed identities. However, some of the flows require additional AAD application setup: 
