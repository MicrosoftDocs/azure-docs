...
The expected component name in your model is **"azureDeviceUpdateAgent"** when implementing this interface. [Learn more about Azure IoT PnP Components](https://docs.microsoft.com/azure/iot-pnp/concepts-components)
...
|aduVer|string|device to cloud|Version of the Device Update agent running on the device. This value is read from the build only if during compile time ENABLE_ADU_TELEMETRY_REPORTING is set to 1 (true). Customers can choose to opt-out of version reporting by setting the value to 0 (false). [How to customize agent properties](https://github.com/Azure/iot-hub-device-update/blob/main/docs/agent-reference/how-to-build-agent-code.md).|
|doVer|string|device to cloud|Version of the Delivery Optimization agent running on the device. The value is read from the build only if during compile time ENABLE_ADU_TELEMETRY_REPORTING is set to 1 (true). Customers can choose to opt-out of the version reporting by setting the value to 0 (false).[How to customize agent properties](https://github.com/microsoft/do-client/blob/main/README.md#building-do-client-components).|
...
The expected component name in your model is **deviceInformation** when implementing this interface. [Learn about Azure IoT PnP Components](https://docs.microsoft.com/azure/iot-pnp/concepts-components)
