---
title: Device Update for Azure IoT Hub agent check
...
| Compatibility property names | `CompatPropertyNames` is a field that the Device Update agent reports to the device twin. It describes what `deviceProperties` fields to use to determine the device's compatibility with a given deployment. This field's value must be a string of comma-delimited names. The string must contain at least one and no more than five names. Each name must be fewer than 32 characters. |
...
| ADU group | The ADU Group tag is an optional tag that you define in the device's device twin. It determines what device group the device belongs to. If specified, the tag string is limited to 255 characters and can only contain alphanumeric characters and the following special characters: "." "-" "_" "~" |
