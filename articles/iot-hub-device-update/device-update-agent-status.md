...
This API is available starting in Device Update agent reference implementation version 1.3.0. It's designed for device-local coordination, where applications such as supervisors, power managers, or orchestration services running on the device need to understand what the update process is doing in real time.
...
> [!IMPORTANT]
> This is a device-local API for on-device apps. It doesn't report status to IoT Hub or provide granular progress updates in IoT Hub.
...
- **Power management (battery-powered or low-power devices)**  
  Before entering sleep or low-power mode, a device can check the agent’s current state (for example, `Downloading` or `Installing`) and delay sleep until the agent is idle. This check helps prevent interrupted downloads, incomplete installs, or unnecessary retries.

- **Orchestration and supervision**  
  A device orchestrator or supervisor can wait until the agent is idle before triggering its own firmware update, factory reset, or configuration change. This approach reduces the risk of conflicting operations that could disrupt the device or update workflow.
...
- **The library communicates with the running Device Update agent locally.**  
  At runtime, the library communicates with the Device Update agent on the same device by using a local communication channel. This communication is fast and doesn't require a network request or a cloud round-trip.

Because this call is device-local, it can return the agent’s current state even if the device is temporarily offline. 
...
- **Idle:** The agent isn't actively processing an update workflow.
...
- **Paused:** The agent is in a quiet or hold period. See [Quiet period and the `Paused` state](#quiet-period-and-the-paused-state).

For the authoritative list of states and their definitions, see [Service Status API — GetAduServiceStatus](https://github.com/Azure/iot-hub-device-update/blob/develop/docs/agent-reference/GetAduServiceStatus.md).
...
This status lets on-device software distinguish between:
...
- **Idle**: The agent resumed normal operation and can process new work.


## Relationship to Device Update diagnostics and troubleshooting

The agent status API is **device-local** and is intended for applications running on the device (for example, power management or an on-device supervisor). It **doesn't** replace Device Update's service-side diagnostics in the Azure portal.
