...
Following example shows changing the word 'Device' to 'Asset' for viewing the application in English language

1. Click Add application text and select the English language from the drop down
1. Download the default text file which is a JSON object
1. Open the file with an JSON editor and edit the right hand side strings to replace word 'device' with 'asset' as showin in the following example customization file
```json
{
  "Device": {
...
```
1. Upload the edited customization file and click save to see your new text (asset) in the application

You can re-upload the customization file with new changes by clicking on the relevant language from the list in **Text** page of **Customization** section
