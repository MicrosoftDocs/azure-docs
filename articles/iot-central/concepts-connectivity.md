...
Azure IoT Central supports onboarding & connecting of devices at scale. IoTCentral uses Azure IoT [Device Provisioning service (DPS)](https://docs.microsoft.com/en-us/azure/iot-dps/about-iot-dps) for device registration and connection. This enables devices to connect to a IoT Central app at scale with zero touch. There is one consistent way to connect devices to IoTCentral with all the benefits of device security out-of-the-box. Devices can register and connect using either Shared Access Signatures (SAS) or X509 certificate based schemes depending on the use case. Once the device is registered every device that connects to your application, connects to the [endpoints](https://docs.microsoft.com/azure/iot-hub/iot-hub-devguide-endpoints) that are exposed by the IoT hub Azure IoT Central uses. IoT Hub enables scalable, secure, and reliable connections from your connected devices.
...
1. Get connection the details such as **Scope ID, Device ID, Primary key** for the  added device from the device page.
    ...
    * **Device ID** is the unique device ID per device, the device needs to sends this as part of the registration call.   
...
This happens when  
    ...
    *   A set of device are added using **Import** on **Explorer**
