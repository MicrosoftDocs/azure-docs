...
The most practical way to build up this JSON document is to use [the portal](https://portal.azure.com/) to interactively add and position your tiles. You then export the JSON. Finally, you templatize the result for later use in scripts, programs, and deployment tools.
...
Here are are two versions of our example dashboard JSON. The first is the version that we exported from the portal that was already bound to a resource. The second is the templatized version that can be programmatically bound to any VM and deployed using Azure Resource Manager.
