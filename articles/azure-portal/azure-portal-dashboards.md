...
This action creates a new, empty, private dashboard and puts you into customization mode where you can name your dashboard and add or rearrange tiles.  When in this mode, the collapsible tile gallery takes over the left navigation menu.  The tile gallery lets you find tiles for your Azure resources in a variety of ways: you can browse by [resource group](../resource-group-overview.md#resource-groups), by resource type, by [tag](../resource-group-using-tags.md), or by searching for your resource by name.  
...
By hovering and selecting "…" or right-clicking on a tile you can see all the contextual commands for that tile. By default, there will always be two items:
...
By selecting customize, you can resize and reorder tiles. To resize a tile, select the new size from the contextual menu, as shown below.
...
When you create a new dashboard it is private by default. This means you are the only person who can see it.  In order to make it visible to others use the **Share** button that appears alongside the other dashboard commands.

![share dashboard](./media/azure-portal-dashboards/share-dashboard.png)

You are asked to choose a subscription and resource group for your dashboard to be published to. In order to seamlessly integrate dashboards into the ecosystem, we've implemented shared dashboards as Azure resources (so you can't share by typing an email address).  Access to the information displayed by most of the tiles in the portal are governed by [Azure Role Based Access Control](../active-directory/role-based-access-control-configure.md ). From an access control perspective, shared dashboards are no different from a virtual machine or a storage account.  

Here is an example to make this clear.  Let's say you have an Azure subscription and members of your team have been assigned the roles of **owner**, **contributor**, or **reader** of the subscription.  Users who are owners or contributors will be able to list, view, create, modify, or delete dashboards within that subscription.  Users who are readers will be able to list and view dashboards, but cannot modify or delete them.  Users with reader access will be able to make local edits to a shared dashboard (e.g. when troubleshooting an issue), but will not be able to publish those changes back to the server.  However, they will have the option to make a private copy of the dashboard for their own use.  As always, individual tiles on the dashboard enforce their own access control rules based on the resources they correspond to.  
...
You can also choose to publish a dashboard to a particular resource group.  When you do this the access control for that dashboard will match the access control for the resource group.  This means that when you give users access to manage an entire resource group full of resources, then those users will inherently have access to the dashboards within it.
