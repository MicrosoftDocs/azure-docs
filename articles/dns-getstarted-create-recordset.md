...
## Creating CNAME record for Web apps custom domain

You can use Azure DNS to configure a custom domain for your web apps. You will need to add a CNAME record for your domain to validate you own it and assign to the web app following the steps in [Configuring a custom domain name for App Service](web-sites-custom-domain-name.md#create-the-dns-records). 

Assuming your domain is already managed by Azure DNS (see [DNS domain delegation](dns-domain-delegation.md)), we can use the following the example to create a CNAME record for contoso.azurewebsites.net:

### Step 1

Open powershell and create a new CNAME record set and assign to a variable $rs:

	PS C:\> $rs=new-AzureDnsRecordSet -ZoneName contoso.com -ResourceGroupName myresourcegroup -Name "www" -RecordType "CNAME"-Ttl 60
...
This will create a record set type CNAME with a "time to live" of 60 seconds in DNS zone name "contoso.com"

### Step 2

Once the CNAME record set is created, we need to assign the alias value which will point to the web app.Using the previously assigned variable "$rs" we will create the alias for the web app contoso.azurewebsites.net. 
	...
	RecordType        : CNAME
	Records           : {contosodns.azurewebsites.net}
	Tags              : {}

### Step 3

Let's commit the changes using Set-AzureDnsRecordSet to complete the operation:


	PS C:\>Set-AzureDnsRecordSet -RecordSet $rs

You can validate the record was created correctly querying the "www.contoso.com" using nslookup:
...
Now you can continue following the steps of [Configuring a custom domain name for App Service](web-sites-custom-domain-name.md) to get your web app using a custom domain.
