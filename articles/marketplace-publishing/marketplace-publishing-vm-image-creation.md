...
3.	Create a PowerShell (or other script executable) file with following code and save it locally
    ...
    c. **`<Permission Start Date>`**: To safeguard for UTC time, select the day before the current date. For example, if the current date is October 26, 2016, then value should be 10/25/2016. If using Azure CLI 2.0 (az command) ensure to provide both date and time in the Start and End Dates, for example, 10-25-2016T00:00:00Z.

    d. **`<Permission End Date>`**: Select a date that is at least 3 weeks after the **Start Date**. Then value should be **11/02/2016**. If using Azure CLI 2.0 (az command) ensure to provide both date and time in the Start and End Dates, for example, 11-02-2016T00:00:00Z.
...
4.	Open Powershell editor with “Run as Administrator” mode and open file in step #3. Or use any other script editor on your OS.
