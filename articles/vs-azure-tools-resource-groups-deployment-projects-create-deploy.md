...
1. You see all the deployed resources.
...
1. After Visual Studio creates your web app, add a reference in the resource group project to the web app project.
...
1. Add a resource to the template, and this time select **Web Deploy for Web Apps**. 

    ![add web deploy](./media/vs-azure-tools-resource-groups-deployment-projects-create-deploy/add-web-deploy.png)
    
1. Redeploy your resource group project to the resource group. This time there are some new parameters. You do not need to provide values for **_artifactsLocation** or **_artifactsLocationSasToken** because Visual Studio automatically generates those values. Set the folder and file name to the path that contains the deployment package.
