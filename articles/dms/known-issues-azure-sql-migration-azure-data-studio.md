...
- **Message**: `Cutover failed or canceled for database <DatabaseName>. Error details: The restore plan is broken because firstLsn <First LSN> of log backup <URL of backup in Azure Storage container>' is not <= lastLsn <last LSN> of Full backup <URL of backup in Azure Storage container>'. Restore to point in time.`
...
- **Message**: `Pre Copy steps finished successfully before canceling completed. Target database Foreign keys and temporal tables have been altered. Schema migration might be required again for future migrations. Target server: <Target Server>, Target database: <Target Database>.`
...
Note: To view error detail, Open Microsoft Integration runtime configuration manager > Diagnostics > logging > view logs.  
It opens the Event viewer > Application and Service logs > Connectors - Integration runtime and now filter for errors.

- **Message**: Deployed failure: Index can't be created on computed column '{0}' of table '{1}' because the underlying object '{2}' has a different owner. Object element: {3}.
