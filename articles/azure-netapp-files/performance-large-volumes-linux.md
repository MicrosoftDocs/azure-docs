...
Azure NetApp Files large volumes offers three service levels (*Ultra*, *Premium*, and *Standard*), each with its throughput limits. See [Service levels for Azure NetApp Files](azure-netapp-files-service-levels.md#supported-service-levels).
...
All scenarios documented in this article used FIO, which is a synthetic workload generator designed as a storage stress test. Fundamentally, there're two models of storage performance testing:
...
* **Application level**   
    ...
    * Except for databases (for example, Oracle, SAP HANA, MySQL (InnoDB storage engine), PostgreSQL, and Teradata), few applications use direct I/O. Instead, most applications leverage a large memory cache for repeated reads and a write-behind cache for asynchronous writes.
    * SPECstorage 2020 (EDA, VDA, AI, genomics, and software build), HammerDB for SQL Server, and Login VSI are typical examples of application-level testing tools. None of them leverages direct I/O.
...
* **Storage stress test**  
    ...
    * Except for database, few applications use direct I/O. Instead, they leverage the advantages of a large memory cache for repeated reads and a write-behind cache for asynchronous writes. In short, using direct I/O turns the test into a micro benchmark.
...
The graph below represents a 256-Kibibyte (KiB) sequential workload ranging in ten percent increments from 100% read to 100% write. A 1-TiB working set was used for these tests. This graph shows that a single Azure NetApp Files large volume can handle between ~8,518 MiB/s pure sequential writes and ~9,970 MiB/s pure sequential reads.
...
Twelve e32s_v5 VMs were used in testing. As the [Linux scale-up](#linux-scale-up-tests) section shows, using twelve VMs ensured that storage was the gating factor.
...
The following graphs show 256-KiB sequential reads of ~10,000 MiB/s reads with `nconnect`, approximately ten times the throughput achieved without  `nconnect`. 10,000 MiB/s is bandwidth proffered by a large volume in the Ultra service level. 6,400 MiB/s is the bandwidth for a Premium large volume. 1,600 MiB/s is the bandwidth for a Standard large volume.
