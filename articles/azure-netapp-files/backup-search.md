...
3. In the **Search Backups** box, enter the backup name that you want to search for.
    ...
    * The Backups list includes all backups of all volumes (both active and deleted) that belong to all NetApp accounts of a subscription.

    * The Backups list can be used for restoring a backup to a new volume and also for deleting a backup. The workflow for restoring a backup is the same as [volume-level backup restore](backup-restore-new-volume.md). The workflow for deleting a backup is same as [volume-level backup deletion](backup-delete.md).

    * The best practice suggested for [manual backup](backup-configure-manual.md) naming convention becomes useful in this listing to identify which volume the backup belongs to. This practice is especially useful in a case where the backup belongs to a volume that has been deleted (with backups retained).
