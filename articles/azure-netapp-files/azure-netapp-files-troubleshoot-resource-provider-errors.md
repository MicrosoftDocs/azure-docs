...
This error occurs when you are updating the capacity pool size, and the size is smaller than the total `usedBytes` value of all volumes in that capacity pool.  This error can also occurs when you are creating a new volume or resizing an existing volume, and the new volume size exceeds the free space in the capacity pool.
