...
An LDAP server provides UNIX user and group identities for use with NAS volumes. In Azure NetApp Files, Active Directory is currently the only supported LDAP server that can be used. This includes both Active Directory Domain Services (ADDS) and Azure Active Directory Domain Services (AADDS).
...
* LDAP servers can store user and group information as well as netgroup. However, Azure NetApp Files currently cannot leverage netgroup functionality in LDAP on Windows Active Directory.
...
* Azure NetApp Files also supports specifying preferred LDAP servers to help speed up requests. Use this if you want to ensure the LDAP server closest to your Azure NetApp Files region is being used.
...
* *Symmetric* name mapping is implicit name mapping between UNIX and Windows users who leverage the same user name; for example, Windows user `CONTOSO\user1` maps to UNIX user `user1`.  
...
By default, Azure NetApp Files will use symmetric name mapping rules. If asymmetric name mapping rules are required, consider configuring the LDAP user objects to use them.

## Custom name mapping using LDAP

LDAP can be a name mapping resource, if the LDAP schema attributes on the LDAP server have been populated. For example, to map UNIX users to corresponding Windows user names that do not match one-to-one (that is, *asymmetric*), you can specify a different value for `uid` in the user object than what is configured for the Windows user name.
...
For example, if a UID number (such as root=0) must be queried by Azure NetApp Files, then the schema attribute RFC 2307 `uidNumber Attribute` is used. If no UID number `0` exists in LDAP in the `uidNumber` field, then the lookup request will fail.

The schema type currently used by Azure NetApp Files is a form of schema based on RFC 2307bis and cannot be modified.
