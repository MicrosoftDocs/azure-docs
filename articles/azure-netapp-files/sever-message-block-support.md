...
Azure NetApp Files provides cloud-resident storage via a volumes as a service offering, using NAS protocols as the delivery mechanism to end users. SMB is one of the supported NAS protocols offered for use with Azure NetApp Files volumes and includes the following general capabilities:

- Centralized data for access by one or more clients simultaneously at any given time.
- Coordinated lock management to control access to files to prevent writes to locked files by users or applications who do not own the locks.
