...
An Azure NetApp Files snapshot is a point-in-time file system (volume) image. It can serve as an ideal online backup. You can use a snapshot to [create a new volume](snapshot-restore-to-new-volume.md), [restore a file](snapshot-restore-file-with-client.md), or [revert a volume](snapshot-revert-volume.md). In specific application data stored on Azure NetApp Files volumes, extra steps might be required to ensure application consistency.  
...
* Automated, by using:
    * Snapshot policies, via the [Azure portal](snapshot-manage-policy.md), [REST API](/rest/api/netapp/snapshotpolicies), [Azure CLI](/cli/azure/netappfiles/snapshot/policy), or [PowerShell](/powershell/module/az.netappfiles/new-aznetappfilessnapshotpolicy) tools
...
If the [Snapshot Path visibility](snapshot-edit-hide-path.md) is not set to `hidden`, users can directly access snapshots to recover from accidental deletion, corruption, or modification of their data. The security of files and directories are retained in the snapshot, and snapshots are read-only by design. As such, the restoration is secure and simple. 
...
See [Restore a file from a snapshot using a client](snapshot-restore-file-with-client.md) about restoring individual files or directories from snapshots.
...
See [Restore a snapshot to a new volume](snapshot-restore-to-new-volume.md) about volume restore operations.
...
See [Revert a volume using snapshot revert](snapshot-revert-volume.md) about how to use this feature.
...
See [Delete snapshots](snapshot-delete.md) about how to manage snapshot deletion. See [Manage snapshot policies](snapshot-manage-policy.md) about how to automate this process.
