...
* When you enable cool access on a newly created or nearly empty volume, there can be a short delay before tiering activity begins while the volume accumulates enough eligible data for the tiering engine to act on.
...
* During testing, a workload can appear to consume capacity without becoming eligible for tiering. This is most common with synthetic test data, which may not accumulate enough eligible data for the tiering engine to act on.
