...
the Azure Backup site on how to [Run SAP HANA native client backup to local disk on a database with Azure Backup enabled](../backup/sap-hana-db-manage.md#run-sap-hana-native-client-backup-to-local-disk-on-a-database-with-azure-backup-enabled).  
