...
:::image type="content" source="media/domain-name-system-concept/integrated-ad-dns.png" alt-text="Diagram of AD DNS configuration." lightbox="media/domain-name-system-concept/integrated-ad-dns.png:::
...
\* Active Directory DNS does not create these SRV records by default. It is highly recommended to create them if NFS Kerberos is used.
...
Considerations
In Azure NetApp Files, dynamic DNS updates will send two different requests to the configured DNS server – one for a PTR and one for an A/AAAA record creation/update. 
- Azure NetApp Files volumes created with hostnames will automatically notify the DNS server to create an A/AAAA record. This happens immediately after the volume creation completes.
- If a DNS A/AAAA record already is present for the IP address/hostname combination, then no new records will be created.
- If a DNS A/AAAA record is present for the same hostname with a *different* IP address, then a 2nd A/AAAA record with the same name will be created.
- Azure NetApp Files volumes created without hostnames (such as NFSv3 volumes), dynamic DNS will not create the DNS records since there is no hostname assigned in the service and the records must be manually created.
- If a reverse lookup zone for the interface’s IP subnet exists, then the DNS server also creates a PTR record. If the necessary reverse lookup zone doesn't exist, then a PTR record will not be created automatically, meaning you would need to create it manually. 
- If a DNS entry created by dynamic DNS is deleted on the DNS server, it will be recreated within 24 hours by a new dynamic DNS update from Azure NetApp Files.
- Secure DDNS gets enabled when SMB or dual protocol volumes are created. NFS volumes will not enable secure DDNS, but will enable DDNS. If secure DDNS is disabled on the DNS server or the Kerberos authentication fails, then DDNS updates will not work.
...
Packet captures can be useful in troubleshooting service issues that may not have available logging to analyze. The following examples show packet captures for both secure and standard DDNS updates from an Azure NetApp Files volume, mapped to the steps listed in the previous sections.

**Standard dynamic DNS packet capture**

| Step | Output | 
| - | ------ | 
| 1 | 1135 x.x.x.y 	x.x.x.x	DNS	86	Standard query 0xd29a SOA y.x.x.x.in-addr.arpa
...
**Secure dynamic DNS packet capture**
...
Azure NetApp Files makes use of DNS caches like any other standard DNS client. When the service requests a DNS record, that record has a TTL defined. By default, Active Directory DNS entries have a TTL of 600 seconds (10 minutes) unless specified otherwise. If a DNS record is updated and lives in the Azure NetApp Files cache and the TTL is 10 minutes, then the new record will not update in Azure NetApp Files until the cache is purged after the timeout value. There is currently no way to manually purge this cache. If a lower TTL is desired, make the change from the DNS server.
...
You can manually query a DNS record's TTL using `nslookup` or `dig` commands. For exmamples, see [Using nslookup and dig for DNS queries](#using-nslookup-and-dig-for-DNS-queries).
