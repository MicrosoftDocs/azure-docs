...
description: This article provides an introduction for the Azure Application Consistent Snapshot Tool that you can use with Azure NetApp Files. 
...
- **`-c backup`** is the primary command to execute database consistent storage snapshots for
    data (SAP HANA data volumes) & other (e.g. shared, logbackups, or boot) volumes.
...
- **`azacsnap.json`** the JSON configuration file.  The name can be customised by using the `--configfile` command line option and passing the config filename as a parameter (e.g `--configfile H80.json`)
