...
> [!NOTE]
> Throughput is calculated per volume, not per capacity pool. Volumes within the same pool that have different amounts of data in the cool tier will have different maximum throughput. This is expected behavior. To find any volume's maximum throughput, apply the formulas below to each volume separately.
