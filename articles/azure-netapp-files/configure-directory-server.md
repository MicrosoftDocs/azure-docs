...
* Authentication by using the specified Bind DN and password (simple authentication type) isn't supported in the government region.
* For LDAP over TLS, upload one server root CA certificate per file. Certificate bundles and chains, such as root and intermediate certificates in the same PEM file, aren't supported.
