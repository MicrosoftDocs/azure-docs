...
The capacity pool that you want to move the volume to must already exist and have sufficient capacity and throughput. The capacity pool can contain other volumes. If you want to move the volume to a brand-new capacity pool, you need to [create the capacity pool](azure-netapp-files-set-up-capacity-pool.md) before you move the volume. 
...
* After the volume is moved to another capacity pool, you no longer have access to the volume metrics. The volume starts with new activity logs and metrics under the new capacity pool.
