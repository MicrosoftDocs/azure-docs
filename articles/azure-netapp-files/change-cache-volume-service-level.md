...
The capacity pool that you want to move the cache volume to must already exist and have sufficient capacity and throughput. The capacity pool can contain other cache volumes. If you want to move the cache volume to a brand-new capacity pool, you need to [create the capacity pool](azure-netapp-files-set-up-capacity-pool.md) before you move the cache volume. 
