...
`groupMembershipFilter` can be specified with the “(gidNumber=*)” format. | Yes | None | Group membership filter will be used while querying group membership of a user from LDAP server. | None | 
...
For more information about SeSecurityPrivilege and SQL Server, see [SQL Server installation fails if the Setup account doesn't have certain user rights](/troubleshoot/sql/install/installation-fails-if-remove-user-right.md). | Allows non-administrator accounts to use SQL severs on top of ANF volumes.  | None | 
