...
* We have a capacity pool with 4 TiB of provisioned capacity.  It contains three volumes.  
...
*  The capacity pool is metered for 4 TiB of capacity (the provisioned amount).
    3.8 TiB of capacity is consumed (2 TiB and 1 TiB of quota from Volumes 1 and 2, and 800 GiB of actual consumption for Volume 3). 200 GiB of capacity is remaining.

   ![Capacity pool with three volumes](../media/azure-netapp-files/azure-netapp-files-capacity-pool-with-three-vols.png)

When the total used capacity of a pool exceeds its provisioned capacity, data writes are still permitted.  After a grace period of one hour, if the used capacity of the pool still exceeds its provisioned capacity, the size of the pool will be automatically increased in increments of 1 TiB until the provisioned capacity is greater than the total used capacity.  For example, in the illustration above, if Volume 3 continues to grow and the actual consumption reaches 1.2 TiB, then after the one-hour grace period, the pool will automatically be resized to 5 TiB.  The result is that the provisioned pool capacity (5 TiB) exceeds the used capacity (4.2 TiB).  
...
* Data writes will still be permitted (provided that the volume is below the system maximum of 100 TiB).
...
* The additional provisioned and billed pool capacity in excess of 500 TiB cannot be used to assign volume quota.  It also cannot be used to expand performance QoS limits. 
    See the [service levels](azure-netapp-files-service-levels.md) article about performance limits and QoS sizing.

The diagram below illustrates these concepts:
* We have a capacity pool with a Premium storage tier and a 500 TiB capacity. The capacity pool contains nine volumes.
