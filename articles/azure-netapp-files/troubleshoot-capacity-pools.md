...
| Error changing volume's pool. Pool: `'{target pool name}'` not available or does not exit | You can't change a volume's capacity pool when the destination capacity pool isn't healthy. Check the status of the destination capacity pool. If the pool is in a failed state (not "Succeeded"), try performing an update on the capacity pool by adding a tag name and value pair, then save. |
