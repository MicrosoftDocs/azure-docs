...
The general workflow of cross-zone replication (CZR) is similar to that of cross-region replication (CRR). If necessary, begin by familiarizing yourself with the concepts and requirements about setting up volume replication for data protection. See:
...
The requirements for using CZR are the same as the requirements for using CRR. See [Requirements and considerations for using cross-region replication](cross-region-replication-requirements-considerations.md) for a full list of requirements and considerations.
...
* To establish CZR, the source volume needs to be created in an availability zone.  
* You can’t use CZR and CRR together on the same source volume.
