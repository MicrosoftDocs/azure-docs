...
> [!NOTE]
> For Azure Database for MySQL, this feature is available in Preview in all regions of Azure public cloud.
...
However as of January 2018, the Azure Database for MySQL service is not yet among the services that can be assigned directly to a subnet.
...
- At present, an Azure Web App in a subnet that has **Service Endpoints** turned on does not yet function as expected. We are working on enabling this functionality.
    - Until this feature is fully implemented, we recommend that you move your Web App to a different subnet that does not have service endpoints turned on for Azure Database for MySQL.
...
- Turning ON virtual network service endpoints to Azure Database for MySQL also enables the endpoints for the Azure Database for MySQL and Azure SQL Database services.
