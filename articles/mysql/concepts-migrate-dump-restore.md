...
-	Use the `exclude-triggers` option in mysqldump when dumping database. Exclude triggers from dump files to avoid those triggering upon restore, since triggers can be added explicitly after restoring is complete. 
-	Avoid the `single-transaction` option in mysqldump when dumping database. Dumping tables within a single transaction causes extra storage and memory resources to be consumed during restore and may cause performance delays or resource constraints.
-	Use multi-value inserts when loading with SQL to minimize statement execution overhead when dumping databases. When using dump files generated by mysqldump utility, this is done automatically. 
-  Use the `order-by-primary` option in mysqldump when dumping database, so that the data is scripted in primary key order.
-	Use the `disable-keys` option in mysqldump when dumping data. This disables foreign key constraints before load. Disabling foreign key checks will provide significant performance gains. Enable the constraints and verify the data after the load to ensure referential integrity.
...
-	Use the `defer-table-indexes` option in mysqlpump when dumping database. This scripts the index creation to happen after loading table rows. 
