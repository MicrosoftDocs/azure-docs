...
Creating the database first, can help you identify which availability zone the server is in and then create the AKS clusters in the same availability zone. This can improve performance for the application by reducing networking latency.
...
## Use Azure premium fileshare
 Use [Azure premium fileshare](../storage/files/storage-how-to-create-premium-fileshare.md?tabs=azure-portal) for persistent storage that can be used by one or many pods, and can be dynamically or statically provisioned. To learn more , see [how to enable Azure Files](../aks/azure-files-dynamic-pv.md).
