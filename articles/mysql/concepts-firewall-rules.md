...
It is recommended that you find the outgoing IP address of any application or service and explicitly allow access to those individual IP addresses or ranges. For example, you can find the outgoing IP address of an Azure App Service or use a public IP tied to a virtual machine or other resource (see below for info on connecting with a virtual machine's private IP over service endpoints). You can also enable connections from all IP addresses or from all Azure datacenter IP addresses. A firewall setting with starting and ending address equal to 0.0.0.0 indicates that connections from any IP address are allowed. Alternatively, you can allow access from all Azure datacenter IP address by setting the **Allow access to Azure services** option to **ON** in the portal from the **Connection security** pane and hitting **Save**. If the connection attempt is not allowed, the request does not reach the Azure Database for MySQL server.
