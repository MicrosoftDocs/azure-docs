...
However, it’s important to note that once a burstable instance exhausts its CPU credits, it operates at its base CPU performance. For example, the base CPU performance of a Standard_B1s is 20% that is, 0.2 Vcore. If Burstable tier server is running a workload that requires more CPU performance than the base level, and it has exhausted its CPU credits, the server may experience performance limitations and eventually system operations like (Stop/Start/Restart) of server may get affected.

And due to this throttling and server may have connectivity and system operations like (Stop/Start/Restart) issues, the best action for such scenarios is to stop workload on server to gain credit per [credit banking formula](../../virtual-machines/b-series-cpu-credit-model/b-series-cpu-credit-model.md#b-series-cpu-credit-model) or [scale server](./how-to-manage-server-portal.md#scale-compute-and-storage) to General Purpose or Business Critical tier Tiers.
