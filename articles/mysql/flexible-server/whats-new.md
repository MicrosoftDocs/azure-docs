...
- **Known issue**
  ...
  - If you have an existing server with storage provisioned less than 20GB in size, and if you try to perform a compute scale up or down operation, the compute scaling operation will fail. This issue will be fixed in the next release of Flexible Server. You can workaround the issue by scaling up the provisioned storage to 20GB and retrying the compute scaling operation.
