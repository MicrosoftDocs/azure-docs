...
It is not supported to configure Data-in replication for servers which have high availability (HA) option enabled. On servers were HA is enabled, the stored procedures for replication `mysql.az_replication_*` won't be available. 
> [!Tip]
