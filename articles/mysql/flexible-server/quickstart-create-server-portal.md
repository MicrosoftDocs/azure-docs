...
description: This article walks you through using the Azure portal to create an Azure Database for MySQL - Flexible Server instance in minutes.
ms.service: mysql
...
6. Select **Review + create** to review your Azure Database for MySQL flexible server configuration.

7. Select **Create** to provision the server. Provisioning can take a few minutes.

8. Select **Notifications** on the toolbar (the bell button) to monitor the deployment process. After the deployment is done, you can select **Pin to dashboard**, which creates a tile for the Azure Database for MySQL flexible server instance on your Azure portal dashboard. This tile is a shortcut to the server's **Overview** page. When you select **Go to resource**, the server's **Overview** page opens.
...
## Clean up resources
When no longer needed, you can delete the resource group with the Azure Database for MySQL flexible server instance. To do so, select the resource group for the Azure Database for MySQL flexible server resource and select **Delete**, then confirm the name of the resource group to delete.
...
> [!div class="nextstepaction"]
> [Connect to an Azure Database for MySQL flexible server instance in a virtual network](./quickstart-create-connect-server-vnet.md)
