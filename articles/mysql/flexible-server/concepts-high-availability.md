...
Azure Database for MySQL Flexible Server (Preview), allows configuring high availability with automatic failover using two different options, zone redundant high availability and same-zone high availability. When high availability is configured, flexible server automatically provisions and manages a standby replica in the same or a different availability zone.
