...
**MySQL on Azure VM** falls into the industry category *Infrastructure-as-a-Service (IaaS)* and allows you to run MySQL server inside a virtual machine (VM) in Azure cloud. MySQL on Azure virtual machines (VM) is a good option for migrating on-premises MySQL databases and applications without any database change. All recent versions and editions of MySQL can be installed on an IaaS virtual machine. The most significant difference from **Azure Database for MySQL** is that **MySQL on Azure VMs** allow full control over the database engine. You can choose when maintenance/patching will start, to change the recovery model, to pause or start engine when needed. With this additional control comes added responsibility to manage the virtual machines and numerous DBA tasks.
