...
* **Time to move to Azure** – MySQL on Azure VM is the exact match of your environment, so migration from on-premises to Azure VM is not different than moving the databases from one on-premises server to another. Managed(Paas) instance also enables extremely easy migration; however, there might be some changes that you need to apply before you migrate your databases.
