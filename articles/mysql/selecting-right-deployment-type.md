...
| **High Availability** | The high availability (HA) model is based on built-in fail-over mechanisms when a node-level interruption occurs. In such cases, the database server automatically creates a new node and attaches data storage to the new node. | High Availability is architected, implemented, tested, and maintained by the customer. This may include, Always-On (Failover Clustering or Group replication), Log Shipping, Transactional Replication depending on the version of MySQL engine that is being used.|
...
In addition, you are billed for outgoing Internet traffic at regular [data transfer rates](https://azure.microsoft.com/pricing/details/data-transfers/). 
...
With MySQL on Azure VMs you can use any of the MySQL version and run on the VMs. Regardless of the MySQL version you use, you pay for the VM provisioned and the costs for the specific license type used for MySQL. For more information on bring-your-own licensing, see [License Mobility through Software Assurance on Azure](https://azure.microsoft.com/pricing/license-mobility/).
