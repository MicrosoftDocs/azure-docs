...
>[!NOTE]
>This article is only applicable to Azure Database for MySQL Single Server to ensure drivers are compatible with [connectivity architecture](concepts-connectivity-architecture.md) of Single Server service. [Azure Database for MySQL Flexible Server](/../flexible-server/overview.md) is compatible with all the drivers and tools supported and compatible with the MySQL community edition. 
