...
The replication lag on the secondary read replicas depends on number of factors. These factors include but aren't limited to: 

- Network latency.
- Transaction volume on the source server.
- Compute tier of the source and secondary read replica server.
- Queries running on the primary and secondary server. 

In this article, you'll learn on how to troubleshoot replication latency in Azure Database for MySQL. You'll also understand some common causes of increased replication latency on replica servers.

## Replication concepts

When binary log is enabled, the source server writes committed transactions into the binary log, which is used for replication. The binary log is turned on by default for all newly provisioned servers that support up to 16 TB of storage. On replica servers, two threads run in each replica server. One thread is the *IO thread*, and the other is the *SQL thread*:

- The IO thread connects to the source server and requests updated binary logs. This thread receives the binary log updates, and then they're saved on a replica server, in a local log called the *relay log*.
- The SQL thread reads the relay log and then applies the data changes on replica servers.

## Monitoring replication latency

Azure Database for MySQL provides the replication lag in seconds in [Azure Monitor](concepts-monitoring.md). This metric is available only on read replica servers. The metric is calculated by the seconds_behind_master metric that's available in MySQL. 

To understand the cause of increased replication latency, connect to the replica server by using [MySQL Workbench](connect-workbench.md) or [Azure Cloud Shell](https://shell.azure.com). Then execute following command.

>[!NOTE] 
> In your code, replace the values with your replica server name and admin username. The admin username requires `@\<servername>` for Azure Database for MySQL.
...
|Metric|Description|
|---|---|
|Slave_IO_State| Represents the current status of the IO thread. Normally, the status is "Waiting for master to send event" if the primary (master) server is synchronizing. A status such as "Connecting to master" indicates that the replica lost the connection to the primary server. Make sure the primary server is running, or check to see whether a firewall is blocking the connection.|
|Master_Log_File| Represents the binary log file to which the primary server is writing.|
|Read_Master_Log_Pos| Indicates where the primary server is writing in the binary log file.|
|Relay_Master_Log_File| Represents the binary log file that the replica server is reading from the primary server.|
...
|Slave_SQL_Running_State| Indicates the current SQL thread status. In this state, `System lock` is normal. It's also normal to see a status of `Waiting for dependent transaction to commit`. This status indicates that the replica is waiting for the primary server to update committed transactions.|

If Slave_IO_Running is `Yes` and Slave_SQL_Running is `Yes`, then the replication is running fine. 

Next, check Last_IO_Errno, Last_IO_Error, Last_SQL_Errno, and Last_SQL_Error.  These fields display the error number and error message of the most-recent error that caused the SQL thread to stop. An error number of `0` and an empty message means there's no error. Investigate any nonzero value in the error by looking up the error code in the [MySQL server error message reference](https://dev.mysql.com/doc/mysql-errors/5.7/en/server-error-reference.html).
...
In this case, the IO thread is running and is waiting on the primary server. The primary server has already written to binary log file number 20. The replica has received only up to file number 10. The primary factors for high replication latency in this scenario are network speed or high CPU utilization on the source server.  
...
The output shows that the replica can retrieve the binary log behind the primary server. But the replica IO thread indicates that the relay log space is full already. 

Network speed isn't causing the delay. The replica has been trying to catch up as fast as it can. But the updated binary log size exceeds the upper limit of the relay log space. 

To troubleshoot this issue, enable the [slow query log](concepts-server-logs.md) on the primary server. Use slow query logs to identify long-running transactions on the source server. Then tune the identified queries to reduce the latency on the server. 
...
We recommend that you add a primary key on tables in the source server before you create the replica server. Add primary keys on the source server and then recreate read replicas to help improve replication latency.
...
Slow queries can increase resource consumption or slow down the server so that the replica can't catch up with the primary server. In this scenario, tune the slow queries. Faster queries prevent blockage of the SQL thread and improve replication latency significantly.


#### DDL queries on the source server
On the source server, a long-running data definition language (DDL) command like [`ALTER TABLE`](https://dev.mysql.com/doc/refman/5.7/en/alter-table.html) can take a long time. While a DDL command is running, thousands of other queries might be running in parallel on the source server. 

When the DDL is replicated, to ensure database consistency, the MySQL engine has to run the DDL in a single replication thread. During this task, all other replicated queries are blocked and must wait until the DDL operation finishes on the replica server. Even online DDL operations cause this delay. DDL operations increase replication latency.

If you enabled the [slow query log](concepts-server-logs.md) on the source server, you can detect this problem by checking for a DDL command that ran on the source server. Through index dropping, renaming, and creating, you can use the INPLACE algorithm for the ALTER TABLE. You might need to copy the table data and rebuild the table. 
...
Unfortunately, for a DDL statement that requires a lock, you can't avoid replication latency. To reduce the potential effects, perform these types of DDL operations during off-peak hours, for instance during the night.

#### Downgraded replica server

In Azure Database for MySQL, read replicas use the same server configuration as the primary server. You can change the replica server configuration after it has been created. 

If the replica server is downgraded, the workload can consume more resources, which in turn can lead to replication latency. To find this problem, use Azure Monitor to check the CPU and memory consumption of the replica server. 

In this scenario, we recommend that you keep the replica server's configuration at values equal to or greater than the values of the source server. In this configuration, the replica can keep up with the primary server.
...
For low-concurrency workloads that include many singleton transactions, you can set binlog_group_commit_sync_delay to add to the latency. The IO thread waits for bulk binary log updates, and only a few transactions might be committed. 
