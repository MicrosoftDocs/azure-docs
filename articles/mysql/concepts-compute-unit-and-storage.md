---
title: Explaining Azure Database for MySQL Compute Units and Storage  | Microsoft Docs
description: Explains Compute Unit and Storage options and what happens when you hit the maximum Compute Unit or Storage.
services: mysql
author: v-chenyh
ms.author: v-chenyh
manager: jhubbard
...
The amount of memory per Compute Unit is optimized for the Basic and Standard service tiers. Doubling the Compute Units by increasing the performance level equates to doubling the set of resource available to that single Azure Database for MySQL. 
...
Azure Database for MySQL server provides your business and applications flexibility in scale and cost. You can create one database per server, or use multiple databases within a single server, and dial the performance of the server up or down flexibly to meet your demands.
...
## Configuring Storage
The Storage configuration defines the amount of storage capacity available to an Azure Database for MySQL server.  Consider the size of storage needed to host your databases and the performance requirements (IOPS) when selecting the Storage configuration.
...
If your existing on-premise or virtual machine server is currently utilizing 4 cores (without counting CPU hyperthread), start by configuring 400 Compute Units in your Azure Database for MySQL server. Compute Units can be dynamically scaled up or down depending on your workload needs with virtually no application downtime. 
...
If your workload reaches the maximum limits on number of connections, explicit errors are raised. See [Limitations in Azure Database for MySQL](concepts-limits.md) for more information on resources limits.

## Next steps
- For more information on Compute Units and Storage, see [Azure Database for MySQL service tiers](./concepts-service-tiers.md).
