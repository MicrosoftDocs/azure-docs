...
# Note that a sink must be configured for each source.
- source_id: sftp-source01
  source:
    sftp:
      ...
      # The name of the user on the SFTP server which the agent should use to connect.
      user: sftp-user
      auth: 
        ...
        # Optional. Only for use with 'type: ssh_key'. The passphrase for the SSH key. This can be omitted if the key   is not encrypted.
        passphrase_secret_name: sftp-user-ssh-key-passphrase
    # Optional. A regex pattern to specify which files in the base_path folder which should be ingested. If not specified, the STFP agent will attempt to ingest all files in the base_path folder (subject to exclude_pattern, settling_time_secs and exclude_before_time).
    include_pattern: "pmstats[0-9].*.txt$"
    # Optional. A regex pattern to specify any files in the base_path folder which should not be ingested. Takes priority over include_pattern, so files which match both regex patterns will not be ingested.
    exclude_pattern: '\.backup$'
    # A duration in seconds. When the handler runs, any files last modified within the settling time are not selected for upload, as they may still be being modified.
    settling_time_secs: 60
    # A datetime that adheres to the RFC 3339 format. Any files last modified before this datetime will be ignored by the handler.
    exclude_before_time: "2022-12-31T21:07:14-05:00"
    # An expression in cron format, specifying when the file source handler should run for this source. All times refer to UTC. The cron schedule should include fields for: second, minute, hour, day of month, month, day of week, and year. E.g.:
  ...
  # A string giving the name of the Azure Blob Storage container to use with this file source. Each data type must be sent to a specific container. e.g. MCC EDRs are sent to the `edrs` container.
  container_name: edrs
