...
| Private Git repo – SSH – Flux-created keys | ssh://[user@]server/repo.git or [user@]server:repo.git | Public key generated by Flux needs to be added to the user account in your Git service provider. If the deploy key is added to repo instead of user account, use `git@` in place of `user@` . More details can be found [here](#apply-configuration-from-a-private-git-repository) |
...
**Option 1: Add the public key to your user account**
