---
title: "Tutorial: Use GitOps with Flux v2 in Azure Arc-enabled Kubernetes or AKS clusters"
...
# Tutorial: Use GitOps with Flux v2
...
The `microsoft.flux` extension will be installed on the cluster if it isn't already. When the flux configuration is first installed, the initial compliance state may be `Pending` or `Non-compliant` because reconciliation is still ongoing. 
...
After a minute or so, query the configuration again to see the final compliance state.
