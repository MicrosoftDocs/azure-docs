...
| **Control Plane IP address** | Azure Arc resource bridge (preview) runs a Kubernetes cluster, and its control plane always requires a static IP address. Provide a static IP address that meets the following requirements. The IP address must: <ul> <li>have internet access. </li><li>be a part of the range defined by IP address prefix.</li> <li>be outside of the IP address range provided for the VM (Start range IP - End range IP) if you are using static IP address option for resource bridge VM IP address.</li> <li> be outside of DHCP range if there is a DHCP service on the network. </li> </ul>|
