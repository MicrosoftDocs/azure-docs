...
> [!IMPORTANT]
> This article describes a way to connect a generic vCenter Server to Azure Arc. If you are trying to enable Arc for Azure VMware Solution (AVS) private cloud to Azure Arc, please follow this guide instead - [Deploy Arc for Azure VMware Solution](../../azure-vmware/deploy-arc-for-azure-vmware-solution.md). That onboarding process is specifically built for AVS private clouds and is therefore more straightforward and enables Arc capabilities to show up in AVS private cloud portal experiences. 
