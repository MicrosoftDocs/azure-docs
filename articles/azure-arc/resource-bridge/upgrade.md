...
The outbound connection from the Appliance VM IPs (`k8snodeippoolstart/end`, VM IP 1/2) to `msk8s.sb.tlu.dl.delivery.mp.microsoft.com`, port 443 should be enabled. Be sure the full list of [required endpoints for Arc resource bridge](network-requirements.md) are also enabled.

If you are performing a manual upgrade, the upgrade command should be run from the management machine used to initially deploy the Arc resource bridge or one that meets the [management machine requirements](system-requirements.md#management-machine-requirements).
