...
1. If your subscription is not registered with all the required resource providers, select **Register** to proceed to next step.
...
1. To see the status of your onboarding after you run the script on your workstation, select **Next:Verfication**. The onboarding is not affected when you close this page.
...
1. Navigate to the folder where you have downloaded the PowerShell script.
...
## Script runtime
The script execution will take up to half an hour and you will be prompted for various details. See the following table for related information:
...
| **Appliance proxy settings** | Type ‘Y’ if there is proxy in your appliance network, else type ‘N’.|
...
1. If for any reason, the appliance creation fails, you need to retry it. Run the command with ```-Force``` to clean up and onboard again.
