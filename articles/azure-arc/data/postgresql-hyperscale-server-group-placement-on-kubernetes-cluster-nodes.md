...
description: How are the PostgreSQL instances forming a PostgreSQL Hyperscale server group placed on the Kubernetes cluster nodes?
services: azure-arc
...
# Azure Arc enabled PostgreSQL Hyperscale server group placement on a Kubernetes cluster explained.
...
In this example we are using an Azure Kubernetes (AKS) that has four physical nodes. 

> [!WARNING] Mike insert bitmap1 here
...
> [!WARNING] Mike insert bitmap2 here
...
> [!WARNING] Mike insert bitmap3 here
...
## Scale out the Azure Arc enabled PostgreSQL Hyperscale
...
And notice that the pod of the new worker (postgres01-3) has been placed on the same node as the coordinator. The architecture looks like:

> [!WARNING]  Mike insert bitmap4 here
...
_*the suffix on pod names will vary on other deployments. Also, we are listing here only the pods hosted inside the Kubernetes namespace of the Azure Arc Data Controller._

The architecture looks like:

> [!WARNING] Mike insert bitmap5 here
...
> [!WARNING]  Mike insert bitmap6 and 7 here, side by side in a table
 	 

The architecture looks like:

> [!WARNING] Mike insert bitmap8 here
...
> [!WARNING]  Mike insert bitmap9 here
...
Namespace:    arc3
Priority:     0
Node:         aks-agentpool-42715708-vmss000004
...
> [!WARNING] Mike insert bitmap10 here
