---
title: Placement of a PostgreSQL server on the Kubernetes cluster nodes
description: Explains how PostgreSQL instances forming a PostgreSQL server are placed on the Kubernetes cluster nodes
services: azure-arc
...
# Azure Arc-enabled PostgreSQL server placement

In this article, we are taking an example to illustrate how the PostgreSQL instances of Azure Arc-enabled PostgreSQL server are placed on the physical nodes of the Kubernetes cluster that hosts them. 
...
The Kubernetes cluster hosts one Azure Arc Data Controller and one Azure Arc-enabled PostgreSQL server. 
...
```
Each of those pods host a PostgreSQL instance. Together, the pods form the Azure Arc-enabled PostgreSQL server:
...
Each pod that is part of the Azure Arc-enabled PostgreSQL server hosts the following three containers:
...
It means that, at this point, each PostgreSQL instance constituting the Azure Arc-enabled PostgreSQL server is hosted on specific physical host within the Kubernetes container. This configuration provides the most performance out of the Azure Arc-enabled PostgreSQL server as each role (coordinator and workers) uses the resources of each physical node. Those resources are not shared among several PostgreSQL roles.

## Scale out Azure Arc-enabled PostgreSQL server
...
|----|----|----
|bootstrapper-jh48b|A service which handles incoming requests to create, edit, and delete custom resources such as SQL managed instances, PostgreSQL servers, and data controllers|aks-agentpool-42715708-vmss000003
...
The implication of hosting too many workloads on the Kubernetes cluster is throttling may happen for the Azure Arc-enabled PostgreSQL server. If that happens, you will not benefit so much from its capability to scale horizontally. The performance you get out of the system is not just about the placement or the physical characteristics of the physical nodes or the storage system. The performance you get is also about how you configure each of the resources running inside the Kubernetes cluster (including Azure Arc-enabled PostgreSQL server), for instance the requests and limits you set for memory and vCore. The amount of workload you can host on a given Kubernetes cluster is relative to the characteristics of the Kubernetes cluster, the nature of the workloads, the number of users, how the operations of the Kubernetes cluster are done…

## Scale out AKS

Let’s demonstrate that scaling horizontally both the AKS cluster and the Azure Arc-enabled PostgreSQL server is a way to benefit the most from the high performance of Azure Arc-enabled PostgreSQL server.
...
## Scale out Azure Arc-enabled PostgreSQL server again

The fifth physical node is not hosting any workload yet. As we scale out the Azure Arc-enabled PostgreSQL server, Kubernetes will optimize the placement of the new PostgreSQL pod and should not collocate it on physical nodes that are already hosting more workloads. 
Run the following command to scale the Azure Arc-enabled PostgreSQL server from 3 to 4 workers. At the end of the operation, the server group will be constituted and distributed across five PostgreSQL instances, one coordinator and four workers.
...
- between the Azure Arc-enabled PostgreSQL server and other workloads hosted on the same Kubernetes cluster
- between all the PostgreSQL instances that constitute the Azure Arc-enabled PostgreSQL server

You can achieve this in several ways:
- Scale out both Kubernetes and Azure Arc-enabled PostgreSQL server: consider scaling horizontally the Kubernetes cluster the same way you are scaling the Azure Arc-enabled PostgreSQL server. Add a physical node to the cluster for each worker you add to the server group.
- Scale out Azure Arc-enabled PostgreSQL server without scaling out Kubernetes: by setting the right resource constraints (request and limits on memory and vCore) on the workloads hosted in Kubernetes (Azure Arc-enabled PostgreSQL server included), you will enable the colocation of workloads on Kubernetes and reduce the risk of resource contention. You need to make sure that the physical characteristics of the physical nodes of the Kubernetes cluster can honor the resources constraints you define. You should also ensure that equilibrium remains as the workloads evolve over time or as more workloads are added in the Kubernetes cluster.
- Use the Kubernetes mechanisms (pod selector, affinity, anti-affinity) to influence the placement of the pods.

## Next steps

[Scale out your Azure Arc-enabled PostgreSQL server by adding more worker nodes](scale-out-in-postgresql-hyperscale-server-group.md)
