...
Set a maintenance window for the data controller, enabling all SQL Managed Instances attached to the data controller that have the `desiredVersion` set to `auto` to be upgraded in a specific time. See [Maintenance Window](maintenance-window.md).
...
- SQL MI deployment wizard now reflects the exact wording of the experience in Portal 
- SQL MI deployment wizard now has fields in the same order as in Portal 
