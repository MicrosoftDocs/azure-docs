...
# Scenario: Backup and Restore for Azure Database for PostgreSQL Hyperscale server groups
...
First of all, we have to provision a backup volume for each node in the server group. We can do this by editing the server group configuration:
...
You will then see that the system provisions backup volumes next to the data volumes, one for each node:
...
You can also specify an existing persistent volume claim like this:
...
## Running and restoring a manual backups

We can now issue a manual backup like this:
...
We can verify that the backup completed by using the `azdata postgres backup list` command:
...
Now, we can restore this backup:
...
This will put the server group in the state `Pending` - once it is back in `Running`, the backup was restored:
...
Now, we'd like to run backups on a scheduled basis, instead of triggering them manually.

There are two kinds of backups:

* **Full backups** - This is the same as we've executed earlier, and performs a complete physical copy of the PostgreSQL data directory
* **Delta backups** - This backs up the PostgreSQL WAL archive, and is required for Point-in-time-Restore between full backups (commonly you would set this to a low number, such as once per minute)

For testing, we'll go with a very low full backup setting, and a standard delta backup setting:
...
To avoid our backup volume running out of storage, we can configure backup retention settings, to automatically remove backups after a certain amount of time.

For example, to do so after 7 days:
...
To perform a point-in-time restore, specify the -t parameter to the "azdata postgres server restore" command. You can either specify a timestamp (e.g. "2019-12-17 17:34:02", expressed in your local time) or a time span (e.g. "30m", "6h", "2.5d", or "1w" for 30 minutes, 6 hours, 2.5 days, or 1 week, respectively).
x`
...
You must also specify the -f parameter and give the ID of the server to restore from. The ID of a server can be obtained from "azdata postgres server list". You can restore from a server even after it's been deleted, if you know its ID. In a later release we will allow specifying servers by name in addition to ID.
...
You can delete a backup from just one tier rather than deleting it from all tiers. Most backup-related commands accept a --tier parameter, which can either be the ordinal of a tier (e.g. 1, 2, 3…) to restrict the command to, or "all" (the default) to operate on all tiers
