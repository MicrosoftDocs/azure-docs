...
```console
az postgres arc-server backup create [--name <backup name>] --server-name <server group name> [--no-wait] 
```
Where:
...
```console
az postgres arc-server backup create --name backup12082020-0250pm --server-name postgres01
```
...
```console
az postgres arc-server backup list --server-name <servergroup name>
```

For example:

```console
az postgres arc-server backup list --server-name postgres01
```
...
```console
az postgres arc-server backup restore --server-name <target server group name> [--source-server-name <source server group name> --backup-id <backup id>]
or
az postgres arc-server backup restore -sn <target server group name> [-ssn <source server group name> --backup-id <backup id>]
```
<!--To read the general format of restore command, run: az postgres arc-server backup restore --help -->
...
```console
az postgres arc-server backup restore -sn postgres01 --backup-id d134f51aa87f4044b5fb07cf95cf797f
```
...
```console
az postgres arc-server backup restore -sn postgres02 -ssn postgres01 --backup-id d134f51aa87f4044b5fb07cf95cf797f
```
...
```console
az postgres arc-server backup restore --server-name <target server group name> --source-server-name <source server group name> --time <point in time to restore to>
or
az postgres arc-server backup restore -sn <target server group name> -ssn <source server group name> -t <point in time to restore to>
```
...
```console
az postgres arc-server backup restore -sn postgres02 -ssn postgres01 -t "2020-12-08 04:23:48.751326+00"
``` 
...
```console
az postgres arc-server backup restore -sn postgres02 -ssn postgres01 -t "22m"
```
...
```console
az postgres arc-server backup delete  [--server-name, -sn] {[--name, -n], -id}
```
...
```console
az postgres arc-server backup delete -sn postgres01 -n MyBackup091720200110am
{
...
```console
az postgres arc-server backup delete --help
```
