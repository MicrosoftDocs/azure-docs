...
The deployment templates that are provided out of the box have a default storage class specified that is appropriate for the target environment, but it can be overridden at deployment time.  See the detailed steps to [alter the deployment profile](/data-controller) to change the storage class configuration for the data controller pods at deployment time.
