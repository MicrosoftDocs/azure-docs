...
A Red Hat pull secret enables your cluster to access Red Hat container registries along with other content such as operators from [OperatorHub](https://operatorhub.io/). This step is optional but recommended (if you decide add the pull secret later, follow [this guidance](howto-add-update-pull-secret.md)). The field `cloud.openshift.com` is removed from your secret even if your pull-secret contains that field. This field enables an extra monitoring feature, which sends data to RedHat and is thus disabled by default. To enable this feature, see https://docs.openshift.com/container-platform/4.11/support/remote_health_monitoring/enabling-remote-health-reporting.html . 
