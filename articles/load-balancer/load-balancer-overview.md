...
- In general, forwarding IP fragments or performing IP fragmentation of UDP and TCP packets are not supported on load balancing rules.  [HA Ports load balancing rules](ad-balancer-ha-ports-overview.md) are an exception to this general statement and can be used to forward existing IP fragments.
