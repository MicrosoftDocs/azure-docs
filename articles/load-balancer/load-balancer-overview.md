...
- Load balancing rules or inbound NAT rules are supported for TCP and UDP and not supported for other IP protocols including ICMP. Load Balancer is a TCP or UDP product offering load balancing for these specific IP protocols.  Load Balancer does not terminate, respond, or otherwise interact with the payload of a UDP or TCP flow. It is not a proxy. Successful validation of connectivity to a frontend must take place in-band with the same protocol used in a load balancing or inbound NAT rule (TCP or UDP) _and_ at least one of your virtual machines must generate a response for a client to see a response from a front-end.  Not receiving a response from the Load Balancer frontend indicates no virtual machines were able to respond.
