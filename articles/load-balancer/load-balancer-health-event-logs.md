...
| **Warning** | The load balancer resource can need to be monitored or reviewed. The functionality of the load balancer can be impacted in certain scenarios or operating in a partially degraded state. |
...
Once a health event has been published, there will be an extended window of time where the event won't be republished to avoid redundant logs due to a persisting issue. Following this redetection interval, the health event is republished if the issue still persists. 
