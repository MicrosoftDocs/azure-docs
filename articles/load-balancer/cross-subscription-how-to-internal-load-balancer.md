...
```
This section updates the backend pool you created earlier by attaching the cross-subscription virtual network and enabling the `syncMode` property required for cross-subscription backends. For details on the `syncMode` requirement, see [Configure the backend address pool and syncMode property](cross-subscription-how-to-attach-backend.md#configure-the-backend-address-pool-and-syncmode-property).
