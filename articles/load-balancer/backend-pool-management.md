...
### Limitations
  ...
  * If the backend pools are constantly changing, there is a chance that existing connections will get rehashed and sent to a new backend pool member. This may cause reset signals sent back to the source from the backend resource. To avoid resets, you can use retries as a workaround.
