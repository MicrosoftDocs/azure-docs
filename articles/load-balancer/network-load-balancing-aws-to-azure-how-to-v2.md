...
If you're currently using AWS Network Load Balancer (NLB) and planning to migrate your workload to Azure, this guide helps you understand the migration process, feature mappings, and best practices. 

On Azure, [Azure Load Balancer](load-balancer-overview.md) provides low-latency Layer 4 load balancing capabilities. It enables you to manage TCP and UDP traffic to your applications. You'll learn how to assess your current environment, plan and prepare the migration, and execute the transition while maintaining application availability and performance.
...
In this example, a multi-player gaming platform uses AWS Network Load Balancer (NLB) to handle both TCP and UDP traffic simultaneously from game clients. 

The workload's architecture includes:
- Session management services running on EC2 instances handling player authentication and lobbies over TCP on port 7777
- Real-time game data services deployed on EC2 instances processing low-latency gameplay packets over UDP on port 7778
...
This architecture example showcases common network load balancing features in AWS and Azure. These include multi-protocol support, static IP addresses, cross-zone distribution, and client IP preservation. 

The goal is to migrate this architecture from AWS Network Load Balancer to Azure Load Balancer while maintaining equivalent functionality and performance. In our architecture diagram, TCP traffic represents game session management services and UDP traffic represents real-time game data services.
...
- **Cross-zone load balancing**: Hash-based traffic distribution across availability zones (distribution depends on client connection patterns, session persistence settings, and traffic entropy; Azure Load Balancer doesn't guarantee even distribution across zones due to its hash-based algorithm)
- **Client IP preservation**: Original client IP addresses maintained for analytics and anti-cheat systems
- **Low latency**: Azure Load Balancer provides low latency Layer 4 load balancing with minimal processing overhead; actual latency depends on network topology, VM performance, datacenter design, and geographic proximity. Azure Load Balancer has no performance SLA for latency or throughput
- **High throughput**: Azure Load Balancer handles millions of flows for TCP and UDP applications with appropriate VM sizes and configuration; actual capacity depends on SKU, VM network limits, and tuning. Azure Load Balancer has no specific throughput SLA
- **Advanced health monitoring**: TCP and HTTP/HTTPS health probes. Azure Load Balancer doesn't support native UDP health probing - UDP services must use TCP or HTTP health probes on alternate ports
...
- **Monitoring capabilities**: Detailed metrics, access logs, and health monitoring for troubleshooting and optimization

## Step 1: Assessment

Before migrating from AWS Network Load Balancer to Azure Load Balancer, it's crucial to assess the existing architecture and identify the capabilities that need to be mapped or replaced. This assessment helps facilitate an effective migration process and maintain the functionality of your gaming platform.
...
| **[AWS NLB Protocol Support (TCP/UDP)](https://docs.aws.amazon.com/elasticloadbalancing/latest/network/load-balancer-target-groups.html#target-group-routing-configuration)** | **[Load Balancer TCP/UDP Support](load-balancer-overview.md)** | Configure load balancing rules for both TCP and UDP protocols. Azure Load Balancer supports TCP and UDP protocols; HA Ports rules (all ports load balancing) are available only on internal Standard Load Balancers, not on public load balancers. Public load balancers require specific port configurations for each rule. Create separate rules for different ports and protocols as needed for your services. Note: Health probes don't support UDP so custom health probes utilizing TCP or HTTP/HTTPS must be used. |
...
| **[AWS NLB Cross-zone Load Balancing](https://docs.aws.amazon.com/elasticloadbalancing/latest/network/availability-zones.html#cross-zone-load-balancing)** | **[Load Balancer availability zone support](../reliability/reliability-load-balancer.md)** | Enable zone redundancy on Load Balancer to automatically distribute traffic across all availability zones. Zone-redundant deployment provides automatic failover and hash-based traffic distribution. Azure Load Balancer doesn't guarantee even load distribution across zones due to its hash-based algorithm - distribution evenness depends on traffic entropy and client connection patterns. Configure backend pools with VMs distributed across multiple zones for improved fault tolerance. |
| **[AWS NLB Client IP Preservation](https://docs.aws.amazon.com/elasticloadbalancing/latest/network/edit-target-group-attributes.html#client-ip-preservation)** | **[Load Balancer Floating IP (DSR)](load-balancer-floating-ip.md)** | Enable Floating IP (Direct Server Return) on load balancing rules to preserve client IP addresses. Azure Floating IP requires additional guest OS configuration (for example, a loopback interface and OS network settings) while AWS client IP preservation is enabled by default for instance targets. Configure DSR when targets are in the same VNet/subnet to ensure backend servers receive real client IP addresses for analytics and security systems. **Note:** With Floating IP enabled, outbound behavior can be affected—the application must use the primary IP configuration for outbound traffic and binding to the frontend loopback IP can prevent outbound SNAT translation. Consider using an Azure NAT Gateway or explicit outbound rules to provide predictable outbound connectivity when using Floating IP. |
| **[AWS NLB Health Checks](https://docs.aws.amazon.com/elasticloadbalancing/latest/network/target-group-health-checks.html)** | **[Load Balancer Health Probes](load-balancer-custom-probe-overview.md)** | Configure health probes for TCP services and alternate health check approaches for UDP services. Set probe interval (default 5 seconds), timeout, unhealthy threshold, and protocol to match AWS NLB configuration. Azure supports TCP, HTTP, and HTTPS health probes with configurable intervals (5-100 seconds, default 5 seconds) and failure thresholds. For UDP services, use TCP or HTTP health probes on alternate ports since Azure Load Balancer doesn't support native UDP health probing. Note: Health probes originate from IP 168.63.129.16 which must be allowed in NSG rules. |
...
A successful migration requires detailed documentation of the existing AWS NLB configurations and dependent services for an effective transition to Azure Load Balancer.
...
Prepare your Azure environment by deploying the necessary infrastructure and configuring security and monitoring components to support the migrated workload. This preparation helps facilitate an effective transition and reduces potential issues during the migration process.
...
- Configure static public IP addresses and zone redundancy before cutover for platform stability.
...
The general flow of changes is infrastructure deployment, service migration, and then configuration of load balancing rules, health probes, and monitoring. This sequence helps maintain functionality throughout the migration.
...
1. **DNS preparation** - Reduce TTL values and prepare DNS record updates for your domains. This step helps reduce user disconnection during cutover.
...
- **Jitter minimization**: Consistent packet delivery timing for acceptable experience
...
Define methods for validating the success criteria established for your workloads. This includes both automated and manual testing approaches for coverage of all capabilities.

#### Automated testing

- Create automated tests covering all capabilities including:
...
With your migration plan in place and preparatory changes completed, you're ready to move forward. Follow these detailed steps to execute the migration while reducing impact on active sessions.

### Migration execution

Migration execution involves a series of steps for transition from AWS NLB to Azure Load Balancer. The process includes final validation, parallel testing, DNS cutover, post-cutover validation, and AWS resource decommissioning.
...
1. **User experience validation**: Verify sessions work correctly across both service types
...
**Plan for reduced downtime**: Reduce DNS TTL values beforehand and prepare all infrastructure in parallel. The actual cutover involves only DNS changes, reducing session disruption and user impact.

**Monitor and optimize post-migration**: Use the improvement process to fine-tune performance, routing accuracy, and high availability. Azure Load Balancer provides monitoring capabilities to optimize your configuration over time with real traffic patterns.

**Platform-specific considerations**: Focus on latency-sensitive optimization, multi-protocol traffic handling, client IP preservation for security systems, and zone redundancy for high availability. Azure Load Balancer provides reliability and performance suitable for production platforms, though actual latency performance depends on datacenter design and network topology.
