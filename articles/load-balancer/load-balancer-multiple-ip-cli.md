...
8. Create the two frontend IP configurations, *contosofe* and *fabrikamfe* respectively:
...
9. Create your backend address pools - *contosopool* and *fabrikampool*, a probe - *HTTP*, and your load balancing rules - *HTTPc* and *HTTPf*:
...
10. Run the following command below and then check the output to verify your load balancer was created correctly:

```azurecli
azure network lb show --resource-group contosofabrikam --name mylb
```

11. Create a public IP, *myPuclicIp*, and storage account, *mystorageaccont1* for your first virtual machine VM1 as shown below:
...
12. Create a network interface, *VM1-NIC*, for VM1, add a second IP configuration, *VM1-ipconfig2*, and create the VM as shown below:

```azurecli
azure network nic create --resource-group contosofabrikam --location westcentralus --subnet-vnet-name myVnet --subnet-name mySubnet --name myNic --ip-config-name VM1-ipconfig1 --public-ip-name myPublicIP --lb-address-pool-ids "/subscriptions/366db6b2-be71-49ce-84b3-84a26f93b59f/resourceGroups/contosofabrikam/providers/Microsoft.Network/loadBalancers/mylb/backendAddressPools/contosopool"
azure network nic ip-config create --resource-group contosofabrikam --nic-name myNic --name VM1-ipconfig2 --lb-address-pool-ids "/subscriptions/366db6b2-be71-49ce-84b3-84a26f93b59f/resourceGroups/contosofabrikam/providers/Microsoft.Network/loadBalancers/mylb/backendAddressPools/fabrikampool"
azure vm create --resource-group contosofabrikam --name VM1 --location westcentralus --os-type linux --nic-name myNic --vnet-name VNet1 --vnet-subnet-name Subnet1 --availset-name myAvailabilitySet --vm-size Standard_DS3_v2 --storage-account-name mystorageaccount1 --image-urn canonical:UbuntuServer:16.04.0-LTS:latest --admin-username annahar  --admin-password rdPa$$w0rd01
```

13. Repeat steps 11-12 for your second VM:
...
azure network nic ip-config create --resource-group contosofabrikam --nic-name myNic2 --name VM2-ipconfig2 --lb-address-pool-ids "/subscriptions/366db6b2-be71-49ce-84b3-84a26f93b59f/resourceGroups/contosofabrikam/providers/Microsoft.Network/loadBalancers/mylb/backendAddressPools/fabrikampool"
