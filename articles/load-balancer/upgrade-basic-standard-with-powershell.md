...
Before you begin, review the [unsupported scenarios](#unsupported-scenarios), complete the [pre-migration steps](#pre-migration-steps), and run the validation example before starting an upgrade. The upgrade removes the Basic Load Balancer before creating the Standard Load Balancer, so plan for application downtime.
...
- For public load balancers, creates and associates a network security group with the Virtual Machine Scale Set or Virtual Machine to ensure load balanced traffic reaches backend pool members. This step follows Standard Load Balancer's move to a default-deny network policy. The upgrade process doesn't configure network security groups during an internal load balancer upgrade.
...
Before you run the module, make sure you have:
...
- **Subscription access**: The subscription containing the Basic Load Balancer, selected with `Select-AzSubscription`.
- **Permissions**: Rights to delete and create load balancers, and to modify the associated Virtual Machine Scale Sets and network interfaces.
- **No resource locks**: All locks removed from the load balancer, its resource group, and any related resources.

Complete the [pre-migration steps](#pre-migration-steps) before you start an upgrade.
...
> [!IMPORTANT]
> Run the validation example first and resolve any reported issues before you run an upgrade. The upgrade process removes the Basic Load Balancer before it creates the Standard Load Balancer, so your application is unavailable during migration. For expected duration, see [How long does the Upgrade take?](#how-long-does-the-upgrade-take)
...
- View recovery process @32:54 - <a href="https://learn-video.azurefd.net/vod/player?id=8e203b99-41ff-4454-9cbd-58856708f1c6#time=0h32m45s" target="_blank">Recovery</a>
...
If your failed migration was targeting multiple load balancers at the same time, using the `-MultiLBConfig` parameter, recover each Load Balancer individually using the following process:
  ...
  1. [Remove the new Standard Load Balancer](./configure-inbound-NAT-rules-vm-scale-set.md) (if created). Depending on which stage of the migration failed, you can have to remove the Standard Load Balancer reference from the Virtual Machine Scale Set, Virtual Machine network interfaces (IP configurations), and/or Health Probes to remove the Standard Load Balancer.
