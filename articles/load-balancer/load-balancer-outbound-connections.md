...
You can employ a connection pooling scheme in your application, where requests are internally distributed across a fixed set of connections (each reusing where possible).  This can increase the throughput of requests even if you reuse a connection.  It also constraints the number of ephemeral ports in use and creates a more predictable environment.  You may also find controls for connection pooling in the libraries used for a given implementation or configurable settings for applications.  Combined with connection reuse, your multiple requests will consume a fixed and predictable number of ports to the same destination IP address and port while also benefiting from very efficient use of TCP transactions reducing latency and resource utilization.

### Modify application to use less aggressive retry logic
You can reduce demand for ephemeral ports by using a less aggressive retry logic.  When ephemeral ports used for SNAT are exhausted or application failures occur, aggressive or brute force retries without decay and backoff logic cause exhaustion to persist.  Ephemeral ports have a 4-minute idle timeout (not adjustable) and if the retries are too aggressive, the exhaustion has no opportunity to clear up on its own.  Therefore, considering how your application retries transactions is a critical part of the design.
