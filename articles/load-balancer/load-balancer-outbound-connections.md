...
| SNAT port release behavior | TCP | If either server or client but not both sends FINACK, SNAT port will be released after 240 seconds. If both server and client sends FINACK, SNAT port will be released after 15 seconds. If a RST is seen, SNAT port will be released after 15 seconds. If idle timeout has been reached, port is released.|
