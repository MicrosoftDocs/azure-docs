...
Azure Standard Load Balancer supports global load balancing, which you can use for geo-redundant high availability scenarios such as:
...
- Ability to [scale up or down](#ability-to-scale-up-or-down-behind-a-single-endpoint) behind a single endpoint.
...
If one region fails, the traffic routes to the next closest healthy regional load balancer. 
...
Traffic that starts from a client hits the closest participating region and travels through the Microsoft global network backbone to arrive at the closest regional deployment. 
...
### Ability to scale up or down behind a single endpoint
...
Global load balancer comes with a static public IP, which ensures the IP address remains the same. Both IPv4 and IPv6 configurations are supported. To learn more about static IP, see [here](../virtual-network/ip-services/public-ip-addresses.md#ip-address-assignment).
...
The **home region** is where you deploy the global load balancer or Public IP Address of Global tier. 
...
A **participating region** is where the Global public IP of the load balancer is advertised.

Traffic that users start travels to the closest participating region through the Microsoft core network. 
...
> [!NOTE]
> You can deploy the backend regional load balancers in any publicly available Azure region. You're not limited to just participating regions.

## Limitations of global load balancer

- Global frontend IP configurations are public only. An internal frontend isn't currently supported.

- You can't add a private or internal load balancer to the backend pool of a global load balancer. 

- NAT64 translation isn't supported. The frontend and backend IPs must be of the same type (IPv4 or IPv6).

- UDP traffic on port 3 isn't supported on global load balancer.

- Outbound rules aren't supported on global load balancer. For outbound connections, use [outbound rules](./outbound-rules.md) on the regional load balancer or [NAT gateway](../nat-gateway/nat-overview.md).

- You can't upgrade regional load balancers to the global tier. You can only create new load balancers as the global tier.

- When you place the same NICs behind multiple regional load balancers with global load balancer, you must configure the load balancing rules on each regional load balancer with the same frontend port to use the same backend port.

- ICMP protocol isn't supported for global load balancer and ICMP Ping is expected to fail.
...
- To create a global load balancer, see [Tutorial: Create a global load balancer using the Azure portal](tutorial-cross-region-portal.md).
