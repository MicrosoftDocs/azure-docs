...
The custom HTTP Load Balancer probe overrides the default guest agent probe and allows you to create your own custom logic to determine the health of the role instance. Load Balancer probes your endpoint (every 15 seconds, by default) and the instance is considered to be in the Load Balancer rotation if it responds with an HTTP 200 within the timeout period (31 seconds by default).
...
-  The HTTP server does not respond at all after the timeout period. Note that depending on the timeout value that is set, this might mean that multiple probe requests go unanswered before the probe gets marked as being down (that is, before SuccessFailCount probes are sent).
...
- The first time the VM boots, the load balancer gets a positive probe.
