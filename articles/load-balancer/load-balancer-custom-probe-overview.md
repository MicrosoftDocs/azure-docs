...
Azure Load Balancer offers the capability to monitor the health of server instances by using probes. When a probe fails to respond, Load Balancer stops sending new connections to the unhealthy instance. If there is at least one healthy instance remaining, existing connections to backend instances that do not respond to probe remain unaffected. If no healthy instances remain, all existing connections to unhealthy instances will be terminated. 
