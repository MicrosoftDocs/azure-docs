...
Health probes govern whether new flows are established to healthy backend instances. When a health probe fails, Load Balancer stops sending new flows to the respective unhealthy instance.  Established TCP connections continue after health probe failure.  Existing UDP flows will move from the unhealthy instance to another healthy instance in the backend pool. UDP is connectionless and there is no flow state tracked for UDP.  As long as the hashing function produces the same result, the flow of UDP datagrams will remain on a specific instance.  A change of a health probe's status either from up to down or from down to up, in the backend pool may move new datagrams to a different instance in the backend pool.
