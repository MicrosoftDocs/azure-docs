...
Horizon is an Open Development Environment (ODE) used to secure IoT/ICS devices running proprietary protocols.
...
Contact <support@cyberx-labs.com> for information not covered in this guide.
...
Enables immediate integration and real time monitoring of custom dissector plugins in the CyberX platform, without the need to upgrade CyberX platform versions.

## CyberX services layer
...
Enables creation of plugins using the CyberX proprietary SDK (including C++ implementation and JSON configuration) to: 
...
### Requirements 
  ...
  - For compilation process, use GCC 7.4.0 or higher. Use any standard class from stdlib that is supported under C++17.

  - CyberX version 3.0 and above.

### Process

1.  Download Eclipse IDE for C/C++ Developers from [here](https://www.eclipse.org/) (You can use any other IDE you prefer. This document guides you through configuration using Eclipse IDE.

2.  After launching Eclipse IDE and configuring the workspace (where your projects will be stored), press Ctrl + n, create it as a C++ project.

3.  On the next screen, set the name to protocol you want to develop and select the project type as Shared Library and AND Linux GCC.

4.  Edit the project properties, under C/C++ Build -> Settings -> Tool Settings -> GCC C++ Compiler -> Miscellaneous -> Tick Position Independent Code

5.  Paste the example code that you received with the SDK and compile it.

6.  Add the artefacts (library, config.json, metadata) to a tar.gz file, and change the file extension to XXX.hdp, where is XXX is the name of the plugin.
...
2.  Know which protocol fields you plan to extract.

3.  Have planned your mapping objectives.
...
This file should define the dissector ID and declarations, dependencies, integration requirements, validation parameters, and mapping definitions to translate values to names, numbers to text. See [Prepare the Configuration File (JSON)](.\prepare-the-configuration-file-json.md), as well as:

  - [Prepare Implementation Code Validations](.\prepare-implementation-code-validations.md)

  - [Extract Asset MetaData](.\extract-asset-metadata.md)

  - [Connect to an Indexing Service (Baseline)](.\connect-to-an-indexing-service-baseline.md) Connect to an Indexing Service

### Implementation code: C++ (required)

The Implementation Code (CPP) parses raw traffic, and maps it to values such as services, classes and function codes. It extracts the layer fields and maps them to their index names from the JSON configuration files. The fields to extract from CPP are defined in config file. See [Prepare the Implementation Code (C++)](.\prepare-the-implementation-code-c++.md) for details.
...
1.  Review the [About the Environment and Setup](.\before-you-begin.md#about-the-environment-and-setup) section.


2.  [Prepare the Implementation Code (C++)](.\prepare-the-implementation-code-c++.md). Copy the template.json file and edit it to meet your needs. Do not change the keys. 

3.  [Prepare the Configuration File (JSON)](.\prepare-the-configuration-file-json.md). Copy the template.cpp file and implement an override method. See [horizon::protocol::BaseParser](.\prepare-the-implementation-code-c++.md#horizonprotocolbaseparser) for details.

4.  [Prepare Implementation Code Validations](.\prepare-implementation-code-validations.md)

## Prepare the implementation code (C++)

The CPP file is a parser responsible for:

  - Validating the packet header & payload (for example header length, or payload structure)

  - Extracting data from the header & payload into defined fields

  - Implementing configured fields extraction by the JSON file

## What to do

Copy the template **.cpp** file and implement an override method. See [horizon::protocol::BaseParser](#horizonprotocolbaseparser) for details.

## Basic C++ template sample 

This section provides the basic protocol template, with standard functions for a sample CyberX Horizon Protocol.
...
This section provides the basic protocol template, with a description of standard functions for a sample CyberX Horizon Protocol. 
...
This is the communication interface between the Horizon infrastructure and the Plugin layer. See [Horizon Architecture](.\horizon-architecture.md) for an overview on the layers.

The processLayer is the method used to process data.

  - The first parameter in the function code is the processing utility used for retrieving data previously processed, creating new fields, layers etc.
...
A layer is also created to store data. Use the *fieldsManager* from the processing utils to create new fields. A field can have only one of the following types: STRING, NUMBER, RAW DATA, ARRAY (of specific type), COMPLEX (may contain a number/raw/string, with id).
...
This section describes minimal JSON configuration declarations, the related structure and provides a sample config file that defines a protocol. This protocol is automatically integrated with the asset discovery service.
...
This section describes the JSON configuration fields you will be defining. The fields *labels* should not be changed.
...
This section describes additional fields.
...
Field values can be mapped to text in mapping files and seamlessly updated without impacting processing.
...
You can extract the following metadata on assets:
  ...
  - Is_router_potential - Indicates if the protocol is used mainly by routers, for example, LLDP, CDP, STP
...
| **expression** | When you want for this alert to popup.                                            | Use any numeric field found in lower layers, or the current layer.</br></br> Each field should be wrapper with {}, in order the SDK to detect it as a field, the logical operators we support are</br> == - Equality</br> <= - Less than or equal</br> >= - More than or equal</br> > - More than</br> < - Less than</br> ~= - Not equal |
...
Multiple expressions can be under same alert, for example:

“`{CyberXHorizonProtocol.function} == 25 and {IPv4.src} == 269488144`”


In this expression, we validate the function code only when the packet ipv4 src is 10.10.10.10. This is a raw representation of the IP in numeric representation.

You can use and/or in order to connect expressions.
...
- **firmware_index:**
  ...
  Choose one:  
  - model: The asset model. Enables detection of CVEs.
  - serial: The asset serial number. Not always available for all protocols. This value is generally unique per asset.
  - rack: Indicates the rack identifier, if the asset is part of a rack.
  - slot: The slot identifier, if the asset is part of a rack.  
  - module_addres: use to present a hierarchy if the module can be presented behind another device. Applicable instead if a rack slot combination, which is a simpler presentation.  
  - firmware_version: Indicates the asset version. Enables detection of CVEs.
    - alert_text: Indicates text describing firmware deviations, for example, version changes.  
    - index_by: Indicates the fields used to identify and index the asset. In the example below the asset is identified by its IP. In certain protocols, a more complex index can be used. For example, if another asset connected, and you know how to extract its internal path. For example, the MODBUS Unit ID, can be used as part of the index, as a different combination of IP + Unit Identifier.
    - firmware_fields: Indicates which fields are asset metadata fields. In this example, the following are used: model, revision and name. each protocol can define its own firmware data
...
You can enhance asset the information available in the Asset in Inventory, Data Mining and other reports.
...
Contains the file name that defines how to process each key in “key” fields. The config file itself should be in JSON format and be included as part of the plugin protocol folder.  
...
The properties that can be configured in this file are: 
  ...
  - the vendor - Indicates device vendor
...
The plugin pane lists:
  ...
  - Infrastructure plugins: Infrastructure plugins installed by default with CyberX.

  - Application plugins: Application plugins installed by default with CyberX and additional plugins developed by CyberX or external developers.
...
After creating and packaging your plugin, you can upload it to the CyberX sensor. To achieve full coverage of your network, you should upload the plugin to each sensor in your organization.
