...
1. **Security**, and **Operational** widgets are available at the top right of the window. Choose from a variety of categories and protocols. A brief description with a miniature graphic appears with each widget. Use the scroll bar to see all available widgets.
...
:::image type="content" source="media/how-to-create-trends-and-statistics-reports/incident-by-type.png" alt-text="A view of the the incident by type widget.":::
...
This widget displays a pie chart that shows the number of discovered devices per VLAN. The size of each slice of the pie is proportional to the amount of discovered devices relative to the other slices.
