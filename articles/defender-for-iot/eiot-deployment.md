...
1. Run `ipconfig` every minute for 3 minutes ensure data is streaming. You will know it is running appropriately if the number of RX packets increases each time. Repeat this step for each interface you have. 
...
1. If you are using a proxy environment, you will need to get your credentials and set the proxy to allow a connection to Azure. Proxies without passwords are not currently supported.
