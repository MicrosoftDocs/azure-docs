...
description: Defender for IoT security agents collect data and system events from your local device, and sends the data to the Azure cloud for processing, and analytics.
author: shhazam-ms
manager: rkarlin
ms.author: shhazam
ms.date: 1/20/2021
...
When the agent collects an identical event to one that is already kept in memory, the agent will increases the hit count of this specific event, in order to reduce the memory footprint of the agent. When the aggregation time window passes, the agent sends the hit count of each type of event that occurred. Event aggregation is simply the aggregation of the hit counts of each collected type of event. 
