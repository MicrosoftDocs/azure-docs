...
4.  Create client certificate key and the server certificate, and convert them to java keystore format. You’ll need to copy these to each Defender for IoT sensor in your network. 
...
3.  Select **Attribute** and use IOT device dictionaries to build a profiling rule based on the attributes injected. The following attributes are injected.
