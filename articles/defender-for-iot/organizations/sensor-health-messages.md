...
|---------|---------|---------|---------|
|**Disconnected**     |   This sensor is not communicating with Azure     |      Sensor is disconnected   |   Try signing into the sensor to check for errors or networking failures. <br><br>We also recommend reviewing the sensor networking configuration and verifying the sensor’s ability to communicate with Azure.      |
...
|Title  |Message  |Description  |Recommendation  |
|---------|---------|---------|---------|
|**DownloadingError** |There was an error uploading the file to the sensor |Upload error | Verify the sensor’s ability to communicate with the Microsoft Download Center and retry. <br><br>If the problem persists,  open a support ticket.|
|**ExtractionError**, **preupgradecheckerror**, **migrationerror**, **installationerror**, **generalerror** | There was an error installing the update.| Installation error |Open a support ticket. |
...
|**No OT/IoT traffic detected**|This sensor seems to not be monitoring OT/IoT networks |Sensor is not monitoring OT/IoT traffic |Check that SPAN traffic includes traffic from OT/IoT systems within the last 12 hours. |

## Healthy messages

|Title  |Message  |Description  |Recommendation  |
|---------|---------|---------|---------|
|**Pending activation** |Waiting for sensor to connect for the first time |Pending activation | Upload the activation file to the sensor. If this does not resolve the problem, verify the sensor’s ability to communicate with Azure.|
|**Pending reactivation** |Waiting for reactivation with new license |Pending reactivation |Upload the new activation file to the sensor. If this does not resolve the problem, verify the sensor’s ability to communicate with Azure. |
