...
|  Convert .cer file to .crt file  |  `openssl x509 -inform PEM -in <filepath>/certificate.cert -out certificate.crt` <br> Make sure to specify the full path. <br><br>**Note**: Other options are available for the -inform flag. The value is usually `DER` or `PEM` but might also be `P12` or another value. For more information, see [openssl-format-options](/docs/manmaster/man1/openssl-format-options.html) and [openssl-x509](/docs/manmaster/man1/openssl-x509.html)  |
