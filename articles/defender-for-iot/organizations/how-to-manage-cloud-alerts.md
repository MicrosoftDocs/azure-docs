...
1. Select **Alerts (Preview)**. The following alert information is available by default.
   ...
   | Parameter | Description
    ...
    | **Last detection** | The last time the alert was detected. <br>- If an alert's status is **New**, and the same traffic is seen again, the **Last detection** time is updated for the earlier alert. <br>- If the alert's status is **Closed** and traffic is seen again, the **Last detection** time is *not* updated, and a new alert is triggered.|
