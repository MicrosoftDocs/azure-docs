...
It's not clear when we are moving from Sensor APIs to CM APIs
The Bi-directional API is not only for Qradar... (QRadar alerts section)

Split into 3 pages - sensor, CM, Azure cloud
Intro to Azure Defender for IoT - APIs short intro paragraph
for sensor and CM - create a table with  (based on the table below)
sensor versions
supported APIs (eg. sensor 10.3 supports v1 and v2 apis)
