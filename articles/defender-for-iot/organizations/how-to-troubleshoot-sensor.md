...
|**Time drift detected** |Occurs when the UTC time of the sensor is not synchronized with Defender for IoT on the Azure portal.<br><br>
In this case, configure a Network Time Protocol (NTP) server to synchronize the sensor in UTC time. This can be done through the sensor settings. |
