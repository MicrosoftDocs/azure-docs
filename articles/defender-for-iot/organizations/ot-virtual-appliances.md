...
This article is one in a series of articles describing the [deployment path](ot-deploy/ot-deploy-path.md) for operational technology (OT) monitoring with Microsoft Defender for IoT. The article lists the specifications required if you want to install Microsoft Defender for IoT software on your own virtual appliances.
