...
1. Select the **Active Directory Integration Enabled** option and enter the following values for an Active Directory server:
    ...
    |Field  |Description  |
    |---------|---------|
    |**Domain Controller FQDN**     |  The fully qualified domain name (FQDN), exactly as it appears on your LDAP server. For example, enter `host1.subdomain.domain.com`. <br><br> If you encounter an issue entering the FQDN, check your DNS configuration. You can also enter the explicit IP of the LDAP server instead of the FDQN.      |
