...
1. Sign into your OT sensor and select **Device map**. All devices detected by the OT sensor are displayed by default according to Purdue layer.
...
By default, IT devices are automatically aggregated by subnet, so that the map focuses on OT and ICS networks.
...
| **No subnets configured** | No subnets are currently configured in your network. <br /><br /> We recommend configuring subnets for the ability to differentiate between OT and IT devices on the map. | - **Open Subnets Configuration** and configure subnets. <br />- **Dismiss**: Remove the notification. |**Dismiss** |
