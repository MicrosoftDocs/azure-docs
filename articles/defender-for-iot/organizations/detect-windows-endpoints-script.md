...
This article describes how to use a Defender for IoT Windows-based tool to get extended information from Windows-based devices, such as workstations, servers, and more. The Defender for IoT Windows-based tool allows users to run this tool directly from Windows devices to get extended information to increase inventory and security coverage by using a WMI script. While you can use [active scanning](configure-active-monitoring.md) and [scheduled WMI scans](configure-windows-endpoint-monitoring.md) to obtain this data, working with local scripts bypasses the risks of running WMI polling on an endpoint. Running a local script is also useful for regulated networks that have waterfalls and one-way elements.
