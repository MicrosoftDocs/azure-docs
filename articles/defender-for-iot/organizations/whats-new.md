...
| **OT networks** | **Sensor version 22.3.8**: <br>- [Proxy support for client SSL/TLS certificates](#proxy-support-for-client-ssltls-certificates) <br>- [Detect Windows workstations and servers with a local script (Public preview)](#detect-windows-workstations-and-servers-with-a-local-script-public-preview)) <br>- [Automatically resolved OS notifications](#automatically-resolved-os-notifications) <br>- [UI enhancement when uploading SSL/TLS certificates](#ui-enhancement-when-uploading-ssltls-certificates) |
...
Use a local script, available from the OT sensor UI, to detect Microsoft Windows workstations and servers. The script runs as a utility to detects enriched Windows data, and can be run manually or using standard automation tools.
 
For more information, see [Detect Windows workstations and servers with a local script](detect-windows-endpoints-script.md)
