...
By default, each OT network sensor is installed with the default *cyberx*, *support*, and *cyberx_host* users, which have access to advanced tools for troubleshooting and setup.

When setting up a sensor for the first time, sign in with these user credentials, create an initial user with an **Administrator** role, and then create extra users for security analysts and read-only users.
...
This procedure describes how to create new users for a specific OT network sensor, and is available for the *cyberx* and *support* users, and any user with the **Administrator** role.
...
By default, on-premises users are signed out of their sessions after 30 minutes of inactivity. Administrator users can use the local CLI to either turn this feature on or off, or to adjust the inactivity thresholds. This procedure describes how to control user session timeouts on a specific sensor.
...
1. Sign in to your sensor as an **Administrator** user.
...
**To reset a user's password on a sensor**:
...
<!--does this work on a sensor? needs validation
### Recover a sensor user's password
...
Allow Active Directory users to sign in to your OT sensors by configuring the integration with Active Directory. You can also create Active Directory groups and assign specific user permissions to all users in that group, such as if you want a specific group of users to have read-only permissions.
...
1. Sign in to your OT sensor console as an **Administrator** user.

1. From the left navigation menu, select **System Settings** > **Integrations** > **Active Directory**.
