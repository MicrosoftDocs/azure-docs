---
title: Passive vs active monitoring - Microsoft Defender for IoT
description: Describes how Microsoft Defender for IoT provides both passive and active monitoring on your network devices, and provides guidance as to when to use each method.
ms.date: 05/10/2022
ms.topic: conceptual
---

# Active and passive monitoring in Defender for IoT

This article describes how Microsoft Defender for IoT supports both passive monitoring, as the default network monitoring method, and active monitoring, which provides additional insights into your network assets and data.

## Passive monitoring with Defender for IoT

about passive monitoring

## Active monitoring with Defender for IoT

describe how this works

Options for active monitoring
can configure on windows
but also can configure active scans aside from windows (wait when do you want to do wmi vs active scanning?)

## When to use active monitoring

## Cautions for active monitoring

## Next steps

Onboard sensors (default - for passive monitoring with SPAN port)
Configure active monitoring on your system