...
| **Download endpoint details** | Available from the **Sites and sensors** toolbar **More actions** menu, for OT sensor versions 22.x only. <br><br>Download the list of domains that must be enabled as secure endpoints from OT network sensors. Make sure that HTTP traffic is enabled over port 443 to the listed endpoints for your sensor to connect to Azure. <br><br>To enable this option, select a sensor with a supported software version, or a site with one or more sensors with supported versions. |
