---
title: Migrate on-premises SMB file shares to Azure Files using the agentless migration capability in Storage Mover
description: Learn how to migrate SMB file data from on-premises file shares to Azure Files SMB shares by using the agentless migration capability in Azure Storage Mover.
...
# Migrate data from on-premises SMB to Azure Files with Azure Storage Mover (Preview)

Azure Storage Mover supports agentless migration from on-premises SMB file shares to Azure Files (SMB).
...
> [!IMPORTANT]
> Agentless SMB migrations require private connectivity between Azure and the network that hosts your source SMB share.
...
The on-premises SMB to Azure Files scenario is currently in preview. Review these limits and behaviors before production use:

- Each migration job supports up to **500 million objects**. If your source share contains more objects, split migration scope across multiple jobs.
...
- Data is copied, not moved. Source data remains on your SMB server until you decommission it.

For the latest limits and constraints, see [Cloud migration basics](migration-basics.md) and [Release notes](release-notes.md).

## Create the on-premises SMB source endpoint

Create a source endpoint that points to your on-premises SMB share.
...
1. In your Storage Mover resource, expand the **Resource Management** item in the left navigation. Select **Storage endpoints** > **Source endpoints** > **Create endpoint** to open the **Create source endpoint** pane.
    ...
    :::image type="content" source="media/agentless-on-premises-files-migration/source-endpoint-sml.png" alt-text="Screen capture of the Storage Mover's Source Endpoint pane, highlighting the steps required to access it." lightbox="media/agentless-on-premises-files-migration/source-endpoint-lrg.png"::: 

1. In the **Create source endpoint** pane, select the migration type and source type values that correspond to an agentless SMB source. Add values for the following fields:

  - **Host name or IP address**: The DNS/FQDN name or private IP of your source SMB server.
   ...
   - **Source description**: An optional description for your new source endpoint resource.
        ...
        :::image type="content" source="media/agentless-on-premises-files-migration/source-endpoint-create-sml.png" alt-text="Screen capture of the Storage Mover's Source Endpoint pane, highlighting the fields required to create a new source endpoint resource." lightbox="media/agentless-on-premises-files-migration/source-endpoint-create-lrg.png"::: 
...
1. In your Storage Mover resource, expand the **Resource Management** item in the left navigation. Select **Storage endpoints** > **Target endpoints** > **Add endpoint** to open the **Create target endpoint** pane.
    ...
    :::image type="content" source="media/agentless-on-premises-files-migration/target-endpoint-sml.png" alt-text="Screen capture of the Storage Mover's Target Endpoint pane, highlighting the steps required to access it." lightbox="media/agentless-on-premises-files-migration/target-endpoint-lrg.png":::  
...
1. Select **Create** to save your changes and create a new target endpoint resource.
    ...
    :::image type="content" source="media/agentless-on-premises-files-migration/target-endpoint-create-sml.png" alt-text="Screen capture of the Storage Mover's Target Endpoint pane, highlighting the fields required to create a new target endpoint resource." lightbox="media/agentless-on-premises-files-migration/target-endpoint-create-lrg.png":::
...
1. Navigate to the **Projects** page in the [Azure portal](https://portal.azure.com) to access your projects. The default **All projects** view displays the names of any provisioned projects and a summary of the jobs they contain.
    ...
    :::image type="content" source="media/agentless-on-premises-files-migration/project-explorer-sml.png" alt-text="Screenshot of the Overview tab within the Azure portal." lightbox="media/agentless-on-premises-files-migration/project-explorer-lrg.png":::
...
1. Select **Create project** to open the **Create a Project** pane. Enter a project name in the **Project name** field and an optional description in the **Project description** field. Finally, select **Create** to provision the project.
    ...
    :::image type="content" source="media/agentless-on-premises-files-migration/project-explorer-create-sml.png" alt-text="Screenshot of the Storage Mover's Create a project page." lightbox="media/agentless-on-premises-files-migration/project-explorer-create-lrg.png":::
...
1. In **Project explorer**, select the project you created in the previous section. Next, select **Create job definition** within the **All projects** view.
    ...
    :::image type="content" source="media/agentless-on-premises-files-migration/project-selected-sml.png" alt-text="Screen capture of the Project Explorer's Overview tab within the Azure portal highlighting the use of filters." lightbox="media/agentless-on-premises-files-migration/project-selected-lrg.png":::

1. In the **Basics** tab, configure the following settings:

    - **Migration type**: The migration type corresponding to your agentless SMB workload.
    - **Source type**: The source type corresponding to SMB sources in your tenant.
    - **Job name**: For example, `OnPremSmbToAzureFilesJob`
...
1. For final cutover, stop writes to the source SMB share.
...
1. Keep the source share available in read-only mode for a short fallback period, then decommission when validated.
...
- **Source path values**: Confirm source SMB host and share name are correct.
