...
The **Storage Account Contributor** and **Storage Blob Data Owner** RBAC roles are automatically assigned during the start job operation. No other steps are required.
...
1. In the **Settings** tab, select **Mirror or Merge source to target** from the **Copy mode** drop-down list. Verify that the **Migration outcomes** results are appropriate for your use case, then select **Next** and review your settings.
