...
An AWS VPC endpoint (VPCE) for FSx provides a private endpoint to an S3 service that allows your VPC to reach FSx privately. Learn how to create your FSx share and attach it to a VPC by following the steps within the [AWS documentation](https://docs.aws.amazon.com/fsx/latest/WindowsGuide/getting-started.html). Make sure your share is within a VPC which has AWS connectivity and that FSx security allows secure traffic on port 445 by default.
