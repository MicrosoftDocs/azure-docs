...
description: Learn about agent VM resourcing and deployment. #Required; article description that is displayed in search results. 
...
In this article you'll learn how to successfully deploy a Storage Mover agent VM.


## Prerequisites

The Storage Mover agent is a VM that you need to run, ideally close to the source storage you intend to migrate. The first step is to identify a capable Windows HyperV host to run the VM on. The following sections in this article have more details about compute and memory resource requirements of the agent VM.

> [!NOTE]
> During public preview, Windows HyperV is the only supported virtualization environment for your agent VM. Other virtualization environments have not been tested and are not supported at this time.
...
The image is hosted on Microsoft Download Center as a zip file:
[https://aka.ms/StorageMover/agent](https://aka.ms/StorageMover/agent)

Download the zip file and extract the agent image (*.vhdx file) to your virtualization host.

## Determine required resources for the VM

Like every VM, the agent requires available compute, memory and storage space resources on the host. These requirements primarily scale with the number of files and folders in any given migration job, rather than with the overall data size of the migration.
...
The article on [Performance targets](performance-targets.md) shares test results for different source share sizes and VM resources.
