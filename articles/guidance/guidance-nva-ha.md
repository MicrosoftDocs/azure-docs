...
You can use different NVAs available in the Azure marketplace to extend the functionality of Azure in the same way you use appliances in your on-premises datacenter. The figure below shows a sample deployment of a [single NVA][nva-scenario] as a firewall appliance. 

![[0]][0]

Although the deployment above works, it introduces a single point of failure. If the virtual appliance fails, no traffic flows. To solve that problem, you need to use multiple NVAs. However, that also requires other settings and resources to be used depending on your requirements.

You can use one of the solution below to provide a highly available NVA environment.
...
If you are providing connectivity to workloads hosted in Azure through a small set of server side ports (such as HTTP and HTTPS), you can use a set of NVAs behind an Azure load balancer as shown in the figure below to provide high availability at the NVA level.
...
You can extend the architecture above and add another set of NVAs to handle traffic originating from Azure to be handled by NVAs, as shown in the figure below.
...
## PIP-UDR switch
You can avoid creating multiple NVA stacks by using two NVAs in active-passive mode, and switching the public IP address (PIP) and user defined routes (UDRs) when the active node stops.  

![[3]][3]

This scenario is similar to the single NVA scenario. The only difference is that the PIP and UDRs must be changed to switch traffic between the NVAs. These changes can be done manually, but you can also automated them by creating an application running on both NVAs that checks for the health of the active node, and once the active node is down, changes the PIP and UDRs.

A possible implementation of this solution is to use a [ZooKeeper][zookeeper] daemon on the NVAs to handle leader election (deciding which node is the active node) and once a leader is elected, make calls to the Azure service management API to remove the PIP from the failed node and attach it to the leader, and change the routes that point to the failed node's private IP address to point to the new leader's private IP address.
