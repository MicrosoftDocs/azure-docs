...
* **Availability set**. The [availability set][availability set] contains the VMs and is necessary for supporting the [availability service level agreement (SLA) for Azure VMs][vm-sla].
...
* If you provision your VM with extensions, it may take longer to start up than a VM with no extensions.
