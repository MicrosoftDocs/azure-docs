...
> [AZURE.NOTE] This guidance assumes some basic familiarity with Elasticsearch and its purpose. For more detailed information, visit the [Elasticsearch website](https://www.elastic.co/products/elasticsearch).
...
```
> [AZURE.NOTE] Similar functionality is available to Java applications through the [Transport Client API](https://www.elastic.co/guide/en/elasticsearch/client/java-api/current/transport-client.html).
...
> [AZURE.NOTE] You can use the [Azure Load Balancer](load-balancer-overview/) to expose the cluster to the public Internet, or you can use an [internal load balancer](load-balancer-internal-overview/) if the client applications and cluster are contained entirely within the same private virtual network (VNET).
...
> [AZURE.NOTE] Tribe nodes require multicast discovery to connect to clusters, which may present a security concern. See the section [Node Discovery](#node-discovery) for more details.
...
You could consider using Azure Scale Sets to start and stop VMs as demands dictates (see [Automatically scale machines in a Virtual Machine Scale Set](virtual-machines-vmss-walkthrough/) for the details). However, this approach might not be appropriate for an Elasticsearch cluster for the following reasons:
...
> [AZURE.NOTE] Azure now supports disk encryption for Linux and Windows VMs. For more information, see [Azure Disk Encryption for Windows and Linux IaaS VMs Preview](azure-security-disk-encryption/).
