...
Front-end JavaScript frameworks or libraries often rely on client-side routing for web app navigation. These client-side routing rules often update the browser's window location without making requests back to the server. If you navigate directly to locations generated by client-side routing rules, or refresh the page, a server-side fallback route is required to serve the appropriate HTML page.

A common fallback route is shown in following example:
