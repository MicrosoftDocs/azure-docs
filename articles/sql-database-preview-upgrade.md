...
- How to copy your database to a new server with  SQL Database Update (preview) features enabled **(feature coming soon)**


## How to Update all Databases on a Server with the Latest SQL Database Update (preview) Features Enabled ##
...
| 4. Select the server you want to copy to a new server with  SQL Database Update preview features enabled. | ![Shows a list of SQL Servers][4] |
| 5. A blade opens that shows the properties of the server you selected. Click **Latest SQL Database Update V12 (preview)**. | ![Latest preview feature][5] |
...
- The validation process checks the service tier of your database and checks whether Geo-replication is enabled. The blade will show the results after the validation is complete. This could take hours based on the size of your server. 
...
- As you click on each database name, a new blade provides service pricing tier recommendation based on your current usage. You can also browse various pricing tiers and select the one that suits your environment best. All the databases that are setup for Geo Replication need to be reconfigured.
- Note that a recommendation on pricing tier will not be displayed if enough data is not found. 

| Action | Screen shot |
| :--- | :--- |
| 7. Once you have completed the actions that prepares your server for the upgrade, type the name of the server as displayed in the notification bar to upgrade and Click **OK**. | ![Confirm the server name to upgrade][7] |
| 9. The upgrade process is initiated. Upgrade can take up to 24 hours. Once the process is complete, the status **Enabled** is displayed on the server blade. | ![Confirms preview features are enabled][8] |
 

##How to Copy your Database to a New Server with SQL Database Update  V12 (preview) Features Enabled **(coming soon)**
