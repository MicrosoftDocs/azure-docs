...
Another reason for this is that application gateway backend health probe supports HTTP/HTTPS protocols only and if the server does not respond to probes, it would be taken out of backend pool and no requests including WebSocket requests, would be able to reach this backend. 
