...
- A [2022 Predefined](./application-gateway-ssl-policy-overview.md#predefined-tls-policy) or Custom v2 policy enhances SSL security and performance for the entire Application Gateway (SSL Policy and SSL Profile), so you can't have different listeners using both legacy and new SSL policies simultaneously. For details, see [SSL policy overview](./application-gateway-ssl-policy-overview.md).
