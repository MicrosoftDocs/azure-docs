...
# Azure Application Gateway components
 ...
 An Azure application gateway serves as the single point of contact for clients. It distributes incoming application traffic across multiple backend pools. These include Azure VMs, virtual machine scale sets, App Services, and on-premises/external servers. To distribute traffic, an application gateway uses several components described in this article.
...
Before you use an application gateway, you must add at least one listener. A listener is a logical entity that checks for incoming connection requests. It accepts requests if the protocol, port, host, and IP address that are associated with the request match the same elements that are associated with the listener configuration.

There can be multiple listeners attached to an application gateway, and they can be used for the same protocol. After a listener detects incoming requests from clients, the application gateway routes these requests to members in the backend pool. The application gateway uses the request routing rules defined for the listener that received the incoming request.
...
- Specify between the HTTP and HTTPS protocols in the listener configuration. Support for [WebSockets and HTTP/2 protocols](https://docs.microsoft.com/azure/application-gateway/overview#websocket-and-http2-traffic) is provided natively.
...
- Accomplish graceful removal of backend pool members by using [connection draining](https://docs.microsoft.com/azure/application-gateway/overview#connection-draining).

- Associate custom probe to monitor the backend health, set the request timeout interval, override host name and path in the request, and provide one-click ease to specify backend settings for App service backend.

## Backend pools

A backend pool routes requests to the backend servers, which serve the request. Backend pools can be composed of:
...
- FQDN
- Multitenant backends (such as Azure App Service)

Application Gateway backend pool members aren't tied to an availability set. It can communicate with instances outside of the virtual network that it's in. As a result, the members of the backend pools can be across clusters, data centers, or outside of Azure, as long as there's IP connectivity.

If you use internal IPs as backend pool members, it requires [VNET Peering](https://docs.microsoft.com/azure/virtual-network/virtual-network-peering-overview) or [VPN Gateway](https://docs.microsoft.com/azure/vpn-gateway/vpn-gateway-about-vpngateways). VNet peering is supported and beneficial for load-balancing traffic in other virtual networks.

An application gateway can also communicate with to on-premises servers when they are connected by ExpressRoute or VPN tunnels, if traffic is allowed.
...
By default, Application Gateway monitors all resources in its backend pool and automatically removes unhealthy resources. It then monitors unhealthy instances and adds them back to the healthy backend pool. This happens once they become available and respond to health probes.

Along with using default health probe monitoring, you can also customize the health probe to suit your application's requirements. Custom probes allow more granular control over the health monitoring. When using custom probes, you can configure the probe interval, the URL and path to test, and how many failed responses to accept before the backend pool instance is marked as unhealthy. It's strongly recommended that you configure custom probes to monitor the health of each backend pool.
