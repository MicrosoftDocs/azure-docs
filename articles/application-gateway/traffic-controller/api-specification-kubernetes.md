...
| "InvalidBackendTLSPolicy" | BackendTLSPolicyReasonInvalid is the reason when the BackendTLSPolicy is not Accepted |
| "InvalidKind" | BackendTLSPolicyReasonInvalidKind is used when the kind/group is invalid |
| "NoTargetReference" | BackendTLSPolicyReasonNoTargetReference is used when there is no target reference |
| "RefNotPermitted" | BackendTLSPolicyReasonRefNotPermitted is used when the ref is not permitted |
| "ServiceNotFound" | BackendTLSPolicyReasonServiceNotFound is used when the ref service is not found |
| "Degraded" | ReasonDegraded is the the backendTLSPolicyConditionReason when the backendTLSPolicy has been incorrectly programmed | 
