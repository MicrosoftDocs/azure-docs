...
Application Gateway and Web Application Firewall (WAF) are now available in a new SKU that provides additional features along with the functionality that is included in their generally available SKUs. The new SKUs include the following enhancements:

- **Autoscaling**: Application Gateway or WAF deployments under the autoscaling SKU can scale up or down based on changing traffic load patterns. Autoscaling also removes the requirement to choose a deployment size or instance count during provisioning. Hence, the SKU offers true elasticity. 
- **Zone resiliency**: An Application Gateway or WAF deployment can span multiple Availability Zones, removing the need to provision and spin separate Application Gateway instances in each zone with a Traffic Manager. You can choose a single zone or multiple zones where Application Gateway instances are deployed, thus ensuring zone failure resiliency. The backend pool for applications can be similarly distributed across availability zones.
...
## Supported regions
Autoscaling SKU is available in East US 2, West US2, France Central, West Europe and South East Asia.
...
|HTTP/2, FIPS mode, WebSocket|These are currently not supported.|
