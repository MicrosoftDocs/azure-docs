...
- **Static VIP**: Application gateway v2 SKU supports the static VIP type exclusively. This ensures that the VIP associated with application gateway doesn't change for the lifecycle of the deployment, even after a restart. Since we did not have a static VIP in V1, there was a need to use the App Gateway URL for a domain name routing to the App Services via the Application Gateway, and not the IP address.
