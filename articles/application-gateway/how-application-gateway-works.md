...
`X-Azure-JA4-Fingerprint` is a TLS client fingerprint that helps identify and correlate client traffic across services for security investigations.

For requests received through an Application Gateway private endpoint, Application Gateway also adds the `X-Azure-PrivateEndpoint-ID` header before forwarding the request to the backend. The header contains the decimal `LINKID` of the private endpoint connection and matches the `LinkId` property in the Application Gateway access log. The header isn't added to requests sent directly to a public or private frontend IP address. For more information, see [Identify traffic from a private endpoint](private-link.md#identify-traffic-from-a-private-endpoint).
