...
|Scenario | v1 | v2 |
| --- | --- | --- |
| When an FQDN or SNI is configured | Set as FQDN from the backend pool. As per [RFC 6066](https://tools.ietf.org/html/rfc6066), literal IPv4 and IPv6 addresses aren't permitted in SNI hostname. | The SNI value is determined based on the [TLS validations](configuration-http-settings.md?tabs=backendhttpsettings#backend-https-validation-settings) in the Backend Settings.<br><br> 1. **Complete validation** – This uses the SNI in the following order of precedence:<br> a) Custom Health Probe hostname<br> b) Backend Settings hostname (as per Overridden value or Pick from backend server) <br><br> 2.	**Configurable** <br> Use specific SNI: Fixed hostname used for validation.<br> Skip SNI: No Subject Name validation.
