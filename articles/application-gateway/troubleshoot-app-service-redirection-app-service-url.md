...
* [Incorrect redirect URLs](/azure/architecture/best-practices/host-name-preservation.md#incorrect-redirect-urls)  
  ...
  * an example: an OIDC authentication flow is broken because of a redirect with wrong hostname; this includes the use of [App Service Authentication and Authorization](../app-service/overview-authentication-authorization.md)
...
In case you have either:
- HTTP settings: **Pick Hostname from Backend Address** enabled
- or have **Override with specific domain name** set to a value different from what the browser will use

your setup is subject to the instructions in this article.
...
The production-recommended solution is to setup Application Gateway and App Service to not override the hostname.  Follow the instructions for **"Custom Domain (recommended)"** in [Configure App Service with Application Gateway](./configure-web-app.md)

Only consider applying another workaround (like a rewrite of the Location header as described below) after assessing the implications thereof.
