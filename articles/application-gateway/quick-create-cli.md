...
In Azure, all resources are allocated in a resource management group. Create a resource group by using [az group create](/cli/azure/group#az-group-create). 
...
Install NGINX on the virtual machines to verify that the application gateway was successfully created. You can use a cloud-init configuration file to install NGINX and run a "Hello World" Node.js app on a Linux virtual machine. 

In your current shell, create a file named cloud-init.txt and copy and paste the following configuration into the file:
...
It may take up to 30 minutes for the application gateway to be created. After the application gateway is created, you can see the following features:
...
Although it's not required to create the application gateway, you installed NGINX in this quickstart to verify whether the application gateway was successfully created. To get the public IP address of the application gateway, use [az network public-ip show](/cli/azure/network/public-ip#az-network-public-ip-show). Copy and paste the public IP address into the address bar of your browser.
...
When the resources that were created with the application gateway are no longer needed, use the [az group delete](/cli/azure/group#az-group-delete) command to remove the resource group, application gateway, and all related resources.
