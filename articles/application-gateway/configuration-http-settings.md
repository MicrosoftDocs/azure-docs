...
This setting is the number of seconds that the application gateway waits to receive a response from the backend server. The default value is 20 seconds in the Azure portal. Azure PowerShell or Azure CLI have no default value so they need to be specified. You may wish to adjust this setting to the needs of your application.
