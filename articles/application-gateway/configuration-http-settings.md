...
> [!NOTE]
> Some vulnerability scans may flag the Applicaton Gateway affinity cookie because the Secure or HttpOnly flags are not set. These scans do not take into account that the data in the cookie is encrypted using a one-way hash. The cookie does not contain any user information and is used purely for routing. 
