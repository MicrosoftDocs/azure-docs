...
If you plan to configure end-to-end SSL with the backend pool, the Application Gateway must be configured to trust the servers in the backend pool. This trust is configured by using the public root certificate of the certificate that is installed on the backend servers. If the backend servers have a certificate that is issued by a well-known Certificate Authority, then this setting can be enabled with no additional action required. If the certificate on the backend pool is issued by a private or unknown Certificate Authority, then the public certificate of the root CA server must be provided to the Application Gateway. This certificate must be in base64 .CER file format.
