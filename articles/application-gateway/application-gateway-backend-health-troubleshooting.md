...
1.  Check whether the VNet is configured with a custom DNS server. If it is, check the DNS server about why it can't resolve to the IP address of the specified FQDN.
...
1.  If the domain is private or internal, try to resolve it from a VM in the same VNet. If you can resolve it, restart
...
| Probe status code mismatch: Received 404 | Page not found. Check whether the hostname path is accessible on the backend server. Change the hostname or path parameter to an accessible value. | |
...
1.  Press Win+R or right-click the **Start** button, and then select **Run**.

1.  Enter `certmgr.msc` and press Enter. You can also search for Certificate Manager on the **Start** menu.
...
If there's a custom probe associated with the HTTP settings, SNI will be set from the hostname mentioned in the custom probe configuration. Or, if **Pick hostname from backend HTTP settings** is selected in the custom probe, SNI will be set from the hostname mentioned in the HTTP settings.

If **Pick hostname from backend address** is set in the HTTP settings, the backend address pool must contain a valid FQDN.

If you receive this error message, the Common Name (CN) of the backend certificate doesn't match the hostname configured in the custom probe or the HTTP settings (if **Pick Hostname from Backend HTTP Settings** is selected). If you're using a default probe, the hostname will be set as **127.0.0.1**. If that’s not a desired value, you should create a custom probe and associate it with the HTTP settings.
...
1.  Press Win+R or right-click the **Start** button and
    select **Run**.
...
1.  Verify the CN of the certificate from the details and enter the same in the hostname field of the custom probe or in the HTTP settings (if **Pick Hostname from Backend HTTP Settings** is selected). If that's not the desired hostname for your website, you must get a certificate for that domain or enter the correct hostname in the custom probe or HTTP setting configuration.
...
1.	NSG on the Application Gateway subnet is blocking inbound access to ports 65503-65534 (v1 SKU) or 65200-65535 (v2 SKU) from “Internet."
1.	UDR on the Application Gateway subnet is set to the default route (0.0.0.0/0) and with next hop not as "Internet."
1.	The default route is advertised by ExpressRoute/VPN connection to the VNet over BGP.
1.	The custom DNS server is configured on a VNet that can't resolve public domain names.
