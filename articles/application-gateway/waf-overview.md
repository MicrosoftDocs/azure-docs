...
Web application firewall (WAF) is a feature of Azure Application Gateway that provides centralized protection of your web applications from common exploits and vulnerabilities.

Web applications are increasingly the target of malicious attacks that exploit commonly known vulnerabilities. SQL-injection and cross-site scripting attacks are among the most common of these exploits.
...
A WAF solution can also react to a security threat faster by centrally patching a known vulnerability, instead of securing each individual web application. You can easily convert existing application gateways into a web application firewall-enabled application gateway.

WAF is based on [Core Rule Set (CRS)](https://www.owasp.org/index.php/Category:OWASP_ModSecurity_Core_Rule_Set_Project) 3.0 or 2.2.9 from Open Web Application Security Project (OWASP). A WAF automatically updates to include protection against new vulnerabilities, with no additional configuration needed.

![Diagram of Application Gateway WAF shown between incoming traffic and websites](./media/waf-overview/WAF1.png)
...
Application Gateway security enhancements include SSL policy management and end-to-end SSL support. Application security is strengthened by WAF integration into the ADC offering. This provides an easy-to-configure central location to manage. And it and protects your web applications against common web vulnerabilities.

## Benefits

This section describes the core benefits that Application Gateway and web application firewall provide.

### Protection

* Protect your web application from web vulnerabilities and attacks without modification to back-end code.

* Protect multiple web applications at the same time. An instance of Application Gateway can host of up to 20 websites that you protect with a web application firewall.
...
- Configure request size limits within lower and upper bounds.
- Use exclusion lists to omit certain request attributes from a WAF evaluation. A common example is Active Directory-inserted tokens that are used for authentication or password fields.

### Core rule sets

Application Gateway supports two rule sets, CRS 3.0 and CRS 2.2.9. These collections of rules protect your web applications from malicious activity.

Web application firewall comes preconfigured with CRS 3.0 by default. But you can choose to use CRS 2.2.9. CRS 3.0 offers reduced false positives compared with CRS 2.2.9. You can also [customize rules to suit your needs](application-gateway-customize-waf-rules-portal.md).

Common web vulnerabilities that web application firewall protects against include:

- SQL injection attacks
- Cross-site scripting  attacks
- Attacks such as command injection, HTTP request smuggling, HTTP response splitting, and remote file inclusion
- Protection against HTTP protocol violations
- HTTP protocol anomalies, such as missing host user-agent and accept headers
- Bots, crawlers, and scanners
- Common application misconfigurations (for example, of Apache or Internet Information Services)
...
|**[crs_40_generic_attacks](application-gateway-crs-rulegroups-rules.md#crs40)**|Protect against generic attacks (such as session fixation, remote file inclusion, as PHP injection)|
...
* **Detection mode** – Monitors and logs all threat alerts to a log file. Turn on logging diagnostics for Application Gateway in the **Diagnostics** section. You also need to ensure that the WAF log is selected and turned on. Web application firewall doesn't block incoming requests when it's operating in Detection mode.
...
In Traditional mode, traffic that matches any rule is considered independently of any other rules matches. 

While this mode is easy to understand, the lack of information on how many rules are matched by a specific request is a limitation. So, Anomaly Scoring mode was introduced. It's the default for OWASP 3.*x*.

In Anomaly Scoring mode, traffic that matches any rule is not immediately blocked when the firewall is in Prevention mode. Rules have a certain severity: Critical, Error, Warning, or Notice. That severity affects a numeric value for the request, which is called the Anomaly Score. For example, matching one Warning rule contributes a value of 3 to the score. One matching Critical rule contributes a value of 5.

There's a threshold of 5 for the Anomaly Score. For a score that's less than 5, traffic is not blocked. So, a single Critical rule match is enough for Azure WAF to block a request, even in Prevention mode. But one Warning-level match only increases the Anomaly Score by 3, which is not enough by itself to block the traffic.

> [!NOTE]
> The message that's logged when a WAF rules matches traffic includes the action field value "Blocked." But the traffic is actually only blocked for an Anomaly Score of 5 or higher.  

### WAF Monitoring

Monitoring the health of your application gateway is important. Monitoring the health of your web application firewall and the applications that it protects is supported by integration with Azure Security Center, Azure Monitor, and Azure Monitor logs.

![Diagram of Application Gateway and WAF diagnostics](./media/waf-overview/diagnostics.png)

#### Azure Monitor

Application Gateway logs are integrated with [Azure Monitor](../monitoring-and-diagnostics/monitoring-overview.md). This allows you to track diagnostic information, including WAF alerts and logs. This capability is provided on the **Diagnostics** tab in the Application Gateway resource in the portal or directly through Azure Monitor. To learn more about enabling logs, see [Application Gateway diagnostics](application-gateway-diagnostics.md)

#### Azure Security Center

[Security Center](../security-center/security-center-intro.md) helps you prevent, detect, and respond to threats. It provides increased visibility into and control over the security of your Azure resources. Application Gateway now [integrates into Azure Security Center](application-gateway-integration-security-center.md). Security Center scans your environment to detect unprotected web applications. It can now recommend an Application Gateway WAF to protect these vulnerable resources. You can directly create these firewalls from Security Center. These WAF instances are integrated with Security Center. The WAF sends alerts and health information to Security Center for reporting.

![Sample Security Center - Overview windows](./media/waf-overview/figure1.png)
...
Pricing is based on per hour gateway instance and data processing charges. [Application Gateway pricing](https://azure.microsoft.com/pricing/details/application-gateway/) for the WAF SKU differs from standard SKU charges. Data processing charges are the same. There's no per rule or rule group charges. You can protect multiple web applications behind the same web application firewall. You aren't charged for supporting multiple applications.
