...
> [!NOTE]
> Specifying **Azure Key Vault** certificates that are subject to the **role-based access control** permission model isn't supported via the **Azure portal**.
...
> [!NOTE]
> **Common pitfalls:**
> - **Key Vault** access removed → listener disabled; traffic impact
> - Secret rotated to a version the MI can't read → binding fails; explain how to pin/latest vs. explicit version
> - **Key Vault** behind **Private Endpoint** without proper DNS/NSG → **Application Gateway** can't fetch secret even if RBAC is correct
