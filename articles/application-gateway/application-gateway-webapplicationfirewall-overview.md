...
Web applications are increasingly targets of malicious attacks which exploit common known vulnerabilities. Common among these exploits are SQL injection attacks, cross site scripting attacks to name a few.
...
- Protect multiple web applications at the same time behind an application gateway. Application gateway supports hosting up to 20 websites behind a single gateway which could all be protected against web attacks.
...
During preview there are no additional charges for Application Gateway WAF usage. You will continue to be charged at existing Basic SKU charges. We will communicate the WAF SKU charges at GA time. Customers who have chosen to deploy Application Gateway in WAF SKU would start accruing WAF SKU pricing only after GA announcement. 

## Next steps

After learning more about the capabilities of WAF, please visit How to configure Web Application Firewall on Application Gateway.
