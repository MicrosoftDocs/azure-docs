...
[ ![A diagram showing the Application Gateway for Containers returning a redirect URL to a client.](./media/how-to-url-rewrite-gateway-api/url-redirect.png) ](./media/how-to-url-rewrite-gateway-api/url-redirect.png#lightbox)
...
Create an HTTPRoute resource for `contoso.com` that handles a redirect for the path /summer-promotion to a specific URL.  By eliminating sectionName, demonstrated in the http to https HTTPRoute resources, this redirect rule will apply to both http and https requests.
