...
This eample also demonstrates addition of a new header called `AGC-Header-Add` with a value of `agc-value` and removes a request header called `client-customer-header`.
...
Specifying a client-customer-header header with the value `moo` should be stripped from the request when AGC initiates the connection to the backend service:

```bash
fqdnIp=$(dig +short $fqdn)
curl -k --resolve contoso.com:80:$fqdnIp http://contoso.com -H "client-customer-header: moo"
```
