...
Deploying a new AKS cluster with the AGIC add-on enabled without specifying an existing application gateway instance will automatically create a Standard_v2 SKU application gateway instance. You'll need to specify a name and subnet address space for the new application gateway instance. In this tutorial, use **myApplicationGateway** for the application gateway name and **10.225.0.0/16** for its subnet address space.
