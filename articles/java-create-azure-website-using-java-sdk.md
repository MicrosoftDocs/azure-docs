<properties 
	pageTitle="Create an Azure Web App - Azure SDK for Java" 
	description="Learn how to create a Web App on Azure programmatically." 
	services="" 
...
# Creating an Azure Web App Using the Azure SDK for Java

This walkthrough shows you how to create an Azure SDK for Java application that creates an Azure Web App, then deploy an application to it. It consists of two parts:

- Part 1 demonstrates how to build a Java application that creates an Azure Web App.
- Part 2 demonstrates how to create a simple JSP "Hello World" application, then use an FTP client to deploy code to the Azure Web App.
...
The code in this procedure uses a self-signed certificate to authenticate with Azure. For this procedure, you need to create a certificate and upload it to the Azure Management Portal (AMP) beforehand. This involves the following steps:
...
# Building a Web App Creation Application
...
6. In **Dependencies**, click **Add**. In **Enter Group ID...** enter `azure-management`. Select the packages for base management and Web Apps management:
...
## Calling the Azure API to Create a Web App
...
Because the purpose of the AzureWebDemo application is to create an Azure Web App, name the main class for this application `WebAppCreator`. This class provides the main entry point code that calls the Azure Service Management API to create the Azure Web App.
...
Next, define a method to create the Web App. This method, `createWebApp`, specifies the parameters of the Web App and the webspace. It also creates and configures the Web App management client, which is defined by the [WebSiteManagementClient](http://dl.windowsazure.com/javadoc/com/microsoft/windowsazure/management/websites/WebSiteManagementClient.html) object. The management client is key to creating Azure Web Apps. It provides RESTful web services that allow applications to manage Azure Web Apps (performing operations such as create, update, and delete) by calling the service management API.
...
Provide the main() method code that calls createWebApp() to create the Azure Web App.
...
Log into AMP and click **Web Apps**. The new Web App should appear in the Web Apps list within a few minutes.


# Deploying Applications to the Web App

After you have run AzureWebDemo and created the new Web App, log into AMP, click **Web Apps**, and select **WebDemoWebApp** in the **Web Apps** list. In the Web App's dashboard page, click **Browse** (or click the URL, `webdemowebapp.azurewebsites.net`) to navigate to it. You will see a blank placeholder page, because no content has been published to the Web App yet.
...
In order to demonstrate how to deploy an application to the web, the following procedure shows you how to create a simple "Hello World" application and transfer it to the Web App that your application created.
...
- [Manage web apps through the Azure Management Portal: FTP Credentials](web-sites-manage/#ftp-credentials)
...
1. Log into AMP and click **Web Apps**. Make sure **WebDemoWebApp** appears in the list of Web Apps, and make sure that it is running. Click **WebDemoWebApp** to open its **Dashboard** page.
...
2. Under **Quick Glance**, find and copy **Deployment / FTP user**. This will have the form *webappname\deployment-username*; for example `WebDemoWebApp\deployer77`.

To obtain FTP connection information from the publish profile:

1. In the Web App's **Dashboard**, under **Quick Glance**, click **Download the publish profile**. This will download a .publishsettings file to your local drive.
...
1. In AMP, go to the Web App's **Dashboard** page and click **Configure**. On the **Configure** page, specify the following settings.
...
3. In **Default Documents**, add index.jsp and move it up to the top of the list. (Tthe default file for Azure Web Apps is hostingstart.html.)
...
2. In the **Site Manager** dialog, click **New Site**. A new blank FTP site will appear in **Select Entry** prompting you to provide a name. For this procedure, name it `AzureWebDemo-FTP`.
    ...
    - **Host:** Enter the **FTP Host Name** that you copied from the AMP dashboard.
    - **Port:** (Leave this blank, as this is a passive transfer and the server will determine the port to use.)
    - **Protocol:** FTP File Transfer Protocol
    - **Encryption:** Use plain FTP
    - **Logon Type:** Normal
    - **User:** Enter the Deployment / FTP user that you copied from the AMP dashboard. This is the full FTP username, which has the form *webappname\username*.
