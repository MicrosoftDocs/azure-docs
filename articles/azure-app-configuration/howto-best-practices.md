...
Excessive requests to App Configuration can result in throttling or overage charges. Applications take advantage of the caching and intelligent refreshing currently available to optimize the number of requests sent. This process can be mirrored in high volume client applications by not connecting directly to their configuration store. Instead, client applications connect to a custom service, and this service communicates with the configuration store. These customized caching solutions can ensure the client applications do not approach the throttling limit on the configuration store. 
