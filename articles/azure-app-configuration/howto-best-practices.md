...
An important thing to keep in mind is that keys are what your application code references to retrieve the values of the corresponding settings. A key shouldn't change, or else you'll have to modify your code each time that happens.
...
Let's look at an example. Suppose you have a setting named **Asset1**, whose value may vary based on the development environment. You create a key named "Asset1" with an empty label and a label named "Development". In the first label, you put the default value for **Asset1**, and you put a specific value for "Development" in the latter.

In your code, you first retrieve the key values without any labels, and then you use those values with a "Development" label to overwrite any previous values of the same keys. With a modern programming framework, such as .NET Core, you get this stacking capability for free if you use a native configuration provider to access App Configuration. The following code snippet shows how you can implement stacking in a .NET Core application:
...
You can provide access to App Configuration for web apps and functions by using any of the following methods:
