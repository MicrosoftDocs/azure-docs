...
In this quickstart, you create a feature flag in Azure App Configuration and use it to dynamically control the availability of a new web page in an [Aspire](https://aspire.dev/get-started/what-is-aspire/) app without restarting or redeploying the app.
...
1. Go to the `Web` project's directory that you created in the [Prerequisites](./enable-dynamic-configuration-aspire.md#prerequisites) steps. Run the following command to add the [`Microsoft.FeatureManagement`](https://www.nuget.org/packages/Microsoft.FeatureManagement) NuGet package.
