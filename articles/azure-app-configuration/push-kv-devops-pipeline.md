...
The [Azure App Configuration Push](https://marketplace.visualstudio.com/items?itemName=AzureAppConfiguration.azure-app-configuration-task-push) task lets you push key-values into your App Configuration store through yaml, json, and properties configuration files. This task enables full circle functionality within the pipeline as you are now able to pull settings from the App Configuration store as well as push settings to the App Configuration store.

## Prerequisites

- Azure subscription - [create one for free](https://azure.microsoft.com/free/)
- App Configuration resource - create one for free in the [Azure portal](https://portal.azure.com).
- Azure DevOps Account - [create one for free](https://go.microsoft.com/fwlink/?LinkId=2014881)
...
A service connection allows you to access resources in your Azure subscription from your Azure DevOps instance.
...
1. Click on the service connection that you created in the previous section.
...
1. Under Role select **App Configuration Data Owner**. This role allows the task to read from and write to the App Configuration store. Select the service principal associated with the service connection in the previous section.
...
1. Navigate to the build pipeline page by clicking **Pipelines** > **Pipelines**. Documentation for build pipelines can be found [here](https://docs.microsoft.com/azure/devops/pipelines/create-first-pipeline?view=azure-devops&tabs=tfs-2018-2).
  - If you are creating a new build pipeline:
    1. Select where your code is and then configure a pipeline to use.
    1. Click **Show assistant** on the right side of the pipeline and search for the **Azure App Configuration Push** task.
  - If you are using an existing build pipeline:
    1. Click the **Edit** button in the top-right corner to edit the build pipeline. 
...
1. Save and queue a build. The build log will display any failures encountered during the execution of the task.

## Use in Releases

This section will cover how to use the Azure App Configuration Push task in the DevOps Release Pipelines.

1. Navigate to release pipeline page by clicking **Pipelines** > **Releases**. Documentation for release pipelines can be found [here](https://docs.microsoft.com/azure/devops/pipelines/release?view=azure-devops).
1.	Choose an existing release pipeline. If you don’t have one, click **+ New** to create a new one.
1.	Click the **Edit** button in the top-right corner to edit the release pipeline.
...
1.	Click **+** for that Job, then add the **Azure App Configuration Push** task under the **Deploy** tab.
...
- **App Configuration Name**: A drop-down that will load your available configuration stores under the selected subscription. To update and refresh your list of available configuration stores; press the **Refresh App Configuration Name** button to the right of the textbox.
...
- **Separator**: The separator that will be used to flatten .json and .yml files.
- **Depth**: The depth that the .json and .yml files will be flattened to.
- **Prefix**: A string to be appended to the beginning of each key being pushed to the App Configuration store.
- **Label**: A string that will be added to each key-value as the label within the App Configuration store.
- **Content Type**: A string that will be added to each key-value as the content type within the App Configuration store.
- **Tags**: A JSON object in the format of `{"tag1":"val1", "tag2":"val2"}`, which will be added as tags for each key-value pushed to your App Configuration store.
...
## FAQ
 I am having a 409 error when attempting to push key-values to my configuration store.
