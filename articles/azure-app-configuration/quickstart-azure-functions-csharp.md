...
# [.NET 5] (#tab/dotnet5)
In .NET 5, dependency injection no longer requires `Startup.cs`, instead, the configuration changes are made in `Program.cs`.

1. Right-click your project, and select **Manage NuGet Packages**. On the **Browse** tab, search for and add following NuGet packages to your project.
   - [Microsoft.Extensions.Configuration.AzureAppConfiguration](https://www.nuget.org/packages/Microsoft.Extensions.Configuration.AzureAppConfiguration/) version 4.4.0 or later
   - [Microsoft.Azure.Functions.Extensions](https://www.nuget.org/packages/Microsoft.Azure.Functions.Extensions/) version 1.1.0 or later

2. In your *Program.cs* file, change your *HostBuilder* instance to include the following code.
   ````csharp
   var host = new HostBuilder()
       .ConfigureAppConfiguration(options =>
       {
           ...
           {
               config.Connect(Environment.GetEnvironmentVariable("ConnectionString"));
           });
       })
...
4. Add a constructor used to obtain an instance of `IConfiguration` through dependency injection.
...
5. Update the `Run` method to read values from configuration.
    ...
    public HttpResponseData Run([HttpTrigger(AuthorizationLevel.Anonymous, "get", "post")] HttpRequestData req, FunctionContext executionContext)
    {
        ...
        if (message != null)
        {
            ...
            error.Headers.Add("Content-Type", "application/text; charset=utf-8");
            error.WriteString($"Please create a key-value with the key '{keyName}' in App Configuration."));
            return error;
        }
    }
    ```
