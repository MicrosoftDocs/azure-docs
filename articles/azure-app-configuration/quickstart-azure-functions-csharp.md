...
This project will use [dependency injection in .NET Azure Functions](/azure/azure-functions/functions-dotnet-dependency-injection) and add Azure App Configuration as an additional configuration source.

1. Right-click your project, and select **Manage NuGet Packages**. On the **Browse** tab, search for and add followng NuGet packages to your project.
...
2. Add a new file *Startup.cs* with following code. It created a new `Startup` class that implements the `FunctionsStartup` abstract and an assembly attrbute is added to specify the type name used during Azure Functions startup.

    The `ConfigureAppConfiguration` method is overriden and Azure App Configururation provider is added as an additional configuration source by calling `AddAzureAppConfiguration()`. No additional services need to be registered at this point, so the `Configure` method is left empty.
...
3. Open *Function1.cs*, and add the namespaces of the .NET Core configuration
   ...
   and add a constructor where you will obtain an instance of `IConfiguration` through dependency injection.
...
4. Update the `Run` method to read values from the configuration.
   ...
   Neither the `Function1` class nor the `Run` method should be static. Remove the `static` modifier if it was auto-generated.
...
In this quickstart, you created a new App Configuration store and used it with an Azure Functions app via the [App Configuration provider](/dotnet/api/Microsoft.Extensions.Configuration.AzureAppConfiguration). To learn how to update your Azure Functions app to get dynamically refreshed configuration, continue to the next tutorial.
