...
A new version of the Azure App Configuration library for Spring Boot is available. The version introduces new features such as Azure Spring global properties, but also some breaking changes. These changes aren't backward compatible with configuration setups that were using the previous library version.

This article provides a reference on the change and actions needed to migrate to the new library version for the following topics:
...
As of the 4.7.0 version, the App Configuration and Feature Management libraries are part of the spring-cloud-azure-dependencies BOM. The BOM file makes it so that you no longer need to specify the version of the libraries in your project. The BOM automatically manages the version of the libraries.
...
[Azure Spring common configuration properties](/azure/developer/java/spring-framework/configuration) enables you to customize your connections to Azure services. The new App Configuration library will pick up any global or app configuration setting configured with Azure Spring common configuration properties. Your connection to app configuration will change if the configurations have been set for another Azure Spring library.
