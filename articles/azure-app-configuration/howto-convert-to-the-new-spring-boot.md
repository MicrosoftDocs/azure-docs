...
A new version of the App Configuration library for Spring Boot is now available. The version introduces new features such as Azure Spring global properties, but also a number of breaking changes. These changes aren't backwards compatible with configuration setups that were using the previous library version. For the following topics.
...
As of the 4.7.0 version, the App Configuration and Feature Management libraries are now part of the spring-cloud-azure-dependencies BOM. This means that you no longer need to specify the version of the libraries in your project. The BOM will automatically manage the version of the libraries.
...
* `ConfigurationClientBuilderSetup` has been renamed to `ConfigurationClientCustomizer` and it's `setup` method has been renamed to `customize`
* `SecretClientBuilderSetup` has been renamed to `SecretClientCustomizer` and it's `setup` method has been renamed to `customize`
...
[Azure Spring common configuration properties](azure/developer/java/spring-framework/configuration) enables you to customize your connections to Azure services. The new App Configuration library will now pick up any global or app configuration setting configured with Azure Spring common configuration properties. This means that your connection to app configuration will change if these are currenly being used with another Azure Spring library.
