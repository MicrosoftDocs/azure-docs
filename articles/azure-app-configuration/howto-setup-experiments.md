...
When updating your app earlier, you added `_telemetryClient.TrackEvent("Like")` to your application code. `Like` is a telemetry event that represents a user action, in this case as a button selection. This event is sent to the Application Insights resource, which you'll connect to the metric you're about to create.
The app we created only specifies one event, but you may have multiple events that take in user actions, in which case you would create several metrics.
