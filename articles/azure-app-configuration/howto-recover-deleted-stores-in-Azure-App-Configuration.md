...
* An Azure App Configuration store  - if you do not have a store, you can follow the steps provided below to create one.
...
1. On the **Create App Configuration** pane, enter the following settings:
    ...
    | **Resource name** | Globally unique name | Enter a unique resource name to use for the App Configuration store. This can not be the same name as the previous configuration store. |
...
1. Clicking on the Enabled value will take you to the properties of the store and you can see more information such as  retention period under "Days to retain deleted stores" Default value for this is 7. Range for the same is 1-7 days. You can not change this value once set.
    :::image type="content" source="../media/HowtosoftdeleteAppConfig_2.png" alt-text="In Properties, Soft-delete, Days to retain is highlighted.":::
1. Purge Protection check box shows whether the purge protection is enabled for this particular store or not. Once enabled, purge protection can not be disabled.

#### List, recover, or purge a soft-deleted App Configuration store

1. Log in to the Azure portal.
1. Click on the search bar at the top of the page.
1. Under "Recent Services" click "App Configuration" or search for "App Configuration". Do not click on an individual App Configuration store.
...
1. If you have deleted one or more App Configuration stores with enabled soft-delete option these will appear in the context pane on the right.
...
1. Select the recover option at the bottom of the context pane if you would like to recover the store. Please note that the store will be recovered entirely with all of its contents including the Customer Managed Keys (CMK) if you were using any. To start using these retained CMKs you will have to recreate the Managed identity for your service and grant the required permissions. Learn more about the usage of [CMKs in App Configuration stores](./concept-customer-managed-keys.md)
1. Select the purge option if you would like to permanently delete the store. You will not be able to purge a store with purge protection enabled.
