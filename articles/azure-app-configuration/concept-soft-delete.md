...
* **Purge Protection**: With Purge protection enabled, soft-deleted store can't be deleted permanently in the retention period. If disabled, the soft deleted store can be purged and deleted permanently before the retention period expires.
...
* **Purge**: Purge is the operation to permanently delete or purge out the stores in soft deleted state provided the store doesn't have purge-protection enabled.
...
* **Recovery of a deleted App Configuration store**: A deleted App Configuration store could be recovered in 1-7 days depending upon the retention time period set. In the App Config stores list, click on Manage deleted stores and recover the stores, which are in soft-deleted state. A deleted store will be recoverable for the configured retention period.

* **Permanent deletion of App Configuration store**: This feature makes the permanent deletion of an App Configuration store a two-step process. To delete an App configuration store permanently one will first have to delete the store, which will mark it as "soft-deleted". Then user will have to purge the store to make sure that the item is permanently deleted. Also any deleted store can only be purged if Purge protection isn't enabled on that. The stores that have been deleted with Purge Protection enabled, cannot be purged or permanently deleted until the retention period has passed. Once the store is permanently deleted the same name could be used for another App Configuration store.
...
The soft-delete feature is available through the [REST API](https://docs.microsoft.com/rest/api/appconfiguration/configuration-stores), *.NET/Java Config Providers, App Config SDKs(TODO)* and ARM templates.

## Billing implications

As one won't be able to send any requests to App Configuration stores in soft-deleted state, there won't be any charges for these stores. Once you recover a soft-deleted store, it's back to the active state and usual charges will start applying.
