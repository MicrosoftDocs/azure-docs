...
As an alternative to manually getting `IConfigurationRefresher` in the code above, you can instead resolve the instance of `IConfigurationRefresherProvider` that is registered in `IServiceCollection` by `AddAzureAppConfiguration`. You can then refresh the configuration by calling `TryRefreshAsync` on each of its refreshers. This can be seen in `DependencyInjectionExample` in the following code. 
