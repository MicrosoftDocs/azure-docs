...
:::image type="content" source="./media/event-grid-functional-model.png" alt-text="Screenshot that shows Event Grid Model.":::
...
> [!div class="checklist"]
> * Multiple subscriptions can be configured to route events to the same event handler, so don't assume events are from a particular source. Instead, check the topic of the message to ensure the App Configuration instance sending the event.
...
To learn more about Event Grid and to give Azure App Configuration events a try, continue to:
