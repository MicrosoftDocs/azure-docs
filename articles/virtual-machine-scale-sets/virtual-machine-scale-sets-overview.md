...
If you are redeploying a template to change the capacity, you could define a much smaller template which only includes the 'SKU' property packet with the updated capacity. An example of this is shown [here.](https://github.com/Azure/azure-quickstart-templates/tree/master/201-vmss-scale-existing)
...
The [Azure portal](https://portal.azure.com) lists scale sets and shows basic properties and operations, including listing VMs in the set and a resource usage graph. For more detail you can use the [Azure Resource Explorer](https://resources.azure.com) to view VM scale sets. VM scale sets are a resource under Microsoft.Compute, so from this site you can see them by expanding the following links:
...
* Plan for no more than 20 VMs per storage account when using scale sets with unmanaged disks (unless you set the *overprovision* property to "false", in which case you can go up to 40). With managed disks, these per-storage account limits do not apply.
* Spread out the first letters of storage account names as much as possible when using scale sets with unmanaged disks. The example VMSS templates in [Azure Quickstart templates](https://github.com/Azure/azure-quickstart-templates/) provide examples of how to do this.
* If using custom VMs with unmanaged disks, plan for no more than 40 VMs per VM scale set, in a single storage account. You will need the image pre-copied into the storage account before you can begin VM scale set deployment. Scale sets with managed disks will support up to 100 custom image VMs. See the FAQ for more information.
...
**A.** With managed disks your scale set can have between 0 and 1,000 VMs based on platform images, or 0-100 VMs based on custom images. For unmanaged disks (where you define your own storage accounts) the limit is 100 VMs for platform images and up to 40 for custom images (if the *overprovision* property is set to "false", 20 by default), since custom images with unmanaged disks are limited to a single storage account.

**Q.** Are Data Disks Supported within VM scale sets?

**A.** Yes. A scale set defined with managed storage can define an attached data drives configuration that will apply to all VMs in the set. Scale sets which are not defined with managed storage do not have attached data drives. Other options for storing data include:
...
**A.** Leave the vhdContainers property blank (or omit it) and provide image property's uri. For example: [201-vmss-windows-customimage].(https://github.com/Azure/azure-quickstart-templates/tree/master/201-vmss-windows-customimage)
