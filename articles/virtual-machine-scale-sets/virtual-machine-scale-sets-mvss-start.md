...
All resources require a `type`, `name`, `apiVersion`, and `location`. Our first resource is of type `Microsft.Network/virtualNetwork` with name `myVnet` and apiVersion `2016-03-30`. To figure out the latest api versions for a resource type, refer to the [Azure REST API documentation](https://docs.microsoft.com/rest/api/).
