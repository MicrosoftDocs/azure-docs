...
1. Add the following rules:
    ...
    1. If your cluster is backed by WASB and you are not using the service endpoints above, then add a rule for WASB:
        1. In the **Target FQDNs** section, provide a **Name**, and set **Source addresses** to `*`.
        1. Enter `http` or [https] depending on if you are using wasb:// or wasbs:// under **Protocol:Port** and the storage account url under **Target FQDNS**. The format will be similar to <storage_account_name.bloblcore.windows.net>.
1. Click **Add**.
