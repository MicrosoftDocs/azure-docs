...
* OOM happens freqently in the Apache Ambari Metrics Collector log `/var/log/ambari-metrics-collector/ambari-metrics-collector.log*`.
    ```
...
* High GC
    ...
    ```
    3. check GC using `jstat -gcutil <pid> 1000 100`. If you see the **FGCT** increase a lot in short seconds, this indicates Apache Ambari Metrics Collector is busy in Full GC and unable to process the other requests.

### Resolution

To avoid these issues, consider using one of the following options:

* Increase the heap memory of Apache Ambari Metrics Collector from `Ambari -> Ambari Metric Collector -> Configuration -> Metics Collector Heap Size`
