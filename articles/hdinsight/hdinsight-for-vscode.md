...
3. Open the command palette by pressing **CTRL+SHIFT+P**. 
4. Enter **HDInsight: Interactive Hive**.  You can also right-click a hive script editor, and then click **HDInsight: Interactive Hive** to query the result quickly.
5. Select **LLAP** to submit your query. Soon after, the query result tab is shown on the left.

![interactive hive result](./media/hdinsight-for-vscode/interactive-hive-result.png)
You can save the result as CSV,JSON,EXCEL to local path in **RESULTS** panel. Clicking **Line** number in **MESSAGES** panel, it jumps to the first line of the runing script.
