...
3. Do you use network security groups, user-defined routes, or Virtual Network Appliances to restrict traffic into or out of the virtual network?
    ...
    * Network security groups
        ...
        > [!IMPORTANT]
        > Network security group rules are applied in order based on rule priority. The first rule that matches the traffic pattern is applied, and no others are applied for that traffic. This means that you must order rules from most permissive to least permissive. Keep this in mind when adding the rules required for HDInsight. For more information, see
...
3. Find the Azure assigned DNS suffix for your virtual network. This value is similiar to `0owcbllr5hze3hxdja3mqlrhhe.ex.internal.cloudapp.net`. For information on finding the DNS suffix, see the [Example: Custom DNS](#example-dns) section.
...
4. Configure forwarding between the DNS servers. The configuration depends on the type of remote network.
    ...
    * If the remote network is an on-premises network, configure DNS as follows:
        ...
        * __Custom DNS__ (in the virtual network):
            ...
            * Forward requests for the DNS suffix of the virtual network to the Azure Recursive Router (168.63.129.16). Azure handles requests between resources within the virtual network.

            * Forward all other requests to the on-premises DNS server. The on-premises DNS handles all other name resolution requests, even those for internet resources such as Microsoft.com.

        * __On-premises DNS__: Forward requests for the virtual network DNS suffix to the Azure Recursive Router (168.63.129.16). The Azure Recursive Router handles name resolution for the resources in the virtual network.

    * If the remote network is another Azure Virtual Network, configure DNS as follows:

        * __Custom DNS__ (in each virtual network):

            * Forward requests for the DNS suffix __of the other virtual network__ to the custom DNS server in the other virtual network. The DNS in each virtual network is responsible for resolving resources within its network.

            * Forward all other reqeusts to the Azure Recursive Router. The recursive router is responsible for resolving local and internet resources.
...
The following list contains the IP address of the Azure health and monitoring services that monitor HDInsight clusters. This list is only important if you use network security groups or user defined routes to control traffic within the subnet that contains HDInsight. For more information, see the [Controlling network traffic](#networktraffic) section.
...
For a list of ports for specific services hosted by HDInsight, see the [Ports used by Hadoop services on HDInsight](hdinsight-hadoop-port-settings-for-services.md) document.
...
2. On the custom DNS server for the virtual network, use the following text as the contents of the `/etc/bind/named.config.local` file:
    ...
    ```
    // Forward requests for the virtual network suffix to Azure Recursive Router
    zone "0owcbllr5hze3hxdja3mqlrhhe.ex.internal.cloudapp.net" {
        type forward;
        forwarders {168.63.129.16;}; # Azure Recursive Router
    };
    ```
...
2. On the custom DNS server for the virtual network, use the following text as the contents of the `/etc/bind/named.conf.options` file:
    ...
    * Replace the `10.0.0.0/16` value with the IP address range of your virtual network. This allows name resolution requests from within the virtual network.

    * Add the IP address range of the on-premises network to the `acl goodclients { ... }` section. This allows name resolution requests from resources in the on-premises network.
...
4. On the on-premises DNS server, add a conditional forwarder that forwards traffic for the DNS suffix from step 1 to the custom DNS server in the virtual network.
...
2. On the custom DNS servers in both virtual neworks, use the following text as the contents of the `/etc/bind/named.config.local` file on the custom DNS server:
...
3. On the custom DNS servers in both virtual networks, use the following text as the contents of the `/etc/bind/named.conf.options` file:
    ...
    options {
            ...
            forwarders {
            168.63.129.16;   # Azure Recursive Router         
            };
    ...
    * Replace the `10.0.0.0/16` and `10.1.0.0/16` values with the IP address ranges of your virtual networks. This allows resources in each network to make requests of the DNS servers.

    Any requests that are not for the DNS suffixes of the virtual networks (for example, microsoft.com) is handled by the Azure Recursive Router.

4. To use the configuration, restart Bind. For example, `sudo service bind9 restart` on both DNS servers.

After completing these steps, you can connect to resources in the remote virtual network by using fully qualified domain names (FQDN). You can now install HDInsight into the virtual network.
