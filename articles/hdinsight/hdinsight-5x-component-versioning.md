...
On February 27, 2023, we started rolling out a new version of HDInsight 5.1. This version is backward compatible with HDInsight 4.0. and 5.0. All new open-source releases will be added as incremental releases on HDInsight 5.1.
...
> [!NOTE]
> Enterprise Security Package (ESP) isn't supported for Kafka and HBase in this release.

### Spark versions supported in Azure HDInsight

Azure HDIinsight supports the following Apache Spark versions.
...
On June 1, 2022, we started rolling out a new version of HDInsight 5.0. This version is backward compatible with HDInsight 4.0. All new open-source releases will be added as incremental releases on HDInsight 5.0.
...
If you're using the Azure user interface to create a Spark cluster for HDInsight, the dropdown list contains an additional version along with the older versions: Spark 3.1.(HDI 5.0). This version is a renamed version of Spark 3.1.(HDI 4.0), and it's backward compatible.  
...
For backward compatibility, ARM supports creating Spark 3.1 with HDI 4.0 and 5.0 versions, which map to same versions for Spark 3.1 (HDI 5.0).
...
If you're creating an Interactive Query cluster, the dropdown list contains another version as Interactive Query 3.1 (HDI 5.0). If you're going to use the Spark 3.1 version along with Hive (which require ACID support via HWC), you need to select this version Interactive Query 3.1 (HDI 5.0).
...
We have fixed the ARM template issue.
