...
Encryption of data in transit on HDInsight is achieved with [Transport Layer Security (TLS)](https://docs.microsoft.com/azure/hdinsight/transport-layer-security) for accessing the cluster gateways and [Internet Protocol Security (IPSec)](https://en.wikipedia.org/wiki/IPsec) between cluster nodes. IPSec can be optionally enabled between all head nodes, worker nodes, edge nodes, and zookeeper nodes. It is not enabled for traffic between gateway or [id broker](https://docs.microsoft.com/azure/hdinsight/domain-joined/identity-broker) nodes, which are Windows-based VMs and other linux-based nodes in the cluster.
