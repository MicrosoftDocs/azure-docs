...
This article describes how to set up and use the HDInsight ID Broker (HIB). This feature allows you to sign in to Apache Ambari using Multi-Factor Authentication (MFA) and get the required Kerberos tickets without needing password hashes in Azure Active Directory Domain Services (AAD-DS).
...
The HIB feature will add one extra VM to the cluster. This VM is the ID Broker Node and includes server components to support authentication. The HIB node is domain-joined to the AAD-DS domain as well.

![Option to enable HIB](./media/identity-broker/identity-broker-enable.png)

## Tools Integration

HDInsight [IntelliJ plugin](https://docs.microsoft.com/en-us/azure/hdinsight/spark/apache-spark-intellij-tool-plugin#integrate-with-hdinsight-identity-broker-hib) is updated to support OAuth. You can use this plugin to connect to the cluster and submit jobs.

## SSH access without password hash in Azure AD

After HIB is enabled, you'll still need a password hash stored in AAD-DS for SSH scenarios with domain accounts. To SSH to a domain-joined VM, or to run the `kinit` command, you need to provide a password. SSH authentication requires the hash to be available in AAD-DS. If you would like to use ssh for administrative scenarios only, you can create one cloud-only account and use that to SSH to the cluster. Other users can still use Ambari or HDInsight tools (such as IntelliJ plugin) without having the password hash available in AAD-DS.
