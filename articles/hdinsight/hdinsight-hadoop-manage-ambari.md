...
Working with users, groups, and permissions are supported when using a [domain joined](hbase/apache-domain-joined-introduction.md) HDInsight cluster. For information on using the Ambari Management UI on a domain-joined cluster, see [Manage domain-joined HDInsight clusters](hbase/apache-domain-joined-introduction.md).
