...
[Apache Phoenix](http://phoenix.apache.org/) is an open source, massively parallel relational database layer on top of [HBase](hdinsight-hbase-overview.md). Phoenix allows you to use SQL-like queries with HBase through SSH tools such as [SQLLine](hdinsight-hbase-phoenix-squirrel-linux.md). Phoenix also provides an HTTP server called Phoenix Query Server (PQS), a thin client that supports two transport mechanisms for client communication: JSON and Protocol Buffers. Protocol Buffers is the default mechanism, and offers more efficient communication than JSON.
