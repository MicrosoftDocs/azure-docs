...
|Zero RPO & RTO |Requiring zero data loss (RPO=0) and zero downtime (RTO=0) |Design RPO and RTO in ways to reduce the number of components that need to fail over. For more information on RTO and RPO, see [Azure HDInsight business continuity architectures](hdinsight-business-continuity-architecture.md).|
...
* [Azure reliability](/azure/architecture/framework/resiliency/overview)