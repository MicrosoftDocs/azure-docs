...
2. Connect to HiveServer2 with your Beeline client from your open SSH session by entering the following command:
    ...
    ```
   > [!NOTE]  
   > Refer to "To HDInsight Enterprise Security Package (ESP) cluster using Kerberos" part in [Connect to HiveServer2 using Beeline or install Beeline locally to connect from your local](../hadoop/connect-install-beeline) if you are using an Enterprise Security Package (ESP) enabled cluster
   >
