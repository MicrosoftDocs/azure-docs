...
1. From the **Storage** tab, provide the following values:
    ...
    Each cluster has an [Azure Storage account](../hdinsight-hadoop-use-blob-storage.md) or an [Azure Data Lake Gen1 account](../hdinsight-hadoop-use-data-lake-storage-gen1.md) dependency. It's referred as the default storage account. HDInsight cluster and its default storage account must be colocated in the same Azure region. Deleting clusters doesn't delete the storage account.
