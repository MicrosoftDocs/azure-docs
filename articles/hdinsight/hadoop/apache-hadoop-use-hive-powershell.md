...
2. Open a new **Azure PowerShell** command prompt. Change directories to the location of the `hivejob.ps1` file, then use the following command to run the script:
    ...
    When the script runs, you are prompted to enter the cluster name and the HTTPS/Admin (for non-secure) and Domain Admin (for secure) account credentials for the cluster. You may also be prompted to log in to your Azure subscription.
