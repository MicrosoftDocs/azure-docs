...
Power Query can be installed from [Microsoft Power Query for Excel](https://www.microsoft.com/download/details.aspx?id=39379). See the download page for the system requirements.

1. Open **Microsoft Excel**.

2. Select **Data** > **Get Data** > **From Azure** > **From Azure Table Storage**.

    ![HDInsight Hadoop Excel PowerQuery open Azure Table storage](./media/apache-hadoop-debug-jobs/using-excel-power-query-open.png)

3. Enter the storage account name, either the short name or the FQDN.

4. Enter the storage account key, and then select **Connect**.
...
5. Right-click the hadoopservicelog table in the **Navigator** pane and select **Edit**. You shall see four columns. Optionally, delete the **Partition Key**, **Row Key**, and **Timestamp** columns by selecting them, then select **Remove Columns** from the options in the ribbon.

6. Select the expand icon on the Content column to choose the columns you want to import into the Excel spreadsheet. For this demonstration, I chose TraceLevel, and ComponentName: It can give me some basic information on which components had issues.
...
7. Select **OK** to import the data.

8. Select the **TraceLevel**, Role, and **ComponentName** columns, and then select **Group By** control in the ribbon.

9. Select **OK** in the Group By dialog box

10. Select **Apply & Close**.
...
1. Expand **Storage Accounts**, the default storage account for your cluster, and then **Tables**.
1. Double-click **hadoopservicelog**.
1. Add a filter. For example:
...
The logs written to Azure Tables provide one level of insight into what is happening with an HDInsight cluster. However, these tables don't provide task-level logs, which can be helpful in drilling further into issues when they occur. To provide this next level of detail, HDInsight clusters are configured to write task logs to your Blob Storage account for any job that is submitted through Templeton. Practically, this means jobs submitted using the Microsoft Azure PowerShell cmdlets or the .NET Job Submission APIs, not jobs submitted through RDP/command-line access to the cluster.
...
From a web browser, navigate to `https://CLUSTERNAME.azurehdinsight.net`, where `CLUSTERNAME` is the name of your cluster.

### Access the Yarn UI

From the Apache Ambari UI, select **YARN** from the list of services on the left. Select **Quick Links**, then select the Active head node entry and then **ResourceManager UI**.
...
From the Apache Ambari UI, select **HBase** from the list of services. Select **Quick links** on the top of the page, point to the active Zookeeper node link, and then select HBase Master UI.
...
Some of these error messages could also be seen in the Azure portal when it's used to manage HDInsight clusters. But other error messages you might come across there are less granular because of the constraints on the remedial actions possible in this context. Other error messages are provided in the contexts where the mitigation is obvious.

### AtLeastOneSqlMetastoreMustBeProvided

* **Description**: Provide Azure SQL Database details for at least one component in order to use custom settings for Hive and Oozie metastores.
