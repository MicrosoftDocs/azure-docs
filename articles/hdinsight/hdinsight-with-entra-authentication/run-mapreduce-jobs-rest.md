...
1. To check the status of the job, use the following command. Replace the value for `JOBID` with the **actual** value returned in the previous step. Revise location of **jq** as needed.
   ...
   **Both methods**
    ...
    You can list and download these files by using the [Azure CLI](/cli/azure/install-azure-cli.md). For more information on using the Azure CLI to work with Azure Blob storage, see [Quickstart: Create, download, and list blobs with Azure CLI](/storage/blobs/storage-quickstart-blobs-cli.md).
