...
| Support for ML Server | You can set up an ML Server in HDInsight Spark cluster to run distributed R computations with the speeds promised with a Spark cluster. For more information, see [Get started with ML Servces in HDInsight](../r-server/r-server-get-started.md). |
