...
* A text editor or Java integrated development environment (IDE).
  ...
  * An SSH client. See one of the following articles for more information on using SSH with HDInsight:
    ...
    * [Use SSH with Linux-based Hadoop on HDInsight from Linux, Unix, OS X, or Bash on Windows 10](hdinsight-hadoop-linux-use-ssh-unix.md).
...
**com.microsoft.example.EventHubReader** reads data from Event Hub (the data written by EventHubWriter,) and stores it to the clusters default storage in the /devicedata directory.
...
* Rename license files for the dependencies: if this isn't done it can result in an error at runtime.
...
You must populate the `EventHubs.properties` file with information about your Event Hub and HDInsight cluster.
...
5. Select **Configure**, then create two new access policies by using the following information.
    ...
    After You create the permissions, select the **Save** icon at the bottom of the page. This creates the shared access policies that are used to send (writer) and listen (reader) to this Event Hub.
   
    ![policies](./media/hdinsight-storm-develop-csharp-event-hub-topology/policy.png)

6. After you save the policies, use the **Shared access key generator** at the bottom of the page to retrieve the key for the **writer** and **reader** policies. Save these because they are used later.
...
2. Use the following to build and package the project:
    ...
    This downloads required dependencies, builds, and then packages the project. The output is stored in the **/target** directory as **EventHubExample-1.0-SNAPSHOT.jar**.
...
3. Use the following command to start the topologies:
    ...
    This starts the topologies and give them a friendly name of "reader" and "writer".
...
### If using a Windows-based HDInsight cluster

> [!IMPORTANT]
> Linux is the only operating system used on HDInsight version 3.4 or greater. For more information, see [HDInsight Deprecation on Windows](hdinsight-component-versioning.md#hdi-version-32-and-33-nearing-deprecation-date).

1. Open your browser to https://CLUSTERNAME.azurehdinsight.net. When prompted, enter the administrator credentials for your HDInsight cluster. You arrive at the Storm Dashboard.
...
7. On the **Query Console**, select **Hive Editor** and replace the default `select * from hivesampletable` with the following:
    ...
    Click **Select** to run the query. This returns 10 rows from the data written to the cluster storage by the EventHubReader. Once the query completes, you should see data similar to the following:
...
You can export and import the persisted checkpoints to WASB (the Azure Storage used by Windows-based HDInsight clusters.) The scripts to do this are located on the Storm on HDInsight cluster, at **c:\apps\dist\storm-0.9.3.2.2.1.0-2340\zkdatatool-1.0\bin**.
