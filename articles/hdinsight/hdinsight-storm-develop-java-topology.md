...
> [!NOTE]
> If you are using PowerShell, you must surround the`-D` parameters with double quotes. The following is an example of the command when using PowerShell:
>
> `mvn archetype:generate "-DarchetypeArtifactId=maven-archetype-quickstart" "-DgroupId=com.microsoft.example" "-DartifactId=WordCount" "-DinteractiveMode=false"`

This creates a new directory named `WordCount` at the current location, which contains a basic Maven project.
...
> [!NOTE]
> Notice the `<scope>provided</scope>` line in this section. This tells Maven to exclude **storm-core** from any JAR files that are created, because it is provided by the system. This allows the packages you create to be a little smaller, and it ensures that they use the **storm-core** bits that are included in the Storm on HDInsight cluster.
...
> [!NOTE]
> Note that the `<mainClass>` entry uses `${storm.topology}`. This isn't defined in the properties section of the `pom.xml`. Instead, you will set this value from the command-line when running the topology on your development environment in a later step.

Another useful plug-in is the [Apache Maven Compiler Plugin](http://maven.apache.org/plugins/maven-compiler-plugin/), which is used to change compilation options. The primary reason this is needed is to change the Java version that Maven uses for the source and target for your application.
...
By looking at the logging emitted by the WordCount bolt, you can see that 'and' has been emitted 113 times. The count will continue to go up as long as the topology runs because the spout continuously emits the same sentences.
...
1. Move the `WordCountTopology.java` file out of the project. Previously, this defined the topology, but isn't needed with Flux.
...
3. Make the following changes to the `pom.xml` file.
   ...
   * In the **exec-maven-plugin** `<configuration>` section, change the value for `<mainClass>` to `org.apache.storm.flux.Flux`. This allows Flux to handle running the topology when it is ran locally in development.
