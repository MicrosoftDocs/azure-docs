...
For HDInsight Interactive cluster, by default, there's one Tez AM per worker node that acts as a query coordinator. The number of Tez AMs can be configured based on a number of concurrent queries to be served.
...
This configuration controls the number of executors that can execute tasks in parallel per LLAP daemon. This value depends on the number of vcores, the amount of memory given per executor, and the amount of total memory available for LLAP daemon. There are 16 vcores on D14 v2 VMs. However, not all of the vcores can be taken because other services like NodeManager, DataNode, Metrics Monitor etc. also need portion of available vcores. 
This value can be set to minimum 75% of the total vcores available on that node, however it can be oversubscribed if more memory is available for more executors.
If you need to adjust the number of executors for performance improvements, it's recommended that you consider at least 3 GB of memory per executor and make sure total memory needed for all executors do not exceed the total memory available for LLAP daemon container.  

For D14 v2, the recommended value for num of executors is **19** on each worker node considering 3GB per executor.
...
For D14 v2, the recommended value is ceil(81 GB x 0.06) ~= **5 GB**.  
...
For D14 v2, with HDI 4.0, SSD cache in-memory metadata size = 81 GB - (5 GB + 57 GB) = **19 GB**  
For D14 v2, with HDI 3.6, SSD cache in-memory metadata size = 81 GB - (5 GB + 57 GB + 2 GB slider) = **17 GB**
...
Size of in-memory metadata for SSD cache =  8% of size of SSD Cache  = 19 GB     
Size of SSD cache = size of in-memory metadata for SSD cache(19 GB) / 0.08 (8%)  

For D14 v2 and HDI 4.0, the recommended SSD cache size = 19 GB / 0.08 ~= **237 GB**  
For D14 v2 and HDI 3.6, the recommended SSD cache size = 17 GB / 0.08 ~= **212 GB**
...
It's recommended to keep both values same as number of worker nodes in Interactive Hive cluster.
...
With 20 GB in workload management queue capacity, a resource plan can specify `QUERY_PARALLISM` value as five, which means workload management can launch five Tez AMs with 4 GB container size each. If `QUERY_PARALLISM` is higher than the capacity, you may see some Tez AMs hung in `ACCEPTED` state. The Hiveserver2 Interactive cannot submit query fragments to the Tez AMs that not in `RUNNING` state.
