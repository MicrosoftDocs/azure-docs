...
Using workload management feature, you can configure who and how the resources are utilized. The feature also improves the average response time for a user
...
For a cluster topology with 4 D14v2 nodes with 100 GB per node gives **y=400**  and setting wm queue size as 10% gives **x=10**
```
N = 10/100 * 400/4 = 10
```
Note: It is always better to have a slightly more capacity in wm queue than required to avoid tez AMs getting stuck in accepted state that is, `wm` queue capacity can be made to 10.01% and `default` queue capacity can be reduced to 4.99%.
