---
title: Install GNU R on Linux-based HDInsight | Microsoft Docs
...
---
# Install and use GNU R on Linux-based HDInsight Hadoop clusters

You can install GNU R on any type of cluster in Hadoop on HDInsight by using **Script Action** cluster customization. Data scientists and analysts can use R to process large amounts of data on Hadoop clusters that are deployed in HDInsight.

> [!WARNING]
> This document provides instructions for installing GNU R on HDInsight as a custom action. It does __not__ provide the same capabilities as __Microsoft R Server on HDInsight__. We recommend using R Server on HDInsight instead of the steps in this document.
>
> For information on how to use R Server on HDInsight, see [Get started using R Server on HDInsight](hdinsight-hadoop-r-server-get-started.md). 
...
* [r-base-dev](http://packages.ubuntu.com/precise/r-base-dev): Auxiliary GNU R packages
...
This section provides instructions about how to use the script when creating a cluster using the Azure portal. 
...
2. On the **Optional Configuration** blade, select **Script Actions**, and use the following information to complete the form:
...
3. Enter the following R program. This code generates the numbers 1 to 100 and then multiplies them by 2.
...
4. Next, use the following to see the temporary path that the MapReduce output was stored to:
    ...
    This command returns a value similar to `/tmp/file5f615d870ad2`. To view the actual output, use the following command:
...
5. To exit R, enter the following command:
