...
* Kafka-reader: Reads data from Kafka and then stores it to the HDFS compatible file store for the Storm cluster.
    ...
    >
    > The script action is located at `https://https:\//managementactions.blob.core.windows.net/linuxstormextlibv01/stormextlib.sh` and is applied to the supervisor and nimbus nodes of the Storm cluster. For more information on using script actions, see the [Customize HDInsight using script actions](hdinsight-hadoop-customize-cluster-linux.md) document.
