...
To help you get started quickly with the Azure Video Indexer connectors, the example in this article will create Logic App flows. The Logic App and Power Automate capabilities and their editors are almost identical, thus the diagrams and explanations are applicable to both.
...
- Create a Azure Storage account. Keep note of the access key for your Storage account.
    ...
    Create two containers: one to store the media files, second to store the insights generated by Azure Video Indexer. In this tutorial the containers are `videos` and `insights`.
...
1. Create the [Logic App](https://ms.portal.azure.com/#create/Microsoft.LogicApp). We create a Logic App in the same region of the Azure Video  Indexer region. We call the logic app `UploadVideos`.

    Select **Consumption** for the T**Plan type**.
...
1. Generate an access token.
    ...
    Set up system assigned managed identity for permission on Azure Video Indexer resource.
          
    Go to the Azure Video Indexer resource, on the left side blade, and select **Access control**.

    1. Select **Add** -> **Add role assignment**.
    1. Select **Contributor** -> **Next**.
    1. Select **User, group, or service principal** -> Select members.
    1. Search the Logic Apps name you created and select.
1. Create an **Upload video and index** action.
    
    1. Search for "Video Indexer" -> From Video Indexer(V2) chose "Upload Video and index".
