<properties urlDisplayName="End-to-End Troubleshooting using Azure Storage Metrics and Logging, AzCopy, and Message Analyzer " pageTitle="End-to-End Troubleshooting using Azure Storage Metrics and Logging, AzCopy, and Message Analyzer | Microsoft Azure" metaKeywords="troubleshooting  diagnostics  Azure storage  Azure storage troubleshooting Azure storage diagnostics  Azure blob  Azure table  Azure queue  Azure file  Azure blob storage  Azure queue storage   Azure table   Azure table storage  Azure file storage  Azure  Microsoft Message Analyzer  Message Analyzer  AzCopy" description="An overview of Microsoft Azure Storage." metaCanonical="" disqusComments="1" umbracoNaviHide="1" services="storage" documentationCenter=".NET" title="End-to-End Troubleshooting using Azure Storage Metrics and Logging, AzCopy, and Message Analyzer" authors="tamram" manager="adinah" />

<tags ms.service="storage" ms.workload="storage" ms.tgt_pltfrm="na" ms.devlang="na" ms.topic="article" ms.date="01/19/2015" ms.author="tamram" />
...
![Configure Fiddler Options](./media/storage-e2e-troubleshooting/fiddler-options.PNG)
...
![Choose Metrics](./media/storage-e2e-troubleshooting/choose-metrics-portal.PNG)
...
![Metrics chart in portal](./media/storage-e2e-troubleshooting/portal-metrics-chart.PNG)
...
![Message Analyzer Start Page](./media/storage-e2e-troubleshooting/mma-start-page.PNG)
...
![Configure Message Analyzer Session](./media/storage-e2e-troubleshooting/configure-mma-session.PNG)
...
![Azure Storage View Layout](./media/storage-e2e-troubleshooting/400-range-errors.PNG)
...
![Azure Storage View Layout](./media/storage-e2e-troubleshooting/404-filtered-errors.PNG)
...
![Filtered Server and Network Trace Logs](./media/storage-e2e-troubleshooting/server-filtered-404.PNG)
...
![Client log showing 404 errors](./media/storage-e2e-troubleshooting/client-log-analysis-grid.PNG)
...
|    To Investigate…                                                                                               |    Use Filter Expression…                                                                                                                                                                                                                                        |    Expression Applies to Log (Client, Server,   Network, All)    |    Notes                                                                                                                                                                                                                               |
|------------------------------------------------------------------------------------------------------------------|------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|------------------------------------------------------------------|----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
|    Unexpected delays in message delivery on a queue                                                              |    AzureStorageClientDotNetV4.Description   contains "Retrying failed operation."                                                                                                                                                                                |    Client                                                        |    Check for   retries in the server log.                                                                                                                                                                                              |
|    HTTP Increase in PercentThrottlingError                                                                       |    HTTP.Response.StatusCode   == 500 &#124;&#124; HTTP.Response.StatusCode == 503                                                                                                                                                                                          |    Network                                                       |    Typically,   you identify these issues from Azure Storage metrics that show aggregated   values. However, the HTTP log data enables you to identify specific requests   that were throttled or timed-out by the storage service.    |
|    Increase in PercentTimeoutError                                                                               |    HTTP.Response.StatusCode   == 500                                                                                                                                                                                                                             |    Network                                                       |    Typically,   you identify these issues from Azure Storage metrics that show aggregated   values. However, the HTTP log data enables you to identify specific requests   that were throttled or timed-out by the storage service.    |
|    Increase in PercentTimeoutError (all)                                                                         |    *StatusCode   == 500                                                                                                                                                                                                                                          |    All                                                           |                                                                                                                                                                                                                                        |
|    Increase in PercentNetworkError                                                                               |    AzureStorageClientDotNetV4.EventLogEntry.Level   < 2                                                                                                                                                                                                          |    Client                                                        |    This filter   shows all client log entries that have a level of error or critical.                                                                                                                                                  |
...
|    404 (Not Found)                                                                                                     |    *StatusCode   == 404                                                                                                                                                                                                                                          |    Server, Network                                                           |                                                                                                                                                                                                                                        |
|    Shared Access Signature (SAS) authorization issue                                                             |    AzureStorageLog.RequestStatus ==  "SASAuthorizationError"                                                                                                                                                                                                     |    Network                                                       |    403 and 404   are the most common HTTP status codes for SAS-related errors.                                                                                                                                                         |
|    HTTP 409 (Conflict) messages                                                                                  |    HTTP.Response.StatusCode   == 409                                                                                                                                                                                                                             |    Network                                                       |                                                                                                                                                                                                                                        |
|    409 (Conflict)                                                                                                     |    *StatusCode   == 409                                                                                                                                                                                                                                          |    Server, Network                                                           |                                                                                                                                                                                                                                        |
...
|    Nagle   Warning                                                                                               |    ((AzureStorageLog.EndToEndLatencyMS   - AzureStorageLog.ServerLatencyMS) > (AzureStorageLog.ServerLatencyMS *   1.5)) and (AzureStorageLog.RequestPacketSize <1460) and (AzureStorageLog.EndToEndLatencyMS -   AzureStorageLog.ServerLatencyMS >= 200)        |    Server                                                        |    Identify log   messages where disabling Nagle may be useful                                                                                                                                                                         |
