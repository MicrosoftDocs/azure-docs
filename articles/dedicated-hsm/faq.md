...
Azure Dedicated HSM (hardware security module) provides HSMs hosted in Azure datacenters. These HSMs are directly connected to a customer' VNET. They are dedicated network HSM (Gemalto's SafeNet Network HSM 7 Model A790) available in a customers' private IP address space.  Microsoft does not have access to the cryptographic functionality of the HSMs. Only a customer has full administrative control and cryptographic control over these HSMs. Customers are the only ones who can get full activity logs directly from the HSM. Using Dedicated HSMs help customers meet requirements such as those outlined in FIPS 140-2 Level 3, GDPR, HIPAA, PCI-DSS, and eIDAS and many others.
...
You can provision HSMs in specific regions using PowerShell or the Azure command-line interface. During creation, you can specify the VNET the HSMs should use. The HSMs will be available in the designated subnet at the assigned IP addresses in your private IP address space when the provisioning process completes. You can manage the appliance, set up HSM client connections, initialize the HSM, create partitions, and assign roles all using SSH to connect to the appliance. You can then use the Gemalto provided HSM client tools/SDK/software to perform cryptographic operations from your applications.
...
Once Dedicated HSMs are provisioned, they will be available in the designated subnet at the assigned IP addresses.  Then a customer will use Gemalto provided HSM client tools/SDK/software to perform cryptographic operations from their applications, such as applications running inside a VM or a Web App etc.

## Q: Can an application connect to Dedicated HSM from a different VNET in or across regions?

Yes, you will need to use VNET peering (in same region or across regions) to establish connectivity across VNETs.
...
Azure Dedicated HSM gives full administrative control of the HSM appliance to the customer, including upgrading software/firmware. However, Microsoft will only support the HSM software/firmware that comes installed by default when the device is provisioned. Usually newer software/firmware versions are do not immediately receive FIPS certification, so upgrading firmware/software may make the device fall out of FIPS compliance.
...
At this time there is no SLA provided for Dedicated HSM service.  Microsoft will ensure network level access to the device, and hence standard Azure networking SLA’s apply.
...
## Q: How does a customer ensure that when an HSM is deprovisioned all the key material is completely wiped out?
...
Dedicated HSM service provisions SafeNet Network HSM 7 appliances. These appliances support physical and logical tamper detection are response. In case of a tamper event the HSMs are automatically zeroized. 
...
Yes. Dedicated HSM service provisions SafeNet Network HSM 7 appliances that use FIPS 140-2 Level 3 validated HSMs. For many customers it is a requirement to use FIPS 140-2 Level 3 validated HSMs to store keys used for document signing, operating a public Certificate Authority for SSL certificates, or data encryption.
...
Dedicated HSM service provisions SafeNet Network HSM 7 appliances, that use FIPS 140-2 Level 3 validated HSMs. The default deployed configuration (OS and Firmware) are also FIPS validated. You do not need to take any action for FIPS 140-2 Level 3 compliance.
...
## Q: Does Dedicated HSM service comply with PCI-DSS, SOC 1, SOC 2, or any other industry standards/certifications?
...
Dedicated HSM service provisions SafeNet Network HSM 7 appliances (model A790). Here's a summary of maximum performance for some operations: 
Maximum Performance:
...
Maximum number of keys (key pair in case of asymmetric keys)
...
Microsoft does not have ability to connect to HSMs allocated to customers. Customer must upgrade and patch the HSMs.
...
Yes, if you have on-premises SafeNet HSMs. There are multiple methods. Please refer to Gemalto HSM documentation.
...
You need to install Gemalto HSM client software to manage the HSMs and partitions.
