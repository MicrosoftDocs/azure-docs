...
The TSI environment is where device data is collected and stored. Once stored in the TSI environment, the [TSI Explorer](time-series-quickstart.md) and [TSI Query API](/rest/api/time-series-insights/ga-query-api) can be used to query and analyze the data. IoT Hub is the connection point used by all devices (simulated or physical) to securely connect and transmit data to the Azure cloud. The [TSI Overview](time-series-insights-overview.md) notes that Azure IoT Hub also serves as an event source for streaming data into a TSI environment. This tutorial uses an [IoT solution accelerator](/azure/iot-accelerators/) to generate and stream sample telemetry data to IoT Hub.
...
1. On the **Time Series Insights environment** page, fill in the required parameters:
   ...
   ---|---
   **Environment name** | Choose a unique name for the TSI environment. The name's used by TSI Explorer and  Query APIs.
   **Subscription** | Subscriptions are containers for Azure resources. Choose a subscription to create the TSI environment for.
...
1. Return to the TSI environment's resource group **Overview** page. Select the TSI environment.
