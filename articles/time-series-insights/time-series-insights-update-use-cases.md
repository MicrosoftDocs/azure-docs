...
These patterns often start with IoT enabled platforms that ingest billions of events from devices and sensors spanning various locales. Next, these systems process and analyze streaming data to derive real-time insights and actions, the data is then archived to warm and cold storage for near real time and batch analytics. Data that is collected goes through series of processing to enable cleanse and contextualization for enabling downstream querying and analytics scenarios. Microsoft Azure offers rich services that can be applied for these IoT scenarios (Asset Maintenance, Manufacturing, etc.) including Azure Time Series Insights, Azure IoT Hub, Azure Event Hubs, Azure Stream Analytics, Azure Functions, Azure Logic Apps, Azure Databricks, Azure Machine Learning, and Microsoft Power BI.

This solution architecture can be achieved as follow – ingest data via Azure IoT Hub or Azure Event Hub forbest-in-class security, throughput, and latency. Data ingested that can be funneled to Azure Stream Analytics, Azure Logic Apps and Azure Functions, this enables scenarios around custom computations on time-series data points in near time. The processed data may be pushed to Power BI for real time dashboarding, as well as can be loaded into Azure Time Series Insights for anomaly detection and monitoring.  Azure Time Series Insights offers near real-time data exploration and ad hoc querying of historical data. The data loaded is ready to be queried along with unlimited historical data for operational analysis In addition, data can further be refined and processed by connecting Azure Time Series Insights data to HDInsight for Map/Reduce, and Hive, jobs. Finally making this data available in Power BI, and in any customer applications via our public surface query APIs to integrate insights back into operational systems to drive efficiencies, historical trends, etc.
