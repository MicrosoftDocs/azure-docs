...
1.	First, it is fully integrated with cloud gateways like Azure IoT Hub and Azure Event Hub.  It easily connects to these event sources and parses JSON from messages and structures that data in clean rows and columns.  It joins metadata with telemetry and indexes your data in a columnar store.
2.	Second, Time Series Insights manages the storage of your data.  To ensure data is always easily accessible, it stores your data in memory and SSD’s for up to 400 days.  This enables you to interactively query billions of events in seconds – on demand.
3.	Third, Time Series Insights provides out-of-the-box visualization via the TSI explorer.  
4.	Fourth, Time Series Insights provides a query service, both in the explorer and via API’s that are easy to integrate if you would like to embed your time series data into custom applications.  
