...
**Learn more about data retention in Azure Time Series Insights and how to plan for it**:<br /><br />
...
You can change your Time Series Insights environment’s **Data retention time** setting. You can enable up to 400 days of retention. 
...
The second area to focus on for planning is ingress capacity. Ingress capacity is a derivative of the per-minute allocation.
...
For ingress capacity, first determine the total ingress you require on a per-month basis. Next, determine what your per-minute needs are. Throttling and latency play a role in per-minute capacity.

If you have a spike in your data ingress that lasts less than 24 hours, Time Series Insights can "catch up" at an ingress rate of 2x the rates listed in the preceding table.

For example, if you have a single S1 SKU, you ingress data at a rate of 720 events per minute, and the data rate spikes for less than 1 hour at a rate of 1,440 events or less, there's no noticeable latency to your environment. However, if you exceed 1,440 events per minute for more than one hour, you would likely experience latency in data that is visualized and available for query in your environment.
...
- Verify that your ingress capacity is above your average per-minute rate and that your environment is large enough to handle your anticipated ingress equivalent to 2x your capacity for less than 1 hour.
...
It's important to ensure that the way you send events to Time Series Insights supports the size of the environment you are provisioning. (Conversely, you can map the size of the environment to how many events Time Series Insights reads and the size of each event.) It'a also important to think about the attributes that you might want to slice and filter by when you query your data.
...
> [!NOTE]
> Reference data isn't joined retroactively. Only current and future ingress data is matched and joined to the reference dataset after it is configured and uploaded. If you plan to send a large amount of historical data to Time Series Insights and don't first upload or create reference data in Time Series Insights, you might have to redo your work (hint: not fun).  
