...
Time Series Insights does not have built-in business disaster recovery (BCDR).  However, customers that require BCDR can still impliment a recovery strategy by creating a second Time Series Insights environment in a backup Azure region and sending events to this secondary environment from their primary event source leveraging a second dedicated consumer group and that event source's BCDR guidelines.  
