...
# Add an IoT hub event source to a Time Series Insights environment
...
> [!NOTE]
> These instructions apply both to Azure Time Series Insights GA and to Time Series Insights Preview environments.
...
Consumer groups are used by applications to pull data from Azure IoT Hub. Provide a dedicated consumer group, for use by this Time Series Insights environment only, to reliably read data from your IoT hub.
...
1. Under **Consumer groups**, enter a unique name for the consumer group. Use this same name in Time Series Insights environment when you create a new event source.
...
1. Locate your Time Series Insights environment. In the left menu, select **All resources**. Select your Time Series Insights environment.
...
1. In **Event source name**, enter a name that's unique to this Time Series Insights environment. For example, enter **event-stream**.
...
1. The following table describes the required properties for the **Use IoT Hub from available subscriptions** option:
   ...
   | IoT hub consumer group | The consumer group to read events from the IoT hub. We highly recommend that you use a dedicated consumer group for your event source.
