...
Data in your warm store is available only via [Time Series Query](./concepts-query-overview.md) and the [Azure Time Series Insights Preview explorer](./concepts-ux-panels.md). Your warm store will contain recent data within the [retention period](./how-to-plan-your-environment.md#the-preview-environment) selected when creating the Time Series Insights environment.
