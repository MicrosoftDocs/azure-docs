...
Your Azure Time Series Insights environment contains an *ingestion engine* to collect, process, and store streaming time series data. A data stream is an unbounded sequence of events over time. As data is pushed into your event source(s) TSI will consume and store your data in near real-time. 
...
The following articles cover streaming ingestion in depth, including best practices to follow:
...
* Continue on to learn more about [event sources](concepts-streaming-ingestion-event-sources.md) for your Time Series Insights environment. 
