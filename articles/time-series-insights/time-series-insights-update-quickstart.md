...
The demo environment shows a electricity generation company, Contoso, using Time Series Insights to find actionable insights in their data and conduct a short root-cause analysis. Contoso operates two wind turbine farms, each with ten turbines, and each turbine has 20 sensors reporting data every minute to Azure IoT Hub. Sensors gather information on weather conditions, blade pitch & yaw position, generator performance, gearbox behavior, and safety monitors.

The Time Series Insights update is used to analyze the ever-growing data set from the last two years – currently at 40GB – to better understand and predict both critical failures and slow-moving maintenance issues.
