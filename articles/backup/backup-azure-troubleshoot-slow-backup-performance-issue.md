...
This article provides troubleshooting guidance to help you diagnose the cause of slow backup performance for files and folders in using Azure Backup. When you use the Azure Backup agent to back up files, the backup process may take longer than expected. This issue may be caused by one or more of the following:
...
-	[You are backing up a large number (millions) of files](#cause4).
...
Bottlenecks on the machine that's being backed up can cause delays. For example, the computer's ability to read or write to disk, or available bandwidth to send data over the network, can cause bottlenecks.
...
<a id="cause3"></a>
## The Backup agent is running on an Azure virtual machine

If you are running the Backup agent on a VM, performance will be slower than when you run it on a physical machine. This is expected due to IOPS limitations.  However, you can optimize the performance by switching the data drives that are being backed up to Azure Premium Storage. We are working on fixing this issue and the fix will be available in a future release.

<a id="cause4"></a>
## You are backing up a large number (millions) of files

Moving a large volume of data will take longer than moving a smaller volume of data. In some cases, backup time is related to not only the size of the data, but also to the number of files or folders. This is especially true when millions of small files (a few bytes to a few kilobytes) are being backed up.

This behavior occurs because while you are backing up the data and moving it to Azure, Azure is simultaneously cataloging your files. In some rare scenarios, the catalog operation may take longer than expected.
