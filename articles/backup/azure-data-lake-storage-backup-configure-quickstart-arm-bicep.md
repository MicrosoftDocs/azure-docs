...
The following example ARM template shows how to configure vaulted backup for an explicit list of containers in a storage account with a backup policy. This backup policy runs daily and retains backups for 30 days, as well as weekly, monthly, and yearly backups for longer retention. Use an explicit container list when you want to protect only selected containers instead of auto-protecting all present and future containers. To configure auto-protection, use the backup configuration generated by the Azure CLI `initialize-backupconfig --auto-protection true` command or the Azure PowerShell `New-AzDataProtectionBackupConfigurationClientObject -AutoProtection` cmdlet instead of `containersList`.
...
```json
{
  ...
  "parameters": {
    ...
    "containerList": {
      ...
      "metadata": {
        "description": "Explicit list of containers to be protected"
      }
    },
    "location": {
      "type": "string",
...
The following example Bicep template shows how to configure vaulted backup for an explicit list of containers in a storage account with a backup policy. This backup policy runs daily and retains backups for 30 days, as well as weekly, monthly, and yearly backups for longer retention. Use an explicit container list when you want to protect only selected containers instead of auto-protecting all present and future containers. To configure auto-protection, use the backup configuration generated by the Azure CLI `initialize-backupconfig --auto-protection true` command or the Azure PowerShell `New-AzDataProtectionBackupConfigurationClientObject -AutoProtection` cmdlet instead of `containersList`.
