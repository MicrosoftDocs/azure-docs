...
- **Stop protection and delete backup data**.
  - This option will stop all future backup jobs from protecting your data. If the [vault security features are not enabled](backup-azure-security-feature.md#enable-security-features), all recovery points are immediately deleted.
  - If the security features are enabled, the deletion is delayed by 14 days, and you'll receive a delete Backup data alert email with a message *Your data for this Backup item has been deleted. This data will be temporarily available for 14 days, after which it will be permanently deleted* and recommended action *Reprotect the Backup item within 14 days to recover your data.*
  - In this state, the retention policy continues to apply, and the backup data remains billable.
