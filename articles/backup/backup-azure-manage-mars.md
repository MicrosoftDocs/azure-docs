...
- Azure Backup doesn't support automatic adjustment of clock for daylight savings time (DST). Modify the policy to ensure day light saving is taken into account to prevent discrepancy between the actual time and scheduled backup time.

Create a policy as follows:
1. After downloading and registering MARS agent, launch the agent console. You can find it by searching your machine for **Microsoft Azure Backup**.  
...
10. On the **Select Retention Policy** page, specify how you store historical copies of your data. Then click **Next**.
    ...
    - As an example, in the following screenshot:
            ...
            - Backups taken on Saturday on the last week of the month at midnight and Six PM are kept for 12 months. - Backups taken on a Saturday in the last week of March are kept for 10 years.
...
Create policy  has to be done on each machine where the agent is installed.
...
4. In **Select items to backup** tab, click **Add items** to add the items that you want to backup. 
...
You can add exclusion rules to skip files and folders that you don't want to be backed up. You can do this during when defining a new policy or modify an existing policy. 
1. From Actions pane click **Schedule Backup**. Goto **Select items to Backup** and click **Exclusion Settings**.
...
1. From Actions pane click **Schedule Backup**. Goto **Select items to Backup**. From the list of select the files and folders that you want to remove from backup schedule and click **Remove items**.
...
- **Stop protection and retain backup data**. 
   ...
   - If you decide to resume protection, then you can use *Re-enable backup schedule* option. Post that data would be retained based on the new retention policy.
...
1. In **select policy items** page, select **Modify a backup schedule for your files and folders** click next 
...
You had stop protection with retain data and decided to resume protection then you can can re-enable backup schedule using modify backup policy. 
