...
6. Select **OK** to save the policy.
    > [!NOTE]
    > The **Backup** service creates a separate resource group (RG), which is different than the RG of the VM, to store the restore point collection (RPC). [Learn more](backup-during-vm-creation.md#azure-backup-resource-group-for-virtual-machines).
