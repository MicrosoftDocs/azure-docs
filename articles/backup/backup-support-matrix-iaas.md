...
[Azure Dedicated Host](../virtual-machines/dedicated-hosts.md) | Supported<br></br>While restoring Azure VMs through the [Create New](backup-azure-arm-restore-vms.md#create-a-vm) option, though the restore would be successful, Azure VMs can't be restored in the dedicated host. However, while [restoring as disks](backup-azure-arm-restore-vms.md#restore-disks) with the template, you can achieve it by creating a VM in dedicated host, and then attaching the disks. This is also applicable in secondary region while performing [Cross Region Restore][Cross Region Restore](backup-azure-arm-restore-vms.md#cross-region-restore).
