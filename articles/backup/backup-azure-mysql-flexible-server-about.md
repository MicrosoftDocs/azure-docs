...
1. The Backup service invokes the backup based on the policy schedules on the ARM API of MySQL Flex server, writing data to a secure blob container with a SAS for enhanced security.
2. Backup job runs independently, preventing disruptions during long-running tasks.
3. The retention and recovery point lifecycles align with the backup policies for effective management.
4. During the restore, the Backup service invokes restore on the ARM API of the MySQL Flex server using the SAS for asynchronous, nondisruptive recovery.
...
The Azure Backup service needs to connect to the Azure MySQL - Flexible server while taking each backup. 
...
- [Support matrix for Azure Database for MySQL - Flexible Server retention for long term by using Azure Backup (preview)](backup-azure-mysql-flexible-server-support-matrix.md).