...
![Manage Azure Backup tabs](./media/backup-azure-manage-windows-server/backup-management-tabs.png)

### Dashboard
Select **Dashboard** to see the usage overview for the server.
...
From the **Configure** tab you can select the appropriate storage redundancy option. The best time to select the storage redundancy option is right after creating a vault and before any machines are registered to it. Once an item has been registered to the vault, the storage redundancy option is locked and cannot be modified.
...
![Policies](./media/backup-azure-manage-windows-server/policies.png)

Click ![](./media/backup-azure-manage-windows-server/policy-arrow.png) to see the details associated with that policy.

On the **Details** page you can make changes to the:

- policy details
- schedule
- retention range

Once you've made changes, you can **save** or **discard** them using the options at the bottom of the page.

![Policies](./media/backup-azure-manage-windows-server/save-discard.png)
...
9. After the update is completed, the **Jobs** view in the Azure Backup console indicates the "Job completed" status.

    ![IR complete](./media/backup-azure-manage-windows-server/ircomplete.png)
