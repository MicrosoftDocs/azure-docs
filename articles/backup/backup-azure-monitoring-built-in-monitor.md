...
Currently, there are two main types of inbuilt alerts made available by Azure Backup via Azure Monitor:

1. **Security Alerts**: For scenarios such as deletion of backup data, or disabling of soft-delete functionality for a vault, security alerts (of severity Sev 0) are fired and can be viewed in the Azure portal or consumed via other clients like PowerShell, CLI and REST API. Security alerts are generated by default and cannot be turned off. However, you can control the scenarios for which notifications (for example, emails) should be generated. Refer to the section on Action Rules for more details on how to configure notifications.
2. **Job Failure Alerts**: For scenarios like backup failure and restore failure, Azure Backup provides inbuilt alerts via Azure Monitor (of Severity Sev 1). Unlike security alerts, you can choose to turn off Azure Monitor alerts for job failure scenarios (for example, if you have already configured custom alert rules for job failures via Log Analytics, and don't need inbuilt alerts to be fired for every job failure). By default, alerts for job failures are turned off. Refer to the [section on turning on alerts for these scenarios](#turning-on-azure-monitor-alerts-for-job-failure-scenarios) for more details.

The below table summarizes the different backup alerts currently available (in preview) via Azure Monitor and the supported workload/vault types:
...
3. Click on **Register** to enable this feature for your subscription. Note that it may take up to 30 minutes for the registration to take effect. If you wish to enable this feature for multiple subscriptions, repeat the above process by selecting the relevant subscription at the top of the screen.
...
Clicking on any of the numbers opens up a list of all alerts fired with the given severity. You can click on any of the alerts to get more details about the alert, such as the affected datasource, alert description and recommended action, and so on.
...
To configure notifications for Azure Monitor alerts, you must create an action rule. The below steps demonstrate how to create an action rule to send email notifications to a given email address. Similar instructions will apply for routing these alerts to other notification channels, such as ITSM, webhook, logic app and so on.

1. Navigate to **Azure Monitor** in the Azure portal. Click on the **Alerts** menu item and select **Manage actions**.

![Manage Actions](media/backup-azure-monitoring-laworkspace/azure-monitor-manage-actions.png) 

2. Navigate to the **Action rules (preview)** tab and click on **New action rule**.
...
4. Next, you must create an action Group. An action group is the destination to which the notification for an alert should be sent, for example, an email address.
 
![Create Action Group](media/backup-azure-monitoring-laworkspace/azure-monitor-create-action-group.png) 

5. In the **Basics** tab, select the name of the action group and the subscription and resource group under which it should be created.

![Action Groups Basic](media/backup-azure-monitoring-laworkspace/azure-monitor-action-groups-basic.png) 

6. In the **Notifications** tab, select **Email/SMS message/Push/Voice** and enter the recipient email id.

![Action Groups Notification](media/backup-azure-monitoring-laworkspace/azure-monitor-email.png) 

7. Click on **Review+Create** and then **Create** to deploy the action group.
