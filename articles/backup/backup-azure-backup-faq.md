...
Supported DPM versions are summarized in the [support matrix](backup-azure-dpm-introduction#prerequisites-and-limitations). We recommend that you install the latest DPM updates, and run the [latest version](http://aka.ms/azurebackup_agent) of the Azure Backup agent on the DPM server.
