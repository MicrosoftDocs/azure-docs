...
* If you delete or move (to another resource group or subscription) Azure VMs configured for Azure Backup without stopping protection, the on-demand backup job fails with the error *UserErrorVmNotFoundV2*. The backup pre-check appears as critical only for failed on-demand backup jobs (failed scheduled jobs don't appear).
* These backup items remain active in the system adhering to the backup and retention policy set by the user. The backed-up data for these Azure VMs will be retained according to the retention policy. The expired recovery points are cleaned according to the retention range set in the backup policy. However, up to two of the most recent recovery points (such as the last hardened (vaulted) restore point and the latest restore point) may be retained beyond policy expiry, depending on garbage collection cycles.
