...
Initial full backups to Azure typically transfer large amounts of data online and require more network bandwidth when compared to subsequent backups that transfer only incremental changes. Remote offices or datacenters in certain geographies don't always have sufficient network bandwidth, so these initial backups may take several days. This ends up using the same network that was provisioned for applications running on the on-premises datacenter.

Azure Backup supports "Offline Backup", which allows transferring initial backup data offline. It provides a mechanism to copy backup data onto physical storage-devices that are then shipped to a nearby Azure Datacenter and uploaded onto a Recovery Services Vault. This process ensures robust transfer of backup data without using any network bandwidth.
...
This mode is currently supported in only in preview. With the MARS Agent, this option takes advantage of the [Azure Data Box service](https://azure.microsoft.com/services/databox/) to ship Microsoft-proprietary, secure, and tamper-resistant transfer appliances with USB connectors, to your datacenter or remote office and writes backup data onto these devices. **This option saves the effort required to procure your own Azure compatible disks and connectors or provisioning temporary storage as a staging location.** In addition, Microsoft handles the end-to-end transfer logistics, which you can track through the Azure portal. An architecture describing the movement of backup data with this option is shown below.
...
| ------------------------------------------------------------ | ------------------------------------------------------------ | ------------------------------------------------------------ |
| Azure  Backup Deployment models                              | MARS  (Preview)                                              | MARS,  MABS, DPM-A                                           |
