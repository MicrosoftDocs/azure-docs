...
| ---     | ---    |
|Unable to download vault credential <br><br> Failed to download the vault credential file. (ID: 403) | <ul><li> Try downloading vault credentials using different browser <br> OR <br> <ul><li>	Launch IE, press F12. </li><li> Go to **Network** tab to clear IE cache and cookies </li> <li> Refresh the page </li> </ul> <li> Check if the subscription is disabled/expired<br><li> Check if any firewall rule is blocking vault credential file download <br> <li> Ensure you have not exhausted the limit on the vault (50 machines per vault) <br> <li> Ensure user has required Azure Backup permission to download vault credential and register server with vault, see [article](backup-rbac-rs-vault.md)</li></ul> | 
