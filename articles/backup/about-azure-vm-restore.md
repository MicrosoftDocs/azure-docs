...
- **Tier (snapshot vs. vault)**:  Azure VM backup happens in two phases:
  ...
  - Snapshot tier restores are faster (than restore from vault) because they eliminate the wait time for snapshots to copy to the vault before triggering the restore. So restore from the snapshot tier is also referred as **Instant Restore**.
...
- **Item Level Restore (ILR):** restoring individual files/folders inside the VM from the PIT
...
| ------------------------------------ | ------------------------------------------------------------ | ------------------------------------------------------------ |
| Restore an entire VM                 | Restores the entire VM to OLR (if the source VM still exists) or ALR | <li>     If the  source VM is lost or corrupt, then you can restore entire VM   <li>     Create  a copy of the VM  <li>     Restore  drill for audit or compliance  <li>     This  option might not work because of [pre-requisites/limitations](https://docs.microsoft.com/azure/backup/backup-support-matrix-iaas#supported-restore-methods) or [VMs with special   configuration](https://docs.microsoft.com/azure/backup/backup-azure-arm-restore-vms#restore-vms-with-special-configurations) |
| Restore specific disk of the VM      | Restore disks attached to the VM                             | <li>     If  the disk is lost or corrupted, then can you restore the disk and attach it to an existing impacted VM  <li>     If  you can't use the "Restore an entire VM" option because you want to customize or configure  the VM, then you can create a new VM from the restored disk.   <li>     Create  a copy of the disk  <li>     This  option might not work because of [pre-requisites/limitations](https://docs.microsoft.com/azure/backup/backup-support-matrix-iaas#supported-restore-methods) |
| Restore specific files within the VM | Choose restore point, browse, select files, and restore them to the same (or compatible) OS as the backed-up VM. | <li>     If  you know which specific files to restore, then use this option instead of  restoring the entire VM  <li>     This  option might not work because of [pre-requisites/limitations](https://docs.microsoft.com/azure/backup/backup-support-matrix-iaas#support-for-file-level-restore) |
| Restore an encrypted VM              | Restores the entire VM to ALR or restore specific disks      | <li>     If  the source backed-up VM is encrypted, then use this restore option  <li>     If  the entire VM is lost <li>  Drill for audit or compliance <li>  To create a copy of the VM |
| Cross Region Restore                 | Restore VM in a secondary region                             | <li> If the primary region isn’t available because of an outage <li> For performing a drill  for audit or compliance. |
