...
- Run the agent on a Microsoft Azure Backup Server (MABS) or a System Center Data Protection - Manager (DPM) server. In this scenario, machines and workloads back up to MABS/DPM, and then MABS/DPM backs up up to a vault in Azure using the MARS agent.
...
4. Select the storage replication type, and click **Save**.
...
2. In **Proxy Configuration**, specify how the agent running on the Windows machine will connect to the internet. Then click **Next**.
    ...
    - If you're using using a custom proxy specify the proxy settings, and credentials if needed.
