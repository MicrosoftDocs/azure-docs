...
This template helps you configure backup for an explicit list of containers in a storage account by using a vaulted backup policy with a daily schedule and retention of 30 days, weeks, months, and years for daily, weekly, monthly, and yearly backup, respectively. Use an explicit container list when you want to protect only selected containers instead of auto-protecting all present and future containers. To configure auto-protection, use the backup configuration generated by the Azure CLI `initialize-backupconfig --auto-protection true` command or the Azure PowerShell `New-AzDataProtectionBackupConfigurationClientObject -AutoProtection` cmdlet instead of `containersList`.
