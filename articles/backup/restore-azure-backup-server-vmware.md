...
This article describes how to use Microsoft Azure Backup Server (MABS) to restore VMware Virtual Machine (VM) recovery points. It explains the recovery workflow, highlights how to search or browse for recoverable data in the MABS Administrator Console, and outlines your options for restoring data or a full VM to the same location. For an overview of how MABS recovers protected data, see [Recover protected data](./backup-azure-alternate-dpm-server.md).
...
| **Individual file location recovery (ILR)** | If the protected VM is a Windows Server VM, individual files/folders inside the VM can be recovered using MABS’s Item Level Restore (ILR) capability. To recover individual files, see the procedure later in this article. |

## Restore a VMware VM recovery point
...
## Restore an individual file from a VMware VM recovery point
