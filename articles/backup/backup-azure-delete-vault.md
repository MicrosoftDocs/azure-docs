...
- **Step 3**: You must check all of the following three places to verify if there are any protected items:
  ...
  - **SQL Server instance**: Go to the vault dashboard menu > manage > **Backup Infrastructure** > Management servers > **Protected Servers**. In Protected Servers, select the server to unregister. To delete the vault, you must unregister all the servers. Right-click the protected server, and select Unregister.
