...
[Azure Backup service](/azure/backup/backup-overview) allows you to You can back up data from SAP HANA databases in an application in consistent manner. This article describes the Azure Backup architecture components and processes.
...
2. The Azure VM running an SAP HANA server is registered with the vault, and the databases to be backed up are [discovered](/azure/backup/tutorial-backup-sap-hana-db#discover-the-databases). To enable the Azure Backup service to discover databases, you must run this [preregistration script](https://go.microsoft.com/fwlink/?linkid=2173610) on the HANA server as a root user. 
   >[!Note]
...
6. To [configure backup](/azure/backup/tutorial-backup-sap-hana-db#configure-backup) on the discovered databases, choose the required backup policy and enable backups.
...
5. Azure Backup attempts to achieve speeds up to 420 MB/sec for non-log backups and up to 100 MB/sec for log backups. [Learn more](/azure/backup/tutorial-backup-sap-hana-db#understanding-backup-and-restore-throughput-performance) about backup and restore throughput performance.
