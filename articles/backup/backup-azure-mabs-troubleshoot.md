...
| Adding vCenter to MABS | Unable to communicate with VMware Server | **If the recommended action shown in the product doesn't work**, <br> Please ensure that the specified VMware server is up and running and reachable from DPM Server. Also, ensure that right credentials are specified for interacting with specified VMware Server. Please refer to [this article](https://technet.microsoft.com/en-us/library/mt742101(v=sc.12).aspx) for detailed steps on how to import certificates and disable secure authentication. |
