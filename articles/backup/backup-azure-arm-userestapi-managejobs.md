...
An operation such as triggering backup will always return a jobID. For eg: The final response of a [trigger backup REST API operation](backup-azure-arm-userestapi-backupAzureVM.md#example-responses-3) is as follows:
...
The Azure VM backup job is identified by "jobId" field and can be tracked as mentioned [here](https://docs.microsoft.com/rest/api/backup/jobdetails/) using a simple *GET* request.
...
The `{jobName}` is "jobId" mentioned above. The response is always 200 OK with the "status" field indicating the current status of the job. Once it is "Completed", the 'extendedInfo ' section reveals more details about the job.
