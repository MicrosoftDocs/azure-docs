...
description: Use an Azure Backup server to back up your system state and provide bare-metal recovery (BMR) protection.
services: backup
documentationcenter: ''
author:  markgalioto
manager:  carmonm
...
The Azure Backup server backs up system state and provides bare-metal recovery (BMR) protection.
...
-   **Bare-metal backup**: Backs up operating system files and all data except user data on critical volumes. By definition a BMR backup includes a system state backup. Provides protection when a computer won't start and you have to recover everything.

The following table summarizes what you can back up and recover. For detailed information about app versions that can be protected with system state and BMR, see [What can Azure Backup Server back up](backup-mabs-protection-matrix.md)
...
When a system state backup runs, Azure Backup Server communicates with Windows Server Backup to request a backup of the server's system state. By default Azure Backup Server and Windows Server Backup will use the drive with the most available free space, and information about this drive is saved in the PSDataSourceConfig.XML file. This is the drive Windows Server Backup will use to do backups to.
...
Windows Server Backup then creates a folder called WindowsImageBackup on the root of the. As it creates the backup, all of the data is placed in this folder. When the backup is finished, the file is transferred to the Azure Backup Server. Note that:

    -   This folder and its contents are not cleaned up when the backup or transfer is done. The best way to think of this is that the space is being reserved for the next time a backup is done.

    -   The folder is created every time a backup is done. The time and date stamp reflect the time of your last system state backup.

## BMR backup

1.  For BMR (including a system state backup), the backup job is performed directly to a share on the Backup Server server, and not to a folder on the protected server.

2.  Azure Backup Server calls Windows Server Backup and shares out the replica volume for that BMR backup. In this case, it doesn't tell Windows Server Backup to use the drive with the most free space. Instead, it uses the share that was created for the job.
...
-   For BMR protection, unlike for system state protection, Azure Backup Server doesn't have any space requirements on the protected computer. Windows Server Backup directly transfers the backups to the Azure Backup Server. Note that the job for this doesn't appear in the Azure Backup Server Jobs view.
...
2.  **Set up storage**. You can store backup data on disk, on tape, and in the cloud with Azure. For more information, see [Prepare data storage](http://docs.microsoft.com/system-center/dpm/plan-long-and-short-term-data-storage.md).
...
14. On the  **Summary** page, review your settings. After you click **Create Group**, initial replication of the data occurs. When data replication finishes, on the **Status** page, the protection group status shows as **OK**. Backup then takes place per the protection group settings.
...
1.  In the **Recovery** pane, find the computer you want to recover, and then click **Bare Metal Recovery**.
...
6.  Set up notification options. On the **Confirmation** page, click **Recover**.
...
5.  On  the **Choose how to restore the backup** page, select **Format and repartition disks**. On the next page, verify settings. To begin the restore, click **Finish**. A restart is required.
...
2.  Click **Another Server**, select the **Specify Location Type** page, and then select **Remote shared folder**. Provide the path to the folder that contains the recovery point.
