...
* Azure VM backup with Write Accelerator(WA) disk exclusion is now available, see this [article](backup-azure-vm-backup-faq.md#can-we-enable-azure-backup-service-for-a-VM-having-write-accelerator(WA)-enabled-disk?). However, VM backup doesn’t provide DB consistency since information on WA disks is not backed up and DB is not quiesced as part of backup. Hence this solution works if the user wants OS backup and backup of data present on non-WA disks.
* **Azure backup for SAP HANA**: A separate solution is built by Azure backup for backup of SAP HANA with an RPO of 15 mins. It is built in similar model as that of SQL DB backup and uses the backInt interface for 3rd party backup solutions certified by SAP HANA. The solution is in private preview and one can email to AskAzureBackupTeam@microsoft.com with the subject “Sign up for private preview for backup of SAP HANA in Azure VMs” for onboarding to private preview.

The combination of these two solutions will provide both DB backup and OS and important data backup in Azure VM with WA disks.
