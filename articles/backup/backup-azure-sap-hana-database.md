...
> [!NOTE]
> There is no service level proxy support i.e., traffic via the proxy from only few or selected services (Azure backup services) is not supported. The entire data/traffic could be routed by proxy or not.
