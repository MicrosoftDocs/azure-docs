...
While you restore disks for a Managed VM from a Vault-Standard tier recovery point, it restores the Managed disk and Azure Resource Manager (ARM) templates, along with VHD files of the disks in staging location. If you restore disks from a snapshot tier recovery point, it restores the Managed disks and ARM templates only.
