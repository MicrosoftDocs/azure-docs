...
This option creates a VM quickly. You can specify basic VM settings. [Review](#restore-options) all restore options.
...
This option restores the disk, and then you can do one of the following:
...
- [Attach the restored disks](https://docs.microsoft.com/azure/virtual-machines/windows/attach-disk-ps) to an existing VM.
- [Create a new VM](backup-azure-vms-automation.md#restore-an-azure-vm) from the restored disks using PowerShell.

[Review](#restore-options) all restore options.
...
This option replace the existing VM disk with the selected restore point. [Review](#restore-options) all restore options.
...
## Restore VMs using Hybrid Use Benefit

> If a Windows VM uses [Hybrid Use Benefit (HUB) licensing](../virtual-machines/windows/hybrid-use-benefit-licensing.md), restore the disks, and create a new VM using the provided template (with **License Type** set to **Windows_Server**), or PowerShell.  This setting can also be applied after creating the VM.


## Restore a VM during an Azure datacenter disaster

If the vault is using GRS, Azure Backup supports restoring backed-up VMs to the paired datacenter, if the primary datacenter is which the VM is running goes down. In this case you select a storage account in the paired datacenter, and restore as normal. Azure Backup uses the compute service in the paired location to create the restored VM. [Learn more](../resiliency/resiliency-technical-guidance-recovery-loss-azure-region.md) about datacenter resiliency.

## Restore domain controller VMs

You can back up Azure VMs acting as domain controllers. The restore process depends on the domain structure. In the simplest case, you have a single domain controller in a single domain. More commonly for production loads, you have a single domain with multiple domains controllers, perhaps some on-premises. Finally, you might have a forest with multiple domains.

- **Single domain controller in single domain**: Restore the VM like any other VM. 
- **Multiple domain controllers in a single domain**:  If  other domain controllers in the same domain can be reached over the network, the domain controller can be restored like any VM. If it's the last remaining domain controller in the domain, or a recovery in an isolated network is performed, use a forest recovery. [Learn more](https://docs.microsoft.com/windows-server/identity/ad-ds/manage/ad-forest-recovery-single-domain-in-multidomain-recovery).
- **Multiple domains in one forest**: We recommend a forest recovery. [Learn more](https://docs.microsoft.com/windows-server/identity/ad-ds/manage/ad-forest-recovery-single-domain-in-multidomain-recovery).

Note that:
- From an Active Directory perspective, the Azure VM is like any other VM.
- The major difference between Azure VMs and on-premises hypervisors is that there's no VM console available in Azure. A console is required for certain scenarios, such as recovering by using a bare-metal recovery (BMR)-type backup.
- However, VM restore from the vault is a full replacement for BMR.
- Directory Services Restore Mode (DSRM) is also available, so all Active Directory recovery scenarios are viable.
- [Learn more](https://docs.microsoft.com/windows-server/identity/ad-ds/get-started/virtual-dc/virtualized-domain-controllers-hyper-v) about backup and restore considerations for virtualized domain controllers. 


## Restore VMs with special network configurations

You can back up and restore VMs with special network configurations, but there are some considerations. Special network configurations include:

- VMs using internal/external load balancing.
- VMs with multiple reserved IPs
- VMs with multiple NICs.


Restore these VMs as follows:


1. Restore the disks from the vault with [PowerShell](backup-azure-vms-automation.md#restore-an-azure-vm).
2. Create the VM configuration using the PowerShell cmdlets:
   - Create a VM in the cloud service with an [internal load balancer](https://azure.microsoft.com/documentation/articles/load-balancer-internal-getstarted/).
   - Create a VM to connect to an [internet-facing load balancer](https://azure.microsoft.com/documentation/articles/load-balancer-internet-getstarted/).
   - Create a VM with [multiple NICs](../virtual-machines/windows/multiple-nics.md).
   - Create a VM with [multiple reserved IP addresses](../virtual-network/virtual-network-multiple-ip-addresses-powershell.md).
