...
- **Restore an existing database to a point-in-time in the past** within the retention period using the Azure portal, Azure PowerShell, Azure CLI, or REST API. In Single database and Elastic pools, this operation will create a new database in the same server as the original database. In Managed Instance, this operation can create a copy of the database or same or different Managed Instance under the same subscription.
  - **[Change Backup Retention Period](#change-pitr-backup-retention-period)** between 7 to 35 days to configure your backup policy.
...
The backup storage consumption varies between the General Purpose / Business Critical and Hyperscale service tier.
...
While the database is active, the total backup storage usage is calculated as follows: `Backup storage size = (size of full backups + size of diff backups + size of log backups) – database size`. Backups that are older than the retention period are automatically purged based on their timestamp. Because the differential backups and log backups require an earlier full backup to be useful, they are purged together in weekly chunks. 
...
The backup operations in this service tier reflect the design differences between Hyperscale and other service tiers. Unlike the General Purpose and Business Critical tiers, the Hyperscale tier does not use the traditional SQL server backup technology. Instead, the restore operation relies on the base blob (which is the data file of the database residing in Azure blob storage), periodic blob snapshots generated by the page servers, and the copy of the active log file in the log storage for point-in-time restore (PITR). The process of copying the active log file is called de-staging. Both snapshots and transaction log backups are stored in the standard RA-GRS blob. Every page server has two replicas for high availability, each with its own data file, but only one replica generates the billable snapshots. 
