...
Azure SQL Database is continuously monitoring queries being executed and looks for indexes that could improve the performance. Once enough confidence is built that a certain index is missing it will be recommended. Azure SQL Database builds confidence by measuring the expected benefits an index would bring through time. If the index benefit is continuously improving through time the index creation is recommended. Estimated impact (High/Medium/Low) is measured by looking at the slope of the benefit that a certain index would bring. 

Indexes created using recommendations are always flagged as auto_created indexes. You can see which indexes are auto_created by looking at sys.indexes view. Auto created indexes don’t block ALTER/RENAME commands. This means that if you try to drop the column that has an auto created index over it, the command will pass and the auto created index will be dropped with the command as well. Regular indexes would block the ALTER/RENAME command on columns that are indexed.

Once the create index recommendation is applied, Azure SQL Database will monitor its performance by comparing the new query performance with the performance from the baseline. If new index brought improvements in the performance it will be flagged as successful and impact report will be available. In case the index didn’t bring the benefits, it will be automatically reverted. This way Azure SQL Database ensures that using recommendations will only improve the database performance.

Any Create index recommendation has a back off policy that won’t allow applying the recommendation if the database or pool DTU usage was above 80% in last 20 minutes or if the storage is above 90% of usage. If the back off policy kick in, the recommendation implementation will be postponed.
