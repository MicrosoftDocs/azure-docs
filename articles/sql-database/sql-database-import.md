...
The following SqlPackage command imports the **AdventureWorks2008R2** database from local storage to an Azure SQL Database server called `<serverName>`. It creates a new database called `<migratedDatabase>` with a **Premium** service tier and a **P6** Service Objective. Change these values as appropriate for your environment.
