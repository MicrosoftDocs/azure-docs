...
If the virtual network includes a custom DNS, the custom DNS server must be able to resolve host names in \*.core.windows.net zone. Using additional features lik Azure AD Authentication might require resolving additional FQDNs. For more information, see [Set up a custom DNS](sql-database-managed-instance-custom-dns.md).
