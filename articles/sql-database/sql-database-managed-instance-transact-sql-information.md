...
[Replication](sql-database-managed-instance-transactional-replication.md) is available for public preview on managed instance with some constraints:
- All types of replication participants (Publisher, Distributor, Pull Subscriber, and Push Subscriber) can be placed on managed instance, but Publisher and Distributor cannot be placed on different instances.
- Transactional, Snapshot, and Bi-directional replication types are supported. Merge replication, Peer-to-peer replication, and updatable subscriptions are not supported.
