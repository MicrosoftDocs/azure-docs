...
Restore process will block these operations on the Managed instances and instance pools in the same subnet where restore process is running. Instances that already in instance pools are not affected. Create or change service tier operation will not fail or timeout, and they will proceed once the restore process is finished.
