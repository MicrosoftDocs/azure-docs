...
description: A tutorial that configures replication between a Publisher Managed Instance, a Distributor Managed Instance, and a SQL Server subscriber (either on-premises or on an Azure VM). 
...
ms.date: 02/20/2019
---
# Tutorial: Configure transactional replication between Managed Instances and SQL Server
...
> - Configure a SQL Server VM as a subscriber. 
...
- 
- Two Azure SQL Database managed instances within _the same virtual network_, one that will act as the Publisher, and one as the Distributor. 
...
- An instance of SQL Server on-premises, or hosted on a Windows VM in Azure, that acts as the Subscriber and has [SQL Server Management Studio (SSMS)](/sql/ssms/download-sql-server-management-studio-ssms) installed
- [VPN Peering]() configured between the vNet of the two Managed Instances and the SQL Server VM. 
