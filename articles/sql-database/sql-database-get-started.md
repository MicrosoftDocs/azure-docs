...
3. Click **SQL Database** to open the SQL Database blade. The content on this blade will vary depending on the number of your subscriptions and your existing objects (such as existing servers).

    ![New sql database 2](./media/sql-database-get-started/sql-database-new-database-2.png)

4. In the **Database name** text box, provide a name for your first database - such as "my-database". A green check mark will indicate that you have provided a valid name.
...
6. Under **Resource group**, click **Create new** and provide a name for your first resource group - such as "my-resource-group". A green check mark will indicate that you have provided a valid name.
...
9. On the Server blade, click **Create a new server**. An Azure SQL database is created within a server object. This can be either a new server or an existing server.
...
11. In the **Server name** text box, provide a name for your first server - such as "my-new-server-object". A green check mark will indicate that you have provided a valid name.

    ![New sql database 9](./media/sql-database-get-started/sql-database-new-database-9.png)
 
12. Under **Server admin login**, provide a user name for the administrator login for this server - such as "my-admin-account". This login is known as the server principal login. A green check mark will indicate that you have provided a valid name.

    ![New sql database 10](./media/sql-database-get-started/sql-database-new-database-10.png)

13. Under **Password** and **Confirm password**, provide a password for the server principal login account - such as "p@ssw0rd1"
...
15. Under **Create V12 server (Latest update), notice that you do not have an option to create an older version of Azure SQL server.

    ![New sql database 13](./media/sql-database-get-started/sql-database-new-database-13.png)

16. Notice that, by default, the checkbox for **Allow azure services to access server** is selected and cannot be changed here. This is an advanced option. For getting started, you will want to enable other Azure services to connect to your Azure SQL database through the SQL Database firewall. You can change this in the server firewall settings for this server object. 

    ![New sql database 14](./media/sql-database-get-started/sql-database-new-database-14.png)

17. On the New server blaade, review your selections and then click **Select** to select this new server for your new database.
...
>[AZURE.IMPORTANT]When deployment completes, you will have an Azure SQL server and database. However, you will not be able to connect to it using SQL Server tools until create a firewall rule to open the SQL Database firewall to connections from outside of Azure.
