...
Please follow these steps to create Azure Automation Account through the method of selecting and configuring Automation app from the Marketplace:
...
- For the “**Create Azure Run As account**” option, select **Yes**. This is required in order to configure the type of account under which PowerShell script will run with the help of Azure Automation. To learn more about account types, see [Run As account](https://docs.microsoft.com/en-us/azure/automation/automation-create-runas-account)
- Conclude creation of the automation account by clicking on **Create**

> [!TIP]
> Please **record** your Azure Automation account name, subscription ID and resources (i.e. copy-paste to a notepad) exactly as entered while creating the Automation app., as **this information** will be needed in the **further steps** below.
>

Please note that if you have several Azure subscriptions for which you would like to build the same automation, you will need to repeat this process for your other subscriptions.
...
Please follow these steps to update Azure PowerShell modules:
...
- In the Modules pane, click on “**Update Azure Modules**” at the top, and wait until the message “Azure modules have been updated” is displayed. Please note that this process can take a few minutes to complete.

![Update Azure automation modules](./media/sql-database-automatic-tuning/howto-email-02.png)

Required version of AzureRM.Resources and AzureRM.Sql modiles needs to be version 4 and above.

## Create Azure Automation Runbook

The next step is to create a Runbook in Azure Automation inside which the PowerShell script for retrieval of tuning recommendations will reside.

Please follow these steps to create a new Azure Automation runbook:

- Access the Azure Automation account you’ve just created in the previous step
- Once in the automation account pane, click on the “**Runbooks**” menu item on the left hand side. Please note this is the place where new Azure Automation runbook with the PowerShell script will be created. To learn more about creating automation runbooks, see [Creating a new runbook]().
...
Please follow these steps to load a PowerShell script inside the runbook created:

- Inside the “**Edit PowerShell Runbook**” pane, select “**RUNBOOKS**” on the menu tree and expand the view until you see the name of your runbook (i.e. in this example “**AutomaticTuningEmailAutomation**”). Select this runbook.
- On the first line of the “Edit PowerShell Runbook” (starting with the number 1), copy-paste the PowerShell script code provided to you below. Please note that this PowerShell script is provided as-is to get you started. Please feel free to modify the script to suite your needs.

Please note that in the header of the provided PowerShell script you need to replace "<SUBSCRIPTION_ID_WITH_DATABASES>" with your Azure subscription ID. To learn how to retrieve your Azure subscription ID, see [Getting your Azure Subscription GUID](https://blogs.msdn.microsoft.com/mschray/2016/03/18/getting-your-azure-subscription-guid-new-portal/).
...
Please click on the “**Save**” button in the upper right corner to save the script. When you are satisfied with the script, please click on the “**Publish**” button to publish this runbook. 

At the main runbook pane, you can choose to click on the “**Start**” button to **test** the script. Click on the “**Output**” to view results of the script executed. Please note that this output is going to be the content of your email. Sample output from the script can be seen in the screenshot below.

![Run view automatic tuning recommendations with Azure Automation](./media/sql-database-automatic-tuning/howto-email-04.png)

Please ensure to adjust the content by customizing the PowerShell script to your needs.
...
To complete the solution, as the final step, I will create an automation flow in Microsoft Flow consisting of three actions (jobs): 
...
3. “**Office 365 Outlook – Send an email**” – used to send out email. Please note emails will be sent out using the Office 365 account of the individual creating the flow.

To learn more about Microsoft Flow capabilities, please see [Getting started with Microsoft Flow](https://docs.microsoft.com/en-us/flow/getting-started).

Prerequisite for this step is to sign up for [Microsoft Flow](https://flow.microsoft.com) account and to log in. Once inside the solution, please follow these steps to setup a **new flow**:
...
- In the search field type “**recurrence**”, and select “**Schedule - Recurrence**” from the search results. This is how we are going to schedule our email delivery job to run.
- In the Recurrence pane in the Frequency field, select the scheduling frequency for this flow to execute, i.e. send automated email each Minute, Hour, Day, Week, etc.

The next step is to add three jobs (create, get output and send email) to the newly created recurring flow. To accomplish adding the required jobs to the flow, please follow these steps:
...
- Populate fields required. This is similar to creating the previous job: populate your Azure subscription ID, Resource Group and Automation Account (as entered in the Automation Account pane)
...
Statistics of running the automated jobs, i.e. showing success of email notifications sent out, can be seen from the Flow analytics pane.

![Running flow for automatic tuning email notifications](./media/sql-database-automatic-tuning/howto-email-06.png)

The Flow analytics is helpful for monitoring the success of job executions, and if required for troubleshooting. Please note that in case of troubleshooting, you also might want to examine the PowerShell script execution log accessible through Azure Automation app.

The final output of the automated email will look similar to the following email received after building and running this solution:

![Sample email output from automatic tuning email notifications](./media/sql-database-automatic-tuning/howto-email-07.png)

Please note that by adjusting the PowerShell script you can adjust the output and formatting of the automated email to your needs.
