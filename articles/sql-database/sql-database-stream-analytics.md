...
- A database in Azure SQL Database. For details, see [Create a single database in Azure SQL Database](sql-database-single-database-get-started.md).
- A firewall rule allowing your computer to connect to the server. For details, see [Create a server-level firewall rule](../azure-sql/database/quickstart-firewall-create-server-level-portal.md).
