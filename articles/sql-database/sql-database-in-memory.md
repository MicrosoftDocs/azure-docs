...
- analytics (online analytical processing (OLAP)) where most of the queries have complex calculations for reporting purposes, with a certain number of queries that load and append data to the exising tables (so called bulk load), or delete the data from the tables. 
