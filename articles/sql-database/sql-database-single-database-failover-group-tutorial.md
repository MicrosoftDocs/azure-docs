...
In this step, you will create a [failover group](sql-database-auto-failover-group.md) between an existing server and a new server in another region. Then add the sample database to the failover group.
...
1. On the **Failover Group** page, enter or select the following values, and then select **Create**:
   ...
   ![Add SQL Database to failover group](media/sql-database-single-database-failover-group-tutorial/add-sqldb-to-failover-group.png)
...
| [az sql db create](/cli/azure/sql/db?view=azure-cli-latest) | Creates a database in Azure SQL Database. |
