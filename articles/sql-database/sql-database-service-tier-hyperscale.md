...
Azure SQL Database is the SQL Server Database Engine architecture adjusted for the cloud environment in order to provide up 99.995% availability, even in the cases of infrastructure failures. There are three architectural models that are used in Azure SQL Database:
...
Unlike the General purpose and Business critical tiers, the Hyperscale tier does not use traditional SQL server backup technology. Instead, the restore operation relies on the periodic blob snapshots generated by the page servers, and the copy of the active log file in the log storage for PITR. The process of copying the active log file is called de-staging . Both snapshots and transaction log backups are stored in the standard RA-GRS blob. Every page server has two replicas for high availability, each with its own data file, but only one replica generates the billable snapshots. 

The Hyperscale backup service is designed to dramatically reduce the restore time and to make it independent of database size. For that reason, databases with more activity require snapshots at more frequent intervals. To strike a balance between restore performance and overall backup cost, an intelligent pruning algorithm is informed by the statistics of the point-in-time restores. For example, the most recent 3 days of retention contain more frequent snapshots than the remaining days of retention.  

The log storage is allocated dynamically in 32GB chunks. Its size directly depends on the transaction rate of the database. Each log backup in the log storage is associated with a specific snapshot. Once the snapshot is out of retention, it is automatically deleted along with the log backups associated with.  

The total backup storage usage is calculated as follows. For "live" databases, backup storage size = size of snapshots + (size of log storage blob –  32GB). While the base blob is required to restore the database from a snapshot, the size of the base blob is excluded from the calculated storage size. It is accounted for as data storage size. For the dropped database, the backups storage size is calculated the same way and the billing for it will continue for the duration of the retention period. While we will stop billing for the data storage after the database is dropped, the base blob itself will not be dropped as it is required to restore from a snapshot. This is done to guarantee consistency of the backup charges after the database is dropped.  
