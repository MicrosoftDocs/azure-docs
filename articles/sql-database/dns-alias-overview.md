---
title: DNS alias
description: Your applications can connect to an alias for the name of your logical SQL server used for Azure SQL Database. Meanwhile, you can change the SQL Database the alias points to anytime, to facilitate testing and so on.
...
The Internet relies on the DNS. The DNS translates your friendly names into the name of your logical SQL server in Azure.

## Scenarios with one DNS alias

Suppose you need to switch your system to a new logical SQL server in Azure. In the past you needed to find and update every connection string in every client program. But now, if the connection strings use a DNS alias, only an alias property must be updated.

The DNS alias feature of Azure SQL Database can help in the following scenarios:

### Test to production

When you start developing the client programs, have them use a DNS alias in their connection strings. You make the properties of the alias point to a test version of your logical SQL server in Azure.

Later when the new system goes live in production, you can update the properties of the alias to point to the production logical SQL server. No change to the client programs is necessary.

### Cross-region support

A disaster recovery might shift your logical SQL server to a different geographic region. For a system that was using a DNS alias, the need to find and update all the connection strings for all clients can be avoided. Instead, you can update an alias to refer to the new logical SQL server that now hosts your Azure SQL Database.

## Properties of a DNS alias

The following properties apply to each DNS alias for your logical SQL server:

- *Unique name:* Each alias name you create is unique across all logical SQL servers in Azure, just as server names are.
- *Server is required:* A DNS alias cannot be created unless it references exactly one server, and the server must already exist. An updated alias must always reference exactly one existing server.
  - When you drop a logical SQL server, the Azure system also drops all DNS aliases that refer to the server.
- *Not bound to any region:* DNS aliases are not bound to a region. Any DNS aliases can be updated to refer to a logical SQL server that resides in any geographic region.
...
- [New-AzSqlServerDNSAlias](https://docs.microsoft.com/powershell/module/az.Sql/New-azSqlServerDnsAlias): Creates a new DNS alias in the Azure SQL Database service system. The alias refers to logical SQL server 1.
- [Get-AzSqlServerDNSAlias](https://docs.microsoft.com/powershell/module/az.Sql/Get-azSqlServerDnsAlias): Get and list all the DNS aliases that are assigned to logical SQL server 1.
...
- _Table auditing is not supported:_ You cannot use a DNS alias on a logical SQL server that has *table auditing* enabled on a database.
