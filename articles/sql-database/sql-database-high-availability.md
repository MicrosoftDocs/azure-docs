...
- Local storage is used in the Premium or Business Critical databases and pools, which are designed for mission critical OLTP applications with high IOPS requirements. 
...
> [!IMPORTANT]
> Zone redundant databases and elastic pools are only supported in the Premium and Business Critical service tiers. During public preview, backups and audit records are stored in RA-GRS storage and therefore may not be automatically available in case of a zone-wide outage. 
...
## Read scale-out
As described, Premium and Business Critical service tiers leverage quorum-sets and AlwaysON technology for High Availability both in single zone and zone redundant configurations. One of the benefits of AlwasyON is that the replicas are always in the transactionally consistent state. Because the replicas have the same performance level as the primary, the application can take advantage of that extra capacity for servicing the read-only workloads at no extra cost (read scale-out). This way the read-only queries will be isolated from the main read-write workload and will not affect its performance. Read scale-out feature is intended for the applications that include logically separated read-only workloads such as analytics, and therefore could leverage this additional capacity without connecting to the primary. 

In order to use the feature with a particular database, users must explicitly enable it when creating the database or afterwards by altering its configuration using [Azure CLI](./),[ PowreShell](./) or [REST API](./).

After the feature has been enabled for a given database, the SQL sessions must be configured for [read-only routing](./) by specifying ApplicationIntent=ReadOnly in the connection string. It will ensure that the session will connected to a read-only replica.
