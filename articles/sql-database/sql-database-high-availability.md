...
- Remote storage is used for Basic and Standard service tiers, designed for budget oriented business workloads small that require storage and compute power to scale independently. They use a single page blob for database and log files, and built-in storage replication and failover mechanisms.
