...
In addition, Business Critical cluster provides built-in read-only node that can be used to run read-only queries (for example reports) that should not affect performance of your primary workload. Find more information about [Read Scale-Out here](sql-database-read-scale-out.md).
