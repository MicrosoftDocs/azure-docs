...
The database per tenant pattern isolates tenant data effectively, but increases the number of databases to manage and maintain. [Elastic Jobs](sql-database-elastic-jobs-overview.md) facilitates administration and management of SQL databases. Jobs enable you to securely and reliably, run tasks (T-SQL scripts) independent of user interaction or input, against a group of databases. Jobs can be used to deploy schema and common reference data changes across all tenant databases in an application. Elastic Jobs can also be used to maintain a *template* database that is used to create new tenants, ensuring it always has the latest schema and reference data.
...
Each tenant database includes a set of supported venue types that define the kind of events that can be hosted at a venue. These venue types correspond to the background images you see in the venue events app. To support new venue types this reference data must be updated and new venue types added.  In this exercise, you deploy an update to all the tenant databases to add two additional venue types: *Motorcycle Racing* and *Swimming Club*.
...
* **sp\_add\_target\_group\_member** is used to define the set of target databases.  First the _tenants1-dpt-&lt;user&gt;_ server is added, which causes all the databases in that server at the time of job execution to be included. Then it is used to add the _basetenantdb_ database  and the *adhocreporting* database that is used in a later tutorial.
...
Like the previous exercise, this exercise creates a job to rebuild the index on the reference table primary key.  This is a typical database management operation an administrator might do after loading large amounts of data.
...
Next, try the [Ad-hoc reporting tutorial](saas-tenancy-adhoc-analytics.md) to explore running distributed queries across tenant databases.
