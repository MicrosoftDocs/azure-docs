...
- **Failover group**: One or many failover groups can be created between two servers in different regions (primary and secondary servers). Each group can include one or several databases that are recovered as unit in case all or some primary databases become unavailable due to an outage in the primary region.
- **Use auto-failover groups**: It will create geo-secondary database with the same service objective as the primary. If you add an existing geo-replication relationship to the failover group make sure the geo-secondary is configured with the same service level objective as the primary.
- Use **ApplicationIntent=ReadOnly**: If you have the read-only portions of the workload that is tolerant to certain staleness of data, indicate in connection string read intent by using **ApplicationIntent=ReadOnly** and the connection will be automatically directed to the secondary. 
- If an outage is detected, SQL will automatically trigger read-write failover if there is zero data loss to the best of our knowledge. Otherwise, it will wait for the period you specified by **GracePeriodWithDataLossHours**. 
...
| Create failover group | Creates a new failover group and registers it on both primary and secondary servers |
| Remove Failover Group | Removes the failover group from the server |
