...
The secondary database must be in the same service tier as the primary, so migrating your primary database to a different service tier requires you to terminate or drop the secondary database. Migrate the primary to the new service tier, and reconfigure geo-replication.
