...
- **Use secondary databases for read-only workload**: If you have the read-only portions of the workload that is tolerant to certain staleness of data, you can use the secondary database to process these read-only requests. Use **<failover-group-name>.secondary.database.windows.net** as the server URL and the connection will be automatically dirceted to the secondary. It is also recommended that you indicate in connection string read intent by using **ApplicationIntent=ReadOnly**. 
