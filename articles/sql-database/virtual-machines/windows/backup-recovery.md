...
Automated Backup provides an automatic backup service for SQL Server Standard and Enterprise editions running in a Windows VM in Azure. This service is provided by the [SQL Server IaaS Agent Extension](sql-iaas-agent-extension.md), which is automatically installed on SQL Server Windows virtual machine images in the Azure portal.
...
|---|---|---|---|
| Requires additional Azure service |   | ![Yes](./media/backup-recovery/yes.png) |   |
| Configure backup policy in Azure portal | ![Yes](./media/backup-recovery/yes.png) | ![Yes](./media/backup-recovery/yes.png) |   |
| Restore databases in Azure portal |   | ![Yes](./media/backup-recovery/yes.png) |   |
| Manage multiple servers in one dashboard |   | ![Yes](./media/backup-recovery/yes.png) |   |
| Point-in-time restore | ![Yes](./media/backup-recovery/yes.png) | ![Yes](./media/backup-recovery/yes.png) | ![Yes](./media/backup-recovery/yes.png) |
| 15-minute Recovery Point Objective (RPO) | ![Yes](./media/backup-recovery/yes.png) | ![Yes](./media/backup-recovery/yes.png) | ![Yes](./media/backup-recovery/yes.png) |
| Short-term backup retention policy (days) | ![Yes](./media/backup-recovery/yes.png) | ![Yes](./media/backup-recovery/yes.png) |   |
| Long-term backup retention policy (months, years) |   | ![Yes](./media/backup-recovery/yes.png) |   |
| Built-in support for SQL Server Always On |   | ![Yes](./media/backup-recovery/yes.png) |   |
| Backup to Azure Storage account(s) | ![Yes](./media/backup-recovery/yes.png)(automatic) | ![Yes](./media/backup-recovery/yes.png)(automatic) | ![Yes](./media/backup-recovery/yes.png)(customer managed) |
| Management of storage and backup files | | ![Yes](./media/backup-recovery/yes.png) |  |
| Backup to attached disks on the VM |   |   | ![Yes](./media/backup-recovery/yes.png) |
| Central customizable backup reports |   | ![Yes](./media/backup-recovery/yes.png) |   |
| Consolidated email alerts for failures |   | ![Yes](./media/backup-recovery/yes.png) |   |
| Customize monitoring based on Azure Monitor logs |   | ![Yes](./media/backup-recovery/yes.png) |   |
| Monitor backup jobs with SSMS or Transact-SQL scripts | ![Yes](./media/backup-recovery/yes.png) | ![Yes](./media/backup-recovery/yes.png) | ![Yes](./media/backup-recovery/yes.png) |
| Restore databases with SSMS or Transact-SQL scripts | ![Yes](./media/backup-recovery/yes.png) |   | ![Yes](./media/backup-recovery/yes.png) |

## Next steps

If you are planning your deployment of SQL Server in an Azure VM, you can find provisioning guidance in the following guide: [How to provision a Windows SQL Server virtual machine in the Azure portal](create-vm-portal.md).

Although backup and restore can be used to migrate your data, there are potentially easier data migration paths to SQL Server on an Azure VM. For a full discussion of migration options and recommendations, see [Migrating a Database to SQL Server on an Azure VM](migrate-sql-server.md).
