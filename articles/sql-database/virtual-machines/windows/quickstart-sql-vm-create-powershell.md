...
> [!TIP]
> - This quickstart provides a path for quickly provisioning and connecting to a SQL VM. For more information about other Azure PowerShell options for creating SQL VMs, see the [Provisioning guide for SQL Server VMs with Azure PowerShell](create-vm-powershell.md).
...
To get portal integration and SQL VM features, you must install the [SQL Server IaaS Agent Extension](sql-iaas-agent-extension.md). To install the agent on the new VM, run the following command after the VM is created.
...
You're now connected to SQL Server locally. If you want to connect remotely, you must [configure connectivity](connect.md) from the portal or manually.
...
> [Migrate a database to a SQL VM](migrate-sql-server.md)
