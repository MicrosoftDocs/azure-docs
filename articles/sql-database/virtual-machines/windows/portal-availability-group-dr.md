...
The following image shows a common deployment of an availability group on Azure virtual machines:
   ...
   ![Availability Group](./media/portal-availability-group-dr/00-availability-group-basic.png)

In this deployment, all virtual machines are in one Azure region. The availability group replicas can have synchronous commit with automatic failover on SQL-1 and SQL-2. To build this architecture, see [Availability Group template or tutorial](portal-availability-group-overview.md).
...
This architecture is vulnerable to downtime if the Azure region becomes inaccessible. To overcome this vulnerability, add a replica in a different Azure region. The following diagram shows how the new architecture would look:
   ...
   ![Availability Group DR](./media/portal-availability-group-dr/00-availability-group-basic-dr.png)
...
The following diagram shows how the networks communicate between data centers.
   ...
   ![Availability Group](./media/portal-availability-group-dr/01-vpngateway-example.png)
...
1. [Create a SQL Server virtual machine in the new region](create-vm-portal.md).

1. [Create an Azure load balancer in the network on the new region](portal-availability-group-tutorial.md#configure-internal-load-balancer).
...
1. [Add Failover Clustering feature to the new SQL Server](portal-availability-group-prereq.md#add-failover-clustering-features-to-both-sql-server-vms).

1. [Join the new SQL Server to the domain](portal-availability-group-prereq.md#joinDomain).

1. [Set the new SQL Server service account to use a domain account](portal-availability-group-prereq.md#setServiceAccount).

1. [Add the new SQL Server to the Windows Server Failover Cluster](portal-availability-group-tutorial.md#addNode).
...
1. Add an IP address resource to the cluster.
   ...
   ![Cluster properties](./media/portal-availability-group-dr/cluster-name-properties.png)
   ...
   ![Add cluster IP](./media/portal-availability-group-dr/add-cluster-ip-address.png)
...
1. Add the IP address as a dependency for the core cluster name.
   ...
   ![Cluster properties](./media/portal-availability-group-dr/cluster-ip-dependencies.png)
...
1. Add an IP address resource to the availability group role in the cluster. 
   ...
   ![Create IP Address](./media/portal-availability-group-dr/20-add-ip-resource.png)
...
1. Add the IP address resource as a dependency for the listener client access point (network name) cluster.
   ...
   ![Availability Group](./media/portal-availability-group-dr/50-configure-dependency-multiple-ip.png)
...
1. [Set the cluster parameters in PowerShell](portal-availability-group-tutorial.md#setparam).
...
1. [Open firewall ports on the new SQL Server](portal-availability-group-prereq.md#endpoint-firewall).
...
* [Azure Load Balancers](portal-availability-group-tutorial.md#configure-internal-load-balancer)
