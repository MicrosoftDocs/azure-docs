...
For more information about the resource provider, see [SQL VM resource provider](register-with-resource-provider.md). 

## Prerequisites

To register your SQL Server VM with the resource provider, you'll need the following: 

- An [Azure subscription](https://azure.microsoft.com/free/) that has been [registered with the resource provider](register-with-resource-provider.md#register-subscription-with-rp) and contains unregistered SQL Server virtual machines. 
...
- Registration with the resource provider requires a guest agent running on the SQL Server VM. Windows Server 2008 images do not have a guest agent, so these virtual machines will fail and must be registered manually using the [NoAgent management mode](register-with-resource-provider.md#management-modes).
