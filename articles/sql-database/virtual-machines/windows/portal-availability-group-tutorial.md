...
![Availability Group](./media/portal-availability-group-tutorial/00-EndstateSampleNoELB.png)
...
|----- |----- |----- |
|![Square](./media/portal-availability-group-tutorial/square.png) | Two SQL Servers | - In an Azure availability set <br/> - In a single domain <br/> - With Failover Clustering feature installed |
...
|![Square](./media/portal-availability-group-tutorial/square.png)|Add Failover Clustering Feature | Both SQL Servers require this feature |
...
Before you begin the tutorial, you need to [Complete prerequisites for creating Always On Availability Groups in Azure Virtual Machines](portal-availability-group-prereq.md). If these prerequisites are completed already, you can jump to [Create Cluster](#CreateCluster).

  >[!NOTE]
  > Many of the steps provided in this tutorial can now be automated with [Azure SQL VM CLI](availability-group-cli.md) and [Azure Quickstart Templates](availability-group-quickstart-template.md).
...
1. RDP to the first SQL Server using a domain account that is an administrator on both SQL Servers and the witness server.
   ...
   >[!TIP]
   >If you followed the [prerequisites document](portal-availability-group-prereq.md), you created an account called **CORP\Install**. Use this account.
...
3. In the left pane, right-click **Failover Cluster Manager**, and then click **Create a Cluster**.
   ![Create Cluster](./media/portal-availability-group-tutorial/40-createcluster.png)
...
2. Right-click the failed **IP Address** resource, and then click **Properties**.
   ...
   ![Cluster Properties](./media/portal-availability-group-tutorial/42_IPProperties.png)
...
1. In the browser tree, right-click the cluster and click **Add Node**.
    ...
    ![Add Node to the Cluster](./media/portal-availability-group-tutorial/44-addnode.png)
...
8. In the **Confirmation** page if you are using Storage Spaces, clear the checkbox labeled **Add all eligible storage to the cluster.**
   ...
   ![Add Node Confirmation](./media/portal-availability-group-tutorial/46-addnodeconfirmation.png)
...
1. Right-click **Shares**, and click **New Share...**.
   ...
   ![New Share](./media/portal-availability-group-tutorial/48-newshare.png)
...
1. Make sure that the account used to create the cluster has full control.
   ...
   ![New Share](./media/portal-availability-group-tutorial/50-filesharepermissions.png)
...
1. In **Failover Cluster Manager**, right-click the cluster, point to **More Actions**, and click **Configure Cluster Quorum Settings...**.
   ...
   ![New Share](./media/portal-availability-group-tutorial/52-configurequorum.png)
...
3. Click the **AlwaysOn High Availability** tab, then select **Enable AlwaysOn Availability Groups**, as follows:
    ...
    ![Enable AlwaysOn Availability Groups](./media/portal-availability-group-tutorial/54-enableAlwaysOn.png)
...
1. Make sure that the SQL Server and SQL Server Agent service accounts for both servers have full control.
   ...
   ![New Share](./media/portal-availability-group-tutorial/68-backupsharepermission.png)
...
1. On remote desktop session to the first SQL Server. In **Object Explorer** in SSMS, right-click **AlwaysOn High Availability** and click **New Availability Group Wizard**.
    ...
    ![Launch New Availability Group Wizard](./media/portal-availability-group-tutorial/56-newagwiz.png)
...
2. In the **Introduction** page, click **Next**. In the **Specify Availability Group Name** page, type a name for the Availability Group, for example **AG1**, in **Availability group name**. Click **Next**.
    ...
    ![New AG Wizard, Specify AG Name](./media/portal-availability-group-tutorial/58-newagname.png)
...
3. In the **Select Databases** page, select your database and click **Next**.
   ...
   ![New AG Wizard, Select Databases](./media/portal-availability-group-tutorial/60-newagselectdatabase.png)
4. In the **Specify Replicas** page, click **Add Replica**.

   ![New AG Wizard, Specify Replicas](./media/portal-availability-group-tutorial/62-newagaddreplica.png)
5. The **Connect to Server** dialog pops up. Type the name of the second server in **Server name**. Click **Connect**.
   ...
   ![New AG Wizard, Specify Replicas (Complete)](./media/portal-availability-group-tutorial/64-newagreplica.png)

6. Click **Endpoints** to see the database mirroring endpoint for this Availability Group. Use the same port that you used when you set the [firewall rule for database mirroring endpoints](portal-availability-group-prereq.md#endpoint-firewall).

    ![New AG Wizard, Select Initial Data Synchronization](./media/portal-availability-group-tutorial/66-endpoint.png)
...
8. In the **Select Initial Data Synchronization** page, select **Full** and specify a shared network location. For the location, use the [backup share that you created](#backupshare). In the example it was, **\\\\\<First SQL Server\>\Backup\\**. Click **Next**.
   ...
   >Full synchronization takes a full backup of the database on the first instance of SQL Server and restores it to the second instance. For large databases, full synchronization is not recommended because it may take a long time. You can reduce this time by manually taking a backup of the database and restoring it with `NO RECOVERY`. If the database is already restored with `NO RECOVERY` on the second SQL Server before configuring the Availability Group, choose **Join only**. If you want to take the backup after configuring the Availability Group, choose **Skip initial data synchronization**.
    ...
    ![New AG Wizard, Select Initial Data Synchronization](./media/portal-availability-group-tutorial/70-datasynchronization.png)

9. In the **Validation** page, click **Next**. This page should look similar to the following image:

    ![New AG Wizard, Validation](./media/portal-availability-group-tutorial/72-validation.png)
...
10. In the **Summary** page, click **Finish**, then wait while the wizard configures the new Availability Group. In the **Progress** page, you can click **More details** to view the detailed progress. Once the wizard is finished, inspect the **Results** page to verify that the Availability Group is successfully created.
     ...
     ![New AG Wizard, Results](./media/portal-availability-group-tutorial/74-results.png)
...
1. In **Object Explorer**, expand **AlwaysOn High Availability**, then expand **Availability Groups**. You should now see the new Availability Group in this container. Right-click the Availability Group and click **Show Dashboard**.
   ...
   ![Show AG Dashboard](./media/portal-availability-group-tutorial/76-showdashboard.png)

   Your **AlwaysOn Dashboard** should look similar to this.

   ![AG Dashboard](./media/portal-availability-group-tutorial/78-agdashboard.png)
...
2. In **Failover Cluster Manager**, click your cluster. Select **Roles**. The Availability Group name you used is a role on the cluster. That Availability Group does not have an IP address for client connections, because you did not configure a listener. You will configure the listener after you create an Azure load balancer.
   ...
   ![AG in Failover Cluster Manager](./media/portal-availability-group-tutorial/80-clustermanager.png)
...
1. Search for **Load Balancer**. Choose the load balancer published by Microsoft.
   ...
   ![AG in Failover Cluster Manager](./media/portal-availability-group-tutorial/82-azureloadbalancer.png)
...
1. Configure the following parameters for the load balancer.
   ...
   ![Create Load Balancer](./media/portal-availability-group-tutorial/84-createloadbalancer.png)
...
1. In the Azure portal, go to your availability group. You might need to refresh the view to see the newly created load balancer.
   ...
   ![Find Load Balancer in Resource Group](./media/portal-availability-group-tutorial/86-findloadbalancer.png)
...
> [!NOTE]
> This tutorial shows how to create a single listener - with one ILB IP address. To create one or more listeners using one or more IP addresses, see [Create Availability Group listener and load balancer | Azure](portal-ps-alwayson-int-listener.md?toc=%2fazure%2fvirtual-machines%2fwindows%2ftoc.json).
>
>
...
- [Add an IP address to a load balancer for a second availability group](portal-ps-alwayson-int-listener.md#Add-IP).
