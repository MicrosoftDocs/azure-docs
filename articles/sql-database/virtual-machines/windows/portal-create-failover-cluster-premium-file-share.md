...
> [!IMPORTANT]
> At this time, SQL Server failover cluster instances on Azure virtual machines are only supported with the [lightweight management mode](register-with-resource-provider.md#management-modes) of the [SQL Server IaaS Agent Extension](sql-iaas-agent-extension.md). To change from full extension mode to lightweight, delete the **SQL Virtual Machine** resource for the corresponding VMs and then register them with the SQL VM resource provider in lightweight mode. When deleting the **SQL Virtual Machine** resource using the Azure portal, **clear the checkbox next to the correct Virtual Machine**. The full extension supports features such as automated backup, patching, and advanced portal management. These features will not work for SQL VMs after the agent is reinstalled in lightweight management mode.

Premium file shares provide IOPS and throughout capacities that will meet the needs of many workloads. For IO-intensive workloads, consider [SQL Server Failover Cluster Instances with Storage Spaces Direct](portal-create-failover-cluster.md), based on managed premium disks or ultra disks.  
...
To compare pay-as-you-go and BYOL licensing for SQL Server on Azure virtual machines, see [Get started with SQL VMs](overview.md#get-started-with-sql-vms).
...
Filestream isn't supported for a failover cluster with a premium file share. To use filestream, deploy your cluster by using [Storage Spaces Direct](portal-create-failover-cluster.md).
...
1. Create the virtual machines in the availability set.
   ...
   Provision two SQL Server virtual machines in the Azure availability set. For instructions, see [Provision a SQL Server virtual machine in the Azure portal](create-vm-portal.md).
   ...
   Choose an image from Azure Marketplace. You can use an Azure Marketplace image that includes Windows Server and SQL Server, or use one that just includes Windows Server. For details, see [Overview of SQL Server on Azure virtual machines](overview.md).
...
1. If you're using one of the SQL Server-based virtual machine images, remove the SQL Server instance.
   ...
   1. Remove all features under **Database Engine Services**. Don't remove **Shared Features**. You'll see something like the following screenshot:
        ...
        ![Select Features](./media/portal-create-failover-cluster-premium-file-share/03-remove-features.png)
...
1. [Add the virtual machines to your pre-existing domain](portal-availability-group-prereq.md#joinDomain).
...
1. Select the drive letter you want to use from the drop-down list and then copy both code blocks to Notepad.
   ...
   :::image type="content" source="media/portal-create-failover-cluster-premium-file-share/premium-file-storage-commands.png" alt-text="Copy both PowerShell commands from the file share connect portal":::
...
1. [Add Failover Clustering to each virtual machine](portal-availability-group-prereq.md#add-failover-clustering-features-to-both-sql-server-vms).
...
1. Under **Test Selection**, select all tests except for **Storage** and **Storage Spaces Direct**, as shown here:
   ...
   :::image type="content" source="media/portal-create-failover-cluster-premium-file-share/cluster-validation.png" alt-text="Select cluster validation tests":::
...
:::image type="content" source="media/portal-create-failover-cluster-premium-file-share/test-cluster-failover.png" alt-text="Test cluster failover by moving the core resource to the other nodes":::
...
1. Select **New SQL Server failover cluster installation**. Follow the instructions in the wizard to install the SQL Server FCI.
   ...
   :::image type="content" source="media/portal-create-failover-cluster-premium-file-share/use-file-share-as-data-directories.png" alt-text="Use file share as SQL data directories":::
...
For more information, see [Create and configure an Azure load balancer](portal-availability-group-tutorial.md#configure-internal-load-balancer).
...
1. Set up the load balancer by using the following values:
   ...
   ![Set up the load balancer](./media/portal-create-failover-cluster-premium-file-share/30-load-balancer-create.png)
