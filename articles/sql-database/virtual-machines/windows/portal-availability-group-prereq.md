...
This tutorial shows how to complete the prerequisites for creating a [SQL Server Always On availability group on Azure virtual machines (VMs)](portal-availability-group-tutorial.md). When you've finished the prerequisites, you have a domain controller, two SQL Server VMs, and a witness server in a single resource group.
...
![Availability group](./media/portal-availability-group-prereq/00-EndstateSampleNoELB.png)
...
2. Click **+** to create a new object in the portal.
   ...
   ![New object](./media/portal-availability-group-prereq/01-portalplus.png)

3. Type **resource group** in the **Marketplace** search window.

   ![Resource group](./media/portal-availability-group-prereq/01-resourcegroupsymbol.png)
...
9. Verify that **Pin to dashboard** is checked. This optional setting places a shortcut for the resource group on the Azure portal dashboard.
   ...
   ![Resource group](./media/portal-availability-group-prereq/01-resourcegroup.png)
...
1. In the Azure portal, in your resource group, click **+ Add**. 
   ...
   ![New item](./media/portal-availability-group-prereq/02-newiteminrg.png)
2. Search for **virtual network**.

     ![Search virtual network](./media/portal-availability-group-prereq/04-findvirtualnetwork.png)
...
5. Click **Create**.
   ...
   ![Configure the virtual network](./media/portal-availability-group-prereq/06-configurevirtualnetwork.png)
...
3. On the **autoHAVNET** virtual network, under **Settings** select **Subnets**.
   ...
   ![Configure the virtual network](./media/portal-availability-group-prereq/07-addsubnet.png)
...
7. Click **OK**.
    ...
    ![Configure the virtual network](./media/portal-availability-group-prereq/08-configuresubnet.png)
...
1. In the portal, open the **SQL-HA-RG** resource group and select the **ad-primary-dc** machine. On **ad-primary-dc**, click **Connect** to open an RDP file for remote desktop access.
    ...
    ![Connect to a virtual machine](./media/portal-availability-group-prereq/20-connectrdp.png)
...
4. Click the **Add roles and features** link on the dashboard.
    ...
    ![Server Manager - Add roles](./media/portal-availability-group-prereq/22-addfeatures.png)
...
6. Select the **Active Directory Domain Services** and **DNS Server** roles. When you're prompted, add any additional features that are required by these roles.
   ...
   >
    ...
    ![Add Roles dialog](./media/portal-availability-group-prereq/23-addroles.png)
...
11. Click the **More** link on the yellow warning bar.
    ...
    ![AD DS dialog on the DNS Server VM](./media/portal-availability-group-prereq/24-addsmore.png)
...
![Network interfaces](./media/portal-availability-group-prereq/25-primarydcip.png)
...
4. In **Network and Sharing Center**, click the network interface.
   ![Network interface](./media/portal-availability-group-prereq/26-networkinterface.png)
...
17. Under **Deployment Configuration**, select **Add a domain controller to an existing domain**.
    ![Deployment configuration](./media/portal-availability-group-prereq/28-deploymentconfig.png)
18. Click **Select**.
...
4. On the right **Tasks** pane, select **New**, and then click **User**.
   ![Active Directory Administrative Center](./media/portal-availability-group-prereq/29-addcnewuser.png)
...
1. In the **Active Directory Administrative Center**, select **corp (local)** in the left pane. Then in the right-hand **Tasks** pane, click **Properties**.
    ...
    ![CORP user properties](./media/portal-availability-group-prereq/31-addcproperties.png)
...
6. Select the **Create Computer objects** check box.
     ...
     ![Corp user permissions](./media/portal-availability-group-prereq/33-addpermissions.png)
...
> [!NOTE]
> The machine sizes suggested here are meant for testing availability groups in Azure VMs. For the best performance on production workloads, see the recommendations for SQL Server machine sizes and configuration in [Performance best practices for SQL Server in Azure virtual machines](performance.md?toc=%2fazure%2fvirtual-machines%2fwindows%2ftoc.json).
>
>
...
Repeat the steps on the other SQL Server VM.
  ...
  >[!NOTE]
  > This step, along with actually joining the SQL Server VMs to the failover cluster, can now be automated with [Azure SQL VM CLI](availability-group-cli.md) and [Azure Quickstart Templates](availability-group-quickstart-template.md).
...
4. For the port, specify **TCP** and type the appropriate port numbers. See the following example:
   ...
   ![SQL firewall](./media/portal-availability-group-prereq/35-tcpports.png)
...
* [Create a SQL Server Always On availability group on Azure virtual machines](portal-availability-group-tutorial.md)
