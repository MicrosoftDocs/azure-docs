...
The Read Scale-Out feature is available on Premium, Business Critical, and Hyperscale databases. The feature is disabled by default for new Premium and Business Critical databases, and can be enabled at database creation time, or at a later time. For Hyperscale databases, one readable replica is created by default, thus enabling Read Scale-out. Removing all readable replicas for a Hyperscale database disables Read Scale-out.
