...
Azure SQL Managed Instance must be deployed within an Azure [virtual network](../virtual-network/virtual-networks-overview.md) and the subnet dedicated for SQL Managed Instances only. You can use the existing virtual network and subnet if it's configured according to the [SQL Managed Instance virtual network requirements](sql-database-managed-instance-connectivity-architecture.md#network-requirements).
...
> [!Note]
> You can create a SQL Managed Instance only in virtual networks created through the Azure Resource Manager deployment model. Azure virtual networks created through the classic deployment model are not supported. Calculate subnet size by following the guidelines in the [Determine the size of subnet for SQL Managed Instances](sql-database-managed-instance-determine-size-vnet-subnet.md) article. You can't resize the subnet after you deploy the resources inside.
>
> After the SQL Managed Instance is created, moving the instance or VNet to another resource group or subscription is not supported.
...
- For an overview, see [What is SQL Managed Instance?](sql-database-managed-instance.md).
