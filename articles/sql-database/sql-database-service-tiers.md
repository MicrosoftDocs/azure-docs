...
|vCore-based model|This model allows you to independently scale compute, storage and IO resources. It also allows you to use Azure Hybrid Benefit for SQL Server to gain cost savings.|Best for customers who value flexibility, control and transparency.|
||||  
...
Service tiers are differentiated by a range of performance levels, high availability design, fault isolation, types of storage and IO range. The customer  must separately configure the required storage and retention period for backups. When using the vCore model the single databases and elastic pools are eligible for up to 30 percent savings with the  [Azure Hybrid Use Benefit for SQL Server](../virtual-machines/windows/hybrid-use-benefit-licensing.md).
...
- You can select any database size between 5GB and the supported maximum with the 1GB increments
