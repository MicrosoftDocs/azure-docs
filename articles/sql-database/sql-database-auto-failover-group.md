...
- **Initial Seeding** 
  ...
  There is an initial seeding phase that may take some time, depending on the size of the data. Once initial seeding completes, the data is synchronized, and then only subsequent changes are replicated. As such, the initial seeding phase during failover group configuration is the longest and most expensive operation. Typical seeding speed is about 18-35 GB an hour for a managed instance, and 50-500 GB for a single database or elastic pool, under normal circumstances. For managed instances, the speed of replication is also goverened by the Express Route bandwidth setup between the two managed instances. If the speed of the link is slow, the time to seed will be even longer. Additionally, every database is synchronized in parallel.  You can use the typical seeding rate per hour, as well as the number and size of databases to estimate the size of the link needed. For example, if there is a single database of 100 GB, it would typically take anyhwere from 2.8 - 5.5 hours for the initial seeding to complete. If there are multiple datbases that are seeding, this would be done in parallel which is likely to take longer due to the combination of link speed and parallel seed data consumption exceeding the link bandwidth.
