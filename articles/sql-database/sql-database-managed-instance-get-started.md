...
4. Fill out the **SQL managed instance** form with the requested information, using the information in the following table:
   ...
   |**Allow access from**   |Select one of the options: **Azure services**, **Internet**, **No access**   |Portal experience enables configuring security group with public endpoint. </br></br> Based on your scenario, select one of the following options: </br> </br> <ul> Azure services - recommended when connecting from Power BI or other multi-tenant service. </br> </br>Internet - use for test purposes when you want to quickly spin up a managed instance. It’s not recommended for use in production environments. </br></br> No access - this option creates a deny security rule. You’ll need to modify this rule in order to make managed instance accessible through public endpoint. </ul> </br></br> For more information on public endpoint security, see [using Azure SQL Database managed instance securely with public endpoint](sql-database-managed-instance-public-endpoint-securely.md).|
