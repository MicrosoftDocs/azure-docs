...
||Storage|Storage amount|The amount of storage available to a single database, elastic pool, or managed instance|
...
||Database-server-level firewall rules|[Database-server-level firewall rules](sql-database-firewall-configure.md#server-level-firewall-rule-versus-a-database-level-firewall-rule)|
