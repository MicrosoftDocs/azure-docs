...
- When re-scaling pool eDTUs, database connections are briefly dropped. This is the same behavior as occurs when re-scaling DTUs for a single database (not in a pool). For details on the duration and impact of dropped connections for a database during re-scaling operations, see [Change compute resources (DTUs)](sql-database-single-database-scale#dtu-based-purchasing-model-change-compute-resources-dtus).
