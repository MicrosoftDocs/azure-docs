---
title: Azure SQL Database and SQL Data Warehouse IP firewall rules | Microsoft Docs
...
> [!NOTE]
> This article applies to Azure SQL servers, and to both Azure SQL Database and Azure SQL Data Warehouse databases that are created on the Azure SQL server. For simplicity, *SQL Database* is used when referring to both SQL Database and SQL Data Warehouse.

> [!IMPORTANT]
> This article does *not* apply to **Azure SQL Database Managed Instance**. For information about the networking configuration, see [Connect your application to Azure SQL Database Managed Instance](sql-database-managed-instance-connect-app.md).
...
If yes, grant access using database-level IP firewall rules. This step avoids using server-level IP firewall rules, which permit access through the firewall to all databases, reducing the depth of your defenses.
...
### Does the person or team configuring the IP firewall rules only have access through the Azure portal, PowerShell, or the REST API?
You must use server-level IP firewall rules. Database-level IP firewall rules can only be configured using Transact-SQL.  

### Is the person or team configuring the IP firewall rules prohibited from having high-level permission at the database level?
...
### Is the person or team configuring or auditing the IP firewall rules, centrally managing IP firewall rules for many (perhaps hundreds) of databases?
...
### Can I use a mix of both server-level and database-level IP firewall rules?
...
- If the IP address of the request is not within one of the ranges specified in the database-level IP firewall rule, the server-level IP firewall rules are checked. If the IP address of the request is within one of the ranges that are specified in the server-level IP firewall rules, the connection is granted. Server-level IP firewall rules apply to all SQL databases on the Azure SQL server.  
...
To allow applications that are hosted inside Azure to connect to your Azure SQL server, Azure connections must be enabled. When an application from Azure tries to connect to your database server, the firewall verifies that Azure connections are allowed. A firewall setting with starting and ending address equal to 0.0.0.0 indicates that Azure connections are allowed. If the connection attempt is not allowed, the request doesn't reach the SQL Database server.

> [!IMPORTANT]
> This option configures the firewall to allow all connections from Azure, including connections from the subscriptions of other customers. When you select this option, make sure that your login and user permissions limit access to only authorized users.

## Create and manage IP firewall rules

You can create the first server-level firewall setting by using the [Azure portal](https://portal.azure.com/) or programmatically using [Azure PowerShell](https://docs.microsoft.com/powershell/module/az.sql), [Azure CLI](/cli/azure/sql/server/firewall-rule#az-sql-server-firewall-rule-create), or the [REST API](https://docs.microsoft.com/rest/api/sql/firewallrules/createorupdate). Subsequent server-level IP firewall rules can be created and managed using these methods or through Transact-SQL.
...
1. To set a server-level IP firewall rule from the database overview page, select **Set server firewall** on the toolbar, as shown in the following image. The **Firewall settings** page for the SQL Database server opens.
...
1. To set a server-level rule from server overview page, select **Firewall** from the **Settings** menu on the left side.

2. Select **Add client IP** on the toolbar to add the IP address of the computer that you're using, and then select **Save**. A server-level IP firewall rule is created for your current IP address.

## Manage IP firewall rules by using Transact-SQL
...
## Manage server-level IP firewall rules by using PowerShell
...
The following example sets a server-level IP firewall rule by using PowerShell:
...
## Manage server-level IP firewall rules by using CLI
...
The following example sets a server-level IP firewall rule by using CLI:
...
## Manage server-level IP firewall rules by using a REST API
...
- **Network address translation (NAT):**
  ...
  Due to NAT, the IP address that's used by your computer to connect to SQL Database may be different than the IP address that's shown in your computer IP configuration settings. To view the IP address that your computer is using to connect to Azure:
  1. Log in to the portal.
  1. Navigate to the **Configure** tab on the server that hosts your database. 
