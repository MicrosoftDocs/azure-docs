...
To allow applications hosted inside Azure to connect to your SQL server, Azure connections must be enabled. When an application from Azure tries to connect to your database server, the firewall verifies that Azure connections are allowed. A firewall setting that has starting and ending IP addresses equal to *0.0.0.0* indicates that Azure connections are allowed. This can be turned on directly from Azure Portal blade for Setting Firewall Rules as well using switch "Allow Azure Services and resources to access this server".  If the connection isn't allowed, the request doesn't reach the SQL Database server.
