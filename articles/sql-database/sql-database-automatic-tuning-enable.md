...
> [!NOTE]
> For Managed Instance, the supported option FORCE_LAST_GOOD_PLAN can be configured through [T-SQL](https://docs.microsoft.com/sql/t-sql/statements/alter-database-transact-sql-set-options?view=azuresqldb-current) only. Portal based configuration and automatic index tuning options does not apply to Managed Instance.

> [!NOTE]
> Configuring Automatic tuning options through ARM template is not supported at this time.
