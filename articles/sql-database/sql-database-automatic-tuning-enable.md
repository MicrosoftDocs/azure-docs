...
> [!NOTE]
> For Managed Instance supported option FORCE_LAST_GOOD_PLAN can be configured through [T-SQL](https://docs.microsoft.com/sql/t-sql/statements/alter-database-transact-sql-set-options?view=azuresqldb-current) only. Portal based configuration and automatic index tuning options do not apply to Managed Instance.
