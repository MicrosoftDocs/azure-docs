...
azfeatureregister --namespace Microsoft.Peering--name AllowPeeringService
...
```azurecli-interactive
az group create -n PeeringServiceResourceGroup -l "West US"
```

### 2. Register your subscription with the resource provider and feature flag  

Register your subscription with the resource provider and feature flag. The CLI commands are specified below:
...
Register the Peering Service using the below the set of commands using Azure CLI. This example registers the Peering Service named myPeeringService:
