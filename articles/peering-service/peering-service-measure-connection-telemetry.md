...
# How to measure the Peering Service connection telemetry
...
1. Click on ALL resources from the left pane and click on the prefix listed on the All resources page to which the latency reports need to be checked. 

2. Doing so, latency report page specific to that prefix appears as depicted below:  

> [!Note]
>By default, the report is generated for every 1 hour that is displayed in this page. However, to view the report for different timelines against the chosen prefix, choose the suitable option listed next to Show data for last. 
> 

3. To view the latency reports for multiple prefixes, choose those prefixes by checking the boxes against it. 

( multiple prefix selected screenshot) 

4. To view the failover events, click on the specific prefix in the Prefixes page. Now, click on the Prefix Events on the left pane. Doing so, the failover events that are captured will be displayed as depicted below:
