...
The tutorial assumes that you have product information in an existing on-premises system, and uses Azure Relay to reach into that system. This is simulated by a web service that runs in a simple console application and is backed by an in-memory set of products. You can run this console application on your own computer or deploy the web role into Azure. By doing so, you can see how the web role running in the Azure datacenter calls into your computer, even though your computer resides behind at least one firewall and a network address translation (NAT) layer.

## Create a namespace
To begin using the relay features in Azure, create a service namespace first. A namespace provides a scoping container for addressing Azure resources within your application.
