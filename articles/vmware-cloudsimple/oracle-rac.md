--- 
title: Optimize your CloudSimple Private Cloud for Oracle RAC
description: Describes how to deploy a new cluster and to optimize a VM for the Oracle RAC solution so you can install and configure Oracle RAC
author: sharaths-cs
ms.author: b-shsury 
ms.date: 08/06/2019 
ms.topic: article 
...
2. From the CloudSimple portal, [expand the Private Cloud](/expand-private-cloud) with a new cluster.  This cluster will be used to deplot Oracle RAC.  Select the number of nodes based on the desired fault tolerance (minimum three nodes).
 
## Set up networking for Oracle RAC

1. In your Private Cloud, [create two VLANs](/create-vlan-subnet), one for the Oracle public network and one for the Oracle private network and assign appropriate subnet CIDRs. 
2. After the VLANs are created, create the [distributed port groups on the Private Cloud vCenter](create-vlan-subnet/#use-vlan-information-to-set-up-a-distributed-port-group-in-vsphere). 
3. Set up a [DHCP and DNS server virtual machine](dns-dhcp-setup) on your management cluster for the Oracle environment. 
4. [Configure DNS forwarding on the DNS server](on-premises-dns-setup/#create-a-conditional-forwarder) installed in the Private Cloud.
...
1. [Sign in to the vSphere client](/vsphere-access) of your Private Cloud.
...
1. In the CloudSimple portal, [escalate the privileges](escalate-private-cloud-privileges) of the cloudowner user.
2. [Log in to the vSphere client](/vsphere-access) of your Private Cloud.
...
* <a href="https://docs.vmware.com/en/VMware-vSphere/6.7/com.vmware.vsphere.virtualsan.doc/GUID-08911FD3-2462-4C1C-AE81-0D4DBC8F7990.html" target="_blank">About vSAN Policies</a>
* <a href="https://docs.vmware.com/en/VMware-Cloud-on-AWS/solutions/VMware-Cloud-on-AWS.df6735f8b729fee463802083d46fdc75/GUID-A7642A82B3D6C5F7806DB40A3F2766D9.html" target="_blank">VMware Multi-Writer Attribute for Shared VMDKs</a>