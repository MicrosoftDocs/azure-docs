...
manager: dikamath 
---
# Quickstart - Configure an AVS Private Cloud environment

In this article, learn how to create an AVS Private Cloud and set up your AVS Private Cloud environment.
...
An AVS Private Cloud is an isolated VMware stack that supports ESXi hosts, vCenter, vSAN, and NSX.
...
10. Enter the CIDR range for vSphere/vSAN subnets. Make sure that the CIDR range doesn't overlap with any of your on-premises or other Azure subnets (virtual networks) or with the gateway subnet.
    ...
    **CIDR range options:** /24, /23, /22, or /21. A /24 CIDR range supports up to 26 nodes, a /23 CIDR range supports up to 58 nodes, and a /22 and /21 CIDR range supports 64 nodes (the maximum number of nodes in an AVS Private Cloud). To learn more and VLANs and subnets, see [VLANs and subnets overview](cloudsimple-vlans-subnets.md).
...
2. Search for **AVS Services**.
3. Select the AVS service on which you want to create your AVS Private Cloud.
4. From overview, click **Go to the AVS portal** to open a new browser tab for AVS portal. If prompted, sign in with your Azure sign in credentials. 

    ![Launch AVS portal](media/launch-cloudsimple-portal.png)

## Create Point-to-Site VPN

A Point-to-Site VPN connection is the simplest way to connect to your AVS Private Cloud from your computer. Use Point-to-Site VPN connection if you're connecting to the AVS Private Cloud remotely. For quick access to your AVS Private Cloud, follow the steps below. Access to the AVS region from your on-premises network can be done using [Site-to-Site VPN](vpn-gateway.md) or [Azure ExpressRoute](on-premises-connection.md).
...
After creating an AVS Private Cloud, create a VLAN where you'll deploy your workload/application VMs.

1. In the AVS portal, select **Network**.
2. Click **VLAN/Subnets**.
3. Click **Create VLAN/Subnet**.

    ![Create VLAN/Subnet](media/create-new-vlan-subnet.png)

4. Select the **AVS Private Cloud** for the new VLAN/subnet.
...
The VLAN/subnet will be created. You can now use this VLAN ID to create a distributed port group on your AVS Private Cloud vCenter.

## Connect your environment to an Azure virtual network

AVS provides you with an ExpressRoute circuit for your AVS Private Cloud. You can connect your virtual network on Azure to the ExpressRoute circuit. For full details on setting up the connection, follow the steps in [Azure Virtual Network Connection using ExpressRoute](https://docs.azure.cloudsimple.com/cloudsimple-azure-network-connection/).

## Sign in to vCenter

You can now sign in to vCenter to set up virtual machines and policies.

1. To access vCenter, start from the AVS portal. On the Home page, under **Common Tasks**, click **Launch vSphere Client**. Select the AVS Private Cloud and then click **Launch vSphere Client** on the AVS Private Cloud.
...
2. Select your preferred vSphere client to access vCenter and sign in with your username and password. The defaults are:
    * User name: **CloudOwner@AVS.local**
    * Password: **AVS123!**  
...
AVS recommends that you change your password the first time you sign in to vCenter. 
...
You can find the fully qualified domain name (FQDN) and IP address of NSX manager on AVS portal.

1. Launch AVS portal and select **Resources**.
2. Click on the AVS Private Cloud, which you want to use.
