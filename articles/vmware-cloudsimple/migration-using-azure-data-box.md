...
Using Data Box, you can bulk-migrate your VMware data to your private cloud. Data from your on-premises VMware vSphere environment is copied to Data Box by using the Network File System (NFS) protocol. Bulk data migration involves saving a point-in-time copy of virtual machines, configuration, and associated data to Data Box and shipping it to Azure.

In this article, you learn about:

* Setting up Data Box
* Copying data from the on-premises VMware environment to the Data Box by using NFS
...
2. Enter the IP address of the VMware ESXi hosts and select **Add**. You can configure access for all hosts in your vSphere cluster by repeating this step. Select **OK**.
...
## Mount NFS share as a datastore on your on-premises vCenter cluster and copy the data
...
6. In step 4, select the ESXi hosts where you want the datastore to be mounted and then select **Next**.  In a cluster, select all hosts to ensure migration of the virtual machines.
...
Virtual machines can be migrated or cloned to the new datastore.  Any unused virtual machines that you want to migrate can be migrated to the Data Box NFS datastore by using **storage vMotion** option. Active virtual machines can be cloned to the Data Box NFS datastore.

* Identify the list of virtual machines that can be **moved**.
* Identify the list of virtual machines that must be **cloned**.
...
> [!TIP]
> You can select multiple virtual machines in the same power state (turned on or turned off) and migrate them in bulk.

The virtual machine will be migrated to the NFS datastore from Data Box. After all virtual machines are migrated, you can turn off or shut down the active virtual machines in preparation for migration of data to Azure VMware Solution.
...
6. Select the customization options if you want to customize any options for the cloned object and then select **Next**.
...
> [!TIP]
> If you already have ISO files on your on-premises datastore, you can select the files and **Copy to** to copy the files to the Data Box NFS datastore.
...
2. Right-click the Data Box NFS datastore and select **Unmount Datastore**.
...
NFS share from your Linux virtual machine must be mounted as a datastore on your private cloud vCenter cluster for copying the data. After it's mounted, data can be copied from the NFS datastore to the private cloud vSAN datastore.
...
4. In step 2, select **NFS 3** as NFS version and then select **Next**.
...
6. In step 4, select the ESXi hosts where you want the datastore to be mounted and then select **Next**.  In a cluster, select all hosts to ensure migration of virtual machines.
