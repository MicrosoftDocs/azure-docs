--- 
title: Back up workload virtual machines on Private Cloud using Veeam
...
# Back up workload virtual machines on CloudSimple Private Cloud using Veeam B&R 
...
4. [Azure Portal: Connect your virtual network to the Private Cloud](#azure-portal-connect-your-vnet-to-the-private-cloud)
5. [Azure Portal: Create a backup repository in Azure](#azure-portal-connect-your-vnet-to-the-private-cloud)
...
* A [Private Cloud](create-private-cloud) created using the CloudSimple portal.  
...
* You can add an external identity provider as described in [Set up vCenter identity sources to use Active Directory](set-vcenter-identity) if any of the following apply:<br>
...
Create a VLAN for the backup network and assign it a subnet CIDR. For instructions, see [Create and manage VLANs/Subnets](create-vlan-subnet).

Create firewall rules between the management subnet and the backup network to allow network traffic on ports used by Veeam. See the Veeam topic <a href="https://helpcenter.veeam.com/docs/backup/vsphere/used_ports.html?ver=95" target="_blank">Used Ports</a>. For instructions on firewall rule creation, see [Set up firewall tables and rules](firewall). 
...
Create firewall rules between the workload subnet and the backup network as described in [Set up firewall tables and rules](firewall).  For application aware backup and restore, <a href="https://helpcenter.veeam.com/docs/backup/vsphere/used_ports.html?ver=95" target="_blank">additional ports</a> must be opened on the workload VMs that host specific applications. 
...
To continue the setup, you need the authorization key and peer circuit URI and access to your Azure Subscription.  This information is available on the Virtual Network Connection page in the CloudSimple portal. For instructions, see [Obtain peering information for Azure virtual network to CloudSimple connection](virtual-network-connection). If you have any trouble obtaining the information, <a href="https://portal.azure.com/#blade/Microsoft_Azure_Support/HelpAndSupportBlade/newsupportrequest" target="_blank">contact support</a>.

### CloudSimple Private Cloud: Escalate Privileges for cloudowner
The default 'cloudowner' user doesn't have sufficient privileges in the Private Cloud vCenter to install VEEAM, so the user's vCenter privileges must be escalated. For more information, see [Escalate privileges](escalate-private-cloud-privileges).

###Azure Portal: Connect your virtual network to the Private Cloud 
Connect your virtual network to the Private Cloud by following the instructions in [Azure Virtual Network Connection using ExpressRoute](azure-expressroute-connection).
...
4. Create a username and password based user account for the new VM. For instructions, see <a href="https://docs.microsoft.com/en-us/azure/virtual-machines/linux/quick-create-portal" target="_blank">Create a Linux virtual machine in the Azure portal</a>.
5. Create 1x512 GiB standard HDD and attach it to the repository VM.  For instructions, see <a href="https://docs.microsoft.com/en-us/azure/virtual-machines/windows/attach-managed-disk-portal" target="_blank">How to attach a managed data disk to a Windows VM in the Azure portal</a>.
6. <a href=" https://www.digitalocean.com/docs/volumes/how-to/format-and-mount/" target="_blank">Create an XFS volume on the managed disk</a>. Log in to the VM using the previously mentioned credentials. Execute the following script to create a logical volume, add the disk to it, create an XFS filesystem partition and mount the partition under the /backup1 path.

Example script: <br>
```sudo pvcreate /dev/sdc```<br>
```sudo vgcreate backup1 /dev/sdc```<br>
```sudo lvcreate -n backup1 -l 100%FREE backup1```<br>
```sudo mkdir -p /backup1```<br>
```sudo chown veeamadmin /backup1```<br>
```sudo chmod 775 /backup1```<br>
```sudo mkfs.xfs -d su=64k -d sw=1 -f /dev/mapper/backup1-backup1```<br>
```sudo mount -t xfs /dev/mapper/backup1-backup1 /backup1```<br><br>
7. Expose /backup1 as an NFS mount point to the Veeam backup server that is running in the Private Cloud. For instructions, see the Digital Ocean article <a href="https://www.digitalocean.com/community/tutorials/how-to-set-up-an-nfs-mount-on-centos-6" target="_blank">How To Set Up an NFS Mount on CentOS 6</a>. Use this NFS share name when you configure the backup repository in the Veeam backup server. <br>
...
1. Create a general purpose storage account (GPv2) of standard type and a blob container as described in the Microsoft video <a href="https://azure.microsoft.com/en-gb/resources/videos/get-started-with-azure-storage/" target="_blank">Getting Started with Azure Storage</a>.
2. Create an Azure storage container, as described in the Microsoft topic <a href="https://docs.microsoft.com/en-us/rest/api/storageservices/create-container" target="_blank">Create Container</a>.
...
<br>
```wget -O azcopy.tar.gz https://aka.ms/downloadazcopylinux64```<br>
```tar -xf azcopy.tar.gz```<br>
```sudo ./install.sh```<br>
```sudo yum -y install libunwind.x86_64```<br>
```sudo yum -y install icu```<br>
3. Use the ```azcopy``` command to copy backup files to and from the blob container.  See the Microsoft topic <a href="https://docs.microsoft.com/en-us/azure/storage/common/storage-use-azcopy-linux" target="_blank">Transfer data with AzCopy on Linux</a> for detailed commands.
...
1. Create a new role named ‘Veeam Backup Role’ and assign it necessary permissions as recommended by Veeam. For details see the Veeam topic <a href="https://helpcenter.veeam.com/docs/backup/vsphere/required_permissions.html?ver=95" target="_blank">Required Permissions</a>.
2. Create a new ‘Veeam User Group’ group in vCenter and assign it the ‘Veeam Backup Role’. 
3. Create a new ‘Veeam Service Account’ user and add it to the ‘Veeam User Group’.<br>
...
Create a public IP address for the Veeam backup and recovery server. For instructions, see [Allocate public IP addresses](public-ips).

Create a firewall rule using to allow the Veeam backup server to create an outbound connection to Veeam website for downloading updates/patches on TCP port 80. For instructions, see [Set up firewall tables and rules](firewall).

To de-scalate privileges, see [De-escalate privileges](escalate-privileges/#de-escalate-privileges).
