...
For Arc-enabled servers, allow traffic using the
following patterns:
...
When using [private link with Arc-enabled servers](../../azure-arc/servers/private-link-security.md), built-in policy packages will
automatically be downloaded over the private link. You do not need to set any tags
on the Arc-enabled server to enable this feature.
