...
distinct resources that are applicable and only one is **Non-compliant**. Thus, the overall
resource compliance is 19/20 or 95%.
...
resources and non-compliant policies for each assignment are also provided. Clicking on a policy or
...
The list of resources on the **Resource compliance** tab (which defaults to **Non-compliant** but
can be filtered) reflects the evaluation status of existing resources for the current assignment, events
(append, audit, deny, deploy) triggered by the request to create a resource are shown under the
**Events** tab.
...
- Discover how to [remediate non-compliant resources](remediate-resources.md)
