...
| MissingRegistrationForType          | MissingRegistrationForResourceType: The subscription is not registered for the resource type '{ResourceType}'. Check that the resource type exists and that the resource type is registered.                                                                                                                                                                                                             |
...
| InvalidConstraint/Template       | The resource was rejected because of one of the following reasons: invalid constraint template Rego content, invalid YAML, or a parameter type mismatch between constraint and constraint template (i.e. providing a string value when an integer was expected).                  |
