---
title: Sample - IRS 1075 blueprint - Deploy steps
description: Deploy steps of the IRS 1075 blueprint sample.
author: DCtheGeek
ms.author: dacoulte
ms.date: 08/07/2019
ms.topic: sample
...
To deploy the Azure Blueprints IRS 1075 blueprint sample, the following steps must
be taken:
...
1. Find the **IRS 1075** blueprint sample under _Other Samples_ and select **Use
   this sample**.

1. Enter the _Basics_ of the blueprint sample:

   - **Blueprint name**: Provide a name for your copy of the IRS 1075 blueprint
     sample.
...
it away from alignment with IRS 1075ntrols.
...
1. Select **Publish blueprint** at the top of the page. In the new page on the right, provide a
   **Version** for your copy of the blueprint sample. This property is useful for if you make a
   modification later. Provide **Change notes** such as "First version published from the IRS 1073 R4 blueprint sample." Then select **Publish** at the bottom of the page.
...
|-|-|-|-|
|\[Preview\]: Audit IRS 1075 controls and deploy specific VM Extensions to support audit requirements|Policy assignment|Log Analytics workspace ID that VMs should be configured for|This is the ID (GUID) of the Log Analytics workspace that the VMs should be configured for.|
|\[Preview\]: Audit IRS 1075 controls and deploy specific VM Extensions to support audit requirements|Policy assignment|List of resource types that should have diagnostic logs enabled|List of resource types to audit if diagnostic log setting is not enabled. Acceptable values can be found at [Azure Monitor diagnostic logs schemas](../../../../azure-monitor/platform/diagnostic-logs-schema.md#supported-log-categories-per-resource-type).|
|\[Preview\]: Audit IRS 1075 controls and deploy specific VM Extensions to support audit requirements|Policy assignment|List of users that should be excluded from Windows VM AdmiIRS 1075A semicolon-separated list of members that should be excluded in the AdmiIRS 1075group. Ex: AdmiIRS 10751; myUser2|
|\[Preview\]: Audit IRS 1075 controls and deploy specific VM Extensions to support audit requirements|Policy assignment|List of users that should be included in Windows VM AdmiIRS 1075A semicolon-separated list of members that should be included in the AdmiIRS 1075group. Ex: AdmiIRS 10751; myUser2|
...
Now that you've reviewed the steps to deploy the IRS 1075 blueprint sample, visit
