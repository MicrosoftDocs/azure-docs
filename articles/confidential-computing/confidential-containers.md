...
Confidential containers are a set of set of features and capabilities to further secure your standard container workloads to achieve higher data security running them in a Trusted Execution Environment (TEE). Azure offers portfolio of capabilities through different container compute options as discussed below.

## Benefits
Confidential containers on Azure run within a enclave based TEE or VM based TEE enviornments. Both deployment models help achieve high isolation and memory encryption through hardware-based assurances. Confidential computing can enhance your deployment security posture by protecting your memory space through encryption through code and data integrity.

The below are the qualities of confidential containers

1. Allows running existing standard container images with no code changes (lift-and-shift) within a TEE
2. Allows establishing a hardware root of trust thorugh remote guest attestation
3. Provides strong assurances of data confidentilaity, code integrity in a cloud environment
4. Helps isolate your containers from other container groups/pods, as well as VM node OS kernel

The below diagram will guide to different offerings in this portfolio
