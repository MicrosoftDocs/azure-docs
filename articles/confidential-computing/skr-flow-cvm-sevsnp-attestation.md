...
Follow the quickstart instructions on how to "[Deploy confidential VM with ARM template](<path-to-docs>/confidential-computing/quick-create-confidential-vm-arm-amd)"

## Enable system-assigned managed identity

[Managed identities](<path-to-docs>/active-directory/managed-identities-azure-resources/) for Azure resources provide Azure services with an automatically managed identity in Azure Active Directory. You can use this identity to authenticate to any service that supports Azure AD authentication, without having credentials in your code.

To enable system-assigned managed identity on a CVM, your account needs the [Virtual Machine Contributor](<path-to-docs>/role-based-access-control/built-in-roles.md#virtual-machine-contributor) role assignment.  No other Azure AD directory role assignments are required.
...
> [!NOTE]
> You can find the managed identity object ID in the virtual machine identity options, in the Azure Portal. Alternatively you can retrieve it with [PowerShell](<path-to-docs>/active-directory/managed-identities-azure-resources/how-to-assign-app-role-managed-identity-powershell), [Azure CLI](<path-to-docs>/active-directory/managed-identities-azure-resources/how-to-assign-app-role-managed-identity-cli), [Bicep](<path-to-docs>/templates/microsoft.compute/2021-11-01/virtualmachines?pivots=deployment-language-bicep#virtualmachineidentity) or [ARM templates](<path-to-docs>/azure-resource-manager/templates/template-functions-resource#get-managed-identity).
...
Key Vault Secure Key Release Policies are modeled after __Azure Policy__, with a [slightly different grammar](https://learn.microsoft.com/azure/key-vault/keys/policy-grammar).
...
- Been attested by [Microsoft Azure Attestation (MAA)](https://learn.microsoft.com/en-us/azure/attestation/overview) service endpoint "https://sharedweu.weu.attest.azure.net".
...
In this example, there is only one key under the `$.x-ms-runtime.keys` path. Key Vault uses the `TpmEphemeralEncryptionKey` key as the key-encryption key.
...
We can verify that Key Vault has created a new, __HSM-backed__, key and that it contains our secure key __release policy__ by navigating to the Azure Portal and selecting our key. Our key will also be marked as "__exportable__".

![Image of the Azure Portal with the settings for key named 'my SKR key' visible. It shows another panel that shows the details of the secure key release policy.](media/skr-flow-cvm-sevsnp-attestation/skr-onboard-key-with-policy.png)

## Guest attestation client

Attestation helps us to _cryptographically assess_ that something is running in the intended operating state. It is the process by which one party, the verifier, assesses the trustworthiness of a potentially untrusted peer, the attester. In the case of guest attestation, the trusted execution environment offers a platform that allows you to run an entire operating system inside of it.
...
> [!NOTE]
> A token from the Azure Attestation service is valid for [8 hours](https://learn.microsoft.com/en-us/azure/attestation/faq#how-long-is-an-attestation-token-valid-).
...
1. Navigate inside the unzipped folder and run `VC_redist.x64.exe`. This will install Microsoft C and C++ ([MSVC](https://docs.microsoft.com/en-US/cpp/windows/latest-supported-vc-redist?view=msvc-170)) runtime libraries on the machine.
1. To run the sample client, navigate inside the unzipped folder and run the below command:
...
The header contains a `jku`, aka [JWK Set URI](https://www.rfc-editor.org/rfc/rfc7515#section-4.1.2), that links to a set of JSON-encoded public keys, one of which corresponds to the key used to digitally sign the JWS. The `kid` indicates which key was used to sign the JWS.
...
The documenation for Microsoft Azure Attestation service has an extensive list containing descriptions of all of these [SEV-SNP-related claims](https://learn.microsoft.com/en-us/azure/attestation/claim-sets#sev-snp-attestation).
...
The response's JWT token body looks incredibly similar to the response that you get when invoking the `get` key operation. However, the `release` operation will also include the `key_hsm` property, amongst other things.
