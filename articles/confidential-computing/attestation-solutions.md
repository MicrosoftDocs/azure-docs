...
2. Host country receives the evidence of policy compliance from the individual and verifies whether the supplied evidence proves that the individual complies with the policies for being issues a passport. 
	1. Birth certificate is valid and hasn't been altered.
	2. Issuer of the birth certificate is trusted
	3. Individual isn't part of a restricted list
...
5. The Foreign Country Border Patrol Agent checks a series of rules on the passport before trusting it  
	1. Passport is authentic and hasn't been altered.
	2. Passport was produced by a trusted country.
	3. Passport isn't expired or revoked.
...
![Diagram of remote attestation with the passport model for an immigration desk.](media/attestation-solutions/passport_model_immigration.png)
...
5. The Employer, otherwise known as the Relying Party, may check a series of rules on the Attestation Result before trusting it.  
	1. Attestation Result is authentic, hasn't been altered, and truly comes from the School.
...
![Diagram of remote attestation with the background check model for education background.](media/attestation-solutions/background_check_model_school.png)
...
3. The Verifier service evaluates whether the evidence given by the TEE meets defined policy requirements for being trusted. 
	1. Certificate is valid and hasn't been altered.
	2. Issuer of the certificate is trusted.
	3. TEE evidence isn't part of a restricted list.
...
![Diagram of remote attestation with the background check model for computing.](media/attestation-solutions/background_check_model_computing.png)
...
[VMs with Application Enclaves](~/azure/confidential-computing/confidential-computing-enclaves.md) are enabled by Intel SGX, which allows organizations to create enclaves that protect data, and keep data encrypted while the CPU processes the data. Customers can attest Intel SGX enclaves in Azure with MAA and on their own. 
- [Intel SGX Attestation Home Page](~/azure/confidential-computing/attestation.md)
...
[Confidential Virtual Machines](~azure/confidential-computing/confidential-vm-overview.md) are enabled by AMD SEV-SNP, which allows organizations to have hardware-based isolation between virtual machines, and underlying host management code (including hypervisor). Customers can attest their managed confidential virtual machines in Azure with MAA and on their own. 
...
- [Cloud Provider: What is guest attestation for confidential VMs?](~/azure/confidential-computing/guest-attestation-confidential-vms.md)
