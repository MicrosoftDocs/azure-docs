...
---
# Standard quota: Increase regional vCPU quota limits 
...
When a new subscription is created, the default total of regional vCPUs might not be equal to the sum of the default vCPU quotas for all individual VM series. This difference can result in a subscription with enough quota for each individual VM series that you want to deploy. But there might not be enough quota for the total regional vCPUs for all deployments. In this case, you need to submit a request to explicitly increase the limit of the total number of regional vCPUs. The total regional vCPU limit can't exceed the total approved quota across all VM series for the region.
...
To learn more about increasing spot VM vCPU limits, see [Spot quota: Increase vCPU quota limits for all VM series](https://docs.microsoft.com/azure/azure-supportability/low-priority-quota).

You can request an increase in your regional vCPU standard quota limit in either of two ways, as described in the next sections.

## Request a regional vCPU quota increase from the "Help + support" pane

To request a regional vCPU quota increase from the **Help + support** pane, do the following: 
...
## Request a regional vCPU quota increase from the "Subscriptions" pane

To request a regional vCPU quota increase from the **Subscriptions** pane, do the following: 
