...
When a new subscription is created, a default quota of vCPUs is assigned to it to it. Any time a new VM is to be deployed using Classic deployment model, the sum of new and existing vCPUs usage across all regions must not exceed the vCPU quota approved for the Classic deployment model. 
