...
**Microsoft Power BI Embedded** is an Azure service which enables application developers to add interactive Power BI reports into their own applications. **Power BI Embedded** works with existing applications without the need to redesign those applications or change the way their users log in.
...
This blade contains the information that you will need to call the APIs that  create workspaces and deploy content to them.
...
Copy these keys and store them securely in your application. It is very important that you treat these keys as if you would a password as they will provide access to all the content in your workspace collection.
