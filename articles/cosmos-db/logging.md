...
2. In the **Diagnostic settings** page, do the following steps: 
    ...
    * **Log MongoRequests**: Select this option to log user-initiated requests from the Azure Cosmos DB front end for serving Azure Cosmos DB for MongoDB API accounts. If you're archiving to a storage account, you can select the retention period for the diagnostic logs. Logs are auto-deleted after the retention period expires.
