...
| `TextP` | String filtering functions. These functions are predicates for the `has()` step to filter a string property based on a substring comparison. The supported functions are `TextP.startingWith(string)`, `TextP.endingWith(string)`, `TextP.containing(string)`; and their negative equivalents `TextP.notStartingWith(string)`, `TextP.notEndingWith(string)` and `TextP.notContaining(string)`. | [TextP predicates](http://tinkerpop.apache.org/docs/3.4.0/reference/#a-note-on-predicates) |
