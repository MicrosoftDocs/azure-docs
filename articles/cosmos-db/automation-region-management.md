...
Azure Cosmos DB database accounts are the only Azure Cosmos DB resources that can be created/modified using Azure Resource Manager templates and Azure CLI 1.0. 

To create Azure Cosmos DB DocumentDB API, Table API, Graph API, or Mongo DB account using CLI 2.0, see [Azure CLI samples for Azure Cosmos DB](cli-samples.md).
...
![Screenshot showing the device login screen for Microsoft Azure CLI 1.0](media/automation-region-management/azure-cli-login-code.png)
...
![Screenshot showing where to select the Microsoft identity account associated with the Azure subscription you want to use](media/automation-region-management/identity-cli-login.png)
...
![Screenshot showing confirmation of login to the Microsoft Azure Cross-platform Command Line Interface](media/automation-region-management/login-confirmation.png)
...
- Error information is also available in the Azure Portal as shown in the following screenshot. To navigate to the error info: click Resource Groups in the Jumpbar, select the Resource Group that had the error, then in the Essentials area of the Resource group blade click the date of the Last Deployment, then in the Deployment history blade select the failed deployment, then in the Deployment blade click the Operation detail with the red exclamation mark. The Status Message for the failed deployment is displayed in the Operation details blade. 
    ...
    ![Screenshot of the Azure portal showing how to navigate to the deployment error message](media/automation-region-management/portal-troubleshooting-deploy.png) 
...
- The Azure portal, as described in each of the [quickstarts](create-documentdb-dotnet.md).
...
After creating your database, you need to add one or more containers to the database, then add items to the collections. 

After you have documents in a collection, you can use [SQL](documentdb-sql-query.md) to [execute queries](documentdb-sql-query.md#ExecutingSqlQueries) against your documents.
...
[distribute-globally]: distribute-data-globally.md
