...
description: Learn how to upgrade your existing .NET application from the V2 SDK to the newer .NET SDK 3.0 (com.azure.cosmos package) for Core (SQL) API.
author: stefArroyo
ms.author: esarroyo
...
Information is also provided to assist in migrating the following external libraries that are now part of the Azure Cosmos DB .NET SDK v3 for Core (SQL) API:
* .NET Change Feed Processor library 2.0
* .NET Bulk Executor Library 1.1 or greater

## New on Azure Cosmos DB v3 .NET SDK
...
**The Azure Cosmos DB .NET SDK 3.0 is now open source.** We welcome any pull requests and will be logging issues and tracking feedback on GitHub.
...
While there are no immediate plans to [retire,  support for the 2.0 SDKs](https://github.com/Azure/azure-cosmos-dotnet-v2/blob/master/changelog.md#release--retirement-date), the SDKs will be replaced by newer versions in the future and support for older versions will be retired. For the best development experience, we recommend always starting with the latest supported version of SDK.
...
* `DocumentClient` renamed to `CosmosClient`
* `Collection` renamed to `Container`
* `Document` renamed to `Item`

All resource objects have been renamed with additional properties included to the name for clarity.
...
### Classes removed from .NET v3 SDK

The following classes have been removed from the 3.0 SDK:
...
The SDK v3 now defaults to Direct + TCP connection modes compared to the previous v2 SDK, which defaulted to Gateway + HTTPS connections modes, providing enhanced performance and scalability.

### Changes to FeedOptions (now QueryRequestOptions)
...
For more information on migrating from the change feed processor library to the Azure Cosmos DB .NET v3 SDK, see [here.](https://docs.microsoft.com/en-us/azure/cosmos-db/how-to-migrate-from-change-feed-library)
...
For more information on migrating from the bulk executor library to  bulk support in Azure Cosmos DB .NET V3 SDK, see [here.](https://docs.microsoft.com/en-us/azure/cosmos-db/how-to-migrate-from-bulk-executor-library)
...
### Create a container (Autoscale + Time to Live Expiration)
...
```csharp
private static async Task CreateContainerWithCustomIndexingPolicy(Database database)
{
    // Create a container with custom index policy (consistent indexing)
    string containerIdManualwithIndexing = ContainerName + "_ManualIndexing";
    ContainerProperties containerProperties = new ContainerProperties(
        id: containerIdManualwithIndexing,
        partitionKeyPath: partitionKeyPath);
...
```C#
private static async Task CreateContainerWithCustomIndexingPolicy(DocumentClient client)
{
    // Create a container with custom index policy (consistent indexing)
    string containerIdManualwithIndexing = ContainerName + "_ManualIndexing";
    DocumentCollection collectionDefinition = new DocumentCollection();
    collectionDefinition.Id = containerIdManualwithIndexing;
