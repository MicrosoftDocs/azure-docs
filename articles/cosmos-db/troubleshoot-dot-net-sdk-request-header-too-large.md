---
title: Troubleshoot a "Request header too large" or 400 bad request in Azure Cosmos DB 
description: Diagnose and fix the "Request header too large" exception.
author: j82w
...
* The exception started without making any changes to the application. The session token grows as the number of partitions increases in the container. The numbers of partitions increase as the amount of data increases or if the throughput is increased.
...
#### Solution:
1. Follow the guidance in the [.NET v3](performance-tips-dotnet-sdk-v3-sql.md) or [.NET v2](performance-tips.md) performance tips articles. Convert the application to use the direct connection mode with the TCP. The direct connection mode with the TCP doesn't have the header size restriction like the HTTP protocol, so it avoids this issue. 
3. If the direct connection mode with the TCP isn't an option for your workload, set the `ResponseContinuationTokenLimitInKb` option. You can find this option in `FeedOptions` in v2 or `QueryRequestOptions` in v3.
