...
|  <input type="checkbox"/>    | Avoid using any special characters in identifiers   | Some characters are restricted and cannot be used in some identifiers: '/', '\\', '?', '#'. The general recommendation is to not use any special characters in database, collection, item, or partition key identifiers to avoid any unexpected behavior. |
