...
> [!Note]
> In latest version mode, you will receive objects that represent the item that changed with some [extra metadata](change-feed-latest-version.md#parsing-the-response-object). All versions and deletes mode returns a different data model, and you can find more information [here](change-feed-all-versions-and-deletes.md#parsing-the-response-object).
...
Below is an example for obtaining a `responseIterator` in latest version mode:
