...
The options of region name, class, replication_factor, datacenter  in create keyspace command are ignored at present- Distribution of data is implemented in underlying Cosmos DB platform and exposed via portal or powershell for the account. Durable_writes can't be disabled - as Cosmos DB ensures every write is durable. In every region Cosmos DB replicates data across the replicaset made up of 4 replicas and this can't changed or modified. 
