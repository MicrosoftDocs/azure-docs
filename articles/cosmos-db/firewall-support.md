...
Azure PaaS services like Azure Stream analytics, Azure Functions etc. are used in conjunction with Azure Cosmos DB. To allow applications from other Azure PaaS services to connect to your Azure Cosmos DB resources, a firewall setting must be enabled. A firewall setting with starting and ending address equal to 0.0.0.0 indicates that connections from all Azure datacenter IP address range are allowed to connect to your Azure Cosmos DB resources. To enable this setting, add the IP address- 0.0.0.0 to the list of allowed IP addresses. 
