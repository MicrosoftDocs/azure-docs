...
* Expect different behavior in regard to explicit `null` values:
  ...
  * SQL serverless pools in Azure Synapse will read these values as `0` (zero) if the first document of the collection has, for the same property, a value that is `numeriic`.
