...
* Expect different behavior in regard to explicit `null` values:
  ...
  * SQL serverless pools in Azure Synapse will read these values as `NULL` if the first document has, for the same property, a value with a datatype different of `integer`.
  * SQL serverless pools in Azure Synapse will read these values as `0` (zero) if the first document has, for the same property, a value that is an integer.
