...
Azure Cosmos DB burst capacity (preview) allows you to take advantage of your database or container's idle throughput capacity to handle spikes of traffic. With burst capacity, each physical partition can accumulate up to 5 minutes of idle capacity, which can be consumed at a rate up to 3000 RU/s. Burst capacity enables a scenario where requests that would have otherwise been rate limited (429) can now be served with burst capacity while it's available.
