...
|Indexing policy|Azure Cosmos DB API for MongoDB gives you the option of creating a wildcard indexes for all fields for flexible and efficient queries. Many other [indexing options](mongodb-indexing.md) exist. If you choose **off**, none of the properties are indexed. This results in the lowest RU charge for writes. Select **off** policy if you only plan to query using the _id field and the shard key (both per query)|
|Total data stored in transactional store |Total estimated data stored(GB) in the transactional store in a single region.|
|Use analytical store| Choose **On** if you want to use [Synapse analytical store](synapse-link.md). Enter the **Total data stored in analytical store**, it represents the estimated data stored (GB) in the analytical store in a single region.  |
