...
| `numLists` | integer | This integer is the number of clusters that the IVF index uses to group the vector data. We recommend that `numLists` be set to `rowCount()/1000` for up to 1 million rows and to `sqrt(rowCount)` for more than 1 million rows. |
