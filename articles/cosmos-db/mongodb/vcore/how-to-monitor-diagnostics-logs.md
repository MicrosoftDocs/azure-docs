...
1. Use `az monitor diagnostic-settings create` to create the setting.
    ...
    >
    > If `--export-to-resource-specific true` is not configured, the API for MongoDB vCore request log events will be directed to the general `AzureDiagnostics` table.
    > 
