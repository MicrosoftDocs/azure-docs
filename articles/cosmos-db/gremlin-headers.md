...
Please keep in mind that taking dependency on these headers you are limiting portability of your application to other Gremlin implementations. In return, you are gaining tighter integration with Cosmos DB Gremlin. These headers are not a TinkerPop standard.
...
| Status | Explanation |
| --- | --- |
| **401** | Error message `"Unauthorized: Invalid credentials provided"` is returned when authentication password is not a valid Cosmos DB account key for a given account. Please navigate to your Cosmos DB Gremlin account in Azure Portal and confirm that the key is correct.|
...
| **408** | `"Server timeout"` indicates that traversal took more than **30 seconds** and was cancelled by the server. Please optimize your traversals to run quickly by filtering vertices or edges on every hop of traversal to narrow down search scope.|
