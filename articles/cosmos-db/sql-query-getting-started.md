...
* A Cosmos container is a schema-free collection of JSON items. The relations within and across container items are implicitly captured by containment, not by primary key and foreign key relations. This feature is important for the intra-item joins discussed later in [joins in Azure Cosmos DB](https://docs.microsoft.com/en-us/azure/cosmos-db/sql-query-join) article.
