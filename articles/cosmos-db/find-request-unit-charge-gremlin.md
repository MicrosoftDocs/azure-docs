...
This article presents the different ways you can find the [request unit](request-units.md) (RU) consumption for any operation executed against a container in Azure Cosmos DB Gremlin API. If you are using a different API, see [API for MongoDB](find-request-unit-charge-mongodb.md), [Cassandra API](cassandra/find-request-unit-charge-cassandra.md), [SQL API](find-request-unit-charge.md), and [Table API](table/find-request-unit-charge.md) articles to find the RU/s charge.
