...
With Azure Cosmos DB, you pay for the throughput you provision and the storage you consume on an hourly basis. Throughput must be provisioned to ensure that sufficient system resources are available for your Cosmos database all the time to meet or exceed the Cosmos DB SLA.
...
* **Containers**. For more information, see [how to provision throughput on a Cosmos container.](how-to-provision-container-throughput.md)
* **Databases**. For more information, see [how to provision throughput on a Cosmos database.](how-to-provision-database-throughput.md)
...
* **Document indexing** - By default, each item is automatically indexed. Fewer request units are consumed if you choose not to index some of your items in a container.
