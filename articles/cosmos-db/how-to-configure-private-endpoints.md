...
* When you're using an Azure Cosmos DB's API for MongoDB account that has Private Link, tools/libraries must support SNI or pass the `appName` parameter from the connection string to properly connect. Some older tools/libraries may not be compatible.
