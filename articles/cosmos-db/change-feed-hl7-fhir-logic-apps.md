...
We are using the [`CreateDocumentChangeFeedQuery`](/dotnet/api/microsoft.azure.documents.client.documentclient.createdocumentchangefeedquery#overloads) class from the Azure Cosmos DB SQL .NET API. For more information, see the [change feed article](change-feed.md). 
