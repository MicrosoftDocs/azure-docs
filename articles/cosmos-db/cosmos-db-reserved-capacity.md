...
4. Fill in the required fields as described in the following table:
   ...
   |Reserved capacity type   |  Throughput provisioned as request units. You can make reservations for to have all regions to process the writes (multi-master) or exactly one region to process the writes (single-master).|
