...
This article will enumerate some of the key benefits, and challenges, of using NoSQL databases. We will also discuss how this differs from relational databases. For an in-depth look at the different data stores that exist, have a look at our article on [choosing the right data store](https://docs.microsoft.com/azure/architecture/guide/technology-choices/data-store-overview).
...
Ironically, given their name, relational databases can present a less than optimal solution for modeling deep and complex relationships. This is due to the fact that relationships between entities do not actually exist in the database, and need to be computed at runtime, with complex relationships requiring cartesian joins in order to allow mapping using queries. This can become exponentially more expensive in terms of computation as relationships increase. In some cases, this can reach a point where a relational database attempting to manage these entities can become unusable. 
