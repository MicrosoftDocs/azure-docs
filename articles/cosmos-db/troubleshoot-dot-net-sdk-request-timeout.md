...
`CosmosClientOptions.RequestTimeout` (or `ConnectionPolicy.RequestTimeout` for SDK V2) allow to set a timeout that affects each individual network request. An operation started by a user can span multiple network requests (for example, there could be throttling) and this configuration would only apply for each request, not for the scope of the complete operation.
