...
To implement this architecture, you'll need:
...
Now that you have your query, let's save it back to a collection by using the Spark Connector to save the output data into a different collection.  In this example, use Scala to showcase the connection. Similar to the previous example, creating the configuration connection to save our Apache Spark DataFrame to a different Azure Cosmos DB collection.
...
As noted in the previous sections, you can simplify the original lambda architecture by using the following:
* Azure Cosmos DB
