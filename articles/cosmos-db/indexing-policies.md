...
To learn how indexing works in Azure Cosmos DB, you must understand that when you manage indexing policy, you can make fine-grained trade-offs between index storage overhead, write and query throughput, and query consistency.  
...
You can customize the trade-offs between storage, write/query performance, and query consistency by overriding the default indexing policy on an Azure Cosmos DB collection and configuring the following aspects:
...
* **Configure various index types**. For each of the included paths, you can specify the type of index the path requires for a collection. You can specify the type of index based on the path's data, expected query workload, and numeric/string “precision.”
...
The following Microsoft .NET code snippet shows how to set a custom indexing policy when you create a collection. In this example, we set the policy with Range index for strings and numbers at the maximum precision. This policy lets us execute ORDER BY queries against strings.
...
1. In the portal, open your Azure Cosmos DB account, and then select your collection. 
...
The index is generally in catch up mode with ingested data. With Lazy indexing, time to live (TTL) changes result in the index being dropped and recreated, which makes the COUNT and query results inconsistent for a period of time. Because of this, most Azure Cosmos DB accounts should use the Consistent indexing mode.

**None**: A collection that has a "None" index mode has no index associated with it. This is commonly used if Azure Cosmos DB is utilized as a key-value storage, and documents are accessed only by their ID property. 

> [!NOTE]
> Configuring the indexing policy with “None” has the side effect of dropping any existing index. Use this if your access patterns only require “id” and/or “self-link”.
> 
> 
...
| Spatial     | Range over /prop/? (or /) can be used to efficiently serve the following queries:<br><br>SELECT FROM collection c<br><br>WHERE ST_DISTANCE(c.prop, {"type": "Point", "coordinates": [0.0, 10.0]}) < 40<br><br>SELECT FROM collection c WHERE ST_WITHIN(c.prop, {"type": "Polygon", ... }) --with indexing on points enabled<br><br>SELECT FROM collection c WHERE ST_WITHIN({"type": "Point", ... }, c.prop) --with indexing on polygons enabled              |
...
Spatial indexes always use the default index precision for all types (Point, LineString, and Polygon). The default index precision for spatial indexes cannot be overridden. 
...
Similarly, you can completely exclude paths from indexing. The next example shows how to exclude an entire section of the documents (a *subtree*) from indexing by using the **\*** wildcard character.
...
* When you move to None, the index is dropped effective immediately. Moving to None is useful when you want to cancel an in-progress transformation, and start fresh with a different indexing policy. 
