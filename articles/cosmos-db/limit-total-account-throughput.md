...
When using an Azure Cosmos DB account in [provisioned throughput](./set-throughput.md) mode, most of your costs usually come from the amount of throughput that is provisioned across your account. In particular, these costs are directly influenced by:
...
It can be challenging to keep track of the total amount of throughput that is provisioned across your account, especially when you're getting started with Azure Cosmos DB. This can lead to unexpected charges when this amount ends up going over a certain budget that you didn't expect to exceed. To help you better control your costs, Azure Cosmos DB lets you limit the total throughput provisioned on your account.

After you've set a limit to your account's total throughput, any operation that would result in exceeding this limit is blocked and will explicitly fail. This includes the following operations:
...
This section shows a summary of the total throughput provisioned on your account and lets you configure the total throughput limit. 3 options are available:
