...
* **Monitor with diagnostic logs in Azure Monitor:** You can monitor the logs of your Azure Cosmos DB account and create dashboards from the Azure Monitor. Data such as events and traces that occur at a second granularity are stored as logs. For example, if the throughput of a container changes, the properties of an Azure Cosmos DB account are changed and these events are captures within the logs. You can analyze these logs by running queries on the gathered data. To learn more, see the [Analyze log data](#analyzing-logs) section of this article.
...
:::image type="content" source="media/monitor/monitoring-options-portal.png" alt-text="Monitoring options available in Azure portal" border="false":::
...
:::image type="content" source="media/monitor/overview-page.png" alt-text="Overview page":::
...
:::image type="content" source="./media/monitor/add-metrics-filter.png" alt-text="Add a filter to select the metric granularity":::
...
:::image type="content" source="./media/monitor/apply-metrics-splitting.png" alt-text="Add apply splitting filter":::
