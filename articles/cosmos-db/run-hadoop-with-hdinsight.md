...
<table border='1'>
    ...
    <tr><th>Supported HDInsight Versions</th>
        <td>3.1, 3.2</td></tr>
    <tr><th>Change Log</th>
        <td>Updated Cosmos DB Java SDK to 1.6.0</br>
...
2. <p>Let's begin constructing your query string. We'll write a Hive query that takes all documents' system generated timestamps (_ts) and unique ids (_rid) from a Cosmos DB collection, tallies all documents by the minute, and then stores the results back into a new Cosmos DB collection.</p>

    <p>First, let's create a Hive table from our Cosmos DB collection. Add the following code snippet to the PowerShell Script pane <strong>after</strong> the code snippet from #1. Make sure you include the optional DocumentDB.query parameter t trim our documents to just _ts and _rid.</p>
...
8. Check the results. Sign into the [Azure Portal][azure-portal].
   ...
   3. Find and click <strong>Cosmos DB Accounts</strong>. </br>
   4. Next, find your <strong>Cosmos DB Account</strong>, then <strong>Cosmos DB Database</strong> and your <strong>Cosmos DB Collection</strong> associated with the output collection specified in your Hive query.</br>
...
2. <p>Let's begin constructing your query string. We'll write a Pig query that takes all documents' system generated timestamps (_ts) and unique ids (_rid) from a Cosmost DB collection, tallies all documents by the minute, and then stores the results back into a new Cosmos DB collection.</p>
...
10. Check the results. Sign into the [Azure Portal][azure-portal].
    ...
    1. Click <strong>Browse</strong> on the left-side panel. </br>
    2. Click <strong>everything</strong> at the top-right of the browse panel. </br>
    3. Find and click <strong>Cosmost DB Accounts</strong>. </br>
    4. Next, find your <strong>Cosmos DB Account</strong>, then <strong>Cosmos DB Database</strong> and your <strong>Cosmos DB Collection</strong> associated with the output collection specified in your Pig query.</br>
    5. Finally, click <strong>Document Explorer</strong> underneath <strong>Developer Tools</strong>.</br></p>
...
## <a name="RunMapReduce"></a>Step 5: Run a MapReduce job using Cosmos DB and HDInsight
1. Set the following variables in your PowerShell Script pane.

        $subscriptionName = "<SubscriptionName>"   # Azure subscription name
        $clusterName = "<ClusterName>"             # HDInsight cluster name
2. We'll execute a MapReduce job that tallies the number of occurrences for each Document property from your Cosmos DB collection. Add this script snippet **after** the snippet above.
...
6. Check the results. Sign into the [Azure Portal][azure-portal].
   ...
   4. Next, find your <strong>Cosmos DB Account</strong>, then <strong>Cosmos DB Database</strong> and your <strong>Cosmos DB Collection</strong> associated with the output collection specified in your MapReduce job.
