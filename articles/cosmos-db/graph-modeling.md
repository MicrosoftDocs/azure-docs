...
The following document is designed to provide graph data modeling recommendations. This is a vital step in ensuring the scalability and performance of a graph database system through the evolution of the data. An efficient data model is especially important with large-scale graphs.

### Key assumptions

The process outlined in this guided is based on the following assumptions:
 * There are well identified **entities** in the problem-space, and they are meant to be consumed in an _atomic_ way per request. In other words, the database system isn't designed to retrieve a single entity's data in multiple query requests.
 * There are well identified read and write requirements for the database system. These requirements will guide the optimizations needed towards the graph data model.
...
* There are **cyclic relationships** or **self-referenced entities**. This is often a challenge when using relational or document databases.
* There are **dynamically evolving relationships** between entities. This is especially applicable to hierarchical or tree-structured data with many levels.
...
If the above criteria is satisfied, it is likely that a graph database approach will provide advantages in terms of **query complexity**, **data model scalability** and even **query performance**.
...
| Object | Property | Type | Required | Notes |
| --- | --- | --- | --- | --- |
| Vertex | ID | String | No | Uniquely enforced per partition. If a value isn't supplied upon insertion, and auto-generated GUID will be stored. |
| Vertex | label | String | Yes | This property is used to define the type of entity that the vertex represents. |
| Vertex | properties | String, Boolean, Numeric | No | This is a list of separate properties stored as key-value pairs in each vertex. |
| Vertex | partition key | String | Yes | This property defines where the vertex and its outgoing edges will be stored. Read more about [graph partitioning](graph-partitioning.md). |
| Edge | ID | String | No | Uniquely enforced per partition. Auto-generated by default. Edges usually don't have the need to be uniquely retrieved by an ID. |
| Edge | label | String | Yes | This property is used to define the type of relationship that two vertices have. |
| Edge | properties | String, Boolean, Numeric | No | This is a list of separate properties stored as key-value pairs in each edge. |

## Entity and relationship modeling guidelines

The following are a set of guidelines to approach data modeling for an Azure Cosmos DB Gremlin API graph database. These guidelines assume that there is an existing definition of a data domain and queries for it.
...
However, traversing in the opposite direction of an edge, using the `in()` function, will always result in a cross-partition query. Learn more about [graph partitioning](graph-partitioning.md). If there is a need to constantly traverse using the `in()` function, it is recommended to add edges in both directions.
...
Using descriptive relationship labels can improve the efficiency of edge resolution operations. This can be achieved in the following ways:
