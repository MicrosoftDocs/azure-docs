...
|HTTPS/SSL/TLS encryption|All connections to Azure Cosmos DB support HTTPS. Azure Cosmos DB supports TLS levels up to 1.2 (included).<br>It's possible to enforce a minimum TLS level server-side. Learn more in [Self-Serve minimum TLS version enforcement in Azure Cosmos DB](./self-serve-tls.md).|
