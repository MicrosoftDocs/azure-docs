...
|Active directory integration (RBAC)| You can also provide or restrict access to the Cosmos account, database, container and offers (throughput) using Access control (IAM) in the Azure portal. IAM provides role-based access control and integrates with Active Directory. You can use built in roles or custom roles for individuals and groups. See [Active Directory integration](database-security-rbac.md) for more information.|
