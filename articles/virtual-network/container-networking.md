...
-	Pods in a subnet that has service endpoints enabled can securely connect to service-endpoint enable Azure services, such as Azure Storage and Azure SQL Database.
...
- Outbound traffic from a pod is network address translated to the private IP address of the primary IP configuration of the primary network interface in each node of the Kubernetes cluster, resulting in the following behavior:
    ...
    - All outbound communication to the Internet is sent using the public IP address assigned to the primary IP configuration of the primary network interface. 

## Next steps

Learn how to deploy a Kubernetes cluster in your own virtual network. @@@ Aanand to provide link @@@