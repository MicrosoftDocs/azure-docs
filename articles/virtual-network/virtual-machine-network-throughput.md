...
[!IMPORTANT]
**Network Virtual Appliances (NVAs)** such as gateways, proxies, firewalls, and other applications that forward traffic should use **half** the recommended connection limits shown above. Forwarding traffic consumes twice the number of flows compared to typical client-server communication. For example, an F16s_v2 VM with a recommended limit of 700,000 connections has an effective NVA limit of ~350,000 flows. Monitor NVA flow counts using the **Inbound Flows** and **Outbound Flows** metrics in Azure Monitor to detect when you are approaching this limit. Consider adding additional NVA instances to distribute load when flow counts trend toward this threshold.
