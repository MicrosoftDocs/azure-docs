...
5. Enter values for the following parameters:
    ...
    - **Subnet delegation:** A subnet can have zero to multiple delegations enabled for it. Delegation enable the service to be deployed with a unique identifier. To delegate for a service, select the service you want to delegate to from the **Services** list. 
...
4. In the list of subnets, select the subnet you want to change settings for. You can change the following settings:
    ...
    - **Subnet delegation:** See service endpoints in step 5 of [Add a subnet](#add-a-subnet). Subnet delegation can be modified to zero or multiple delegations enabled for it. If the service is already deployed in the subnet, subnet delegation cannot be removed until the service is un-deployed. To delegate for a different service, select the service you want to delegate to from the **Services** list. 
5. Select **Save**.
