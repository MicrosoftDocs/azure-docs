...
|Port range     |You can specify an individual or range of ports. For example, you could specify 80 or 10000-10005. Specifying ranges enables you to create fewer security rules. The ability to specify multiple ports and ranges is in preview release. Features in preview do not have the same level of availability or reliability as features in general release. You can specify multiple individual ports and ranges in network security groups in only the following Azure [regions](https://azure.microsoft.com/regions): East US, West US, West US 2, Australia East, Australia Southeast, and UK South. Specifying multiple ports and port ranges is only possible when creating rules in network security groups created through the Resource Manager deployment model. You cannot specify multiple ports and port ranges in network security groups created through the classic deployment model.    |
...
Application security groups enable you to group virtual machines with like port filtering requirements together and specify the group as the source or destination address for [security rules](#security-rules). For example, if you wanted to always allow ports 80 and 443 inbound to web servers, you could:
