...
- Azure VPN Gateway and ExpressRoute Gateway publish the summarized prefixes list from the virtual network with the gateway (hub). Azure VPN Gateway and ExpressRoute Gateway then check the hub address space and don't publish any addresses that are covered by the summarized space. If the address space isn't covered, it's still advertised.
