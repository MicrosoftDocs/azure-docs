...
There are 2 ways to resolve general internet names from your own DNS server hosted in Azure.  The default installation of the DNS roll is to do recursive queries to the internet root servers.  The second way is to forward all non-authoritative queries to 168.63.129.16.  Both are acceptable DNS practices.  However, in azure the default behavior may lead to some issues.  You may experience timeouts when doing recursive queries to DNS root servers from DNS running on an azure based VM.  There are several reasons for this.  The best practice for these timeouts is to create the general forwarder to 168.63.129.16.  
