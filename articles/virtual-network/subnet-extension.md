...
Migrations can get a bit challenging when the above requirement is coupled with an additional requirement to keep some applications on-premises. This would mean that you have to extend your on-premises network into Azure because applications on both sides cannot change IP addresses. While extending your network is not a good practice in general, below use cases might make it necessary.
...
In the next section, you will find details on third-party solutions, we have tested on Azure, and steps to set them up.
