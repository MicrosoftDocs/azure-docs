...
Migrations can get a bit challenging when the above requirement is coupled with an additional requirement to keep some applications on-premises. In such as a situation, you will have to split the applications between Azure and on-premises, without renumbering the IP addresses on either side. Additionally, you will have to allow the applications to communicate as if they are in the same network.
...
While extending your network is not a good practice in general, below use cases might make it necessary.
...
- **Latency**: Low latency requirements can be another reason for you to keep some applications on-premises to ensure that they are as close as possible to your datacenters.
...
In the next section, we will discuss how you can extend your subnets into Azure.
...
The IP addresses from the subnet are assigned to applications on Azure and on-premises. Both Azure and on-premises have an NVA inserted in their networks. When an application in Azure tries to talk to an application in on-premises network, the Azure NVA captures the packet, encapsulates it, and sends it over VPN/Express Route to the on-premises network. The on-premises NVA receives the packet, decapsulates it and forwards it to the intended recipient in its network. The return traffic uses a similar path and logic.
...
In the next section, you will find details om subnet extension solutions we have tested on Azure.
