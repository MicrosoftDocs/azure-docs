...
* A virtual network named `azurevnet` is segmented as follows:
  ...
  * `azsn1`: An external firewall subnet used exclusively for the external firewall. All internet traffic comes in through this subnet. This subnet only contains a NIC linked to the external firewall.
  ...
  * `azsn4`: A management subnet used exclusively to provide management access to all firewall virtual appliances. This subnet only contains a NIC for each firewall virtual appliance used in the solution.
...
In this scenario, the only traffic flowing from on-premises to Azure is used to manage the firewalls by connecting to `AZF3`, and that traffic must go through the internal firewall, `AZF2`. Only one route is necessary in `GatewaySubnet`, as shown here:
...
Without IP forwarding enabled for `OPFW`, the Azure virtual networking logic drops the packets because it only allows packets to be sent to a VM if the VM's IP address is the destination for the packet.
