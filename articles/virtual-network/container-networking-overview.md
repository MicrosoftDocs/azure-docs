...
- **ACS-Engine**: ACS-Engine is a tool that generates an Azure Resource Manager template for the deployment of a Kubernetes cluster in Azure. The plug-in is the default networking plugin for clusters created using the ACS-Engine. The cluster configuration is specified in a JSON file that is passed to the tool when generating the template. To learn more about the entire list of supported cluster settings and their descriptions, see [Microsoft Azure Container Service Engine - Cluster Definition](https://github.com/Azure/acs-engine/blob/master/docs/clusterdefinition.md). The following network configuration settings are important when configuring the plug-in
  ...
  For more information about the network configuration required when using the plug-in, see [Using Azure integrated networking (CNI)](https://github.com/Azure/acs-engine/blob/master/docs/kubernetes/features.md#using-azure-integrated-networking-cni).
