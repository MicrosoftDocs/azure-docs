...
- For virtual network peering traffic to work correctly, a system route with a next hop type of *VNet Peering* must exist for the peered virtual network's prefix range. If such a route doesn’t exist, and the virtual network peering link is **Connected**:
    ...
    - If you have multiple network interfaces attached to the VM, only the first network interface is assigned a default gateway, by default. Learn how to create a default route for additional network interfaces for a [Windows](../virtual-machines/windows/multiple-nics.md?toc=%2fazure%2fvirtual-network%2ftoc.json#configure-guest-os-for-multiple-nics) or [Linux](../virtual-machines/linux/multiple-nics.md?toc=%2fazure%2fvirtual-network%2ftoc.json#configure-guest-os-for-multiple-nics) VM.
