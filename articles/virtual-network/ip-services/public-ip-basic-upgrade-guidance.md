...
6. Depending on the resource associated with your Basic SKU public IP addresses, perform the upgrade based on the following table:
  ...
  | Azure Databricks (using Basic IPs) | For ephemeral workloads, Standard SKU public IP addresses will be deployed as virtual machines (VMs) cycle out through regular usage attrition with new VMs. For long running workloads, it is recommended to [restart compute](..//databricks/compute/clusters-manage.md#restart) which will replace the exisiting Basic IP with Standard IP.  |
