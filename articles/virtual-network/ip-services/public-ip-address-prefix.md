...
>[!NOTE]
>If you're [deriving a Public IP Prefix from a Custom IP Prefix (BYOIP range)](manage-custom-ip-address-prefix.md#create-a-public-ip-prefix-from-a-custom-ip-prefix), the prefix size can be as large as the Custom IP Prefix.
...
| v2 NAT Gateway | You can use a v2 public IP from a prefix for your v2 NAT gateway | To associate an IP from a prefix to your v2 NAT gateway:</br> 1. [Create a v2 prefix.](manage-public-ip-address-prefix.md)</br> 2. [Create a v2 IP from the prefix.](manage-public-ip-address-prefix.md)</br> 3. When you deploy the [v2 NAT Gateway](/azure/nat-gateway/quickstart-create-nat-gateway-v2), be sure to select the v2 IP you previously gave from the prefix.|
...
| v2 NAT Gateway | A public IP prefix can be used to scale a v2 NAT gateway by using the public IPs in the prefix for outbound connections. | To associate a prefix to your v2 NAT Gateway:</br> 1. [Create a v2 prefix.](manage-public-ip-address-prefix.md)</br> 2. When creating the v2 NAT Gateway, select the v2 IP prefix as the Outbound IP. (A v2 NAT Gateway can have no more than 16 IPs in total. A public IP prefix of /28 length is the maximum size that can be used.) |
