...
> [!NOTE]
> Azure Virtual Network is one of the services that make up the Network Foundations category in Azure. Other services in this category include [Azure DNS](/azure/dns/dns-overview.md) and [Azure Private Link](/azure/private-link/private-link-overview.md). Each service has its own unique features and use cases. For more information on this service category, see [Network Foundations](/azure/networking/foundations/network-foundations-overview.md).
