...
Additionally, ou can expect that traffic to flow from virtual public IP address 168.63.129.16 to the endpoint that is configured for a [Load Balancer health probe](../load-balancer/load-balancer-custom-probe-overview.md). In a non-virtual network scenario, the health probe is sourced from a private IP. 
