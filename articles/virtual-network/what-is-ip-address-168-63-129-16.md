...
Additionally, traffics from virtual public IP address 168.63.129.16 to the endpoint that configured for a [Load Balancer health probe](../load-balancer/load-balancer-custom-probe-overview.md) should not be considered attack traffic. In a non-virtual network scenario, the health probe is sourced from a private IP.
