---
title: Security baseline for Virtual Network
description: Security baseline for Virtual Network
author: msmbaldwin
ms.service: security
ms.topic: conceptual
ms.date: 06/30/2020
...
* [For more information, see Azure Firewall threat intelligence-based filtering](https://docs.microsoft.com/azure/firewall/threat-intel.)

Use Azure Security Center threat protection to detect communications with known malicious IP addresses.

* [For more information, see Threat protection in Azure Security Center](https://docs.microsoft.com/azure/security-center/threat-protection.)
...
* [For more information, see Adaptive Network Hardening in Azure Security Center](https://docs.microsoft.com/azure/security-center/security-center-adaptive-network-hardening.)
...
**Guidance**: You may use a VPN gateway packet capture in addition to commonly available packet capture tools to record network packets on the customer side of the network, the Azure side of the network, or somewhere in between.
...
You may also use Azure Blueprints to simplify large scale Azure deployments by packaging key environment artifacts, such as Azure Resource Manager templates, role-based access control assignments, and policies, in a single blueprint definition. You can apply the blueprint to new subscriptions, and fine-tune control and management through versioning.
...
**Guidance**: Use Azure Activity Log to monitor resource configurations and detect changes to your virtual network. Create alerts within Azure Monitor that will trigger when changes to critical resources take place.
...
**Guidance**: Old ValueWithin Azure Monitor, set your Log Analytics Workspace retention period according to your organization's compliance regulations. Use Azure Storage Accounts for long-term/archival storage.
...
**Guidance**: Use Azure Active Directory(Azure AD) built-in administrator roles that can be explicitly assigned and are queryable. Use the Azure AD PowerShell module to perform ad hoc queries to discover accounts that are members of administrative groups.
...
**Guidance**: Use Azure AD Risk and Identity Protection features to configure automated responses to detected suspicious actions related to user identities for your Virtual network. You can also ingest data into Azure Sentinel for further investigation.
...
**Guidance**: Use Tags to assist in tracking Azure virtual networks that contains Azure resources which store or process sensitive information.
...
**Guidance**: Encrypt all sensitive information in transit. Ensure that any clients connecting to your Azure resources in your virtual networks are able to negotiate TLS 1.2 or greater.
...
**Guidance**: Use Azure Monitor with the Azure Activity Log to create alerts for when changes take place to critical Azure resources like Virtual network, network security group.
...
**Guidance**: Follow recommendations from Azure Security Center on performing vulnerability assessments on your Azure virtual network resources
...
**Guidance**: Use Azure Resource Graph to query and discover all networking resources like Virtual networks, subnets within your subscription(s). Ensure you have appropriate (read) permissions in your tenant and are able to enumerate all Azure subscriptions as well as resources within your subscriptions.
...
**Guidance**: Use Azure Resource Manager templates and Azure Policies to securely configure Azure resources associated with the Virtual network and related resources. Azure Resource Manager templates are JSON based files used to deploy Virtual machine along with Azure resources and custom template will need to be maintained. Microsoft performs the maintenance on the base templates. Use Azure policy [deny] and [deploy if not exist] to enforce secure settings across your Azure resources.
...
**Guidance**: Use Azure Security Center to perform baseline scans for your Azure virtual network and related resources. Additionally, use Azure Policy to alert and audit Azure resource configurations.
...
**Guidance**: Use Managed Service Identity in conjunction with Azure Key Vault to simplify and secure secret management for your Azure resources hosted in the Azure virtual network .
...
**Guidance**: Use Azure Resource Manager to deploy the Virtual network and related resources. Azure Resource Manager provides ability to export templates which can be used as backups to restore Virtual network and related resources. Use Azure Automation to call the Azure Resource Manager template export API on a regular basis.
