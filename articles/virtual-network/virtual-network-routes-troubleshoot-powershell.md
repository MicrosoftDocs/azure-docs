...
1. Start an Azure PowerShell session and login to Azure. If you’re not familiar with Azure PowerShell, read the [How to install and configure Azure PowerShell](/powershell/azure/overview) article. Your account must be assigned the *Microsoft.Network/networkInterfaces/effectiveRouteTable/action* operation. To learn how to assign operations to accounts, see [Create custom roles for Azure Role-Based Access Control](../active-directory/role-based-access-control-custom-roles?toc=%2fazure%2fvirtual-network%2ftoc.json#actions)
