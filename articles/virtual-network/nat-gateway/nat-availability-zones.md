...
Virtual networks and their subnets are regional constructs, which means that subnets aren't restricted to a zone. While NAT gateway is a zonal resource, it is a highly resilient and reliable method by which to connect outbound to the internet from virtual network subnets. This is because NAT gateway leverages [software defined networking](/azure-stack/hci/concepts/software-defined-networking) in order to operate as a fully managed and distributed service. NAT gateway infrastructure has built in redundancy which means it can survive multiple infrastructure component failures. Availability zones build on this resiliency with zone isolation scenarios for NAT gateway. 

## Zonal

You can place you NAT gateway resource in a specific zone for a region. When NAT gateway is deployed to a specific zone, it will provide outbound connectivity to the internet explicitly from that zone. The public IP address or prefix configured to NAT gateway, must match the same zone. NAT gateway resources with public IP addresses from a different zone, zone-redundancy or with no zone aren't allowed.
...
While virtual machines in subnets from different availability zones can all be configured to a single zonal NAT gateway resource, this setup does not provide the most effective method for ensuring zone-resiliency against zonal outages. See [Design considerations](#design-considerations) below for additional information on how to safeguard against zonal outages with NAT gateway. 

## Non-zonal
If no zone is selected at the time that the NAT gateway resource is deployed, then it is placed in ‘no zone’ by default. When NAT gateway is placed in ‘no zone’, Azure places the resource in a zone for you and you will not have visibility into which zone Azure chooses for your NAT gateway. Also, after NAT gateway is deployed, zonal configurations cannot be changed. ‘No zone’ NAT gateway resources, while still zonal resources can be associated to public IP addresses from a zone, no zone, or that are zone-redundant. 
...
Failure of outbound connectivity due to a zone outage will be isolated to the specific zone that goes down and will not impact the other zonal stacks (where other NAT gateways are deployed with their own subnets and zonal public IPs). 
...
### Integration of inbound with Standard Load balancer  
