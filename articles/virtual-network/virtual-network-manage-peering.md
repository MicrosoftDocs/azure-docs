...
6. In the **Add peering** blade, enter or select values for the following settings:
	...
	- **Virtual network:** Select the VNet you want to peer with. You can select a VNet created through either Azure deployment model, but the VNet must be in the same location as the VNet you're initiating the peering from. You must have read access to the VNet for it to be visible in the list. If a VNet is listed, but grayed out, it may be because the address space for the VNet overlaps with the address space for this VNet. VNets cannot be peered if their address spaces overlap. If you checked the **Resource ID** checkbox, this setting isn't available.
