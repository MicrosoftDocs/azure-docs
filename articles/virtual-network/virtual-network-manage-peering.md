...
* <a name="cross-region"></a>You can peer virtual networks in the same, or different regions. The following constraints do not apply when both virtual networks are in the *same* region, but do apply when the virtual networks are in *different* regions: 
    - The virtual networks can only exist in the following regions: West Central US (Wyoming), West US 2 (Washington), Central US (Iowa), US East 2 (Virginia), Canada Central (Toronto), Canada East (Quebec City), Korea South (Buscan), South India (Chennai), Central India (Pune), West India (Mumbai), UK South (London), UK West (Cardiff)
