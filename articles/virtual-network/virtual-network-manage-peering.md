...
- When peering virtual networks in different regions, the virtual networks can exist in any Azure public cloud region, but neither virtual network can exist in an Azure national cloud. You can however, peer two virtual networks within the same national cloud.
- If the peered virtual networks are in different regions, resources in one virtual network cannot communicate with the IP address of an Azure internal load balancer in the peered virtual network. The load balancer and the resources that communicate with it must be in the same virtual network. When the peered virtual networks are in the same region, resources in either virtual network can communicate with the IP address of an Azure internal load balancer in the peered virtual network.
- If the virtual networks are in different regions, the resources in one virtual network cannot transit a gateway in the peered virtual network, nor can a gateway in one virtual network be transited by resources in the peered virtual network. To use remote gateways or allow gateway transit, both virtual networks in the peering must exist in the same region. 
