...
- <a name="cross-region"></a>You can peer virtual networks in the same region, or different regions. The following constraints do not apply when both virtual networks are in the *same* region, but do apply when the virtual networks are globally peered: 
    ...
    - Communication across globally peered virtual networks through the following VM types is not supported: [High performance compute](../virtual-machines/windows/sizes-hpc.md) and [GPU](../virtual-machines/windows/sizes-gpu.md). This includes H, NC, NV, NCv2, NCv3, and ND series VMs.
