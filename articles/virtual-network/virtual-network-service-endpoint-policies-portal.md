...
Customer intent: I want to set up service endpoint policies, and associate with a subnet. 
...
Service endpoint policies enable you to filter VNet traffic to specific Azure resources, over service endpoints. If you're not familiar with service endpoint policies, see [Service endpoint policies overview](virtual-network-service-endpoint-policies-overview.md) to learn more.
...
If you prefer, you can complete this tutorial using the [Azure CLI](virtual-network-service-endpoint-policies-cli.md) or [Azure PowerShell](virtual-network-service-endpoint-policies-powershell.md).
...
6. Select **Review + Create**. Validate the information and Click **Create**. To make further edits, click **Previous**. 
 ...
 ![Create service endpoint policy definitions](./media/virtual-network-service-endpoint-policies-portal/virtual-network-endpoint-policies-create-finalcreatereview.png)
...
2. Under **Subscriptions**, select your subscription and resource group, as shown in the following picture:
    ...
    ![Show policies](./media/virtual-network-service-endpoint-policies-portal/virtual-network-endpoint-policies-viewpolicies.png)
 
3. Select the policy and click on **Policy Definitions** to view or add more policy definitions. 

    ![Show policy definitionss](./media/virtual-network-service-endpoint-policies-portal/virtual-network-endpoint-policies-viewpolicy-adddefinitions.png)
4. Select **Associated subnets** to view the subnets the policy is associated. 
To associate policy to a subnet, click on "Navigate to virtual network in the same region".

  ![Show policies](./media/virtual-network-service-endpoint-policies-portal/virtual-network-endpoint-policies-view-associatedsubnets.png)
