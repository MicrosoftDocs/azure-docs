...
- `isReceiving`:
    - Will inform the application if remote video stream data is being received or not. Such scenarios are:
        ...
        - I'm viewing the video of a remote participant who is on mobile browser and they lock the device. I see the RemoteVideoStream.isReceiving flag goes to false. Once the remote participant unlocks the device and navigates to the Azure Communication Services call, then ill see the flag go back to true. Same behavior when remote participant is on desktop and the desktop locks/sleeps
