...
- `endpointDetails`: Get the details of all the endpoints for this remote participant
    ...
    ```
    *Note: A remote participant could be in the call from many endpoints, and each endpoint will have its own unique `participantId`. `participantId` is different from the RemoteParticipant.identifier's raww Id.*
