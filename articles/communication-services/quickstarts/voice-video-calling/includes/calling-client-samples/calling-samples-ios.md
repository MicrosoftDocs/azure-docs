...
```swift
let serverCallId = call.info.getServerCallId(completionHandler: { (serverId, error) in })
...
```swift
let isTranscriptionActive = callTranscriptionFeature.isRecordingisTranscriptionActiveActive();
```
