...
Please check out [Azure Function integration](../../../tutorials/integrate-azure-function.md) for details. We highly recommand integrate with Azure Function to avoid hard-coding application contants.

### Set up application constants:
Create a class 'ApplicationConstants' which stores all application contants:
```java
...
```
1. 'ACS_ENDPOINT' is the endpoint of your Communication Services resource.
2. `<first_user_id>` and `<second_user_id>` are valid Communication Services user IDs generated by your Communication Services resource.
3. `<first_user_access_token>` with the Communication Services access token for `<first_user_id>`.
...
> [!NOTE]
> Inilitialize 'ApplicationConstants' needs to be added in file `MainActivity.java` following the placeholder, if EITHER condition meets. 1. push notification feature is NOT enabled. 2. Android chat SDK version < '2.0.0'. Otherwise, please refer to step 11 in [Android push notifications](../../../tutorials/chat-android-push-notification.md).
Code to initialize application constants:
```java
try {
...
```
ACS_ENDPOINT, FIRST_USER_ID and FIRST_USER_ACCESS_TOKEN are returned from calling Azure Function. Please check out [Azure Function integration](../../../tutorials/integrate-azure-function.md) for details.
