...
To create a chat client, you'll use Communications Service endpoint and the access token that was generated as part of pre-requisite steps. You need to use the `CommunicationIdentityClient` class from the `Administration SDK` to create user and issue a token to pass to your chat client. For details, see [here](../../user-access-tokens.md)
