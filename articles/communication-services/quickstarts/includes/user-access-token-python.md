...
Azure Communication Services maintains a lightweight identity directory. Use the `create_user` method to create a new entry in the directory with a unique `Id`. Store received identity for later use with mapping to your application's users (for example, by storing them in your application server's database).
