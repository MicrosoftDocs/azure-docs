...
Note by default the dev tunnels are disabled in Visual Studio. To enable dev tunnels, go to Tools, than Options and enable dev tunnels in Preview Features menu.
...
The code sample also illustrates how you can control the callback URI by setting your own context/ID when you answer the call. All events generated by the call is sent to the specific route you provide when answering an inbound call and the same applies to when you place an outbound call.  
