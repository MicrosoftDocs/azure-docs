...
- The latest [nodejs](https://nodejs.org/en) for your operating system. latest stable LTS version is recommended. It is also recommended to use [TypeScript](https://www.npmjs.com/package/typescript) as `@azure/communication-call-automation` package provides all the necessary type declarations, but it is optional.
...
Install the following dependencies to your project. In this quickstart we will use `express` as our web API framework.
...
First, decide the port for your application. `3000` is the default endpoint of an exress app.
