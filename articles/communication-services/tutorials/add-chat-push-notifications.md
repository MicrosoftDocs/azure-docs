...
1. Create an Azure Notification Hub within the same subscription as your Communication Services resource and link the Notification Hub to your Communication Services resource. See [Notification Hub provisioning](../concepts/notifications.md#notification-hub-provisioning).

1. Create a .p12 APNS cert and set it in Notification Hub.  
   ...
   1. Log in to the Apple Developer Portal. Navigate to `Certificates, IDs & Profiles > Identifiers > App IDs` and click the App ID associated with your app.
   ...
   1. On the screen for your App ID, check  `Capabilities > Push Notifications`. Click Save and respond "Confirm" to the Modify App Capabilities dialog box that appears.
   ...
   1. In the same page, click `Capabilities > Push Notifications > Configure`. Click one of the following buttons:
   ...
   1. Click "Download" and save the file to local disk.
   ...
   1. Go to your notification hub, click "Apple (APNS)" under Settings and select "Certificate" under Authentication Mode. Also select the Application Mode based on your need. Then upload the .p12 file you just created.
...
**Step 1. Configure User Access Token Auto Refresh**
...
1.1 Establish a Service Tier for Token Issuance
...
1.2 Implement the Token Refresher in Your Chat Application

Once your Azure Function is configured:

Obtain the Token Issuer URL from the Azure Portal, which your chat application uses to request new tokens.
...
**Step 2. Follow Apple Instruction to enable and schedule Background tasks**
...
1. Handle Notifications in Your App<br>
