...
description: Learn how to enable push notification in an iOS app by using the Azure Communication Services Chat SDK.
author: angellan
...
# Tutorial: Enable push notifications in your chat app

This tutorial will guide you to enable push notifications in your iOS app by using the Azure Communication Services Chat SDK.
...
- **Basic version**: The user will be able to see a badge number of 1 on the app's icon, receive a notification sound and see a pop-up alert banner.
  ...
  > ![Screenshot of basic version of push notification.](./media/add-chat-push-notification/basic-version.png)
...
- **Advanced version**: Except for the features supported in basic version, the Contoso will be able to customize the title & message preview section in alert banner.
  ...
  > [!div class="mx-imgBorder"]
  > ![Screenshot of advanced version of push notification.](./media/add-chat-push-notification/advanced-version.png)
...
- Finish all the prerequisite steps in [Chat Quickstart](../quickstarts/chat/get-started.md?pivots=programming-language-swift).
...
## Create a .p12 APNS cert and set it in Notification Hub

If you are not a Microsoft internal client, follow step 1 to step 9 in the following procedure.

If you are a Microsoft internal client, [submit a ticket](https://aka.ms/mapsupport) and provide the bundle ID of your app to get a .p12 cert. Once you get a valid certificate issued, execute the step 9.

1. Log in to the Apple Developer Portal. Navigate to `Certificates, IDs & Profiles > Identifiers > App IDs` and select the App ID associated with your app.

   > [!div class="mx-imgBorder"]
   > ![Screenshot of APNS Cert Configuration step 1.](./media/add-chat-push-notification/cert-1.png)

1. On the screen for your App ID, check  `Capabilities > Push Notifications`. Select Save and respond "Confirm" to the Modify App Capabilities dialog box that appears.

   > [!div class="mx-imgBorder"]
   > ![Screenshot of APNS Cert Configuration step 2-1.](./media/add-chat-push-notification/cert-2.png)

   > [!div class="mx-imgBorder"]
   > ![Screenshot of APNS Cert Configuration step 2-2.](./media/add-chat-push-notification/cert-3.png)

1. In the same page, select `Capabilities > Push Notifications > Configure`. Select one of the following buttons:
   ...
   > [!div class="mx-imgBorder"]
   > ![Screenshot of APNS Cert Configuration step 3.](./media/add-chat-push-notification/cert-4.png)

1. Then you're navigated to the below page. Here, you'll upload a Certificate Signing Request (CSR). Follow the next step to create a CSR.

   > [!div class="mx-imgBorder"]
   > ![Screenshot of APNS Cert Configuration step 4.](./media/add-chat-push-notification/cert-5.png)

1. In a new browser tab, follow this [help page](https://help.apple.com/developer-account/#/devbfa00fef7) to create a CSR and save the file as "App name.cer".

   > [!div class="mx-imgBorder"]
   > ![Screenshot of APNS Cert Configuration step 5 - 1.](./media/add-chat-push-notification/cert-6.png)

   > [!div class="mx-imgBorder"]
   > ![Screenshot of APNS Cert Configuration step 5 - 2.](./media/add-chat-push-notification/cert-7.png)

1. Drag the .cer file to "Choose File" area. Then hit "continue" on the right top corner.

   > [!div class="mx-imgBorder"]
   > ![Screenshot of APNS Cert Configuration step 6.](./media/add-chat-push-notification/cert-8.png)

1. Select "Download" and save the file to local disk.

   > [!div class="mx-imgBorder"]
   > ![Screenshot of APNS Cert Configuration step 7.](./media/add-chat-push-notification/cert-9.png)

1. Open the .cer file you downloaded; it will open Keychain Access. Select your certificate, right-click, and export your certificate in .p12 format.

   > [!div class="mx-imgBorder"]
   > ![Screenshot of APNS Cert Configuration step 8.](./media/add-chat-push-notification/cert-11.png)

1. Go to your notification hub, select "Apple (APNS)" under Settings and select "Certificate" under Authentication Mode. Also select the Application Mode based on your need. Then upload the .p12 file you just created.

   > [!div class="mx-imgBorder"]
   > ![Screenshot of APNS Cert Configuration step 9.](./media/add-chat-push-notification/cert-12.png)

## Configure XCode

1. In XCode, go to  `Signing & Capabilities`. Add a capability by selecting "+ Capability", and then select "Push Notifications".  

1. Add another capability by selecting "+ Capability", and then select "Background Modes". Also select "Remote Notifications" under Background Modes.

   > [!div class="mx-imgBorder"]
   > ![Screenshot of Enable Push Notifications and Background modes in Xcode.](./media/add-chat-push-notification/xcode-config.png)

1. Set "Require Only App-Extension-Safe API" as "No" for Pod Target - AzureCore.
...
Create a persistent data storage in IOS device. This data storage should be able to share data between Main App and App Extensions (Refer to Item 2 for more information about App Extension – Notification Service Extension).
...
Implement a "Notification Service Extension" bundled with main app. This is used for decrypting the push notification payload when receiving it.
...
#### Implementation of PushNotificationKeyStorage Protocol
...
For PushNotificationKeyStorage, it defines two methods:

- `onPersistKey(encryptionKey:expiryTime)`: This method is used to persist the encryption key in the storage of user's IOS device. Chat SDK will set 45 minutes as the expiry time of the encryption key. If you want Push Notification to be effect for more than 45 minutes, you need to schedule to call `chatClient.startPushNotifications(deviceToken:)` on a comparatively frequent basis (eg. every 15 minutes) so a new encryption key could be registered before the old key expires.

- `onRetrieveKeys() -> [String]`: This method is used to retrieve the valid keys previously stored. You have the flexibility to provide the customization based on the data storage (item 1) you choose.
...
5. Plug the IOS device into your mac, run the program and select "allow" when asked to authorize push notification on device.
...
Background Tasks offer a way to perform activities even when the app is not in the foreground. By implementing a background task, your app can request additional time to complete a specific task, such as renewing the push notification registration. Here's how you can leverage background tasks for registration renewal:

#### Configure automatic refresh of user access tokens

To ensure uninterrupted access to chat services, you need to maintain valid user access tokens. Tokens typically have a default validity period (24 hours), after which they expire, requiring renewal. Implementing an auto-refresh mechanism guarantees that the token is always valid whenever the chat application is activated.

The Chat SDK streamlines token management by automating the refresh process when a custom access token refresher is implemented. Complete the following steps to configure your chat application to support token auto-refresh:

1. To ensure your chat application maintains continuous and secure user access, you need to implement a service tier dedicated to issuing tokens. One option is to leverage Azure Functions for this purpose.
...
1. Once your Azure Function is configured:

   1. Obtain the Token Issuer URL from the Azure Portal, which your chat application uses to request new tokens.

   1. Create and Integrate the Token Refresher within your chat application. This implements a token refresher component that requests fresh tokens and initializes the Chat Client with this component for seamless automatic token renewal.

      For easy reference and implementation, sample code for the token refresher and its integration with the Chat Client is available in our [Sample App Repository](https://github.com/Azure-Samples/communication-services-ios-quickstarts/tree/main/add-chat-push-notifications-auto-renew).
...
1. Set up an Azure Notification Hub.

  Azure Notification Hub provides a scalable, easy-to-use push notification infrastructure that can send notifications to any platform (iOS, Android, Windows, and so on) from any backend (cloud or on-premises). It's essential for managing and pushing notifications to devices.
  
  You can reuse the same notification hub that you use for regular push notifications. If you want to set up a new one, see [Azure Notification Hubs Documentation](../../notification-hubs/notification-hubs-push-notification-overview.md).

1. Configure Azure Function App for regular notifications.
