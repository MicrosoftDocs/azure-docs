...
You can configure Microsoft Teams tenant to allow Azure Communication Services users to join Teams meetings scheduled by the organization. In this article, you will learn how to optimize the user experience when connecting Azure Communication Services. Microsoft Teams users might have available actions that are not supported by Azure Communication Services or are not implemented by the application that uses Azure Communication Services. For those cases, we provide best practices for improving the experience of virtual appointments. 
