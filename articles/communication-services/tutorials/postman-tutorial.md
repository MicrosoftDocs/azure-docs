...
In this tutorial, you set up and use Postman to make a request against Azure Communication Services by using HTTP. By the end of this tutorial, you successfully send an SMS message by using Communication Services and Postman. You can then use Postman to explore other APIs in Communication Services.
...
After you create an account or skip the step, you now see Postman's main window.
...
To handle authentication and make requests easier, you specify two collection variables within the newly created Communication Services collection. These variables are available to all requests in your collection. To start creating variables, use the **Variables** tab.
...
Enter these values in the **Initial Value** column on the **Variables** pane. Select **Persist All** in the upper-right corner. When configured correctly, your Postman pane should look something like the following image.
...
Lastly, you sign this string by using your Communication Services key. Then add that to your request in the `Authorization` header.
...
Now that everything is set up, you're ready to create a Communication Services request in Postman. To get started, select the Plus sign (+) icon next to the Communication Services collection.

:::image type="content" source="media/postman/create-request.png" alt-text="Screenshot that shows the Postman Plus sign (+) icon.":::
...
Now that everything is set up, you can send the request and get an SMS message on your phone. To do this step, ensure that your request is selected and then select **Send** on the right.
