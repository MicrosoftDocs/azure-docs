...
To provide the details of your TURN servers, you need to pass details of what TURN server to use as part of `CallClientOptions` while initializing the `CallClient`. For more information how to setup a call, see [Azure Communication Services Windows SDK](../../quickstarts/voice-video-calling/get-started-with-video-calling.md?pivots=platform-windows) for the Quickstart on how to setup Voice and Video.
...
iceServer1.Uris = new List<Uri>() { new Uri("turn:turn.azure.com"), new Uri("turn:20.202.255.255") }.AsReadOnly();
...
iceServer2.Uris = new List<Uri>() { new Uri("turn:turn1.azure.com") }.AsReadOnly();
...
callNetworkOptions.IceServers = new List<IceServer>() { iceServer1, iceServer2 }.AsReadOnly();
