...
### When do you receive an error code? 
If an API call is made and it is not accepted, an error code will be given by ACS describing why the API call was rejected.  If an API call is made and is accepted but something still goes wrong, both a callback event and an error code will be received.  An example of a common callback event for call failures would be our callDisconnected event that is surfaced whenever a call is disconnected.  Once the callDisconneted event is received, there will not be any more callback events for that given call.  For a list of call signaling callback events see (link), and for a list of callback events for media actions see (). 
...
In addition to the code and subcode, more detailed troubleshooting information can be found in the call logs.  To learn more about how to access call logs, see this page (Troubleshooting in Azure Communication Services | Microsoft Learn).  If you have not yet enabled calling logs, you can enable them using Azure Monitor (more information here Azure Communication Services-Enable Azure Monitor - An Azure Communication Services concept document | Microsoft Learn).  

## Call Automation SDK error codes

The following table contains the most common codes and subcodes.  If your error is not in this table, below are a table of generic codes and subcodes that you can search through for more information regarding your specific error.

### Most common Call Automation error codes
| Code | Subcode | Meaning | Advice |
| -- | -- | -- | -- |
| 404 | 8522 |  A generic error code that indicates that the resource is not found. Resources can include calls and participants. | The call may have already ended, or the participant has left the call. |
...
| 400 | 8500 | Invalid Media Mode | Check the status of your media operations to see if it is already active. |
...
| 409 | 8519 | Conflict | Check to make sure multiple actions are not being performed on the same resource in parallel. |
| 403 | 7507 | Call Source Identity Invalid | Application identity from authorization token did not match application identity in call source. |
...
| 400 | 8567 | ACS Resource Service Principal Not Enabled | Service principal for the Azure Communication Service Resource is not configured. |
...
#### 5xx codes
A 5xx code represents a server error.  - Raise a ticket with customer support if this is a recurring issue 
| Status Code | Meaning |
| -- | -- |
...
#### 6xx codes
A 6xx code represents a global error – Raise a ticket with support if this a recurring issue 
| Status Code | Meaning |
| -- | -- |
| 603 | Declined |
