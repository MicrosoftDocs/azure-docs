...
It is an unexpected error from SDK's perspective as this indicates a discrepancy between signaling and media transport.
## How to detect
### SDK
You'll get an error while invoking the `createView` API.
...
If other participants try to subscribe to a video from the user who drops the call unexpectedly, they'll get an error as no incoming video frames are received.
...
It is because there will be no incoming video frames while the sender is still in the lobby.
