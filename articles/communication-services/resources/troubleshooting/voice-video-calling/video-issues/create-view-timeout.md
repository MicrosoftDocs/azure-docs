...
SDK detects this and throws an createView timeout error.

This is an unexpected error from SDK's perspective as this indicates a discrepancy between signaling and media transport.
## How to detect
### SDK
...
Some mobile devices doesn't send any video frame when the browser is in the background or the user locks the screen.
The `createView` API detects no incoming video frames and considers this a subscription failure, therefore, it throws a createView timeout error.
...
This requires the application to transmit this information through some messaging mechanisms, such as Data Channel or websocket.
...
Sometimes, users do not end a call properly but instead directly terminate the browser process.
The server is unaware that the user has dropped the call until some timeout mechanism is triggered.
Currently the timeout value is 40 seconds, which means the participant will remain on roster list until the server removes it.
If other participants try to subscribe to a video from the user who drops the call unexpectedly, they will get an error as no incoming video frames are received.
...
In fact, we are not able to distinguish whether the lack of response is due to a network issue or because of the process termination on the sender's side.
...
This is an unexpected error from SDK's perspective currently as on the receiver's side, as we are unable to know the reason why the sender doesn't send video.
For example, if the sender experiences a temporarily network disconnection, other participants are unable to receive video frames from the sender.
No further detailed information is available because the currently the SDK doesn't support notifying receivers that the sender has network issues.
...
However, in a WebRTC call, the default STUN connectivity check mechanism typically triggers a disconnection event after about 10-15 seconds of no response from the other party.
This means if there is a networkReconnect UFD, the receiver side might not have received packets for already 15 seconds.

If the application detects network disconnection on the receiver's side. It should subscribe to the video until networkReconnect UFD is recovered.
...
In an ACS-Teams call, if other participants attempt to subscribe to a video while the sender is still in the lobby, the `createView` API won't succeed.
This is because there will be no incoming video frames while the sender is still in the lobby.
