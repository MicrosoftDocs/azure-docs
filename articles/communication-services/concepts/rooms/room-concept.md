...
- **Pre-built roles and permissions (coming soon).** Roles can drive customization of end-user experience and capabilities, such as restricting attendee ability to add other users, turning on video, or muting others.
...
Note while you can use either Group CallID or Rooms if you just need an ephemeral coordinate. We recommend using Rooms API if you plan to add more modalities in the future to a room. While there is only calling supported now, we are looking into adding more modalities moving forward. 
