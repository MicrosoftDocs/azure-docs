...
|     correlationIdentifier     |     The `correlationIdentifier` identifies   correlate events from all of the Participants and Endpoints that connect   during a single “call”. `correlationIdentifier`   is the unique ID for a Call. The correlationID is useful when   you open a case with Microsoft support to easily identify the call in   question.                                                                                                                                                                      |
...
|     callType                  |     Will   contain either “P2P” or “Group”. A “P2P” call is a direct 1:1 connection   between only 2, non-server endpoints. A “Group” call is a call that has more   than 2 endpoints or is created as “group” call prior to the connection.                                                                                                                                                                                                                                 |
|     teamsThreadId             |     This ID is only relevant   when the call is organized as a Microsoft Teams meeting , representing the   Microsoft Teams – Azure Communications Services interoperability use-case.   This ID is exposed in operational logs and developers can get the id from   Chat APIs.                                                                                                                                                                                              |
...
|     participantDuration       |     The duration of each Participant connection   in seconds, from `participantStartTime`   to timestamp when connection is ended.                                                                                                                                                                                                                                                                                                                                             |
|     participantEndReason      |     Contains Calling SDK error codes returned by   the SDK emitted when relevant for each `participantId`. See   Calling SDK error codes (also below)                                                                                                                                                                                                                                                                                                                          |
|     endpointId                |     Unique ID that represents each Endpoint connected   to the call, with Endpoint defined by `endpointType`. When the value   is null, it means that the connected entity is the ACS server (`endpointType`=”Server”). `endpointId` can sometimes persist   for the same user across multiple calls (`correlationIdentifier`) for native   clients. The number of `endpointId`s   will determine the number of Call Summary logs. A distinct Summary Log is   created per each `endpointId`.    |
...
|     sdkVersion                |     Version   string for the ACS Calling SDK version used by each relevant Endpoint.   (Example: “1.1.00.20212500”)                                                                                                                                                                                                                                                                                                                                                          |
...
|     endpointId            |     Unique   ID that represents each Endpoint connected to the call, with Endpoint defined   by endpointType.   When the value is null, it means that the connected entity is the ACS server.   endpointId   can persist for the same user across multiple calls (`correlationIdentifier`) for native clients   but will likely be unique for every call when the client is a web browser.                                                                                                                                                                                                                                                                                  |
|     endpointType          |     This   value describes the properties of each `endpointId`. Can contain   “Server”, “VOIP”, “PSTN”, “BOT”, or “Unknown”.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
|     mediaType             |     This string   value describes the type of media being transmitted between Endpoints within   each Stream. Possible values = “Audio”, “Video”, “VBSS” (Video Based Screen   Sharing), “AppSharing”.                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
...
|     transportType         |     String value describes the network transport   protocol per `participantId`.   Can contain "UDP”, “TCP”, or “Unrecognized”. Unrecognized indicates that   the system could not determine if the `transportType` was TCP or UDP                                                                                                                                                                                                                                                                                                                                                                                                                                   |
|     roundTripTimeAvg      |     This is the average time it takes to get an IP   packet from one Endpoint to within a participantDuration. This network   propagation delay is essentially tied to physical distance between the two points   and the speed of light, including additional overhead taken by the various   routers in between. The latency is measured as one-way or Round-trip Time   (RTT).  Value expressed in ms, and a RTT   >500 ms should be considered as negatively impacting the call quality.                                                                                                                                                                      |
...
|     packetLossRateAvg     |     This is the average percentage of packets that   are lost. Packet loss directly affects audio quality—from small, individual   lost packets that have almost no impact to back-to-back burst losses that   cause audio to cut out completely. The packets being dropped and not arriving   at their intended destination cause gaps in the media, resulting in missed   syllables and words, and choppy video and sharing. Packet loss rate >10%  (0.1) should be considered a rate that   is likely having a negative quality impact. This is measured per media stream   over the `participantDuration`   in a Group call or `callDuration`   in a P2P call.    |
|     packetLossRateMax     |     This value represents the maximum packet loss   rate (%)per media stream over the participantDuration in a Group call or callDuration in a P2P call.     Bursts in network conditions can cause the   issues in the audio/video traffic flow                                                                                                                                                                                                                                                                                                                                                                                                                  |
|                           |                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
