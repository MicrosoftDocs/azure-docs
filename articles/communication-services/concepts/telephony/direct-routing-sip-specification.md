...
An ACS identity might be used in multiple endpoints (applications) at the same time. For example, ACS web app, iPhone app, and Android app. Each endpoint might signal an HTTP rest as follows:
...
The Refer request that comes from the client terminates on the SIP proxy. The Refer request from the client is shown as “Call transfer to Dave” in the following diagram.  For more information, see section 7.1 of [RFC 3892](https://www.ietf.org/rfc/rfc3892.txt).
   ...
   [![Diagram showing call transfer with SIP Proxy acing as a Referee.](../media/direct-routing-sip-specification/direct-routing-protocol-3.png)](../media/direct-routing-sip-specification/direct-routing-protocol-3.png#lightbox)
...
- The call is transferred from one ACS endpoint to another ACS endpoint in the same resource via the SBC.

If the call is transferred from one ACS endpoint to another ACS endpoint via the SBC, the SBC is expected to issue a new Invite (start a new dialog) for the transfer target using the information received in the Refer message.
...
The size of the Refer Header can be up to 400 symbols in this case. The SBC must support handling Refer messages with size up to 400 symbols.
   ...
   [![Diagram showing call transfer with SIP Proxy acing as a Transferor.](../media/direct-routing-sip-specification/direct-routing-protocol-4.png)](../media/direct-routing-sip-specification/direct-routing-protocol-4.png#lightbox)

## Call forwarding

An ACS Call Automation SDK can redirect incoming calls to another number or ACS/Teams endpoint, ring other user or users in parallel (simultaneous ring), or ring a group of users or numbers. Things to consider:
...
The History-Info header is used for retargeting SIP requests and “provide(s) a standard mechanism for capturing the request history information to enable a wide variety of services for networks and end-users.” For more information, see [RFC 4244 – Section 1.1](http://www.ietf.org/rfc/rfc4244.txt). For ACS, this header is used in simultaneous ring and call forwarding scenarios.  
