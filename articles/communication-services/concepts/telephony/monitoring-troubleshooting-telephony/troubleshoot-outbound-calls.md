...
You might experience various issues when you use direct routing to make outbound calls from an app built on Azure Communication Services (ACS) Software Development Kit (SDK) to a Session Border Controller (SBC). These issues include the following:
...
- The phone number that's used as an `alternateCallerId` property of a `startCall` method in ACS [Client Calling SDK](../../../quickstarts/telephony/pstn-call.md).
  If an `alternateCallerId` was not provided, it's replaced with "anonymous".
...
- The phone number of the original caller if the call was redirected by an ACS [Call Automation SDK](../../../how-tos/call-automation/actions-for-call-control.md#redirect-a-call).
...
If the connection between the Microsoft and the SBC is working correctly, but some users or applications cannot make calls, the issue might be caused by incorrect scope of an ACS access token
...
None of the patterns in the Voice Routes match the number that was dialed by the user.

### Resolution 2

Make sure that the following conditions are true:

- There is a pattern in the Voice Route that matches the dialed number.
...
The SBC is not responding to SIP OPTIONS messages because some device on the network, such as a firewall, is blocking the messages.
