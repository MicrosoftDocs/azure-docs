...
You might get this type of notification if the ratio between “good” traces (traces logged with a level of *Info* or *Verbose*) and “bad” traces (i.e. traces logged with a level of *Warning*, *Error* or *Fatal*) is degrading in a specific day, compared to a baseline calculated over the previous 7 days.

### Does my app definitely have a problem?
No, a notification doesn't mean that your app definitely has a problem. Although a degradation in the ratio between “good” and “bad” traces might indicate an application issue, this change in ratio might be benign (for example, due to a new flow in the application emitting more “bad” traces than existing flows).
