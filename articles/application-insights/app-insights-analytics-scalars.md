...
| [samplingPercentage](#samplingpercentage) | [split](#split) | [sqrt](#sqrt) 
...
This field can be present in any data point (except in the metrics and availabilityResults streams), and reflects the current rate at which [sampling](app-insights-sampling.md) is operating, either in the SDK or at the portal. While sampling is operating, each item that you see in the data stream takes the place of several data points that were generated by the original data. If, for example, the sampling rate is 33%, then only one in three wil
