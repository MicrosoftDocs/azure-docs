...
The Application Insights SDKs for .NET, .NET Core, Java, Node.js, and JavaScript all support distributed tracing natively. Instructions for installing and configuring each Application Insights SDK are available:
...
To add distributed tracing capabilities to an application with OpenCensus, first [install and configure the Application Insights Local Forwarder](). From there, configure OpenCensus to route distributed trace data through the Local Forwarder. Both [Python]() and [Go]() are supported.

The OpenCensus website maintains API reference documentation for [Python]() and [Go](), as well as various different guides for using OpenCensus. 
