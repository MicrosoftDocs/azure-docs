...
You can set *ingestion sampling* on the Quotas + pricing blade. This form of sampling operates at the point where telemetry from your app enters the Application Insights service. It doesn't affect the volume of telemetry sent from your app, but it reduces the volume retained by the service.
...
> [AZURE.NOTE] If you are using the Application Insights SDK for ASP.NET, version 2.0.0-beta3 or later, you shouldn't need to use ingestion sampling. Instead, the SDK performs [adaptive sampling](app-insights-sampling.md) to reduce the volume of telemetry that it sends.
