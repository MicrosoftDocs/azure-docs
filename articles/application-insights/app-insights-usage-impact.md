...
Impact analyzes how load times and other properties influence conversion rates for parts of your app. To put it more precisely, it discovers how **any dimension** of a **page view**, **custom event**, or **request** affects how users convert to  a different **page view** or **custom event**. 

![Impact tool](./media/app-insights-usage-impact/0001-impact.png)

## Still not quite sure what Impact does?

Think of it as the ultimate tool for settling arguements with someone on your team about how some slowness in some aspect of your site is affecting whether users stick around. Oftentimes users may tolerate a certain amount of slowness, but Impact gives you insight into how best to balance optimization and performance to maximize user conversion.
...
2. Leave the **analyze how this** dropdown on the default selection of **Duration**
...
Pick a page view in the first dropdown 
Pick “Country or region” in the second dropdown 
Pick a custom event the corresponds to a UI element on the page view you selected.

## How does the Impact tool calculate these conversion rates?

Let A = the main page view/custom event/request you select (“For the…”). Let B = the secondary page view/custom event you select (“impacts the usage of…”).  
Impact looks at a sample of all the sessions from all your users in the selected time range. For each session, it looks for each occurrence of A. From one A to the next is considered a “sub-session.” If B occurs in a sub-session, eg. between one occurrence of A and the next, then that sub-session is counted as a conversion. So conversion rates are the fraction of A-to-A or A-to-end-of-session sub-sessions that included at least one B. 
That’s super dense…let me know if you want help making it more consumable…or drawing a diagram… 
