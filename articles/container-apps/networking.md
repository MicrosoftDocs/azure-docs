...
|-----------|-------------|
| Workload profiles architecture (preview) | Supports user defined routes (UDR) and egress through NAT Gateway. The minimum required subnet size is /27. As workload profiles are currently in preview, the number of regions is limited. To learn more, visit the [workload profiles overview](./workload-profiles-overview.md#supported-regions).|
