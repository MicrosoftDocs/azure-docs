...
- If you are using the Consumption only plan, or only the Consumption workload profile in the Consumption + Dedicated plan then Billing is relevant only to individual container apps and their resource usage. There is no cost associated with the Container Apps environment.
- If you are using any Dedicated workload profiles in the Consumption + Dedicated plan there is fixed cost for the Dedicated plan management. This cost is for the entire environment regardless of how many Dedicated workload profiles you are using.
