...
Azure Container Apps offers a selection of fully managed Dapr APIs, components, and features, catered specifically to microservice scenarios. Simply [enable and configure Dapr][dapr-enable] as usual in your container app environment.
...
:::image type="content" source="media/dapr-overview/dapr-in-aca.png" alt-text="Diagram demonstrating Dapr pub sub and how it works in Container Apps." lightbox="media/dapr-overview/dapr-in-aca.png":::
...
Instead of following a fixed release schedule, Dapr in Azure Container Apps releases new features and functionality based on the prioritization and stability of the Dapr binaries. To learn more about Dapr releases in Azure Container Apps, see [How often are Dapr versions released](./faq.yml#how-often-are-dapr-versions-released-for-azure-container-apps-).

Dapr versions in Azure Container Apps include:
- A semantic versioning prefix, such as `1.13.6`, which denotes compatibility with the corresponding OSS Dapr runtime version APIs and related tools like SDKs and CLI.
...
> [!NOTE]
> The `<number>` in the suffix might not always increment sequentially. For example, version `1.13.6-msft.3` might skip `1.13.6-msft.2`.
...
> [!NOTE] 
> Currently, the Dapr server extensions, actor, and workflow SDK packages aren't compatible with Azure Container Apps. To learn more about all of the Dapr SDK packages, see [SDKs](https://docs.dapr.io/developing-applications/sdks/).

[!INCLUDE [component-support](~/reusable-content/ce-skilling/azure/includes/dapr-in-azure/dapr-support-policy.md)]

To learn more about using the supported Dapr components, see [Dapr components in Azure Container Apps][dapr-components].
...
- **APIs and components support**: Only the Dapr APIs and components [listed as GA, Tier 1, or Tier 2](#supported-dapr-apis-components-and-tooling) are supported in Azure Container Apps.
...
- Deploy a Dapr application by using:
