...
By default, each container app issues its own unique cookie or token for authentication. When encrypting, decrypting, and signing authentication cookies and tokens, you can provide your own signing and encryption keys. A client can use the same signing and encryption keys with multiple apps to seamlessly share authentication information across different Container Apps.
