...
If your container app has HTTP ingress enabled and you don't set a target port, Azure Container Apps automatically detects the target port by scanning all listening ports on your container. If there's only one port detected, that port is set as the target port for your container app. If there's more than one port detected, the container app doesn't automatically set the target port, and you need to set the target port manually. 
 
- Automatic port detection only works for HTTP traffic, not TCP traffic.
- If you have HTTP health probes listening on ports 80 or 443, this configuration can interfere with automatic port detection. The default ingress configuration uses TCP health probes. For more information, see [health probes](health-probes.md).

## Enable ingress

You can configure ingress for your container app by using the Azure CLI, an ARM template, or the Azure portal.

::: zone pivot="azure-cli"

The `az containerapp ingress enable` command enables ingress for your container app. You must specify the target port, and you can optionally set the exposed port if your transport type is `tcp`.
...
- Set the `external` property to *true* for external ingress or *false* for internal ingress.
...
You can disable ingress for your container app by using the portal.
...
You can expose extra TCP ports from your application. For more information, see the [ingress concept article](ingress-overview.md#additional-tcp-ports).

> [!NOTE]
> To use this feature, you need the container apps CLI extension. Run `az extension add -n containerapp` to install the latest version of the container apps CLI extension.


::: zone pivot="azure-cli"

You can add extra TCP ports through the CLI by referencing a YAML file with your TCP port configurations.
...
The following example shows a YAML file you can reference in the preceding CLI command. The configuration for the extra TCP ports is under `additionalPortMappings`.
...
:::image type="content" source="media/ingress/additional-tcp-ingress-ports-portal.png" lightbox="media/ingress/additional-tcp-ingress-ports-portal.png" alt-text="Screenshot of adding extra TCP ports for Container Apps ingress through the portal.":::

1. Expand the **Additional TCP ports** section within the Ingress blade.
1. Add extra TCP ports that your application accepts traffic on in the _Target port_ field. If you leave _Exposed port_ empty, it uses the same value as _Target port_.
1. Change the _Ingress traffic_ field as needed. This field configures where ingress traffic is limited to for each port.
...
The following ARM template provides an example of how you can add extra ports to your container apps. Add each extra port under `additionalPortMappings` within the `ingress` section for `configuration` within `properties` for the container app. The following example shows how to add extra ports:
