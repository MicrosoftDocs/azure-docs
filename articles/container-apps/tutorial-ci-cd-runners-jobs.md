...
GitHub Actions and Azure Pipelines allow you to run CI/CD workflows with self-hosted runners and agents. Self-hosted runners are useful when you need to run workflows that require access to local resources or tools that are not available in a cloud-hosted runner. For example, you might need to run a workflow that requires access to resources inside the virtual network of your Container Apps environment that is not accessible from a cloud-hosted runner.
