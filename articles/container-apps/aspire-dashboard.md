...
description: Use real-time log data with the Aspire dashboard in Azure Container Apps.
...
ms.date: 09/04/2026
ms.author: cshoe
zone_pivot_groups: azure-aspire-cli-portal
---

# Read real-time app data with Aspire dashboard in Azure Container Apps

The [Aspire dashboard](https://aspire.dev/dashboard/overview/) displays live data about how applications and other resources are running within an environment.
...
- OpenTelemetry (OTel), an open-source observability framework for collecting **traces**, **metrics**, and **logs** from your applications.
...
- The Aspire resource service provides the application resource model, status, endpoints, and console logs.

For more information, see [Telemetry in Aspire](https://aspire.dev/fundamentals/telemetry/) and [Explore dashboard features](https://aspire.dev/dashboard/explore/).
...
> [!TIP]
> For Aspire-managed environments, configure and deploy the dashboard with the [Aspire CLI](aspire-dashboard.md?pivots=aspirecli). If you have problems using the portal, [create an issue in the Container Apps GitHub repo](https://github.com/microsoft/azure-container-apps/issues).
...
::: zone pivot="aspirecli"

The Azure Container Apps environment integration includes the Aspire dashboard by default. To create and deploy an Aspire app with the dashboard:

1. Create an Aspire starter app and add the Azure Container Apps integration:
...
1. In *AppHost.cs*, add an Azure Container Apps environment:
...
The `aspire deploy` command returns the dashboard URL that you can open in a browser. To omit the dashboard, chain `.WithDashboard(false)` on the environment resource.

For more information, see [Deploy to Azure Container Apps](https://aspire.dev/deployment/azure/container-apps/) and [`WithDashboard`](https://aspire.dev/reference/api/csharp/aspire.hosting.azure.appcontainers/azurecontainerappextensions/methods/#withdashboard).
