...
|--|--|--|
| **Consumption** | Serverless environment where apps can scale to zero. You only pay for compute apps as they're running. | No |
| **Consumption with Dedicated workload profiles** | Serverless environment where apps can scale to zero. You only pay for compute apps as they're running. In addition, you can run apps with customized hardware or increased cost control requirements on Dedicated workload profiles. | Yes |
...
The Consumption + Dedicated plan features fully managed, isolated environments with access to customized infrastructure using [workflow profiles](workload-profiles-overview.md). Each workload profile can scale in and out as demand requires. When demand falls, profiles created on-demand are shut down, while the [minimum threshold of resources](billing.md#dedicated-plan) stays running.
