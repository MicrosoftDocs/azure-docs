...
> [!NOTE]
> External TCP ingress is only supported for Container Apps environments that use a [custom VNET](vnet-custom.md). TCP ingress is not supported for apps that are accepting inbound traffic through a [private endpoint](n/networking.md#private-endpoint).
