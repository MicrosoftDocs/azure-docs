...
Azure Media Services team has released Azure Active Directory (Azure AD) authentication support for Azure Media Services access and has also announced the plan to deprecate Azure Access Control service authentication for Media Services access. Because every Azure subscription, and so, every Media Services account, is attached to an Azure AD tenant, this authentication support brings many security benefits. For details about this change and migration (if you use the Media Services .NET SDK for your app), see the following blog posts and articles:
...
- [Accessing Azure Media Services API by using Azure AD authentication](media-services-use-aad-auth-to-access-ams-api.md)
...
*	They use a programming language that is different from Microsoft .NET or C#, or the runtime environment is not Windows.
...
Some customers have developed applications by using REST API for both Access Control authentication and Azure Media Services access. For these customers, we need a way to use only REST API for Azure AD authentication and subsequent Azure Media Services access, without relying on any of the Azure AD libraries or on the Media Services .NET SDK. This document presents a solution and sample code for this scenario. Because the code is all REST API calls, with no dependency on any Azure AD or Azure Media Services library, the code can easily be translated to any other programming language.
...
We use the following authentication and authorization design:

*  Authorization protocol: OAuth 2.0. OAuth 2.0 is a web security standard that covers both authentication and authorization. It is supported by Google, Microsoft, Facebook, and PayPal. It was ratified in October 2012. Microsoft is firmly behind OAuth 2.0 and OpenID Connect. Both of these standards are supported in a number of services and client libraries, including Azure Active Directory, Open Web Interface for .NET (OWIN) Katana, and Azure AD libraries.
...
|Secure Token Service (STS)/OAuth server | Azure AD as STS | After authentication by the IDP (or OAuth Server in OAuth 2.0 terms), an access token or JSON Web Token (JWT) is issued by Azure AD as STS/OAuth Server for access to the mid-tier resource: in our case, Media Services REST API endpoint. |
...
You can find the sample code in [this GitHub repo](https://github.com/willzhan/WAMSRESTSoln).

The sample code has two parts:

*	A DLL library project that has all the REST API code for Azure AD authentication and authorization. It also has a method for making REST API calls to the Azure Media Services REST API endpoint, with the access token.
...
*	Azure AD authentications via Client Credentials grant by using only the REST API.
