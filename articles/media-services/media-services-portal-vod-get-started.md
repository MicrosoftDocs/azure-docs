...
This tutorial walks you through the steps of implementing a basic Video-on-Demand (VoD) content delivery with Azure Media Services (AMS) application using the Azure Portal.
...
2. Click **+New** > **Media Service + CDN** > **Media Services**.
...
3. In **CREATE MEDIA SERVICES ACCOUNT** enter required values.
	...
	3. In **Location**,  select the geographic region that will be used to store the metadata records for your Media Services account. Only the available Media Services regions appear in the drop-down list box. 
...
- Get at least one streaming unit for the *streaming endpoint* from which you plan to delivery your content. The steps below show how to change the number of streaming units.
...
2. Click on the default streaming endpoint. 
	...
	The **DEFAULT STREAMING ENDPOINT DETAILS** winodow appears.
...
4. Browse to a file on your computer.  
...
- Encode your source file into a set of adaptive bitrate MP4 files or adaptive bitrate Smooth Streaming files (the encoding steps are demonstrated later in this section).
...
4.  In the **Encode an asset** window, select the "Media Encoder Standard" processor and a preset. For more information about presets, see [this](https://msdn.microsoft.com/library/azure/mt269960.aspx) article.
...
- Progressive (SAS) locators, used for progressive files download.
