...
9. Open the App.config file and add an *appSettings* section to the file.     
	...
	>[!Important]
	>Samples in the Azure Media Services documentation set use "user authentication" to connect to the AMS API. This authentication method will work well for "local/desktop playground" type of apps. This authentication method is not suitable for server, web services, APIs type of development.  For more information, see [Access the AMS API with Azure AD authentication](media-services-use-aad-auth-to-access-ams-api.md).
