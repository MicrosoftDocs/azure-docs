...
- minimumMarkerGap {OPTIONAL, string}: The minimum size of a subclip (in seconds). Note: the value should be greater or equal to 6, which is also the default.
...
- keymap {OPTIONAL, json object} Allows customizing the keyboard shortcuts of the widget. For more information, see Customizable keymaps.
...
- resetOnJobDone (OPTIONAL, bool): resetOnJobDone allows to reset the subclipper to an initial state when a job is submitted successfully.
- autoplayVideo (OPTIONAL, bool): autoplayVideo allows to autoplay the video on load. The default value is true.
