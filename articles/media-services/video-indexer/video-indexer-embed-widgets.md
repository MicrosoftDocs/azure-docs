...
|locale|Short language code|Controls the insights language. Default is `en`. For example: `language=de`.|
|tab|Default selected tab|Controls the insights tab that is rendered by default. `tab=timeline` renders the insights with the timeline tab selected.|
