...
>|Suggested solution| APPID is a GUID token in RTMP ingest URL. Please make sure it matches with URL from one of the live event's input endpoints' URLs. |
>|**MPE_INGEST_ENCODER_CONNECTION_DENIED** ||
>| Description |Encoder IP is not present in the configured IP allow list |
>| Suggested solution| Make sure the encoder's IP is in the IP allow list specified in the live event's input access control property. Use an online tool such as *whoismyip* or *CIDR calculator* to set the proper value.  Make sure the encoder can reach the server before the actual live event. |
>|**MPE_INGEST_RTMP_SETDATAFRAME_NOT_RECEIVED** ||
>| Description|The RTMP encoder did not send the `setDataFrame` command. |
>| Suggested solution|Most commercial encoders send stream metadata. For an encoder that pushes a single bitrate ingest, this may not be issue. The live event is able to calculate incoming bitrate when the stream metadata is missing.  For multi-bitrate ingest for a pass-through live events or double push scenario, you can try to append the query string with 'videodatarate' and 'audiodatarate' in the ingest URL. The approximate value may work. The unit is in Kbit. For example,  `rtmp://hostname:1935/live/GUID_APPID/streamname?videodatarate=5000&audiodatarate=192` |
>|**MPE_INGEST_CODEC_NOT_SUPPORTED** ||
>| Description|The codec specified isn't supported.|
>| Suggested solution| The live event received unsupported codec. Please check [live event supported codecs](live-event-types-comparison.md) for the list of supported codecs. |
>|**MPE_INGEST_DESCRIPTION_INFO_NOT_RECEIVED** ||
>| Description |The media description information was not received before the actual media data was delivered. |
>| Suggested solution|The live event did not receive the stream description (header or FLV tag) from the encoder. This is a protocol violation. Please contact the encoder vendor. |
...
>| Suggested solution|Please check the contribution encoder's settings and lower the frame rate to under 36 fps. |
