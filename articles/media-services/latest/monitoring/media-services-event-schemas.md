...
| `latencyResultCode` | string | The result code for the archive latency. `S_OK` indicates that the live event ingest was receive without any problems. If latencyResultCode has an error condition, it can indicate that there was an issue in calculating the latency. If there is an error case, the channelLatencyMs will not display a millisecond value.  'MPE_KEY_FRAME_INTERVAL_TOO_LARGE' error code indicates that the ingested video source has a very large GOP (key frame distance) size that is not compatible with Media Services live streaming and has an impact on the channel latency.  'MPE_INGEST_DISCONTINUITY' error code indicates that discontinuities were detected on the source stream which can add long-latencies to the channel. |
