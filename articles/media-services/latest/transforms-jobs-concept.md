...
While the [Transform](https://docs.microsoft.com/rest/api/media/transforms) entity supports the Update operation, you should take care to make sure all in-progress Jobs complete before you make a modification. Typically, you would update an existing Transform to change the description, or modify the priorities of the underlying TransformOutputs. If you wanted to rewrite the recipe, then you would create a new Transform.
...
While the [Job](https://docs.microsoft.com/rest/api/media/jobs) entity supports the Update operation, the only properties that can be modified after the Job is submitted are the description, and the priority. Further, a change to the priority is effective only if the Job is still in a queued state. If the job has begun processing, or has finished, changing the priority has no effect.
