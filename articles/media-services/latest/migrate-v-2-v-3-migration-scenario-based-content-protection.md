...
As you migrate to the v3 API, you will find that you need to access some properties or content keys from your v2 Assets. One key difference is that the v2 API would use the **AssetId** as the primary identification key and the new v3 API uses the ARM name of the entity as the primary identifier.  The v2 **Asset.Name** property is not a unique identifier, so when migrating to v3 you will find that your v2 Asset Ids now appear in the **Asset.Description** field.
...
To find the **ContentKeys** used in your **StreamingLocators** for content protection, you can call the [StreamingLocator.ListContentKeysAsync](https://docs.microsoft.com/en-us/dotnet/api/microsoft.azure.management.media.streaminglocatorsoperationsextensions.listcontentkeysasync?view=azure-dotnet) method.  

Any **Assets** that were created and published using the v2 API will have both a [Content Key Policy](https://docs.microsoft.com/azure/media-services/latest/content-key-policy-concept) and a Content Key defined on them in the v3 API, instead of using a default content key policy on the [Streaming Policy](https://docs.microsoft.com/en-us/azure/media-services/latest/streaming-policy-concept).

For more details on content protection in the v3 API see the article [Protect your content with Media Services dynamic encryption.](https://docs.microsoft.com/en-us/azure/media-services/latest/content-protection-overview)
...
Note that it is possible to delete the v2 locator using hte v3 API, but this will not remove the content key or the content key policy used if they were created in the v2 API.  
