...
|---|---|---|
|**AudioAnalyzerPreset**|Analyzing audio|The preset applies a pre-defined set of AI-based analysis operations, including speech transcription. Currently, the preset supports processing of content with a single audio track.<br/>You can specify the language for the audio payload in the input using the BCP-47 format of 'language tag-region' (for example, 'en-US'). 'en-US', 'en-GB', 'es-ES', 'es-MX','fr-FR', 'it-IT', 'ja-JP', 'pt-BR', 'zh-CN', 'de-DE', 'ar-EG', 'ru-RU', 'hi-IN'. If language is not specified or set to Null, automatic language detection will be employed. The automatic language detection feature currently supports English, Chinese, French, German, Italian, Japanese, Spanish, Russian, and Portuguese. The automatic language detection feature works best with audio recordings with clearly discernable speech. If automatic language detection fails to find the language, the transcription will fallback to English.|
