...
You can add a  narration track to your video to help visually impaired clients to follow the video recording by listening to the narration. In Media Services v3, you signal audio description tracks by annotating the audio track in the manifest file.
...
When your encoding job is done, the output asset will contain the files generated by the encoding job. Among the files, you will find the .ism file. Edit the .ism file by adding the information about the uploaded audio-only MP4 file (AAC codec) containing descriptive audio. 
