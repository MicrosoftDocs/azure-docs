---
title: Detect motion & record video on edge devices using SDK- Azure
...
# Quickstart: Detect motion and record video on edge devices using SDK
...
This article is based on a [C# SDK sample code](https://github.com/Azure-Samples/live-video-analytics-iot-edge-csharp-sdk). It builds on the [Detect motion and emit events](detect-motion-emit-events-quickstart.md) quickstart. 
...
1. Start a debugging session by selecting the F5 key. The **TERMINAL** window will display some messages. Then, you'll be prompted to pick a topology: choose `Event Based Video Recording to edge device`
...
1. The output in the **TERMINAL** window pauses at `Press Enter to continue`. Don't select Enter yet. Scroll up to see the JSON response payloads for the direct methods the SDK wrapped and invoked.
