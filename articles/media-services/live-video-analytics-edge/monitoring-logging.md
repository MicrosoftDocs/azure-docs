...
The analytics events are generated by nodes such as the motion detection processor, or the HTTP extension processor, and the IoT Hub Sink <!--<TODO Link> -->is used to send them to the IoT Edge Hub. 
...
Represents the media account associated with the graph (same topic is applied to Edge, per the account the Live Video Analytics on IoT Edge module <!--repled "AMS Edge module"--> is linked to).
