...
How-to Guide: Playback of recordings 
PREREAD
•	Concept link on video playback
•	Continuous Video Recording
•	Event based Video Recording
BACKGROUND
You can use Live Video Analytics on IoT Edge for continuous video recording (CVR : TODO, link to that scenario page), whereby you can record video into the cloud for weeks or months. You can also limit your recording to clips that are of interest, via event-based recording (EVR : TODO Link to that page). 
Your Media Service   account is linked to an Azure Storage account, and when you record video to the cloud, the content is written to a Media Service Asset. Media Services allows you to stream that content (TODO: add link to the entry in the Terminology doc), either after the recording is complete, or while the recording is ongoing. Media Services provides you with the necessary capabilities (TODO: here, link to the concept page on playback) to deliver streams via HLS or MPEG-DASH protocols to playback devices (clients). You would use Media Service APIs to generate the requisite streaming URLs – used by clients to play back the video & audio. To construct the streaming URL for an Asset, see this article. Once the streaming URL has been created for an Asset, you can and should store the association of the URL with the video source (i.e. camera) in your content management system.
...
This concept also applies to HLS and MPEG-DASH streams from Assets which contain video recorded using Live Video Analytics on IoT Edge.
...
1.	A way to determine what dates/hours of video you have in an  recording
2.	A way to select a portion (eg. 9AM to 11AM on New Years Day) of that recording to playback
...
o	If the startTime is omitted, it is assumed to be the first year in the recording
o	If the endTime is omitted, it is assumed to be the last year in the recording
o	For example, if your recording started in 2011, and has continued until 2020, then:
...
If the Asset does not contain an recording from a Media Graph, then an HTTP 400 response will be returned with an error message explaining that this feature is only available for content recorded via a Media Graph.
...
RESPONSE EXAMPLES FOR AVAILABLEMEDIA
Example 1: Live recording with no gaps
Here is a response showing all of the available media for December 21, 2019, where the recording was 100% complete. Note that response has only one start/end pair.
...
Suppose you recorded video only when the school was in session, and recording was stopped from June 17th through September 6th. A query for available months would look like:
...
• starttime is an ISO 8601 DateTime stamp which describes the desired start time of the video timeline in the returned manifest
• endtime is an ISO 8601 DateTime stamp which describes the desired end time of the video timeline returned in the manifest
...
With such an recording:
A.	If you request a manifest where both startTime/endTime range filters were inside the ‘available’ parts of the timeline, namely from midnight to 4AM, or noon to midnight, then the service would return a manifest that covers the time from startTime to endTime, such as:
...
B.	If you request a manifest where the startTime and endTime were inside the ‘hole’ in the middle – say from 8AM to 10AM UTC, then the service would behave the same way as if an Asset Filter were to result in an empty result.
...
C.	If you request a manifest where only one of the startTime or endTime is inside the ‘hole’, then the returned manifest would only include a portion of that timespan. It would snap the startTime or endTime valus to the nearest valid boundary. For example, if you asked for a 3hr stream from 10AM to 1PM, the response would contain 1hr worth of media for 12 noon to 1PM
