...
3. Click **Create Channel**.  
	![Elemental](./media/media-services-elemental-live-encoder/media-services-elemental2.png)
...
1. Navigate to the **Elemental Live** web interface, and set up the encoder for **RTMP** streaming. 

2. Once a new event is created, scroll down to the output groups and add the **Adobe RTMP** output. 
...
6. Get the channel's input URL and assign it to the Element's **RTMP Endpoint**.
...
- **Problem**: When configuring the encoder to stream using the RTP (MPEG-TS) protocol, there is no place to enter a host name. 
	...
	- **Troubleshooting steps**: To find the IP address, open a command prompt on any computer. To do this in Windows, open the Run launcher (WIN + R) and type “cmd” to open.  
		...
		![Elemental](./media/media-services-elemental-live-encoder/media-services-elemental10.png)
