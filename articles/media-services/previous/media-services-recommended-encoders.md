...
7. Create, start a Streaming endpoint, use a player such as [Azure Media Player](http://aka.ms/azuremediaplayer) to watch the archived asset to ensure that playback has no visible glitches for all quality levels (Or alternatively watch and validate via the Preview URL during the live session before step 6)
...
6. Use a player such as [Azure Media Player](http://aka.ms/azuremediaplayer) to watch the live streaming from time to time (or archived asset) to ensure that playback has no visible glitches
