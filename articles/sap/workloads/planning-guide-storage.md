...
Several more redundancy methods exist. The article [Azure Storage replication](../../storage/common/storage-redundancy.md?toc=%2fazure%2fstorage%2fqueues%2ftoc.json) describes these methods and the different storage types they apply to. 
...
The sections describing the different Azure storage types give you more background about the restrictions and possibilities when using the SAP supported storage. 

### Storage choices when using DBMS replication

The reference architectures use DBMS (Database Management System) functionality like SQL Server Always On, HANA System Replication, Db2 HADR, or Oracle Data Guard. If you use these technologies between two or multiple Azure virtual machines, you need to choose the same storage types for each of the VMs. The storage configuration between active node and replica node in DBMS HA configuration needs to be the same.
...
- A default set of IOPS and storage throughput per disk
- The option to add more IOPS and throughput to each disk and pay separately for these extra provisioned resources
...
In contrast to Azure premium storage, Azure Premium SSD v2 fulfills SAP HANA storage latency KPIs. As a result, you **don't need to use Azure Write Accelerator caching** as described in the article [Enable Write Accelerator](/azure/virtual-machines/how-to-enable-write-accelerator). 
...
- The provisioned storage bandwidth. Different maximum bandwidth applies dependent on the capacity of the disk. For more information, see [Ultra Disk](/azure/virtual-machines/disks-types#ultra-disks).

You determine the cost of a single disk by the three dimensions you define for the particular disks.
...
- SAP on MAXDB in SUSE or Red Hat Linux guest OS.
...
- With large NFS volumes in the double-digit TiB capacity space, the throughput accessing such a volume out of a single VM plateaus based on limits of Linux for a single session interacting with the shared volume. 

As you upsize Azure VMs in the lifecycle of an SAP system, evaluate the IOPS and storage throughput limits of the new and larger VM type. In some cases, it might also make sense to adjust the storage configuration to the new capabilities of the Azure VM.
