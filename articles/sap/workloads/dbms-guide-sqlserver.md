...
- You can't define how many threads are used to apply data encryption to the database. The number of threads mainly depends on the number of disk volumes that the SQL Server data and log files are distributed over. The more distinct volumes (drive letters), the more threads run in parallel to perform the encryption. Such a configuration contradicts the earlier disk configuration suggestion to build one, or a small number of, storage spaces for the SQL Server database files in Azure VMs. A configuration with a few volumes leads to a few threads executing the encryption. A single encrypting thread reads 64 KB extents, encrypts them, and then writes a record into the transaction log file to note that the extent was encrypted. As a result, the load on the transaction log is moderate.
