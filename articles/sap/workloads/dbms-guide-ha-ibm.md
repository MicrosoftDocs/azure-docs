---
title: Deploy and configure IBM Db2 HADR on Azure VMs
description: Learn how to establish high availability of IBM Db2 LUW on Azure VMs for deployment.
...
![A diagram of a basic IBM Db2 high availability environment.](./media/dbms-guide-ha-ibm/ha-db2-hadr-lb.png)
...
![A diagram of an IBM DB2 full environment with high availability.](.//media/dbms-guide-ha-ibm/end-2-end-ha.png)
...
- Deploy the VMs.
- Update the SUSE Linux OS and configure file systems.
- Install and configure Pacemaker.
- Install [highly available Network File Share (NFS)][nfs-ha].
- Install [ASCS/ERS on a separate cluster][ascs-ha].
- Install IBM Db2 database with Distributed/High Availability option (Software Provisioning Manager (SWPM)).
...
Complete the planning process before you execute the deployment. Planning builds the foundation for deploying a configuration of Db2 with HADR in Azure. Key elements that need to be part of planning for IBM Db2 LUW (database part of SAP environment) are listed in the following table:
...
| Azure fencing | Azure fencing or Storage-Based Death (SBD) (highly recommended). Method to avoid split brain situations. |
...
The resource agent for IBM Db2 LUW is included in SUSE Linux Enterprise Server for SAP Applications. For the setup described in this document, you must use SUSE Linux Server for SAP Applications. Azure Marketplace contains an image for SUSE Enterprise Server for SAP Applications 12 that you can use to deploy new Azure VMs. Review the support and service models SUSE offers in Azure Marketplace before choosing a VM image.
...
Make sure that the selected OS for IBM/SAP for IBM Db2 LUW is supported. The list of supported OS versions for Azure VMs and Db2 releases is available in SAP Note [1928533]. The list of OS releases by individual Db2 release is available in the SAP Product Availability Matrix. We highly recommend a minimum of SLES 12 SP4 because of Azure-related performance improvements in this or later SUSE Linux versions.
...
1. Create Virtual Machine 1.
   1. Use SLES for SAP image in Azure Marketplace.
...
Links to this documentation are provided in the introductory section of this article. Check the SAP installation manuals about installing NetWeaver-based applications on IBM Db2 LUW. You can find the guides on the SAP Help portal by using the [SAP Installation Guide Finder][sap-instfind].
...
>
> ![A screenshot of the SAP wizard "Define Parameters" screen.](./media/high-availability-guide-rhel-ibm-db2-luw/hadr-swpm-db2-port.png)
...
1. When you reach the exit step to restore the database for homogeneous system copy, exit the installer. Restore the database from a backup of the primary host. All subsequent installation phases were already executed on the primary database server.
...
During VM configuration, you can create or select an exiting load balancer in the networking section. Follow these steps to set up standard load balancer for high availability setup of DB2 database.
...
>
> The change requires brief downtime. For existing Pacemaker clusters, if the configuration was already changed to use `socat` as described in [Azure Load-Balancer Detection Hardening](https://www.suse.com/support/kb/doc/?id=7024128), there's no requirement to switch immediately to azure-lb resource agent.
...
![A screenshot of a HADR VM overview screen pre-migration.](./media/dbms-guide-ha-ibm/hadr-sap-mgr-org.png)
...
![A screenshot of a HADR VM overview screen post-migration.](./media/dbms-guide-ha-ibm/hadr-sap-mgr-post.png)
...
You can test a manual takeover by stopping the Pacemaker service on the *azibmdb01* node:

```bash
service pacemaker stop
```

Here's a sample status for *azibmdb02*:
...
After the failover, you can start the service again on *azibmdb01* with the following command:
...
The Db2 instance is going to fail, and Pacemaker reports the following status:
...
Pacemaker either restarts the Db2 primary database instance on the same node or fails over to the node hosting the secondary instance, and an error is reported.
...
The node falls into a failed stated and the error is reported.
...
The Db2 instance gets restarted in the secondary role it was assigned before.
...
As user db2\<sid> executes the command `db2stop force`:
...
### Simulated VM primary node failure with automatic reboot in HADR
...
If there's a "halting" event on the node, the failed node has to be restarted via Azure Management tools (in the Azure portal, PowerShell, or the Azure CLI). After the failed node is back online, it starts the Db2 instance into the secondary role.
