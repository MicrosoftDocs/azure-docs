...
For DBMS encryption, each database supported for SAP NetWeaver or S/4HANA deployment supports native encryption. Transparent database encryption is entirely independent of any infrastructure encryption in place in Azure. Both database encryption and [storage side encryption](/azure/virtual-machines/disk-encryption) (SSE) can be used at the same time. Of utmost importance when using encryption, is the location, storage and safekeeping of encryption keys. Any loss of encryption keys lead to data loss due to an impossible to start or recover a database.
