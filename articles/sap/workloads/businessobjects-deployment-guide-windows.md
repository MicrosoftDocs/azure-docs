...
To mount an Azure file share on a Windows server, ensure port 445 is open. The SMB protocol requires TCP port 445 to be open. Connections will fail if port 445 is blocked. You can check if the firewall is blocking port 445 with the `Test-NetConnection` cmdlet mentioned in [Troubleshooting Azure Files connectivity and access issues (SMB)](../../storage/files/files-troubleshoot-smb-connectivity.md&tabs=windows#cause-1-port-445-is-blocked).
