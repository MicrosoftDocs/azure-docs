---
title: Set up the SAP LaMa connector for Azure
description: Learn how to set up and configure the SAP Landscape Management (LaMa) connector for Azure to manage SAP systems on Azure virtual machines.
author: MSSedusch
manager: timlt
...
SAP Landscape Management (LaMa) is a tool for operating and monitoring SAP landscapes. This article describes how to set up the SAP LaMa connector for Azure, create VMs, and configure them for adaptive SAP systems. Starting with version 3.0 SP05, SAP LaMa includes a connector to Azure that you can use to:
...
* Enable **Automatic Mountpoint Creation** in **Setup** > **Settings** > **Engine**. If SAP LaMa mounts volumes by using SAP Adaptive Extensions (SAPACEXT) on a VM, the mount point must exist if this setting isn't enabled.

* Use a separate subnet, and don't use dynamic IP addresses to prevent IP address "stealing" when you deploy new VMs and SAP instances are unprepared. If you use dynamic IP address allocation in the subnet that SAP LaMa also uses, preparing an SAP system with SAP LaMa might fail. If an SAP system is unprepared, the IP addresses aren't reserved and might get allocated to other VMs.

* If you sign in to managed hosts, don't block file systems from being unmounted. If you sign in to a Linux VM and change the working directory to a directory in a mount point (for example, `/usr/sap/AH1/ASCS00/exe`), the volume can't be unmounted and a relocate or unprepare operation fails.

* Disable `CLOUD_NETCONFIG_MANAGE` on SUSE SLES Linux VMs. For more information, see [SUSE KB 7023633](https://www.suse.com/support/kb/doc/?id=7023633).

For more information about SAP LaMa on Azure, see the following SAP Notes:
...
The connector for Azure is included in SAP LaMa as of version 3.0 SP05. We recommend that you always install the latest support package and patch for SAP LaMa 3.0.

The connector for Azure uses the Azure Resource Manager API to manage your Azure resources. SAP LaMa can use a service principal or a managed identity to authenticate against this API. If your SAP LaMa instance is running on an Azure VM, we recommend that you use a managed identity.
...
1. Select **New client secret**.
   1. Enter a description for a new key, and choose when the secret should expire.
   1. Select **Add**.
1. Record the value, and use it as the password for the service principal.
1. Record the application ID, and use it as the username of the service principal.
...
To be able to use a managed identity, your SAP LaMa instance has to run on an Azure VM that has a system-assigned or user-assigned identity. For more information about managed identities, see [What are managed identities for Azure resources?](/entra/identity/managed-identities-azure-resources/overview), and [Configure managed identities for Azure resources on a VM using the Azure portal](/entra/identity/managed-identities-azure-resources/qs-configure-portal-windows-vm).
...
Open the SAP LaMa website.

1. Go to **Infrastructure**, select the **Cloud Managers** tab, then select **Add**.
1. Select **Microsoft Azure Cloud Adapter**, then select **Next**. Enter the following information:
   1. **Label**: Choose a name for the connector instance.
   1. **User Name**: Enter the service principal application ID or the ID of the user-assigned identity of the VM.
   1. **Password**: Enter the service principal key/password. You can leave this field empty if you use a system-assigned or user-assigned identity.
   1. **URL**: Keep the default `https://management.azure.com/`.
   1. **Monitoring Interval (Seconds)**: Enter an interval of at least 300.
   1. **Use Managed Identity**: Select to enable SAP LaMa to use a system-assigned or user-assigned identity to authenticate against the Azure API.
   1. **Subscription ID**: Enter the Azure subscription ID.
   1. **Microsoft Entra tenant ID**: Enter the ID of the Microsoft Entra tenant.
   1. **Proxy host**: Enter the host name of the proxy if SAP LaMa needs a proxy to connect to the internet.
   1. **Proxy port**: Enter the TCP port of the proxy.
   1. **Change Storage Type to save costs**: Enable this setting if the Azure adapter should change the storage type of the managed disks to save costs when the disks aren't in use.
   ...
   If you stop a VM in SAP LaMa, the adapter changes the storage type of all attached disks, including the OS disk, to Standard Storage. If you start a VM in SAP LaMa, the adapter changes the storage type back to the original storage type.
...
You can manually deploy a new VM or use one of the Azure templates in the [quickstart repository](https://github.com/Azure/azure-quickstart-templates). The repository contains templates for [SAP NetWeaver ASCS](https://github.com/Azure/azure-quickstart-templates/tree/master/application-workloads/sap/sap-lama-ascs), [SAP NetWeaver application servers](https://github.com/Azure/azure-quickstart-templates/tree/master/application-workloads/sap/sap-lama-apps), and the [database](https://github.com/Azure/azure-quickstart-templates/tree/master/application-workloads/sap/sap-lama-database). You can also use these templates to provision new hosts as part of a system copy, clone, or similar activity.

We recommend that you use a separate subnet for all VMs that you want to manage with SAP LaMa. We also recommend that you don't use dynamic IP addresses to prevent IP address "stealing" when you're deploying new VMs and SAP instances are unprepared.

> [!NOTE]
> If possible, remove all VM extensions. They might cause long runtimes for detaching disks from a VM.

Make sure that the following users and group exist on the target machine with the same ID and group ID (or use LDAP):

* *\<hanasid\>adm*
* *\<sapsid\>adm*
* The group *sapsys*

Enable and start the Network File Sharing (NFS) server on the VMs that should be used to run SAP NetWeaver ABAP Central Services (ASCS) or SAP Central Services (SCS).

### Manual deployment

SAP LaMa communicates with the VM by using the SAP Host Agent. If you deploy the VMs manually or aren't using the Azure Resource Manager template from the quickstart repository, install the latest SAP Host Agent and SAP Adaptive Extensions. For more information about the required patch levels for Azure, see SAP Note [2343511].

#### Manual deployment of a Linux VM

Create a new VM with one of the supported operating systems listed in SAP Note [2343511]. Add more IP configurations for the SAP instances. Each instance needs at least one IP address and must be installed using a virtual host name.

The SAP NetWeaver ASCS instance needs disks for */sapmnt/\<SAPSID\>*, */usr/sap/\<SAPSID\>*, */usr/sap/trans*, and */usr/sap/\<sapsid\>adm*. The SAP NetWeaver application servers don't need more disks. Store everything related to the SAP instance on ASCS and export it via NFS. Otherwise, you currently can't add more application servers by using SAP LaMa.

![Diagram that shows SAP NetWeaver ASCS on Linux.](media/lama/sap-lama-ascs-app-linux.png)

#### Manual deployment for SAP HANA

Create a new VM with one of the supported operating systems for SAP HANA, as listed in SAP Note [2343511]. Add one extra IP configuration for SAP HANA and one per HANA tenant.
...
Create a new VM with one of the supported operating systems for Oracle databases, as listed in SAP Note [2343511]. Add one extra IP configuration for the Oracle database.
...
Create a new VM with one of the supported operating systems for Microsoft SQL Server, as listed in SAP Note [2343511]. Add one extra IP configuration for the SQL Server instance.

The SQL Server database server needs disks for the database data and log files. It also needs disks for *c:\usr\sap*.

![Diagram that shows a SQL Server database on Windows and the disks it needs.](media/lama/sap-lama-db-sql.png)

Make sure that you install a supported Microsoft ODBC driver for SQL Server on a VM. This driver allows you to use a target for relocating an SAP NetWeaver application server or as a system copy/clone target. SAP LaMa can't relocate SQL Server itself, so a VM that you want to use for these purposes needs SQL Server preinstalled.

### Deploy a VM by using an Azure template

Download the following latest available archives from the [SAP Software Download Center](https://support.sap.com/swdc) for the operating system of the VMs:
...
* `computerName`: The computer name of the new VM. SAP LaMa also uses this parameter. When you use this template to provision a new VM as part of a system copy, SAP LaMa waits until the host with this computer name can be reached.
...
* `sapSystemSize`: The size of the SAP system that you want to deploy. It's used to determine the type and size of the VM instance.

* `adminUsername`: The username for the VM.

* `adminPassword`: The password for the VM. You can also provide a public key for SSH.

* `sshKeyData`: The public SSH key for the VM (only supported on Linux operating systems).

* `subnetId`: The ID of the subnet that you want to use.

* `deployEmptyTarget`: An empty target that you can deploy if you want to use the VM as a target for an instance relocation or something similar. In this case, no extra disks or IP configurations are attached.
...
* `sapacExtLocation`: The location of the SAP Adaptive Extensions. The minimum patch level required for Azure is found in SAP Note [2343511].

* `vcRedistLocation`: The location of the Variant Configuration runtime required to install the SAP Adaptive Extensions. This parameter is required only for Windows.
...
Before you start the SAP Software Provisioning Manager (SWPM), you need to mount the IP address of the virtual host name of ASCS. We recommend using SAPACEXT. If you mount the IP address by using SAPACEXT, make sure that you remount the IP address after a reboot.
...
Azure NetApp Files provides NFS for Azure. In the context of SAP LaMa, it simplifies the creation of the ASCS instances and the subsequent installation of application servers. Previously, the ASCS instance also had to act as an NFS server, and the parameter `acosprep/nfs_paths` had to be added to the host profile of the SAP Host Agent.
...
1. Create the delegated subnet for *Microsoft.NetApp/volumes*.
   ...
   ![Screenshot that shows selections for adding a subnet.](media/lama/sap-lama-addsubnet-50.png)
...
1. Define the NFS volumes.
   ...
   Because one pool might contain volumes for multiple systems, choose a self-explanatory naming scheme. Adding the SID helps to group related volumes together.
...
1. Perform the installation with SWPM. You must also perform these steps for at least one AS instance.

   After successful installation, SAP LaMa must discover the system. The mount points should look like the following screenshot for the ASCS and AS instances.

   ![Screenshot that shows SAP LaMa mount points.](media/lama/sap-lama-ascs.png)

   > [!NOTE]
   > In this example, the IP addresses and export path are different from the ones that you used before.
...
Add the IP address of the virtual host name of the database to a network interface. We recommend using SAPACEXT. If you mount the IP address by using SAPACEXT, make sure that you remount the IP address after a reboot.
...
Run the database instance installation of SWPM on the SQL Server VM. Use `SAPINST_USE_HOSTNAME=as1-db` to override the host name that's used to connect to SQL Server. If you deployed the VM by using the Azure Resource Manager template, set the directory that's used for the database data files to `C:\sql\data`. Set the database log file to `C:\sql\log`.
...
* An error occurred when full copy wasn't enabled in the storage step.
...
* The path `/usr/sap/AH1` isn't allowed for NFS re-exports.

  * **Solution**: Add ASCS exports to the ASCS Host Agent profile. See SAP Note [2628497].

* A function isn't implemented in relocating ASCS.

  * **Command output**:

    > exportfs: host:/usr/sap/AX1: Function not implemented

  * **Solution**: Make sure that the NFS server service is enabled on the target VM for relocation.
...
* An error occurred in executing the SAPinst `checkClient" version="1.0"` step.
  ...
  * **Solution**: Make sure that the Microsoft ODBC driver for SQL Server is installed on the VM on which you want to install the application server.
...
* An error occurred in executing the SAPinst `askPasswords` step.
  ...
  * **Solution**: To allow communication from the VM to the domain controller, add a host rule in the isolation step.
