...
SAP doesn't offer private connectivity through private endpoints to SAP RISE workloads for customers, in general. Some rare and isolated scenarios might utilize such private endpoint for customers to use, but SAP doesn't enable such private endpoints for managed SAP landscape.

### Firewall as a service and subnet peering

For customers requiring stronger network-based security than NSGs, RISE offers an optional premium service called "Firewall as a Service (FWaaS)". The firewall is a managed service provided by SAP RISE/ECS and provides macro-segmentation of SAP workloads into different tiers. Systems within the same tier can bypass the firewall and communicate directly, while inter-tier communication has to pass through the firewall and follow traffic control rules. You can modify the necessary rule set and obtain firewall logs and audit trails through RISE LogServ service.

To enable the FWaaS service, [subnet peering](../../virtual-network/how-to-configure-subnet-peering.md) is established between one subnet from the RISE VNet and one subnet on the customer side. Unlike full VNet peering, subnet peering only enables communication between the selected subnets involved in the peering. When you subscribe to RISE's FWaaS offering, subnet peering is established between a RISE subnet hosting a firewall and a customer subnet hosting an NVA or other routing appliance. Each NVA is configured as the primary route target for all RISE communication. As a result, all traffic between SAP RISE workloads and your network will thus pass through both appliances.
...
:::image type="complex" source="./media/sap-rise-integration/sap-rise-fwaas.png" alt-text="Diagram of SAP RISE/ECS firewall as a service design.":::
   This diagram shows a typical SAP customer's hub and the RISE virtual network. Firewalls are deployed in a dedicated subnet on either side. Subnet peering is established between the two firewall subnets. No virtual network peering is used in this architecture. Three different groups of RISE workload VMs with communication paths to and from the respective VM group and RISE firewall.
:::image-end:::
...
> [!NOTE]
> Both Azure provided DNS and Azure private zones **do not** support DNS zone transfer capability, hence, can't be used to accept DNS replication from SAP RISE/PCE/ECS DNS servers. Additionally, SAP typically doesn't support external DNS service providers for the delegated zone.
...
Applications within your own virtual network connect to the Internet from respective virtual network directly or through customer’s centrally managed services such as Azure Firewall, NAT Gateway and others. Connectivity to SAP BTP from non-SAP RISE/ECS applications takes the same network Internet bound path on your side. Should an SAP Cloud Connecter be needed for such integration, run it on customer’s VMs. In other words, communication from your own applications to SAP BTP or any public RISE endpoint is on a network path managed by you. Communication from your SAP RISE landscape to SAP BTP or other public SAP endpoints is on a network path managed by SAP RISE/ECS.
...
See [SAP's documentation](https://help.sap.com/docs/private-link/private-link1/consume-azure-services-in-sap-btp) and a series of blog posts on the architecture of the SAP BTP Private Link Service and private connectivity methods from BTP, dealing with DNS and certificates in following SAP blog series [Getting Started with BTP Private Link Service for Azure](https://blogs.sap.com/2021/12/29/getting-started-with-btp-private-link-service-for-azure/).

Connection into BTP doesn't utilize Private Link and communicates with a public interface. If both SAP BTP and your SAP RISE landscape are running in Azure, the default routing behavior is to keep such Internet bound communication on Azure backbone. For more information, see the documentation on [Azure traffic routing](../../virtual-network/virtual-networks-udr-overview.md) for more details.
