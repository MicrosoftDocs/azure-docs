...
## New deployment of SAP workload

1. **How to create flexible virtual machine scale set without VM profile from Azure Portal?**

   Currently, it's not possible to create flexible virtual machine scale set without a VM profile through the Azure portal. However, you can use an alternative client to [create a scale set without a VM profile](./vmss-flex-sap-deployment-guide.md).

2. **Can I create flexible scale set with scaling or VM profile for SAP workload to use auto-scaling feature for SAP application servers?**
...
3. **Does setting FD=1 for flexible scale set zonal deployment imply that all VMs within the scale set would belong to a single fault domain?**
...
4. **Flexible virtual machine scale set with FD=1 is used for zonal deployment, what is the method for deploying flexible scale set in a region that does not have any zones?**
...
## High Availability and Disaster Recovery (HA/DR) of SAP workload

1. **How can I use Azure Site Recovery for VMs deployed in flexible scale set for disaster recovery?**
...
2. **To configure fence agent for high availability SAP workload, how to enable system managed identity on the VMs deployed in flexible scale set without VM profile?**
...
1. **How can I migrate my current Availability set or Availability zone deployment of SAP workload to flexible scale set with zonal deployment (FD=1)?**

2. **How can an existing deployment of SAP HANA, which is [pinned](../../azure-netapp-files/application-volume-group-considerations.md#best-practices-about-proximity-placement-groups) to Azure NetApp Files, be migrated to flexible scale set with FD=1?**
