---
title: Generate SAP application installation templates
description: Learn how to generate SAP application installation templates for use with SAP Deployment Automation Framework.
author: kimforss
ms.author: kimforss
ms.date: 04/14/2026
ms.topic: how-to
...
# Generate SAP application installation templates
...
- [Get, download, and prepare your SAP installation media and related files](bom-get-files.md). Make sure to have the [name of the SAPCAR utility file that you downloaded](bom-get-files.md#acquire-media) available.
- [Prepare your BOM](bom-prepare.md). Make sure to have the BOM file that you created available.
...
1. Change the root user password to a known value. Use this password later to connect to the SAP Software Provisioning Manager (SWPM).
...
To generate your unattended installation parameter file for Advanced Business Application Programming SAP Central Services (ASCS):
...
1. Keep the Advanced Business Application Programming (ABAP) message server port settings. These default settings are `3600` and `3900`. Then, select **Next**.
...
> [!IMPORTANT]
> You might not see some of these settings in the 2020 versions of SAP products. In that case, skip the step.
...
1. On **SLD Destination for the SAP System OS Level**, configure these settings:
   ...
   1. Set user password for **TMSADM** in **Client 000** to the main password. Then, select **Next**.
...
## Generate extra application servers parameter file
...
1. In the drop-down menu, go to **SAP S/4HANA Server 2020** &gt; **SAP HANA Database** &gt; **Installation** &gt; **Application Server ABAP** &gt; **High-Availability System** &gt; **Additional Application Server Instance**.
...
1. Align all the equal signs. For example:
...
1. To use Ansible variables, update the following lines:
...
- [Get SAP installation media](bom-get-files.md)
