---
title: Deploy SAP Deployment Automation Framework
description: Learn how to deploy SAP Deployment Automation Framework for enterprise scale on Azure, including a control plane, workload zone, and SAP system infrastructure.
author: hdamecharla
...
# Deploy SAP Deployment Automation Framework for enterprise scale

This article shows you how to perform deployments by using [SAP Deployment Automation Framework](deployment-framework.md). This example uses Azure Cloud Shell to deploy the control plane infrastructure. The deployer virtual machine (VM) creates the remaining infrastructure and SAP HANA configurations.
...
There are several workflows in the deployment automation process. This article focuses on one workflow for ease of deployment. You can deploy this workflow, the SAP S4 HANA standalone environment, by using Bash. The following sections describe the general hierarchy and different phases of the deployment.

## Prerequisites

- An Azure subscription. If you don't have one, [create a free account](https://azure.microsoft.com/pricing/purchase-options/azure-account?cid=msft_learn).
- [Azure CLI](/cli/azure/install-azure-cli) version 2.5.0 or later.
- [Terraform](https://developer.hashicorp.com/terraform/downloads) version 1.5 or later.
- [Git](https://git-scm.com/downloads) for cloning automation framework repositories.
- An SSH client or [Azure Bastion](/azure/bastion/bastion-overview) for connecting to the deployer VM.
- An Azure account with permissions to create service principals and assign roles (Contributor and User Access Administrator) on the target subscription.
- Sufficient Azure subscription core quota for DdSV4 and EdsV4 family SKUs in the selected region. About 50 cores available for each VM family should suffice.
- A valid SAP user account (SAP-User or S-User account) with software download privileges to download the SAP software.
- The [SAP Deployment Automation Framework repository](https://github.com/Azure/sap-automation) on GitHub.
- (Optional) [.NET SDK](https://dotnet.microsoft.com/download) if you plan to deploy the configuration web application.
...
:::image type="content" source="media/devops/automation-devops-tutorial-design.png" alt-text="Diagram that shows the infrastructure design.":::
...
The SAP automation deployment framework uses service principals for deployment. Create a service principal for your control plane deployment. To create service principals, make sure to use an account with the appropriate permissions.
...
>
> Ensure that the file you use to save the SSH key can save the file by using the correct format, that is, without carriage return (CR) characters. Use Visual Studio Code or a basic text editor.
...
1. If necessary, register the service principal. For this example, this step isn't needed.
   ...
   The first time an environment is instantiated, a service principal must be registered. In this example, the control plane is in the `LAB` environment and the workload zone is also in `LAB`. For this reason, a service principal must be registered for the `LAB` environment.
...
1. To download the software, run the Ansible playbook. One way you can run the playbooks is to use the **Downloader** menu. Run the `download_menu` script.
...
You now deployed and configured a standalone HANA system. If you need to configure a highly available (HA) SAP HANA database, run the HANA HA playbook.
...
It's important to clean up your SAP installation after you're finished. Otherwise, you continue to incur costs related to the resources.

To remove the entire SAP infrastructure you deployed, you need to:

1. Remove the SAP system infrastructure resources.
1. Remove all workload zones (the landscape).
