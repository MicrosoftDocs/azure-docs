...
description: Learn how to upgrade SAP Deployment Automation Framework.
author: kimforss
ms.author: kimforss
ms.date: 04/16/2026
ms.topic: how-to
...
SAP Deployment Automation Framework is updated regularly. You can upgrade the pipeline definitions, the control plane, and the workload zone to get the latest features and fixes. This article describes how to upgrade each component.

## Prerequisites

- A deployed SAP Deployment Automation Framework environment, including a deployer virtual machine (VM) and SAP library.
- A backup of the remote state files from the `tfstate` storage account in the SAP library.
- An [Azure DevOps](configure-devops.md) project with pipelines configured for SAP Deployment Automation Framework.
- [Azure CLI](/cli/azure/install-azure-cli) installed, with permissions to manage resources in the target subscription.
- SSH access to the deployer VM.
...
1. Go to the pipelines folder in your repository and select **New** to create a new file.

1. Name the file `21-update-pipelines.yml` and paste the following content into the file:
...
1. Commit the changes to save the file to the repository.

1. In the **Pipelines** section, select **New Pipeline**. Select **Azure Repos Git** as the source for your code. Configure your pipeline to use an existing Azure Pipelines YAML file with the following settings:
...
1. Save the pipeline. To see the **Save** option, select the chevron next to **Run**.

1. Go to the **Pipelines** section and select the pipeline. Rename the pipeline to `Upgrade pipelines` by selecting **Rename/Move** from the ellipsis menu on the right.

1. Run the pipeline to upgrade all pipeline definitions.
...
### Upgrade the control plane to version 3.8.1
...
### Upgrade Terraform on the agents
...
### Upgrade the SAP Automation code base on the deployer
...
The workload zone is the second component you need to upgrade. To upgrade the workload zone, rerun the `SAP Workload Zone deployment` pipeline or rerun the `install_workloadzone.sh` script.

### Upgrade the workload zone to version 3.8.1
