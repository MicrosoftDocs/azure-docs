...
> [!NOTE]
> For all analytics content, some of the resources available in Enterprise Insights can require customization and tailoring to meet your specific SAP system version and release. This is important for functional insights where some objects can be present in a different version and could require further changes to ensure you can extract and process them successfully.

## Implementation Requirements

Prior to deploying Enterprise Insights, it's advisable to implement the following SAP Notes, along with any dependencies, to help prevent common issues during data extraction:

- **2930269** – ABAP CDS CDC: Common issues, troubleshooting, and components (review all notes listed in point 9 of this SAP Note)
...
SAP recommends you perform the following transactions in sequence:
...
2. **HRRP_REP** – Complete the replication or import of hierarchies already created in SAP for Financial Statements (FSVNs), Profit Centers (0106), and Cost Centers (0101) for the relevant organizational units.

It's important you complete both transactions in order and mark the appropriate hierarchy entries for replication, which are essential for the extraction of data using the CDS Views (`I_PROFITCENTERHIERARCHY*`, `I_COSTCENTERHIERARCHY*`).
