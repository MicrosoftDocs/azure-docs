...
3. Assign **Kubernetes node IPs**. These static IP addresses are for the compute VM. 
    ...
    For an *n*-node device, a contiguous range of a minimum of *n+1* IPv4 addresses (or more) are provided for the compute VM using the start and end IP addresses. Given Azure Stack Edge Pro is a 1-node device, a minimum of 2 contiguous IPv4 addresses are provided. 
