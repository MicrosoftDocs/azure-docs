---
title: Access Azure Stack Edge Pro with GPU device from outside network | Microsoft Docs
description: Describes network settings to enable access to an Azure Stack Edge Pro GPU device from an outside network.
services: databox
author: v-dalc
...
Before you configure your network to enable device access from outside the network, make sure that:
...
Follow these steps to configure the network for your device to enable device access from outside the network.

**WILL ADAPT STEPS WHEN I HAVE PROCEDURES FROM THE PRODUCT TEAM. 02/04/2022**
