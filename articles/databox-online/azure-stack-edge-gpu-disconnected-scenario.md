...
| IoT Edge | IoT Edge modules need to be deployed and updated while connected to Azure. If disconnected from Azure, they will continue to run.<!--REPLACES: IoT Edge modules that are deployed on the device continue to run while the device is disconnected, but you can't update the IoT Edge deployment manifest. For example, you can't manage images or update configuration options. You can make these updates when the device can communicate with Azure again.--> |
