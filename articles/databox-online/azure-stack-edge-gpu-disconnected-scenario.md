...
Some environments require that your Azure Stack Edge device not be connected to the internet. As a result, Azure Stack Edge becomes a standalone deployment that is disconnected from and doesn't communicate with Azure and other Azure services. In a disconnected scenario, certain functionality of the device is limited or unusable.
...
| VM cloud management | Virtual machine (VM) images can't be downloaded to the device from the cloud. A virtual machine can be managed locally via local Azure Resource Manager (ARM). |
...
| 5G | In a 5G deployment, Virtual Network Functions (VNF) aren't available:<!--This may be an overstatement.--><ul><li>To register, deploy, and manage VNF, your device must be connected.</li><li>Metrics collection (every 1-5 minutes) and VNF heartbeat monitoring to Azure private Multi-access Edge Compute (MEC) (every 10 minutes) also require a connection to Azure.</li></ul> |
