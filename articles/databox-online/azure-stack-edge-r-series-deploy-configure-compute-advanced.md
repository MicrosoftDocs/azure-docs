...
1. In the Azure portal of your Azure Stack Edge resource, go to **Overview**. In the right-pane, on the **Compute** tile, select **Get started**.
    ...
    ![Get started with compute](./media/azure-stack-edge-r-series-deploy-configure-compute-advanced/configure-compute-1.png)

2. On the **Configure Edge compute** tile, select **Configure compute**.

    ![Get started with compute](./media/azure-stack-edge-r-series-deploy-configure-compute-advanced/configure-compute-2.png)
...
3. On the **Configure Edge compute** blade, input the following:
    ...
    ![Get started with compute](./media/azure-stack-edge-r-series-deploy-configure-compute-advanced/configure-compute-3.png)

4. Select **Create**. The IoT Hub resource creation takes a couple minutes. After the IoT Hub resource is created, the **Configure Edge compute** tile updates to show the compute configuration. 

    ![Get started with compute](./media/azure-stack-edge-r-series-deploy-configure-compute-advanced/configure-compute-4.png)

5. To confirm that the Edge compute role has been configured, select **View config** on the **Configure compute** tile.
    
    ![Get started with compute](./media/azure-stack-edge-r-series-deploy-configure-compute-advanced/configure-compute-5.png)
...
1. Add an Edge share on the device by doing the following steps:
    ...
    5. Select the **Storage account**, **Storage service**, an existing user, and then select **Create**.
        ...
        ![Add an Edge share](./media/azure-stack-edge-r-series-deploy-configure-compute-advanced/add-edge-share-1.png)
...
2. Add an Edge local share on the Edge device by repeating all the steps in the preceding step and selecting the check box for **Configure as Edge local share**. The data in the local share stays on the device.
    ...
    ![Add an Edge local share](./media/azure-stack-edge-r-series-deploy-configure-compute-advanced/add-edge-share-2.png)

3. On the **Shares** blade, you see the updated list of shares.

    ![Updated list of shares](./media/azure-stack-edge-r-series-deploy-configure-compute-advanced/add-edge-share-3.png)
...
4. To view the properties of the newly created local share, select the share from the list. In the **Local mount point for Edge compute modules** box, copy the value corresponding to this share.
    ...
    ![The "Local mount point for Edge compute modules" box](./media/azure-stack-edge-r-series-deploy-configure-compute-advanced/add-edge-share-4.png)
...
5. To view the properties of the Edge share that you created, select the share from the list. In the **Local mount point for Edge compute modules** box, copy the value corresponding to this share.
    ...
    ![Add custom module](./media/azure-stack-edge-r-series-deploy-configure-compute-advanced/add-edge-share-5.png)
...
1. Go to **Edge compute > Triggers**. Select **+ Add trigger**.
    ...
    ![Add trigger](./media/azure-stack-edge-r-series-deploy-configure-compute-advanced/add-trigger-1.png)
...
2. In the **Add trigger** blade, input the following values.
    ...
    ![Add trigger](./media/azure-stack-edge-r-series-deploy-configure-compute-advanced/add-trigger-2.png)

3. You are notified after the trigger is created. The list of triggers is updated to display the newly created trigger. 

    ![Add trigger](./media/azure-stack-edge-r-series-deploy-configure-compute-advanced/add-trigger-3.png)
...
There are no custom modules on this Edge device. You could add a custom or a pre-built module. To learn how to create a custom module, go to [Develop a C# module for your Azure Stack Edge device](azure-stack-edge-r-series-placeholder.md).
...
1. Go to **Edge compute > Get started**. On the **Add modules** tile, select the scenario type as **advanced**. Select **Go to IoT Hub**.
    ...
    ![Select advanced deployment](./media/azure-stack-edge-r-series-deploy-configure-compute-advanced/add-module-1.png)
...
2. In your IoT Hub resource, go to **IoT Edge device** and then select your IoT Edge device.
    ...
    ![Go to IoT Edge device in IoT Hub](./media/azure-stack-edge-r-series-deploy-configure-compute-advanced/add-module-2.png)

3. On **Device details**, select **Set Modules**.

    ![The Set Modules link](./media/azure-stack-edge-r-series-deploy-configure-compute-advanced/add-module-3.png)
...
4. Under **Add Modules**, do the following:
    ...
    The name, address, and listed credentials are used to retrieve modules with a matching URL. To deploy this module, under **Deployment modules**, select **IoT Edge module**. This IoT Edge module is a docker container that you can deploy to the IoT Edge device that's associated with your Azure Stack Edge device.
        ...
        ![The Set Modules page](./media/azure-stack-edge-r-series-deploy-configure-compute-advanced/add-module-4.png) 
    ...
    2. Specify the settings for the IoT Edge custom module. Input the following values.
        ...
        ![The Container Create Options box](./media/azure-stack-edge-r-series-deploy-configure-compute-advanced/add-module-5.png) 
...
5.	Under **Specify Routes**, set routes between modules.  
    ...
    ![The Specify Routes](./media/azure-stack-edge-r-series-deploy-configure-compute-advanced/add-module-7.png)
    ...
    ![The Specify Routes section](./media/azure-stack-edge-r-series-deploy-configure-compute-advanced/add-module-8.png)

6.	Under **Review deployment**, review all the settings, and then select **Submit** to submit the module for deployment.

    ![The Set Modules page](./media/azure-stack-edge-r-series-deploy-configure-compute-advanced/add-module-9.png)
 
    This action starts the module deployment. After the deployment is complete, the **Runtime status** of module is **running**.

    ![Add custom module](./media/azure-stack-edge-r-series-deploy-configure-compute-advanced/add-module-10.png)
...
1.	In File Explorer, connect to both the Edge local and Edge shares you created previously.
    ...
    ![Verify data transform](./media/azure-stack-edge-r-series-deploy-configure-compute-advanced/verify-data-2.png)
 
1.	Add data to the local share.

    ![Verify data transform](./media/azure-stack-edge-r-series-deploy-configure-compute-advanced/verify-data-3.png)
 
    The data gets moved to the cloud share.

    ![Verify data transform](./media/azure-stack-edge-r-series-deploy-configure-compute-advanced/verify-data-4.png)  
