...
The native IoT Edge solution on Azure Stack Edge runs an IoT Edge runtime version 1.1.0 on a Kubernetes cluster. Going forward, this version of IoT Edge will have limited support. We strongly recommend that you do not use the native version of IoT Edge solution, rather deploy the IoT Edge runtime on an Ubuntu VM using the procedure described in this article.
...
- You have access to an Azure market place VM image. 
...
- You have access to the latest version of IoT Edge runtime that you'll deploy on your Ubuntu VM.
...
1. In the Azure portal, go to Azure marketplace. 
    ...
    1. Search the Azure Marketplace and identify your preferred image. 
...
1. After the VM is running, you'll install the IoT Edge runtime on the VM. 

    1.  You can register and manually provision a single Linux IoT Edge device, including installing IoT Edge runtime. For detailed instructions, see [Deploy IoT Edge runtime for a single device](../iot-edge/how-to-provision-single-device-linux-symmetric.md?view=iotedge-2020-11&tabs=azure-portal%2Cubuntu&preserve-view=true).
    1. You can also use the Azure IoT Edge configuration command-line tool to install IoT Edge runtime version 1.2. You can then configure Azure IoT Edge on the VM using [IoT Hub with DPS or IoT Central]().
