...
- You've activated your Azure Stack Edge device as described in [Connect, set up, and activate Azure Stack Edge](azure-stack-edge-r-series-deploy-connect-setup-activate.md).
...
1. In the Azure portal of your Azure Stack Edge resource, go to **Overview**. In the right-pane, on the **Compute** tile, select **Get started**.
    ...
    ![Get started with compute](./media/azure-stack-edge-r-series-deploy-configure-compute/configure-compute-1.png)

2. On the **Configure Edge compute** tile, select **Configure compute**.

    ![Configure compute](./media/azure-stack-edge-r-series-deploy-configure-compute/configure-compute-2.png)
...
3. On the **Configure Edge compute** blade, input the following:
    ...
    ![Get started with compute](./media/azure-stack-edge-r-series-deploy-configure-compute/configure-compute-3.png)
...
4. Select **Create**. The IoT Hub resource creation takes several minutes. After the IoT Hub resource is created, the **Configure compute** tile updates to show the compute configuration. 
    ...
    ![Get started with compute](./media/azure-stack-edge-r-series-deploy-configure-compute/configure-compute-4.png)

5. To confirm that the Edge compute role has been configured, select **View Compute** on the **Configure compute** tile.
    
    ![Get started with compute](./media/azure-stack-edge-r-series-deploy-configure-compute/configure-compute-5.png)
...
1. Add an Edge share on the device by doing the following steps:
    ...
    2. On the **Add share(s)** tile, select **Add**.
        ...
        ![Add share tile](./media/azure-stack-edge-r-series-deploy-configure-compute/add-share-1.png) 
    ...
    5. Select the **Storage account**, **Storage service**, an existing user, and then select **Create**.
        ...
        ![Add an Edge share](./media/azure-stack-edge-r-series-deploy-configure-compute/add-edge-share-1.png) 
...
2. Add an Edge local share on the Edge device by repeating all the steps in the preceding step and selecting the check box for **Configure as Edge local share**. The data in the local share stays on the device.
    ...
    ![Add an Edge local share](./media/azure-stack-edge-r-series-deploy-configure-compute/add-edge-share-2.png)

  
3. Select **Add share(s)** to see the updated list of shares.

    ![Updated list of shares](./media/azure-stack-edge-r-series-deploy-configure-compute/add-edge-share-3.png) 
...
2. In the **Configure and add module** blade, input the following values:
    ...
    ![Add and configure module](./media/azure-stack-edge-r-series-deploy-configure-compute/add-module-1.png)
...
3. Select **Add**. The module gets added. Go to the **Overview** page. The **Modules** tile updates to indicate that the module is deployed. 
    ...
    ![Module deployed](./media/azure-stack-edge-r-series-deploy-configure-compute/add-module-2.png)
...
1. Select the **Add module** tile. This takes you to the **Modules** blade. In the list of modules, identify the module you deployed. The runtime status of the module you added should be *running*.
    ...
    ![View deployed module](./media/azure-stack-edge-r-series-deploy-configure-compute/add-module-3.png)
 
1.	In File Explorer, connect to both the Edge local and Edge shares you created previously.

    ![Verify data transform](./media/azure-stack-edge-r-series-deploy-configure-compute/verify-data-2.png) 
 
1.	Add data to the local share.

    ![Verify data transform](./media/azure-stack-edge-r-series-deploy-configure-compute/verify-data-3.png) 
 
    The data gets moved to the cloud share.

    ![Verify data transform](./media/azure-stack-edge-r-series-deploy-configure-compute/verify-data-4.png)  

    The data is then pushed from the cloud share to the storage account. To view the data, you can use Storage Explorer.

    <!--![Verify data transform](./media/azure-stack-edge-r-series-deploy-configure-compute/verify-data-5.png)-->
