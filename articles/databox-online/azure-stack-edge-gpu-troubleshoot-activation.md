...
| If the activation key generation fails due to any error, then you receive this error. Additional details are present in the notification. <br> ![Key vault error 4](./media/azure-stack-edge-gpu-troubleshoot-activation/key-vault-error-4.png)   | Ensure that the ports and URLs specified in [Access Azure Key Vault behind a firewall](../key-vault/general/access-behind-firewall) are open on your firewall in order to access the Key Vault. Wait a few minutes and retry the operation. If the problem persists, contact Microsoft Support. |
