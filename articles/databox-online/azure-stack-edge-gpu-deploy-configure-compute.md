...
- You've activated your Azure Stack Edge device as described in [Activate Azure Stack Edge](azure-stack-edge-gpu-deploy-activate.md).
...
To configure compute on your Azure Stack Edge, you'll create an IoT Hub resource via the Azure portal.
...
3. On the **Configure Edge compute** blade, input the following:
    ...
    ![Get started with compute](./media/azure-stack-edge-j-series-deploy-configure-compute/configure-compute-3.png)
...
4. Select **Create**. The IoT Hub resource creation takes several minutes. After the IoT Hub resource is created, the **Configure compute** tile updates to show the compute configuration. 
    ...
    ![Get started with compute](./media/azure-stack-edge-j-series-deploy-configure-compute/configure-compute-4.png)
...
4. While you are in the local web UI, you can:
    ...
    - You can also go to **Kubernetes dashboard** endpoint and download an `aseuser` config file. 
        ...
        The `aseuser` config file allows you to debug any issues related to `iotedge` namespace in your Kubernetes cluster. For more information, see [Debug Kubernetes issues](azure-stack-edge-gpu-connect-powershell-interface.md#debug-kubernetes-issues). 
