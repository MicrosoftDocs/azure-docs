...
## Boot up in non-DHCP environment (Gateway)
...
## Deactivate your device (Edge)

To deactivate your device, you need to securely wipe out all the data on the data disk and the boot disk of your device. 
...
If you use the device reset in the local web UI, only the data disks are securley wiped but the boot disk is kept intact. The boot disk contains the device configuration.
