...
|**12.**|Kubernetes cluster|Existing Azure IoT Edge marketplace modules will not run on the Kubernetes cluster as the hosting platform for IoT Edge on Azure Stack Edge device.|The modules will need to be modified before these are deployed on the Azure Stack Edge device. For more information, see [Modify Azure IoT Edge modules from marketplace to run on Azure Stack Edge device](azure-stack-edge-placeholder.md).|
