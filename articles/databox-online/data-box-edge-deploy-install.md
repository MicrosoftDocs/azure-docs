...
---
# Tutorial: Provision Azure Data Box Edge in Hyper-V (Preview)
...
* You have completed all the steps in [Prepare the portal for Data Box Edge](data-box-gateway-deploy-prep.md).
    * You have created Data Box Edge resource to deploy your device.
    * You have generated the activation key to activate your device with the Data Box Edge resource.
...
- Make sure that one 1 U slot is available on the rack in which you intend to mount the device.
...
2. Inspect the box and the packaging foam for crushes, cuts, water damage, or any other obvious damage. If the box or packaging is severely damaged, do not open the box. Please contact Microsoft Support to help you assess whether the device is in good working order.
3. Unpack the box. After unpacking the box, make sure that you have:
    - 1 single enclosure Edge device
    - 2 power cords
    - 1 tool-less slide rackmount kit (2 side rails and mounting hardware included)
4. If you did not receive any of the items listed above, contact Data Box Edge Support. The next step is to rack-mount your device.
...
- Your Edge physical device, completely unpacked and rack mounted.
- 2 power cables. 
- 2 X 1 GbE RJ-45 network cable, and 4 X 25 GbE SFP+ copper cables.
- Access to 2 Power Distribution Units (recommended).

Your Edge device has 8 NVMe SSDs. The front panel also has status LEDs and power buttons. The device includes redundant Power Supply Units (PSUs) at the back. Your device has six network interfaces: two 1 Gbps, and four 25 Gbps Ethernet ports. Your device has a baseboard management controller (BMC). Identify the various ports on the backplane of your device.

 

 

Perform the following steps to cable your device for power and network.

1. Connect the power cords to each of the PSUs in the enclosure. Both PSUs must be installed and connected to different power sources to ensure high availability.
...
3. Connect the 1 GbE network interface PORT 1 to the computer that you are using to configure the physical device. PORT 1 is the dedicated management interface.
