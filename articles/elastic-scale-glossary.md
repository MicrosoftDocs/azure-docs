...
**Shard Map Manager** – A management service (via client APIs) and data store that contains the shard map(s), shard locations, and mappings for one or more shard sets. 


**Shardlet** – All of the data associated with a single value of a sharding key - including all rows within one or more sharded tables that share that key.  All of the data associated with a Shardlet must be co-located within a single shard.  A shardlet is the smallest granularity of data movement possible when redistributing sharded tables. 
