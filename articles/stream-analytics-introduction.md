...
##<a name="#motivation" />Motivation & overview
...
Azure Stream Analytics currently connects directly to Azure Event Hub for stream ingestion, and Azure Blobs for historical data.  Using the open streaming interfaces of Azure Event Hub, it is easy to integrate Azure Stream Analytics with other data sources and processing engines without losing the streaming nature of the computations.  Data can be written from Stream Analytics to Azure Storage where it can then later be processed again as a series of events, or used in other forms of batch analytics using HDInsight.  Azure Stream Analytics leverages years of Microsoft Research work in developing highly-tuned streaming engines for time-sensitive processing, as well as language integrations for intuitive specifications of such.  Azure Stream Analytics is built leveraging the open source community of Hadoop, YARN and REEF for scale out processing.

##<a name="capabilities" />Key capabilities
...
###Low Latency
Stream Analytics is currently optimized for end-to-end observable latencies in the sub-second range.  This allows the system to process continually at a high rate throughput. The communication model is a pull-based, adaptive batching model that operates based on configured timeouts and size limits.  Hence, events and other records are batched until the limits are reached.  However, even with the ability to process millions of events per second, the service is built to provide very low latencies. 

###Exactly once processing
Stream Analytics provides stream processing with exactly-once semantics as opposed to at-most-once or at-least-once.  The complexity of implementing such a system is hidden as part of our out-of-the-box features.  While our communication model handles this semantic in how we handle time, we provide users the ability to keep or drop late arriving events.  This can be configured easily via the Azure Portal UI.

###Reference Data
Stream Analytics provides users the ability to specify and use reference data.  These could be historical data or data that changes less frequently over time.  This data set can be treated like any other incoming event stream to join with other event streams ingested in real-time to perform transformations.  With the system’s ability to guarantee repeatability as mentioned above, none of the computations will be allowed to merely “look up the current value” of something without a guarantee that it will be able to repeat such a lookup in the future and receive the same result.  Hence, Stream Analytics converts all reference data into historical reference data capable of answering the question of “what was your value at time T” for any reasonable T. For additional information on construction and usage of reference data, please see the Resources section.

##<a name="decision" />Business motivations with choosing Azure Stream Analytics

###Cost
The Stream Analytics service is optimized to provide users very low cost to get going and maintain real time analytics solutions.  The service is built for you to pay as you go based on usage.  The usage is derived based on the volume of events processed and the amount of compute power provisioned within the cluster to handle the respective streaming jobs.  Stream Analytics is built enabling multi-tenancy with high levels of isolation within the streaming clusters which helps us keep the costs down for customers.  In order to execute multiple customers’ streaming jobs on the same VM, the service is built with strong security.
...
##<a name="scenarios" />Scenarios & use cases
