...
1. Click the gear icon at the top of the **Search twitter** action and select **Repeat over a list**. 
 
2. In the text box, type the following:

    	@actions('twitterconnector').outputs.body

	As with the basic flow, this outputs a list of tweets. 

3. Replace the **Content** value with `@repeatItem().TweetText`. The **repeatItem** is a function that returns each element in the list. 

4. Replace the **File Path** value with the following:

		`@contact('/',repeatItem(),'.txt')`

	The  **concat** function joins together two or more strings. 

5. Click the check mark to accept the changes.

### Conditional
This logic app still results in a lot of files being uploaded to Dropbox. The following steps add additional logic to make sure that you only receive a file when the person tweeting has a minimum number of followers. 
...
2. In the text box, type the following:
	...
	The function **greaterThan** compares two values and only allows the action to be executed when the first value is greater than the second value. You access a given property as a dot (.) followed by the property name, such as `.Followers` above. 

3. Click the check mark to save the new condition.
...
You can use this callback to invoke a logic app from inside your custom application. For more information, see [the REST documentation](http://go.microsoft.com/fwlink/?LinkID=525617&clcid=0x409). 
