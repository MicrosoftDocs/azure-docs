...
**TCP protocol:**  If you’ve chosen TCP as the protocol for monitoring, open the Firewall port on the agents being used for Performance Monitor and ExpressRoute Monitor capabilities, to ensure that the agents can connect to each other. To do this, run the [EnableRules.ps1](https://aka.ms/npmpowershellscript) PowerShell script without any parameters in a power shell window with administrative privileges.  
...
>[!NOTE]
> You do not need to run the [EnableRules.ps1](https://aka.ms/npmpowershellscript ) PowerShell script for Service Endpoint Monitor 
