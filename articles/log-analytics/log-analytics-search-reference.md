...
>[!NOTE]
> This article describes log searches using the current query language in Log Analytics.  If your workspace has been upgraded to the [next generation Log Analytics query language](log-analytics-log-search-upgrade.md), then you should refer to [the language reference for the next generation language](https://docs.loganalytics.io/queryLanguage/query_language.html).
