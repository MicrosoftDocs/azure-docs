...
---
# Understanding data security in Log Analytics
...
This article explains how data is collected, processed, and secured by Log Analytics. You can use agents to connect to the web service, use System Center Operations Manager to collect operational data, or retrieve data from Azure diagnostics for use by Log Analytics. The collected data is sent over the Internet using certificate-based authentication & SSL 3.0 to the Log Analytics service, which is hosted in Microsoft Azure. Data is compressed by the agent before it is sent.
...
Log Analytics management solutions include data acquisition rules to support the functionality the solution is designed to deliver.  Depending on the solutions you have enabled, they can collect data from different sources on the computer generated by the operating system or hosted application, from Azure resources, System Center Operations Manager, etc.  The documentation for each solution will describe the specific type of data collected, data collection sources, methods to collect the data, and detailed information about the data in order to properly understand it as you search and analyze it.

Microsoft deletes customer data 30 days after the Azure resource is closed. Microsoft also deletes the Azure Storage Account where the data resides. When customer data is removed, no physical drives are destroyed.

## Physical security
The Log Analytics service is managed by Microsoft personnel and all activities are logged and can be audited. The service runs completely in Azure and complies with the Azure common engineering criteria. You can view details about the physical security of Azure assets on page 18 of the [Microsoft Azure Security Overview](http://download.microsoft.com/download/6/0/2/6028B1AE-4AEE-46CE-9187-641DA97FC1EE/Windows%20Azure%20Security%20Overview%20v1.01.pdf). Physical access rights to secure areas are changed within one business day for anyone who no longer has responsibility for the service, including transfer and termination. You can read about the global physical infrastructure we use at [Microsoft Datacenters](https://www.microsoft.com/en-us/server-cloud/cloud-os/global-datacenters.aspx).
...
If loss of any customer data occurs, we notify each customer within one day. However, customer data loss has never occurred with Log Analytics. Additionally, we maintain copies of data that was created and it is geographically distributed.
...
Log Analytics meets the following requirements:
...
The following diagram shows the cloud security architecture as the flow of information from your company and how it is secured as is moves to the Log Analytics service, ultimately seen by you in the portal. More information about each step follows the diagram.
...
For your organization to send data to Log Analytics, you configure agents running on Azure Windows/Linux virtual machines or running on-premises Windows/Linux computers. If you use Operations Manager, then you use a configuration wizard in the Operations console to configure the management group and agents reporting to it. Users (which might be you, other individual users, or a group of people) create one or more Log Analytics resource, and register agents by using one of the following accounts:
...
A Log Analytics resource is where data is collected, aggregated, analyzed, and presented. It is primarily used as a means to partition data, and each resource is unique. For example, you might want to have your production data managed with one Log Analytics resource and your test data managed with another resource. Resources also help an administrator control user access to the data. Each resource can have multiple user accounts associated with it, and each user account can access multiple resources. You create resources based on datacenter region. Each resource is replicated to other datacenters in the region, primarily for service availability.

For Operations Manager, when the configuration wizard completes, each Operations Manager management group establishes a connection with the Log Analytics service. You then use the Add Computers Wizard to choose which computers in the management group are allowed to send data to the service. For other agent types, each connects securely to the service.
...
Each type of agent collects data for Log Analytics. The type of data that is collected is depends on the types of solutions used. You can see a summary of data collection at [Add Log Analytics solutions from the Solutions Gallery](log-analytics-add-solutions.md). Additionally, more detailed collection information is available for most solutions. A solution is a bundle of predefined views, log search queries, data collection rules, and processing logic. Only administrators can use Log Analytics to import a solution. After the solution is imported, it is distributed to the Operations Manager management servers (if used), and then to any agents that you have chosen. Afterward, the agents collect the data.

2. Send data from agents
You register all agent types with an enrollment key and a secure connection is established between the agent and the Log Analytics service using certificate-based authentication and SSL over port 443. Log Analytics uses a secret store to generate and maintain keys. Private keys are rotated every 90 days and are stored in Azure and are managed by the Azure operations who follow strict regulatory and compliance practices.

With Operations Manager, you register a Log Analytics resource with the Log Analytics service and a secure HTTPS connection is established between the Operations Manager management server.
...
You can sign in to Log Analytics in the Azure portal by using the organizational account or Microsoft account that you set up previously. All traffic between the portal and Log Analytics is sent over a secure HTTPS channel. When using the Azure portal, a session ID is generated on the user client (web browser) and data is stored in a local cache until the session is terminated. When terminated, the cache is deleted. Client-side cookies, which do not contain personally identifiable information, are not automatically removed. Session cookies are marked HTTPOnly and are secured. After a pre-determined idle period, the Azure portal session is terminated.
