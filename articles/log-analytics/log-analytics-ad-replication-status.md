...
* Add the Active Directory Replication Status solution to your Log Analytics workspace using the process described in [Add Log Analytics solutions from the Solutions Gallery](../monitoring/monitoring-solutions.md).  There is no further configuration required.
...
From here, you can filter further, modify the search query, and so on. For more information about using the Log Search, see [Log searches](log-analytics-log-search.md).
...
## Next steps
* Use [Log searches in Log Analytics](log-analytics-log-search.md) to view detailed Active Directory Replication status data.
