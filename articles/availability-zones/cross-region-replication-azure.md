...
You are not required to use regional pairs when designing your solutions; however, a resilient business continuity and disaster recovery implementation will use regional pairs to benefit from [isolation](../security/fundamentals/isolation-choices) and improve [availability](../availability-zones/az-overview). For applications that support multiple active regions, both regions in a region pair should be used when possible. Using both regions in a region pair ensures optimal availability for applications and minimized recovery time if there is a disaster. Design your application for maximum resiliency and ease of disaster recovery.
