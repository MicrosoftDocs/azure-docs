...
# Script Action development in HDInsight 

Script actions are scripts that run on the cluster nodes when HDInsight clusters are deployed. They are used to install additional software on a Hadoop cluster or to change the configuration of applications installed on a cluster. The provision process can take a list of script actions to be executed in the order in which they are specified.

>[AZURE.IMPORTANT] Only [HDInsight version 3.1 (Hadoop 2.4)](hdinsight-component-versioning.md/#hdinsight-versions-and-hadoop-components) and above support using Script Action to install custom components on a cluster. In your custom script, you must use the **Get-HDIHadoopVersion** helper method to check the Hadoop version before proceeding with performing other tasks in the script.
...
## Best practices for script action development
...
## Test custom scripts using HDInsight Emulator
