---
title: Update management center support matrix
...
Update management center will scale to all regions for both Azure VMs and Azure Arc-enabled servers. Listed below are the Azure public cloud where you can use update management center.

# [Azure virtual machine](#tab/azurevm)

Update management center is available in all Azure public regions where compute virtual machines are available.

# [Azure Arc-enabled servers](#tab/azurearc)
Update management center is supported in the following regions currently. It implies that VMs must be in below regions:
...
> - Update management center doesn't support CIS hardened images.
...
>
> For the above limitations, we recommend that you use [Automation update management](../automation/update-management/overview.md) till the support is available in Update management center.
...
As the Update management center depends on your machine's OS package manager or update service, ensure that the Linux package manager, or Windows Update client are enabled and can connect with an update source or repository. If you're running a Windows Server OS on your machine, see [configure Windows Update settings](configure-wu-agent.md).
