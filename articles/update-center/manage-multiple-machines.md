...
Instead of performing these actions from a selected Azure VM or Arc-enabled server, you can manage all of your machines in Azure subscription.
...
1. To view update assessment across all machines, including Azure Arc-enabled servers from Update management center (preview), navigate to **Update management center(preview)** and select **Overview** from the left menu.
   ...
   In the **Overview** page, the summary tiles shows the following status:

   - **Updates status of machines** — shows the update status information for assessed machines that had applicable or needed updates. You can filter the results based on classification types. By default, all classifications are selected. 
      ...
      - **Reboot required** — pending a reboot in order for the updates to take effect.
      - **No data available** — no assessment data is available for these machines. 
   
      There could be multiple reasons for no assessment data. No assessment has been done over the last seven days, machine has an unsupported OS, or the machine is in an unsupported region and you can't perform an assessment.

   - **Patch orchestration configuration of Azure virtual machines** — all of the Azure or Arc-enabled machines inventoried in the subscription are summarized by each update orchestration method. Values are: 
   ...
   For more information about each orchestration method see, [automatic VM guest patching](/azure/virtual-machines/automatic-vm-guest-patching#patch-orchestration-modes) article. 

   - **Update installation status** — the tile shows the number of update deployments that have failed and successfully completed by the time range selected. By default, the tile shows the status for last 30 days. Using the **Time** picker, you can choose a different range.

   Use filters to focus on a subset of your resources. The selectors above the tiles return **Subscription**, **Resource group**, **Resource type**, (Azure VMs and Arc-enabled servers) **Location**, and **OS** type (Windows or Linux) based on the Azure role-based access rights you've been granted. You can combine filters to scope to a specific resource. 

- Click any one of the tiles to open the **Machines** page.
- Click the **Update status of machines** or **Patch orchestration configuration of Azure Virtual machines** to go to **Machines** page. 
- Click the **Update installation status**, to go to **History** page. 
...
Update management center (preview) in Azure enables you to browse information about your Azure VMs and Arc-enabled servers across your Azure subscriptions relevant to update management center (preview). The section shows how you can filter information to understand the update status of your machine resources, and for multiple machines, initiate an update assessment, update deployment, and manage their update settings. 
 ...
 In Update management center (preview) page, select **Machines** from the left menu.
   ...
   On the page, the table lists all of the machines in the specified subscription, and for each machine it helps you understand the following details that show up based on the latest assessment.
...
For assessed machines that are reporting updates available, select one or more of the machines from the list and initiate an update deployment that starts immediately. Select the machine and go to **Manage updates** > **One-time updates**. 
...
For the selected machines, confirm if you want to proceed with installing updates on them. Review the information and then select **Install Now** to proceed. Complete the following steps in the **Install one-time updates** wizard to configure your update deployment.

1. In the **Machines** page, verify the machines selected are listed. You can add or remove machines from the list. Select **Next** to continue.
1. In the **Updates** page, specify the updates to include in the deployment by specifying:

   * Include Update classification — 
   * Include KB ID/package — for Windows, you can refer to [MSRC link](https://msrc.microsoft.com/update-guide/deployments) to get the details of latest KBs released.
   * Exclude KB IDs or package — you don’t want to get installed as part of this process. Updates not shown in the list could be installed based on the time between last assessment and release of newer updates since then that are available for the selected machines. You can choose to exclude Windows updates that require a reboot by selecting the option **Exclude KBs requiring reboot for Windows server**.
...
You can create a recurring update deployment by selecting some machines. Select **Manage updates > Scheduled updates**. This opens up the [Create new maintenance configuration](scheduled-patching.md) flow.
...
Provides a summarized status of update and assessment actions performed against your Azure VMs and Arc-enabled servers. You can also drill into a machine specifics to view update-related details and manage it directly, review the detailed update or assessment history for the machine, and other related details in the table. 
   ...
   - The **Status** field reports if an assessment or update deployment is **Failed**, **Successful**, or **InProgress** to indicate the action is underway. 
   - The **Update operation** field reports if the action performed was an **Assessment** or **Install Updates**. 
   - The **Operation type** field reports that the assessment and deployment were manually initiated. 
