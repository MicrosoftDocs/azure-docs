...
- **Patch orchestration**—platform orchestrates the updates for a group of virtual machines across regions or within a region and within a availability set.
...
Update management center (Preview) uses maintenance control schedule instead of using creating its own schedules. Maintenance control enables customers to manage platform updates. For more information, refer [Maintenance control documentation](/azure/virtual-machines/maintenance-control).
...
-  If you remove the patch mode **AutomaticByPlatform** after the configure assignment, then machine is skipped and deployment is marked as *failed with no error message*.
