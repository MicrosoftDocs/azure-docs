...
- For the Azure Update Manager, both AMA and MMA aren't a requirement as it relies on the Microsoft Azure VM Agent for managing patching workflows on Azure VMs and Azure connected machine agent for managing Arc-enabled servers. When you perform an update operation for the first time on a machine, an extension is pushed to the machine and it interacts with the agents to assess missing updates and install updates.
