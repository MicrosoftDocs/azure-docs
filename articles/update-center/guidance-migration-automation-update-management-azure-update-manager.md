---
title: Guidance on migration from Automation Update Management to Azure Update Manager
...
# Guidance on migration from Automation Update Management to Azure Update Manager

**Applies to:** :heavy_check_mark: Windows VMs :heavy_check_mark: Linux VMs :heavy_check_mark: On-premises environment :heavy_check_mark: Azure Arc-enabled servers.

The Azure Log Analytics Agent also known as Microsoft Monitoring Agent (MMA) will be retired in [**August 2024**](https://azure.microsoft.com/updates/were-retiring-the-log-analytics-agent-in-azure-monitor-on-31-august-2024/) and will be replaced by Azure Monitoring Agent (AMA). As a result, Azure Automation Update Management solution that relies on MMA will be impacted. This article provides the updated strategy with [Azure Update Manager](overview.md) that doesn't rely either on MMA or AMA. [Learn more](overview.md#key-benefits).

## Key capabilities

1. For the Azure Update Manager, both AMA and MMA aren't a requirement as it relies on the Microsoft Azure VM Agent for managing patching workflows on Azure VMs and Azure connected machine agent for managing Arc-enabled servers. When you perform an update operation of the first time on a machine, an extension is pushed to the machine and it interacts with the agents to assess missing updates and install updates.
1. All capabilities of Azure Automation Update Management will be available on Azure Update Manager before the deprecation date.
> [!NOTE] 
...
## Migration guidance from Automation Update Management to Azure Update Manager

The following table summarizes the strategy to move to the new Azure Update Manager:

**S.No** | **Capability** | **Automation on Update Management Solution** | **Azure Update Manager** | **Migration using Azure portal** | **Migration using API/script** | 
--- | --- | --- | ---| ---| ---| 
...
6 | Reporting | Custom update reports using Log Analytics queries | Update data is stored in Azure Resource Graph (ARG) free of cost. Customers can query ARG, build custom dashboards, workbooks etc. | The Old Automation Update Management data stored in Log analytics can be accessed, but there's no migration guidance to move data to ARG. You can write fresh ARG queries, build dashboards and workbooks using instructions below: </br> 1. [Log structure of Azure Resource graph updates data](query-logs.md) </br> 2. [Sample ARG queries](sample-query-logs.md) </br> 3. [Create workbooks](manage-workbooks.md) | NA |
...
8 | Alerts (For example, an alert when the update deployment fails or an alert when a security update is available on a group of machines.) | Alerts can be set up on updates data stored in Log Analytics | | |
