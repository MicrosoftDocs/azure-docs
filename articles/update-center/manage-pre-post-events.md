...
> [!NOTE]
> When you select a time to run a schedule, the pre maintenance events is triggered 40 minutes before the specified time. For example, if you select the time as 3:00 p.m. to run the schedule, the pre maintenance event is triggered at 2:20 p.m. (40 mins before the stipulated time.)
