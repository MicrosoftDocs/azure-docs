---
title: Query logs and results from Update management center
description: The article provides details on how you can review logs and search results from update management center in Azure using Azure Resource Graph
...
# Overview of query logs in update management center

Logs created from operations like update assessments and installations are stored by Update management center in an [Azure Resource Graph](../governance/resource-graph/overview.md). The Azure Resource Graph is a service in Azure designed to be the store for Azure service details without any cost or deployment requirements. Update management center uses the Azure Resource Graph to store its results, and you can view the update history of the last 30 days from the resources.
...
Update management center sends the results of all its operation into Azure Resource Graph as logs, which are available for 30 days. Listed below are the structure of logs being sent to Azure Resource Graph.
...
|`rebootBehavior` |Behavior set in the OS update installation runs job when configuring the update deployment if update management center can reboot the target machine. |
...
|`rebootBehavior` |Behavior set in the OS update installation runs job by user, regarding allowing update management center to reboot the OS. |
