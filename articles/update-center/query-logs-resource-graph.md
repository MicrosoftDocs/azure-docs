...
Update management center (preview) sends the results of all its operation into Azure Resource Graph as logs, which are available for 30 days. Listed below are the structure of logs sent to Azure Resource Graph.
...
|`KB ID` |If the machine's OS is Windows Server, the value includes the unique KB ID for the update provided by the Windows Update service.|
...
|`rebootRequired` |Flag to specify if the specific update requires the OS to reboot to complete installation. As provided by machine's OS update service or package manager. If your OS package manager or update service doesn't provide regarding need of OS reboot, then the value of the field will be set of 'false'. |
