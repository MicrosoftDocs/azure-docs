...
The following tables list the Operations or actions that can be completed when the BizTalk Service is in a specific state. A U+2713 means the operation is allowed while in that state. A blank entry means the operation cannot be performed while in that state.
