...
|VM's linked workspace | The MMA agent communicates with the workspace | See [Network requirements](./azure-monitor/agents/agent-windows-troubleshoot.md#connectivity-issues). To validate, check VMs connected workspace or Heartbeat table of corresponding log analytics. </br> Heartbeat where Computer =~ "" </br>|
...
| WSUS | Updates from WSUS | If the environment is set to get updates from WSUS, ensure that the approved in WSUS before the update deployment. For more information, see [WSUS configuration settings](./automation/update-management/configure-wuagent.md#make-wsus-configuration-settings). </br> If your environment is not using WSUS, ensure that you remove the WSUS server settings and [reset Windows Update Component](./windows/deployment/update/windows-update-resources.md#how-do-i-reset-windows-update-components).|
