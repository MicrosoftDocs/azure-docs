...
* The Log Analytics agent for Windows isn't running.
...
On the Log Analytics agent machine, navigate to **C:\Program Files\Microsoft Monitoring Agent\Agent\Tools** and run the following commands:
...
> [!NOTE]
> The Log Analyticss agent enables error tracing by default. To enable verbose error messages as in the preceding example, use the `VER` parameter. For information traces, use `INF` when invoking `StartTracing.cmd`.
...
See [Automate resources in your datacenter or cloud by using Hybrid Runbook Worker](../automation-hybrid-runbook-worker.md#network-planning) to learn about addresses and ports that must be allowed for Change Tracking and Inventory to work.
...
You don't see any Inventory and Change Tracking results for Linux machines that are onboarded for the solution. 
...
The Log Analytics agent for Linux might not be configured correctly for log and command line output collection using the OMS Log Collector tool. See [Track changes in your environment with the Change Tracking and Inventory solution](../change-tracking.md).
