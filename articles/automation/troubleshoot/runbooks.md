...
If your runbook is suspended or unexpectedly fails:
...
Take the following steps to determine if you have authenticated to Azure and have access to the subscription you're trying to select:
...
The subscription context might be lost when invoking multiple runbooks. To ensure that the subscription context is passed to the runbooks, pass the context to the cmdlet in the *AzureRmContext* parameter. Use the **Disable-AzureRmContextAutosave** cmdlet with the **Process** scope to ensure that the specified credentials are only used for the current runbook.
...
This error occurs if the credential asset name isn't valid. This error might also occur if the username and password that you used to set up the Automation credential asset aren't valid.
...
This error is caused when the PowerShell engine can't find the cmdlet you're using in your runbook. It is possible that the module containing the cmdlet is missing from the account, there's a name conflict with a runbook name, or the cmdlet also exists in another module and Automation can't resolve the name.
...
* If you're executing the runbook on-premises in a hybrid worker group, then make sure that the module and cmdlet are installed on the machine that hosts the hybrid worker.
...
* Reduce the number of streams to be retrieved when running the cmdlet. To follow this behavior, you can set the value of the *Stream* parameter for the **Get-AzureRmAutomationJobOutput** cmdlet to retrieve only output streams. ​
...
This error might indicate that runbooks run in an Azure sandbox can't run in the [Full language mode](/powershell/module/microsoft.powershell.core/about/about_language_modes).
...
* If your runbook has this error message, try running it on a Hybrid Runbook Worker.
