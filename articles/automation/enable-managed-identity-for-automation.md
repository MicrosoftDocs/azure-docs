...
- To assign an Azure role to the managed identity, you must have ```Microsoft.Authorization/roleAssignments/write``` permissions, which is granted to either to [User Access Administrator](../role-based-access-control/built-in-roles.md#user-access-administrator) or [Owner](../role-based-access-control/built-in-roles.md#owner).
