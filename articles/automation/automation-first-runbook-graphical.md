...
5. A job blade is opened for the runbook job that we created. Verify that the **Job status** shows **Completed**.
...
11. To view all the jobs for the runbook close the **Job** blade and select **Jobs** under **RESOURCES**. This lists all the jobs created by this runbook. We should only see one job listed since we only ran the job once.
...
2. We don't need the **Write Hello World to output** anymore, so click the ellipses and select **Delete**.
