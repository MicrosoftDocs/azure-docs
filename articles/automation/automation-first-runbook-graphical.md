...
-	Azure subscription. If you don't have one yet, you can [activate your MSDN subscriber benefits](https://azure.microsoft.com/pricing/member-offers/msdn-benefits-details/) or <a href="/pricing/free-account/" target="_blank">[sign up for a free account](https://azure.microsoft.com/en-us/free/).
...
5.	In this case, we're going to create a [graphical runbook](automation-graphical-authoring-intro.md) so select **Graphical** for **Runbook type**.<br> ![New runbook](media/automation-first-runbook-graphical/new-runbook.png)<br>
...
5.	Click **Parameters** to provide values for the cmdlet's parameters.  
	Some cmdlets have multiple parameter sets, and you need to select which you will use. In this case, **Write-Output** has only one parameter set, so you don't need to select one. <br> ![Write-Output properties](media/automation-first-runbook-graphical/write-output-properties-update.png)
...
9.	Save the runbook by clicking **Save**.<br> ![Save runbook](media/automation-first-runbook-graphical/runbook-toolbar-save.png)

## Step 3 - Test the runbook

Before we publish the runbook to make it available in production, we want to test it to make sure that it works properly. When you test a runbook, you run its **Draft** version and view its output interactively.

1.	Click **Test pane** to open the Test blade.<br> ![Test pane](media/automation-first-runbook-graphical/runbook-toolbar-test.png)
...
1.	Click **Publish** to publish the runbook and then **Yes** when prompted.<br> ![Publish](media/automation-first-runbook-graphical/runbook-toolbar-publish.png)
...
4.	We just want to start the runbook so click **Start** and then **Yes** when prompted.<br> ![Start runbook](media/automation-first-runbook-graphical/runbook-controls-start.png)
...
We've tested and published our runbook, but so far it doesn't do anything useful. We want to have it manage Azure resources. Before we setup authentication, we will create a variable to hold the subscription ID and reference it when we setup the activity to authenticate in step 6 below.  Before proceeding, copy your subscription ID from the Subscriptions option off of the Navigation pane.  
...
Now that we have a variable to hold our subscription ID, we can configure our runbook to authenticate using that ID and credentials that are referred to in the [prerequisites](#prerequisites). We do that by adding the **Add-AzureRMAccount** cmdlet. But before we do that, we need to create a runbook input parameter for the credential because it cannot be called directly from the **Add-AzureRMAccount** **Credential** parameter.

1.	Open the graphical editor by clicking **Edit** on the MyFirstRunbook blade.<br> ![Edit runbook](media/automation-first-runbook-graphical/runbook-controls-edit.png)
2.	We don't need the **Write Hello World to output** anymore, so right click it and select **Delete**.
3.	Click **Input and output** and then **Add input** to open the Runbook Input Parameter blade.<br> ![Runbook Input and Output](media/automation-first-runbook-graphical/runbook-toolbar-InputandOutput.png)
4.	Specify *AzureCredentialAssetName* for the **Name**. Keep *string* for the **Type**, and for the **Custom default value** enter your Azure Automation credential. Click **OK**.
5.	In the Library control, expand **Assets**, **Variables** and add **AzureSubscriptionId** to the canvas by selecting **"Add Get Variable" to canvas**.
6.	On the canvas, select **Get-AutomationVariable** and in the Configuration control pane type **Get Subscription ID** in the **Label** textbox.  
7.	In the Library control, type **Get-AutomationPS** in the search textbox.
8.	Add **Get-AutomationPSCredential** to the canvas.<br>![Search Get-AutomationPSCred cmdlet](media/automation-first-runbook-graphical/search-get-automationpscredential-cmdlet.png)
9.	On the canvas, select **Get-AutomationPSCredential** and in the Configuration control pane type **Get Azure Credential** in the **Label** textbox.
10.	Click **Parameters** and the Activity Parameter Configuration blade appears. 
11. In the Activity Parameter Configuration blade, click **Name** and the Parameter Value blade appears.  
12.	 In the Parameter Value blade, select **Runbook input** for the **Data source** and select **AzureCredentialAssetName** from the list and then click **OK** twice.   
13.	Hover over **Get Subscription Id** until a circle appears on the bottom of the shape. Click the circle and drag the arrow to **Get Azure Credential**.
    The arrow that you just created is a *link*. The runbook will start with **Get Subscription ID** and then run **Get Azure Credential**.<br> ![Create link between activities](media/automation-first-runbook-graphical/runbook-link-getsubscription-getcredential.png)
14.	In the Library control, type **Add-AzureRm** in the search textbox.
15. Add **Add-AzureRmAccount** to the canvas.<br> ![Add-AzureRMAccount](media/automation-first-runbook-graphical/search-powershell-cmdlet-addazurermaccount.png)
16.	Hover over **Get Azure Credential** until a circle appears on the bottom of the shape. Click the circle and drag the arrow to **Add-AzureRmAccount**.
17.	Select **Add-AzureRmAccount** and then click **Parameters** in the Configuration control pane.
18.	**Add-AzureRmAccount** has multiple parameter sets, so we need to select one before we can provide parameter values. Click **Parameter Set** and then select the **SubscriptionId** parameter set.
19.	Once you select the parameter set, the parameters are displayed in the Activity Parameter Configuration blade. Click **Credential**.<br> ![Add Azure RM account parameters](media/automation-first-runbook-graphical/add-azurermaccount-parameters.png)
20.	In the Parameter Value blade, select **Activity output** for the **Data source** and select **Get Azure Credential** from the list and then click **OK**.   
21.	Click **SubscriptionId** and in the Parameter Value blade, select **Get Subscription ID** from the list and then click **OK** twice to return to the canvas.
22.	In the Configuration control pane type **Connect to Azure** in the **Label** textbox.

Your runbook should look like the following at this point: <br>![Runbook authentication configuration](media/automation-first-runbook-graphical/runbook-authentication-config.png)
...
3. Hover over **Connect to Azure** until a circle appears on the bottom of the shape. Click the circle and drag the arrow to **Start-AzureRmVM**. 
4.	Select **Start-AzureVM**. Click **Parameters** and then **Parameter Set** to view the sets for **Start-AzureRmVM**. Select the **ResourceGroupNameParameterSetName** parameter set. Note that **ResourceGroupName** and **Name** have exclamation points next them. This indicates that they are required parameters.  Also note both expect string values.
...
Your runbook should look like the following at this point: <br>![Runbook authentication configuration](media/automation-first-runbook-graphical/runbook-startazurermvm.png)
...
2. Click **Input and output** and then **Add input** to open the Runbook Input Parameter pane.<br> ![Runbook Input and Output](media/automation-first-runbook-graphical/runbook-toolbar-InputandOutput.png)
...
9. Create a link from **Connect to Azure** to **Get-AzureRmVM**.
...
16. Create a link from **Get Status** to **Start-AzureRmVM**.<br> ![Runbook with Code Module](media/automation-first-runbook-graphical/runbook-startazurermvm-get-status.png)  
17. Select the link and in the Configuration pane, change **Apply condition** to **Yes**.  Note the link turns to a dashed line indicating that the target activity will only be run if the condition resolves to true.  
18. For the **Condition expression**, type *$ActivityOutput['Get Status'] -eq "Stopped"*. **Start-AzureRmVM** will now only run if the virtual machine is stopped.
19.	In the Library control, expand **Cmdlets** and then **Microsoft.PowerShell.Utility**.
20.	Add **Write-Output** to the canvas twice.<br> ![Runbook with Write-Output](media/automation-first-runbook-graphical/runbook-startazurermvm-final.png)
21. On the first **Write-Output** control, click **Parameters** and change the **Label** value to *Notify VM Started*.
...
25. Create a link from **Start-AzureRmVM** to **Notify VM Started** and **Notify VM Start Failed**.
26. Select the link to **Notify VM Started** and change **Apply condition** to **True**.
27. For the **Condition expression**, type *$ActivityOutput['Start-AzureRmVM'].IsSuccessStatusCode -eq $true*. This Write-Output control will now only run if the virtual machine is successfully started.
28. Select the link to **Notify VM Start Failed** and change **Apply condition** to **True**.
29. For the **Condition expression**, type *$ActivityOutput['Start-AzureRmVM'].IsSuccessStatusCode -ne $true*. This Write-Output control will now only run if the virtual machine is not successfully started.
30.	Save the runbook and open the Test pane.
31.	Start the runbook with the virtual machine stopped, and it should start.
