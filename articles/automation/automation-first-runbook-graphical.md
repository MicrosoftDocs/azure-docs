...
1. Select the *InputObject* parameter. This is the parameter that you use specify the text to send to the output stream.
...
1. When the runbook job completes, the Test pagn displays its output. In this case, you see "Hello World".<br> ![Hello World](media/automation-first-runbook-graphical/runbook-test-results.png)
...
1. A Job pane is opened for the runbook job that has been created. Verify that the **Job status** shows Completed.
...
You've tested and published your runbook, but so far it doesn't do anything useful to manage Azure resources. Before you configure the runbook to authenticate, you must create a variable to hold the subscription ID, set up an activity to authenticate, and then reference the variable. Including a reference to the subscription context allows you to easily work with multiple subscriptions.
...
1. Select **Set-AzContext** on the canvas. In the Configuration control pane, type "Specify Subscription Id" in the **Label** field.
1. Click **Parameters** and the Activity Parameter Configuration page appears.
...
Now you must add a **Start-AzVM** activity to start a virtual machine. You can pick any VM in your Azure subscription, and for now you are hardcoding that name into the [Start-AzVM](https://docs.microsoft.com/powershell/module/az.compute/start-azvm?view=azps-3.5.0) cmdlet.
...
1. Select the **ResourceGroupNameParameterSetName** parameter set. The fields **ResourceGroupName** and **Name** have exclamation points next to them to indicate that they are required parameters. Note that both fields expect string values.
1. Select **Name**. Select **PowerShell expression** for the **Data source** field. For the VM that you use to start this runbook, type in the machine name surrounded with double quotes. Click **OK**.
...
1. Click **Start** to start the test. Once it completes, make sure that the VM has started. 
...
1. Select **Get-AzVM** and then **Parameter Set** to view the sets for **Get-AzVM**. 
1. Select the GetVirtualMachineInResourceGroupNameParamSet parameter set. The **ResourceGroupName** and **Name** fields have exclamation points next to them, indicating that they specify required parameters. Note that both fields expect string values.
...
1. In the code editor, paste the following code snippet.
