...
If auto sync was set when configuring source control integration, the initial sync would start automatically. If auto sync was not set, select the source from the table on the **Source control (Preview)** page. Click **Start Sync** to start the sync process.  
...
Clicking on a job allows you to view the job output. The following is an example of the output from a source control sync job.
