...
This article describes how to update Azure PowerShell (Az) modules in Azure Automation. The Az PowerShell module is a set of cmdlets for managing Azure resources directly from PowerShell. Azure Automation provides common PowerShell modules by default in each Automation account. For more information, see [Default modules](shared-resources/modules.md#default-modules). As the Azure team updates the Azure modules regularly, changes can occur with the included cmdlets. For example, renaming a parameter or deprecating a cmdlet entirely. These changes can affect existing runbooks and automation processes if the modules aren't updated accordingly. To help manage these updates, Azure Automation provides two methods to update Az modules:

- [Update Az modules through the Azure portal](#update-az-modules-through-the-azure-portal)
- [Update Az modules through a runbook](#update-az-modules-through-runbook)

> [!NOTE]
> You can't delete global modules, which are modules that Automation provides out of the box.
> [!IMPORTANT]
> New Runtime environment experience allows you to manage modules and packages by allowing you to configure the job execution environment. In the new experience, Modules and Packages blades aren't available. To manage modules and packages, see [Manage Runtime environment and associated runbooks](manage-runtime-environment.md).
...
If you develop your scripts locally, it's recommended to have the same module versions locally that you have in your Automation account when testing to ensure that you receive the same results. After the results are validated and you apply any changes required, you can move the changes to production.
...
The following sections explain how you can update Az modules either through the **portal** (recommended) or through the runbook.

### Update Az modules through the Azure portal 
...
1. Sign in to the [Azure portal](https://portal.azure.com) and go to your Automation account.
1. Under **Shared Resources**, select **Modules**.
1. Select **Update Az modules**.
1. Select **Module to Update**. By default, it shows the Az module.  
1. From the drop-down list, select **Module Version** and **Runtime version**.
1. Select **Update** to update the Az module to the version that you selected.
...
The Azure team regularly updates the module version and provides an option to update the **default** Az modules by selecting the module version from the drop-down list.  
...
The **Update-AutomationAzureModulesForAccount** runbook supports updating the Azure, AzureRM, and Az modules by default. Review the [Update Azure modules runbook README](https://github.com/microsoft/AzureAutomation-Account-Modules-Update/blob/master/README.md) for more information on updating Az. Automation modules with this runbook. There are other important factors that you need to take into account when using the Az modules in your Automation account. To learn more, see [Manage modules in Azure Automation](shared-resources/modules.md).

#### Use the update runbook code as a regular PowerShell script

You can use the runbook code as a regular PowerShell script instead of a runbook. To do this, sign in to Azure by using the [Connect-AzAccount](/powershell/module/az.accounts/connect-azaccount) cmdlet first, then pass `-Login $false` to the script.

#### Use the update runbook on sovereign clouds
To use this runbook on sovereign clouds, use the `AzEnvironment` parameter to pass the correct environment to the runbook. Acceptable values are AzureCloud (Azure public cloud), AzureChinaCloud, AzureGermanCloud, and AzureUSGovernment. You can retrieve these values by using `Get-AzEnvironment | select Name`. If you don't pass a value to this cmdlet, the runbook defaults to AzureCloud.
...
* For details about using modules, see [Manage modules in Azure Automation](shared-resources/modules.md).
