<properties
	...
	description="Tutorial that walks you through the creation, testing, and publishing of a simple text runbook using PowerShell.  Several concepts are covered such as authenticating to Azure resources and input parameters."
	services="automation"
	documentationCenter=""
	authors="SnehaGunda"
	manager="stevenka"
...
2. Click on the **Runbooks** tile to open the list of runbooks.  
    ![Runbooks Control](media/automation-first-runbook-textual-PowerShell/automation_01_RunbooksControl.png)  
...
5. In this case, we're going to create a [PowerShell runbook](automation-runbook-types.md#powershell-runbooks) so select **Powershell** for **Runbook type**.  
    ![Runbook Type](media/automation-first-runbook-textual-PowerShell/automation_02_RunbookType.png)  
...
1. Our runbook is currently empty, type *Write-Output "Hello World."*.  
    ![Hello World](media/automation-first-runbook-textual-PowerShell/automation_03_HelloWorld.png)  
2. Save the runbook by clicking **Save**.  
    ![Save Button](media/automation-first-runbook-textual-PowerShell/automation_04_SaveButton.png)  
...
1. Click **Test pane** to open the Test pane.  
    ![Test Pane](media/automation-first-runbook-textual-PowerShell/automation_05_TestPane.png)  
...
4. When the runbook job completes, its output is displayed.  In our case, we should see *Hello World*  
    ![Test Pane Output](media/automation-first-runbook-textual-PowerShell/automation_06_TestPaneOutput.png)  
...
1. Click **Publish** to publish the runbook and then **Yes** when prompted.  
    ![Publish button](media/automation-first-runbook-textual-PowerShell/automation_07_PublishButton.png)  
...
4. We just want to start the runbook so click **Start** and then click **Ok** when the Start Runbook blade opens.  
    ![Start button](media/automation-first-runbook-textual-PowerShell/automation_08_StartButton.png)  
...
6. The job status is shown in **Job Summary** and matches the statuses that we saw when we tested the runbook.  
    ![Job Summary](media/automation-first-runbook-textual-PowerShell/automation_09_JobSummary.png)  
7. Once the runbook status shows *Completed*, click **Output**.  The Output pane is opened, and we can see our *Hello World*.  
    ![Job Output](media/automation-first-runbook-textual-PowerShell/automation_10_JobOutput.png)
...
9. Click **All Logs** to open the Streams pane for the runbook job.  We should only see *Hello World* in the output stream, but this can show other streams for a runbook job such as Verbose and Error if the runbook writes to them.  
    ![All Logs](media/automation-first-runbook-textual-PowerShell/automation_11_AllLogs.png)  
...
11. Click **Jobs** to open the Jobs pane for this runbook. This lists all of the jobs created by this runbook.  We should only see one job listed since we only ran the job once.  
    ![Job List](media/automation-first-runbook-textual-PowerShell/automation_12_JobList.png)  
...
1. Open the textual editor by clicking **Edit** on the MyFirstRunbook-PowerShell pane.  
    ![Edit Runbook](media/automation-first-runbook-textual-PowerShell/automation_13_EditRunbook.png)  
...
6. On the next line, type *Add-AzureAccount -Credential $Credential*.   
    ![Credential](media/automation-first-runbook-textual-PowerShell/automation_14_GetCredential.png)
...
1. After *Add-AzureAccount*, type *Start-AzureVM -Name 'VMName' -ServiceName 'VMServiceName'* providing the name and service name of the virtual machine to start.  
    ![StartVM](media/automation-first-runbook-textual-PowerShell/automation_15_StartVM.png)  
...
1. Add parameters for *VMName* and *VMServiceName* to the runbook and use these variables with the **Start-AzureVM** cmdlet as in the following image.  
    ![Add Parameter](media/automation-first-runbook-textual-PowerShell/automation_16_AddParameter.png)  
...
6. Click **Start** to start the runbook. Type in the **VMName** and **VMServiceName** for the virtual machine that you're going to start.  
    ![Pass Parameter](media/automation-first-runbook-textual-PowerShell/automation_17_PassParameter.png)  
