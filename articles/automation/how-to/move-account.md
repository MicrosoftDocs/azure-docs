...
> [!NOTE]
> If you do not see any alert rules on the **Rules** page, change the **Status** to show **Disabled** alerts, as you might have disabled them.

When the alert rules are removed, remove the action group that was created for the Start/Stop VM solution notifications.

In the Azure portal, select **Monitor** > **Alerts** > **Manage action groups**.

Select **StartStop_VM_Notification** from the list. On the action groups page, select **Delete**.
...
![Run As Accounts show as incomplete](../media/move-account/run-as-accounts.png)
...
To turn on the Start/Stop VMs during off-hours solution, you'll need to redeploy the solution. Under **Related Resources**, select **Start/Stop VM** > **Learn more about and enable the solution** > **Create** to start the deployment.
