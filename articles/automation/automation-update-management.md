...
At the date and time specified in the update deployment, the target computers executes the deployment in parallel. A scan is first performed to verify the updates are still required and installs them. It is important to note for WSUS client computers, if the updates are not approved in WSUS, the update deployment fails. The results of the applied updates are forwarded to Log Analytics to be processed and summarized in the dashboards or by the searching the events.     
...
* Windows agents are configured to communicate with a Windows Server Update Services (WSUS) server or have access to Microsoft Update.  
...
The Update Management solution creates two types of records in the Log Analytics workspace, Update and UpdateSummary records.
...
| Classification for Linux |Classification of the update.<br>Possible values are:<br>-Critical Updates<br>- Security Updates<br>- Other Updates |
...
| BulletinURL | URL providing additional information on the update|
...
| WindowsUpdateSetting |Setting for how the computer installs important updates.<br>Possible values are:<br>- Disabled<br>- Notify before installation<br>- Scheduled installation |
...
| --- | --- |
|
Update<br>&#124; where OSType != "Linux"and UpdateState == "Needed"<br>&#124; where Optional == false and Approved != false<br>&#124; summarize AggregatedValue = count() by Computer | Windows-based server computers that need updates |
| Update<br>&#124; where OSType == "Linux" and UpdateState != "Not needed"<br>&#124; summarize AggregatedValue = count() by Computer |Linux servers that need updates | 
|Update<br>&#124; where UpdateState == "Needed" and Optional == false<br>&#124; project Computer, Title, KBID, Classification, PublishedDate |All computers with missing updates |
...
| Update<br>&#124; where UpdateState == "Needed" and Optional == false<br>&#124; where (Classification == "Security Updates" or Classification == "Critical Updates") |All computers with missing critical or security updates | 
...
| Update<br>&#124; where Optional == false and Classification == "Update Rollups"<br>&#124; where UpdateState == "Needed"<br>&#124; project Computer, Title, KBID, Classification, PublishedDate |All computers with missing update rollups | 
| Update<br>&#124; where UpdateState == "Needed" and Optional == false<br>&#124; distinct Title |Distinct missing updates across all computers | 
| UpdateRunProgress<br>&#124; where InstallationStatus == "failed" and OSType == "Windows" <br>&#124; summarize AggregatedValue = count() by Computer, Title, UpdateRunName |Windows-based server computer with updates that failed in an update run | 
| UpdateRunProgress<br>&#124; where InstallationStatus == "failed" and OSType == "Linux" <br>&#124; summarize AggregatedValue = count() by Computer, Product, UpdateRunName |Linux server with updates that failed an update run | 
| UpdateSummary<br>&#124; summarize AggregatedValue = count() by WSUSServer |WSUS computer membership | 
| UpdateSummary<br>&#124; summarize AggregatedValue = count() by WindowsUpdateSetting |Automatic update configuration | 
| UpdateSummary<br>&#124; where WindowsUpdateSetting == "Manual" |Computers with automatic update disabled | 
| Update<br>&#124; where OSType == "Linux" and UpdateState != "Not needed"<br>&#124; summarize AggregatedValue = count() by Computer |List of all the Linux machines, which have a package update available | 
...
| Update<br>&#124; where OSType == "Linux" and UpdateState != "Not needed" |List of all packages that have an update available | 
| Update<br>&#124; where OSType == "Linux" and UpdateState != "Not needed"<br>&#124; where (Classification == "Critical Updates" or Classification == "Security Updates") |List of all packages that have an update available, which addresses Critical or Security vulnerability | 
| UpdateRunProgress<br>&#124; summarize AggregatedValue = count() by UpdateRunName |List what update deployments have modified computers | 
...
| Update<br>&#124; where OSType == "Linux" and OSName == "Ubuntu"<br>&#124; summarize AggregatedValue = count() by Computer |List of all the “Ubuntu” machines with any update available |
