...
Here we compose the policy rule. If you aren't yet familiar with the policy language, reference [policy definition structure](../governance/policy/concepts/definition-structure.md) for how to structure the policy definition.
...
The definition can be used to create a new policy in the Azure portal, the Azure CLI, Azure PowerShell, and REST API. Each method accepts the definition in different ways, so review the commands for each to validate correct usage. 