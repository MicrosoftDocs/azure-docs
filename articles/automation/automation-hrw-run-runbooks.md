...
>[!NOTE]
>  A user with VM Contributor rights or having permissions to run commands against the hybrid worker machine can use the Automation Account **Run As** certificate from the Hybrid Worker Machine. The other sources like Azure cmdlets could potentially allow malicious user access as a subscription Contributor and that could jeopardize the security of your Azure environment.
