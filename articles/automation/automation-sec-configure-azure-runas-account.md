...
After the script completes successfully, if you created a Classic Run As account, follow the steps to [upload management API certificate](../azure-api-management-certs.md) to the Azure classic portal.  If you created a Classic Run As account with self-signed public cert (.cer format), you can find the a copy of the certificate created in the temporary files folder on your computer under the user profile used to execute the PowerShell session - *%USERPROFILE%\AppData\Local\Temp*.  Otherwise, if you configured the Classic Run As account to use a certificate generated by your enterprise CA (.cer format), you will need to use this certificate.  Once the certificate is uploaded, refer to the [sample code](#sample-code-to-authenticate-with-service-management-resources) to validate credential configuration with Service Management resources.  
