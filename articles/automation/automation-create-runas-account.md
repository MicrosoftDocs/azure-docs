...
You can update your existing Automation account from the portal or use PowerShell if:
...
* The script can be run only on Windows 10 and Windows Server 2016 with Azure Resource Manager modules 3.0.0 and later. It is not supported on earlier versions of Windows.
...
* Your AD user account needs to be added to a role with permissions equivalent to the Contributor role for Microsoft.Automation resources as outlined in article [Role-based access control in Azure Automation](automation-role-based-access-control#contributor-role-permissions).  
