...
> [!NOTE]
> Before you install this version (v1), we would like you to know about the [next version](../azure-functions/start-stop-vms/overview.md), which is has gone to General Availability (GA). This new version (v2) offers all the same functionality as this one, but is designed to take advantage of newer technology in Azure. It adds some of the commonly requested features from customers, such as multi-subscription support from a single Start/Stop instance. 
>
> Since the new version (v2) is no generally available (GA), Start/Stop VMs during off-hours (v1) will be deprecated and the date will be announced soon. 
