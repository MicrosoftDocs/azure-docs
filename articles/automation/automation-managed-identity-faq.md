...
Automation Run As account would be supported for the next three years until Sept 14, 2025. While we continue to support for exisiting users, we recommend all new users to use Managed Idenetities as the preferred way of runbook authentication. Existing users can see the Run As account properties and renew the certificate upon expiration, but users will not be able to create a new Run As account from the Azure portal. Users could still create a new Run As account through [PowerShell script](/azure/automation/create-run-as-account#create-account-using-powershell) until the supported time.
...
In Run As accounts, you must manage credentials (certificate). However, in Managed Identity, you don't have to do it.
...
Managed identities eliminate this overhead by providing a secure method for the users to authenticate & access resources that supports Azure AD authentication via the managed identity of an Automation account without worrying about any certificate or credential management.
...
Yes, you can audit and enforce the Managed identities through Azure Policy. Follow the steps.
