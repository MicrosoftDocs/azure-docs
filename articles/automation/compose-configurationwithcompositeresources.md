...
resources](/powershell/dsc/authoringresourcecomposite.md). A composite resource is a nested and
...
Azure Automation enables the [import and compilation of composite resources](automation-dsc-compile.md#composite-resources).
Once composite resources have been imported into your Automation account, you will be able to use the
**Compose configuration** experience in the **State Configuration (DSC)** page.
...
compose it. This can be done using **Compose configuration** on the **State Configuration (DSC)** page
while on either the **Configurations** or **Compiled configurations** tabs.
...
1. On the **State configuration (DSC)** page, click either the **Configurations** or **Compiled configurations** tab, then click **Compose configuration** in the menu at the top of the page.
   ![Screenshot of the compose configuration page](./media/compose-configurationwithcompositeresources/compose-configuration-basics.png)
...
1. The **Source code** step will show what the configuration will look like that combines each of the composite resources you selected in the previous step. You can see the merging of all parameters and how they are passed to the composite resource. When you are done reviewing the new source code, click **Next** or click the **Parameters** step.
1. On the **Parameters** step, the parameter that each composite resource has will be exposed so that they can be provided. If a parameter has a description, it will be displayed next to the parameter field. If a field is a **PSCredential** type parameter, the drop-down to configure will provide a list of **Credential** objects in the current Automation account. A **+ Add a credential** option will also be available. Once all required parameters have been provided, click **Save and compile**.

Once the new configuration is saved, it will be submitted for compilation. Status of the
...
When compilation has completed successfully, the new configuration will appear in the **Compiled
configurations** tab. Once it is visible in this tab, it can be assigned to a managed node using
the steps in [Reassigning a node to a different node configuration](automation-dsc-getting-started.md#reassigning-a-node-to-a-different-node-configuration).
