...
Process Automation supports automation in Azure with the help of runbooks. A runbook is a set of tasks that perform some automated process in Azure Automation.  It may be a simple process such as starting a virtual machine or creating a log entry, or you may have a complex runbook that combines other smaller runbooks to perform a complex process across multiple resources or even multiple clouds. 
...
#### Scope of Automation with runbooks
...
### Configuration Management (Desire State Configuration)

Configuration Management supports automation in Azure with the help of Desire State Configuration. DSC is a management platform which allows you to manage and deploy configuration for physical hosts and virtual machines using a declarative PowerShell syntax. Azure Automation DSC allows you to manage Windows or Linux VMs, physical hosts in any cloud or on premise environments. You can manage, author, compile, import DSC resources and easily onboard VMs to PowerShell DSC using Azure Automation DSC. 
For example, if there are thousands of virtual machines where you need to install or remove server roles and features, instead of configuring each machine, you can define a configuration to do that and Azure Automation places it in a central location called pull server. All the virtual machines can be pointed to the pull server, now if any of those VMs drifts out of configuration, then it pulls the configuration from this Pull server and ends up being in the desired state. Also as applications demand change, you may have to update these resources, and all you do is change the configuration and Azure Automation DSC identifies that the configuration was changed and updates those VMs to match the new configuration.


#### Creating DSC with Azure Automation

You can create a configuration declaratively using any editor on your local machine and import it into Azure Automation. Support for editing a configuration in Azure Automation will be added soon. Compile the configuration after its imported, any node configurations/MOF files generated while compiling will be automatically placed on the Azure Automation DSC pull server and checksum for these MOF files is generated. Azure Automation provides you with the required infrastructure to instantiate DSC pull server and its environment. If node configurations with the same name exist on the pull server, they will be overwritten. To know more about nodes and resources in DSC please refer to [Azure Automation DSC overview](automation-dsc-overview.md)
All the nodes/VMs having outbound access to the pull server can pull the configuration and make sure that they are in desired state. To know more about registering a virtual machine in cloud or on-premises please refer to [Onboarding machines for management by Azure Automation DSC](automation-dsc-onboarding.md)


#### DSC resource Kit

The [DSC Resource Kit](http://www.powershellgallery.com/packages) is a collection of experimental DSC Resources released by the PowerShell team, designed to help you get started with DSC. There are resources that allow you to configure Active Directory, SQL Server, IIS, and many more. To import a DSC resource to Azure Automation, navigate to the resource you want and click “Deploy to Azure Automation”. You can create your own DSC resources or use the existing ones and customize them for your own purposes. You can even contribute your own DSC resources to the gallery that you think other users may find useful.
...
* Copy files from a local Machine to Azure Blob Storage container
* Protecting a VM against Denial-of-Service attacks
* Building an Active Directory Forest in Azure
* Truncating table in SQL DB if DB is approaching Max size
* Back up an Azure Virtual Machine using Azure Automation
* Backup Runbooks to a storage account
* Deploying new software
* Monitoring Azure websites to alert the user of any issues
...
[PowerShell DSC](https://technet.microsoft.com/library/dn249912.aspx) is intended for managing fewer machines,  Azure Automation DSC is preferable if you are working to configure machines as part of larger processes like in production servers. PowerShell DSC can operate in both push and pull modes and in pull mode, you have to setup your pull server whereas Azure Automation provides you with the required infrastructure to instantiate DSC pull server and its environment. So Azure Automation DSC provides an easier configuration management experience over PowerShell DSC.

[ConfigMgr DCM in SCCM](https://technet.microsoft.com/library/hh528537.aspx) is intended if you are managing machines on-premises, in a stable environment with unique configuration for each server whereas Azure Automation DSC is preferable when working with a rapidly changing environment.
