...
5. Preview the action and make any necessary changes before implementing against production VMs. When ready, manually execute the **monitoring-and-diagnostics/monitoring-action-groupsrunbook** with the parameter set to **False**. Alternatively, let the Automation schedule `Sequenced-StartVM` and `Sequenced-StopVM` run automatically following your prescribed schedule.
...
By default, the solution is pre-configured to evaluate the percentage CPU metric to see if average utilization is 5 percent or less. This scenario is controlled by the following variables and can be modified if the default values don't meet your requirements:
...
When you run the **AutoStop_CreateAlert_Parent** runbook, it verifies that the targeted subscription, resource group(s) and VMs exist. If the VMs exist, the runbook then calls the **AutoStop_CreateAlert_Child** runbook for each verified VM by the parent runbook. This child runbook performs the following:

* Creates a metric alert rule for each verified VM.

* Triggers the **AutoStop_VM_Child** runbook for a particular VM if the CPU drops below the configured threshold for the specified time interval. This runbook then attempts to stop the VM.
...
### To target the auto stop action against all VMs in a resource group or multiple resource groups

1. Add a comma-separated list of resource group names to the `External_Stop_ResourceGroupNames` variable.

2. If you want to exclude some of the VMs from the auto shutdown, you can add a comma-separated list of VM names to the `External_ExcludeVMNames` variable.
...
To change email notifications after the solution is deployed, modify the action group created during deployment.  

> [!NOTE]
> Subscriptions in the Azure Government Cloud don't support the email functionality of this solution.
...
* To select multiple VMs, set `External_Start_ResourceGroupNames` and `External_Stop_ResourceGroupNames` with the resource group names that contain the VMs you want to start or stop. You can also set the variables to a value of `*` to have the solution run against all resource groups in the subscription.
...
Configuring the solution to just stop VMs at a certain time is supported. In this scenario you just create a **Stop** schedule and no corresponding **Start** schedule. To do this, you need to:
