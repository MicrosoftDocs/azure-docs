...
1. On the left menu, select **Inventory** under **Operations**. The Inventory page opens.

![Enable change](./media/automation-tutorial-troubleshoot-changes/enableinventory.png)

Configure the location, Log analytics workspace, and Automation account to use and click **Enable**. If the fields are grayed out, that means another automation solution is enabled for the VM and the same workspace and Automation account must be used.
...
During onboarding, the VM is provisioned with the Log Analytics agent for Windows and a Hybrid Runbook Worker.
The agent is used to communicate with the VM and obtain information about installed software.
...
To search the logs by running queries, select **Log Analytics** at the top of the Change tracking page.
...
On the Workspace Configuration page, add the Windows Registry keys, Windows files, or Linux files to be tracked, as outlined in the next three sections.
...
From the Change tracking page on your VM, select **Manage Activity Log Connection**. This task opens the Azure Activity log page. Click **Connect** to connect Change Tracking to the Azure activity log for your VM.
...
Click **Select** to choose a resource. On the Select a resource page, select **Log Analytics** from the **Filter by resource type** dropdown menu. Select your Log Analytics workspace, and then select **Done**.
...
In the Email/SMS/Push/Voice pane, enter a name. Select the **Email** check box, and then enter a valid email address. Click **OK** on the pane, then click **OK** on the Add action group page.
