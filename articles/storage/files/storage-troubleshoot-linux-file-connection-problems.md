...
- Setting the file size in advance, instead of making every write an extending write, helps improve copy speed in scenarios where the file size is known in advance. If extending writes need to be avoided, you can allocate a destination file size in advance with `truncate - size <size><file>` command. After that, `dd if=<source> of=<target> bs=1M conv=notrunc`command will copy a source file without metadata updates of a target file. For example, you can allocate destination files for every file you want to copy (assume a share is mounted under /mnt/share):
