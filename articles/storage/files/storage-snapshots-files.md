...
A share snapshot is a point-in-time, read-only copy of your data. You can create, delete, and manage snapshots by using the REST API. The same capabilities are also available in the client library, Azure CLI, and Azure portal. 
...
Share snapshot capability is provided at the file share level. Retrieval is provided at the individual file level, to allow for restoring individual files. You can restore a complete file share by using SMB, the REST API, the Azure portal, the client library, or PowerShell/CLI.
