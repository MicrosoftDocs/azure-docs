...
| `Clean` | n/a | A non-TLS mount may fail if a prior TLS mount to the same server ended abruptly, leaving stale entries. To resolve this issue, remount the share using the clean option, which immediately clears any stale entries. This applies only for AZNFS mount. |
