...
1	**Initial one-time provisioning**: When you first setup Azure File Sync groups and you ingest the server data into Azure File shares. Please refer to [Onboarding with Azure File Sync](storage-sync-files-planning.md) for details.
...
-	The Agent File Sync agent’s object throughput approximately scales in proportion to the number of sync groups on the server. Splitting data into multiple sync groups on a server yields better throughput, which is also limited by the server and network.
