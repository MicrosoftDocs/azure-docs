---
title: Azure Files Scale and Performance Targets
...
Two types of limits apply to storage accounts and classic file shares:

- Control plane limits, which the `Microsoft.Storage` resource provider enforces. These limits apply to management requests such as creating, updating, or deleting the storage account or other child resources, including but not limited to classic file shares.

- Data plane limits, which the Azure storage platform enforces. These limits apply to operations like creating and deleting files and folders through SMB, NFS, FileREST, and other protocols. For legacy reasons, some management operations, like creating, updating, or deleting classic file shares, are also available through the data plane (FileREST protocol). `Microsoft.Storage` limits don't apply to management requests that you make directly to the Azure storage platform.
...
Storage accounts have slightly different limits depending on the SKU and kind of the storage account. The SKU of the storage account is a combination of the media tier, the iteration of the billing model, and redundancy. The kind of the storage account is an additional modifier that determines which storage services, features, and billing models it supports. For classic file shares, there are four combinations:

- **SSD provisioned v2 storage accounts**, which are represented by the `FileStorage` storage account kind and the `PremiumV2_LRS` or `PremiumV2_ZRS` storage account SKUs. These storage accounts can contain only classic file shares and can't be used to deploy other storage resources such as blob containers, queues, or tables. Classic file shares deployed in these storage accounts are always on the SSD media tier and billed using the provisioned v2 billing model.

- **HDD provisioned v2 storage accounts**, which are represented by the `FileStorage` storage account kind and the `StandardV2_LRS`, `StandardV2_ZRS`, `StandardV2_GRS`, or `StandardV2_GZRS` storage account SKUs. These storage accounts can contain only classic file shares and can't be used to deploy other storage resources such as blob containers, queues, or tables. Classic file shares deployed in these storage accounts are always on the HDD media tier and billed using the provisioned v2 billing model.

- **SSD provisioned v1 storage accounts**, which are represented by the `FileStorage` storage account kind and the `Premium_LRS` or `Premium_ZRS` storage account SKUs. These storage accounts can contain only classic file shares and can't be used to deploy other storage resources such as blob containers, queues, or tables. Classic file shares deployed in these storage accounts are always on the SSD media tier and billed using the provisioned v1 billing model.
...
> [!NOTE]  
> Although you can deploy classic file shares into storage accounts with the `Standard_RAGRS` or `Standard_RAGZRS` storage account SKUs, Azure Files doesn't support read-accessibility mode for geo-redundant storage accounts. These classic file shares implicitly use the `Standard_GRS` or `Standard_GZRS` storage account SKUs. Other storage resources, such as blob containers, do support read-accessibility mode, and can be intermingled in these storage accounts.
...
File scale targets apply to individual files stored in classic file shares. Your ability to reach the limits on an individual file is subject to the limits of the classic file share and of the storage account in which it resides.
...
- Control plane limits, which the `Microsoft.FileShares` resource provider enforces. These limits apply to management requests such as creating, updating, or deleting the file share or child resources such as file share snapshots.

- Data plane limits, which the Azure storage platform enforces. These limits apply to actions like creating and deleting files and folders through the NFS file sharing protocol.
...
<sup>1</sup> `Microsoft.FileShares` uses a similar throttling algorithm for management requests as Azure Resource Manager itself uses. API throttling is managed by using a [token bucket algorithm](https://en.wikipedia.org/wiki/Token_bucket). The token bucket represents the maximum number of requests that you can send for each second. When you reach the maximum number of requests, the refill rate determines how quickly new requests are added to the bucket.
...
| Minimum provisioned IOPS size | 3,000 IOPS |
...
<sup>1</sup> Metadata operations are operations that manipulate file handles, such as opening a file or folder. The maximum metadata IOPS limit specifies the maximum IOPS that metadata can use, regardless of the amount of provisioned IOPS a file share has.
