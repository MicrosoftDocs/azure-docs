...
This tutorial demonstrates the basics of using .NET to develop applications or services that use [Azure Files](storage-files-introduction.md) to store file data. In this tutorial, we create a simple console application and show how to perform basic actions with .NET and Azure Files:
...
It's important to understand which .NET APIs should be used from your application when using Azure Files.  Azure Files provides two broad approaches to client applications, SMB and REST.  Within .NET, these approaches are abstracted by the `System.IO` and `WindowsAzure.Storage` APIs.

API | When to use | Notes
----|-------------|------
[System.IO](https://docs.microsoft.com/dotnet/api/system.io) | Your application: <ul><li>Needs to read/write files via SMB</li><li>Is running on a device that has access over port 445 to your Azure Files account</li><li>Doesn't need to manage any of the administrative settings of the file share</li></ul> | The code for file I/O with Azure Files over SMB is generally the same for I/O with any local network file share or local storage devices. See [this tutorial](https://docs.microsoft.com/dotnet/csharp/tutorials/console-teleprompter) for an introduction to a number of features in .NET, including file I/O.
