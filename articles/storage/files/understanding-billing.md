...
description: Azure Files provides 4 tiers of storage to help you optimize your storage costs - but how do you choose the correct one to save the most money? Learn how to intrepret Azure billing information and telemetry to choose the ideal tier of storage.
author: roygara
ms.service: storage
ms.topic: how-to
ms.date: 12/1/2020
...
Azure Files provides two distinct business models: provisioned and pay-as-you-go. The provisioned model is only available for premium file shares, which are file shares deployed in the **FileStorage storage account** kind. The pay-as-you-go model is only available for standard file shares, which are file shares deployed in the **general purpose version 2 (GPv2) storage account** kind. This article explains how both models work with the objective of helping you understanding your monthly Azure Files bill.
...
We have decided with Azure Files to use the correct label for the units that we actually measure in because: 
1. Azure Files entered the market *after* the standardization of these units.
2. As a service, Azure Files bills you directly for storage you use rather than on hardware you bought from someone else (like Windows).
3. It's good to use the correct terminology and notation.
...
## Provisioned billing
In a provisioned business model, you proactively specify to the Azure Files service what your storage requirements are, rather than being billed reactively based on what you actually use. This is similar to buying hardware on-premises, in that when you provision an Azure file share with a certain amount of storage, you pay for that storage regardless of whether you use it or not, just like you don't start paying the costs of physical media on-premises when you start to use space. Unlike purchasing physical media on-premises, provisioned file shares can be dymanically scaled up or down depending your storage and IO performance characteristics.

When you provision a premium file share, you specify how many GiBs you workload requires. Each GiB that you provision entitles you to additional IOPS and throughput on a fixed ratio. In addition to the baseline IOPS for which you are guaranteed, each premium file share supports bursting on a best effort basis. The formulas for IOPS and throughput are as follows:
...
Straightforwardly, if you put an infrequently accessed workload in the transaction optimized tier, you will pay almost nothing for the few times in a month that you do transactions against you share, but you will pay a high amount for the data storage costs. If you were to move your share to the cool tier, you would still pay almost nothing for the transaction costs, simply because you are very infrequently doing any transactions for this workload, but the cool tier has a much cheaper data storage price, which enables you to reduce your costs considerably.

Conversely, if you put a highly accessed workload in the cool tier, you will pay a lot more in transaction costs, but a lot less on the data storage costs. This can lead to a situation where the increased costs from the transaction prices increase outweigh the savings from the decreased data storage price, leading you to pay more money on cool than you would have on transaction optimized. This effect can also lead to situations where hot is the most cost efficient tier, but cool is more expensive than transaction optimized.

Ultimately, your workload and activity level will determine the most cost efficient tier for your standard file share. In practice, the best way to pick the most cost-optimum tier picking the correct tier will involve looking at the actual resource consumption of each resource (data stored, write transactions, etc.).

### What are transactions?
Transactions are operations or requests against Azure Files to upload, download, or otherwise manipulate the contents of the file share. Every action that you take on a file share translates to one or more transactions, and on standard shares that use the pay-as-you-go billing model, that translates to transaction costs.
