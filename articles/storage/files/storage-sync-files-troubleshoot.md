...
| 0x80c80243 | -2134375869 | ECS_E_SECURITY_DESCRIPTOR_SIZE_TOO_LARGE | The file failed to sync because the security descriptor size exceeds the 64 KiB limit. | To resolve this issue, remove access control entries (ACE) on the file to reduce the security descriptor size. |
| 0x8000ffff | -2147418113 | E_UNEXPECTED | The file failed to sync due to an unexpected error. | If the error persists for several days, please open a support case. |
