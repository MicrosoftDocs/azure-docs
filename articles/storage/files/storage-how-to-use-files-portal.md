...
[Azure Files](storage-files-introduction.md) is Microsoft's easy to use cloud file system. Azure File shares can be mounted in Windows and Windows Server. This guide walks you through the basics of working with Azure file shares using the the [Azure portal](https://portal.azure.com). Learn how to:
...
In the resulting create resource group blade, enter the name of the resource group we will use in this guide, **myResourceGroup**.
...
In the resource group pane, click the entry for the **myResourceGroup** you just created in the previous section (you may have to click the refresh button in the resource group pane). In the resulting resource group contents pane, you add a new storage account by clicking the add button and searching for "storage account" the resulting search box (you may have to hit the "enter" key to execute a search).

![Select the storage account resource](media/storage-how-to-use-files-portal/create-storage-account-1.png)

Click the create button on the storage account information pane. This opens the create storage account pane, where are asked to fill in the following information:
...
- **Deployment model**: This allows you to select whether you want a Azure Resource Manager (ARM) storage account deployment or a classic storage account deployment for legacy reasons. Since this is a new deployment, leave this selector on the default "Resource manager" selection. To learn more about the differences between Azure Resource Manager (ARM) and classic deployment, see [Understand deployment models and the state of your resources](../../azure-resource-manager/resource-manager-deployment-model.md).
...
- **Replication**: This refers to the redundancy setting of the storage account. For this purposes of this guide, select *Locally-redundant storage (LRS)*, which is the cheapest option. To learn more about redundancy options supported by Azure Files, see the [Data redundancy](storage-files-planning.md#data-redundancy) section of the Planning for an Azure Files deployment guide.
...
## Create a file share
Now you can create your first Azure file share. You can do this by navigating into the storage account you just created in your resource group (you may need to click the refresh button to see the storage account in the resource group).
...
### Download a file
You can download a copy of the file you just uploaded by right clicking on the file. After clicking the download button, the exact experience will depend on the operating system and browser you are using.
...
![A screenshot of the delete resource group group button](media/storage-how-to-use-files-portal/cleanup-resources-1.png)
