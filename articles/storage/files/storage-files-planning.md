...
- LRS: All regions, except for South Africa North and South Africa West.
   - National clouds (Germany, China) are supported via PowerShell and Azure Command Line Interface (CLI). No portal support. 
   - East US, West Europe: All new accounts are supported. A small number of existing accounts have not completed the upgrade process. You can check if your existing storage accounts have completed the upgrade process by attempting to [Enable large file shares](storage-files-how-to-create-large-file-share.md).
