...
```
> [!WARNING]
> This deprecated option is about to be retired. After May 15, 2022 you will no longer be able to create a server endpoint in the "offline data transfer" mode. Migrations in progress with this method must finish before July 15, 2022. If your migration continues to run with an "offline data transfer" enabled server endpoint, the server will begin to upload remaining files from the server on July 15 and no longer leverage files transferred with Azure Data Box.
