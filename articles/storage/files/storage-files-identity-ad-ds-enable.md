...
- Run the script using an on-premises AD DS credential that is synced to your Azure AD. The on-premises AD DS credential must have either **Owner** or **Contributor** Azure role on the storage account. If this RBAC role is assigned only on storage account level make sure the resource group has atleast a reader role, else it might fail when you run Join-AzStorageAccountForAuth with the error "Get-AzResourceGroup : Provided resource group does not exist" exception.
