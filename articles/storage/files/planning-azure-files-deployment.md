...
| Access Control | Enforced via SAS token. | File System ACLS enforced by on-premises file server; Azure File Sync stores them and replicates them to each Server in the Storage Sync Group. |
...
There are several easy options to bulk transfer data from an existing file share, such as an on-premises file share, into Azure Files:
