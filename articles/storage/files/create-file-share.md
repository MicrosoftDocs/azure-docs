...
For private endpoint configurations, each Azure file share will have its own private endpoint. To get started, click on **+ Create private endpoint**. Leave **Subscription** and **Resource group** the same. Choose the same location as the virtual network and desired name for the private endpoint. Choose FileShare for storage sub-resource. Next, under configure virtual network section, choose the desired virtual network, and subnet setting. Select **Yes** for **Enable Private DNS zone**. Select **Add** after everything is all set up.
...
The final step to create the Azure file share is to select the **Create** button on the **Review + create** tab. This button isn't available until all the required fields for an Azure file share are completed.
