...
![Migration steps overview](\media\storage-files-migration-storsimple-shared\SS1200-migration.png "Migration route overview of the steps further below in this article.")
...
If you plan to make changes to your storage structure, meaning the number of volumes, the association of data folders to volumes or the sub-folder structure above or below your current SMB/NFS shares, then now is the time to take this into consideration.
Changing this after Azure File Sync is configured, is cumbersome and should be avoided.
This article will assume you are mapping 1:1, so please take your changes into consideration when you follow the steps in this article.
...
* Provision the same amount of volumes on your Windows Server as you have on your StorSimple 1200 virtual appliance.
...
Create **all** folders, that will sync each to it's own Azure file share.
...
:::row:::
   ...
   :::column-end:::
   :::column span="1":::
      Allows to run this RoboCopy command several times, sequentially on the same target/desitnation. It identifies what has been copied before and omits it. Only changes, additions, deletes will be processed, that occured since the last run. If the command wasn't run before, nothing is omitted. This is an excellent options for source locations that are still actively used and changing.
   :::column-end:::
:::row-end:::
:::row:::
   :::column span="1":::
...
When that is the case, take the StorSimple location offline. (e.g. remove the SMB share so that no user can access the folder or other, appropriate step that prevents content to change in this folder on the StorSimple.)
...
You can try and run a few of these copies in parallel. We recommend processing the scope of one Azure file share at a time.
...
The most likely issue you can run into, is that the RoboCopy command fails with *"Volume full"* on the Windows Server side. If that is the case, then your download speed is likely better than your upload speed. Cloud tiering acts once every hour to evacuate content from the local Windows Sever disk, that has synced.

Let sync progress and cloud tiering free up disk space. You can see that in File Explorer on your Windows Server.

Once the WIndows Server has again sufficient capacity available, simple re-run the RoboCopy command. Nothing breaks when you get into this situation and you can move forward with confidence. Inconvenience of running the command again is the only consequence.
