...
Site Recovery is an Azure service that contributes to your business continuity and disaster recovery strategy by orchestrating the replication of on-premises physical servers and virtual machines to the cloud (Azure) or to a secondary datacenter. When outages occur in your primary location, you fail over to the secondary location to keep applications and workloads available. You fail back to your primary location when it returns to normal operation. Site Recovery provides test failovers to support disaster recovery drills without affecting production environments. You can run unplanned failovers with minimal data loss (depending on replication frequency) for unexpected disasters. In the scenario of migrating to Premium Storage, you can use [Unplanned Failover](../site-recovery/site-recovery-failover.md#types-of-failover) in Azure Site Recovery to migrate target disks to a Premium Storage account.

We recommend migrating to Premium Storage by using Site Recovery because this option provides minimal downtime and avoids the manual execution of copying disks and creating new VMs. Site Recovery will systematically copy your disks and create new VMs during failover. Site Recovery supports a number of types of failover with minimal or no downtime. To plan your downtime and estimate data loss, see the table in [Failover in Site Recovery](../site-recovery/site-recovery-failover.md#types-of-failover). If you [prepare to connect to azure VMs after failover](../site-recovery/site-recovery-vmware-to-azure.md#prepare-to-connect-to-azure-vms-after-failover), you should be able to connect to the Azure VM using RDP after failover.
...
* **Configuration server**: An Azure VM that coordinates communication and manages data replication and recovery processes. On this virtual machine you will run a single setup file to install the configuration server and an additional component, called a process server, as a replication gateway. Read about [configuration server prerequisites](../site-recovery/site-recovery-vmware-to-azure.md#configuration-server-or-additional-process-server-prerequisites). Configuration server only need to be configured once and can be used for all migrations to the same region.

* **Process server**: A replication gateway that receives replication data from source standard VMs, optimizes the data with caching, compression, and encryption, and sends it to a storage account. It also handles push installation of the mobility service to source standard VM and performs automatic discovery of source standard VMs. The default process server is installed on the configuration server. You can deploy additional standalone process servers to scale your deployment. Read about [best practices for process server deployment](https://azure.microsoft.com/en-us/blog/best-practices-for-process-server-deployment-when-protecting-vmware-and-physical-workloads-with-azure-site-recovery/) and [deploying additional process servers](../site-recovery/site-recovery-vmware-to-azure.md#deploy-additional-process-servers). Process server only need to be configured once and can be used for all migrations to the same region.

* **Mobility service**: A component that is deployed on every standard virtual machine you want to replicate. It captures data writes on the standard virtual machine and forwards them to the process server. Read about [protected machine prerequisites](../site-recovery/site-recovery-vmware-to-azure.md#replicated-machine-prerequisites).
...
You can use Site Recovery to migrate Azure IaaS VMs between regions or within same region. The instructions in this article have been tailored for this scenario of migrating to Premium Storage from [Replicate VMware VMs or physical servers to Azure](../site-recovery/site-recovery-vmware-to-azure.md). Please follow the links for detailed steps in additional to the instructions in this article.

1. **Create a Recovery Services vault**. Create and manage the Site Recovery vault through the [Azure portal](https://portal.azure.com). Click **New** > **Management** > **Backup** and **Site Recovery (OMS)**. Alternatively you can click **Browse** > **Recovery Services Vault** > **Add**. Virtual machines will be replicated to the region you specify in this step. For the purpose of migration in the same region, select the region where your source standard VMs and source storage accounts are. Note that migration to Premium Storage accounts is only supported in the [Azure portal](https://portal.azure.com), not the [Azure Management Portal](https://manage.windowsazure.com).
...
7. **Install mobility service and enable replication**. You can choose to [push installation](../site-recovery/site-recovery-vmware-to-azure.md#prepare-for-automatic-push-on-windows-machines) to your source standard VMs or to [manually install mobility service](../site-recovery/site-recovery-vmware-to-azure.md#install-the-mobility-service-manually) on your source standard VMs. You can find the requirement of pushing installation and the path of the manual installer in the link provided. If you are doing a manual installation, you might need to use an internal IP address to find the configuration server. 
...
**Now enable replication as follows:**
  ...
  * In step 3, add protected virtual machines by IP address (you might need an internal IP address to find them). 
...
2. **Delete old VMs and disks**. Before deleting these, please make sure the Premium disks are consistent with source Standard disks and the new VMs perform the same function as the source standard VMs. To delete disks along with the VMs, find your old VMs in the classic portal or the Azure portal and delete the VMs that have attached disks. If you need to delete only old disks for Classic VMs, see [How to detach a disk from a Windows virtual machine](../virtual-machines/virtual-machines-windows-classic-detach-disk.md#detach-the-disk). In the Resource Manager deployment model, delete the disks from your old storage accounts in the Azure portal.
