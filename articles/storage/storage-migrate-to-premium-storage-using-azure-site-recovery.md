...
1. **Create a Recovery Services vault**. Create and manage the Site Recovery vault through the [Azure portal](https://portal.azure.com). Click **New** > **Management** > **Backup** and **Site Recovery (OMS)**. Alternatively you can click **Browse** > **Recovery Services Vault** > **Add**. VMs will be replicated to the region you specify in this step. For the purpose of migration in the same region, select the region where your source standard VMs and source storage accounts are. Note that migration to Premium storage accounts is only supported in the [Azure portal](https://portal.azure.com), not the [Classic Portal](https://manage.windowsazure.com).
...
2. **Delete old VMs and disks**. Before deleting these, please make sure the Premium disks are consistent with source standard disks and the new VMs perform the same function as the source standard VMs. In the Resource Manager (RM) deployment model, delete the VM and delete the disks from your source Standard storage accounts in the Azure portal. In the classic deploement model, you can delete the VM and disks in the classic portal, or delete VM and delete disks from source Standard storage account in Azure portal. If there is an issue in which the disk is not deleted even though you deleted the VM, please see [Troubleshoot errors when you delete VHDs in an RM deployment](storage-resource-manager-cannot-delete-storage-account-container-vhd.md) or [Troubleshoot deleting VHDs in a classic deployment](storage-cannot-delete-storage-account-container-vhd.md).
