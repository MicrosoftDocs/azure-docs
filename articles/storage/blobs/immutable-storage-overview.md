...
|--|--|--|--|--|
| A blob version is protected by an active retention policy with version-level scope | Delete Blob, Set Blob Metadata, Put Page, and Append Block<sup>1</sup> | The version is cannot be deleted. User metadata cannot be written. <br /><br /> Overwriting a blob with Put Blob, Put Block List, or Copy Blob creates a new version. | Container deletion fails if at least one blob exists in the container, regardless of whether policy is locked or unlocked. | Storage account deletion fails if there is at least one container with version-level immutable storage enabled. |
...
<sup>2</sup> You cannot rename or move a blob when the blob is in the immutable state and the account has a hierarchical namespace enabled. Both the blob name and the directory structure provide essential container-level data that cannot be modified once the immutable policy is in place.
...
Blob soft delete applies to all containers within a storage account regardless of whether a legal hold or time-based retention policy is in effect. Microsoft recommends enabling soft delete for additional protection before any immutability policies are applied.
