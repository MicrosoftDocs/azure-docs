---
title: Immutable Storage for Blob Data Overview
titleSuffix: Azure Storage
description: Learn how immutable storage for Blob Storage uses WORM support with time-based retention and legal hold policies to protect data.
services: storage
author: normesta
...
You can set these policies together. For example, you can have both a time-based retention policy and a legal hold set at the same level and at the same time. For a write to succeed, you must either have versioning enabled or have neither a legal hold nor a time-based retention policy on the data. For a delete to succeed, there must not be a legal hold or time-based retention policy on the data.

The following diagram shows how time-based retention policies and legal holds prevent write and delete operations while they're in effect.
...
There are two features under the immutable storage umbrella: _container-level WORM_ and _version-level WORM_. Container-level WORM allows you to set policies at the container level only, while version-level WORM allows you to set policies at the account, container, or version level.

## About immutable storage for blobs

Immutable storage helps healthcare organizations, financial institutions, and related industries (particularly broker-dealer organizations) store data securely. Immutable storage can be used in any scenario to protect critical data against modification or deletion.
...
A time-based retention policy stores blob data in a WORM format for a specified interval. When you set a time-based retention policy, clients can create and read blobs, but can't modify or delete them. After the retention interval expires, blobs can be deleted but not overwritten.

### Scope

You can configure a time-based retention policy at the following scopes:

- Version-level WORM policy: Configure a time-based retention policy at the account, container, or version level (versioning must be enabled on the account). If you configure it at the account or container level, all blobs in the respective account or container inherit the policy. If there's a legal hold on a container, you can't create version-level WORM for the same container. This restriction exists because the legal hold prevents generating the versions.
- Container-level WORM policy: A time-based retention policy configured at the container level applies to all blobs in that container. You can't configure individual blobs with their own immutability policies.
...
The Azure Activity log provides a more comprehensive log of all management service activities. Azure resource logs retain information about data operations. You are responsible for storing those logs persistently, as might be required for regulatory or other purposes.
...
- Version-level WORM policy: A legal hold can be configured on an individual blob version level for granular management of sensitive data (versioning must be enabled on the account).
...
You must associate a container-level legal hold with one or more user-defined alphanumeric tags that serve as identifier strings. For example, a tag might include a case ID or event name.
...
|----------|-----|-----|
| Policy granularity level | Configure policies only at the container level. Each object that you upload into the container inherits the immutable policy set. | Configure policies at the account, container, or blob level. If you set a policy at the account level, all blobs that you upload into that account inherit the policy. The same logic follows with containers. If you set a policy at multiple levels, the order of precedence is always Blob -> Container -> Account. |
| Types of policies available | Set two different types of policies at the container level: Time-based retention policies and legal holds. | At the account and container level, set only time-based retention policies. At the blob level, set both time-based retention policies and legal holds. |
...
| Enablement for existing accounts and containers | Enable this feature at any time for existing containers. | Depending on the level of granularity, this feature might not be enabled for all existing accounts and containers. |
| Account/container deletion | After you lock a time-based retention policy on a container, you can delete containers only if they're empty. | After you enable version-level WORM on an account or container level, you can delete them only if they're empty. |
| Support for Azure Data Lake Storage (storage accounts that have a hierarchical namespace enabled)| Support container-level WORM policies in accounts that have a hierarchical namespace.  | Version-level WORM policies aren't yet supported in accounts that have a hierarchical namespace. |

To learn more about container-level WORM, see [Container-level WORM policies](immutable-container-level-worm-policies.md). To learn more about version-level WORM, see [version-level WORM policies](immutable-version-level-worm-policies.md).
...
All blob access tiers support immutable storage. You can change the access tier of a blob with the [Set Blob Tier](/rest/api/storageservices/set-blob-tier) operation. For more information, see [Access tiers for blob data](access-tiers-overview.md).
...
Microsoft recommends that you configure immutability policies mainly for block blobs and append blobs. Configuring an immutability policy for a page blob that stores a VHD disk for an active virtual machine is discouraged as writes to the disk are blocked, or if versioning is enabled, each write is stored as a new version. Microsoft recommends that you thoroughly review the documentation and test your scenarios before locking any time-based policies.

## Immutable storage with blob soft delete

When you configure blob soft delete for a storage account, it applies to all blobs within the account regardless of whether a legal hold or time-based retention policy is in effect. Microsoft recommends enabling soft delete for extra protection before any immutability policies are applied.

If you enable blob soft delete and then configure an immutability policy, any blobs that you already soft deleted are permanently deleted once the soft delete retention policy expires. You can restore soft-deleted blobs during the soft delete retention period. A blob or version that you didn't yet soft delete is protected by the immutability policy and can't be soft deleted until after the time-based retention policy expires or the legal hold is removed.
...
> [!NOTE]
> You can't configure an inventory policy in an account if support for version-level immutability is enabled on that account, or if support for version-level immutability is enabled on the destination container that you define in the inventory policy.

## Configuring policies at scale

You can use a _storage task_ to configure immutability policies at scale across multiple storage accounts based on a set of conditions that you define. A storage task is a resource available in _Azure Storage Actions_; a serverless framework that you can use to perform common data operations on millions of objects across multiple storage accounts. To learn more, see [What is Azure Storage Actions?](../../storage-actions/overview.md)

## Pricing

There's no extra capacity charge for using immutable storage. Immutable data is priced in the same way as mutable data. If you're using version-level WORM, the bill might be higher because you enabled versioning, and there's a cost associated with extra versions being stored. Review the versioning pricing policy for more information.  For pricing details on Azure Blob Storage, see the [Azure Storage pricing page](https://azure.microsoft.com/pricing/details/storage/blobs/).

Creating or deleting a time-based retention policy or legal hold on a blob version results in a write transaction charge. Modifying a time-based retention policy (either locking it or extending it) results in an Other operations charge. For more information about transaction charges, see [Operations and Data Transfer](https://azure.microsoft.com/pricing/details/storage/blobs/#pricing).
...
This feature is compatible with customer-managed unplanned failover. However, any changes that you make to the immutable policy after the last sync time (such as locking a time-based retention policy or extending it) don't sync to the secondary region. After failover is complete, you can redo the changes to the secondary region to ensure it is up-to-date with your immutability requirements.
...
Some workloads, such as SQL Backup to URL, create a blob and then add to it. If a container has an active time-based retention policy or legal hold in place, this pattern doesn't succeed. For more information, see [Allow protected append blob writes](immutable-container-level-worm-policies.md#allow-protected-append-blobs-writes).
