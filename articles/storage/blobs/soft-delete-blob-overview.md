...
In accounts that have a hierarchical namespace, the Undelete Blob operation can also be used to restore a soft-deleted directory and all its contents. You cannot restore a directory or a blob to a filepath that has a directory or blob of that name already there. If you wish to restore a soft-deleted directory but retain a current version of that directory, renaming the current version of the directory will disconnect the soft-deleted blobs from the current directory and allow an Undelete of the soft-deleted version of that directory. For example, if you delete a directory named "directory1" and create a current version of that directory in the same location with the same name, attempting to Undelete the soft-deleted version of that directory will give you an error and you will be unable to restore the soft-deleted version of the directory until the current version has either been deleted or renamed. Renaming the current version of the directory to "directory1.current" for example, would allow you to restore the soft-delete version of the name "directory1" and all of its contents. You cannot access the contents of a soft-deleted directory until after the directory has been undeleted.
