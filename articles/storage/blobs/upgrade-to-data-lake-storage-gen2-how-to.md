...
6. Expand the **Step 2: Validate account before upgrading** section and then click **Start validation**.
   ...
   Open the downloaded file to determine why the account did not pass the validation step. In case you have a Blob Storage feature that is fully supported, but in ADLS Gen 2 supported only at the preview level or not yet supported, the validation might fail. To see how each Blob Storage feature is supported with Data Lake  Storage Gen2, see [Blob Storage feature support in Azure Storage accounts](storage-feature-support-in-storage-accounts.md).
