...
description: "Learn how to configure anonymous read access for containers and blobs by setting the storage account and container access levels."
...
> [!WARNING]
> When a container is configured for anonymous access, any client can read data in that container. Anonymous access presents a potential security risk, so if your scenario doesn't require it, remediate anonymous access for the storage account.
...
Anonymous access to your data is always prohibited by default. Two settings affect anonymous access:
...
When anonymous access is permitted for a storage account and configured for a specific container, the service accepts a request to read a blob in that container that is passed *without* an `Authorization` header, and returns the blob's data in the response. However, if the request is passed *with* an `Authorization` header, the service ignores anonymous access on the storage account and authorizes the request based on the provided credentials.

## Allow or disallow anonymous read access for a storage account

When you allow anonymous access for a storage account, a user with the appropriate permissions can modify a container's anonymous access setting to enable anonymous access to the data in that container. Blob data is never available for anonymous access unless the user takes the extra step to explicitly configure the container's anonymous access setting.

Anonymous access to a container is always turned off by default and must be explicitly configured to permit anonymous requests. Regardless of the setting on the storage account, your data is never available for anonymous access unless a user with appropriate permissions takes this extra step to enable anonymous access on the container.

Disallowing anonymous access for the storage account overrides the access settings for all containers in that storage account, preventing anonymous access to blob data in that account. When you disallow anonymous access for the account, you can't configure the access setting for a container to permit anonymous access, and any future anonymous requests to that account fail. Before changing this setting, ensure you understand the impact on client applications that might be accessing data in your storage account anonymously. For more information, see [Prevent anonymous read access to containers and blobs](anonymous-read-access-prevent.md).

> [!IMPORTANT]
> After you disallow anonymous access for a storage account, clients that use the anonymous bearer challenge find that Azure Storage returns a 403 error (Forbidden) rather than a 401 error (Unauthorized). Make all containers private to mitigate this issue. For more information on modifying the anonymous access setting for containers, see [Set the access level for a container](anonymous-read-access-configure.md#set-the-anonymous-access-level-for-a-container).
...
Restrict assignment of these roles only to administrative users who require the ability to create a storage account or update its properties. Use the principle of least privilege to ensure that users have the fewest permissions they need to accomplish their tasks. For more information about managing access with Azure RBAC, see [Best practices for Azure RBAC](../../role-based-access-control/best-practices.md).

These roles don't provide access to data in a storage account via Microsoft Entra ID. However, they include the **Microsoft.Storage/storageAccounts/listkeys/action**, which grants access to the account access keys. By using this permission, a user can use the account access keys to access all data in a storage account.

The **Microsoft.Storage/storageAccounts/listkeys/action** itself grants data access via the account keys, but it doesn't grant a user the ability to change the **AllowBlobPublicAccess** property for a storage account. For users who need to access data in your storage account but shouldn't have the ability to change the storage account's configuration, consider assigning roles such as [Storage Blob Data Contributor](../../role-based-access-control/built-in-roles.md#storage-blob-data-contributor), [Storage Blob Data Reader](../../role-based-access-control/built-in-roles.md#storage-blob-data-reader), or [Reader and Data Access](../../role-based-access-control/built-in-roles.md#reader-and-data-access).
...
To allow or disallow anonymous access for a storage account, set the account's **AllowBlobPublicAccess** property. This property is available for all storage accounts that you create with the Azure Resource Manager deployment model. For more information, see [Storage account overview](../common/storage-account-overview.md).
...
1. Find the **Configuration** setting under **Settings**.
1. Set **Allow Blob anonymous access** to **Enabled** or **Disabled**.

    :::image type="content" source="media/anonymous-read-access-configure/blob-public-access-portal.png" alt-text="Screenshot of how to allow or disallow anonymous access for a storage account.":::
...
The following example creates a storage account and explicitly sets the **AllowBlobPublicAccess** property to **false**. Replace the placeholder values in brackets with your own values:
...
> [!NOTE]
> Disallowing anonymous access for a storage account doesn't affect any static websites hosted in that storage account. The **$web** container is always publicly accessible.
>
> After you update the anonymous access setting for the storage account, it might take up to 30 seconds before the change is fully propagated.

When a container is configured for anonymous access, requests to read blobs in that container don't need to be authorized. However, any firewall rules that you configure for the storage account remain in effect and block traffic inline with the configured ACLs.

The following examples show how to read the **AllowBlobPublicAccess** property for the storage account to determine whether anonymous access is currently allowed or disallowed. To learn how to verify that an account's anonymous access setting is configured to prevent anonymous access, see [Remediate anonymous access for the storage account](anonymous-read-access-prevent.md#remediate-anonymous-access-for-the-storage-account).

## Set the anonymous access level for a container

To grant anonymous users read access to a container and its blobs, first allow anonymous access for the storage account, then set the container's anonymous access level. Before you begin, ensure that anonymous access is allowed at the account level (through the **AllowBlobPublicAccess** property) and that you have write permission on the container. If anonymous access is denied for the storage account, you can't configure anonymous access for a container.

> [!CAUTION]
> Microsoft recommends against permitting anonymous access to blob data in your storage account.

When you allow anonymous access for a storage account, you can configure a container with the following permissions:

- **No public read access** (portal: **Private**; PowerShell: **Off**): The container and its blobs can be accessed only with an authorized request. This option is the default for all new containers.
- **Public read access for blobs only** (portal and PowerShell: **Blob**): Blobs within the container can be read by anonymous request, but container data isn't available anonymously. Anonymous clients can't enumerate the blobs within the container.
- **Public read access for container and its blobs** (portal and PowerShell: **Container**): Container and blob data can be read by anonymous request, except for container permission settings and container metadata. Clients can enumerate blobs within the container by anonymous request, but can't enumerate containers within the storage account.

You can't change the anonymous access level for an individual blob. Set the anonymous access level only at the container level. You can set the container's anonymous access level when you create the container, or you can update the setting on an existing container.

# [Azure portal](#tab/portal)

To update the anonymous access level for one or more existing containers in the Azure portal, follow these steps:

1. Go to your storage account overview in the Azure portal.
...
1. Select **Change access level** to open the anonymous access settings.
1. Select the desired anonymous access level from the **Anonymous access level** dropdown and select **OK** to apply the change to the selected containers.

    :::image type="content" source="media/anonymous-read-access-configure/configure-public-access-container.png" alt-text="Screenshot of how to set the anonymous access level for a container in the portal." lightbox="media/anonymous-read-access-configure/configure-public-access-container.png":::

1. To verify the change, reopen **Change access level** and confirm that **Anonymous access level** shows your selected value.

When anonymous access is disallowed for the storage account, you can't set a container's anonymous access level. If you attempt to set the container's anonymous access level, the setting is disabled because anonymous access is disallowed for the account.

:::image type="content" source="media/anonymous-read-access-configure/container-public-access-blocked.png" alt-text="Screenshot of a container's anonymous access level setting blocked when anonymous access is disallowed for the account.":::
...
When anonymous access is disallowed for the storage account, you can't set a container's anonymous access level. If you attempt to set the container's anonymous access level, Azure Storage returns an error indicating that anonymous access isn't permitted on the storage account.
