...
You will still be able to access and recover soft deleted data when the feature
is turned off assuming soft deleted data was saved when the feature was
previously turned on. When turning on soft delete you also need to configure the
retention period.

Retention period indicates the amount of time that soft deleted data is stored
...
the clock starts when the snapshot is generated. Currently you can retain
...
[recovery](#recovery) to learn how to list soft deleted objects.
...
*Caption: Soft deleted data is grey, while active data is blue. More recently
written data appears beneath older data. Here, “Undelete Blob” is called on blob
B, thereby resorting the base blob, B1, and all associated snapshots, here just
...
the desired snapshot over the blob. The following example recovers a blob to its
...
prefix: blob.Name, useFlatBlobListing: true, blobListingDetails: BlobListingDetails.Snapshots);

// Restore the most recently generated snapshot to the active blob
blob.StartCopy(allBlobVersions.Last(version => ((CloudBlob) version).IsSnapshot &&
    ((CloudBlob) version).Name == blob.Name).Uri);
```

## Should I use soft delete?

If there is a chance that your data is accidentally modified or deleted by an
application or other storage account user, we recommend turning on soft delete.
...
Yes, but you must call Undelete on the snapshots first.
...
Services REST API](https://msdn.microsoft.com/library/azure/dd894041.aspx)
