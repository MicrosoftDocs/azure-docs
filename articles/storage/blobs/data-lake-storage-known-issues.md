...
> [!NOTE]
> If you enroll in the public preview of Multi-Protocol Access on Data Lake Storage, then blob APIs and Data Lake Storage Gen2 APIs can operate on the same data. To learn more, see [Multi-Protocol Access on Data Lake Storage](data-lake-storage-interop.md).
...
* **Option 2**: Enroll in the public preview of [Multi-Protocol Access on Data Lake Storage](data-lake-storage-interop.md). Tools and applications that call Blob APIs, as well as Blob storage features, such as diagnostic logs, can work with accounts that have a hierarchical namespace.

### What to do with unmanaged Virtual Machine (VM) disks

These depend upon the disabled Blob Storage APIs, so if you want to enable a hierarchical namespace on a storage account, and you don't want to enroll in the public preview of [Multi-Protocol Access on Data Lake Storage](data-lake-storage-interop.md), then consider placing them into a storage account that doesn't have the hierarchical namespace feature enabled.
...
|--------|-----------|
| **APIs for Data Lake Storage Gen2 storage accounts** | Partially supported <br><br>Blob interoperability with Data Lake Storage Gen2 is currently in public preview. This preview enables you to use Blob APIs in the .NET, Java, Python SDKs with accounts that have a hierarchical namespace.  The SDKs don't yet contain APIs that enable you to interact with directories or set access control lists (ACLs). To perform those functions, you can use Data Lake Storage Gen2 **REST** APIs. |
...
| **Azure Blob storage lifecycle management policies** | Supported only if you enroll in the the [Blob interoperability with Data Lake Storage Gen2](data-lake-storage-interop.md) preview. Cool and archive access tiers are supported only by the preview. The deletion of blob snapshots is not yet supported. |
...
| **Object-level tiers** |Cool and archive tiers are supported only if you enroll in the the [Multi-Protocol Access on Data Lake Storage](data-lake-storage-interop.md) preview. <br><br> All other access tiers are not yet supported.|
| **Powershell and CLI support** | Limited functionality <br><br>Management operations such as creating an account is supported. Data plane operations such as uploading and downloading files is in public preview as part of [Multi-Protocol Access on Data Lake Storage](data-lake-storage-interop.md). Working with directories and setting access control lists (ACLs) is not yet supported. |
