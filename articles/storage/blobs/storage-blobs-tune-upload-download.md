...
This article walks through several considerations and best practices to tune data transfer options. When properly tuned, the client library can efficiently distribute data across multiple requests, which can result in improved operation speed, memory usage, and network stability. The focus of this article is primarily on block blobs, but the guidance applies to any API that accepts `StorageTransferOptions` as a parameter.
...
BlobUploadOptions options = new BlobUploadOptions
{
    TransferOptions = new StorageTransferOptions
    {
        ...
        // Set the initial transfer length
        InitialTransferSize = initialTransferSize,

        // Set the maximum length of a transfer
