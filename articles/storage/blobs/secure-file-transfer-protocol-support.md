...
Standard transaction, storage, and networking prices for the underlying account apply for SFTP-enabled accounts. Transaction and storage costs are based on factors such as storage account type and the endpoint that you use to transfer data to the storage account. Enabling the SFTP endpoint also has an hourly cost. To learn more, see [Understand the full billing model for Azure Blob Storage](../common/storage-plan-manage-costs.md#understand-the-full-billing-model-for-azure-blob-storage).
