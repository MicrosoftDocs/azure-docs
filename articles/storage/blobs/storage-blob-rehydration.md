...
While a blob is in the archive access tier, it's considered offline and can't be read or modified. The blob's metadata remains online and available, enabling you to list the blob and its properties. Reading and modifying blob data is possible only with the online access tiers (hot and cool). There are two options for retrieving and accessing data stored in the archive tier:
...
The first option for moving a blob from the archive tier to an online tier is to rehydrate the blob by calling [Set Blob Tier](/rest/api/storageservices/set-blob-tier). With this operation, you can change the tier of the archived blob to either hot or cool.
...
Once a rehydration request is initiated, it cannot be canceled. During the rehydration operation, the blob's access tier setting continues to show as archive until the rehydration process is complete.

To check the status of the [Set Blob Tier](/rest/api/storageservices/set-blob-tier) operation, call [Get Blob Properties](/rest/api/storageservices/get-blob-properties) and read the `x-ms-archive-status` and `x-ms-rehydrate-priority` blob properties. The archive status property may return *rehydrate-pending-to-hot* or *rehydrate-pending-to-cool*, depending on the target tier for the rehydration operation. The rehydrate priority property indicates whether the operation priority is high or standard. When the rehydration operation is complete, the blob's access tier property updates to reflect the new tier.
...
The following table shows the behavior of the **Copy Blob** operation, depending on the tier of the source and destination blob.
...
Azure Storage raises an event through Azure Event Grid when a blob rehydration operation is initiated, and again when it is complete. You can subscribe to these events so that your application is notified when an archived blob is rehydrated.
