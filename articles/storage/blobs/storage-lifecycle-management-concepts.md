...
When a blob is moved from one access tier to another access tier, its last modification time doesn't change. If you manually rehydrate an archived blob to hot tier, it would be moved back to archive tier by lifecycle management engine. You can prevent it by disabling the rule which effects this blob temporarily. You can copy the blob to another location if it needs to stay in hot tier permanently. You can re-enable the rule when the blob can be safely moved back to archive tier. 
