...
The following table describes feature support in the standard general-purpose v2 accounts when you enable a hierarchical namespace, Network File System (NFS) 3.0 protocol, or the SSH File Transfer Protocol (SFTP). This table does not indicate whether a feature is supported when you interact with blobs by using the API or protocol. 

For example, if you enable NFS 3.0 but never use the NFS protocol to upload a blob, a check mark in the **NFS 3.0 enabled** column indicates that feature support is not negatively impacted by merely enabling NFS 3.0 capability on the account. To assess whether the use of a capability is compatible with a feature, see the [Support by API and protocol](#support-by-api-and-protocol) section of this article.
...
The following table describes feature support in the premium block blob accounts when you enable a hierarchical namespace, Network File System (NFS) 3.0 protocol, or the SSH File Transfer Protocol (SFTP). This table does not indicate whether a feature is supported when you interact with blobs by using the API or protocol. 
