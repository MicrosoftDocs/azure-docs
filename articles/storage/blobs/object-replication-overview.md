...
A source account can replicate to no more than two destination accounts, with one policy for each destination account. Similarly, a destination account may have no more than two source accounts. Each replication policy may have up to 10 replication rules, meaning that a maximum of 10 source containers and 10 destination containers may participate in each replication policy.
