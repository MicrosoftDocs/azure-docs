...
description: Authorize access to Azure blobs and ADLS Gen2 using Azure role assignment conditions and Azure attribute-based access control (Azure ABAC). Define conditions on role assignments using Storage attributes.
services: storage
author: santoshc
...
However, in some cases you may need to enable finer-grained access to Storage resources or simplify the management of hundreds of role assignments for a storage resource. In such cases, conditions can be configured as [conditions on role assignments](../../role-based-access-control/conditions-overview.md) for [DataActions](../../role-based-access-control/role-definitions.md#dataactions). Conditions are not currently supported for management operations on the [Storage resource provider](/rest/api/storagerp).
