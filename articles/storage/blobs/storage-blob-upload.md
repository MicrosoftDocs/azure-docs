...
> [!NOTE]
> If the checksum specified in the request doesn't match the checksum calculated by the service, the upload operation fails with error code `400 (Bad Request)` and is not retried when using a default retry policy.
