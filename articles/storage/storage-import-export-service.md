...
Azure Import/Export Service allows you to securely transfer large amounts of data to Azure blob storage by shipping hard disk drives to an Azure data center. You can also use this service to transfer data from Azure blob storage to hard disk drives and ship to your on-premise site. This service is suitable in situations where you want to transfer several TBs of data to or from Azure, but uploading or downloading over the network is not feasible due to limited bandwidth or high network costs.
...
> [AZURE.IMPORTANT] You can create and manage import and export jobs using Classic portal or REST API for classic storage accounts. Currently, ARM storage accounts are not supported.
...
- Backup: Take backups of your on-premise data to store in Azure blob storage.
- Data recovery: Recover large amount of data stored in blob storage and have it delivered to your on-premise location.
...
You can create an import or export job using [Classic Portal](https://manage.windowsazure.com/) or [Azure Storage Import/Export REST API](http://go.microsoft.com/fwlink/?LinkID=329099)
...
For import jobs, you must bitlocker encrypt the drives. Either, you can use the /encrypt parameter when running the client tool during drive preparation. Or, you can enable bitlocker encryption manually on the drive and specify the encryption key in the client tool command line during drive preparation. 

For export jobs, after your data is copied to the drives, service will bitlocker encrypt the drive before shipping it back to you. The encryption key will be provided to you via Classic Portal.  

### Operating System

You can use one of the following 64-bit Operating Systems to prepare the hard drive using Azure Import/Export Tool before shipping to Azure: 
...
You can use carriers like FedEx, DHL, UPS, or Postal Service to ship your drives to the shipping address. 
...
You must also provide a valid FedEx or DHL carrier account number depending on the location, for return shipping drives. A FedEx account number is required for return shipping drives from US and Europe locations. A DHL account number is required for return shipping drives from Asia and Australia locations. You can create a [FedEx](http://www.fedex.com/us/oadr/) (for US and Europe) or [DHL](http://www.dhl.com/) (Asia and Australia) carrier account if you do not have one. If you already have a carrier account number, please verify that it is valid.
...
You can transfer data between your on-premise site and Azure blob storage using the Azure Import/Export service by creating “jobs” and shipping hard disk drives to an Azure data center. Each hard disk drive you ship, is associated with a single job. Each job is associated with a single storage account. Review the pre-requisites section carefully to learn about the specifics of this service such as, supported blob types, disk types, locations, and shipping. 
...
- Use the Azure Import/Export Tool to copy your data to one or more hard disk drives and Bitlocker encrypt them.  
- Create an import job on the Classic Portal or using REST API, in your target classic storage account. If using classic portal, upload the drive journal files.
...
- Create an export job on the Classic Portal or using REST API, in your source storage account.
...
The time it takes to process an import/export job varies depending on different factors, like shipping time, job type, type and size of data being copied, size of disks provided. Service does not have an SLA. You can use the REST API to track the job progress more closely. There is a percent complete parameter in the List Jobs operation which gives an indication of copy progress. Reach out to us if you need an estimate to complete a time critical import/export job.
...
When you ship drives to Azure, you pay the shipping cost to the shipping carrier. When Azure returns the drives back to you, the return shipping cost is charged to your carrier account which you provided at the time of job creation.
...
The first step when importing data using Azure Import/Export service is to prepare your drives using the Azure Import/Export client tool. Follow the steps below to prepare your drives:
...
3.	Identify the target storage account, container, virtual directors and blobs. 
...
For subsequent copy sessions to the same hard disk drive, specify the same journal file name, provide a new session ID, no need to provide the storage account key and target drive, and do not format or encrypt the drive. In this example we are copying the “H:\Photo” folder and its sub directories to the same target drive, into the storage container called “photo”. 

	WAImportExport.exe PrepImport /j:FirstDrive.jrn /id:Photo /srcdir:H:\Photo /dstdir:photo/ /MetadataFile:c:\WAImportExport\SampleMetadata.txt

Azure Import/Export client tool PrepImport command for the first copy session to copy a file:

	WAImportExport PrepImport /sk:<StorageAccountKey> /csas:<ContainerSas> /t: <TargetDriveLetter> [/format] [/silentmode] [/encrypt] [/bk:<BitLockerKey>] [/logdir:<LogDirectory>] /j:<JournalFile> /id:<SessionId> /srcfile:<SourceFile> /dstblob:<DestinationBlobPath> [/Disposition:<Disposition>] [/BlobType:<BlockBlob|PageBlob>] [/PropertyFile:<PropertyFile>] [/MetadataFile:<MetadataFile>]

Azure Import/Export client tool PrepImport command for subsequent copy sessions to copy a file:
...
7. You can track your job progress on the portal dashboard. See what each job state means in the previous section on [Viewing your job status](#viewing-your-job-status). 
...
1. Check the number of disks required using Azure Import/Export tool's PreviewExport command. For more information, see [Previewing Drive Usage for an Export Job](https://msdn.microsoft.com/library/azure/dn722414.aspx). It helps you preview drive usage for the blobs you selected, based on the size of the drives you are going to use. 
...
3.	In Step 3, specify which blob data you wish to export from your storage account to your blank drive or 	drives. You can choose to export all blob data in the storage account, or you can specify which blobs or sets of blobs to export.
	...
	You must provide the blob paths in valid formats to avoid errors during processing. See an example below,
...
7.	You can track your job progress on the portal dashboard. See what each job state means in the previous section on “Viewing your job status”.
...
**Can I copy Azure Files using Azure Import/Export service?**
...
**Is Azure Import/Export service available for CSP subscriptions?**
...
No. All drives are BitLocker-encrypted.
...
When preparing a hard drive for an import job, the destination is specified by the “/dstdir:” parameter. This is the destination container in the storage account to which data from hard drive is copied. Within this destination container, “virtual directories” are created for folders from the hard drive and “blobs” are created for files.
...
When preparing the drive, you can specify whether the destination files can be overwritten or ignored using the parameter called, /Disposition:<rename|no-overwrite|overwrite>. By default, the service will not overwrite any blobs, and will rename the new files.

**Is the Azure Import/Export client tool compatible with 32-bit operating system?**
No. The client tool is only compatible with 64-bit Windows operating system. Please refer the Operating Systems section in [Pre-requisites](#pre-requisites) for complete list of supported OS versions.
...
You can ship drives to the data center using any known carrier like FedEx, DHL, UPS, or Postal service. However, for return shipping drives from data center, you must provide a FedEx account number in US and EU, and a DHL account number in Asia and Australia regions.
...
**Why does my job status on Classic Portal say “Shipping” when the Carrier website shows my package is delivered?**
