---
title: Client-side encryption for queues with .NET
titleSuffix: Azure Storage
description: The Queue Storage client library for .NET supports client-side encryption and integration with Azure Key Vault for maximum security for your Azure Storage applications.
services: storage
author: tamram
...
# Client-side encryption for queues with .NET
...
Azure Key Vault helps safeguard cryptographic keys and secrets used by cloud applications and services. Azure Key Vault users can protect keys and other secrets in hardware security modules (HSMs). For more information, see [What is Azure Key Vault?](../../key-vault/general/overview.md).

The Azure Storage client libraries leverage the Key Vault interfaces to provide a common framework across Azure for managing keys (???still true???). Developers can also take advantage of the additional benefits provide by the Key Vault libraries, such as useful functionality around simple and seamless symmetric/RSA local and cloud key providers as well as help with aggregation and caching. ???can we omit this para???
...
Keep in mind these important points when using client-side encryption:
