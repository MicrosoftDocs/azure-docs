...
> [!IMPORTANT]
> Don't hardcode a storage account's IP address in hosts files, DNS, or firewall rules. Azure Files IP addresses can change at any time, including >on failover or when an account is migrated to another tenant or region. A stale IP breaks connectivity, causing sync failures and preventing file >recall. Let the agent resolve the endpoint via DNS, and use a [private endpoint](file-sync-networking-endpoints) with its private DNS zone if you >need private routing.
