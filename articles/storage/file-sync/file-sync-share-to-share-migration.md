...
The following instructions assume you have one Azure File Sync server in your sync group. If you have more than one Azure File Sync server connected to the existing share, [remove all the other server endpoints](file-sync-server-endpoint-delete.md) first. Perform the full migration on one endpoint, and then reconnect the other server endpoints to the new sync group.
...
1. Deploy a Windows Server VM (IaaS VM) in the same Azure region as your source and target file shares. For good performance, use a multi-core VM type with at least 56 GiB of memory and premium storage.
...
You can now go to the IaaS VM and start the [initial data copy](#initial-data-copy-with-robocopy) between the source and target shares.
...
You can now start the [initial data copy](#initial-data-copy-with-robocopy) between the source and target shares on the target Azure File Sync VM.

### Initial data copy with Robocopy
