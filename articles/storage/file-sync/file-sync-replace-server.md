...
description: Learn how to replace an Azure File Sync server due to hardware decommissioning or end of support for old operating systems. 
author: khdownie
...
This article provides guidance on how to replace an Azure File Sync server due to hardware decommissioning or end of support.
...
4. Optional: To reduce the amount of data that needs to be downloaded to the new server from the Azure file share, use Robocopy to copy the files in the cache from the old server to the new server. 
   ...
   After the initial copy completes, run the Robocopy command again to copy any remaining changes.

5. In the Azure portal, navigate to the Storage Sync Service. Go to the sync group which has a server endpoint for the old server and [create a server endpoint](file-sync-server-endpoint-create.md#create-a-server-endpoint) on the new server. Repeat this step for every sync group that has a server endpoint for the old server. For example, if the old server has four server endpoints (four sync groups), then you should create four server endpoints on the new server.
...
1. Follow the steps in [Deprovision or delete your Azure File Sync server endpoint](file-sync-server-endpoint-delete.md#scenario-1-i-want-to-delete-a-server-endpoint-and-i-dont-need-azure-file-sync-server-local-data) to verify that all files are synced to the Azure file share prior to deleting one or more server endpoints on the old server.
1. Once all server endpoints are deleted on the old server, you can [unregister the server](file-sync-server-registration.md#unregister-the-server).
