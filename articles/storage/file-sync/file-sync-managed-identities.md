...
> [!IMPORTANT]
> **Cross-tenant topologies are not supported.** The **Storage Sync Service**, the **server resource** (Azure Arc–enabled server or Azure VM), the **managed identity**, and the **RBAC assignments** on the storage account must all be in the **same Microsoft Entra tenant**. Cross-tenant setups fail authentication/authorization and the server cannot connect.
