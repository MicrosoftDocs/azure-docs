...
This article explains how to configure your Azure File Sync deployment to use system-assigned managed identities. Managed Identity support eliminates the need for shared keys as a method of authentication by using a system-assigned managed identity provided by Microsoft Entra ID. 
...
To enable a system-assigned managed identity on a registered server that has the Azure File Sync v20 agent installed, follow these steps:
...
> [!NOTE]
> After you configure the Storage Sync Service to use managed identities, registered servers that don't have a system-assigned managed identity continue to use a shared key to authenticate to your Azure file shares.
 
### How to check if your registered servers have a system-assigned managed identity

# [Portal](#tab/azure-portal) 
To check if your registered servers have a system-assigned managed identity, follow these steps in the Azure portal:  

1. Go to your **Storage Sync Service** in the Azure portal, expand **Settings** and select **Managed identity**. 

1. In the **Registered Servers** section, select the **Ready to use Managed ID** tile. This tile displays a list of servers that have a system-assigned managed identity. If your server isn't listed, perform the steps to [Enable a system-assigned managed identity on your registered servers](#enable-a-system-assigned-managed-identity-on-your-registered-servers).  
...
> [!NOTE]
> After you configure a server to use the system-assigned managed identity by following the steps in the following section, the **LatestApplicationId** property is no longer used (it's empty), the **ActiveAuthType** property value changes to **ManagedIdentity**, and the **ApplicationId** property has a GUID which is the system-assigned managed identity.
...
To configure the Storage Sync Service and registered servers to use system-assigned managed identities, follow these steps in the Azure portal: 

1. Go to your **Storage Sync Service** in the Azure portal, expand **Settings** and select **Managed identity**. 

1. Select **Turn on Managed identity** to begin setup. 
...
> [!NOTE] 
> After you configure the registered servers to use a system-assigned managed identity, it can take up to 15 minutes before the server uses the system-assigned managed identity to authenticate to the Storage Sync Service and file shares.
...
To check if the Storage Sync Service is using a system-assigned managed identity, follow these steps in the Azure portal: 

1. Go to your **Storage Sync Service** in the Azure portal, expand **Settings**, and select **Managed identity**. 

1. In the **Registered Servers** section, if you have at least one server listed in the **Using Managed ID** tile, your service is configured to use managed identities.  
...
To check if a registered server is configured to use a system-assigned managed identity, follow these steps in the Azure portal: 

1. Go to your **Storage Sync Service** in the Azure portal, expand **Settings**, and select **Managed identity**. 

1. In the **Registered Servers** section, select the **Using Managed ID** tile and verify the server is listed. 
...
Verify the **ApplicationId** property has a GUID which indicates the server is configured to use the managed identity. The value for the **ActiveAuthType** property updates to **ManagedIdentity** after the server uses the system-assigned managed identity.

> [!NOTE]
> After you configure the registered servers to use a system-assigned managed identity, it can take up to 15 minutes before the server uses the system-assigned managed identity to authenticate to the Storage Sync Service and Azure file shares.

---

## After configuration: next steps and troubleshooting

After you configure the Storage Sync Service and registered servers to use a system-assigned managed identity, new sync endpoints (cloud endpoints or server endpoints on already configured servers) use a system-assigned managed identity to authenticate to the Azure file share.

However, if you register an entirely new Windows Server machine with the Storage Sync Service, run the [Set-AzStorageSyncServiceIdentity](/powershell/module/az.storagesync/set-azstoragesyncserviceidentity) cmdlet again for that server (or use **Turn on Managed identity** in the portal). New registered servers don't automatically inherit the managed identity configuration, so you must explicitly enable it.
