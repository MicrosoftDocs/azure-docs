...
When you change the storage size of your volumes, make sure the size is less than the available capacity of single node temp disk. See [Check node temp disk capacity](#check-node-temp-disk-capacity).
...
An emphemeral volume is allocated on single node. When you configure the size of your emphemeral volumes, the size should be less than the available capacity of single node temp disk.
