...
> [!IMPORTANT]
> Local disks are ephemeral, meaning that they're created on the local virtual machine (VM) storage and not saved to an Azure storage service. Data will be lost on these disks if you stop/deallocate your VM. You can only create [Kubernetes generic ephemeral volumes](https://kubernetes.io/docs/concepts/storage/ephemeral-volumes/#generic-ephemeral-volumes) from a temp SSD storage pool. If you want to create a persistent volume, you have to [use local NVMe with volume replication](use-container-storage-with-local-nvme-replication.md).
