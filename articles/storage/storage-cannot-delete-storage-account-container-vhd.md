<properties
	pageTitle="Troubleshooting problems with deleting Azure storage account, container or VHD | Microsoft Azure"
	description="Troubleshooting problems with deleting Azure storage account, container or VHD"
	services="storage"
	documentationCenter=""
	authors="genlin"
	manager="felixwu"
...
# Troubleshooting problems with deleting Azure storage account, container or VHD

You may receive errors when you try to delete the Azure storage account, container or VHD in the Azure portal or the Azure classic portal. The problem can be caused by:

-	The storage account still has VHD or disk of a deleted Virtual Machine (VM). If you delete a VM using the Azure portal or other ways such as the Azure PowerShell, the disk and VHD are not automatically deleted, in case you want and keep them to mount to another VM.
...
### Symptom
When you try to delete a storage account that you are no longer using by navigating to the storage account in the Azure Portal or Azure classic portal, and select Delete, you may see the following error message:
...
The container in the storage account contains a active disk or images.

### Resolution
To resolve the issues, delete the active disk or the image from the container. The disks is not visible in the Azure portal, but you can view them in the Azure classic Portal. To delete the virtual machine disk, follow these steps:
...
When you try to delete the storage container, you may see the following error on the Azure portal:
...
V1 VMs which have been retained will show as in the stopped deallocated state on either Azure portal or Azure classic portal.
