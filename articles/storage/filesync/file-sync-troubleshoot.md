...
[Azure File Sync scale targets](../files/storage-files-scale-targets.md#azure-file-sync-scale-targets?toc=%2fazure%2fstorage%2ffilesync%2ftoc.json).
...
- If **GetNextJob completed with status: 0** is logged, the server can communicate with the Azure File Sync service. 
    - Open Task Manager on the server and verify the Storage Sync Monitor (AzureStorageSyncMonitor.exe) process is running. If the process is not running, first try restarting the server. If restarting the server does not resolve the issue, upgrade to the latest Azure File Sync [agent version](../files/storage-files-release-notes.md?toc=%2fazure%2fstorage%2ffilesync%2ftoc.json). 
...
A server endpoint may not log sync activity for several hours due to a bug or insufficient system resources. Verify the latest Azure File Sync [agent version](../files/storage-files-release-notes.md?toc=%2fazure%2fstorage%2ffilesync%2ftoc.json) is installed. If the issue persists, open a support request.
...
| 0x80c86002 | -2134351870 | ECS_E_AZURE_RESOURCE_NOT_FOUND | The file failed to recall because it's not accessible in the Azure file share. | To resolve this issue, verify the file exists in the Azure file share. If the file exists in the Azure file share, upgrade to the latest Azure File Sync [agent version](../files/storage-files-release-notes.md#supported-versions?toc=%2fazure%2fstorage%2ffilesync%2ftoc.json). |
