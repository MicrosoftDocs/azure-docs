...
**Change 1:** You enabled cloud tiering, set a volume free space policy of 20%, and kept the date policy disabled. With that configuration, cloud tiering ensures 20% (in this case 100 GB) of space is kept free and available on the local machine. As a result, the total capacity of the local cache is 400 GB. That 400 GB will store the most recently and frequently accessed files on the local volume.

With this configuration, only files 1 through 4 would be stored in the local cache, and file 5 would be tiered. This is only 370 GB out of the 400 GB that could be used. File 5 is 140 GB and would exceed the 400-GB limit if it was locally cached. 

**Change 2:** Say a user accesses file 5. This makes file 5 the most recently accessed file in the share. As a result, File 5 would be stored in the local cache and to fit under the 400-GB limit, file 4 would be tiered. The following table shows where the files are stored, with these updates:
