...
- **Azure AD authorization over SMB for Azure Files.** Azure Files supports identity-based authorization over SMB (Server Message Block) through Azure Active Directory Domain Services (Azure AD DS), and through on-premises Active Directory Domain Services (preview). Your domain-joined Windows virtual machines (VMs) can access Azure file shares using Azure AD credentials. For more information, see [Overview of Azure Active Directory authorization over SMB for Azure Files)](../files/storage-files-active-directory-overview.md).
