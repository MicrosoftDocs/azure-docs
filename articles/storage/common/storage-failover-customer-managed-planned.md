---
title: Understand how an Azure Storage account failover to perform disaster recovery testing (preview) works
...
# How failover for disaster recovery testing (preview) works

Customer-managed storage account planned failover enables you to fail over your entire geo-redundant storage account to the secondary region to do disaster recovery testing. During failover, the original secondary region becomes the new primary and all storage service endpoints for blobs, tables, queues and files are redirected to the new primary region. After testing has been completed, you can perform another failover operation to *fail back* to the original primary region. A *failback* simply restores a storage account to its original regional configuration.
...
> [!IMPORTANT]
> Customer-managed failover for disaster recovery testing (planned failover) is currently in PREVIEW.
>
...
During the planned failover process, the storage service endpoints to the primary region become read-only and any remaining updates are allowed to finish replicating to the secondary region. The DNS entries for the storage service endpoints are then changed such that those for the secondary region become the new primary endpoints for your storage account, and the original primary region becomes the new secondary. Data replication within each region remains unchanged even though the primary and secondary regions are switched. Replication within the new primary (the original secondary) will always be LRS, and replication within the new secondary (the original primary) will be the same as it was when it was primary (LRS or ZRS).

Azure stores the otiginal redundancy configuration of your storage account in the metadata of the account to allow you eventually return to it when you fail back.

After failover, the new redundancy configuration of your storage account temporarily becomes GRS. How data is replicated within the primary region at a given point in time determines the zone-redundancy configuration of the storage account. Data in the new primary region (the original secondary) continues to replicate only three times locally (LRS), so the account is temporarily non-zonal. Azure immediately begins copying data from the new primary region to the new secondary. If your storage account was originally configured for read access (RA) to the secondary region, that access will follow whichever region becomes the secondary during the entire failover and failback process.

The failback process is essentially the same as the failover process except Azure stores the otiginal redundancy configuration of your storage account and restores it to its original state upon failback. So, if your storage account was originally configured as GZRS, the storage account will be GZRS after failback.

> [!NOTE]
> Unlike [customer-managed failover to recover from an actual outage](storage-failover-customer-managed-unplanned.md), during planned failover, replication from the primary to secondary region is allowed to finish before the DNS entries for the endpoints are changed to the new secondary. Because of this, data loss is not expected during failover or failback as long as both the primary and secondary regions are available throughout the process.
...
When testing has been completed, perform another failover to failback to the original primary region. During the failover process, as shown in the image below:
