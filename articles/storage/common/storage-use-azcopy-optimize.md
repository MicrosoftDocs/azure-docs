...
AzCopy is a command-line utility that you can use to copy blobs or files to or from a storage account. This article helps you optimize performance.

> [!NOTE]
> For content to help you get started with AzCopy, see [Get started with AzCopy](storage-use-azcopy-v10.md).
...
Throughput can decrease when transferring large numbers of files. Each copy operation translates to one or more transactions that the storage service must execute. When you transfer a large number of files, consider the number of transactions that need to be executed and the potential impact those transactions can have if other activities occur in the storage account at the same time. 

To maximize performance, you can reduce the size of each job by limiting the number of files that you copy in a single job. For download and upload operations, increase concurrency as needed, decrease log activity, and turn off features that incur high performance costs.
...
Use the `include-pattern` parameter to copy files that have a specific extension (for example: `*.pdf`). In a separate job, use the `exclude-pattern` parameter to copy all files that don't have the `*.pdf` extension. See [Upload specific files](storage-use-azcopy-blobs-upload.md#upload-specific-files) and [Download specific blobs](storage-use-azcopy-blobs-download.md#download-specific-blobs) for examples.

After you decide how to divide large jobs into smaller ones, consider running jobs on more than one virtual machine (VM).
...
If you're copying blobs between storage accounts, consider setting the value of the `AZCOPY_CONCURRENCY_VALUE` environment variable to a value greater than `1000`. You can set this variable high because AzCopy uses server-to-server APIs, so data is copied directly between storage servers and doesn't use your machine's processing power.
...
If your computer has fewer than five CPUs, the value of this variable is set to `32`. Otherwise, the default value is equal to 16 multiplied by the number of CPUs. The maximum default value of this variable is `300`, but you can manually set this value higher or lower.
...
Before you set this variable, we recommend that you run a benchmark test. The benchmark test process reports the recommended concurrency value. Alternatively, if your network conditions and payloads vary, set this variable to the word `AUTO` instead of to a particular number. That setting causes AzCopy to always run the same automatic tuning process that it uses in benchmark tests.

## Limit the throughput data rate

Use the `cap-mbps` flag in your commands to place a ceiling on the throughput data rate. For example, the following command resumes a job and caps throughput to `10` megabits (Mb) per second. This feature is only supported for uploading or downloading files, not copying between accounts.
...
> [!NOTE]
> Job tracking always adds overhead in memory usage. The amount varies based on the number of transfers in a job. Buffers are the largest component of memory usage. You can help control overhead by using `AZCOPY_BUFFER_GB` to approximately meet your requirements, but there's no flag available to strictly cap the overall memory usage.

## Optimize file synchronization

The [sync](storage-ref-azcopy-sync.md) command identifies all files at the destination, then compares file names and last modified timestamps before starting the sync operation. If you have a large number of files, you can improve performance by eliminating this up-front processing.

To accomplish this goal, use the [azcopy copy](https://github.com/Azure/azure-storage-azcopy/wiki/azcopy_copy) command instead, and set the `--overwrite` flag to `ifSourceNewer`. AzCopy compares files as it copies them without performing any up-front scans and comparisons. This approach provides a performance edge in cases where there are a large number of files to compare.

The [azcopy copy](https://github.com/Azure/azure-storage-azcopy/wiki/azcopy_copy) command doesn't delete files from the destination. If you want to delete files at the destination when they no longer exist at the source, use the [azcopy sync](https://github.com/Azure/azure-storage-azcopy/wiki/azcopy_sync) command with the `--delete-destination` flag set to a value of `true` or `prompt`.

## Use multiple clients to run jobs in parallel

AzCopy performs best when only one instance runs on the client. If you want to transfer files in parallel, use multiple clients and run only one instance of AzCopy on each one.  
