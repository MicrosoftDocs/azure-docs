...
Azure Storage offers two options for geo-redundant replication: [Geo-redundant storage (GRS)](storage-redundancy.md#geo-redundant-storage) and [Geo-zone-redundant storage (GZRS)](storage-redundancy.md#geo-zone-redundant-storage). Make sure that your storage account is configured for read-access geo-redundant storage (RA-GRS) or read-access geo-zone-redundant storage (RA-GZRS) before you get started. If it's not, you can learn more about how to [change your storage account replication type](redundancy-migration.md).
...
You can design your application to handle transient or lengthy issues by reading from the secondary region when there is a problem that interferes with reading from the primary region. When the primary region is available again, your application can return to reading from the primary region.

Keep in mind these key considerations when designing your application for RA-GRS or RA-GZRS:
...
- For blobs, tables, and queues, you can query the storage account's **Last Sync Time** property, which indicates the last time that data from the primary region was written successfully to the secondary region. This functionality isn't supported for Azure Files, which doesn't offer RA-GRS at this time.

- You can use the Storage client library to read and write data in the primary region, or read data in the primary or secondary region. You can also redirect read requests automatically to the secondary region if a read request to the primary region times out.
...
For example, if you use queues and blobs in your application, you may decide to put in separate code to handle retryable errors for each service. That way, if you get a retry from the blob service, but the queue service is still working, the impact will be limited to the part of your application that handles blobs. If you decide to handle all storage service retries together, then requests to both the blob service and the queue service will be impacted if either of the services returns a retryable error.

Ultimately, this depends on the complexity of your application. You may decide to handle failures by service to limit the impact of retries. Or you may decide to redirect read requests for all storage services to the secondary region when you detect a problem with any storage service in the primary region.

### Further considerations

The rest of this article will discuss these and other considerations in greater detail:

- Handling retries of read and update requests

- Eventually consistent data and **Last Sync Time**
...
There are many ways to handle update requests when running in read-only mode. This section focuses only on a few general patterns to consider.
...
Update requests can't be redirected to secondary storage, which is read-only. For these requests,  you should use a **primary only** approach, which is the default behavior.

In this example for Azure Queue storage, `QueueClientOptions` is configured to only send requests to the primary region, perhaps in a section of code that mainly issues update requests, such as insertions or deletions, which can't be handled in the secondary region.
...
QueueClientOptions queueClientOptions = new QueueClientOptions()
{
    ...
    // Since GeoRedundantSecondaryUri is not being set, retry attempts will only point to the primary Uri
};

// Create a QueueServiceClient object using the configuration options above
QueueServiceClient queueServiceClient = new QueueServiceClient(primaryAccountUri, new DefaultAzureCredential(), queueClientOptions);
```

The Circuit Breaker pattern can also be applied to update requests. To handle these errors, you could set a failure threshold in the code, such as 10 consecutive failures, and track the number of failures for primary region requests. Once the threshold is met, you can switch the application to read-only mode and issue read requests to the secondary region.
...
For example, you could decide to perform the switch if there are 10 failures in a row with no successes. Then you track this by keeping a count of the failures in the code. If there's a success before reaching the threshold, set the count back to zero. If the count reaches the threshold, then switch the application to use the secondary region for read requests.

As an alternative approach, you could decide to implement a custom monitoring component in your application. This component could continuously ping your primary storage endpoint with trivial read requests (such as reading a small blob) to determine its health. This approach would take up some resources, but not a significant amount. When a problem is discovered that reaches your threshold, you would switch to **secondary only** and read-only mode. For this scenario, when pinging the primary storage endpoint becomes successful again you can switch back to **primary only** and continue allowing updates.
...
It's important to test that your application behaves as expected when it encounters retryable errors. For example, you need to test that the application switches to the secondary region when it detects a problem, and then switches back when the primary region becomes available again. To do this, you need a way to simulate retryable errors and control how often they occur.
