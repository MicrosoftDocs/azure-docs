...
The types of operations that a resource instance can perform on storage account data is determined by the [Azure role assignments](authorize-data-access.md#assign-azure-roles-for-access-rights) of the resource instance. Resource instances must be from the same tenant as your storage account, but they can belong to any subscription in the tenant.
