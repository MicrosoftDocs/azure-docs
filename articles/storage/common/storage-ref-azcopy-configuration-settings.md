...
| AZCOPY_AUTO_LOGIN_TYPE | Set this variable to `DEVICE`, `MSI`, `SPN`, `AZCLI`, and `PSCRED`. This variable provides the ability to authorize without using the `azcopy login` command. See [Authorize access to blobs with AzCopy and Microsoft Entra ID](storage-use-azcopy-v10.md#authorize-with-microsoft-entra-id). |
