...
The OAuth token is used to access the destination storage account:
...
|--block-size-mb float |Use this block size (specified in MiB) when uploading to Azure Storage, and downloading from Azure Storage. Default is automatically calculated based on file size. Decimal fractions are allowed (For example: 0.25)|
