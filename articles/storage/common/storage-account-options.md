...
* **General Purpose v2 (GPv2)** accounts provide all the latest features, and support Blob, File, Queue, and Table storage. Today, these latest features include blob-level tiering, archive storage, higher scale account limits, and storage events. Pricing has been designed to deliver the lowest per-GB prices, and industry competitive transaction prices.

* **Blob Storage** accounts provide all the latest features for block blobs, but only support block blobs. Pricing is broadly similar to that in General Purpose v2. 
...
## Upgrading a storage account to GPv2
...
To upgrade a GPv1 account to a GPv2 account using PowerShell, ensure that you have installed version 4.1 or later of the AzureRm.Storage module. You can use this command to install the module:

```powershell
Install-Module AzureRm.Storage -MinimumVersion 4.1.0 -Force
```

To upgrade a GPv1 account to a GPv2 account using PowerShell, use the following command, substituting the name of your resource group and storage account:

```powershell
Set-AzureRmStorageAccount -ResourceGroupName <resource-group> -AccountName <storage-account> -UpgradeToStorageV2
```

### Upgrading with Azure CLI
...
## General Purpose v1
**General Purpose v1 (GPv1) storage accounts** are the oldest type of storage account, and the only kind that can be used in the classic deployment model. Features like cool and archive storage are not available in GPv1. GPv1 has generally higher GB storage costs, but lower transaction costs than either GPv2 or Blob storage accounts.
