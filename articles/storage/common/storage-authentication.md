...
- **Azure Active Directory (Azure AD) integration (Preview)** for blobs and queues. Azure identity management provides role-based authentication (RBAC) for fine-grained control over a client's access to resources in a storage account. For more information, see [Authenticating requests to Azure Storage using Azure Active Directory (Preview)](storage-authentication-aad.md).
