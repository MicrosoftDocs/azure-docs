---
title: "Use the Azure Storage Emulator (Deprecated)"
...
> [!NOTE]
> Data created in one version of the Storage Emulator isn't guaranteed to be accessible when using a different version. If you need to persist your data for the long term, store that data in an Azure storage account rather than in the Storage Emulator.
>
> The Storage Emulator depends on specific versions of the OData libraries. Replacing the OData DLLs used by the Storage Emulator with other versions isn't supported, and might cause unexpected behavior. However, any version of OData supported by the storage service can be used to send requests to the emulator.
...
The Storage Emulator connects to SQL Server or LocalDB by using Windows authentication.
...
When the Storage Emulator starts, a Command Prompt window appears. Use this console window to start and stop the Storage Emulator. You can also clear data, get status, and initialize the emulator from the command prompt. For more information, see the [Storage Emulator command-line tool reference](#storage-emulator-command-line-tool-reference) section later in this article.

> [!NOTE]
> The Azure Storage Emulator might not start correctly if another storage emulator, such as Azurite, is running on the system.

When the emulator is running, you see an icon in the Windows taskbar notification area.

When you close the Storage Emulator Command Prompt window, the Storage Emulator continues to run. To bring up the Storage Emulator console window again, follow the preceding steps as if starting the Storage Emulator.

The first time you run the Storage Emulator, it initializes the local storage environment. The initialization process creates a database in LocalDB and reserves HTTP ports for each local storage service.

The default installation path for the Storage Emulator is `C:\Program Files (x86)\Microsoft SDKs\Azure\Storage Emulator`.

> [!TIP]
> Use the [Microsoft Azure Storage Explorer](https://storageexplorer.com) to work with local Storage Emulator resources. Look for **(Emulator - Default Ports) (Key)** under **Local & Attached** in the Storage Explorer resources tree after you install and start the Storage Emulator.
>

### Initialize the Storage Emulator to use a different SQL database

Use the Storage Emulator command-line tool to initialize the Storage Emulator to point to a SQL database instance other than the default LocalDB instance:
...
> [!TIP]
> Use the [Microsoft SQL Server Management Studio](/sql/ssms/download-sql-server-management-studio-ssms) (SSMS) to manage your SQL Server instances, including the LocalDB installation. In the SSMS **Connect to Server** dialog, specify `(localdb)\MSSQLLocalDb` in the **Server name:** field to connect to the LocalDB instance.

## Authenticating requests against the Storage Emulator

After you install and start the Storage Emulator, you can test your code against it. Every request you make against the Storage Emulator must be authorized, unless it's an anonymous request. Authorize requests against the Storage Emulator by using Shared Key authentication or by using a shared access signature (SAS).
...
You can create a SAS token by using [Storage Explorer](https://storageexplorer.com/) or another application that supports Shared Key authentication.
...
The service endpoints for the Storage Emulator are different from the endpoints for an Azure storage account. The local computer doesn't do domain name resolution, so the Storage Emulator endpoints are local addresses.
...
Starting with version 3.1, the Storage Emulator supports read-access geo-redundant storage (RA-GRS). You can access the secondary location by appending `-secondary` to the account name. For example, use the following address to access a blob through the read-only secondary in the Storage Emulator:
...
Starting in version 3.0, a console window displays when you start the Storage Emulator. Use the command line in the console window to start and stop the emulator. You can also query for status and perform other operations from the command line.

> [!NOTE]
> If you have the Microsoft Azure Compute Emulator installed, a system tray icon appears when you launch the Storage Emulator. Right-click the icon to reveal a menu that provides a graphical way to start and stop the Storage Emulator.
>
>
...
| --- | --- | --- | --- |
| **Start** |Starts the Storage Emulator. |`AzureStorageEmulator.exe start [-inprocess]` |*-inprocess*: Start the emulator in the current process instead of creating a new process. |
...
| **Init** |Does one-time initialization to set up the emulator. |<code>AzureStorageEmulator.exe init [-server serverName] [-sqlinstance instanceName] [-forcecreate&#124;-skipcreate] [-reserveports&#124;-unreserveports] [-inprocess]</code> |*-server serverName\instanceName*: Specifies the server hosting the SQL instance. <br/>*-sqlinstance instanceName*: Specifies the name of the SQL instance to use in the default server instance. <br/>*-forcecreate*: Forces creation of the SQL database, even if it already exists. <br/>*-skipcreate*: Skips creation of the SQL database. This argument takes precedence over -forcecreate.<br/>*-reserveports*: Attempts to reserve the HTTP ports associated with the services.<br/>*-unreserveports*: Attempts to remove reservations for the HTTP ports associated with the services. This argument takes precedence over -reserveports.<br/>*-inprocess*: Performs initialization in the current process instead of spawning a new process. The current process must be launched with elevated permissions if changing port reservations. |
...
- As described in [Addressing resources in the Storage Emulator](#addressing-resources-in-the-storage-emulator), resources are addressed differently in the Storage Emulator versus an Azure storage account. The difference exists because domain name resolution is available in the cloud but not on the local computer.
- Beginning with version 3.1, the Storage Emulator account supports read-access geo-redundant storage (RA-GRS). In the emulator, all accounts have RA-GRS enabled and there's never any lag between the primary and secondary replicas. The Get Blob Service Stats, Get Queue Service Stats, and Get Table Service Stats operations are supported on the account secondary and always return the value of the `LastSyncTime` response element as the current time according to the underlying SQL database.
- The File service and SMB protocol service endpoints aren't currently supported in the Storage Emulator.
- If you use a version of the storage services that the emulator doesn't support, the emulator returns a VersionNotSupportedByEmulator error (HTTP status code 400 - Bad Request).
...
- The Storage Emulator doesn't reject version 2019-02-02 of the storage services on Blob, Queue, and Table service endpoints.

### Version 5.8

- The Storage Emulator doesn't reject version 2018-11-09 of the storage services on Blob, Queue, and Table service endpoints.

### Version 5.7

- Fixed a bug that causes a crash if logging is enabled.

### Version 5.6

- The Storage Emulator supports version 2018-03-28 of the storage services on Blob, Queue, and Table service endpoints.

### Version 5.5

- The Storage Emulator supports version 2017-11-09 of the storage services on Blob, Queue, and Table service endpoints.
- Added support for the blob **Created** property, which returns the blob's creation time.
...
- The Storage Emulator supports version 2017-07-29 of the storage services on Blob, Queue, and Table service endpoints.
...
- The Storage Emulator installer no longer creates the database as part of install. The installer still creates the database if needed as part of startup.
...
- The emulator now returns a meaningful error message for unsupported versions of storage services. Use the latest version of the emulator. If you get a VersionNotSupportedByEmulator error (HTTP status code 400 - Bad Request), download the latest version of the emulator.
- Fixed a bug where a race condition caused table entity data to be incorrect during concurrent merge operations.
...
- The Storage Emulator now supports read-access geo-redundant storage (RA-GRS). It supports the `Get Blob Service Stats`, `Get Queue Service Stats`, and `Get Table Service Stats` APIs for the account secondary and always returns the value of the `LastSyncTime` response element as the current time according to the underlying SQL database. For programmatic access to the secondary with the Storage Emulator, use the Storage Client Library for .NET version 3.2 or later. For more information, see the [Azure Storage client libraries for .NET](/dotnet/api/overview/azure/storage).

### Version 3.0

- The Azure Storage Emulator is no longer included in the same package as the compute emulator.
- The Storage Emulator graphical user interface is deprecated. Microsoft replaced it with a scriptable command-line interface. For details on the command-line interface, see [Storage Emulator Command-Line Tool Reference](storage-use-emulator.md#storage-emulator-command-line-tool-reference). The graphical interface remains in version 3.0, but you can access it only when the Compute Emulator is installed. To access it, right-click the system tray icon and select **Show Storage Emulator UI**.
- The Storage Emulator now fully supports version `2013-08-15` of the Azure storage services. (Previously, this version was supported only by Storage Emulator version 2.2.1 Preview.)
...
- Use the [Microsoft Azure Storage Explorer](https://storageexplorer.com) to work with resources in your cloud Storage account and in the Storage Emulator.
