...
| Access tiers | Deleting, overwriting, or moving a blob to another tier | Tools or applications that use the [Copy](/rest/api/storageservices/copy-blob) operation to update a blob will overwrite the blob. Blobs are subject to an early deletion penalty if they are deleted, overwritten or moved to a different tier before the minimum number of days required by the tier have transpired.  |
