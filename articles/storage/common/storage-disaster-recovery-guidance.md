...
- Azure File Sync doesn't support customer-managed storage account failover. Storage accounts containing Azure file shares being used as cloud endpoints in Azure File Sync shouldn't be failed over. Doing so will cause sync to stop working and may also cause unexpected data loss in the case of newly tiered files. See [here](../file-sync/file-sync-disaster-recovery-best-practices#geo-redundancy) for details.
