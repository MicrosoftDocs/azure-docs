...
- Storage accounts that have [Change Feed](../blobs/storage-blob-change-feed.md) enabled are not supported for failover. Since [operational backup of Azure Blob Storage uses the change feed feature](../../backup/blob-backup-support-matrix.md#limitations), accounts that have blob backups configured do not support failover either.
