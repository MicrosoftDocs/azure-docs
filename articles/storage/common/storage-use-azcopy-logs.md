---
title: Use logs to troubleshoot AzCopy with Azure Storage | Microsoft Docs
...
# Troubleshoot AzCopy by using logs

AzCopy is a command-line utility that you can use to copy blobs or files to or from a storage account. This article helps you to troubleshoot issues that can arise as you use AzCopy.
...
By default, the log and plan files are located in the `%USERPROFILE%\.azcopy` directory on Windows or `$HOME$\.azcopy` directory on Mac and Linux, but you can change that location if you want. To change that location, see [Configure AzCopy](storage-use-azcopy-configure.md).
