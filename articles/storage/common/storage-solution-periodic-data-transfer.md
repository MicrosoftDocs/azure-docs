...
- **Continuous data ingestion tools** – For continuous, ongoing data ingestion, you can select one of the following options. Object replication automatically copies block blob data from a source storage account to a destination account. Data Box online transfer device or Azure Data Factory are set up by IT professionals and can transparently automate data transfer.

    - **Object replication** - Object replication asynchronously copies block blobs between a storage account that you designate as the source account and another account that you designate as the destination. Use object replication as a solution to keep containers in two different storage accounts in sync.
