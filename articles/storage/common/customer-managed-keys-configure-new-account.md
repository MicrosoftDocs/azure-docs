...
The user-assigned managed identity must have permissions to access the key in the key vault. Assign the **Key Vault Crypto Service Encryption User** role to the user-assigned managed identity to grant these permissions.
...
```azurecli
principalId=$(az identity show --name <user-assigned-identity> --resource-group $rgName --query principalId --output tsv)
identityResourceId=$(az identity show --name sample-user-assigned-identity --resource-group $rgName --query id --output tsv)
