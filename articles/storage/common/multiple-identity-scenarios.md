...
Applications often require secure connections between multiple Azure services at the same time. For example, an enterprise Azure App Service instance might connect to several different blob storage accounts, an Azure SQL database instance, a Key Vault service, and more. 
...
The Azure Identity client library allows you to configure credential-free service connections between multiple Azure resources simultaneously using managed identities. It also allows this scenario to work while developing locally. This tutorial explores how to manage connections between multiple services using managed identities and the Azure Identity client library.
...
* **System-assigned** managed identities are directly tied to a single Azure resource. When you enable a system-assigned managed identity on a service, Azure will create a linked identity and handle administrative tasks for that identity internally. When the Azure resource is deleted, so is the identity. System-assigned identities are a great option when managing simple connections between services where the identity should be tied to the application life cycle.
...
You can read more about when to use system-assigned identities versus user-assigned identities and best practices in the [identities best practice recommendations](/azure/active-directory/managed-identities-azure-resources/managed-identity-best-practice-recommendations).

The guidance in this tutorial focuses on applying a user-assigned managed identities to connect multiple Azure services together. A user-assigned identity can be shared across multiple services and lives independently of other resource life cycles.
...
## Connect Azure hosted app to multiple Azure services using credential-free connections
...
You plan to share a user-assigned identity between both applications, since they have the same access requirements to the services. When the app is deployed to Azure it will use user-assigned managed identities to achieve this configuration. During local development it will use Role Based Access Control (RBAC) through your local sign-in credentials to the Azure CLI or Visual Studio. This overall strategy can be accomplished by using `DefaultAzureCredential`.
...
The following steps demonstrate how to create a user-assigned managed identity and use it to configure security and access between services.
...
### 4) Add the Identity package to your app
...
### 5) Write the code using DefaultAzureCredential
...
When this application code runs locally, `DefaultAzureCredential` will search down a credential chain to retrieve the credentials from your local Azure CLI or Visual Studio sign-in. You can see the full list of supported authentication options on the [readme]().

When the application is deployed to Azure, `DefaultAzureCredential` will automatically retrieve the `Managed_Identity_Client_ID` environment variable from the app service. This process ensures that your app can run securely locally and in Azure without the need for any code changes.
