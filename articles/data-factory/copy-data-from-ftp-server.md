...
You can copy data from FTP server to any supported sink data store. For a list of data stores supported as sources/sinks by the copy activity, see the [Supported data stores](copy-activity-overview.md#supported-data-stores-and-formats) table.
...
For a full list of sections and properties available for defining datasets, see the [datasets](concepts-datasets.md) article. This section provides a list of properties supported by FTP dataset.
...
| useBinaryTransfer | Specify whether to use the binary transfer mode. The values are true for binary mode (this is the default value), and false for ASCII. |No |
