...
If you need to move data to/from a data store that is not supported by the **Copy Activity**, build a **custom activity** with your own logic for moving the data to/from data stores and then use the custom activity in your Data Factory pipeline. See [Use custom activities in an Azure Data Factory pipeline](data-factory-use-custom-activities.md) article for details on creating and using a custom activity. 
