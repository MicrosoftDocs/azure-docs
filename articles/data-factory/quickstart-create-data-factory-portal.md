...
This quickstart describes how to use the Azure Data Factory UI to create and monitor a data factory. The pipeline you create in this data factory *copies* data from one folder to another folder in Azure Blob storage. For a tutorial on how to *transform* data by using Azure Data Factory, see [Tutorial: Transform data using Spark](tutorial-transform-data-spark-portal.md). 
...
2. Select **New** on the left menu, select **Data + Analytics**, and select **Data Factory**. 
...
4. For the **Resource Group**, use one of the following steps:
...
5. Select the **location** for the data factory. Only locations that are supported by Data Factory are shown in the list. The data stores (like Azure Storage and Azure SQL Database) and computes (like Azure HDInsight) used by data factory can be in other locations.
...
10. Select **Author & Monitor** tile to launch the Azure Data Factory user interface (UI) application in a separate tab. 
11. On the get started page, switch to the **Edit** tab in the left panel as shown in the following image: 
...
2. Select **Connections**, and then select the **New** button on the toolbar. 

   ![New connection](./media/quickstart-create-data-factory-portal/new-connection-button.png)    
3. On the **New Linked Service** page, select **Azure Blob Storage**, and select **Continue**. 

   ![Select Azure Storage](./media/quickstart-create-data-factory-portal/select-azure-storage.png)
4. On the **New Linked Service** page, complete the following steps: 

   a. Enter **AzureStorageLinkedService** for the **Name**.

   b. Select the name of your Azure storage account for the **storage account name**.
...
## Create datasets
In this step, you create two datasets: **InputDataset** and **OutputDataset**. These datasets are of type **AzureBlob**. They refer to the **Azure Storage linked service** that you created in the previous step. 

The input dataset represents the source data in the input folder. In the input dataset definition, you specify the blob container (**adftutorial**), the folder (**input**), and the file (**emp.txt**) that contains the source data. 
...
1. Select the **+ (plus)** button, and select **Dataset**.

   ![New dataset menu](./media/quickstart-create-data-factory-portal/new-dataset-menu.png)
2. On the **New Dataset** page, select **Azure Blob Storage**, and select **Finish**. 
...
4. Switch to the **Connection** tab and complete the following steps: 
   ...
   a. Select **AzureStorageLinkedService** for the linked service.

   b. Select the **Browse** button for the **File path**.

      ![Browse for the input file](./media/quickstart-create-data-factory-portal/file-path-browse-button.png)
   c. In the **Choose a file or folder** window, browse to the **input** folder in the **adftutorial** container, select the **emp.txt** file, and select **Finish**.
...
5. Repeat the steps to create the output dataset.  
   ...
   a. Select the **+ (plus)** button in the left pane, and select **Dataset**.

   b. On the **New Dataset** page, select **Azure Blob Storage**, and select **Finish**.
   ...
   e. Enter `@CONCAT(pipeline().RunId, '.txt')` for the file name. Each time you run a pipeline, the pipeline run has a unique ID associated with it. The expression concatenates the run ID of the pipeline with **.txt** to evaluate the output file name. For the supported list of system variables and expressions, see [System variables](control-flow-system-variables.md) and [Expression language](control-flow-expression-language-functions.md).
...
1. Select the **+ (plus)** button, and select **Pipeline**. 
...
3. In the **Activities** toolbox, expand **Data Flow**, and drag the **Copy** activity from the **Activities** toolbox to the pipeline designer surface. You can also search for activities in the **Activities** toolbox. Specify **CopyFromBlobToBlob** for the **name**.
...
1. Before triggering a pipeline, you must publish entities to Data Factory. To publish, select **Publish** in the left pane. 

   ![Publish button](./media/quickstart-create-data-factory-portal/publish-button.png)
2. To trigger the pipeline manually, select **Trigger** on the toolbar, and select **Trigger Now**. 
...
3. To view details about the copy operation, select the **Details** (eye glasses image) link in the **Actions** column. For details about the properties, see [Copy Activity overview](copy-activity-overview.md). 
...
5. You can switch back to the **Pipeline Runs** view from the **Activity Runs** view by selecting **Pipelines** link. 
...
8. Switch to the **Monitor** tab on the left. Select **Refresh** to refresh the list. You see the pipeline runs once every minute from the publish time to the end time. Notice the values in the **Triggered By** column. The manual trigger run was from the step (**Trigger Now**) that you did earlier. 
