...
3. In the **New Linked Service** window, do the following steps:
    ...
    9. To save the linked service, click **Save**.
...
3. In the **New Linked Service** window, do the following steps:
    ...
    3. For **Server name**, select the name of your Azure SQL server from the drop-down list. 
    4. For **Database name**, select the Azure SQL database in which you created customer_table and project_table as part of the prerequisites. 
    6. For **User name**, enter the name of user that has access to the Azure SQL database. 
    7. For **Password**, enter the **password** for the user. 
    8. To test whether Data Factory can connect to your SQL Server database, click **Test connection**. Fix any errors until the connection succeeds. 
    9. To save the linked service, click **Save**.
...
3. You see a new tab opened in the Web browser for configuring the dataset. You also see a dataset in the treeview. In the **General** tab of the Properties window at the bottom, enter **SourceDataset** for **Name**. 

   ![Source dataset - name](./media/tutorial-incremental-copy-multiple-tables-portal/source-dataset-general.png)
4. Switch to the **Connection** tab in the Properties window, and select **SqlServerLinkedService** for **Linked service**. You do not select a table here. The Copy activity in the pipeline uses a SQL query to load the data rather than load the entire table.
...
4. Switch to the **Connection** tab, and do the following steps: 
    ...
    2. Select **[dbo].[watermarktable]** for **Table**.
...
15. Switch to the **Parameters** tab, and do the following steps:
    ...
    2. For **Sink Table Type** property, enter `@{item().TableType}`.
    3. In the **Sink Dataset** section, for **SinkTableName** parameter, enter `@{item().TABLE_NAME}`.
