...
In order to make the journey be easy to start with, we directly use these Stored Procs passing the delta data in via a table variable and then merge the them into destination store. Please be cautious that it is not expecting a "large" number of delta rows (more than 100) to be stored in the table variable.  
