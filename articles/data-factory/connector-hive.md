...
| authenticationType | The authentication method used to access the Hive server. <br/>Allowed values are: **Anonymous**, **UsernameAndPassword**, **WindowsAzureHDInsightService**. Kerberos authentication is not supported now. <br/> For WindowsAzureHDInsightService authentication type, `httpPath` will be filled with "/hive2".| Yes |
...
| DECIMAL | Decimal | Decimal (For precision greater than 28, String will be returned) | 
