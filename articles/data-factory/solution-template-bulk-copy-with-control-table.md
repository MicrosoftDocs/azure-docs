...
2. The parameter *Control_Table_Schema_PatitionID* is the column name in your external control table to store each Partition ID. Please make sure the Partition ID is unique for each partition in source database.
...
7. Click **Debug**, input the parameters and click **Finish**
...
9. (Optional) If you select SQL Data Warehouse as data destination, you also need to input the connection of an Azure blob storage as a staging, which is required by SQL Data Warehouse Polybase.  Please make sure the container in blob storage has already created.  
