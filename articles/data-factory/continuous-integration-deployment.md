...
If your development factory has an associated git repository, you can override the default Resource Manager template parameters of the Resource Manager template generated by publishing or exporting the template. You might want to override the default ARM parameter configuration in these scenarios:
...
To override the default ARM parameter configuration, go to the **Manage** hub and select **ARM template** in the "Source control" section. Under **ARM parameter configuration** section, click **Edit** icon in "Edit parameter configuration" to open the ARM parameter configuration code editor.
...
Creating a custom ARM parameter configuration creates a file named **arm-template-parameters-definition.json** in the root folder of your git branch. You must use that exact file name.
...
> [!NOTE]
> A custom ARM parameter configuration doesn't change the ARM template parameter limit of 256. It lets you choose and decrease the number of parameterized properties.
...
Here's an example of what an ARM parameter configuration might look like:
