...
-   **Integration Runtimes and sharing**. Integration Runtimes are one of the infrastructural components in your data factory, which undergo changes less often, and are similar across all stages in your CI/CD. This is why we expect you to have the same name and same type of Integration Runtimes across all stages of CI/CD. This also means that if you are looking to share Integration Runtimes across all stages - for instance the Self-hosted Integration Runtimes - one way to achieve this is by hosting the Self-hosted IR in a ternary factory, just for containing the shared Integration Runtimes. Then you can use them in Dev/Test/Prod as a Linked IR type.
