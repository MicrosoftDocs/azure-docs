...
If you are in GIT mode, you can control your ARM templates in such a way that, you can override what properties gets parameterized in the ARM template vs what stays hard coded. Here are the reasons why you want to override the default parameterization template.

1. If you are doing automated CICD, and you want to change some properties during the ARM deployment, but these properties are not parameterized by default.
2. If you have a big factory, that the ARM template generated by default is invalid because it has more than the max allowed parameters (256).

Under these conditions, to override the default parameterization template, you simply need to have a file named `arm-template-parameters-definition.json` in the root folder of the repository. The file name must exactly match this. Data Factory will try to read this file from whichever branch you are currently on in the Azure Data Factory portal, not just from the collaboration branch. This allows you to create/edit this file from a private branch, where you can test your changes via Export ARM template in the UI. Then you can merge it into the collaboration branch. If no file is found, the default template will be used.


### Syntax of a custom parameters file

Here are some guidelines to use when authoring the custom parameters file. The file consists of section for each entity type - trigger/pipeline/linkedservice/dataset/integrationruntime etc.
1. You start by providing the property path under the relevant entity type.
2. When you set a property name to '\*'', you indicate that you want to parameterize all properties under it, (only down to the 1st level not recursively). You can also provide any exceptions to this if desired.
3. When you set the value of a property as a string, you indicate that you want to parameterize the property. Use the format `<action>:<name>:<stype>`.
	1. <action> can be one of the following characters:
		...
		3. `|` is a special case for secrets from Azure Key Vault for connection strings or keys.
	2. <name> is the name of the parameter. If it is blank, it takes the name of the property. If the value starts with a `-` character, the name is shortened. For ex. `AzureStorage1_properties_typeProperties_connectionString` would be shortened to `AzureStorage1_connectionString`.
	3. <stype> is the type of the parameter. If <stype> is blank, the default type is a `string`. Supported values: string/bool/number/object/securestring.
4. When you specify array in the definition file, you indicate that the matching property in the template is an array. Data Factory iterates through all the objects in the array using the definition specified in the Integration Runtime object of the array. The second object, a string, becomes the name of the property, which is used as the name for the parameter for each iteration.
5. It's not possible to have definition specific for a resource instance, any definition will apply to all resources of that type.
6. By default all secure strings like AKV secrets, and secure strings like connection strings, keys, tokens are parameterized.
