...
invalidFileName | One of the key-value pairs within skipErrorFile property bag to determine if you want to skip the particular files, when the file name is invalid for the destination store. <br/> -True: you want to copy the rest by skipping the files having invalid file name. <br/> - False: you want to abort the copy activity once any files have invalid file name. <br/>Be aware this property works when copying binary file from any storage store to ADLS Gen2 or copying binary files from AWS S3 to any storage store only. | True <br/>False(default) | No
