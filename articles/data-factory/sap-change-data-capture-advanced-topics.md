...
For SAP CDC, the properties that are most commonly set via parameters are found in the tabs **Source optons** and **Optimize**.
...
- **Run mode**: valid parameter values are
    ...
    - **fullLoad** for **Full on every run**, which extracts a current full data snapshot without initiating a changa data capture process.
...
- **Key colunms**: key columns are provided as an array of (double-quoted) strings. For example, when working with SAP table **VBAP** (sales order items), the key definition would have to be: ["VBELN", "POSNR"] (or ["MANDT","VBELN","POSNR"] in case the client field is taken into account as well).
...
3. the resulting conditions of steps 1 and 2 are
    ...
    - combined with **AND NOT** for **excluding** conditons.
