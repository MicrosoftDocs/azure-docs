...
| numberPrecision | Specify the maximum number of significant decimal digits. Allowed values range from 1 to 256. Defaults to 256 if not specified. <br>This applies only to NUMBER types that do not have precision and scale explicitly defined in the Oracle database. If you are using Amazon RDS for Oracle version 2.0, this property will only be allowed to be set when `supportV1DataTypes` is `false` and the self-hosted integration runtime version is 5.56.0 or above. | No |
| numberScale | Specify the number of digits after the decimal point. Allowed values range from 0 to 130 and must be less than the precision. Defaults to 130 if not specified. <br>This applies only to NUMBER types that do not have precision and scale explicitly defined in the Oracle database. If you are using Amazon RDS for Oracle version 2.0, this property will only be allowed to be set when `supportV1DataTypes` is `false` and the self-hosted integration runtime version is 5.56.0 or above. | No |
