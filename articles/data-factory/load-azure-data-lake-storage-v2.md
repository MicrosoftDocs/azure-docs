---
title: Load data into Azure Data Lake Storage (Preview) with Azure Data Factory
description:  'Use Azure Data Factory to copy data into Azure Data Lake Storage (Preview)'
services: storage
documentationcenter: ''
author: seguler
manager: jahogg
...
Azure Data Factory (ADF) offers a scale-out, managed data movement solution as opposed to AzCopy, a command line data transfer utility. Due to the scale-out architecture of ADF, it can ingest data at a high throughput. For details, see [Copy activity performance](copy-activity-performance.md).

This article shows you how to use the Data Factory Copy Data tool to load data from _Amazon Web Services S3 service_ into _Azure Data Lake Storage Gen2 (Preview)_. You can follow similar steps to copy data from other types of data stores.

## Prerequisites

* Azure subscription: If you don't have an Azure subscription, create a [free account](https://azure.microsoft.com/free/) before you begin.
* Azure Storage account: If you don't have a Storage account, click [here](https://ms.portal.azure.com/#create/Microsoft.StorageAccount-ARM) to create one.
...
1. On the left menu, select **New** > **Data + Analytics** > **Data Factory**:
   ...
   ![Create a new data factory](./media/load-azure-data-lake-storage-v2/new-azure-data-factory-menu.png)
2. In the **New data factory** page, provide values for the fields that are shown in the following image: 
      
   ![New data factory page](./media/load-azure-data-lake-storage-v2//new-azure-data-factory.png)
...
4. After creation is complete, go to your data factory. You see the **Data Factory** home page as shown in the following image: 
   ...
   ![Data factory home page](./media/load-azure-data-lake-storage-v2/data-factory-home-page.png)

   Select the **Author & Monitor** tile to launch the Data Integration Application in a separate tab.

## Load data into Azure Data Lake Storage (Preview)

1. In the **Get started** page, select the **Copy Data** tile to launch the Copy Data tool: 

   ![Copy Data tool tile](./media/load-azure-data-lake-storage-v2/copy-data-tool-tile.png)
2. In the **Properties** page, specify **CopyFromAmazonS3ToADLS** for the **Task name** field, and select **Next**:

    ![Properties page](./media/load-azure-data-lake-storage-v2/copy-data-tool-properties-page.png)
3. In the **Source data store** page, click **+ Create new connection**:

    ![Source data store page](./media/load-azure-data-lake-storage-v2/source-data-store-page.png)
	
	Select **Amazon S3**, and select **Continue**
	
	![Source data store s3 page](./media/load-azure-data-lake-storage-v2/source-data-store-page-s3.png)
...
4. In the **Specify Amazon S3 connection** page, do the following steps: 
   ...
   ![Specify Amazon S3 account](./media/load-azure-data-lake-storage-v2/specify-amazon-s3-account.png)
...
5. In the **Choose the input file or folder** page, browse to the folder and file that you want to copy over. Select the folder/file, select **Choose**, and then select **Next**:
    ...
    ![Choose input file or folder](./media/load-azure-data-lake-storage-v2/choose-input-folder.png)

6. Choose the copy behavior by selecting the **Copy files recursively** and **Binary copy** (copy files as-is) options. Select **Next**:

    ![Specify output folder](./media/load-azure-data-lake-storage-v2/specify-binary-copy.png)
	
7. In the **Destination data store** page, click **+ Create new connection**, and then select **Azure Data Lake Storage Gen 2 (Preview)**, and select **Continue**:

    ![Destination data store page](./media/load-azure-data-lake-storage-v2/destination-data-storage-page.png)

8. In the **Specify Data Lake Storage connection** page, do the following steps: 

   1. Select your storage account from the "Storage account name" drop down list.
   2. Select **Next**.
   
   ![Specify Azure Data Lake Store account](./media/load-azure-data-lake-storage-v2/specify-adls.png)

9. In the **Choose the output file or folder** page, enter **copyfroms3** as the output folder name, and select **Next**: 

    ![Specify output folder](./media/load-azure-data-lake-storage-v2/specify-adls-path.png)

10. In the **Settings** page, select **Next**:

    ![Settings page](./media/load-azure-data-lake-storage-v2/copy-settings.png)
11. In the **Summary** page, review the settings, and select **Next**:

    ![Summary page](./media/load-azure-data-lake-storage-v2/copy-summary.png)
12. In the **Deployment page**, select **Monitor** to monitor the pipeline (task):

    ![Deployment page](./media/load-azure-data-lake-storage-v2/deployment-page.png)
13. Notice that the **Monitor** tab on the left is automatically selected. The **Actions** column includes links to view activity run details and to rerun the pipeline:
    ...
    ![Monitor pipeline runs](./media/load-azure-data-lake-storage-v2/monitor-pipeline-runs.png)
...
14. To view activity runs that are associated with the pipeline run, select the **View Activity Runs** link in the **Actions** column. There's only one activity (copy activity) in the pipeline, so you see only one entry. To switch back to the pipeline runs view, select the **Pipelines** link at the top. Select **Refresh** to refresh the list. 
    ...
    ![Monitor activity runs](./media/load-azure-data-lake-storage-v2/monitor-activity-runs.png)
...
15. To monitor the execution details for each copy activity, select the **Details** link under **Actions** in the activity monitoring view. You can monitor details like the volume of data copied from the source to the sink, data throughput, execution steps with corresponding duration, and used configurations:
    ...
    ![Monitor activity run details](./media/load-azure-data-lake-storage-v2/monitor-activity-run-details.png)

16. Verify that the data is copied into your Azure Data Lake Storage.
...
- If your source is in the cloud, configure higher "Data Integration Units" to scale out the ADF service (Azure Integration Runtime) which empowers copy.
...
* [Create an HDInsight cluster with ADLS v2](./storage-adfs-quickstart-hdinsight.md)
* [Use an ADLS v2 account in Azure Databricks](./storage-adfs-quckstart-databricks.md)