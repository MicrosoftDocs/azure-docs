...
> [!NOTE]
> Outbound controls vary by service for Azure IR. In Synapse, workspaces have options to limit outbound traffic from the [managed virtual network](../synapse-analytics/security/synapse-workspace-managed-vnet.md) when you use Azure IR. In Data Factory, all ports are opened for [outbound communications](managed-virtual-network-private-endpoint.md#outbound-communications-through-public-endpoint-from-a-data-factory-managed-virtual-network) when you use Azure IR. Azure-SSIS IR can be integrated with your virtual network to provide [outbound communications](azure-ssis-integration-runtime-standard-virtual-network-injection.md) controls.
...
Azure integration runtime supports connecting to data stores and compute services with publicly accessible endpoints. When you enable Managed Virtual Network, Azure integration runtime supports connecting to data stores by using Private Link in a private network environment. In Synapse, workspaces have options to limit outbound traffic from the IR [managed virtual network](../synapse-analytics/security/synapse-workspace-managed-vnet.md). In Data Factory, all ports are opened for [outbound communications](managed-virtual-network-private-endpoint.md#outbound-communications-through-public-endpoint-from-a-data-factory-managed-virtual-network). The Azure-SSIS IR can be integrated with your virtual network to provide [outbound communications](azure-ssis-integration-runtime-standard-virtual-network-injection.md) controls.

### Azure IR compute resource and scaling

Azure integration runtime provides a fully managed, serverless compute in Azure. You don't have to worry about infrastructure provisioning, software installation, patching, or capacity scaling. In addition, you only pay for what you use.
...
Activity dispatch is a lightweight operation to route the activity to the target compute service, so there's no need to scale up the compute size for this scenario.
...
> [!NOTE] 
> Azure integration runtime has properties related to Data Flow runtime, which defines the underlying compute infrastructure used to run the data flows. 
...
> [!NOTE] 
> Use self-hosted integration runtime to support data stores that require a bring-your-own driver, such as SAP HANA and MySQL. For more information, see [supported data stores](copy-activity-overview.md#supported-data-stores-and-formats).
...
The self-hosted IR is logically registered to the Data Factory or Synapse workspace, and you provide the compute used to support its functionalities. Therefore there's no explicit location property for self-hosted IR. 
...
Each external transformation activity that uses an external compute engine has a target compute linked service, which points to an integration runtime. This IR instance determines the location from where that external hand-coded transformation activity is dispatched.

### Data Flow activity

Data Flow activities are executed on their associated Azure integration runtime. The data flow properties in your Azure IR determine the Spark compute used, and are fully managed by the service.
