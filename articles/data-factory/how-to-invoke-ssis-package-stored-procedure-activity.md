...
### Azure SQL Database 
The walkthrough in this article uses an Azure SQL database that hosts the SSIS catalog. You can also use an Azure SQL Managed instance.
...
4. You can run the following **query** against the SSISDB database in your logical SQL server to verify that the package executed. 
...
6. Run the following command after the next hour. For example, if the current time is 3:25 PM UTC, run the command at 4 PM UTC. 
    ...
    You can run the following query against the SSISDB database in your logical SQL server to verify that the package executed. 
