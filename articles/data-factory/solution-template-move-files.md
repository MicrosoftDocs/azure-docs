...
This article describes a solution template that you can use to move files from one folder to another folder between file stores. The pipeline generated by this template will enumerate the files under a folder on the source file store, and then copies each of the files from this folder to another folder on the destination file store. After each file has been successfully copied to the data destination store, the same file on the source file store wil be deleted.

> [!NOTE]
> This template is widely used in the scenario like this: after some period of time, some files are dropped to a landing folder of your source store. And then ADF pipeline is triggered to move those files from source to destination store. After ADF pipeline complete the job of files copying to the destination and files deleting from source store, the landing folder become empty again. The pipeline will be waiting for files dropping in next batch. 
Please be aware that this template is designed to move files rather than moving folders.  If you want to move the folder by changing the dataset to make it contain a folder path only, and then using the copy activity and delete activity to reference to the same dataset representing a folder, you need to be very careful. It is because you have to make sure that there will NOT be new files arriving into the folder between copying operation and deleting operation. If there are new files arriving at the folder at the moment when your copy activity just completed the copy job but the Delete activity has not been stared, it is possible that the Delete activity will delete this new arriving file which has NOT been copied to the destination yet by deleting the entire folder.

## About this solution template

This template enumerates the files from your source storage store. It then moves each of them to the destination store.

The template contains five activities:
- **GetMetadata** gets the list of objects including the files and sub-folders from your folder on source file store. It will not retrive the list recursively. 
...
1. Go to the **Move files** template. Select existing connection or create a **New** connection to your source file store where you want to move files from. Please be aware that **DataSource_Folder** and **DataSource_File** are reference to the same connection of your source file store.
