...
| connectionString | An ODBC connection string to connect to Azure Database for MariaDB. Mark this field as a SecureString to store it securely in Data Factory, or [reference a secret stored in Azure Key Vault](store-credentials-in-key-vault.md). | Yes |
