---
title: Copy data to or from Azure Cosmos DB (MongoDB API) by using Data Factory | Microsoft Docs
description: Learn how to copy data from supported source data stores to or from Azure Cosmos DB (MongoDB API) to supported sink stores by using Data Factory.
...
---
# Copy data to or from Azure Cosmos DB (MongoDB API) by using Azure Data Factory
...
You can copy data from Azure Cosmos DB (MongoDB API) to any supported sink data store, or copy data from any supported source data store to Azure Cosmos DB (MongoDB API). For a list of data stores that Copy Activity supports as sources and sinks, see [Supported data stores and formats](copy-activity-overview.md#supported-data-stores-and-formats).

You can use the Azure Cosmos DB (MongoDB API) connector to:

- Copy data from and to the Azure Cosmos DB [MongoDB API](https://docs.microsoft.com/azure/cosmos-db/mongodb-introduction).
...
The following sections provide details about properties you can use to define Data Factory entities that are specific to Azure Cosmos DB (MongoDB API).

## Linked service properties

The following properties are supported for the Azure Cosmos DB (MongoDB API) linked service:

| Property | Description | Required |
|:--- |:--- |:--- |
| type | The **type** property must be set to **CosmosDbMongoDbApi**. | Yes |
| connectionString |Specify the connection string for your Azure Cosmos DB MongoDB API. You can find it in the Azure portal -> your Cosmos DB blade -> primary or secondary connection string, with the pattern of `mongodb://<cosmosdb-name>:<password>@<cosmosdb-name>.documents.azure.com:10255/?ssl=true&replicaSet=globaldb`. <br/><br />Mark this field as a **SecureString** type to store it securely in Data Factory. You can also [reference a secret stored in Azure Key Vault](store-credentials-in-key-vault.md). |Yes |
...
This section provides a list of properties that the Azure Cosmos DB (MongoDB API) source and sink support.
...
| writeBatchSize | The **writeBatchSize** property controls the size of documents to write in each batch. You can try increasing the value for **writeBatchSize** to improve performance and decreasing the value if your document size being large. |No<br />(the default is **5**) |
