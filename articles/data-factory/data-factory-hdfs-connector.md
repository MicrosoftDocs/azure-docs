...
| compression | Specify the type and level of compression for the data. Supported types are: **GZip**, **Deflate**, and **BZip2** and supported levels are: **Optimal** and **Fastest**. Note that compression is supported only for data in the **TextFormat**. See [Compression support](#compression-support) section for more details.  | No |
