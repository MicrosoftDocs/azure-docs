...
| 3203           | The cluster is in Terminated state, not available to receive jobs. Please fix the cluster or retry later. | The cluster has been terminated.    For interactive clusters, this might be a race condition. | The best way to avoid this error is to use job clusters.             |
...
| 2328         | Internal server error occurred while processing the request. Please retry the request   or contact support. | This error happens in HDInsight on-demand clusters.                              | This error comes from an HDInsight service when the HDInsight provisioning fails. Contact the HDInsight team and provide the on-demand cluster name. |
