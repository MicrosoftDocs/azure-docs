...
Based on a condition applies one value or the other. "Other" is considered NULL if it is unspecified. Both the values must be compatible(numeric, string...)
...
Right pads the string by the supplied padding until it is of a certain length. If the string is equal to or greater than the length it is a no-op
* ``rpad('great', 10, '-') -> 'great-----'``
* ``rpad('great', 4, '-') -> 'great'``
* ``rpad('great', 8, '<>') -> 'great<><'``
*********************************
...
Converts a string to a date given an optional timestamp format. If the timestamp is omitted the default pattern yyyy-[M]M-[d]d hh:mm:ss[.f...] is used
...
Checks if the row is marked for insert. Transformations that take more than one input stream can pass the (one-based) index of the stream. Default value for the stream index is one
* ``isInsert() -> true``
* ``isInsert(1) -> false``
*********************************
<code>isUpdate</code>
...
Checks if the row is marked for update. Transformations that take more than one input stream can pass the (one-based) index of the stream. Default value for the stream index is one
* ``isUpdate() -> true``
* ``isUpdate(1) -> false``
*********************************
<code>isDelete</code>
...
Checks if the row is marked for delete. Transformations that take more than one input stream can pass the (one-based) index of the stream. Default value for the stream index is one
* ``isDelete() -> true``
* ``isDelete(1) -> false``
*********************************
<code>isMatch</code>
...
Checks if the row is matched at lookup. Transformations that take more than one input stream can pass the (one-based) index of the stream. Default value for the stream index is 1
* ``isMatch() -> true``
* ``isMatch(1) -> false``
*********************************
<code>isError</code>
...
Checks if the row is marked as error. Transformations that take more than one input stream can pass the (one-based) index of the stream. Default value for the stream index is one
* ``isError() -> true``
* ``isError(1) -> false``
*********************************
<code>isIgnore</code>
...
Checks if the row is marked to be ignored. Transformations taking more than one input stream can pass the (one-based) index of the stream. Default value for the stream index is one
* ``isIgnore() -> true``
* ``isIgnore(1) -> false``
*********************************
<code>sum</code>
...
Gets the first value of a column group. If the second parameter ignoreNulls is omitted, it is assumed to be false
...
Gets the value of the first parameter evaluated n rows before the current row. The second parameter is the number of rows to look back. The default value is one. If there are not as many rows a value of null is returned unless a default value is specified
* ``lag(amount, 2) -> 60``
...
Gets the value of the first parameter evaluated n rows after the current row. The second parameter is the number of rows to look forward. The default value is one. If there are not as many rows a value of null is returned unless a default value is specified
* ``lead(amount, 2) -> 60``
