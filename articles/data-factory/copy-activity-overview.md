...
When copying data into SQL database/Azure Synapse Analytics, if the destination table does not exist, Azure Data Factory [copy activity](copy-activity-overview.md) now supports automatically creating it based on the source data. It aims to help you quickly get started to load the data and evaluate SQL database/Azure Synapse Analytics. After the data ingestion, you can review and adjust the sink table schema according to your needs.

This feature is supported for the following connectors. You can find the option on *ADF authoring UI* –> *Copy activity sink* –> *Table option* –> *Auto create table*, or via **tableOption** property in copy activity sink payload.
