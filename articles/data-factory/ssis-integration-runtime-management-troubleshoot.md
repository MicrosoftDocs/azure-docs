...
If there's any issue in provisioning or deprovisioning SSIS IR, you'll see an error message in the Azure Data Factory (ADF) portal or returned from a PowerShell cmdlet. The error always appears in the format of an error code with a detailed error message.

If the error code is InternalServerError, this means that the service has transient issues and that you should retry the operation later. Contact Azure Data Factory support team if a retry doesn’t help.

There are three major external dependencies that can cause errors: an Azure SQL Database server or managed instance, a custom setup script, and a virtual network configuration (if the error code is not InternalServerError).

## Azure SQL Database server or managed instance issues

An Azure SQL Database server or managed instance is required if you're provisioning SSIS IR with an SSIS catalog database. The Azure SQL Database server or managed instance should be accessible by the SSIS IR. The account of the Azure SQL Database server or managed instance should have permission to create an SSIS catalog database (SSISDB). If there's an error, an error code with a detailed SQL exception message will be shown in the ADF portal. Use the following steps to troubleshoot the error codes.

### AzureSqlConnectionFailure

You might see this issue when you're provisioning a new SSIS IR or while IR is running.

If you experience this error during IR provisioning, and if you get a detailed SqlException message in the error message, it might occur for one of the following reasons:
...
* Login failed during AAD authentication (managed identity). Add the managed identity of your factory into an AAD group, and make sure the managed identity has access permissions to your catalog database server.
...
If you see the error when IR is running, it’s likely that network security group or firewall changes are preventing the SSIS IR worker node from accessing the Azure SQL Database server or managed instance. Unblock the SSIS IR worker node to access the Azure SQL Database server or managed instance.

### CatalogCapacityLimitError

Here's what this kind of error message looks like: “The database 'SSISDB' has reached its size quota. Partition or delete data, drop indexes, or consult the documentation for possible resolutions.” 
...
* A timeout that occurs during database creation, such as an execution timeout or a DB operation timeout. You can retry later to see whether the issue is now resolved. If the retry doesn’t work, contact the Azure SQL Database server or Managed Instance support team.
...
The custom setup script container will also be checked while IR is running, as SSIS IR is regularly updated. This updating requires access to the container to download the custom setup script and install it again. Also checked: whether the container is accessible and whether the main.cmd file exists.
...
Stop the IR first if the IR is in a running state, reconfigure the IR with new custom setup container SAS URI, and then restart the IR.
...
This error means the execution of custom setup script (main.cmd) failed. Try the script on your local machine first, or check custom setup execution logs on your blob container.

### CustomSetupScriptTimeout

This error indicates an execute custom setup script timeout. Make sure that your blob container contains only the necessary custom setup files. You can also check custom setup execution logs in your blob container. The maximum period for custom setup is 45 minutes before it times out, and the maximum period includes the time to download all files from your container and install them on SSIS IR. If you need a longer period, raise a support ticket.
...
This type of error might resemble this: “SubnetId is not enabled for current account. Microsoft.Batch resource provider is not registered under the same subscription of VNet.”

These details mean that Azure Batch can't access your VNet. Register Microsoft.Batch resource provider under the same subscription of VNet.

### InvalidPropertyValue

This type of error might resemble one of the following: 
...
This type of error can look like this: “Failed to provision Integration Runtime in Vnet. If the DNS server or NSG settings are configured, make sure the DNS server is accessible and NSG is configured properly.”

In this situation, you probably have a customized configuration of DNS server or NSG settings, which prevents the Azure Server name required by SSIS IR from being resolved or accessed. For more information, see [SSIS IR VNet configuration](https://docs.microsoft.com/azure/data-factory/join-azure-ssis-integration-runtime-virtual-network) document. If you’re still having problems, contact the Azure Data Factory support team.
