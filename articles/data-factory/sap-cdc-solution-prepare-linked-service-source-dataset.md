...
description: This topic introduces and describes preparation of the linked service and source dataset for SAP change data capture (Preview) in Azure Data Factory.
author: swinarko
...
1.	On ADF Studio, go to the **Linked services** section of **Manage** hub and select the **New** button to create a new linked service.
    ...
    :::image type="content" source="media/sap-cdc-solution/sap-cdc-new-linked-service.png" alt-text="Shows a screenshot of the manage hub in Azure Data Factory with the New Linked Service button highlighted.":::
...
1.	Drag & drop the **Copy data** activity onto the canvas of new pipeline, go to the **Source** tab of ADF copy activity, and select the **New** button to create a new source dataset.
    ...
    :::image type="content" source="media/sap-cdc-solution/sap-cdc-copy-source-configuration.png" alt-text="Shows a screenshot of the Copy data activity's Source configuration.":::
...
1.	Select your new SAP ODP linked service for the new source dataset and set the rest of its properties.
    ...
    1.	For the **Context** property, select the context of data extraction via ODP, such as: 
        ...
        1.	_BW_ for extracting InfoProviders or InfoObjects from SAP BW or BW/4HANA
        1.	_SAPI_ for extracting SAP extractors from SAP ECC
        1.	_SLT~_<_your queue alias_> for extracting SAP application tables from SAP source systems via SLT replication server as a proxy
    ...
    1.	For the **Object name** property, select the name of data source object to extract under the selected data extraction context.  If you connect to your SAP source system via SLT replication server as a proxy, the **Preview data** feature is not supported for now.
...
1.	For the **Extraction** mode property of ADF copy activity, select one of the following modes:
    ...
    1.	_Recovery_ for repeating the last extraction that was part of a failed pipeline run
...
1.	For the **Subscriber process** property of ADF copy activity, enter a unique name to register and identify this ADF copy activity as a “delta” subscriber of the selected data source object, so your SAP system can manage its subscription state to keep track of data changes produced in ODQ and consumed in consecutive extractions, eliminating the need for watermarking them yourself.  For example, you can name it <_your pipeline name_>_<_your copy activity name_>.
    ...
    :::image type="content" source="media/sap-cdc-solution/sap-cdc-copy-source-configuration.png" alt-text="Shows a screenshot of the SAP CDC (Preview) source configuration in a Copy activity.":::
...
1.	If you want to extract only data from some columns/rows, you can use the column projection/row selection features:
    ...
    :::image type="content" source="media/sap-cdc-solution/sap-cdc-copy-source-projection-configuration.png" alt-text="Shows a screenshot of the SAP CDC (Preview) source configuration with the Projection, Selection, and Additional columns sections highlighted.":::
    ...
    1.	For the **Selection** property of ADF copy activity, select the **New** button to add a new row selection condition containing **Field name/Sign/Option/Low/High** arguments. 
        ...
        :::image type="content" source="media/sap-cdc-solution/sap-cdc-copy-selection-additional-columns.png" alt-text="Shows a screenshot of the SAP CDC (Preview) source configuration for a Copy activity with the Selection and Additional columns sections highlighted."::: 
...
1.	Go to the **Sink** tab of ADF copy activity and select an existing sink dataset or create a new one for any data store, such as Azure Blob Storage/ADLS Gen2.
    ...
    :::image type="content" source="media/sap-cdc-solution/sap-cdc-staging-dataset.png" alt-text="Shows a screenshot of the staging dataset configuration for the solution.":::
...
1.	Go to the **Settings** tab of ADF copy activity and increase throughput by setting the **Degree of copy parallelism** property to concurrently extract data packages produced in ODQ by your SAP system.
    ...
    :::image type="content" source="media/sap-cdc-solution/sap-cdc-copy-settings-parallelism.png" alt-text="Shows a screenshot of a Copy activity with the Degree of parallelism setting highlighted.":::
...
1.	Adjust the maximum size of data packages produced in ODQ to fine-tune parallel extractions.  The default size is 50 MB, so 3 GB of SAP table/object will be extracted into 60 files of raw SAP data in ADLS Gen2.  Lowering it to 15 MB could increase throughput, but will produce more (200) files.  To do so, select the **Code** button of ADF pipeline to edit the **maxPackageSize** property of ADF copy activity.
    ...
    :::image type="content" source="media/sap-cdc-solution/sap-cdc-copy-code-configuration.png" alt-text="Shows a screenshot of a pipeline with the Code configuration button highlighted.":::

    :::image type="content" source="media/sap-cdc-solution/sap-cdc-copy-code-1.png" alt-text="Shows a screenshot of the code configuration for a pipeline with the maxPackageSize setting highlighted.":::
...
:::image type="content" source="media/sap-cdc-solution/sap-cdc-simple-custom-table.png" alt-text="Shows a screenshot of a simple custom table in SAP.":::
...
:::image type="content" source="media/sap-cdc-solution/sap-cdc-sample-data-in-adf.png" alt-text="Shows sample CDC data from SAP loaded into ADF.":::
...
:::image type="content" source="media/sap-cdc-solution/sap-cdc-sample-data-after-deletions.png" alt-text="Shows sampe CDC data from SAP loaded into ADF after deletions were made.":::

## Next steps

[Debug ADF copy activity issues by sending SHIR logs](sap-cdc-debug-shir-logs.md)