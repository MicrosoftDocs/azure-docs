...
- When developing a pipeline, developer can add place holder inactive activities before filling all the required fields. For instance, I need a Copy activity from SQL Server to Data warehouse, but I haven't set up all the connections yet. So I'll use an _inactive_ copy activity as the place holder for iterative development process.
...
Note that an inactive activity never actually runs. This means the activity won't have an output or an error field. Any reference to these fields throw errors downstream.
