...
| OPTION 2: S3 prefix<br>- prefix | Prefix for the S3 key name under the given bucket configured in dataset to filter source S3 files. S3 keys whose name starts with `bucket_in_dataset/this_prefix` are selected. It utilizes S3's service side filter which provide better performance than wildcard filter. | No |
