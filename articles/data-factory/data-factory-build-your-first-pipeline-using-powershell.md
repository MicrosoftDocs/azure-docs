<properties
	...
	description="This tutorial shows you how to create a sample data pipeline that transforms data using Azure HDInsight using Azure PowerShell"
...
In this article, you will learn how to use the Azure PowerShell to create your first pipeline. This tutorial consists of the following steps:
...
1.	Create a JSON file named **HDInsightOnDemandLinkedService.json** in the **C:\ADFGetStartedPSH** with the following content. 
...
2. Run the **New-AzureDataFactoryLinkedService** cmdlet to create the linked service: **HDInsightOnDemandLinkedService**.
...
2. Run the following command in the Azure PowerShell to create the Data Factory table.
...
3.	Run **Get-AzureDataFactoryRun** to get the details of activity runs for a **specific** slice. 
	...
	You can keep running this cmdlet until you see the slice in Ready state or Failed state. When the slice is in Ready state, check the partitioneddata folder in the data container in your blob storage for the output data.  Note that the creation of an on-demand HDInsight cluster usually takes sometime. 
