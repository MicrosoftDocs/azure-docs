...
- A single self-hosted integration runtime can be used for multiple on-premises data sources. A **single self-hosted integration runtime  can** be shared with another data factory within the same Azure Active Directory tenent. For more details, see [sharing a self-hosted integration runtime](#sharing-the-self-hosted-integration-runtime-ir-with-multiple-data-factories).
