...
The integration runtime (IR) is the compute infrastructure that Azure Data Factory and Synapse Analytics pipelines use to provide data-integration capabilities across different network environments. For details about IR, see [Integration runtime overview](concepts-integration-runtime.md).
...
3. Azure Data Factory/Synapse Analytics communicates with the self-hosted integration runtime to schedule and manage jobs. Communication is via a control channel that uses a shared [Azure Relay](../azure-relay/relay-what-is-it.md#wcf-relay) connection. When an activity job needs to be run, the service queues the request along with any credential information. It does so in case credentials aren't already stored on the self-hosted integration runtime. The self-hosted integration runtime starts the job after it polls the queue.
...
Use the following steps to create a self-hosted IR using Azure Data Factory/Synapse Analytics UI.  In Synapse Analytics the steps are the same, but may differ slightly from the screenshots below:

1. On the **Let's get started** page of Azure Data Factory or Synapse Analytics UI, select the [Manage tab](./author-management-hub.md) from the leftmost pane.
...
- Higher availability of the self-hosted integration runtime so that it's no longer the single point of failure in your big data solution or cloud data integration with Data Factory/Synapse Analytics. This availability helps ensure continuity when you use up to four nodes.
...
If you see error messages like the following ones, the likely reason is improper configuration of the firewall or proxy server. Such configuration prevents the self-hosted integration runtime from connecting to Data Factory/Synapse Analytics to authenticate itself. To ensure that your firewall and proxy server are properly configured, refer to the previous section.
...
> For the communication to Azure Data Factory and Synapse Analytics workspaces, you can use service tag **DataFactoryManagement** in the NSG rule setup.
