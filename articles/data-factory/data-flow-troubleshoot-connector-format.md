...
### 17. Consume JSON files generated by copy activities

#### Symptoms

If you use the copy activity to generate some JSON files, and then try to read these files in data flows, it may fail with the error message below:<br/>
...
So you will experience the issues if the following criteria are met:
...
- The copy sink is set to use "Array of objects" file pattern (no matter whether "Single document" is enabled or not in the data flow JSON source).
...
>[!Note]
> Using "Set of objects" is also the recommended practice from the performance perspective. As the "Single document" JSON in the data flow can't enable parallel read for single large files, this recommendation won't have any negative impact.

### 22. The query with parameters does not work
...
You encounter this error due to the wrong configuration.
...
### 16. Account kind of Storage (general purpose v1) doesn't support service principal and MI authentication
...
### 18. Unable to connect to the Snowflake linked service

#### Symptoms

You encounter the following error when you create the linked service to a Snowflake that is in the public network, and you use the auto-resolve integration runtime.
...
To solve the issue, you need to change the account name format. The role should be one of the roles shown in the following picture, but the default one is **Public**.
...
### 19. SQL access control error: "Insufficient privileges to operate on schema"
...
### 20. Failed with the error: "SnowflakeSQLException: IP x.x.x.x is not allowed to access Snowflake. Contact your local security administrator"
...
1. If you are running a data flow in the "southcentralus" region, you need to allow the access from all addresses with name "AzureCloud.southcentralus", for example:
...
### 21. Query in the source does not work
...
If you meet up error with the Snowflake query, please check whether some identifiers (table name, schema name, column name, etc.) are case-sensitive with the following steps:
...
### 23. Unable to connect to the SQL Database
...
When you meet up this error, it may be caused by the following issues:
...
- For the Cause 1, you can use the copy activity to move the table data from database “Supplier TAR” to another database “xxxx” which does not contain spaces, then use the data flow to set the transformation that you need based on “xxxx”.

- For the Cause 2, please check the firewall settings of Azure SQL Database and set it as "Allow access to Azure services" rather than set the fixed IP range.

    :::image type="content" source="./media/data-flow-troubleshoot-connector-format/allow-access-to-azure-service.png" alt-text="Screenshot that shows how to allow access to Azure service in firewall settings."::: 

### 24. Syntax error when using queries as input
...
### 25. Failed with the error: "SQLServerException: 111212; Operation cannot be performed within a transaction."
...
#### Recommendation
You need to confirm if your SQL Database is a Synapse dedicated SQL pool. If so, please use Azure Synapse Analytics as a connector shown in the picture below.
...
### 26. Data with the decimal type become null
...
You can use toDecimal (IDecimal, scale, precision) to figure out if the original data can be cast to target scale and precision. If it returns null, it means that the data cannot be cast and furthered when inserting.

## ADLS Gen2

### 27. Failed with error: "Error while reading file XXX. It is possible the underlying files have been updated"

#### Symptoms

When you use the ADLS Gen2 as a sink in the data flow (preview data, debug/trigger run, etc.) and the partition setting in **Optimize** tab in the **Sink** stage is not default, you may find job fail with the following error message:
...
1. You do not assign a proper permission to your MI/SP authentication.
1. You might have a customized job to handle files that you don't want, which will affect the data flow's middle output.

#### Recommendation
1. Check if your linked service has the R/W/E permission for Gen2. If you use the MI auth/SP authentication, please at least grant the Storage Blob Data Contributor role in the Access control (IAM).
...
### 28. Failed with SQLServerException: "Not able to validate external location because the remote server returned an error: (403)"
...
For Cause 3, please work around it with one of the following options:
...
### 29. Failed with error: "This operation is not permitted on a non-empty directory"
...
#### Recommendation
Create an Azure Data Lake Gen2 linked service to the storage, and select the Gen2 storage as the staging linked service in the "Execute Mapping Data flow" activity.


## ADLS Gen1

### 30. Fail to create files with service principle authentication

#### Symptoms
When you try to move/transfer data from different sources into the ADLS gen1 sink, if the linked service's authentication method is the service principle authentication, you may find job will fail with following error message:
...
#### Cause
The RWX permission is not set correctly or the dataset property is not set correctly.
...
- If the target folder has the correct permission and you use the file name property in the data flow to target to the right folder and file name, but the file path property of the dataset is not set to the target file path (usually leave not set), as the example shown in the following pictures, you will encounter this failure because the backend system tries to create files based on the file path of the dataset, and the file path of the dataset doesn't have the correct permission.
    ...
    1. Assign the WX permission into the file path of the dataset.
