...
- You need to install ADF self-hosted IR on a Windows machine behind corporate firewall for submitting DistCp command to your hadoop cluster, and monitoring the copy status. Given this machine will not be the engine to move data (for control purpose only), the capacity of the machine does not impact the throughput of data movement.
