...
Azure Data Factory provides a performant, robust, and cost-effective mechanism to migrate data at scale from on premise HDFS to Azure Blob Storage or Azure Data Lake Storage Gen2. Basically, Azure Data Factory supports two approaches to migrate data from on premise HDFS to Azure.  

- ADF distcp mode. ADF support using [DistCp](https://hadoop.apache.org/docs/current3/hadoop-distcp/DistCp.html) to copy files as-is into Azure Blob (including [staged copy](https://docs.microsoft.com/en-us/azure/data-factory/copy-activity-performance#staged-copy)) or Azure Data Lake Store, in which case it can fully leverage your cluster's power instead of running on the Self-hosted Integration Runtime. It will provide better copy throughput especially if your cluster is very powerful. Based on your configuration in Azure Data Factory, copy activity automatically construct a distcp command, submit to your Hadoop cluster, and monitor the copy status. By using ADF integrated with DistCp, customer can not only leverage your existing powerful cluster to achieve the best copy throughput but also get the benefit of flexible scheduling and unified monitoring experience from ADF. So by default, ADF Distcp mode is the recommended way to migrate data from on premise Hadoop cluster to Azure.
- ADF built-in IR mode. Under some scenarios, DistCp cannot work for your cases (For example, in VNET environment, Distcp tool does not support ExpressRoute private peering + Storage VNet endpoint). Besides, sometime you do not want to use your existing Hadoop cluster as the engine to migrate the data since it will bring heavy loads on your cluster which may impact the performance of the existing ELT work. If that case, you can use ADF built-in capability, where ADF’s integration runtime can be used to copy the data from on-prem HDFS to Azure.
...
In ADF built-in IR mode, it also allows parallelism at different levels, which can fully utilize your network bandwidth as well as storage IOPS and bandwidth to maximize data movement throughput by manually scaling up the self-hosted IR machine or scale out to self-hosted IR multiple machines.

- A single copy activity can take advantage of multiple self-hosted IR machines. You can manually scale up the machine or scale out to multiple machines  ([up to 4 nodes](https://docs.microsoft.com/azure/data-factory/create-self-hosted-integration-runtime#high-availability-and-scalability)), and a single copy activity will partition its file set across all nodes. 
...
You can get more details from [copy activity performance guide](https://docs.microsoft.com/en-us/azure/data-factory/copy-activity-performance)

## Resilience

In ADF distcp mode, you can leverage different distcp command line parameters (For example, -i, ignore failures; -update, write data when source file and destination file differ in size) to achieve different level of resilience.
...
- You need to install one ADF self-hosted IR in your windows machine behind corporate firewall for submitting distcp command to your Hadoop cluster, and monitor the copy status. Given this machine will not be used for moving data, so the CPU and memory for the machine is not required to be high.
...
- In this architecture, data migration is done over a private peering link Azure Express Route such that data never traverses over public Internet.
- Distcp tool does not support ExpressRoute private peering + Storage VNet endpoint, so you are encouraged to use ADF built-in capability via integration runtime to migrate the data.
...
- To authenticate to HDFS, you can use [either Windows (Kerberos) or Anonymous](https://docs.microsoft.com/en-us/azure/data-factory/connector-hdfs#linked-service-properties). 
...
In ADF built-in IR mode, data partition is recommended especially when migrating more than 10 TB of data. To partition the data, leverage the folder names on HDFS, and then each ADF copy job can copy one folder partition at a time. You can run multiple ADF copy jobs concurrently for better throughput.
...
- Migrating data using second solution architecture (ADF built-in IR mode)
- 1 PB is divided into 1 K partitions and each copy moves one partition
- Each copy is configured with one self-hosted IR against 4 machines and achieves 500 MBps throughput.
...
- [HDFS connector](https://docs.microsoft.com/en-us/azure/data-factory/connector-hdfs)
