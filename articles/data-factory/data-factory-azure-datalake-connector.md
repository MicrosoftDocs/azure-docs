...
| AAD user and the OAuth-based source is in the same tenant as the tenant of data factory. | <p> 90 days if a slice based on your Azure Data Lake Store linked service runs at least once every 14 days. </p><p>14 days if no slice has run for 14 days after the last slice run.</p> |
