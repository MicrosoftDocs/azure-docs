...
| AAD user and the OAuth-based source is in the same tenant as the tenant of data factory. | 14 days after the last slice run, If no slices based on OAuth-based linked service run for 14 days since the last slice run.<p>90 days, if a slice based on OAuth-based linked service runs at least once every 14 days.</p> |
