...
| :----------- | :------- |
| `sslMode` is supported. Valid values including `Disable`, `Allow`, `Prefer`, `Require`, `Verify-CA`, `Verify-Full`. The default value of `sslMode` is `Verify-Full`. | `sslMode` is not supported.|
| `UseDataEncryption` is supported. The default value of `UseDataEncryption` is 1. | `UseDataEncryption` is not supported. |
