...
2. Create a **Web Activity** to get the access token from SharePoint Online:
    ...
    - **Body**:  `grant_type=client_credentials&client_id=[Client-ID]@[Tenant-ID]&client_secret=[Client-Secret]&resource=00000003-0000-0ff1-ce00-000000000000/[Tenant-Name].sharepoint.com@[Tenant-ID]`. Replace the client ID(Application ID), client secret(Application key), tenant ID and tenant name(Sharepoint tenant).
