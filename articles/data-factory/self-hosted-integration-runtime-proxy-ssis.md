...
## Using Windows authentication in staging task
If your staging task on Self-Hosted IR requires Windows authentication, you need to [configure SSIS package to use the same Windows authentication](https://docs.microsoft.com/en-us/sql/integration-services/lift-shift/ssis-azure-connect-with-windows-auth?view=sql-server-ver15). Then the staging task will be launched with specified account on Self-Hosted IR. To ensure staging task can be started properly, Self-Hosted IR service account (`NT SERVICE\DIAHostService` by default) and Windows Authentication account require particular security policies. On Self-Hosted IR server, please open `Local Security Policy` -> `Local Policies` -> `User Rights Assignment`:
...
Please assign necessary policy to your Windows Authentication account. `NT SERVICE\DIAHostService` should be assigned both polices during installation. If you are using customized account to launch Self-Hosted IR service, please assign necessary policies to your account too. 
