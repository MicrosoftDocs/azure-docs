...
- To enable/disable SSIS package executions on Self-hosted integration runtime nodes, please refer to this document https://learn.microsoft.com/azure/data-factory/create-self-hosted-integration-runtime?tabs=data-factory#set-up-an-existing-self-hosted-ir-via-local-powershell to manage self-hosted IR via powershell. ExecuteSsisPackage property is true by default if self-hosted IR upgrades to versions higher than 5.28.0. If self-hosted IR is newly installed with a version higher than 5.28.0, then ExecuteSsisPackage property is by default false.
