...
This feature can only be enabled on SSIS Data Flow Task and Execute SQL Task for now. 
...
Enabled on Execute SQL Task, this feature will run it on your self-hosted IR. 
...
By using the latest SSDT as either the SSIS Projects extension for Visual Studio or a standalone installer, you can find a new `ConnectByProxy` property in the connection managers for supported data flow components and `ExecuteOnProxy` property in Execute SQL Task.
...
When you design new packages containing Execute SQL Tasks that run on premises, you can enable the `ExecuteOnProxy` property by setting it to *True* in the **Properties** pane of relevant tasks themselves.

![Enable ConnectByProxy property](media/self-hosted-integration-runtime-proxy-ssis/shir-proxy-properties.png)
...
The on-premises staging tasks/Execute SQL Tasks that run on your self-hosted IR are billed separately, just as any data movement activities that run on a self-hosted IR are billed. This is specified in the [Azure Data Factory data pipeline pricing](https://azure.microsoft.com/pricing/details/data-factory/data-pipeline/) article.
