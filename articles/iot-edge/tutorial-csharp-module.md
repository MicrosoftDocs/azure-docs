...
The IoT Edge module that you create in this tutorial filters the temperature data generated by your device so that it only sends messages upstream when the temperature is above a specified threshold. This type of analysis at the edge is useful for organizations that want to reduce the ammount of data communicated to and stored in the cloud. 
...
3. Push the image to your Docker repository. Use the **View | Command Palette ... | Edge: Push IoT Edge module Docker image** menu command and enter the image name in the pop-up text box at the top of the VS Code window. Use the same image name you used in step 1.d.
