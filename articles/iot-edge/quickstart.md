...
Try out Azure IoT Edge in this quickstart by deploying containerized code to a Linux on Windows IoT Edge device. IoT Edge enables you to remotely manage code on your devices so that you can send more of your workloads to the edge. For this quickstart, it's recommended that you use your own Windows Client device to see how easy it is to use Azure IoT Edge for Linux on Windows. If you want to use Windows Server or an Azure VM to create your deployment, follow the steps in the how-to guide on [installing and provisioning Azure IoT Edge for Linux on a Windows device](how-to-provision-single-device-linux-on-windows-symmetric.md).
...
This quickstart walks you through how to set up your Azure IoT Edge for Linux on Windows device. Then, you deploy a module from the Azure portal to your device. The module you use is a simulated sensor that generates temperature, humidity, and pressure data. Other Azure IoT Edge tutorials build on the work you do here by deploying modules that analyze the simulated data for business insights.
...
Create a cloud resource group to manage all the resources you use in this quickstart.
...
Start by creating an IoT hub by using the Azure CLI.
...
The free tier of Azure IoT Hub works for this quickstart. If you previously used IoT Hub and already have a hub, you can use that IoT hub.
...
1. Copy the value of the `connectionString` key from the JSON output and save it. This value is the device connection string. Use it to configure the IoT Edge runtime in the next section.
...
1. In an elevated PowerShell session, run the following command to enable Hyper-V. For more information, see [Hyper-V on Windows 10](/virtualization/hyper-v-on-windows/quick-start/enable-hyper-v).
...
1. Set the execution policy on the target device to `AllSigned` if it isn't already. You can check the current execution policy in an elevated PowerShell prompt by using:

   ```powershell
   Get-ExecutionPolicy -List
   ```

      If the execution policy of `local machine` isn't `AllSigned`, set the execution policy by using the following command:
...
1. Enter **O** or **R** to toggle **Optional diagnostic data** on or off, depending on your preference. A successful deployment is shown in the following image.
...
1. Provision your device by using the device connection string that you retrieved in the previous section. Replace the placeholder text with your own value.
...
1. On the upper bar, select **Set Modules**.
   ...
   Choose which modules you want to run on your device. You can choose from modules that you already created, modules from Microsoft Artifact Registry, or modules that you built yourself. In this quickstart, you deploy a module from the Microsoft Artifact Registry.
...
1. View the messages sent from the temperature sensor module to the cloud by using the following Linux command:
...
If you want to continue to the IoT Edge tutorials, skip this step. You can use the device that you registered and set up in this quickstart. Otherwise, delete the Azure resources that you created to avoid charges.

If you created your virtual machine and IoT hub in a new resource group, delete that group and all the associated resources. If you don't want to delete the whole group, delete individual resources instead.
...
In the next tutorial, you learn how to monitor the activity and health of your device from the Azure portal.
