...
Connecting to IoT Edge hub follows the same steps as described in the [connecting to IoT Hub with a generic MQTT client article](iot-hub-mqtt-support) or in the [conceptual description of the IoT Edge hub article](iot-edge-runtime.md). These steps are:
...
In order to receive twins, the clien needs to subscribe to an IoT Hub specific topic `$iothub/twin/res/#`. This topic name is inherited from IoT Hub, and all clients need to subscribe to the same topic. It does not mean that devices or modules receive the twin of each other. IoT Hub and IoT Edge hub knows which twin should be delivered where, even if all devices listen to the same topic name. 
