...
1. Create the root CA certificate and one intermediate certificate. These certificates are placed in *\<WRKDIR>*.
   ...
   The script creates several certificates and keys. Make note of one, which we'll refer to when adding your enrollment to DPS:
...
1. Create the IoT Edge device identity certificate and private key with the following command. Replace `<name>` with your preferred device ID.
   ...
   The script creates several certificates and keys, including two that we'll use when installing the IoT Edge runtime:
...
1. Create the root CA certificate and have it sign one intermediate certificate. The certificates are all placed in your working directory.
   ...
   This script command creates several certificates and keys. Make note of one, which we'll refer to when adding your enrollment to DPS:
...
> [!TIP]
> Group enrollments are also possible when using X.509 certificates and involve the same decisions as individual enrollments, although you also have the option of using either root CA certificates or intermediate certificates.
...
1. Select **Add individual enrollment** then complete the following steps to configure the enrollment:  
   ...
   1. Upload a **Primary Certificate .pem or .cer file** by selecting the root CA certificate you created earlier:
      `<WRKDIR>/certs/azure-iot-test-only.root.ca.cert.pem`
