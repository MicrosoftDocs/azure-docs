...
* The config file has a new name and location. Formerly `/etc/iotedge/config.yaml`, your device configuration information is now in `/etc/aziot/congig.toml`. The change from YAML to TOML formatting should make the configuration process easier since TOML doesn't have specific whitespace requirements.
