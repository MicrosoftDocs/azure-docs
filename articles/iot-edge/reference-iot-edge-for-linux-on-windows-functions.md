...
| gpuPassthroughType | DirectDeviceAssignment, Paravirtualization,  "" (CPU only) |  GPU Passthrough type |
| gpuCount | Integer value between 1 and the number of device's GPU cores | Number of GPU devices for the VM **Note**: If using Paravirtualization, please make sure to use `gpuCount = 1` |
