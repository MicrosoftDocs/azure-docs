...
- Maintenance takes longer as each node take roughly 20 minutes to update. Other maintenance operations are blocked while your cache is under maintenance.

## Set up clustering

Clustering is enabled  **New Azure Cache for Redis** during cache creation.
...
1. On the **New Redis Cache** page, configure the settings for your new premium cache.
   ...
   | ------------ |  ------- | -------------------------------------------------- |
   | **DNS name** | Enter a globally unique name. | The cache name must be a string between 1 and 63 characters. The string can only contain numbers, letters, or hyphens. The name must start and end with a number or letter, and can't contain consecutive hyphens. Your cache instance's _host name_ is _\<DNS name>.redis.cache.windows.net_. |
...
:::image type="content" source="media/cache-how-to-premium-clustering/redis-cache-redis-cluster-size.png" alt-text="Screenshot of Resource menu with Cluster size selected. ":::
