...
[Monitor metrics](monitor-cache.md#platform-metrics) such as server load. Watch for spikes in `Server Load` usage that correspond with timeouts. [Create alerts](monitor-cache.md#alerts) on metrics on server load to be notified early about potential impacts.
...
The "Cache Read" and "Cache Write" metrics can be used to see how much server-side bandwidth is being used. You can [view these metrics](monitor-cache.md#view-cache-metrics) in the portal. [Create alerts](monitor-cache.md#alerts) on metrics like cache read or cache write to be notified early about potential impacts.
