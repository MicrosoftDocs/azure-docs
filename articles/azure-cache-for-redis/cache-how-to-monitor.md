...
- Server Load
  - The percentage of CPU cycles in which the Redis server is busy processing and *not waiting idle* for messages. If this counter reaches 100, the Redis server has hit a performance ceiling, and the CPU can't process work any faster, and you may expect a large latency impact. If you're seeing a very high Redis Server Load such as 100 because you are sending many expensive commands to the server, then you may see timeout exceptions in the client. In this case, you should consider scaling up, scaling out to a Premium cluster, or partitioning your data into multiple caches. When Server Load is only moderately high, such as 50 to 80 percent, then aerage latency usually remains low, and timeout exceptions could have other causes than high server latency.
  - Known issue: because the Server Load metric is sensitive to other processes on the machine using up CPU cycles that reduces Redis's share of 'idle' time, on the C1 SKU background tasks like virus scanning cause Server Load to spike higher, for no obvious reason. Therefore its recommended to pay attention to other metrics such as operations, latency, and CPU, in addition to Server Load.
