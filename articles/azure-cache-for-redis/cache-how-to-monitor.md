...
| Geo Replication Connectivity Lag | Depicts the last successful IO time (in seconds) between geo-primary & geo-secondary. If the link goes down, this value continues to increase, indicating a problem. This metric is emitted from the geo-secondary cache instance.  _This metric is only available for Premium tier geo-replication enabled caches._ |
