...
| Geo Replication Connectivity Lag (preview) | Depicts the time (in seconds) since the last successful connection between geo-primary & geo-secondary. If the link goes down, this value continues to increase, indicating a problem. This metric is emitted from the geo-secondary cache instance.  This metric is only available for Premium tier geo-replication enabled caches. |
