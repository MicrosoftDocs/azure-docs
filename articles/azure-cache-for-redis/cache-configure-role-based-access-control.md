...
Managing access to your Azure Cache for Redis instance is critical to ensure that the right users have access to the right set of data and commands. In Redis version 1, the [Access Control List](https://redis.io/docs/management/security/acl/) (ACL) was introduced. ACL limits which user can can execute certain commands, and the keys that a user can be access. For example, you can prohibit specific users from deleting keys in the cache using [DEL](https://redis.io/commands/del/) command.
...
Azure Cache for Redis offers three built-in access policies: *Owner*, *Contributor*, and *Reader*. If the built-in access policies do not satisfy your data protection and isolation requirements, you can create and use your own custom data access policy as described in Configure custom data access policy.
...
- Azure AD authentication and authorization is supported for SSL connections only.
