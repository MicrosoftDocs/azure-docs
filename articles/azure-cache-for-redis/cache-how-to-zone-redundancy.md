...
The REST API spec for different types of zonal allocation can be found at: [ZonalAllocationPolicy](/rest/api/redis/redis/create#zonalallocationpolicy).
...
- This can also be done by passing `ZonalAllocationPolicy` as `Automatic`in the request body while updating the cache. For more information regarding the update process using REST API, see [ZonalAllocationPolicy](/rest/api/redis/redis/update#zonalallocationpolicy).
