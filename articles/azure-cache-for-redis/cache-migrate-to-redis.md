...
| Cache emulator |Managed Cache provides a local cache emulator. |Azure Cache for Redis does not have an emulator, but you can [run Redis locally](cache-development-faq.md#is-there-a-local-emulator-for-azure-cache-for-redis) to provide an emulator experience. |
