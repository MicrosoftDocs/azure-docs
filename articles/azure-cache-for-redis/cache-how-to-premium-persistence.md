...
All RDB persistence backups, except for the most recent one, are automatically deleted. This deletion might not happen immediately, but older backups aren't persisted indefinitely. Please note that if you have Soft Delete turned ON on your storage account, then that soft delete setting is respected, and the existing backups will continue to reside in the soft delete state.
...
After a rewrite, two sets of AOF files exist in storage. Rewrites occur in the background and append to the first set of files. Set operations, sent to the cache during the rewrite, append to the second set. A backup is temporarily stored during rewrites if there's a failure. The backup is promptly deleted after a rewrite finishes. Please note that if you have Soft Delete turned ON on your storage account, then that soft delete setting is respected, and the backups might continue to reside in the soft delete state.
