...
- In some cases, such as scaling migrating your cache to a different cluster, the underlying IP address of the cache may change. This is done in a way that is transparent to most applications (by updating the DNS records for the cache), but if you use an IP address to configure the connection to your cache, or to configure NSGs or firewalls allowing traffic to the cache, your application may have trouble connecting from around the time that the DNS record updates.
