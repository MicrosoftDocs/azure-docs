...
| EPC_VSANSTORAGEUTILIZATION | This error is encountered when vSAN utilization on the cluster is above 75%. This could lead to performance degradation and would make the cluster unmaintainable. | If vSAN utilization on the cluster is above 75%, you can either add nodes to increase available capacity or reduce the data utilization on the cluster. Follow this document [Tutorial - Scale clusters in a private cloud](/azure/azure-vmware/tutorial-scale-private-cloud) to scale up vSAN. Follow this document [Backup solutions for Azure VMware Solution virtual machines](/azure/azure-vmware/ecosystem-back-up-vms) to backup and remove VMs which are not essential. |
