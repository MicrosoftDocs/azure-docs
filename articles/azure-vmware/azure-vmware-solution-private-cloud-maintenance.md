...
- **SCSI shared controller:** When using SCSI bus sharing, use with bus type as "Physical" for VMs. VMs connected to SCSI bus controllers in "Virtual" mode may be powered-off during maintenance mode since these cannot be live migrated off hosts that need to be put into maintenance mode. For more information, see [Broadcom KB article](https://knowledge.broadcom.com/external/article?legacyId=2147661).
