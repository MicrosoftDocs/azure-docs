...
|29    |Virtual Machine’s serial port is connected to a device that can't be accessed on the destination host (EPC_SERIAL_PORT)                 |Customer intervention required|
|30    |NSX–T alarms for license expiration                                                                                                     |Customer intervention required|
|31    |NSX Alarm - Customer Internet Protocol Security (IPSec) tunnel is down                                                                  |Customer intervention required|
|32    |NSX Alarm - LDAP issue                                                                                                                  |Customer intervention required|
|33    |NSX Alarm - Edge VM CPU usage very high                                                                                                 |Customer intervention required|
