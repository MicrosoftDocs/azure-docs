...
- Management Server Virtual Appliance (MSA) is installed and configured prior to DR protection. 
...
- Azure VMware Solution private cloud must be deployed as a secondary region. [I think this is for the Recovery site]

- Customer has setup an Ubuntu Linux jump box with an ExpressRoute connection to their Azure VMware Solution private cloud. [I think this is for the Recovery site]

- Install the latest PowerShell onto the Linux jump box.

- Install the latest 3rd Party Module from PowerShell gallery.

### Protected site

The protected site hosts a “service cluster” for administrative services (such as vCenter, DNS, Active Directory, etc.) and “computer clusters” where protected line-of-business applications run. The protected site can be located on-premises, or be hosted in AVS.
...
It’s strongly recommended on the compute cluster for resource balancing. 
...
#### Ports
When JetStream DR software is installed, a range of ports are opened automatically on the source ESXi hosts. For most users, no additional action is necessary. In cases where the on-premises/source setup has special firewall rules blocking these ports, you’ll need to manually open these ports.
...
## References [or should this be the “next steps” section?]
