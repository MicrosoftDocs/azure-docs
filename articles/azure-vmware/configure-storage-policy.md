...
You'll run the `Set-VMStoragePolicy` cmdlet to Modify vSAN based storage policies on an default cluster or individual VM or on a group of VMs sharing a similar VM name. For example, if you have 3 VMs named "MyVM1", "MyVM2", "MyVM3", supplying "MyVM*" to the VMName parameter would change the StoragePolicy on all three VMs.
