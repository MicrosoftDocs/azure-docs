...
In this tutorial, you learn how to create an Azure VMware Solution Generation 2 (Gen 2) private cloud. The solution combines the first-party Azure VMware Solution with Azure hardware and network to simplify your networking architecture, and enhance performance and security. The initial launch of this generation utilizes the AV64 SKU.

With this generation, you can directly create the private cloud using the AV64 SKU. You're no longer required to deploy a minimum of three hosts of AV36, AV36P, or AV52 to create a cluster with AV64.
...
>[!IMPORTANT]
>When deploying an Azure VMware Solution Generation 2 Private cloud, select the AV64 SKU that is the correct host size. There's no SKU with a Gen2 suffix available to select.
...
1. On the **Azure VMware Solution** window, select **Create**. The following information is needed to deploy the private cloud from your planning phase:
   ...
   |**Availability zone _(Optional)_**| Select the Availability Zone where you want your private cloud deployed. If no Availability Zone is selected, the private cloud uses the default placement. The Availability Zone is used to place workloads closer to on-premises environments or other Azure services to meet latency and data-proximity requirements. It also supports disaster recovery and high-availability strategies by deploying across multiple zones.|
   |**Number of hosts**         | Number of hosts allocated for the private cloud cluster. The default value is 3, which you can increase after deployment or decrease if you have four or more nodes deployed. If these nodes aren't listed as available, contact support to request a quota increase. You can also select the link labeled If you need more hosts, request a quota increase in the Azure portal. |
   |**Virtual Network**         | Azure Virtual Network with a minimum network address space of a /22 or 4 /24s. The Virtual Network can be a new or existing one. The /22 or 4 /24s provided in your Virtual Network can be the same address space as the CIDR block you're planning to for your private cloud as long as the address space isn't in use.|
   ...
   |**DNS Forward Lookup**      | Input on your DNS Forward Lookup Zone, select either Private or Public. If no option is selected, the default is Public, which can be changed after your private cloud is created. |
