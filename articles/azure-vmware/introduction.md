...
1. Get AV36, AV36P, AV52 and AV64 [quota approval from Microsoft](https://learn.microsoft.com/azure/azure-vmware/request-host-quota-azure-vmware-solution) with a minimum of three nodes each. 
...
A user can go to the vSphere user interface to get the current state of vSAN FDs and hosts associated with each of them. This relation helps to identify hosts (based on the previous examples) that can be removed without impacting the vSAN FD balance and avoid any errors in the removal operation. 
