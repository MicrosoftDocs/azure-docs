...
>  VMware Cloud Director Instances can establish connections to AVS SDDC in regions where latency remains under 150 ms.
 ...
 - Plan and deploy Azure VMware solution private cloud using the following links:
    - [Plan Azure VMware solution private cloud SDDC.](https://learn.microsoft.com/azure/azure-vmware/plan-private-cloud-deployment)
    - [Deploy and configure Azure VMware Solution - Azure VMware Solution.](https://learn.microsoft.com/azure/azure-vmware/deploy-azure-vmware-solution?tabs=azure-portal) 
...
- Public IP on NSX-T edge can be used to provide outbound access for the VMware Reverse proxy VM as leveraged in this article. Learn more on, [How to configure a public IP in the Azure portal](./https://learn.microsoft.com/en-us/azure/azure-vmware/enable-public-ip-nsx-edge#configure-a-public-ip-in-the-azure-portal) and [Outbound Internet access for VMs](./https://learn.microsoft.com/azure/azure-vmware/enable-public-ip-nsx-edge#outbound-internet-access-for-vms)
...
![Figure show vcenter server is connected and enabled](./media/vmware-cds-avs/pic17.png)
18.	A newly created Provider VDC is visible in Cloud Resources. 
19.	In your Azure VMware solution private cloud, when login in vCenter you will see that a Resource Pool is created as a result of the above association.
...
- Configure tenant networking on VMware Cloud director service on Azure VMware solution using link [Enable VMware Cloud Director service with Azure VMware Solution](./https://learn.microsoft.com/en-us/azure/azure-vmware/enable-vmware-cds-with-azure).
