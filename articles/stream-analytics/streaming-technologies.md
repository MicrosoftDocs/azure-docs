...
Several services are available for real-time analytics and streaming processing on Azure. This article helps you decide which technology is the best fit for your application.

## When to use Azure Stream Analytics

Azure Stream Analytics is the recommended service for stream analytics on Azure. Use it for a wide range of scenarios that include but aren't limited to:
...
* Support for slow changing [reference data](stream-analytics-use-reference-data.md) (also known as lookup tables), including joining with geospatial reference data for geofencing.
...
Azure Stream Analytics supports user-defined functions (UDFs) and user-defined aggregates (UDAs) in JavaScript for cloud jobs and C# for IoT Edge jobs. C# user-defined deserializers are also supported. If you want to implement a deserializer, a UDF, or a UDA in other languages, such as Java or Python, use Spark Structured Streaming. You can also run the Event Hubs **EventProcessorHost** on your own virtual machines to do arbitrary streaming processing.
