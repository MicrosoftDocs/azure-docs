...
This section demonstrates how to configure a Stream Analytics job to run a function that writes data to Azure Redis Cache. The Stream Analytics job reads events from Event Hubs, and runs a query that invokes the function. This function reads data from the Stream Analytics job, and writes it to Azure Redis Cache.
...
4. Go back to the Azure portal. From the **Platform features** tab, browse to your Azure function. Under **Development Tools**, select **App Service Editor**. 
