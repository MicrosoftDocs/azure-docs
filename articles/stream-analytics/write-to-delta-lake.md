...
Regular table maintenance via [OPTIMIZE and VACUUM commands](https://docs.delta.io/delta-utility/) should be performed periodically with the property ["delta.deletedFileRetentionDuration"](https://docs.delta.io/delta-utility/#table-property-overrides) set to a reasonable value that balances customer need for time-travel/rollback operations with the streaming job's ability to process data. [`delta.deletedFileRetentionDuration`](https://docs.delta.io/delta-utility/#table-property-overrides) is used to determine which [`Remove File action`](https://github.com/delta-io/delta/blob/master/PROTOCOL.md#add-file-and-remove-file)s are included in the checkpoint file. Any `Remove File action` that is older than the `deletedFileRetentionDuration` will not be included the in the checkpoint which will truncate the checkpoint but prevent any time-travel or rollback functionality. [The default value of `deletedFileRetentionDuration` is 1 week](https://docs.delta.io/table-properties/), although some engines may have a different default value.
