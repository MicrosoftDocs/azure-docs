...
description: This article describes how to export an Azure Resource Manager template for you Azure Stream Analytics job.
...
You can redeploy an Azure Stream Analytics job with the same configuration as an existing Stream Analytics job by exporting the Azure Resource Manager template.
...
The next step is to compile the job script to an Azure Resource Manager template. Before compile the script, ensure that your job has both an input and an output configured. If no input or output is configured, you need to configure the input and output first.

1. In Visual Studio Code, navigate to your job's *Transformation.asaql* file.

1. From the Visual Studio Code command palette (**CTRL+SHIFT+P**), select **ASA: Compile Script**. 
...
1. Open the *JobTemplate.parameters.json* file located in the **Deploy** folder of you Stream Analytics job workspace in Visual Studio Code.
...
You are ready to deploy your Azure Stream Analytics job using the Azure Resource Manager templates you generated in the previous section.
...
If you do not have Azure PowerShell configured, follow the steps in [Install Azure PowerShell module](https://docs.microsoft.com/powershell/azure/install-Az-ps).
