...
This quickstart shows you how to create and run an Azure Stream Analytics (ASA) job using the ASA Tools extension for Visual Studio Code. You learn to define an ASA job that reads real-time streaming data from IoT Hub, filters events with a temperature greater than 27° and writes the output results to a file in blob storage. The input data used in this quickstart is generated by a Raspberry Pi online simulator.
...
4. Once you are signed in, you can see the subscriptions under your Azure account.
...
4. Select **Next: Management** at the bottom of the page. 
5. On the **Management** page, for **Pricing and scale tier**, select **F1: Free tier**, if it's still available on your subscription. For more information, see [Azure IoT Hub pricing](https://azure.microsoft.com/pricing/details/iot-hub/).
...
7. After the resource (IoT hub) is created, select **Go to resource** to navigate to the **IoT Hub** page. 
8. On the **IoT Hub** page, select **Devices** on the left menu, and then select **\+ Add Device**.
...
9. Enter a **Device ID** and click **Save**.
...
4. An **IoTHub1.json** file is created. with the following values. Keep default values for fields not mentioned here.
