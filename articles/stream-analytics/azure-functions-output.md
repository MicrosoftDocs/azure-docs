...
Azure Functions output from Stream Analytics is not available in the Microsoft Azure operated by 21Vianet and Azure Germany (T-Systems International) regions. Connection to Azure Functions inside a virtual network (VNet) from an Stream Analytics job that is running in a multi-tenant cluster is also not supported.
