...
Once the timeout is above 15 minutes, the max batch size hint to `BulkCopy` is reduced to half until 100 events per batch are left.
    ...
    > [!IMPORTANT]
    > For non-[network injected ASA jobs](./stream-analytics/run-job-in-virtual-network), please do not rely on source IP address of connections coming from ASA in any way. They can be public or private IPs depending on service infrastructure operations that happen from time to time.
