...
1. First, you can check which partition has the watermark delay increasing by selecting watermark delay metric and splitting it by “Partition ID” dimension. For example, we identify that the partition#465 has high watermark delay. 
...
For this example, we can start by splitting the watermark delay metric by **Partition ID** dimension.
...
4. If the utilization of CPU and SU is very high (>80%) in all streaming nodes, we can conclude that this job has a large amount of data being processed within each streaming node. Let’s further check how many partitions are allocated to one streaming node by checking the “Input Events” metrics with filter by a "Streaming node” dimension and splitting by "Partition ID”. See the screenshot below:
