...
| Container   | A logical grouping for blobs stored in the Azure Blob service. When you upload a blob to the Blob service, you must specify a container for that blob. <br /><br /> Dynamic container name is optional. It supports one and only one dynamic {field} in the container name. The field must exist in the output data, and follow the [container name policy](../docs-ref-conceptual/storageservices/Naming-and-Referencing-Containers--Blobs--and-Metadata.md).<br /><br />The field data type must be string and it is recommended to stringfy the field in the query. To use multiple dynamic fields, or combine static text along with dynamic field, you can define it in the query with built-in string functions, like CONCAT, LTRIM, etc. |
