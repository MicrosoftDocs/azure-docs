...
You can optimize the cost of your Stream Analytics jobs by increasing or decreasing the number of Streaming Units (SUs) to match the change in your input load by setting up autoscaling. You can configure your jobs to autoscale in two ways.
1. **Pre-defining a schedule**. You can do this when you have a predictable input load. For example, you expect a higher rate of input events during the daytime between 9:00 AM to 6:00 PM and want your job to run with more SUs.
2. **Triggering scale up/down operations based on job metrics**. This is helpful when you don't have a predictable input load and want to dynamically change the number of SUs based on your job metrics such as number of input events.

## Prerequisites
Before you start configure autoscaling for your job, you should complete the following steps.
1. Your job is optimized to have a [parallel topology](https://docs.microsoft.com/azure/stream-analytics/stream-analytics-parallelization). If you are able to change the scale of your job while it is running, then your job has a parallel topology and can be configured to autoscale.
...
![Setup variables in Azure Automation](./media/autoscale/variables.png)
...
3. Now open your **ScaleUpRunbook** and then select **Schedules** under **Resources**. You can then link your runbook to a schedule you created in the previous steps. You can have multiple schedules linked with the same runbook. This is helpful when you want to run the same scale operation at different times of the day.
...
There might be cases where you cannot predict input load. In such cases it more optimal to scale up/down in steps within a minimum and maximum bound. You can configure alert rules in your Stream Analytics jobs such that your runbooks are trigged when job metrics go above or below a threshold.
1. In your Azure Automation account, create 2 more Integer variables called **minSU** and **maxSU**. This sets the bounds within which your job will scale in steps.
2. You then create 2 new runbooks. You can reuse the [autoscaleup PowerShell script](https://github.com/Azure/azure-stream-analytics/blob/master/Autoscale/Autoscaleup.ps1) which will increase SUs of your job in increments till **maxSU**. You can also reuse the [autoscaledown PowerShell script](https://github.com/Azure/azure-stream-analytics/blob/master/Autoscale/autoscaledown.ps1) that will decrease the SUs of your job in steps till **minSU**. Alternatively, you can also use the runbooks from the previous section if you have specific SU values you want to scale to.
