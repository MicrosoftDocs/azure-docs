...
Azure Stream Analytics supports processing events in protocol buffer data formats. You can use the built-in protobuf deserializer when configuring your inputs. To use the built-in deserializer, you must specify the protobuf definition file, message type, and prefix style.
...
1. Protobuf Deserializer takes only one (1) proto file at a time. Imports to custom-made proto files are not supported.
    For example:
    :::image type="content" source="./media/protobuf/one-proto-example.png" alt-text=" Screenshot showing how an example of a custom-made proto file." lightbox="./media/protobuf/one-proto-example.png" :::

    This protobuf definition file refers to another proto file in its imports. Because the protobuf deserializer would have only the current proto file and not know what carseat.proto is, it would be unable to deserialize correctly.
...
4. When a proto file contains a namespace or pack. Then, the message type must include it.
    For example:
    :::image type="content" source="./media/protobuf/proto-namespace-example.png" alt-text=" Screenshot showing example of a proto file with a namespace." lightbox="./media/protobuf/proto-namepace-example.png" :::
