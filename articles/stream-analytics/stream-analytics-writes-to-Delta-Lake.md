---
title: Azure Stream Analytics writes to Delta Lake table 
...
# Azure Stream Analytics writes to Delta Lake table (Public Preview)
...
To see the full list of ADLS Gen2 configuration, please see [ALDS Gen2 Overview](https://learn.microsoft.com/azure/stream-analytics/blob-storage-azure-data-lake-gen2-output).
...
You can choose to use one or more path segments to define the path to the delta table and the delta table name. A path segment is the string between consecutive delimiter characters (e.g., the forward slash '/') that corresponds to the name of a virtual directory.

The segment name is alphanumeric and can include spaces, hypens, and underscores. The last path segment will be used as the table name.

Restrictions on Delta Path name include the following:

- Field names are not case-sensitive. For example, the service can't differentiate between column "ID" and "id".
- No dynamic {field} name is allowed. For example, {ID} will be treated as text {ID}.
- The number of path segments comprising the name cannot exceed 254.
...
To create a new Delta Lake table, you need to specify a Delta Path Name that does not lead to any existing tables. If there is already a Delta Lake table existing with the same name and in the location specified by the Delta Path name, by default, Azure Stream Analytics writes new records to the existing table.
...
1. Dynamic partition key is not supported.
1. Writing to Delta lake is append only.
1. Schema checking in query testing is not available.
1. Checkpoints for delta lake is not taken by Stream Analytics.
