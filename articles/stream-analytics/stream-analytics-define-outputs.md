...
| Azure SQL Database | Yes | Based on the PARTITION BY clause in the query | Follows the input partitioning for [fully parallelizable queries](stream-analytics-scale-jobs.md). You can [learn more](stream-analytics-sql-output-perf.md) about achieving better write throughput performance when you're loading data into SQL Azure Database. | 
| Azure Blob storage | Yes | Use {date} and {time} tokens from your event fields in the Path pattern. Choose the Date format, such as YYYY/MM/DD, DD/MM/YYYY, MM-DD-YYYY. HH is used for the Time format. As part of the [preview](https://aka.ms/ASApreview1), blob output can be partitioned by a single custom event attribute {fieldname} or {datetime:\<specifier>}. | Follows the input partitioning for [fully parallelizable queries](stream-analytics-scale-jobs.md). | 
...
If your output adapter is not partitioned, lack of data in one input partition will cause a delay upto [late arrival amount of time](stream-analytics-out-of-order-and-late-events.md). In such cases, the output is merged to a single writer which might cause bottlenecks in your pipeline.
