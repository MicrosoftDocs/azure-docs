...
| Username | The Username, which has access to write to the databas. |
| Password | The password to connect to the databas.e |
...
| Event Hub namespace |A Event Hub namespace is a container for a set of messaging entities. When you created a new Event Hub, you also created a Event Hub namespace. |
...
| Azure Blob storage | Yes | Use {date} and {time} tokens or a single {fieldname} from your event fields in the Path pattern. Choose the Date format, such as YYYY/MM/DD, DD/MM/YYYY, MM-DD-YYYY. HH is used for the Time format. | Follows the input partitioning for [fully parallelizable queries](stream-analytics-scale-jobs.md). | 
