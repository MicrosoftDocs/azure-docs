...
1. Enter a name for the Stream Analytics job, then select **Create**.  
    ...
    :::image type="content" source="./media/no-code-filter-ingest-adx/create-new-stream-analytics-job.png" alt-text="Screenshot showing where to enter a job name." lightbox="./media/no-code-filter-ingest-adx/create-new-stream-analytics-job.png" :::
...
1. Specify the **Serialization type** of your data in the Event Hubs window and the **Authentication method** that the job will use to connect to the Event Hubs. Then select **Connect**.  
    :::image type="content" source="./media/no-code-filter-ingest-adx/event-hub-configuration.png" alt-text="Screenshot showing the Event Hubs connection configuration." lightbox="./media/no-code-filter-ingest-adx/event-hub-configuration.png" :::
...
1. When the connection is established successfully and you have data streams flowing into your Event Hubs instance, you'll immediately see two things:
    - Fields that are present in the input data. You can choose **Add field** or select the three dot symbol next to a field to remove, rename, or change its type.  
        :::image type="content" source="./media/no-code-filter-ingest-adx/no-code-schema.png" alt-text="Screenshot showing the Event Hubs field list where you can remove, rename, or change the field type." lightbox="./media/no-code-filter-ingest-adx/no-code-schema.png" :::
    - A live sample of incoming data in the **Data preview** table under the diagram view. It automatically refreshes periodically. You can select **Pause streaming preview** to see a static view of the sample input data.  
        :::image type="content" source="./media/no-code-filter-ingest-adx/no-code-sample-input.png" alt-text="Screenshot showing sample data under Data Preview." lightbox="./media/no-code-filter-ingest-adx/no-code-sample-input.png" :::
...
1. Select the **Filter** tile to aggregate the data. In the Filter area, select a field to filter the incoming data with a condition.
    ...
    :::image type="content" source="./media/no-code-filter-ingest-adx/filter-operator-configuration.png" alt-text="Screenshot that shows the filter operator configuration." lightbox="./media/no-code-filter-ingest-adx/filter-operator-configuration.png" :::
...
1. Select the **Manage** tile. In the **Manage fields** configuration panel, choose the fields you want to output to event hub. If you want to add all the fields, select **Add all fields**.
    ...
    :::image type="content" source="./media/no-code-filter-ingest-adx/manage-fields-configuration.png" alt-text="Screenshot that shows the manage field operator configuration." lightbox="./media/no-code-filter-ingest-adx/manage-fields-configuration.png" :::
...
1. Select **Azure Data Explorer** tile. In the configuration panel, fill in needed parameters and connect. 
    ...
    :::image type="content" source="./media/no-code-filter-ingest-adx/adx-output-configuration.png" alt-text="Screenshot that shows the Kusto output configuration." lightbox="./media/no-code-filter-ingest-adx/adx-output-configuration.png" :::

1. Optionally, select **Get static preview/Refresh static preview** to see the data preview that will be ingested in event hub.  
    :::image type="content" source="./media/no-code-filter-ingest-adx/no-code-output-static-preview.png" alt-text="Screenshot showing the Get static preview/Refresh static preview option." lightbox="./media/no-code-filter-ingest-adx/no-code-output-static-preview.png" :::

1. Select **Save** and then select **Start** the Stream Analytics job.  
    :::image type="content" source="./media/no-code-filter-ingest-adx/no-code-save-start.png" alt-text="Screenshot showing the Save and Start options." lightbox="./media/no-code-filter-ingest-adx/no-code-save-start.png" :::
...
1. To start the job, specify:  
    ...
    - **Output data error handling** – It allows you to specify the behavior you want when a job’s output to your destination fails due to data errors. By default, your job retries until the write operation succeeds. You can also choose to drop such output events.  
        :::image type="content" source="./media/no-code-filter-ingest-adx/no-code-start-job.png" alt-text="Screenshot showing the Start Stream Analytics job options where you can change the output time, set the number of streaming units, and select the Output data error handling options." lightbox="./media/no-code-filter-ingest-adx/no-code-start-job.png" :::

1. After you select **Start**, the job starts running within two minutes, and the metrics will be open in tab section below.   

    :::image type="content" source="./media/no-code-filter-ingest-adx/job-metrics-after-started.png" alt-text="Screenshot that shows the job metrics data after it's started." lightbox="./media/no-code-filter-ingest-adx/job-metrics-after-started.png" :::
    ...
    :::image type="content" source="./media/no-code-filter-ingest-adx/no-code-list-jobs.png" alt-text="Screenshot of the Stream Analytics jobs tab where you view the running jobs status." lightbox="./media/no-code-filter-ingest-adx/no-code-list-jobs.png" :::
