...
3. Create an [Azure Synapse Analytics workspace](../synapse-analytics/get-started-create-workspace.md) with a [Dedicated SQL pool](../synapse-analytics/get-started-analyze-sql-pool.md#create-a-dedicated-sql-pool).
    > [!NOTE]
    > If you'd like to build the real-time Power BI dashboard directly without storing the data into database, you can skip step#3 and 4, then go to this guide to [build the dashboard with the Power BI dataset produced by Stream Analytics job](./no-code-build-power-bi-dashboard.md). 
