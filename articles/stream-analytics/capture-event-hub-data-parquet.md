...
ms.date: 05/08/2022
---
# Capture data from Azure Data Lake Storage Gen2 in Parquet format

This article explains how to use the node code editor to automatically capture streaming data in Event Hubs in an Azure Data Lake Storage Gen2 account in Parquet format. You have the flexibility of specifying a time or size interval.
...
1. Select **Features** > **Process Data**, and select **Start** on the **Capture data to Azure Data Lake Storage Gen2 in Parquet format** card.  
...
1. Select the **Streaming blob** tile to edit the configuration. 
    ...
    1. Select **Connect**
1. When the connection is established, you see:
    1. Fields that are present in the output data. You can choose **Add field** or you can select the three dot symbol next to a field to optionally remove, rename, or change its name.  
...
:::image type="content" source="./media/capture-event-hub-data-parquet/stream-analytics-jobs-list.png" alt-text="Screenshot showing the Stream Analytics jobs list where you monitor job status." lightbox="./media/capture-event-hub-data-parquet/stream-analytics-jobs-list.png" :::
