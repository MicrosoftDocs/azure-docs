...
This tutorial is designed to help you quickly set up a simple Azure Stream Analytics job, with Azure Machine Learning integration. We will use a sentiment analytics Machine Learning model from the Cortana Intelligence Gallery to analyze streaming text data, and determine the sentiment score in real time. This is a good tutorial to help you understand scenarios such as real-time sentiment analytics on streaming Twitter data, analysis of records of customer chats with support staff, and comments on forums, blogs, and videos, in addition to many other real-time, predictive scoring scenarios.

This tutorial offers a sample CSV file with text (as shown in Figure 1) as input in Azure Blob storage. The job applies the sentiment analytics model as a user-defined function (UDF) on the sample text data from the Blob storage. The end result is placed in the same Blob storage in another CSV file. Figure 2 is a diagram of this configuration. For a more realistic scenario, you can replace Blob storage with streaming Twitter data from an Azure Event Hub input. Additionally, you could build a [Power BI](https://powerbi.microsoft.com/) real-time visualization of the aggregate sentiment.
...
-	A CSV file with some data in it. You can download the file shown in Figure 1 from [GitHub](https://github.com/jeffstokes72/azure-stream-analytics-repository/blob/master/sampleinputs.csv), or you can create your own. This tutorial is written with the assumption that you use the one provided for download.

At a high level, in this tutorial you'll do the following steps:
...
1.	In Visual Studio, expand **Azure**, right-click **Storage**, and then choose **Attach External Storage**. Enter an **Account Name** and an **Account Key**.  
...
3.	To upload the CSV file, click the **Upload Blob** icon, and then choose **file from the local disk**.  
...
3.	Sign in to go to the workspace. Choose the location that best suits your location.
...
Click Apps link for **Excel 2010 or earlier workbook** to get your API key and the URL that you’ll need later to set up the Stream Analytics job. (This step is required only to use a Machine Learning model from another Azure account workspace. This tutorial assumes this is the case, to address that scenario.)  
...
2.	Click **New** > **Data Services** > **Stream Analytics** > **Quick Create**. Fill in the **Job Name**, the appropriate **Region** for the job, and choose a **Regional Monitoring Storage Account**.    
...
4.	On the first page of the **Add Input** wizard, choose **Data stream**, and then click **next**. On the second page, choose **Blob Storage** as the input, and then click **next**.  
...
7.	Choose **Blob Storage**, and then enter the same parameters, except for the container. The value for **Input** was configured to read from the container named “test” where the **CSV** file was uploaded. For **Output**, enter “testoutput”. Container names must be unique. Verify that this container exists.     
8.	Click **Next** to configure the output’s **Serialization settings**. As with **Input**, choose **CSV**, and then click the **OK** button.
...
10.	On the **Machine Learning Web Service Settings** page, locate the Machine Learning workspace, web service, and default endpoint. For this tutorial, apply the settings manually to gain familiarity with configuring a web service for any workspace, as long as you know the URL and you have the API key. Enter the endpoint **URL** and **API key**. Click **OK**.    
...
11.	On the **Query** tab, modify the query as follows, and then click **Save** to save the query.    
...
5.	Double-click the file to open the default CSV editor. Something similar to the following should be displayed:  
