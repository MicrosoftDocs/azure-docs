...
Before defining the Stream Analytics job, you should prepare the data, which is configured as input to the job. To prepare the input data required by the job, complete the following steps:
...
3. In the **IoT Hub** pane, enter the following information:
   ...
   |IoT Hub Name  | MyASAIoTHub  |   Select a name for yout IoT Hub.   |
...
8. Once the device is created, open the device from the **IoT devices** list. Copy the **Connection string -- primary key** and save it to a notepad to use later.
   ...
   ![Copy IoT Hub device connection string](./media/stream-analytics-quick-create-portal/add-device-iot-hub.png)
...
3. Fill out the **IoT Hub** page with the following values:
   ...
   |IoT Hub  |  MyIoTHub |  Enter the name of the IoT Hub you created in the previous section. |
