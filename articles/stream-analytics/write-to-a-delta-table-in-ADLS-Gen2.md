---
title: Write to a Delta Table in ADLS Gen2
...
# Write to a Delta Table stored in Azure Data Lake Storage Gen2 (Public Preview)
...
3. Fill the output form with the following details and select **Save**:
    ...
    6. For **Event Serialization Format**, select **Delta Lake**. Please note that although Delta lake is listed as one of the options here, it is not a data format. Delta Lake uses versioned Parquet files to store your data. To learn more about [Delta lake](/stream-analytics/stream-analytics-writes-to-Delta-Lake).
...
2. Enter the following query into the query window. In this example, the query reads the data from Event Hub and copies selected values to a Delta table in ADLS Gen2.
...
> [ASA writes to Delta Lake output](https://aka.ms/azure-stream-analytics-writes-to-Delta-Lake)
