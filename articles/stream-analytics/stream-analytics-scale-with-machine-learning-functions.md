...
## What is an Azure Machine Learning Studio (classic) function in Stream Analytics?

A Machine Learning Studio (classic) function in Stream Analytics can be used like a regular function call in the Stream Analytics query language. Behind the scenes, however, these function calls are actually Azure Machine Learning Studio (classic) Web Service requests.

You can improve the throughput of Machine Learning Studio (classic) web service requests by "batching" multiple rows together in the same web service API call. This grouping is called a mini-batch. For more information, see [Azure Machine Learning Studio (classic) Web Services](../machine-learning/classic/consume-web-services.md). Support for Azure Machine Learning Studio (classic) in Stream Analytics is in preview.

## Configure a Stream Analytics job with Machine Learning Studio (classic) functions

There are two parameters to configure the Machine Learning Studio (classic) function used by your Stream Analytics job:

* Batch size of the Machine Learning Studio (classic) function calls.
...
Increasing the batch size will increase the latency of your Azure Machine Learning Studio (classic) requests and the latency of the Stream Analytics job.

Increasing the batch size allows the Stream Analytics job to process **more events** with the **same number** of Machine Learning Studio (classic) web service requests. The increase of Machine Learning Studio (classic) web service latency is usually sublinear to the increase of batch size. 

It's important to consider the most cost-efficient batch size for a Machine Learning Studio (classic) web service in any given situation. The default batch size for web service requests is 1000. You can change this default size using the [Stream Analytics REST API](https://docs.microsoft.com/previous-versions/azure/mt653706(v=azure.100) "Stream Analytics REST API") or the [PowerShell client for Stream Analytics](stream-analytics-monitor-and-manage-jobs-use-powershell.md).
...
Every 6 SUs get 20 concurrent connections to the Machine Learning Studio (classic) web service. However, 1 SU job and 3 SU jobs get 20 concurrent connections.  

If your application generates 200,000 events per second, and the batch size is 1000, then the resulting web service latency is 200 ms. This rate means that every connection can make five requests to the Machine Learning Studio (classic) web service each second. With 20 connections, the Stream Analytics job can process 20,000 events in 200 ms and 100,000 events in a second.

To process 200,000 events per second, the Stream Analytics job needs 40 concurrent connections, which come out to 12 SUs. The following diagram illustrates the requests from the Stream Analytics job to the Machine Learning Studio (classic) web service endpoint – Every 6 SUs has 20 concurrent connections to Machine Learning Studio (classic) web service at max.

![Scale Stream Analytics with Machine Learning Studio (classic) Functions two job example](./media/stream-analytics-scale-with-ml-functions/stream-analytics-scale-with-ml-functions-00.png "Scale Stream Analytics with Machine Learning Studio (classic) Functions two job example")
...
![Scale Stream Analytics with Machine Learning Studio (classic) Functions Formula](./media/stream-analytics-scale-with-ml-functions/stream-analytics-scale-with-ml-functions-02.png "Scale Stream Analytics with Machine Learning Studio (classic) Functions Formula")

You can also configure the 'max concurrent calls' on the Machine Learning Studio (classic) web service. It's recommended to set this parameter to the maximum value (200 currently).
...
The following example includes a Stream Analytics job with the sentiment analysis Machine Learning Studio (classic) function, as described in the [Stream Analytics Machine Learning Studio (classic) integration tutorial](stream-analytics-machine-learning-integration-tutorial.md).
...
Using 1 SU, could this Stream Analytics job handle the traffic? The job can keep up with the input using the default batch size of 1000. The default latency of the sentiment analysis Machine Learning Studio (classic) web service (with a default batch size of 1000) creates no more than a second of latency.

The Stream Analytics job's **overall** or end-to-end latency would typically be a few seconds. Take a more detailed look into this Stream Analytics job, *especially* the Machine Learning Studio (classic) function calls. With a batch size of 1000, a throughput of 10,000 events takes about 10 requests to the web service. Even with one SU, there are enough concurrent connections to accommodate this input traffic.
...
With the second option, you will have to provision more SUs to have more concurrent Machine Learning Studio (classic) web service requests. This greater number of SUs, increases the job **cost**.
...
By now, you should already have a good understanding of how Machine Learning Studio (classic) functions in Stream Analytics work. You likely also understand that Stream Analytics jobs "pull" data from data sources and each "pull" returns a batch of events for the Stream Analytics job to process. How does this pull model impact the Machine Learning Studio (classic) web service requests?

Normally, the batch size we set for Machine Learning Studio (classic) functions won't exactly be divisible by the number of events returned by each Stream Analytics job "pull". When this occurs, the Machine Learning Studio (classic) web service is called with "partial" batches. Using partial batches avoids incurring additional job latency overhead in coalescing events from pull to pull.
...
![Scale Stream Analytics with Machine Learning Studio (classic) Functions Metrics](./media/stream-analytics-scale-with-ml-functions/stream-analytics-scale-with-ml-functions-01.png "Scale Stream Analytics with Machine Learning Studio (classic) Functions Metrics")
...
To scale a Stream Analytics job with Machine Learning Studio (classic) functions, consider the following factors:

1. The input event rate.
2. The tolerated latency for the running Stream Analytics job (and thus the batch size of the Machine Learning Studio (classic) web service requests).
3. The provisioned Stream Analytics SUs and the number of Machine Learning Studio (classic) web service requests (the additional function-related costs).
