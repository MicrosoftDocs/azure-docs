...
Note that a job with complex query logic could have high SU% utilization even when it is not continuously receiving input events. This usually happens after a sudden spike in input and output events as the job might continue to maintain state in memory based on the complexity of the query.
