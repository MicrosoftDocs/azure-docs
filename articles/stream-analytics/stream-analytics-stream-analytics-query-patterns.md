...
The query design can express simple pass-through logic to move event data from one input stream into another output data store. Or it can do rich temporal analysis to calculate aggregates over various time windows as as the TollApp sample, with pattern matching. You can join data from multiple inputs to combine streaming events, and do lookups against static reference data to enrich the event values. Also you can write data to multiple outputs.
