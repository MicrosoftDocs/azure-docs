...
The storage size for documents that belong to the same partition key value is limited to 20 GB (50 GB for new accounts). An [ideal partition key](../cosmos-db/partitioning-overview.md#choose-partitionkey) is one that appears frequently as a filter in your queries and has sufficient cardinality to ensure that your solution is scalable.
