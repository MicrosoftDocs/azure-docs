---
title: Azure Virtual Machines security overview
titleSuffix: Azure security
description: This article provides an overview of the core Azure security features that you can use with Azure Virtual Machines.
services: security
author: msmbaldwin
...
Azure Virtual Machines helps you deploy a wide range of computing solutions in an agile way. The service supports Microsoft Windows, Linux, Microsoft SQL Server, Oracle, IBM, SAP, and Azure BizTalk Services. You can deploy any workload and any language on nearly any operating system.
...
Azure helps you build security-enhanced, compliant solutions that:
...
- **Server-side encryption (SSE)**: Also called encryption at rest or Azure Storage encryption, SSE is always enabled and automatically encrypts data on Azure managed disks (OS and data disks) when persisting to storage clusters. SSE transparently encrypts data by using 256-bit AES encryption, which is FIPS 140-2 compliant. SSE doesn't affect disk performance and has no extra cost. However, SSE doesn't encrypt temp disks or disk caches.
...
By default, managed disks use platform-managed keys with no extra configuration required. For customer-managed keys, you can use Azure Key Vault, Azure Key Vault Managed HSM, or Azure Cloud HSM to control and manage your own encryption keys.
...
- **Independent and isolated backups**: The Recovery Services vault stores backups and provides built-in management of recovery points. It also guards against accidental destruction of data.
...
- **Soft delete protection**: Retains deleted backup data for 14 more days, so you can recover with no data loss.
...
An Azure virtual network is a logical construct built on top of the physical Azure network fabric. Each logical Azure virtual network is isolated from all other Azure virtual networks. This isolation helps ensure that network traffic in your deployments isn't accessible to other Microsoft Azure customers.
...
Microsoft compliance offerings and audit reports identify which cloud services are in scope for each formal third-party audit. Scope can vary by offering, so use the current Microsoft compliance offerings and audit-scope documentation instead of a static certification list for Azure Virtual Machines.

For more information, see [Microsoft compliance offerings](/compliance/regulatory/offering-home), [Azure and other Microsoft cloud services in compliance audit scope](/azure/compliance/offerings/cloud-services-in-audit-scope), [Microsoft Trust Center: Compliance](https://www.microsoft.com/trust-center/compliance/compliance-overview), and [Azure compliance documentation](../../compliance/index.yml).
