---
title: Incident response overview for Azure
description: Understand how to respond to security incidents in Azure environments by using Microsoft security services and best practices.
services: security
author: msmbaldwin
ms.service: security
...
- Cloud-native investigation by using Azure Monitor logs, Microsoft Entra audit logs, and NSG flow logs
...
**Test response procedures regularly.** Organizations discover gaps during actual incidents rather than controlled testing. Conduct quarterly tabletop exercises by using attack scenarios such as ransomware, data exfiltration, and insider threats.
...
- **NSG flow logs**: Network traffic with source and destination mapping for lateral movement detection.
...
- **Immutable storage**: Export logs and evidence to Azure Storage by using [immutable storage policies](../../storage/blobs/immutable-storage-overview.md) and legal hold.
- **Network packet capture**: Use Azure Network Watcher for network forensics.
- **Memory dumps**: Collect volatile evidence by using Azure extensions.

Maintain chain of custody by using cryptographic hashing and access logging for legal proceedings.
...
- **User account response**: Disable accounts, reset passwords, terminate sessions, and revoke privileges.
