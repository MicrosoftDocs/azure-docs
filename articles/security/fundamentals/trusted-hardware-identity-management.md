...
THIM service by default gives an older tcbinfo as not all customers migrate to latest Intel SDK. Updating it to the latest from Intel will cause attestation failures to those customers and potential outage.

Open Enclave SDK does not look at nextUpdate date and will pass attestation, so will MAA. There is a feature coming soon where customers will be able to select their own tcbinfo (including the latest), which will empower the customers to upgrade the software at their own cadence and do attestation against the newer tcbinfo.
...
I could get my certificates directly from Intel PCK with Coffeelake. With Icelake, why do I need to get the certificates from THIM and what do I need to do to fetch those certificates?

The certificates are fetched and cached in THIM service using platform manifest and indirect registration. The impact of this is, Key Caching Policy will be set to never store platform root keys for a given platform. Direct calls to the Intel service from inside the VM are expected to fail.

To retrieve the certificate, you would need to install Azure DCAP library which replaces Intel QPL. This library directs the fetch requests to THIM service running in Azure cloud. For the downloading the latest DCAP packages, please see: Where can I download the latest DCAP packages?

How do I request collateral in a Confidential Virtual Machine (CVM)?

Use the following sample in a CVM guest for requesting AMD collateral that includes the VCEK certificate and certificate chain. For details on this collateral and where it originates from, please see Versioned Chip Endorsement Key (VCEK) Certificate and KDS Interface Specification (amd.com)
