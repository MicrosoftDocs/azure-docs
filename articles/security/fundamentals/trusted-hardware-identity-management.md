...
- [Windows](https://github.com/microsoft/Azure-DCAP-Client/releases)

For newer versions of Ubuntu (for example, Ubuntu 24.04), you must use the [Intel QPL](#how-do-i-use-intel-qpl-with-trusted-hardware-identity-management).

### Why do Trusted Hardware Identity Management and Intel have different baselines?

Trusted Hardware Identity Management and Intel provide different baseline levels of the trusted computing base. When you assume that Intel has the latest baselines, ensure that all the requirements are satisfied. This approach can lead to a breakage if you didn't update to the specified requirements.

Trusted Hardware Identity Management takes a slower approach to updating the TCB baseline, so you can make the necessary changes at your own pace. Although this approach provides an older TCB baseline, you don't experience a breakage if you didn't meet the requirements of the new TCB baseline. For this reason, the TCB baseline from Trusted Hardware Identity Management is a different version from Intel's baseline. This approach helps you meet the requirements of the new TCB baseline at your pace instead of forcing an update and causing a disruption that would require reprioritization of workstreams.
...
Starting with the 4th Generation of Intel® Xeon® Scalable Processors, Azure performs indirect registration at Intel's Registration Service by using the Platform Manifest and stores the resulting PCK certificate in the Trusted Hardware Identity Management (THIM) service.
Azure uses indirect registration, because Intel's registration service won't store root keys for a platform in this case and this condition is reflected by `false` in the `CachedKeys` flag in PCK certificates.
Because Azure uses indirect registration, all subsequent communication to Intel PCS requires the Platform Manifest, which Azure doesn't provide to virtual machines (VMs).
Instead, VMs must reach out to THIM to receive PCK certificates.
...
You might want the flexibility to use Intel QPL to interact with Trusted Hardware Identity Management without downloading another dependency from Microsoft, such as the Azure DCAP client library. To use Intel QPL with the Trusted Hardware Identity Management service, adjust the Intel QPL configuration file, *sgx_default_qcnl.conf*.

The quote generation and verification collateral that's used to generate the Intel SGX or Intel TDX quotes can be split into:

- The PCK certificate. To retrieve it, you must use a Trusted Hardware Identity Management endpoint.
- All other quote generation and verification collateral. To retrieve it, you can use a Trusted Hardware Identity Management endpoint or an Intel Provisioning Certification Service (PCS) endpoint.

The Intel QPL configuration file (*sgx_default_qcnl.conf*) contains three keys for defining the collateral endpoints. The `pccs_url` key defines the endpoint that's used to retrieve the PCK certificates. The `collateral_service` key can define the endpoint that's used to retrieve all other quote generation and verification collateral. If you don't define the `collateral_service` key, all quote verification collateral is retrieved from the endpoint defined with the `pccs_url` key.

The following table shows how you can configure these keys.
...
1. Ensure read permissions to the file from the following registry location and key/value pair:
...
1. Restart the AESMD service. For example, open PowerShell as an administrator and use the following commands:
...
1. Make changes to the configuration file. For example, use Vim by running the following command:

   ```bash
   sudo vim /etc/sgx_default_qcnl.conf
   ```

1. Restart the AESMD service. Open any terminal and run the following commands:
...
|--|--|--|
| `Metadata` | Boolean | Set to `True` to return collateral. |
...
1. Start by creating an Azure Kubernetes Service (AKS) cluster on a CVM node or by adding a CVM node pool to an existing cluster:
    - Create an AKS cluster on a CVM node:
       ...
       1. Create an AKS cluster with one CVM node in the resource group:
...
1. Verify the connection to your cluster by using the `kubectl get` command. This command returns a list of the cluster nodes.

    ```bash
    kubectl get nodes
    ```

    The following output example shows the single node that you created in the previous steps. Verify that the node status is `Ready`.
...
1. Run the job by applying the *curl.yaml* file:

    ```bash
    kubectl apply -f curl.yaml
    ```

1. Check and wait for the pod to complete its job:
...
1. Run the following command to get the job logs and validate that it works. A successful output should include `vcekCert`, `tcbm`, and `certificateChain`.
...
- Learn more about [Azure confidential computing](../../confidential-computing/overview.md).
