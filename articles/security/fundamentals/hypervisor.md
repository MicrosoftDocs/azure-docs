...
description: Learn how Azure hypervisor security uses isolation, integrity controls, restricted access, auditing, exploit mitigations, and assurance processes.
author: yosharm
ms.service: security
...
The Azure hypervisor is built with the following security objectives in mind:
...
| Platform integrity | The integrity of the hypervisor depends on the integrity of the hardware and software on which it relies. Although the hypervisor doesn't have direct control over the integrity of the platform, Azure relies on hardware and firmware mechanisms such as the [Cerberus](project-cerberus.md) chip to protect and detect the underlying platform integrity. Azure prevents the VMM and guests from running if platform integrity is compromised. |
| Restricted access | Only authorized administrators connected over secure connections exercise management functions. Azure role-based access control (Azure RBAC) mechanisms enforce least privilege. |
...
Microsoft's Red Team threat models, code reviews, fuzzes, and tests all VM attack surfaces for security boundary violations. Microsoft also has a [bug bounty program](https://www.microsoft.com/msrc/bounty-hyper-v) that pays an award for relevant vulnerabilities in eligible product versions for Microsoft Hyper-V.
