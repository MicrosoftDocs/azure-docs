...
- **Review Microsoft’s [Enterprise access model](/privileged-access-workstations/privileged-access-access-model)**.
...
>
> Ensure that any actions taken are performed from a trusted device, built from a [clean source](/privileged-access-workstations/privileged-access-access-model). For example, use a fresh, [privileged access workstation](/privileged-access-workstations/privileged-access-deployment).
>
...
|**Remove unnecessary admin users**     |   Remove unnecessary members from Domain Admins, Backup Operators, and Enterprise Admin groups. For more information, see [Securing Privileged Access](/privileged-access-workstations/overview). |
