...
**Detail**: You can configure your Azure Pipelines to [automatically build and deploy](https://docs.microsoft.com/ja-jp/azure/devops/pipelines/index?view=vsts) to Azure web apps or cloud services. Azure Pipelines automatically deploys the binaries after doing a build to Azure after every code check-in. The package build process is equivalent to the Package command in Visual Studio, and the publishing steps are equivalent to the Publish command in Visual Studio.
