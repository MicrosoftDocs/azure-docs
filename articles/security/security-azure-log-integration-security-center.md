...
This article expects that you completed successfully the steps in the [Get started](security-azure-log-integration-get-started.md) article.
...
2. Next you will run a command that assigns reader access on the subscription to the service principal created in step 2. If you don’t specify a SubscriptionID, then the command will attempt to assign the service principal reader role to all subscriptions to which you have any access. </br>
``azlog authorize <SubscriptionID>`` for example </br>
