...
For example, the risk of attack increases with the size or surface area of the application. You can reduce the surface area by using whitelisting to close down the exposed IP address space and listening ports that are not needed on the load balancers ([ALB](../load-balancer/load-balancer-get-started-internet-portal.md)/[App Gateway](../application-gateway/application-gateway-create-probe-portal.md)). [Network Security Groups (NSGs)](../virtual-network/virtual-networks-nsg.md) are another way to reduce the attack service.
