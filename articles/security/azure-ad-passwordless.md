...
![See the source image](./media/azure-ad/azure-ad-pwdless-image2.jpeg)

The following steps illustrate the sign-in authentication to Azure Active Directory.

![Windows 10 lock screen](./media/azure-ad/azure-ad-pwdless-image3.png)
...
Prerequisites for using the Microsoft Authenticator app to sign in to Azure AD include the following:
...
> [!NOTE]
> A potential point of failure for this solution is when a roaming user is in a location where there is no Internet connectivity. FIDO security keys and Windows Hello for Business are not subject to the same limitation.
...
#### User using Microsoft Authenticator for sign-in
...
![Authenticator sign-in](./media/azure-ad/azure-ad-pwdless-image4.png)
...
![Authenticator process](./media/azure-ad/azure-ad-pwdless-image5.png)
...
#### User manages their Microsoft Authenticator credentials
...
![FIDO2 sign-in](./media/azure-ad/azure-ad-pwdless-image6.png)
...
![FIDO2 sign-in process](./media/azure-ad/azure-ad-pwdless-image9.png)

1. The user plugs the FIDO2 device into the computer.

2. Windows detects the FIDO2 device.

3. Windows sends an authentication request.

4. Azure AD sends back a nonce.

5. The user completes their gesture to unlock the private key stored in the FIDO2 device's secure enclave.

6. The FIDO2 device signs the nonce with the private key.

7. The PRT token request with signed nonce is sent to Azure AD.

8. Azure AD verifies the FIDO key.

9. Azure AD returns PRT to enable access to on-premises resources.

#### User manages their FIDO2 security key credentials

Similar to the Microsoft Authenticator app, credential management for FIDO2 security keys relies on combined registration.
...
|**Information worker**|Productivity work| Mobile or non-windows device|Microsoft Authenticator|
