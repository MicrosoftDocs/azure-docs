...
- Large applications, such as SAP, MongoDB, or Apache Cassandra, are installed and running in the OS before encryption. The extension is unable to properly shut these applications down. If the applications maintain open file handles to the OS drive, the drive cannot be unmounted, and this causes failure.
- Custom scripts being run in close time proximity to encryption. Errors can also occur if any other changes are made on the VM during the encryption process. Errors can happen when an Azure Resource Manager template defines multiple extensions to execute simultaneously. Errors can also happpen when a custom script extension or other action runs simultaneously with disk encryption. Serializing and isolating such steps might resolve the issue.
