...
- Azure Disk Encryption requires the vfat module to be present on the system to read the key volume.  Removing or disabling this module from an otherwise supported image will prevent the system from being able to read the key volume, which in turn will prevent boot. This situation can sometimes occur when carrying over hardening steps that were designed for other environments.  
