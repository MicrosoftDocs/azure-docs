...
### DoSGuard
Denial-of-service (DoS) attacks are further reduced by an SQL Database Gateway service called DoSGuard that actively tracks failed logins from IP addresses, and in the event of multiple failed logins from a specific IP address within a period of time, the IP address is blocked from accessing any resources in the service for a pre-defined time period.
...
Approved mechanisms to implement network-based ACLs include: ACLs on routers and load balancers managed by Azure Networking, Guest VM firewall and Microsoft Azure SQL Database gateway firewall rules (configured by the customer).

## Data segregation and customer isolation
The Azure Production network is structured such that publicly-accessible system components are segregated from internal resources. Physical and logical boundaries exist between web servers providing access to the public-facing Azure Management Portal and the underlying Azure virtual infrastructure where customer application instances and customer data reside.

All publicly-accessible information is managed within the Azure Production network, which is subject to two-factor authentication and boundary protection mechanisms and uses the firewall and security feature set described in the previous section and specifically uses data isolation functions as noted below.
...
Standard secure configurations are maintained by respective operations teams in Azure and Microsoft Azure SQL Database of all components within the production environment. All configuration changes to production systems are documented and tracked through a central tracking system. Software and hardware changes are tracked through the central tracking system, and networking changes relating to ACL are tracked using ACL Management Service (AMS).

All configuration changes to Microsoft Azure are developed and tested in the staging environment; and thereafter deployed in production environment. Software builds are reviewed for their features as part of testing and security and privacy checks as part of entry checklist criteria. Changes are deployed on scheduled intervals by the respective deployment team. Releases are reviewed and signed off by the respective deployment team personnel before they are deployed into production.

Changes are monitored for success, and on a failure scenario the change is rolled-back to its previous state or a hotfix is deployed to address the failure with approval of the designated personnel. Source Depot, Git, TFS, MDS, Runners, Azure Security Monitoring (ASM), the FC, and the WinFabric platform (formerly Service Management Platform, used by Microsoft Azure SQL Database) are used to centrally manage, apply and verify the configuration settings in the Azure virtual environment.
