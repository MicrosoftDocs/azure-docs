...
| IsMfaRegistered | Yes |boolean | Indicates whether the user already enrolled a method for multi-factor authentication. If `false`, the Conditional Access policy evaluation returns a `block` challenge if action is required. |
