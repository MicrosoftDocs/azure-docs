...
The seamless migration flow still requires bulk migration of user accounts, but then uses a [custom policy](restful-technical-profile.md) to query a [REST API](custom-policy-rest-api-claims-validation.md) (which you create) to set each users' password at first sign-in.
