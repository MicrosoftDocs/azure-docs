...
1. Select **New policy** and follow the Azure AD Conditional Access documentation to create a new policy. For risk-based policies, you will need to configure separate policies based on [user risk](https://docs.microsoft.com/en-us/azure/active-directory/conditional-access/howto-conditional-access-policy-risk-user#enable-with-conditional-access-policy) or [sign-in risk](https://docs.microsoft.com/en-us/azure/active-directory/conditional-access/howto-conditional-access-policy-risk#enable-with-conditional-access-policy) depending on which type of risk you want to use as a condition. We do not recommend using both risk types in a single policy.
