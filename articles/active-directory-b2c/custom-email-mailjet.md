...
> [!NOTE]
> One-time passwords that are generated by the Web.TPEngine.Providers.OneTimePasswordProtocolProvider protocol are tied to the browser session. This means a user can generate unique passwords in different browser sessions that are each valid for their corresponding sessions. By contrast, a password generated by the built-in user flow is independent of the browser session, so if a user generates a new password in a new browser session, it replaces the previous password.
