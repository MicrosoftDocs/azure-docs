...
| AllowInsecureAuthInProduction| No| Indicates whether the `AuthenticationType` can be set to `none` in production environment (where the `DeploymentMode` of the [TrustFrameworkPolicy](trustframeworkpolicy.md) is set to `Production`). Possible values: true, or false (default). |
