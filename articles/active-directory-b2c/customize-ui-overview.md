...
You can customize the user interface (UI) of pages that are served by Azure Active Directory (Azure AD) B2C to provide your customers with a seamless and branded experience. The experiences that you can customize include sign-up, sign-in, profile editing, and password resetting.
...
- If you're using [built-in policies](https://docs.microsoft.com/en-us/azure/active-directory-b2c/active-directory-b2c-reference-policies) to provide sign-up or sign-in, password reset, or profile-editing experiences in your application, you can use the [Azure portal to customize the UI](tutorial-customize-ui.md).
...
- If a customer intiates an edit profile policy before signing in, the user is redirected to a page that can be customized using the same steps used for customizing the Azure AD sign-in page.
...
- If you need to provide dynamic content based on a customer’s decision, you can use [custom policies that can change page content](active-directory-b2c-ui-customization-custom-dynamic.md) depending on a parameter that is sent in a query string. For example, you can change the background image on the Azure AD B2C sign-up or sign-in page, based on a parameter that you pass from your web or mobile application.
...
- Azure AD B2C injects HTML content into your pages. Don't copy the default content that Azure AD B2C provides and attempt to modify it. It's best to build your HTML5 content from scratch and use the default content as reference.
...
You can host your UI content anywhere, such as on [Azure Blob storage](../storage/blobs/storage-blobs-introduction.md), web servers, CDNs, AWS S3, or file sharing systems. The content needs to be hosted on a publicly available HTTPS endpoint with CORS enabled.
...
Or, you can pull content from different places based on the locale that's used. In your CORS-enabled endpoint, you can set up a folder structure to host content for specific languages. You'll call the right one if you use the wildcard value {Culture:RFC5646}. For example, your custom page URI might look like `https://contoso.blob.core.windows.net/{Culture:RFC5646}/myHTML/unified.html`.
