---
title: Azure Active Directory B2C global identity framework proof of concept
description: Learn how to create a proof of concept for Azure AD B2C to provide customer identity and access management for global customers.
...
# Azure Active Directory B2C global identity framework proof of concept configuration
...
Sign-up journey:
* Display a screen to collect the users username, password, and any other attributes
...
The below diagram shows the block diagram of the proof of concept. The guidance will show how to configure the the Azure AD B2C tenants. The External API layer and Geo distributed lookup table is not included as part of this guide.
...
* The response should be a HTTP 200 if the user does not exist.
...
* The response should be a HTTP 200 if the user exists.
...
The response should be a HTTP 200 with the users registered region and objectId.
...
The API should respond with a HTTP 409 if the user does not exist, or encounters an error.
...
The response should be a HTTP 200 if the process suecceeds, or HTTP 409 if there is an error.

## Region-based Azure AD B2C configuration

The following sections prepare the Azure AD B2C tenant to track the region in which the user signed-up and perform cross tenant authentications or password resets if required.
...
Modify the `LocalAccountSignUpWithLogonEmail` technical profile in the Azure AD B2C starter pack as below:
...
1.	Verify if the user already exists in the user-region mapping via your secured external REST API endpoint:
    * This API call is made before all sign-up’s, it is critical to make sure this API has appropriate load balancing, resiliency, and failover mechanisms to uphold uptime requirements.

    * An example of a technical profile to query a user-region mapping via an external REST API is below:
      ...
      ```
     * This API should respond with HTTP 409 if the user exists, with appropriate error message to be displayed on screen. Otherwise, respond with a HTTP 200 if the user does not exist.
1.	Write the user-region mapping via your secured external REST API endpoint

      * This API call is made before all sign up’s, it is critical to make sure this API has appropriate load balancing, resiliency, and failover mechanisms to uphold uptime requirements.

      * An example of a technical profile to write the user-region mapping via an external REST API is below:
...
Modify the `SelfAsserted-LocalAccountSignin-Email` technical profile in the Azure AD B2C starter pack to perform the user-region lookup, and perform cross tenant authentication when the user is from a different region to that of the tenant they have reached. Update the `ValidationTechnicalProfiles` as shown below:
...
1.	Get a token to call your protected API endpoints using the `REST-getTokenforExternalApiCalls` technical profile.
    ...
    * 	Follow the documentation [here](secure-rest-api.md?tabs=windows&pivots=b2c-custom-policy#using-oauth2-bearer) to obtain and protect your API using an Azure AD bearer token
1.	Lookup the user-region mapping via your secured external REST API endpoint
    * This API call is made before all sign-up’s, it is critical to make sure this API has appropriate load balancing, resiliency, and failover mechanisms to uphold uptime requirements.
...
1.	Conditionally, perform a cross tenant authentication via the `REST-login-NonInteractive-[region]` technical profiles for each respective region.
    ...
    * An example of a technical profile to perform user-region mapping via an external REST API is below:
        ```xml
...
1.	Conditionally, fetch the user profile using a cross tenant REST API call via the `REST-fetchUserProfile-[region]` technical profiles for each respective region.
    ...
    * Below is an example technical profile to read the users profile via MS Graph API
        ```xml
...
Modify the `LocalAccountSignUpWithLogonEmail` technical profile in the Azure AD B2C starter pack to perform the user user-region lookup, and update the password in the respective tenant. Update the `ValidationTechnicalProfiles` as shown below:
...
1.	Lookup the user-region mapping via your secured external REST API endpoint
    * This API call is made before all password reset attempts, it is critical to make sure this API has appropriate load balancing, resiliency, and failover mechanisms to uphold uptime requirements.
...
The following section describes how to create proof of concept implementations for funnel tenant orchestration. The completed Azure Active Directory B2C (Azure AD B2C) custom policies can be found [here](https://github.com/azure-ad-b2c/samples/tree/master/policies/global-architecture-model).
...
1. Configure `client_id` mapping to region – use the [lookup claim transformation](general-transformations.md) to emulate.
...
1. Configure the federations to be enabled based on client_id–region mapping
...
1. [Create a federation](../active-directory-b2c/identity-provider-azure-ad-b2c.md?pivots=b2c-custom-policy) between funnel and regional tenants.
...
1. Configure API based authentication for traveling user, and MS Graph API call to fetch users profile.
   * Check credentials and get MS Graph API token from respective tenant. Register **Native** App registration in each regional tenant with permissions to MS Graph API for delegated permission: *user.read*.
