...
| response_type |Required |The response type, which must include `code` for the authorization code flow. You can receive and ID token if you include it, such as `code+id_token`, and in this case, the scope needs to include `openid`.|
