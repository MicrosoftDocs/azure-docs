...
- Check whether the `ClaimTypeReferenceId` value is misspelled, or does not exist in the schema.
- If the claim is defined in the extensions policy, but it's also being used in the base policy. Make sure the claim is defined in the policy it's in used, or in an upper level policy.
