...
|`<Response>` | `IssueInstant` | The time instant of issue of the response. The time value is encoded in UTC.  To change the settings on your token lifetimes, set the `TokenNotBeforeSkewInSeconds` [metadata](saml-issuer-technical-profile.md#metadata) of the SAML token issue technical profile. | 
|`<Response>` | `Destination`| A URI reference indicating the address to which this response has been sent. The value is always identical to the SAML request `AssertionConsumerServiceURL`. | 
...
|`<Response>`,`<Assertion>`,`<Subject>`,`<Conditions>` |`NotBefore` |The time at which the token becomes valid, represented in epoch time. This time is usually the same as the time the token was issued. Your application should use this claim to verify the validity of the token lifetime. To change the settings on your token lifetimes, set the `TokenNotBeforeSkewInSeconds` [metadata](saml-issuer-technical-profile.md#metadata) of the SAML token issue technical profile. |
|`<Response>` `<Assertion>` `<Subject>` `<Conditions>` | `NotOnOrAfter` | The time at which the token becomes invalid, represented in epoch time. Your application should use this claim to verify the validity of the token lifetime. The value is 15 minutes after the `NotBefore` and cannot be changed.   |
|`<Response>` `<Assertion>` `<Subject>` `<Conditions>` `< AudienceRestriction>` `<Audience>` | |  A URI reference that identifies an intended audience. It identifies the intended recipient of the token. The value is always identical to the SAML request `AssertionConsumerServiceURL`.       |
