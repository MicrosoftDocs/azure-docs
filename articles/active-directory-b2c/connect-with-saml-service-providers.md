...
|`<Response>` `<Assertion>` `<saml:AttributeStatement>` collection of `<Attribute>` | | List of assertions (claims) as configured in the [relying party technical profile](relyingparty.md#technicalprofile) output claims. You can configure the name of the assertiong, by setting the `PartnerClaimType` of the output cliam. |
