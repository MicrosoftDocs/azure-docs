...
To Encrypt SAML Assertions sent back to the Service Provider, Azure AD B2C will use the Service providers public key certificate. This must exist in the SAML Metadata outlined in the above ["samlMetadataUrl"](#samlmetadataurl) as a KeyDescriptor with a use of 'Encryption'.
...
To enable Azure AD B2C to send encrypted assertions add the **'WantsEncryptedAssertion'** metadata item to the Relying Party Technical Profile as shown below;
