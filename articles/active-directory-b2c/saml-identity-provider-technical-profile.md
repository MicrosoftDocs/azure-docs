...
| --------- | -------- | ----------- |
| PartnerEntity | Yes | URL of the metadata of the SAML identity provider. Or copy the identity provider metadata and embed it inside the CDATA element `<![CDATA[Your IDP metadata]]>`. Embedding the identity provider metadata is not recommended. The identity provider may change the settings, or update the certificate. In case, get the new metadata and update your policy with the new one. |
