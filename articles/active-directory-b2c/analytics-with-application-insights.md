...
# Track user behavior in Azure Active Directory B2C by using Application Insights
...
To enable custom event logs, add an Application Insights technical profile. In the technical profile, you'll define the Application Insights instrumentation key, the event name, and the claims to record. To post an event, add the technical profile as an orchestration step in a [user journey](userjourneys.md).
...
A claim provides temporary storage of data during an Azure AD B2C policy execution. You declare your claims in the [ClaimsSchema](claimsschema.md).
...
Technical profiles can be considered functions in the custom policy. The function uses the [technical profile inclusion](technicalprofiles.md#include-technical-profile) approach, where a technical profile includes another technical profile and changes settings or adds new functionality. The following table defines the technical profiles that are used to open a session and post events.

| Technical Profile | Task |
| ----------------- | -----|
| AppInsights-Common | This is the common technical profile with typical configuration. It includes the Application Insights instrumentation key, a collection of claims to record, and developer mode. The other technical profiles include the common technical profile and add more claims, such as the event name. |
...
You can use [input claims transformations](custom-policy-trust-frameworks.md#manipulating-your-claims) to modify the input claims or generate new ones before sending them to Application Insights. In the following example, the technical profile includes the **isAdmin** input claims transformation.
...
To disable Application Insight logs, change the **DisableTelemetry** metadata to **true** in the **AppInsights-Common** technical profile:
