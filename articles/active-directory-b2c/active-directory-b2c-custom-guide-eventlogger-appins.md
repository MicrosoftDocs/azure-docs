...
Application Insights can unify the events by using a correlation ID to record a user session. Application Insights makes the event and session available in seconds and presents many visualization, export, and analytical tools.
...
For more information, see the [full documentation for Application Insights](https://docs.microsoft.com/azure/application-insights/).
...
2. Select **Events** in the **USAGE** submenu.
3. Set **During** to **Last hour** and **By** to **3 minutes**. You might need to select **Refresh** to view results.

![Graph for Application Insights usage events](./media/active-directory-b2c-custom-guide-eventlogger-appins/app-ins-graphic.png)
...
### Non-protocol parameters included with OIDC and OAuth2 requests
...
```
You can then add the claims by adding an `InputClaim` element to the Application Insights event:
```
...
Some system claims must be added to the claims bag before they're available to record as events. The technical profile `SimpleUJContext` must be called as an orchestration step or a validation technical profile before these claims are available.
