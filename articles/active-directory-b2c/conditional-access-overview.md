...
You can enhance the security of your Azure Active Directory B2C (Azure AD B2C) implementation with the Identity Protection and Conditional Access capabilities in Azure AD. [Identity Protection](https://docs.microsoft.com/azure/active-directory/identity-protection/overview-identity-protection) allows organizations to automate the detection and remediation of identity-based risks, investigate risks, and export risk detection data to third-party utilities for further analysis. [Conditional Access](https://docs.microsoft.com/azure/active-directory/conditional-access/overview) is the policy engine that brings signals together to make decisions and enforce organizational policies. Together, these capabilities give application owners significantly greater control over risky authentications and access policies.

- You can use Conditional Access with an existing Azure AD B2C subscription.  
- To use Identity Protection and risk-based Conditional Access policies, you must [upgrade your subscription to Azure AD B2C Premium 2].  

If you're already familiar with Identity Protection and Conditional Access capabilities in Azure AD, using them for Azure AD B2C will be a familiar experience, with a few minor differences as discussed in this article.
...
Identity Protection risk detections are only available for local B2C accounts, not for social identities (Google, Facebook, etc). 
