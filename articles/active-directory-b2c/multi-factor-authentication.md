...
1. Under **Usable authentication methods**, find **Software OATH token (Preview)**, and then select the 3-dot menu next to it. If you don't see this interface, select the option to **"Switch to the new user authentication methods experience! Click here to use it now"**, located below the Authentications methods menu in the Portal, to switch to the new authentication methods experience.
