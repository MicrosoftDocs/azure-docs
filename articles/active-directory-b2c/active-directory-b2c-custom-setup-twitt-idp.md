...
## Create a Twitter account application
...
## Add the Twitter account application key to Azure AD B2C
...
## Register the Twitter account claims provider to Sign up or Sign in user journey
At this point, the identity provider has been set up. However, it is not available in any of the sign-up/sign-in screens. Now you need to add the Twitter account identity provider to your user `SignUpOrSignIn` user journey. To make it available, we create a duplicate of an existing template user journey. Then we add the Twitter identity provider:
...
## [Optional] Register the Twitter account claims provider to Profile-Edit user journey
You may want to add the Twitter account identity provider also to your user `ProfileEdit` user journey. To make it available, we repeat the last two steps:
