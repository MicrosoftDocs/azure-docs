...
By using Azure Active Directory B2C (Azure AD B2C) custom policies, you can send through a parameter in a query string. The parameter is passed on to your HTML endpoint, and it can dynamically change the page content. For example, you can change the B2C sign-up or sign-in the background image, based on a parameter you pass from your web or mobile application. 
...
| *api.signuporsignin* | [unified.html](https://login.microsoftonline.com/static/tenant/default/unified.cshtml) | **Unified sign-up or sign-in page**. This page handles user sign-up and sign-in. Users can use enterprise identity providers, social identity providers such as Facebook or Google+, or local accounts.  |
...
* Add the custom HTML5 template _unified.cshtml_. 
...
Locate the `<img>` element with ID `background_background_image`, and replace the `src` value with **https://kbdevstorage1.blob.core.windows.net/asset-blobs/19889_en_1**, or any other background image you want.
...
Debug your web app, and make sure that the _unified_ page is accessible (for example, `http://localhost:<Port number>/Home/unified`).
...
1. In the [Azure portal](https://portal.azure.com/), click **App Services**, and then select the name of your API app.
...
3. In the **CORS** window, in the **Allowed Origins** text box, do either of the following:
...
5. Search for the `<ContentDefinition>` node that includes `Id="api.signuporsignin"` in the XML that you copied.
...
7. Change the value to *Tokyo*, and then press **Enter**.  
    The window displays the Tokyo background.
...
If you select the **Sign up now** link on the sign-in page, the browser displays the default background image, not the image you defined. This behavior results because you've changed only the sign-up or sign-in page. To change the rest of the Self-Assert content definitions:
1. Go to "Step 2," and do the following:
...
2. Go to "Step 4," and do the following: 
...
3. Go to "Step 8.2," and change your code to accept query string parameters. 
