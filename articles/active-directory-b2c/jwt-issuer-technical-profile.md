...
The **Name** attribute of the **Protocol** element needs to be set to `None`. The **OutputTokenFormat** element needs to be set to `JWT`.

The following shows a technical profile for `JwtIssuer`:
...
| SendTokenResponseBodyWithJsonNumbers | No | Always set to `true`. For legacy format where numeric values are given as strings instead of JSON numbers, set to `false`. This is needed for clients that have taken a dependency on an earlier implementation that returned such properties as strings. | 
...
| refresh_token_lifetime_secs | No | Refresh token lifetimes. The maximum time period before which a refresh token can be used to acquire a new access or ID token and optionally, a new refresh token, if your application had been granted the offline_access scope. The default is 120,9600 seconds (14 days). The minimum (inclusive) is 86,400 seconds (24 hours). The maximum (inclusive) is 7,776,000 seconds (90 days). | 
...
| issuer_refresh_token_key | Yes | The X509 certificate (RSA key set) to use to encrypt the refresh token. This is the `B2C_1A_TokenEncryptionKeyContainer` key you cofigured in [Get started with custom policies](active-directory-b2c-get-started-custom.md) |
