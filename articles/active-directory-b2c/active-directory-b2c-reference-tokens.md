...
| Refresh tokens |Up to 14 days |A single refresh token is valid for a maximum of 14 days. However, a refresh token can become invalid at any time for any number of reasons. Your app should continue to try to use a refresh token until the request fails, or until your app replaces the refresh token with a new one. A refresh token can also become invalid if 90 days has passed since the user last entered credentials. |
