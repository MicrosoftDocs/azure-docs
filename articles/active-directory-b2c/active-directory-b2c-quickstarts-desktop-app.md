...
Click **Sign in** in the sample application to start the **Sign Up or Sign In** workflow. When creating an account, you can use an existing social identity provider account or an email account.
...
You need to authenticate (sign-in) using your social account credentials and authorize the application read information from your social account. By granting access, the application can retrieve profile information from the social account such as your name and city. For example, if you chose to use Twitter, the authentication and authorization looks similar to the following.
...
![New account signup profile details](media/active-directory-b2c-quickstarts-desktop-app/new-account-sign-up-profile-details-wpf.png)

You have successfully created a new Azure B2C user account that uses an identity provider. After sign-in, the access token is shown in the *Token info* text box. The access token is used when accessing the API resource.
...
You have successfully created a new Azure B2C local user account. After sign-in, the access token is shown in the *Token info* text box. The access token is used when accessing the API resource.
...
Click **Call API** to to make a request to the Azure AD B2C secured resource https://fabrikamb2chello.azurewebsites.net/hello. 

![Call API](media/active-directory-b2c-quickstarts-desktop-app/call-api-wpf.png)

When making the request, the application includes the access token displayed in the *Token info* text box as part of the request. The API sends back the display name contained in the access token.
