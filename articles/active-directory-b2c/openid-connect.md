...
OpenID Connect also enables applications to securely acquire *access tokens*. You can use access tokens to access resources that the  [authorization server](protocols-overview.md) secures. OpenID Connect is recommended if you're building a web application that's hosted on a server and accessed through a browser. For more information about tokens, see the [Overview of tokens in Azure Active Directory B2C](tokens-overview.md)
...
| {policy} | Yes | The user flow or policy to be run. Specify the name of a user flow that you created in your Azure AD B2C tenant. For example: `b2c_1_sign_in`, `b2c_1_sign_up`, or `b2c_1_edit_profile`. |
