...
In this request, the client indicates the permissions that it needs to acquire from the user in the `scope` parameter, and specifies the user flow to run. To get a feel for how the request works, try pasting the request into a browser and running it. Replace `{tenant}` with the name of your tenant. Replace `90c0fe63-bcf2-44d5-8fb7-b8bbc0b29dc6` with the app ID of an application you've previously registered in your tenant or [register a web application](tutorial-register-applications.md#register-a-web-application). Replace `{application-id-uri}/{scope-name}` with the **Application ID URI** and **Scope** of an application that you've previously registered in your tenant. Go to the Azure AD B2C blade, select **App registrations**, select **All applications** and select an application. Under the **Manage** section, select **Expose an API**. If it's preferable to start fresh, [register a new web api](add-web-api-application.md#app-registrations) and [configure scopes](add-web-api-application.md#configure-scopes). Also change the policy name (`{policy}`) to the policy name that you have in your tenant, for example `b2c_1_sign_in`.
...
| response_type | Yes | The response type, which must include `code` for the authorization code flow. If your web application also needs tokens for calling a web API, you can use `code+token`. When adding `token` to the response_type parameter value you must add a valid resource in the `scope` parameter. |
| scope | Yes | A space-separated list of scopes. The `openid` scope indicates a permission to sign in the user and get data about the user in the form of ID tokens. The `offline_access` scope is optional for web applications. It indicates that your application will need a *refresh token* for extended access to resources. The `{application-id-uri}/{scope}` indicates a permission to protected resources, such as a web API. For more information, see [Request an access token](access-tokens.md#scopes). |
...
| response_mode | Recommended | The method that is used to send the resulting authorization code back to your application. It can be either `query`, `form_post`, or `fragment`.  The `form_post` response mode is recommended for best security. |
...
A successful response using `response_mode=fragment` and `response_type=code+token` would look like:

```http
GET https://jwt.ms/#
access_token=eyJ0eXAiOiJKV1QiLCJhbGciOiJSUzI1NiIsIng1dCI6Ik5HVEZ2ZEstZnl0aEV1Q...
&code=eyJraWQiOiJBQjk4dkdFRHY2Nl9ScmJEajJmclFFMUFOM1hSX3k4ZVU3X3V5NDBjN...
&state=arbitrary_data_you_can_receive_in_the_response
```

| Parameter | Description |
| --------- | ----------- |
| access_token | The access token for a target resource that the application requested if you used `response_type=code+token`. |
| code | The authorization code that the application requested. The application can use the authorization code to request an ID token and an access_token, if a target resource was included in the scope parameter. Authorization codes typically expire after about 10 minutes. |
...
To verify the tokens from Azure AD B2C, you need to generate the public key using the exponent(e) and modulus(n). You need to determine how to do this in your respective programming language accordingly. The official documentation on Public Key generation with the RSA protocol can be found at this [Internet Engineering Task Force (IETF) page](https://tools.ietf.org/html/rfc3447#section-3.1).
