...
1. For **Authentication Type**, select the authentication that's required and enabled on your database in Azure SQL Database or Azure SQL Managed Instance:
   ...
   | **Managed Identity** | - Supports both the non-ISE and ISE SQL Server connector. <p><p>- Requies a valid managed identity that has [access to your database](../active-directory/managed-identities-azure-resources/tutorial-windows-vm-access-sql.md), SQL DB Contributor role access to the SQL Server resource, as well as Contributor access to the resource group that the SQL Server is in. <p>For more information, see these topics: <p>- [SQL - Server-Level Roles](https://docs.microsoft.com/en-us/sql/relational-databases/security/authentication-access/server-level-roles?view=sql-server-ver15)
   |||
