...
In Standard logic app workflows, peek-lock operations are available for *stateless* workflows only by default. To run this operation in stateful mode, see [Enable stateful mode for stateless built-in connectors](../connectors/enable-stateful-affinity-built-in-connectors.md).
