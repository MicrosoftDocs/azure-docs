...
Today configuration settings for the Service Bus built-in trigger are shared between the [functions host extension](../azure-functions/functions-bindings-service-bus.md#hostjson-settings) defined in the [host.json file](../logic-apps/edit-app-settings-host-settings.md) and the trigger settings defined in the Logic Apps workflow and set through the designer or code view. Settings for both locations are covered in this section. In the future more settings will be made available on the workflow definition of the trigger which will simplify configuration.
