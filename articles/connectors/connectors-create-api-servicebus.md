...
1. In the search box, enter "azure service bus" as your filter. From the triggers list, select the trigger that you want.
   ...
   Some triggers, such as the **When one or more messages arrive in a queue (auto-complete)** trigger, can return one or more messages. When these triggers fire, they return between one and the number of messages that's specified by the trigger's **Maximum message count** property.
    ...
    > [!NOTE]
    > The auto-complete trigger automatically completes a message, but completion happens only at the next trigger run. This behavior can affect your logic app's design. If you have set concurrency control on the logic app, a throttled trigger will be skipped with `WorkflowRunInProgress` code and the completion operation will not be carried out and next trigger operation will be after the polling interval.  You need to set the service bus lock duration to a value that's longer than the polling interval. Even with that the message may still not be able to be completed if the logic app is still in throttling state at next polling interval. Thus, to avoid duplicate messages we suggest not implemnting concurrency control if you want to use the auto-complete trigger. 
