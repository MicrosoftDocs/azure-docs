...
For example, if you create a single-tenant logic app, both built-in operations and [managed connector operations](managed.md) are available for a few services, specifically Azure Blob, Azure Event Hubs, Azure Cosmos DB, Azure Service Bus, DB2, IBM Host File, MQ, and SQL Server. In few cases, a built-in operation is available only in one environment, for example, AS2 (V2) is currently available only in multi-tenant, while IBM Host File is available only in single-tenant. In most cases, the built-in version provides better performance, capabilities, pricing, and so on.
