...
| **Filter array** | Filter a result set to the entries that interest you.  For example, search all tweets with `#Azure`, and then “filter” the returned tweets to only return results that are `Tweeted_by_followers > 50`. |
...
The Enterprise Integration Pack (EIP) includes connectors that are very well known for those in the BizTalk Server space. When you purchase an [integration account](../logic-apps/logic-apps-enterprise-integration-create-integration-account.md), you also get the following connectors: 
