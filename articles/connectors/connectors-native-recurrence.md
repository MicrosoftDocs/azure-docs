...
1. To set advanced scheduling options, open the **Add new parameter** list. Any options that you select appear on the trigger after selection.
   ...
   | **At these minutes** | `minutes` | No | Integer or integer array | If you select "Day" or "Week", you can select one or more integers from 0 to 59 as the minutes of the hour when you want to run the workflow. <p>For example, you can specify "30" as the minute mark and using the previous example for hours of the day, you get 10:30 AM, 12:30 PM, and 2:30 PM. <p>In some runs with recurrence trigger, the triggered run timestamp might vary upto 1 minute from the scheduled time. If a usecase of exact scheduled timestamp is needed to be passed for subsequent actions, then you could use template expressions to change the timestamp accordingly.|
   |||||
