...
-	**Frequent communication between application and database**. Applications that cannot leverage application-layer caching or [request batching](../../sql-database/sql-database-use-batching-to-improve-performance.md) and need to send many SQL queries that must be quickly processed are good candidates for the Business Critical tier.
