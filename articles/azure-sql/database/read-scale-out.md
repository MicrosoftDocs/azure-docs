...
> [!NOTE]
> Query Store and SQL Profiler features are not supported on the read-only replicas. 
...
You can verify whether you are connected to a read-only replica by running the following query. It will return READ_ONLY when you are connected to a read-only replica.
...
> [!NOTE]
> At any given time, only one of the AlwaysON replicas is accessible by the Read-Only sessions.

## Monitoring and troubleshooting read-only replica

When connected to a read-only replica, Dynamic Management Views (DMVs) reflect the state of the replica, and can be queried for monitoring and troubleshooting. The database engine provides multiple views to expose a wide variety of monitoring data. 

For monitoring and troubleshooting purposes, commonly used views are:

| Name | Purpose |
|---|---|
|[sys.dm_db_resource_stats](https://docs.microsoft.com/sql/relational-databases/system-dynamic-management-views/sys-dm-db-resource-stats-azure-sql-database)| Provides resource utilization metrics for the last hour.|
...
|[sys.dm_database_replica_states](https://docs.microsoft.com/sql/relational-databases/system-dynamic-management-views/sys-dm-database-replica-states-azure-sql-database)| Provides replica health state and synchronization statistics, including redo queue size and redo rate. |
...
|[sys.dm_exec_query_stats](https://docs.microsoft.com/sql/relational-databases/system-dynamic-management-views/sys-dm-exec-query-stats-transact-sql)| Provide query execution statistics such as number of executions, CPU time used, etc.|
...
|[sys.dm_io_virtual_file_stats()](https://docs.microsoft.com/sql/relational-databases/system-dynamic-management-views/sys-dm-io-virtual-file-stats-transact-sql)| Provides IOPS, throughput, and latency statistics for all database files. |
...
An extended event session cannot be created when connected to a read-only replica. However, in Azure SQL Database, the definitions of database-scoped [Extended Event](xevent-db-diff-from-svr.md) sessions created and altered on the primary replica replicate to read-only replicas, including geo-replicas.
