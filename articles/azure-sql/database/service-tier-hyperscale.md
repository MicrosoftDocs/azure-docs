...
| Migration of databases with persistent In-Memory OLTP objects | Hyperscale only supports non-persistent In-Memory OLTP objects such as memory-optimized table types, table variables, and natively compiled modules. In-Memory OLTP tables, both durable and non-durable, must be dropped and recreated as disk-based objects before migrating a database to the Hyperscale service tier.|
