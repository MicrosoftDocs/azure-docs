...
In this quickstart, you create a [Hyperscale](service-tier-hyperscale.md) database in Azure SQL Database using either the Azure portal, Transact-SQL, a PowerShell script, or an Azure CLI script. You then query the database using **Query editor** in the Azure portal.

TODO: update steps below here to work with Hyperscale, add Transact-sql option (requires that logical server exists already), customize for Hyperscale. Consider putting Transact-SQL option outside of the tabs as it's a bit different.
