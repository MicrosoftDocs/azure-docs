...
Azure SQL Database and Azure SQL Managed Instance are based on the SQL Server database engine, modified to provide a managed relational database service in Azure. Two [vCore](service-tiers-vcore.md) service tiers are available in both Azure SQL Database and Azure SQL Managed Instance:
...
The following table describes the key differences between service tiers. Note that service tier characteristics might be different in SQL Database and SQL Managed Instance.
...
| **Database size** | SQL Database | 1 GB – 4 TB | Up to 100 TB | 1 GB – 4 TB |
| | SQL Managed Instance  | 32 GB – 8 TB | N/A | 32 GB – 4 TB |
| **Storage size** | SQL Database | 1 GB – 4 TB | Up to 100 TB | 1 GB – 4 TB |
| | SQL Managed Instance  | 32 GB – 8 TB | N/A | 32 GB – 4 TB |
...
| **Log write throughput** | SQL Database | [4.5 MB/s per vCore (max 50 MB/s)](resource-limits-vcore-single-databases.md) | 100 MB/s | [12 MB/s per vCore (max 96 MB/s)](resource-limits-vcore-single-databases.md) |
...
- The allocated storage is used by data files and log files.
...
- You can select any maximum data size between 1 GB and the supported maximum, in 1 GB increments.
...
> [!IMPORTANT]
> In the General Purpose and Business Critical tiers, you are charged for the maximum data and log storage size configured for a database, elastic pool, or managed instance. In the Hyperscale tier, you are charged for the allocated data storage.

To monitor the current allocated and used storage size of data files in SQL Database, use *allocated_data_storage* and *storage* Azure Monitor [metrics](/azure/azure-monitor/essentials/metrics-supported#microsoftsqlserversdatabases) respectively. To monitor total allocated instance storage size for SQL Managed Instance, use the *storage_space_used_mb* [metric](/azure/azure-monitor/essentials/metrics-supported#microsoftsqlmanagedinstances). To monitor the current allocated and used storage size of individual data and log files in a database using T-SQL, use the [sys.database_files](/sql/relational-databases/system-catalog-views/sys-database-files-transact-sql) view and the [FILEPROPERTY(... , 'SpaceUsed')](/sql/t-sql/functions/fileproperty-transact-sql) function.
