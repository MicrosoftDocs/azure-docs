...
> [!TIP]
> System updates will take precedence over database restores in progress. All pending restores in case of a system update will be cancelled and restarted from scratch once the update has been applied. This system behavior might prolong the time of restores and might be especially impactful to long-running restores. To achieve a predictable time of database restores, consider configuring [maintenance window](maintenance-window.md) scheduling the system updates at a specific day/time, and consider running database restores outside of the maintenance window day/time schedule.
