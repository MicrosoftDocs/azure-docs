...
In this step, you will create a [failover group](../../sql-database/sql-database-auto-failover-group.md) between an existing server and a new server in another region. Then add the sample database to the failover group.
...
1. Failover groups can be configured at the server level. Select the name of the server under **Server name** to open the settings for the server.
   ...
   ![Open server for database](./media/tutorial-add-single-database-failover-group/open-sql-db-server.png)

1. Select **Failover groups** under the **Settings** pane, and then select **Add group** to create a new failover group.

   ![Add new failover group](./media/tutorial-add-single-database-failover-group/sqldb-add-new-failover-group.png)
...
1. On the **Failover Group** page, enter or select the following values, and then select **Create**:
   ...
   - **Secondary server**: Select the option to *configure required settings* and then choose to **Create a new server**. Alternatively, you can choose an already-existing server as the secondary server. After entering the following values, select **Select**.
     ...
     ![Create a secondary server for the failover group](./media/tutorial-add-single-database-failover-group/create-secondary-failover-server.png)
   ...
   ![Add SQL Database to failover group](./media/tutorial-add-single-database-failover-group/add-sqldb-to-failover-group.png)
...
1. Select **Failover groups** under the **Settings** pane and then choose the failover group you created in section 2.
   ...
   ![Select the failover group from the portal](./media/tutorial-add-single-database-failover-group/select-failover-group.png)
...
1. Select **Yes** on the warning that notifies you that TDS sessions will be disconnected.
   ...
   ![Fail over your failover group containing your SQL database](./media/tutorial-add-single-database-failover-group/failover-sql-db.png)
...
You can find other Azure SQL Database scripts here: [Azure PowerShell](../../sql-database/sql-database-powershell-samples.md) and [Azure CLI](../../sql-database/sql-database-cli-samples.md).
...
> [Tutorial: Add an Azure SQL Database elastic pool to a failover group](tutorial-add-elastic-pool-failover-group.md)
