...
Serialization of virtual cluster management operations is general behavior that applies to the default maintenance policy as well. With maintenance window configured however, period between two adjacent windows can be few days long and submitted operations can also be on hold for few days if the maintenance operation spans two windows. That is very rare case, but creation of new instances or resize of the existing instances (if additional compute nodes are needed) may be blocked during this period.
