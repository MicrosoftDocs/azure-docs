...
- Remediate potential threats in real-time with intelligent [advanced threat detection](https://docs.microsoft.com/azure/security/fundamentals/threat-detection#advanced-threat-detection-features-other-azure-services) and proactive vulnerability assessment alerts. 
...
- [Administration](#administration) - PaaS options reduce the amount of time that you need to invest to administer the database. However, it also limits the range of custom administration tasks and scripts that you can perform or run. For example, the CLR is not supported with single or pooled databases, but is supported for an instance of SQL Managed Instance. Also, no deployment options in PaaS support the use of trace flags.
