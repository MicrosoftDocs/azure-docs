...
The migration entails making full database backups on SQL Server and copying backup files to Azur Blob storage. LRS is used to restore backup files from Azure Blob storage to SQL managed instance. LRS will monitor Azure Blob storage for any new differential, or log backups added after the full backup has been restored, and will automatically restore any new files added. Databases being restored during the migration process will be in a restoring mode and cannot be used to read or write until the process has been completed. The progress of backup files being restored on SQL managed instance can be monitored using the service, and the process can also be aborted if required. 
