...
description: Auto-failover groups let you manage geo-replication and automatic / coordinated failover of all databases on a managed instance in Azure SQL Managed Instance.
...
- **Failover group (FOG)**
  ...
  A failover group allows for all databases within a managed instance to fail over as a unit to another region in case the primary managed instance becomes unavailable due to a primary region outage. Since failover groups for SQL Managed Instance contain all databases within the instance, only one failover group can be configured on an instance. 
...
- **Secondary**
  ...
  The managed instance that hosts the secondary databases in the failover group. The secondary cannot be in the same region as the primary.
