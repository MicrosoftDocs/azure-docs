...
You will then use the Azure AD Hybrid Authentication Management PowerShell module to set up a Trusted Domain Object in the on-premises AD domain and register trust information with Azure AD. This creates an in-bound trust relationship into the on-premises AD, which enables the on-premises AD to trust Azure AD.
...
1. Set the common parameters. Customize the script below prior to running it.
    ...
    ```powershell
    $domain = "your on-prem domain name, for example contoso.com" 
...
1. Check the current Kerberos Domain Settings.
    ...
    If this is the first time you are configuring Azure AD Kerberos settings, the `Get-AzureAdKerberosServer` cmdlet will display empty information, as in the following screenshot.

    TODO: add screenshot.

    If your domain already supports FIDO authentication, the `Get-AzureAdKerberosServer` cmdlet will display the following Azure AD Service account information. Note that the `CloudTrustDisplay` field returns an empty value.
...
1. Deploy the following Group Policy setting to down-level versions of Windows. 
    ...
    - Define the KDC proxy servers settings using mappings as follows. Substitute your Azure AD tenant ID for the `your_Azure_AD_tenant_id` placeholder. Note the space following `https` and the space prior to the closing `/` in the value mapping.
...
For management purposes, you can update the keys for added Azure AD Service account and Trusted Domain Object. 
