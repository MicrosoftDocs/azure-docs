...
> [!Note]
> You can create a SQL Managed Instance only in virtual networks created through the Azure Resource Manager deployment model. Azure virtual networks created through the classic deployment model are not supported. Calculate subnet size by following the guidelines in the [Determine the size of subnet for SQL Managed Instances](vnet-subnet-determine-size.md) article. You can't resize the subnet after you deploy the resources inside.
>
...
- For a tutorial that shows how to create a virtual network, create a SQL Managed Instance, and restore a database from a database backup, see [Create an Azure SQL Managed Instance](instance-create-quickstart.md).
