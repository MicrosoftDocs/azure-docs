...
| **TDE-encrypted backup performance improvements** | It's now possible to set the PITR backup retention period, and automated compression of backups encrypted with transparent data encryption (TDE) are now 30 percent more efficient in consuming backup storage space, saving costs for the end user. |
| 
| 
