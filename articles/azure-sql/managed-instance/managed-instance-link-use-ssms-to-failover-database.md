...
Use the **Failover database to Managed Instance** wizard in SSMS to fail over your database from SQL Server to SQL Managed Instance. The wizard takes you through failing over your database, breaking the link between the two instances in the process. 
...
3. On the **Log in to Azure** page, select **Sign-in** to provide your credentials and sign in to your Azure account. Select the subscription that's hosting your SQL Managed Instance from the drop-down, and then select **Next**. 
...
4. On the **Failover Type** page, choose the type of failover you're performing. Check the box to confirm that you've stopped the workload for a planned failover, or you understand that you might lose data for a forced failover. Select **Next**. 

   :::image type="content" source="./media/managed-instance-link-use-ssms-to-failover-database/link-failover-failover-type.png" alt-text="Screenshot that shows Failover Type page.":::
...
You can validate this by reviewing the database on SQL Server: 
