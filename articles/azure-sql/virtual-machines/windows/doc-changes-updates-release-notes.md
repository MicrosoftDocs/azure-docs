...
| Changes | Details |
| --- | --- |
| **DNN for AG** | You can now configure a [distributed network name (DNN) listener (Preview)](availability-group-distributed-network-name-dnn-listener-configure.md) on SQL Server 2019 CU8 and later to replace the traditional [VNN listener](availability-group-overview.md#connectivity), negating the need for an Azure Load Balancer.   | 
