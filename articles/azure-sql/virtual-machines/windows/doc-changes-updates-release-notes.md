...
| **Bulk resource provider registration** | You can now [bulk register](sql-resource-provider-bulk-register.md) SQL virtual machines with the resource provider. | 
...
| **Changes to SQL VM resource provider** | You can [register your SQL Server VM with the SQL VM resource provider](sql-resource-provider-register-with.md) by using the new SQL IaaS modes. This capability includes [Windows Server 2008](sql-resource-provider-register-with.md#management-modes) images.|
...
| **Automatic registration to the SQL VM resource provider** | SQL Server VMs deployed after this month are automatically registered with the new SQL Server resource provider. SQL Server VMs deployed before this month still need to be manually registered. For more information, see [Register a SQL Server virtual machine in Azure with the SQL VM resource provider](sql-resource-provider-register-with.md).|
|**New SQL VM resource provider** |  A new resource provider (Microsoft.SqlVirtualMachine) provides better management of your SQL Server VMs. For more information on registering your VMs, see [Register a SQL Server virtual machine in Azure with the SQL VM resource provider](sql-resource-provider-register-with.md). |
