...
To use more than one availability group, add an additional IP address to the load balancer. Each IP address requires its own load-balancing rule, probe port, and front port. Ensure to add only the primary IP address of VM to back-end pool of load balancer as [secondary IP address of VM does not support floating IP](/azure/load-balancer/load-balancer-floating-ip).
