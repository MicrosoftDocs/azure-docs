...
![Availability Group](./media/tutorial-manually-configure-availability-group/00-EndstateSampleNoELB.png)
...
|----- |----- |----- |
|![Square](./media/tutorial-manually-configure-availability-group/square.png) | Two SQL Server instances | - In an Azure availability set <br/> - In a single domain <br/> - With Failover Clustering feature installed |
|![Square](./media/tutorial-manually-configure-availability-group/square.png)| Windows Server | File share for cluster witness |  
|![Square](./media/tutorial-manually-configure-availability-group/square.png)|SQL Server service account | Domain account |
|![Square](./media/tutorial-manually-configure-availability-group/square.png)|SQL Server Agent service account | Domain account |  
|![Square](./media/tutorial-manually-configure-availability-group/square.png)|Firewall ports open | - SQL Server: **1433** for default instance <br/> - Database mirroring endpoint: **5022** or any available port <br/> - Availability group load balancer IP address health probe: **59999** or any available port <br/> - Cluster core load balancer IP address health probe: **58888** or any available port |
|![Square](./media/tutorial-manually-configure-availability-group/square.png)|Add Failover Clustering Feature | Both SQL Server instances require this feature |
|![Square](./media/tutorial-manually-configure-availability-group/square.png)|Installation domain account | - Local administrator on each SQL Server <br/> - Member of SQL Server sysadmin fixed server role for each instance of SQL Server  |


Before you begin the tutorial, you need to [Complete prerequisites for creating Always On Availability Groups in Azure Virtual Machines](tutorial-prerequisites-manually-configure-availability-group.md). If these prerequisites are completed already, you can jump to [Create Cluster](#CreateCluster).

  >[!NOTE]
  > Many of the steps provided in this tutorial can now be automated with [Azure SQL VM CLI](configure-availability-group-az-cli.md) and [Azure Quickstart Templates](configure-availability-group-quickstart-template.md).
...
1. RDP to the first SQL Server using a domain account that is an administrator on both SQL Servers and the witness server.
   ...
   >[!TIP]
   >If you followed the [prerequisites document](tutorial-prerequisites-manually-configure-availability-group.md), you created an account called **CORP\Install**. Use this account.
...
3. In the left pane, right-click **Failover Cluster Manager**, and then click **Create a Cluster**.
   ![Create Cluster](./media/tutorial-manually-configure-availability-group/40-createcluster.png)
...
2. Right-click the failed **IP Address** resource, and then click **Properties**.
   ...
   ![Cluster Properties](./media/tutorial-manually-configure-availability-group/42_IPProperties.png)
...
1. In the browser tree, right-click the cluster and click **Add Node**.
    ...
    ![Add Node to the Cluster](./media/tutorial-manually-configure-availability-group/44-addnode.png)
...
8. In the **Confirmation** page if you are using Storage Spaces, clear the checkbox labeled **Add all eligible storage to the cluster.**
   ...
   ![Add Node Confirmation](./media/tutorial-manually-configure-availability-group/46-addnodeconfirmation.png)
...
1. Right-click **Shares**, and click **New Share...**.
   ...
   ![New Share](./media/tutorial-manually-configure-availability-group/48-newshare.png)
...
1. Make sure that the account used to create the cluster has full control.
   ...
   ![New Share](./media/tutorial-manually-configure-availability-group/50-filesharepermissions.png)
...
1. In **Failover Cluster Manager**, right-click the cluster, point to **More Actions**, and click **Configure Cluster Quorum Settings...**.
   ...
   ![New Share](./media/tutorial-manually-configure-availability-group/52-configurequorum.png)
...
3. Click the **AlwaysOn High Availability** tab, then select **Enable AlwaysOn Availability Groups**, as follows:
    ...
    ![Enable AlwaysOn Availability Groups](./media/tutorial-manually-configure-availability-group/54-enableAlwaysOn.png)
...
1. Make sure that the SQL Server and SQL Server Agent service accounts for both servers have full control.
   ...
   ![New Share](./media/tutorial-manually-configure-availability-group/68-backupsharepermission.png)
...
1. On remote desktop session to the first SQL Server. In **Object Explorer** in SSMS, right-click **AlwaysOn High Availability** and click **New Availability Group Wizard**.
    ...
    ![Launch New Availability Group Wizard](./media/tutorial-manually-configure-availability-group/56-newagwiz.png)
...
2. In the **Introduction** page, click **Next**. In the **Specify Availability Group Name** page, type a name for the Availability Group, for example **AG1**, in **Availability group name**. Click **Next**.
    ...
    ![New AG Wizard, Specify AG Name](./media/tutorial-manually-configure-availability-group/58-newagname.png)
...
3. In the **Select Databases** page, select your database and click **Next**.
   ...
   ![New AG Wizard, Select Databases](./media/tutorial-manually-configure-availability-group/60-newagselectdatabase.png)
4. In the **Specify Replicas** page, click **Add Replica**.

   ![New AG Wizard, Specify Replicas](./media/tutorial-manually-configure-availability-group/62-newagaddreplica.png)
5. The **Connect to Server** dialog pops up. Type the name of the second server in **Server name**. Click **Connect**.
   ...
   ![New AG Wizard, Specify Replicas (Complete)](./media/tutorial-manually-configure-availability-group/64-newagreplica.png)

6. Click **Endpoints** to see the database mirroring endpoint for this Availability Group. Use the same port that you used when you set the [firewall rule for database mirroring endpoints](tutorial-prerequisites-manually-configure-availability-group.md#endpoint-firewall).

    ![New AG Wizard, Select Initial Data Synchronization](./media/tutorial-manually-configure-availability-group/66-endpoint.png)
...
8. In the **Select Initial Data Synchronization** page, select **Full** and specify a shared network location. For the location, use the [backup share that you created](#backupshare). In the example it was, **\\\\\<First SQL Server\>\Backup\\**. Click **Next**.
   ...
   >Full synchronization takes a full backup of the database on the first instance of SQL Server and restores it to the second instance. For large databases, full synchronization is not recommended because it may take a long time. You can reduce this time by manually taking a backup of the database and restoring it with `NO RECOVERY`. If the database is already restored with `NO RECOVERY` on the second SQL Server before configuring the Availability Group, choose **Join only**. If you want to take the backup after configuring the Availability Group, choose **Skip initial data synchronization**.
    ...
    ![New AG Wizard, Select Initial Data Synchronization](./media/tutorial-manually-configure-availability-group/70-datasynchronization.png)

9. In the **Validation** page, click **Next**. This page should look similar to the following image:

    ![New AG Wizard, Validation](./media/tutorial-manually-configure-availability-group/72-validation.png)
...
10. In the **Summary** page, click **Finish**, then wait while the wizard configures the new Availability Group. In the **Progress** page, you can click **More details** to view the detailed progress. Once the wizard is finished, inspect the **Results** page to verify that the Availability Group is successfully created.
     ...
     ![New AG Wizard, Results](./media/tutorial-manually-configure-availability-group/74-results.png)
...
1. In **Object Explorer**, expand **AlwaysOn High Availability**, then expand **Availability Groups**. You should now see the new Availability Group in this container. Right-click the Availability Group and click **Show Dashboard**.
   ...
   ![Show AG Dashboard](./media/tutorial-manually-configure-availability-group/76-showdashboard.png)

   Your **AlwaysOn Dashboard** should look similar to this.

   ![AG Dashboard](./media/tutorial-manually-configure-availability-group/78-agdashboard.png)
...
2. In **Failover Cluster Manager**, click your cluster. Select **Roles**. The Availability Group name you used is a role on the cluster. That Availability Group does not have an IP address for client connections, because you did not configure a listener. You will configure the listener after you create an Azure load balancer.
   ...
   ![AG in Failover Cluster Manager](./media/tutorial-manually-configure-availability-group/80-clustermanager.png)
...
1. Search for **Load Balancer**. Choose the load balancer published by Microsoft.
   ...
   ![AG in Failover Cluster Manager](./media/tutorial-manually-configure-availability-group/82-azureloadbalancer.png)
...
1. Configure the following parameters for the load balancer.
   ...
   ![Create Load Balancer](./media/tutorial-manually-configure-availability-group/84-createloadbalancer.png)
...
1. In the Azure portal, go to your availability group. You might need to refresh the view to see the newly created load balancer.
   ...
   ![Find Load Balancer in Resource Group](./media/tutorial-manually-configure-availability-group/86-findloadbalancer.png)
...
> [!NOTE]
> This tutorial shows how to create a single listener - with one ILB IP address. To create one or more listeners using one or more IP addresses, see [Create Availability Group listener and load balancer | Azure](configure-availability-group-listener-powershell.md?toc=%2fazure%2fvirtual-machines%2fwindows%2ftoc.json).
>
>
...
- [Add an IP address to a load balancer for a second availability group](configure-availability-group-listener-powershell.md#Add-IP).
