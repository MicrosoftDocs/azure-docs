...
Filestream isn't supported for a failover cluster with a premium file share. To use filestream, deploy your cluster by using [Storage Spaces Direct](manually-configure-failover-cluster-instance-storage-spaces-direct.md).
...
1. [Create an Azure availability set](../../../virtual-machines/windows/tutorial-availability-sets.md).
...
1. If you're using one of the SQL Server-based virtual machine images, remove the SQL Server instance.
   ...
   1. Remove all features under **Database Engine Services**. Don't remove **Shared Features**. You'll see something like the following screenshot:
        ...
        ![Select Features](./media/manually-configure-failover-cluster-instance-premium-file-share/03-remove-features.png)
...
1. [Add the virtual machines to your pre-existing domain](tutorial-prerequisites-manually-configure-availability-group.md#joinDomain).
...
1. Select the drive letter you want to use from the drop-down list and then copy both code blocks to Notepad.
   ...
   :::image type="content" source="media/manually-configure-failover-cluster-instance-premium-file-share/premium-file-storage-commands.png" alt-text="Copy both PowerShell commands from the file share connect portal":::
...
1. [Add Failover Clustering to each virtual machine](tutorial-prerequisites-manually-configure-availability-group.md#add-failover-clustering-features-to-both-sql-server-vms).
...
1. Under **Test Selection**, select all tests except for **Storage** and **Storage Spaces Direct**, as shown here:
   ...
   :::image type="content" source="media/manually-configure-failover-cluster-instance-premium-file-share/cluster-validation.png" alt-text="Select cluster validation tests":::
...
For more information, see [Create and configure an Azure load balancer](tutorial-manually-configure-availability-group.md#configure-internal-load-balancer).
...
1. Set up the load balancer by using the following values:
   ...
   ![Set up the load balancer](./media/manually-configure-failover-cluster-instance-premium-file-share/30-load-balancer-create.png)
