...
Use a single NIC per server (cluster node). Azure networking has physical redundancy, which makes additional NICs unnecessary on an Azure virtual machine guest cluster. The cluster validation report will warn you that the nodes are reachable only on a single network. You can ignore this warning on Azure virtual machine guest failover clusters. Adding a second NIC doesn't improve performance in Azure VMs as the network bandwidth limit for a particular VM size will be shared across the NICs. Hence, there is no need to add additional NIC in Azure VMs to improve SQL Always On Availability Group performance. 
