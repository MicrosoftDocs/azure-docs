...
The following image shows a common deployment of an availability group on Azure virtual machines:
   ...
   ![Availability Group](./media/manually-configure-availability-group-multiple-regions/00-availability-group-basic.png)
...
This architecture is vulnerable to downtime if the Azure region becomes inaccessible. To overcome this vulnerability, add a replica in a different Azure region. The following diagram shows how the new architecture would look:
   ...
   ![Availability Group DR](./media/manually-configure-availability-group-multiple-regions/00-availability-group-basic-dr.png)
...
The following diagram shows how the networks communicate between data centers.
   ...
   ![Availability Group](./media/manually-configure-availability-group-multiple-regions/01-vpngateway-example.png)
...
1. [Create an Azure load balancer in the network on the new region](tutorial-manually-configure-availability-group.md#configure-internal-load-balancer).
...
1. [Add Failover Clustering feature to the new SQL Server](tutorial-prerequisites-manually-configure-availability-group.md#add-failover-clustering-features-to-both-sql-server-vms).

1. [Join the new SQL Server to the domain](tutorial-prerequisites-manually-configure-availability-group.md#joinDomain).

1. [Set the new SQL Server service account to use a domain account](tutorial-prerequisites-manually-configure-availability-group.md#setServiceAccount).

1. [Add the new SQL Server to the Windows Server Failover Cluster](tutorial-manually-configure-availability-group.md#addNode).
...
1. Add an IP address resource to the cluster.
   ...
   ![Cluster properties](./media/manually-configure-availability-group-multiple-regions/cluster-name-properties.png)
   ...
   ![Add cluster IP](./media/manually-configure-availability-group-multiple-regions/add-cluster-ip-address.png)
...
1. Add the IP address as a dependency for the core cluster name.
   ...
   ![Cluster properties](./media/manually-configure-availability-group-multiple-regions/cluster-ip-dependencies.png)
...
1. Add an IP address resource to the availability group role in the cluster. 
   ...
   ![Create IP Address](./media/manually-configure-availability-group-multiple-regions/20-add-ip-resource.png)
...
1. Add the IP address resource as a dependency for the listener client access point (network name) cluster.
   ...
   ![Availability Group](./media/manually-configure-availability-group-multiple-regions/50-configure-dependency-multiple-ip.png)
...
1. [Set the cluster parameters in PowerShell](tutorial-manually-configure-availability-group.md#setparam).
...
1. [Open firewall ports on the new SQL Server](tutorial-prerequisites-manually-configure-availability-group.md#endpoint-firewall).
...
* [Azure Load Balancers](tutorial-manually-configure-availability-group.md#configure-internal-load-balancer)
