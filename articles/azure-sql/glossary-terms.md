...
|Server entity| Logical server | A [logical server](database/logical-servers.md) is a construct that acts as a central administrative point for a collection of databases in Azure SQL Database. All databases managed by a server are created in the same region as the server. There are no guarantees regarding location of the databases in relation to the server that manages them. There is no instance-level access or instance features for a server. |
