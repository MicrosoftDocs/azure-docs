...
|**ClrAssemblies**|**Title: SQL CLR assemblies are not supported in Azure SQL Database** </br></br> More information: []()  |         |
|**BulkInsert**|**Title: BULK INSERT with non Azure blob data source is not supported in Azure SQL Database.** </br></br> More information: []()  |         |
|**CryptographicProvider**|**Title: A use of CREATE CRYPTOGRAPHIC PROVIDER or ALTER CRYPTOGRAPHIC PROVIDER was found, which is not supported in Azure SQL Database** </br></br> More information: []()  |         |
|**MSDTCTransactSQL**|**Title: BEGIN DISTRIBUTED TRANSACTION is not supported in Azure SQL Database.** </br></br> More information: []()  |         |
|**ComputeClause**|**Title: COMPUTE clause is discontinued and has been removed.** </br></br> More information: []()  |         |
|**DatabasePrincipalAlias**|**Title: SYS.DATABASE_PRINCIPAL_ALIASES is discontinued and has been removed.** </br></br> More information: []()  |         |
|**DisableDefCNSTCHK**|**Title: SET option DISABLE_DEF_CNST_CHK is  discontinued and has been removed.** </br></br> More information: []()  |         |
|**FastFirstRowHint**|**Title: FASTFIRSTROW query hint is discontinued and has been removed.** </br></br> More information: []()  |         |
|**NextColumn**|**Title: Tables and Columns named NEXT will lead to an error In Azure SQL Database.** </br></br> More information: []()  |         |
|**NonANSILeftOuterJoinSyntax**|**Title: Non ANSI style left outer join is discontinued and has been removed.** </br></br> More information: []()  |         |
|**NonANSIRightOuterJoinSyntax**|**Title: Non ANSI style right outer join is discontinued and has been removed.** </br></br> More information: []()  |         |
|**RAISERROR**|**Title: Legacy style RAISERROR calls should be replaced with modern equivalents.** </br></br> More information: []()  |         |
|**DatabaseCompatibility**|**Title: Azure SQL Database doesn’t support compatibility levels below 100.** </br></br> More information: []()  |         |



|Description  |Recommendation  |
|---------|---------|---------|
| **Title: Azure SQL Database does not support database size greater than 100 TB.** </br></br> The size of the database is greater than the maximum supported size of 100 TB. More information: [vCore resource limits](../../database/resource-limits-vcore-single-databases.md) </br> </br> **RuleID: SQLDBDatabaseSize** | Evaluate if the data can be archived or compressed or sharded into multiple database(s).|
|**Title: xp_cmdshell is not supported in Azure SQL Database.** </br></br> needs to be rewritten </br> </br> **RuleID: XpCmdshell**  | Review impacted objects section to see all objects using xp_cmdshell and evaluate if the reference to xp_cmdshell or the impacted object can be removed.Also consider exploring Azure Automation that delivers cloud based automation and configuration service.  |
|**Title: Change Data Capture (CDC) is not supported in Azure SQL Database** </br></br> needs to be rewritten. More information: [Enable Azure SQL change tracking](https://social.technet.microsoft.com/wiki/contents/articles/2976.azure-sql-how-to-enable-change-tracking.aspx) </br> </br> **RuleID: CDC** |Change Data Capture (CDC) is not supported in Azure SQL Database. Evaluate if Change Tracking can be used instead or consider migrating to Azure SQL Managed Instance.|
|**Title: Cross-database queries are not supported in Azure SQL Database** </br></br> Some selected databases on this server use cross-database queries, which are not supported in Azure SQL Database. More information: [Check Azure SQL Database elastic database query (Preview)](../../database/elastic-query-overview.md) </br> </br> **RuleID: CrossDataseReferences** |Azure SQL Database does not support cross-database queries. The following actions are recommended: Consider migrating to Azure SQL Managed Instance, Consider migrating the dependent database(s) to Azure SQL Database and use 'Elastic Database Query' (preview) functionality to query across Azure SQL databases, Consider moving the dependent datasets from other databases into the database that is being migrated.|
|**Title: Filestream is not supported in Azure SQL Database** </br></br> The Filestream feature, which allows you to store unstructured data such as text documents, images, and videos in NTFS file system, is not supported in Azure SQL Database. More information: [Streaming blobs to and from Azure SQL blog](https://azure.microsoft.com/en-us/blog/streaming-blobs-to-and-from-sql-azure/) </br> </br> **RuleID: FileStream** | Consider migrating to SQL Server on Azure Virtual machines or Upload the unstructured files to Azure Blob storage and store metadata related to these files (name, type, URL location, storage key etc.) in Azure SQL Database. You may have to re-engineer your application to enable streaming blobs to and from Azure SQL Database.    |
|**Title: Linked server functionality is not supported in Azure SQL Database** </br></br> More information: [Check Azure SQL Database elastic query (Preview)](../../database/elastic-query-overview.md) </br> </br> **RuleID: LinkedServer** |Azure SQL Database does not support linked server functionality. The following actions are recommended to eliminate the need for linked servers: 1. Identify the dependent datasets from remote SQL servers and consider moving these into the database being migrated. 2. Migrate the dependent database(s) to Azure and use 'Elastic Database Query' (preview) functionality to query across databases in Azure SQL Database. |
|**Title: Service Broker feature is not supported in Azure SQL Database** </br></br> SQL Server Service Broker provides native support for messaging and queuing applications in the SQL Server Database Engine. </br> </br> **RuleID: ServiceBroker**  |Service Broker feature is not supported in Azure SQL Database. Consider migrating to Azure SQL Managed Instance that supports service broker within the same instance.  |
|**Title: OpenRowSet used in bulk operation with non blob data source is not supported in Azure SQL Database.** </br></br> This method is an alternative to accessing tables in a linked server and is a one-time, ad hoc method of connecting and accessing remote data by using OLE DB.)  </br> </br> **RuleID: OpenRowsetWithNonBlobDataSourceBulk** | Azure SQL Database supports OPENROWSET only to import from Azure blob storage.|
| **Title: OpenRowSet with SQL or non SQL provider is not supported in Azure SQL Database.** </br></br> This method is an alternative to accessing tables in a linked server and is a one-time, ad hoc method of connecting and accessing remote data by using OLE DB. </br> </br> **RuleID: OpenRowsetWithSQLAndNonSQLProvider** | Azure SQL Database supports OPENROWSET only to import from Azure blob storage. |
