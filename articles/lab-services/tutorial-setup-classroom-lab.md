...
## Set up the template 
A template in a lab is the template from which all users’ virtual machines are created. Set up the template virtual machine so that it is configured with exactly what you need to provide to the lab users. You can provide a name and description of the template that the lab users see, and set the visibility to “Public” to make instances of the template VM available to your lab users. 
...
Once you set the visibility of a template to **Public**, the lab creates virtual machines using the template. All virtual machines have the same configuration as the template. The number of VMs created in this process equals the maximum number of users allowed into the lab, which you can set in usage policy. 
