...
After they have set up a lab plan, administrators or educators create the labs that are associated with the lab plan. This article provides a high-level overview of the Azure resources that are involved and the guidance for creating them.
...
- Lab plans, a compute gallery, and image versions and are hosted within your subscription.
...
- You can have your lab plan and the compute gallery in the same resource group. In this diagram, they are in different resource groups.
...
- You can grant users in the subscription's Azure Active Directory (Azure AD) tenant the ability to manage Azure Lab Services lab plans and labs. You can add a user as a lab plan owner, lab plan contributor, lab creator, or lab owner. For more information about built-in RBAC roles, see [Manage identity](#rbac-roles).
...
We recommend that you invest time up front to plan the structure of your resource groups.  It is *not* possible to change a lab plan or compute gallery resource group once it is created. If you need to change the resource group for these resources, you will need to delete and re-create them.

## Lab plan

A lab plan set of configurations that influence the creation of a lab.  A lab plan can be associated with zero or more labs. When you are getting started with Azure Lab Services, it is most common to have a single lab plan. As your lab usage scales up, you can choose to create more lab plans later.
...
- Create a new set of labs for each semester, quarter, or other academic system you are using. For classes that need to use the same image, you should use a [compute gallery](#azure-compute-gallery). This way, you can reuse images across labs and academic periods.

When you are determining how to structure your labs, consider the following points:
...
- **The usage quota is set at the lab level and applies to all users within the lab**
    ...
    To set different quotas for users, you must create separate labs. However, it is possible to add more hours to specific users after you have set the quota.
...
Instructors can publish an image version from the compute gallery when they create a new lab. Although the gallery stores multiple versions of an image, educators can select only the most recent version during lab creation.  The most recent version is chosen based on the highest value of MajorVersion, then MinorVersion, then Patch.  For more information about versioning, see [Image versions](../virtual-machines/shared-image-galleries.md#image-versions).

The compute gallery is an optional resource that you might not need immediately if you are starting with only a few labs. However, a compute gallery offers many benefits that are helpful as you scale up to more labs:

- **You can save and manage versions of a template VM image**

    It is useful to create a custom image or make changes (software, configuration, and so on) to an image from the Azure Marketplace gallery.  For example, it is common for educators to require different software or tooling be installed. Rather than requiring students to manually install these prerequisites on their own, different versions of the template VM image can be exported to the compute gallery. You can then use these image versions when you create new labs.

- **You can share and reuse template VM images across labs**

    You can save and reuse an image so that you do not have to configure it from scratch each time that you create a new lab. For example, if multiple classes need to use the same image, you can create it once and export it to the compute gallery so that it can be shared across labs.
...
In the proceeding table, we used some terms and tokens in the suggested name patterns.  Let us go over those terms in a little more detail.
...
- **Lab Contributor**
  ...
  A key difference between the lab Owner and Contributor roles is that only an Owner can grant other users access to manage a lab. A Contributor *cannot* grant other users access to manage a lab.

- **Lab Operator**

  When applied to a resource group or a lab, this role enables the user to have limited ability to manage existing labs. This role will not give the user the ability to create new labs. In an existing lab, the user can manage users, adjust individual users’ quota, manage schedules, and start/stop VMs. The user account will be able to publish a lab. The user will not have the ability to change lab capacity or change quota at the lab level. The user will not be able to change the template title or description.

- **Lab Assistant**

  When applied to a resource group or a lab, enables the user to view an existing lab and can only perform actions on the lab VMs (reset, start, stop, connect) and send invites to the lab. The user will not have the ability to change a lab, create a lab, publish a lab, change lab capacity, or manage quota and schedules. This user cannot adjust individual quota.
...
When you are assigning roles, it helps to follow these tips:
...
- To give educators the ability to manage specific labs, but *not* the ability to create new labs, assign them either the Owner or Contributor role for each lab that they will manage. For example, you might want to allow a professor and a teaching assistant to co-own a lab.

## Content filtering

Your school may need to do content filtering to prevent students from accessing inappropriate websites.  For example, to comply with the [Children's Internet Protection Act (CIPA)](https://www.fcc.gov/consumers/guides/childrens-internet-protection-act).  Lab Services does not offer built-in support for content filtering.
...
By default, Azure Lab Services hosts each lab's virtual network within a Microsoft-managed Azure subscription.  You will need to use [advanced networking](how-to-connect-vnet-injection.md) in the lab plan.  Make sure to check known limitations of VNet injection before proceeding.
