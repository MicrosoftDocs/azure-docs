...
Azure DevTest Labs is a service that enables developers on a team to efficiently self-service virtual machines and/or PaaS resources that they need for developing, testing, training, and demos etc. without waiting for constant approvals on the tools they need. The lab already consists of pre-configured bases or Resource Manager templates with all the necessary tools and software installed on them that developers can use to create environments. Thus, the time required for a developer to create their environments is reduced to a few minutes as opposed to hours or days.  

You can test the latest version of your application by performing the following tasks:
...
- Choose from a curated list of virtual machine bases that are configured, approved, and authorized by the team lead or central IT.
...
- Install artifacts that are extensions deployed on the VM after it is provisioned.
...
Along with a self-service model for developers on a team, the service enables central IT to control wastes, optimize costs on resources and stay within budgets by doing the following tasks: 
...
- Tracking costs and setting targets on the lab, getting notified on high projected costs for the lab to be able to take necessary actions and so on. 
...
- Eenables you to track costs on virtual machines and PaaS resources spun up inside your lab so that you can stay within your predefined budget. 
