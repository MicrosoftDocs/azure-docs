...
- All VMs in the lab and resource groups associated with these VMs are automatically deleted. When you create a VM in a lab, the service creates a seperate resource group for the VM with resources (disk, network interface, public IP address, etc.) needed for running the VM. Howerver, if you manually create any additional resources in these resource groups, the DevTest Lab service does not delete those resources and the resource group. 
...
-	Availability set, load balancer, public IP address in the VM resource group.

Virtual machine, network interface, and disk were deleted. 
