...
Nested virtualization enables you to create a lab in Azure Lab Services that contains a multi-VM environment. You can prepare a lab template for a multi-VM environment. Users don't need to enable nested virtualization on their lab VM and install the nested VMs inside it. When you publish the lab, each lab user has a lab VM that already contains the nested virtual machines.
...
15. Select **Install**.
16. Wait for the **Installation progress** page to indicate that the Hyper-V role is complete. The machine might restart in the middle of the installation.
17. Select **Close**.
...
After you install all the necessary roles, you can create the NAT network. The creation process involves creating a switch and the NAT network, itself. 
...
6. In the **Internet Protocol Version 4 (TCP/IPv4) Properties** dialog, enter these values.
