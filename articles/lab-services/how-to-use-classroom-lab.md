...
- If using Windows, save the **RDP** file. Then open the RDP file to connect to the virtual machine. Use the **user name** and **password** you get from your educator to sign in to the machine.
- If using a Mac, see [Connect to a VM using RDP on a Mac](connect-virtual-machine-mac-remote-desktop.md).  
- If using a Chromebook, see [Connect to a VM using RDP on a Chromebook](connect-virtual-machine-chromebook-remote-desktop.md).
...
1. Once the VM is running, select **Connect**, which will pop up a dialog box that provides the SSH command string.  The command will look like the following sample:
