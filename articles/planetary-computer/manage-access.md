...
1. Select on "Add role Assignment" and then select "GeoCatalog Administrator" under "Job function roles"
    ...
    ![RBAC role assigment screenshot](media/RBAC_role_assigment.png)

1. Select on Next button and then select radio button of "User, group, or service principal"

    ![RBAC role assigment members section screenshot](media/RBAC_members_section.png)
...
1. Select on Next button and then select radio button of "Managed identity"

    ![RBAC choose managed identity screenshot](media/RBAC_managed_id.png)
...
1. Select on "Select members" and select the subscription and the User-assigned managed identity on the "Select managed identities" pane on the right hand side.
    ...
    ![RBAC selected members screenshot](media/RBAC_selected_members.png)
...
> [!NOTE]
> If your application has multiple managed identities assigned to it, you need to explicitly pass the right one: `azure.identity.DefaultAzureCredential(managed_identity_client_id=<client_id>)`. Alternatively, you may configure environment variables of your application on Azure portal to add `"AZURE_CLIENT_ID"` with the right managed identity client id.

> [!NOTE]
> You may add `.default` or `user_impersonation` as scope to `credential.get_token()` based on your expected user authentication behavior.
...
See [Access tokens in the Microsoft identity platform](/entra/identity-platform/access-tokens) for more information of access tokens. Note that when you acquire access tokens via calling DefaultAzureCredentials(), the acquired tokens are [cached](https://aka.ms/azsdk/net/identity/token-cache) by the credential instance. Token lifetime and refreshing is handled automatically. You can pass DefaultAzureCredential instance around and invoke `GetToken()` or `GetTokenAsync()` right before you need a token so that you always get a token that isn't expired. If you need to maintain a long open session, you may handle token expiration in an error handler to catch the exception and acquire a new token.  
