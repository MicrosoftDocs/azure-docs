---
title: Configure Application Authentication for Microsoft Planetary Computer Pro 
...
This article provides step-by-step guidance for developers and administrators to set up secure application authentication and access to Microsoft Planetary Computer Pro (MPC Pro). By applying Microsoft Entra ID and managed identities, applications can authenticate seamlessly without managing credentials, ensuring secure interaction with MPC Pro resources. Whether your application runs on Azure or other environments, this guide outlines the necessary configurations, including role-based access control (RBAC) and token acquisition, to enable secure access.

> [!NOTE]
> For applications that use Azure AD B2C or Microsoft Entra External ID supporting features like social identity providers, the applications need to continue using these identity solutions to proxy authentication traffic since MPC Pro doesn't support non Microsoft Entra ID authentication today.  
...
In this access scenario, the application acts on its own with no user signed in as the default behavior. You can proceed to the section [MPC Pro RBAC configuration for applications]() to assign the appropriate roles to the application.
