...
You have many options for deploying your own content to a Windows Azure Web site.  For each of the following options, this topic provides a brief overview and links to more information.
...
[FTP](http://en.wikipedia.org/wiki/File_Transfer_Protocol) is the most commonly used publishing protocol. When you create a Windows Azure Web Site, you can create FTP credentials and use any standard FTP application, including a browser such as Internet Explorer, to deploy content to your site. There are many full-featured free tools that you can use, such as [FileZilla](https://filezilla-project.org/). 
...
[Visual Studio](http://www.visualstudio.com/en-us/downloads/download-visual-studio-vs.aspx) is an advanced IDE that can be used for developing web applications. It offers many more features than WebMatrix, which also means it is a much bigger install and can be more difficult to learn how to use. Like WebMatrix, it has built-in deployment features that use Web Deploy to automate related deployment tasks such as database deployment and connection string changes. In addition, it has built-in management tools for Windows Azure Web Sites. From within the Visual Studio IDE you can create, stop, start, and delete Windows Azure Web Sites, you can view logs as they are created in real-time, you can debug remotely, and much more. Visual Studio also is integrated with [source control systems](http://asp.net/aspnet/overview/developing-apps-with-windows-azure/building-real-world-cloud-apps-with-windows-azure/source-control) such as [Visual Studio Online](#vso), [Team Foundation Server](#tfs), and [Git repositories](#git).
...
Windows Azure provides a REST management API and several related APIs that make the REST API easier to use. One of these APIs consists of a collection of Windows PowerShell cmdlets.
...
You can use command line tools in Mac or Linux to access the Windows Azure REST management API. Tutorials specifically for deployment using the xpat-cli are not available yet, but for general documentation about these tools, see the following resource:
