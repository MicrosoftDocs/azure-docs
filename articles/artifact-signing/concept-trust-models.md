...
This article explains the concept of trust models, the primary trust models that Artifact Signing provides, and how to use them in a wide variety of signing scenarios that Artifact Signing supports.
...
We recommend using Public Trust to sign any artifact you intend to share publicly. The signer should be a validated identity with Artifact Signing. Apps signed with Artifact Signing's Public Trust model result in users enjoy a productive experience on Windows with modern security protection features enabled such as Smart App Control and SmartScreen.  

> [!NOTE]
> Artifact Signing includes options for "test" certificate profiles under the Public Trust collection, but the certificates are not publicly trusted. The Public Trust Test certificate profiles are intended to be used for inner-loop dev/test signing and should *not* be trusted.

## Private Trust model

Private Trust is the second trust model that's provided in Artifact Signing. It's for opt-in trust when signatures aren't broadly trusted across the ecosystem. The CA hierarchy that's used for Artifact Signing Private Trust resources isn't default-trusted in any root program and in Windows. Rather, it's designed to use in [App Control for Business (formerly Windows Defender Application Control, *WDAC*)](/windows/security/application-security/application-control/windows-defender-application-control/wdac) features, including:
