...
Below claims are generated by Azure Attestation and can be used by policy authors to define authorization rules in a custom policy. These are applicable for all attestation types.
...
Below parameters are defined by [IETF JWT](https://tools.ietf.org/html/rfc7519) and are included as claims in the attestation token by the service. These are applicable for all attestation types.
...
Below parameters are defined by the [IETF EAT](https://tools.ietf.org/html/draft-ietf-rats-eat-03#page-9) and are included as claims in the attestation token by the service:
- **"Nonce claim" (nonce)**
...
Below claims are generated by the service and can be used by policy authors to define authorization rules in a custom policy. These are applicable for SGX attestation only.
...
Below claims are generated and included in the attestation token by the service. These are applicable for SGX attestation only.
- **x-ms-sgx-is-debuggable**: A Boolean, which indicates whether or not the enclave has debugging enabled or not
- **x-ms-sgx-product-id**
- **x-ms-sgx-mrsigner**: hex encoded value of the “mrsigner” field of the quote
- **x-ms-sgx-mrenclave**: hex encoded value of the “mrenclave” field of the quote
