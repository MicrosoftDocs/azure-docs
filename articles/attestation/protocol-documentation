---
title: VBS attestation protocol documentation
description: VBS attestation protocol documentation
...
# VBS attestation protocol documentation

- The Enclave starts the protocol by requesting a challenge from the service (here shown as the Init message)
- The Attestation Service generates a random challenge and creates a session context which contains such challenge and a timestamp. The session context is encrypted and signed     using K-Context **ENC_SIGN(K-Context)[Timestamp || Challenge]**.  Context is opaque to the client
- The Enclave then:
...
- The Attestation Service receives the message from the client and:
...
- Client application can now request the Attestation Report from the Enclave. The report can then be parsed and trusted by relying parties as it is signed by the Attestation       Service
