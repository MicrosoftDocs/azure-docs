...
1. After you create the storage account, select **Access control (IAM)** in the left pane. Then assign the following roles or make sure they are already assigned.
...
1. From the left pane, select **Containers** and create a container.
...
In the following step, you'll configure your Azure Synapse workspace to use this storage account as the "primary" storage account and the container to store workspace data. The workspace stores data in Apache Spark tables. It also stores Spark application logs under a folder called **/synapse/workspacename**.
...
1. Assign the following roles or make sure they are already assigned. We use the same name for the workspace identity and the workspace name.
...
When you perform Spark activity in Azure Synapse, you specify a Spark pool to use. The pool informs Azure Synapse how many Spark resources to use. You only pay for the resources that you use. When you actively stop using the pool, the resources automatically time out and are recycled.
...
## Load NYC Taxi sample data into the SQL pool
...
## Load NYC Taxi sample data into the Spark database
...
1. Go to the **Data** hub, right-click **Databases** and select **Refresh**. You should see these databases:

    - **SQLDB** (SQL pool)
    - **nyctaxi** (Spark)

## Analyze NYC Taxi data using Spark and notebooks

1. Return to your notebook.
1. Create a new code cell, enter the following text, and run the cell to illustrate the NYC Taxi data we loaded into the **nyctaxi** Spark database.
...
## Customize visualization data with Spark and notebooks
...
1. Select **users (Primary)"**. You should see the folder called **NYCTaxi**. Inside you should see two folders called **PassengerCountStats.csv** and **PassengerCountStats.parquet**.
1. Open the **PassengerCountStats.parquet** folder, right-click the parquet file inside, and then select **new notebook**. It creates a notebook with a cell like this:
...
1. Open the downloaded **.pbids** file. This launches Power BI desktop and automatically connects it to **SQLDB1** in your Azure Synapse workspace.
...
1. Hover over **Power BI** and click the refresh the **Power BI reports** node.
1. Now, under **Power BI** you should see:
...
1. In this location, view the history of all activities taking place in the workspace and which ones are active now.
