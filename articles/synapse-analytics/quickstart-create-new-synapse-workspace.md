...
1. Navigate to the Synapse workspace listing by typing the service name into the search bar:
...
![Command to create new Synapse workspace highlighted.](media/quickstart-create-a-synapseworkspace-02.png).
1. Fill out the **Synapse workspace** form with the following information:
    ...
    ![Workspace provisioning flow - Basics tab.](media/quickstart-create-a-synapseworkspace-03.png).
...
1. The **Summary** tab will run the necessary validations to ensure that the workspace can be successfully created. Once the validation succeeds, press **Create** ![Workspace provisioning flow - confirmation tab.](media/quickstart-create-a-synapseworkspace-05.png).
1. Once the resource provisioning process completes successfully, you'll see an entry for the newly created workspace in the list of Synapse workspaces. ![Listing of Synapse workspaces showing the newly provisioned workspace.](media/quickstart-create-a-synapseworkspace-07.png).

## Clean up resources

Follow the steps below to delete the Synapse workspace.
> [!WARNING]
> Deleting a Synapse workspace will remove the analytics engines and the data stored in the database of the contained SQL pools and workspace metadata. It will no longer be possible to connect to the SQL endpoints, Apache Spark endpoints. All code artifacts will be deleted (queries, notebooks, job definitions and pipelines). 
>
> Deleting the workspace will **not** affect the data in the Data Lake Store Gen2 linked to the workspace.

If you want to delete the Synapse workspace, do the following:

1. Navigate to the Synapse workspace to delete..
1. Press **delete** on the command bar.
 ![Synapse workspace overview - delete command highlighted.](media/quickstart-create-a-synapseworkspace-10.png)
1. Confirm the deletion, and press **Delete** button.
 ![Synapse workspace overview - delete workspace confirmation dialog.](media/quickstart-create-a-synapseworkspace-11.png)
1. When the process completes successfully, the Synapse workspace will no longer be listed in the list of workspaces. 
