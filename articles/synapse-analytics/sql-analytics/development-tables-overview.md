---
title: Designing tables using the SQL Analytics resources | Microsoft Docs
...
# Designing tables using the SQL Analytics resources

This document includes key concepts for designing tables in the SQL pools and SQL on-demand resources within SQL Analytics. 

[SQL on-demand](on-demand.md) is a query service over the data in your data lake. It doesn't have local storage for data ingestion. Some topics described below don't apply to SQL on-demand. 

The table below lists the topics that are relevant to SQL pools vs. SQL on-demand:
...
If you're migrating multiple databases from an on-prem solution to SQL pools, the best practice is to migrate all of the fact, dimension, and integration tables to one SQL pool schema. For example, you could store all the tables in the [WideWorldImportersDW](/sql/sample/world-wide-importers/database-catalog-wwi-olap) sample data warehouse within one schema called wwi. 

To show the organization of the tables in SQL pools, you could use fact, dim, and int as prefixes to the table names. The table below shows some of the schema and table names for WideWorldImportersDW.  
...
### External table
[External tables](development-tables-external-tables.md) point to data located in Azure Storage blob or Azure Data Lake Store. 

In the SQL pools resource, when used in conjunction with the CREATE TABLE AS SELECT statement, selecting from an external table imports data into SQL pools. As such, external tables are useful for loading data. For a loading tutorial, see [Use PolyBase to load data from Azure blob storage](../../sql-data-warehouse/load-data-from-azure-blob-storage-using-polybase.md).
...
## Data types
SQL pools support the most commonly used data types. For a list of the supported data types, see [data types in CREATE TABLE reference](/sql/t-sql/statements/create-table-azure-sql-data-warehouse#DataTypes) in the CREATE TABLE statement. For more information on using data types, see [Data types](development-tables-data-types.md).

## Distributed tables
A fundamental feature of SQL pools is the way it can store and operate on tables across [distributions](../../sql-data-warehouse/massively-parallel-processing-mpp-architecture.md#distributions).  SQL pools support three methods for distributing data: 
...
You can also maintain the data through partition switching. Since the data in SQL pools is already distributed, too many partitions can slow query performance. For more information, see [Partitioning guidance](../../sql-data-warehouse/sql-data-warehouse-tables-partition.md).  
...
PRIMARY KEY is only supported when NONCLUSTERED and NOT ENFORCED are both used.  UNIQUE constraint is only supported when NOT ENFORCED is used.  For more information, see the [SQL pools Table Constraints](../../sql-data-warehouse/sql-data-warehouse-table-constraints.md)article.
...
If data is coming from multiple data stores, you can port the data into the data warehouse and store it in an integration table. Once data is in the integration table, you can use the power of SQL pools to implement transformation operations. Once the data is prepared, you can insert it into production tables.

## Unsupported table features
SQL pools support many, but not all, of the table features offered by other databases.  The following list shows some of the table features that aren't supported in SQL pools.
...
## Next steps
After creating the tables for your data warehouse, the next step is to load data into the table.  For a loading tutorial, see [Loading data into SQL pools](../../sql-data-warehouse/load-data-wideworldimportersdw.md#load-the-data-into-your-data-warehouse).