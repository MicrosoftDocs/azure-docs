...
This section contains sample queries you can use to query files in storage via Synapse Analytics SQL on-demand.
...
Starlight Query offers a T-SQL querying surface area, which is slightly enhanced/extended in some aspects to accommodate for experiences around querying of semi-structured and unstructured data, while some aspects of T-SQL language are not supported.

Following T-SQL aspects are supported:
...
- Creating and using [Views](https://docs.microsoft.com/en-us/sql/t-sql/statements/create-view-transact-sql?view=sql-server-2017). Check [Creating views](creating-and-using-views.md) section for some samples.
- Creating and using [Credentials](https://docs.microsoft.com/en-us/sql/t-sql/statements/create-credential-transact-sql?view=sql-server-2017)
- Metadata exploration and [Catalog Views](https://docs.microsoft.com/en-us/sql/relational-databases/system-catalog-views/catalog-views-transact-sql?view=sql-server-2017)

Not supported:
- DDL statements, with exception of Views and security
- DML statements

## Extensions 
In order to enable smooth experience for in place querying of data residing in files in Azure Storage, Starlight Query extended the existing [OPENROWSET](https://docs.microsoft.com/en-us/sql/t-sql/functions/openrowset-transact-sql?view=sql-server-2017) function by adding following capabilities:
