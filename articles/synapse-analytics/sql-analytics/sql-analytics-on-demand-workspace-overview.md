...
SQL Analytics on-demand endpoint is provided within every Azure Synapse workspace. You can create a workspace and start querying data instantly using tools you are familiar with. 
...
SQL Analytics on-demand has no local storage, only metadata objects are stored in databases. Therefore, T-SQL related to following concepts is not supported:
...
SQL Analytics on-demand offers mechanisms to secure access to your data.
...
SQL Analytics on-demand authentication refers to how users prove their identity when connecting to the endpoint. Two types of authentication are supported:
