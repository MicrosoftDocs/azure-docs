...
## SQL Analytics pool catalog views
...
## SQL Analytics pool dynamic management views (DMVs)
...
## SQL Server DMVs applicable to SQL Analytics pool
The following DMVs are applicable to SQL Analytics pool, but must be executed by connecting to the **master** database.
...
## SQL Server DMVs available in SQL Analytics pool
SQL Analytics pool exposes many of the SQL Server dynamic management views (DMVs). These views, when queried in SQL Analytics pool, are reporting the state of SQL Databases running on the distributions.

SQL Analytics pool and Analytics Platform System's Parallel Data Warehouse (PDW) use the same system views. Each DMV has a column called pdw_node_id, which is the identifier for the Compute node. 
...
## SQL Server 2016 PolyBase DMVs available in SQL Analytics pool
The following DMVs are applicable to SQL Analytics pool, but must be executed by connecting to the **master** database.
