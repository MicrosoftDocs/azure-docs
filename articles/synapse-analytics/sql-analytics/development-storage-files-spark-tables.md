...
The SQL on-demand can automatically synchronize metadata from Apache Spark pools within Synapse workspace. A SQL on-demand database will be created for each database existing in Apache Spark pools. For each Spark table based on Parquet or CSV, an external table is created in the SQL on-demand database. As such, you can shut down your Apache Spark pools and still query Spark tables from SQL on-demand.
...
Metadata synchronization is automatically configured for each Apache Spark pool provisioned in the Azure Synapse workspace. You can start querying Spark tables instantly.
