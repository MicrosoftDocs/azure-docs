...
- Gather the architecture and dataflow diagram(s) for the current analytical environment.
- Where are the data sources for your planned analytical workloads located (Azure, other cloud providers, or on-premises)?
- What is the total size of existing dataset(s) (historical and incremental)? What is the current rate of growth of your dataset(s)? What is the projected rate of growth of your dataset(s) for the next 2-5 years?
...
- Gather data on existing cloud environment. Is it a single-cloud provider or a multi-cloud provider?
- Gather plans on future cloud environment. Will it be a single-cloud provider or a multi-cloud provider?
...
- What is your current data load and update requirements (real-time, micro batch, hourly, daily, weekly, or monthly)?
- Describe the transformation requirements for each layer (big data, data lake, data warehouse)?
- What is the current programming approach to transforming the data (no-code, low-code, programming like SQL, Python, Scala, C#, or other)?
...
- Is there test data available to use during implementation development and testing?
...
- Do you use Azure ExpressRoute? What is its bandwidth?
- Do you have budget approval to provision the required Azure services?
- How do you provision and manage resource today (Azure Resource Manager (ARM) or Terraform)?
...
- Identify what tools you plan to use to perform activities like ingest, explore, prepare, and visualize the data.
- What applications are planned to interact with the analytical platform (Power BI, Excel, SQL Server Reporting Services, Tableau, or others)?
- Identify all data consumers.
- Identify data export and data sharing requirements.

## Azure Synapse services assessment

The Azure Synapse services assessment is concerned with the services within Azure Synapse analytics. Azure Synapse has the following components for compute and data movement:

- **Synapse SQL:** A distributed query system for T-SQL that enables data warehousing and data virtualization scenarios. It also extends T-SQL to address streaming and machine learning (ML) scenarios. Synapse SQL offers both *serverless* and *dedicated* resource models.
...
- **Data integration pipelines:** Azure Synapse contains the same data integration engine and experiences as Azure Data Factory (ADF). It allows you to create rich at-scale ETL pipelines without leaving Azure Synapse Analytics.
...
- Do you want to a build a logical warehouse on top of a data lake?
...
- For a migration workload, provide the make and model of your appliance for each environment? Include details of CPUs, GPUs, and memory.
...
- Identify all ETL, ELT, and data loading scenarios (batch windows, streaming, near real-time). Identify existing SLAs for each scenario and document the expected service level agreements (SLAs) in the new environment.
...
- Which tools are currently in place for data movement (ADF, SQL Server Integration Services (SSIS), robocopy, Informatica, SFTP, or others)?
- Are you planning to load real-time or near real-time data?

For dedicated SQL pool assessment, evaluate the following database points.

- What is the number of objects in each data warehouse (schemas, tables, views, stored procedures, functions)? Overall complexity: Is it a star, snowflake or other data structure?
...
For dedicated SQL pool assessment, evaluate the following workload characteristics.

- What is the estimated number of concurrent users or jobs accessing the data warehouse during peak hours?
- What is the estimated number of concurrent users or jobs accessing the data warehouse during off peak hours (normal hours)?
- Is there a period of time when there will be zero users or jobs?
...
- How complex will the most the commonly executed queries be?
...
- Identify the data consuming patterns and platforms:
    ...
    - What is the nature of data access (interactive, ad hoc, exports, or others)?
...
- Do you have requirements to integrate ML model scoring with SQL?
...
- **Data analysts** can [explore data and Spark external tables](../sql/develop-storage-files-spark-tables.md) created by data scientists or data engineers by using familiar T-SQL language or their favorite tools.
...
- Identify whether there are use cases to transform data in the data lake without first moving data out from the data lake.
...
- When you're migrating from an existing Spark/Hadoop workload:
    ...
    - If it's a migration from on-premises, has hardware depreciated or license expired? If not, when will depreciation or expiry happen?
    ...
    - What type of workload (big data, ML, or other)?
