...
To create a Delta Lake table, write out a DataFrame in the delta format. You can change the format from parquet, csv, json, and so on, to delta.

The code below creates a new Delta Lake table using the schema that was inferred from your DataFrame. For, the full set of options available when you create a new Delta Lake table.
...
Delta Lake supports several operations to modify tables using standard DataFrame APIs, support for modification is one of the most significant enhancements that the delta format adds. The example below runs a batch job to overwrite the data in the table.
...
Delta Lake provides programmatic APIs to conditional update, delete, and merge (commonly referred to as an upsert) data into tables. For more information on these operations, see Table Deletes, Updates, and Merges.
