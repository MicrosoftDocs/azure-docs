...
Apache Spark is a parallel processing framework that supports in-memory processing to boost the performance of big-data analytic applications. Apache Spark in Azure Synapse Analytics is one of Microsoft 's implementations of Apache Spark in the cloud. Synapse Analytics makes it easy to create and configure a Spark pool in Azure. Spark pools in Synapse Analytics are compatible with Azure Storage and Azure Data Lake Generation 2 Storage. So you can use Synapse Analytics Spark pools to process your data stored in Azure. 

![Spark: a unified framework](./media/apache-spark-overview/spark-overview.png)

## What is Apache Spark

Spark provides primitives for in-memory cluster computing. A Spark job can load and cache data into memory and query it repeatedly. In-memory computing is much faster than disk-based applications. Spark also integrates into the Scala programming language to let you manipulate distributed data sets like local collections. There's no need to structure everything as map and reduce operations.
...
| Integration with third-party IDEs | Synapse Analytics provides an IDE plugin that is useful to create and submit applications to an Synapse Analytics Spark pool. |
...
| Scalability | Synapse Analytics allows you to change the number of pool template nodes. Pool templates can enable Auto-Scale and pools will scale down as needed. Also, Spark pools can be shut down with no loss of data since all the data is stored in Azure Storage or Data Lake Storage. |
...
## Spark in Synapse Analytics use cases

Spark clusters in Synapse Analytics enable the following key scenarios:
