...
| Integration with third-party IDEs | Synapse Analytics provides an IDE plugin for Jetbrains IntelliJ that is useful to create and submit applications to an Synapse Analytics Spark pool. |
...
| Scalability | Apache Spark in Synapse Analytics pools can have Auto-Scale enabled, pools will scale up and down as needed. Also, Spark pools can be shut down with no loss of data since all the data is stored in Azure Storage or Data Lake Storage. |
