...
2. In this tutorial, we will use several different libraries to help us visualize the dataset. To do this, we will need to import the following libraries: 
...
1. To make development easier and less expensive, we will down sample the dataset. To do this, we will use the built-in Apache Spark sampling capability. In addition, both Seaborn and Matplotlib require a Pandas dataframe or Numpy array. To do this, we will use the ```toPandas()``` command to convert our dataframe.
...
2. First, we  want to understand the distribution of tips in our dataset. To do this, we will use Matplotlib to create a histogram showing the distribution of tip amount and count. Based on the distribution, we can see that tips are skewed towards amounts less than or equal to $10.
...
3. Next, we want to understand the relationship between the tips for a given trip and the day of the week. To do this, we will use Seaborn to create a boxplot summarizing the trends for each day of the week. Based on the results, we can see that <<>>
