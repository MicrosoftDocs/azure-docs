...
an Apache Spark pool is created in the Azure portal. It is the definition of an Apache Spark pool that, when instantiated, is used to create a Spark instance that processes data. When an Apache Spark pool is created, it exists only as metadata; no resources are consumed, running, or charged for. an Apache Spark pool has a series of properties that control the characteristics of a Spark instance; these characteristics include but are not limited to name, size, scaling behavior, time to live.

As there is no dollar or resource cost associated with creating Apache Spark pools, any number can be created with any number of different configurations. Permissions can also be applied to Apache Spark pools allowing users only to have access to some and not others.

A best practice is to create smaller Apache Spark pools that may be used for development and debugging and then larger ones for running production workloads.

You can read how to create an Apache Spark pool and see all their properties here [Get started with Apache Spark pools in Synapse Analytics](apache-spark-notebook-create-spark-use-sql.md#create-a-spark-pool)

## Spark instances

Spark instances are created when you connect to an Apache Spark pool, create a session, and run a job. As multiple users may have access to a single Apache Spark pool, a new Spark instance is created for each user that connects. When you submit a second job, then if there is capacity in the pool, the existing Spark instance also has capacity then the existing instance will process the job; if not and there is capacity at the pool level, then a new Spark instance will be created.

## Examples

### Example 1

- You create an Apache Spark pool called SP1; it has a fixed cluster size of 20 nodes.
...
- You create an Apache Spark pool call SP2; it has an autoscale enabled 10 – 20 nodes
- You submit a notebook job, J1 that uses 10 nodes, a Spark instance, SI1, is created to process the job.
