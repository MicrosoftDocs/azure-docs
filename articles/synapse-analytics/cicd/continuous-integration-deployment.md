...
* Setting the value of a property to `string` indicates that you want to parameterize the property. Use the format `<action>:<name>:<stype>`.
   *  `<action>` can be one of these characters:
      ...
      * `|` is a special case for Azure Key Vault secrets that are used for connection strings or keys.
   ...
   * `<stype>` is the type of parameter. If `<stype>` is blank, the default type is `string`. Supported values are `string`, `securestring`, `int`, `bool`, `object`, `secureobject`, and `array`.
