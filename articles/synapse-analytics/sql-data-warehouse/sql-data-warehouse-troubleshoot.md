...
| Cannot connect with tool or driver                           | Synapse SQL pool recommends using [SSMS](/sql/ssms/download-sql-server-management-studio-ssms?view=sql-server-ver15), [SSDT for Visual Studio](sql-data-warehouse-install-visual-studio.md), or [sqlcmd](sql-data-warehouse-get-started-connect-sqlcmd.md) to query your data. For more information on drivers and connecting to Azure Synapse, see [Drivers for Azure Synapse](../sql/connection-strings.md) and [Connect to Azure Synapse](../sql/connect-overview.md) articles. |
