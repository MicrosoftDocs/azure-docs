...
- In file explorer, right-click on the downloaded script file, and select "Run with PowerShell" or navigate to the path of the PowerShell file within the PowerShell application using `cd <path to powershell script>` . Then run the command `.\Test-AzureSynapse.ps1`
![Run downloaded script file with PowerShell](media/troubleshooting-synapse-studio-powershell/run-with-powershell.png)

 - In case you are getting the below error,then run the command  `Set-ExecutionPolicy -ExecutionPolicy RemoteSigned -Scope CurrentUser`

```
\Test-AzureSynapse.ps1 is not digitally signed. You cannot run
