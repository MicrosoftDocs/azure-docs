...
| **Azure Blob Storage** | Yes | Yes, you can use external tables and the `OPENROWSET` function to read data from Azure Blob Stroage. |
| **Azure SQL (remote)** | No | No, sererless SQL pool cannot reference Azure SQL. You can reference serverless SQL pools from Azure SQL using [elastic queries](https://devblogs.microsoft.com/azure-sql/read-azure-storage-files-using-synapse-sql-external-tables/) or [linked servers](https://devblogs.microsoft.com/azure-sql/linked-server-to-synapse-sql-to-implement-polybase-like-scenarios-in-managed-instance) |
| **Dataverse** | No | Yes, using [Synapse link](../../powerapps/maker/data-platform/azure-synapse-link-data-lake) |
