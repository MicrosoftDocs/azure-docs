...
| **Schemas** | [Yes](/sql/t-sql/statements/create-schema-transact-sql?view=azure-sqldw-latest&preserve-view=true) | [Yes](/sql/t-sql/statements/create-schema-transact-sql?view=azure-sqldw-latest&preserve-view=true), schemas are supported. Use schemas to isolate different tenanats and place their tables per schemas. |
...
| **CTAS statement**  | Yes | No, [CREATE TABLE AS SELECT](/sql/t-sql/statements/create-table-as-select-azure-sql-data-warehouse?view=azure-sqldw-latest) statement is not supported in serverless SQL pool. |
...
| **Permissions - [Database-level](/sql/relational-databases/security/authentication-access/database-level-roles?view=azure-sqldw-latest&preserve-view=true)** | Yes | Yes, you cn grant, deny or revoke permissions on the database objects. |
...
| **Azure Analysis Service** | Yes | Yes, you can load data in AAS from the serverless SQL pools. |
