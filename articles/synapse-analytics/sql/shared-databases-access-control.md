...
A user with minimal permission can:
-	Connect to a database that is replicated from Spark
-	Select data via external tables and access the underlying ADLS data.

## Solution

Below will be present a solution that will allow non-admin users to have server-level permissions to connect to any database. It will also allow users to view data from all schema-level objects, such as tables or views. Data access security can be managed on the storage layer. 
...
Access to the data on storage account can be managed via [ACL](/azure/storage/blobs/data-lake-storage-access-control) or regular [Storage Blob Data Owner/Reader/Contributor roles](/azure/storage/blobs/data-lake-storage-access-control-model) for AAD users/groups. For Service Principals (AAD apps), make sure you use ACL setup.
