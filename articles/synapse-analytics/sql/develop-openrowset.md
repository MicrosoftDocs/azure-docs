...
The `OPENROWSET(BULK...)` function allows you to access files in Azure Storage. `OPENROWSET` function reads content of some remote data soururce (for example file) and return content as a set of rows. Within the SQL on-demand (preview) resource, the OPENROWSET bulk rowset provider is accessed by calling the OPENROWSET function and specifying the BULK option.  
...
```sql
SELECT * FROM OPENROWSET(BULK 'http://storage..../container/folder/*.parquet', TYPE = 'PARQUET') AS file
```
...
```sql
SELECT * FROM OPENROWSET(BULK '/folder/*.parquet', DATA_SOURCE='storage', TYPE = 'PARQUET') AS file
```
