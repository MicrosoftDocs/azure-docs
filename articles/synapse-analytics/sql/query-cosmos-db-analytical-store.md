...
- Make sure that you have applied all [best practices](best-practices-serverless-sql-pool.md), such as:
  ...
  - Ensure that the client application (PowerBI, Analysis service) is in the same region as serverless SQL pool.
...
- Visit [Synapse link for Cosmos DB self-help page](resources-self-help-sql-on-demand.md#cosmos-db) if you are getting some errors or experiencing perfomance issues.
