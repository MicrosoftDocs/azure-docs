...
As a best practice, for the clients that support it, use ConnectionRetryCount and ConnectRetryInterval connection string keywords to control the reconnect behavior. Most SQL client drivers have the default connection timeout set to 15 seconds. Make sure the connection timeout is configured to allow all retry attemps. For example the chosen values should satisfy the following condition:
