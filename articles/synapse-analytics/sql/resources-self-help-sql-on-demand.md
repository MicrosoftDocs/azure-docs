...
- Delta Lake tables that created in the Apache Spark pools are automatically available in serverless SQL pool, but the schema is not updated (public preview limitation). If you add the columns in the Delta table using Spark pool, the changes will not be shown in serverless SQL pool database.
