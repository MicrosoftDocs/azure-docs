...
> 
> When the public network access is disabled, access to GIT mode in Synapse Studio will not be blocked as long as the user has enough permission to access the integrated Git repo or the corresponding Git branch, which is by design. But please be assured that the publish button will not work as the access to Live mode is blocked by the firewall settings. 
