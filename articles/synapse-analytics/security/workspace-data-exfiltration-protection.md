...
After the workspace is created with data exfiltration enabled, the owners of the workspace resource can manage the list of approved Azure AD tenants for the workspace. Users with the [right permissions](./access-control.md) on the workspace can use the Synapse Studio to create managed private endpoint connection requests to resources in the workspace’s approved Azure AD tenants. Managed private endpoint creation will be blocked if the user attempts to create a private endpoint connection to a resource in an unapproved tenant.
