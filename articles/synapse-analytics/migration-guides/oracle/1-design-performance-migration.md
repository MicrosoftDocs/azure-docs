...
> In Azure Synapse a similar effect can be achieved by use of > materialised and/or use of replicated tables to minimise the I/O > required at query run time.
...
> In Oracle a sequence is a named database object created via CREATE > SEQUENCE that can provide the unique value via the CURRVAL and NEXTVAL > methods. These can be used to generate unique numbers that can be used > as surrogate key values for primary key values. > > Within Azure Synapse there is no CREATE SEQUENCE so sequences are > handled via use of IDENTITY columns or using SQL code to create the > next sequence number in a series.
