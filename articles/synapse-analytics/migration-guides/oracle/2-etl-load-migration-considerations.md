...
If you have licensed the [Oracle Diagnostic Pack](https://www.oracle.com/technetwork/database/enterprise-edition/overview/diagnostic-pack-11g-datasheet-1-129197.pdf), then you have access to Active Session History, which you can use to determine when a table was last accessed.
...
If some or all of the existing Oracle warehouse ETL/ELT processing is handled by custom scripts that utilize Oracle-specific utilities, such as SQL\*Plus, SQL\*Developer, SQL\*Loader, or Data Pump, then you'll need to recode these scripts for the new Azure Synapse environment. Similarly, if ETL processes have been implemented using stored procedures in Oracle, then these will also have to be recoded.
...
If you use the direct path unload method, you must install Fast Clone on the Oracle source system and grant some system and object permissions to the Fast Clone user. **Informatica Intelligent Cloud Services for Azure** offers a best-in-class solution for self-service data migration, integration, and management capabilities. You can quickly and reliably import and export petabytes of data to Azure from a variety of sources. Informatica Intelligent Cloud Services for Azure provides native, high volume, high-performance connectivity to Azure Synapse, SQL Database, Blob Storage, Data Lake Store, and Azure Cosmos DB. **Informatica PowerCenter** is a metadata-driven data integration platform that jump starts and accelerates data integration projects in order to deliver data to the business faster than manual hand coding. It serves as the foundation for your data integration investments.
...
Talend can also [leverage Azure tools such as PolyBase](https://www.talend.com/blog/2017/02/08/leverage-load-data-microsoft-azure-sql-data-warehouse-using-polybase-talend-etl/) to guarantee the most efficient data loading into Azure Synapse.
...
- **File Extract**: Extract the data from the Oracle tables to flat delimited files, normally in CSV format. You can do this in several ways:
...
The recommended approach when moving to Azure Synapse is to orchestrate the data extract and loading from the Azure environment using [SQL Server Migration Assistant](/sql/ssma/oracle/sql-server-migration-assistant-for-oracle-oracletosql.md) (SSMA) or [Azure Data Factory](../../../data-factory/concepts-pipelines-activities.md?msclkid=b6ea2be4cfda11ec929ac33e6e00db98&tabs=data-factory), as well as associated utilities, such as PolyBase or COPY INTO, for the most efficient data loading. This approach leverages Azure capabilities and provides an easy method to build reusable data loading pipelines.
...
A number of techniques can be used to extract Oracle data when migrating from Oracle to Azure Synapse Analytics. More details on Oracle SQL Developer and Azure Data Factory Oracle Connector are included below:

### Oracle SQL Developer Data Extract

SQL Developer GUI has an export option that allows export to many formats, including CSV, using a very simple GUI-based method:
...
Other export options include JSON and XML. There's also an option in the GUI that allows you to build a "cart" that consists of a set of table names. You can then apply the export to all of those:
...
Note that this Oracle data export capability is also available via Oracle SQL Developer command line option. It can be automated as part of a shell script.
...
If you use ADF, we recommended that you enable parallel copy with data partitioning, especially when you plan to migrate large amounts of data from your Oracle database. The following are suggested configurations for different scenarios. When copying data into file-based data store, writing to a folder as multiple files (only specify folder name) is faster than writing to a single file.

| **Sc | **Suggested settings** | | enario** | |
| Full | **Partition option:** Physical partitions of table\ | | load | \ | | from | During execution, Data Factory automatically detects the | | large | physical partitions, and copies data by partitions | | table, | | | with | | | physical | | | pa | | | rtitions | | 

| Full | **Partition options:** Dynamic range partition\ | | load | **Partition column:** Specify the column used to | | from | partition data. If not specified, the primary key column | | large | is used | | table, | | | without | | | physical | | | par | | | titions, | | | while | | | with an | | | integer | | | column | | | for data | | | part | | | itioning | |

| Load a | **Partition option:** Physical partitions of table\ | | large | **Query: **SELECT \* FROM \<TABLENAME\> | | amount | PARTITION(\"?AdfTabularPartitionName\") WHERE | | of data | \<your_additional_where_clause\> | | by using | | | a custom | **Partition name:** Specify the partition name(s) to copy | | query, | data from. If not specified, Data Factory automatically | | with | detects the physical partitions on the table you | | physical | specified in the Oracle dataset\ | | pa | \ | | rtitions | During execution, Data Factory | | | replaces ?AdfTabularPartitionName with the actual | | | partition name, and sends to Oracle |

| Load a | **Partition options**: Dynamic range partition.\ | | large | **Query:** SELECT \* FROM \<TABLENAME\> WHERE | | amount | ?AdfRangePartitionColumnName \<= | | of data | ?AdfRangePartitionUpbound AND | | by using | ?AdfRangePartitionColumnName \>= | | a custom | ?AdfRangePartitionLowbound AND | | query, | \<your_additional_where_clause\> | | without | | | physical | **Partition column:** Specify the column used to | | par | partition data. You can partition against the column with | | titions, | integer data type\ | | while | **Partition upper bound **and **partition lower bound:** | | with an | Specify if you want to filter against partition column to | | integer | retrieve data only between the lower and upper range\ | | column | \ | | for data | During execution, Data Factory | | part | replaces | | itioning |  ?AdfRangePartitionColumnName, ?AdfRangePartitionUpbound, | | | and ?AdfRangePartitionLowbound with the actual column | | | name and value ranges for each partition, and sends to | | | Oracle. \ | | | For example, if your partition column \"ID\" is set with | | | the lower bound as 1 and the upper bound as 80, with | | | parallel copy set as 4, Data Factory retrieves data by 4 | | | partitions. Their IDs are between \[1,20\], \[21, 40\], | | | \[41, 60\], and \[61, 80\], respectively |
