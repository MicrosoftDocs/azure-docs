...
An API in the inventory can be managed by any API management solution, such as Azure [API Management](../api-management/api-management-key-concepts.md) or a platform such as Apigee API Management, Kong API Gateway, MuleSoft API Management, or others. An API represented in Azure API Center can also be unmanaged.
