...
Cloud Shell utilizes Azure Files to persist files across sessions. On initial start, Cloud Shell
prompts you to associate a new or existing fileshare to persist files across sessions.
...
The fileshare also contains a 5-GB image that's created for you that automatically persists data
in your `$HOME` directory. This applies for both Bash and PowerShell.
...
The populated storage options filter for locally redundant storage (LRS), geo-redundant storage
(GRS), and zone-redundant storage (ZRS) accounts.
...
![The Resource group setting][08]

## Securing storage access

For security, each user should provision their own storage account. For Azure role-based access
control (Azure RBAC), users must have contributor access or above at the storage account level.
...
region as the Cloud Shell machine that you're mounting them to. For example, Jane can set her
storage account to be located in Canada East, a secondary region, but the machine she is mounted to
is still located in a primary region. Her data at rest is located in Canada, but it's processed in
the United States.
...
![Running the "clouddrive" command][10]
...
![Running the `clouddrive mount`command][11]
...
![Running the `clouddrive unmount`command][12]
...
If the Azure fileshare has been removed, you will be prompted to create and mount a new Azure
fileshare in the next session.

![Running Dismount-CloudDrive][06]

[!INCLUDE [PersistingStorage-endblock][01]]
