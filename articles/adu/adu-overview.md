...
devices to receive updates. Both an ADU Agent Simulator binary and Raspberry Pi reference Yocto images are provided.
...
Once updates are published to ADU, they are then scanned for malware. More metadata is generated that allows
...
then downloads the update from an ADU-specified location or a Gateway.

## ADU workflows

ADU functionality can be broken down into three areas: Agent Integration,
...
Importing is the ability to import your update into ADU. ADU supports rolling out a single update per device, making it ideal for
...
right content to a specific set of compatible devices. ADU orchestrates the
...
[Getting Started With Azure Device Update](overview-quickstart.md)
