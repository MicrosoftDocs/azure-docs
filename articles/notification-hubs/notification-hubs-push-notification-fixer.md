...
description: Learn how to diagnose common issues with Azure Notification Hubs.
...
One of the most common questions from Azure Notification Hubs customers is how to troubleshoot when notifications that are sent from an application don't appear on client devices. They want to know where and why notifications were dropped, and how to fix the issue. This article identifies why notifications might get dropped or not be received by devices. You can learn how to analyze and determine the root cause. 

It's critical to first understand how Notification Hubs pushes notifications to a device.

![Notification Hubs architecture][0]

In a typical send notification flow, the message is sent from the *application back end* to Notification Hubs. Notification Hubs does some processing on all the registrations. The processing takes into account the configured tags and tag expressions to determine "targets." 

Targets are all the registrations that need to receive the push notification. These registrations can span any or all our supported platforms: iOS, Google, Windows, Windows Phone, Kindle, and Baidu for China Android.

With the targets established, Notification Hubs pushes notifications to the device platform-specific *push notification service*. Examples include the Apple Push Notification service (APNs) for Apple or Firebase Cloud Messaging (FCM) for Google. Notification Hubs pushes notifications split across multiple batches of registrations. Notification Hubs authenticates with the respective push notification service based on the credentials that you set in the Azure portal, under **Configure Notification Hub**. The push notification service then forwards the notifications to the respective *client devices*. 

This is the platform-recommended way to deliver push notifications. Note that the final leg of notification delivery takes place between the platform push notification service and the device. Of the four major components to the push notification process (*client*, *application back end*, *Notification Hubs*, and the *push notification service*), any component might cause notifications to be dropped. For more information about the Notification Hubs architecture, see [Notification Hubs overview].

Failure to deliver notifications might happen during the initial test/staging phase, which might indicate a configuration issue. Or, it might occur in production, where either some or all of the notifications might be dropped. In this case, a deeper application or messaging pattern issue is indicated. 

The next section looks at various scenarios in which notifications might be dropped, ranging from common to more rare.
...
* **General**
    ...
    * Ensure that your notification hub name (without typos) is the same in these locations:

        * Where you register from the client.
        * Where you send notifications from the back end.
        * Where you have configured the push notification service credentials.
    
    * Ensure that you use the correct SAS configuration strings on the client and on the application back end. Generally, you must use **DefaultListenSharedAccessSignature** on the client and **DefaultFullSharedAccessSignature** on the application back end (which grants permissions to send notifications to Notification Hubs).
...
* **FCM configuration** 
    ...
    1. Ensure that the *server key* that you obtained from Firebase matches the service key that you registered in the Azure portal.
...
* **Tags and tag expressions**
    ...
    Review your registrations to ensure that there are matching tags when you send a notification. Then, verify the notification receipt only from the clients with those registrations. 
    
    As an example, if all your registrations with Notification Hubs were made by using the tag "Politics" and you send a notification with the tag "Sports", the notification isn't sent to any device. A complex case might involve tag expressions in which you registered by using "Tag A" OR "Tag B," but while sending notifications, you are targeting "Tag A && Tag B." In the self-diagnosis tips section later in the article, we show you how to review your registrations and their tags. 
...
* **Invalid registrations**
    ...
    For more error information for the failed delivery attempt against a registration by using the Notification Hubs REST APIs [Per Message Telemetry: Get Notification Message Telemetry](https://msdn.microsoft.com/library/azure/mt608135.aspx) and [PNS feedback](https://msdn.microsoft.com/library/azure/mt705560.aspx). For sample code, see the [Send REST example](https://github.com/Azure/azure-notificationhubs-samples/tree/master/dotnet/SendRestExample).

## Push notification service issues
After the notification message has been received by the respective push notification service, it is the responsibility of the push notification service to deliver the notification to the device. At this point, Notification Hubs is out of the picture and has no control over when or if the notification is delivered to the device. 
...
Only one recent notification is stored for any app. If multiple notifications are sent while the device is offline, each new notification causes the prior notification to be discarded. Keeping only the newest notification is referred to as *coalescing notifications* in APNs, and *collapsing* in FCM (which uses a collapsing key). If the device remains offline for a long time, any notifications that were being stored for the device  are discarded. For more information, see [APNs guidance] and [FCM guidance].
...
## Self-diagnosis tips
Here are paths to diagnosing the root cause of Notification Hubs issues:
...
* **Azure portal**
    ...
    To review and match the credentials with those obtained from the push notification service developer portal, in the Azure portal, go to the **Access Policies** tab. 
...
* **Visual Studio**
    ...
    If you use Visual Studio for development, you can connect to Azure to view and manage multiple Azure services, including Notifications Hub, in Server Explorer. This is primarily useful for your dev/test environment. 
...
* **Azure portal**
    ...
    To send a test notification to your clients without having a service back end up and running, under **Support + Troubleshooting**, select the **Test Send** tab. 
...
* **Visual Studio**
    ...
    For more information about the Visual Studio Notification Hubs Azure explorer functionality, see these articles: 
...
To get insight into push notification service errors, you can use the [EnableTestSend] property. This property is automatically enabled when you send test messages from the portal or Visual Studio client. You can use this property to see detailed debugging information. You can also use the property via APIs. For example, currently, you can use it in the .NET SDK. Eventually, it will be added to all client SDKs. 
...
This message indicates that either invalid credentials are configured in Notification Hubs, or there's an issue with the registrations on the hub. We recommend that you delete this registration, and let the client re-create the registration before sending the message. 

> [!NOTE]
> The use of the **EnableTestSend** property is heavily throttled. Use this option only in a dev/test environment that has a limited set of registrations. We send debug notifications to only 10 devices. We also have a limit of processing debug sends to 10 per minute. 
> 
> 
...
* **Programmatic access**
    ...
    * [Telemetry access via APIs sample] 
    ...
    > [!NOTE]
    > Several telemetry-related features, like exporting and importing registrations and telemetry access via APIs are available only in the Standard service tier. If you attempt to use these features from the Free or Basic service tier, you receive an exception message if you use the SDK, and an HTTP 403 (Forbidden) error if you use the features directly from the REST APIs. 
    >
