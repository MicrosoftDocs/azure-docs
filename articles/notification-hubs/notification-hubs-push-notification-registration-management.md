...
An installation can contain the following properties. For a complete listing of the installation properties, see [Create or Overwrite an Installation with REST API](/previous-versions/azure/reference/mt621153(v=azure.100)) or [Installation Properties](/dotnet/api/microsoft.azure.notificationhubs.installation#microsoft_azure_notificationhubs_installation#properties_summary).
...
Each template name maps to a template body and an optional set of tags. Moreover, each platform can have additional template properties. For Windows Store (using WNS) and Windows Phone 8 (using MPNS), an additional set of headers can be part of the template. In the case of APNs, you can set an expiry property to either a constant or to a template expression. For a complete listing of the installation properties see, [Create or Overwrite an Installation with REST](/previous-versions/azure/reference/mt621153(v=azure.100)) topic.
...
At this time, this is only supported using the [Notification Hubs REST API](/previous-versions/azure/reference/mt621153(v=azure.100)).
