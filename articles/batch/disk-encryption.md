...
> [!NOTE]
> When you target the **temporary disk** for encryption on a VM family that doesn't support encryption at host, Batch applies [Azure Disk Encryption (ADE)](/azure/virtual-machines/disk-encryption-overview). As described in [Azure Disk Encryption for Linux VMs](/azure/virtual-machines/linux/disk-encryption-overview#supported-vms), ADE isn't available on Basic, A-series, v6-series, v7-series (or newer) VM sizes, or on virtual machines with less than 2 GB of memory. If you request temporary-disk encryption on one of these sizes and encryption at host isn't available as a fallback, Batch rejects the pool at creation time with an `AzureDiskEncryptionNotSupportedOnVMSize` error, instead of the pool later becoming unusable. To encrypt the temporary disk on these sizes, use a VM size that [supports encryption at host](/azure/virtual-machines/disks-enable-host-based-encryption-portal), or enable encryption at host in the pool's security profile (if your subscription has encryption at host enabled).
