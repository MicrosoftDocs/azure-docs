...
| AZ_BATCH_AUTHENTICATION_TOKEN   | An authentication token that grants access to a limited set of Batch service operations. This environment variable is only present if the [authenticationTokenSettings](/rest/api/batchservice/tasks/create-task#authenticationtokensettings) are set when the [task is added](/rest/api/batchservice/tasks/create-task#request-body). The token value is used in the Batch APIs as credentials to create a Batch client. With Azure.Compute.Batch, pass the token through a custom [TokenCredential](/dotnet/api/azure.core.tokencredential) to the [BatchClient(Uri, TokenCredential)](/dotnet/api/azure.compute.batch.batchclient.-ctor) constructor. The token doesn't support private networking. | All tasks. | OAuth2 access token |
