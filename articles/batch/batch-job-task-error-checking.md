...
Due to pools and pool nodes frequently being ephemeral, with nodes being continuously added and deleted, then it is recommended that log files are persisted. [Task output files](./batch-task-output-files.md) are a convenient way to save log files to Azure Storage.
