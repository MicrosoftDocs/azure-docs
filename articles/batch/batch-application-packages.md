...
> [!NOTE]
> While you can define application values in the [Microsoft.Batch/batchAccounts](/templates/microsoft.batch/batchaccounts) resource of an [ARM template](quick-create-template), it's not currently possible to use an ARM template to upload application packages to use in your Batch account. You must upload them to your linked storage account as described [below](#add-a-new-application).
