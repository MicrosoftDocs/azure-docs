...
Maximize resource usage on a smaller number of compute nodes in your pool by running more than one task simultaneously on each node. This can result in shorter job times and lower cost for some workloads.

While some scenarios work best with all of a node's resources dedicated to a single task, several situations benefit from allowing multiple tasks to share those resources:
