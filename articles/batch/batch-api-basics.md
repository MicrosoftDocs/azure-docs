...
- The **command line** of the task. This is the command line that runs your application or script on the compute node.
	...
	It is important to note that command line does not run under a shell, and therefore cannot natively take advantage of shell features such as [environment variable](#environment-settings-for-tasks) expansion. To take advantage of such features, you should **invoke the shell in the command line**. For example, by launching `cmd.exe` on Windows nodes or `/bin/sh` on Linux:
