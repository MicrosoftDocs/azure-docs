...
- Receive and respond to requests (prompts) autonomously.
...
| **Context** | An agent maintains a log history by keeping a maximum number of tokens or messages as context and passing that context into the model for the next interaction. Each model has different *context length* limits. For more information, see [Limitations for agent history context length (token limit)](create-agent-workflows.md#limitations-and-known-issues). |
