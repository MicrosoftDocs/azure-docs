...
| **Developer** | The base unit has fixed capacity and is [billed at an hourly rate for the Developer SKU](https://azure.microsoft.com/pricing/details/logic-apps). <p><p>**Important**: This SKU doesn't have any service-level agreement (SLA), scale up capability, published limits, and redundancy during recycling, which means that you might experience delays or downtime. <p><p>Use this SKU only for exploration, experiments, development, and testing, not production or performance testing. Backend updates might intermittently interrupt service. Make sure that you keep loads on your ISE under 500 executions per minute. |
|||
