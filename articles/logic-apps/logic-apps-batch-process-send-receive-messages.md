...
* The ["batch receiver"](#batch-receiver) logic app, which accepts and collects messages into a batch until your specified criteria is met for releasing and processing those messages.

  Make sure that you first create the batch receiver so that you can later select the batch destination when you create the batch sender.

* One or more ["batch sender"](#batch-sender) logic apps, which send the messages to the previously created batch receiver.
...
Make sure that your batch receiver and batch sender share the same Azure subscription *and* Azure region. If they don't, you can't select the batch receiver when you create the batch sender because they're not visible to each other.
...
1. In Logic Apps Designer, add the **Batch** trigger, which starts your logic app workflow. In the search box, enter `batch`, and select this trigger: **Batch messages**
...
1. Now add one or more actions that process each batch.
   ...
   1. Select this action: **Send an email - <*email provider*>**
...
1. If you're using Visual Studio, make sure that you [deploy your batch receiver logic app to Azure](../logic-apps/quickstart-create-logic-apps-with-visual-studio.md#deploy-logic-app-to-azure). Otherwise, you can't select the batch receiver when you create the batch sender.
...
* Make sure that you've already [created your batch receiver](#batch-receiver) so when you create your batch sender, you can select the existing batch receiver as the destination batch. While batch receivers don't need to know anything about batch senders, batch senders must know where to send messages.
...
1. Create another logic app with this name: `BatchSender`
   ...
   1. In the search box, enter "recurrence" as your filter. Select this trigger: **Recurrence**
   ...
   1. Set the frequency and interval to run the sender logic app every minute.
...
1. Add a new action for sending messages to a batch.
   ...
   1. In the search box, enter "batch" as your filter. Select the **Actions** list, and then select this action: **Choose a Logic Apps workflow with batch trigger - Send messages to batch**
   ...
   1. Select your batch receiver logic app that you previously created.
      ...
      >
      > If you're using Visual Studio, and you don't see any batch receivers to select, check that you deployed your batch receiver to Azure. If you haven't, 
...
1. Set the batch sender's properties:
   ...
   1. Enter the expression `utcnow()`, and then select **OK**.
...
1. Now set up a partition for the batch. In the "BatchReceiver" action, select **Add new parameter**, and set these properties:
