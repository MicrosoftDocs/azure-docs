...
In [Azure Logic Apps](./logic-apps-overview.md), the following options are available:
  ...
  - For Standard logic app workflows, you can enable Insights support, offering curated visualizations as a foundation for monitoring Azure services. These visualizations enable developers to monitor Logic Apps Standard applications and workflows effectively via dashboards. The monitoring dashboards are designed specifically for Logic Apps Standard. The scope of the dashboard are the workflows inside the Logic App. The dashboard is built using [Azure Workbooks](../azure-monitor/visualize/workbooks-overview), offering a variety of visualizations. Workbooks are easily extendable, allowing for customization to meet specific needs.
.
