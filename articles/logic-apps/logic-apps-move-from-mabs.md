...
Microsoft Azure BizTalk Services (MABS) is retiring. Use this article to move your MABS integration solutions to Azure Logic Apps. 
...
Logic Apps provides more sophisticated logic capabilities with [Condition](../logic-apps/logic-apps-control-flow-conditional-statement.md) and [Switch](../logic-apps/logic-apps-control-flow-switch-statement.md), enabling advanced control flow and routing. *If* there are only two options, converting routing filters in BizTalk Services is best achieved using a **condition**. If there are more than two, then use a **switch**.
...
### Use custom code
BizTalk Services provides the ability to [run custom code](https://msdn.microsoft.com/library/azure/dn232389.aspx) uploaded in your own assemblies. This functionality is implemented by the [IMessageInspector](https://msdn.microsoft.com/library/microsoft.biztalk.services.imessageinspector) interface. Each stage in the bridge includes two properties (On Enter Inspector, and On Exit Inspector) that provide the .Net type you created that implements this interface. Custom code allows you to perform more complex processing on the data,  as well as reuse existing code in assemblies that perform common business logic. 
...
## Next
* [What is Logic Apps?](logic-apps-overview.md)
