...
|workflow|This function provides you all the details for the workflow itself at the runtime. You can get everything that's available in the Management API, such as name, location, and resourceID. See the [Rest API](http://go.microsoft.com/fwlink/p/?LinkID=525617) for details on those properties. For example, this function returns a location like `westus`: <p>`workflow().location` <p> When invoked withn an action, the object returned by the `@workflow()` function contains details about the run as well, including its id, name and type. For example, to get the current run name like `08587058775309234062515637936`: <p>`workflow().run.name` |
