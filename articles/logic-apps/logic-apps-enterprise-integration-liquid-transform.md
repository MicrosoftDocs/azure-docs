...
## Overview
Logic Apps support basic JSON transformations through native Data Operation actions like Compose or Parse JSON. Logic Apps now also supports advanced JSON transformations via Liquid templates. [Liquid](https://shopify.github.io/liquid/) is an open-source, safe, customer-facing template language for flexible web apps.
...
* Choose **Add** and provide the Map details
  ...
  * **Map**: The liquid map or template for transformation. Use file picker to select the file, which is SimpleJsonToJsonTemplate.liquid in this example.
    ...
    Liquid template defines how you want to transform JSON input. Here is the content of Map used in previous step.
