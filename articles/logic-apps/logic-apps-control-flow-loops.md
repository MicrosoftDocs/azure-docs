...
1. In the [Azure portal](https://portal.azure.com), create an example logic app workflow with the following steps in the specified order:
...
1. Now build the loop:
   ...
   1. Form the list, under **When a feed item is published**, select **Feed links**, which is an array output from the RSS trigger.
...
1. In the **Until** action, provide the following values to set up the stop condition for the loop.
   ...
   1. Select inside the **Loop Until** box, which automatically opens the dynamic content list.
