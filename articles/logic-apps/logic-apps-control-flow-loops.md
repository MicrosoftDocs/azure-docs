...
To repeat actions in a logic app workflow, Azure Logic Apps supports adding loop actions to your workflow.
...
The **For each** action works only on arrays. It repeats one or more actions on each item in an array. Keep in mind the following considerations:
...
1. Now build the loop:
   ...
   1. From the dynamic content list, under **When a feed item is published**, select **Feed links**, which is an array output from the RSS trigger.
...
1. To manually test your workflow, from the **Run** option on the designer toolbar, select **Run**.
...
1. Select the **For each** action to open its properties, then select **Settings**.
...
If you're working in code view with the `For_each` action in your workflow's JSON definition, you can use the `Sequential` option by adding the `operationOptions` parameter:
...
1. In the designer, follow these general steps to add the **Recurrence** built-in trigger named **Schedule** to your workflow for a [Consumption](create-workflow-with-trigger-or-action.md?tabs=consumption#add-trigger) or [Standard](create-workflow-with-trigger-or-action.md?tabs=standard#add-trigger) logic app.
...
1. Follow these general steps to add the **Variables** built-in action named **Increment variable** to the **Until** action for your [Consumption](create-workflow-with-trigger-or-action.md?tabs=consumption#add-action) or [Standard](create-workflow-with-trigger-or-action.md?tabs=standard#add-action) logic app.
...
1. In the **Increment variable** action, provide the following values to increment the **Limit** variable's value by 1:
   ...
   :::image type="content" source="./media/logic-apps-control-flow-loops/do-until-loop-increment-variable.png" alt-text="Screenshot shows a workflow and built-in action named Until with Limit set to Limit and Value set to 1." lightbox="./media/logic-apps-control-flow-loops/do-until-loop-increment-variable.png":::
...
1. In the email action, provide the following values:
   ...
   | **Subject** | **Current value for "Limit" variable is:** **Limit** | The email subject. For this example, make sure that you include the **Limit** variable to confirm that the current value meets your specified condition: <br><br>1. Select inside the **Subject** box so that the lightning bolt icon appears. <br><br>2. In the dynamic content list, next to the **Variables** section header, select **See more**. <br><br>3. Select **Limit**. |
