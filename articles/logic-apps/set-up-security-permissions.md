...
description: Secure access to triggers, inputs, outputs, run history, workflows, and other resources in Azure Logic Apps.
...
ms.custom:
  - sfi-image-nochange
  - sfi-ropc-nochange
#Customer intent: As an automation and integration developer who works with Azure Logic Apps, I want to understand how to secure my workflows, their operations, and data.
---

# Secure access and data for workflows in Azure Logic Apps

Azure Logic Apps stores and automatically [encrypts data at rest](../security/fundamentals/encryption-atrest.md) by using [Azure Storage](../storage/index.yml). This encryption protects your data by using Microsoft-managed keys and helps you meet your organizational security and compliance commitments. For more information, see [Azure Storage encryption for data at rest](../storage/common/storage-service-encryption.md).

This guide shows how you can further secure and protect sensitive data in the following main areas for Azure Logic Apps:

| Task | Go to |
|---|---|
| Control who can manage or edit workflows | [Secure logic app operations](#secure-operations) |
| Hide inputs, outputs, passwords, and secrets in run history | [Secure run history data](#secure-run-history) |
| Protect parameter values across environments | [Secure parameter inputs](#secure-action-parameters) |
| Understand authentication types | [Review authentication types for workflow operations](#authentication-types-supported-triggers-actions) |
| Authenticate calls that start your workflow | [Secure inbound calls](#secure-inbound-requests) |
| Authenticate calls your workflow sends | [Secure outbound calls](#secure-outbound-requests) |
| Block specific connectors | [Block connections](#block-connections) |
| Meet isolation requirements | [Isolation guidance](#isolation-logic-apps) |
| Review security baseline | [Azure Security Baseline for Azure Logic Apps](security-baseline.md) |

For information about Azure-wide security, see:
...
## Secure access to logic app operations with Azure RBAC

In Azure, [role-based access control (RBAC)](/azure/role-based-access-control/role-assignments-portal) controls who has access and permissions to work with resources. Before others can create or manage logic apps, workflows, and connections, you need to assign them a role that provides the necessary permissions. You can set up permissions so that only specific users or groups can run specific tasks, such as managing, editing, and viewing logic apps. To control their permissions, assign built-in or customized roles to members who have access to your Azure subscription.

> [!IMPORTANT]
>
> To reduce your attack surface, always assign roles based on the [*principle of least privilege*](/entra/identity-platform/secure-least-privileged-access).
>
> Before you assign a role, review the following best practices, considerations, and impacts:
>
> -  Allow users, apps, and [managed identities](/entra/identity/managed-identities-azure-resources/overview) to access *only* the data and operations they need to perform their jobs.
>
> - Assign contributor-level roles or workflow editing permissions, based on least privilege, to *trusted principals* only.
>
> - Assign only the minimum necessary permissions for managed identities on logic app workflows to do their work.
>
> - Contributor or workflow editing permissions equate to holding the permissions for every [managed identity](/entra/identity/managed-identities-azure-resources/overview) assigned to that workflow.
>
...
Azure Logic Apps has the following specific roles, based on whether you have a Consumption or Standard workflow:
...
To control access to the inputs and outputs in your workflow run history, use the following options:
...
1. On the logic app sidebar, under **Settings**, select **Workflow settings**.
...
1. On the logic app sidebar, under **Settings**, select **Networking**.
...
- Make sure that you turn on **Secure Inputs** or **Secure Outputs** in downstream actions where you expect the run history to obscure that data.
  ...
  **Secure Outputs setting**
  ...
  :::image type="content" source="./media/set-up-security-permissions/secure-outputs-as-inputs-flow.png" alt-text="Diagram that shows how secured outputs affect visibility as hidden inputs for downstream actions in the workflow run history." lightbox="./media/set-up-security-permissions/secure-outputs-as-inputs-flow.png":::
  ...
  :::image type="content" source="./media/set-up-security-permissions/secure-outputs-as-inputs-flow-special.png" alt-text="Diagram that shows how secured outputs affect visibility as inputs for the Compose, Parse JSON, and Response actions." lightbox="./media/set-up-security-permissions/secure-outputs-as-inputs-flow-special.png":::
  ...
  **Secure Inputs setting**
  ...
  :::image type="content" source="./media/set-up-security-permissions/secure-inputs-impact-on-downstream.png" alt-text="Diagram that shows how secured trigger or action inputs affect visibility as inputs for downstream workflow actions." lightbox="./media/set-up-security-permissions/secure-inputs-impact-on-downstream.png":::

  If the **Compose**, **Parse JSON**, and **Response** actions explicitly use the visible outputs from the trigger or action that has the secured inputs, Azure Logic Apps hides these actions' inputs and outputs, but *doesn't enable* these actions' **Secure Inputs** setting. If a downstream action explicitly uses the hidden outputs from the **Compose**, **Parse JSON**, or **Response** actions as inputs, Azure Logic Apps *doesn't hide this downstream action's inputs or outputs*.

  :::image type="content" source="./media/set-up-security-permissions/secure-inputs-flow-special.png" alt-text="Diagram that shows how secured inputs affect the Compose, Parse JSON, and Response workflow actions." lightbox="./media/set-up-security-permissions/secure-inputs-flow-special.png":::
...
1. On the information pane that opens, select **Settings**, and expand **Security**.
   ...
   :::image type="content" source="media/set-up-security-permissions/open-action-trigger-settings-standard.png" alt-text="Screenshot that shows the Azure portal, workflow designer, and trigger or action with opened settings." lightbox="media/set-up-security-permissions/open-action-trigger-settings-standard.png":::

1. Turn on either **Secure Inputs**, **Secure Outputs**, or both.

   :::image type="content" source="media/set-up-security-permissions/turn-on-secure-inputs-outputs-standard.png" alt-text="Screenshot that shows a workflow with an action's Secure Inputs or Secure Outputs settings enabled." lightbox="media/set-up-security-permissions/turn-on-secure-inputs-outputs-standard.png":::
   ...
   :::image type="content" source="media/set-up-security-permissions/select-secured-token-standard.png" alt-text="Screenshot that shows a workflow with a subsequent action's dynamic content list open, and the previous action's token for secured output with lock icon." lightbox="media/set-up-security-permissions/select-secured-token-standard.png":::

1. After the workflow runs, you can view the history for that run.

   1. On the logic app sidebar, select **Overview**.
   ...
   1. On the workflow run history pane, select the actions that you want to review.
      ...
      :::image type="content" source="media/set-up-security-permissions/hidden-data-run-history-standard.png" alt-text="Screenshot that shows a Standard workflow run history view with hidden inputs and outputs." lightbox="media/set-up-security-permissions/hidden-data-run-history-standard.png":::
...
## Secure access to parameter inputs across environments

If you deploy across different environments, consider parameterizing the values in your workflow definition that vary based on those environments. By using an [Azure Resource Manager template](../azure-resource-manager/templates/overview.md) to deploy your logic app, you can avoid hard-coded data, protect sensitive data by defining secured parameters, and pass that data as separate inputs through the [template's parameters](../azure-resource-manager/templates/parameters.md) by using a [parameter file](../azure-resource-manager/templates/parameter-files.md).

For example, if you authenticate HTTP actions with [OAuth with Microsoft Entra ID](#oauth-microsoft-entra), you can define and obscure the parameters that accept the client ID and client secret that are used for authentication. To define these parameters in your logic app workflow, use the `parameters` section in your logic app's workflow definition and Resource Manager template for deployment. To help secure parameter values that you don't want shown when editing your logic app or viewing run history, define the parameters by using the `securestring` or `secureobject` type and use encoding as necessary. Parameters that have this type aren't returned with the resource definition and aren't accessible when viewing the resource after deployment. To access these parameter values during runtime, use the `@parameters('<parameter-name>')` expression inside your workflow definition. This expression is evaluated only at runtime and is described by the [Workflow Definition Language](logic-apps-workflow-definition-language.md).

> [!NOTE]
>
> If you use a parameter in a request header or body, this parameter might be visible when you view your workflow's run history and outgoing HTTP request. Make sure that you also set your content access policies accordingly. You can also use [obfuscation](#obfuscate) to hide inputs and outputs in your run history.
>
> By default, `Authorization` headers aren't visible through inputs or outputs. If you use a secret there, you can't retrieve that secret.
...
If you [automate deployment for logic apps by using Resource Manager templates](logic-apps-azure-resource-manager-templates-overview.md), you can define secured [template parameters](../azure-resource-manager/templates/parameters.md), which are evaluated at deployment, by using the `securestring` and `secureobject` types. To define template parameters, use your template's top level `parameters` section, which is separate and different from your workflow definition's `parameters` section. To provide the values for template parameters, use a separate [parameter file](../azure-resource-manager/templates/parameter-files.md).

For example, if you use secrets, you can define and use secured template parameters that retrieve those secrets from [Azure Key Vault](/azure/key-vault/general/overview) at deployment. You can then reference the key vault and secret in your parameter file. For more information, see:

- [Pass sensitive values at deployment by using Azure Key Vault](../azure-resource-manager/templates/key-vault-parameter.md).
- [Secure parameters in Azure Resource Manager templates](#secure-parameters-deployment-template) later in this article.
...
> [!IMPORTANT]
>
> For optimal security, use [Microsoft Entra ID](/entra/identity/authentication/overview-authentication) with [managed identities](/entra/identity/managed-identities-azure-resources/overview) for authentication when possible.  This option provides superior security without requiring credentials. Azure manages this identity and helps keep  authentication information secure so that you don't have to manage this sensitive information.
>
> To set up a managed identity for Azure Logic Apps, see [Authenticate access and connections to Azure resources with managed identities in Azure Logic Apps](authenticate-with-managed-identity.md).
...
A [Resource Manager template](logic-apps-azure-resource-manager-templates-overview.md) for a logic app resource and workflow has multiple `parameters` sections. To protect passwords, keys, secrets, and other sensitive information, define secured parameters at the template level and workflow definition level by using the `securestring` or `secureobject` type. You can then store these values in [Azure Key Vault](/azure/key-vault/general/overview) and use the [parameter file](../azure-resource-manager/templates/parameter-files.md) to reference the key vault and secret. Your template then retrieves that information at deployment. For more information, review [Pass sensitive values at deployment by using Azure Key Vault](../azure-resource-manager/templates/key-vault-parameter.md).
...
>
> For example, you might find the following cipher suites if you inspect the TLS handshake messages in Azure Logic Apps or by using a security tool on your logic app's URL. Again, *don't use* these older suites:
>
...
The Microsoft Authentication Library (MSAL) libraries provide PoP tokens for you to use. If the Consumption logic app workflow that you want to call requires a PoP token, you can get this token by using the MSAL libraries. The following samples show how to acquire PoP tokens:
...
1. On the logic app sidebar, under **Settings**, select **Authorization**.

1. On the **Authorization** page, select **Add policy**.

   :::image type="content" source="media/set-up-security-permissions/add-azure-active-directory-authorization-policies.png" alt-text="Screenshot that shows the Azure portal, Authorization page, and selected button to add policy." lightbox="media/set-up-security-permissions/add-azure-active-directory-authorization-policies.png":::
...
1. Provide information about the authorization policy by specifying the [claim types](/entra/identity-platform/developer-glossary#claim) and values that your logic app expects in the access token presented by each inbound call to the **Request** trigger:
   ...
   :::image type="content" source="media/set-up-security-permissions/set-up-authorization-policy.png" alt-text="Screenshot that shows the Azure portal, Authorization page, and authorization policy details." lightbox="media/set-up-security-permissions/set-up-authorization-policy.png":::
   ...
   :::image type="content" source="media/set-up-security-permissions/pop-policy-example.png" alt-text="Screenshot that shows the Azure portal, Authorization page, and proof-of-possession policy information." lightbox="media/set-up-security-permissions/pop-policy-example.png":::
...
In your ARM template, define an authorization policy following these steps and syntax:
...
1. Provide a name for the authorization policy, set the policy type to `AAD`, and include a `claims` array where you specify one or more claim types.
...
- [Schema reference for trigger and action types - Request trigger](logic-apps-workflow-actions-triggers.md#request-trigger).
- [Schema reference for trigger and action types - HTTP Webhook trigger](logic-apps-workflow-actions-triggers.md#http-webhook-trigger).
- [Schema reference for trigger and action types - Operation options](logic-apps-workflow-actions-triggers.md#operation-options).
...
:::image type="content" source="media/set-up-security-permissions/request-trigger-url.png" alt-text="Screenshot that shows the Azure portal, Consumption workflow, and Request trigger endpoint property named HTTP URL." lightbox="media/set-up-security-permissions/request-trigger-url.png":::
...
> [!IMPORTANT]
>
> Protect your SAS key just as you protect an account key from unauthorized use. Set up or have a plan for revoking a compromised access key. Use discretion when you distribute URIs that use access keys, and only distribute such URIs over a secure connection such as HTTPS. Make sure to only perform operations that use an access key over an HTTPS connection. Anyone that has a URI with valid key can access the associated resource.
>
> To maintain security and protect access to your workflow, [regenerate access keys](#regenerate-access-keys) on a regular schedule as they might need to comply with security policies or become compromised. This way, you can make sure that only authorized requests can trigger your workflow, which protects your data and processes from unauthorized access.
>
> If you use an SAS key to access storage services, [create a user delegation SAS](/rest/api/storageservices/create-user-delegation-sas), which is secured with [Microsoft Entra ID](/entra/identity/authentication/overview-authentication), rather than an account key.
>
> For optimal security, use [Microsoft Entra ID](/entra/identity/authentication/overview-authentication) with [managed identities](/entra/identity/managed-identities-azure-resources/overview) for authentication whenever possible. This option provides superior security without requiring credentials. Azure manages this identity and helps keep authentication information secure so that you don't have to manage this sensitive information. To set up a managed identity for Azure Logic Apps, see [Authenticate access and connections to Azure resources with managed identities in Azure Logic Apps](authenticate-with-managed-identity.md).
...
> [!NOTE]
>
> This action disables SAS authentication for incoming requests and blocks existing SAS keys or signatures from working. However, your SAS keys or signatures remain valid and still work if you enable SAS authentication again.
>
> To disable SAS keys and signatures by creating new versions, see [Regenerate access keys](#regenerate-access-keys).
...
#### Prerequisites for disabling SAS authentication

To disable SAS authentication on a request-based trigger, you need a tool to send REST API calls, for example:
...
> [!IMPORTANT]
>
> Protect your access key just as you protect an account key from unauthorized use. Set up or have a plan for revoking a compromised access key. Use discretion when you distribute URIs that use access keys, and only distribute such URIs over a secure connection such as HTTPS. Make sure to only perform operations that use an access key over an HTTPS connection. Anyone that has a URI with a valid key can access the associated resource.
>
> If you use an SAS key to access storage services, [create a user delegation SAS](/rest/api/storageservices/create-user-delegation-sas), which is secured with [Microsoft Entra ID](/entra/identity/authentication/overview-authentication), rather than an account key.
>
> For optimal security, use [Microsoft Entra ID](/entra/identity/authentication/overview-authentication) with [managed identities](/entra/identity/managed-identities-azure-resources/overview) for authentication when possible. This option provides superior security without requiring credentials. Azure manages this identity and helps keep authentication information secure so that you don't have to manage this sensitive information.
...
If you share the endpoint URL for a request-based trigger with other parties, you can generate callback URLs that use specific keys and have expiration dates. By using this approach, you can seamlessly roll keys or restrict access to triggering your logic app based on a specific timespan. To specify an expiration date for a URL, use the [Azure Logic Apps REST API](/rest/api/logic/workflowtriggers), for example:
...
1. In the **Access control configuration** section, under **Allowed inbound IP addresses**, choose the path for your scenario:
   ...
   - To make your workflow callable by using the [**Azure Logic Apps** built-in action](logic-apps-http-endpoint.md#call-other-workflows), but only as a nested workflow, select **Only other Logic Apps**. This option works *only* when you use the **Azure Logic Apps** action to call the nested workflow.
   ...
   - To make your workflow callable by using the HTTP action, but only as a nested workflow, select **Specific IP ranges**. When the **IP ranges for triggers** box appears, enter the parent workflow's [outbound IP addresses](logic-apps-limits-and-config.md#outbound). A valid IP range uses these formats: *x.x.x.x/x* or *x.x.x.x-x.x.x.x*.
...
1. Optionally, under **Restrict calls to get input and output messages from run history to the provided IP addresses**, specify the IP address ranges for inbound calls that can access input and output messages in run history.
  ...
  - Connect through Azure API Management.
    ...
    **Consumption workflows**
    ...
    1. Based on whether you're adding an API Management trigger or action, follow these steps:
       ...
       - Trigger: Add the trigger named **Choose an Azure API Management Trigger** by following the [general steps](add-trigger-action-workflow.md?tabs=Consumption#add-trigger).

       - Action: Add the action named **Choose an Azure API Management action** by following the [general steps](add-trigger-action-workflow.md?tabs=Consumption#add-action).
    ...
    **Standard workflows**
    ...
    1. On the workflow designer, add the action named **Call an Azure API Management API** by following the [general steps](add-trigger-action-workflow.md?tabs=standard#add-action).

    1. From the API Management service instance list, select your previously created API Management service instance.

    1. From the API operations list, select the API operation to call, and then select **Create New**.

       :::image type="content" source="media/set-up-security-permissions/select-api-standard.png" alt-text="Screenshot that shows the Azure portal, Standard workflow designer, and selected API to call." lightbox="media/set-up-security-permissions/select-api-standard.png":::
...
> [!IMPORTANT]
>
> To protect sensitive information that your logic app workflow handles, use secured parameters and encode data as necessary. For more information about using and securing parameters, review [Access to parameter inputs](#secure-action-parameters).
>
> For optimal security, use [Microsoft Entra ID](/entra/identity/authentication/overview-authentication) with [managed identities](/entra/identity/managed-identities-azure-resources/overview) for authentication when possible. This option provides superior security without requiring credentials. Azure manages this identity and helps keep authentication information secure so that you don't have to manage this sensitive information.
>
> To set up a managed identity for Azure Logic Apps, see [Authenticate access and connections to Azure resources with managed identities in Azure Logic Apps](authenticate-with-managed-identity.md).
...
> [!NOTE]
>
> If you try to authenticate with a client certificate by using OpenSSL, you might get the following error:
>
...
> 3. Resign your certificate by using the new update.
> 4. Add the new certificate to the HTTP operation when using client certificate authentication.

When you use [secured parameters](#secure-action-parameters) to handle and secure sensitive information, for example, in an [Azure Resource Manager template for automating deployment](logic-apps-azure-resource-manager-templates-overview.md), you can use expressions to access these parameter values at runtime. This example HTTP action definition specifies the authentication `type` as `ClientCertificate` and uses the [parameters() function](workflow-definition-language-functions-reference.md#parameters) to get the parameter values:
...
> [!IMPORTANT]
>
> If you have a Standard logic app resource in single-tenant Azure Logic Apps, and you want to use an HTTP operation with a TLS/SSL certificate, client certificate, or Microsoft Entra ID OAuth with the `Certificate` credential type, make sure you complete the extra setup steps for this authentication type. Otherwise, the call fails. For more information, see [Authentication in single-tenant environment](../connectors/connectors-native-http.md#single-tenant-authentication).

For more information about securing services by using client certificate authentication, see:
...
When you use [secured parameters](#secure-action-parameters) to handle and secure sensitive information, such as in an [Azure Resource Manager template for automating deployment](logic-apps-azure-resource-manager-templates-overview.md), you can use expressions to access these parameter values at runtime. This example HTTP action definition specifies the authentication `type` as `Raw`, and uses the [parameters() function](workflow-definition-language-functions-reference.md#parameters) to get the parameter values:
...
- A Standard logic app resource supports having the [system-assigned managed identity *and* multiple user-assigned managed identities](authenticate-with-managed-identity.md) enabled at the same time, though you still can only select one identity to use at any time.
  ...
  > [!NOTE]
  >
  > By default, the system-assigned identity is already enabled to authenticate connections at run time. This identity differs from the authentication credentials or connection string that you use when you create a connection. If you disable this identity, connections don't work at run time. To view this setting, on your logic app sidebar, under **Settings**, select **Identity**.
...
1. In the trigger or action that supports using a managed identity, provide this information:
   ...
   **Built-in triggers and actions**
   ...
   | **Managed Identity** | `identity` | No | <*user-assigned-identity-ID*> | The user-assigned managed identity to use. **Note**: Don't include this property when using the system-assigned managed identity. |
      | **Audience** | `audience` | Yes | <*target-resource-ID*> | The resource ID for the target resource that you want to access. <br><br>For example, `https://storage.azure.com/` makes the [access tokens](/entra/identity-platform/access-tokens) for authentication valid for all storage accounts. However, you can also specify a root service URL, such as `https://fabrikamstorageaccount.blob.core.windows.net` for a specific storage account. <br><br>**Note**: The **Audience** property might be hidden in some triggers or actions. To make this property visible, in the trigger or action, open the **Advanced parameters** list, and select **Audience**. <br><br>**Important**: Make sure this target resource ID *exactly matches* the value that Microsoft Entra ID expects, including any required trailing slashes. So, the `https://storage.azure.com/` resource ID for all Azure Blob Storage accounts requires a trailing slash. However, the resource ID for a specific storage account doesn't require a trailing slash. To find these resource IDs, review [Azure services that support Microsoft Entra ID](/entra/identity/managed-identities-azure-resources/managed-identities-status). |

   When you use [secured parameters](#secure-action-parameters) to handle and secure sensitive information, such as in an [Azure Resource Manager template for automating deployment](logic-apps-azure-resource-manager-templates-overview.md), you can use expressions to access these parameter values at runtime. For example, this HTTP action definition specifies the authentication `type` as `ManagedServiceIdentity` and uses the [parameters() function](workflow-definition-language-functions-reference.md#parameters) to get the parameter values:
...
## Block connections created by specific connectors

If your organization doesn't permit connecting to specific resources by using their connectors in Azure Logic Apps, you can [block the capability to create those connections](block-connections-connectors.md) for specific connectors in logic app workflows by using [Azure Policy](../governance/policy/overview.md). For more information, review [Block connections created by specific connectors in Azure Logic Apps](block-connections-connectors.md).
...
- To run your own code or perform XML transformation, [create and call an Azure function](call-azure-functions-from-workflows.md), rather than use the [inline code capability](logic-apps-add-run-inline-code.md) or provide [assemblies to use as maps](logic-apps-enterprise-integration-maps.md), respectively. Also, set up the hosting environment for your function app to comply with your isolation requirements.
...
For more information about isolation, see:
