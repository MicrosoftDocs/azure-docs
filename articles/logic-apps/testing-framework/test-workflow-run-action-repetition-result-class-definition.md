...
description: The result from a loop action in a unit test flow run for a Standard logic app workflow.
services: logic-apps
...
This class extends the **`TestWorkflowRunActionResult`** class to represent the result from an action in a loop interation during unit test execution for a Standard logic app workflow. Such actions can appear inside **For each** loops or **Until** loops.
