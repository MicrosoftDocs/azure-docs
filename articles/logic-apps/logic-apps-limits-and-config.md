...
| Maximum waiting runs | * 0 when the concurrency control is turned off <p><p>* When the concurrency control is turned on, then 10 + the number of concurrent runs (trigger concurrency) is the minimum number of waiting runs. You can set the limit up to and including 100 as the maximum. | This limit describes the highest number of logic app instances that can wait to run when your logic app is already running the maximum concurrent instances. <p><p>To change the default limit, see [Change waiting runs limit](../logic-apps/logic-apps-workflow-actions-triggers.md#change-waiting-runs). | 
