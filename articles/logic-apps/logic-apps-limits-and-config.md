...
| Name | Limit | Notes | 
| ---- | ----- | ----- | 
| Trigger concurrency | 20 | The default maximum number of logic app instances that can run at the same time, or in parallel <p><p>To change this default to a value between 1 and 50 inclusively, see [Change trigger concurrency](../logic-apps/logic-apps-workflow-actions-triggers.md#change-trigger-concurrency) or [Trigger instances sequentially](../logic-apps/logic-apps-workflow-actions-triggers.md#sequential-trigger). |  
| Maximum waiting runs in queue | 10 | The default maximum number of logic app instances that can wait in the queue when your logic app is already running the maximum number of instances <p><p>To change this default to a value between 0 and 100 inclusively, see [Change waiting runs limit](../logic-apps/logic-apps-workflow-actions-triggers.md#change-waiting-runs). | 
| Foreach items | 100,000 | The maximum number of array items that a "for each" loop can process <p><p>To filter larger arrays, you can use the [query action](../connectors/connectors-native-query.md). | 
| Foreach iterations | 20 | The default maximum number of "for each" loop iterations that can run at the same time, or in parallel <p><p>To change this default to a value between 1 and 50 inclusively, see [Change "for each" concurrency](../logic-apps/logic-apps-workflow-actions-triggers.md#change-for-each-concurrency) or [Run "for each" loops sequentially](../logic-apps/logic-apps-workflow-actions-triggers.md#sequential-for-each). | 
...
| Name | Limit | Notes | 
| ---- | ----- | ----- | 
| Actions executions per 5 minutes | 100,000 | You can raise this limit to 300,000 by [running your logic app in "high throughput" mode](../logic-apps/logic-apps-workflow-actions-triggers.md#run-high-throughput-mode), which is in preview. Or, you can distribute the workload across more than one logic app as necessary. | 
