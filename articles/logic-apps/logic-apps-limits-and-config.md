...
| Storage retention | 90 days from the run's start time | 366 days | The retention period is calculated by using the run completion timestamp and the current timestamp. When this period exceeds the limit, the run is removed from the runs history. If a logic app times out, no completion timestamp is available. In this case, retention calculation uses the run's starting timestamp instead. <p><p>To change the default limit, see [change storage retention](#change-retention). |
