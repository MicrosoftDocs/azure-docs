...
1. Select your logic app. On your logic app's menu, select **Overview**. On the Overview pane, select **Trigger history**.
   ...
   ![Screenshot showing the Overview pane with "Trigger history" selected.](./media/monitor-logic-apps/overview-logic-app-trigger-history.png)
   ...
   > select **Run Trigger**, and select the trigger, which forces a check. Or, select **Run Trigger** 
   > on designer toolbar.
...
1. To view information about a specific trigger attempt, select that trigger event.
   ...
   ![Screenshot showing a specific trigger entry selected.](./media/monitor-logic-apps/select-trigger-event-for-review.png)

   You can now review information about the selected trigger event, for example:

   ![Screenshot showing specific trigger history information.](./media/monitor-logic-apps/view-specific-trigger-details.png)

<a name="review-runs-history"></a>

## Review runs history

Each time the trigger successfully fires, Azure Logic Apps creates an individual workflow instance and runs that instance. By default, each instance runs in parallel so that no workflow has to wait before starting a run. You can review what happened during each run, including the status for each step in the workflow plus the inputs and outputs for each step.
...
1. Select your logic app. On your logic app's menu, select **Overview**. On the Overview pane, select **Runs history**.
   ...
   ![Screenshot showing Overview pane with "Runs history" selected.](./media/monitor-logic-apps/overview-logic-app-runs-history.png)
...
1. To review the steps and other information for a specific run, under **Runs history**, select that run. If the list shows many runs, and you can't find the entry that you want, try filtering the list.
   ...
   ![Screenshot showing a specific workflow run selected.](./media/monitor-logic-apps/select-specific-logic-app-run.png)
   ...
   ![Screenshot showing each action in the specific run.](./media/monitor-logic-apps/logic-app-run-pane.png)

   To view this information in list form, on the **Logic app run** toolbar, select **Run Details**.

   ![Screenshot showing "Logic app run" toolbar with "Run Details" selected.](./media/monitor-logic-apps/toolbar-select-run-details.png)
...
1. To get more information about a specific step, select either option:
   ...
   * In the **Logic app run** pane, select the step so that the shape expands. You can now view information such as inputs, outputs, and any errors that happened in that step.
     ...
     ![Screenshot showing "Logic app run" pane with the expanded shape for an example failed step.](./media/monitor-logic-apps/specific-step-inputs-outputs-errors.png)
   ...
   * In the **Logic app run details** pane, select the step that you want.
   ...
   > [hide inputs and outputs in run history](../logic-apps/logic-apps-securing-a-logic-app.md#obfuscate)
...
* [Monitor logic apps with Azure Monitor](../logic-apps/monitor-logic-apps-log-analytics.md)
