...
1. In the **Advanced options** section, you can select the following options:
   ...
   | **Export custom connectors as API Management actions** | Off | If enabled, custom connectors are created as API Management actions, rather than HTTP actions, so that you can implement custom actions as API operations in API Management. For more information, see [Azure Logic Apps Standard options for connecting to customer services or APIs](https://techcommunity.microsoft.com/blog/integrationsonazureblog/la-standard-options-for-connecting-to-custom-services-or-apis/3814148). |
