...
* If you're using an integration account, make sure that you've set up the account with Azure Monitor logging to send that information to a Log Analytics workspace. Learn how to [set up Azure Monitor logging for your integration account](../logic-apps/logic-apps-monitor-b2b-messages-log-analytics.md).
...
1. On any results page, at the bottom, select **See All**.
   ...
   ![See all results](./media/logic-apps-create-monitoring-tracking-queries/logic-app-see-all.png)

   The Logs page opens and shows the query behind the previous results page.

   ![Logs page - query view](./media/logic-apps-create-monitoring-tracking-queries/view-query-behind-results.png)
...
1. Start from an empty query or any available existing queries.
   ...
   * To check whether any existing queries are available, on the query toolbar, select either **Samples queries** > **History**, which shows queries from previous query runs, or select **Query explorer**, which shows prebuilt queries.
     ...
     ![Start with "Logic Apps B2B" solution prebuilt queries](./media/logic-apps-create-monitoring-tracking-queries/b2b-prebuilt-queries.png)
   ...
   * To start from an empty query, in the query editor, start typing the [Kusto query language](../azure-monitor/log-query/query-language.md) for your query.
     ...
     ![Start with empty query](./media/logic-apps-create-monitoring-tracking-queries/create-query-from-blank.png)
