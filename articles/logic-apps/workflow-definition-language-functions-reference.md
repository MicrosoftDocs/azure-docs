...
This example adds the property "accountNumber" to the "customerProfile" object, 
...
This example returns the array [1, 2, 3] for the string "[1, 2, 3]":

```json
json('[1, 2, 3]')
```

This example returns the specified input as a JSON type: {"fullName": "Sophia Owen"}
...
```json
json(<?xml version="1.0"?> <root> <person id='1'> <name>Sophia Owen</name> <occupation>Engineer</occupation> </person> </root>)
```
