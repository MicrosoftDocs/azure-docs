...
description: Compose XML documents in Azure Logic Apps with the XML Operations connector.
services: logic-apps
...
In enterprise integration business-to-business (B2B) or BizTalk migration scenarios, you might have to create XML documents from JSON data. Your logic app workflow can compose XML documents by using the **XML compose with schema** action. This action requires an XSD schema.
...
* A logic app workflow that already starts with a trigger so that you can add the **XML compose with schema** action where necessary in your workflow.
...
1. You can also **Switch to input entire array** if you want to use an array as input for the 'Root' element.
