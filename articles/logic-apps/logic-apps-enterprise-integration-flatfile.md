...
Each time that the flat file schema allows the choice of the next fragment, the Azure Logic Apps engine that parses the schema generates a *symbol* and a *prediction* for that fragment. If the schema contains too many such constructs, for example, more than 10,0001, the schema expansion becomes excessively large, which consumes too many resources and much time.
