...
### [Standard](#tab/standard)
   ...
   * **Logic App (Standard)** resource type: On the designer, select **Choose an operation**. On the **Choose an operation** pane that opens, under the search box, select **Built-in**. In the search box, enter `HTTP request`. From the triggers list, select the Request trigger named **When an HTTP request is received**.
