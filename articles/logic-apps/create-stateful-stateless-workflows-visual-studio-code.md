...
* You can deploy stateful and stateless workflows with the new **Logic App (Preview)** resource type only to a [Premium or App Service hosting plan in Azure](../azure-functions/functions-scale.md) or to a [Docker container](#deploy-docker), and not [integration service environments (ISEs)](../logic-apps/connect-virtual-network-vnet-isolated-environment-overview.md).
