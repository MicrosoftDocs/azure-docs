...
1. Trigger your logic app by sending an HTTP POST request with the 
message content to the URL generated by your HTTP request trigger. 
To the send the request, you can use a tool such as [Postman](https://www.getpostman.com/apps). 

   So, for the example in this article, the request sends an IDoc file, 
   which must be in XML format and includes the namespace from the SAP action: 
...
2. After you send your HTTP request, wait for the response from your logic app.
