...
- An Azure DevOps subscription. [Create an account for free](https://azure.microsoft.com/services/devops/?WT.mc_id=A261C142F).
...
1. In **Import a Git repository**, select or enter the following:
    - **Source type**: Git
    - **Clone URL**: `https://github.com/Azure/deployment-environments.git`
...
1. Under **Manage** in the sidebar menu, select **Projects**, and then select the project that you want to use.
...
1. Use the following steps to add the three environment types: *Sandbox*, *FunctionApp*, and *WebApp*. Add them one at a time.
   1. In **Add environment type to \<project-name\>**, enter or select the following information:
       ...
       |Name     |Value     |
       |---------|----------|
       |**Type**| Select a dev center-level environment type to enable for the project.|
       ...
       |**Environment creator role(s)**|  Select the roles to give access to the environment resources.|
...
1. Select **Project settings**. Under **Pipelines**, select **Service connections** > **Create service connection**.
...
1. Enter the following service connection details, and then select **Save** to create the service connection.
    ...
    | **Scope level for service connection** | Choose **Subscription**. |
...
1. On the **Role** tab, search for and select **Deployment Environments User** in the list of job function roles. Select **Next**.
...
1. On the **Role** tab, search for and select **Deployment Environments Reader** in the list of job function roles. Select **Next**.
...
1. In the **azure-pipelines.yml** file, edit the existing content:
   ...
   - In the `Inline script` input, replace each of the following placeholders with values that are appropriate to your Azure environment:
      ...
      | `<DevCenterProjectName>`        | The name of your project. |
      | `<DevCenterCatalogName>`        | The name of your catalog. |
      | `<environment-definition-name>` | Do not change. Defines the environment definition that's used. |
      | `<DevCenterProjectEnvironmentTypeName>`            | The environment type. |
      | `<DeploymentEnvironmentInstanceToCreateName>`      | Specify a name for your new environment. |
...
You can insert this job anywhere in a CI and/or CD pipeline. To learn more about creating and managing pipelines, see the [Azure Pipelines documentation](/azure/devops/pipelines/?view=azure-devops&preserve-view=true).
