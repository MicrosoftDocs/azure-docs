...
To add a new catalog, complete these tasks:

- Get the clone URL for your repository.
- Create a personal access token and store it as a key vault secret in Azure Key Vault.
- Connect your repository as a catalog.

### Get the clone URL for your repository

You can use one of two types of repositories:
...
#### Get the clone URL of your Azure DevOps repository
...
Next, create a personal access token and store it as a key vault secret in Azure Key Vault. Depending on the type of repository you use, create a personal access token either in GitHub or in Azure DevOps.
...
1. Ensure that the [identity](./how-to-configure-managed-identity.md) that's attached to the dev center has [access to the key vault secret](./how-to-configure-managed-identity.md#assign-the-managed-identity-access-to-the-key-vault-secret) where your personal access token is stored.
...
When you add or sync a catalog, you might encounter a sync error. A sync error indicates that some or all of the catalog items have errors. Use CLI or a REST API to GET the catalog. The GET response shows you the errors:
