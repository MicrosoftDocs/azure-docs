...
Please keep in mind that the partition count on an Event Hub can only be modified during setup. With that in mind, it is important to think about how many partitions you need before getting started. 
...
However, if you have an application model in which your application has an affinity to a particular partition, increasing the number of partitions may not be of any benefit to you. For more information on this please see [availability and consistency](event-hubs-availability-and-consistency.md).
