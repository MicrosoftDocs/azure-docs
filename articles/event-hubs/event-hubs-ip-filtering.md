...
description: How to use IP filtering to block connections from specific IP addresses for to Azure Event Hubs. You can block connections from individual or ranges of IP addresses.
services: event-hubs
documentationcenter: ''
author: clemensv
manager: timlt
...
In scenarios where Event Hubs shall only be accessible from certain well-known sites, the _IP filter_ feature enables you to configure rules for rejecting or accepting traffic originating from specific IPv4 addresses. These addresses may, for instance, be those of a corporate NAT gateway.

## When to use

Two important use-cases when it is useful to block the Event Hubs endpoints for certain IP addresses:
...
By default, the **IP Filter** grid in the portal for Event Hubs is empty. This default setting means that your hub accepts connections any IP address. This default setting is equivalent to a rule that accepts the 0.0.0.0/0 IP address range.
...
For constraining access to Event Hubs to Azure virtual networks see

- [Virtual Network Service Endpoints for Event Hubs][lnk-vnet]

<!-- Links -->

[express-route]:  https://azure.microsoft.com/documentation/articles/expressroute-faqs/#supported-services
