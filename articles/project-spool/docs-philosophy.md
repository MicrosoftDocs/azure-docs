...
- Spool is the product that we're building to expose our comms infrastructure to the broader market. The way that we do that is a product of ideas held by two teams: IC3 and Azure.
...
- **We can use docs to unify our thinking in service of our customer's need for simplicity.** To facilitate that, a docs skeleton has been created. We can all add meat to the skeleton that answers the most obvious developer questions: **How can this help me? How easy is it to use?**
...
- To make progress, **all primitive owners can contribute docs and samples**. Mick's role is not to create docs - it's editorial and docs-ops support.
