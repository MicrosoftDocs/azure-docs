...
|**source** | **EventGrid** | Set this property if you're using the Event Grid based blob trigger. In .NET, this can be done by setting the `Source = BlobTriggerSource.EventGrid` parameter. For more information, refer to the [Event Grid Blob Trigger guide](./functions-event-grid-blob-trigger.md).|
