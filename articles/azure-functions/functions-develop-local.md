...
When you develop your functions locally, you need to take trigger and binding behaviors into consideration. The simplest way test bindings to Azure services during local development and testing is to set connection strings to target live Azure services. You do this by adding the appropriate connection string settings in the `Values` array in the local.settings.json file. When you do this, local testing impacts live service data. Because of this, consider setting-up separate services to use during development and testing, and then switch to difference service during production. You could also use a local storage emulator.

## Local storage emulator

During local development, you can use the [Azurite emulator](/storage/common/storage-use-azurite.md) locally to test Azure Storage bindings (Queue Storage, Blob Storage, and Table Storage), without having to rely on remote services. Azurite integrates with Visual Studio Code and Visual Studio, and you can run it from from command prompt using npm. For more information, see [Use the Azurite emulator for local Azure Storage development](/storage/common/storage-use-azurite.md).
...
With this setting in place, any Azure storage binding that has its connection set to `AzureWebJobsStorage` uses Azurite when running locally. During local execution, you must have Azurite installed and running. When you publish your project, don't publish this setting. You need to instead use an Azure Storage connection string.
