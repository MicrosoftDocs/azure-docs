...
* *This article uses as its starting point the resources created in [Create your first function from the Azure portal](./functions-create-function-app-portal.md).
...
1. Click **AI + Machine Learning** > **Text Analytics**. Then, use the settings as specified in the table to create the resource.
...
1. Click **Overview** and copy the value of the **Endpoint** to a text editor. This value is used when creating a connection to the Cognitive Services API.

    ![Cognitive Services Settings](media/functions-twitter-email/02-cognitive-services.png)

1. In the left navigation column, click **Keys**, and then copy the value of **Key 1** and set it aside in a text editor. You use the key to connect the logic app to your Cognitive Services API.
...
1. From the **New Function** page, select **Create Function**.

1. In your new HTTP trigger function, select **Code + Test** from the left menu, replace the contents of the `run.csx` file with the following code, and then select **Save**:
...
1. To test the function, select **Test** from the top menu. On the **Input** tab, enter a value of `0.2` in the **Body**, and then select **Run**. A value of **RED** is returned in the **HTTP response content** on the **Output** tab.
...
1. Click **Web** > **Logic App**.

1. Then, type a value for **Name** like `TweetSentiment`, and use the settings as specified in the table.
...
1. Once you have entered the proper settings values, click **Create** to create your logic app.
...
1. Use the Twitter trigger settings as specified in the table. 
...
1. Click  **Save** to connect to your Twitter account. 
...
1. Type a connection name such as `MyCognitiveServicesConnection`, paste the key for your Cognitive Services API and the Cognitive Services endpoint you set aside in a text editor, and click **Create**.

    ![New Step, and then Add an action](media/functions-twitter-email/12-connection-settings.png)

1. Next, enter **Tweet text** in the text box and then click **New Step**.
...
1. Select the function app you created earlier.
...
1. Select the function you created for this tutorial.

    ![Select function](media/functions-twitter-email/16-select-function.png)

1. In **Request Body**, click **Score** and then **Save**.
...
1. Click **Choose a value**, then click **Body**. Select **is equal to**, click **Choose a value** and type `RED`, and click **Save**.

    ![Choose an action for the condition.](media/functions-twitter-email/19-condition-settings.png)

1. In **IF TRUE**, click **Add an action**, search for `outlook.com`, click **Send an email**, and sign in to your Outlook.com account.
...
1. In the **Send an email** action, use the email settings as specified in the table. 
...
1. Go to your function, view the logs, and verify that sentiment values were received and processed.
