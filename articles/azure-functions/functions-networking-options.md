...
When you create a function app, you must create or link to a general-purpose Azure Storage account that supports Blob, Queue, and Table storage.  You can replace this storage account with one that is secured with service endpoints or private endpoint.  This feature currently only works with Windows Premium and Standard plans (not for Consumption plan).  To set up a function with a storage account restricted to a private network:
