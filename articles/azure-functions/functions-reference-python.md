...
>[!NOTE]
>To use the OpenCensus Python extensions, you need to enable [Python worker extensions](#python-worker-extensions) in your function app by setting `PYTHON_ENABLE_WORKER_EXTENSIONS` to `1` and `APPLICATIONINSIGHTS_CONNECTION_STRING` to <Applicationinsights connection string from your ApplicationInsights resource>, in your [application settings](functions-how-to-use-azure-function-app-settings.md#settings).
