...
> [!NOTE]
> If your function app's requirements.txt contains an `azure-functions-worker` entry, we highly recommend removing it. The functions worker is automatically managed by Azure Functions platform and we regularly update it with new features and bug fixes. Manually installing a previous version may cause unexpected issues.
