...
> [!NOTE]
> When a function app that runs on the [Consumption plan](consumption-plan.md) or the [Premium plan](functions-premium-plan.md) is scaled, a new range of outbound IP addresses may be assigned. When running on either of these plans, you can't rely on the reported outbound IP addressed. To be able to allowlist all potential outbound addresses during dynamic scaling, you'll need to add the entire data center to your allowlist.
