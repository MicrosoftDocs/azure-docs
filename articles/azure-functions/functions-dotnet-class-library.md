...
SendGrid ApiKey is reqired. You can write `[SendGrid(ApiKey = "MySendGridKey")]` and add an app setting `"MySendGridKey": "SG..."`. When any ApiKey is not set, default app setting `"AzureWebJobsSendGridApiKey"` is used in the library. So, with that name in the app settings, a simple `[SendGrid]` works fine.
