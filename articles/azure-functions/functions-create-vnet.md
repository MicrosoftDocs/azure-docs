...
1. On the **Basics** tab, use the following table to configure the Service Bus settings. All other settings can use the default values.
    ...
    | **Pricing tier** | Premium | Select this tier to use private endpoints with Azure Service Bus. |
...
1. On the **IP Addresses** tab, select **Add subnet**. Use the following table to configure the subnet settings:
    ...
    | **Subnet address range** | 10.0.1.0/24 | The subnet address range. In the preceding image, notice that the IPv4 address space is 10.0.0.0/16. If the value is 10.1.0.0/16, the recommended subnet address range would be 10.1.1.0/24. |

1. Select **Review + create**. After validation finishes, select **Create**.

## Lock down your storage account

Azure private endpoints are used to connect to specific Azure resources by using a private IP address. This connection ensures that the network traffic remains within the chosen virtual network and access is available only for specific resources.
...
1. On the **Basics** tab, use the private endpoint settings shown in the following table:
    ...
    | ------------ | ---------------- | ---------------- |
    | **Subscription** | Your subscription | The subscription under which your resources are created. |
    | **[Resource group](../azure-resource-manager/management/overview.md)**  | myResourceGroup | Select the resource group you created with your function app. |
    | **Name** | file-endpoint | The name of the private endpoint for files from your storage account. |
    | **[Region](https://azure.microsoft.com/regions/)** | myFunctionRegion | Select the region where you created your storage account. |
...
1. On the **Configuration** tab, for the **Subnet** setting, select **default**.
...
1. Select **Review + create**. After validation finishes, select **Create**.
1. After the private endpoint is created, return to the **Public access** section of your Service Bus namespace.
...
1. Ensure that the **Route All** configuration setting is set as **Enabled**.
...
1. In the menu on the left, select **Service Bus Explorer**. Under **Send**, for **Content Type**, select **Text/Plain**. Then enter a message.
