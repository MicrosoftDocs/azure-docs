...
If all Function execution succeed without errors, checkpoints are added to the associated storage account. When check-pointing succeeds, all 1000 messages should never be retrieved again.

* **Add N more function instances** - Azure Functions' scaling logic determines that both Function_0 and Function_1 have more messages than they can process, it will scale again for Function_2...N, where N is greater than Event Hub paritions. Event Hub will load balance the partitions across Function_0...9 instances.
