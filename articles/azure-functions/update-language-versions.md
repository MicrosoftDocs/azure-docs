...
#customer intent: As a developer who supports functions in Azure Functions, I want to update the language stack of my apps so that I can take advantage of new language features.
---

# Update language stack versions in Azure Functions 

In Azure Functions, support for a language stack is limited to [specific versions](functions-versions.md#languages). As new versions become available, you might want to update your apps to take advantage of their features. Support in Functions can also end for older versions, typically aligned to the community end-of-support timelines. For more information, see the [language runtime support policy](./language-support-policy.md). 

To ensure your apps continue to receive support, follow the instructions in this article to update them to the latest available versions. The way that you update your function app depends on several factors:

- The language you use to write your functions. Make sure to choose your programming language at the top of this article. 
...
> [!NOTE]
> This article shows you how to update the .NET version of an app that uses the [isolated worker model](dotnet-isolated-process-guide.md). Apps that run on older versions of .NET with [the in-process model](functions-dotnet-class-library.md) can [update to target .NET 8](./functions-dotnet-class-library.md#updating-to-target-net-8), or they can [migrate from the in-process model to the isolated worker model](migrate-dotnet-to-isolated-model.md).
...
Use the following [az functionapp config appsettings list](/cli/azure/functionapp/config/appsettings#az-functionapp-config-appsettings-list) command to check your runtime version: 
...
> [!TIP]  
> To streamline the update process, minimize downtime for your functions, and provide a potential version for rollback, you should publish your updated app to a staging slot. For more information, see [Azure Functions deployment slots](functions-deployment-slots.md#add-a-slot). 
...
The way that you update the stack configuration depends on whether your app runs on Windows or on Linux in Azure.
...
> [!NOTE]  
> If you have a function app on Linux that's hosted in a [Premium plan](./functions-premium-plan.md) or a [Dedicated (App Service) plan](./dedicated-plan.md), you can use the Azure portal to update your app. But if you have an app on Linux that's hosted in a [Consumption plan](./consumption-plan.md), use the [Azure CLI](update-language-versions.md?tabs=azure-cli#update-the-stack-configuration).
...
1. On the **General settings** tab, set the **Python version** value to the desired version. 
   ...
   > [!NOTE]   
   > You can't change the Python version when your app runs in a Consumption plan.  
...
Run the following [az functionapp list-runtimes](/cli/azure/functionapp#az-functionapp-list-runtimes) command to view the supported version values for your language. Then, run the following [az functionapp config set](/cli/azure/functionapp/config#az-functionapp-config-set) command to update the language version of your function app:

::: zone-end

::: zone pivot="programming-language-csharp" 

Run the following [az functionapp list-runtimes](/cli/azure/functionapp#az-functionapp-list-runtimes) command to view the supported version values for .NET on the isolated worker model:
...
Run the following [az functionapp config set](/cli/azure/functionapp/config#az-functionapp-config-set) command to update the .NET version of your function app:
...
Use the following [az functionapp list-runtimes](/cli/azure/functionapp#az-functionapp-list-runtimes) command to view the supported version values for your language stack (Node.js). Then, run the following [az functionapp config set](/cli/azure/functionapp/config#az-functionapp-config-set) command to update the Node.js version of your function app:
...
In the preceding command, replace `<APP_NAME>` and `<RESOURCE_GROUP>` with the name of your function app and resource group, respectively. Also replace `<VERSION>` with the supported language version to which you're updating. If you aren't using a staging slot, remove the `--slot` parameter.
...
Run the following [az functionapp list-runtimes](/cli/azure/functionapp#az-functionapp-list-runtimes) command to view the supported [linuxFxVersion](functions-app-settings.md#linuxfxversion) site setting for your language version:

::: zone-end

::: zone pivot="programming-language-csharp,programming-language-javascript,programming-language-typescript"

Run the following [az functionapp list-runtimes](/cli/azure/functionapp#az-functionapp-list-runtimes) command to view the supported [linuxFxVersion](functions-app-settings.md#linuxfxversion) site setting for your language stack version:
...
Run the following [az functionapp config set](/cli/azure/functionapp/config#az-functionapp-config-set) command to update the site setting for the new language version of your function app: 

::: zone-end

::: zone pivot="programming-language-csharp,programming-language-javascript,programming-language-typescript"

Run the following [az functionapp config set](/cli/azure/functionapp/config#az-functionapp-config-set) command to update the site setting for the new stack version of your function app:
...
In the preceding command, replace `<APP_NAME>` and `<RESOURCE_GROUP>` with the name of your function app and resource group, respectively. Also replace `<LANGUAGE|VERSION>` with the `linuxFxVersion` value for your update. If you aren't using a staging slot, remove the `--slot` parameter.
...
If you used a staging slot to deploy your code project and update settings, swap the staging slot into production. For more information, see [Swap slots](functions-deployment-slots.md#swap-slots). 
