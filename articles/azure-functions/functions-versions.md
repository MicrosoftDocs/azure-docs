...
- Application Insights is no longer included by default in 4.x. Application Insights is now available as a separate extension.
    ...
    - The extension no longer supports the `APPINSIGHTS_INSTRUMENTATIONKEY` app setting. You must now supply a [connection string](../azure-monitor/app/sdk-connection-string) in an app setting named `APPINSIGHTS_CONNECTION_STRING`.
