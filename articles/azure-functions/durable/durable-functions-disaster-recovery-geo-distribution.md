...
> [!NOTE]
> - The proposed Active-Passive configuration ensures that a client is always able to trigger new orchestrations via HTTP. However, as a consequence of having two function apps sharing the same task hub in storage, background processing will be distributed between both of them. This configuration incurs in added egress costs for the secondary function app.
