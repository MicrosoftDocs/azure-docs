...
You should also have a disaster recovery plan in place for handling a regional service outage. An important part of a disaster recovery plan is being prepared to fail over to the secondary replicas of both your app and your storage account when the primary replicas becomes unavailable.

This article describes example scenarios for configuring disaster recovery and geo-distribution in the Durable Functions feature of Azure Functions.

## Background

In Durable Functions, all state is persisted in Azure Storage by default. A [task hub](durable-functions-task-hubs.md) is a logical container for Azure Storage resources that are used for [orchestrations](durable-functions-types-features-overview.md#orchestrator-functions) and [entities](durable-functions-types-features-overview.md#entity-functions). Orchestrator, activity, and entity functions can only interact with each other when they belong to the same task hub. This article refers to task hubs when describing scenarios for keeping these Azure Storage resources highly available.
...
- The proposed active/passive configuration ensures that a client is always able to trigger new orchestrations via HTTP. However, when two function apps share the same task hub in storage, some background storage transactions can be distributed between both apps. As a result of this distribution, this configuration can result in added egress costs for the secondary function app.
...
- All function apps that are redundantly deployed must share the same function access keys when they're activated via HTTP. The Functions runtime exposes a [management API](https://github.com/Azure/azure-functions-host/wiki/Key-management-API) that you can use to programmatically add, delete, and update function keys. You can also manage keys by using [Azure Resource Manager APIs](https://www.markheath.net/post/managing-azure-functions-keys-2).
...
Traffic Manager is configured to detect problems in the primary function app and automatically redirect traffic to the function app in the secondary region. This function app shares the same Azure Storage account and task hub. Therefore, the state of the function apps isn't lost and work can resume normally. After health is restored to the primary region, Azure Traffic Manager starts routing requests to that function app automatically.
...
- This scenario depends on Traffic Manager. It can take some time before a client application needs to again request the function app address from Traffic Manager. For more information, see [How Traffic Manager works](../../traffic-manager/traffic-manager-how-it-works.md).
...
>
> For extension versions before v2.3.0, function apps that are configured to use the same storage account processes messages and update storage artifacts concurrently. This concurrent processing results in higher overall latencies and egress costs. If the primary and replica apps ever have different code deployed to them, even temporarily, then orchestrations could also fail to run correctly because of orchestrator function inconsistencies across the two apps.
>
> All apps that require geo-distribution for disaster recovery should use v2.3.0 or later of the Durable Functions extension.
...
To ensure continuous operation of durable functions, this scenario deploys a dedicated storage account or a durable task scheduler in each region where function apps are hosted. We currently recommend this disaster recovery approach when you're using a durable task scheduler.
...
- **No added latency on failover**: During a failover, a function app and state provider (storage or durable task scheduler) are colocated, so there's no added latency in the failover region.
...
To summarize, the second scenario preserves latency and minimizes egress costs, but existing orchestrations and entities are unavailable during the downtime. Whether this tradeoff is acceptable depends on the requirements of the application.
...
Functionally, this scenario provides the same advantages as Scenario 1, but it also enables other data recovery advantages:
...
> [!NOTE]
> As described for Scenario 1, we recommend that function apps deployed with this strategy use v2.3.0 or later of the Durable Functions extension.
