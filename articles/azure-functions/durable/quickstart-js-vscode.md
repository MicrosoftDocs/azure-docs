---
title: Create your first durable function in Azure by using JavaScript
...
* Install the [Azure Functions](https://marketplace.visualstudio.com/items?itemName=ms-azuretools.vscode-azurefunctions) Visual Studio Code extension version 1.10.4 or later.
...
* Make sure that you have [Azure Functions Core Tools](../functions-run-local.md) version 4.0.5382 or later.
...
You've added an HTTP-triggered function that starts an orchestration. Open *DurableFunctionsHttpStart/index.js* to see that it uses `client.startNew` to start a new orchestration. Then it uses `client.createCheckStatusResponse` to return an HTTP response that contains URLs that you can use to monitor and manage the new orchestration.
...
1. Paste this new URL for the HTTP request into your browser's address bar. You should get the same status response as before when you use the published app.
