...
This document will guide you through the step by step process of creating an Open API Definition for a simple API hosted on Azure Functions.
...
#### <a name="prepare-function"></a>Creating a Function with a simple API
  In order to create an Open API definition, we first need to create a Function with a simple API. If you already have an API hosted on a Function App, you can skip straight to the next section
...
1. Create a new HTTP trigger function inside your new Function App
  1. Your function will be pre-populated with code defining a very simple REST API.
...
1. Go to the `Integrate` tab of your new HTTP Trigger function
  ...
  1. This will simplify your API definition later on.

#### <a name="enable"></a>Enabling API Definition Support
1. Navigate to `API Definition (preview)`
1. Set `Swagger Source` to `Internal`
  1. This will enable a suite of Open API options for your Function App, including an endpoint to host a Swagger file from your Function App's domain, an inline copy of the [Swagger Editor](http://editor.swagger.io), and a quickstart definition generator.

#### <a name="create-definition"></a>Creating your API Definition from a template
1. Click `Load Generated API Definition`
  1. This will scan your Function App for HTTP Trigger functions and use the info in functions.json to generate a Swagger document.
1. Add an operation object to `paths: /api/yourfunctionroute post:`
  ...
  1. This the sample operation object below has a filled out produces/consumes section, a parameter object, and a response object.
  ```yaml
  post:
...
1. You can import your Swagger document to any number of tools for testing and integration using that URL.
  1. Many Azure resources will be able to automatically import your swagger using the API Definition URL that is saved in your Function application settings. As a part of the internal Swagger Source, we update that url for you.
