...
> [!NOTE]
> Event Grid triggers are not natively supported in Internal Load Balancer (ILB) App Service Environments (ASE). The trigger uses an HTTP request that cannot reach the function app without a gateway into the VNET.
