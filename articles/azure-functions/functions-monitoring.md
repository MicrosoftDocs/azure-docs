...
On the [Failures](../application-insights/app-insights-asp-net-exceptions.md) tab, you can create charts and alerts based on function failures and server exceptions. The **Operation Name** is the function name.
...
The image shows a list of traces (logs) generated by the Functions runtime.

The tables that are available are shown in the **Schema** tab of the left pane. The ones that include data generated by function invocations are the following:
...
The Azure functions logger also includes a *log level* with every log. The log level is associated with an integer code that indicates relative importance:
...
To suppress all logs for a category, you can use log level `None` because no logs are written with that category and there is no log level above it.
...
## Write logs in C# function code

You can write logs in function code that are handled by Application Insights the same way as logs that the runtime creates.

### ILogger

Use an `ILogger` parameter in your functions instead of a `TraceWriter` parameter. You can then use `Log<level>` methods to create logs. For example, the following code writes `Information` logs with category "Function".
...
Placeholders are handled this way so that you can do [semantic logging, also known as structured logging](https://softwareengineering.stackexchange.com/questions/312197/benefits-of-structured-logging-vs-basic-logging). Because the arguments themselves are passed to the logging system, not just the formatted message string, Application Insights can store the parameter values as fields in addition to the message string. You can then query on those fields.

For example, if your logger method call looks like the previous example, you could write a query in Application Insights Analytics looking for a specific rowKey value without having to parse message text.

### Write logs in JavaScript function code
...
## Create custom telemetry data in C# function code
...
## Create custom telemetry data in JavaScript function code
