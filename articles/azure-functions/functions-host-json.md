...
The same will work if you define these values in an `"environmentVariables"` section of your launchSettings.json file if you wish to specify them per-profile instead of using app settings file.

Overriding host.json settings using environment variables follows the ASP.NET Core naming conventions. When the element structure includes an array, the numeric array index should be treated as an additional element name in this path. For more information, see [Naming of environment variables](/aspnet/core/fundamentals/configuration/#naming-of-environment-variables). For example: `{"version": "2.0", "functions": [ "myFunc1", "myFunc2" ]}` must be split into `"AzureFunctionsJobHost__functions__0": "myFunc1"` and `"AzureFunctionsJobHost__functions__1": "myFunc2"` as environment variables.
