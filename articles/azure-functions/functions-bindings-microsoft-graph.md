...
|**userId**	|Needed if and only if _identity_ is set to `userFromId`. A user principal ID associated with a previously logged-in user.|
|**userToken**|Needed if and only if _identity_ is set to `userFromToken`. A token valid for the function app. |
|**action**|Required - specifies the action the binding should perform. The value can be one of the following values:<ul><li><code>create</code>&mdash;Registers a new subscription.</li><li><code>delete</code>&mdash;Deletes a specified subscription.</li></ul>|
