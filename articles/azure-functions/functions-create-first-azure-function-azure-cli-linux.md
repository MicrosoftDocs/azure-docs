...
To continue creating a function within the function app project, make the function app project directory the current working directory.
...
As outlined in [Azure Functions scale and hosting](functions-scale.md), there are two ways in which a function can be hosted:
...
In order to create your function on a regular App Service plan, execute:

```azurecli-interactive
az functionapp create --resource-group myResourceGroup --plan <plan_name>  --os-type Linux \
--name <app_name> --storage-account  <storage_name> --runtime <language>
```

If instead your function is to be created on a consumption plan, execute:
...
> [!NOTE]
> If you have an existing resource group named `myResourceGroup` with any non-Linux App Service apps, you must use a different resource group. You can't host both Windows and Linux apps in the same resource group.  In the same way, a function running on a consumption plan cannot be deployed in a resource group which already has a regular App Service plan deployed.
