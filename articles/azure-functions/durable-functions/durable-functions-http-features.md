...
> [!NOTE]
> - Orchestrator functions also natively support the server-side polling consumer pattern, as described in [Async operation tracking](#async-operation-tracking). This support means that orchestrations in one function app can easily coordinate the orchestrator functions in other function apps. This is similar to the [sub-orchestration](../../durable-task/common/durable-task-sub-orchestrations.md) concept, but with support for cross-app communication. This support is particularly useful for microservice-style app development.
...
> [Learn about durable entities](../../durable-task/common/durable-task-entities.md)
