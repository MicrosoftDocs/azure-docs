...
You see the `FUNCTIONS_WORKER_RUNTIME` in the following output, which has been truncated for clarity:
...
To set your app to run on isolated mode, you can update the `FUNCTIONS_WORKER_RUNTIME` setting in the function app with the [az functionapp config set](/cli/azure/functionapp/config) command. 
...
You can view and set the `netFrameworkVersion` from the Azure CLI.  

Using the Azure CLI, view the current `netFrameworkVersion` with the [az functionapp config show](/cli/azure/functionapp/config) command.
...
You see the `netFrameworkVersion` in the following output, which has been truncated for clarity:
...
```output
{
  ...
  "minTlsVersion": "1.2",
  "name": "net5testing20210624115430",
...
You can update the `netFrameworkVersion` setting in the function app with the [az functionapp config set](/cli/azure/functionapp/config) command.

```azurecli-interactive
az functionapp config set --name <FUNCTION_APP> \
--resource-group <RESOURCE_GROUP> \
--net-framework-version <NET_FRAMEWORK_VERSION>
```

Replace `<FUNCTION_APP>` with the name of your function app. Also replace `<RESOURCE_GROUP>` with the name of the resource group for your function app. Also, replace `<NET_FRAMEWORK_VERSION>` with the values desired. For example, to update your language runtime to .NET 5, set `netFrameworkVersion` to `v5.0`.
