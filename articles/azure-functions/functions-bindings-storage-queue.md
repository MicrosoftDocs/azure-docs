...
|messageEncoding|base64| This setting is only available in Storage Queues extension version 5.x and higher. For .NET apps, this means NuGet package [Microsoft.Azure.Functions.Worker.Extensions.Storage.Queues](https://www.nuget.org/packages/Microsoft.Azure.Functions.Worker.Extensions.Storage.Queues) 5.x+. For other languages using [extension bundles](./extension-bundles.md), `messageEncoding` is supported in bundle version 4.x. It represents the encoding format for messages. Valid values are `base64` and `none`.|
