...
|messageEncoding|base64| This setting is only available in [Storage extension version 5.x and higher](#storage-extension-5x-and-higher) (this refers to the NuGet package version, not the extension bundle version). When using [extension bundles](./extension-bundles.md), `messageEncoding` is supported in bundle version 4.x. It represents the encoding format for messages. Valid values are `base64` and `none`.|
