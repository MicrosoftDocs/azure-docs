...
|**runOnStartup**|**RunOnStartup**|If `true`, the function is invoked when the runtime starts. For example, the runtime starts when the function app wakes up after going idle due to inactivity. when the function app restarts due to function changes, and when the function app scales out. A function app's runtime does not necessarily restart as the result of a deployment. For information about how to reliably trigger a timer-triggered function outside of the timer schedule, see [Trigger outside of schedule](#trigger-outside-of-schedule) later in this article.|
...
## Trigger outside of schedule

You might want to trigger a function in response to some event other than the schedule. For example, you want it to run after a deployment. To do that, you can create a second function and share the code between the two functions. The second function can then be triggered in whatever way you need. For the deployment example, you can [customize your deployment](https://github.com/projectkudu/kudu/wiki/Customizing-deployments) to invoke the second function by making an HTTP request when the deployment completes.
