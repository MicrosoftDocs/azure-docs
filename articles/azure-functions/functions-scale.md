...
You can host Azure Functions instances running in Linux containers in both Premium and Dedicated hosting plans. However, it might be a better option to instead deploy containerized function apps to Kubernetes clusters or to Azure Container Apps. To directly compare the various hosting options for running in containers, see [Linux container support in Azure Functions](container-concepts.md).  
...
| Plan | Benefits |
| --- | --- |  
|**[Consumption plan]**| Scales automatically and you only pay for compute resources when your functions are running.<br/><br/>On the Consumption plan, instances of the Functions host are dynamically added and removed based on the number of incoming events.<br/><br/> ✔ Default hosting plan that provides true _serverless_ hosting.<br/>✔ Pay only when your functions are running.<br/>✔ Scales automatically, even during periods of high load.|  
|**[Flex Consumption plan]**| Scales automatically and you pay just for compute resources when your functions are running, plus any _always ready_ instances.<br/><br/>On the Flex Consumption plan, instances of the Functions host are dynamically added and removed based on the number of incoming events. <br/><br/> ✔ Reduce cold starts by specifying a number of pre-provisioned (always ready) instances.<br/> ✔ Supports virtual networking for added secrity.<br/>✔ Pay when your functions are running.<br/>✔ Scales automatically, even during periods of high load.| 
...
| **[Flex Consumption plan]** | ✅ Code-only<br/>❌ Container (not supported) | ✅ Code-only |
...
| **[Flex Consumption plan]** | Billing is based on number of executions, execution time, and memory used, plus the cost of any [always ready instances](./flex-consumption-plan.md#always-ready-instances). For more information, see [Flex Consumption plan billing](flex-consumption-plan.md#billing).
