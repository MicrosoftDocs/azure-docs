...
The default timeout for the connection is 2 hours. You can change the timeout by adding an app setting named [SCM_LOGSTREAM_TIMEOUT](functions-app-settings.md#scm_logstream_timeout), with a timeout value in seconds. Not yet supported for Linux in a [Flex Consumption](../articles/azure-functions/flex-consumption-plan.md) or [Consumption](../articles/azure-functions/consumption-plan.md) plan. For these apps, use the `--browser` option to view logs in the portal.
