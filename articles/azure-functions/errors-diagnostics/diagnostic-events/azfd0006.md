...
+ Renew the SAS token so that it doesn't expire within the next 45 days and set the new value in application settings. For more information, see [Manually upload a package to Azure Blob Storage](../../deployment-zip-push.md#manually-upload-a-package-to-azure-blob-storage).

+ Switch to using managed identities instead of relying on an SAS URI. For more information, see [Fetch a package from Azure Blob Storage by using a managed identity](../../deployment-zip-push.md#fetch-a-package-from-azure-blob-storage-by-using-a-managed-identity).
