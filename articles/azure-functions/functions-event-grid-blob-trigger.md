...
1. Open the function_app.py project file, and add `source = "EventGrid"` to the `blob_trigger` decorator. 
...
1. Create a new endpoint URL for the Blob Storage trigger based on the following example: 
    ...
    In this example, replace `<FUNCTION_APP_NAME>` with the name of your function app, `<BLOB_EXTENSION_KEY>` with the value you got from the portal, and `<FUNCTION_NAME>` with your function name.
