...
The Functions runtime receives a message in [PeekLock mode](../service-bus-messaging/service-bus-performance-improvements.md#receive-mode). It calls `Complete` on the message if the function finishes successfully, or calls `Abandon` if the function fails.

If the function runs longer than the `PeekLock` timeout, the lock is automatically renewed as long as the function is running. If the function app is on the Consumption plan, there is a limit to renewals because there is a limit to how long a function can run. The default timeout for functions is 5 minutes. The value can be increased to 10 minutes for the function app by changing the property `functionTimeout` in the *host.json* file. When a function times out, the queue message becomes visible in the queue and will be picked up by another function instance. If your functions need to do work that takes longer than 10 minutes, and the default behavior for long-running functions is not what you want, consider using [Durable Functions](durable-functions-overview.md). 
