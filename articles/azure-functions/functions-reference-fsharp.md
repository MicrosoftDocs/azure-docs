...
The following example shows a [trigger binding](./functions-bindings-http-webhook.md#trigger) in a *function.json* file and an F# function that uses the binding. The function looks for a `name` parameter either in the query string or the body of the HTTP request.
...
The [HTTP triggers and bindings configuration](./functions-bindings-http-webhook.md#trigger---configuration) section explains these properties.
