...
In SignalR Service serverless mode, SignalR Service use the [Upstream](../azure-signalr/concept-upstream.md) feature to send messages from client to Function App. And Function App use SignalR Service trigger binding to handle these messages. The general architecture shown as below:
...
After you set `parameterNames`, the name you defined will respectively correspond to the arguments sent in client side. 
...
The `Function_App_URL` can be found at Function App's Overview page and The `API_KEY` is generated by Azure Function. You can get the `API_KEY` from `signalr_extension` in the **App keys** blade of Function App.
