...
The Azure Functions Premium plan is a dynamic scale hosting option for function apps. For other hosting plan options, see the [hosting plan article](functions-scale.md).

>[!IMPORTANT]
>Azure Functions runs on the Azure App Service platform. In the App Service platform, the plan that hosts Premium plan function apps is referred to as an *Elastic* Premium plan, with an SKU name like `EP1`. If you choose to run your function app on a Premium plan in App Service, make sure not to create a plan with an SKU name that starts with "P", such as `P1V2`, which is a Premium V2 Small plan. These Premium App Service plans are actually Dedicated plans, and they won't scale dynamically. Scalable Premium plans used by Functions have SKU names that start with "E."
