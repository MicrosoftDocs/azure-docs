...
| MellanoxRxCorrectedRate | Mellanox | Corrected Bit Rate | Percent | Rate of corrected bits by the network device, the rate is determined by the number of corrected bits divided by the number of RX bytes recieved in the given time frame. In the absence of data, this metric will retain the most recent value emitted | Host, Interface Name |
