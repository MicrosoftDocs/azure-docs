...
- A `replace` on a healthy (powered on, ready, provisioned, joined to cluster) Bare Metal Machine is rejected under the default safeguard mode. If a `replace` is truly required (for example after hardware component replacement), then this rejection can be overridden with `--safeguard-mode None`.
- Validate that there are no running firmware upgrade jobs.
  Follow steps in section [Determine if Firmware Update Jobs are Running](#determine-if-firmware-update-jobs-are-running).
