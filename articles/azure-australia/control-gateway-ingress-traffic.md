...
These documents outline the context for implementing network security and controlling traffic and provide practical recommendations for network design and configuration. Microsoft has published [Cloud Computing Security for Tenants of Microsoft Azure](https://aka.ms/au-irap) in the Australian page of the Service Trust Portal to highlight specific Microsoft technologies that enable out customers to meet the advice in the ACSC publication.

The following key requirements have been identified in the publications from the ACSC for controlling ingress traffic in Azure:
