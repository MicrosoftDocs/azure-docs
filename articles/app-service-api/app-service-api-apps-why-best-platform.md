...
- **Simple access control** - You can protect an API app from unauthenticated access with no changes to your code. The App Service SDK also enables you to use a simplified syntax for authorization code. Built-in authentication services shared with Web Apps and Mobile Apps can secure APIs for access by clients or by service principals. Supported identity providers include Azure Active Directory and third-party providers such as Facebook and Twitter. For more information, see [Authentication for API apps and mobile apps in Azure App Service](../app-service/app-service-authentication-overview.md).
