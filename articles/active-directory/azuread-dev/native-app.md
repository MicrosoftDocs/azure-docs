---
title: Native apps in Microsoft Entra ID
...
1. Using a browser pop-up, the native application makes a request to the authorization endpoint in Microsoft Entra ID. This request includes the Application ID and the redirect URI of the native application as shown in the Azure portal, and the application ID URI for the web API. If the user hasn't already signed in, they are prompted to sign in again
1. Microsoft Entra authenticates the user. If it is a multi-tenant application and consent is required to use the application, the user will be required to consent if they haven't already done so. After granting consent and upon successful authentication, Microsoft Entra ID issues an authorization code response back to the client application's redirect URI.
1. When Microsoft Entra ID issues an authorization code response back to the redirect URI, the client application stops browser interaction and extracts the authorization code from the response. Using this authorization code, the client application sends a request to Microsoft Entra ID's token endpoint that includes the authorization code, details about the client application (Application ID and redirect URI), and the desired resource (application ID URI for the web API).
1. The authorization code and information about the client application and web API are validated by Microsoft Entra ID. Upon successful validation, Microsoft Entra ID returns two tokens: a JWT access token and a JWT refresh token. In addition, Microsoft Entra ID returns basic information about the user, such as their display name and tenant ID.
...
> [!NOTE]
> The refresh token issued by Microsoft Entra ID can be used to access multiple resources. For example, if you have a client application that has permission to call two web APIs, the refresh token can be used to get an access token to the other web API as well.
