---
title: Microsoft Entra ID for developers (v1.0)
description: Learn authentication basics for Microsoft Entra ID for developers (v1.0) such as the app model, API, provisioning, and the most common authentication scenarios.
...
ms.custom: aaddev
ROBOTS: NOINDEX
#Customer intent: As an application developer, I want to learn about the basic authentication concepts in Microsoft Entra ID for developers (v1.0), including the app model, API, provisioning, and supported scenarios, so I understand what I need to do when I create apps that integrate Microsoft sign-in.
---
...
Microsoft Entra ID for developers (v1.0) (Microsoft Entra ID) simplifies authentication for application developers by providing identity as a service, with support for industry-standard protocols such as OAuth 2.0 and OpenID Connect, as well as open-source libraries for different platforms to help you start coding quickly.

There are two primary use cases in the Microsoft Entra programming model:
...
<a name='authentication-basics-in-azure-ad'></a>

## Authentication basics in Microsoft Entra ID
...
* Microsoft Entra ID is the identity provider. The identity provider is responsible for verifying the identity of users and applications that exist in an organization's directory, and issues security tokens upon successful authentication of those users and applications.
* An application that wants to outsource authentication to Microsoft Entra ID must be registered in Microsoft Entra ID. Microsoft Entra registers and uniquely identifies the app in the directory.
* Developers can use the open-source Microsoft Entra authentication libraries to make authentication easy by handling the protocol details for you. For more info, see Microsoft identity platform [v2.0 authentication libraries](../develop/reference-v2-libraries.md?toc=/azure/active-directory/azuread-dev/toc.json&bc=/azure/active-directory/azuread-dev/breadcrumb/toc.json) and [v1.0 authentication libraries](active-directory-authentication-libraries.md).
...
Now that you have an overview of the basics, read on to understand the identity app model and API, how provisioning works in Microsoft Entra ID, and links to detailed info about the common scenarios that Microsoft Entra ID supports.

## Application model

Microsoft Entra ID represents applications following a specific model that's designed to fulfill two main functions:

* **Identify the app according to the authentication protocols it supports** - This involves enumerating all the identifiers, URLs, secrets, and related information that are needed at authentication time. Here, Microsoft Entra ID:
    ...
    * Provides the infrastructure for implementing app provisioning within the app developer's tenant and to any other Microsoft Entra tenant.

* **Handle user consent during token request time and facilitate the dynamic provisioning of apps across tenants** - Here, Microsoft Entra ID:
...
In Microsoft Entra ID, an **application object** describes an application as an abstract entity. Developers work with applications. At deployment time, Microsoft Entra ID uses a given application object as a blueprint to create a **service principal**, which represents a concrete instance of an application within a directory or tenant. It's the service principal that defines what the app can actually do in a specific target directory, who can use it, what resources it has access to, and so on. Microsoft Entra ID creates a service principal from an application object through **consent**.

The following diagram shows a simplified Microsoft Entra provisioning flow driven by consent.  In it, two tenants exist (A and B), where tenant A owns the application, and tenant B is instantiating the application via a service principal.  
...
1. Microsoft Entra ID uses the application object in tenant A as a blueprint for creating a service principal in tenant B
...
<a name='claims-in-azure-ad-security-tokens'></a>

## Claims in Microsoft Entra security tokens

Security tokens (access and ID tokens) issued by Microsoft Entra ID contain claims, or assertions of information about the subject that has been authenticated. Applications can use claims for various tasks, including:
...
A brief description of each type of claim emitted by Microsoft Entra ID is provided in the table below. For more detailed information, see the [access tokens](../develop/access-tokens.md?toc=/azure/active-directory/azuread-dev/toc.json&bc=/azure/active-directory/azuread-dev/breadcrumb/toc.json) and [ID tokens](../develop/id-tokens.md?toc=/azure/active-directory/azuread-dev/toc.json&bc=/azure/active-directory/azuread-dev/breadcrumb/toc.json) issued by the Microsoft Entra ID.
...
| First Name | Provides the given name of the user as set in Microsoft Entra ID. |
| Groups | Contains object IDs of Microsoft Entra groups that the user is a member of. |
...
| Issuer | Identifies the STS that emitted the token as well as the Microsoft Entra tenant. |
| Last Name | Provides the surname of the user as set in Microsoft Entra ID. |
| Name | Provides a human readable value that identifies the subject of the token. |
| Object ID | Contains an immutable, unique identifier of the subject in Microsoft Entra ID. |
| Roles | Contains friendly names of Microsoft Entra Application Roles that the user has been granted. |
