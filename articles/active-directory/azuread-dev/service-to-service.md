---
title: Service-to-service apps in Microsoft Entra ID
...
1. First, the server application needs to authenticate with Microsoft Entra ID as itself, without any human interaction such as an interactive sign-on dialog. It makes a request to Microsoft Entra ID's token endpoint, providing the credential, Application ID, and application ID URI.
1. Microsoft Entra authenticates the application and returns a JWT access token that is used to call the web API.
