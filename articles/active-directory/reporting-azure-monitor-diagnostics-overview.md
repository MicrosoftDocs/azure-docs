...
**A**: The sign-in activity logs and audit logs are both available for routing through this feature, although B2C-related audit events are currently not included. To find out which types of logs and which feature-based logs are currently supported, see [Audit log schema](reports-monitoring/reference-azure-monitor-audit-log-schema.md) and [Sign-in log schema](reporting-azure-monitor-diagnostics-sign-in-log-schema.md). 
...
**A**: Currently, Azure Monitor is supported by Splunk, QRadar, and Sumo Logic. However, Splunk is the only SIEM tool that's supported for Azure AD logs. For more information about how the connectors work, see [Stream Azure monitoring data to an event hub for consumption by an external tool](../monitoring-and-diagnostics/monitor-stream-monitoring-data-event-hubs.md).
...
* [Archive activity logs to a storage account](reports-monitoring/quickstart-azure-monitor-route-logs-to-storage-account.md)
* [Route activity logs to an event hub](reports-monitoring/quickstart-azure-monitor-stream-logs-to-event-hub.md)
