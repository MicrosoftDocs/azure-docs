...
| Name | Description | Example SAML Value | Example JWT Value |
|------|-------|-------------|-----------------|
...
| Audience | The intended recipient of the token. The application that receives the token must verify that the audience value is correct and reject any tokens intended for a different audience. | `<AudienceRestriction>`<p>`<Audience>`<p>`https://contoso.com`<p>`</Audience>`<p>`</AudienceRestriction>` | `"aud":"https://contoso.com"` |
| Application Authentication Context Class Reference | Indicates how the client was authenticated. For a public client, the value is 0. If client ID and client secret are used, the value is 1. ||`"appidacr": "0"`|
| Authentication Context Class Reference | Indicates how the subject was authenticated, as opposed to the client in the Application Authentication Context Class Reference claim. A value of "0" indicates the end-user authentication did not meet the requirements of ISO/IEC 29115. | |`"acr": "0"`|
| Authentication Instant | Records the date and time when authentication occurred. | `<AuthnStatement AuthnInstant="2011-12-29T05:35:22.000Z">` | N/A |
| Authentication Method | Identifies how the subject of the token was authenticated. | `<AuthnContextClassRef>`<p>`http://schemas.microsoft.com/ws/2008/06/identity/claims/authenticationmethod/password`<p>`</AuthnContextClassRef>`|`“amr”: ["pwd"]` |
| First Name | Provides the first or "given" name of the user, as set on the Azure AD user object. | `<Attribute Name=”http://schemas.xmlsoap.org/ws/2005/05/identity/claims/givenname”>`<p>`<AttributeValue>Frank<AttributeValue>` |`"given_name": "Frank"` |
...
| Identity Provider | Records the identity provider that authenticated the subject of the token. This value is identical to the value of the Issuer claim unless the user account is in a different tenant than the issuer. | `<Attribute Name=” http://schemas.microsoft.com/identity/claims/identityprovider”>`<p>`<AttributeValue>https://sts.windows.net/cbb1a5ac-f33b-45fa-9bf5-f37db0fed422/<AttributeValue>` | `"idp":”https://sts.windows.net/cbb1a5ac-f33b-45fa-9bf5-f37db0fed422/”` |
| IssuedAt | Stores the time at which the token was issued. It is often used to measure token freshness.| `<Assertion ID="_d5ec7a9b-8d8f-4b44-8c94-9812612142be" IssueInstant="2014-01-06T20:20:23.085Z" Version="2.0" xmlns="urn:oasis:names:tc:SAML:2.0:assertion">` | `"iat": 1390234181` |
...
| Last Name | Provides the last name, surname, or family name of the user as defined in the Azure AD user object.| `<Attribute Name=” http://schemas.xmlsoap.org/ws/2005/05/identity/claims/surname”>`<p>`<AttributeValue>Miller<AttributeValue>` | `"family_name": "Miller"` |
