...
2. Your application has been deployed in a way that requires the NameIdentifier claim to be something other than the username (user principal name) stored in Azure Active Directory.

  ![view claims in SAML token](media/active-directory-b2b-claims-mapping/view-claims-in-saml-token.png)

For information on how to add and edit claims for these scenarios, check out this article on claims customization, [Customizing claims issued in the SAML token for pre-integrated apps in Azure Active Directory](develop/active-directory-saml-claims-customization.md). For B2B users, for security reasons, mapping NameID and UPN cross-tenant are prevented.
