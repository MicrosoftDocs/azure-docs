...
Users who are assigned to the Global administrator role can read and modify every administrative setting in your Azure AD organization. When a user signs up for for a service and receives a new Azure AD tenant, the user is assigned the Global Administrator role in the tenant. When you add a subscription to an existing tenant, you aren't assigned to the Global Administrator role. Only Global administrators and Privileged Role administrators can delegate administrator roles. To reduce the risk to your business, we recommend that you assign this role to the fewest possible people in your organization.
