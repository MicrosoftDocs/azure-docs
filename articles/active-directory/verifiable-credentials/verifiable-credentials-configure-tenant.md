...
Your application needs to get access tokens when it wants to call into Microsoft Entra Verified ID so it can issue or verify credentials. To get access tokens, you have to register an application and grant API permission for the Verifiable Credentials Service Request API. For example, use the following steps for a web application:
