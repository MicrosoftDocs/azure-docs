...
| `status` | string | status of the service, this will always be `enabled` |
...
There can be 2 different didModels. 1 is `ion` and the other supported method is `web`

> [name=Matthijs Hoekstra] figure out how did:web exactly works
...
It is possible to update the domain related to the DID. This functionality needs to write an update operation to ION to get this update distributed around the world. The update can take some time, currently up to an hour before it is processed and available for other users.
...
>Although it is technically possible to publish multiple domains, we currently only support a single domain per authority. If there is a need to support multiple domains for a single issuer/DID, please contact us with your request and scenario, we would love to learn where this can be useful and for us to understand when we need to add this support. We weren't sure how to display this information in Authenticator if we need to validate several different domains (we currently only display one domain with a checkmark, what do we need to show when there are multiple domains for example)
...
>It is possible to point multiple DIDs to the same domain. If that is the case you need to combine generated tokens and put them in the same did-configuration.json file. The file contains an array of these tokens. 
...
This endpoint helps you search for specific issued verifiable credentials, check if these credentials are revoked. And provides the functionality to revoke a specific verifiable credential
...
### Search credentials
You are able to search for verifiable credentials with specific index claims. You can read more about how to set this up in the [documentation](https://docs.microsoft.com/en-us/azure/active-directory/verifiable-credentials/how-to-issuer-revoke).

Since no information beyond a hash is stored you need to search for the specific calculated value. The algorithm you need to use is: Base64Encode(SHA256(contractid + claim value))
An example in C# looks like this:
...
At this moment only the filter=indexclaim eq format is supported.
...
> [name=Matthijs Hoekstra]The next format is not supported yet. it needs paging
Return all issued verifiable credentials for this contract
`GET /v1.0/verfiableCredentials/authorities/:authorityid/contracts/:contractid/credentials`
...
## Opt-out
This method will completely remove all instances of the verifiable credential service in this tenant. It removes all configured contracts. Every issued verifiable credential can't be checked for revocation. 
This action can't be undone

> WARNING this action cannot be undone and will invalidate all issued verifiable credentials and created contracts
> 
#### HTTP Request
