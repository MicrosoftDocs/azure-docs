...
- The number of **Users** and **Groups** provisioned, and the number of roles created. During the initial cycle, **Users** counts up by 1 when a user is created or a user is updated, and it counts down by 1 when a user is deleted. During an incremental cycle, user updates don’t affect the **Users** count; the number changes only when users are created or deleted.
