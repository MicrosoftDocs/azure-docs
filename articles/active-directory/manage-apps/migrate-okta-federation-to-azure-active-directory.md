...
You can migrate federation to Azure Active Directory (Azure AD) in a staged manner to ensure the desired authentication experience for users. In a staged migration, you can also test reverse federation access back to remaining Okta SSO applications.
...
4. Currently the server is configured for federation with Okta. Change the selection to **Password Hash Synchronization**. Select the checkbox to **Enable single sign-on**.
...
2. On the final page, select **Configure** to update the Azure AD Connect Server.
   ...
   ![Screenshot showing update the azure ad connect server](media/migrate-okta-federation-to-azure-active-directory/update-azure-ad-connect-server.png)
...
3. Ignore the warning for Hybrid Azure AD join for now, however the **Device options** needs to be reconfigured after disabling federation from Okta.
   ...
   ![Screenshot showing reconfigure device options](media/migrate-okta-federation-to-azure-active-directory/reconfigure-device-options.png)
...
3. After confirming, select **Enable staged rollout for managed user sign-in**
   ...
   ![Screenshot showing enable staged rollout](media/migrate-okta-federation-to-azure-active-directory/enable-staged-rollout.png)
...
5. After enabling Password Hash Sync, select **Manage Groups**.
   ...
   ![Screenshot showing enable password hash sync](media/migrate-okta-federation-to-azure-active-directory/password-hash-sync.png)
...
![Screenshot showing example of security group](media/migrate-okta-federation-to-azure-active-directory/example-security-group.png)
...
2. Next, select **Enterprise Applications** from the menu under the Manage section.
   ...
   ![Screenshot showing enterprise applications](media/migrate-okta-federation-to-azure-active-directory/enterprise-application.png)

3. In the **All Applications** menu, select **New Application**.

   ![Screenshot showing new applications](media/migrate-okta-federation-to-azure-active-directory/new-application.png)
...
4. Select **Create your own application** and on the side menu that pops up, give the Okta app a name and select the radial for **Register an application you're working on to integrate with Azure AD** and then select **Create**.
   ...
   ![Screenshot showing register an application](media/migrate-okta-federation-to-azure-active-directory/register-application.png)
...
5. After registering the application, change the application to account in any organizational directory such as any Azure AD Directory - Multi-tenant and then select **register**.
   ...
   ![Screenshot showing register an application and change the application account](media/migrate-okta-federation-to-azure-active-directory/register-change-application.png)
...
6. After adding registration, go back to the Azure AD menu, and select **App Registrations** and then open the newly created registration.
   ...
   ![Screenshot showing app registration](media/migrate-okta-federation-to-azure-active-directory/app-registration.png)
...
7. After opening the application, record your tenant ID and application ID.
   ...
   ![Screenshot showing record tenant id and application id](media/migrate-okta-federation-to-azure-active-directory/record-ids.png)
...
9. Record the value and ID of the secret before leaving this page.
   ...
   ![Screenshot showing record secrets](media/migrate-okta-federation-to-azure-active-directory/record-secrets.png)
...
12. From the OpenID permissions section, add **Email**, **OpenID**, and **Profile**, then and select **Add permissions**.
    ...
    ![Screenshot showing add permissions](media/migrate-okta-federation-to-azure-active-directory/add-permissions.png)

13. Select the **Grant admin consent for Tenant Domain Name** option and wait for the Granted status to appear.

    ![Screenshot showing grant consent](media/migrate-okta-federation-to-azure-active-directory/grant-consent.png)

14. Once the permissions have been consented, add in the Home page URL under **Branding** for your user's application homepage.

    ![Screenshot showing adding branding](media/migrate-okta-federation-to-azure-active-directory/add-branding.png)
...
15. After configuring the application, transition to the Okta Administration portal and configure Microsoft as an Identity Provider. Select **Security** > **Identity Providers** and add a new Identity Provider, and add the default **Microsoft** Option.
    ...
    ![Screenshot showing configure idp](media/migrate-okta-federation-to-azure-active-directory/configure-idp.png)
...
18. Finalize your selection for auto provisioning. By default, if a user doesn't match to Okta, it will attempt to provision them in Azure AD. If you have migrated Provisioning away from Okta, select the **Redirect to Okta Sign-in page** option.
    ...
    ![Screenshot showing redirect okta sign-in](media/migrate-okta-federation-to-azure-active-directory/redirect-okta.png)
...
20. Configure the policy as shown to direct sign-ins from all devices and IPs to Azure AD.
    ...
    ![Screenshot showing division for idp routing](media/migrate-okta-federation-to-azure-active-directory/division-idp-routing.png)

21. After adding the routing rule, record the Redirect URI, and add it to the **Application Registration**.

    ![Screenshot showing application registration](media/migrate-okta-federation-to-azure-active-directory/application-registration.png)

22. Navigate back to your application registration and select the authentication tab, followed by **Add a platform** and **Web**.

    ![Screenshot showing add platform](media/migrate-okta-federation-to-azure-active-directory/add-platform.png)
...
25. While editing the profile, add **ToAzureAD** to match the example and select **save**
    ...
    ![Screenshot showing profile editing](media/migrate-okta-federation-to-azure-active-directory/profile-editing.png)
...
>Users and groups can only be added from the Enterprise Applications selection, you cannot add users under the App Registrations menu.
   ...
   ![Screenshot showing adding a group](media/migrate-okta-federation-to-azure-active-directory/add-group.png)
...
3. After about 15 minutes, sign in as one of the Managed Authentication Pilot users and access [Myapplications](https://myapplications.microsoft.com).
   ...
   ![Screenshot showing access myapplications](media/migrate-okta-federation-to-azure-active-directory/my-applications.png)
