...
# An error message appears on an app page after the user signs in

In this scenario, Azure Active Directory (Azure AD) signed the user in. But the application displayed an error message and didn't let the user finish the sign-in flow. The app didn't accept the response that was issued by Azure AD.

There are several reasons why the app didn’t accept the response from Azure AD. If the error message doesn't clearly identify what's missing in the response, try the following:

-   If the app is the Azure AD gallery, verify that you followed all the steps at [How to debug SAML-based single sign-on to applications in Azure AD](https://azure.microsoft.com/documentation/articles/active-directory-saml-debugging).
...
7. After the app loads, select **Single sign-on** from the navigation pane.

8. In the **User Attributes** section, select **View and edit all other user attributes under** to edit the attributes to send to the app in the SAML token when users sign in.
...
Signing in to the appl fails because the SAML response is missing attributes such as roles, or because the app expects a different format for the EntityID attribute.
...
2. Select **All services** at the top of the navigation pane on the left side to open the Azure Active Directory Extension.
...
7. After the application loads, select **Single sign-on** from the navigation pane on the left side.
...
7. After the application loads, select **Single sign-on** from the navigation pane.
...
The next time that the user signs in to the app, Azure AD signs the part of the SAML response that you selected.

## The app expects the SHA-1 signing algorithm

By default, Azure AD signs the SAML token by using the most-secure algorithm. We recommend that you don't change the signing algorithm to *SHA-1* unless the appl requires SHA-1.
...
6. Select the app that you want to configure for single sign-on for.
...
The next time that the user signs in to the application, Azure AD signs the SAML token by using the SHA-1 algorithm.
