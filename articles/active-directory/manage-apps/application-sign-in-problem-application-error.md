...
There are several reasons why the app didn’t accept the response from Azure AD. If the error message doesn't clearly identify what's missing from the response, try the following:
...
8. In the **User Attributes** section, select **View and edit all other user attributes** to change which attributes to send to the app in the SAML token when users sign in.
...
9. Select **SHA-1** as the **Signing Algorithm**.
   ...
   The next time that the user signs in to the application, Azure AD will sign the SAML token by using the SHA-1 algorithm.
