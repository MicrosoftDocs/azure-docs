...
1. Configure the following settings:
   ...
   - **Selected users will receive request expiration reminders** - Enable or disable reminder email notifications to the reviewers when a request is about to expire. The 1st about-to-expire reminder email is likely sent out in the middle of the configured "Consent request expires after (days)". For example, if consent is configured to expire in 3 days, the 1st reminder email is usually sent out on the 2nd day, and the last expiration email is almost immediately out once the consent is expired.
