...
Without an admin consent workflow, a user in a tenant where user consent is disabled will be blocked when they try to access an app that requires admin permissions. The user will see a generic error message telling them they're unauthorized to access the app, and that they should ask their admin for help. Often, it's unlikely that the user knows whom to contact, and they either give up or create a new local account in the application. Even if the admin is notified, typically there isn't a streamlined process to help admins grant access and notify their users.
 
The admin consent workflow gives admins a secure way to grant access to applications that require admin approval. When a user tries to access an application but is unable to provide consent, they can send a request for admin approval. The request is sent via email to admins who have been designated as reviewers. When a reviewer takes action on a request, the user is notified of the action.

In order to approve requests, a reviewer must be a global administrator, cloud application administrator, or application administrator. The reviewer must already have one of these admin roles assigned; simply designating them as a reviewer doesn't elevate their privileges.

## Limitations

* Turning on the feature may take an hour to take effect.
* Reviewers may retain the ability to review requests even after they are removed.

## Enable the admin consent workflow

To enable the admin consent workflow and designate reviewers:
...
6. Under **Admin consent requests**, set **Users can request admin consent to apps they are unable to consent to** to **Yes**.
   ...
   ![Enable the admin consent workflow](media/configure-admin-consent-workflow/enable-admin-consent-workflow.png)
...
6. Configure the following settings:
   ...
   * **Should selected admins receive reminders about requests that are about to expire?** - Enable or disable reminder email notifications to the reviewers when a request is about to expire.  
...
After the admin consent workflow is enabled, users can request admin approval for an application they're unauthorized to consent to. Only the first justification from the user will be submitted to the admin. Here is an example of the user's experience:  

![Admin consent user experience](media/configure-admin-consent-workflow/admin-consent-user-experience.png)
 
## Review and take action on admin consent requests

A reviewer grants admin consent when registering an app:
...
8. Evaluate the request and take the appropriate action:
   ...
   * **Deny the request**. To deny a request, you must provide a justification that will be provided to all requestors. Once a request is denied, all requestors are notified that they have been denied access to the application. Denying a request will not prevent users from requesting admin consent to the app again in the future.  
   * **Block the request**. To block a request, you must provide a justification that will be provided to all requestors. Once a request is blocked, all requestors are notified that they have been denied access to the application. Blocking a request creates a service principal object for the application in your tenant and disables the application. Therefore, users won't be able to request admin consent to the application in the future.
...
|Scenario  |Audit Service  |Audit Category  |Audit Activity  |Audit Actor  |Audit logs limitations  |
|---------|---------|---------|---------|---------|---------|
...
After turning on the feature, it may take up to 60 minutes for end users to see the update. You can verify that the configuration has properly taken affect by viewing the **EnableAdminConsentRequests** value in the `https://graph.microsoft.com/beta/settings` API.

**As a reviewer, why can’t I see all pending requests?**

A reviewer can see any requests that were created during the time they were configured and assigned as a reviewer. So if you were recently added as a reviewer, you won't see any requests that were created before your assignment.

**As a requestor, why do I see multiple requests for the same application?**
  
If an application developer has configured their app to use static and dynamic consent to request access to their end user’s data, you will see two admin consent requests. One of the admin consent requests will represent the static permissions, and the other will represent the dynamic permissions.
