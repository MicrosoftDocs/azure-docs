...
> [!NOTE]
> When you configure to trust inbound multi-factor (MFA) claim, in case you have an MFA registration policy enforced for all users (https://docs.microsoft.com/en-us/azure/active-directory/identity-protection/howto-identity-protection-configure-mfa-policy), you should exclude the external users to avoid access restrictions.
