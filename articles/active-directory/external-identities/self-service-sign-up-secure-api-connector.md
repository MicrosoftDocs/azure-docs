...
1. Sign in to the [Azure portal](https://portal.azure.com/).
1. Make sure you're using the directory that contains your Azure AD tenant. Select the **Directory + subscription** filter in the top menu and choose your Azure AD directory.
1. Choose **All services** in the top-left corner of the Azure portal, and then search for and select **Azure AD**.
...
Bearer token authentication is defined in [OAuth2.0 Authorization Framework: Bearer Token Usage (RFC 6750)](https://www.rfc-editor.org/rfc/rfc6750.txt). In bearer token authentication, Azure AD sends an HTTP request with a token in the authorization header.

```http
Authorization: Bearer <token>
```

A bearer token is an opaque string. It can be a JWT access token or any string that the REST API expects Azure AD to send in the authorization header. Azure AD supports the following types:
...
A claim provides temporary storage of data during an Azure AD policy execution. The [claims schema](claimsschema.md) is the place where you declare your claims. The access token must be stored in a claim to be used later. 
...
Before the technical profile can interact with Azure AD to obtain an access token, you need to register an application. Azure AD relies the Azure AD platform. You can create the app in your Azure AD tenant, or in any Azure AD tenant you manage. To register an application:

1. Sign in to the [Azure portal](https://portal.azure.com).
1. Select the **Directory + subscription** filter in the top menu, and then select the directory that contains your Azure AD, or Azure AD tenant.
...
#### Create Azure AD policy keys

You need to store the client ID and the client secret that you previously recorded in your Azure AD tenant.

1. Sign in to the [Azure portal](https://portal.azure.com/).
2. Make sure you're using the directory that contains your Azure AD tenant. Select the **Directory + subscription** filter in the top menu and choose the directory that contains your tenant.
3. Choose **All services** in the top-left corner of the Azure portal, and then search for and select **Azure AD**.
