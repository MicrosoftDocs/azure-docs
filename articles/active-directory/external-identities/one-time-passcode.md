...
4. Under **Enable Email One-Time Passcode for guests**, select **Disable email one-time passcode for guests**.

    ![Email one-time passcode settings](media/one-time-passcode/otp-admin-settings.png)

   > [!NOTE]
   > The following are the options under Email one-time passcode for guests
   > 
...
If you've already opted into the public preview of email one-time passcode authentication, the automatic enabling of the feature in March 2021 doesn't apply to you. 

If you're a public preview, you won't see this new UI at all, you'll see (either new UI with auto greyed out). You can opt back into automatic enabling of the feature by using the API. **(UI/API INSTRUCTIONS TBD)**
