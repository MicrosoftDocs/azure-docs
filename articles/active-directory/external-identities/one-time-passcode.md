...
1. Select **Email one-time passcode**, and then under **Email one-time passcode for guests**, you'll see the following **Yes** or **No** toggle to enable or disable email one-time passcode. The email one-time passcode feature is now turned on by default for all new tenants and for any existing tenants where you haven’t explicitly turned it off.

![Screenshot showing Email one-time passcode opted in.](media/one-time-passcode/enable-email-otp-opted-in.png)

   > [!NOTE]
   >  The email one-time passcode feature is now turned on by default for all new tenants and for any existing tenants where you haven’t explicitly turned it off.
