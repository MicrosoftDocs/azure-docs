...
If you have an on-premises Active Directory environment, and you want to join your domain-joined devices to Azure AD, you can accomplish this by setting up hybrid Azure AD-joined devices. In this article, we describe the steps you can take to do this. 


## Before you begin

Before you start configuring hybrid Azure AD-joined devices in your environment, you should familiarize yourself with supported scenarios and constraints.  
...
- The registration of Windows down-level devices *is* supported in non-federated environments through [Azure Active Directory Seamless Single Sign-On](https://aka.ms/hybrid/sso).
...
- Keeps the association between the computer account in your on-premises Windows Server Active Directory (Windows Server AD) instance and the device object in Azure AD. 
...
You can set up hybrid Azure AD-joined devices for various types of Windows device platforms. This topic includes the required steps for all typical configuration scenarios.  
...
| :--                                        | :-:                                    | :-:                            | :-:                |
| Step 1: Set up service connection point | ![Check][1]                            | ![Check][1]                    | ![Check][1]        |
...
## Step 1: Set up service connection point
...
For example, for a forest with the AD domain name *fabrikam.com*, the configuration naming context is: `CN=Configuration,DC=fabrikam,DC=com`.
...
## Step 2: Set up issuance of claims
...
> [!NOTE]
> When you use AD FS, you must enable either **adfs/services/trust/13/windowstransport** or **adfs/services/trust/2005/windowstransport**. If you are using the Web Authentication Proxy, also ensure that this endpoint is published through the proxy. To see what endpoints are enabled, in the AD FS management console, go to **Service** > **Endpoints**.
>
> If you don’t use AD FS as your on-premises federation service, follow the instructions of your vendor to make sure they support WS-Trust 1.3 or 2005 endpoints, and to ensure that these are published through the Metadata Exchange (MEX) file.
>

The following claims must exist in the token that is received by Azure AD Device Registration Service for device registration to complete. Azure AD Device Registration Service uses some of this information to create a device object in Azure AD, which Azure AD Connect then uses to associate the newly created device object with the on-premises computer account.
...
- How a definition look in AD FS

The definition helps you verify whether the values are present. or whether you need to create them.
...
### Issue objectSID of the on-premises computer account
...
### Issue ImmutableID for a computer when one for users exists (alternate login ID is set)
...
The following script helps you create the issuance transform rules described in the preceding sections:
...
- This script appends the rules to the existing rules. Do not run the script twice. Running the script twice will add the set of rules twice. Make sure that no corresponding rules exist for these claims (under the corresponding conditions) before you run the script again.

- If you have multiple verified domain names (as shown in the Azure AD portal or via the `Get-MsolDomains` cmdlet), in the script, set the value of **$multipleVerifiedDomainNames** to **$true**. Also, ensure that you remove any existing issuerID claim that might have been created by Azure AD Connect or by any other means. Here is an example for this rule:


        c:[Type == "http://schemas.xmlsoap.org/claims/UPN"]
        => issue(Type = "http://schemas.microsoft.com/ws/2008/06/identity/claims/issuerid", Value = regexreplace(c.Value, ".+@(?<domain>.+)",  "http://${domain}/adfs/services/trust/")); 

- If you have already issued an **ImmutableID** claim  for user accounts, in the script, set the value of **$immutableIDAlreadyIssuedforUsers** to **$true**.
...
- To avoid certificate prompts when authenticating the device, add the Azure AD device authentication endpoint to the Local Intranet zones.
...
When this kind of request comes, the on-premises federation service must authenticate the user by using IWA. Upon success, it must issue the following two claims:
...
To join domain-joined Windows down-level computers in a federated environment, you can download and install these Windows Installer (.msi) files from Download Center at [Microsoft Workplace Join for non-Windows 10 computers](https://www.microsoft.com/en-us/download/details.aspx?id=53554).
...
The output of this cmdlet shows devices that are registered and joined with Azure AD. To get all devices, use the *-All* parameter. Then, filter the devices by using the **deviceTrustType** property. Domain-joined devices have a value of **Domain Joined**.
