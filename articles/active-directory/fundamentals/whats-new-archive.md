...
The offline Azure AD Threat Intelligence risk detection can now have a risk reason that helps customers with the risk investigation. If a risk reason is available, it shows up as **Additional Info** in the risk details of that risk event. The information can be found in the Risk detections report. It will also be available through the additionalInfo property of the riskDetections API. [Learn more](../identity-protection/howto-identity-protection-investigate-risk.md).
