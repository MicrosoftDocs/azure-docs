...
- You can enable your users to automatically get signed-on to Cisco Webex (Single Sign-On) with their Azure AD accounts.
...
4. In the search box, type **Cisco Webex**, select **Cisco Webex** from result panel then click **Add** button to add the application.
	...
	![Cisco Webex in the results list](./media/active-directory-saas-cisco-webex-tutorial/tutorial_ciscowebex_addfromgallery.png)
...
For single sign-on to work, Azure AD needs to know what the counterpart user in Cisco Webex is to a user in Azure AD. In other words, a link relationship between an Azure AD user and the related user in Cisco Webex needs to be established.

In Cisco Webex, assign the value of the **user name** in Azure AD as the value of the **Username** to establish the link relationship.
...
1. In the Azure portal, on the **Cisco Webex** application integration page, click **Single sign-on**.
...
3. On the **Cisco Webex Domain and URLs** section, perform the following steps:
	...
	a. In the **Sign-on URL** textbox, type a URL using the following pattern: `https://<subdomain>.webex.com`

    b. In the **Identifier** textbox, type the URL: `http://www.webex.com`

	c. In the **Reply URL** textbox, type a URL using the following pattern: `https://company.webex.com/dispatcher/SAML2AuthService?siteurl=company`
...
6. Click **Save** button.
	...
	![Configure Single Sign-On Save button](./media/active-directory-saas-cisco-webex-tutorial/tutorial_general_400.png)
	
6. On the **Cisco Webex Configuration** section, click **Configure Cisco Webex** to open **Configure sign-on** window. Copy the **Sign-Out URL, SAML Entity ID, and SAML Single Sign-On Service URL** from the **Quick Reference section.**

	![Configure Single Sign-On](./media/active-directory-saas-cisco-webex-tutorial/tutorial_ciscowebex_configure.png) 

7. In a different web browser window, log in to your Cisco Webex company site as an administrator.

8. In the menu on the top, click **Site Administration**.

	![Site Administration](./media/active-directory-saas-cisco-webex-tutorial/ic777621.png "Site Administration")

9. In the **Manage Site** section, click **SSO Configuration**.
   
	![SSO Configuration](./media/active-directory-saas-cisco-webex-tutorial/ic777622.png "SSO Configuration")

10. In the **Federated Web SSO Configuration** section, perform the following steps:
	...
	b. As **SSO profile**, select **SP Intiated**.

	c. Open your downloaded certificate in notepad, and then copy the content of it.

	d. Click **Import SAML Metadata**, and then paste the copied content of the certificate.

	e. In the **Issuer for SAML (IdP ID)** textbox, paste the value of **SAML Entity ID** which you have copied from Azure portal.

	f. In the **Customer SSO Service Login URL** textbox, paste **SAML Single Sign-On Service URL** which you have copied from Azure portal.
	...
	i. In the **Customer SSO Service Logout URL** textbox, paste **Sign-Out URL** which you have copied from Azure portal.
...
To enable Azure AD users to log in to Cisco Webex, they must be provisioned into Cisco Webex. In the case of Cisco Webex, provisioning is a manual task.
...
3. On the Add User section, perform the following steps:
	...
	d. In the **Username** textbox, type the email of user like Brittasimon@contoso.com.

	e. In the **Email** textbox, type the email address of user like Brittasimon@contoso.com.

	f. In the **Password** textbox, type the password of user.

	g. In the **Confirm** Password textbox, reenter the password of user.
...
**To assign Britta Simon to Cisco Webex, perform the following steps:**
...
2. In the applications list, select **Cisco Webex**.
	...
	![The Cisco Webex link in the Applications list](./media/active-directory-saas-cisco-webex-tutorial/tutorial_ciscowebex_app.png)  
...
When you click the Cisco Webex tile in the Access Panel, you should get automatically signed-on to your Cisco Webex application.
