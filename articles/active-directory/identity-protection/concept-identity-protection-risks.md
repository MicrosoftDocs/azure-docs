...
| Password Spray | Offline | This risk detection is triggered when a password spray attack has been performed and the attacker was able to access the account. A password spray attack is when multiple usernames are attacked by using common passwords in a unified brute force attack to again unauthorized access by guessing the password through multiple continuous attempts. 
