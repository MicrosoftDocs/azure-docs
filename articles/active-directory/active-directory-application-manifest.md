...
- Add a new App ID URI for your application (identifierURIs[]). App ID URIs are used to uniquely identify the Application entity when requesting permissions or acquiring a token to a resource application, within its Azure AD tenant, or across multiple Azure AD tenants (for multi-tenant scenarios) when qualified via a verified custom domain. When you update this element, the same update is made to the corresponding service principal's servicePrincipalNames[] collection.
