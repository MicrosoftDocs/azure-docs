...
1. The first box displays a percentage and a rating which represents how many compliance recommendations were made for the selected compliance standard, and how many recommendations the account passed as of the current date shown.

     - To view the differentiators between the two levels, hover over the information **(i)** icon in the Level 1 and Level 2 boxes.

      > [!NOTE]
      > If the recommendation has been further categorized in the standard, a **Pass**/**Fail** status displays for that category. You can click on the details in the these boxes to view select details. For example, click the number under **Passed** to display only the recommendations in which a resource passed.
...
1. The first box displays a percentage and a rating which represents how many compliance recommendations were made for the selected compliance standard, and how many recommendations the account passed as of the current date shown.
     ...
     > [!NOTE]
     > If the recommendation has been further categorized in the standard, a **Priority** status displays for that category. You can click on the details in the these boxes to view select details. For example, click the number under **Priority 1** to display only the recommendations in which a resource passed.
...
1. The first box displays a percentage and a rating which represents how many compliance recommendations were made for the selected compliance standard, and how many recommendations the account passed as of the current date shown.
     ...
     If the recommendation has been further categorized in the standard, a **Priority** status displays for that category. You can click on the details in the these boxes to view select details. For example, clicking the number under **Priority 1** displays only the recommendations in which a resource passed.
...
1. The first box displays a percentage and a rating which represents how many compliance recommendations were made for the selected compliance standard, and how many recommendations the account passed as of the current date shown.
     ...
     If the recommendation has been further categorized in the standard then a **Priority** status displays for that category. You have the option to click on the details in the these boxes to view select details. For example, clicking the number under **Priority 1** displays only the recommendations in which a resource passed.
...
1. Click the account name to open a detailed view of the recommendations with the following details:
     ...
     - **Ignore** – Users can toggle this option **On** or **Off** to ignore or view the recommendation.
       ...
       - When turning the recommendation back on, a pop up box opens asking the user to confirm they want to turn the recommendation on. Click **Confirm**.
       ...
       - Some items are do not have the ability to be scored because they are test items and will be marked in the column as **Not Scored**. These items are not counted in the recommendations.
...
## How to view details about the recommendation

1. To view details about the recommendation, see [How to view recommendation details by account]().
...
3. The table defaults to the **Results** tab, and displays the following details:
    ...
    - **Remediate** – If a recommendation can be remediated, the action for remediation is listed in this column.
        - Click the ellipses (**...**) to view available option for remediation, i.e., disabling or deleting an access key.
...
There are several filtering options to choose from when filtering the recommendations. When you click into a specific account, additional filters (listed beginning on step 5) display. For more information, see [How to view Recommendation details by account]().
...
2. To search for your criteria, press **Enter** or click **Search**. 
     ...
     The search results display on the screen. The top right corner indicates **Showing 1 of XX** to inform the user how many pages of results have been found and which page is currently displayed.
