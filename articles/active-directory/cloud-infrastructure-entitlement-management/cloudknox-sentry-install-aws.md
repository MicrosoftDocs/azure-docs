...
# Install Microsoft CloudKnox Permissions ManagementSentry on Amazon Web Services (AWS)
...
The CloudKnox Sentry is an agent packaged in a virtual appliance that gathers information on users, their privileges, activity, and resources for any monitored Amazon Web Services (AWS) account(s). It can be deployed in two ways:
...
To provide visibility and insights, the CloudKnox Sentry collects information from a number of sources. It uses *Read*, *List*, and *Describe* privileges to help CloudKnox enumerate resource information, for example, to catalogue ec2 instances and s3 buckets.

CloudKnox gathers CloudTrail event logs and ties them to individual identities too gain insight into activity within the AWS account.

### Architecture

The CloudKnox Sentry is a Linux Photon-based appliance which collects information about the AWS account by using an IAM Role to read identity entitlements, resource information, and CloudTrail data on an hourly basis. It then uploads that data to the CloudKnox Software as a Service (SaaS) application for processing.
...
- To view the Google Cloud Platform (GCP) architecture, click [here](cloudknox-sentry-install-gcp.md#architecture).
...
| ------------ | ---- | --------------------------------------------------------------------------------------------------------- |
| [TCP Inbound](https://docs.cloudknox.io/Product%20Documentation%2098db130474114c96be4b3c4f27a0b297/Sentry%20Installation%20-%20AWS%20bef8e66cf2834aa69867b628f4b0a203/Required%20Ports%20for%20the%20CloudKnox%20Sentry%2094690cf56f754d5281d9a8e3421b3072/TCP%20Inbound%20e3ae4234a50446378b86dd78581f81d1.html)  | 9000 | Configuration. </p> Request this information from the administrators source IP only when you are performing a configuration. |
...
For AWS organizations with multiple AWS accounts, you can set up one Sentry to collect from multiple AWS accounts by allowing the Sentry's IAM role to assume a cross account role in the account from which you want to collect data. To allow this, you must configure a *trust relationship*.
...
The Sentry first collects entitlement and resource information from the target AWS account, and then assume a separate cross account role in the AWS account that holds the S3 bucket storing the CloudTrails. 

During Sentry configuration, you must specify the AWS AccountID and IAM Role in that account that the Sentry assumes. You must configure a trust relationship to allow this cross account access.
...
2. If it is not able to collect CloudTrail locally, the Sentry assumes a role in Account C and pulls event logs from Account A.
...
2. Verify that CloudTrail is turned on for the target AWS account.
    ...
    If it is not already turned on, create a new CloudTrail. The CloudTrail must be enabled for CloudKnox to function.
...
Using the CloudKnox Sentry AMI, the Sentry's CloudFormation template creates an IAM Role, AutoScale group, Secrets Manager secret, Security group, and EC2 instance.
...
2. To configure the CloudFormation template:
   ...
   2. Select a subnet and Virtual Private Cloud (VPC) in which to deploy the sentry.  
      You must access the Sentry that is accessed through the web browser over port 22 during configuration. 
   3. Choose a VPC and subnet that you can route from your machine to the appliance.

3. Enter an existing Security Group ID.   
   Leave this blank if you want the CloudFormation template to create the security group.

4. To configure the sentry after deployment using SSH access into the instance, click **Allow**.  
    To update the stack later and remove the inbound port 22 access from security group, click **Deny**.
...
8. In **Poseidon URL**, accept the default unless CloudKnox support recommends something else.
...
10. In **Instance Count Desired/Min/Max**, enter the counts needed for CloudKnox sentry instances. (Approximately 1 for collection of 30 AWS accounts.) Then click **Next**.

11. Create any necessary tags, and then click **Next**.

12. Review the information you entered, and then click **Create Stack**.

## Multi-account collection from one Sentry configurations

The Sentry automatically collects data from the account in which it is deployed. To collect data from other AWS accounts, you must configure the the Sentry to allow it to assume a role in which it can collect the target account's information.

### Cross account collection configuration (For multi-account collection from one Sentry)

You must create a cross account role that allows the Sentry's EC2 instance to assume the data collection role for each of the accounts from which the Sentry collects data, aside from the account in which it was deployed.
...
2. Modify the role's trust relationships.   
...
1. In the AWS account with the S3 bucket holding the organization's CloudTrails, create **Cross Account Role for Collection**.
   ...
   3. Create a new policy (or skip this and add *Inline Policy* after creating the role).
...
3. Name the policy: `IAM_R_KNOX_SECURITY_CLOUDTRAIL` and then click **Create** to create the policy.

4. Modify the role's trust relationship.   
    In this step, you limit who and what can assume this cross account role by specifying the `instanceId` of the Sentry.

   1. Click the **Trust relationships** tab and then click **Edit trust relationship**.
   2. Replace the **Principal** with the following string:

       `arn:aws:iam::<accountofSentry>:role/IAM_R_KNOX_SECURITY`

       where *accountofSentry* is the AccountId of the AWS holding the sentry. 
   3. If the role name is not IAM_R_KNOX_SECURITY, update the string with EC2 role name.
...
4. In **Role Name**, enter `IAM_R_KNOX_SECURITY_XA` (This is configurable.)

5. Modify the role's **Trust Relationships**.   
    In this step, you limit who and what can assume this cross account role by specifying the instanceId of the Sentry.
...
6. To run this creation of role using CloudFormation template, use [this CFT](https://knox-software.s3.amazonaws.com/cloud-formation/master-account.yaml).
...
1. Login to CloudKnox and click the gear icon to open the **Data Collectors** tab. 

2. Under the AWS section, click **Deploy**.

<!---Add screenshot.--->

3. Click **Next**.

4. Click **Next** again if all the above steps are completed from the documentation link.

5. Enter the appliance DNS name or IP of the Sentry that you deployed in AWS, and then click **Next**.
...
4. Enter **Y** to collect automatically accounts listed from from master account role. (Also perform steps 5-6.). 

5. If you configured CloudKnox with auto-detect in step 4, and if you are performing the configuration without a master account, enter each account ID and role name to collect data manually:

    `Auto Detect Collection Accounts From Master Account (Y/N) :`

6. If you configured CloudKnox with auto-detect in step 4, enter the following details:
