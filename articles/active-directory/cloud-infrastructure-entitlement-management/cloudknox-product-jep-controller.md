...
2. To view the **Policy Summary**, select the icon next to the AWS policy name.

3. To expand details about the role/AWS policy and view tasks and identities assigned to the role/AWS policy, select **Role**. 

4. To view the **Role Summary**, select the icon next to the username under **Identities Assigned to**.
...
1. On the **Roles/Policies** tab, select **Create Policy**.
...
7. To select the identity to which the policy should apply, select the icon next to the identity name.

     - To move the identity from the **Available Users** tab to the **Selected Users** tab, select the icon. 
...
9. To adjust the user's access, select the icon in the **Available Tasks** column and/or the icon in **Selected Tasks** column. 
   <!---Add screenshot.--->

      - To view available options, select the right caret icon next to the service name.
...
11. To open a list of all available resources, select **Specific Resources*. All the resources that the selected identities have accessed are  entered in the **Selected Resources** column by default.
     ...
     1. To adjust the identity's access, select the icon in the **Available Resources** column and/or the icon in **Selected Resources** column. 

     2. Scroll to find a resource or type in the resource name into the **Search** box.

    3. To add a resource, in the **Enter the resource ARN...** box, enter the Azure Resource Notification (ARN), and then select the icon.

     4. To expand options, select the icon next to the resource name. 
...
14.  To modify the operations (tasks and resources) further, select the icon next to each statement, if necessary.
...
15. Optional: To add a JSON condition, under **Request Conditions**, select the icon next to JSON. Then select **Done**.

16. Under **Effect**, select **Allow** or **Deny**, then select **Next**.
...
17. In the **JSON** tab on the **Preview** screen, view the **character count** to see if the policy needs to be split.
     ...
     If permissions haven't been granted, the **Controller Disabled** is displayed, and the system administrator can create policy and a script by copying and pasting from the **JSON** tab and **Script** tab, or select **Download JSON** and **Download Script**.
...
19. To edit the policy, select the **Edit** icon.

20. To save your edits, select the **Save** icon.

21. To copy the policy to the clipboard, select the **Copy** icon.

22. Select **Submit**.

     If **Controller Disabled** displays under the icon, the **Submit** button is disabled.

     When you select **Submit**, a number circled in red displays next to the icon at the top-right of the page.
     - Select the hourglass to view the **Active** tab, which  displays the policies being created.
...
1. On the **Roles/Policies** tab, select **Create Role**.
...
7. To select the user to whom the policy should apply, select the icon next to the user's name.
...
10. To adjust the identity's access, select the icon in the **Available Tasks** column and/or the icon in **Selected Tasks** column.

     - To expand options, select the icon next to the service name. 

       Only system administrators can add individual tasks within the service to the identity's selected tasks.

    **For Azure only:** The **Include inherited read-only tasks** option is already checked. If it's unchecked, only the tasks the user has performed or the write tasks the identity has permissions to perform display.
...
12. Select **Specific Resources** to open a list of all available resources. All the resources that the selected identities have accessed are by default put in the **Selected Resources** column.
     ...
     1. To adjust the user's access, select the icon in the **Available Resources** column and/or the icon in **Selected Resources** column. 
     2. Scroll to find a resource or type in the resource name into the **Search** box.

     3. Enter a specific resource into the **Enter the resource ARN...** box and select icon to add it.

     4. Select the caret icon next to the resource name to expand options. 
...
15. To modify the operations (tasks and resources) further, select each statement, and then select the icon.

16. Optional: To add a JSON condition, under **Request Conditions**, select ![icon](images/icons/Add circle fill.svg) next to JSON. Then select **Done**.

17. Under **Effect**, select **Allow** or **Deny**, and then select **Next**.
...
20. To edit the policy, select the icon.

21. When you have finished making your edits, select **Save**.

22. To copy the policy to the clipboard, select the icon. Then select **Submit**.

     If **Controller Disabled** displays under the authorization system icon, the **Submit** button is disabled.

     When you select **Submit**, a number is circled in red next to the icon at the top-right of the page. 
     -  To view the policies being created, in the **Active** tab, select the hourglass. 
...
1. To make a copy of the policy, select **Clone**.
...
1. Select **Modify** next to the applicable role/AWS policy.
...
4. Review the edits on the **Preview** screen, and then select **Modify**.
...
1. Next to the applicable role/AWS policy, select **Delete**.

     The system administrator can only delete role/AWS policy if that role/policy isn't assigned to an identity. System roles or policies can't be deleted.

2. On the **Preview** screen, review the role/AWS policy about to be deleted.

     If the controller is disabled, select **Download Script**.

3. To remove the role/AWS policy, select **Delete**.
...
1. Filter from the following options on the screen:
    ...
    - **Task Usage** - Select **Any**, **Granted**, **Used**, or **Unused**.
        ...
        - **Tasks** - To view a list of **Available Tasks** and add new tasks, select in this field, and then select the **Add** icon.
...
2. To select the policies you want to give the user, from the **Attachable** column, select the icon. The policy moves into the **Selected to Attach** block.
    
3. To move the policy into the **Selected to Detach** block, from the **Detachable** column, select appropriate policies that should be removed from the user and select the icon.

    - If any policy is added to the **Selected to Attach** or **Selected to Detach** blocks in error, select the icon to remove the policy from the block.
    - Use **Search** to locate a specific policy or type of policy.

4. Select **Submit**.
    
5. When the pop-up message appears asking **Are You Sure You Want to Perform the Quick Action?** select one of the following options:
...
1. When you select to open **Modify Permissions**, it populates to  display which permissions are associated with the user(s).
    
2. To select the user and modify permissions, select next to the username.

      To select all users, or select users individually, select next to **Username**.

     If a role was granted through a group, the permission can't be edited without modifying the group itself.

3. Select one of the following tabs to continue:
    ...
    - **Roles**
        1. To change users' role assignment, select the **Roles** tab.
        2. To select the roles you want to assign to the user, from the **Add Role** column, select the icon. 
     
           The role moves into the **Selected to Add** block.
        3. To move the role into the **Selected to Remove** block, from the **Remove Role** column, select the roles you want to remove and select the icon.
             ...
             If any role is added to the **Selected to Add** or **Selected to Remove** blocks by mistake, select the icon to remove the role from the block.

             To search for a specific role or type of role, use **Search**.

    - **Tasks**
        1. Select the **Tasks** tab.
        2. To move the tasks into the **Selected to Grant** block, from the **Available** column, select appropriate tasks that should be given to the user, and select the icon.
        3. To move the task into the **Selected to Revoke** block, from the **Revocable** column, select appropriate tasks that should be removed from the user, select the icon.
             ...
             If any role is added to the **Selected to Grant** or **Selected to Revoke** blocks by mistake, select the icon to remove the task from the block.
...
1. Select the **Role/Policy Template** tab from the top of the screen, and view the following table of information: 
     ...
     - To view or edit the template, select the **View/Edit** icon.
     - To delete a template, select the **Delete** icon.

         When you choose **Delete**, the **Delete Alert** appears to ask you to confirm that you want to delete the template. Select **Yes** or **No**.
...
2. From the upper right corner, select **Create Template**.
...
8. To select the applicable task name, select the icon.
...
10. Under **Request Conditions**, select the **JSON +** button, if applicable, to open the **Add JSON** screen.
...
5. Select **Next**. 
    ...
    The information displayed under **Tasks** is pre-populated and can't be changed. 
...
6. Under **Resources**, update the information appropriately.
     ...
     Select the icon under **Available Resources** to add to the **Selected Resources** column. Alternatively, you can enter the ARN and select **Add**. 

     For **Resource Template**, users will update the **region**, **account-id**, and **resource-id** boxes.

7. Under **Effect**, select **Allow** or **Deny**.

8. On the **Confirmation** page, select **Next** and then confirm that the details are correct.

9. In the **Request Summary** box, fill in details of the request.

10. Select **Submit**.

11. The **Validate OTP to Create JEP Request** box appears. Enter the One Time Passcode (OTP) from your email and select **Verify**.
...
1. Select the **Settings** tab from the top of the screen, and view the following table of information:
     ...
     Select **Apply** once you've made your selections.
    ...
    - To view or edit the filter, open the menu and select the **View/Edit** icon.
    - To delete the filter, open the menu and select the **Delete** icon.

         The **Delete Alert** box  appears to confirm you want to delete the filter. Select **Yes** or **No**.

### Request Role/Policy Filters tab

1. On the **Request Role/Policy Filters** tab, select **Create Filter** from the top-right corner. 

     The **Edit Policy** window opens.

2. In the **Filter Name** box, enter a name for the filter.

3. In the **Authorization System Type** box, select the icon to view or edit the details.

     This box defaults to the **Authorization System Type** selected from the **Request Role/Policy Filters** main page.

4. In the **Authorization System Is** box, select the icon to view the details 

    Or, to edit which **Authorization Systems** you want to include, select the icon.

5. To view the details or to edit the role names, in the **Role Name Contains** box, select the icon.

6. Select **Save**.

### Request Settings tab

1. Select the **Request Settings** tab.
...
1. View the **Pending Requests** section for the following details:
    ...
    - **Submitted By** - Lists the name of the user making the request.
    - **On Behalf Of** - Lists the name of the user the request is being made for, if not the current user or requestor.
    ...
    - To perform the following actions, from the right side of the screen:
        - **Details** - To view the details of the request, select **Details**.

            The request can be approved or rejected from the detail view.

        - **Approve** - To approve the request, select **Approve**.

            Enter a reason of approval in **Approve Request** box, if necessary, and select **Confirm**.

        - **Reject** - To reject the request, select **Reject**.

            Enter a reason of rejection in **Reject Request** box (required), and select **Confirm**.
...
3. View the **Processed Requests** section for the following details:
   ...
   - To perform the following actions, select the icon from the right side of the screen:
     ...
     - **Details** - To view the details of the approval, select **Details**.
     - **Delete** - To delete the approval, select **Delete**.

4. Select the **Filters** button in any of the sections, and then select the appropriate option from the **Authorization System**  drop-down list.

5. To  display or hide columns, select **Columns**, and then select the icon next to the column name.
...
3. Select the appropriate option from the **Auth System** drop-down.
    ...
    - **AWS**
        1. In the **Identity** field, select the username of the user for whom the permission is being requested.
             ...
             If a Security Assertions Markup Language (SAML)  user is selected, the **Role** field will open. Select the appropriate role from the  drop-down list.
        ...
        4. Select the appropriate policies, tasks, or templates from the **Available Policies** or **Available Tasks** column. To add them to the **Selected Policies** or **Selected Tasks** column, select the icon.

             You can search for a specific policy, task, or template using **Search**, and then filter by selecting the **All**  drop-down list and selecting **All**, **System**, **Custom**, or **CloudKnox Only**.

             To view details about the policy or task, select the icon next to the policy or task name.
        ...
        6. Select **Next**.
            ...
            1. In the **Resource** section, select **All Resources**, **Specific Resources**, or **Not Resource**.
                 ...
                 To move the selection for **Specific Resources** to the **Selected Resources** column, select the icon from the **Available Resources** column. You can also use **Search** to find a specific resource.

            2. From **Request Conditions**, select the icon next to **JSON**, and then enter details in the **Add JSON** box. Select **Done**.
            3. From the **Effect** section, select **Allow** or **Deny**.
            4. Select **Next**.

        7. Enter details in the **Request Summary** field.
        8. Add notes, if needed, to the **Notes (Optional)** field.
        9. Select in the **Schedule** section to set schedule details:
        ...
        11. Select the **Selected Policies/Tasks** and **Selected Scope** tabs to view a summary of the selections made.
    ...
    - **Azure/GCP/vCenter**
        ...
        4. Select the appropriate policies or tasks from the **Select Roles** or **Available Tasks** column and then select the icon to add it to the **Selected Roles** or **Selected Tasks** column.

             You can search for a specific role or task using **Search**. To filter your results, select the **All**  drop-down list and then select **All**, **System**, **Custom**, or **CloudKnox Only**.

        5. To display a list of roles attached to the select user, select **Existing Roles**.
        6. Select **Next**.
        7. Enter details in the **Request Summary** field.
        8. Add notes, if needed, to the **Notes (Optional)** field.
        9. To set schedule details, select in the **Schedule** section:
            - From the **Frequency**  drop-down list, select **ASAP**, **Once**, **Daily**, **Weekly**, or **Monthly**.
            ...
            - Select **Schedule**.
        10. View the details summary, including **User**, **User Type**, **Authorization System Type**, and **Authorization System**.
        11. To view a summary of the selections made, select the **Selected Roles/Tasks** and **Selected Scope** tabs.

4. Select **Submit.**

5. View the **Pending Requests** tab for the new request.

6. To view the request details, select **Details**, or select **Cancel** if the request is no longer required.
