...
1. Scroll to the bottom of the page, and in the **Capabilities** box, select **I acknowledge that AWS CloudFormation might create IAM resources with custom names**. Then select **Create stack.**
    ...
    This AWS CloudFormation stack, creates an OIDC Identity Provider (IdP) representing Azure AD STS, an AWS IAM role with a trust policy that allows external identities from Azure AD to assume it via the OIDC IdP. These entities are listed on the **Resources** page.

1. Return to CloudKnox, and in the **CloudKnox On Boarding - AWS OIDC Account Setup & IDP Access** page, select **Next**.

1. Optional but recommended: In the **CloudKnox On Boarding - AWS Master Account Details** page you should setup the master account connection if your organization has Service Control Policies (SCPs) that govern some or all of the member accounts. Setting up the master account connection will also allow CloudKnox to auto detect and onboard any aws member accounts that have the correct CloudKnox role.
