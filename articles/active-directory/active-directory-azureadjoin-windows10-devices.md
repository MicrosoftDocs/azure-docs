...
- **A new simplified BYOD experience** that allows users to add a work account to Windows to easily access work resources on personal devices.
...
| Strong authorization and convenient sign-in for a work account with Microsoft Passport and Windows Hello                                                                   | Yes             | Yes           | Yes             |
| Access to enterprise Windows Store using work or school account (not a Microsoft Account)                                                                                    | Yes             | Yes           | Yes             |
...
- Domain join
 ...
 Both are valid options depending on the organization's needs and requirements. In some cases, organizations might benefit from enabling both methods of deployment.

## When to use Azure Active Directory Join

Azure Active Directory Join (Azure AD Join) is a new self-service work provisioning experience in Windows 10.  It is targeted for workers who access work resources such as Office 365  primarily in the cloud. It is a lightweight way to configure computers, tablets, and phones for the enterprise. Devices are managed via mobile device management (MDM) using consistent controls across Windows platforms.
...
3.	You want to manage a set of users in Azure AD instead of in Windows Server Active Directory, such as seasonal workers, contractors, or students.
...
- •	Have an Azure AD Premium subscription, such as mobile device management auto-enrollment, if you require more capabilities.
...
- Set policy that allows conditional access to domain-joined devices to Azure AD.
- For more information about domain join in Windows 10, see <link-to-DJ-in-Win10-deployment-guide>.
- For conditional access you can create policy that allows access to ‘domain joined’ devices if you want to be able to restrict access for some devices.
- Acquire System Center Configuration Manager version 1509 for Technical review to enable rules for requiring compliant devices. (please see TechNet documentation and blog post).
