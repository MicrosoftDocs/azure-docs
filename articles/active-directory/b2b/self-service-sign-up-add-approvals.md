...
With [API connectors](api-connectors-overview.md), you can integrate your own custom approval system with your application's self-service sign-up user flow so you can manage which user accounts are created in your tenant.
...
You need to register your approval system as an application in your Azure AD tenant so it can authenticate with Azure AD and so it has permission to create users. Learn more about [authentication and authorization basics for Microsoft Graph](https://docs.microsoft.com/graph/auth/auth-concepts).
...
Next you'll [create the API connectors](self-service-sign-up-add-api-connector.md#create-an-api-connector) for your self-service sign-up user flow. Your approval system API needs two connectors and the corresponding endpoints, like the examples shown below. These API connectors do the following:
...
Your approval system can use the [API response types](api-connectors-overview.md) from the two API endpoints to control the sign up flow.
...
| \<otherBuiltInAttribute>  | No  | Other built-in attributes like 'displayName', 'city', and more. Parameter names are the same as the parameters sent by the API connector.|
