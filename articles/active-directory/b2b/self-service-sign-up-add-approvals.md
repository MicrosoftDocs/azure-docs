...
In this example, we connect to the external approval system so we can collect user data during the sign-up process, pass it to the approval system, and allow someone to manually review and approve the request.
...
16. Configure your approval system to use the **client ID** and the **client secret** to authenticate with Azure AD.
...
> [!NOTE]
> The `userPrincipalName` parameter can be generated by taking the `email_address` claim sent from a user flow, replacing the `@`character with `_`, and pre-pending it to `#EXT@<tenant-name>`.
>
> The `mail` parameter should be equivalent to `email_address` claim sent from a user flow. 

> [!NOTE]
> Aside from `userPrincipalName`, `accountEnabled`, and `mail`, the rest of the user attributes, including custom attributes, are the same serialized format as that of the claims sent by the API connector.  
