...
In this example, we connect to the external approval system so we can collect user data during the sign-up process, pass it to the approval system, and either :
1. Automatically approve them.
2. Someone can manually review and approve the request. If a request is approved, the approval system can use Microsoft Graph to provision the user and notify the user that their account has been approved.
...
16. Configure your approval system to use the **Application ID** as the client ID and the **client secret** you just generated to authenticate with Azure AD.
...
> [!NOTE]
> Note that if you approval system only does automatic approval decisions, this API connector is not needed.
...
#### Blocking response
The check approval status API endpoint should return an **Blocking Response** if:
...
{
 "userPrincipalName": "johnsmith_gmail.com#EXT@contoso.onmicrosoft.com",
 "accountEnabled": true,
