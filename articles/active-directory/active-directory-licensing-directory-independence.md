...
If you create or delete a resource in one tenant, it has no impact on any resource in another tenant, with the partial exception of external users. If you use the of one of your domain names, with one tenant, it cannot be used with any other tenant.
...
* The administrators of tenant 'Contoso' have no direct administrative privileges to tenant 'Test' unless an administrator of 'Test' specifically grants them these privileges. Administrators of 'Contoso' can control access to tenant 'Test' if they control the user account which created 'Test.'
* If you change (add or remove) an administrator role for a user in one tenant, the change does not affect any administrator role that the user might have in another tenant.

## Synchronization independence
You can configure each Azure AD tenant independently to get data synchronized from a single instance of either:

* The tenant Synchronization (DirSync) tool, to synchronize data with a single AD forest.
