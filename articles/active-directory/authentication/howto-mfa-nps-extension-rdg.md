...
> [!NOTE]
> The sign in behavior for Remote Desktop Gateway doesn't provide the option to enter a verification code with Azure Multi-Factor Authentication. A user account must be configured for phone verification or the Microsoft Authenticator App with push notifications. If one of these two authentication methods isn't configured for a user, they won't be able to complete the Azure Multi-Factor Authentication challenge and sign in to the Remote Desktop Gateway.
