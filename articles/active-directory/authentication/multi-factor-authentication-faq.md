...
### What is the lifetime of the OATH software token generated by Microsoft Authenticator App?

Microsoft Authenticator App generates a new OATH TOTP every 30 seconds.

Five (5) OATH codes are valid at a given moment: the current OATH code, the two (2) previous OATH codes, and the two (2) next OAUTH codes.  The current OATH code is determined by the current server time plus the time difference that was captured between the server and device (Authenticator App). 

> [!NOTE]
> Entering a valid OATH code that is older to what was initially determined as current will automatically adjust the time difference between server and device (Authenticator App) and set a new current for the next attempt. 
