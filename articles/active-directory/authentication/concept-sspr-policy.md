...
| Password expiry duration (Maximum password age) |<ul><li>Default value: **90** days. (If the tenant was created after 2021, it has no default expiration value. You can check cuurent policy with [Get-MsolPasswordPolicy](Get-MsolPasswordPolicy.md)) </li><li>The value is configurable by using the `Set-MsolPasswordPolicy` cmdlet from the Azure Active Directory Module for Windows PowerShell.</li></ul> |
