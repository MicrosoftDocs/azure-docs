...
* Smart lockout tracks the last three bad password hashes to avoid re-incrementing the lockout counter. If someone enters the same bad password multiple times, this will not cause the account to lockout.
...
Using smart lockout does not guarantee that a genuine user will be never be locked out. When we lockout a user account, we try our best to not lockout the genuine user. The lockout service attempts to ensure that bad actors can’t gain access to a genuine users account.  However, there are some situations where this won’t be possible:

* Each Azure Active Directory data center tracks lockout independently. So technically a user will have (threshold_limit * datacenter_count) number of attempts, if the user hits each data center.
