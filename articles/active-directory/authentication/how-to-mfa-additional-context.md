...
>[!NOTE]
>The Authenticator app does not retrieve policy information just in time for each passwordless phone sign-in request. Instead, the Authenticator app does a best effort retrieval of the policy once every 7 days. We understand this limitation is less than ideal and are working to optimize the behavior. In the meantime, if you want to force a policy update to test using additional context with passwordless phone sign-in, you can remove and re-add the account in the Authenticator app. 
