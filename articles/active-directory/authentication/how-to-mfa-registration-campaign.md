...
|state|"enabled"<br>"disabled"<br>"default"|Allows you to enable or disable the feature.<br>Default value is used when the configuration hasn't been explicitly set and will use Microsoft Entra ID default value for this setting. Currently maps to enabled for voice call and text message users across all tenants.<br>Change states to enabled or disabled as needed.|
