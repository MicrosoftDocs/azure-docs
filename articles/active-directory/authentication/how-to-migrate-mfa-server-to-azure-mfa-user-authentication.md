...
If your federated domain(s) have the [federatedIdpMfaBehavior](/graph/api/resources/federatedIdpMfaBehavior?view=graph-rest-beta) set to `acceptIfMfaDoneByFederatedIdp` or **SupportsMfa** flag set to `$false`, you are likely enforcing MFA on AD FS using claims rules. 
