...
* Use policies that restrict the access within the apps if a certain authentication level is not attained instead of simply falling back to full access. For example:
  ...
  * If your organization uses Microsoft Defender for Cloud Apps, consider falling back to a policy that engages Defender for Cloud Apps and then Defender for Cloud Apps Allows read-only access but not uploads.
