...
---
# Complete a passwordless authentication deployment in Azure Active Directory

A majority of cyberattacks begin with a username and password stolen from someone in an organization. Organizations try to counter the threat by requiring users to use one of the following approaches:
...
- Better user experience. Give users a convenient way to access data from anywhere, and provide easy access to applications and services such as Outlook, OneDrive, or Office while mobile.
- Robust insights. Gain insights into users passwordless activity with robust logging and auditing.

With passwordless, the password is replaced with something you have plus something you are or something you know. For example, in Windows Hello for Business, a biometric gesture, like a face or fingerprint, or a device-specific PIN that isn't transmitted over a network.
...
Enabling Windows 10 sign-in using FIDO2 security keys requires enabling the credential provider functionality in Windows 10 in one of the following ways:
