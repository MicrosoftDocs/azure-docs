...
1. Type the number they see on the login screen into the Microsoft Authenticator app dialog.
1. Choose **Approve**.
...
1. To apply the new policy, click **Save**. If you see an error when you try to save, the cause might be due to the number of users or groups being added. As a workaround, replace multiple users or groups with a single group, in the same operation, and then click **Save** again.
