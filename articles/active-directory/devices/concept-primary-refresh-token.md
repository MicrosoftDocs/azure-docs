...
A PRT is an opaque blob sent from Azure AD whose contents aren't known to any client components. You can't see what’s inside a PRT.
...
| C | If the user is managed, CloudAP gets the nonce from Azure AD. If the user is federated, CloudAP plugin requests a SAML token from the federation provider with the user’s credentials. Nonce is requested before the SAML token is sent to Azure AD. |
