...
> [!NOTE]
> If a device object with the same displayMame as the hostname of a VM where an extension is installed exists, the VM fails to join Azure AD with a hostname duplication error. Avoid duplication by [modifying the hostname](azure/virtual-network/virtual-networks-viewing-and-modifying-hostnames#modify-a-hostname).
