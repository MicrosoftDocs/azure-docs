...
Servers with active **Health service data is not up to date** error alerts continuously for over 30 days suggest that no data has reached Connect Health during the time span. These servers will be disabled and not shown in Connect Health portal. Please note that this does not apply to **warnings** with the same alert type. Warnings indicate that Connect Health is receiving partial data. To re-enable the servers, you must uninstall and [reinstall the health agent](active-directory-aadconnect-health-agent-install.md).
