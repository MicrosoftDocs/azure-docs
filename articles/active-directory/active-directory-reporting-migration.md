...
The Azure portal gives you a new look at Azure Active Directory activity logs if you are moving over from the Azure classic portal. In a recent [blog post](https://blogs.technet.microsoft.com/enterprisemobility/2016/11/08/azuread-weve-just-turned-on-detailed-auditing-and-sign-in-logs-in-the-new-azure-portal/), we explain how you can see activity logs in the context of the resource you are working on in the Azure portal. In this article, we describe how we translated reports from the Azure classic portal in the Azure portal.
...
Using context-based reporting, in the Azure portal, we merged existing reports into a single view, with a single, underlying API that provides the data to the view.
...
![Sign-ins view](./media/active-directory-reporting-migration/483.png "Audit logs")

The **Sign-ins** view includes all user sign-ins. You can use this information to get application usage information. You also can view application usage information in the **Enterprise applications** overview.
...
In response to customer feedback, in the Azure portal, you can use advanced filtering to get the data you want. One filter you can use is an **activity category**, which lists the different types of activity logs that Azure Active Directory provides. To narrow results to what you are looking for, you can select a category.

For example, if you are interested only in activities related to **Self-service Password Reset**, you can choose the **Self-service Password Management** category. The categories you see are based on the resource you are working in.  
...
![Filter Sign-In Events page screenshot](./media/active-directory-reporting-migration/07.png "Audit logs")


### Security reports

Security reports are consolidated to give you a complete overview of all security-related risk events that Azure Active Directory can detect and report on. For a complete overview of security reports, see
[Azure Active Directory risk events](active-directory-identity-protection-risk-events.md).  In the Azure Active Directory risk events article, you can see an overview of how the Azure Active Directory anomalous activity reports map to the risk events in Azure Active Directory, in the [Azure Active Directory anomalous activity reports](active-directory-identity-protection-risk-events.md#azure-ad-anomalous-activity-reports) section.
...
| Account provisioning errors          | For the **Activity Category**, select **Account User Provisioning**.        |
