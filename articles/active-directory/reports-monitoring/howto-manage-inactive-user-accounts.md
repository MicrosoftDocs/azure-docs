...
- To generate a `lastSignInDateTime` timestamp, you must attempt a sign-in. Either a failed or successful sign-in attempt, as long as it is recorded in the [Azure AD sign-in logs](azure/active-directory/reports-monitoring/concept-sign-ins), will generate a `lastSignInDateTime` timestamp. The value of the `lastSignInDateTime` property may be blank if:
