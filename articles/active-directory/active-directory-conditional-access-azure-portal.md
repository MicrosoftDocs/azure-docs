...
- **[Sign-in risk](active-directory-conditional-access-conditions#sign-in-risk.md)**: Azure AD Identity Protection detects sign-in risks. How do you restrict access if a detected sign-in risk indicates a bad actor? What if you would like to get a stronger evidence that a sign-in was really performed by the legitimate user or your doubts are strong enough to even block specific users from accessing an app?
...
- **[Device management](active-directory-conditional-access-conditions#device-platforms.md)**: In Azure AD, users can access cloud apps from a broad range of devices including mobile and also personal devices. What if you demand that access attempt should only be performed user using devices that are managed by your IT department? What if you even want to block certain device types form access in cloud apps in your environment? 

- **[Client application](active-directory-conditional-access-conditions#client-apps.md)**: Today, you can access many cloud apps using different app types such as web-based apps, mobile apps or desktop apps. What if an access attempt is performed using a client app type that causes known issues? What if you require a device that is managed by your IT department for certain app types? 
