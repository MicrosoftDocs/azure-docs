...
The rest of the RBAC roles in Azure allow management of specific Azure resources. For instance, the Virtual Machine Contributor role allows the user to create and manage virtual machines but does not give them access to the virtual network or the subnet that the virtual machine connects to.
...
Azure RBAC is supported only by the Azure portal and the new Azure Resource Manager APIs. Users and applications that are assigned RBAC roles cannot use the classic management portal and the Azure Service Management APIs.
...
### Add Access
You grant access from within the resource, resource group, or subscription. This will be the scope of the role assignment.
...
Inherited assignments can not be removed from child scopes. You have to remove the role assignment from the parent scope.
