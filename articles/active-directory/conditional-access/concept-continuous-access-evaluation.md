...
| --- | --- | --- | --- |
| New tenants that did not configure anything in the old UX. | No | Yes | Old CAE setting will be hidden given these customers likely did not see the UX prior to this. |
| Tenants that explicitly enabled for All users with the old UX.	 | No | Yes | Old CAE setting will be greyed out. Since these customers explicitly enabled this for All users, they don't need to migrate. |
| Tenants that explicitly enabled some users in their tenants with the old UX.| Yes | No | Old CAE settings will be greyed out. Clicking **Migrate** launches the New conditional access policy wizard which places **All users** on the Include, while excluding users and groups copied from CAE. It also sets the new **Customize continuous access evaluation** Session control to **Disabled**. |
| Tenants that explicitly disabled the preview. | Yes | No | Old CAE settings will be greyed out. Clicking **Migrate** launches the New conditional access policy wizard which places **All users** on the Include and sets the new **Customize continuous access evaluation** Session control to **Disabled**. |
