...
In persistent browsers, cookies stay stored in the user’s device even after a user closes the browser. These cookies could have access to AAD artifacts and those artifacts are useable until their expiry regardless of the CA policies placed on the resource environment. Token caching can be in direct violation of desired security policies for authentication. While it may seem convenient to store tokens beyond the current session, doing so can create a security vulnerability by allowing unauthorized access to AAD artifacts.
