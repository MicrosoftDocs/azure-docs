...
Cross-services roles are only going to work for accounts that are members of a tenant. If you have tenants that are not federated with each other the user will be added as a guest/external user and even though you can add the security roles to them they will not be able to perform any actions on those features. 
