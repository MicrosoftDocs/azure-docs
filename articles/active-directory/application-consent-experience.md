...
This article is to help you learn more about the Azure Active Directory (Azure AD) application consent user experience so you can manage and develop applications more effectively.
...
| # | Definition |
| ----- | ----- |
| 1 | Is the identifier that represents the user. The client application is requesting to access protected resources on behalf of this user. |
| 2 | Is the title of the consent prompt. The title changes depending on whether the users are going through the user consent flow or the admin consent flow. |
| 3 | Is the app logo that's provided by the application developer. This image provides a visual cue for the user whether this is app the app they intended to access. Ownership of this image isn't validated. |
| 4 | Is the app name that's provided by the application owner. This value informs users which application is requesting access to their data. Ownership of this app name isn't validated. |
| 5 | Is the publisher domain that's provided by the application developer. This value provides users with a domain that they may be able to evaluate for trustworthiness. Ownership of this publisher domain is validated. |
...
| 7 | Is the description of the permission that's provided by the service exposing the permissions. To see the permission descriptions, toggle the chevron next to the permission. |
...
| 9 | Is the link to where users can review and remove any non-Microsoft applications that currently have access to their data. |
