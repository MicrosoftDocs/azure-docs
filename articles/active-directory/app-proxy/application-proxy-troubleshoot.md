...
| 13006 - Connection to the backend server failed. Error: (0x80072efe). | If the internal URL is HTTPS, ensure you meet the prerequisites for [Windows Server](application-proxy-add-on-premises-application.md#windows-server) and the [TLS requirements](application-proxy-add-on-premises-application.md#tls-requirements). If the connector is running on the same server as the application, and the application is served with IIS, verify that IIS serves the application on HTTPS and that the binding listens on *all unnassigned IP addresses*. | 
