...
You can also use entitlement management for bringing in users from organizations which do not have their own Azure AD directory.  You can configure an federated identity provider for their domain, or use email-based authentication.  You can also bring in users from social identity providers, including those with Microsoft accounts.
