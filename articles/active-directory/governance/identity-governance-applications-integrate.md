...
If neither of those criteria are met for an application, as the application doesn't rely upon Azure AD, then identity governance can still be used, however there may be some limitations. For instance, users that aren't in your Azure AD or are not assigned to the application roles in Azure AD, won't be included in access reviews of the application. See [Preparing for an access review of users' access to an application](access-reviews-application-preparation.md) for more information.
