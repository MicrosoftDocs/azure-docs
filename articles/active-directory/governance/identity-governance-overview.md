...
Once you have configured these features, you can easily automate common identity governance scenarios. The following table shows how to get started:
...
| Adding and removing a user's group memberships, based on changes to the user's attributes | [Create a dynamic group](../enterprise-users/groups-create-rule.md)|
| Adding and removing a user's group memberships, application roles, and SharePoint site roles, based on changes to the user's attributes | [Configure an automatic assignment policy for an access package in entitlement management](entitlement-management-access-package-auto-assigment-policy.md) (preview)|
...
<!-- add between first and second row | Sending emails, adding users to groups, and running custom tasks, when a user is scheduled to join or leave| [Configure lifecycle workflows](link TBA) (preview) |  -->
