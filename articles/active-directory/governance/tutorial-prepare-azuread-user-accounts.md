...
We will use the Graph Explorer to quickly create two users needed to execute the Lifecycle Workflows in the tutorials.  One user will represent our new employee and the second will represent the new employee's manager.

You will need to edit the POST and replace the &lt;your tenant name here&gt; portion with the name of your tenant.  For example:   $UPN_manager = "bsimon@&lt;your tenant name here&gt;" to $UPN_manager = "bsimon@contoso.onmicrosoft.com".  
...
First we will create our employee, Melva Prince.
...
Next, we will create Britta Simon.  This is the account that will be used as our manager.
...
The employeeHireDate attribute is new to Azure AD.  It is not exposed through the UI and must be updated using Graph.  To edit this attribute, we can use [Graph Explorer](https://developer.microsoft.com/graph/graph-explorer).
...
In order to do this, we must get the object id for our user Melva Prince.
 ...
 9. Copy the following in to the **Request body** and select **Run query**
   ...
   ```
   :::image type="content" source="media/tutorial-lifecycle-workflows/update-1.png" alt-text="PATCH employeeHireDate" lightbox="media/tutorial-lifecycle-workflows/update-1.png":::
 ...
 :::image type="content" source="media/tutorial-lifecycle-workflows/update-3.png" alt-text="GET employeeHireDate" lightbox="media/tutorial-lifecycle-workflows/update-3.png":::
...
In this scenario, we will use this feature of Azure AD to generate a temporary access pass for our new employee.  It will then be mailed to the employee's manager.
...
### Setup user with groups and Teams with team membership
