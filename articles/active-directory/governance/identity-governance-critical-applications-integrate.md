...
<!-- TODO link to app discovery worksheet -->
...
Otherwise, if this is an existing application, we recommend performing an access review of the users who already have access to the application, to ensure that those users are authorized for continued access.  You will need to be in the `Global administrator`, `User administrator`, or `Identity Governance administrator` role to create reviews, and you will need to identify one or more people to be reviewers.

1. If the application relies upon Azure AD application role assignments to determine who has access, then create a one-time [access review](../governance/create-access-review.md) of that application.
1. If the application relies upon Azure AD group memberships to determine who has access, then create a one-time [access review](../governance/create-access-review.md) of the membership of those groups.
1. <!-- TODO * If the application uses an AD DS group, ... -->
1. <!-- TODO * If the application has a local data store accessible via SCIM ... -->
1. <!-- TODO * If the application uses a SQL database or another LDAP directory, ... -->
