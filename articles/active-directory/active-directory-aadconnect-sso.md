...
1.	The Azure AD resource challenges the user, via a 401, to provide a Kerberos token.
2.	The user requests a token from Active Directory for the Azure AD resource sending the ticket the user received when they logged onto their computer. 
3.	Active Directory locates the machine account associated with the Azure AD resource and returns a Kerberos token to the user, encrypted with the machines secret. 
4.	The user sends the Kerberos ticket it acquired from Active Directory to the Azure AD.
...
For each forest listed provide the appropriate account details and single sign on will be enabled for your tenant.

## Ensuring Clients sign-in automatically
By default, browsers do not attempt to send credentials to web servers unless the URL is defined in the intranet zone.  Generally, the browser can calculate the right zone by simply looking at the URL, for example if the URL is http://intranet/ the browser will automatically send credentials as it will map the URL to the intranet zone.  However, if the URL contains a period for example http://intranet.contoso.com/ the server will not automatically send credentials and will treat the URL as it would any internet site.

As the URLs used for single sign on in Azure AD contain a period they need to be the intranet zone, so the browser will automatically send the currently logged in users credential in the form of a Kerberos ticket to Azure AD.  The easiest way to do this is to simply create a group policy in Active Directory.
...
3.	Navigate to Current User\Administrative Templates\Windows Components\Internet Explorer\Internet Control Panel\Security Page and select Site to Zone Assignment List 
...
>[!NOTE]Chrome will use the same settings as Internet Explorer but if you have configured different settings for Chrome you will need to update these separately.
...
## Event log entries
Every time a user logs in with single sign on an entry is recorded in the event log of the domain controller, if success auditing is enable.  To find these events, you can review the Event logs for the security Event 4769 associated with computer account AzureADSSOAcc$.  The filter below finds all security events associated with the computer account:
