...
* On-demand provisioning supports disabling users that have been unassigned from the application, but does not support disabiling or deleting users that have been disabled or deleted from Azure Active Directory (those users won't appear when searching for a user).
