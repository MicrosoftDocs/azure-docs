...
description: Frequently asked questions (FAQs) about API-driven inbound provisioning
...
- **Payload format**: The MS Graph Users API endpoint expects data in OData format. The request payload format for the new inbound provisioning /bulkUpload API uses SCIM schema elements. When invoking this API, set the 'Content-Type' header to `application/scim+json`.
