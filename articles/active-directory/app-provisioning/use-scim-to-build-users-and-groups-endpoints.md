...
The System for Cross-domain Identity Management, [SCIM](https://aka.ms/scimoverview), is the standard definition of two endpoints, `/Users` and `/Groups`, with common REST verbs to create, update, and delete objects, and a pre-defined schema for common attributes like group name, username, first name, last name and email. 

Use SCIM 2.0 REST API in apps to reduce the overhead of working with a proprietary user management API, for example, a SCIM-compliant client can make an HTTP POST with a JSON object to the `/Users` endpoint to create a new user entry without the need of additional code.

This tutorial descibes how to build a [SCIM](https://docs.microsoft.com/azure/active-directory/manage-apps/use-scim-to-provision-users-and-groups) endpoint and contains guidance on how to implement basic requirements for CRUD operations on a user and group object (also known as resources in SCIM) with optional features such as filtering and pagination.

> [!div class="checklist"]
> * View the reference code
> * Develop and deploy a SCIM endpoint
> * Test the deployment
> * Integrate the SCIM endpoint with a SCIM client
...
The **Microsoft.SystemForCrossDomainIdentityManagement** project contains [reference code](https://github.com/AzureAD/SCIMReferenceCode) developed as a .Net core MVC web API, to build and provision a SCIM API. The **Microsoft.SCIM.WebHostSample** project is also included as a working example.
...
All requests to the SCIM endpoints require authorization and the SCIM standard leaves authentication and authorization relatively open. You can use cookies, basic authentication, TLS client authentication, or any of the other methods listed in [RFC 7644](https://tools.ietf.org/html/rfc7644#section-2).

Be careful to avoid insecure methods such as username/password in favor of more secure methods such as OAuth. Azure AD supports long-lived bearer tokens (for gallery and non-gallery applications) as well as the OAuth authorization grant (for applications published in the app gallery).

> [!NOTE]
> These authorization methods provided by this repo are solely for testing. When integrating with Azure AD, review the authorization guidance, see [Plan provisioning for a SCIM endpoint](https://docs.microsoft.com/azure/active-directory/app-provisioning/use-scim-to-provision-users-and-groups#authorization-for-provisioning-connectors-in-the-application-gallery). 

###  Use multiple environments

The SCIM reference code uses ASP.NET Core environments to control the way authorization is performed while in development or after deployment, see [Use multiple environments in ASP.NET Core](https://docs.microsoft.com/aspnet/core/fundamentals/environments?view=aspnetcore-3.1).
...
1. If you chose to copy the link, open Visual Studio and choose **Clone or check out code**.
1. Use the copied link from Github to make a local copy of all files.
1. The Solution Explorer should open. Navigate to *Microsoft.SCIM.sln* view by double-clicking on it.
1. Click **IIS Express** to execute. The project will launch as a web page with the local host URL.

### Configure Postman

1. Download the [Postman client](https://www.getpostman.com/downloads/).
1. Import the Postman collection by copying the link [https://aka.ms/ProvisioningPostman](https://aka.ms/ProvisioningPostman) and pasting it into Postman as shown below:

	![postman collection](media/use-scim-to-build-users-and-groups-endpoints/postman_collection.png)

1. Create a Postman environment for testing by specifying the following variables below:
