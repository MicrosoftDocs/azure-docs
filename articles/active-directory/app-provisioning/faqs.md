...
- **Operation end-result**:
    ...
    - Azure AD provisioning service processes identity data, then sends that data to the Provisioning API endpoint. The provisioning service applies scoping rules, attribute mapping and transformation configured by the IT admin. This initiates a ```Create/Update/Delete``` operation on the Azure AD user profile or the AD user profile.

- **IT admin retains control**: With Provisioning APIs, the IT admin retains absolute control on how the incoming identity data is processed. They can define scoping rules to exclude certain types of identity data (for example, contractor data) and use transformation functions to derive new values before setting the attribute values on the user profile.


## Is this API a true/standardized SCIM Endpoint?
...
The new MS Graph Inbound Provisioning API is designed to handle a different enterprise identity integration scenario shaped by three unique requirements:
...
The SCIM 2.0 standard doesn't support all these requirements (for example, async processing and requirement #3). However, since most customers are familiar with SCIM as provisioning protocol, wherever possible, we have used aspects of the SCIM standard in the implementation of the MS Graph Provisioning API. For example:
...
The provisioning job runs every 40 minutes. This interval can't be changed.

If you have sent a payload and you’d like to instantly process it, then you can use the following workaround:

Select **Stop provisioning** to pause the provisioning, then select **Start provisioning** to resume provisioning.
...
here's how the provisioning job related to the API endpoint processes incoming user payloads:
...
- The job checks the value matching identifier in the SCIM request (by default: SCIM attribute externalId) and uses it to check if there's a user in Azure AD with matching employeeId value.
- If the job doesn't find a matching user, then the job applies the sync rules and creates a new user in the target directory.
...
You can achieve this using the **Scoping filter**. In the provisioning app configuration, you can define a source object scope and exclude certain users from processing either using an **inclusion rul** (for example, only process users where department EQUALS **Sales**) or an “exclusion rule” (for example, exclude users belonging to Sales, department NOT EQUALS **Sales**).
...
<td>Browser-based interactive login that autogenerates OAuth Access Token</td>
...
The current API only supports inbound data. Here are some options to consider for implementing writeback of attributes like email / username / phone that can be generated by Azure AD, and that you can flow back to the HR system:
