...
In this tutorial, build a JavaScript single-page application (SPA) that signs in users and calls Microsoft Graph by using the implicit flow of OAuth 2.0. This SPA uses the Microsoft Authentication Library (MSAL) v1.x for JavaScript v1.0.
...
>[!NOTE]
> Use [MSAL.js v2.x and the authorization code flow with PKCE and CORS](tutorial-v2-javascript-auth-code.md) support for all new applications. This tutorial uses MSAL.js v1.x which is limited to using the implicit grant flow for single-page applications. 
...
1. After the sign-in step, the SPA acquires an *access token*, generated by the identity platform, which will be used to gain access to the external API, **MS Graph**.
1. This *access token* is added to the HTTP requests through the authorization header.
1. After the *access token* is extracted, the user's profile and mails are then called by the SPA and displayed to the user.
...
3. The final consent prompt will contain the following on the assumption no values were entered in the previous step. Press **Enter** and the JSON written to a file called *package.json*.
...
1. In the **Redirect URI** section, select the **Web** platform from the drop-down list. When entering the web page to the right, for simplicity, this application is run on a local host. Enter either of the following options:
...
> [!TIP]
> A *redirect URI* (or *reply URL*) is the location where the authorization server sends the user after the application has been successfully authorized and granted an authorization code or access token. In this example, `http://localhost:3000/` is used as the *redirect URI*. The default `3000` value is the port being used, which is set in the *server.js* file.
>
...
1. In the *JavaScriptSPA* folder, create a new *.js* file named *authConfig.js*, and copy the following code. This code contains the configuration parameters for authentication (*Client ID*, *Tenant ID*, *Redirect URI*).
...
After the initial sign-in, users should not need to reauthenticate every time they need to request a token to access a resource. Therefore, `acquireTokenSilent` should be used most of the time to acquire tokens. There are situations, however, where you force users to interact with Microsoft identity platform. Examples include when:
...
1. In the *JavaScriptSPA* folder create a *.js* file named *graphConfig.js*, which stores the Representational State Transfer ([REST](https://docs.microsoft.com/en-us/rest/api/azure/)) endpoints. Add the following code:
...
Now that the code is setup, it needs to be tested.
