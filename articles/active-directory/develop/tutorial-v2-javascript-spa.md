...
1. After this sign in step, the SPA acquires an *access token* which will be used to gain access to the external API, in this case, **MS Graph**.
...
Make sure you have [Node.js](https://nodejs.org/en/download/) installed, and then create a folder to host your application. Here, we can call this folder `sampleApp`. There, we will implement a simple [Express](https://expressjs.com/) web server to serve your `index.html` file.
...
2. You will be presented with a series of prompts to create your app. Notice that the folder, `sampleApp` is now all lowercase. The items in brackets `()` are generated by default. Feel free to experiment, however for the purposes of this tutorial, you do not need to enter anything, and can press `Enter` to continue.
...
3. Assuming nothing was entered in the previous step, the list will be as shown below, along with a consent prompt. If you are happy, you can press `Enter` and it will be written to a file called `package.json`.
