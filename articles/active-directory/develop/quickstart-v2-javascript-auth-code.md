...
In this quickstart, you'll download and run a Javascript code sample that demonstrates how a JavaScript single-page application (SPA) can sign in users of personal, work or school accounts by using the authorization code flow. The code sample also demonstrates how to obtain an access token to call the Microsoft Graph API or any web API. 
