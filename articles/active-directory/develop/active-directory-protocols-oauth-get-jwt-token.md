...
This article shows how to get a JSON Web Token (JWT) to access resources secured by Azure AD. It assumes that you have an [an authorization token](/azure/active-directory/develop/active-directory-protocols-oauth-code#request-an-authorization-code) either from granted permission from the user or through a service principal.
...
```http
https://{tenant}/oauth2/v2.0/token?client_id={client-id}
&scope={scope}
