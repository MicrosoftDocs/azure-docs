...
Conditional Access is the Zero Trust control plane that allows you to target policies for access to all your apps – old or new, private, or public, on prem or multi-cloud. And now, with Conditional Access authentication context, you can apply different policies within those apps. 
...
Use the Azure AD Conditional access engine’s new auth context feature to trigger a demand for step-up authentication from within your application and services. Developers now have the power to demand enhanced stronger authentication, selectively, like MFA from their end users from within their applications. This helps them build more smoother user experiences for most parts of their application, while access to more secure operations and data remain behind stronger authentication controls.
...
For example, users can browse and work on a certain web application with standard authentication, but access to a certain portion of the app containing highly sensitive documents should be possible only for users who have performed additional methods of authentication, like MFA (2FA) and also satisfy additional polices like accessing the document from within a known IP range. 
...
Second, Conditional Access requires Azure AD Premium P1 licensing. More information about licensing can be found on the [Azure AD pricing page](https://azure.microsoft.com/pricing/details/active-directory/).

Third, today it is only available to applications that sign-in users. Applications that authenticate as themselves are not supported. Please use the Authentication flows and application scenarios guide to learn about the supported authentication app types and flows in the Microsoft Identity Platform.

### Integration steps

Once your application is integrated using the supported authentication protocols and registered in an Azure AD tenant that has the Conditional Access feature available for use, you can kick start the process to integrating this in your applications that sign-in users.

First, declare and make the authentication contexts available in your tenant. They can be created via the portal (TODO Link) or MS Graph (TODO Link).
...
1. See this sample as an example on how it is done.

These steps are the changes that you need to carry in your in your code base. The steps broadly comprise of
...
1. The following diagram shows the interaction between between the user, client app and the web API.
   ...
   > [!NOTE]
   > The format of the claims challenge is described in the “Claims Challenge in the Microsoft Identity Platform” 

1. In the client application, Intercept the claims challenge and redirect the user back to Azure AD for further policy evaluation. This following code snippet is from this code sample.
...
1. (Optional) Declare client capability. Client capabilities help resources providers (RP) like our Web API above to detect if the calling client application understands the claims challenge and can then customize its response accordingly. This capability could be useful where not all of the APIs clients are capable of handling claim challenges and some older ones still expect a different response. Read up on Client Capabilities if your needs are such.

## Caveats and recommendations

Do not hardcode auth context values in your app. You should ideally read and apply auth context using MS Graph calls [Link to auth context APIs]. This is especially a must in multi-tenant applications. The auth context values will vary between Azure AD tenants and will not be available in Azure AD’s free edition. See this code sample as how an app should query, set and use auth context in their code. 
...
- Conditional Access authentication context: https://aka.ms/authcontextca
- Claims challenge, claims request and client capabilities in the Microsoft Identity Platform.
- Using authentication context with Microsoft Information Protection and SharePoint: https://aka.ms/authcontextmip
- Using authentication context with Azure AD Privileged Identity Management: https://aka.ms/authcontextpim
