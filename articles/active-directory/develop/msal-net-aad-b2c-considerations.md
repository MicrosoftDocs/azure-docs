...
This page is for MSAL 3.x. If you are interested in MSAL 2.x, please see [AAD B2C specifics in MSAL 2.x](https://github.com/AzureAD/microsoft-authentication-library-for-dotnet/wiki/AAD-B2C-Specifics-MSAL-2.x).
...
### Limitations of the ROPC flow
 - The ROPC flow **only works for local accounts** (where you register with B2C using an email or username). This flow does not work if federating to any of the identity providers supported by B2C (Facebook, Google, etc.).
...
More details about acquiring tokens interactively with MSAL.NET for B2C applications are provided in the following sample.
