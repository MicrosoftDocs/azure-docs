...
> [!IMPORTANT]
> After hearing from customers during the preview, we've replaced the configurable token lifetimes feature with [authentication session management capabilities](../conditional-access/overview.md) in Azure AD Conditional Access. This feature will be deprecated on November 1, 2019. If you're using the configurable token lifetime policy, switch to the new conditional access feature. 
