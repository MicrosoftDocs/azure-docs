---
title: Azure AD v2 ASP.NET Web API Getting Started | Microsoft Docs
description: Protect a Web API using ASP.NET and the Azure v2 endpoint
...
# Expose a Web API protected by the Azure AD v2 endpoint using ASP.NET

This guide demonstrates how to expose an API using Azure Active Directory v2 endpoint protected by with OAuth 2.0 bearer access tokens using an ASP.NET MVC solution.

At the end of this guide, your Web APIs will accept requests containing access tokens from personal accounts (including outlook.com, live.com, and others) as well as work and school accounts from any company or organization that has integrated with Azure Active Directory
...
This guide is based on the scenario where a client acquire an access token, then uses the token to access your Web API. For the purpose of testing the application used by this guide, we use a the client used is a JavaScript SPA.
