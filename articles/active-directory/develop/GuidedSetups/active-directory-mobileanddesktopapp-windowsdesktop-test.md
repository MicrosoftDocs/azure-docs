...
If a backend API does not require a scope (not recommended), you can use the `ClientId` as the scope in the `AcquireTokenAsync` call. In this case, you also need to use the `IdToken` property in HTTP Authorize header instead of `AccessToken`.
<!--end-collapse-->
