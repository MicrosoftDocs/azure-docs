...
4. When the **Create** page appears, enter your application's registration information: 
  ...
  - **Application type:** 
    - Select "Native" for [client applications](azure-ad-dev-glossary.md#client-application) that are installed locally on a device. This setting is used for OAuth public [native clients](azure-ad-dev-glossary.md#native-client).
    - Select "Web app / API" for [client applications](azure-ad-dev-glossary.md#client-application) and [resource/API applications](azure-ad-dev-glossary.md#resource-server) that are installed on a secure server. This setting is used for OAuth confidential [web clients](azure-ad-dev-glossary.md#web-client) and public [user-agent-based clients](azure-ad-dev-glossary.md#user-agent-based-client). The same application can also expose both a client and resource/API.
...
For more information about using the consent framework with OAuth2.0 authorization grants, see [Authorize access to web applications using OAuth 2.0 and Azure AD](v1-protocols-oauth-code.md) and [Authentication Scenarios for Azure AD](authentication-scenarios.md). For information about getting authorized access to Office 365 via Microsoft Graph, see [App authentication with Microsoft Graph](https://graph.microsoft.io/docs/authorization/auth_overview).
...
You can develop a web API and make it available to client applications by exposing access [scopes](azure-ad-dev-glossary.md#scopes) and [roles](azure-ad-dev-glossary.md#roles). A correctly configured web API is made available just like the other Microsoft web APIs, including the Graph API and the Office 365 APIs. Access scopes and roles are exposed through your [application's manifest](azure-ad-dev-glossary.md#application-manifest), which is a JSON file that represents your application’s identity configuration. 
...
- The appRoles member, which is a collection of [AppRole](https://msdn.microsoft.com/Library/Azure/Ad/Graph/api/entity-and-complex-type-reference#approle-type) entities, used to define [application permissions](azure-ad-dev-glossary.md#permissions) for a web API. 
- The oauth2Permissions member, which is a collection of [OAuth2Permission](https://msdn.microsoft.com/Library/Azure/Ad/Graph/api/entity-and-complex-type-reference#oauth2permission-type) entities, used to define [delegated permissions](azure-ad-dev-glossary.md#permissions) for a web API.

For more information on application manifest concepts in general, see [Understanding the Azure Active Directory application manifest](reference-azure-ad-app-manifest.md).
...
By default, OAuth 2.0 implicit Grant is disabled for applications. You can enable OAuth 2.0 Implicit Grant for your application by setting the `oauth2AllowImplicitFlow` value in its [application manifest](reference-azure-ad-app-manifest.md).
...
- To learn more about the role the app manifest plays, see [Understanding the Azure Active Directory application manifest](reference-azure-ad-app-manifest.md)
- See the [Azure AD developer glossary](azure-ad-dev-glossary.md) for definitions of some of the core Azure AD developer concepts.
