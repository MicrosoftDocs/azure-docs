...
| `logout_hint` | Optional | Enables sign-out to occur without prompting the user to select an account. Your client application should enable the `login_hint` [optional claim](active-directory-optional-claims.md), and use the value of this claim as the `logout_hint` parameter. Do not use UPNs or phone numbers as the value of this parameter. 
