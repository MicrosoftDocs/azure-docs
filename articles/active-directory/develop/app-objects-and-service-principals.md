...
The third type of service principal represents a legacy app (an app created before app registrations were introduced). The service principal can have credentials, service principal names, reply URLs, and other properties which are editable by the user, but does not have a backing application. The service principal only works in the tenant where it was created.
