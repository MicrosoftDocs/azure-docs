...
Shared device mode is a feature of Azure Active Directory that allows you to build and deploy applications that support frontline workers and educational scenarios that require shared Android and iOS devices.
...
### Automatic single sign-in and single sign-out (SSO)
...
To support all data loss prevention scenarios, we also recommend you integrate with the [Intune App SDK](https://learn.microsoft.com/mem/intune/developer/app-sdk). By using the Intune App SDK, you can allow your application to support Intune [App Protection Policies](https://learn.microsoft.com/mem/intune/apps/app-protection-policy). In particular, we recommend that you integrate with Intune's [selective wipe](https://learn.microsoft.com/mem/intune/developer/app-sdk-android-phase5#selective-wipe) capabilities and [deregister the user on iOS](https://learn.microsoft.com/mem/intune/developer/app-sdk-ios#deregister-user-accounts) during a sign-out.
...
For data protection capabilities along with Shared Device Mode, Microsoft’s supported data protection solution for Microsoft 365 applications on Android and iOS is Microsoft Intune Application Protection Policies.  See [App protection policies overview - Microsoft Intune | Microsoft Learn](https://learn.microsoft.com/mem/intune/apps/app-protection-policy) for more information.
