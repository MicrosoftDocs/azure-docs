...
**login:** This will force the user to enter credentials on the authentication request.

**select_account:** This will provide the user with an account selection experience listing all the accounts in session.

**consent:** This will invoke the OAuth consent dialogue which allows users to grant permissions to the app.

**none:** This will ensure that the user does not see any interactive prompt. It is recommended not to pass this value to interactive methods in MSAL.js as it can have unexpected behaviors. Instead, please use the `acquireTokenSilent` method to achieve silent calls.
