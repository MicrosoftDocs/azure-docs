...
* If a non-password method of authentication was used (Windows Hello, the Authenticator app, biometrics like a face or fingerprint) to attain the token, changing the user's password will not force the user to re-authenticate.  This is because their chosen authentication input (a face, e.g.) has not changed, and therefore can be used again to re-authenticate.
