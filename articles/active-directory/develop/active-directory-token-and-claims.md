...
|`hasgroups` | Group overflow indicator | If present, always `true`, denoting the user is in at least one group.  Used in place of the `groups` claim for implicit grant flows if the full groups claim would extend the URI fragment beyond the URL length limits (approximately 4 or more groups).  Indicates that the client should use the [User memberOf Graph API](https://developer.microsoft.com/en-us/graph/docs/api-reference/v1.0/api/user_list_memberof) to determine the user's groups. |
