...
To [surface this error back](https://datatracker.ietf.org/doc/html/rfc6750#section-3.1) to the client, the middle-tier service must reply with HTTP 401 Unauthorized and with a WWW-Authenticate header contauining the error and the claim challenge. The client must acquire a new token from the token issuer, by presenting the claims challenge if one exists. Clients should not retry to access the middle-tier service using cached access tokens.
