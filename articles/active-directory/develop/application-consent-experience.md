...
| 5 | Publisher | The app publisher, if the app is publisher verified.  "unverified", if the app is not publisher verified. Selecting the publisher displays more app info, including the name, publisher, publisher domain, date created, and certification details, and reply URLs. |
| 6 | Publisher verified | The blue "verified" badge means that the app publisher has verified their identity using a Microsoft Partner Network account and has completed the verification process. For more information, read about [Publisher Verification](publisher-verification-overview.md).|
| 7 | App certified | The Microsoft 365 Certification logo means that an app has been vetted against controls derived from leading industry standard frameworks, and that strong security and compliance practices are in place to protect customer data.  For more information, read about [Microsoft 365 Certification](/microsoft-365-app-certification/docs/enterprise-app-certification-guide).|
| 8 | Publisher information  | Displays whether the application is published by Microsoft. |
| 9 | Permissions | This list contains the permissions being requested by the client application. Users should always evaluate the types of permissions being requested to understand what data the client application will be authorized to access on their behalf if they accept. As an application developer it is best to request access, to the permissions with the least privilege. |
| 10 | Permission description | This value is provided by the service exposing the permissions. To see the permission descriptions, you must toggle the chevron next to the permission. |
