...
* Clone it from the shell or command line:
...
1. Enter a **Name** for the application, for example `AppModelv2-NativeClient-DotNet-TodoListService`.  Users of the app will see this name, and can be changed later.
...
In this step, the sample code will configure the service project to match the registered web API.

1. Open the solution in Visual Studio, and then open the *Web.config* file under the root of the TodoListService project.

1. Replace the value of the `ida:ClientId` parameter with the Client ID (Application ID) value from the application registered in the **App registrations** portal.
...
1. Paste the Application ID from the application that was registered for the TodoListService project in the `TodoListServiceScope` parameter, replacing the `{Enter the Application ID of your TodoListService from the app registration portal}` string.
...
Register the TodoListClient app in **App registrations** in the Azure portal, and then configure the code in the TodoListClient project. If the client and server are considered the same application, the application that's registered in step 2 can be reused. Use the same application if you want the application to allow sign-in with a personal Microsoft account.
...
1. When the **Register an application page** opens, enter the application's registration information:
   ...
   >
   > - `common`: Sign-in by using a work or school account or a personal Microsoft account (because  **Accounts in any organizational directory** was selected in a previous step).
   > - `organizations`: Sign-in by using a work or school account.
   > - `consumers`: Sign-in only by using a Microsoft personal account.
...
1. Select **API permissions**, and then complete these steps to add permissions:
    ...
    1. In the list of APIs, select **AppModelv2-NativeClient-DotNet-TodoListService API** or the name entered for the web API.
    1. Select the **access_as_user** permission check box if it's not already selected. Use the Search box if necessary.
    1. Select the **Add permissions** button.

### Configure your project

Configure the TodoListClient project by adding the Application ID to the *app.config* file.
...
1. In the TodoListClient, at the upper right, select **Sign in**, and then sign in with the same credentials used to register the application, or sign in as a user in the same directory.

   If sign-in is occurring for the first time, there may be a prompt to consent to the TodoListService web API.

   To help access the TodoListService web API and manipulate the *To-Do* list, the sign-in also requests an access token to the *access_as_user* scope.

## Pre-authorize your client application

 Users from other directories can also be allowed to access the web API by pre-authorizing the client application to access the web API. This can be achieved by adding the Application ID from the client app to the list of pre-authorized applications for the web API. By adding a pre-authorized client, this allows users to access the web API without having to provide consent.

1. In the **App registrations** portal, open the properties of the TodoListService app.
...
1. Press <kbd>F5</kbd> to run the project. The TodoListClient app opens.
1. At the upper right, select **Sign in**, and then sign in by using a personal Microsoft account, such as *live.com*, *hotmail.com*, or a work or school account.

## Optional: Limit sign-in access to certain users

By default, any personal accounts, such as *outlook.com* or *live.com* accounts, or work or school accounts from organizations that are integrated with Azure AD can request tokens and access the web API.

To specify who can sign in to the application, use one of the following options:

### Option 1: Limit access to a single organization (single tenant)

Sign-in access can be limited in the application to user accounts that are in a single Azure AD tenant, including guest accounts of that tenant. This scenario is common for line-of-business applications.
