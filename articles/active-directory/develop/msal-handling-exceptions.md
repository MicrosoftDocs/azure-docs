...
* *ClientAuthError:* Error class which denotes an issue with Client authentication. Most errors that come from the library will be ClientAuthErrors. These may be errors such as calling a login method when login is in progress, users cancelling login, etc.
