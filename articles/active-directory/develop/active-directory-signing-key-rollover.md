...
For security purposes, Microsoft identity platform’s signing key rolls on a periodic basis and, in the case of an emergency, could be rolled over immediately. There is no set or guaranteed time between these key rolls - any application that integrates with Microsoft identity platform should be prepared to handle a key rollover event no matter how frequently it may occur. If it doesn’t, and your application attempts to use an expired key to verify the signature on a token, the sign-in request will fail.  We recommend checking every 24 hours at a minimum, with the ability to handle emergency rolls of all keys at any time as a fall-back.  
