...
When a client acquires an access token to access a protected resource, the client also receives a refresh token. The refresh token is used to obtain new access/refresh token pairs when the current access token expires. A refresh token is bound to a combination of user and client. Refresh tokens are used only with authorization servers and are never sent to resource servers. Refresh tokens can be used to obtain additional access tokens with similar or lesser permissions and scope. 
A refresh token can be revoked at any time, and the token's validity is checked every time the token is used. Refresh tokens are not revoked when used to fetch new access tokens - it's best practice, however, to securely delete the old token when getting a new one.

Whenever access tokens expire but need to still access a resource that is protected, refresh tokens are used to acquire new access tokens. The user, therefore, is not prompted to re-authenticate. It's important to make a distinction between confidential clients and public clients, as this impacts how long refresh tokens can be used. For more information about different types of clients, see RFC 6749.
...
Using [token lifetime configuration](active-directory-configurable-token-lifetimes#refresh-and-session-token-lifetime-policy-properties), the lifetime of refresh tokens can be altered.  It is normal and expected for some tokens to go without use (e.g. the user does not open the app for 3 months) and therefore expire.  Apps will encounter scenarios where the login server rejects a refresh token due to its age.
