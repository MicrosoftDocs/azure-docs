...
The Microsoft authentication library (MSAL) abstracts the concept of refresh tokens. If MSAL with its token cache, you don't need to store, lookup, or update refresh tokens. Users will also see fewer sign-in prompts because refresh tokens can usually be updated without user intervention. In the situations where it can't (the app or user is no longer registered, for example), the MSAL library automatically takes care of prompting the user.
