...
- To distinguish [device code flow](scenario-desktop-acquire-token-device-code-flow.md), [Integrated Windows Authentication](scenario-desktop-acquire-token-iwa.md), and a [username and a password](scenario-desktop-acquire-token-usernamepassword.md) from a confidential client application using a client credential flow used in [daemon applications](scenario-daemon-overview.md), none of which requires a redirect URI, configure it as a public client application. To achieve this configuration:
