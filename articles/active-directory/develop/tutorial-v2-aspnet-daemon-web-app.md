---
title: Build a multitenant daemon with the Microsoft identity platform endpoint
...
# Build a multitenant daemon with the Microsoft identity platform endpoint
...
- One or more user accounts in your Azure AD tenant. This sample won't work with a Microsoft account (formerly Windows Live account). If you signed in to the [Azure portal](https://portal.azure.com) with a Microsoft account and have never created a user account in your directory before, you need to do that now.
...
If you don't want to use this automation, use the steps in the following sections.
...
1. If your account is present in more than one Azure AD tenant, select your profile on the menu at the top of the page, and then select **Switch directory**.
...
1. When the **Register an application page** appears, enter your application's registration information:
   ...
   - In the **Redirect URI (optional)** section, select **Web** in the combo box and enter the following redirect URIs:
     ...
     If there's more than one redirect URI, you'll need to add them from the **Authentication** tab later, after the app has been created successfully.
...
Clean the solution, rebuild the solution, run the UserSync application, and begin by signing in as an administrator in your Azure AD tenant. If you don't have an Azure AD tenant for testing, you can [follow these instructions](quickstart-create-new-tenant.md) to get one.

When you sign in, the app first asks you for permission to sign you in and read your user profile. This consent allows the application to ensure that you're a business user.

![User consent](./media/tutorial-v2-aspnet-daemon-webapp/firstconsent.png)

The application then tries to sync a list of users from your Azure AD tenant, via Microsoft Graph. If it can't, it asks you (the tenant administrator) to connect your tenant to the application.

The application then asks for permission to read the list of users in your tenant.

![Admin consent](./media/tutorial-v2-aspnet-daemon-webapp/adminconsent.PNG)

After you grant permission, you're signed out from the app. This sign-out ensures that any existing access tokens for Microsoft Graph is removed from the token cache. When you sign in again, the fresh token obtained will have the necessary permissions to make calls to Microsoft Graph.


When you grant the permission, the application can then query for users at any point. You can verify this by selecting the **Sync Users** button and refreshing the list of users. Try adding or removing a user and resyncing the list. (But note that it syncs only the first page of users.).
...
- Acquiring permissions from the tenant admin by using the admin consent endpoint: **Controllers\AccountController.cs**.

## Re-create this sample app
...
1. From the **Overview** tab of the app service, download the publish profile by selecting the **Get publish profile** link and save it. You can use other deployment mechanisms, such as from source control.
...
To provide a recommendation, visit the [User Voice page](https://feedback.azure.com/forums/169401-azure-active-directory).
...
For a simpler multitenant console daemon application, check out the [.NET Core daemon quickstart](quickstart-v2-netcore-daemon.md).
