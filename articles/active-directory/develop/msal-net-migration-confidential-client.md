...
- [Web api calling downstream web apis](?tabs=obo#migrate-web-api-calling-downstream-web-apis) supported by web APIs calling downstream web APIs on behalf of the user.
- [Web app calling web apis](?tabs=authcode#migrate-web-apps-calling-web-apis) supported by Web apps that sign in users and call a downstream web API.
...
It doesn't have a parameter of type `UserAssertion`. If it does, then your app is a web API, and it's using [on behalf of flow](?tabs=obo#migrate-web-api-calling-downstream-web-apis) scenario.
