...
In this quickstart, you'll learn how to write a .NET Core application that can get an access token, and then call the Microsoft Graph API using the app's own identity. This is useful for non-interactive processes, for example, schedule tasks, batch processes or Windows Services that run without a user's identity.
...
To request a token using app's identity, use `AcquireTokenForClientAsync`:
