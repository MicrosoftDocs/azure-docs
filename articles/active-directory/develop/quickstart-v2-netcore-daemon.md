...
In this quickstart, you download and run a code sample that demonstrates how a .NET Core application can obtain an access token to call the Microsoft Graph API and display a [list of users](/graph/api/user-list) in the directory. The code sample also demonstrates how a job or a windows service can run with an application identity, instead of a user's identity. 
