...
For the authentication flows which require a user interaction, MSAL caches the access, refresh, and ID tokens, as well as the `IAccount` object, which represents information about a single account. Learn more about [`IAccount`](https://docs.microsoft.com/dotnet/api/microsoft.identity.client.iaccount?view=azure-dotnet). For application flows, such as [client credentials](https://docs.microsoft.com/azure/active-directory/develop/msal-authentication-flows#client-credentials), only access tokens are cached, because the IAccount object and ID token require a user, and the refresh token is not applicable.
