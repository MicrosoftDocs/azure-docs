...
1. Edit **App.Xaml.cs** and replace the line starting with `private static string ClientId` with the following code:
...
Then, you can initialize MSAL using the following code:
...
#### Get a user token interactively
 ...
 Some situations require forcing users to interact with the Azure Active Directory v2.0 endpoint through a popup window to either validate their credentials or give consent. Some example scenarios include:

- The first time a user signs in to the application
- When a user needs to reenter their credentials because their password has expired
...
You don't want to require users to validate their credentials every time they need to access a resource. Most of the time, you want token acquisition and renewal without any user interaction. `AcquireTokenSilentAsync` is the method commonly used to obtain tokens used to access protected resources after the initial `AcquireTokenAsync` method:
