...
> [!IMPORTANT]
> Dynamic consent can be convenient, but presents a big challenge for permissions that require admin consent.  The admin consent experience in the **App registrations** and **Enterprise applications** blades in the portal don't know about those dynamic permissions at consent time. We recommend that a developer list all the admin privileged permissions that are needed by the app in the portal.  This enables tenant admins to consent on behalf of all their users in the portal, once.  Users don't need to go through the consent experience for those permissions on sign in. The alternative is to have an individual admin sign in to the app, trigger a consent prompt for the appropriate permissions, and select **consent for my entire org** in the consent dialogue.
