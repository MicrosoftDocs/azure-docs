...
This quickstart is applicable to both iOS and macOS apps. Some steps are needed only for iOS apps. Those steps are prefixed with [iOS only step] tag.
...
>      - Note that iOS configuration is also applicable to macOS applications. 
...
>    ```
> 1. [iOS only step] Right click **Info.plist** and select **Open As** > **Source Code**.
> 1. [iOS only step] Under the dict root node, replace with your ***Bundle Id***: 
>
>    ```xml
...
### [iOS only step] Additional App Requirements
