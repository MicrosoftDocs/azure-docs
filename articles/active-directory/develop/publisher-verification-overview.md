...
- The domain or sub-domain of the email address used during MPN account verification must either match the publisher domain configured on the app or a DNS-verified [custom domain](../fundamentals/add-custom-domain.md) added to the Azure AD tenant. The following domain matches are allowed: 
    - Root domain (Partner Center) to Root domain (Azure Active Directory) 
    - Exact sub-domain (Partner Center) to exact sub-domain (Azure Active Directory) 
    - Root domain (Partner Center) to sub-domain (Azure Active Directory) 
...
Publisher verification is currently not supported for:

  - Applications registered under consumer accounts (Hotmail, Messenger, OneDrive, MSN, Xbox Live, or Microsoft 365) 
  - Azure Active Directory B2C tenants 
  - Government tenants 
  - Email verified tenants 
  - xxx.onmicrosoft.com domain
