...
| `groups` | Optional formatting for group claims | JWT, SAML | | For more information, see [Group claims](#configuring-groups-optional-claims). The `groups` claim is used with the GroupMembershipClaims setting in the [application manifest](reference-app-manifest.md), which must be set as well. |
...
1. Add the following entry using the manifest editor:
   ...
   By default group bbject IDs are emitted in the group claim value. To modify the claim value to contain on premises group attributes, or to change the claim type to role, use `OptionalClaims` configuration as follows:
...
- The SAML tokens contains the `skypeId` directory schema extension (in this example, the app ID for this app is `ab603c56068041afb2f6832e2a17e237`). The SAML tokens exposes the Skype ID as `extension_ab603c56068041afb2f6832e2a17e237_skypeId`.
