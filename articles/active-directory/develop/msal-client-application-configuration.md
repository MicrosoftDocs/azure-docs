...
## Cloud instance
The *instance* is used to specify if your application is signing users from the Azure public cloud or from national clouds. By using MSAL in your code, you can set the Azure cloud instance by using an enumeration or by passing the URL to the [national cloud instance](authentication-national-cloud.md#azure-ad-authentication-endpoints) as the `Instance` member (if you know it).
...
## Client ID
The client ID is unique application (client) ID assigned to your app by Azure AD when the app was registered.
...
You can override the redirect URI by using the `RedirectUri` property (for example, if you use brokers). Here are some examples of redirect URIs in that case:
