...
For single tenant applications, adding or updating the AppId URI validates that the value uses either the HTTPS scheme or the default scheme (`api://{appId}`) provided by Azure AD.  If using the HTTPS scheme, the domain in the URI must be in the verified domain list in the customer tenant.  This could prevent applications from adding an AppId URI if the domain isn't in the verified domain list or value does not use the default scheme.
