...
Sometimes the intended meaning of an Azure Active Directory (Azure AD) "application" can be misunderstood, especially if supporting context is missing. The goal of this article is to make it clearer, by defining the conceptual and concrete aspects of Azure AD application integration, with an example of registration and consent for a [multi-tenant application](active-directory-dev-glossary.md#multi-tenant-application).
