...
One of the endpoints implemented by the [authorization server](#authorization-server) to support OAuth2 [authorization grant flows](#authorization-grant). Among other things, it provides an [authorization code](#authorization-code) to a [client application](#client-application), during the "authorization code grant" flow, upon successful authentication (and [consent](#consent)) of the [resource owner](#resource-owner). The client uses the authorization code later in the flow, to obtain an [access token](#access-token) from the [token endpoint](#token-endpoint), in exchange for the authorization code.
...
A class of [client application](#client-application) registered in Azure AD, that is designed to permit sign in and [consent](#consent) from user accounts that are provisioned in any Azure AD [tenant](#tenant), including ones other than the one where the application itself is registered. By contrast, an application registered as single-tenant, would only allow sign-ins from user accounts provisioned in the same tenant as the one where the application is registered. 
