...
To see the PowerShell script that created an out-of-box rule, select the rule in the sync rules editor and click **Export**. This gives you the PowerShell script that created the rule.
...
1. Mark the first out-of-box sync rule (this is the **In from AD-User Join**) in the sync rule editor and select **Export**. Copy the SR Identifier value.  
