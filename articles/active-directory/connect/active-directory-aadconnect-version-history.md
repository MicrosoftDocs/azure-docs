...
* Fixed an issue with the *Change user sign-in* task in Azure AD Connect wizard:
  ...
  * Note that the same issue also occurs if you have an existing Azure AD Connect deployment with Password Synchronization enabled and the user sign-in method is currently configured as *Pass-through Authentication*. While the user sign-in method remains configured as "Pass-through Authentication", you try to check/uncheck the *Enable Seamless Single Sign-On* option. Before the change is applied, the wizard incorrectly shows the "*Disable Password Synchronization*" prompt. However, Password Synchronization remains enabled after the change is applied. With this fix, the wizard no longer shows the prompt.
...
* Fixed an issue with the *Change user sign-in* task in Azure AD Connect wizard:
  ...
  * Note that the same issue also occurs if you have an existing Azure AD Connect deployment with Password Synchronization disabled and the user sign-in method is currently configured as *Pass-through Authentication*. While the user sign-in method remains configured as "Pass-through Authentication", you try to check/uncheck the *Enable Seamless Single Sign-On* option. Before the change is applied, the wizard incorrectly shows the "*Disable Password Synchronization*" prompt. When the change is applied, the wizard enables Password Synchronization. With this fix, the wizard no longer enables Password Synchronization.
