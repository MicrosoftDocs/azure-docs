...
The installer creates a Scheduled Task on the system that runs in the user’s context and is triggered on user sign-in. The task silently registers the user and device with Azure AD after the user signs-in is complete. The Scheduled Task can be found in the Task Scheduler Library under Microsoft > Workplace Join
...
The Windows 7 PC is registered to the user in Azure AD.

A device object and certificate is created in Azure AD. The object represents the user@device.
