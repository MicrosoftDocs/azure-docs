...
When you open it, you will see the default out-of-box rules.

![Sync Rule Editor](./media\active-directory-aadconnectsync-change-the-configuration/SRE.png)

### Navigating in the editor
The drop-downs at the top of the editor allows you to quickly find a particular rule. For example, if you want to see the rules where the attribute proxyAddresses is included, you would change the drop-downs to the following:  
...
At the bottom you have buttons that will take an action on a selected sync rule. **Edit** and **Delete** will do what you expect them to. **Export** will produce a PowerShell script for recreating the sync rule. This allows you to move a sync rule from one server to another.
...
The [scheduler](active-directory-aadconnectsync-feature-scheduler.md) will run every 30 minutes by default. You want to make sure it is not starting while you are making changes and troubleshoot your new rules. To temporarily disable the scheduler, start PowerShell and run `Set-ADSyncScheduler -SyncCycleEnabled $false`
...
![Inbound rule filtering](./media/active-directory-aadconnectsync-change-the-configuration/description.png)  
	...
	- Connected system: The system the object can be found in. In this case we select the Active Direcotory Connector.
	- Connected System/Metaverse Object Type: Select **User** and **Person** respectively.
	- Link Type: Change this to **Join**.
	- Precedence: Provide a value which is unique in the system. A lower numeric value indicates higher precedence.
...
Note that the sync engine is case sensitive both on the function name and the name of the attribute. If you type something wrong, you will see a warning when you add the rule. The editor will allow you to save and continue, so you would have to reopen the rule and correct the rule.
6. Click on **Add** to save the rule.

Your new custom rule should be visible with the other sync rules in the system.

### Verify the change
With this new change, you want to make sure it is working as expected and is not throwing any errors. Depending on the number of objects you have, there are two different way to do this.
