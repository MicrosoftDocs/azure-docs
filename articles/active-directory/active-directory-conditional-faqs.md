...
Policies are enforced for B2B users. However, in some cases the user may not be able to satisfy the policy requirement, for example, if their organization does not support multi-factor authentication. Policy is not currently enforced for SharePoint guest users. The guest relationship is maintained within SharePoint, so guest users accounts are not subject to access polices at the authenticatoin server. Guest access can be managed at SharePoint.
