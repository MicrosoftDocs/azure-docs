...
Transient objects are not always a problem and you may see them even in a healthy environment. With [Azure AD Connect sync V2 endpoint API](https://docs.microsoft.com/en-us/azure/active-directory/hybrid/how-to-connect-sync-endpoint-api-v2), transient objects should auto-resolve in subsequent delta synchronization cycles. A common example where you may find transient objects being generated occurs on AADConnect servers installed in staging mode, when an admin user permanently deletes an object directly in Azure AD using PowerShell and later synchronizes the object again.
