...
[!WARNING]
Device registration with federated domain does not require syncing devices to Azure AD using AD connect. So you may have device objects without the correct certificate populated under userCertificate attribute. Those devices will be deleted from Azure AD with this version if their number is less than the deletion threshold or if “export deletion threshold” feature is disabled. To fix those devices, you need to rejoin them again by running the commands "dsregcmd /leave" and "dsregcmd /join" (you can run these commands using GPO).
