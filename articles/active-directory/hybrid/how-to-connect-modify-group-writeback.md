...
description: This article describes how to modify the default behavior in Group Writeback in Azure AD Connect. 
...
To learn more about the group writeback feature and to understand which approach is best for your organization, refer to the [What you need to know](link article 1) article. 

 

You can change the default behavior in the following ways: 
...
If the original version of group writeback is already enabled and in use in your environment, then all your Microsoft 365 groups have already been written back to AD. Instead of disabling all Microsoft 365 groups, as described in [disabling automatic writeback](link same article ‘Disabled automatic writeback of Microsoft 365 groups’ section), you will want to review any use of the previously written back groups and disable only those that are no longer needed in on-prem AD. 
...
Configure directory settings to disable automatic writeback of newly created Microsoft 365 groups.  Update the NewUnifiedGroupWritebackDefault setting to false. 

Powershell: [New-AzureADDirectorySetting](https://docs.microsoft.com/en-us/azure/active-directory/enterprise-users/groups-settings-cmdlets) 
...
MS Graph: [Update group](https://docs.microsoft.com/en-us/graph/api/group-update?view=graph-rest-beta&tabs=http) 
...
Disable [Azure AD Connect sync scheduler](https://docs.microsoft.com/en-us/azure/active-directory/hybrid/how-to-connect-sync-feature-scheduler) 
...
Create a custom synchronization rule in Azure AD Connect to delete written back groups when they are disabled for writeback or soft deleted  
...
[Enable group writeback](link article 3) 

Enable Azure AD Connect sync scheduler 
...
Open the [synchronization rule editor](https://docs.microsoft.com/en-us/azure/active-directory/hybrid/how-to-connect-create-custom-sync-rule) 
...
If you are updating the default behavior to delete groups when disabled for writeback or soft deleted, we recommend that you enable the [Active Directory Recycle Bin](https://docs.microsoft.com/en-us/azure/active-directory/hybrid/how-to-connect-sync-recycle-bin) feature for your on-premises instances of Active Directory.  This will allow you to manually restore previously deleted AD groups, so that they can be rejoined to their respective Azure AD groups, if they were accidentally disabled for writeback or soft deleted. 
...
Next Steps: 
What you need to know before configuring Azure AD Connect group writeback 
