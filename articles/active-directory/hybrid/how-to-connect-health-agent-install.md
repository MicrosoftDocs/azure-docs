...
The following PowerShell command is required *only* when the health agent registration fails after a successful installation and configuration of Azure AD Connect. The Azure AD Connect Health services will start after the agent has been successfully registered.

Manually register the Azure AD Connect Health agent for Sync by using the following PowerShell command:
