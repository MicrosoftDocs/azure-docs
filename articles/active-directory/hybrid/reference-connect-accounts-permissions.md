...
The account you specify on the **Connect your directories** page must be created in Windows Server AD as a normal user object (VSA, MSA or gMSA are not supported), before installation. Azure AD Connect version 1.1.524.0 and later has the option to let the Azure AD Connect wizard create the AD DS Connector account that's used to connect to Windows Server AD.
