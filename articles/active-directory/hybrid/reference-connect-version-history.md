...
To learn more about what has changed in V2.0 and how this change affects you, see [Azure Active Directory Connect V2.0](whatis-azure-ad-connect-v2.md).
...
> [!NOTE]
> This release is an update release of Azure AD Connect. It's intended to be used by customers who are running an older version of Windows Server and can't upgrade their server to Windows Server 2016 or newer at this time. You can't use this version to update an Azure AD Connect V2.0 server.
...
> [!NOTE]
> This release is a hotfix update release of Azure AD Connect. It's intended for customers who're running Azure AD Connect on a server with Windows Server 2012 or 2012 R2.
...
We fixed a bug where, when you're syncing a large number of Password Hash Sync transactions, the Event log entry length exceeded the maximum-allowed length for a Password Hash Sync event entry. We now split the lengthy log entry into multiple entries.
...
> [!NOTE]
> This release is a major release of Azure AD Connect. For more details, see the [Azure Active Directory V2.0 article](whatis-azure-ad-connect-v2.md).
...
### Functional changes
 ...
 - With this release, you can use the user role Hybrid Identity Administrator to authenticate when you install Azure AD Connect. You no longer need to use the Global Administrator role.
 ...
 - Passwords are now reevaluated when an expired password is "unexpired," no matter if the password itself is changed. If for a user the password is set to "Must change password at next logon," and this flag is cleared (thus "unexpiring" the password), the "unexpired" status and the password hash are synced to Azure AD. When the user attempts to sign in in Azure AD, they can use the unexpired password.
To sync an expired password from Active Directory to Azure AD, use the [Synchronizing temporary passwords](how-to-connect-password-hash-synchronization.md#synchronizing-temporary-passwords-and-force-password-change-on-next-logon) feature in Azure AD Connect. You need to enable password writeback to use this feature so that the password the user updates is written back to Active Directory too.
...
- We revamped ADSyncTools with several new and improved cmdlets. For more details about these cmdlets, see the [ADSyncTools article](reference-connect-adsynctools.md).
...
- We now use the V2 endpoint for import and export, and we fixed an issue in the Get-ADSyncAADConnectorExportApiVersion cmdlet. To learn more about the V2 endpoint, see the [Azure AD Connect sync V2 endpoint article](how-to-connect-sync-endpoint-api-v2.md).
...
- We updated the Generic LDAP connector and the Generic SQL Connector to the latest versions. To read more about these connectors, see:
...
- We fixed a bug where the Single Object Sync cmdlet fails if the attribute flow data is null, for example, on exporting delete operation.
...
- We addressed an issue where users were allowed to deselect objects and attributes used in sync rules by using the UI and PowerShell. We now show friendly error messages if you try to deselect any attribute or object that's used in any sync rules.
...
- We fixed a bug where, when changing connector account permissions, if an object came in scope that hadn't changed since the last delta import, a delta import wouldn't import it. We now display a warning to alert the user of the issue.
- We fixed an accessibility issue where the screen reader wasn't reading the radio button position. We added added positional text to the radio button accessibility text field.
...
- We fixed a bug where a "stopped-extension-dll-exception" on Azure AD Connector exported after clean installing the Azure AD Connect version 1.6.X.X, which defaulted to using DirSyncWebServices API V2, by using an existing database. Previously, the setting export version to v2 was only being done for upgrades. We changed it so that it's set on clean install too.
...
This release fixes a bug in version 1.6.2.4 where, after upgrade to that release, the Azure AD Connect Health feature wasn't registered correctly and didn't work. If you've deployed build 1.6.2.4, update your Azure AD Connect server with this build to register the Health feature correctly.

## 1.6.2.4

> [!IMPORTANT]
> Update per March 30, 2021: We've discovered an issue in this build. After installation of this build, the Health services aren't registered. We recommend not installing this build. We will release a hotfix shortly.
...
- We updated default sync rules to limit membership in writeback groups to 50,000 members.
   ...
   - We added new default sync rules for limiting membership count in group writeback (Out to AD - Group Writeback Member Limit) and group sync to Azure AD (Out to AAD - Group Writeup Member Limit) groups.
   - We added member attribute to the 'Out to AD - Group SOAInAAD - Exchange' rule to limit members in written back groups to 50,000.
 - We updated Sync Rules to support Group Writeback v2. If the "In from AAD - Group SOAInAAD" rule is cloned and Azure AD Connect is upgraded:
   ...
   -If the "Out to AD - Group SOAInAAD" rule is cloned and Azure AD Connect is upgraded.
     ...
     - If the Cloned Custom Sync Rule doesn't flow some Mail and Exchange attributes, then new Exchange Sync Rule will add those attributes.
     - Group Writeback V2 is in private preview at this moment and not publicly available.
 ...
 - We updated AADConnectHealth agent to 3.1.83.0.
 ...
 - We removed Explicit column from CS Search page in the Old Sync UI.
 - We added additional UI to the Group Writeback flow to prompt the user for credentials or to configure their own permissions by using the ADSyncConfig module if credentials haven't already been provided in an earlier step.
 - Auto-create MSA for ADSync Service Account on a DC.
 - We added the ability to set and get Azure AD DirSync feature Group Writeback V2 in the existing cmdlets:
 ...
 - Changes to synchronization rules are now tracked to assist troubleshooting changes in the service. The cmdlet "Get-ADSyncRuleAudit" retrieves tracked changes.
 - We updated the Add-ADSyncADDSConnectorAccount cmdlet in the [ADSyncConfig PowerShell module](./how-to-connect-configure-ad-ds-connector-account.md#using-the-adsyncconfig-powershell-module) to allow a user in ADSyncAdmin group to change the AD DS Connector account.
...
### Bug fixes
 ...
 - We updated screen reader output for some items in the Azure AD Connect wizard. We updated the button hover color to satisfy contrast requirements. We updated the Synchronization Service Manager title color to satisfy contrast requirements.
 ...
 - We fixed an issue in the sync engine that caused CS objects to have an inconsistent link state.
 - We added import counters to Get-ADSyncConnectorStatistics output.
 - We fixed an unreachable domain de-selection (selected previously) issue in some corner cases during pass2 wizard.
 ...
 - We fixed an issue with build 1.5.18.0 if you use mS-DS-ConsistencyGuid as the source anchor and have clone the In from the Active Directory - Group Join rule.
 - Fresh Azure AD Connect installations will use the Export Deletion Threshold stored in the cloud if there's one available and there isn't a different one passed in.
 - We fixed an issue where Azure AD Connect doesn't read AD displayName changes of hybrid-joined devices.
...
- We fixed an issue where admin can't enable "Seamless Single Sign On" if AZUREADSSOACC computer account is already present in the "Active Directory".
- We fixed an issue that caused a staging error during V2 API delta import for a conflicting object that was repaired via the health portal.
- We fixed an issue in the import/export configuration where disabled custom rule was imported as enabled.
