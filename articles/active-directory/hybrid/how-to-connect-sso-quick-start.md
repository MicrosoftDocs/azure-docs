...
- **Set up your Azure AD Connect server**: If you use [pass-through authentication](how-to-connect-pta.md) as your sign-in method, no other prerequisite check is required. If you use [password hash synchronization](how-to-connect-password-hash-synchronization.md) as your sign-in method, and if there's a firewall between Azure AD Connect and Azure AD, ensure that:
  ...
  - If your firewall or proxy allows, add the connections to the allowlist for `*.msappproxy.net` URLs over port 443. If you require a specific URL instead of a wildcard for proxy configuration, you can configure `tenantid.registration.msappproxy.net`, where `tenantid` is the GUID of the tenant for which you're configuring the feature. If URL-based proxy exceptions aren't possible in your organization, you can instead allow access to the [Azure datacenter IP ranges](https://www.microsoft.com/download/details.aspx?id=41653), which are updated weekly. This prerequisite is applicable only when you enable the Seamless SSO feature. It isn't required for direct user sign-ins.
    ...
    > - If you have an outgoing HTTP proxy, make sure that the URL `autologon.microsoftazuread-sso.com` is on the allowlist. You should specify this URL explicitly because the wildcard might not be accepted.
...
- **Set up domain administrator credentials**: You must have domain administrator credentials for each Windows Server Active Directory (Windows Server AD) forest that:

  - You sync to Azure AD through Azure AD Connect.
  - Contains users you want to enable for Seamless SSO.

- **Enable modern authentication**: You must enable [modern authentication](/office365/enterprise/modern-auth-for-office-2013-and-2016) on your tenant to use this feature.
...
> [!NOTE]
> If Azure AD Connect doesn't meet your requirements, you can [enable Seamless SSO by using PowerShell](tshoot-connect-sso.md#manual-reset-of-the-feature). Use this option if you have more than one domain per Windows Server AD forest, and you want to be more targeted about the domain you want to enable Seamless SSO for.
...
> [!NOTE]
> The following instructions work only for Internet Explorer, Microsoft Edge, and Google Chrome on Windows (if it shares a set of trusted site URLs with Internet Explorer). Learn how to set up [Mozilla Firefox](#mozilla-firefox-all-platforms) and [Google Chrome on macOS](#google-chrome-all-platforms).

### Why do you need to modify user intranet zone settings?

By default, the browser automatically calculates the correct zone, either internet or intranet, from a specific URL. For example, `http://contoso/` maps to the intranet zone and `http://intranet.contoso.com/` maps to the internet zone (because the URL contains a period). Browsers don't send Kerberos tickets to a cloud endpoint, like to the Azure AD URL, unless you explicitly add the URL to the browser's intranet zone.
...
1. Go to **User Configuration** > **Policies** > **Administrative Templates** > **Windows Components** > **Internet Explorer** > **Internet Control Panel** > **Security Page**. Then select **Site to Zone Assignment List**.
...
1. Go to **User Configuration** > **Policies** > **Administrative Templates** > **Windows Components** > **Internet Explorer** > **Internet Control Panel** > **Security Page** > **Intranet Zone**. Then select **Allow updates to status bar via script**.

    :::image type="content" source="media/how-to-connect-sso-quick-start/sso11.png" alt-text="Screenshot that shows the Intranet Zone page with Allow updates to status bar via script selected.":::
...
Consider information that's specific to different types of browsers.
