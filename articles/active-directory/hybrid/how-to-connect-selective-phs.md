...
Important!
The settings to configure selective password hash synchronization will only produce effect over the user objects that have the attribute **adminDescription** populated in Active Directory with the string used for scoping filter in the custom rules created and after they have synchronized that attribute update. 
