...
Group writeback allows you to write cloud groups back to your on-premises Active Directory using Azure Active Directory Connect (Azure AD Connect) sync. This feature enables you to manage groups in the cloud, while controlling access to on-premises applications and resources.  
...
- Devices that are members of writeback enabled groups in Azure AD, will be written back as members to Active Directory. Azure AD registered and Azure AD Joined devices require device writeback to be enabled for group membership to be written back.
...
- **Optional**: Exchange Server 2016 CU15 or later   
  ...
  - If you haven't [prepared Active Directory for Exchange](/Exchange/plan-and-deploy/prepare-ad-and-domains?view=exchserver-2019), mail related attributes of groups won't be written back. 
...
- Groups that have been written back won't be deleted in Active Directory, if they're disabled for writeback or soft deleted. They will remain in Active Directory until they're hard deleted in Azure AD. Changes made to these groups in Azure AD won't be written back, until the groups are re-enabled for writeback or restored from soft delete state. This requirement protects the Active Directory groups from accidental deletion, should they be unintentionally disabled for writeback or soft deleted in Azure AD. 
...
- Cloud [distribution list groups](/exchange/recipients-in-exchange-online/manage-distribution-groups/manage-distribution-groups) created in Exchange Online cannot be written back to Active Directory, only Microsoft 365 and Azure AD security groups are supported. 
...
- Group Writeback does not support writeback of nested group members that have a scope of **Domain local** in Active Directory, since Azure AD security groups are written back with scope **Universal**. If you have a nested group like this, you'll see an export error in Azure AD Connect with the message “A universal group cannot have a local group as a member.”  The resolution is to remove the member with scope **Domain local** from the Azure AD group or update the nested group member scope in Active Directory to **Global** or **Universal** group. 
