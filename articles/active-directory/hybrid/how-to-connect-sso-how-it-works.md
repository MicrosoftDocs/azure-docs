...
- In addition, two Kerberos service principal names (SPNs) are created to be used during the Azure AD sign-in process.
...
>[!IMPORTANT]
> The `AZUREADSSOACC` computer account needs to be strongly protected for security reasons. We recommend that you store the computer account in an Organization Unit (OU) where only Domain Admins can manage them and where they are safe from accidental deletions. We also recommend that you turn off Kerberos delegation control on the computer account. The Kerberos decryption key on the computer account should also be treated as extremely sensitive. We highly recommend that you [roll over the Kerberos decryption key](how-to-connect-sso-faq.md#how-can-i-roll-over-the-kerberos-decryption-key-of-the-azureadssoacc-computer-account) of the `AZUREADSSOACC` computer account at least every 30 days.
