...
1. In the search box, type **MCM**.

	![Creating an Azure AD test user](./media/mcm-tutorial/tutorial_mcm_search.png)

1. In the results panel, select **MCM**, and then click **Add** button to add the application.
...
1. **[Creating a MCM test user](#creating-a-mcm-test-user)** - to have a counterpart of Britta Simon in MCM that is linked to the Azure AD representation of user.
...
1. To configure single sign-on on **MCM** side, you need to send the downloaded **Metadata XML** to [MCM support team](http://mcmtechnology.com/support/). They set this setting to have the SAML SSO connection set properly on both sides.
