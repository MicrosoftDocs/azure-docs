...
3. **[Create a Flock test user](#create-a-flock-test-user)** - to have a counterpart of Britta Simon in Flock that is linked to the Azure AD representation of user.
...
3. On the **Flock Domain and URLs** section, perform the following steps:
...
6. On the **Flock Configuration** section, click **Configure Flock** to open **Configure sign-on** window. Copy the **SAML Entity ID and SAML Single Sign-On Service URL** from the **Quick Reference section.**

	![Flock Configuration](./media/flock-tutorial/tutorial_flock_configure.png) 

7. In a different web browser window, log in to your Flock company site as an administrator.

8. Select **Authentication** tab from the left navigation panel and then select **SAML Authentication**.
