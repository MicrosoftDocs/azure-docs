...
1. In the search box, type **Cimpl**.

	![Creating an Azure AD test user](./media/cimpl-tutorial/tutorial_cimpl_search.png)

1. In the results panel, select **Cimpl**, and then click **Add** button to add the application.
...
1. **[Creating a Cimpl test user](#creating-a-cimpl-test-user)** - to have a counterpart of Britta Simon in Cimpl that is linked to the Azure AD representation of user.
...
1. On the **Cimpl Domain and URLs** section, perform the following steps:
...
1. On the **Cimpl Configuration** section, click **Configure Cimpl** to open **Configure sign-on** window. Copy the **SAML Entity ID, and SAML Single Sign-On Service URL** from the **Quick Reference section.**

	![Configure Single Sign-On](./media/cimpl-tutorial/tutorial_cimpl_configure.png) 

1. To configure single sign-on on **Cimpl** side, you need to send the downloaded **Certificate (Base64)**, **SAML Entity ID, and SAML Single Sign-On Service URL** to Cimpl support at **+1 866-982-8250**.
