...
1. In the search box, type **Ariba**.

	![Creating an Azure AD test user](./media/ariba-tutorial/tutorial_ariba_search.png)

1. In the results panel, select **Ariba**, and then click **Add** button to add the application.
...
For single sign-on to work, Azure AD needs to know what the counterpart user in Ariba is to a user in Azure AD. In other words, a link relationship between an Azure AD user and the related user in Ariba needs to be established.

In Ariba, assign the value of the **user name** in Azure AD as the value of the **Username** to establish the link relationship.
...
1. **[Creating an Ariba test user](#creating-an-ariba-test-user)** - to have a counterpart of Britta Simon in Ariba that is linked to the Azure AD representation of user.
...
1. On the **Ariba Domain and URLs** section, perform the following steps:
...
1. To get SSO configured for your application, call Ariba support team on **1-866-218-2155** and they'll assist you further on how to provide them the downloaded **Certificate (Base64)** file.  
