...
1. In the search box, type **Condeco**.

	![Creating an Azure AD test user](./media/condeco-tutorial/tutorial_condeco_search.png)

1. In the results panel, select **Condeco**, and then click **Add** button to add the application.
...
1. **[Creating a Condeco test user](#creating-a-condeco-test-user)** - to have a counterpart of Britta Simon in Condeco that is linked to the Azure AD representation of user.
...
1. On the **Condeco Domain and URLs** section, perform the following steps:
...
1. To configure single sign-on on **Condeco** side, you need to send the downloaded **Metadata XML** to [Condeco support team](mailTo:supportna@condecosoftware.com). They set this setting to have the SAML SSO connection set properly on both sides.
