...
4. On the **Basic SAML Configuration** section, enter the following values into the corresponding input fields:

	| Input Field Name | Value | Note | 
	| ---------------------- | ----- | ---- |
	| **Reply URL (Assertion Consumer Service URL)** | `https://app.trakstar.com/auth/saml/callback?namespace=<YOUR_NAMESPACE>` | Replace `<YOUR_NAMESPACE>` with a real value, visible in the ACS (Consumer) URL field in Trakstar Perform. See below note. |
	| **Sign on URL** | `https://app.trakstar.com/auth/saml/?namespace=<YOUR_NAMESPACE>` | This URL is _similar_ to the above URL, but lacks the `/callback` portion. |
	| **Identifier (Entity ID)** | `https://app.trakstar.com` | |
	
	> [!NOTE]
	> These values are only examples. You must use the values specific to your namespace in Trakstar Perform, which are visible by logging into the application and navigating to Settings -> Authentication & SSO -> SAML 2.0 -> Configure.
	> If you don't see the "Authentication & SSO" tab within Settings, you might not have the feature and should contact Trakstar customer support. You can also refer to the patterns shown in the **Basic SAML Configuration** section in the Azure portal.
