...
1. In the search box, type **BitaBIZ**, select **BitaBIZ** from result panel then click **Add** button to add the application.
...
1. **[Create a BitaBIZ test user](#create-a-bitabiz-test-user)** - to have a counterpart of Britta Simon in BitaBIZ that is linked to the Azure AD representation of user.
...
1. On the **BitaBIZ Domain and URLs** section, perform the following steps if you wish to configure the application in IDP initiated mode:
...
1. On the **BitaBIZ Configuration** section, click **Configure BitaBIZ** to open **Configure sign-on** window. Copy the **SAML Entity ID, and SAML Single Sign-On Service URL** from the **Quick Reference section.**
...
1. Click on **SETUP ADMIN**.

	![BitaBIZ Configuration](./media/bitabiz-tutorial/settings1.png)

1. Click on **Microsoft integrations** under **Add value** section.

	![BitaBIZ Configuration](./media/bitabiz-tutorial/settings2.png)

1. Scroll down to the section **Microsoft Azure AD (Enable single sign on)** and perform following steps:
...
1. Click on **Add users** under **Organization** section.
...
1. On the **“Add new employee”** dialog page, perform the following steps:
