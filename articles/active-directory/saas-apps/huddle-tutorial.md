...
1. In the search box, type **Huddle**.

	![Creating an Azure AD test user](./media/huddle-tutorial/tutorial_huddle_search.png)

1. In the results panel, select **Huddle**, and then click **Add** button to add the application.
...
1. **[Creating a Huddle test user](#creating-a-huddle-test-user)** - to have a counterpart of Britta Simon in Huddle that is linked to the Azure AD representation of user.
...
1. On the **Huddle Domain and URLs** section, perform the following steps:
...
1. On the **Huddle Configuration** section, click **Configure Huddle** to open **Configure sign-on** window. Copy the **SAML Entity ID, and SAML Single Sign-On Service URL** from the **Quick Reference section.** 

	![Configure Single Sign-On](./media/huddle-tutorial/tutorial_huddle_configure.png) 
	
1. To configure single sign-on on Huddle side, you need to send the downloaded  **Certificate**, **SAML Single Sign-On Service URL**, and **SAML Entity ID** to [Huddle Client support team](https://huddle.zendesk.com). They set this setting to have the SAML SSO connection set properly on both sides.  
...
1. Click **Workspace**.
1. Click **People \> Invite People**.
   
   ![People](./media/huddle-tutorial/IC787838.png "People")

1. In the **Create a new invitation** section, perform the following steps:
