...
4. In the search box, type **BenSelect**.

	![Creating an Azure AD test user](./media/benselect-tutorial/tutorial_benselect_search.png)

5. In the results panel, select **BenSelect**, and then click **Add** button to add the application.
...
3. **[Creating a BenSelect test user](#creating-a-benselect-test-user)** - to have a counterpart of Britta Simon in BenSelect that is linked to the Azure AD representation of user.
...
3. On the **BenSelect Domain and URLs** section, perform the following steps:
...
8. On the **BenSelect Configuration** section, click **Configure BenSelect** to open **Configure sign-on** window. Copy the **Sign-Out URL, SAML Entity ID, and SAML Single Sign-On Service URL** from the **Quick Reference section.**
