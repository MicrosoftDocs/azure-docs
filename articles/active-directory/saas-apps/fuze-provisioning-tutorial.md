...
Before configuring and enabling automatic user provisioning, you should decide which users and/or groups in Azure AD need access to Fuze. Once decided, you can assign these users and/or groups to Fuze by following the instructions here:

* [Assign a user or group to an enterprise app](../manage-apps/assign-user-or-group-access-portal.md)

## Important tips for assigning users to Fuze

* It is recommended that a single Azure AD user is assigned to Fuze to test the automatic user provisioning configuration. Additional users and/or groups may be assigned later.

* When assigning a user to Fuze, you must select any valid application-specific role (if available) in the assignment dialog. Users with the **Default Access** role are excluded from provisioning.
...
Before configuring Fuze for automatic user provisioning with Azure AD, you need to add Fuze from the Azure AD application gallery to your list of managed SaaS applications.

**To add Fuze from the Azure AD application gallery, perform the following steps:**
...
4. In the search box, enter **Fuze**, select **Fuze** in the results panel, and then click the **Add** button to add the application.

	![Fuze in the results list](common/search-new-app.png)

## Configuring automatic user provisioning to Fuze 
...
> [!TIP]
> You may also choose to enable SAML-based single sign-on for Fuze , following the instructions provided in the [Fuze Single sign-on tutorial](Leapsome-tutorial.md). Single sign-on can be configured independently of automatic user provisioning, though these two features compliment each other
...
8. Under the **Mappings** section, select **Synchronize Azure Active Directory Users to Fuze**.
	...
	![Fuze User Mappings](media/fuze-provisioning-tutorial/image01.png)
...
9. Review the user attributes that are synchronized from Azure AD to Fuze in the **Attribute Mapping** section. The attributes selected as **Matching** properties are used to match the user accounts in Fuze for update operations. Select the **Save** button to commit any changes.
	...
	![Fuze User Mappings](media/fuze-provisioning-tutorial/image00.png)
...
This operation starts the initial synchronization of all users and/or groups defined in **Scope** in the **Settings** section. The initial sync takes longer to perform than subsequent syncs, which occur approximately every 40 minutes as long as the Azure AD provisioning service is running. You can use the **Synchronization Details** section to monitor progress and follow links to provisioning activity report, which describes all actions performed by the Azure AD provisioning service on Fuze.
