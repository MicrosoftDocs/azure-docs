...
* Automatically sign your users in to Jamf Pro with their Azure AD accounts
* Manage your accounts in one central location: the Azure portal

To learn more about SaaS app integration with Azure AD, see [Single sign-on with Azure Active Directory](https://docs.microsoft.com/azure/active-directory/active-directory-appssoaccess-whatis).
...
* A Jamf Pro SSO-enabled subscription.
...
1. **[Configure SSO in Azure AD](#configure-sso-in-azure-ad)** so that your users can use this feature.
    ...
    1. **[Add the Azure AD test user to Jamf Pro](#add-the-azure-AD-test-user-to-jamf-pro)** so that B.Simon can use SSO in Azure AD.
...
1. In the **User** properties, follow these steps:
   1. In the **Name** field, type `B.Simon`.  
   1. In the **User name** field, type [name]@[companydomain].[extension]. For example, `B.Simon@contoso.com`.
   1. Select the **Show password** check box, and then write down the value that's displayed in the **Password** box.
   1. Select **Create**.

### Add the Azure AD test user to Jamf Pro
...
3. To set up Jamf Pro manually, open a new web browser window and sign in to your Jamf Pro company site as an administrator. Then, do the following steps.
...
6. On the **Single Sign-On** page, do the following steps:
	...
	b. Select **Other** as an option from the **IDENTITY PROVIDER** dropdown menu.

	c. In the **OTHER PROVIDER** field, type **Azure AD**.
...
To provision a user account, do the following steps.
...
6. On the **New Account** dialog box, do the following steps:

	![New account setup options in Jamf Pro system settings.](./media/jamfprosamlconnector-tutorial/user4.png)

	a. In the **USERNAME** field, type `Britta Simon`, the full name of the test user.

	b. Select the options for **ACCESS LEVEL**, **PRIVILEGE SET**, and **ACCESS STATUS** that are in accordance with your organization.

	c. In the **FULL NAME** field, type `Britta Simon`.

	d. In the **EMAIL ADDRESS** field, type the email address of Britta Simon's account.

	e. In the **PASSWORD** field, type the user's password.

	f. In the **VERIFY PASSWORD** field, type the user's password again.
