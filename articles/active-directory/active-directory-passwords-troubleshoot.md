...
| 6329 | MMS(3040): admaexport.cpp(2837): The server does not contain the LDAP password policy control. | This issue occurs if LDAP_SERVER_POLICY_HINTS_OID control (1.2.840.113556.1.4.2066) is not enabled on the domain controllers. To use the password writeback feature, you must enable the control. To do so, the domain controllers must be on Windows Server 2008 (with latest SP) or later. If your DCs are on 2008 (pre-R2), then you must also apply hotfix [KB2386717](http://support.microsoft.com/kb/2386717). |
...
The most common point of failure is that firewall and or proxy ports and idle timeouts are incorrectly configured. Review the connectivity prerequisites in the article [Prerequisites for Azure AD Connect](./connect/active-directory-aadconnect-prerequisites.md) for more information.
