...
RBAC (role-based access control) is an Azure portal only feature allowing the owners of Azure subscription to assign granular roles to other users who can manage specific resource scopes in their Azure environment. As this type of granularity provided by the RBAC feature is available only in the Azure portal consequently cannot be used or accessed in the Azure classic portal. This feature has been one of the major updates of the Azure portal.

The reasoning behind it allows better security management for large organizations and for SMBs working with external collaborators, vendors or freelancers which need access to specific resources in your Azure environment but not necessarily to the entire infrastructure or any billing related scopes. RBAC allows the flexibility of owning one Azure subscription managed by the administrator account (service administrator at a subscription level role) and have multiple users invited to work under the same subscription but without any administrative rights for it. From a management perspective as well as from a billing scope, the RBAC feature proves to be a time and management efficient option for using Azure in various scenarios.
...
> [!NOTE]
> Office 365 subscriptions or Azure Active Directory licenses (e.g.: Access to Azure Active Directory) provisioned from the O365 portal do not quality for using RBAC.
...
* Resource group
* Resource scope (the lowest level offering targeted permissions to an individual Azure resource scope)
...
* Working with users inside of the organization (they are part of the user's Azure Active Directory tenant) but part of different teams or groups which need granular access either to the whole subscription or to certain resource groups or resource scopes in the environment
...
From the Azure portal, after the admin is signed in, he must select “Subscriptions” and select the desired one.
    ![subscription blade in Azure portal](./media/role-based-access-control-create-roles-for-internal-external-users/0.png)
By default, if the admin user has purchased the Azure subscription, he will show up as **Account Admin**, this being the subscription role. For more details on the Azure subscription roles see [Add or change Azure administrator roles that manage the subscription or services](/billing/billing-add-change-azure-subscription-administrator.md).

In this example the user "alflanigan@outlook.com" is the **Owner** of the "Free Trial" subscription in the AAD tenant "Default tenant Azure". Since this user is the creator of the Azure subscription with the initial Microsoft Account “Outlook” (Microsoft Account = Outlook, Live etc.) the default domain name for all other users added in this tenant will be **"@alflaniganuoutlook.onmicrosoft.com"**. By design, the syntax of the new domain will be formed by putting together the username and domain name of the user who created the tenant and adding the extension **".onmicrosoft.com"**.
Further, users can login with a custom domain name in the tenant after adding and verifying it for the new tenant. For more details on how to verify a custom domain name in an Azure Active Directory tenant see [Add a custom domain name to your directory](/active-directory/active-directory-add-domain)

In this example, the "Default tenant Azure" directory will contain only users with the domain name "@alflanigan.onmicrosoft.com".

After selecting the subscription, the admin user must click on **Access Control (IAM)** and then **Add a new role**.
...
The next step is to select the role to be assigned and the user whom the RBAC role will be assigned to. In the **Role** dropdown menu the admin user will see only the built-in RBAC roles which are available in Azure. For more detailed explanations of each role and their assignable scopes see [Built-in roles for Azure Role-Based Access Control](/active-directory/role-based-access-built-in-roles.md).

The admin user then needs to add the email address of the external user. The expected behaviors is for the external user to not show up in the existing tenant. After the external user has been invited, the he will be visible under **Subscriptions > Access Control (IAM)** with all the current users which are currently assigned an RBAC role at the Subscription scope.
...
The external user will show up in the Azure Active Directory tenant from now on as external user and this can be viewed both in the Azure portal as well as in the classic portal.
...
However, granting **Owner** or **Contributor** access to an external user at the **Subscription** scope, does not allow him access to the admin user's directory, unless the **Global Admin** allows this. In the user proprieties,  the **User Type** which has two common parameters **Member** and **Guest** can be identified. A member is a user which is registered in the directory while a guest is a user invited to the directory from an external source. For more information see [How do Azure Active Directory admins add B2B collaboration users](active-directory-b2b-admin-add-users).

> [!NOTE]
> Make sure that after entering the credentials in the portal, the external user selects the correct directory he is signed in. The same user can have access to multiple directories and he can select either one of  them by clicking the username in the top right hand side in the Azure portal and then choose the appropriate one from the dropdown list.

While the external user is a guest in the directory, he can manage all resources for the Azure subscription, but he will not have access to the directory.
    ...
    ![subscription blade for RBAC user in Azure portal](./media/role-based-access-control-create-roles-for-internal-external-users/10.png)

The Azure Active Directory and an Azure subscription don't have a child-parent relation like other Azure resources (e.g. virtual machines, virtual networks, web apps, storage etc.) have with an Azure subscription. All the latter are created, managed and billed under an Azure subscription while an Azure subscription is used to manage the access to an Azure directory. For more details see [How an Azure subscription is related to Azure AD](active-directory-how-subscriptions-associated-directory)
...
The process flow is identical to adding an external user, both from the admin perspective granting the RBAC role as well as the user being granted access to the role. The only difference is that the invited user will not receive any email invitations, he will just be able to see all the resource scopes within the subscription he has been assigned the RBAC role directly in his dashboard, after he will log in.

## Assigning RBAC roles at the resource group scope
Assigning an RBAC role at a **Resource Group** scope has an identical process for assigning the role at the subscription level, for both types of users - either external or internal (part of the same directory). The normal behavior for the users which are assigned the RBAC role is to see in their environment only the resource group they have been assigned access from the **Resource Groups** icon in the Azure portal.
...
An important aspect for RBAC both at resource group scope or resource scope is for the users to make sure to log in to the correct directory.
...
One typical scenario which might be extremely helpful is to understand the prerequisites of creating a custom role which can grant granular access at the subscription level and also allow the invited user the flexibility of opening support requests.
...
Last, it's mandatory that the RBAC role contains the explicit subscription IDs where it will be used. This is listed under the **AssignableScopes**, otherwise you will not be allowed to import the role in your subscription.
...
In this example, the custom name for this RBAC role is "Reader support tickets access level" allowing the user to view everything in the subscription and additionally to open support requests.

> [!NOTE]
> The only two built-in RBAC roles allowing the action of opening of support requests are **Owner** and **Contributor**. For a user to be able to open support requests, he must be assigned a RBAC role only at the subscription scope, because all support requests are created based on an Azure subscription.

This new custom role has been assigned to the user "radu@andreicraduoutlook.onmicrosoft.com" which is in the same directory.
...
* Can't create new resource scopes (e.g. virtual machine)
