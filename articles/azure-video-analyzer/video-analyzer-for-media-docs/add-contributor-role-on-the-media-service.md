...
> [!NOTE]
> You'll need an Azure subscription where you have access to both the [Contributor](articles/role-based-access-control#contributor) role and the [User Access Administrator](articles/role-based-access-control#user-access-administrator) role to the Azure Media Services and the User-assigned managed identity. If you don't have the right permissions, ask your account administrator to grant you those permissions. The associated Azure Media Services must be in the same region as the Video Analyzer for Media account.
