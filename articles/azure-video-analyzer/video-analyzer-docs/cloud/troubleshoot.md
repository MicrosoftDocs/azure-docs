...
metricsVideo analyzer also emits metrics for pipelines which can help in identifying issues as follows.
o	IngressBytes - The total number of bytes received by a pipeline. A steadily increasing value indicates the pipeline is healthy, and is receiving video data from the RTSP camera
o	Pipeline- Helps with checking pipeline status and counts.
...
If you are using the Video Analyzer widget to play the video, try using the Azure portal if you have access to it. If the video plays in the Azure portal, but not in the widget, then you should proceed to check the widget section below<TODO: link> <!-- ToDo - Add LINK -->
...
If the service is not receiving video data from the RTSP camera, the next step is to [view the relevant diagnostic logs](todo – link to the section above)<!-- ToDo - Add LINK -->.
...
- If you see Microsoft.VideoAnalyzer.Diagnostics.ProtocolError in diagnostics logs, with a code set to 401 as follows, then the first step is to recheck the RTSP credentials.
...
- If you are using a **remote device adapter** <TODO: link>, then try the following steps.
   ...
   - Run ‘remoteDeviceAdapterList’ direct method on the edge module and verify IP address. Sample request and response is shown in TODO: link to section after https://github.com/MicrosoftDocs/azure-docs-pr/pull/176525 is merged
...
- If the above steps do not help resolve the issue and video playback still does not work, then log into the Azure portal and open a support ticket. You may need to attach the logs from the Video Analyzer edge module <TODO: link to the section in edge troubleshooting doc where sudo iotedge support-bundle is called>

### Unable to record to a video resource – TBD error code
...
o	Make sure you have generated the Client API JWT Token and corresponding access policy, refer to the documentation here and here. The player will not work if the access policy has not been setup correctly, and the JWT token does not match with the policy. 
o	If issue is not resolved, gather the widget logs, and file a support ticket via Azure portal
...
When self-guided troubleshooting steps don't resolve your problem and there are any additional issues that you may need help with, please collect logs and submit a support ticket. You can also reach out to us by sending us an email at videoanalyzerhelp@microsoft.com.
Always try to reproduce the issue, gather the relevant logs that should be added to the ticket, and upload the log files in the Details pane of the support request.
