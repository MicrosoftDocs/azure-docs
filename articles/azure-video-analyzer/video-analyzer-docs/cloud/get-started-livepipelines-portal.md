...
**Deployment steps:**
1. Deploy a standard D2 series Azure Linux VM, [see here](../../../virtual-machines/linux/quick-create-portal.md).
1. Set up its networking configuration by going to VM Networking settings:
    - Add inbound port rule: Set Destination port as 554, Protocol as any, ‘Allow’ Action for RTSP stream with other default values [see details here](../../../virtual-machines/windows/nsg-quickstart-portal.md)
...
1. Start the rtsp live555 media simulator container by command below, using pre-built rtspsim image that exists on the MCR 
    ...
    - Replace <localfolder> string with the folder created in previous steps, it will map your host VM local folder to RTSP mediaserver container
...
1. In the Topologies plane, select the **Create** option from the top to create a live topology. Follow the portal wizard steps to create a live topology
    - **Create a pipeline topology** wizard will appear on the portal
    - For this getstarted document we will use prebuild sample topologies. Select **Try sample topologies**-> select **CVR from public camera** topology-> Select 'Proceed' on **Load sample topology** dialog box.
...
1. Next step is to create a live pipeline using the topology created in previous step. 
    - Select **Pipelines**-> Select **Create**e -> then select the ‘Live pipeline topology’ created in previous step to a create a pipeline. After selecting the topology click **Create**
