...
1. The *operations.json* code starts off with calls to the direct methods `PipelineTopologyList` and `PipelineInstanceList`. If you cleaned up resources after you completed previous quickstarts/tutorials, then this process will return empty lists and then pause. To continue, select the Enter key.
