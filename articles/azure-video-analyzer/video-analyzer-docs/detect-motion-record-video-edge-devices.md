...
This article is based on sample code written in C#. It builds on the Detect motion and emit events quickstart.
...
In the Generate and deploy the IoT Edge deployment manifest step, in Visual Studio Code, expand the ava-sample-device node under AZURE IOT HUB (in the lower-left section). You should see the following modules deployed:
...
1. Right-click and select Extension Settings.
    ...
    > :::image type="content" source="./media/analyze-live-video-use-your-model-http/verbose-message.png" alt-text= "Show Verbose Message"::: <!--change path to common-->
...
When motion is detected, the signal gate processor node is activated, and the file sink node in the media graph starts to the write an MP4 file. The file sink node sends an operational event. The type is set to motion to indicate that it's a result from the motion detection processor. The eventTime value is the UTC time at which the motion occurred. For more information about this process, see the Overview section in this quickstart.
...
If you examine the properties of the signal gate processor node in the graph topology, you see that the activation times are set to 5 seconds. So about 5 seconds after the RecordingStarted event is received, you get:
...
2.	Sign in by using the credentials that were generated when you set up your Azure resources.
3.	At the command prompt, go to the relevant directory. The default location is /var/media. You should see the MP4 files in the directory.
 
4.	Use Secure Copy (SCP) to copy the files to your local machine.
5.	Play the files by using VLC media player or any other MP4 player.
