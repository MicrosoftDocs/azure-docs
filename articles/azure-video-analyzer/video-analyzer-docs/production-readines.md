...
When you deploy the Azure Video Analyzer on IoT Edge module to an edge device, by default it runs with elevated privileges. When you do this, if you check the logs on the module (`sudo iotedge logs {name-of-module}`), you will see the following:
...
You can and should run the Azure Video Analyzer on IoT Edge module in production using an account with as few privileges as possible. The following commands, for example, show how you can create a local user account on a Linux VM:
...
The Azure Video Analyzer on IoT Edge module requires the ability to write files to the local file system when:
...
Pipelines allows for creation of videos in the cloud or mp4 files on the edge. These can be generated by [continuous video recording](use-continuous-video-recording.md) or by [event-based video recording](record-event-based-live-video.md). While these  can be named as you want, the recommended naming structure for continuous video recording-based video is "<anytext>-${System.TopologyName}-${System.PipelineName}".
...
```
"fileNamePattern": "sampleFilesFromEVR-${fileSinkOutputName}--${System.TopologyName}-${System.PipelineName} ${System.DateTime}"
```
...
System.PreciseDateTime and System.DateTimecannot be used when naming videos in the cloud
...
4. `sudo docker image ls` – Provides a list of Docker images on your edge system
...
[Quickstart: Get started - Azure Video analyzer on IoT Edge](get-started-detect-motion-emit-events.md)