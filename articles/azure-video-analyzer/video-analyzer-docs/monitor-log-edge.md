...
Video Analyzer on IoT Edge emits events, or telemetry data, according to the following taxonomy:
...
You can use the following module twin properties to route the operational and diagnostic events published by the Video Analyzer on IoT Edge module. These properties are documented in the [module twin JSON schema](module-twin-configuration-schema.md).
...
In the preceding JSON, `avaedge` is the name of the Video Analytics on IoT Edge module. The routing rule follows the schema defined in [Declare routes](../../iot-edge/module-composition.md#declare-routes).
...
Events that originate on the edge device can be consumed at the edge or in the cloud. Events generated by Video Analytics on IoT Edge conform to the [streaming messaging pattern](../../iot-hub/iot-hub-devguide-messages-construct.md) established by Azure IoT Hub. The pattern consists of system properties, application properties, and a body.
...
These metrics will be reported from the Video Analytics on IoT Edge module through a [Prometheus endpoint](https://prometheus.io/docs/instrumenting/exposition_formats/#text-based-format) running on port 9600 of the module.
...
> [!NOTE]
> If you name your Video Analyzer on IoT Edge module `avaedge`, the metrics can be accessed by sending a GET request to `http://avaedge:9600/metrics`. Depending on your deployment, you may need to map this port to the device for it to be reachable.
...
[Troubleshoot Azure Video Analyzer on IoT Edge](troubleshoot.md)
