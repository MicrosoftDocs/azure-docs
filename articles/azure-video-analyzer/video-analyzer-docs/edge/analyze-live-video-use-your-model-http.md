...
description: Analyze a live video feed from an IP camera; apply a computer vision model to detect objects. Some frames in the feed are sent to an inference service; results are sent to IoT Edge Hub.
...
# Quickstart: analyze a live video feed from an IP camera using your own model - HTTP
