...
To learn more about how to deploy StandardV2 NAT Gateway, see [Create a StandardV2 NAT Gateway](./quickstart-create-nat-gateway-v2.md).
...
* IPv6 outbound traffic that uses Load balancer outbound rules is disrupted when you associate StandardV2 NAT Gateway to a subnet. If you require both IPv4 and IPv6 outbound connectivity, use either Load balancer outbound rules for both IPv4 and IPv6 traffic or use Standard NAT Gateway for IPv4 traffic and Load balancer outbound rules for IPv6 traffic.
...
* Outbound connections that use Load balancer, Azure Firewall, or virtual machine instance level public IPs may be interrupted when you add StandardV2 NAT gateway to a subnet. All net new outbound connections use the StandardV2 NAT gateway. 
...
You can associate Standard NAT Gateway to subnets within the same virtual network in order to provide outbound connectivity to the internet. Standard NAT Gateway operates out of a single availability zone. 
...
Deployments with NAT Gateway are intentionally simple. Attach NAT Gateway to a subnet and public IP address, and start connecting outbound to the internet right away. There's zero maintenance and routing configurations required. You can add more public IPs or subnets later without affecting your existing configuration. 
...
NAT Gateway is scaled out from creation. There's no ramp-up or scale-out operation required. Azure manages the operation of NAT Gateway for you. 

Attach NAT Gateway to a subnet to provide outbound connectivity for all private resources in that subnet. All subnets in a virtual network can use the same NAT Gateway resource. You can scale out outbound connectivity by assigning up to 16 public IP addresses to NAT Gateway. When you associate a NAT Gateway to a public IP prefix, it automatically scales to the number of IP addresses needed for outbound.
...
A NAT Gateway doesn't affect the network bandwidth of your compute resources. For more information, see [NAT Gateway's performance](nat-gateway-resource.md#performance).
...
>[!NOTE]
>On March 31, 2026, new virtual networks default to using private subnets, so [default outbound access](/azure/virtual-network/ip-services/default-outbound-access#when-is-default-outbound-access-provided) isn't provided by default. Use an explicit form of outbound connectivity instead, like NAT Gateway. 
...
* NAT Gateway doesn't require any routing configurations on a subnet route table. After you attach NAT Gateway to a subnet, it provides outbound connectivity right away.
...
* NAT Gateway doesn't have the same limitations of SNAT port exhaustion as [default outbound access](../virtual-network/ip-services/default-outbound-access.md) and [outbound rules of a Load Balancer](../load-balancer/outbound-rules.md).
...
* The subnet has a [system default route](/azure/virtual-network/virtual-networks-udr-overview#default) that routes traffic with destination 0.0.0.0/0 to the internet automatically. After you configure NAT Gateway to the subnet, virtual machines in the subnet communicate to the internet by using the public IP of the NAT Gateway.

* When you create a user defined route (UDR) in your subnet route table for 0.0.0.0/0 traffic, you override the default internet path for this traffic. A UDR that sends 0.0.0.0/0 traffic to a virtual appliance or a virtual network gateway (VPN Gateway and ExpressRoute) as the next hop type instead overrides NAT Gateway connectivity to the internet.

### How NAT gateway works
 * **No route table configuration** - NAT gateway works at the subnet level. When you add it, NAT gateway gives you outbound connectivity without needing routing configurations on the subnet route table. 
...
* A NAT Gateway can't span multiple virtual networks. However, you can use a NAT Gateway to provide outbound connectivity in a hub and spoke model. For more information, see the [NAT Gateway hub and spoke tutorial](/azure/nat-gateway/tutorial-hub-spoke-route-nat).

* A Standard SKU NAT Gateway resource can use up to 16 IPv4 public IP addresses. StandardV2 SKU NAT Gateway can use up to 16 IPv4 and 16 IPv6 public IP addresses.
...
* You can associate NAT Gateway to an Azure Firewall subnet in a hub virtual network and provide outbound connectivity from spoke virtual networks peered to the hub. To learn more, see [Azure Firewall integration with NAT Gateway](../firewall/integrate-with-nat-gateway.md).

### Availability zones

* You can create a Standard SKU NAT Gateway in a specific availability zone or place it in **no zone**. 
...
> [!NOTE]
> On March 31, 2026, new virtual networks default to using private subnets, meaning that [default outbound access](/azure/virtual-network/ip-services/default-outbound-access#when-is-default-outbound-access-provided) is no longer provided by default, and you must enable an explicit outbound method to reach public endpoints on the Internet and within Microsoft. Use an explicit form of outbound connectivity instead, like NAT Gateway.

### NAT Gateway and basic resources

* Standard NAT Gateway works with Standard public IP addresses or public IP prefixes. StandardV2 NAT Gateway works only with StandardV2 public IP addresses or public IP prefixes.

* You can't use NAT Gateway with subnets that have basic resources. Basic SKU resources, such as basic Load Balancer or basic public IPs, don't work with NAT Gateway. You can upgrade basic Load Balancer and basic public IP to standard to work with a NAT Gateway.
...
* NAT Gateway sends a TCP Reset (RST) packet for any connection flow that it doesn't recognize as an existing connection. The connection flow no longer exists if the NAT Gateway idle timeout is reached or the connection was closed earlier. 
...
* SNAT ports aren't readily available for reuse to the same destination endpoint after a connection closes. NAT Gateway places SNAT ports in a cooldown state before they can be reused to connect to the same destination endpoint. 
...
* The NAT Gateawy TCP idle timeout timer defaults to 4 minutes but can be increased up to 120 minutes. Any activity on a flow can reset the idle timer, including TCP keepalives. To learn more, see [Idle Timeout Timers](./nat-gateway-resource.md#idle-timeout-timers).
