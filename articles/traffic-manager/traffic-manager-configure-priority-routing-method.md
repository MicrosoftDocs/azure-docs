...
description: This article explains how to configure the priority traffic routing method using Azure Traffic Manager
...
# Configure the priority traffic routing method using Traffic Manager
...
6. In the **Endpoints** blade, review the priority order for your endpoints. When you select the **Priority** traffic routing method, the order of the selected endpoints matters. Verify the priority order of endpoints.  The primary endpoint is on top. Double-check on the order its displayed. all requests will be routed to the first endpoint and if Traffic Manager detects it be unhealthy, the traffic will failover automatically to the next endpoint. 
