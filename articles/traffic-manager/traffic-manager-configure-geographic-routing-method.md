...
4. Select **Type** depending upon the type of endpoint you are adding. 
    > [!NOTE] For geographic routing profiles used in production, we strongly recommend using nested endpoint types containing a child profile with more than one endpoint. For more details, see [FAQs about nested endpoints](traffic-manager-FAQs.md#traffic-manager-nested-profiles).
