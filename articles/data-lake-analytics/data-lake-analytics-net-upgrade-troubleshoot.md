...
Check for the potential of backwards-compatibility breaking issues by running the .NET compatibility checks in your U-SQL custom assemblies.
...
2. If the tool indicates that your code may be impacted, common examples below, by any of the possible backwards-incompatibilities, you can further check using:
...
You can submit your job against the old runtime version (which is built targeting 4.5.2), however due to the lack of .NET Framework side-by-side capabilities, it will run in 4.5.2 compatibility mode.  This means you may still encounter some of the backwards-compatibility issues.

### What are the most common backwards-compatibility issues you may come across?

The most common backwards-incompatibilities that the checker is likely to identify are, this list was generated by running the checker on our own internal ADLA jobs, which libraries are impacted (note: that you may call the libraries only indirectly, thus it is important to take required action #1 to check if your jobs are impacted), and possible actions to remedy. Note that almost all cases, for our own jobs, the warnings turned out to be false positives due to the narrow natures of most breaking changes.
