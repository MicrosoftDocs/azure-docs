...
Azure Data Lake Tools for VS Code supports Windows, Linux, and MacOS. Local run and local debug works only in Windows.
...
The local run and local debug feature only works in Windows environments, and is not supported on Mac and Linux based operating systems.
