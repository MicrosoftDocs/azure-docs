...
By default, the Data Lake Analytics service archives the jobs for 30 days. The expiration period can be configured from the Azure portal by configuring the customized expiration policy. You won't be able to access the job information after expiration. 
...
* Job Summary
    ...
    Refresh the Job View to see the more recent information about running jobs.
  ...
  * Job Status (graph):
    ...
    * Queued: Jobs are queued when they're waiting for enough resources, or the jobs exceed the max concurrent jobs per account limitation. The priority setting determines the sequence of queued jobs - the lower the number, the higher the priority.
  ...
  * Basic Information
    ...
    * Job Result: Succeeded or failed. The job might fail in every phase.
    ...
    * Priority: The priority of the job. The lower the number, the higher the priority. It only affects the sequence of the jobs in the queue. Setting a higher priority doesn't preempt running jobs.
    * Parallelism: The requested maximum number of concurrent Azure Data Lake Analytics Units (ADLAUs), also known as vertices. Currently, one vertex is equal to one VM with two virtual cores and six-GB RAM, though this could be upgraded in future Data Lake Analytics updates.
...
* Job Graph
  ...
  * <a name="state-information"></a>Stage information
    ...
    * 84 vertices: The total count of vertices in this stage. The figure indicates how many pieces of work are divided in this stage.
    ...
    * Colors: Colors are used in the stage to indicate different vertex status.
      ...
      * White indicates the vertex is Waiting. The vertex could be waiting to be scheduled once an ADLAU becomes available, or it might be waiting for input since its input data might not be ready.
  ...
  * Job Playback
      ...
      Data Lake Analytics runs jobs and archives the vertices running information of the jobs, such as when the vertices are started, stopped, failed and how they're retried, etc. All of the information is automatically logged in the query store and stored in its Job Profile. You can download the Job Profile through “Load Profile” in Job View, and you can view the Job Playback after downloading the Job Profile.
  ...
  * Job Heat Map Display 
    ...
    * Compute time: The heat map of SUM (every vertex execution time), you can consider this as how long it would take if all work in the stage is executed with only one vertex.
...
* Metadata Operations
    ...
    You can perform some metadata operations in your U-SQL script, such as create a database, drop a table, etc. These operations are shown in Metadata Operation after compilation. You could find assertions, create entities, drop entities here.
...
* Diagnostics
    ...
    The tool diagnoses job execution automatically. You'll receive alerts when there are some errors or performance issues in your jobs. Please note that you need to download Profile to get full information here. 
  ...
  * Warnings: An alert shows up here with compiler warning. You can select “x issue(s)” link to have more details once the alert appears.
  * Vertex runs too long: If any vertex runs out of time (say 5 hours), issues will be found here.
  * Resource usage: If you allocated more or not enough Parallelism than need, issues will be found here. Also you can select Resource usage to see more details and perform what-if scenarios to find a better resource allocation (for more details, see this guide).
  * Memory check: If any vertex uses more than 5 GB of memory, issues will be found here. Job execution could get killed by system if it uses more memory than system limitation.
...
* Script
    ...
    The U-SQL script of the job is stored in the query store. You can view the original U-SQL script and resubmit it if needed.
* Resources
