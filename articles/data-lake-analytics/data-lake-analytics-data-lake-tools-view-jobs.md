...
Job View shows the detailed information of a job. To open a job, you can double-click a job in the Job Browser, or open it from the Data Lake menu by clicking Job View.  The job URL will be filled in the dialog automatically from your copy-paste buffer [jgao: needs some details here].
...
- Job Summary
    ...
    - Job Status (graph):
        ...
        - Queued: Jobs are queued waiting for enough resources;  Jobs also get queued because of exceeding the max concurrent jobs per account limitation. Priority decides sequence of queued jobs, the lower the number, the higher the priority.
        ...
        The job may fail in every phase. For example, compilation error in the Preparing phase, timeout error in the Queued phase,  and execution error in  the Running phase, etc.
    ...
    - Basic Information
        ...
        - Account: The Data Lake Analytics account which the job has been submitted to.
        - Author: The user who submitted the job, it can be a real person’s account or a system account.
        - Priority: The priority of the job. The lower the number, the higher the priority. It will only affect the sequence of the jobs in the queue. Please note that setting a higher priority will not preempt running jobs.
        - Parallelism: The requested maximum number of concurrent Azure Data Lake Analytics Units (ADLAUs), aka vertices. Currently, one vertex is equal to 1 VM with 2 virtual core and 6GB RAM, though this could be upgraded in future Data Lake Analytics updates.
        ...
        - Total vertices: The job is broken up into many pieces of work, each piece of work is called a vertex. This value describes how many pieces of work the job consists of. You can consider a vertex as a basic process unit, aka Azure Data Lake Analytics Unit (ADLAU), and vertices can be run in parallelism. See the figure below for more explanation on the term “vertex”.
        - Completed/Running/Failed: The count of completed/running/failed vertices. Vertices can fail due to both user code and system failures, but the system will retry failed vertices automatically a few times. If the vertex is still failed after retrying, the whole job will fail.
...
- Job Graph
    ...
    Every vertex in a stage is doing the same kind of work with different pieces of the same data. For example, if you have a file with 1TB data, and there are hundreds of vertices reading from it, each of them is reading a chunk. Those vertices are grouped in the same stage and doing same work on different pieces of same input file.
    ...
    - Stage information
        ...
        - Colors: There are a few colors that will be used in the stage to indicate different vertex status.

            - Green indicates the vertex is succeeded.
            - Orange indicates the vertex had been retried. The retried vertex was failed but is retried automatically and successfully by the system, and the overall stage is completed successfully. If the vertex retried but still failed, the color turns red and the whole job failed.
            - Red indicates failed, which means a certain vertex is retried a few times by the system but still fails. This will cause the whole job to fail.
        ...
        - Vertex duplicate discards: If a vertex run slowly abnormally, the system may schedule multiple vertices to run the same piece of work. Reductant vertices will be discarded once one of the vertices complete successfully. Vertex duplicate discards records how many vertices are discarded as duplicate in the stage.
    ...
    - Job Heat Map Display 
        ...
        ![Azure Data Lake Analytics job graph heapmap display](./media/data-lake-analytics-data-lake-tools-view-jobs/data-lake-tools-job-graph-heat-map-display.png)
        ...
        ![Azure Data Lake Analytics job graph heapmap example](./media/data-lake-analytics-data-lake-tools-view-jobs/data-lake-tools-job-graph-heat-map-example.png)
