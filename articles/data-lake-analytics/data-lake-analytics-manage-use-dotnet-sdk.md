...
## Required packages for Data Lake Store and Data Lake Analytics
You need these Nuget packages. Some packages are only available in preview.
...
If you do want to use your own Azure AD domain and application client ID, you must create an Azure AD native application and then use the Azure AD domain, client ID, and redirect URI for the application you created. See [Create an Active Directory Application](data-lake-store-end-user-authenticate-using-active-directory.md) for instructions.
...
* DataLakeAnalyticsAccountManagementClient - Use to create and manage Data Lake Analytic accounts.
...
### Get an account
The following code uses a DataLakeAnalyticsAccountManagementClient to return a Data Lake Analytics account. First a check is made to see if the account exists. 
    ...
    public DataLakeAnalyticsAccount GetDlaAccount(string strName)
    {
        ...
        {
            adlaGet = _adlaClient.Account.Get(_resourceGroupName, strName);
...
Every Data Lake Analytics account requires a default Data Lake Store account. Use this code to determine the default Store account for an Analytics account.
    ...
    public void GetDefaultDLStoreAccount(string ADLAaccountName)
    {
        if (_adlaClient.Account.Exists(_resourceGroupName, ADLAaccountName))
        {
            DataLakeAnalyticsAccount adlaGet = _adlaClient.Account.Get(_resourceGroupName, ADLAaccountName);
...
The following code lists the containers for a specified Azure Storage account.
    ...
    var containers = _adlaClient.StorageAccounts.ListStorageContainers(_resourceGroupName, ADLAName, azStorageName);
...
Among the several things you can list, the most common are databases and their schema. The following code obtains a collection of databases, and then enumerates the schema for each database.
    ...
    private void ListCatalogItems(string adlaAccountName)
    {
        var databases = _adlaCatalogClient.Catalog.ListDatabases(adlaAccountName);
