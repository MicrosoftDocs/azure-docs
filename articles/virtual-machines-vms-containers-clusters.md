...
Container technology -- especially [Docker](https://www.docker.com) and its ecosystem -- is dramatically expanding the agility with which we develop, deploy, test, and repair distributed systems in public cloud systems like Microsoft Azure. 

Azure, like other public cloud platforms, is built on [hypervisor](http://en.wikipedia.org/wiki/Hypervisor) technology -- in which a virtual machine host provides an emulation of physical computer hardware -- to enable agile movement of deployments and increased usage of hardware resources. Many hypervisor products and entire businesses have been created around the development, hosting, and maintainance advantages of virtual machines during the past 20 years.

**But that's old news.** The *new* news is that you can use *container* technologies on Microsoft Azure to dramatically increase developer productivity and architectural and operations agility in order to reduce your overall costs and bring your ideas to the world more rapidly. 
...
Both virtual machines and containers provide levels of abstraction for computing resources to make computing more efficient to manage and scale, but the abstractions -- or **virtualizations** they create are at different places. 

Virtual machines -- also called [hypervisors](http://en.wikipedia.org/wiki/Hypervisor) -- virtualize the underlying *hardware* to enable complete operating systems to run inside the hypervisor as if they were an application. When you create Azure Virtual Machines, you are creating hypervisors running the operating system you chose to use and you are installing an entire operating system. This approach is time-tested and there are many tools available to manage the operating system and to configure applications you install and run. 

[Docker](https://www.docker.com/) and other [Linux container](http://en.wikipedia.org/wiki/LXC) technologies (and there are many others) are not hypervisors. Instead, the container host uses the process and file system isolation features of the Linux kernel to expose only certain kernel features and a file system (at a minimum) that is relative to the application installed in the container. The kernel on the host OS is shared among containers, each of which installs only those files inside the container that they need, and no more. Because in this model the kernel -- which is already running -- is shared and the disk usage is radically reduced, both the start-up time consumed and storage overhead are much, much smaller. It's pretty cool.

[Windows Server Containers](https://msdn.microsoft.com/virtualization/windowscontainers/about/about_overview) have a slightly different underlying architecture, but provide much the same advantages as Linux containers. Windows Server Containers support the docker image format, the docker remote API. As a result, an application using Windows Server Containers can be developed, published, retrieved, and deployed using the exact same commands as developers on Mac and Linux and have a much reduced disk footprint and start-up time compared to VMs. 
