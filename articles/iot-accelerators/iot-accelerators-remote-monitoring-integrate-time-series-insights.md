...
## Configure the Time Series Insights data access policy
To make sure all users who have access to your Remote Monitoring solution are able to explore data in the Time Series Insights explorer, you will need to add your application and users under data access policies in the Azure Portal. 
...
1. Find the Azure Stream Analyics streaming job in the list of resources. The resource name should start with **streamingjobs-**.

1. At the top, click the button to stop the ASA streaming jobs.

1. Edit the ASA query and remove the `SELECT`, `INTO` and `FROM` clauses that point to the messages stream in Cosmos DB. This should be at the bottom of the query and look like the following example:
