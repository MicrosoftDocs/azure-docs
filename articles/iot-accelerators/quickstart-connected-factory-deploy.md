...
The **Alerts** panel shows alerts generated when a reported value or a calculated OEE/KPI value goes over a threshold. This panel displays alerts at each level of the hierarchy, from the station level to the enterprise. Each alarm includes a description, date, time, location, and number of occurrences:
...
You can analyze the data that caused the alarm from the dashboard. If you're an Administrator, you can take default actions on the alerts such as:

* Close the alarm.
* Acknowledge the alarm.

Click one of the alerts, in the **Choose action** drop-down, choose **Acknowledge alert**, and then click **Apply**:
...
These alerts are generated by rules that are specified in a configuration file in the solution accelerator. These rules can generate alerts when the OEE or KPI figures or OPC UA Node values go over a threshold. You can set this threshold value.
...
In this quickstart, you deployed the Connected Factory solution accelerator and learned how to navigate through your factories, production lines, and stations. You also saw how to view the OEE and KPI values at any level in the hierarchy, and how to respond to alerts.
