...
Because Protobuf requires compiled code to serialize the data, you must build a custom version of Device Simulation and deploy this version to your Azure VM using a Docker image.
...
1. Run a simulation sending messages using Protobuf
...
* Docker. Download the [Docker Community Edition](https://www.docker.com/products/docker-engine#/download).
...
Open the **remote-monitoring-services-dotnet-master\storage-adapter** folder in Visual Studio Code. Click any **Restore** buttons to fix unresolved dependencies.
...
1. Define the device capabilities in the device model **assettracker-01.json** file. The telemetry section of a Protobuf device model must :
...
<!-- TODO Maybe make this a separate how-to -->

In order to deploy your own changes to device simulation, you need to edit the build and deployment scripts in **scripts/docker** folder to upload the containers to your docker-hub account

### Prerequisites

1. [Create a Docker Hub account](https://hub.docker.com/)

1. Create a Docker Hub repo for device simulation.

### Update project with custom changes

<!-- TODO - can't find this -->
Increment the version number on your project. The version file located in the **version** folder.

#### Modify the docker scripts with your docker hub repo info

Modify the Docker scripts in the **scripts\docker** folder with your Docker Hub repository information:

Update the following line in each script files with your own information:

`DOCKER\_IMAGE="{your-docker-hub-username/{your-docker-hub-repo}"`

If you're working on a Windows machine, update the following scripts:

* build.cmd
* publish.cmd
* run.cmd

If you're working on a Linux machine, update the following scripts:
...
#### Configure the solution to include new device models

If you added any new device model files, you need to explicitly include them in the solution. Add an entry to the **services/services.csproj** for each additional file to include. For example:
...
Publish the new Docker image to Docker Hub using your docker-hub account. The following commands assume you're using the command prompt on Windows.

1. Open a command prompt and navigate to you local copy of the device simulation repository.
...
1. Run the following command to publish the Docker image to your Docker Hub repository:
...
#### Update the Device Simulation service container on the VM

* SSH into your deployment's VM
...
* Change the image to point to the image created above:
    ```
    image: {your-docker-hub-username}/{your-docker-hub-repo}
    ```

* Restart the Service. Run the following command to restart the microservices:
    ```sh
    sudo start.sh
    ```
...
You can now run a simulation using your new Protobuf-enabled device model
...
TODO: Do we need to call out other areas of concern and things that do work? (e.g. ASA, etc)
