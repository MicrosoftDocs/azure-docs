...
- Publish JSON encoded telemetry data from these servers in OPC UA Pub/Sub format, using a JSON payload, to Azure IoT Hub.
...
OPC Publisher implements retry logic to establish connections to endpoints that haven't responded to a certain number of keep alive requests. For example, if the OPC UA server on this endpoint had a power outage.
...
OPC Publisher supports batching of the data sent to IoT Hub, to reduce network load. This batching sends a packet to IoT Hub only if the configured package size is reached.
...
## Configure OPC UA nodes

This section describes to options for configuring OPC UA node publishing:
...
This section describes options for running the application:

- [Command line options](#command-line-options)
- [Run natively on Windows](#run-natively-on-windows)
- [Use a self-built container](#use-a-self-built-container)
- [Use a container from Microsoft Container Registry](#use-a-container-from-microsoft-container-registry)
- [Run as an Azure IoT Edge module](#run-as-an-azure-iot-edge-module)
- [Considerations when using a container](#considerations-when-using-a-container)
- [OPC UA X.509 certificates](#opc-ua-x509-certificates)
...
OPC Publisher is ready to be used as an [Azure IoT Edge](https://docs.microsoft.com/en-us/azure/iot-edge) module. When you use OPC Publisher as IoT Edge module, the only supported transport protocols are **Amqp_Tcp_Only** and **Mqtt_Tcp_Only**.
