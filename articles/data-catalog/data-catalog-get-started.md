...
This tutorial helps you get started with Azure Data Catalog. You will perform the following actions in this tutorial:

| Step | Description |
| :--- | :---------- |
| [Provision data catalog](#provision-data-catalog) | In this step, you will provision or setup Azure Data Catalog. You will do this step only if the catalog has not been set up before. You can  have only one data catalog per organization (Azure Active Directory Domain) even though there are multiple subscriptions associated with your Azure account. |
...
| [Manage data assets](#manage-data-assets) | In this step, you will set up security for your data assets. Data Catalog does not give users access to the data itself. Data access is controlled by the owner of the data source. <br/><br/> With Data Catalog, you can  discover data sources and view the **metadata** related to the sources registered in the catalog. There may be situations, however, where data sources should only be visible to specific users or to members of specific groups. For these scenarios, you can use Data Catalog to take ownership of registered data assets within the catalog and control the visibility of the assets you own. |
| [Remove data assets](#remove-data-assets) | In this step, you will learn how to remove data assets from the data catalog. |  

## Tutorial prerequisites

Before you begin this tutorial you must have the following three items:
...
To set up Azure Data Catalog, you must be logged in using an Azure Active Directory (Azure AD) user account, and the user must be the owner or co-owner of an Azure subscription.  

Azure Active Directory provides an easy way for your business to manage identity and access, both in the cloud and on-premises. Users can use a single work or school account to sign in to any cloud or on-premises web application. Azure Data Catalog uses Azure AD to authenticate sign in. To learn more, see [What is Azure Active Directory](../active-directory/active-directory-whatis.md).
...
The registration tool uses *forms authentication* to validate user sign ins against Active Directory. For successful sign in, an Active Directory administrator must enable forms authentication in the *global authentication policy*.

With the global authentication policy, you can enable authentication separately for intranet and extranet connections, as illustrated below. Logon errors may occur if forms authentication is not enabled for the network from which you're connecting.
...
2. Sign in using a user account that is the owner or co-owner of an Azure subscription. You will see the following page after signing in.
...
### Find a data catalog in Azure Portal
1. On a separate tab in the web browser or in a separate web browser window, go to [Azure Portal](https://portal.azure.com) and sign in using the same account that you used to create the data catalog in the previous step.
2. Click **Browse** and then click **Data Catalog**.

	![Azure Data Catalog - browse Azure Portal](media/data-catalog-get-started/data-catalog-browse-azure-portal.png)
...
4.  Click the catalog you created. You will see the **Data Catalog** blade in the portal.
...
2. To restore the database on your machine, follow the instructions in [Restore a Database Backup using SQL Server Management Studio](http://msdn.microsoft.com/library/ms177429.aspx). or the following steps:
	1. Launch SQL Server Management Studio and connect to the SQL Server Database Engine.
	2. Right-click **Databases** and select **Restore Database**.
	3. Under **Restore Database**, select the **Device** option for **Source** and click **Browse (...)**.
...
You can now register data assets from the Adventure Works sample database with **Azure Data Catalog**.
...
1.	Go to [azuredatacatalog.com](https://azuredatacatlog.com) and click **Publish Data**.
...
4. In the **Microsoft Azure Data Catalog** page,  choose **SQL Server** and **Next**.

    ![Azure Data Catalog - data sources](media/data-catalog-get-started/data-catalog-data-sources.png)

5.	Enter the SQL Server connection properties for **AdventureWorks2014** (see the following example), and choose  **CONNECT**.

    ![Azure Data Catalog - SQL Server connection settings](media/data-catalog-get-started/data-catalog-sql-server-connection.png)

6.	You will now need to register the metadata of your data asset. In this example, you register **Production/Product** objects from the AdventureWorks Production namespace:

	1. In the **Server Hierarchy** tree, expand **AdventureWorks2014** and click **Production**.
	2. Select **Product**, **ProductCategory**, **ProductDescription**, and **ProductPhoto** using Ctrl+click.
...
### Basic search
Basic search helps you search a catalog using one or more search terms. Results are any assets that match on any property with one or more of the terms specified.

1. Click **Home** in the Azure Data Catalog portal. If you have closed the web browser, go to [https://www.azuredatacatalog.com](https://www.azuredatacatalog.com).
2. In the search box, enter **cycles** and select **ENTER**.

	![Azure Data Catalog - basic text search](media/data-catalog-get-started/data-catalog-basic-text-search.png)
3. Confirm that you see all four tables and the database (AdventureWorks2014) in the results. You can switch between **grid view** and **list view** by selecting buttons in the toolbar as shown in the following image. Notice that the search keyword is highlighted in the search results because the **Highlight** option is **ON**. You can also specify the number of **results per page** in search results.

	![Azure Data Catalog - basic text search results](media/data-catalog-get-started/data-catalog-basic-text-search-results.png)

	The **Searches** panel is on the left and the **Properties** panel is on the right. The **Searches** panel allows you to change search criteria and filter results. The **Properties** panel displays properties of a selected object in the grid or list.

4. Click **Product** in the search results. Select the **Preview**, **Columns**, **Data Profile**, and **Documentation** tabs, or select the arrow to expand the bottom pane.  
...
6. Select the **Data Profile** tab to see the profiling of data (for example: number of rows, size of data, minimum value in a column, etc.) in the data asset.
7. Filter the results by using **Filters** on the left. For example, select **Table** for **Object Type**, and you will see only the four tables, not the database.
...
2. In the search box, enter **tags:cycles** and select **ENTER**. See [Data Catalog Search syntax reference](https://msdn.microsoft.com/library/azure/mt267594.aspx) for all the properties you can use for searching the data catalog.
...
### Save the search
1. In the **Searches** pane in the **Current Search** section, enter a name for the search and choose **Save**.
...
1. In the search box, enter **tags:cycles AND objectType:table**, and select **ENTER**.
...
1. In the search box, enter **name:product AND (tags:cycles AND objectType:table)** and select **ENTER**.
...
In this exercise, you use the **Azure Data Catalog** portal to annotate (add descriptions, tags, experts, etc.) data assets you have previously registered in the catalog. The annotations you provide will supplement and enhance the structural metadata extracted from the data source during registration and will make the data assets much easier to discover and understand.

In this step, you will annotate a single data asset (ProductPhoto). You will add a friendly name and description to the ProductPhoto data asset.  
...
- Add experts for the data asset. Choose **Add...** in the **Experts** area.
- Add tags at the dataset level. Choose **Add...** in the **Tags** area. A tag can be a user tag or a glossary tag. The Standard Edition of Data Catalog includes a business glossary that allows catalog administrators to define a central business taxonomy. Catalog users can then annotate data assets with glossary terms. For more information, see  [How to set up the Business Glossary for Governed Tagging](data-catalog-how-to-business-glossary.md)
- Add tags at the column level. Choose **Add...** under **Tags** for the column you want to annotate.
...
1. Select **Product** from search results. Choose **Open In** on the toolbar and select **Excel**.
...
3. In the **Microsoft Excel Security Notice** window, choose **Enable**.
...
In this exercise, you connected to data assets discovered using **Azure Data Catalog**. With the **Azure Data Catalog** portal, you can connect directly using the client applications integrated into its **Open in…** menu. You can also connect using any application you choose using the connection location information included in the asset metadata. For example, you can use SQL Server Management Studio to connect to the AdventureWorks2014 database to access the data in the data assets registered in this tutorial.
...
1. Go to the [Azure Data Catalog home page](https://www.azuredatacatalog.com). In the Search text box, enter **tags:cycles** and select **ENTER**.
2. Select an item in the result list and click **Take Ownership** on the toolbar.
3. In the **Management** section of the **Properties** panel, choose **Take Ownership**.

	![Azure Data Catalog - take ownership](media/data-catalog-get-started/data-catalog-take-ownership.png)
4. To restrict visibility, choose **Owners & These Users** in the **Visibility** section and click **Add**. Enter user email addresses in the text box and select **ENTER**.
...
3. Select an item in the result list and choose **Delete** on the toolbar as shown in the following image.
