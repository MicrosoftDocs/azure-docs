...
This article describes the process workflow and customer-managed shipping (CMS) tasks to order, pick-up, and drop off of an Azure Data Box Disk device. You can manage the Data Box Disk using the Azure portal. The customer can designate a point of contact from their organization, or use any carrier of their choice to pick up the Azure Data Box Disk from a datacenter and also return the device back to the datacenter. The customer will need to follow datacenter's security procedures as both parties will sign off at particular points between pick-up and drop-off. At pick-up and drop-off, the customer or point of contact will be assigned an authorization code (authcode). This authcode needs to be presented during device pick-up and drop-off at the datacenter.
...
## Order an Azure Data Box Disk from your supported region
...
4. Follow the instructions in the **Schedule pickup for Azure**. Before you can get your authcode, you must email [adbops@microsoft.com](mailto:adbops@microsoft.com) to schedule the device pickup from your region's datacenter.
...
    Once you have finished data copy, you will need to contact operations to schedule an appointment for the drop off. If you plan on using a point of contact, you will need to include their personal details. The datacenter will also need to the authcode.

   > [!NOTE]
   >
   > Do not share the authcode over email. This is only to be verified at the datacenter during drop off.

8. You should now be at **Ready to receive** success state of the Azure portal. This means that you have scheduled an appointment for drop off and shared the drop off point of contact's details. You will also need to confirm that the datacenter also has the authcode available.
