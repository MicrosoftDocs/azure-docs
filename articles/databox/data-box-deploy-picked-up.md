...
1. Keep the original box used to ship the device for return shipment.
1. Make sure the data copy to device is complete, and the **Prepare to ship** step completed successfully.
1. Note the reference number on the **Prepare to ship** page of the device local web UI.
...
1. Pack the device for return shipment in the original box.
1. Email [Azure Data Box Operations](mailto:adbops@microsoft.com) to get an ID that will be used to identify the package when it arrives back at the datacenter.
1. Write down this ID on the printed shipping label, next to the return address so that it’s clearly visible.  
1. Book a pickup online by going to [DHL Express UAE, Schedule a Pickup](https://mydhl.express.dhl/ae/en/home.html#/schedulePickupTab).
...
1. If you come across any issue with the DHB booking tool, contact DHL using one of these methods:
