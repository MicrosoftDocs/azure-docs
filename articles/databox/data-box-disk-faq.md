...
|Shipping               |Microsoft takes care of forward and return shipping. <br>Suitable for data movement within the commerce boundary (for example within EU only, within US only). The same is true for Australia and Canada.                                                | Customer has to take care of shipping. <br>Suitable when data needs to move across geographic boundaries (for example, US to EU).<br>Support for within the region also available.|
