...
| SsemUserErrorCrossTenantIdentityAccessForbidden | Managed identity access operation failed. <br> Note: This error can occur when a subscription is moved to different tenant. The customer has to manually move the identity to the new tenant. | Move the identity selected to the new tenant under which the subscription is present. For more information, see how to [Enable the key](#enable-key). |
