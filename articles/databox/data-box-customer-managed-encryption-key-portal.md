...
| SsemUserErrorEncryptionKeyTypeNotSupported | Encryption key type not supported for the operation. | Enable the appropriate encryption type - for example, RSA or RSAHSM. *QUERIES: 1) Requirement unclear. Does this relate to the need to enable the Get, Unwrap Key and Wrap Key operations on the key? 2) What does RSAHSM stand for?* |
