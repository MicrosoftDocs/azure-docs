...
| SsemUserErrorEncryptionKeyTypeNotSupported | The encryption key type isn't supported for the operation. | Enable a supported encryption type on the key - for example, RSA or RSA-HSM - on the key. *QUERIES: 1) Requirement unclear. Does this relate to the need to enable the Get, Unwrap Key and Wrap Key operations on the key? 2) What does RSAHSM stand for?* |
...
| SsemUserErrorKeyVaultUrlInvalidHost | The key vault URL host is not an allowed host in the geographical region. | In the public cloud, the key vault URL should end with `vault.azure.net`. In the Azure Government cloud, the key vault URL should end with `vault.usgovcloudapi.net`. |  
