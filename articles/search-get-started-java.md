<properties 
	pageTitle="Get started with Azure Search in Java" 
	description="Walk through building a custom Azure Search application using Java as your programming language." 
	services="search" 
	documentationCenter="" 
	authors="HeidiSteen" 
	manager="mblythe" 
	editor=""/>

<tags 
	ms.service="search" 
	ms.devlang="" 
	ms.workload="search" 
	ms.topic="article" 
	ms.tgt_pltfrm="na" 
	ms.date="03/23/2015" 
	ms.author="heidist"/>

#Get started with Azure Search in Java#

This page is a placeholder for a new tutorial and sample that demonstrates how to code with Azure Search in NodeJS. Actual content will be published within the next few days.

As a workaround, consider this alternative tutorial, [Get started with Azure Search in .NET](../search-get-started-dotnet/), to learn the fundamentals of programming with Azure Search.

<!--Image references-->
